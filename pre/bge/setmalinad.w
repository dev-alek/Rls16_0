...
DO:
    ...
    case cb-type:
      when "mal" then do:
        if RADIO-SET_objects = 1 then do:
...
on ' ' of d_date in frame Dialog-Frame
do:
...
on delete-character of d_date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of d_date in frame Dialog-Frame
do:
...
on ctrl-b of d_date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of d_date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of d_date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date8
    ...
    .
  if d_date :POPUP-MENU in frame Dialog-Frame = ?
  then do:
    ASSIGN
      d_date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date8 :HANDLE
      d_date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle8 as handle no-undo .
  assign
    v-label-handle8 = d_date :side-label-handle in frame Dialog-Frame
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date8-1 in menu m-ed-date8 DO:
    apply "ctrl-b":U to d_date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date8-2 in menu m-ed-date8 DO:
    apply "ctrl-d":U to d_date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date8-3 in menu m-ed-date8 DO:
    apply "ctrl-e":U to d_date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date8-4 in menu m-ed-date8 DO:
    apply "ctrl-f":U to d_date in frame Dialog-Frame .
  END.
MAIN-BLOCK:
    ...
    case cb-type:
      when "mal" then do:
        ...
        if num-entries(c_value,",") = 3 then do:
          ...
          no-error.
          display d_date i_shift i_op-id with frame Dialog-Frame.
          create userobjs_temp-user-obj.
          assign
            userobjs_temp-user-obj.obj-code = v-cntxt-obj-code
            userobjs_temp-user-obj.obj-type = v-cntxt-obj-type
