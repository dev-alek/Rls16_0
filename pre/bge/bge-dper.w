...
define output parameter p-pay-type-list     as character            no-undo .
...
DO:
    ...
    do v-ii = 1 to num-entries(v-grp-recid-list):
        ...
        run grplib-get-full-name in this-procedure( input buf_gds-grp.node-code, output v-Grp_Name).
            EDITOR-gds-grp:screen-value = EDITOR-gds-grp:screen-value + v-Grp_Name + chr(10).
    end.
END.
...
DO:
  ...
  if RADIO-SET-gds-grp = 1 then do:
      bt-sel-gds-grp :visible in frame Dialog-Frame = no.
      EDITOR-gds-grp:screen-value = "".
  end.
  else do:
      bt-sel-gds-grp :visible in frame Dialog-Frame = yes.
