...
initial "@(#)$Workfile$ $Revision$".
define variable v-fdoctype-type-list as character extent 18 init
[
     "приходный кассовый ордер"             ,     'пко':U      ,   "ic"
  ,  "расходный кассовый ордер"             ,     'рко':U     ,   "ec"
  ,  "приходное платежное поручение"        ,     'ппп':U  ,   "ii"
  ,  "расходное платежное поручение"        ,     'рпп':U ,   "ei"
  ,  "приходный АПЗ"                        ,     'апп':U    ,   "io"
  ,  "расходный АПЗ"                        ,     'апр':U   ,   "eo"
] no-undo.
define variable vss-include-info2 as character format "X(65)" no-undo
initial "@(#)$Workfile$ $Revision$".
define variable v-fdoctype-bank-type-list as character extent 3 init
[
    "расходное платежное поручение"        ,     'рпп':U ,   "ei"
] no-undo.
define variable vss-include-info3 as character format "X(65)" no-undo
initial "@(#)$Workfile$ $Revision$".
define variable v-conttype-type-list as character extent 4 init
[
...
:
    ...
    OPEN QUERY BROWSE-1 FOR EACH temp_doc-type SHARE-LOCK     BY temp_doc-type.dtp-key .
...
:
    ...
    :
        ...
        :
            ...
            then do:
                do
                v-counter = 1 to num-entries( 'ie,ee,ep,es,re,rs,we,vt,vp,iv,ev,rv,em,wm,im,ot,ap,mp,pc,io,eo':U )
                :
                    create buf_temp_doc-type.
                    assign
                        buf_temp_doc-type.dtp-key           = ( v-what-doc-counter - 1) * v-what-doc-amount + v-counter
                        buf_temp_doc-type.doc-type          = entry( v-counter, 'ie,ee,ep,es,re,rs,we,vt,vp,iv,ev,rv,em,wm,im,ot,ap,mp,pc,io,eo':U )
                        buf_temp_doc-type.doc-type-label    = entry( v-counter, 'приход внешний,расход внешний,возврат пост.,касса продажа,возврат внешний,касса возврат,списание,инвентаризация,пересортица,приход внутренний,расход внутренний,возврат внутренний,расход  произв.,списан. произв.,приход  произв.,переоценка,коррекция учетных цен,корректировка отрицательных партий,смена типа приобретения,приход внутриобъектный,расход внутриобъектный':U )
                    .
            ...
            then do:
                ...
                do
                v-counter = 1 to num-entries( 'пко,рко,ппп,рпп,апп,апр,':U )
                :
                    if entry( v-counter, 'пко,рко,ппп,рпп,апп,апр,':U ) = "":U
                    then do:
                    ...
                    assign
                        ...
                        buf_temp_doc-type.doc-type          = entry( v-counter, 'пко,рко,ппп,рпп,апп,апр,':U )
                        buf_temp_doc-type.doc-type-label    = entry( v-counter, 'приходный кассовый ордер,расходный кассовый ордер,приходное платежное поручение,расходное платежное поручение,апп,апр':U )
                    .
                    if p-init-doc-type-list = "":U
                    or lookup( buf_temp_doc-type.doc-type, p-init-doc-type-list ) <> 0
                    then do:
