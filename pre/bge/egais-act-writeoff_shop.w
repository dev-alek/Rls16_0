...
assign
  v-str-recid12 = trim( string( recid( tt-gds-act ) , "->>>>>>>>>>>9":U ) )
  v-num-entry12 = lookup( v-str-recid12 , select-list )
.
if v-num-entry12 > 0 then do:
  assign
    entry( v-num-entry12, select-list ) = "":U
    select-list = trim( replace( select-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    select-list = select-list + ( if select-list = "":U then "":U else chr(44) ) + v-str-recid12
  .
end.
  end.
...
DO:
  ...
  then do :
      enable b-marks with frame Dialog-Frame .
  end.
  else do :
      disable b-marks with frame Dialog-Frame .
  end.
END.
...
DO:
    ...
    find first buf_clob-bind exclusive-lock where buf_clob-bind.uniq-key-rec = tt-act-header.num
                                              and buf_clob-bind.field-name_  = 'egais-awo_shop':U no-error .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    if p-mode = 'ДОБАВЛЕНИЕ':U then do :
        ...
        .
        display tt-act-header.num tt-act-header.date_ tt-act-header.type_ with frame Dialog-Frame.
        enable  tt-act-header.date_ tt-act-header.type_ b-good with frame Dialog-Frame.
    ...
    if p-mode = 'ИЗМЕНЕНИЕ':U or p-mode = 'ПРОСМОТР':U then do :
        find last buf_clob-bind where buf_clob-bind.uniq-key-rec = bh-act-header:buffer-field ("num"):buffer-value
                                  and buf_clob-bind.field-name_ = 'egais-awo_shop':U
...
assign
  v-str-recid15 = trim( string( recid( tt-gds-act ) , "->>>>>>>>>>>9":U ) )
...
PROCEDURE enable_UI :
  ...
  VIEW FRAME Dialog-Frame.
    if p-mode = 'ПРОСМОТР':U then do :
        disable  tt-act-header.num tt-act-header.date_ tt-act-header.type_ with frame Dialog-Frame.
