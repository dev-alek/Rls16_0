...
ON CHOOSE OF btn_asw IN FRAME Dialog-Frame
DO:
  ...
  do while qh-journal-egais:get-next ():
    ...
    if glog then do :
        msg = msg + chr(10) + egaisDictGds:Msg.
    end.
    else do:
      extGdsValueObj = egaisDictGds:GetExtGdsObjValueFromUTM() .
      if egaisDictGds:StatusErr = true
      then do:
        put stream str1 unformatted chr(10) + egaisDictGds:Msg.
...
ON choose OF btn_look IN FRAME Dialog-Frame
do:
  run msdblcl.
end.
...
ON CHOOSE OF btn_refresh IN FRAME Dialog-Frame
DO:
  run reopen-browse.
END.
ON CHOOSE OF btn_req IN FRAME Dialog-Frame
DO:
  def var egaisDictGds as class DictGds no-undo.
  ...
  apply "choose" to Btn_desmark in frame Dialog-Frame.
END.
ON leave OF f-alcgds IN FRAME Dialog-Frame
do:
  if f-alcgds:screen-value = f-alcgds
    then return.
  run refresh-view.
end.
ON return OF f-alcgds IN FRAME Dialog-Frame
do:
  run refresh-view.
end.
ON leave OF f-gds IN FRAME Dialog-Frame
do:
  if f-gds:screen-value = f-gds
    then return.
  run refresh-view.
end.
ON return OF f-gds IN FRAME Dialog-Frame
do:
  run refresh-view.
end.
ON leave OF f-gdsname IN FRAME Dialog-Frame
do:
  if f-gdsname:screen-value = f-gdsname
    then return.
  run refresh-view.
end.
ON return OF f-gdsname IN FRAME Dialog-Frame
do:
  run refresh-view.
end.
ON VALUE-CHANGED OF t-incorr IN FRAME Dialog-Frame
DO:
  run refresh-view.
END.
ON VALUE-CHANGED OF c_verxsd IN FRAME Dialog-Frame
DO:
  run refresh-view.
END.
...
PROCEDURE reopen-browse :
  ...
  if valid-handle (browse-hdl-egais-goods) then do:
