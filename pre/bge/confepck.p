...
if not available buf_esys-pck-sent then do:
    if p-silent then do:           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найден пакет для ВС &1 за номером &2", p-esys-id, p-esps-pack-num)).                  end.                                               else do:                                             message substitute("Не найден пакет для ВС &1 за номером &2", p-esys-id, p-esps-pack-num) view-as alert-box.      end.
end.
else do:
  if buf_esys-pck-sent.esps-rcvd = yes then do:
        if p-silent then do:           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Отправленный Пакет &1 для ВС УЖЕ подтвержден"                                 ,buf_esys-pck-sent.esps-pack-num                                  ,buf_esys-pck-sent.esys-id)).                  end.                                               else do:                                             message substitute("Отправленный Пакет &1 для ВС УЖЕ подтвержден"                                 ,buf_esys-pck-sent.esps-pack-num                                  ,buf_esys-pck-sent.esys-id) view-as alert-box.      end.
    return ''.
  end.
  ...
  .
    if p-silent then do:           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Отправленный Пакет &1 для ВС помечен как подтвержденный &3 &4"                                ,buf_esys-pck-sent.esps-pack-num                                 ,buf_esys-pck-sent.esys-id                                      ,string(buf_esys-pck-sent.esps-rcvddate, "99/99/9999")                                ,buf_esys-pck-sent.esps-rcvdtime)).                  end.                                               else do:                                             message substitute("Отправленный Пакет &1 для ВС помечен как подтвержденный &3 &4"                                ,buf_esys-pck-sent.esps-pack-num                                 ,buf_esys-pck-sent.esys-id                                      ,string(buf_esys-pck-sent.esps-rcvddate, "99/99/9999")                                ,buf_esys-pck-sent.esps-rcvdtime) view-as alert-box.      end.
end.
