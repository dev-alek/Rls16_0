...
DO:
  ...
  code_system:screen-value =  entry(2,v-rid-list,chr(3)) no-error.
    code_system = integer(entry(2,v-rid-list,chr(3))) no-error.
  end.
...
if v-place = 1 then   v-param-list =  v-directory + chr(4) + string(v-place) + chr(4)  +  v-login  + chr(4) + v-password  + chr(4) + string(code_TNP) + chr(4) + string(code_pool) + chr(4) + string(code_system) + chr(4) + string(v-long-code).
if v-place = 2 then do:
     v-ftp-address = trim(trim(replace(v-ftp-address,'ftp:',""),chr(47)),chr(92)).
      v-param-list =  v-ftp-address + chr(4) + string(v-place) + chr(4) +
                     v-login       + chr(4) +
                     v-password    + chr(4) +
                     string(code_TNP) + chr(4) +  string(code_pool) +  chr(4) + string(code_system) +  chr(4) + string(v-long-code).
                     end.
                ...
                do:
                    ...
                    do:
                        assign
                            ...
                            code_TNP    = entry(5,v-param-list,chr(4))
                            code_pool   = entry(6,v-param-list,chr(4))
                            code_system = integer(entry(7,v-param-list,chr(4)))
                            v-long-code = integer(entry(8,v-param-list,chr(4)))
   NO-ERROR.
                        display v-directory v-place  code_tnp code_pool code_system v-long-code with frame Dialog-Frame.
...
DEFINE INPUT PARAMETER p-param-list AS CHARACTER NO-UNDO.
  ...
  FIND FIRST buf_schedule-attr NO-LOCK WHERE
    ...
    and buf_schedule-attr.attr-code = ('schd-free-id':U + chr(4) + 'exp-VBBR') NO-ERROR.
  IF AVAILABLE  buf_schedule-attr
    AND buf_schedule-attr.task-num <> p-task-num
