...
define variable vss-workfile    as character no-undo init "$Workfile: bgefo.p $":U .
define variable vss-archive     as character no-undo init "$Archive: bge/bgefo.p $":U .
  ...
  ,output v-host-code
    ...
    assign
        v-parameter-list         =  substitute( "&1,&2,&3,&4,&5,&6,&7,&8,&9"
                                               , 7
                                               , "docName"          , "finOb":U
                                               , "version"          , replace("14.0 " + replace( vss-revision + vss-date, "$", " " ),',','')
...
define buffer buf_fin-ob for ub.fin-ob.
    ...
    find first buf_fin-ob no-lock where
      ...
      AND buf_fin-ob.status_ = 'факт':U no-error .
