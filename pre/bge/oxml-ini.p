...
:
  ...
  :
    ...
    if oxml-exch-dir = ? then
      v-str-err = v-str-err + substitute( "Отсутствует настройка каталога OpenXML (oxml-exch-dir).&1", chr(10) ) .
    else if oxml-heap-dir = ? then
      v-str-err = v-str-err + substitute( "Отсутствует настройка каталога OpenXML (oxml-dir).&1", chr(10) ) .
      ...
      if file-info:file-type = ? or not ( file-info:file-type begins "D":U ) then assign
        v-str-err = v-str-err + substitute( "Каталог &1 отсутствует.&2", oxml-exch-dir, chr(10) )
      .
      else assign
      ...
      if file-info:file-type = ? or not ( file-info:file-type begins "D":U ) then assign
        v-str-err = v-str-err + substitute( "Каталог &1 отсутствует.&2", oxml-heap-dir, chr(10) )
      .
      else assign
        oxml-heap-dir = file-info:full-pathname
        log-file-name = oxml-heap-dir + chr(92) + "openxml.log"
      .
    end.
    assign
      v-str-err = right-trim( v-str-err, chr(10) )
    .
