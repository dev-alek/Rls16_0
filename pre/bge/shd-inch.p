...
define input parameter p-need-checks    as logical    no-undo.
...
define variable v-is-found as logical no-undo .
    ...
    :
        ...
        then do:
            ...
            find current buf_clients-attr no-lock.
            next object-of-list.
        end.
        v-is-found = true .
        find current buf_clients-attr no-lock.
...
define variable v-is-found as logical no-undo .
    ...
    for each buf_shift-obj no-lock
       where buf_shift-obj.obj-type     = p-obj-type
         ...
         and buf_shift-obj.status_      = 'зкр':U
    use-index pi
    on error  undo export-shifts, return error substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
...
:
    ...
    on error undo, return error
    :
      if not (buf_temp_bge-xml_clients.obj-type = 'маг':U
              or
              buf_temp_bge-xml_clients.obj-type <> 'скл':U) then do:
