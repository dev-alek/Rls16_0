...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    if p-mode = 'ДОБАВЛЕНИЕ':U then do :
        ...
        enable  tt-act-header.date_ b-add with frame Dialog-Frame.
    end.
    if p-mode = 'ИЗМЕНЕНИЕ':U or p-mode = 'ПРОСМОТР':U then do :
        find last buf_clob-bind where buf_clob-bind.uniq-key-rec = bh-act-header:buffer-field ("num"):buffer-value
                                  and buf_clob-bind.field-name_ = 'egais-qb':U
        ...
        open QUERY br-gds-act FOR each tt-gds-act exclusive-lock .
        if p-mode = 'ПРОСМОТР':U then disable b-add with frame Dialog-Frame.
        else enable b-add with frame Dialog-Frame.
    end.
...
PROCEDURE enable_UI :
  ...
  VIEW FRAME Dialog-Frame.
    if p-mode = 'ПРОСМОТР':U then do :
        ...
        hide b-add b-save b-del in FRAME Dialog-Frame.
    end.
    else hide b-print in FRAME Dialog-Frame.
END PROCEDURE.
