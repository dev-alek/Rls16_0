...
for each buf_goods where buf_goods.stts = 0  no-lock :
    ...
    if buf_goods.gds-type = 'у':U and  can-find (first buf_goods-attr where buf_goods-attr.gds-code = buf_goods.gds-code and buf_goods-attr.attr-code = 'office-type':U )  then next.
    if not buf_goods.gds-type = 'у':U then
    do:
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_is-petrl in g#lib-trn
  ...
  , output v-is-pieces
    ...
    if buf_goods.gds-type = 'у':U or  v-is-petrol then
    do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pgtxvalg in g#library
  ...
  ,output v-vat-pc
        ...
        hSAXWriter:end-ELEMENT ("GoodsLocalItem") no-error.
                                     ...
                                     run write-log-and-file in p-log-handle (           input 1                                                               , input log-file-name                                                   , input 1                                                               , input SUBSTITUTE("Товар: &1 &2.", buf_goods.gds-name, buf_goods.gds-code)).
                                     run write-log-and-file in p-log-handle (           input 1                                                               , input log-file-name                                                   , input 1                                                               , input substitute("Ошибка: нет соответствующего кода внешней системы ")).
         end.
  end.
        else
        do:
...
p-ext-class-grp = "".
    ...
    do:
        find first buf_ext-classif no-lock where buf_ext-classif.classif-subject = 'gds-grp':U
            and buf_ext-classif.classif-name = 'gds-grp':U
            and buf_ext-classif.key#_two = p-code_system
            and buf_ext-classif.key#_three = 0
...
procedure ftp-send :
    ...
    do
        on error undo, return error
        :
        p-directory = trim(trim(replace(p-directory,'ftp:',""),chr(47)),chr(92)).
