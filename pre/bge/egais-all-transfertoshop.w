...
DEFINE VARIABLE f-date-2 AS DATE FORMAT "99/99/99":U
     ...
     VIEW-AS FILL-IN
     SIZE 9.5 BY 1 NO-UNDO.
DEFINE VARIABLE f-num AS character FORMAT "X(30)":U
     LABEL "номер"
     VIEW-AS FILL-IN
...
DEFINE FRAME Dialog-Frame
     ...
     f-num AT ROW 2.5 COL 45 COLON-ALIGNED WIDGET-ID 56
     SPACE(2) SKIP(22.2)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE
         TITLE "Передача продукции в торговый зал ЕГАИС"
...
DO:
    ...
    run bge/egais-TransferToShop.w (parparentproc, 'ПРОСМОТР':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
END.
ON CHOOSE OF Btn_Edit IN FRAME Dialog-Frame
DO:
    if not bh-act-header:available then return no-apply .
    find last buf_clob-bind where buf_clob-bind.field-name_ = 'egais-tts':U and buf_clob-bind.uniq-key-rec = bh-act-header:buffer-field ("num"):buffer-value .
...
DO:
    run bge/egais-TransferToShop.w (parparentproc, 'ДОБАВЛЕНИЕ':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
...
DO:
    ...
    run bge/egais-TransferToShop.w (parparentproc, 'ИЗМЕНЕНИЕ':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
    bh-act-header = egais:GetHndlTable(3, "").
    run refresh-query.
END.
...
DO:
    ...
    os-delete "temp-TransferToShop.xml".
    copy-lob
...
ON return OF f-num IN FRAME Dialog-Frame
do:
  run refresh-query.
end.
...
if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run getcurus in g#library2
  ...
  egais = new TransferToShop (v-cntxt-obj-type, v-cntxt-obj-code, v-fs-rar, v-ext-sys).
  ...
  create browse browse-hdl-act-header
    assign
      title     = 'Акты о передаче продукции в торговый зал'
  ...
  then do:
    ...
    browse-hdl-act-header:get-browse-column (2):width-chars = 37.
        browse-hdl-act-header:get-browse-column (3):width-chars = 10.
        browse-hdl-act-header:get-browse-column (4):width-chars = 20.
  end.
  run enable_UI.
...
END PROCEDURE.
PROCEDURE enable_UI :
  DISPLAY RADIO-SET-1 f-date f-date-2 f-num
      WITH FRAME Dialog-Frame.
  ENABLE Btn_OK RADIO-SET-1 f-date f-date-2 f-num
      WITH FRAME Dialog-Frame.
...
if bh-act-header = ?
  ...
  v-proposition =
    ...
    (if f-num <> ? and f-num <> "" then (" and tt-act-header.num matches '*" + f-num + "*'") else "")
    .
case RADIO-SET-1 :
    when 1  then
    do:
