...
END.
ON CHOOSE OF MENU-ITEM m_new-list
DO:
  assign
  list-option = "new":U.
  apply "choose" to b-list in frame Dialog-Frame.
END.
ON CHOOSE OF MENU-ITEM m_from-file
DO:
  assign
  list-option = "file":U.
  apply "choose" to b-list in frame Dialog-Frame.
END.
ON CHOOSE OF MENU-ITEM m_from-db
DO:
  assign
  list-option = "db":U.
  apply "choose" to b-list in frame Dialog-Frame.
...
assign
  v-str-recid10 = trim( string( recid( tt-objs ) , "->>>>>>>>>>>9":U ) )
            ...
            if available tt-objs-EG and not ambiguous tt-objs-EG then do transaction :
                ...
                if not available buf_clients-attr then do :
                    ...
                    assign
                        ...
                        buf_clients-attr.attr-code = 'requisite-alc-decl':U
                    .
                end.
                ...
                if tt-objs.regID <> "" then do :
                    ...
                    find first X_ext-classif exclusive-lock  where X_ext-classif.classif-subject = 'clients':U
                                                               and X_ext-classif.classif-name = 'clients-esys':U
                    ...
                    if available X_ext-classif then do :
                        ...
                        run ref/extclas1.p ( INPUT 'ДОБАВЛЕНИЕ':U
                                            ...
                                            ,input 0
                                            ,input '':U
                                            ,input (tt-objs.obj-type + string(tt-objs.obj-code))
                                            ,input tt-objs.regID
                                            ,input 0
...
PROCEDURE proc-b-mark :
  ...
  assign varlog = br-objects :select-next-row( ) in frame Dialog-Frame.
  apply "ENTRY":U to br-objects in frame Dialog-Frame.
  br-objects:refresh() in frame Dialog-Frame .
END PROCEDURE.
...
assign
  v-str-recid15 = trim( string( recid( tt-objs ) , "->>>>>>>>>>>9":U ) )
 ...
 .
        if par-alcohol = "yes" then do :
            ...
            run gen-key-rec in this-procedure   ( input 'clients':U
                                                 ,input buffer buf_clients:handle
                                                 ,output v-obj-uniq-key-rec).
            find first X_ext-classif no-lock  where X_ext-classif.classif-subject = 'clients':U
                                               and X_ext-classif.classif-name = 'clients-esys':U
                                               AND X_ext-classif.db-num = 0
                                               and X_ext-classif.key#_one = v-ext-sys
                                               and X_ext-classif.key#_two = 0
            ...
            if available buf_clients-attr then do :
                ...
                run gen-key-rec in this-procedure   ( input 'clients':U
                                                     ,input buffer buf_clients:handle
                                                     ,output v-obj-uniq-key-rec).
                find first X_ext-classif-attr no-lock  where X_ext-classif-attr.classif-subject = 'clients':U
                                                           and X_ext-classif-attr.classif-name = 'clients-esys':U
...
assign
  v-str-recid17 = trim( string( recid( CLIENTS ) , "->>>>>>>>>>>9":U ) )
  v-num-entry17 = lookup( v-str-recid17 , clients-list )
.
if v-num-entry17 > 0 then do:
  assign
    entry( v-num-entry17, clients-list ) = "":U
    clients-list = trim( replace( clients-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    clients-list = clients-list + ( if clients-list = "":U then "":U else chr(44) ) + v-str-recid17
  .
end.
                end.
            end.
        ...
        when "file" then do :
            ...
            if not glog then do:
                OPEN QUERY br-objects FOR EACH tt-objs.
                return error.
            end.
        end.
        when "db" then do :
...
assign
  v-str-recid19 = trim( string( recid( CLIENTS ) , "->>>>>>>>>>>9":U ) )
  v-num-entry19 = lookup( v-str-recid19 , clients-list )
.
if v-num-entry19 > 0 then do:
  assign
    entry( v-num-entry19, clients-list ) = "":U
    clients-list = trim( replace( clients-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    clients-list = clients-list + ( if clients-list = "":U then "":U else chr(44) ) + v-str-recid19
  .
end.
              end.
            end.
