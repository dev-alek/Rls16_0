...
ON VALUE-CHANGED OF RADIO-SET-2 IN FRAME Dialog-Frame
  DO:
    ...
    CASE RADIO-SET-2:
      ...
      THEN
        DO:
          run ref/bpa.p(INPUT parparentproc,INPUT 'ВЫБОР':U,OUTPUT pList) no-error.
          if pList <> ? or pList <> ""
            THEN
          DO:
            EDITOR-2 = "":U.
            ...
            v-text-oss = "" .
            trim (pList,chr(6)) .
            DO v-ii1 = 1 to num-entries (pList,chr(6)):
              FIND FIRST ub.OperServ no-lock
                WHERE ub.OperServ.id = integer(entry(v-ii1,pList,chr(6)))
                no-error
                .
                if available (ub.OperServ) then do:
              ASSIGN
                v-text-oss = v-text-oss + chr(10) + ub.OperServ.ext-code.
              .
              buffer-copy ub.OperServ to tt-oss-ref .
              end.
            END.
            ASSIGN
              v-text-oss = TRIM(v-text-oss, chr(10))
              EDITOR-2   = v-text-oss
              .
          END.
          else do:
        ...
        DO:
          ...
          do:
            ...
            for each gds-list no-lock, first ub.goods-attr
              where ub.goods-attr.attr-code  = 'oper-serv-idd':U
              and ub.goods-attr.gds-code  = gds-list.gds-code
              NO-LOCK:
              ASSIGN
                v-text-gds = v-text-gds + chr(10) + gds-list.gds-name.
              .
              CREATE tt-gds-list.
              ASSIGN
            ...
            END.
            ASSIGN
              v-text-gds = TRIM(v-text-gds, chr(10))
              EDITOR-3   = v-text-gds
              .
          END.
          else do:
      ...
      WHEN 1 THEN
        DO:
          ...
          FOR EACH ub.goods-attr
            where ub.goods-attr.attr-code  = 'oper-serv-idd':U no-lock,
            ...
            ASSIGN
              ...
              .
            ASSIGN
              v-text-gds = v-text-gds + chr(10) + ub.goods.gds-name
              .
          END.
        END.
      OTHERWISE
...
on ' ' of date-from in frame Dialog-Frame
do:
...
on delete-character of date-from in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of date-from in frame Dialog-Frame
do:
...
on ctrl-b of date-from in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of date-from in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of date-from in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date8
    ...
    .
  if date-from :POPUP-MENU in frame Dialog-Frame = ?
  then do:
    ASSIGN
      date-from :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date8 :HANDLE
      date-from :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle8 as handle no-undo .
  assign
    v-label-handle8 = date-from :side-label-handle in frame Dialog-Frame
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date8-1 in menu m-ed-date8 DO:
    apply "ctrl-b":U to date-from in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date8-2 in menu m-ed-date8 DO:
    apply "ctrl-d":U to date-from in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date8-3 in menu m-ed-date8 DO:
    apply "ctrl-e":U to date-from in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date8-4 in menu m-ed-date8 DO:
    apply "ctrl-f":U to date-from in frame Dialog-Frame .
  END.
define variable vss-include-info9 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on ' ' of date-to in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on delete-character of date-to in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of date-to in frame Dialog-Frame
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of date-to in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of date-to in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of date-to in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date10
    ...
    .
  if date-to :POPUP-MENU in frame Dialog-Frame = ?
  then do:
    ASSIGN
      date-to :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date10 :HANDLE
      date-to :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle10 as handle no-undo .
  assign
    v-label-handle10 = date-to :side-label-handle in frame Dialog-Frame
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date10-1 in menu m-ed-date10 DO:
    apply "ctrl-b":U to date-to in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date10-2 in menu m-ed-date10 DO:
    apply "ctrl-d":U to date-to in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date10-3 in menu m-ed-date10 DO:
    apply "ctrl-e":U to date-to in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date10-4 in menu m-ed-date10 DO:
    apply "ctrl-f":U to date-to in frame Dialog-Frame .
  END.
ASSIGN
  date-from = v-today
  date-to   = v-today
          ...
          do:
            ...
            do ii = 1 to num-entries(v-obj-list, chr(4))
              ...
              assign
                ...
                .
              FIND FIRST ub.clients
                WHERE ub.clients.obj-type = ENTRY(1, v-entry-1, chr(44))
                AND ub.clients.obj-code = INTEGER(ENTRY(2, v-entry-1, chr(44)))
                NO-LOCK
                NO-ERROR
                .
            ...
            IF v-oss-list <> "":U
              and RADIO-SET-2 = 2
              THEN
            do ii = 1 to num-entries(v-oss-list, chr(4))
              on error undo, next
              :
              assign
                v-entry-2 = entry(ii, v-oss-list, chr(4))
                .
              Find first ub.OperServ
                WHERE ub.OperServ.id = INTEGER(ENTRY(1, v-entry-2, chr(44)))
                NO-LOCK no-error .
            ...
            IF v-gds-list <> "":U
              and RADIO-SET-3 = 2
              THEN
            do ii = 1 to num-entries(v-gds-list, chr(4))
              on error undo, next
              :
              assign
                v-entry-3 = entry(ii, v-gds-list, chr(4))
                .
              find first ub.goods-attr no-lock
                where ub.goods-attr.attr-code  = 'oper-serv-idd':U
                and ub.goods-attr.gds-code = INTEGER(ENTRY(1, v-entry-3, chr(44)))
                no-error .
...
PROCEDURE proc-save :
  ...
  for each buf_tt-oss-ref :
    assign
      v-oss-list = v-oss-list + string (buf_tt-oss-ref.id) + chr(4)
      .
  end.
  ...
  for each buf_tt-gds-list :
    assign
      v-gds-list = v-gds-list
                 + string (buf_tt-gds-list.gds-code)  + chr(4)
      .
  end.
  if v-file = "" then
  do:
  ...
  v-oss-list = TRIM(v-oss-list, chr(4)).
  v-gds-list = TRIM(v-gds-list, chr(4)).
  v-param-list = v-folder + chr(4) +
    v-file   + chr(4) +
    STRING( RADIO-SET-1 ) + chr(4) +
    STRING( RADIO-SET-2 ) + chr(4) +
    STRING( RADIO-SET-3 ) + chr(4) +
    string(if date-from <> ? then string(date-from) else "") + chr(4) +
    string(if date-to <> ? then string(date-to) else "") + chr(4)
    .
  IF p-mode = 'shd' THEN
  DO:
