...
assign
    ...
    v-location = LOGICAL(entry(9,v-param-list,chr(4)))
    v-categ-loc = entry(10,v-param-list,chr(4))
    NO-ERROR.
run schedule-attr-value in this-procedure (input integer(p_db-num-char),
                                           input p_task-type,
                                           input p_task-num,
...
if v-obj-list begins 'орг':U then do:
    c_entry = entry(1,v-obj-list,chr(4)).
    for each buf_clients where buf_clients.host-code = integer(entry(2,c_entry,chr(44))):
    ...
    do ii = 1 to num-entries(v-obj-list, chr(4)) on error undo, next:
        c_entry = entry(ii,v-obj-list,chr(4)).
        find first buf_clients where buf_clients.obj-type = entry(1,c_entry,chr(44))
            and buf_clients.obj-code = integer(entry(2,c_entry,chr(44))) no-lock no-error.
