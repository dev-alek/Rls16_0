...
do
  ...
  ASSIGN
    ...
    vRadio-set-1 = INTEGER( ENTRY(3, v-param-list, chr(4)) )
    vRadio-set-2 = INTEGER( ENTRY(4, v-param-list, chr(4)) )
    vRadio-set-3 = INTEGER( ENTRY(5, v-param-list, chr(4)) )
  ...
  do ii-oss = 1 to num-entries(v-oss-list, chr(4))
    on error undo, next
    :
    assign
      v-entry-oss = entry(ii-oss, v-oss-list, chr(4))
      .
    Find first ub.OperServ
      WHERE ub.OperServ.id = INTEGER(ENTRY(1, v-entry-oss, chr(44)))
      NO-LOCK no-error .
  ...
  do ii-gds = 1 to num-entries(v-gds-list, chr(4))
    on error undo, next
    :
    assign
      v-entry-gds = entry(ii-gds, v-gds-list, chr(4))
      .
    find first ub.goods-attr no-lock
      where ub.goods-attr.attr-code  = 'oper-serv-idd':U
      and ub.goods-attr.gds-code = INTEGER(ENTRY(1, v-entry-gds, chr(44)))
      no-error .
