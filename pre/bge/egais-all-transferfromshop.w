...
define variable egais               as class     TransferFromShop no-undo.
...
DEFINE FRAME Dialog-Frame
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER
         ...
         TITLE "Возврат продукции из торгового зала на склад ЕГАИС"
...
DO:
    ...
    run bge/egais-TransferFromShop.w (parparentproc, 'ПРОСМОТР':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
END.
ON CHOOSE OF Btn_Edit IN FRAME Dialog-Frame
DO:
    if not bh-act-header:available then return no-apply .
    find last buf_clob-bind where buf_clob-bind.field-name_ = 'egais-tfs':U and buf_clob-bind.uniq-key-rec = bh-act-header:buffer-field ("num"):buffer-value .
...
DO:
    run bge/egais-TransferFromShop.w (parparentproc, 'ДОБАВЛЕНИЕ':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
...
DO:
    ...
    run bge/egais-TransferFromShop.w (parparentproc, 'ИЗМЕНЕНИЕ':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
    bh-act-header = egais:GetHndlTable(3, "").
    run refresh-query.
END.
...
DO:
    ...
    os-delete "temp-TransferFromShop.xml".
    copy-lob
    from  object buf_clob-data.cdata
    to  file 'temp-TransferFromShop.xml'
    no-convert
    no-error .
    run parseXML in this-procedure (input "temp-TransferFromShop.xml") .
    find first tt-act-header .
    v-file = 'TransferFromShop.xml' .
    os-delete "TransferFromShop.xml" .
    run makeXMLegais_v2 .
...
if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run getcurus in g#library2
  ...
  egais = new TransferFromShop (v-cntxt-obj-type, v-cntxt-obj-code, v-fs-rar, v-ext-sys).
  ...
  create browse browse-hdl-act-header
    assign
      title     = 'Акты о возврате продукции из торгового зала на склад'
