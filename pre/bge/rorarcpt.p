...
assign
v-labels = 'RANGE' + chr(4) +
          'SYNCHRONIZATION' + chr(4) +
          'NULL' + chr(4) +
          'PROCESSING' + chr(4) +
          'STRUCTURE' + chr(4) +
          "NO-ERROR"
...
assign
v-rcpt-log-file = v-log-file-name + chr(92) + ora-rcpt_get-rcpt-name(v-file-name-no-ext) + ".LOG"
.
run rul/ora-rcpt.p (
                     input parparentproc
...
if error-status:error then do:
    if p-silent then do:           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 Ошибка при генерации квитанции.&2&3&2&4"                                 ,vss-workfile                                 ,chr(10)                                ,substitute( "&1", error-status:get-message(error-status:num-messages) )                               ,substitute( "&1", return-value )                               )).                  end.                                               else do:                                             message substitute("&1 Ошибка при генерации квитанции.&2&3&2&4"                                 ,vss-workfile                                 ,chr(10)                                ,substitute( "&1", error-status:get-message(error-status:num-messages) )                               ,substitute( "&1", return-value )                               ) view-as alert-box.      end.
end.
else do:
    if p-silent then do:           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Квитанция для пакета &1 сформирована",  p-espr-pack-num)).                  end.                                               else do:                                             message substitute("Квитанция для пакета &1 сформирована",  p-espr-pack-num) view-as alert-box.      end.
end.
