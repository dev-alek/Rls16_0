...
DO:
    ...
    then do:
        ...
        run local-open-query-not-sel in this-procedure .    run local-open-query-sel in this-procedure .
        apply "entry" to br-table-not-sel.
    end.
END.
...
DO:
    ...
    apply "entry" to br-table-sel.
END.
ON CHOOSE OF bt-sel-properties IN FRAME Dialog-Frame
DO:
    if available buf_init_temp_oxmltd_sel
    then do:
        run bge/oxmlextp.w (
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-table-not-sel :handle
      .
