...
:
    ...
    then do:
      ...
      v-err-str = error-status:get-message(error-status:num-messages) + chr(10) + return-value.
       run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Ошибка при инициализации переменных g#... &1&2"                                      , v-err-str                                      , chr(10))).
        return.
    end.
    ...
    if v-param-list = "":U then do:
       run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Не заданы параметры импорта данных из ААЗС &1&2"                                      , p-task-num                                             , chr(10))).
       return.
    end.
    ...
    if index( v-dir-type, "D" ) = 0 then do:
      run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Выбранный для импорта каталог &1 - недоступен&2"                                      , v-dir-name                                      , chr(10))).
      return .
    end.
    run gbl/tcp-info.p
    ...
    if error-status:error then do:
      run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Ошибка при архивации логов&3&1&3&2&3"                                      , return-value                                      , error-status:get-message(1)                                      , chr(10))).
    end.
    else do:
      run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Архивация логов завершена.&1"                                      , chr(10))).
    end.
end.
procedure main-proc :
...
procedure wri-log-and-file:
    define input parameter p-log-string     as char       no-undo.
      run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("&1"                               , p-log-string                               , chr(10))).
end procedure .
