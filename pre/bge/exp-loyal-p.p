...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pgtxvalg in g#library
  ...
  ,output v-vat-pc
            ...
            if not  can-find (first goods-attr where goods-attr.gds-code = buf_goods.gds-code and  goods-attr.attr-code = 'office-type':U)
                then
            do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pgtxvalg in g#library
  ...
  ,output v-vat-pc
         ...
         if buf_goods.gds-type = "т"  then v-gds-type = "g".
                if buf_goods.gds-type = "у" then v-gds-type = "s".
        if not can-find ( goods-attr where goods-attr.gds-code = buf_goods.gds-code and goods-attr.attr-code = 'office-type':U) then
        do:
