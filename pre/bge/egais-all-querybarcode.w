...
DO:
    ...
    run bge/egais-QueryBarcode.w (parparentproc, 'ПРОСМОТР':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
END.
...
DO:
    ...
    find last buf_clob-bind where buf_clob-bind.field-name_ = 'egais-qb':U and buf_clob-bind.uniq-key-rec = bh-act-header:buffer-field ("num"):buffer-value .
...
DO:
    run bge/egais-QueryBarcode.w (parparentproc, 'ДОБАВЛЕНИЕ':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
    run refresh-query.
END.
ON CHOOSE OF Btn_chg IN FRAME Dialog-Frame
DO:
    if not bh-act-header:available then return no-apply .
    run bge/egais-QueryBarcode.w (parparentproc, 'ИЗМЕНЕНИЕ':U, egais, v-ext-sys, v-fs-rar, bh-act-header:handle) .
    run refresh-query.
END.
...
DO:
  if not bh-act-header:available
    ...
    assign
        v-clob-db-num = ?
        v-int64-id = 0
        v-info = tt-act-header.num + chr(4) + string(tt-act-header.date_) + chr(4) + string(tt-act-header.is-sent)
    .
    find first buf_clob-bind exclusive-lock where buf_clob-bind.uniq-key-rec = tt-act-header.num
                                              and buf_clob-bind.field-name_  = 'egais-qb':U no-error .
