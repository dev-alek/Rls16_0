...
define   shared variable g#passwd as character no-undo .
    define buffer   in-vatp-trn-docloc  for ub.trn-doc .
    define buffer   in-vatp-partsloc    for ub.parts   .
    define buffer   in-vatp-docloc      for ub.trn-doc .
    define buffer   in-vatp-goodsloc    for ub.goods   .
    define buffer   in-vatp-sysconfloc  for ub.sysconf .
    define buffer   in-vatp_doc-attrloc for ub.doc-attr.
    define variable in-vatp-have-vat-sltloc       as   logical initial yes    no-undo.
    define variable vat-pc-locloc                 like ub.doc-line.vat-pc     no-undo.
    define variable varinvprbloc                  as   character              no-undo.
    define variable slt-pc-locloc                 like ub.doc-line.slt-pc     no-undo.
    define variable cli-base-rateloc              as   decimal                no-undo.
    define variable price-rubl-with-tax-locloc    like ub.doc-line.price-rubl no-undo.
    define variable price-base-with-tax-locloc    like ub.doc-line.price-base no-undo.
    define variable price-cli-with-tax-locloc     like ub.doc-line.price-cli  no-undo.
    define variable price-rubl-without-tax-locloc like ub.doc-line.price-rubl no-undo.
    define variable price-base-without-tax-locloc like ub.doc-line.price-base no-undo.
    define variable price-cli-without-tax-locloc  like ub.doc-line.price-base no-undo.
    define variable vat-base-locloc               like ub.doc-line.price-base no-undo.
    define variable vat-rubl-locloc               like ub.doc-line.price-rubl no-undo.
    define variable vat-cli-locloc                like ub.doc-line.price-rubl no-undo.
    define variable slt-base-locloc               like ub.doc-line.price-base no-undo.
    define variable slt-rubl-locloc               like ub.doc-line.price-rubl no-undo.
    define variable slt-cli-locloc                like ub.doc-line.price-rubl no-undo.
    define variable road-tax-base-locloc          like ub.doc-line.road-tax   no-undo.
    define variable road-tax-rubl-locloc          like ub.doc-line.road-tax   no-undo.
    define variable road-tax-cli-locloc           like ub.doc-line.road-tax   no-undo.
    define variable transport-base-locloc         like ub.doc-line.price-base no-undo.
    define variable transport-rubl-locloc         like ub.doc-line.price-rubl no-undo.
    define variable transport-cli-locloc          like ub.doc-line.price-rubl no-undo.
    define variable other-base-locloc             like ub.doc-line.price-base no-undo.
    define variable other-rubl-locloc             like ub.doc-line.price-rubl no-undo.
    define variable other-cli-locloc              like ub.doc-line.price-rubl no-undo.
    define variable exch-rate-cli-locloc          like ub.trn-doc.exch-rate   no-undo.
    define variable varinvatp-envdloc             as   character              no-undo.
    define variable varinvatp-typeloc             as   character              no-undo.
...
do on error undo, return error :
  ...
  for each buf_trn-doc no-lock
     where buf_trn-doc.obj-type   = p-obj-type
       ...
       and can-do ('ie,ee,ep,es,re,rs,we,vt,vp,iv,ev,rv,wm,im,ot,fbr':U, buf_trn-doc.ext-doc-type) = true
  :
    run wp-xmltagopen ( input 2, input "expDocument", input "" ).
    run wp-xmltagput( input 3, input "store",         input v-store-name,             input 0 ).
    run wp-xmltagput( input 3, input "shiftNum",      input buf_trn-doc.shift-num,    input 0 ).
    run wp-xmltagput( input 3, input "shiftDate",     input string(buf_trn-doc.shift-date, "99.99.9999"), input 0 ).
    run wp-xmltagput( input 3, input "codeOperation", input buf_trn-doc.ext-doc-type, input 0 ).
    run wp-xmltagput( input 3, input "factOrder"    , input string(buf_trn-doc.fact-order), input 0 ).
    run wp-xmltagput( input 3, input "dateDoc"      , input string(buf_trn-doc.doc-date,   "99.99.9999"), input 0 ).
    run wp-xmltagput( input 3, input "referenceNo"  , input buf_trn-doc.doc-code,     input 0 ).
...
assign
  price-rubl-with-tax-locloc = buf_parts.price-rubl
  price-base-with-tax-locloc = buf_parts.price-base
.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  ...
  if buf_parts.out-code = 'free-zone':U     or
    ...
    assign
      in-vatp-have-vat-sltloc = yes.
  end.
  else do:
    find first in-vatp_doc-attrloc no-lock
      where in-vatp_doc-attrloc.doc-code  = buf_parts.out-code
        and in-vatp_doc-attrloc.attr-code = 'envd':U
      no-error .
    if not available in-vatp_doc-attrloc then do:
      assign
        in-vatp-have-vat-sltloc = yes.
    end.
    else do:
         in-vatp-have-vat-sltloc = no.
    end.
  end.
  assign
   price-cli-with-tax-locloc = buf_parts.price-cli
   cli-base-rateloc          = buf_parts.cli-base-rate.
  ASSIGN   road-tax-base-locloc  = (if buf_parts.road-tax-base  = ? then 0 else buf_parts.road-tax-base)
           road-tax-rubl-locloc  = (if buf_parts.road-tax-rubl  = ? then 0 else buf_parts.road-tax-rubl).
  ASSIGN  transport-base-locloc = (if buf_parts.transport-base = ? then 0 else buf_parts.transport-base)
          transport-rubl-locloc = (if buf_parts.transport-rubl = ? then 0 else buf_parts.transport-rubl)
          other-base-locloc     = (if buf_parts.other-base     = ? then 0 else buf_parts.other-base)
          other-rubl-locloc     = (if buf_parts.other-rubl     = ? then 0 else buf_parts.other-rubl)
          vat-pc-locloc         = (if buf_parts.vat-pc         = ? then 0 else buf_parts.vat-pc)
          slt-pc-locloc         = (if buf_parts.slt-pc         = ? then 0 else buf_parts.slt-pc).
          ASSIGN   slt-base-locloc    = (if in-vatp-have-vat-sltloc = no then 0 else (price-base-with-tax-locloc - ((if road-tax-base-locloc  = ? then 0 else road-tax-base-locloc) + (if transport-base-locloc = ? then 0 else transport-base-locloc) + (if other-base-locloc = ? then 0 else other-base-locloc)))                           * slt-pc-locloc / (100 + slt-pc-locloc))                        vat-base-locloc    = (if in-vatp-have-vat-sltloc = no then 0 else (price-base-with-tax-locloc - ((if road-tax-base-locloc  = ? then 0 else road-tax-base-locloc) + (if transport-base-locloc = ? then 0 else transport-base-locloc) + (if other-base-locloc = ? then 0 else other-base-locloc))) * (1 - slt-pc-locloc / (100 + slt-pc-locloc)) * vat-pc-locloc / (100 + vat-pc-locloc)).
    ASSIGN   slt-rubl-locloc    = (if in-vatp-have-vat-sltloc = no then 0 else (price-rubl-with-tax-locloc - ((if road-tax-rubl-locloc  = ? then 0 else road-tax-rubl-locloc) + (if transport-rubl-locloc = ? then 0 else transport-rubl-locloc) + (if other-rubl-locloc = ? then 0 else other-rubl-locloc)))                           * slt-pc-locloc / (100 + slt-pc-locloc))                        vat-rubl-locloc    = (if in-vatp-have-vat-sltloc = no then 0 else (price-rubl-with-tax-locloc - ((if road-tax-rubl-locloc  = ? then 0 else road-tax-rubl-locloc) + (if transport-rubl-locloc = ? then 0 else transport-rubl-locloc) + (if other-rubl-locloc = ? then 0 else other-rubl-locloc))) * (1 - slt-pc-locloc / (100 + slt-pc-locloc)) * vat-pc-locloc / (100 + vat-pc-locloc)).
  assign
    exch-rate-cli-locloc = (buf_parts.price-rubl - transport-rubl-locloc - other-rubl-locloc - road-tax-rubl-locloc - (if buf_parts.vat-type <> 'в т. ч.':U then vat-rubl-locloc else 0) - (if buf_parts.slt-type <> 'в т. ч.':U then slt-rubl-locloc else 0)) / buf_parts.price-cli .
  assign
    slt-cli-locloc        = slt-rubl-locloc       / exch-rate-cli-locloc
    vat-cli-locloc        = vat-rubl-locloc       / exch-rate-cli-locloc
    road-tax-cli-locloc   = road-tax-rubl-locloc  / exch-rate-cli-locloc
    transport-cli-locloc  = 0
    other-cli-locloc      = 0
  .
ASSIGN
          price-base-without-tax-locloc = price-base-with-tax-locloc - vat-base-locloc - slt-base-locloc - ((if road-tax-base-locloc  = ? then 0 else road-tax-base-locloc) + (if transport-base-locloc = ? then 0 else transport-base-locloc) + (if other-base-locloc = ? then 0 else other-base-locloc))
    price-rubl-without-tax-locloc = price-rubl-with-tax-locloc - vat-rubl-locloc - slt-rubl-locloc - ((if road-tax-rubl-locloc  = ? then 0 else road-tax-rubl-locloc) + (if transport-rubl-locloc = ? then 0 else transport-rubl-locloc) + (if other-rubl-locloc = ? then 0 else other-rubl-locloc))
.
    ...
    case buf_trn-doc.ext-doc-type :
      ...
      otherwise do :
        run wp-xmltagput( input 3, input "quantity"     , input string(buf_trn-doc.fact-qnty), input 0 ).
        run wp-xmltagput( input 3, input "DocSum_SumR"  , input string(buf_trn-doc.tot-doc),   input 0 ).
        run wp-xmltagput( input 3, input "CostSum_SumR" , input string(v-cost-sumr),  input 0 ).
      end .
    end case .
  ...
  :
    ...
    run wp-xmltagput( input 3, input "codeOperation", input 'ot':U,       input 0 ).
