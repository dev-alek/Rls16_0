...
if v-place = 1 then   v-param-list =  v-directory + chr(4) + string(v-place) + chr(4) + v-login + chr(4) + v-password  + chr(4) + string(type-exp) + chr(4) + string(code_pool) + chr(4) + per-izm + chr(4) +  string(v-long-code) + chr(4) +  cb-spl-ptrl.
if v-place = 2 then do:
     v-ftp-address = trim(trim(replace(v-ftp-address,'ftp:',""),chr(47)),chr(92)).
      v-param-list = v-ftp-address + chr(4) + string(v-place) + chr(4) + v-login + chr(4) + v-password + chr(4) + string(type-exp) + chr(4) +  string(code_pool) + chr(4) + per-izm  + chr(4) +  string(v-long-code) + chr(4) +  cb-spl-ptrl.
                     end.
...
DO:
    assign per-izm.
END.
ON VALUE-CHANGED OF cb-spl-ptrl IN FRAME Dialog-Frame
DO:
  assign cb-spl-ptrl.
END.
...
END PROCEDURE.
PROCEDURE my-enable :
    v-directory:screen-value in frame Dialog-Frame = session:temp-directory.
    assign
        v-directory.
    case p-mode:
        when "run" then
        ...
        when "shd" then
            do:
                ...
                do:
                    v-place  = integer(entry(2,v-param-list,chr(4)) ) no-error.
                    if  v-place = 1 then
                    do:
                        assign
                            v-directory = entry(1,v-param-list,chr(4))
                            v-login     = entry(3,v-param-list,chr(4))
                            v-password  = entry(4,v-param-list,chr(4))
                            type-exp    = integer(entry(5,v-param-list,chr(4)))
                            code_pool   = entry(6,v-param-list,chr(4))
                            per-izm     = entry(7,v-param-list,chr(4))
                           v-long-code = integer(entry(8,v-param-list,chr(4)))
                           cb-spl-ptrl = entry(9,v-param-list,chr(4))
   NO-ERROR.
                        display v-directory
                            type-exp
                            per-izm
                            ...
                            code_pool
                            cb-spl-ptrl
                             v-long-code with frame Dialog-Frame.
                        disable v-ftp-address v-login v-password with frame Dialog-Frame.
                        if type-exp = 2 then display per-izm  with frame Dialog-Frame.
                        else disable per-izm  with frame Dialog-Frame.
                    end.
                    if  v-place = 2 then
                    do:
                        assign
