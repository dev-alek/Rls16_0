...
DO:
   ...
   END.
ON choose OF Btn_add_goods IN FRAME Dialog-Frame
DO:
      run bge/goodsGTINbarcode.w (input parparentproc, input-output table tt-goods by-reference, input f-type-mark:screen-value in frame Dialog-Frame, output v-recid)  .
      OPEN QUERY BROWSE-GOODS FOR EACH tt-goods NO-LOCK.
      reposition BROWSE-GOODS to recid v-recid no-error .
   END.
ON choose OF Btn_add_group IN FRAME Dialog-Frame
DO:
      run bge/groupGTINbarcode.w (input parparentproc, input-output table tt-goods by-reference, input f-type-mark:screen-value in frame Dialog-Frame, output v-recid)  .
      OPEN QUERY BROWSE-GOODS FOR EACH tt-goods NO-LOCK.
      reposition BROWSE-GOODS to recid v-recid no-error .
   END.
...
DO:
   ...
   END.
ON choose OF Btn_open IN FRAME Dialog-Frame
DO:
...
DO:
      assign f-type-mark .
   END.
ON CHOOSE OF MENU-ITEM m_save_1
DO:
      ...
      run define-full-path-Report(input 1, input v-dir, output v-file-name-rep-htm, output v-file-name-rep-exl).
      ...
      apply 'choose' to btn_save in frame Dialog-Frame .
   END.
ON CHOOSE OF MENU-ITEM m_save_2
DO:
      define variable undelete as logical no-undo .
      define variable v-dir as character no-undo .
      v-dir = session:temp-directory .
      run bge/to_file.w (input-output v-dir) no-error .
      run define-full-path-Report(input 2, input v-dir, output v-file-name-rep-htm, output v-file-name-rep-exl).
      message "Сохранить данные в файл?"
      view-as alert-box question buttons yes-no update undelete.
      if undelete then do:
      run file-save in this-procedure no-error .
