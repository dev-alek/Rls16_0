...
DEFINE VARIABLE tb-exp-s-f AS LOGICAL INITIAL no
     LABEL "Счета-фактуры"
     VIEW-AS TOGGLE-BOX
     SIZE 20.25 BY .83 NO-UNDO.
...
ASSIGN
       ed-object:READ-ONLY IN FRAME Dialog-Frame        = TRUE.
ASSIGN
...
run userobjs_select-many in this-procedure
  ...
  ,output v-user-select
    ...
    then do:
        empty temp-table temp_obj-list.
    end.
    else do:
        message
            "Объект не выбран"
...
DO:
    ...
    run manage-options in this-procedure.
END.
ON VALUE-CHANGED OF tb-exp-s-f IN FRAME Dialog-Frame
DO:
    assign
        tb-exp-s-f
    .
...
do:
  ...
  then do:
    ASSIGN
      fi-date-from :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date11 :HANDLE
      fi-date-from :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle11 as handle no-undo .
  assign
    v-label-handle11 = fi-date-from :side-label-handle in frame Dialog-Frame
  .
...
do:
  ...
  then do:
    ASSIGN
      fi-date-to :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date13 :HANDLE
      fi-date-to :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle13 as handle no-undo .
  assign
    v-label-handle13 = fi-date-to :side-label-handle in frame Dialog-Frame
  .
...
:
    ...
    define input parameter p-tb-exp-s-f         as logical      no-undo.
...
define output parameter p-tb-exp-s-f            as logical      no-undo.
...
:
    ...
    then do:
        ...
        assign
            ...
            p-exclude-obj-list      = "":U
            v-db-num-char           = buf_schedule.db-num-char
        .
    end.
    if v-db-num-char <> "*":U
    then do:
