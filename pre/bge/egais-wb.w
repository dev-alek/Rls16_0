...
ON CHOOSE OF b-choose-cons IN FRAME Dialog-Frame
  DO:
    ...
    if v-old-regId = ? then do:
      run ref/extclas1.p ( input 'ДОБАВЛЕНИЕ':U
        ...
        ,input bh-wb-gds-EG-header:buffer-field ("regID-cons"):buffer-value
        ,input 0
    ...
    do trans:
      find first ub.clob-bind where ub.clob-bind.uniq-key-rec = v-uniq-key-rec and ub.clob-bind.field-name_ = 'egais-wb':U.
      entry (9, ub.clob-bind.descr, chr(4)) = buf_clients.obj-type + string (buf_clients.obj-code).
    ...
    run refresh-view.
  END.
ON CHOOSE OF b-choose-ship IN FRAME Dialog-Frame
  DO:
...
PROCEDURE msdblcl :
  ...
  do :
    ...
    then do:
      ...
      then do:
          find first buf_clob-bind exclusive-lock where buf_clob-bind.uniq-key-rec = v-uniq-key-rec and buf_clob-bind.field-name_ = 'egais-ref-b':U.
