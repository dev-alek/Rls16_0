...
VIEW FRAME Dialog-Frame.
ENABLE
f-keep-days WHEN p-mode <> 'ПРОСМОТР':U
f-luft-days WHEN p-mode <> 'ПРОСМОТР':U
...
VIEW FRAME Dialog-Frame.
assign
e-shops:read-only in frame Dialog-Frame = yes.
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры удаления с касс партионных кодов по флагам ЗАКОНЧИВШИХСЯ партий по товару, продающемуся по партиям (профайл &1)", p-profile-id).
...
ASSIGN
e-shops:SCREEN-VALUE IN FRAME Dialog-Frame = ''.
for each obj-list :
...
run ref/thobjs.w
    ...
    , input "b-mark,b-sel"
    , input (if v-running-mode then "db" else 'все':U)
    , input 'маг':U
    , input (if v-running-mode then v-cntxt-db-num else ?)
    , input ?
