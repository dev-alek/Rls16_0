...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-ruleset SHARE-LOCK.
...
if p-ruleset-id > 0 then do:
  assign
  frame Dialog-Frame:title = "Набор правил"
  .
end.
else do:
  assign
  frame Dialog-Frame:title = "Кодекс правил"
  .
end.
IF AVAILABLE tt-ruleset THEN
DISPLAY
...
b-quit
B-Help
tt-ruleset.codex_id  when p-mode = 'ДОБАВЛЕНИЕ':U
tt-ruleset.ruleset_id when p-mode = 'ДОБАВЛЕНИЕ':U
tt-ruleset.name       when p-mode <> 'ПРОСМОТР':U
tt-ruleset.documentation when p-mode <> 'ПРОСМОТР':U
WITH FRAME Dialog-Frame .
if p-mode = 'ПРОСМОТР':U then do:
  assign
  b-quit:label = "&Выход"
