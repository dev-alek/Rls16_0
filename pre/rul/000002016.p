...
for each  buf_temp-xml-tables where
  ...
  REPEAT:
    ...
    do on error undo, retry:
      ...
      run get-last-rec-ord in p-cmd-proc-handle ( input p-cmd-code, output v-last-rec-ord).
          IF  ImpData1:current-tbl-name( ) = 'dis-card':U  THEN do:
            v-line-num = ImpData1:route-data_get-field-integer( input 'dis-card':U, input "line-num") .
            Card-number1 = "" .
            v-category  = 0.
            v-type-from-file = ImpData1:route-data_get-field-character( input 'dis-card':U, input "type") .
            if v-type-from-file <> v-type then do:
              next _stroka .
            end.
            Card-number1 = ImpData1:route-data_get-field-character( input 'dis-card':U, input "d-card") .
            v-cli-type = ImpData1:route-data_get-field-character( input 'dis-card':U, input "cli-type") .
            v-cli-code = ImpData1:route-data_get-field-integer( input 'dis-card':U, input "cli-code") .
            v-line-num = ImpData1:route-data_get-field-integer( input 'dis-card':U, input "line-num") .
            v-issue-date = ImpData1:route-data_get-field-date( input 'dis-card':U, input "issue-date") .
            v-valid-date = ImpData1:route-data_get-field-date( input 'dis-card':U, input "valid-date") .
            v-cli-type-code = v-cli-type + string(v-cli-code).
            v-category = ImpData1:route-data_get-field-integer( input 'dis-card':U, input "category") .
        v-line-status_ = ImpData1:route-data_get-field-character( input 'dis-card':U, input "status_") .
            _tr:
        do
            ...
            :
              ...
              IF  Card1:find_dis-card_no-error( INPUT Card-number1) = false  THEN do:
                v-line-status_ = 'N'.
                IF  Card1:create_dis-card_( INPUT 'маг':U, INPUT v-issue-code, INPUT Card-number1, INPUT v-emitent-host-code, INPUT v-type, INPUT v-cli-type-code) = false  THEN do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input (v-last-error-message + chr(10) + "Запись: " + String(v-line-num))).
                  undo _rule, retry _rule .
                end.
              end.
          else do:
            if v-cli-type-code <> Card1:cli-type + string(Card1:cli-code) then do:
                            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input ("Изменение держателя карты НЕДОПУСТИМО" + chr(10) + "Запись: " + String(v-line-num))).
              undo _rule, retry _rule .
            end.
            if v-type <> Card1:type then do:
                            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input ("Изменение ТИПА карты НЕДОПУСТИМО" + chr(10) + "Запись: " + String(v-line-num))).
              undo _rule, retry _rule .
            end.
          end.
              ...
              if v-valid-date < v-today then do:
                Card1:status_ = 'удал':U.
              end.
              if v-valid-date >= v-today then do:
                Card1:status_ = 'тек':U.
              end.
              if v-line-status_ = 'N' then do:
     ...
     _release:
      ...
      if num-rec modulo 100 = 0 then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Записи &1-&2: сохранение/пересылка в СПН...", num-rec - 100, num-rec)).
        find first buf_temp-cmd use-index pi.
        run after-command in p-parent-handle (buffer buf_temp-cmd) no-error.
        if error-status:error then do:
                   run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении/пересылке по СПН:&1&2&1&3"                                       , chr(10)                                       , error-status:get-message(1)                                       , return-value )).
          undo _main, return error .
        end.
        run before-command in p-parent-handle (buffer buf_temp-cmd) no-error.
        if error-status:error then do:
                   run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при инициации сохранения/пересылки пакета по СПН:&1&2&1&3"                                       , chr(10)                                       , error-status:get-message(1)                                       , return-value )).
          undo _main, return error .
        end.
        find first buf_temp-cmd use-index pi.
  ...
  if valid-handle(v_qh) then  v_qh:query-close().
    ...
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Записи &1-&2: сохранение/пересылка в СПН..."                                         , (num-rec - num-rec modulo 100 + 1), num-rec)).
  find first buf_temp-cmd use-index pi.
  run after-command in p-parent-handle (buffer buf_temp-cmd) no-error.
  if error-status:error then do:
        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении/пересылке по СПН:&1&2&1&3"                                 , chr(10)                                 , error-status:get-message(1)                                 , return-value )).
