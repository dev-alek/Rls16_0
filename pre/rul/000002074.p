...
define temp-table Status_-t  no-undo
    ...
    if status_-t.status_ = 0 then do:
      case status_-t.messageclass:
        when "ORDERS" then do:
            ...
            :
              if buf_edi-status.state = '1':U then do:
      ...
      case status_-t.messageclass:
        ...
        when "ORDRSP" then do:
          ...
          find first buf_ord-doc no-lock where
              ...
              and entry(1, buf_ord-doc.cli-out-doc, chr(4)) = status_-t.customericid no-error.
          if not available buf_ord-doc then do:
      ...
      or not v-execute then do:
        ...
        v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ediiterchangeid':U, status_-t.EXiteICID).
        v-date-status = ?.
