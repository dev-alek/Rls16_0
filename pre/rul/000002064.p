...
define new shared temp-table report-headert no-undo
...
report-id
.
define new shared temp-table report-parameterst no-undo
field report-id as character
...
parameter-index
.
define new shared temp-table report-errorst no-undo
field report-id as character
...
report-id
ErrNum.
define new shared temp-table report-destinationt no-undo
field report-id as character
...
p-reporth = widget-handle(v-value-character).
    case p-ruleset-id:
      ...
      then do:
        ...
        or not glog
        then do:
                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("!!Не удалось заполнить блок данными ОБ отчете!!! &1"                                       , error-status:get-message(1) )).
          delete object p-reporth no-error.
          return error.
        end.
