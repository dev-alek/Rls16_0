...
.
define variable vss-include-info72 as character format "x(65)" no-undo initial "@(#)$Workfile$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output varinvprb
  )  .
   find first in-vatp_doc-attr no-lock
...
def var vss-include-info90 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
procedure check-use-bar-code :
  define input  parameter p-b-code    like ub.bar-code.b-code no-undo .
  do
  on error  undo, return error substitute( "&1. &2&3&4", vss-include-info90, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
  on stop   undo, return error substitute( "&1. stop", vss-include-info90 )
  on endkey undo, return error substitute( "&1. endkey", vss-include-info90 )
  :
    define buffer buf_bar-code for ub.bar-code .
    find first buf_bar-code no-lock
    ...
    if not available buf_bar-code then do:
      return error substitute( "&1 (check-use-bar-code). Не найден бар-код &2", vss-include-info90, p-b-code ) .
    end.
...
then do:
  message
  '!!!При выполнении произошли ошибки!!!'  skip
  "!!!Внимательно прочитайте Log-file!!"
  view-as alert-box error .
     define variable v-user-action98   as character no-undo .
  define variable v-printed98       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При выполнении произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action98
    ,output v-printed98
    ) .
end.
if v-view-log = true
...
define buffer ver_price-list for ub.price-list  .
  ...
  if v-is-n = false then return .
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Скидочные ДНЦ")).
  _stroka:
  for each pdf-list
  ...
  if v-is-n = false then next .
    ...
    num-rec = num-rec + 1
    .
        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute ("Обработка ДНЦ &1 &2", pdf-list.pdf-id , pdf-list.pdf-db)).
    if v-err then next.
    ...
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Обработано ДНЦ списка : &1, из них удачно: &2", num-rec, num-rec-ok)).
end.
end procedure.
...
do
  on error undo, return error return-value
  :
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Формирование скидочных ДНЦ по ТПЛ &1" , p-tpl)).
  assign
    v-dis-plt-id = int(entry(1,p-tpl,"-"))
    v-dis-plt-db = int(entry(2,p-tpl,"-"))
  no-error .
    if p-coeff = 0  or p-coeff = ? then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не верно задан коэффициент в настройках машины правил"  )).
        return error return-value .
    end.
    find first buf_price-list-type no-lock where
     ...
     if error-status :error then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найден ТПЛ &1(&2)  для порождения ДНЦ :  &3 &4 "  ,v-dis-plt-id,v-dis-plt-db, return-value , error-status :get-message(1)  )).
        return error return-value .
     end.
     if buf_price-list-type.stts  =  int('1':U)  then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("ТПЛ &1(&2) Удален !!! пропускаем "  ,v-dis-plt-id,v-dis-plt-db  )).
        return  .
     end.
     run metod-gop-obj in this-procedure (
        g#db-num ,
        ...
        if error-status :error then do:
                        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Сбор оъектов по  ТПЛ &1(&2) : &3 &4 "  ,v-dis-plt-id,v-dis-plt-db, return-value , error-status :get-message(1)  )).
            return error return-value .
        end.
  v-exs-obj = false .
  for each buf_price-doc no-lock where
    ...
    if v-exs-obj = false then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("По ТПЛ &1(&2) скидочные ДНЦ созданы не будут"  ,v-dis-plt-id,v-dis-plt-db, return-value , error-status :get-message(1)  )).
        return .
    end.
  for each buf_price-doc no-lock where
      buf_price-doc.pdf-id     = p-pdf-id    and
      ...
      if error-status :error then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Переоценка &1 -> Скидочный ТПЛ &2(&3): ошибка &4 &5" ,buf_price-doc.doc-num,v-dis-plt-id,v-dis-plt-db, return-value , error-status :get-message(1)  )).
        return error return-value .
      end.
  end.
end.
  ...
  find first  buf_price-doc no-lock where
   ...
   for each buf_price-list no-lock where
    ...
    run new-pdf (
      ...
      if error-status :error then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при создании   ДНЦ: &1(&2) &3 &4 ",v-pdf-id, v-pdf-db-num, return-value , error-status :get-message(1))) .
        undo, return error return-value .
      end.
    run make-attr-objexcl (
        ...
        v-code-attr =  'relprpdf':U + chr(4) + string(p-dis-plt-id) + chr(4) + string(p-dis-plt-dB) .
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdatothn in g#trdcalib ( input buf_price-doc.doc-num ,
                       input v-code-attr
                       ) no-error .
      if error-status :error then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при обработке атрибута  ДНЦ: &1(&2) переоценка :&3 &4 &5 &6",v-pdf-id, v-pdf-db-num, buf_price-doc.doc-num , return-value , error-status :get-message(1) , v-code-attr)) .
        undo, return error return-value .
      end.
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Создано ДНЦ &1(&2)", v-pdf-id, v-pdf-db-num )).
  end.
end procedure.
...
define buffer buf_goods for ub.goods  .
  ...
  :
         ...
         if error-status :error then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("4. ошибка &1 &2", return-value,  error-status :get-message(1) )).
                  return error return-value .
         end.
         create ub.price-doc-forming .
         assign
   ...
   find first buf_price-doc-forming  exclusive-lock where recid (buf_price-doc-forming) =  recid ( ub.price-doc-forming ) no-error .
      if error-status :error then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("3. ошибка &1 &2", return-value,  error-status :get-message(1) )).
        return error return-value .
      end.
    v-counter = 0.
    for each temp-tt :
        ...
        run create-line-pdf-mpl-lib (
            ...
            ,input-output v-sec ) no-error .
                if error-status :error then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("1. ошибка &1 &2", return-value,  error-status :get-message(1) )).
                  return error return-value .
                end.
    end.
    for each temp-pdf :
    ...
    ub.price-doc-forming.stts  = integer('4':U).
    release buf_price-doc-forming no-error .
      if error-status :error then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("2. ошибка &1 &2", return-value,  error-status :get-message(1) )).
        return error return-value .
      end.
  end.
end procedure.
