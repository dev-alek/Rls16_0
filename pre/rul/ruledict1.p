...
:
  if lookup(p-entry-type, 'constant,operator,control,prop-name_,prop-name_host,prop-name_obj,node-name_,node-name_host,node-name_obj,prop-script,sum-id,rule,rule-profile,datatype,parameter':u) = 0
  and not (p-mode = 'ИЗМЕНЕНИЕ':U  and p-entry-id = 0)
  then do:
      assign
