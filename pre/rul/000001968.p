...
:
  ...
  for each cgrp-list
  break
  by entry(1, cgrp-list.full-name, chr(47))
  ...
  :
    ...
    if not available buf_cli-grp then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найдена содержащаяся в списке группа клиентов с вн.кодом &1", cgrp-list.node-code)).                      assign v-view-log = yes.
      next _stroka.
    end.
    ...
    if last( entry(1, cgrp-list.full-name, chr(47)))
    or (v-next-file  and last-of(entry(1, cgrp-list.full-name, chr(47))))
    then do:
      v-next-file = no.
