...
define variable v-waiting-stfl-i as integer no-undo .
do
on error  undo, return error substitute( "&1. &2&3&4", vss-include-info12, return-value, chr(10), error-status :get-message (1))
:
if p-newbh = ?  then do:
...
case p-ruleset-id:
  ...
  case p-ruleset-id:
    ...
    if not available buf_price-doc then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найдена переоценка &1", v-doc-code)).
  ...
  if p-ruleset-id = 110 then leave _stroka.
end.
end.
end procedure.
...
:
  if retry then do:
    ...
    v-obj-type-code = (if buf_price-doc.obj-type = 'скл':U then 100000 else 0) + buf_price-doc.obj-code.
