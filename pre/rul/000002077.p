...
define temp-table DESADV-t no-undo
...
field ORDERPARTNER as character format "9999999999999"
...
.
define temp-table POSITION2PACKINGSEQUENCE-t no-undo
field NUMBER  as character
field HIERARCHICALID as integer
field POSITIONNUMBER as integer
...
index pi is unique primary  number positionnumber
.
define dataset DESADV_-t
for
DESADV-t, HEAD-t, POSITION-t
data-relation r1 for DESADV-t, HEAD-t relation-fields ( NUMBER, NUMBER) nested
data-relation r3 for HEAD-t, POSITION-t  relation-fields ( NUMBER, positionnumber, number, positionnumber ) nested
.
...
define buffer buf_pos for position-t.
main-block:
do
...
:
  ...
  define variable v-ps as character no-undo .
   ...
   :
     ...
     if not available buf_ord-line then do:
       ...
       run create-edi-statett in this-procedure (
                                              input 'ord-doc':U
                                            , input (buf_ord-doc.doc-code + chr(4) + string( - buf_pos.positionnumber))
                                            , input buf_ord-doc.cli-type
                                            , input buf_ord-doc.cli-code
                                            , input 'ИЗМЕНЕНИЕ':U
  ...
  :
    ...
    if buf_pos.orderunit <> buf_ord-line.unit-cli then do:
      ...
      v-fatal = yes.
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, "Ошибка в ед.изм.заказа").
    end.
    if buf_pos.deliveredunit <> buf_ord-line.unit-cli then do:
       v-found-unit-cli = yes.
       v-fatal = yes.
       v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, "Ошибка в ед.изм.поставки").
    end.
    v-b-str = ''.
    ...
    if v-b-str <> buf_pos.product then do:
        v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'bstr-change':U, string(v-b-str) + chr(4) + string(buf_pos.product) ).
    end.
    if buf_ord-line.cli-qnty   <  loc-sum-rcv  + buf_pos.deliveredquantity  then do:
      v-fatal = yes.
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'qnty-down':U, string(buf_ord-line.cli-qnty) + chr(4) + string(loc-sum-rcv + buf_pos.deliveredquantity) ).
    end.
    if buf_ord-line.cli-qnty   > buf_pos.deliveredquantity  then do:
      v-psq = yes.
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'qnty-down':U, string(buf_ord-line.cli-qnty) + chr(4) + string(buf_pos.deliveredquantity) ).
    end.
  ...
  if v-found then do:
    v-edist-mess = ''.
    v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, "Превышение заказанного кол-ва").
  end.
  if v-found-unit-cli then do:
    v-edist-mess = ''.
    v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, "Ошибка в ед изм.").
  end.
...
run write-log  in p-log-handle (
  ...
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт поставок из ВС")).                      assign v-view-log = yes.
    ...
    if error-status:error then do:
      ...
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ediiterchangeid':U, (if available head-t then head-t.EDIINTERCHANGEID else '') ).
      v-date-status = ?.
      run create-edi-state in this-procedure (
                                              input 'ord-doc':U
                                            , input buf_ord-doc.doc-code
    ...
    case p-ruleset-id:
      when 8 then do:
       ...
       .
        ...
        if error-status:error
        or not glog then do:
                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка копирования импортируемых данных для дальнейшей обработки&1&2"                                         , chr(10)                                         , error-status:get-message(1)                                         )).                      assign v-view-log = yes.
          undo, return error substitute("Ошибка копирования импортируемых данных для дальнейшей обработки&1&2"                                         , chr(10)                                         , error-status:get-message(1)                                         ).
        end.
      end.
      otherwise do:
