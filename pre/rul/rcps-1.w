...
END.
ON CHOOSE OF b-dis-tot-obj-code IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE rid-list as CHARACTER no-undo.
...
END.
ON CHOOSE OF b-issue-code IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE rid-list as CHARACTER no-undo.
  DEFINE BUFFER buf_shop FOR ub.shop.
  DEFINE BUFFER buf_clients FOR ub.clients.
...
on ' ' of f-issue-date in frame Dialog-Frame
do:
...
on delete-character of f-issue-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of f-issue-date in frame Dialog-Frame
do:
...
on ctrl-b of f-issue-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of f-issue-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of f-issue-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date16
    ...
    .
  if f-issue-date :POPUP-MENU in frame Dialog-Frame = ?
  then do:
    ASSIGN
      f-issue-date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date16 :HANDLE
      f-issue-date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle16 as handle no-undo .
  assign
    v-label-handle16 = f-issue-date :side-label-handle in frame Dialog-Frame
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date16-1 in menu m-ed-date16 DO:
    apply "ctrl-b":U to f-issue-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date16-2 in menu m-ed-date16 DO:
    apply "ctrl-d":U to f-issue-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date16-3 in menu m-ed-date16 DO:
    apply "ctrl-e":U to f-issue-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date16-4 in menu m-ed-date16 DO:
    apply "ctrl-f":U to f-issue-date in frame Dialog-Frame .
  END.
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on ' ' of f-valid-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on delete-character of f-valid-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of f-valid-date in frame Dialog-Frame
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of f-valid-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of f-valid-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of f-valid-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date18
    ...
    .
  if f-valid-date :POPUP-MENU in frame Dialog-Frame = ?
  then do:
    ASSIGN
      f-valid-date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date18 :HANDLE
      f-valid-date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle18 as handle no-undo .
  assign
    v-label-handle18 = f-valid-date :side-label-handle in frame Dialog-Frame
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date18-1 in menu m-ed-date18 DO:
    apply "ctrl-b":U to f-valid-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date18-2 in menu m-ed-date18 DO:
    apply "ctrl-d":U to f-valid-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date18-3 in menu m-ed-date18 DO:
    apply "ctrl-e":U to f-valid-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date18-4 in menu m-ed-date18 DO:
    apply "ctrl-f":U to f-valid-date in frame Dialog-Frame .
  END.
...
run rcps_get-rule-on-off in this-procedure ( input 1
                                            ,input 1
                                            ,input 1303
                                            ,input p-profile-id
                                            ,input p-once-more
...
if f-dis-tot-obj-code > 0 then do:
  FIND FIRST buf2_clients No-LOCK WHERE
            buf2_clients.obj-type = 'маг':U
        AND buf2_clients.obj-code = f-dis-tot-obj-code NO-ERROR.
end.
editor-1:screen-value in frame Dialog-Frame = "данные параметры используются при импорте новых карт из текстового файла для установки значений по умолчанию"  .
display
f-dis-tot-obj-code
f-issue-code
f-issue-date
...
VIEW FRAME Dialog-Frame.
ENABLE
f-issue-date WHEN p-mode <> 'ПРОСМОТР':U
f-valid-date WHEN p-mode <> 'ПРОСМОТР':U
b-cli-grp WHEN p-mode <> 'ПРОСМОТР':U
f-category WHEN p-mode <> 'ПРОСМОТР':U
f-lim-cr WHEN p-mode <> 'ПРОСМОТР':U
b-cli-grp WHEN p-mode <> 'ПРОСМОТР':U
b-issue-code WHEN p-mode <> 'ПРОСМОТР':U
b-dis-tot-obj-code WHEN p-mode <> 'ПРОСМОТР':U
t-on-off WHEN p-mode <> 'ПРОСМОТР':U
...
then do:
  ...
  run rcps_set-rule-on-off in this-procedure ( input 1
                                            ,input 1
                                            ,input 1303
                                            ,input p-profile-id
                                            ,input p-once-more
