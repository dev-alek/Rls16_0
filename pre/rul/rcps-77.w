...
run rcps_get-rule-on-off in this-procedure ( input 18
                                            ,input 105
                                            ,input 2078
                                            ,input p-profile-id
                                            ,input p-once-more
...
VIEW FRAME Dialog-Frame.
ENABLE
t-rule-desadv-sts WHEN p-mode <> 'ПРОСМОТР':U
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры обмена данными по заказам поставщику через систему  Exite-EDI (профайл &1)", p-profile-id).
END PROCEDURE.
PROCEDURE proc-save :
define variable v-ii as integer   no-undo .
v-ii = 0.
...
t-rule-desadv-sts
.
run rcps_set-rule-on-off in this-procedure ( input 18
                                           ,input 105
                                           ,input 2078
                                           ,input p-profile-id
                                           ,input p-once-more
