...
do while true:
  ...
  if v-current-gds-code > 0 then do:
    ...
    ExpData1:route-data_copy-field-logical( INPUT "good", "deleted", INPUT  buf_goods.stts <> integer('0':U) ) .
  ...
  if p-ruleset-id = 5
  or (p-ruleset-id = 2 and  num-rec modulo v-pck-num-rec = 0)
  ...
  if p-ruleset-id = 2 then do:
    run get-stop-state in p-log-handle ( output v-stop) no-error .
    if v-stop then do:
            if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Процесс прерван пользователем")).
      leave _stroka.
    end.
  end.
end.
if p-ruleset-id = 2
then do:
    if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Просмотрено товаров списка &1, обработано  &2, из них удачно: &3", num-rec-pre, num-rec, num-rec-ok)).
end.
end.
end procedure.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run calltree in g#library
  (input  'cb_get-next-gds-by-gds-code'
  ,input  this-procedure:handle
  ,input  p-cont-handle
  ,output v-gds-list-handle
  )  .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run calltree in g#library
  ...
  when 5 then do:
      ...
      if v-has-newbh
      and v-newbh:table <> 'goods':U then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'goods':U).
      end.
