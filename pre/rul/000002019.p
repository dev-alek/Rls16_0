...
define buffer buf_tt-staff for temp-staff_.
...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по клиентам из ВС")).
         ...
         if retry then do:
          ...
          case v-current-tbl-name:
            when 'clients':U  THEN do:
              ...
              :
                v-line-num = ImpData1:route-data_get-field-integer( input 'clients':U, input "line-num") .
                v-present = no.
                num-rec = num-rec + 1.
                v-current-cli-type = ImpData1:route-data_get-field-character( input 'clients':U, input "obj-type") .
                v-current-cli-code = ImpData1:route-data_get-field-integer( input 'clients':U, input "obj-code") .
                v-current-cli-name = ImpData1:route-data_get-field-character( input 'clients':U, input "obj-name") .
                ...
                .
                assign
                glog = buffer buf_temp-clients_:handle:buffer-copy(ImpData1:route-data_get-record('clients':U), "obj-type,obj-code,obj-name") no-error.
                if not glog
                or error-status:error then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не удалось получить данные записи clients: &1&2&3"                                               , error-status:get-message(1)                                               , chr(10)                                               , return-value)).
                ...
                for each buf_temp-rel-handle where
                  ...
                  if error-status:error then do:
                    ...
                    case buf_temp-rel-handle.child-buffer_:
                      when 'firm':U
                      THEN do:
                        v-current-firm-code = ImpData1:route-data_get-field-integer( buffer buf_temp-rel-handle:handle, input 'firm':U , input "firm-code") .
                        ...
                        if not glog
                        or error-status:error then do:
                                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не удалось получить данные записи <&4>: &1&2&3"                                                       , error-status:get-message(1)                                                       , chr(10)                                                       , return-value                                                       , 'firm':U)).
                          undo _rule, retry _rule.
                        end.
                      ...
                      when 'person':U
                      THEN do:
                        v-current-psn-code = ImpData1:route-data_get-field-integer( buffer buf_temp-rel-handle:handle, input 'person':U, input "psn-code") .
                        ...
                        if not glog
                        or error-status:error then do:
                                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не удалось получить данные записи <&4>: &1&2&3"                                                       , error-status:get-message(1)                                                       , chr(10)                                                       , return-value                                                       , 'person':U )).
                          undo _rule, retry _rule.
                        end.
                        release buf_temp-person_ .
                      end.
                      when 'fin-schet':U then do:
                          v-current-fin-cli-type = ImpData1:route-data_get-field-character( buffer buf_temp-rel-handle:handle, input 'fin-schet':U, input "cli-type") .
                          v-current-fin-cli-code = ImpData1:route-data_get-field-integer( buffer buf_temp-rel-handle:handle , input 'fin-schet':U, input "cli-code") .
                        v-current-code-bank = ImpData1:route-data_get-field-integer( buffer buf_temp-rel-handle:handle, input 'fin-schet':U,  input "code-bank") .
                        v-current-r-schet = ImpData1:route-data_get-field-character( buffer buf_temp-rel-handle:handle, input 'fin-schet':U,  input "r-schet") .
                        ...
                        if not glog
                        or error-status:error then do:
                                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не удалось получить данные записи <&4>: &1&2&3"                                                       , error-status:get-message(1)                                                       , chr(10)                                                       , return-value                                                       , 'fin-schet':U )).
                          undo _rule, retry _rule.
                        end.
                      ...
                      when 'staff':U
                      THEN do:
                        v-current-staff-psn-code = ImpData1:route-data_get-field-integer( buffer buf_temp-rel-handle:handle, input 'staff':U, input "psn-code") .
                        ...
                        v-current-staff-code = ImpData1:route-data_get-field-integer( buffer buf_temp-rel-handle:handle, input 'staff':U, input "staff-code") .
...
:
  if retry then do:
    ...
    case buf_temp-clients_.obj-type:
      when 'орг':U then do:
        ...
        run ref/firm1.p (
            ...
            ,input "cli-all":U
            ,input yes
            ,input (if v-mode = 'ДОБАВЛЕНИЕ':U
                    then (- abs(buf_temp-clients_.obj-code))
    ...
    if buf_temp-clients_.obj-type = 'орг':U
    ...
    :
      ...
      for each buf_sysconf no-lock:
        ...
        ,input ""
        ,input buf_sysconf.host-code
        ,input (if v-mode = 'ДОБАВЛЕНИЕ':U then 0 else buf_fin-schet.code-schet)
        ,input (if v-mode = 'ДОБАВЛЕНИЕ':U then '' else buf_fin-schet.c-schet)
        ,input buf_temp-fin-schet_.cli-type
        ,input buf_temp-fin-schet_.cli-code
        ,input buf_temp-fin-schet_.code-bank
        ,input buf_temp-fin-schet_.curr-code
        ,INPUT (if v-mode = 'ДОБАВЛЕНИЕ':U then '' else buf_fin-schet.dop1) + (if buf_temp-fin-schet_.cli-code  >= 100000000
                                                                           and buf_temp-fin-schet_.cli-code  < 299999999
                                                                           then (chr(4) + string(buf_temp-fin-schet_.cli-code ))
                                                                           else '')
        ,INPUT (if v-mode = 'ДОБАВЛЕНИЕ':U then '' else buf_fin-schet.dop2)
        ,input buf_temp-fin-schet_.r-schet
        ,input (if v-mode = 'ДОБАВЛЕНИЕ':U then '' else buf_fin-schet.ps)
        )
        no-error.
        if error-status :error then do:
    ...
    if not (buf_temp-clients_.obj-type = 'маг':U or buf_temp-clients_.obj-type = 'скл':U)
    then do:
