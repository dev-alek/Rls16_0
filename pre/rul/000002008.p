...
:
assign
  v-tax-code  = integer('1':U)
.
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по налогам из ВС")).                      assign v-view-log = yes.
  ...
  repeat:
    ...
    :
      if retry then do:
        ...
        .
        IF v-current-tbl-name = 'tax-rate':U  THEN do:
          trans_save:
          do
          on error  undo _rule, retry _rule
          ...
          :
            assign
              v-line-num  = ImpData1:route-data_get-field-integer  ( input 'tax-rate':U, input "line-num":U )
              v-rate-code = ImpData1:route-data_get-field-integer  ( input 'tax-rate':U, input "rate-code":U )
              v-rate-name = ImpData1:route-data_get-field-character( input 'tax-rate':U, input "rate-name":U )
              v-status    = ImpData1:route-data_get-field-character( input 'tax-rate':U, input "status_":U )
            .
            ...
            if error-status:error then do:
                            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении данных по ставке налога при приеме данных из ВС:" )).                      assign v-view-log = yes.
            ...
            if v-status <> 'D' then do:
              ...
              :
                ...
                if error-status:error then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не удалось получить записи &1 для &2&3&4&3&5"                                               , buf_temp-rel-handle.child-buffer_                                               , v-current-tbl-name                                               , chr(10)                                               , error-status:get-message(1)                                                , return-value )).                      assign v-view-log = yes.
                  undo _rule, retry _rule .
                end.
                _child-stroka:
                repeat:
                  ...
                  case buf_temp-rel-handle.child-buffer_:
                    when 'tax-rate-value':U then do:
                      run proc-save-tax-rate-value in this-procedure
                        ( input v-tax-code
                        , input ImpData1:route-data_get-field-integer  ( buffer buf_temp-rel-handle:handle, input 'tax-rate-value':U, input "rate-code":U )
                        , input ImpData1:route-data_get-field-decimal  ( buffer buf_temp-rel-handle:handle, input 'tax-rate-value':U, input "rate-value":U )
                        , input ImpData1:route-data_get-field-date     ( buffer buf_temp-rel-handle:handle, input 'tax-rate-value':U, input "fact-date":U )
                        , input 0
                        , input "":U
                        , input 0
                        , input ImpData1:route-data_get-field-character( buffer buf_temp-rel-handle:handle, input 'tax-rate-value':U, input "status_":U )
                        ) no-error .
                      if error-status:error then do:
                        undo _rule, retry _rule.
                      end.
...
procedure proc-save-tax-rate :
  ...
  do
  on error  undo, return error substitute( "&1 (proc-save-tax-rate). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
  ...
  :
    ...
    case p-status :
      ...
      then do:
        ...
        if error-status:error then do:
          undo, return error substitute( "&1 (proc-save-tax-rate). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1)) .
        end.
      end.
      when 'D' then do:
        if available buf_tax-rate then do:
...
procedure proc-save-tax-rate-value :
  ...
  define input  parameter p-status     as   character                    no-undo .
  do
  on error  undo, return error substitute( "&1 (proc-save-tax-rate-value). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
  ...
  :
    ...
    case p-status :
      ...
      then do:
        ...
        if error-status:error then do:
          undo, return error substitute( "&1 (proc-save-tax-rate-value). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1)) .
        end.
      end.
      when 'D' then do:
        if available buf_tax-rate-value then do:
    ...
    then do:
      run fill-cash-txr in p-cont-handle (
                                          ...
                                          ,input p-obj-code
                                          ,input buf_tax.tax-type
                                          ,input (if p-status = 'D' then ? else p-rate-value)
