...
if not error-status:error then do:
  OPEN QUERY br-dis-gds-rule FOR EACH gds_tt-rule-call-param where              gds_tt-rule-call-param.param-name = "p-rule-nums"          and gds_tt-rule-call-param.rule_id = 1735          and gds_tt-rule-call-param.codex_id = 1          and gds_tt-rule-call-param.ruleset_id = 1          and gds_tt-rule-call-param.p-index > 0,            FIRST X_dis-rule NO-LOCK WHERE X_dis-rule.rule-num = gds_tt-rule-call-param.param-value-integer .
...
with frame Dialog-Frame .
END.
ON CHOOSE OF b-tot-sum-id-sgd IN FRAME Dialog-Frame
DO:
define variable v-rid-list as character no-undo .
define buffer buf_prop-ref for ub.prop-ref.
run ref/proprefs.w (   input  parparentproc
...
b-tot-sum-id-sgd WHEN p-mode <> 'ПРОСМОТР':U
t-is-over WHEN p-mode <> 'ПРОСМОТР':U
b-dis-gds-add WHEN p-mode <> 'ПРОСМОТР':U
b-dis-gds-delete WHEN p-mode <> 'ПРОСМОТР':U
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры Алгоритма СУММА НАКОПЛЕНИЙ по выборке товаров -> КАТЕГОРИЯ СКИДКИ (профайл &1)", p-profile-id).
OPEN QUERY br-dis-gds-rule FOR EACH gds_tt-rule-call-param where              gds_tt-rule-call-param.param-name = "p-rule-nums"          and gds_tt-rule-call-param.rule_id = 1735          and gds_tt-rule-call-param.codex_id = 1          and gds_tt-rule-call-param.ruleset_id = 1          and gds_tt-rule-call-param.p-index > 0,            FIRST X_dis-rule NO-LOCK WHERE X_dis-rule.rule-num = gds_tt-rule-call-param.param-value-integer .
END PROCEDURE.
