...
assign
  v-str-recid4 = trim( string( recid( X_rule-by-profile ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry4 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-rule-by-profile in frame Dialog-Frame.
  end.
...
apply "entry" to br-rule-by-profile in frame Dialog-Frame.
END.
...
DO:
  ...
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
ON CHOOSE OF MENU-ITEM m_profile
DO:
    ASSIGN
  lkp-option = "profile".
  run proc-b-lkp IN THIS-PROCEDURE ( INPUT lkp-option) NO-ERROR.
...
do:
   v-doc-rec = recid(X_rule-by-profile).    RUn OpenBR in this-procedure.  REPOSITION br-rule-by-profile to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-rule-by-profile.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-rule-by-profile :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-rule-by-profile FOR EACH X_rule-by-profile NO-LOCK,        first X_rule NO-LOCK indexed-reposition.
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
CASE p-list-mode:
  WHEN 'все':U THEN DO:
    frame Dialog-Frame:title = "Все правила всех профайлов".
  ...
  END.
  WHEN "ruleset" THEN DO:
    frame Dialog-Frame:title = substitute("Все правила профайлов для кодекса правил &1 набора правил &2"
                                           , p-codex-id
                                           , p-ruleset-id).
    OPEN QUERY br-rule-by-profile
  ...
  END.
  WHEN "codex" THEN DO:
    frame Dialog-Frame:title = substitute("Все правила профайлов для кодекса правил &1"
                                          , p-codex-id).
    OPEN QUERY br-rule-by-profile
    FOR EACH X_rule-by-profile NO-LOCK WHERE
             X_rule-by-profile.codex_id = p-codex-id,
  ...
  END.
  WHEN "profile" THEN DO:
    frame Dialog-Frame:title = substitute("Все правила для профайла &1"
                                          , p-profile-id).
    OPEN QUERY br-rule-by-profile
    FOR EACH X_rule-by-profile NO-LOCK WHERE
  ...
  END.
  WHEN "rule" THEN DO:
    frame Dialog-Frame:title = substitute("Все профайлы для правила &1"
                                          , p-rule-id).
    OPEN QUERY br-rule-by-profile
    FOR EACH X_rule-by-profile NO-LOCK WHERE
...
CASE p-add:
  when yes then do:
    ...
    .
    run rul/rule-by-profile1.p ( input 'ДОБАВЛЕНИЕ':U
                              ,input no
                              ,input-output v-rec
                              ,input p-profile-id
                              ,input buf_ruleset.codex_id
  ...
  when no then do:
    ...
    v-rec = recid(x_rule-by-profile).
    run rul/rule-by-profile1.p ( input 'ИЗМЕНЕНИЕ':U
                              ,input no
                              ,input-output v-rec
                              ,input X_rule-by-profile.profile_id
                              ,input X_rule-by-profile.codex_id
