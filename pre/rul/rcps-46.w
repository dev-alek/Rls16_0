...
THEN RETURN NO-APPLY.
 ...
 THEN DO:
    ...
    END.
 END.
 apply "entry" to f-tpl in FRAME Dialog-Frame.
 return no-apply.
END.
...
VIEW FRAME Dialog-Frame.
ENABLE
f-coeff WHEN p-mode <> 'ПРОСМОТР':U
B-exit WHEN p-mode <> 'ПРОСМОТР':U
b-quit
button-tpl  WHEN p-mode <> 'ПРОСМОТР':U
editor-1
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры создания ДНЦ определенного типа и определенного содержания по алгоритму raimbek (профайл &1)", p-profile-id).
END PROCEDURE.
PROCEDURE proc-save :
assign
frame Dialog-Frame
