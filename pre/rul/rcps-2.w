...
DEFINE VARIABLE f-rule-num AS INTEGER FORMAT ">>>>>>9":U INITIAL 0
     LABEL "Правило зависимости СУММА НАКОПЛЕНИЙ  -> % СКИДКИ по ДК"
     VIEW-AS FILL-IN
...
substitute("Параметры Простого прогрессивного Алгоритма зависимости Сумма накоплений -> % скидки на товар (профайл &1)", p-profile-id).
END PROCEDURE.
PROCEDURE proc-save :
define buffer buf_prop-ref for ub.prop-ref.
define variable v-ii as integer   no-undo .
