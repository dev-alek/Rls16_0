...
:
  ...
  do v-ii = 1 to num-entries(p-esys-id-list, chr(1)):
    ...
    and buf_ext-classif.key#_one = integer(entry(v-ii, p-esys-id-list, chr(1))):
...
if (valid-handle(g#lib-gate) <> true) then do:   run gbl/lib-gate.p persistent no-error .   if error-status :error or (valid-handle(g#lib-gate) <> true) then do:     message       "Error starting gbl/lib-gate.p" skip       g#lib-gate skip       g#lib-gate :type skip       g#lib-gate :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-gate_route-data_ in g#lib-gate
(
   input parparentproc
...
define buffer buf_gds-obj       for ub.gds-obj.
  IF  ExpData1:route-data_push-xmlschema( INPUT p-xsd-file ) = false  THEN do:
        if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при создании гейта &1 для маршрутизации:&2&3&2&4"                                 , p-xsd-file                                 , chr(10)                                 , error-status:get-message(1)                                 , v-last-error-message)).
  ...
  :
    ...
    THEN do:
      IF  context_begin-esys-command( input v-esys-id-list
                ...
                if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при инициации маршрутизации через гейт &1&2:&3&2&4"                                      ,p-xsd-file                                      , chr(10)                                      , error-status:get-message(1)                                       , v-last-error-message)).
        undo _main, return error ''.
      end.
      assign
      v-goods-dump-ord = no
    ...
    if not available buf_goods then do:
            if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найден содержащийся в списке товар с кодом &1", gds-list.gds-code)).
      next _stroka.
    end.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output v-main-b-code
    ...
    assign
      v-action = if buf_goods.stts = 0 then 'U':U else 'D':U
    .
    assign
      v-producer = ""
    .
    ...
    if v-goods-dump-ord =  no then do:
      IF ExpData1:esys-add-dump( INPUT "goods", INPUT v-esys-cmd-proc-handle, INPUT v-esys-cmd-code, '_dump-order=Rec_ID') = false  THEN do:
                if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при маршрутизации записи по товару с кодом  &1:&2&3"                                 , v-current-gds-code                                 , chr(10)                                 , v-last-error-message                                 )).
        undo _main, return error '' .
      end.
    ...
    IF ExpData1:esys-add-dump( INPUT "goods", INPUT v-esys-cmd-proc-handle, INPUT v-esys-cmd-code, '+update') = false  THEN do:
            if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при маршрутизации записи товара с кодом &1:&2&3"                               , v-current-gds-code                               , chr(10)                               , v-last-error-message                               )).
      undo _main, return error '' .
    end.
    _bar-code:
    ...
    :
      ...
      if error-status:error then do:
                if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Неизвестный код признака &1 у баркода &1 товар &3"                                 , buf_bar-code.node-code                                 , buf_bar-code.b-code                                 , buf_bar-code.gds-code                                 )).
      ...
      if v-barcodes-dump-ord = no then do:
        IF ExpData1:esys-add-dump( INPUT "barcodes", INPUT v-esys-cmd-proc-handle, INPUT v-esys-cmd-code, '_dump-order=Rec_ID') = false  THEN do:
                    if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при маршрутизации записи по товару с кодом  &1, баркод &2:&3&4"                                   , v-current-gds-code                                   , buf_bar-code.b-code                                   , chr(10)                                   , v-last-error-message                                   )).
          undo _main, return error '' .
        end.
      ...
      ExpData1:route-data_copy-field-character( INPUT "barcodes", "Action", INPUT  'U':U ) .
      ...
      IF ExpData1:esys-add-dump( INPUT "barcodes", INPUT v-esys-cmd-proc-handle, INPUT v-esys-cmd-code, '+update') = false  THEN do:
                if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при маршрутизации записи баркода/ДопБК с кодом &1:&2&3"                                 , buf_bar-code.b-code                                 , chr(10)                                 , v-last-error-message                                 )).
        undo _main, return error '' .
      end.
      _prod-bc:
      ...
      :
        ...
        .
        if length(buf_prod-bc.b-str) <= 5 then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbcat in g#library
  ...
  ,output l-prod-bc-pgweight
          ...
          if l-prod-bc-weight then do:
            do v-ii = 1 to num-entries(varscales-pref):
              ...
              v-b-str-list = v-b-str-list +
                            (if v-b-str-list = '' then '' else chr(44)) +
      ...
      :
        ...
        or buf_clients.obj-type = 'скл':U then do:
          ...
          and lookup(string(buf_ext-classif.key#_one), v-esys-id-list, chr(1)) > 0 no-error.
          if not available buf_ext-classif then next _clients.
          v-obj-type-code = (if buf_clients.obj-type = 'скл':U then 100000 else 0) + buf_clients.obj-code.
          v-price-sale = ?.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  ...
  ,output for-excise
          ...
          then do:
            ...
            ExpData1:route-data_copy-field-character( INPUT "smprices", "Action", INPUT  'U':U) .
            ...
            ExpData1:route-data_copy-field-character( INPUT "counts", "Action", INPUT  'U':U) .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run calltree in g#library
  ...
  and buf_temp-rule-call-param.param-name = "p-esys-id-list",
    ...
    v-esys-id-list = v-esys-id-list + (if v-esys-id-list = '' then '' else chr(1)) + string(buf_ext-system.esys-id).
  end.
