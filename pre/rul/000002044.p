...
define buffer buf_gds-prt for ub.gds-prt.
  ...
  case p-ruleset-id:
    ...
    when 7 then do:
      ...
      .
      if v-bc-on-type = 'sclc':U
      or v-bc-on-type = 'scgb':U
      or v-bc-on-type = 'pglc':U then do:
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable varscales-pref-type18 as character no-undo.
varscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,output varscales-pref-type18
  ) no-error .
if varscales-pref = ? then do:
  assign
  varscales-pref = '21,23,25':U.
end.
define variable varpgscales-pref-type18 as character no-undo.
varpgscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'scpgpref':u
  ,input  '':u
  ...
  ,output varpgscales-pref-type18
  ) no-error .
...
:
  ...
  case p-ruleset-id:
    when 6 then do:
      if v-has-newbh
      and v-newbh:table <> 'bar-code':U
      and v-newbh:table <> "temp-" + 'bar-code':U
      then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'bar-code':U).
      end.
      if v-changes-list = ''
      and v-has-newbh
      and v-has-oldbh
    ...
    when 7 then do:
      if v-has-newbh
      and v-newbh:table <> 'prod-bc':U
      and v-newbh:table <> "temp-" + 'prod-bc':U
      then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'prod-bc':U).
      end.
      if v-changes-list = ''
      and v-has-newbh
      and v-has-oldbh
