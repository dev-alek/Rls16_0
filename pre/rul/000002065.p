...
report-id
destination-id.
define dataset reportheaderTypet for report-headert, report-parameterst, report-errorst, report-destinationt
data-relation r1 for report-headert, report-parameterst
relation-fields (report-id, report-id) nested
data-relation r2 for report-headert, report-errorst
relation-fields (report-id, report-id) nested
data-relation r3 for report-headert, report-destinationt
relation-fields (report-id, report-id) nested
.
...
define variable p-without-vat as logical no-undo.
define variable p-break-by-cp as logical no-undo .
define variable p-no-covered-techrfsl as logical no-undo .
define variable p-ext-doc-type-subtotals as logical no-undo .
define variable p-output-type as character no-undo .
define variable p-price-type as integer no-undo .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run basecode in g#library
  ...
  FIND LAST  previous-shift-obj SHARE-LOCK WHERE
            previous-shift-obj.obj-type = buf_shift-obj.obj-type
        and previous-shift-obj.obj-code = buf_shift-obj.obj-code
        and ((previous-shift-obj.shift-date = buf_shift-obj.shift-date
              and previous-shift-obj.shift-num < buf_shift-obj.shift-num)
            or previous-shift-obj.shift-date < buf_shift-obj.shift-date)
              use-index pi NO-ERROR.
  if available previous-shift-obj then do:
    assign
    v-previous-shift-date = previous-shift-obj.shift-date
    v-current-shift-date = buf_shift-obj.shift-date
    .
    run rep/chk-ahz.p (
                  input        buf_shift-obj.obj-type
                  ,input       buf_shift-obj.obj-code
    ...
    if error-status:error then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 &2 &3&4Ошибка при вызове программы chk-ahz.p&4&5&4&6"                                    ,vss-workfile                                    ,vss-revision                                    ,vss-description                                    ,chr(10)                                   ,error-status :get-message(1)                                    ,return-value )).
    ...
    if buf_shift-obj.shift-date < v-previous-shift-date
    or buf_shift-obj.shift-date > v-current-shift-date  then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Объект &1&2 Печать отчета за выбранную смену невозможна&3"  +                                   "Отсутствуют подробные складские архивы&3"  +                                   "Возможные даты отчета: &4-&5&3&6&3"                                   ,buf_shift-obj.obj-type                                    ,buf_shift-obj.obj-code                                    , chr(10)                                   ,string(v-previous-shift-date, '99/99/9999':u)                                   ,string(v-current-shift-date, '99/99/9999':u)                                    ,v-comment)).
      run cb_write-report-error in p-parent-handle ( input (buffer report-errorst:handle)
                                                    ,input v-report-id
                                                    ,input ?
                                                    ,input '3':U
                                                    ,input substitute("Объект &1&2 Печать отчета за выбранную смену невозможна&3"  +                                   "Отсутствуют подробные складские архивы&3"  +                                   "Возможные даты отчета: &4-&5&3&6&3"                                   ,buf_shift-obj.obj-type                                    ,buf_shift-obj.obj-code                                    , chr(10)                                   ,string(v-previous-shift-date, '99/99/9999':u)                                   ,string(v-current-shift-date, '99/99/9999':u)                                    ,v-comment)).
      return error ''.
    end.
  end.
  ...
  X-Set_pay_type  = p-price-type
  X-tog-shift = yes
  x-SET_val_TYPE = (if p-r-b = 'base':U
                    then 2
                    else 1)
  x-SelectGood = 1
  .
  ...
  if lookup('excel':U, p-output-type) > 0 then do:
    make-excel = yes.
  ...
  ,input lookup('text':U, p-output-type) > 0
  ,input lookup('excel':U, p-output-type) > 0
  ,input entry(1, v-dirs, chr(124))
  ) no-error.
  if error-status:error then do:
        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("!!Ошибка при выполнении отчета!!! &1&2&1&3"                                 , chr(10)                                 , error-status:get-message(1)                                 , return-value )).
    if lookup('excel':U, p-output-type) > 0 then do:
  ...
  if error-status:error then do:
        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("!!Не удалось передать OUTPUT параметры правила!!! &1&2&1&3"                                 , chr(10)                                 , error-status:get-message(1) )).
  ...
  or not glog
  then do:
        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("!!Не удалось заполнить блок параметров!!! &1"                                 , error-status:get-message(1) )).
    delete object p-reporth no-error.
    return error ''.
  end.
...
and buf_rule-call-param.param-name = "p-without-vat"
 no-error.
if available buf_rule-call-param then do:
assign p-without-vat = buf_rule-call-param.param-value-logical.
end.
  find first buf_rule-call-param no-lock where
buf_rule-call-param.codex_id = p-codex-id
and buf_rule-call-param.ruleset_id = p-ruleset-id
...
and buf_rule-call-param.order_id = p-order-id
and buf_rule-call-param.rule_id = p-rule-id
and buf_rule-call-param.param-name = "p-break-by-cp"
 no-error.
if available buf_rule-call-param then do:
assign p-break-by-cp = buf_rule-call-param.param-value-logical.
end.
  find first buf_rule-call-param no-lock where
buf_rule-call-param.codex_id = p-codex-id
and buf_rule-call-param.ruleset_id = p-ruleset-id
...
and buf_rule-call-param.order_id = p-order-id
and buf_rule-call-param.rule_id = p-rule-id
and buf_rule-call-param.param-name = "p-no-covered-techrfsl"
 no-error.
if available buf_rule-call-param then do:
assign p-no-covered-techrfsl = buf_rule-call-param.param-value-logical.
end.
  find first buf_rule-call-param no-lock where
buf_rule-call-param.codex_id = p-codex-id
and buf_rule-call-param.ruleset_id = p-ruleset-id
...
and buf_rule-call-param.order_id = p-order-id
and buf_rule-call-param.rule_id = p-rule-id
and buf_rule-call-param.param-name = "p-ext-doc-type-subtotals"
 no-error.
if available buf_rule-call-param then do:
assign p-ext-doc-type-subtotals = buf_rule-call-param.param-value-logical.
end.
  find first buf_rule-call-param no-lock where
buf_rule-call-param.codex_id = p-codex-id
and buf_rule-call-param.ruleset_id = p-ruleset-id
...
and buf_rule-call-param.order_id = p-order-id
and buf_rule-call-param.rule_id = p-rule-id
and buf_rule-call-param.param-name = "p-price-type"
 no-error.
if available buf_rule-call-param then do:
assign p-price-type = buf_rule-call-param.param-value-integer.
end.
  find first buf_rule-call-param no-lock where
buf_rule-call-param.codex_id = p-codex-id
and buf_rule-call-param.ruleset_id = p-ruleset-id
