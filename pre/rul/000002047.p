...
define buffer buf_clients for ub.clients.
  ...
  :
    ...
    if not available buf_clients then do:
            if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найден содержащийся в списке контрагент &1&2", cli-list.obj-type, cli-list.obj-code)).
      next _stroka.
    end.
    case buf_clients.obj-type:
      when 'чел':U
      or when 'орг':U then do:
        v-obj-type-code = (if buf_clients.obj-type = 'орг':U
                          then  1000000000
                          else 0) +
        ...
        ExpData1:route-data_copy-field-character( INPUT "clients", "Action", INPUT  'U':U ) .
      ...
      when 'маг':U
      or when 'скл':U then do:
        v-obj-type-code = (if buf_clients.obj-type = 'скл':U
                          then  100000
                          else 0) +
                          buf_clients.obj-code.
        ...
        ExpData1:route-data_copy-field-character( INPUT "stores", "Action", INPUT  'U':U ) .
