...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по банкам из ВС")).
      ...
      _rule:
       do on error undo _rule, retry _rule:
         if retry then do:
          ...
          IF ImpData1:current-tbl-name( ) = 'fin-bank':U  THEN do:
            _tr:
            do
            on error undo _rule, retry _rule
            :
              v-current-code-bank = 0.
              v-line-num = ImpData1:route-data_get-field-integer( input 'fin-bank':U, input "line-num") .
              v-present = no.
              num-rec = num-rec + 1.
              v-current-code-bank = ImpData1:route-data_get-field-integer( input 'fin-bank':U, input "code-bank") .
              v-bank-name = ImpData1:route-data_get-field-character( input 'fin-bank':U, input "bank-name") .
              v-short-name = ImpData1:route-data_get-field-character( input 'fin-bank':U, input "short-name") .
              v-bik = ImpData1:route-data_get-field-character( input 'fin-bank':U, input "bik") .
              v-addres = ImpData1:route-data_get-field-character( input 'fin-bank':U, input "addres") .
              v-bank-city = ImpData1:route-data_get-field-character( input 'fin-bank':U, input "bank-city") .
              ...
              :
                ...
                run ref/finbank1.p (
                                    ...
                                    ,input v-addres
                                    ,input v-bank-city
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.addres1)
                                    ,input v-bank-name
                                    ,input v-bik
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.cor-acc)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.e-mail)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.fax)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.inn)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.kpp)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.licenz)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.okato)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.okonx)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.okpo)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.otdel)
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.phone)
                                    ,input v-ps
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.rkc)
                                    ,input v-short-name
                                    ,input (if v-mode = 'ДОБАВЛЕНИЕ-ИМПОРТ':U then '' else buf_fin-bank.cl-bank)
                                    )
                                    no-error.
              end.
              if error-status:error then do:
