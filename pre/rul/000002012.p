...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по единицам измерения из ВС")).                      assign v-view-log = yes.
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Импорт данных по единицам измерения из файла &1", file-name)).                      assign v-view-log = yes.
    ...
    _rule:
      do on error undo _rule, retry _rule:
        ...
        IF v_qh:query-off-end then leave _stroka.
          IF  ImpData1:current-tbl-name( ) = 'units':U  THEN do:
            trans_save:
            do transaction
            ...
            :
              assign
                v-line-num  = ImpData1:route-data_get-field-integer  ( input 'units':U, input "line-num":U )
                v-unit-name = ImpData1:route-data_get-field-character( input 'units':U, input "unit-name":U)
                v-long-name = ImpData1:route-data_get-field-character( input 'units':U, input "long-name":U)
                v-unit-type = ImpData1:route-data_get-field-character( input 'units':U, input "type":U)
              .
              ...
              if error-status:error then do:
                if return-value <> '' then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка идентификации принимаемых данных по единице измерения из ВС:&1&2", chr(10), return-value )).                      assign v-view-log = yes.
                end.
              ...
              if error-status:error then do:
                if return-value <> '' then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении данных по единице измерения при приеме данных из ВС:&1&2", chr(10), return-value )).                      assign v-view-log = yes.
                end.
                run set-err-type in p-cont-handle
                  ( input v-err-type
                  ) no-error.
...
procedure proc-save-units :
  ...
  do
  on error  undo, return error substitute( "&1 (proc-save-units). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
  ...
  :
    ...
    if error-status:error then do:
      undo, return error substitute( "&1 (proc-save-units). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1)) .
    end.
  end.
end procedure.
