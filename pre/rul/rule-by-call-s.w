...
assign
  v-str-recid8 = trim( string( recid( X_rule-by-call ) , "->>>>>>>>>>>9":U ) )
...
apply "entry" to br-rule-by-call in frame Dialog-Frame.
END.
...
do:
   v-doc-rec = recid(X_rule-by-call).    RUn OpenBR in this-procedure.  REPOSITION br-rule-by-call to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-rule-by-call.
...
CASE p-list-mode:
  WHEN 'все':U THEN DO:
    frame Dialog-Frame :title = "Все вызовы всех правил".
  ...
  WHEN "call_id" THEN DO:
    frame Dialog-Frame :title = substitute("Вызовы правил в точке вызова &1", calldscr( p-call-id)).
    if p-codex-id > 0 then do:
      frame Dialog-Frame :title = substitute("&1: кодекс &2", frame Dialog-Frame :title, p-codex-id).
    end.
    if p-ruleset-id > 0 then do:
      frame Dialog-Frame :title = substitute("&1: набор правил &2", frame Dialog-Frame :title, p-ruleset-id).
    end.
    if p-rule-id > 0 then do:
      frame Dialog-Frame :title = substitute("&1: правило &2", frame Dialog-Frame :title, p-rule-id).
    end.
    OPEN QUERY br-rule-by-call
  ...
  WHEN "ruleset" THEN DO:
    frame Dialog-Frame :title = substitute("Вызовы правил для &1 набора правил &2", p-codex-id, p-ruleset-id).
    OPEN QUERY br-rule-by-call
    FOR EACH X_rule-by-call NO-LOCK WHERE
  ...
  END.
  WHEN "codex" THEN DO:
    frame Dialog-Frame :title = substitute("Вызовы правил для кодекса правил &1", p-codex-id).
    OPEN QUERY br-rule-by-call
    FOR EACH X_rule-by-call NO-LOCK WHERE
             X_rule-by-call.codex_id = p-codex-id
        , FIRST X_ruleset NO-LOCK WHERE
  ...
  END.
  WHEN "rule" THEN DO:
    frame Dialog-Frame :title = substitute("Вызовы правила &1", p-rule-id).
    OPEN QUERY br-rule-by-call
    FOR EACH X_rule-by-call NO-LOCK WHERE
