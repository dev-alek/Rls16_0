...
assign
  v-str-recid4 = trim( string( recid( X_rule-i-script ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry4 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-rule-i-script in frame Dialog-Frame.
  end.
...
apply "entry" to br-rule-i-script in frame Dialog-Frame.
END.
ON CHOOSE OF b-rule-script IN FRAME Dialog-Frame
DO:
  if not available X_rule-i-script then return no-apply.
...
DO:
  ...
  v-longchar = X_rule-i-script.script-name.
   ...
   .
END.
ON CHOOSE OF b-text-2 IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE v-longchar AS LONGCHAR NO-UNDO.
  DEFINE VARIABLE v-ok AS LOGical NO-UNDO.
  DEFINE BUFFER buf_rule-i-script FOR ub.rule-i-script.
...
do:
   v-doc-rec = recid(X_rule-i-script).    RUn OpenBR in this-procedure.  REPOSITION br-rule-i-script to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-rule-i-script.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-rule-i-script :handle
      .
...
b-rule-script:menu-mouse in frame Dialog-Frame = 1
.
ENABLE
b-quit
B-Help
...
CASE p-list-mode:
  WHEN 'все':U THEN DO:
    frame Dialog-Frame :title = "Все привязки скриптов по всем правилам".
  ...
  WHEN "script-name" THEN DO:
    frame Dialog-Frame :title = substitute("Привязка к правилам скрипта &1 &2", p-script-type, p-script-name).
    OPEN QUERY br-rule-i-script
  ...
  END.
  WHEN "rule" THEN DO:
    frame Dialog-Frame :title = substitute("Привязка скриптов к правилу &1", p-rule-id).
    OPEN QUERY br-rule-i-script
    FOR EACH X_rule-i-script NO-LOCK WHERE
