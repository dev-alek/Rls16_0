...
calldscr(X_rp-by-call.call_id) COLUMN-LABEL "Вызов" FORMAT "X(255)" WIDTH 45
...
assign
  v-str-recid9 = trim( string( recid( X_rp-by-call ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry9 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-rp-by-call in frame Dialog-Frame.
  end.
...
apply "entry" to br-rp-by-call in frame Dialog-Frame.
END.
...
DO:
  ...
  END.
END.
ON ROW-DISPLAY OF br-rp-by-call IN frame Dialog-Frame
DO:
...
do:
   v-doc-rec = recid(X_rp-by-call).    RUn OpenBR in this-procedure.  REPOSITION br-rp-by-call to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-rp-by-call.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-rp-by-call :handle
      .
...
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  run gbl/dftempl.p ( input 'rule-by-profile':U, output template-recid) no-error.
...
ASSIGN
v-ch = br-rp-by-call:FIRST-COLUMN IN FRAME Dialog-Frame.
REPEAT WHILE valid-handle(v-ch):
   IF v-ch:LABEL = "Вызов" THEN do:
     v-ch:resizable  = yes.
...
CASE p-list-mode:
  WHEN 'все':U THEN DO:
    frame Dialog-Frame :title = "Все профайлы по всем типам вызовов".
  ...
  END.
  WHEN "profile-id" THEN DO:
    frame Dialog-Frame :title = substitute("Вызовы для профайла правил &1", p-profile-id).
    OPEN QUERY br-rp-by-call
  ...
  END.
  WHEN "profile-type" THEN DO:
    frame Dialog-Frame :title = substitute("Вызовы для профайла правил типа &1", p-profile-type).
    OPEN QUERY br-rp-by-call
    FOR EACH X_rp-by-call NO-LOCK WHERE
        , FIRST X_rule-profile NO-LOCK WHERE
  ...
  END.
  WHEN "call-id" THEN DO:
    frame Dialog-Frame :title = substitute("Профайлы для &1", calldscr  ( input p-call-id)).
    OPEN QUERY br-rp-by-call
...
if p-parent-profile-id > 0 then do:
  assign
  X_rule-profile.name:BGCOLOR IN BROWSE br-rp-by-call = GRAY_COLOR
    .
end.
else do:
  assign
  X_rule-profile.name:BGCOLOR IN BROWSE br-rp-by-call = ?
    .
end.
END PROCEDURE.
