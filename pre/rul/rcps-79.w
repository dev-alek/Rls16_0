...
X_dis-rule.des COLUMN-LABEL "Описание" FORMAT "X(255)" width 50
    WITH NO-ROW-MARKERS SEPARATORS SIZE 98 BY 13
         TITLE "Правила зависимости СУММА НАКОПЛЕНИЙ -> категория скидки на ДК по фирмам и объектам" FIT-LAST-COLUMN.
...
DO:
  ...
  run resort-rule-call-param in this-procedure( input v-param-name
                                                ,input v-rule-id).
  OPEN QUERY BR-dis-rule     FOR EACH dr-obj         ,                FIRST X_dis-rule no-lock WHERE         X_dis-rule.rule-num = dr-obj.param-value-integer BY X_dis-rule.host-code BY X_dis-rule.obj-type BY X_dis-rule.obj-code .
...
substitute("Параметры прогрессивного Алгоритма зависимости Сумма накоплений -> категория скидки по фирме/объекту (профайл &1)", p-profile-id).
OPEN QUERY BR-dis-rule     FOR EACH dr-obj         ,                FIRST X_dis-rule no-lock WHERE         X_dis-rule.rule-num = dr-obj.param-value-integer BY X_dis-rule.host-code BY X_dis-rule.obj-type BY X_dis-rule.obj-code .
END PROCEDURE.
