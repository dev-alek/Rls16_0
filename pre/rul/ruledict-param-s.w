...
define buffer buf_tt-ruledict-param for tt-ruledict-param.
 ...
 if p-list-mode = 'ИЗМЕНЕНИЕ':U then do:
...
DO:
 ...
 if p-list-mode = 'ИЗМЕНЕНИЕ':U then do:
  ...
  run rul/ruledict-param-i.w ( input parparentproc
                       ...
                       ,input 'ДОБАВЛЕНИЕ':U
                       ,input p-entry-id
                       ,input (if p-list-mode = 'ИЗМЕНЕНИЕ':U
                               then p-entry-type
                               else '':U)
                       ,input '':U
                       ,input 0
...
DO:
  ...
  if p-list-mode = 'ИЗМЕНЕНИЕ':U then do:
    ...
    view-as alert-box question buttons yes-no update glog.
    if not glog then return no-apply.
    run rul/ruledict-param3.p ( input no
                        ,input v-rec
                        ) no-error.
 end.
...
assign
  v-str-recid6 = trim( string( recid( tt-ruledict-param ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry6 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-tt-ruledict-param in frame Dialog-Frame.
    end.
...
assign
  v-str-recid8 = trim( string( recid( X_ruledict-param ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry8 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-ruledict-param in frame Dialog-Frame.
    end.
    if num-entries( v-rid-list ) = 0
    then
        hide mark-num in frame Dialog-Frame.
...
apply "entry" to br-ruledict-param in frame Dialog-Frame.
END.
...
IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
ON ROW-DISPLAY OF br-ruledict-param IN frame Dialog-Frame
DO:
if p-list-mode = 'ИЗМЕНЕНИЕ':U then do:
...
do:
  RUN brwrefre IN THIS-PROCEDURE NO-ERROR.
    apply "VALUE-CHANGED" to br-ruledict-param.
end.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-ruledict-param :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ruledict-param FOR EACH X_ruledict-param.    OPEN QUERY br-tt-ruledict-param FOR EACH tt-ruledict-param.
END PROCEDURE.
...
ENABLE
b-quit
b-exit when (p-list-mode = 'ИЗМЕНЕНИЕ':U)
...
br-ruledict-param when p-list-mode <> 'ИЗМЕНЕНИЕ':U
br-tt-ruledict-param when p-list-mode = 'ИЗМЕНЕНИЕ':U
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
if p-list-mode = 'ИЗМЕНЕНИЕ':U then do:
  hide
...
CASE p-list-mode:
  WHEN 'все':U THEN DO:
    frame Dialog-Frame :title = "Все параметры терминов словаря RULE-машины".
    OPEN QUERY br-ruledict-param
    FOR EACH X_ruledict-param NO-LOCK INDEXED-REPOSITION.
  END.
  WHEN "entry-id" THEN DO:
    frame Dialog-Frame :title = substitute("Все параметры для термина RULE-машины &1", p-entry-id).
    OPEN QUERY br-ruledict-param
    ...
    case buf_ruledict.entry-type :
      ...
      when 'rule-profile':U then do:
        ...
        run ref/rulercps.w ( INPUT parparentproc
                            ...
                            ,input ?
                            ,input 0
                            ,input substitute("Параметры вызова профайла &1 для параметра &2", v-profile-id, IF p-list-mode = 'ИЗМЕНЕНИЕ':U
