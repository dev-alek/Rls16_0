...
procedure clear_tt :
  do
  on error  undo, return error substitute( "&1 (clear-tt). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( 1 ) )
...
define variable v-pay-type-code like ub.pay-type.obj-code no-undo .
define variable v-pay-type-name like ub.pay-type.obj-name no-undo .
...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по видам оплат из ВС")).                      assign v-view-log = yes.
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Импорт данных по видам оплат из файла &1", file-name)).                      assign v-view-log = yes.
    find first buf_ext-system no-lock where
              buf_ext-system.esys-id = p-esys-id
          and buf_ext-system.db-num = 0 no-error.
    if not available buf_ext-system
    or not (buf_ext-system.esys-type  = integer('3':U)) then do:
           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найдена ВС &1 или она не имеет типа ORACLE RETAIL", p-esys-id)).                      assign v-view-log = yes.
  ...
  or
  not glog then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при попытке получить записи &1&2&3&2&4"                                                                 , buf_temp-xml-tables.tbl-name                                                                 , chr(10)                                                                 , error-status:get-message(1)                                                                 , return-value)).                      assign v-view-log = yes.
   undo _main, return error ''.
  end.
  ...
  REPEAT:
    ...
    do on error undo, retry:
      if  retry then do:
        v-retry-action = v-retry-action + 1.
                        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при импорте записи &5 &1&2&3&2&4"                                                                     , buf_temp-xml-tables.tbl-name                                                                     , num-rec                                                                     , chr(10)                                                                     , error-status:get-message(1)                                                                     , return-value)).                      assign v-view-log = yes.
            if valid-handle(v_qh) then do:
              delete object v_qh no-error.
            end.
        undo _main, return error ''.
    ...
    _rule:
      do on error undo _rule, retry _rule:
        if retry then do:
                      run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1&2&3", error-status:get-message(1), chr(10), return-value)).                      assign v-view-log = yes.
          if valid-handle(v_qh) then do:
            delete object v_qh no-error.
          end.
          undo _main, return error ''.
        ...
        IF v_qh:query-off-end then leave _stroka.
          IF  ImpData1:current-tbl-name( ) = 'pay-type':U  THEN do:
            trans_save:
            do transaction
            ...
            :
              if retry then do:
                if return-value <> '' then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка разбора/приема данных по ставке налога из ВС:&1&2&1&3", chr(10), return-value, error-status :get-message( 1 ) )).                      assign v-view-log = yes.
                end.
              ...
              assign
                v-line-num      = ImpData1:route-data_get-field-integer  ( input 'pay-type':U, input "line-num":U )
                v-pay-type-code = ImpData1:route-data_get-field-integer  ( input 'pay-type':U, input "obj-code":U)
                v-pay-type-name = ImpData1:route-data_get-field-character( input 'pay-type':U, input "obj-name":U)
              .
              run get-xcnf_check-imp-rec in p-cont-handle
                ...
                , input g#db-num
                , input v-pck-num
                , input substitute( "&1,&2", v-line-num, v-pay-type-code )
                , output v-present
                ) no-error.
              if error-status:error then do:
                if return-value <> '' then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка идентификации принимаемых данных по ставке налога из ВС:&1&2", chr(10), return-value )).                      assign v-view-log = yes.
                end.
                run set-err-type in p-cont-handle
                  ( input v-err-type
                  ) no-error.
              ...
              run proc-save-pay-type in this-procedure
                ( input v-pay-type-code
                , input v-pay-type-name
                ) no-error .
              if error-status:error then do:
                if return-value <> '' then do:
                                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении данных по ставке налога при приеме данных из ВС:&1&2", chr(10), return-value )).                      assign v-view-log = yes.
                end.
                run set-err-type in p-cont-handle
                  ( input v-err-type
                  ) no-error.
    ...
    do on error undo, retry:
      if  retry then do:
        v-retry-action = v-retry-action + 1.
                  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1&2&3", error-status:get-message(1), chr(10), v-last-error-message )).                      assign v-view-log = yes.
        undo _main, return error ''.
      end.
      if v-retry-action < 1 then do:
            ImpData1:Route-data_dump ( ) .
    ...
    if valid-handle(v_qh) then do:
      ...
      run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Прочитано записей: &1, из них удачно обработано: &2", num-rec, num-rec-ok2)).                      assign v-view-log = yes.
end.
end procedure.
...
assign p-esys-id = buf_rule-call-param.param-value-integer.
    ...
    case p-ruleset-id:
      when 4 then do:
        ...
        .
        if error-status:error then do:
                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибки параметров&1&2 ..."                                         , chr(10)                                         , error-status:get-message(1) )).                      assign v-view-log = yes.
        ...
        or lookup(v-extension, "dat") = 0 then do:
                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Файл &1 имеет недопустимое расширение &3&2пропускаем ..."                                         ,file-name                                         , chr(10)                                         , v-extension                                         )).                      assign v-view-log = yes.
          undo, return error substitute("Файл &1 имеет недопустимое расширение &3&2пропускаем ..."                                         ,file-name                                         , chr(10)                                         , v-extension                                         ).
        end.
        _top-buffers:
...
procedure delete-procedure :
  do
  on error  undo, return error substitute( "&1 (delete-procedure). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( 1 ) )
...
procedure proc-save-pay-type :
  ...
  do
  on error  undo, return error substitute( "&1 (proc-save-pay-type). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
  on stop   undo, return error substitute( "&1 (proc-save-pay-type). stop", vss-workfile )
  on endkey undo, return error substitute( "&1 (proc-save-pay-type). endkey", vss-workfile )
  :
    define buffer buf_pay-type for ub.pay-type .
    define variable v-rid  as recid     no-undo .
    define variable v-mode as character no-undo .
    find first buf_pay-type exclusive-lock
      where buf_pay-type.obj-code = p-pay-type-code
      no-error .
    if available buf_pay-type then do:
      assign
        v-mode = 'ИЗМЕНЕНИЕ':U
        v-rid  = recid( buf_pay-type )
      .
    end.
    else do:
      assign
    ...
    run ref/paytype1.p
      ...
      , input v-mode
      , input true
      , input p-pay-type-code
      , input p-pay-type-name
      ) no-error.
    if error-status:error then do:
      undo, return error substitute( "&1 (proc-save-pay-type). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1)) .
    end.
  end.
end procedure.
