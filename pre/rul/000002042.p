...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action12   as character no-undo .
  define variable v-printed12       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При работе произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action12
    ,output v-printed12
    ) .
end.
if v-view-log = true
...
:
  ...
  then do:
    run write-log  in p-log-handle (
                                    input 0
                                  , "&DLine").
        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Циклические заказы")).
  end.
_obj:
