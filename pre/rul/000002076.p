...
then do:
  ...
  if v-es then do:
      run garbcoll_clear in this-procedure .
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))).
...
:
  ...
  if not available ord-list then return "return".
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Маршрутизация согласованных заказов поставщику")).
  _stroka:
  for each ord-list
  break
  ...
  :
    if retry then do:
      ...
      then do:
        ...
        if ord-list.ord-int1 =  integer('1':U)
        ...
        or ord-list.ord-int1 =  integer('0':U)
        then do:
          next _stroka.
        end.
        if ord-list.doc-type = 'ОП':U
        and ord-list.status_  = 'новый':U
        and (ord-list.ord-int1 = integer('4':U)
             or
        ...
        if v-ord-int1 = integer('3':U)
        or v-ord-int1 = integer('5':U)
        then do:
      ...
      assign
      v-cli-out-time-chr =  (if num-entries(buf_ord-doc.cli-out-doc, chr(4)) > 2
                          then entry(3, buf_ord-doc.cli-out-doc, chr(4))
                          else string(0, "HH:MM"))
      v-cli-out-date-chr = (if num-entries(buf_ord-doc.cli-out-doc, chr(4)) > 1
                            then entry(2, buf_ord-doc.cli-out-doc, chr(4))
      ...
      create ordrsp-t.
      assign
      ordrsp-t.NUMBER       = entry(1, buf_ord-doc.cli-out-doc, chr(4))
      ...
      ordrsp-t.CURRENCY     = string(entry(2, buf_currency.curr-eng-name, chr(4)) , "X(3)")
      ordrsp-t.ACTION       = (if ord-list.ord-int1 = integer('5':U)
                                then 29
                                else 5)
      .
      ...
      :
        ...
        if available position-t then do:
          ...
          create position-t.
          assign
            position-t.NUMBER            = entry(1, buf_ord-doc.cli-out-doc, chr(4))
            ...
            position-t.INFO              = entry(1, buf_ord-line.sub-par, chr(4))
          .
