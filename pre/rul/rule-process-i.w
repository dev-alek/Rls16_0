...
run rul/ruleset-s.w ( INPUT parparentproc
                     ,INPUT 'b-sel':U
                     ,input "profile-type" + chr(4) + tt-rule-process.pchain-type + chr(4) + "all"
                     ,input 0
...
END.
END.
ON VALUE-CHANGED OF tt-rule-process.pchain-type IN FRAME Dialog-Frame
DO:
  assign
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-rule-process SHARE-LOCK.
...
PROCEDURE MyEnable :
ASSIGN
tt-rule-process.pchain-type:LIST-ITEMS IN FRAME Dialog-Frame = 'dis-card-type,goods,clients,gds-grp,cli-grp,chk-doc_IBS-TH,chk-doc_IBS-TH-MOB,edoc,thref,pdf,rep,ord,cmb,fdoc':U .
assign
...
b-quit
B-Help
tt-rule-process.pchain-type  when p-mode = 'ДОБАВЛЕНИЕ':U
tt-rule-process.pchain-id when p-mode = 'ДОБАВЛЕНИЕ':U
tt-rule-process.start-from      when p-mode = 'ДОБАВЛЕНИЕ':U
tt-rule-process.link-id when p-mode = 'ДОБАВЛЕНИЕ':U
tt-rule-process.documentation when p-mode <> 'ПРОСМОТР':U
b-codex when p-mode <> 'ПРОСМОТР':U
t-run-db0 when p-mode <> 'ПРОСМОТР':U
t-run-rdb when p-mode <> 'ПРОСМОТР':U
t-link-btwn-profiles when p-mode <> 'ПРОСМОТР':U
t-is-export when p-mode <> 'ПРОСМОТР':U
t-is-import when p-mode <> 'ПРОСМОТР':U
t-needs-efile when p-mode <> 'ПРОСМОТР':U
t-needs-ifile when p-mode <> 'ПРОСМОТР':U
t-is-routing when p-mode <> 'ПРОСМОТР':U
t-is-esys-import when p-mode <> 'ПРОСМОТР':U
t-main-link when p-mode <> 'ПРОСМОТР':U
t-can-be-start when p-mode <> 'ПРОСМОТР':U
WITH FRAME Dialog-Frame .
