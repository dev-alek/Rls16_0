...
:
  ...
  if not (v-doc-status-old = 'факт':U and not v-has-newbh) and (v-doc-status = v-doc-status-old or not v-doc-status = 'факт':U)
    then return.
...
IF ExpData1:esys-add-dump-data ( INPUT expObj:Data,
  ...
  if v-dump-ord-int64 = 0 THEN do:
    ...
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "Успешно. " + subDocObj:Msg)).
...
:
  ...
  case p-ruleset-id:
    when 155 then do:
      if v-has-newbh
      and v-newbh:table <> 'fbr-doc':U then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'trn-doc':U).
      end.
      if v-has-oldbh
      and v-oldbh:table <> 'fbr-doc':U and v-oldbh:TABLE <> "tt-fbr" then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'trn-doc':U).
      end.
    end.
    otherwise do:
