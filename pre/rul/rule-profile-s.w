...
DO:
  IF NOT AVAILABLE X_rule-profile THEN RETURN NO-APPLY.
  if X_rule-profile.profile-type <> 'trn-doc':U
  and entry(1, X_rule-profile.profile-type, "_") <> 'chk-doc':U
  then do:
    message
...
assign
  v-str-recid6 = trim( string( recid( X_rule-profile ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry6 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-rule-profile in frame Dialog-Frame.
  end.
...
apply "entry" to br-rule-profile in frame Dialog-Frame.
END.
...
DO:
  ...
  and X_rule-profile.profile-type = 'cmb':U
  THEN DO:
    assign
  ...
  END.
END.
ON CHOOSE OF MENU-ITEM m_rp-by-call
DO:
    ASSIGN
  link-option = 'rp-by-call':U.
  RUN proc-b-link IN THIS-PROCEDURE ( INPUT link-option) NO-ERROR.
  IF ERROR-STATUS:ERROR THEN DO:
      link-option = '':U.
      RETURN NO-APPLY.
...
DO:
  ...
  link-option = '':U.
END.
ON CHOOSE OF MENU-ITEM m_term-rule-profile
DO:
  ASSIGN
  link-option = 'rule-profile':U.
  RUN proc-b-link IN THIS-PROCEDURE ( INPUT link-option) NO-ERROR.
  IF ERROR-STATUS:ERROR THEN DO:
      link-option = '':U.
      RETURN NO-APPLY.
...
DO:
  ...
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
ON CHOOSE OF MENU-ITEM m_ruleproc_graph
DO:
   ASSIGN
  ...
  lkp-option = '':U.
END.
ON CHOOSE OF MENU-ITEM m_ruleproc_text
DO:
   ASSIGN
  ...
  lkp-option = '':U.
END.
ON ROW-DISPLAY OF br-rule-profile IN frame Dialog-Frame
DO:
...
do:
   v-doc-rec = recid(X_rule-profile).    RUn OpenBR in this-procedure.  REPOSITION br-rule-profile to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-rule-profile.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-rule-profile :handle
      .
...
if p-list-mode = "general-view" then do:
  case p-general-view:
    when 'dis-card-type':U then do:
      assign
      frame Dialog-Frame :title = substitute("Профайлы работы с ДК").
    end.
    when 'chk-doc':U then do:
      assign
      frame Dialog-Frame :title = substitute("Профайлы для расчета скидок и бонусов на POS IBS TH").
    end.
  end case.
end.
ENABLE
...
if p-list-mode = "term" then do:
  assign
  frame Dialog-Frame:title = substitute("Профайлы, подчиненные профайлу &1", p-general-view).
end.
if lookup("b-add", bttns) = 0 then do:
  hide
  b-cmp in frame Dialog-Frame .
end.
run Openbr in this-procedure .
END PROCEDURE.
PROCEDURE Openbr :
...
CASE p-list-mode:
   when 'все':U then do:
      frame Dialog-Frame :title = "Все профайлы".
      OPEN QUERY br-rule-profile
      FOR EACH X_rule-profile NO-LOCK INDEXED-REPOSITION.
   end.
   when "general-view" then do:
      frame Dialog-Frame :title = substitute("Профайлы типа &1", p-general-view).
      OPEN QUERY br-rule-profile
   ...
   when "term" then do:
      ...
      for each buf_profile-by-profile no-lock where
        ...
        v-term-rp-list = v-term-rp-list + (if v-term-rp-list = '' then '' else chr(44)) + string(buf_profile-by-profile.child-profile_id).
      end.
      OPEN QUERY br-rule-profile
      FOR EACH X_rule-profile NO-LOCK where
...
CASE p-option:
  WHEN 'rule-profile':U THEN DO:
    run rul/rule-profile-s.w ( INPUT parparentproc
                              ...
                              ,INPUT "term"
                              ,INPUT string(X_rule-profile.profile_id)
  ...
  END.
   WHEN 'rule-call-param':U THEN DO:
    ...
    run ref/rulercps.w ( INPUT parparentproc
                        ...
                        ,INPUT "":U
                        ,input 'ПРОСМОТР':U
                        ,input 'rp-rule-param':U + chr(44) + 'все':U
                        ,input X_rule-profile.profile_id
                        ,input ?
                        ,input '':U
                        ,input 0
  ...
  when 'ДОБАВЛЕНИЕ':U then do:
    ...
    if v-rid-list <> '' then do:
        find first buf_rule-profile no-lock where
            ...
            run rul/profile-by-profile1.p ( input 'ДОБАВЛЕНИЕ':U
                                  ,input no
                                  ,input-output v-rec
...
if p-parent-feature = integer('1':U) then do:
  assign
  X_rule-profile.name:BGCOLOR IN BROWSE br-rule-profile = GRAY_COLOR
    .
end.
else do:
  assign
  X_rule-profile.name:BGCOLOR IN BROWSE br-rule-profile = ?
    .
end.
END PROCEDURE.
