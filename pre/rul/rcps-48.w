...
do v-ii = 1 to num-entries('day,week,month,year,hour,hour-last,shift,shift-last,yesterday,week-last,month-last,year-last,halfyear,halfyear-last,quarter,quarter-last':U):
  v-period-type = entry(v-ii, 'day,week,month,year,hour,hour-last,shift,shift-last,yesterday,week-last,month-last,year-last,halfyear,halfyear-last,quarter,quarter-last':U).
  if not (v-period-type = 'yesterday':U
         or
         v-period-type = 'week-last':U
         or
         v-period-type = 'month-last':U)
  then next.
  v-jj = v-jj + 1.
  assign
  v-list = v-list + (if v-jj = 1 then '' else chr(44)) +
                               entry (lookup (v-period-type, 'day,week,month,year,hour,hour-last,shift,shift-last,yesterday,week-last,month-last,year-last,halfyear,halfyear-last,quarter,quarter-last':U) + 1, ',' + 'День,Неделя,Месяц,Год,Час (текущий),Час (прошлый),Смена,Смена (прошлая),Вчера,Неделя (прошлая),Месяц (прошлый),Год (прошлый),Полугодие,Полугодие (прошлое),Квартал,Квартал (прошлый)':U) + chr(44) + v-period-type .
end.
rs-period-type:radio-buttons in frame Dialog-Frame = v-list.
selectobject:RADIO-BUTTONS IN FRAME Dialog-Frame =
    'Все' + chr(44) + string(0) + chr(44) +
    'Фирма' + chr(44) + string(1) + chr(44) +
    'Выборочно' + chr(44) + string(-1).
f-host-code = v-host-code.
if can-find(first obj-list) then do:
  selectobject = integer(string(-1)).
end.
else do:
  if v-host-code = 0 then do:
    selectobject =integer( string(0)).
...
b-quit
selectobject WHEN p-mode <> 'ПРОСМОТР':U
rs-period-type  WHEN p-mode <> 'ПРОСМОТР':U
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры вызова правил: &1", p-title).
RUN rcps_OpenBr in THIS-PROCEDURE.
END PROCEDURE.
PROCEDURE proc-save :
define variable v-ii as integer   no-undo .
    ...
    for each obj-list :
      if p-save-objects then do:
        ...
        if available buf_shop then do:
          v-rid-list = v-rid-list + (if v-rid-list = '' then '' else chr(44)) + string(recID(BUF_SHOP)).
        end.
      end.
      delete obj-list.
    end.
...
END PROCEDURE.
PROCEDURE val-obj :
If SelectObject:SCREEN-VALUE IN FRAME Dialog-Frame = STRING(string(-1))
 THEN  do:
  enable
  BUTTON-obj
  with frame Dialog-Frame .
...
IN FRAME Dialog-Frame =  string(string(1))
then  do:
  view
  f-host-code
  in frame Dialog-Frame .
