...
:
  ...
  find first ub.ord-rcv-attr no-lock
       where ub.ord-rcv-attr.doc-code = p-doc-code
         and ub.ord-rcv-attr.attr-code = 'nids':U
  ...
  :
    ...
    if v-prc-diff <> 0 and lookup('вес':U, buf_units.type) > 0 then do:
      if ( buf_ord-line.cli-qnty  * ( 100 + v-prc-diff ) / 100 ) < (loc-sum-rcv + buf_lineItem.despatchedQuantity) then do:
        ...
        v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, v-mess).
        v-found = yes.
        v-fatal = yes.
      end.
    end.
    ...
    if buf_ord-line.cli-qnty   <  loc-sum-rcv  + buf_lineItem.despatchedQuantity  then do:
      v-fatal = yes.
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'qnty-down':U, string(loc-sum-rcv + buf_lineItem.despatchedQuantity) + chr(4) + string(buf_ord-line.cli-qnty) ).
    end.
    if buf_ord-line.cli-qnty   > buf_lineItem.despatchedQuantity  then do:
      v-psq = yes.
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'qnty-up':U, string(buf_lineItem.despatchedQuantity) + chr(4) + string(buf_ord-line.cli-qnty) ).
    end.
    if buf_ord-line.price-cli < buf_lineItem.netPriceWithVAT then do:
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'price-down':U, string(buf_lineItem.netPriceWithVAT) + chr(4) + string(buf_ord-line.price-cli) ).
    end.
    if buf_ord-line.price-cli > buf_lineItem.netPriceWithVAT then do:
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'price-up':U, string(buf_lineItem.netPriceWithVAT) + chr(4) + string(buf_ord-line.price-cli) ).
    end.
    if v-edist-mess <> '' then do:
      run create-edi-statett in this-procedure (
                                            ...
                                            , input buf_ord-doc.cli-code
                                            , input 'ИЗМЕНЕНИЕ':U
                                            , input (if v-found or v-found-unit-cli then -1 else 0)
                                            , input (if v-fatal
                                                     then integer('4':U)
                                                     else integer('0':U))
                                            , input v-edist-mess
  ...
  if v-found-prc then do:
    v-edist-mess = ''.
    v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'info':U, "По одному или нескольким товарам изменение цены превышает допустимые границы").
  end.
  if v-found then do:
    v-edist-mess = ''.
    v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'info':U, "Превышение заказанного кол-ва").
  end.
  if v-found-prc and v-found then do :
    v-edist-mess = ''.
    v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'info':U, "По одному или нескольким товарам изменение цены превышает допустимые границы; Превышение заказанного количества").
  end.
...
run proc-ord-code in this-procedure
 (input   'main' ,
  ...
  output  loc-rcv-num
      ...
      buf_ord-doc-rcv.transport-cli-type = (if substring(p-transport-cli-type-code, 1, 3) = 'орг':U
                                            or substring(p-transport-cli-type-code, 1, 3) = 'чел':U
                                            then substring(p-transport-cli-type-code, 1, 3)
                                            else '')
      buf_ord-doc-rcv.transport-cli-code = (if substring(p-transport-cli-type-code, 1, 3) = 'орг':U
                                            or substring(p-transport-cli-type-code, 1, 3) = 'чел':U
  ...
  buf_ord-doc.status_ = (if buf_ord-doc.status_ = 'новый':U
                        then 'поставка':U
  ...
  DEFINE VARIABLE v-time as integer no-undo .
  v-date = ?.
  v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, v-ps).
  v-edist-mess2 = "".
  v-edist-mess2 = cr-edist_add-edist-mess( v-edist-mess2, 'pack-num':U, "->" + p-pack-num-chr ).
  ...
  find first ub.ord-rcv-attr exclusive-lock
       where ub.ord-rcv-attr.doc-code = buf_ord-doc-rcv.doc-code
         ...
         and ub.ord-rcv-attr.attr-code = 'nids':U no-error.
  if not available ub.ord-rcv-attr then do :
      if v-desadv-DELIVERYNOTENUMBER <> ? AND v-desadv-DELIVERYNOTENUMBER <> "" then do:
        ...
        assign
          ...
          ub.ord-rcv-attr.attr-code = 'nids':U
        .
  ...
  find first ub.ord-rcv-attr exclusive-lock
       where ub.ord-rcv-attr.doc-code = buf_ord-doc-rcv.doc-code
         and ub.ord-rcv-attr.rcv-code = buf_ord-doc-rcv.rcv-code
         and ub.ord-rcv-attr.attr-code = 'dids':U no-error.
  if not available ub.ord-rcv-attr then do :
      if v-desadv-DELIVERYNOTEDATE <> ? then do:
        ...
        assign
          ub.ord-rcv-attr.doc-code = buf_ord-doc-rcv.doc-code
          ub.ord-rcv-attr.rcv-code = buf_ord-doc-rcv.rcv-code
          ub.ord-rcv-attr.attr-code = 'dids':U
        .
  ...
  find first ub.ord-rcv-attr exclusive-lock
       where ub.ord-rcv-attr.doc-code = buf_ord-doc-rcv.doc-code
         and ub.ord-rcv-attr.rcv-code = buf_ord-doc-rcv.rcv-code
         and ub.ord-rcv-attr.attr-code = 'invoiceNumber':U no-error.
  if not available ub.ord-rcv-attr then do :
      if v-desadv-invoiceNUMBER <> ? AND v-desadv-invoiceNUMBER <> "" then do:
        ...
        assign
          ub.ord-rcv-attr.doc-code = buf_ord-doc-rcv.doc-code
          ub.ord-rcv-attr.rcv-code = buf_ord-doc-rcv.rcv-code
          ub.ord-rcv-attr.attr-code = 'invoiceNumber':U
        .
  ...
  find first ub.ord-rcv-attr exclusive-lock
       where ub.ord-rcv-attr.doc-code = buf_ord-doc-rcv.doc-code
         and ub.ord-rcv-attr.rcv-code = buf_ord-doc-rcv.rcv-code
         and ub.ord-rcv-attr.attr-code = 'invoiceDate':U no-error.
  if not available ub.ord-rcv-attr then do :
      if v-desadv-invoiceDATE <> ? then do:
        ...
        assign
          ub.ord-rcv-attr.doc-code = buf_ord-doc-rcv.doc-code
          ub.ord-rcv-attr.rcv-code = buf_ord-doc-rcv.rcv-code
          ub.ord-rcv-attr.attr-code = 'invoiceDate':U
        .
...
:
  if retry then do:
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при импорте поставок по  заказу &1 из ВС &2&3&4"                                 ,v-current-doc-code                                 , v-esys-id                                  , chr(10)                                 , v-mess)).                      assign v-view-log = yes.
    assign
    v-view-log = yes.
...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт поставок из ВС")).                      assign v-view-log = yes.
    find first desadv.
    num-rec = num-rec + 1.
    find first buf_ord-doc exclusive-lock where
    ...
    if available ub.ord-rcv-attr then do :
        ...
        if available buf_ord-doc-rcv then do :
            ...
            if available buf_ord-chain then do :
                ...
                if available buf_trn-doc then do :
                    v-edist-mess = ''.
                    v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, "Поставка уже в работе" ).
                    v-date-status = ?.
                    run create-edi-statett in this-procedure (
                                                          input 'ord-doc':U
                                                        , input buf_ord-doc.doc-code
                    ...
                    run send-stat_contour ( input "DESADV"
                                           ...
                                           ,input "Сообщение отклонено на стороне получателя"
                                           ,input desadv.id) .
                                        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при импорте поставки по заказу &1 из ВС &2&3Поставка уже в работе"                                             ,v-current-doc-code                                             , v-esys-id                                              , chr(10))).                      assign v-view-log = yes.
                    assign
                    v-view-log = yes.
                    return substitute("Ошибка при импорте поставки по заказу &1 из ВС &2&3Поставка уже в работе"                                             ,v-current-doc-code                                             , v-esys-id                                              , chr(10)).
                end.
            end.
        end.
    end.
    ...
    run edocsord_edi-import in this-procedure (
                                            buffer buf_ord-doc
                                          ,input buf_ord-doc.cli-out-doc
                                          ,input desadv.desadvNumber + chr(4) +
                                                  string(date(substring(desadv.desadvDate, 9, 2) + "/" + substring(desadv.desadvDate, 6, 2) + "/" + substring(desadv.desadvDate, 1, 4)), "99/99/9999") +
                                                  (if v-transport-type-code then (chr(4) + substitute("&1&2", v-transport-cli-type,
    ...
    if error-status:error then do:
      ...
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ediiterchangeid':U, desadv.id ).
      v-date-status = ?.
      run create-edi-state in this-procedure (
                                              input 'ord-doc':U
                                            , input buf_ord-doc.doc-code
      ...
      if v-new-st = ? then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input v-rv).                      assign v-view-log = yes.
      end.
      _edist:
      ...
      :
        ...
        run create-edi-state in this-procedure (
                                              ...
                                              , input (if temp-edi-status.tbl-name = 'ord-line-rcv':U
                                                      then v-new-ps
                                                      else temp-edi-status.doc-code)
      ...
      when 8 then do:
       ...
       .
        ...
        if not available ub.ord-doc then do:
          v-mess =  substitute("Не найден заказ поставщику с номером &1", desadv.ordernumber).
          run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при импорте поставок по  заказу &1 из ВС &2&3&4"                                 ,desadv.ordernumber                                 , v-esys-id                                  , chr(10)                                 , v-mess)).                      assign v-view-log = yes.
        end.
