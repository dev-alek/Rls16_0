...
:
  run write-log in p-log-handle ( input 0, "&DLine").
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по валютам и курсам из ВС")).                      assign v-view-log = yes.
  ...
  REPEAT:
    ...
    :
        if retry then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка разбора/приема данных по валютам и курсам из ВС:&1&2&1&3", chr(10), return-value, error-status :get-message( 1 ) )).                      assign v-view-log = yes.
        ...
        IF ImpData1:current-tbl-name( ) = 'currency':U  THEN do:
          trans_save_currency:
          do transaction
          on error  undo _rule, retry _rule
          ...
          :
            assign
              v-line-num  = ImpData1:route-data_get-field-integer  ( input 'currency':U, input "line-num":U )
            .
            run get-xcnf_check-imp-rec in p-cont-handle
              ...
              , input g#db-num
              , input v-pck-num
              , input substitute( "&1&2", v-line-num, 'currency':U )
              , output v-present
              ) no-error.
            if error-status:error then do:
                            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка идентификации принимаемых данных по валютам и курсам из ВС:" )).                      assign v-view-log = yes.
            ...
            run proc-save-currency in this-procedure
              ( input ImpData1:route-data_get-field-integer  ( input 'currency':U, input "curr-code":U)
              , input ImpData1:route-data_get-field-character( input 'currency':U, input "curr-abbr":U)
              , input "":U
              , input ImpData1:route-data_get-field-character( input 'currency':U, input "curr-name":U)
              , input "":U
              , input "":U
              , input "":U
              , input "":U
              ...
              , input "":U
              , input "":U
              , input ImpData1:route-data_get-field-integer  ( input 'currency':U, input "okv-code":U)
              ) no-error .
            if error-status:error then do:
                            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении данных по валютам при приеме данных из ВС:" )).                      assign v-view-log = yes.
              undo _rule, retry _rule.
            end.
            else do:
              assign
        ...
        IF ImpData1:current-tbl-name( ) = 'curr-accnt':U  THEN do:
          ...
          :
            assign
              v-line-num   = ImpData1:route-data_get-field-integer  ( input 'curr-accnt':U, input "line-num":U )
            .
            run get-xcnf_check-imp-rec in p-cont-handle
              ( input "create"
              , input p-esys-id
              ...
              , input g#db-num
              , input v-pck-num
              , input substitute( "&1&2", v-line-num, 'curr-accnt':U )
              , output v-present
              ) no-error.
            if error-status:error then do:
                            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка идентификации принимаемых данных по валютам и курсам из ВС:" )).                      assign v-view-log = yes.
            ...
            run proc-save-curr-accnt in this-procedure
              ( input ImpData1:route-data_get-field-integer  ( input 'curr-accnt':U, input "curr-code":U )
              , input ImpData1:route-data_get-field-date     ( input 'curr-accnt':U, input "exch-date":U )
              , input ImpData1:route-data_get-field-decimal  ( input 'curr-accnt':U, input "exch-rate":U)
              , input ImpData1:route-data_get-field-integer  ( input 'curr-accnt':U, input "exch-scale":U)
              , input ImpData1:route-data_get-field-character( input 'curr-accnt':U, input "status_":U )
              ) no-error .
            if error-status:error then do:
                            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении данных по курсам валют при приеме данных из ВС:" )).                      assign v-view-log = yes.
              undo _rule, retry _rule.
            end.
            else do:
              assign
...
procedure proc-save-currency :
  ...
  do
  on error  undo, return error substitute( "&1 (proc-save-currency). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
  ...
  :
    ...
    run ref/currenc1.p
      ( input-output v-rid
      , input ( if v-stts = 'N' then 'ДОБАВЛЕНИЕ':U else 'ИЗМЕНЕНИЕ':U )
      , input true
      , input p-curr-code
      , input p-curr-abbr
      , input p-part-abbr
    ...
    if error-status:error then do:
      undo, return error substitute( "&1 (proc-save-currency). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1)) .
    end.
  end.
end procedure.
procedure proc-save-curr-accnt :
  ...
  do transaction
  on error  undo, return error substitute( "&1 (proc-save-curr-accnt). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
