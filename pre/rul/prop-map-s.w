...
DO:
 define variable v-rec as recid no-undo.
  run rul/prop-map-i.w ( input parparentproc
                       ,input 'ДОБАВЛЕНИЕ':U
                       ,input (if p-list-mode = "dtm-code" then p-dtm-code else 0)
                       ,input 0
...
assign
  v-str-recid4 = trim( string( recid( X_prop-map ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry4 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-prop-map in frame Dialog-Frame.
  end.
...
apply "entry" to br-prop-map in frame Dialog-Frame.
END.
...
END.
ON CHOOSE OF MENU-ITEM m_prop-head
DO:
    ASSIGN
  link-option = 'prop-head':U.
...
do:
   v-doc-rec = recid(X_prop-map).    run OpenBR in this-procedure.  REPOSITION br-prop-map to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-prop-map.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-prop-map :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-prop-map FOR EACH X_prop-map NO-LOCK,        FIRST X_prop-head NO-LOCK  indexed-reposition.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
CASE p-list-mode:
  WHEN 'все':U THEN DO:
    frame Dialog-Frame:title = "Свойства объектов-операндов".
  ...
  WHEN "dtm-code" THEN DO:
    frame Dialog-Frame:title = substitute("Свойства объектов-операндов для объекта-операнда &1", p-dtm-code).
    OPEN QUERY br-prop-map
