...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-ruledict SHARE-LOCK.
...
PROCEDURE MyEnable :
ASSIGN
tt-ruledict.entry-type:LIST-ITEMS IN FRAME Dialog-Frame = 'constant,operator,control,prop-name_,prop-name_host,prop-name_obj,node-name_,node-name_host,node-name_obj,prop-script,sum-id,rule,rule-profile,datatype,parameter':u.
IF AVAILABLE tt-ruledict THEN
DISPLAY
...
WITH FRAME Dialog-Frame .
assign
tt-ruledict.script-al:read-only in frame Dialog-Frame = (p-mode = 'ПРОСМОТР':U)
tt-ruledict.script-nl:read-only in frame Dialog-Frame = (p-mode = 'ПРОСМОТР':U)
.
ENABLE
B-exit  when p-mode <> 'ПРОСМОТР':U
b-quit
B-Help
tt-ruledict.entry-type  when (p-mode = 'ДОБАВЛЕНИЕ':U and p-entry-type = '':U)
tt-ruledict.entry-id when p-mode = 'ДОБАВЛЕНИЕ':U
tt-ruledict.documentation when p-mode <> 'ПРОСМОТР':U
tt-ruledict.script-al
tt-ruledict.script-nl
WITH FRAME Dialog-Frame .
if p-mode = 'ПРОСМОТР':U then do:
