...
ExpData1 = new Ibs.Th.Rul.Route-data_( input parparentproc, input p-parent-handle, input p-log-handle, input this-procedure:handle) .
...
then do:
  ...
  if m-es then
  do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "&1. &2&3&4", vss-workfile, m-rv, chr(10), m-esm)).
      run garbcoll_clear in this-procedure .
       return error substitute( "&1. &2&3&4", vss-workfile, m-rv, chr(10), m-esm).
  end.
  run garbcoll_clear in this-procedure .
end.
...
procedure proc-main :
  ...
  do
    ...
    if ExpData1:esys-add-dump-data ( input expObj:Data,
                                     ...
                                     ('+update' + chr(4) + expObj:InitSecTag)) = false
    then
      undo _main, return error v-last-error-message .
...
procedure load-ruleset-context :
 ...
 define buffer buf_ext-system      for ub.ext-system.
  do
    ...
    :
    assign
      m-oldbh        = widget-handle (entry(2, p-doc-code, chr(4))).
      m-newbh        = widget-handle (entry(3, p-doc-code, chr(4))).
      m-changes-list = entry(4, p-doc-code, chr(4)).
    ...
    for each buf_rule-call-param no-lock where
      ...
      m-esys-id-list = m-esys-id-list + (if m-esys-id-list = '' then '' else chr(1)) + string(buf_ext-system.esys-id).
    end.
...
procedure local-load-ruleset-context:
    case p-ruleset-id:
      ...
      then do:
          if    m-has-newbh
            and m-newbh:table <> 'c-user-log':U
          then
            undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'c-user-log':U).
          if    m-has-oldbh
            and m-oldbh:table <> 'c-user-log':U
          then
            undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'c-user-log':U).
        end.
      otherwise
