...
define variable vss-include-info30 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str30  as character no-undo.
  define variable tmp-num30  as character no-undo.
  define variable i30        as integer   no-undo.
  define variable sum30      as integer   no-undo.
  define variable len-code30 as integer   no-undo.
  define variable varcont30  as logical   initial yes no-undo.
  CASE bc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str30 = string( internal-b-code, "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str30 = string( internal-b-code, "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont30 = yes then do:
    if integer( substring( tmp-str30, 1, length( bc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = bc-pfx + substring( tmp-str30, length( bc-pfx ) + 1, length( tmp-str30 ) - length( bc-pfx ) )
        len-code30    = length( full-b-code )
      .
      define variable v-sum-char30 as character no-undo .
      assign
        sum30 = 0
      .
      do i30 = 1 to len-code30 by 2
      :
        assign
          v-sum-char30 = substr(full-b-code, len-code30 - i30 + 1, 1)
        .
        if v-sum-char30 < "0"
        or v-sum-char30 > "9"
        then do:
        ...
        assign
          sum30 = sum30 + integer(v-sum-char30)
        .
      end.
      if varcont30 = yes then do:
        assign
        ...
        if varcont30 = yes then do:
           if sum30 mod 10 = 0 then do:
             ...
             assign
               full-b-code = full-b-code + string(10 - sum30 mod 10)
             .
           end.
        end.
      end.
  ...
  :
    if retry then do:
      ...
      then do:
        ...
        if ord-list.doc-type = 'ОП':U
        and ord-list.status_  = 'новый':U
        and ord-list.ord-int1 = integer('1':U) then do:
          assign
        ...
        if ord-list.ord-int1 <> integer('0':U)
        and ord-list.ord-int1 <> integer('1':U)
        then do:
    ...
    sw:insert-attribute ("id", (buf_ord-doc.doc-code + "_" + string(TODAY) + string(time))) .
        ...
        sw:insert-attribute ("date", iso-date(buf_ord-doc.doc-date)) .
            ...
            sw:start-element ("lineItems") .
                sw:write-data-element ("currencyISOCode", string(entry(2, buf_currency.curr-eng-name, chr(4)) , "X(3)")) .
      assign
          ...
          run ordlineattr-write in this-procedure (
                ...
                sw:start-element ("lineItem") .
                    ...
                    if  buf_ord-line.qnty > buf_ord-line.cli-qnty then do :
                        ...
                        swa:update-attribute ("unitOfMeasure", (if ub.units.type = 'шту':U then "PCE" else "KGM")) .
                    end.
