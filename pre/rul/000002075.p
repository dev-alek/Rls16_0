...
define temp-table ORDRSP-t no-undo
...
field INVOICEPARTNER as character format "9999999999999"
...
field EDIMESSAGE as character format "99999999999999"
index pi is unique primary  number
.
...
.
define dataset ORDRSP_-t
for
ORDRSP-t, HEAD-t, POSITION-t
data-relation r1 for ORDRSP-t, HEAD-t relation-fields ( NUMBER, NUMBER) nested
data-relation r2 for HEAD-t, POSITION-t relation-fields ( NUMBER, NUMBER) nested
.
...
:
  ...
  if entry(1, p-cli-out-doc, chr(4)) <> entry(1, buf_ord-doc.cli-out-doc, chr(4)) then do:
    v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'clioutdoc-change':U
                                         , entry(1, buf_ord-doc.cli-out-doc, chr(4))
                                           + chr(4)
                                           + entry(1, p-cli-out-doc, chr(4))).
  end.
  run create-edi-statett in this-procedure (
                                          input 'ord-doc':U
                                        , input buf_ord-doc.doc-code
  ...
  :
    ...
    if available buf_pos then do:
      if buf_ord-line.ord-dec1 < buf_pos.price then do:
         v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'price-up':U, string(buf_ord-line.ord-dec1) + chr(4) + string(buf_pos.price) ).
      end.
      if buf_ord-line.ord-dec1 > buf_pos.price then do:
         v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'price-down':U, string(buf_ord-line.ord-dec1) + chr(4) + string(buf_pos.price) ).
      end.
      if buf_ord-line.cli-qnty < buf_pos.acceptedquantity then do:
         v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'qnty-up':U, string(buf_ord-line.order-cli-qnty) + chr(4) + string(buf_pos.acceptedquantity) ).
      end.
      if buf_ord-line.cli-qnty > buf_pos.acceptedquantity then do:
         v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'qnty-down':U, string(buf_ord-line.order-cli-qnty) + chr(4) + string(buf_pos.acceptedquantity) ).
      end.
      if buf_ord-line.vat-pc <> buf_pos.vat then do:
         v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'vat-change':U, string(buf_ord-line.vat-pc) + chr(4) + string(buf_pos.vat) ).
      end.
      ...
      if buf_pos.info <> '' then do:
        v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ps':U, buf_pos.info).
      end.
            assign
      buf_ord-line.cli-qnty         = (if buf_pos.producttype = integer('3':U)
                                       then 0
      ...
      buf_ord-line.sub-par          = (if buf_pos.producttype <> integer('1':U)
                                      then (entry (lookup (string(buf_pos.producttype), '1,2,3':U) + 1, ',' + 'Совпадение,Разница,Отмена':U) + chr(32) + buf_pos.info + chr(4))
                                      else (buf_pos.info + chr(4)))
   ...
   :
     ...
     if not available buf_ord-line then do:
       ...
       run create-edi-statett in this-procedure (
                                              input 'ord-doc':U
                                            , input (p-doc-code + chr(4) + string( - buf_pos.positionnumber))
                                            , input buf_ord-doc.cli-type
                                            , input buf_ord-doc.cli-code
                                            , input 'ИЗМЕНЕНИЕ':U
                                            , input buf_ord-doc.ord-int1
...
:
  if retry then do:
    ...
    num-rec = num-rec + 1.
    find first buf_ord-doc exclusive-lock where
              buf_ord-doc.doc-code = ordrsp-t.ordernumber no-error.
    if not available buf_ord-doc then do:
      v-mess =  substitute("Не найден заказ поставщику с номером &1", ordrsp-t.ordernumber).
      undo _ordrsp, retry _ordrsp.
    end.
    if buf_ord-doc.ord-int2 = integer('1':U) then do:
      _edis:
      for each buf_edi-status no-lock where
              buf_edi-status.tbl-name = 'ord-doc':U
          and buf_edi-status.doc-code = ordrsp-t.ordernumber
      by buf_edi-status.tbl-name
      by buf_edi-status.doc-code
      by buf_edi-status.date-status
      by buf_edi-status.time-status
      :
        if buf_edi-status.state = '4':U
        or buf_edi-status.state = '5':U
        then do:
    ...
    assign
    v-ps = ""
    v-ord-int1 = (if ordrsp-t.action = 29
                          then integer('6':U)
                          else (if ordrsp-t.action = 27
                                then integer('99':U)
                                else integer('3':U)
                                )
                          )
    v-cli-out-doc = ordrsp-t.number + chr(4) + iso-date(ordrsp-t.date) + chr(4) + ordrsp-t.time_
    v-ship-date = (if ordrsp-t.deliverydate = ?
                          then ord-list.ship-date
                          else ordrsp-t.deliverydate)
    .
        find first head-t where
              head-t.number = ordrsp-t.number no-error.
    ...
    if error-status:error then do:
      ...
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, 'ediiterchangeid':U, head-t.ediinterchangeid ).
      v-date-status = ?.
...
for each position-t where
        position-t.number = p-cli-out-doc:
  delete position-t.
end.
