   ...
   .
    ...
    if available obj-list then do:
      glog = e-clients:INSERT-STRING ( display-clients(v-obj-type
                                                  , v-obj-code
                                                  ,obj-list.obj-name )) in frame Dialog-Frame .
      glog = e-clients:INSERT-STRING ( chr(10) ) in frame Dialog-Frame .
    end.
  end.
end.
ASSIGN
...
b-quit
button-obj  WHEN p-mode <> 'ПРОСМОТР':U
e-clients
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
assign
e-clients:read-only in frame Dialog-Frame = yes.
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры простановки флагов ЗАКОНЧИВШИХСЯ партий по товару, продающемуся по партиям (профайл &1)", p-profile-id).
...
ASSIGN
e-clients:SCREEN-VALUE IN FRAME Dialog-Frame = ''.
for each obj-list :
    ...
    if available buf_clients then do:
      v-rid-list = v-rid-list + (if v-rid-list = '' then '' else chr(44)) + string(recID(BUF_clients)).
    end.
  delete obj-list.
end.
run ref/thobjs.w
