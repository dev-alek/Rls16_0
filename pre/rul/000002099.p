...
using ibs.th.bge.1crn.subjects.price-doc from propath.
...
:
  ...
  case p-ruleset-id:
    when 110 then do:
      if v-has-newbh
      and v-newbh:table <> 'price-doc':U then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'price-doc':U).
      end.
      if v-has-oldbh
      and v-oldbh:table <> 'price-doc':U and v-oldbh:table <> "tt-price-doc" then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'price-doc':U).
      end.
    end.
    otherwise do:
