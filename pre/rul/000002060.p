  ...
  case p-ruleset-id:
    when 2 then do:
      ...
      if v-current-obj-code > 0 then do:
        ...
        .
        if v-current-obj-type = 'орг':U then do:
        ...
        if v-current-obj-type = 'чел':U then do:
          assign
          firm.selfhost =  no
          .
    ...
    when 6 then do:
      ...
      case v-newbh:table:
        when 'sysconf':U then do:
          ...
          assign
          firm.referenceNo = 'орг':U + string(v-current-obj-code)
          firm.orgtype = 'орг':U
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run calltree in g#library
  (input  'cb_get-next-cli-by-obj-type-code'
  ,input  this-procedure:handle
  ,input  p-cont-handle
  ,output v-cli-list-handle
  )  .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run calltree in g#library
  ...
  when 6 then do:
    ...
    case p-ruleset-id:
      when 6 then do:
        if v-has-newbh
        and lookup(v-newbh:table,  'clients':U + chr(44) +
                                  'sysconf':U + chr(44) +
                                    'firm':U + chr(44) +
                                    'person':U + chr(44) +
                                    'shop':U + chr(44) +
