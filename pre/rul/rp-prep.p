...
define variable ss as character no-undo .
  ...
  on error undo, return error
  :
    input stream instream3 from value(p-file-name).
    repeat:
      import stream instream3
      unformatted ss.
      run temp-string_write in this-procedure ( input ss).
    end.
    input stream instream3  close.
  end.
end procedure.
  ...
  :
    if first-of(buf_rule-i-script.i-script-name) then do:
      find first buf_temp-rule-i-script  where
            ...
            and buf_temp-rule-i-script.i-script-type = 'variable':U
            no-error.
