...
:
  ...
  if can-find(first bc-list no-lock) then do:
      run str/diallog.w ( input parparentproc
                    ...
                    , input (string(obj-list.obj-code) + chr(4) + v-action)
                    , input yes
                    , input ''
                    , input (if v-action = "U"
      ...
      then do:
        ...
        run gbl/filename.p (
                ...
                if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( " Ошибка отсылки/удаления на кассу/с кассы кодов закончившихся партий по расписанию: Ошибка в процессе отсылки на кассы маг&1"                                          , obj-list.obj-code )).
        if valid-handle(p-log-handle) then           run send-msg-to-email in p-log-handle (                 input substitute( "ТН БД &1. Операции с товарами в автоматическом режиме - пересылка/удаление на кассу/с укассы"                                         , g#db-num)               , input substitute( " Ошибка отсылки/удаления на кассу/с кассы кодов закончившихся партий по расписанию: Ошибка в процессе отсылки на кассы маг&1"                                          , obj-list.obj-code )               , input v-full-path).
      ...
      if can-find(first pbc-list no-lock) then do:
        ...
        then do:
          ...
          run gbl/filename.p (
                    ...
                    if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( " Ошибка отсылки/удаления на кассу/с кассы ДопБК кодов закончившихся партий по расписанию: Ошибка в процессе отсылки на кассы маг&1"                                            , obj-list.obj-code )).
          if valid-handle(p-log-handle) then           run send-msg-to-email in p-log-handle (                 input substitute( "ТН БД &1. Операции с товарами в автоматическом режиме - пересылка/удаление на кассу/с укассы"                                         , g#db-num)               , input substitute( " Ошибка отсылки/удаления на кассу/с кассы ДопБК кодов закончившихся партий по расписанию: Ошибка в процессе отсылки на кассы маг&1"                                            , obj-list.obj-code )               , input v-full-path).
          OS-DELETE value(v-full-path).
        end.
      end.
  end.
  else do:
        if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(" При отсылке/удалении на кассу/с кассы кодов закончившихся партий по расписанию не было обнаружено кодов партий для удаления для маг&1", obj-list.obj-code)).
  end.
end.
end.
end procedure.
