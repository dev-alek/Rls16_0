...
def shared temp-table cntrlist no-undo like ub.country
  field to-del as logical
  index obj  is primary unique alpha1
  index iname      short-name
  .
define variable vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define  shared  temp-table cntrlist-hist no-undo
...
define buffer buf_country for ub.country.
  ...
  :
    ...
    if not available buf_country then do:
            if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найдена содержащаяся в списке страна &1", cntrlist.alpha1)).
      next _stroka.
    end.
    ...
    ExpData1:route-data_copy-field-character( INPUT "countries", "Action", INPUT  'U':U ) .
