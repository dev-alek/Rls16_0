...
define temp-table temp_onewin_itemsSelected no-undo
    ...
    index im
        itm-key
.
define variable v-onewin9-itm-key    as integer      no-undo.
...
:
    ...
    if available buf_temp_onewin_items then do:
      v-onewin9-itm-key = buf_temp_onewin_items.itm-key.
    end.
    else do:
      v-onewin9-itm-key = 0.
    end.
    assign
        v-onewin9-itm-key = v-onewin9-itm-key + 1
    .
    create buf_temp_onewin_items.
    assign
    buf_temp_onewin_items.itm-key      = v-onewin9-itm-key
...
if p-mode = 'ДОБАВЛЕНИЕ':U then do:
  for each buf_profile-by-profile no-lock where
    ...
    entry(lookup('cmb':U, p-cmb-call-id, chr(3)), p-call-id, chr(3)) = buf2_rule-profile.profile-type.
    p-profile-id = buf_profile-by-profile.child-profile_id.
    assign
    v-call-id-list = v-call-id-list + (if v-call-id-list = '' then '' else chr(4))  +  p-call-id
    v-profile-id-list = v-profile-id-list + (if v-profile-id-list = '' then '' else chr(44)) + string(p-profile-id)
    v-once-more-list = v-once-more-list + (if v-once-more-list = '' then '' else chr(44))  +  string(1)
    .
  ...
  do v-ii = 1 to num-entries('dis-card-type,goods,clients,gds-grp,cli-grp,chk-doc_IBS-TH,chk-doc_IBS-TH-MOB,edoc,thref,pdf,rep,ord,cmb,fdoc':U):
    assign
    p-call-id = p-cmb-call-id.
    entry(lookup('cmb':U, p-cmb-call-id, chr(3)), p-call-id, chr(3)) = entry(v-ii, 'dis-card-type,goods,clients,gds-grp,cli-grp,chk-doc_IBS-TH,chk-doc_IBS-TH-MOB,edoc,thref,pdf,rep,ord,cmb,fdoc':U).
...
do v-ii = 1 to num-entries(v-call-id-list, chr(4) ):
  assign
  p-call-id = entry(v-ii, v-call-id-list, chr(4) )
  p-profile-id = integer(entry(v-ii, v-profile-id-list, chr(44) ))
  p-once-more = integer(entry(v-ii, v-once-more-list, chr(44) ))
  .
...
run bge/oxmlexts.p (
      input parparentproc
    , input 1
    , input substitute("esys-type = &1", '7':U)
...
if v-ok then do:
  do v-ii = 1 to num-entries(v-rid-list):
    ...
    if not (buf_ext-system.esys-type = integer('7':U)) then do:
      message
