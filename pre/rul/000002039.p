...
:
  for each buf_temp-rule-call-param where
    ...
    if available buf_clients then do:
      v-cli-list = v-cli-list + (if v-cli-list = '' then '' else chr(44)) + string(recid(buf_clients)).
     end.
  end.
  ...
  if error-status:error then do:
    ...
    then do:
      run write-log  in p-log-handle (
                                      input 0
                                    , "&DLine").
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Расчет отчета &1 закончился ошибкой", p-rule-id)).
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............&1", v-esm)).
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............&1", v-rv)).
      v-rv = v-esm + chr(10) + v-rv.
    end.
    return error v-rv.
  end.
  else do:
    ...
    if error-status:error then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("!!Не удалось заполнить передать OUTPUT параметры правила!!! &1&2&1&3"                                  , chr(10)                                   , error-status:get-message(1) )).
    end.
  end.
end.
end procedure.
