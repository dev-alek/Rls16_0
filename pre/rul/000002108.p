...
:
  ...
  case p-ruleset-id:
    when 100 then do:
      if v-has-newbh
      and v-newbh:table <> 'place':U and v-newbh:table <> 'pl-gds':U and v-newbh:table <> 'pl-level':U and v-newbh:table <> 'pl-level-mm':U then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'place':U).
      end.
      if v-has-oldbh
      and v-oldbh:table <> 'place':U and v-oldbh:table <> 'pl-gds':U and v-oldbh:table <> 'pl-level':U and v-newbh:table <> 'pl-level-mm':U then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'place':U).
      end.
    end.
    otherwise do:
