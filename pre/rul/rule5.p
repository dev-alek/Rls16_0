...
on error undo, return error substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1)):
  ...
  if buf1_rule.root_rule_id <> buf1_rule.rule_id then do:
     undo main-block, return error substitute( "&1. &2Скопировать можно только корневое правило", vss-workfile, chr(10) ).
  end.
  create buf2_rule.
  ...
  to buf2_rule
  assign
  buf2_rule.rule_id = next-value(s-rule-id, ub)
  buf2_rule.sts = integer('-10':U)
  ...
  :
    ...
    if not available tree_temp-rule then do:
      ...
      tree_temp-rule.rule_id2 = next-value(s-rule-id, ub)
      .
    end.
    IF tree1_rule.rule_id <> buf1_rule.rule_id then do:
      ...
      if not available upper_temp-rule then do:
        ...
        upper_temp-rule.rule_id2 = next-value(s-rule-id, ub)
        .
      end.
      create tree2_rule.
    ...
    :
      ...
      to buf2_rule-script
      assign
      buf2_rule-script.script_id = next-value(s-rule-script-id, ub)
  ...
  find first buf1_ruledict no-lock where
          buf1_ruledict.entry-type = 'rule':U
