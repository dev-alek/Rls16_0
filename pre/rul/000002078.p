...
:
  ...
  if retry then do:
        if valid-handle(p-log-handle) then           run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при обработке поставки &1 по заказу &2:&3&4"                                    ,v-current-doc-code                                    , v-current-doc-code                                     , chr(10), v-mess)).
    ...
    if v-has-newbh then do:
      ...
      v-cli-rcv-code = entry (1 , v-newbh::sub-par, chr(4)).
    end.
    assign
    v-mess = ''
    .
