  ...
  do on error undo, retry:
    if  retry then do:
      v-retry-action = v-retry-action + 1.
                    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при импорте данных &1&2&1&3"                                                                   , chr(10)                                                                   , error-status:get-message(1)                                                                   , return-value)).
    ...
    v-field-map = "ID,doc-id" + chr(44) +
                  "ExtNum,ext-num" + chr(44) +
                  "AgentID,agent-id" + chr(44) +
                  "FromStoreID,from-store-id" + chr(44) +
                  "ToStoreID,to-store-id" + chr(44) +
                  "StartDate,start-date" + chr(44) +
                  "FinishDate,finish-date".
    v-bh = v-thds:get-buffer-handle("temp_doc-header").
    v-bh:attach-data-source(v_dsheader, v-field-map).
    v-field-map = "DocID,doc-id" + chr(44) +
                  "GoodsID,goods-id" + chr(44) +
                  "StoreID,store-id".
