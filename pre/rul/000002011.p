...
define variable vss-description as character no-undo init "Библиотека процедур для работы с кодексом 18 набор правил 12".
...
function 00180012_get-error-message returns character :
define variable v-ii as integer no-undo .
...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по Документам назначения цены из ВС")).
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Импорт данных по документам назначения цены из файла &1", file-name)).
...
ImpData1:Route-data_dump ( ) .
    ...
    _rule:
      do on error undo _rule, retry _rule:
        ...
        IF v_qh:query-off-end then leave _stroka.
          IF  ImpData1:current-tbl-name( ) = "price-doc"  THEN do:
            create temp-price-doc.
            temp-price-doc.line-num = ImpData1:route-data_get-field-integer ( input "price-doc", input "line-num")   .
            temp-price-doc.doc-date = ImpData1:route-data_get-field-date    ( input "price-doc", input "doc-date")   .
            temp-price-doc.doc-num  = ImpData1:route-data_get-field-integer ( input "price-doc", input "doc-num")    .
            temp-price-doc.obj-type = ImpData1:route-data_get-field-character( input "price-doc", input "obj-type")  .
            temp-price-doc.obj-code = ImpData1:route-data_get-field-integer  ( input "price-doc", input "obj-code")  .
            .
            release temp-price-doc.
          end.
          IF  ImpData1:current-tbl-name( ) = "price-list"  THEN do:
            create temp-price-list.
              temp-price-list.line-num   = ImpData1:route-data_get-field-integer ( input "price-list", input "line-num")    .
              temp-price-list.bar-code   = ImpData1:route-data_get-field-integer  ( input "price-list", input "bar-code")   .
              temp-price-list.doc-num    = ImpData1:route-data_get-field-integer  ( input "price-list", input "doc-num")   .
              temp-price-list.price-sale = ImpData1:route-data_get-field-decimal  ( input "price-list", input "price-sale")  .
              .
             release temp-price-list.
          end.
      end.
    end.
    v-retry-action = 0 .
  ...
  if error-status:error then do:
    if return-value <> '' then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении данных по ДНЦ из ВС:&1&2&1&3", chr(10), return-value , error-status :get-message(1)  )).
    end.
