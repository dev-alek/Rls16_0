...
:
  ...
  :
    ...
    if not available buf_clients then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найден содержащийся в списке клиент &1&2", cli-list.obj-type, cli-list.obj-code)).
      v-view-log = yes.
      next _stroka.
    end.
    if buf_clients.obj-type = 'орг':U then do:
      ...
      if available buf_sysconf then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Нельзя маршрутизировать данные по СВОЕЙ ФИРМЕ (&1&2)", cli-list.obj-type, cli-list.obj-code)).
        v-view-log = yes.
        next _stroka.
      end.
    end.
    v-attr-list = 'cli-alc-producer':U + chr(4) +
                  'foreign-producer':U .
