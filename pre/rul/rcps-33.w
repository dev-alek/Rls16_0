...
if not error-status:error then do:
  OPEN QUERY br-gds FOR EACH gds_tt-rule-call-param where              gds_tt-rule-call-param.param-name = "p-discnt-roles"          and gds_tt-rule-call-param.rule_id = 1971          and gds_tt-rule-call-param.p-index > 0 by gds_tt-rule-call-param.p-index          .
...
if not error-status:error then do:
  OPEN QUERY br-pay FOR EACH pay_tt-rule-call-param where              pay_tt-rule-call-param.param-name = "p-discnt-roles"          and pay_tt-rule-call-param.rule_id = 1973          and pay_tt-rule-call-param.p-index > 0 by pay_tt-rule-call-param.p-index           .
...
DO:
  ...
  apply "entry" to br-pay.
END.
ON CHOOSE OF B-pay-down IN FRAME Dialog-Frame
DO:
define variable v-recid as recid no-undo .
run move-up-down in this-procedure ( buffer pay_tt-rule-call-param
  ...
  apply "ENTRY" to br-pay.
END.
ON CHOOSE OF B-pay-up IN FRAME Dialog-Frame
DO:
define variable v-recid as recid no-undo .
run move-up-down in this-procedure ( buffer pay_tt-rule-call-param
                                     ,input "up"
...
if not error-status:error then do:
  OPEN QUERY br-subtotal FOR EACH subtotal_tt-rule-call-param where              subtotal_tt-rule-call-param.param-name = "p-discnt-roles"          and subtotal_tt-rule-call-param.rule_id = 1972          and subtotal_tt-rule-call-param.p-index > 0 by subtotal_tt-rule-call-param.p-index            .
...
DO:
  ...
  apply "entry" to br-subtotal.
END.
ON CHOOSE OF B-subtotal-down IN FRAME Dialog-Frame
DO:
define variable v-recid as recid no-undo .
run move-up-down in this-procedure ( buffer subtotal_tt-rule-call-param
  ...
  apply "ENTRY" to br-subtotal.
END.
ON CHOOSE OF B-subtotal-up IN FRAME Dialog-Frame
DO:
define variable v-recid as recid no-undo .
run move-up-down in this-procedure ( buffer subtotal_tt-rule-call-param
                                     ,input "up"
  ...
  apply "ENTRY" to br-subtotal.
END.
ON VALUE-CHANGED OF t-rule-gds IN FRAME Dialog-Frame
DO:
  assign
  t-rule-gds.
  case t-rule-gds:
    ...
    when no then do:
      for each gds_tt-rule-call-param where
        ...
        and gds_tt-rule-call-param.rule_id = 1971
...
END.
ON VALUE-CHANGED OF t-rule-pay IN FRAME Dialog-Frame
DO:
    assign
  t-rule-pay.
  case t-rule-pay:
    ...
    when no then do:
      for each pay_tt-rule-call-param where
        ...
        and pay_tt-rule-call-param.rule_id = 1973
...
END.
ON VALUE-CHANGED OF t-rule-subtotal IN FRAME Dialog-Frame
DO:
    assign
  t-rule-subtotal.
  case t-rule-subtotal:
    ...
    when no then do:
      for each subtotal_tt-rule-call-param where
        ...
        and subtotal_tt-rule-call-param.rule_id = 1972
...
br-subtotal
br-pay
t-add-gds-discnts          WHEN p-mode <> 'ПРОСМОТР':U
t-add-subtotal-discnts     WHEN p-mode <> 'ПРОСМОТР':U
t-add-pay-discnts          WHEN p-mode <> 'ПРОСМОТР':U
t-rule-gds                WHEN p-mode <> 'ПРОСМОТР':U
t-rule-subtotal                WHEN p-mode <> 'ПРОСМОТР':U
t-rule-pay                WHEN p-mode <> 'ПРОСМОТР':U
b-gds-add WHEN p-mode <> 'ПРОСМОТР':U
b-gds-delete WHEN p-mode <> 'ПРОСМОТР':U
b-pay-add WHEN p-mode <> 'ПРОСМОТР':U
b-pay-delete WHEN p-mode <> 'ПРОСМОТР':U
b-subtotal-add WHEN p-mode <> 'ПРОСМОТР':U
b-subtotal-delete WHEN p-mode <> 'ПРОСМОТР':U
b-gds-up WHEN p-mode <> 'ПРОСМОТР':U
b-gds-down WHEN p-mode <> 'ПРОСМОТР':U
b-pay-up WHEN p-mode <> 'ПРОСМОТР':U
b-pay-down WHEN p-mode <> 'ПРОСМОТР':U
b-subtotal-up WHEN p-mode <> 'ПРОСМОТР':U
b-subtotal-down WHEN p-mode <> 'ПРОСМОТР':U
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры стандартного расчета скидки в чеке на IBS TH POS (профайл &1)", p-profile-id).
OPEN QUERY br-gds FOR EACH gds_tt-rule-call-param where              gds_tt-rule-call-param.param-name = "p-discnt-roles"          and gds_tt-rule-call-param.rule_id = 1971          and gds_tt-rule-call-param.p-index > 0 by gds_tt-rule-call-param.p-index          .
OPEN QUERY br-subtotal FOR EACH subtotal_tt-rule-call-param where              subtotal_tt-rule-call-param.param-name = "p-discnt-roles"          and subtotal_tt-rule-call-param.rule_id = 1972          and subtotal_tt-rule-call-param.p-index > 0 by subtotal_tt-rule-call-param.p-index            .
OPEN QUERY br-pay FOR EACH pay_tt-rule-call-param where              pay_tt-rule-call-param.param-name = "p-discnt-roles"          and pay_tt-rule-call-param.rule_id = 1973          and pay_tt-rule-call-param.p-index > 0 by pay_tt-rule-call-param.p-index           .
...
and v-ok then do:
  ...
  if term_tt-rule-call-param.rule_id = 1971 then do:
    case v-value-character:
      when 'pcnt-kat':U then do:
        find first dbl_tt-rule-call-param where
                  dbl_tt-rule-call-param.rule_id = term_tt-rule-call-param.rule_id
              and dbl_tt-rule-call-param.param-value-character = 'pcnt-kat-pdf':U
        ...
        if available dbl_tt-rule-call-param then do:
          v-mess = substitute("Уже применяется тип скидки &1.&2" +
                              ...
                              , 'Категорийная через ТПЛ':U
                              , chr(10)
                              , 'Категорийная скидка':U
                              ).
        end.
      end.
      when 'pcnt-kat-pdf':U then do:
        find first dbl_tt-rule-call-param where
                  dbl_tt-rule-call-param.rule_id = term_tt-rule-call-param.rule_id
              and dbl_tt-rule-call-param.param-value-character = 'pcnt-kat':U
              and dbl_tt-rule-call-param.p-index > 0 no-error.
        if available dbl_tt-rule-call-param then do:
          v-mess = substitute("Уже применяется тип скидки &1.&2" +
                              "Нельзя одновременно задавать применение типа скидки <&1>&2и типа скидки <&3>"
      ...
      when 'temp-disc-pdf':U then do:
        find first dbl_tt-rule-call-param where
                  dbl_tt-rule-call-param.rule_id = term_tt-rule-call-param.rule_id
              and dbl_tt-rule-call-param.param-value-character = 'temp-disc':U
              and dbl_tt-rule-call-param.p-index > 0 no-error.
        if available dbl_tt-rule-call-param then do:
          v-mess = substitute("Уже применяется тип скидки &1.&2" +
                              "Нельзя одновременно задавать применение типа скидки <&1>&2и типа скидки <&3>"
      ...
      when 'temp-disc':U then do:
        find first dbl_tt-rule-call-param where
                  dbl_tt-rule-call-param.rule_id = term_tt-rule-call-param.rule_id
              and dbl_tt-rule-call-param.param-value-character = 'temp-disc-pdf':U
              and dbl_tt-rule-call-param.p-index > 0 no-error.
        if available dbl_tt-rule-call-param then do:
          v-mess = substitute("Уже применяется тип скидки &1.&2" +
                              "Нельзя одновременно задавать применение типа скидки <&1>&2и типа скидки <&3>"
  ...
  RUN set-value IN THIS-PROCEDURE (
                                  ...
                                  ,INPUT (if p-list-mode = 'rp-rule-param':U
...
if not can-find(first tt-rule-call-param where
                      tt-rule-call-param.rule_id = 1971
                 and  tt-rule-call-param.p-index > 0) then do:
  t-rule-gds = no.
  display
  t-rule-gds
...
if not can-find(first tt-rule-call-param where
                      tt-rule-call-param.rule_id = 1972
                 and  tt-rule-call-param.p-index > 0) then do:
  t-rule-subtotal = no.
  display
  t-rule-subtotal
...
if not can-find(first tt-rule-call-param where
                      tt-rule-call-param.rule_id = 1973
                 and  tt-rule-call-param.p-index > 0) then do:
  t-rule-pay = no.
  display
  t-rule-pay
