  ...
  if retry then do:
    ...
    then do:
      run write-log  in p-log-handle (
                                      input 0
                                    , "&DLine").
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Удаление ненужных отчетов &1 из БД закончилось ошибкой", p-rule-id)).
    ...
    for each buf_clob-bind share-lock where
            (buf_clob-bind.resource-type = 'report':U
            or
            buf_clob-bind.resource-type = 'report-xml':U)
