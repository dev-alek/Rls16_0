...
:
  ...
  :
    ...
    if not available buf_goods then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найден содержащийся в списке товар с кодом &1", gds-list.gds-code)).                      assign v-view-log = yes.
      next _stroka.
    end.
    ...
    if not available buf_clients then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Для товара с кодом &1 не найден производитель &2&3"                                   , gds-list.gds-code                                   , buf_goods.prod-type                                   , buf_goods.prod-code)).                      assign v-view-log = yes.
      next _stroka.
    end.
    find first buf_gds-prt no-lock WHERE
              buf_gds-prt.upper-code = buf_goods.prt-root NO-error.
    if not available buf_gds-prt then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Для товара с кодом &1 не найден корень ШКАЛЫ ПРИЗНАКОВ с кодом &2"                                   , gds-list.gds-code                                   , buf_goods.prt-root                                   )).                      assign v-view-log = yes.
      next _stroka.
    end.
    ...
    and buf_goods.fbr-grp-code <> 0 then do:
      ...
      if not available buf_fbr-gds-grp then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Для товара с кодом &1 не найдена группа блюд &2"                                     , gds-list.gds-code                                     , buf_goods.fbr-grp-code)).                      assign v-view-log = yes.
        next _stroka.
      end.
    ...
    do v-ii = 1 to num-entries(v-attr-list, chr(4)):
      ...
      temp-goods-attr.attr-code  = entry(v-ii, v-attr-list, chr(4))
      .
      run gds-attr-copy-to  in this-procedure (
                                              input  v-current-gds-code
                                              ,input  entry(v-ii, v-attr-list, chr(4))
    ...
    if error-status:error then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input return-value).                      assign v-view-log = yes.
        next _stroka.
    end.
    ...
    :
      ...
      if not available buf_gds-prt then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Для товара с кодом &1 не найден узел ШКАЛЫ ПРИЗНАКОВ с кодом &2"                                     , gds-list.gds-code                                     , buf_bar-code.node-code                                     )).                      assign v-view-log = yes.
        next _stroka.
      end.
