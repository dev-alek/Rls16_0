...
:
  ...
  :
    ...
    if not available locked_scales then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Не найдены весы &1 по БД &2", buf_scales.scales-num, buf_scales.db-num)).                      assign v-view-log = yes.
      next _stroka.
    end.
