...
assign
  v-str-recid4 = trim( string( recid( X_prop-head ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry4 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-prop-head in frame Dialog-Frame.
  end.
...
apply "entry" to br-prop-head in frame Dialog-Frame.
END.
ON CHOOSE OF b-prop IN FRAME Dialog-Frame
DO:
define variable v-rec as recid no-undo .
if not available X_prop-head then return no-apply.
if not (X_prop-head.storage-place = 'dis-card-property':U
        or
        X_prop-head.storage-place-host = 'dis-card-property':U
        or
        X_prop-head.storage-place-obj = 'dis-card-property':U) then do:
  message
  substitute("Свойства можно настроить только если место хранения &1", 'dis-card-property':U)
...
END.
ON CHOOSE OF b-storage IN FRAME Dialog-Frame
DO:
  IF NOT AVAILABLE X_prop-head THEN RETURN NO-APPLY.
...
DO:
    ASSIGN
   storage-option = 'объект':U.
   run proc-b-storage IN THIS-PROCEDURE ( INPUT storage-option) NO-ERROR.
   ASSIGN
   link-option = '':U.
   IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
ON CHOOSE OF MENU-ITEM m_prop-map
DO:
    ASSIGN
  link-option = 'prop-map':U.
...
DO:
    ASSIGN
  link-option = 'prop-ruleset':U.
  run proc-b-link IN THIS-PROCEDURE ( INPUT link-option) NO-ERROR.
  ASSIGN
  link-option = '':U.
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
ON CHOOSE OF MENU-ITEM m_prop-script
DO:
    ASSIGN
  link-option = 'prop-script':U.
  run proc-b-link IN THIS-PROCEDURE ( INPUT link-option) NO-ERROR.
  ASSIGN
  link-option = '':U.
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
...
do:
   v-doc-rec = recid(X_prop-head).    run OpenBR in this-procedure.  REPOSITION br-prop-head to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-prop-head.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-prop-head :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-prop-head FOR EACH X_prop-head NO-LOCK BY X_prop-head.dtm-code INDEXED-REPOSITION .
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
b-storage:MENU-MOUSE in frame Dialog-Frame = 1
.
ENABLE
b-quit
