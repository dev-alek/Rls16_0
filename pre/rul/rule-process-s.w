...
DO:
if not available X_rule-process then return no-apply.
run proc-b-lkp IN THIS-PROCEDURE ( INPUT 'rule-process':U) NO-ERROR.
...
assign
  v-str-recid5 = trim( string( recid( X_rule-process ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry5 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-rule-process in frame Dialog-Frame.
  end.
...
apply "entry" to br-rule-process in frame Dialog-Frame.
END.
...
do:
   v-doc-rec = recid(X_rule-process).    RUn OpenBR in this-procedure.  REPOSITION br-rule-process to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-rule-process.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-rule-process :handle
      .
...
CASE p-list-mode:
   when 'все':U then do:
      frame Dialog-Frame :title = "Все звенья процессов".
      OPEN QUERY br-rule-process
      FOR EACH X_rule-process NO-LOCK INDEXED-REPOSITION.
   end.
   when "pchain-type" then do:
      frame Dialog-Frame :title = substitute("Звенья процессов типа &1", p-pchain-type).
      OPEN QUERY br-rule-process
