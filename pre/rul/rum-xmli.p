...
:
  ...
  if error-status :error then do:
    run write-log in p-log-handle (           input 2         , input substitute("Файл пакета &1 не найден"                                  , p-xml-file-name                                   ) ).
    undo, return error substitute("Файл пакета &1 не найден"                                  , p-xml-file-name                                   ).
  end.
  ...
  and v-esys-id <> p-esys-id then do:
    run write-log in p-log-handle (           input 2         , input substitute("Ожидался файл от ВС &1, в файле импорта указана ВС &2"                                  , p-esys-id                                  , v-esys-id                                   ) ).
    return error substitute("Ожидался файл от ВС &1, в файле импорта указана ВС &2"                                  , p-esys-id                                  , v-esys-id                                   ).
  end.
  if p-pack-num <> 0
  and v-pack-num <> p-pack-num then do:
    run write-log in p-log-handle (           input 2         , input substitute("Читается пакет № &1, в файле импорта у№ пакета указан как &2"                                  , p-pack-num                                  , v-pack-num                                   ) ).
    return error substitute("Читается пакет № &1, в файле импорта у№ пакета указан как &2"                                  , p-pack-num                                  , v-pack-num                                   ).
  end.
  if v-schema-name <> p-xsd-file then do:
    run write-log in p-log-handle (           input 2         , input substitute("В профайле &1 нет обработки данных по схеме &2"                                  , p-profile-id                                  , v-schema-name                                   ) ).
    return error substitute("В профайле &1 нет обработки данных по схеме &2"                                  , p-profile-id                                  , v-schema-name                                   ).
  end.
  assign
  v-pck-num = 0.
  ...
  if error-status:error then do:
        run write-log in p-log-handle (           input 2         , input substitute("Ошибка при создании структуры маршрутизируемых данных согласно гейту:&1&2"                                , v-gate-rec                                , chr(10), error-status:get-message(1) ) ).
    undo, return error substitute("Ошибка при создании структуры маршрутизируемых данных согласно гейту:&1&2"                                , v-gate-rec                                , chr(10), error-status:get-message(1) ).
  end.
  ...
  :
   ...
   then do:
      ...
      undo, return error substitute("Ошибка при создании заголовка XML файла:&1&2", chr(10), error-status:get-message(1) ).
    end.
  end.
  ...
  if error-status :error then do:
            run write-log in p-log-handle (           input 2         , input substitute("Ошибка при создании временного файла xsd со схемой &1:&2&3"                                     , v_locdataseth:name                                     , chr(10)                                       , error-status:get-message(1) ) ).
    run gate-clear in this-procedure ( input v_locdataseth, input v-xmlh).
    undo, return error substitute("Ошибка при создании временного файла xsd со схемой &1:&2&3"                                     , v_locdataseth:name                                     , chr(10)                                       , error-status:get-message(1) ).
  end.
  assign
  v-rec-cnt = 0
  .
  ...
  if error-status:error then do:
        run write-log in p-log-handle (           input 2         , input substitute("Ошибка при создании структуры маршрутизируемых данных согласно файлу схемы:&1&2&3&2&4"                                , v-schema-tmp-file                                , chr(10), error-status:get-message(1), return-value  ) ).
    undo, return error substitute("Ошибка при создании структуры маршрутизируемых данных согласно файлу схемы:&1&2&3&2&4"                                , v-schema-tmp-file                                , chr(10), error-status:get-message(1), return-value  ).
  end.
  ASSIGN
  ...
  if error-status:error
  then do:
    run write-log in p-log-handle (           input 2         , input substitute("Ошибка при чтении XML файла &1 данными через гейт &2:&3&4"                                     , p-xml-file-name                                     , v-schema-name                                     , chr(10)                                       , error-status:get-message(1)) ).
    run gate-clear in this-procedure ( input v_dataseth, input v-xmlh).
    undo, return error substitute("Ошибка при чтении XML файла &1 данными через гейт &2:&3&4"                                     , p-xml-file-name                                     , v-schema-name                                     , chr(10)                                       , error-status:get-message(1)).
  end.
  if not glog then do:
    v-xml-err = error-status:get-message(1) + chr(10) + error-status:get-message(2).
        run write-log in p-log-handle (           input 2         , input substitute("Не прошел верификацию с помощью схемы &1 XML файл &2:&3&4&3&5"                                     , v-schema-name                                     , p-xml-file-name                                     , chr(10)                                       , v-xml-err ) ).
    run gate-clear in this-procedure ( input v_dataseth, input v-xmlh).
    os-delete value(v-schema-tmp-file).
    undo, return error substitute("Не прошел верификацию с помощью схемы &1 XML файл &2:&3&4&3&5"                                     , v-schema-name                                     , p-xml-file-name                                     , chr(10)                                       , v-xml-err ).
  end.
  ...
  if not available buf_temp-xml-tables then do:
    run write-log in p-log-handle (           input 2         , input substitute("Не найден THHEADER в XML файлe &1:&2&3"                                     , p-xml-file-name                                     , chr(10)                                       , error-status:get-message(1)) ).
    run gate-clear in this-procedure ( input v_dataseth, input v-xmlh).
    os-delete value(v-schema-tmp-file).
    undo, return error substitute("Не найден THHEADER в XML файлe &1:&2&3"                                     , p-xml-file-name                                     , chr(10)                                       , error-status:get-message(1)).
  end.
  ...
  or buf_temp-xml-tables.tbl-handle_:available = no then do:
    run write-log in p-log-handle (           input 2         , input substitute("Не найдена запись THHEADER в XML файлe &1:&2&3"                                     , p-xml-file-name                                     , chr(10)                                       , error-status:get-message(1)) ).
    run gate-clear in this-procedure ( input v_dataseth, input v-xmlh).
    os-delete value(v-schema-tmp-file).
    undo, return error substitute("Не найдена запись THHEADER в XML файлe &1:&2&3"                                     , p-xml-file-name                                     , chr(10)                                       , error-status:get-message(1)).
  end.
  assign
