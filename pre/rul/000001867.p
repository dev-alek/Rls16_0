...
Chk1 = new Dis-chk-doc_obj( input parparentproc, input p-parent-handle, input p-log-handle, input this-procedure:handle, input p-cont-handle, input v-current-lock, input v-current-wait, input v-save) .
...
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:
  _1871:
  do:
  IF  temp-d-card.sale-type = 'inkas':U  THEN do:
    _1872:
    do:
