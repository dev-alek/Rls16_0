...
if v-sht-status = 'факт':U then do:
  ...
  do
    ...
    case p-ruleset-id:
      when 150 then
        do:
          if v-has-newbh
            and v-newbh:table <> 'fin-doc':U then
          do:
            undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'fin-doc':U).
          end.
          if v-has-oldbh
            and v-oldbh:table <> 'fin-doc':U and v-oldbh:TABLE <> "tt-fin" then
          do:
            undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'fin-doc':U).
          end.
        end.
