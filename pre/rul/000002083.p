...
define temp-table temp-dis-rule no-undo like ub.dis-rule.
 ...
 define variable p-rule-num-host as integer no-undo.
...
v-type = p-type.
 ...
 Discount-host1:category = dis-rule_00059( INPUT p-rule-num-host, INPUT v-current-doc-date, INPUT v-current-doc-time, INPUT v-netto-sum1) .
...
and buf_rule-call-param.p-index > 0,
  ...
  if not available temp-dis-rule then do:
    ...
    find first temp-dis-rule no-lock where
                temp-dis-rule.host-code = v-current-host-code
            and temp-dis-rule.obj-type = ''
            and temp-dis-rule.obj-code = 0 no-error.
    if not available temp-dis-rule then do:
      empty temp-table temp-dis-rule.
      return "return".
    end.
    p-rule-num-host = temp-dis-rule.rule-num.
  end.
end procedure.
