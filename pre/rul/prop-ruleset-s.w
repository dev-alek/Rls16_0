...
assign
  v-str-recid4 = trim( string( recid( X_prop-ruleset ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry4 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-prop-ruleset in frame Dialog-Frame.
  end.
...
apply "entry" to br-prop-ruleset in frame Dialog-Frame.
END.
...
do:
   v-doc-rec = recid(X_prop-ruleset).    RUn OpenBR in this-procedure.  REPOSITION br-prop-ruleset to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-prop-ruleset.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-prop-ruleset :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-prop-ruleset FOR EACH X_prop-ruleset NO-LOCK, first X_prop-head NO-LOCK indexed-reposition.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
CASE p-add:
  when yes then do:
    if p-list-mode = "ruleset" then do:
      ...
      do v-ii = 1 to num-entries(v-rid-list ):
        ...
        run rul/prop-ruleset1.p ( input 'ДОБАВЛЕНИЕ':U
                                  ,input no
                                  ,input-output v-rec
                                  ,input p-codex-id
                                  ,input p-ruleset-id
  ...
  when no then do:
    ...
    v-rec = recid(x_prop-ruleset).
    run rul/prop-ruleset1.p ( input 'ИЗМЕНЕНИЕ':U
                              ,input no
                              ,input-output v-rec
