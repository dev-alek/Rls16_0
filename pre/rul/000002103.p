...
if v-has-newbh and v-newbh:table <> "chk-doc" then do:
  ...
  if (v-sht-status = 'зкр':U) or (v-sht-status = 'тек':U) then do:
  ...
  for first buf_shift-obj no-lock
    ...
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "Успешно. ")).
...
:
  ...
  case p-ruleset-id:
    when 140 then do:
      if v-has-newbh
      and v-newbh:table <> 'shift-obj':U AND v-newbh:table <> "chk-doc" then do:
        undo, return error substitute("Передан неверный буфер [&1] вместо буфера для &2", v-newbh:table, 'shift-obj':U).
      end.
      if v-has-oldbh
      and v-oldbh:table <> 'shift-obj':U and v-oldbh:table <> "tt-shift" AND  v-oldbh:table <> "chk-doc" then do:
        undo, return error substitute("Передан неверный буфер [&1] вместо буфера для &2", v-oldbh:table, 'shift-obj':U).
      end.
    end.
    otherwise do:
