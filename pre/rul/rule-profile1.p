...
:
  if p-mode = 'ДОБАВЛЕНИЕ':U then do:
    ...
    if lookup( p-profile-type, 'dis-card-type,goods,clients,gds-grp,cli-grp,chk-doc_IBS-TH,chk-doc_IBS-TH-MOB,edoc,thref,pdf,rep,ord,cmb,fdoc':U) = 0 then do:
      assign
  ...
  if p-mode = 'ИЗМЕНЕНИЕ':U then do:
    ...
    END.
    v-entry-id = buf_ruledict.entry-id.
    if buf_rule-profile.parent-feature = integer('0':U)
    and p-parent-feature = integer('1':U)
  ...
  if lookup(string(p-parent-feature), '0,1':U) = 0 then do:
    assign
    ...
    if not available tt-ruledict-param then do:
      ...
      then do:
        find first buf_tt-ruledict-param where
                  buf_tt-ruledict-param.param-data-type = 'character':U
