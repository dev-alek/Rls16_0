  ...
  :
    ...
    if buf_clients.obj-type = 'орг':U then do:
      ...
      if available buf_sysconf then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Нельзя экспортировать данные по СВОЕЙ ФИРМЕ (&1&2)", cli-list.obj-type, cli-list.obj-code)).
        v-view-log = yes.
        next _stroka.
      end.
    end.
