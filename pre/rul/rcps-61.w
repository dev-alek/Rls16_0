...
find first buf_clob-bind no-lock where
 ...
 DISPlay
 f-list-name
 v-list-id WITH FRAME Dialog-Frame.
END.
...
f-list-name
v-list-id WITH FRAME Dialog-Frame.
rs-method:radio-buttons in frame Dialog-Frame = 'Список':U + chr(44) + 'list':U + chr(44) +
'Макрос формир списка':U + chr(44) + 'list-macro':U .
DEFINE VARIABLE v-ch0 AS widget-handle NO-UNDO.
ASSIGN
v-ch0 = br-rcp:FIRST-COLUMN IN FRAME Dialog-Frame.
REPEAT WHILE valid-handle(v-ch0):
...
rs-action
rs-method
selectobject WHEN (p-mode <> 'ПРОСМОТР':U and v-running-mode)
button-obj  WHEN (p-mode <> 'ПРОСМОТР':U and v-running-mode)
b-lob-sel  WHEN p-mode <> 'ПРОСМОТР':U
e-shops
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
...
END.
ASSIGN
FRAME Dialog-Frame:TITLE = substitute("Параметры передачи/удаления на кассу/с кассы списка товаров по профайлу &1", p-profile-id).
...
case rs-method:
  when 'list':U then do:
    RUN rcps_set-value IN THIS-PROCEDURE (
                                    ...
                                    ,INPUT "p-list"
                                    ,INPUT 0
                                    ,INPUT substitute("gds-list_&1", v-list-id)
                                    ,INPUT ?
                                    ,INPUT 0.0
                                    ,INPUT 0
                                    ,INPUT NO
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  ,output v-obj-db-num
          ...
          if v-obj-db-num <> v-cntxt-db-num then do:
            message
            substitute("&1&2принадлежит другой БД - пропускаю ...", 'маг':U, buf_shop.obj-code)
            view-as alert-box .
            next.
          end.
          find first obj-list where
