...
define variable vss-description as character no-undo init "Библиотека процедур для работы с кодексом 18 набор правил 22".
...
function 00180024_get-error-message returns character :
define variable v-ii as integer no-undo .
...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute(".............Импорт данных по Спецификациям из ВС")).
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Импорт данных по Спецификациям из файла &1", file-name)).
...
ImpData1:Route-data_dump ( ) .
    ...
    _rule:
      do on error undo _rule, retry _rule:
        ...
        IF v_qh:query-off-end then leave _stroka.
          IF  ImpData1:current-tbl-name( ) = "contract-specif"  THEN do:
            create temp_contract-specif.
              temp_contract-specif.line-num       = ImpData1:route-data_get-field-integer ( input "contract-specif", input "line-num")      .
              temp_contract-specif.gds-code       = ImpData1:route-data_get-field-integer ( input "contract-specif", input "gds-code")      .
              temp_contract-specif.contract-code  = ImpData1:route-data_get-field-integer ( input "contract-specif", input "contract-code") .
              temp_contract-specif.price-cli      = ImpData1:route-data_get-field-decimal ( input "contract-specif", input "price-cli")     .
              temp_contract-specif.prc            = ImpData1:route-data_get-field-decimal ( input "contract-specif", input "prc")           .
              temp_contract-specif.vat-pc         = ImpData1:route-data_get-field-decimal ( input "contract-specif", input "vat-pc")        .
              temp_contract-specif.vat-type       = ImpData1:route-data_get-field-character ( input "contract-specif", input "payer-vat")        .
              temp_contract-specif.status_        = ImpData1:route-data_get-field-character ( input "contract-specif", input "status_")       .
              temp_contract-specif.exch-code      = ImpData1:route-data_get-field-integer ( input "contract-specif", input "exch-code")       .
             release temp_contract-specif.
          end.
      end.
    end.
    v-retry-action = 0 .
  ...
  if error-status:error then do:
    if return-value <> '' then do:
            run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Ошибка при сохранении данных по спецификации из ВС:&1&2&1&3", chr(10), return-value , error-status :get-message(1)  )).
    end.
    ...
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Прочитано записей: &1, обработано: &2", num-rec, num-rec-ok2)).
end.
end procedure.
...
procedure delete-procedure :
  ...
  on error undo, return error
  :
    empty temp-table temp_contract.
    empty temp-table temp_contract-specif.
