...
define buffer buf_clients for ub.clients.
  ...
  case v-obj-type:
    ...
    or when 'орг':U then do:
      v-obj-type-code = (if v-obj-type = 'орг':U
                         then  1000000000
                         else 0) +
                         v-obj-code.
      ...
      ExpData1:route-data_copy-field-character( INPUT "clients", "Action", INPUT  (if v-stts = integer('0':U)
                                                                                    then 'U':U
                                                                                    else 'D':U
                                                                                    )).
    ...
    or when 'скл':U then do:
      v-obj-type-code = (if v-obj-type = 'скл':U
                         then  100000
                         else 0) +
                         v-obj-code.
...
:
  ...
  case p-ruleset-id:
    when 6 then do:
      if v-has-newbh
      and v-newbh:table <> 'clients':U then do:
        if lookup(v-newbh:table, 'sysconf':U + chr(44) +
                                  'firm':U + chr(44) +
                                  'person':U + chr(44) +
                                  'shop':U + chr(44) +
                                  'store':U ) > 0 then do:
          ...
          undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'clients':U).
        end.
      end.
    end.
    otherwise do:
