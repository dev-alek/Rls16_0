...
function context_set-esys-command-action return logical
(
    input p-esys-cmd-proc-handle as handle
   ,input p-esys-cmd-code as integer
   ...
   then do:
      ...
      if error-status :error
      then do:
         delete procedure p-esys-cmd-proc-handle no-error.        run set-error in this-procedure ( input substitute("Ошибка при установке кода действия команды &1 маршрутизации во внешнюю систему &2&3&2&4"
                                                     , p-cmd-code
                                                     , chr(10)
                                                     , error-status:get-message(1)
                                                     , return-value
      ...
      delete procedure p-esys-cmd-proc-handle no-error.        run set-error in this-procedure ( input substitute("Ошибка при установке кода действия команды &1 команды маршрутизации во внешнюю систему &2Процедура маршрутизации еще не запущена"
                                                    , p-cmd-code
                                                    , chr(10)
                                                    )).
      return no .
...
:
  ...
  case p-ruleset-id:
    when 8 then do:
      if v-has-newbh
      and v-newbh:table <> 'gds-obj':U then do:
        undo, return error substitute("Передан неверный буфер вместо буфера для &1",  'gds-obj':U).
      end.
    end.
  end case.
  ...
  .
  v-obj-type-code = (if v-obj-type = 'скл':U then 100000 else 0) + v-obj-code.
