...
:
  case p-call-type:
    ...
    then do:
      ...
      run gen-b-code in this-procedure ( input 'cagb':U, output p-call#-id) no-error .
      if error-status:error then do:
        message
        vss-workfile vss-revision vss-description skip
    ...
    then do:
     ...
     when ('layout-elem-rule':U + chr(44) + "minus") then do:
      find first buf_rule-by-call no-lock where
    ...
    when 'some-lk':U then do:
      ...
      if g#db-num = 0 then do:
        p-call#-id = next-value(s-lk-chip, ub).
      end.
      else do:
        message
        vss-workfile vss-revision vss-description skip
