...
:
  ...
  assign
  log-file-name = entry(1 , p-parameter, chr(4)).
   run write-log-and-file in p-log-handle (                             input 1                                                           , input log-file-name                                               , input 1                                                           , input substitute("!!!Проверка отсутствия подключенных пользователей IBS TH (в т.ч. СПН в ручном режиме)")).
    run write-log-and-file in p-log-handle (                             input 1                                                           , input log-file-name                                               , input 1                                                           , input substitute("!!!Проверка отсутствия подключенных пользователей IBS TH&1" +                             "Пожалуйста, ждите......"                             , chr(10))).
  ...
  if can-find(first temp-connect) then do:
    for each temp-connect:
      run write-log-and-file in p-log-handle (                             input 1                                                           , input log-file-name                                               , input 1                                                           , input substitute("Обнаружено подключение к БД IBS TH&1пользователь &2 &3"                             , chr(10)                             , temp-connect._connect-usr                             , temp-connect._connect-name                              )).
    end.
    return error.
  end.
end.
