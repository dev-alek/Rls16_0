...
:
  ...
  assign
  v-free-task-name = entry(buffer temp-schedule-free:buffer-field("free-task-name"):POSITION - 2, v-value, chr(4) )
  v-run-prog-name = entry(buffer temp-schedule-free:buffer-field("proc-run-name"):POSITION - 2, v-value, chr(4) )
  v-conf-par = entry(buffer temp-schedule-free:buffer-field("conf-param"):POSITION - 2, v-value, chr(4) )
  v-enable-concurrent-0 = (entry(buffer temp-schedule-free:buffer-field("enable-concurrent-0"):POSITION - 2, v-value, chr(4) ) = "yes")
  v-enable-concurrent-db = (entry(buffer temp-schedule-free:buffer-field("enable-concurrent-db"):POSITION - 2, v-value, chr(4) ) = "yes")
  no-error .
  if error-status:error then do:
    v-mes = substitute("&1 &2 &3&4Невозможно получить название  произвольного задания по строке расписания&4" +
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  if not v-enable-concurrent-db then do:
    run gbl/lock-prc.p
        ...
        ,input ""
        ,input (
                v-free-task-name + chr(32) + string(p-db-num)
              )
        ,input no
        ,buffer lock-batchprocess
        ) no-error .
  ...
  if v-is-rum then do:
    ...
    run str/ordrum.p
      ...
      ,input  ( string(dynamic-next-value( "next-rep-num":U, "ubflt":U ), "9999999999") + chr(4) +
                string(p-task-num) )
      ,input yes
      ) no-error .
      end.
