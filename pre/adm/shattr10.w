...
tt-cancell.object_-name = entry(lookup(tt-cancell.object_,  ('chk-gds':U + chr(44) + 'chk-doc':U + chr(44) + 'ord-doc':U)),                      "Блюдо/товар,Чек,Заказ").
v-rid = RECID(tt-cancell)
.
OPEN QUERY BR-cancell FOR EACH tt-cancell NO-LOCK INDEXED-REPOSITION.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-cancell :handle
      .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cd-type-magia-xml':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = 'mag-bnal':U THEN DO:
    ASSIGN
    f-mag-bnal = thbjattr_thbj-attr.property-value-INTEGER
    f-mag-bnal:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'magnopay':U THEN DO:
    ASSIGN
    f-magnopay = thbjattr_thbj-attr.property-value-INTEGER
    f-magnopay:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'mag-vip':U THEN DO:
    ASSIGN
    f-mag-vip = thbjattr_thbj-attr.property-value-INTEGER
    f-mag-vip:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'ret-item':U THEN DO:
    ASSIGN
    ret-item = thbjattr_thbj-attr.property-value-character
    .
  END.
  IF v-entry = 'wro-item':U THEN DO:
    ASSIGN
    wro-item = thbjattr_thbj-attr.property-value-character.
  END.
  IF v-entry = 'ret-chk':U THEN DO:
    ASSIGN
    ret-chk = thbjattr_thbj-attr.property-value-character.
  END.
  IF v-entry = 'wro-chk':U THEN DO:
    ASSIGN
    wro-chk = thbjattr_thbj-attr.property-value-character.
  END.
  IF v-entry = 'ret-ord':U THEN DO:
    ASSIGN
    ret-ord = thbjattr_thbj-attr.property-value-character.
  END.
  IF v-entry = 'wro-ord':U THEN DO:
    ASSIGN
...
b-quit
B-Help
b-mag-bnal WHEN p-mode = 'ИЗМЕНЕНИЕ':U
b-magnopay WHEN p-mode = 'ИЗМЕНЕНИЕ':U
b-mag-vip WHEN p-mode = 'ИЗМЕНЕНИЕ':U
...
for each tt-cancell:
  ...
  and tt-cancell.object_ = 'chk-gds':U then
  assign
  ret-item = ret-item + (if ret-item = '':u then '':U else ';':U) +
            string(tt-cancell.grp-code) + chr(44) +
  ...
  .
  if tt-cancell.return-write-off = yes
  and tt-cancell.object_ = 'chk-doc':U then
  assign
  ...
  .
  if tt-cancell.return-write-off = yes
  and tt-cancell.object_ = 'ord-doc':U then
  assign
...
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'ret-item':U.
assign
thbjattr_thbj-attr.property-value-character = ret-item.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'wro-item':U.
assign
thbjattr_thbj-attr.property-value-character = wro-item.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'ret-chk':U.
assign
thbjattr_thbj-attr.property-value-character = ret-chk.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'wro-chk':U.
assign
thbjattr_thbj-attr.property-value-character = wro-chk.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'ret-ord':U.
assign
thbjattr_thbj-attr.property-value-character = ret-ord.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'wro-ord':U.
assign
