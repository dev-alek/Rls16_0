...
entry (lookup (X_layout.layout-type, 'th-pos-screen,th-pos-keyboard':U) + 1, ',' + 'Экран IBS TH POS,Клавиатура IBS TH POS':U) COLUMN-LABEL "Тип раскладки" FORMAT "X(255)" WIDTH 22
X_layout.device-type COLUMN-LABEL "Устройства" FORMAT "X(15)"
entry (lookup (string(X_layout.is-default), '0,1,-1':U) + 1, ',' + 'Пользовательская,Шаблон IBS TH,Обязательная':U) COLUMN-LABEL "Вид раскладки" FORMAT "X(13)"
entry (lookup (string(X_layout.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMn-label "Статус" format "X(6)"
...
DO:
  ...
  if X_layout.is-default = integer('1':U)
  and not v-admin then do:
        message
    substitute("Нельзя редактировать &1", entry (lookup (string(X_layout.is-default), '0,1,-1':U) + 1, ',' + 'Пользовательская,Шаблон IBS TH,Обязательная':U))
...
DO:
if not available X_layout then return no-apply.
run proc-b-lkp IN THIS-PROCEDURE ( INPUT 'layout':U) NO-ERROR.
...
assign
  v-str-recid9 = trim( string( recid( X_layout ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry9 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-layout in frame Dialog-Frame.
  end.
...
apply "entry" to br-layout in frame Dialog-Frame.
END.
...
do:
   v-doc-rec = recid(X_layout).    RUn OpenBR in this-procedure ( input yes, input no, input '':U).  REPOSITION br-layout to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-layout.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-layout :handle
      .
...
define variable vss-include-info22 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  and lookup(p-layout-type, 'th-pos-screen,th-pos-keyboard':U) = 0 then do:
    message
...
ASSIGN
v-h = br-layout:FIRST-COLUMN in frame Dialog-Frame
.
DO while valid-handle(v-h) :
  if v-h:LABEL = "Тип раскладки" then do:
...
if p-open-query then do:
  ...
  if l-filter-open-24 = false then do:
    OPEN QUERY br-layout FOR EACH X_layout
      where  true
      indexed-reposition
  .
      assign
  ...
  assign
    v-doc-rec = recid( X_layout )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if QUERY br-layout:handle:get-buffer-handle(1) = (buffer X_layout:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input QUERY br-layout:handle
                          ,input rowid(X_layout)
                          ,input logical(parameter-2-24)
                          ,input no-lock
                          ,input (buffer X_layout:handle)
                          ,input parameter-4-24
                          ,input parameter-5-24
                          ) no-error.
      .
      ...
      assign
      parameter-2-24 = (if p-find-next then "true":u else "false":u )
      parameter-3-24 =  "FOR EACH X_layout"
      parameter-4-24 =
        (
          if (" true " + " " + where-phrase-24) <> ""
          then " true " + " " + where-phrase-24
  ...
  run waitfram-hide in this-procedure .
        ...
        frame Dialog-Frame :title = "Все раскладки".
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-26 as character no-undo .
      ...
      parameter-4-26 =
        (
          if (" X_layout.layout-id < '_' " + " " + where-phrase-26) <> ""
          then  substitute('X_layout.layout-id < &1_&1 and X_layout.is-default >= 0 ', chr(34))  + " " + where-phrase-26
          else "true"
        )
      parameter-5-26 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-26 =
          (" X_layout.layout-id < '_' " + " " + where-phrase-26 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-layout:handle
  ...
  :
    if QUERY br-layout:handle:get-buffer-handle(1) = (buffer X_layout:handle) then do:
      ...
      parameter-4-26 =
        "where ":u +  substitute('X_layout.layout-id < &1_&1 and X_layout.is-default >= 0 ', chr(34))  + " ":u + where-phrase-26 + " ":u + p-find-condition + " " + ""
      parameter-5-26 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-26 = (if p-find-next then "true":u else "false":u )
      parameter-3-26 =  "FOR EACH X_layout"
      parameter-4-26 =
        (
          if (" X_layout.layout-id < '_' " + " " + where-phrase-26) <> ""
          then  substitute('X_layout.layout-id < &1_&1 and X_layout.is-default >= 0 ', chr(34))  + " " + where-phrase-26
  ...
  run waitfram-hide in this-procedure .
   ...
   when "layout-type" then do:
      ASSIGN
      filter-point-label = substitute("Раскладки типа: &1", entry (lookup (p-layout-type, 'th-pos-screen,th-pos-keyboard':U) + 1, ',' + 'Экран IBS TH POS,Клавиатура IBS TH POS':U)).
      frame Dialog-Frame:title = filter-point-label
      .
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-28 as character no-undo .
      ...
      parameter-4-28 =
        (
          if (" X_layout.layout-type = p-layout-type and X_layout.is-default >= 0 " + " " + where-phrase-28) <> ""
          then  substitute('X_layout.layout-type = &1&2&1 and X_layout.is-default >= 0', chr(34), p-layout-type) + " " + where-phrase-28
          else "true"
        )
      parameter-5-28 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-28 =
          (" X_layout.layout-type = p-layout-type and X_layout.is-default >= 0 " + " " + where-phrase-28 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-layout:handle
  ...
  if l-filter-open-28 = false then do:
    OPEN QUERY br-layout FOR EACH X_layout
      where  X_layout.layout-type = p-layout-type and X_layout.is-default >= 0
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-layout:handle:get-buffer-handle(1) = (buffer X_layout:handle) then do:
      ...
      parameter-4-28 =
        "where ":u +  substitute('X_layout.layout-type = &1&2&1 and X_layout.is-default >= 0', chr(34), p-layout-type) + " ":u + where-phrase-28 + " ":u + p-find-condition + " " + ""
      parameter-5-28 = " "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-28 = (if p-find-next then "true":u else "false":u )
      parameter-3-28 =  "FOR EACH X_layout"
      parameter-4-28 =
        (
          if (" X_layout.layout-type = p-layout-type and X_layout.is-default >= 0 " + " " + where-phrase-28) <> ""
          then  substitute('X_layout.layout-type = &1&2&1 and X_layout.is-default >= 0', chr(34), p-layout-type) + " " + where-phrase-28
...
DEFINE VARIABLE glog AS LOGICAL NO-UNDO.
 IF NOT v-admin
 AND X_layout.is-default = INTEGER('-1':U) THEN DO:
    MESSAGE
 ...
 define variable v-rec as recid no-undo.
  run adm/layout-i.w ( input parparentproc
                       ,input 'КОПИРОВАНИЕ':U + (if v-admin then (chr(44) + "admin") else '')
                       ,input X_layout.layout-id
                       ,input-output v-rec) no-error.
  if v-rec <> ? then do:
    RUN openbr IN THIS-PROCEDURE ( input yes, input no, input '':U).
...
CASE p-option:
  WHEN 'layout':U THEN DO:
      run adm/layout-i.w ( input parparentproc
                           ,input 'ПРОСМОТР':U +  (if v-admin then (chr(44) + "admin") else '')
                           ,input X_layout.layout-id
                           ,input-output v-rec) no-error.
  END.
END CASE.
