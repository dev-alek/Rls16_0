...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'gds-ref_obj':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = 'dfltggrp':U THEN DO:
    ...
    f-dfltggrp:private-data IN FRAME Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
    RUN set-full-grp-name IN THIS-PROCEDURE ( INPUT f-dfltggrp) .
  END.
  IF v-entry = 'chg-bcod':U THEN DO:
    ASSIGN
    t-chg-bcod = thbjattr_thbj-attr.property-value-logical
    t-chg-bcod:private-data IN FRAME Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'gdsscrvw':U THEN DO:
    ...
    .
  END.
  IF v-entry = 'image-dir':U THEN DO:
    ASSIGN
    f-image-dir = thbjattr_thbj-attr.property-value-character
    f-image-dir:private-data IN FRAME Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
...
t-chg-bcod WHEN p-mode = 'ИЗМЕНЕНИЕ':U
f-image-dir WHEN p-mode = 'ИЗМЕНЕНИЕ':U
br-gdsscrvw
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
