...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cd-type-omron-new':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = 'omrnbase':U THEN DO:
    ASSIGN
    f-omrnbase = thbjattr_thbj-attr.property-value-INTEGER
    f-omrnbase:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'omrnnal':U THEN DO:
    ASSIGN
    f-omrnnal = thbjattr_thbj-attr.property-value-INTEGER
    f-omrnnal:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'omrnntnl':U THEN DO:
    ASSIGN
    f-omrnntnl = thbjattr_thbj-attr.property-value-INTEGER
    f-omrnntnl:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'omrncurl':U THEN DO:
    ASSIGN
    v-omrncurl = thbjattr_thbj-attr.property-value-character
    .
  END.
  IF v-entry = 'omrnpayl':U THEN DO:
    ASSIGN
...
b-cash-pay-nal WHEN p-mode = 'ИЗМЕНЕНИЕ':U
b-cash-pay-ntnl WHEN p-mode = 'ИЗМЕНЕНИЕ':U
f-omrnbase WHEN p-mode = 'ИЗМЕНЕНИЕ':U
...
FOR EACH tt-cash-pay-omrn BY tt-cash-pay-omrn.omrn-pay-code:
  ASSIGN
  v-omrncurl = v-omrncurl + (IF v-omrncurl = "":U THEN "":U ELSE chr(44)) + STRING(tt-cash-pay-omrn.omrn-pay-code)
  v-omrnpayl = v-omrnpayl + (IF v-omrnpayl = "":U THEN "":U ELSE chr(44)) + STRING(tt-cash-pay-omrn.cdpay-code)
  .
END.
assign
fh = frame Dialog-Frame:first-child
...
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'omrncurl':U.
assign
thbjattr_thbj-attr.property-value-character = v-omrncurl.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'omrnpayl':U.
assign
