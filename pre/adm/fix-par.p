...
:
  ...
  do transaction
  on error  undo, return error substitute( "&1 (sys-key). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
  ...
  :
    ...
    if locked buf_sys-ctrl then do:
      undo, return error substitute( "Уже начат процесс инициализации ПАРАМЕТРОВ другим пользователем.&1Дождитесь завершения и повторите попытку запуска.", chr(10) ).
    end.
