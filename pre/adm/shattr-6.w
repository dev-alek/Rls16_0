...
vscales-type = "":U
.
DO ii = 1 to num-entries('CAS_LP-15,CAS_LP-6,HELMAC_net,HELMAC_model-Z,HELMAC_model-T,CAS_LP-485,BOLET_P-280,BZB-SC515,DIGI_SM-80,CAS_LP-15v1.6,TIGER,MIRA,TIGER2,CAS_LP-16x,DIGI-SM,TIGER-SPCT2,SHTRIH-M,CAS_LP-II,CAS_CL5000J,CAS_CL5000,DIGI_AW-4600_FX,TIGER-SPCT1':U):
    assign
    vscales-type = vscales-type + chr(44) + entry(ii, 'CAS_LP-15,CAS_LP-6,HELMAC_net,HELMAC_model-Z,HELMAC_model-T,CAS_LP-485,BOLET_P-280,BZB-SC515,DIGI_SM-80,CAS_LP-15v1.6,TIGER,MIRA,TIGER2,CAS_LP-16x,DIGI-SM,TIGER-SPCT2,SHTRIH-M,CAS_LP-II,CAS_CL5000J,CAS_CL5000,DIGI_AW-4600_FX,TIGER-SPCT1':U)
    .
end.
run gbl/d-list.w (
              INPUT "b-sel":U
...
DO:
  ...
  RUN Openbr-Scales-type IN THIS-PROCEDURE no-error.
END.
ON CHOOSE OF B-etc IN FRAME Dialog-Frame
DO:
...
DO:
  ...
  if available X_scales then do:
      if lookup(string( X_scales.scales-num ), v-num-list ) > 0  then do:
          v-num-list = TRIM( v-num-list, chr(44)) .
          v-num-list = chr(44) + v-num-list + chr(44).
          v-num-list = replace( v-num-list, chr(44) + string( X_scales.scales-num ) + chr(44), "") .
          v-num-list = TRIM( v-num-list, chr(44)) .
      end.
      else
      v-num-list = v-num-list + ( if v-num-list = "" then "" else chr(44) ) + string(  X_scales.scales-num ) .
      loc#log = br-scales:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
          loc#log = br-scales:select-next-row ().
          apply "VALUE-CHANGED" to br-scales in frame Dialog-Frame.
      end.
      if num-entries( v-num-list ) = 0
      then
          hide mark-num in frame Dialog-Frame.
      else
          disp num-entries( v-num-list ) @ mark-num with frame Dialog-Frame.
  end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'scale-inf':U
...
for each thbjattr_thbj-attr:
  ...
  .
  IF v-entry = 'scales-type':U THEN DO:
    ...
    END.
  END.
  IF v-entry = 'scales-pr':U THEN DO:
    v-scale-pr-all = thbjattr_thbj-attr.property-value-character.
  END.
  IF v-entry =  'scallist':U THEN DO:
    v-num-list = "":U.
    DO jj = 1 TO NUM-ENTRIES(thbjattr_thbj-attr.property-value-character):
      ...
      IF AVAILABLE buf_scales THEN DO:
        ASSIGN
        v-num-list = v-num-list + (IF v-num-list = "":U THEN "":U else chr(44)) + STRING(buf_scales.scales-num).
      END.
    END.
  END.
  if v-entry = 'noauto-scls':U then do:
    v-noauto-scls = thbjattr_thbj-attr.property-value-logical.
  end.
  IF v-entry = 'sclin-ld':U THEN DO:
    ASSIGN
    rs-sclin-ld = thbjattr_thbj-attr.property-value-integer
    rs-sclin-ld:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  end.
...
B-etc when p-mode = 'ИЗМЕНЕНИЕ':U
b-quit
B-Help
br-scales-type
br-scales
rs-sclin-ld WHEN p-mode = 'ИЗМЕНЕНИЕ':U
b-mark WHEN p-mode = 'ИЗМЕНЕНИЕ':U
...
FOR EACH tt-scales-type NO-LOCK:
 ASSIGN
 v-scales = v-scales + (if v-scales = "":U then "":U else chr(44)) + tt-scales-type.scales-type
 v-scale-pr = v-scale-pr + (if v-scale-pr = "":U then "":U else chr(44)) + tt-scales-type.scales-name
 .
END.
FOR EACH buf_scales NO-LOCK where buf_scales.db-num = v-obj-db-num:
  IF lookup(string(buf_scales.scales-num), v-num-list) > 0  THEN DO:
    ASSIGN
    ii = ii + 1
    v-scallist = v-scallist + (IF ii = 1 THEN "":U ELSE chr(44)) + string(buf_scales.scales-num).
  END.
END.
if v-scallist = '' then do:
  define variable choice as integer no-undo .
  run gbl/d-askw.w (input "Уточнение"
                        ,input  ("Вы не выбрали список весов для работы в магазине"  + chr(10) +
...
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'scales-type':U.
assign
thbjattr_thbj-attr.property-value-character = v-scales
.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'scales-pr':U.
assign
thbjattr_thbj-attr.property-value-character = v-scale-pr
.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'scallist':U.
assign
thbjattr_thbj-attr.property-value-character = v-scallist
.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'noauto-scls':U.
assign
thbjattr_thbj-attr.property-value-logical = v-noauto-scls
.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'sclin-ld':U.
assign
