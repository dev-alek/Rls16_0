...
define buffer buf_sel-client for ub.clients.
...
DO:
    ...
    display fi-cli-text with frame Dialog-Frame.
END.
...
PROCEDURE my-enable :
    if p-mode = 'ИЗМЕНЕНИЕ':U then do:
        enable fi-cli-adr b-sel-cli with frame Dialog-Frame.
    end.
    else do:
...
PROCEDURE proc-load :
    ...
    for each temp-thbj-attr:
        if temp-thbj-attr.prop-code = 'pko-cli':U then do:
            find first buf_sel-client no-lock
        ...
        if temp-thbj-attr.prop-code = 'srv-auth-adr':U then do:
            fi-cli-adr = temp-thbj-attr.property-value-character.
            display fi-cli-adr with frame Dialog-Frame.
        end.
    end.
end procedure.
PROCEDURE proc-save :
    ...
    .
    find first temp-thbj-attr where temp-thbj-attr.prop-code = 'pko-cli':U.
    temp-thbj-attr.property-value-character = subst("&1,&2", buf_sel-client.obj-type, buf_sel-client.obj-code).
    find first temp-thbj-attr where temp-thbj-attr.prop-code = 'srv-auth-adr':U.
