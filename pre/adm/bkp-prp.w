...
DEFINE NEW GLOBAL SHARED VAR adm-broker-hdl    AS HANDLE  NO-UNDO.
    ASSIGN adm-object-hdl = FRAME bkp-prp:HANDLE.
RUN ensure-broker.
...
PROCEDURE adm-disable :
    DISABLE b-exit b-help WITH FRAME bkp-prp.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN b-exit b-help WITH FRAME bkp-prp.
...
PROCEDURE view-page :
  ...
  RUN broker-view-page IN adm-broker-hdl (INPUT THIS-PROCEDURE,
      INPUT p-page#).
  END PROCEDURE.
ON WINDOW-CLOSE OF FRAME bkp-prp
DO:
  APPLY "END-ERROR":U TO SELF.
END.
define variable vss-include-info0 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame bkp-prp
do:
  run gbl/app_help.p
...
on choose of b-help in frame bkp-prp
do:
  apply "help":u to frame bkp-prp .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame bkp-prp:width - 0.3
                fh            = frame bkp-prp:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME bkp-prp:PARENT eq ?
THEN FRAME bkp-prp:PARENT = ACTIVE-WINDOW.
MAIN-BLOCK:
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
  RUN dispatch IN THIS-PROCEDURE ('initialize':U).
  WAIT-FOR GO OF FRAME bkp-prp.
END.
