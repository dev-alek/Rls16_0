...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'chk-view':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = 'ch-bc-ck':U THEN DO:
    ASSIGN
    t-ch-bc-ck = thbjattr_thbj-attr.property-value-logical
    t-ch-bc-ck:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'chk-inf':U THEN DO:
    ASSIGN
    t-chk-inf = thbjattr_thbj-attr.property-value-logical
    t-chk-inf:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr)).
  END.
  IF v-entry = 'chk-spfc':U THEN DO:
    ASSIGN
    t-chk-spfc = thbjattr_thbj-attr.property-value-logical
    t-chk-spfc:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'paycardv':U THEN DO:
    ASSIGN
    S-paycardv:LIST-ITEMS IN FRAME Dialog-Frame  = thbjattr_thbj-attr.property-value-character.
  END.
  if v-entry = 'dc-change':U then do:
    assign
    t-dc-change = thbjattr_thbj-attr.property-value-logical
    t-dc-change:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  end.
...
b-quit
B-Help
t-ch-bc-ck WHEN p-mode = 'ИЗМЕНЕНИЕ':U
t-chk-inf WHEN p-mode = 'ИЗМЕНЕНИЕ':U
t-chk-spfc WHEN p-mode = 'ИЗМЕНЕНИЕ':U
t-dc-change WHEN p-mode = 'ИЗМЕНЕНИЕ':U
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
