...
entry (lookup (X_wi-mode.mode-type, 'cd-IBS-TH':U) + 1, ',' + 'Режимы работы кассы IBS TH POS':U) COLUMN-LABEL "Тип режима" FORMAT "X(255)" WIDTH 30
...
DO:
if not available X_wi-mode then return no-apply.
run proc-b-lkp IN THIS-PROCEDURE ( INPUT 'wi-mode':U) NO-ERROR.
...
assign
  v-str-recid5 = trim( string( recid( X_wi-mode ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry5 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-wi-mode in frame Dialog-Frame.
  end.
...
apply "entry" to br-wi-mode in frame Dialog-Frame.
END.
...
do:
   v-doc-rec = recid(X_wi-mode).    RUn OpenBR in this-procedure.  REPOSITION br-wi-mode to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-wi-mode.
    apply "VALUE-CHANGED" to br-wi-mode.
end.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-wi-mode :handle
      .
...
define variable vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  and lookup(p-mode-type, 'cd-IBS-TH':U) = 0 then do:
    message
...
define variable vh-mode-type-name as handle no-undo .
ASSIGN
v-h = br-wi-mode:FIRST-COLUMN in frame Dialog-Frame
.
DO while valid-handle(v-h) :
  if v-h:LABEL = "Тип режима" then do:
