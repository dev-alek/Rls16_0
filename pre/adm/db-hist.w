...
ON WINDOW-CLOSE OF FRAME d-db-hist
DO:
  APPLY "END-ERROR":U TO SELF.
END.
ON CHOOSE OF b-quit IN FRAME d-db-hist
DO:
  define variable v-ok as logical   no-undo .
END.
ON CHOOSE OF b-sch IN FRAME d-db-hist
DO:
...
DO:
  if available X_c-db then do:
    run proc-full-temp-changes in this-procedure
      ( input (X_c-db.action = 1 )
       ,input (X_c-db.action = 99 )
...
END.
if VALID-HANDLE(ACTIVE-WINDOW) AND FRAME d-db-hist:PARENT eq ?
THEN FRAME d-db-hist:PARENT = ACTIVE-WINDOW.
define variable vss-include-info8 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame d-db-hist
do:
  run gbl/app_help.p
...
on choose of b-help in frame d-db-hist
do:
  apply "help":u to frame d-db-hist .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame d-db-hist:width - 0.3
                fh            = frame d-db-hist:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame d-db-hist :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame d-db-hist :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame d-db-hist :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-db-hist :height = v-frame-height
          .
          if frame d-db-hist :scrollable = true
          then do:
            assign
              frame d-db-hist :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame d-db-hist :height
      v-frame-virtual-height = frame d-db-hist :virtual-height
    ...
    assign
      v-field-group-handle = frame d-db-hist :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame d-db-hist :virtual-height = frame d-db-hist :virtual-height + p-change-value
          no-error .
        if error-status :error
      ...
      assign
        frame d-db-hist :height = frame d-db-hist :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame d-db-hist :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame d-db-hist :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame d-db-hist :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame d-db-hist :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame d-db-hist :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame d-db-hist :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-db-hist :width = v-frame-width
          .
          if frame d-db-hist :scrollable = true
          then do:
            assign
              frame d-db-hist :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame d-db-hist :width
      v-frame-virtual-width = frame d-db-hist :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame d-db-hist :virtual-width = frame d-db-hist :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame d-db-hist :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame d-db-hist :width = frame d-db-hist :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame d-db-hist :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame d-db-hist :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame d-db-hist :height - v-diasize-resize-button :height
                  - 1
                  - (frame d-db-hist :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame d-db-hist :width - v-diasize-resize-button :width
                  - 1
                  - (frame d-db-hist :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame d-db-hist
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame d-db-hist :height
      v-col-delta = v-new-col - frame d-db-hist :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame d-db-hist :width
      v-diasize-current-frame-height = frame d-db-hist :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame d-db-hist :height
        v-diasize-orig-frame-width  = frame d-db-hist :width
        v-diasize-browse-handle     = browse BR-changes :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame d-db-hist :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
do:
  BR-changes :SET-REPOSITIONED-ROW(8, "CONDITIONAL") .
end.
define variable vss-include-info12 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame d-db-hist anywhere
do:
  run OpenBR in this-procedure ( input yes, input no, input no ) .
    apply "VALUE-CHANGED" to BR-changes.
end.
define variable vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
DEF VAR cur-clmn-numbr-db-hist as INT EXTENT 11 no-undo.
DEF VAR varmvibr-db-hist       as INT no-undo.
DEF VAR varmvjbr-db-hist       as INT no-undo.
DEF VAR varmvkbr-db-hist       as INT no-undo.
DEF VAR varmvlbr-db-hist       as INT no-undo.
DEF VAR move-elementbr-db-hist as INT no-undo.
def var jjbr-db-hist           as int no-undo.
do varmvibr-db-hist = 1 to EXTENT(cur-clmn-numbr-db-hist):
  ASSIGN cur-clmn-numbr-db-hist[varmvibr-db-hist] = varmvibr-db-hist.
END.
RUN start-mv-clmnbr-db-hist.
PROCEDURE start-mv-clmnbr-db-hist:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-db-hist do:
  RUN re-move-clmnbr-db-hist ( 1, 11).
END.
ON ctrl-cursor-left OF BROWSE br-db-hist do:
  RUN re-move-clmnbr-db-hist (11, 1).
END.
PROCEDURE re-move-clmnbr-db-hist:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-db-hist = 1 TO EXTENT(cur-clmn-numbr-db-hist):
    if cur-clmn-numbr-db-hist[varmvibr-db-hist] = source-column THEN cur-clmn-numbr-db-hist[varmvibr-db-hist] = -1.
  END.
  if br-db-hist:MOVE-COLUMN(source-column, target-column) IN FRAME d-db-hist then.
  if source-column > target-column THEN
  DO varmvjbr-db-hist = source-column - 1 to target-column BY -1:
    DO varmvibr-db-hist = 1 TO EXTENT(cur-clmn-numbr-db-hist):
        if cur-clmn-numbr-db-hist[varmvibr-db-hist] = varmvjbr-db-hist THEN DO:
          cur-clmn-numbr-db-hist[varmvibr-db-hist] = cur-clmn-numbr-db-hist[varmvibr-db-hist] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-db-hist = source-column + 1 to target-column:
    DO varmvibr-db-hist = 1 TO EXTENT(cur-clmn-numbr-db-hist):
      if cur-clmn-numbr-db-hist[varmvibr-db-hist] = varmvjbr-db-hist THEN DO:
        cur-clmn-numbr-db-hist[varmvibr-db-hist] = cur-clmn-numbr-db-hist[varmvibr-db-hist] - 1.
      END.
  ...
  END.
  DO varmvibr-db-hist = 1 TO EXTENT(cur-clmn-numbr-db-hist):
    if cur-clmn-numbr-db-hist[varmvibr-db-hist] = -1 THEN cur-clmn-numbr-db-hist[varmvibr-db-hist] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-db-hist:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-db-hist = 1 TO EXTENT(cur-clmn-numbr-db-hist):
    if cur-clmn-numbr-db-hist[varmvibr-db-hist] = cur-clmn-loc THEN move-elementbr-db-hist = varmvibr-db-hist.
  END.
  RUN re-move-clmnbr-db-hist (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-db-hist:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-db-hist = 1 to EXTENT(cur-clmn-numbr-db-hist):
    RUN re-move-clmnbr-db-hist (cur-clmn-numbr-db-hist[varmvlbr-db-hist], varmvlbr-db-hist).
  END.
  RUN start-mv-clmnbr-db-hist.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
.
def var sort-labelbr-db-hist   as character no-undo .
def var sort-clmnbr-db-hist    as handle    no-undo .
def var cur-clmnbr-db-hist     as handle    no-undo .
def var cur-clmn-locbr-db-hist as integer   no-undo .
def var re-querybr-db-hist     as logical   initial no no-undo .
on start-search, ctrl-o of br-db-hist in frame d-db-hist do:
   run sort-brbr-db-hist
     (input (if available X_c-db
             then recid(X_c-db)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-db-hist :
  define input parameter p-recid as recid no-undo .
  if re-querybr-db-hist = no then do:
    assign
       cur-clmnbr-db-hist = br-db-hist:current-column in frame d-db-hist
    .
    if sort-clmnbr-db-hist <> ? then sort-clmnbr-db-hist:column-fgcolor = 0.
    if cur-clmnbr-db-hist = sort-clmnbr-db-hist then do:
      assign
         sort-labelbr-db-hist = ""
         sort-clmnbr-db-hist = ?
      .
     end.
     else do:
       assign
         sort-labelbr-db-hist = cur-clmnbr-db-hist:label
         sort-clmnbr-db-hist  = cur-clmnbr-db-hist
         sort-clmnbr-db-hist:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-db-hist = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-db-hist:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-db-hist then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-db-hist = cur-clmn-locbr-db-hist + 1
    .
  end.
  case sort-labelbr-db-hist:
        when X_c-db.corr-user-db-num:label in browse br-db-hist then DO:    assign       v-sort-column-name = "X_c-db.corr-user-db-num"     .     run OpenBR in this-procedure ( input yes, input no, input no ) .   . END.
        when X_c-db.corr-date:label in browse br-db-hist then DO:    assign       v-sort-column-name = "X_c-db.corr-date"     .     run OpenBR in this-procedure ( input yes, input no, input no ) .   . END.
    otherwise do:
      assign
        v-sort-column-name = ""
      .
      run OpenBR in this-procedure ( input yes, input no, input no ) .
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-db-hist') then do:
          run mv-brw-defaultbr-db-hist.
        end.
      if sort-labelbr-db-hist <> "" then do:
        assign
          cur-clmnbr-db-hist:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-db-hist = ?
      .
    end.
  end case.
    if cur-clmn-locbr-db-hist <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-db-hist') then do:
        run ch-clmnbr-db-hist in this-procedure (cur-clmn-locbr-db-hist).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-db-hist to recid p-recid no-error.
    apply "value-changed" to br-db-hist in frame d-db-hist.
  end.
  apply "entry" to br-db-hist in frame d-db-hist.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-db-hist:
if cur-clmnbr-db-hist = ? then do:
   run OpenBR in this-procedure ( input yes, input no, input no ) .
end.
else do:
   assign re-querybr-db-hist = yes.
   run sort-brbr-db-hist
     (input (if available X_c-db
             then recid(X_c-db)
             else ?
...
define input parameter p-filter-name as character no-undo .
  do with frame d-db-hist:
    if p-filter-name > "" then do:
      assign
        frame d-db-hist:title
          = frame d-db-hist:title + "   ФИЛЬТР: " + p-filter-name.
      .
      assign
...
:
  ...
  .
  run OpenBR in this-procedure ( input yes, input no, input no) .
  wait-for go of frame d-db-hist.
end.
RUN disable_UI.
...
PROCEDURE OpenBR :
  ...
  if p-db-num <> ? then do:
    assign
      v-filter-point = v-filter-point0 + "one-db":U + chr(4) + v-filter-pointr
      frame d-db-hist:title = substitute( "История изменения БД &1", p-db-num ) + chr(32)
    .
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-16 as character no-undo .
      ...
      parameter-4-16 =
        (
          if ("X_c-db.db-num = p-db-num" + " " + where-phrase-16) <> ""
          then substitute(' X_c-db.db-num = &1 ', p-db-num  )  + " " + where-phrase-16
          else "true"
        )
      parameter-5-16 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-16 =
          ("X_c-db.db-num = p-db-num" + " " + where-phrase-16 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-db-hist :handle
  ...
  if l-filter-open-16 = false then do:
    OPEN QUERY br-db-hist FOR EACH X_c-db
      where X_c-db.db-num = p-db-num
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  on error undo, leave
  :
    if query br-db-hist :handle:get-buffer-handle(1) = (buffer X_c-db:handle) then do:
      assign
      parameter-2-16 = (if p-find-next then "true":u else "false":u )
      parameter-4-16 =
        "where ":u + substitute(' X_c-db.db-num = &1 ', p-db-num  )  + " ":u + where-phrase-16 + " ":u + p-find-condition + " " + ""
      parameter-5-16 = "  "
    .
      ...
      .
      assign
        v-hist-rec = integer(return-value)
        v-fltopend-rowid[1] = ?
      .
    end.
    else do:
      assign
      parameter-2-16 = (if p-find-next then "true":u else "false":u )
      parameter-3-16 =  "FOR EACH X_c-db"
      parameter-4-16 =
        (
          if ("X_c-db.db-num = p-db-num" + " " + where-phrase-16) <> ""
          then substitute(' X_c-db.db-num = &1 ', p-db-num  )  + " " + where-phrase-16
  ...
  run waitfram-hide in this-procedure .
    ...
    assign
      v-filter-point = v-filter-point0 + "all":U + chr(4) + v-filter-pointr
      frame d-db-hist:title = "История изменения баз данных" + chr(32)
    .
...
if p-open-query then do:
  ...
  :
    ...
    .
      ...
      assign
      parameter-2-18 = (if p-find-next then "true":u else "false":u )
      parameter-3-18 =  "FOR EACH X_c-db"
      parameter-4-18 =
        (
          if ("true" + " " + where-phrase-18) <> ""
          then "true" + " " + where-phrase-18
...
FUNCTION get-action RETURNS CHARACTER
  ...
  define variable dops as character no-undo.
    assign
    dops = entry (lookup (trim(string(p-action)), '99,1,2,3,4,9,51,79':U), 'Удаление,Создание,Изменение,Коррекция,Восстановление,Смена_кода,Смена_артик,Выключ.':U)
    no-error
  .
  return dops.
END FUNCTION.
