...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cd-type-IBS-TH-MOB':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  CASE thbjattr_thbj-attr.upper-prop-code :
    WHEN 'ibs-th-mob_main':U THEN DO:
      CASE v-entry:
       WHEN 'salesman-mandatory':U THEN DO:
          ...
          .
       END.
       WHEN 'pos-type-for-discnt':U THEN DO:
          ASSIGN
          cb-pos-type-for-discnt = thbjattr_thbj-attr.property-value-character
          cb-pos-type-for-discnt:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
          .
       END.
      END CASE.
    END.
    when 'ibs-th-mob_rec-print':U THEN DO:
      case v-entry:
...
cb-pos-type-for-discnt:LIST-ITEMS = 'IBS-TH':U + chr(44) + 'IBS-TH-MOB':U
...
cb-pos-type-for-discnt WHEN p-mode = 'ИЗМЕНЕНИЕ':U
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
...
by thbjattr_thbj-attr.upper-prop-code :
   ...
   if v-loc-same = no then do:
     v-same = v-loc-same.
     if l-write-cd = yes
     and thbjattr_thbj-attr.upper-prop-code <> 'cd-type-IBS-TH-MOB':U
     then do:
...
case p-obj-type:
  when '':U then do:
    ...
    for each buf_Cash-desk no-lock where
            buf_cash-desk.pos-type = 'IBS-TH-MOB':U
