...
entry (lookup (X_layout-elem.layout-type, 'th-pos-screen,th-pos-keyboard':U) + 1, ',' + 'Экран IBS TH POS,Клавиатура IBS TH POS':U) COLUMN-LABEL "Тип раскладки" FORMAT "X(255)" WIDTH 20
X_layout-elem.device-type COLUMN-LABEL "Для устройства" FORMAT "X(15)"
get-cdm-name(X_layout-elem.mode-id) COLUMN-LABEL "Режим" FORMAT "X(255)" width 25
X_layout-elem.widget-id COLUMN-LABEL "Элемент"
X_layout-elem.DES COLUMN-LABEL "Описание" format "X(255)" width 35
entry (lookup (STRING(X_layout-elem.elem-type), '0,-1':U) + 1, ',' + 'Программируемый,Непрограммируемый':U) COLUMN-LABEL "Тип" FORMAT "X(15)"
...
DO:
 define variable v-rec as recid no-undo.
  run adm/layoutei.w ( input parparentproc
                       ,input 'ДОБАВЛЕНИЕ':U + chr(44) + (if v-admin then "admin" else '')
                       ,input cb-layout-type
                       ,input cb-device-type
                       ,input cb-mode-id
                       ,input ''
...
DO:
  ...
  run adm/layoutei.w ( input parparentproc
                       ,input 'ИЗМЕНЕНИЕ':U + chr(44) + (if v-admin then "admin" else '')
...
DO:
if not available X_layout-elem then return no-apply.
run proc-b-lkp IN THIS-PROCEDURE ( INPUT 'layout-elem':U) NO-ERROR.
...
assign
  v-str-recid6 = trim( string( recid( X_layout-elem ) , "->>>>>>>>>>>9":U ) )
  v-num-entry6 = lookup( v-str-recid6 , v-rid-list )
.
if v-num-entry6 > 0 then do:
  assign
    entry( v-num-entry6, v-rid-list ) = "":U
    v-rid-list = trim( replace( v-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rid-list = v-rid-list + ( if v-rid-list = "":U then "":U else chr(44) ) + v-str-recid6
  .
end.
  glog = br-layout-elem:refresh() .
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      glog = br-layout-elem:select-next-row ().
      apply "VALUE-CHANGED" to br-layout-elem in frame Dialog-Frame.
  end.
...
apply "entry" to br-layout-elem in frame Dialog-Frame.
END.
...
END.
ON VALUE-CHANGED OF CB-device-type IN FRAME Dialog-Frame
DO:
    assign
  cb-device-type.
...
DO:
  ...
  run openbr in this-procedure no-error.
END.
ON VALUE-CHANGED OF CB-layout-type IN FRAME Dialog-Frame
DO:
  assign
  cb-layout-type.
  case cb-layout-type:
    when '' then do:
      ASSIGN
     cb-device-type:LIST-ITEMS IN FRAME Dialog-Frame = chr(44) +
                                                             v-keyboard-list-items + chr(44) +
                                                             v-screen-list-items
                                                        .
    end.
...
do:
   v-doc-rec = recid(X_layout-elem).    RUn OpenBR in this-procedure.  REPOSITION br-layout-elem to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-layout-elem.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-layout-elem :handle
      .
...
define variable vss-include-info11 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if (  p-list-mode = "layout-device-type"
        or
        p-list-mode = "layout-device-type-mode")
  and lookup(p-device-type, 'th-pos-screen,th-pos-keyboard':U) = 0 then do:
    message
...
END.
ASSIGN
cb-layout-type:LIST-ITEM-PAIRS IN FRAME Dialog-Frame = v-list-items .
v-list-items = chr(44).
DO v-ii = 1 TO NUM-ENTRIES('0,-1':U):
   v-list-items = v-list-items  + chr(44) +
                 ENTRY(v-ii, 'Программируемый,Непрограммируемый':U) + chr(44) +
                  ENTRY(v-ii, '0,-1':U).
END.
ASSIGN
cb-elem-type:LIST-ITEM-PAIRS IN FRAME Dialog-Frame = v-list-items .
v-keyboard-list-items = 'IBM-50':U.
v-SCREEN-list-items = 'Screen,TouchScreen':U.
ASSIGN
v-h = br-layout-elem:FIRST-COLUMN in frame Dialog-Frame
.
DO while valid-handle(v-h) :
  if v-h:LABEL = "Тип раскладки" then do:
    vh-layout-type-name = v-h.
  end.
  if v-h:LABEL = "Режим" then do:
    vh-cdm-name = v-h.
  end.
  v-h = v-h:NEXT-COLUMN.
END.
...
br-layout-elem
cb-layout-type when p-list-mode = 'все':U
cb-device-type when (p-list-mode = 'все':U or p-list-mode = "layout-type")
cb-mode-id when (p-list-mode = 'все':U or p-list-mode = "layout-type" or p-list-mode = "layout-device-type")
...
CASE v-list-mode:
   when 'все':U then do:
      frame Dialog-Frame :title = "Все элементы раскладки".
    assign
   ...
   when "layout-type" then do:
      frame Dialog-Frame :title = substitute("Элементы раскладок типа &1", p-layout-type).
      assign
   ...
   when "layout-device-type" then do:
      frame Dialog-Frame :title = substitute("Элементы для раскладок типа &1  для &2", p-layout-type, p-device-type).
      assign
      vh-layout-type-name:visible  = no
   ...
   when "layout-device-type-mode" then do:
      frame Dialog-Frame :title = substitute("Элементы раскладок типа &1 для &2 в режиме", p-layout-type, p-device-type, p-mode-id).
     assign
     vh-layout-type-name:visible  = no
     X_layout-elem.device-type:visible in browse br-layout-elem = no
...
define variable v-rec as recid no-undo.
CASE p-option:
  WHEN 'layout-elem':U THEN DO:
      run adm/layoutei.w ( input parparentproc
                           ,input 'ПРОСМОТР':U + chr(44) + (if v-admin then "admin" else '')
                           ,input X_layout-elem.layout-type
                           ,input X_layout-elem.device-type
                           ,input X_layout-elem.mode-id
                           ,input X_layout-elem.widget-id
