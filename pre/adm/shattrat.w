...
ON WINDOW-CLOSE OF FRAME shattrat
DO:
  APPLY "END-ERROR":U TO SELF.
END.
ON CHOOSE OF B-exit IN FRAME shattrat
DO:
  RUN proc-save IN THIS-PROCEDURE NO-ERROR.
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME shattrat:PARENT eq ?
THEN FRAME shattrat:PARENT = ACTIVE-WINDOW.
define variable vss-include-info6 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame shattrat
do:
  run gbl/app_help.p
...
on choose of b-help in frame shattrat
do:
  apply "help":u to frame shattrat .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame shattrat:width - 0.3
                fh            = frame shattrat:first-child
                hh            = fh:first-child
                ii            = 1
                .
  ...
  if p-mode = 'ИЗМЕНЕНИЕ':U then do:
    find first locked_thbj-attr exclusive-lock
      where locked_thbj-attr.obj-type = p-obj-type
        ...
        and locked_thbj-attr.upper-prop-code = 'auto-task':U
  ...
  if p-mode = 'ПРОСМОТР':U then do:
    ...
    enable
     ...
     assign
       b-exit:label = "Вы&ход"
     .
     hide b-quit in frame shattrat .
  end.
  if p-obj-type <> '':U then
  do:
    hide maxColMarks  in frame shattrat .
  end.
  WAIT-FOR GO OF FRAME shattrat.
END.
RUN disable_UI.
...
:
  ...
  for each thbjattr_thbj-attr:
    ...
    .
    case v-entry:
      when 'send-msg-to-email':U then do:
        assign
          f-email-list  = thbjattr_thbj-attr.property-value-character
          f-email-list  :private-data in frame shattrat = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'user-list':U then do:
        assign
          f-user-list  = thbjattr_thbj-attr.property-value-character
          f-user-list  :private-data in frame shattrat = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'maxColMarks':U then do:
        assign
          maxColMarks  = thbjattr_thbj-attr.property-value-integer
          maxColMarks  :private-data in frame shattrat = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
    end case.
...
:
  ...
  assign
    frame shattrat f-email-list
    frame shattrat f-user-list
    fh = frame shattrat:first-child
    wh = fh:first-child
  .
