...
END.
ON VALUE-CHANGED OF tt-layout-elem.layout-type IN FRAME Dialog-Frame
DO:
  assign
  ...
  CASE tt-layout-elem.layout-type:
    WHEN 'th-pos-keyboard':U  THEN DO:
        ASSIGN
        tt-layout-elem.device-type:LIST-ITEMS = 'IBM-50':U.
    END.
    WHEN 'th-pos-screen':U  THEN DO:
        ASSIGN
        tt-layout-elem.device-type:LIST-ITEMS = 'Screen,TouchScreen':U.
    END.
  END CASE.
END.
...
DO v-ii = 1 TO NUM-ENTRIES('0,-1':U):
   v-list-items = v-list-items  + (IF v-ii = 1 THEN '' ELSE chr(44)) +
                  ...
                  ENTRY(v-ii, '0,-1':U).
END.
tt-layout-elem.elem-type:RADIO-BUTTONS IN FRAME Dialog-Frame = V-LIST-ITEMS.
v-list-items = ''.
...
END.
ASSIGN
tt-layout-elem.layout-type:LIST-ITEM-PAIRS IN FRAME Dialog-Frame = v-list-items .
v-list-items = chr(44).
assign
tt-layout-elem.mode-id:list-item-pairs in frame Dialog-Frame = v-list-items.
...
b-quit
B-Help
tt-layout-elem.layout-type WHEN p-mode = 'ДОБАВЛЕНИЕ':U
tt-layout-elem.device-type WHEN p-mode = 'ДОБАВЛЕНИЕ':U
tt-layout-elem.mode-id WHEN p-mode = 'ДОБАВЛЕНИЕ':U
tt-layout-elem.widget-id WHEN p-mode = 'ДОБАВЛЕНИЕ':U
tt-layout-elem.elem-type WHEN p-mode <> 'ПРОСМОТР':U
tt-layout-elem.des
WITH FRAME Dialog-Frame .
if p-mode = 'ПРОСМОТР':U then do:
  assign
