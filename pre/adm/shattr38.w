...
:
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'dc-ref':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = 'l-zeros':U THEN DO:
    ASSIGN
    t-l-zeros = thbjattr_thbj-attr.property-value-logical
    t-l-zeros:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    fixed-l-zeros = t-l-zeros
    .
  END.
  IF v-entry = 'l-mask':U THEN DO:
    ASSIGN
    t-l-mask = thbjattr_thbj-attr.property-value-logical
    t-l-mask:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
...
WITH FRAME Dialog-Frame.
enable
t-l-mask WHEN p-mode = 'ИЗМЕНЕНИЕ':U
WITH FRAME Dialog-Frame.
...
WITH FRAME Dialog-Frame.
enable
t-l-zeros WHEN p-mode = 'ИЗМЕНЕНИЕ':U
WITH FRAME Dialog-Frame.
end.
VIEW FRAME Dialog-Frame.
