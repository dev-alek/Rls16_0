...
def var vss-workfile    as character no-undo init "$Workfile: l-i-async.w $":U .
def var vss-archive     as character no-undo init "$Archive: adm/l-i-async.w $":U .
def var vss-description as character no-undo init "Окно входа в СПН".
...
define new shared variable nws-exch-dir as character no-undo .
define new shared variable nws-heap-dir as character no-undo .
...
define variable mNumSession     as integer   no-undo.
  ...
  do ind = 1 to v-num-entries :
    ...
    if num-entries( v-param, ":":U ) > 1 then do:
      ...
      case v-param-name :
        ...
        when "M":U then do:
          assign
            v-mode = replace( v-param-value, chr(4), ",":U )
          .
        end.
        when "T":U then do:
          assign
            mType = replace( v-param-value, "+", ",":U )
          .
        end.
        when "T":U then do:
          assign
            mNumSession = int( v-param-value)
          .
        end.
        when "A":U then .
        otherwise do:
...
:
  ...
  if error-status :error then do:
    message
      vss-workfile vss-revision vss-description skip
      "Ошибка инициализации переменных для системы передачи новостей" skip
      return-value
      view-as alert-box error.
    leave do1.
  end.
