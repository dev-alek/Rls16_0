...
END.
ON VALUE-CHANGED OF t-noanshftstaff IN FRAME Dialog-Frame
DO:
    ASSIGN
  t-noanshftstaff.
END.
ON VALUE-CHANGED OF t-obyznumbukv IN FRAME Dialog-Frame
DO:
    ASSIGN
  t-obyznumbukv.
END.
ON leave OF t-TimeAvail IN FRAME Dialog-Frame
DO:
   if integer(t-TimeAvail:screen-value in frame Dialog-Frame) > 60 then do:
  ...
  t-TimeAvail.
END.
ON leave OF t-TimeBlock IN FRAME Dialog-Frame
DO:
   if integer(t-TimeBlock:screen-value in frame Dialog-Frame) > 120 then do:
      message "Время до блокировки пользователя после окончания действия пароля не должно превышать 120 дней"
      view-as alert-box.
      return no-apply .
   end.
   if integer(t-TimeBlock:screen-value in frame Dialog-Frame) < 30 and integer(t-TimeBlock:screen-value in frame Dialog-Frame) <> 0 then do:
  ...
  t-TimeBlock.
END.
ON leave OF t-TimeBlockAdm IN FRAME Dialog-Frame
DO:
   if integer(t-TimeBlockAdm:screen-value in frame Dialog-Frame) > 120 then do:
      message "Время до блокировки пользователя после окончания действия пароля не должно превышать 120 дней"
      view-as alert-box.
      return no-apply .
   end.
   if integer(t-TimeBlockAdm:screen-value in frame Dialog-Frame) < 30 and integer(t-TimeBlockAdm:screen-value in frame Dialog-Frame) <> 0 then do:
      message "Время до блокировки пользователя после окончания действия пароля должно быть больше 30 дней"
      view-as alert-box.
      return no-apply .
   end.
  ...
  t-TimeBlockAdm.
END.
ON leave OF t-TimeAvailAdm IN FRAME Dialog-Frame
DO:
   if integer(t-TimeAvailAdm:screen-value in frame Dialog-Frame) > 60 then do:
      message "Время жизни пароля не должно превышать 60 дней"
      view-as alert-box.
      return no-apply .
   end.
  ...
  t-TimeAvailAdm.
END.
ON VALUE-CHANGED OF t-obyznumbukvAdm IN FRAME Dialog-Frame
DO:
    ASSIGN
  t-obyznumbukv.
END.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'staff':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  v-entry = thbjattr_thbj-attr.prop-code.
    IF v-entry = "noanshftstaff" THEN DO:     assign     t-noanshftstaff = thbjattr_thbj-attr.property-value-logical     t-noanshftstaff:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "obyznumbukv" THEN DO:     assign     t-obyznumbukv = thbjattr_thbj-attr.property-value-logical     t-obyznumbukv:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "obyznumbukvAdm" THEN DO:     assign     t-obyznumbukvAdm = thbjattr_thbj-attr.property-value-logical     t-obyznumbukvAdm:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
      IF v-entry = "minparol" THEN DO:     assign     t-minparol = thbjattr_thbj-attr.property-value-integer     t-minparol:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "minparolAdm" THEN DO:     assign     t-minparolAdm = thbjattr_thbj-attr.property-value-integer     t-minparolAdm:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "TimeAvail" THEN DO:     assign     t-TimeAvail = thbjattr_thbj-attr.property-value-integer     t-TimeAvail:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "TimeAvailAdm" THEN DO:     assign     t-TimeAvailAdm = thbjattr_thbj-attr.property-value-integer     t-TimeAvailAdm:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "TimeBlock" THEN DO:     assign     t-TimeBlock = thbjattr_thbj-attr.property-value-integer     t-TimeBlock:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "TimeBlockAdm" THEN DO:     assign     t-TimeBlockAdm = thbjattr_thbj-attr.property-value-integer     t-TimeBlockAdm:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "LastPaswd" THEN DO:     assign     t-LastPaswd = thbjattr_thbj-attr.property-value-integer     t-LastPaswd:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
    IF v-entry = "LastPaswdAdm" THEN DO:     assign     t-LastPaswdAdm = thbjattr_thbj-attr.property-value-integer     t-LastPaswdAdm:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))     .   END.
...
.
    IF p-obj-type = '' and p-obj-code = 0 THEN DO:
       ...
       VsuperAdm = available user-account-attr and logical(user-account-attr.attr-value) eq yes no-error.
        ...
        b-quit
        B-Help
        t-noanshftstaff    WHEN p-mode = 'ИЗМЕНЕНИЕ':U
        t-obyznumbukv      WHEN p-mode = 'ИЗМЕНЕНИЕ':U
        t-minparol         WHEN p-mode = 'ИЗМЕНЕНИЕ':U
        t-TimeAvail        WHEN p-mode = 'ИЗМЕНЕНИЕ':U
        t-TimeBlock        WHEN p-mode = 'ИЗМЕНЕНИЕ':U
        t-LastPaswd        WHEN p-mode = 'ИЗМЕНЕНИЕ':U
        t-obyznumbukvAdm   WHEN p-mode = 'ИЗМЕНЕНИЕ':U and VsuperAdm
        t-minparolAdm      WHEN p-mode = 'ИЗМЕНЕНИЕ':U and VsuperAdm
        t-TimeAvailAdm     WHEN p-mode = 'ИЗМЕНЕНИЕ':U and VsuperAdm
        t-TimeBlockAdm     WHEN p-mode = 'ИЗМЕНЕНИЕ':U and VsuperAdm
        t-LastPaswdAdm     WHEN p-mode = 'ИЗМЕНЕНИЕ':U and VsuperAdm
        WITH FRAME Dialog-Frame.
    END.
    ELSE DO:
        DISPLAY
