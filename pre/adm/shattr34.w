...
END.
ON VALUE-CHANGED OF cb-bgeflold IN FRAME Dialog-Frame
DO:
  run check-bgeflold in this-procedure .
END.
ON VALUE-CHANGED OF cb-bgefmt IN FRAME Dialog-Frame
DO:
  run check-bgefmt in this-procedure .
END.
ON VALUE-CHANGED OF cb-bgeshift IN FRAME Dialog-Frame
DO:
  run check-bgeshift in this-procedure .
END.
ON LEAVE OF fi-bgeflnm IN FRAME Dialog-Frame
DO:
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-tt-clients-iv :handle
      .
  ...
  if p-mode = 'ИЗМЕНЕНИЕ':U then do:
    find first locked_thbj-attr exclusive-lock
      where locked_thbj-attr.obj-type = p-obj-type
        ...
        and locked_thbj-attr.upper-prop-code = 'bge-export':U
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-tt-clients-iv FOR EACH tt-clients-iv NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
...
:
  ...
  for each thbjattr_thbj-attr:
    ...
    .
    case v-entry:
      when 'bgeclall':U then do:
        assign
          tg-bgeclall = thbjattr_thbj-attr.property-value-logical
          tg-bgeclall :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'bgedcard':U then do:
        assign
          tg-bgedcard = thbjattr_thbj-attr.property-value-logical
          tg-bgedcard :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'bgedict':U then do:
        assign
          tg-bgedict = thbjattr_thbj-attr.property-value-logical
          tg-bgedict :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'bgeflnm':U then do:
        assign
          fi-bgeflnm = thbjattr_thbj-attr.property-value-character
          fi-bgeflnm :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'bgeflold':U then do:
        assign
          cb-bgeflold = thbjattr_thbj-attr.property-value-character
          cb-bgeflold :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'bgefmt':U then do:
        assign
          cb-bgefmt = thbjattr_thbj-attr.property-value-character
          cb-bgefmt :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'bgeshift':U then do:
        assign
          cb-bgeshift = thbjattr_thbj-attr.property-value-character
          cb-bgeshift :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'bgecliiv':U then do:
        assign
...
PROCEDURE my-enable :
  ...
  then do:
    assign
      fi-bgeflnm  :fgcolor in frame Dialog-Frame = BROWN_COLOR
      b-exit :hidden  = yes
  ...
  enable
    ...
    br-tt-clients-iv
    tg-bgeclall when p-mode = 'ИЗМЕНЕНИЕ':U
    tg-bgedcard when p-mode = 'ИЗМЕНЕНИЕ':U
    tg-bgedict  when p-mode = 'ИЗМЕНЕНИЕ':U
    fi-bgeflnm  when p-mode = 'ИЗМЕНЕНИЕ':U
    cb-bgeflold when p-mode = 'ИЗМЕНЕНИЕ':U
    cb-bgefmt   when p-mode = 'ИЗМЕНЕНИЕ':U
    cb-bgeshift when p-mode = 'ИЗМЕНЕНИЕ':U
  with frame Dialog-Frame.
  view frame Dialog-Frame.
END PROCEDURE.
PROCEDURE open-br :
...
:
  ...
  then do:
    ...
    .
     br-tt-clients-iv:refresh() in frame Dialog-Frame.
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      br-tt-clients-iv:select-next-row ().
      apply "VALUE-CHANGED" to br-tt-clients-iv in frame Dialog-Frame.
    end.
  end.
end.
END PROCEDURE.
...
:
  ...
  find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'bgecliiv':U.
  assign
