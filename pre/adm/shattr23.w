...
:
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cli-all':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = 'nocorinn':U THEN DO:
    ASSIGN
    t-nocorinn = thbjattr_thbj-attr.property-value-logical
    t-nocorinn:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'inn-uniq':U THEN DO:
    ASSIGN
    rs-inn-uniq = thbjattr_thbj-attr.property-value-integer
    rs-inn-uniq:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
...
PROCEDURE MyEnable :
ASSIGN
l-inn-uniq = "Запрет ввода клиента с повторным ИНН"
t-nocorinn:LABEL IN FRAME Dialog-Frame = "Разрешено добавление некорректного ИНН после подтверждения оператора"
rs-inn-uniq:RADIO-BUTTONS IN FRAME Dialog-Frame =
"Проверка ввода повторного ИНН для фирмы и запрос подтверждения оператора" + chr(44) + STRING(0) + chr(44) +
"Проверка ввода повторного ИНН для фирмы и физ.лица и запрос подтверждения оператора" + chr(44) + STRING(1) + chr(44) +
"Запрет ввода повторного ИНН для фирмы и физ.лица; возникновение коллизии в СПН" + chr(44) + STRING(2) + chr(44) +
"НЕ ПРОВЕРЯТЬ ввод повторного ИНН" + chr(44) + STRING(99)
...
b-quit
B-Help
t-nocorinn WHEN p-mode = 'ИЗМЕНЕНИЕ':U
rs-inn-uniq WHEN p-mode = 'ИЗМЕНЕНИЕ':U
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
