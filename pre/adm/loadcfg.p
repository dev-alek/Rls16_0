...
define input parameter  cnf-fname        as character no-undo .
define input parameter  cnf-struct-fname as character no-undo .
define input parameter  log-fname        as character no-undo .
define input parameter  app-parameter    as logical   no-undo .
define input parameter  app-message      as logical   no-undo .
define input parameter  chg-encode       as logical   no-undo .
define input parameter  err-block        as integer   no-undo .
...
:
  ...
  define variable dbCnf-hdl    as handle    no-undo .
  ...
  if not valid-handle (cnf-hdl) then do:
    do: result = 3 . run go-out in this-procedure . return. end.
  end.
  if log-fname = "" then do:
    assign
      log-fname = "err-conf.txt"
    .
  end.
  run init in cnf-hdl
    (input  log-fname
  ...
  if cnf-struct-fname = "":U then do:
    assign
      cnf-struct-fname = search( "cmp/mold_db.sch" )
    .
