...
for each unld_db by unld_db.db-num :
  ...
  run adm/rest-rdb.p
    ( input unld_db.db-num
     ...
     ,input ('unload-online':U + chr(4) + "yes")
  ...
  then do:
    assign
      v-err = error-status :get-message( error-status :num-messages ) + chr(10) + return-value
    .
    do transaction
    on error undo, return error
    :
