...
ON CHOOSE OF b-attr IN FRAME d-store
DO:
...
or attr-option = 'КОПИРОВАНИЕ':U then do:
...
attr-option = "":u.
END.
ON CHOOSE OF b-cli-attr IN FRAME d-store
DO:
...
DO:
  ...
  APPLY "CHOOSE" to b-attr  in frame d-store.
END.
...
DO:
  ...
  APPLY "CHOOSE" to b-cli-attr  in frame d-store.
END.
...
DO:
  ...
  run enable_UI.
END.
ON CHOOSE OF b-add IN FRAME d-store
DO:
  define variable ri as recid no-undo.
  define buffer buf_store for ub.store.
  define buffer buf_clients for ub.clients.
  ...
  return no-apply.
END.
ON CHOOSE OF b-chg IN FRAME d-store
DO:
  define variable ri as recid no-undo.
...
END.
ON CHOOSE OF b-del IN FRAME d-store
DO:
  define variable ri as recid no-undo.
  if available X_store then do:
    ri = recid(X_store).
...
END.
ON CHOOSE OF b-lkp IN FRAME d-store
DO:
  define variable ri as recid no-undo.
  if available X_store then do:
      ri = recid (X_clients).
...
END.
ON CHOOSE OF b-price IN FRAME d-store
DO:
  if not available X_store then return no-apply.
...
END.
ON CHOOSE OF b-quit IN FRAME d-store
DO:
END.
define variable vss-include-info11 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame d-store
do:
  run gbl/app_help.p
...
on choose of b-help in frame d-store
do:
  apply "help":u to frame d-store .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame d-store:width - 0.3
                fh            = frame d-store:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame d-store :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame d-store :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame d-store :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-store :height = v-frame-height
          .
          if frame d-store :scrollable = true
          then do:
            assign
              frame d-store :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame d-store :height
      v-frame-virtual-height = frame d-store :virtual-height
    ...
    assign
      v-field-group-handle = frame d-store :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame d-store :virtual-height = frame d-store :virtual-height + p-change-value
          no-error .
        if error-status :error
      ...
      assign
        frame d-store :height = frame d-store :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame d-store :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame d-store :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame d-store :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame d-store :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame d-store :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-store :width = v-frame-width
          .
          if frame d-store :scrollable = true
          then do:
            assign
              frame d-store :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame d-store :width
      v-frame-virtual-width = frame d-store :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame d-store :virtual-width = frame d-store :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame d-store :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame d-store :width = frame d-store :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame d-store :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame d-store :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame d-store :height - v-diasize-resize-button :height
                  - 1
                  - (frame d-store :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame d-store :width - v-diasize-resize-button :width
                  - 1
                  - (frame d-store :border-right-pixels / session :pixels-per-column)
    .
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame d-store :height
      v-col-delta = v-new-col - frame d-store :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame d-store :width
      v-diasize-current-frame-height = frame d-store :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame d-store :height
        v-diasize-orig-frame-width  = frame d-store :width
        v-diasize-browse-handle     = browse br-stores :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame d-store :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
ON CHOOSE OF B-hist IN FRAME d-store
DO:
...
END.
ON CHOOSE OF b-print IN FRAME d-store
DO:
  run rep/stor-prt.p (input parparentproc) .
END.
ON CHOOSE OF b-sel IN FRAME d-store
DO:
...
END.
ON CHOOSE OF b-right IN FRAME d-store
DO:
...
END.
on return, MOUSE-SELECT-DBLCLICK of br-stores in frame d-store do:
  if b-sel:sensitive then
    if b-mark:sensitive then apply "choose" to b-mark in frame d-store.
    else apply "choose" to b-sel in frame d-store.
  else if b-chg:sensitive then apply "choose" to b-chg in frame d-store.
end.
on choose of b-mark in frame d-store do:
...
assign
  v-str-recid15 = trim( string( recid( X_store ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry15 > 0 then do:
  ...
  glog = br-stores:refresh() in frame d-store.
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
    glog = br-stores:select-next-row ().
    apply "iteration-changed" to br-stores in frame d-store.
  end.
  if num-entries (v-rid-list) = 0 then hide mark-num in frame d-store.
  else disp num-entries (v-rid-list) @ mark-num with frame d-store.
  apply "entry" to br-stores in frame d-store.
end.
ON RETURN OF sch-code IN FRAME d-store
DO:
define buffer buf_store for ub.store.
assign
sch-code.
...
END.
ON CHOOSE OF B-grp IN FRAME d-store
DO:
...
if g-grp = "" then  do:
  ...
  return no-apply.
    ...
    v-rid-list = "".
    mark-num = 0.
    hide mark-num in frame d-store.
end.
run Openbr in this-procedure .
reposition br-stores to recid ri no-error.
apply "ENTRY" to br-stores.
apply "value-changed" to br-stores.
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME d-store:PARENT eq ?
THEN FRAME d-store:PARENT = ACTIVE-WINDOW.
ON WINDOW-CLOSE OF FRAME d-store APPLY "END-ERROR":U TO SELF.
define variable vss-include-info16 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on INS of frame d-store anywhere do:
  if b-mark :sensitive then DO: apply "CHOOSE":U to b-mark in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info17 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F3 of frame d-store anywhere do:
  if b-lkp :sensitive then DO: apply "CHOOSE":U to b-lkp in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info18 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on CTRL-N, CTRL-Т of frame d-store anywhere do:
  if b-add :sensitive then DO: apply "CHOOSE":U to b-add in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info19 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F4 of frame d-store anywhere do:
  if b-chg :sensitive then DO: apply "CHOOSE":U to b-chg in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info20 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F8 of frame d-store anywhere do:
  if b-del :sensitive then DO: apply "CHOOSE":U to b-del in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info21 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame d-store anywhere do:
  if b-sel :sensitive then DO: apply "CHOOSE":U to b-sel in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info22 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame d-store anywhere do:
  if b-quit :sensitive then DO: apply "CHOOSE":U to b-quit in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info23 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on CTRL-P, CTRL-З of frame d-store anywhere do:
  if b-print :sensitive then DO: apply "CHOOSE":U to b-print in frame d-store. END.
  return no-apply.
end.
define variable vss-include-info24 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame d-store anywhere
do:
   v-doc-rec = recid(X_store).    run OpenBR in this-procedure.   REPOSITION br-stores to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-stores.
end.
define variable vss-include-info25 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-stores :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
assign
B-attr:POPUP-MENU IN FRAME d-store       = MENU MENU-B-attr:HANDLE
b-attr:MENU-MOUSE in frame d-store = 1
B-cli-attr:POPUP-MENU IN FRAME d-store       = MENU MENU-B-cli-attr:HANDLE
b-cli-attr:MENU-MOUSE in frame d-store = 1
...
if p-only-cur-db-num  = yes then do:
  ...
  EACH X_clients WHERE
       X_clients.obj-code = X_store.obj-code
   and X_clients.obj-type = 'скл':U NO-LOCK,
   each X_cli-host where
        X_cli-host.obj-code = X_store.host-code
   and X_cli-host.obj-type = 'орг':U no-lock BY X_store.obj-code.
end.
