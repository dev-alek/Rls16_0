...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cd-type-omron':U
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = 'omrbase':U THEN DO:
    ASSIGN
    f-omrbase = thbjattr_thbj-attr.property-value-integer
    f-omrbase:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'omrnal':U THEN DO:
    ASSIGN
    f-omrnal = thbjattr_thbj-attr.property-value-integer
    f-omrnal:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'omrntnl':U THEN DO:
    ASSIGN
    f-omrntnl = thbjattr_thbj-attr.property-value-integer
    f-omrntnl:private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
    .
  END.
  IF v-entry = 'omrcurl':U THEN DO:
    ASSIGN
    v-omrcurl = thbjattr_thbj-attr.property-value-character.
  END.
  IF v-entry = 'omrpayl':U THEN DO:
    ASSIGN
...
f-omrbase WHEN p-mode = 'ИЗМЕНЕНИЕ':U
...
FOR EACH tt-cash-pay-omr BY tt-cash-pay-omr.omr-pay-code:
  ASSIGN
  v-omrcurl = v-omrcurl + (IF v-omrcurl = "":U THEN "":U ELSE chr(44)) + STRING(tt-cash-pay-omr.omr-pay-code)
  v-omrpayl = v-omrpayl + (IF v-omrpayl = "":U THEN "":U ELSE chr(44)) + STRING(tt-cash-pay-omr.cdpay-code)
  .
END.
assign
fh = frame Dialog-Frame:first-child
...
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'omrcurl':U.
assign
thbjattr_thbj-attr.property-value-character = v-omrcurl.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'omrpayl':U.
assign
