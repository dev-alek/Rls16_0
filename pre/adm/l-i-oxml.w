...
define variable vss-workfile    as character no-undo init "$Workfile: l-i-oxml.w $":U .
define variable vss-archive     as character no-undo init "$Archive: adm/l-i-oxml.w $":U .
...
if v-connection
  ...
  run adm/automain.w ( input 'autooxml':U
                      ,input v-mode
                      ) no-error.
end.
run gbl/dbdiscon.p no-error.
