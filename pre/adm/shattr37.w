...
DO:
  MESSAGE "Help for File: c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\adm\shattr37.w" VIEW-AS ALERT-BOX INFORMATION.
END.
...
procedure minbtn-set :
   ...
   CASE p-obj-type:
      ...
      THEN DO:
         ASSIGN
            FRAME Dialog-Frame:TITLE = FRAME Dialog-Frame:TITLE + SUBSTITUTE(" по объекту &1 &2", p-obj-type, p-obj-code)
         .
      END.
      OTHERWISE DO:
      END.
...
:
   ...
   :
      find first locked_thbj-attr
            where locked_thbj-attr.upper-prop-code = 'fbrattr':U
            exclusive-lock
            no-wait
            no-error
            .
  ...
  for each thbjattr_thbj-attr:
    ...
    .
    case v-entry:
      when 'fbr-frcp':U then do:
        assign
          tb-fbr-frcp = thbjattr_thbj-attr.property-value-logical
          tb-fbr-frcp :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'fbr-ioff':U then do:
        assign
          tb-fbr-ioff = thbjattr_thbj-attr.property-value-logical
          tb-fbr-ioff :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'fbr-qntc':U then do:
        assign
          tb-fbr-qntc = thbjattr_thbj-attr.property-value-logical
          tb-fbr-qntc :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'fbrrcpgb':U then do:
        assign
          tb-fbrrcpgb = thbjattr_thbj-attr.property-value-logical
          tb-fbrrcpgb :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'fbrhstlv':U then do:
        assign
          v-fbrhstlv = thbjattr_thbj-attr.property-value-integer
          v-fbrhstlv :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'fbr-mrgn-min':U then do:
        assign
          v-fbr-mrgn-min = thbjattr_thbj-attr.property-value-decimal
          v-fbr-mrgn-min :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
      when 'fbr-mrgn-max':U then do:
        assign
          v-fbr-mrgn-max = thbjattr_thbj-attr.property-value-decimal
          v-fbr-mrgn-max :private-data in frame Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr))
        .
      end.
    end case.
...
:
   ...
   then do:
    assign
      v-fbr-mrgn-min :fgcolor in frame Dialog-Frame = BROWN_COLOR
      v-fbr-mrgn-max :fgcolor in frame Dialog-Frame = BROWN_COLOR
      v-fbrhstlv     :fgcolor in frame Dialog-Frame = BROWN_COLOR
      b-exit :hidden  = yes
