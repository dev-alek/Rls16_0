...
for each db-rec-attr where ( db-rec-attr.db-num             eq iDb-num
...
no-lock :
   ...
   assign
      v-command = "command":U                                                 + chr(1)
                + "two-commit":U                                              + chr(1)
                + db-rec-attr.attr-code                        + chr(1)
                + db-rec-attr.attr-type                     + chr(1)
                + db-rec-attr.uniq-key-rec               + chr(1)
                + string( db-rec-attr.attr-value-decimal  )   + chr(1)
                + db-rec-attr.attr-value                   + chr(1)
                + string( v-answer-code )                                     + chr(1)
                + v-answer-msg
     .
   ...
   if error-status :error
   then do:
      run write-to-log( substitute( "&1 (two-commit). &2&3&4 переходим к следующей операции", vss-workfile, return-value, chr(10), error-status:get-message( 1 ) ) ).
   end.
end.
