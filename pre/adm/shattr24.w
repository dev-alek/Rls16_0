...
.
release tt-cp-grp.
OPEN QUERY br-cp-grp FOR EACH tt-cp-grp NO-LOCK INDEXED-REPOSITION.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-cp-grp :handle
      .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cashpays':U
...
for each tt-cp-grp:
  assign
  v-cpgrpname = v-cpgrpname + (if v-cpgrpname = '':U then '':U else chr(44)) +
                tt-cp-grp.grp-name  + chr(44) + string(tt-cp-grp.grp-code)
  .
end.
assign
fh = frame Dialog-Frame:first-child
...
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'cpgrpnam':U.
assign
