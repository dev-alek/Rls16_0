...
DO:
   ...
   if v-ref-list <> "" then do:
      ...
      OPEN QUERY BROWSE-2 FOR EACH buf_tt-goods.
   end.
END.
  ...
  for each tt-goods
      ...
      assign
         p-gds-list = IF p-gds-list = "":U THEN STRING(tt-goods.node-code)
                                           ELSE p-gds-list + chr(4) + STRING(tt-goods.node-code)
      .
  end.
  ASSIGN
      p-ok = TRUE
...
DO:
  MESSAGE "Help for File: c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\adm\actngrpg.w" VIEW-AS ALERT-BOX INFORMATION.
END.
...
:
   ...
   ELSE DO:
      ...
      :
         FIND first buf_gds-grp
              where buf_gds-grp.node-code = INTEGER(ENTRY(v-count, p-Gds-List, chr(4)))
              no-lock
              no-error
             .
