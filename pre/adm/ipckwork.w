...
DO ii = 1 TO NUM-ENTRIES(v-rid-list):
 ...
 IF LOOKUP( "ok", ENTRY(1, buf_ext-file.STATUS_, chr(4))) > 0 THEN DO:
     message
 ...
 ELSE DO:
    ...
    ASSIGN
    v-dop = entry(1, buf_Ext-file.STATUS_, chr(4))
    v-dop = REPLACE(v-dop, 'ready':U, "":U)
    v-dop = trim(v-dop, chr(44))
    v-dop = REPLACE(v-dop, chr(44) + chr(44), chr(44))
    v-dop = v-dop + chr(44) + chr(63)
    v-dop = trim(v-dop, chr(44)).
    ENTRY(1, buf_Ext-file.STATUS_, chr(4)) = v-dop.
  END.
  br-ipck:REFRESH().
END.
...
assign
  v-str-recid9 = trim( string( recid( X_ext-file ) , "->>>>>>>>>>>9":U ) )
...
DO:
  ...
  END.
END.
ON CHOOSE OF MENU-ITEM m_auto
DO:
  ASSIGN
  add-option = 'auto':U.
  APPLY "CHOOSE" TO b-add in frame Dialog-Frame .
END.
ON CHOOSE OF MENU-ITEM m_manual
DO:
    ASSIGN
  add-option = 'manual':U.
  APPLY "CHOOSE" TO b-add in frame Dialog-Frame .
...
on start-search, ctrl-o of br-ipck in frame Dialog-Frame do:
   run sort-brbr-ipck
     (input (if available X_ext-file
             then recid(X_ext-file)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-ipck :
  ...
  case sort-labelbr-ipck:
        ...
        when X_ext-file.file-name:label in browse br-ipck then DO:    assign       sort-column-name = "X_ext-file.file-name"     .     run OpenBr in this-procedure ( input yes, input no, input no) .   . END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ipck FOR EACH X_ext-file NO-LOCK WHERE X_ext-file.file-num < 2147483647      INDEXED-REPOSITION .
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-22 as character no-undo .
      ...
      parameter-4-22 =
        (
          if (" X_ext-file.file-num < 2147483647 " + " " + where-phrase-22) <> ""
          then " X_ext-file.file-num < 2147483647 " + " " + where-phrase-22
          else "true"
        )
      parameter-5-22 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-22 =
          (" X_ext-file.file-num < 2147483647 " + " " + where-phrase-22 = "")
      .
  ...
  if l-filter-open-22 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  X_ext-file.file-num < 2147483647
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-22 =
        "where ":u + " X_ext-file.file-num < 2147483647 " + " ":u + where-phrase-22 + " ":u + p-find-condition + " " + ""
      parameter-5-22 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-ipck:handle
                          ,input rowid(X_ext-file)
                          ,input logical(parameter-2-22)
                          ,input no-lock
                          ,input (buffer X_ext-file:handle)
                          ,input parameter-4-22
                          ,input parameter-5-22
                          ) no-error.
      .
      ...
      assign
      parameter-2-22 = (if p-find-next then "true":u else "false":u )
      parameter-3-22 =  "FOR EACH X_ext-file"
      parameter-4-22 =
        (
          if (" X_ext-file.file-num < 2147483647 " + " " + where-phrase-22) <> ""
          then " X_ext-file.file-num < 2147483647 " + " " + where-phrase-22
  ...
  run waitfram-hide in this-procedure .
    ...
    AND p-from-db-num <> ? THEN DO:
      ASSIGN
      frame Dialog-Frame:TITLE = SUBStITUTE("&1 из БД &2", title00, p-from-db-num).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-24 as character no-undo .
      ...
      parameter-4-24 =
        (
          if (" X_ext-file.from-db-num = p-from-db-num                        and X_ext-file.file-num < 2147483647 " + " " + where-phrase-24) <> ""
          then  substitute('X_ext-file.from-db-num = &1                        and X_ext-file.file-num < 2147483647', p-from-db-num)  + " " + where-phrase-24
          else "true"
        )
      parameter-5-24 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-24 =
          (" X_ext-file.from-db-num = p-from-db-num                        and X_ext-file.file-num < 2147483647 " + " " + where-phrase-24 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-24 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  X_ext-file.from-db-num = p-from-db-num                        and X_ext-file.file-num < 2147483647
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-24 =
        "where ":u +  substitute('X_ext-file.from-db-num = &1                        and X_ext-file.file-num < 2147483647', p-from-db-num)  + " ":u + where-phrase-24 + " ":u + p-find-condition + " " + ""
      parameter-5-24 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-24 = (if p-find-next then "true":u else "false":u )
      parameter-3-24 =  "FOR EACH X_ext-file"
      parameter-4-24 =
        (
          if (" X_ext-file.from-db-num = p-from-db-num                        and X_ext-file.file-num < 2147483647 " + " " + where-phrase-24) <> ""
          then  substitute('X_ext-file.from-db-num = &1                        and X_ext-file.file-num < 2147483647', p-from-db-num)  + " " + where-phrase-24
  ...
  run waitfram-hide in this-procedure .
   ...
   AND p-from-db-num = ? THEN DO:
        ASSIGN
        frame Dialog-Frame:TITLE = SUBStITUTE("&1 для БД &2", title00, p-db-num).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-26 as character no-undo .
      ...
      parameter-4-26 =
        (
          if (" X_ext-file.db-num = p-db-num                          and X_ext-file.file-num < 2147483647 " + " " + where-phrase-26) <> ""
          then  substitute('X_ext-file.db-num = &1                          and X_ext-file.file-num < 2147483647', p-db-num ) + " " + where-phrase-26
          else "true"
        )
      parameter-5-26 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-26 =
          (" X_ext-file.db-num = p-db-num                          and X_ext-file.file-num < 2147483647 " + " " + where-phrase-26 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-26 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  X_ext-file.db-num = p-db-num                          and X_ext-file.file-num < 2147483647
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-26 =
        "where ":u +  substitute('X_ext-file.db-num = &1                          and X_ext-file.file-num < 2147483647', p-db-num ) + " ":u + where-phrase-26 + " ":u + p-find-condition + " " + ""
      parameter-5-26 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-26 = (if p-find-next then "true":u else "false":u )
      parameter-3-26 =  "FOR EACH X_ext-file"
      parameter-4-26 =
        (
          if (" X_ext-file.db-num = p-db-num                          and X_ext-file.file-num < 2147483647 " + " " + where-phrase-26) <> ""
          then  substitute('X_ext-file.db-num = &1                          and X_ext-file.file-num < 2147483647', p-db-num ) + " " + where-phrase-26
  ...
  run waitfram-hide in this-procedure .
   END.
    ...
    AND p-from-db-num <> ? THEN DO:
         ASSIGN
         frame Dialog-Frame:TITLE = SUBStITUTE("&1 для БД &2 из БД &3", title00, p-db-num, p-from-db-num).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-28 as character no-undo .
      ...
      parameter-4-28 =
        (
          if (" X_ext-file.db-num = p-db-num and X_ext-file.from-db-num = p-from-db-num                           and X_ext-file.file-num < 2147483647 " + " " + where-phrase-28) <> ""
          then  substitute('X_ext-file.db-num = &1 and X_ext-file.from-db-num = &2                           and X_ext-file.file-num < 2147483647', p-db-num, p-from-db-num )  + " " + where-phrase-28
          else "true"
        )
      parameter-5-28 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-28 =
          (" X_ext-file.db-num = p-db-num and X_ext-file.from-db-num = p-from-db-num                           and X_ext-file.file-num < 2147483647 " + " " + where-phrase-28 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-28 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  X_ext-file.db-num = p-db-num and X_ext-file.from-db-num = p-from-db-num                           and X_ext-file.file-num < 2147483647
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-28 =
        "where ":u +  substitute('X_ext-file.db-num = &1 and X_ext-file.from-db-num = &2                           and X_ext-file.file-num < 2147483647', p-db-num, p-from-db-num )  + " ":u + where-phrase-28 + " ":u + p-find-condition + " " + ""
      parameter-5-28 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-28 = (if p-find-next then "true":u else "false":u )
      parameter-3-28 =  "FOR EACH X_ext-file"
      parameter-4-28 =
        (
          if (" X_ext-file.db-num = p-db-num and X_ext-file.from-db-num = p-from-db-num                           and X_ext-file.file-num < 2147483647 " + " " + where-phrase-28) <> ""
          then  substitute('X_ext-file.db-num = &1 and X_ext-file.from-db-num = &2                           and X_ext-file.file-num < 2147483647', p-db-num, p-from-db-num )  + " " + where-phrase-28
   ...
   IF p-db-num = ?  THEN DO:
      IF p-FILE-NAME = '':U THEN DO:
          IF p-status_ = '':U THEN DO:
              ASSIGN
              frame Dialog-Frame:TITLE = SUBSTITUTE("&1", title01).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-30 as character no-undo .
      ...
      parameter-4-30 =
        (
          if (" X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0                             AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME " + " " + where-phrase-30) <> ""
          then " X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0                             AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME " + " " + where-phrase-30
          else "true"
        )
      parameter-5-30 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-30 =
          (" X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0                             AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME " + " " + where-phrase-30 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-30 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0                             AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-30 =
        "where ":u + " X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0                             AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME " + " ":u + where-phrase-30 + " ":u + p-find-condition + " " + ""
      parameter-5-30 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-30 = (if p-find-next then "true":u else "false":u )
      parameter-3-30 =  "FOR EACH X_ext-file"
      parameter-4-30 =
        (
          if (" X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0                             AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME " + " " + where-phrase-30) <> ""
          then " X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0                             AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME " + " " + where-phrase-30
  ...
  run waitfram-hide in this-procedure .
          ...
          ELSE DO:
              ASSIGN
              frame Dialog-Frame:TITLE = SUBStITUTE("&1 со статусом &2", title01, p-status_).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-32 as character no-undo .
      ...
      parameter-4-32 =
        (
          if (" X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                             and X_ext-file.status_ = p-status_ " + " " + where-phrase-32) <> ""
          then " X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                             and X_ext-file.status_ = p-status_ " + " " + where-phrase-32
          else "true"
        )
      parameter-5-32 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-32 =
          (" X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                             and X_ext-file.status_ = p-status_ " + " " + where-phrase-32 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                             and X_ext-file.status_ = p-status_
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-32 =
        "where ":u + " X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                             and X_ext-file.status_ = p-status_ " + " ":u + where-phrase-32 + " ":u + p-find-condition + " " + ""
      parameter-5-32 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_ext-file"
      parameter-4-32 =
        (
          if (" X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                             and X_ext-file.status_ = p-status_ " + " " + where-phrase-32) <> ""
          then " X_ext-file.file-num < 2147483647                             and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                             and X_ext-file.status_ = p-status_ " + " " + where-phrase-32
  ...
  run waitfram-hide in this-procedure .
      ...
      ELSE DO:
          ASSIGN
          frame Dialog-Frame:TITLE = SUBStITUTE("&1, пакет &2", title01, p-file-name).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_ext-file.file-num < 2147483647 and X_ext-file.file-name = p-file-name " + " " + where-phrase-34) <> ""
          then  substitute('X_ext-file.file-num < 2147483647 and X_ext-file.file-name = &1&2&1', chr(34), p-file-name ) + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_ext-file.file-num < 2147483647 and X_ext-file.file-name = p-file-name " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  X_ext-file.file-num < 2147483647 and X_ext-file.file-name = p-file-name
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_ext-file.file-num < 2147483647 and X_ext-file.file-name = &1&2&1', chr(34), p-file-name ) + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_ext-file"
      parameter-4-34 =
        (
          if (" X_ext-file.file-num < 2147483647 and X_ext-file.file-name = p-file-name " + " " + where-phrase-34) <> ""
          then  substitute('X_ext-file.file-num < 2147483647 and X_ext-file.file-name = &1&2&1', chr(34), p-file-name ) + " " + where-phrase-34
  ...
  ELSE DO:
    IF p-file-name = '':U THEN DO:
      IF p-status_ = '':U THEN DO:
          ASSIGN
          frame Dialog-Frame:TITLE = SUBStITUTE("&1 БД &2", title01, p-db-num).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if (" (X_ext-file.file-num < 2147483647                            and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num) " + " " + where-phrase-36) <> ""
          then " (X_ext-file.file-num < 2147483647                            and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num) " + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          (" (X_ext-file.file-num < 2147483647                            and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num) " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  (X_ext-file.file-num < 2147483647                            and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num)
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-36 =
        "where ":u + " (X_ext-file.file-num < 2147483647                            and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num) " + " ":u + where-phrase-36 + " ":u + p-find-condition + " " + ""
      parameter-5-36 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_ext-file"
      parameter-4-36 =
        (
          if (" (X_ext-file.file-num < 2147483647                            and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num) " + " " + where-phrase-36) <> ""
          then " (X_ext-file.file-num < 2147483647                            and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num) " + " " + where-phrase-36
  ...
  run waitfram-hide in this-procedure .
      ...
      ELSE DO:
          ASSIGN
          frame Dialog-Frame:TITLE = SUBStITUTE("&1 БД &2 со статусом &3", title01, p-db-num, p-status_).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-38 as character no-undo .
      ...
      parameter-4-38 =
        (
          if (" (X_ext-file.file-num < 2147483647                           and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num and X_ext-file.status_ = p-status_) " + " " + where-phrase-38) <> ""
          then  substitute('(X_ext-file.file-num < 2147483647                           and X_ext-file.obj-type = &2&2 AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = &1 and X_ext-file.status_ = &2&3&2)', p-db-num , chr(34), p-status_)  + " " + where-phrase-38
          else "true"
        )
      parameter-5-38 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-38 =
          (" (X_ext-file.file-num < 2147483647                           and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num and X_ext-file.status_ = p-status_) " + " " + where-phrase-38 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-38 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  (X_ext-file.file-num < 2147483647                           and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num and X_ext-file.status_ = p-status_)
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-38 =
        "where ":u +  substitute('(X_ext-file.file-num < 2147483647                           and X_ext-file.obj-type = &2&2 AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = &1 and X_ext-file.status_ = &2&3&2)', p-db-num , chr(34), p-status_)  + " ":u + where-phrase-38 + " ":u + p-find-condition + " " + ""
      parameter-5-38 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-38 = (if p-find-next then "true":u else "false":u )
      parameter-3-38 =  "FOR EACH X_ext-file"
      parameter-4-38 =
        (
          if (" (X_ext-file.file-num < 2147483647                           and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = p-db-num and X_ext-file.status_ = p-status_) " + " " + where-phrase-38) <> ""
          then  substitute('(X_ext-file.file-num < 2147483647                           and X_ext-file.obj-type = &2&2 AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                           and X_ext-file.db-num = &1 and X_ext-file.status_ = &2&3&2)', p-db-num , chr(34), p-status_)  + " " + where-phrase-38
  ...
  run waitfram-hide in this-procedure .
    ...
    ELSE DO:
        ASSIGN
        frame Dialog-Frame:TITLE = SUBStITUTE("&1 БД &2 Пакет &3", title01, p-db-num, p-file-name).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-40 as character no-undo .
      ...
      parameter-4-40 =
        (
          if (" (X_ext-file.file-num < 2147483647                         and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                         and X_ext-file.db-num = p-db-num and X_ext-file.file-name = p-file-name) " + " " + where-phrase-40) <> ""
          then  substitute('(X_ext-file.file-num < 2147483647                         and X_ext-file.obj-type = &2&2 AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                         and X_ext-file.db-num = &1 and X_ext-file.file-name = &2&3&2)', p-db-num,  chr(34), p-file-name)  + " " + where-phrase-40
          else "true"
        )
      parameter-5-40 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-40 =
          (" (X_ext-file.file-num < 2147483647                         and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                         and X_ext-file.db-num = p-db-num and X_ext-file.file-name = p-file-name) " + " " + where-phrase-40 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-ipck:handle
  ...
  if l-filter-open-40 = false then do:
    OPEN QUERY br-ipck FOR EACH X_ext-file
      where  (X_ext-file.file-num < 2147483647                         and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                         and X_ext-file.db-num = p-db-num and X_ext-file.file-name = p-file-name)
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-ipck:handle:get-buffer-handle(1) = (buffer X_ext-file:handle) then do:
      ...
      parameter-4-40 =
        "where ":u +  substitute('(X_ext-file.file-num < 2147483647                         and X_ext-file.obj-type = &2&2 AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                         and X_ext-file.db-num = &1 and X_ext-file.file-name = &2&3&2)', p-db-num,  chr(34), p-file-name)  + " ":u + where-phrase-40 + " ":u + p-find-condition + " " + ""
      parameter-5-40 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-40 = (if p-find-next then "true":u else "false":u )
      parameter-3-40 =  "FOR EACH X_ext-file"
      parameter-4-40 =
        (
          if (" (X_ext-file.file-num < 2147483647                         and X_ext-file.obj-type = '':U AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                         and X_ext-file.db-num = p-db-num and X_ext-file.file-name = p-file-name) " + " " + where-phrase-40) <> ""
          then  substitute('(X_ext-file.file-num < 2147483647                         and X_ext-file.obj-type = &2&2 AND X_ext-file.obj-code = 0 AND X_ext-file.FILE-TYPE = X_EXT-FILE.FILE-NAME                         and X_ext-file.db-num = &1 and X_ext-file.file-name = &2&3&2)', p-db-num,  chr(34), p-file-name)  + " " + where-phrase-40
  ...
  WHEN 'auto':U THEN DO:
    IF p-all-or-upg = 0 THEN DO:
      run nws/sndfnws.w ( INPUT parparentproc
                     ...
                     ,input ?
                     ,input ?
                     ,input 'auto':U ) no-error.
    END.
    IF p-all-or-upg = 1 THEN DO:
      run nws/sndfnws.w ( INPUT parparentproc
                     ,INPUT 'save-install':U
                     ,input ?
                     ,input ?
                     ,input '':U ) no-error.
    END.
  END.
  WHEN 'manual':U THEN DO:
    IF p-all-or-upg = 0 THEN DO:
      run nws/sndfnws.w ( INPUT parparentproc
                     ,INPUT 'save-this-db':U
                     ,input v-cntxt-db-num
                     ,input v-cntxt-db-num
                     ,input '':U ) no-error.
    END.
