...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4',p-cre-db-num,p-task-type,p-task-num)
    .
end procedure.
...
:
  ...
  then do:
    undo, return error
      "Ошибка задания входящих параметров" + chr(10)
      + substitute("Не найдена база данных &1", p-db-num)
      .
  end.
  run write-to-log in this-procedure
    ( substitute("Расчет складских архивов по базе данных &1", buf_db.db-num)
    + (if v-calc-arh  = true then chr(10) + "                         Складской архив по товарам" else "")
    + (if v-calc-ahsp = true then chr(10) + "                         Складской архив по поставщикам" else "")
    + (if v-calc-aht  = true then chr(10) + "                         Складской архив по типам приобретения" else "")
    + (if v-calc-hold = true then chr(10) + "                         Межфирменный архив" else "")
    ) .
  ...
  :
    ...
    then do:
      ...
      then do:
        run write-to-log in this-procedure
          ( vss-workfile + chr(32)
            + substitute("Ошибка при расчете складского архива по товарам. Объект &1 &2" + chr(10)
                          + "&3" + chr(10)
                          + "&4" + chr(10)
    ...
    then do:
      ...
      then do:
        run write-to-log in this-procedure
          ( vss-workfile + chr(32)
            + substitute("Ошибка при расчете складского архива по поставщикам. Объект &1 &2" + chr(10)
                          + "&3" + chr(10)
                          + "&4" + chr(10)
                        ,buf_clients.obj-type
                        ,buf_clients.obj-code
    ...
    then do:
      ...
      then do:
        run write-to-log in this-procedure
          ( vss-workfile + chr(32)
            + substitute("Ошибка при расчете складского архива по типам приобретения. Объект &1 &2" + chr(10)
                          + "&3" + chr(10)
                          + "&4" + chr(10)
                        ,buf_clients.obj-type
                        ,buf_clients.obj-code
  ...
  ,output hold-type
    ...
    then do:
      ...
      then do:
        run write-to-log in this-procedure
          ( vss-workfile + chr(32)
            + substitute("Ошибка при расчете межфирменных архивов" + chr(10) +
                        "&1" + chr(10) +
                        "&2" + chr(10)
                        ,error-status :get-message(1)
                        ,return-value
