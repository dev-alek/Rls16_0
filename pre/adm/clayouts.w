...
assign
  v-str-recid16 = trim( string( recid( X_c-layout ) , "->>>>>>>>>>>9":U ) )
...
do:
  v-doc-rec = recid(X_c-layout). run OpenBr in this-procedure ( input yes, input no, input '':U). reposition br-docs to recid v-doc-rec no-error. v-doc-rec = ?.               apply 'value-changed' to br-docs.
...
on start-search, ctrl-o of br-docs in frame Dialog-Frame do:
   run sort-brbr-docs
     (input (if available X_c-layout
             then recid(X_c-layout)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-docs :
  ...
  case sort-labelbr-docs:
        when X_c-layout.corr-date:label in browse br-docs then DO:    assign       sort-column-name = "X_c-layout.corr-date"     .     run OpenBr  in this-procedure ( input yes, input no, input '').   . END.
        when X_c-layout.layout-id:label in browse br-docs then DO:    assign       sort-column-name = "X_c-layout.layout-id"     .     run OpenBr  in this-procedure ( input yes, input no, input '').   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
for each buf_c-layout-elem-rule no-lock where
  ...
  if available buf_layout-elem-rule then do:
   ...
   v-chg-fields = "mode-id,widget-id,elem-label,elem-tooltip,image-id-down,image-id-insen,image-id-up,is-mandatory,rule_id,sts".
  end.
  _ii:
  do ii = 1 to num-entries(v-chg-fields):
    assign
    v-field-name = entry(ii, v-chg-fields)
    jj = lookup(v-field-name, "mode-id,widget-id,elem-label,elem-tooltip,image-id-down,image-id-insen,image-id-up,is-mandatory,rule_id,sts").
    if jj = 0 then next _ii.
    assign
    v-field-label = entry(jj, "Режим,Элемент,Лейбл элемента,Тултип элемента,Изображение DOWN,Изображение INSEN,Изображение UP,Обяз,Функция,Статус")
    v-field-function = entry(jj, ",,,,,,,get-is-mandatory-name,,get-status-name")
    ...
    temp-changes.l_name = substitute("&4Элемент &1 в режиме &2: &3"
                                     ...
                                     ,buf_c-layout-elem-rule.mode-id
                                     ,v-field-label
                                     ,(if buf_c-layout-elem-rule.action = integer('1':U)
                                       then "+"
                                       else (if buf_c-layout-elem-rule.action = integer('99':U)
                                             then "-"
                                             else chr(126))
                                       )
                                      )
    ...
    if buf_c-layout-elem-rule.action = integer('1':U) then do:
      assign
      temp-changes.v_old = ''
      .
    end.
    if buf_c-layout-elem-rule.action = integer('99':U) then do:
      assign
      temp-changes.v_new = ''
      .
    end.
...
for each buf_c-rule-call-param no-lock where
          buf_c-rule-call-param.call_id begins ('layout-elem-rule':U + chr(3) + p-layout-id)
  ...
  assign
  fields-name-list  = "param-name,param-data-type,param-label,rule_id"
  fields-label-list  = "Имя параметра,Тип данных,Лейбл,№ правила"
  fields-function-list  = ",,,"
  .
  if buf_c-rule-call-param.action = integer('1':U) then do:
    assign
  ...
  if v-type = 'character':U then do:
    assign
    fields-name-list = fields-name-list + chr(44) + "param-value-character"
    fields-label-list = fields-label-list + chr(44) + "Знач.(строковое)"
    fields-function-list = fields-function-list + chr(44)
    .
  end.
  if v-type = 'date':U then do:
    assign
    fields-name-list = fields-name-list + chr(44) + "param-value-date"
    fields-label-list = fields-label-list + chr(44) + "Знач.(дата)"
    fields-function-list = fields-function-list + chr(44)
    .
  end.
  if v-type = 'decimal':U then do:
    assign
    fields-name-list = fields-name-list + chr(44) + "param-value-decimal"
    fields-label-list = fields-label-list + chr(44) + "Знач.(десятичн.)"
    fields-function-list = fields-function-list + chr(44)
    .
  end.
  if v-type = 'integer':U then do:
    assign
    fields-name-list = fields-name-list + chr(44) + "param-value-integer"
    fields-label-list = fields-label-list + chr(44) + "Знач.(целое)"
    fields-function-list = fields-function-list + chr(44)
    .
  end.
  if v-type = 'logical':U then do:
    assign
    fields-name-list = fields-name-list + chr(44) + "param-value-logical"
    fields-label-list = fields-label-list + chr(44) + "Знач.(логич.)"
    fields-function-list = fields-function-list + chr(44)
    .
  end.
  if available buf_rule-call-param then do:
    ...
    v-chg-fields = "param-name,param-data-type,param-label,rule_id".
  end.
  _ii:
  do ii = 1 to num-entries(v-chg-fields):
    assign
    ...
    assign
    v-widget-id =  entry(lookup("widget-id", v-field-list, chr(3)), v-value-list, chr(3))
    v-mode-id = entry(lookup("mode-id", v-field-list, chr(3)), v-value-list, chr(3))
    .
    create temp-changes.
    assign
    ...
    temp-changes.l_name = substitute("&6Пар-р &1 функции &2 эл.&3 режим &4: &5"
                                      ...
                                      ,v-mode-id
                                      ,v-field-label
                                     ,(if buf_c-rule-call-param.action = integer('1':U)
                                       then "+"
                                       else (if buf_c-rule-call-param.action = integer('99':U)
                                             then "-"
                                             else chr(126))
                                       )
                                      )
    ...
    if buf_c-rule-call-param.action = integer('99':U) then do:
      assign
      temp-changes.v_new = ''
      .
    end.
...
title0 = "Список истории раскладок" + chr(32).
...
define variable l-open-query as logical   no-undo .
  CASE p-mode :
    WHEN "one":U        THEN DO:
     ...
     frame Dialog-Frame :title = substitute("История изменения раскладки &1", p-layout-id)
     .
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          if (" X_c-layout.layout-id = p-layout-id " + " " + where-phrase-27) <> ""
          then  substitute('X_c-layout.layout-id = &1&2&1', chr(34), p-layout-id )  + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + "")
      parameter-6-27 = if sort-phrase-27 = ''
        ...
        " " + "  " +
          " " + sort-column-phrase +
        " " + " by X_c-layout.corr-user-db-num  by X_c-layout.chip-num descending  "
        )
                           else
        (
        " " + "  " +
    ...
    :
      assign
        l-disable-where-27 =
          (" X_c-layout.layout-id = p-layout-id " + " " + where-phrase-27 = "")
      .
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-docs FOR EACH X_c-layout
      where  X_c-layout.layout-id = p-layout-id
       by X_c-layout.corr-user-db-num  by X_c-layout.chip-num descending
      indexed-reposition
  .
      assign
  ...
  on error undo, leave
  :
    if QUERY br-docs:handle:get-buffer-handle(1) = (buffer X_c-layout:handle) then do:
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-4-27 =
        "where ":u +  substitute('X_c-layout.layout-id = &1&2&1', chr(34), p-layout-id )  + " ":u + where-phrase-27 + " ":u + p-find-condition + " " + ""
      parameter-5-27 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input QUERY br-docs:handle
                          ,input rowid(X_c-layout)
                          ,input logical(parameter-2-27)
                          ,input no-lock
                          ,input (buffer X_c-layout:handle)
                          ,input parameter-4-27
                          ,input parameter-5-27
                          ) no-error.
      .
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH X_c-layout"
      parameter-4-27 =
        (
          if (" X_c-layout.layout-id = p-layout-id " + " " + where-phrase-27) <> ""
          then  substitute('X_c-layout.layout-id = &1&2&1', chr(34), p-layout-id )  + " " + where-phrase-27
...
if not available new_c-layout then do:
  ...
  do ii = 1 to num-entries(v-chg-fields):
    assign
    v-field-name = entry(ii, v-chg-fields)
    jj = lookup(v-field-name, "layout-id,des,device-type,is-default,layout-name,layout-type,sts").
    if jj = 0 then next _ii.
    assign
    v-field-label = entry(jj, "ID,Описание,Устройство,Обяз/пользв/Шаблон,Название,Тип,Статус")
    v-field-function = entry(jj, ",,,get-is-default-name,,get-layout-type-name,get-status-name")
    v-real-field-name = entry(1, v-field-name, ':')
    .
    create temp-changes.
    assign
...
assign
v-layout-kind-name = entry (lookup (string(p-is-default), '0,1,-1':U) + 1, ',' + 'Пользовательская,Шаблон IBS TH,Обязательная':U) no-error.
RETURN v-layout-kind-name.
END FUNCTION.
FUNCTION get-is-mandatory-name RETURNS CHARACTER
  ( p-is-mandatory as integer ) :
define variable v-layout-elem-kind-name as character no-undo.
assign
v-layout-elem-kind-name = entry (lookup (string(p-is-mandatory), '0,1':U) + 1, ',' + ',Обязательная':U) no-error.
RETURN v-layout-elem-kind-name.
END FUNCTION.
FUNCTION get-layout-type-name RETURNS CHARACTER
  ( input p-layout-type as character ) :
define variable v-layout-type-name as character no-undo.
assign
v-layout-type-name = entry (lookup (p-layout-type, 'th-pos-screen,th-pos-keyboard':U) + 1, ',' + 'Экран IBS TH POS,Клавиатура IBS TH POS':U) no-error.
RETURN v-layout-type-name.
END FUNCTION.
FUNCTION get-status-name RETURNS CHARACTER
  ( input p-sts as integer ) :
define variable v-status-name as character no-undo.
assign
v-status-name = entry (lookup (string(p-sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) no-error.
