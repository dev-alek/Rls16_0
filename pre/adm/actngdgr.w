  ...
  for each tt-goods
      ...
      assign
         p-gds-list = IF p-gds-list = "":U THEN STRING(tt-goods.gds-code)
                                           ELSE p-gds-list + chr(4) + STRING(tt-goods.gds-code)
      .
  end.
  ASSIGN
      p-ok = TRUE
...
DO:
  MESSAGE "Help for File: c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\adm\actngdgr.w" VIEW-AS ALERT-BOX INFORMATION.
END.
...
:
   ...
   ELSE DO:
      ...
      :
            FIND first buf_goods
            where buf_goods.gds-code = INTEGER(ENTRY(v-count, p-Gds-List, chr(4)))
            no-lock
            no-error
            .
