...
END.
END.
ON CHOOSE OF B-add-cpp IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE v-value AS CHARACTER NO-UNDO.
  DEFINE VARIABLE v-log AS logical NO-UNDO.
...
if v-type <> "1" then do:
  ...
  v-value = "0".
   ...
   tt-cash-pay-mar-pet.obj-name-mar = IF tt-cash-pay-mar-pet.emitent = 0
                                      THEN ENTRY(tt-cash-pay-mar-pet.cdpay-code-mar-pet, 'Наличные,Дебетовая ведомость,Кредитная ведомость,Платежные карты')
                                      ELSE ENTRY(tt-cash-pay-mar-pet.cdpay-code-mar-pet, 'Наличные,Дебетовые карты(талоны),Кредитные карты(талоны),Платежные карты')
   v-rid = RECID(tt-cash-pay-mar-pet)
   .
   run openbr-cpp IN THIS-PROCEDURE.
   REPOSITION br-cash-payp TO RECID v-rid.
END.
ON CHOOSE OF B-chg-cp IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE v-value AS CHARACTER NO-UNDO.
  DEFINE VARIABLE v-log AS logical NO-UNDO.
  DEFINE variable v-rid AS RECID NO-UNDO.
...
END.
ON CHOOSE OF B-del-cpp IN FRAME Dialog-Frame
DO:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cd-type-maria':U
...
DO ii = 1 TO  NUM-ENTRIES(v-mariapayg, ";"):
   ...
   v-cdpay-code-mar = integer(entry(1, entry(1, v-entry, chr(47)), chr(44)))
   ...
   FIND FIRST buf_cash-pay NO-LOCK WHERE
    ...
    tt-cash-pay-mar.obj-name-mar = entry(tt-cash-pay-mar.cdpay-code-mar, 'Наличные,Безнал 1,Безнал 2,Безнал 3')
    .
...
DO ii = 1 TO  NUM-ENTRIES(v-mariapayp, ";"):
   ...
   v-cdpay-code-mar-pet = integer(entry(1, entry(1, v-entry, chr(47)), chr(44)))
   v-emitent = integer(entry(2, entry(1, v-entry, chr(47)), chr(44)))
   v-cdpay-code = integer(entry(1, entry(2, v-entry, chr(47)), chr(44)))
   v-curr-code = integer(entry(2, entry(2, v-entry, chr(47)), chr(44)))
   .
   FIND FIRST buf_cash-pay NO-LOCK WHERE
...
b-chg-cp   WHEN p-mode = 'ИЗМЕНЕНИЕ':U
b-add-cpp   WHEN p-mode = 'ИЗМЕНЕНИЕ':U
b-del-cpp   WHEN p-mode = 'ИЗМЕНЕНИЕ':U
b-discnt
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
...
FOR EACH tt-cash-pay-mar BY tt-cash-pay-mar.cdpay-code-mar:
    ...
    v-mariapayg = v-mariapayg  + (IF ii = 1 THEN '':U ELSE ';') +
                  STRING(tt-cash-pay-mar.cdpay-code-mar) + chr(47) +
                  STRING(tt-cash-pay-mar.cdpay-code) + chr(44) +
...
FOR EACH tt-cash-pay-mar-pet
   ...
   v-mariapayp = v-mariapayp + (IF ii = 1 THEN '':U ELSE ';') +
                 STRING(tt-cash-pay-mar-pet.cdpay-code-mar-pet) + chr(44) +
                 STRING(tt-cash-pay-mar-pet.emitent)  + chr(47) +
                 STRING(tt-cash-pay-mar-pet.cdpay-code) + chr(44) +
...
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'mariapayg':U.
assign
thbjattr_thbj-attr.property-value-character = v-mariapayg.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'mariapayp':U.
assign
thbjattr_thbj-attr.property-value-character = v-mariapayp.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'dr-list':U.
assign
thbjattr_thbj-attr.property-value-character = dr-list.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'drgrouprank':U.
assign
thbjattr_thbj-attr.property-value-character =  (if drgrouprank = '':U
                                                then '%-46,%сумма-49':U
                                                else drgrouprank).
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = 'drgdsrank':U.
assign
