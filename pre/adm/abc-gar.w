...
END.
ON LEAVE OF v-a IN FRAME Dialog-Frame
DO:
  ASSIGN v-a.
END.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'abc-sale-day':U
...
FOR EACH thbjattr_thbj-attr:
  IF thbjattr_thbj-attr.prop-code = 'A':U THEN DO:
    ...
    v-a:private-data IN FRAME Dialog-Frame = "recid=" + string(recid(thbjattr_thbj-attr)).
  END.
...
b-quit
B-Help
v-a WHEN p-mode = 'ИЗМЕНЕНИЕ':U
v-b WHEN p-mode = 'ИЗМЕНЕНИЕ':U
v-c WHEN p-mode = 'ИЗМЕНЕНИЕ':U
v-d WHEN p-mode = 'ИЗМЕНЕНИЕ':U
v-E WHEN p-mode = 'ИЗМЕНЕНИЕ':U
v-F WHEN p-mode = 'ИЗМЕНЕНИЕ':U
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
if p-mode = 'ПРОСМОТР':U then do:
  ...
  b-quit:column = 1.
  display
  b-quit with frame Dialog-Frame.
END.
END PROCEDURE.
PROCEDURE proc-save :
define variable v-value-character as character no-undo .
