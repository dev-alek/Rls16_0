...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  IF p-mode = 'ИЗМЕНЕНИЕ':U THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = 'cd-type-IBS-TH':U
...
if rs-rmethod-type = "MROUND" then do:
  ...
  rs-rmethod-coeff = v-rmethod-coeff
  f-rmethod-coeff = 0
  rs-rmethod-coeff:screen-value in frame Dialog-Frame = string(v-rmethod-coeff) no-error
  .
end.
else do:
  assign
  f-rmethod-coeff = v-rmethod-coeff
  rs-rmethod-coeff = 2
  rs-rmethod-coeff:screen-value in frame Dialog-Frame = string(2)
  .
end.
ASSIGN
...
b-rec-print
b-interface
f-max-netto WHEN p-mode = 'ИЗМЕНЕНИЕ':U
t-cash-shift WHEN p-mode = 'ИЗМЕНЕНИЕ':U
...
f-customer-display-port WHEN p-mode = 'ИЗМЕНЕНИЕ':U
...
t-rcpt-ord-slip-print WHEN p-mode = 'ИЗМЕНЕНИЕ':U
t-rcpt-ord-alt-print WHEN p-mode = 'ИЗМЕНЕНИЕ':U
...
by thbjattr_thbj-attr.upper-prop-code :
   ...
   if v-loc-same = no then do:
     v-same = v-loc-same.
     if l-write-cd = yes
     and thbjattr_thbj-attr.upper-prop-code <> 'cd-type-IBS-TH':U
     then do:
...
case p-obj-type:
  when '':U then do:
    ...
    for each buf_Cash-desk no-lock where
            buf_cash-desk.pos-type = 'IBS-TH':U
