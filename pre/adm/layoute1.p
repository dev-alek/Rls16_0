...
:
  ...
  if p-layout-type = 'th-pos-keyboard':U
  or p-layout-type = 'th-pos-screen':U then do:
  ...
  if lookup(string(p-elem-type), '0,-1':U) = 0 then do:
    assign
  ...
  if p-mode = 'ИЗМЕНЕНИЕ':U then do:
    ...
    if p-elem-type = integer('-1':U) then do:
      for each buf_layout-elem-rule no-lock where
        ...
        and (buf_layout.is-default = integer('0':U)
             or
             buf_layout.is-default = integer('1':U)
             )
        then do:
          assign
