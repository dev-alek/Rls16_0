...
define variable vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  run gbl/d-askw.w
    (input "Вопрос 1 из 4"
    ,input "Выберите действие" + chr(10)
    ,input "|^"
  ...
  :
    ...
    then do:
      ...
      :
        assign
          v-db-list       = v-db-list
                          + (if v-db-list <> '':u then chr(44) else '':u )
                          + string(buf_db.db-num)
          v-db-label-list = v-db-label-list
                          + (if v-db-label-list <> '':u then chr(44) else '':u )
                          + buf_db.db-name
        .
      end.
      case v-num :
      ...
      assign
        v-num-entries-select-db-list = num-entries(v-select-db-list, chr(44))
      .
      do v-ind = 1 to v-num-entries-select-db-list
      :
        run waitfram-show in this-procedure
          (input substitute("Обработка базы данных &1"
                           ,integer(entry(v-ind, v-select-db-list, chr(44)))
                           )
          ) .
        run trg/ahdbdis.p
          (input v-archive-type
          ,input integer(entry(v-ind, v-select-db-list, chr(44)))
