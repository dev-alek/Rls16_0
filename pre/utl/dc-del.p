...
:
  ...
  if v-is-remote-dbs then do:
    run gen-key-rec( input 'dis-card':U
    ...
    assign
    v-param = string(buf_dis-card.d-card) + chr(4) +
              buf_dis-card.status_
    .
    run nws/db-rec.p (
                        input 'delete_nu-dis-card':U
                        ,input v-key-rec
                        ,input v-param
                      ) no-error .
  end.
    ...
    then
      run write-log-and-file in p-log-handle (
                                            ...
                                            , input  substitute( "Карта &1: Начат процесс изменения статуса на неисп., окончательное удаление невозможно. &3Обнаружено: &2."
                                                                ,buf_dis-card.d-card, v-msg-not-del, chr(10) )
                                                                        ).
    else
      run write-log-and-file in p-log-handle (
                                            input 1
