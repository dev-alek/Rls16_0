...
procedure unload-send :
  ...
  do
  on error  undo, return error substitute( "&1 (unload-send). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
...
procedure unload-rcvd :
  define input  parameter p-db-num as integer   no-undo .
  do
  on error  undo, return error substitute( "&1 (unload-rcvd). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
