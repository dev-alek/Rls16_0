...
DEFINE BUTTON B-sel
     ...
     SIZE 30 BY 1.
DEFINE VARIABLE RS-client-type AS CHARACTER INITIAL 'орг':U
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS
          "Организация", 'орг':U,
"Физическое лицо", 'чел':U
...
DO:
  ...
  CASE RS-client-type:
    WHEN 'орг':U then do:
      ...
      DISPLAY BROWSE-1 WITH FRAME Dialog-Frame.
    END.
    WHEN 'чел':U then do:
      HIDE browse-1 IN FRAME Dialog-Frame.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1 FOR EACH ub.clients       WHERE ub.clients.is-prod = TRUE  AND ub.clients.obj-type = 'орг':U NO-LOCK,       EACH ub.firm WHERE  ub.firm.firm-code = ub.clients.obj-code       AND ub.firm.city <> "" NO-LOCK.    OPEN QUERY BROWSE-2 FOR EACH ub.clients       WHERE ub.clients.is-prod = TRUE  AND ub.clients.obj-type = 'чел':U NO-LOCK,       EACH ub.person WHERE ub.person.psn-code = ub.clients.obj-code       AND ub.person.city <> "" NO-LOCK.
END PROCEDURE.
