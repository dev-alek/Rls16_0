...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  CASE p-mode:
    ...
    THEN DO:
      ...
      TO tt-dis-time-rule
      assign
      tt-dis-time-rule.templ-rl-root = (if p-mode = 'КОПИРОВАНИЕ':U
                                   then 0
                                   else locked_dis-time-rule.templ-rl-root )
      tt-dis-time-rule.rl-root = (if p-mode = 'КОПИРОВАНИЕ':U
                             then 0
                             else locked_dis-time-rule.rl-root)
      tt-dis-time-rule.time-rule-num = (if p-mode = 'КОПИРОВАНИЕ':U
                              then 0
...
v-list-items = '':U.
DO v-ii = 1 TO NUM-ENTRIES('0,1,2,4,8,16':U):
    ASSIGN
    v-list-items = v-list-items +  (IF v-ii > 1 then chr(44) ELSE '':U) +
                   ENTRY(v-ii, '?,Период времени,Период дат,День недели,Дата,День месяца':U) + chr(44) +
                   ENTRY(v-ii, '0,1,2,4,8,16':U).
END.
ASSIGN
tt-dis-time-rule.value-type:LIST-ITEM-PAIRS IN FRAME Dialog-Frame = v-list-items.
assign
tt-dis-time-rule.sts:radio-buttons in frame Dialog-Frame = "Текущий"   + chr(44) + '0':U +  chr(44) +
...
b-quit
B-Help
tt-dis-time-rule.templ-rl-root WHEN p-mode = 'ДОБАВЛЕНИЕ':U
tt-dis-time-rule.des WHEN p-mode <> 'ПРОСМОТР':U
tt-dis-time-rule.other-inf WHEN p-mode <> 'ПРОСМОТР':U
tt-dis-time-rule.uniq-field WHEN p-mode <> 'ПРОСМОТР':U
tt-dis-time-rule.value-type WHEN p-mode <> 'ПРОСМОТР':U
tt-dis-time-rule.sts when p-mode <> 'ПРОСМОТР':U
