...
define variable vss-revision    as character no-undo init "$Revision: 7c9dcd0c265b, 3134, rls $":U .
define variable vss-author      as character no-undo init "$Author: SSlivenko $":U .
define variable vss-date        as character no-undo init "$Date: 2022/09/15 11:05:54 $":U .
define variable vss-workfile    as character no-undo init "$Workfile: ora-i516-1c.p $":U .
define variable vss-archive     as character no-undo init "$Archive: utl/ora-i516-1c.p $":U .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,output v-cntxp-par-type
    ...
    buffer-copy  temp_trn-doc  to    tt-trn-doc
      ...
      tt-trn-doc.PS                   = if not is-egais then substitute("&1 &2 &3 &5&4 ", temp_trn-doc.doc-code , string(temp_trn-doc.doc-date, "99/99/9999") , temp_trn-doc.creid ,temp_trn-doc.ps ,chr(10) ) else ""
...
.
    ...
    if new_trn-doc.ext-doc-type = 'ee':U
    then do:
    ...
    assign
        ...
        new_trn-doc.rcv-code = if not (is-tsd or is-egais) then "not_delete" else ""
  ...
  for each  temp_doc-line  no-lock  where
      ...
      if v-specif = true and temp_trn-doc.ext-doc-type = 'ie':U  then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjcr in g#library
  ...
  v-str-txt = "" .
     ...
     if temp_trn-doc.ext-doc-type = 'ee':U then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run rootnode in g#library
  ...
  for each tt2-doc-line :
    ...
    for each temp_doc-line no-lock where temp_doc-line.artic = tt2-doc-line.artic
      ...
      if is-tsd and v-ext-doc-type = 'ie':U then do:
        run unitqnty1 (
  ...
  case v-ext-doc-type :
      when  'ie':U or when 'iv':U then do:
           if v-specif
           then do:
              for each tt-parts          :
                 find first buf_goods no-lock  where
...
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_copy-ret in g#lib-trn
  ...
  , input yes
            ...
            if v-ext-doc-type = 'ee':U and not is-tsd then do:
            run clos-trn in this-procedure (new_trn-doc.doc-code) no-error .
                ...
                if temp_trn-doc.price-type   = 'TSFCST':U then do:
...
do trans:
  ...
  if new_trn-doc.ext-doc-type = 'ie':U and new_trn-doc.cli-type ne 'маг'   then do:
    run clos-trn2 in this-procedure (new_trn-doc.doc-code) no-error .
    if error-status:error then do :
  ...
  if new_trn-doc.ext-doc-type = 'ee':U    then do:
...
,input ''
,input trn-doc.ext-doc-type
,input tt2-doc-line.prt-root
,input tt2-doc-line.vat-pc
,input 0
,input 0
) no-error
.
