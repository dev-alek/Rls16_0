...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-drt-prop FOR EACH temp-drt-prop NO-LOCK BY temp-drt-prop.upper-prop-code BY temp-drt-prop.prop-code INDEXED-REPOSITION.
END PROCEDURE.
...
IF rs-add = "add" THEN DO:
  ...
  v-upper-node-code = (if available temp-drt-prop
   ...
   buf_temp-drt-prop.full-prop-name = temp-drt-prop.full-prop-name + f-prop-code + chr(47)
   .
END.
else do:
  assign
