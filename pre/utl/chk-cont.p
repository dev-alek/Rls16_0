...
define variable vss-include-info3 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    ...
    assign
        log-file-name = 'chk-cont.log'
    .
    if log-file-name <> "":U
    then do:
        if search( 'chk-cont.log' ) = ?
        then do:
            output to value( 'chk-cont.log' ).
            output close.
        end.
    end.
    DEF STREAM stm-log.
  ...
  run ref/cli-all.w ( parParentProc, "b-sel,b-mark", 'орг':U, 'все':U, 'текущие':U, ?, "yes,yes,yes,,,,ИЛИ,,":u, "without-obj":U, output doc-list ) .
  if doc-list <> "" then do:
    assign  Counter1 = 0 .
