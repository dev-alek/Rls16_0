...
:
  ...
  if buf_tax-rate-gds.rate-code <> 11 then
    buf_tax-rate-gds.rate-code = 11.
end.
for each tax-rate-gds-grp exclusive-lock where
         tax-rate-gds-grp.tax-code  = 1
     and tax-rate-gds-grp.rate-code = 1
:
  tax-rate-gds-grp.rate-code = 11.
end.
find first buf_code exclusive-lock where
           buf_code.parent = substitute("RunUtils&1&2",chr(4),sys-ctrl.db-num)
