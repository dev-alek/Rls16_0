...
:
  ...
  for each ub.clients where ub.clients.db-num = p-db-num no-lock:
    ...
    run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input ("Начало очистки УТМ" + chr(10) + "Адрес: " + admUtmObj:ConPar + chr(10) + "FSRAR: " + v-fs-rar + chr(10) + "ВС: " + string (v-ext-sys) +  chr(10) + "Объект: " + ub.clients.obj-type + string (ub.clients.obj-code))).
    find first ub.ext-system-attr where ub.ext-system-attr.db-num = 0
      and ub.ext-system-attr.esys-id = v-ext-sys and ub.ext-system-attr.esya-attr-code = 'save-oxml-pck':U no-lock.
    admUtmObj = new admutm (ub.clients.obj-type, ub.clients.obj-code, v-fs-rar, v-ext-sys).
    admUtmObj:Cleaning(integer (ub.ext-system-attr.esya-attr-value), p-log-handle).
    delete object admUtmObj.
        run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input "Окончание очистки УТМ").
  end.
end.
