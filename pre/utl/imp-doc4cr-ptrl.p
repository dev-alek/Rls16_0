...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input tt-trn-close.trn-code ,
  ...
  for each tt-gds:
    find first ub.goods-attr where ub.goods-attr.gds-code = tt-gds.gds-code and ub.goods-attr.attr-code = 'ptrl-without-rvs':U no-error.
    if available (ub.goods-attr)
      then delete ub.goods-attr.
  end.
    run write-log-and-file in p-log-handle ( 1, p-log-filename, 1, substitute("При переносе остатков по топливу отвергнуто &1 записей. Из них:", p-count-err ) ).
...
define buffer new_clients  for ub.clients .
  ...
  :
    if first-of (buf_tt-parts.supp-code) then do:
      ...
      or buf_tt-parts.supp-type = 'скл':U
      or buf_tt-parts.supp-type = 'маг':U then assign
...
do on error undo, return error return-value :
  ...
  if error-status:error then do:
    ...
    run write-log-and-file in p-log-handle ( 1, p-log-filename, 1, substitute("Создание ПН № &1 объект &2&3 контраг &4&5 &6" , n-d  , new_obj-type , new_obj-code ,  new_cli-type ,  new_cli-code , temp-line.contract-code ) ).
  assign
    v-ext-doc-type = 'ie':U
  .
  find first temp_parts where
  ...
  :
    ...
    if not available tt-doc-line then do:
      ...
      then do:
        ...
        find first buf_goods-attr where buf_goods-attr.gds-code = tt-gds.gds-code and buf_goods-attr.attr-code = 'ptrl-without-rvs':U no-error.
        if not available (buf_goods-attr)
        then do:
          create buf_goods-attr.
          buf_goods-attr.gds-code = tt-gds.gds-code.
          buf_goods-attr.attr-code = 'ptrl-without-rvs':U.
