...
entry(2, X_ext-classif.uniq-key-rec, chr(3)) COLUMN-LABEL "Вн.код!группы(TH)" FORMAT "X(9)"
...
if v-num-entry14 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-gds-grp in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-gds-grp in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-gds-grp :handle
      .
...
do:
  if available X_ext-classif then v-doc-rec = recid(X_ext-classif). run openbr in this-procedure ( INPUT YES, INPUT NO, INPUT '':U) no-error. reposition br-gds-grp to recid(v-doc-rec) no-error. APPLy 'ENTRY' to br-gds-grp .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-gds-grp FOR EACH X_ext-classif NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ENABLE
b-quit
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-26 as character no-undo .
      ...
      parameter-4-26 =
        (
          if (" X_ext-classif.classif-subject = 'gds-grp':U                         and X_ext-classif.classif-name = 'rpm_gds-grp':U                         AND X_ext-classif.db-num = - 1" + " " + where-phrase-26) <> ""
          then  substitute('X_ext-classif.classif-subject = &1&2&1                         and X_ext-classif.classif-name = &1&3&1                         AND X_ext-classif.db-num = - 1', chr(34), 'gds-grp':U, 'rpm_gds-grp':U) + " " + where-phrase-26
          else "true"
        )
      parameter-5-26 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-26 =
          (" X_ext-classif.classif-subject = 'gds-grp':U                         and X_ext-classif.classif-name = 'rpm_gds-grp':U                         AND X_ext-classif.db-num = - 1" + " " + where-phrase-26 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-gds-grp:handle
  ...
  if l-filter-open-26 = false then do:
    OPEN QUERY br-gds-grp FOR EACH X_ext-classif
      where  X_ext-classif.classif-subject = 'gds-grp':U                         and X_ext-classif.classif-name = 'rpm_gds-grp':U                         AND X_ext-classif.db-num = - 1
       BY X_ext-classif.charkey_two
      INDEXED-REPOSITION
  .
      assign
...
if available X_ext-classif then do:
    APPLY "VALUE-CHANGED":U to br-gds-grp.
END.
END PROCEDURE.
...
DO WHILE available X_ext-classif :
  ...
  (IF X_ext-classif.uniq-key-rec BEGINS 'gds-grp':U
