...
assign
  v-str-recid4 = trim( string( recid( X_dis-rule ) , "->>>>>>>>>>>9":U ) )
...
ch = br-dis-rule:FIRST-COLUMN IN FRAME Dialog-Frame.
DO ii = 1 TO br-dis-rule:NUM-COLUMNS IN FRAME Dialog-Frame:
    ASSIGN
    ch:RESIZABLE = YES.
    ch = ch:NEXT-COLUMN.
END.
...
FOR EACH X_dis-rule NO-LOCK WHERE X_dis-rule.rule-num <= 99999 INDEXED-REPOSITION.
END PROCEDURE.
FUNCTION rule-name RETURNS CHARACTER
