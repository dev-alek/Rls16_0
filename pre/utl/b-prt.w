...
PROCEDURE adm-display-fields :
      IF AVAILABLE parts THEN
          DISPLAY parts.part-code parts.pay-code parts.cli-qnty parts.cli-base-rate parts.qnty parts.fact-qnty parts.price-cli parts.price-base parts.price-rubl parts.VAT-type parts.VAT-pc parts.SLT-type parts.SLT-pc parts.other-base parts.other-rubl parts.road-tax-base parts.road-tax-rubl parts.transport-base parts.transport-rubl parts.artic parts.cst-code parts.doc-type parts.exch-code parts.fact-date parts.fact-num parts.host-code parts.in-code parts.is-supp parts.last-date parts.obj-code parts.obj-type parts.out-code parts.pl-code parts.prod-code parts.prod-type parts.PS parts.real-qnty parts.rsrv-free parts.status_ parts.supp-code parts.supp-type WITH BROWSE br_table
            NO-ERROR.
...
PROCEDURE adm-open-query-cases :
  OPEN QUERY br_table FOR EACH parts WHERE parts.obj-type = doc-line.obj-type   AND parts.obj-code = doc-line.obj-code   AND parts.out-code = doc-line.doc-code   AND parts.artic = doc-line.artic   AND parts.prod-type = doc-line.prod-type   AND parts.prod-code = doc-line.prod-code NO-LOCK     .
END PROCEDURE.
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "doc-line":U THEN p-rowid-list = p-rowid-list +
        ...
        ELSE "?":U.
    WHEN "parts":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE parts THEN STRING(ROWID(parts))
        ELSE "?":U.
        OTHERWISE
        DO:
