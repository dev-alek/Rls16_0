...
if   tt_obj-list.buy-cons then  tt_obj-list.buy-cons = no.
     ...
     v-log = br-obj:refresh() .
     v-log = br-obj:select-next-row() .
     apply "VALUE-CHANGED" to br-obj in frame Dialog-Frame.
END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-obj FOR EACH tt_obj-list NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE init-proc :
empty temp-table tt_obj-list.
define buffer new_clients for ub.clients  .
for each new_clients no-lock where
         new_clients.obj-type = 'маг':U or
         new_clients.obj-type = 'скл':U
         :
...
for each tt_obj-list  :
  ...
  find first new_ext-classif no-lock where
              new_ext-classif.classif-subject = 'clients':U
...
run str/diallog.w ( input parparentproc
          ...
          , input v-parameter + chr(4) + p-from-version
          , input no
          , input ''
