...
define temp-table temp-fact-order no-undo   field temp-gds-code    like ub.aht-stk-line.gds-code   field temp-sum-type    like ub.aht-stk-line.sum-type   field temp-fact-order  like ub.aht-stk-line.fact-order   field temp-break-value as integer   field temp-need-delete as logical   index xpk is primary unique temp-gds-code temp-sum-type temp-fact-order   index xie1 temp-need-delete   index xie2 temp-gds-code temp-sum-type temp-break-value .
define temp-table temp-last-fact-order no-undo   field temp-gds-code    like ub.aht-stk-line.gds-code   field temp-sum-type    like ub.aht-stk-line.sum-type   field temp-break-value as integer   field temp-fact-order  like ub.aht-stk-line.fact-order   index xpk is primary unique temp-gds-code temp-sum-type temp-break-value .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  then do:
    ...
    message
      vss-workfile vss-revision vss-description skip
      "Неправильный тип атрибута" 'aht-detail':U skip
  ...
  :
    ...
    :
      ...
      run accum-temp-fact-order in this-procedure
        (input  buf_aht-stk-line.gds-code
        ,input  buf_aht-stk-line.sum-type
