...
define variable vss-workfile    as character no-undo initial "$Workfile: cas-aht.p $":U .
define variable vss-archive     as character no-undo initial "$Archive: utl/cas-aht.p $":U .
...
define frame a
  ...
  title "Проверка складского архива по типам приобретения"
  .
do
on error undo, return error return-value
:
  assign
    v-log-file-name   = 'cas-aht.log':U
    v-error-file-name = 'cas-aht.txt':U
  .
...
define variable vss-include-info3 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  run gbl/d-askw.w
    (input "Вопрос"
    ,input "Проверка целостности складского архива по типам приобретения" + chr(10)
    ,input "|^"
  ...
  case v-num :
    ...
    then do:
      ...
      :
        ...
        then do:
          run check-archive-aht in this-procedure
...
run userobjs_select-many in this-procedure
  ...
  then do:
    message
      "Проверка складского архива по типам приобретения закончена" skip
      "" skip
      "Обнаружены ошибки в складском архиве по типам приобретения" skip
...
procedure check-archive-aht :
  ...
  :
    ...
    export stream sout "Начало проверки целостности складского архива по типам приобретения" .
    ...
    run utl/car-aht.p
      ...
      ,output v-err-count
      ,output v-last-date
      ,output v-error-description
      ,output v-detail-error-file-name
      ) .
    if v-err-count <> 0
    then do:
      assign
      ...
      export stream sout '***':u string(v-today, '99/99/9999':u) string(v-time, 'HH:MM:SS':u) .
      export stream sout "Завершена проверка складского архива по типам приобретения" .
      export stream sout "Объект" p-obj-type p-obj-code .
    ...
    output stream sout to value(v-log-file-name) append .
    export stream sout '###':u string(v-today, '99/99/9999':u) string(v-time, 'HH:MM:SS':u) .
    export stream sout "Завершена проверка складского архива по типам приобретени " .
    export stream sout "Объект" p-obj-type p-obj-code .
    export stream sout "Ошибок" v-err-count .
    export stream sout "Последняя ошибочная дата" string(v-last-date, '99/99/9999':u) .
    export stream sout "Подробная информация об ошибках выведена в файл" v-detail-error-file-name .
