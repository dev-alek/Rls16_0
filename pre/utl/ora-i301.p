  ...
  ,output main-b-code
    ...
    if available buf_price-doc then do:
        find first buf_price-doc-forming exclusive-lock where
              ...
              if (temp-price-doc.doc-num-ES <> ? and temp-price-doc.doc-num-ES <> "")
              or (temp-price-doc.doc-id <> ? and temp-price-doc.doc-id <> "")
              then buf_price-doc-forming.des = temp-price-doc.doc-num-ES + chr(4) + temp-price-doc.doc-id + chr(4) + string (temp-price-doc.doc-date , "99/99/9999" ) .
       buf_price-doc.PS = "temp" .
       delete  buf_price-doc.
       .
    end.
    if temp-price-doc.doc-id = "_"
    then do :
      run str/diallog.w
          ( this-procedure
          , p-log-handle
          , if (temp-price-doc.doc-num-ES <> ? and temp-price-doc.doc-num-ES <> "")
            or (temp-price-doc.doc-id <> ? and temp-price-doc.doc-id <> "")
            then ('str/pdf-clos.p':U + chr(4) + '1' + chr(4) + '2' + chr(4) + '1')
      ...
      run str/diallog.w
       ...
       if available new_price-doc then do:
          ...
          assign
            v-end-message =  string(temp-price-doc.obj-type) + string(temp-price-doc.obj-code)
            + chr(9) + "ПЕРЕОЦЕНКА:" + string(new_price-doc.doc-num)
          .
          run pcall-log-file in p-log-handle (input v-end-message) .
    ...
    assign
        v-end-message =  string(temp-price-doc.obj-type) + string(temp-price-doc.obj-code)
        + chr(9) + "№ внешней системы:"  + string(temp-price-doc.doc-num)
        + chr(9) + "ДНЦ:" + string(buf_price-doc-forming.pdf-id) + chr(9) + string( v-counter-gds ) + " товаров"
    .
     run pcall-log-file in p-log-handle (input v-end-message) .
     p-ok-doc = p-ok-doc + 1.
end.
...
define variable vss-include-info95 as character format "x(65)" no-undo initial "@(#)$Workfile$".
...
run adm/shattri.p (
  ...
  for each thbjattr_thbj-attr :
    if thbjattr_thbj-attr.prop-code = 'pr-incpc':U then loc_calc-increase-pc = thbjattr_thbj-attr.property-value-decimal .
    if thbjattr_thbj-attr.prop-code = 'pr-rndmt':U then loc_calc-round-method = thbjattr_thbj-attr.property-value-character .
    if thbjattr_thbj-attr.prop-code = 'pr-rndbs':U then loc_calc-round-base = thbjattr_thbj-attr.property-value-decimal .
  end.
...
find first ub.price-list-type no-lock where
    ...
    v-plt-id = next-value (s-plt, ub)  .
        run type-price-list-ADD (
            v-cntxt-db-num
          , v-plt-id
          , "ГТПЛ по объекту " + p-obj-type + string (p-obj-code)
          , 0
