...
procedure SetPwdsysadm :
      ...
      find first _user no-lock
                    where _user._userid    = mloginsysadm
                    no-error
                    .
      ...
      then do trans:
         ...
         then do:
            put-log("Cмена пароля пользователя " + mloginsysadm + " не требуется.").
            return.
         end.
         else do:
            put-log("Начинаем смену пароля " + mloginsysadm + ".").
         end.
         if _User._Password eq encode(mcurpas)
         then do:
            put-log("Подключаемся пользователем " + mloginsysadm + " и меняем свой пароль.").
            ...
            release _User.
            mConPar = substitute(mConPar, "-U ":U + mloginsysadm + " -P " + mcurpas).
            if isRcode
         ...
         then do:
            run trg/userlog.p (
                  input 'sysadm-pwd'
                , input ("Изменен пароль пользователя " + mloginsysadm + chr(3) + _user._userid )
                , input ?
                , input ?
                , input "") no-error.
            put-log("У пользователя mloginsysadm Установлен новый пароль.").
         end.
         else do trans:
...
if ichgpsw
then do:
define variable vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile:currsysk.i $ $Revision: $".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run currsysk in g#library
  (output v-sys-key
  ) no-error .
   if v-sys-key begins "Rosneft-" or v-sys-key eq "yukos" or v-sys-key eq "ibs" or v-sys-key eq   'ExpertekIBS':U
   then do:
      run SetPwdsysadm .
      mloginsysadm = "odbc".
      run SetPwdsysadm .
   ...
   do on error   undo MAIN-BLOCK, leave MAIN-BLOCK
      ...
      then do:
         ...
         assign
            ...
            user-login.status_                 = 0
            user-account.status_               = 0
         .
define variable vss-include-info6 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    if ub.user-login.status_ = 0
    then do:
      find first ub.user-account
           where ub.user-account.user-id = ub.user-login.user-id
           no-lock
           no-error .
      if not available ub.user-account
      then do:
        create ub.user-account.
        assign
            ub.user-account.user-id               = ub.user-login.user-id
            ub.user-account.status_               = 0
            ub.user-account.first-name            = '':U
            ub.user-account.second-name           = '':U
            ub.user-account.last-name             = ub.user-login.user-login
            ub.user-account.nik                   = ub.user-login.user-login
            ub.user-account.company               = '':U
            ub.user-account.department            = '':U
            ub.user-account.e-mail                = '':U
            ub.user-account.internal-phone-number = '':U
            ub.user-account.mobile-phone-number   = '':U
            ub.user-account.phone-number          = '':U
            ub.user-account.position              = '':U
            ub.user-account.PS                    = '':U
            ub.user-account.room                  = '':U
            ub.user-account.parent-user-id        = '':U
            ub.user-account.check-parent          = false
        .
      end.
      else
         ub.user-login.status_ = ub.user-account.status_.
   end.
   if ub.user-login.status_ = 0
   then do:
      find first ub._user
           where ub._user._userid    = ub.user-login.user-login
           no-error
           .
      if not available ub._user then do:
         create ub._user .
         assign
            ub._user._userid    = ub.user-login.user-login
            ub._user._password  = ub.user-login.user-password-encoded
         .
      end.
      ELSE DO:
         BUFFER-COPY ub._User EXCEPT ub._User._Password ub._User._TenantId TO tempUser ASSIGN tempUser._Password = ub.user-login.user-password-encoded
         .
         DELETE ub._User.
         CREATE ub._User.
      ...
      END.
      assign
        ub._user._user-name = substitute('&1 &2 &3'
                                        ,ub.user-account.last-name
                                        ,ub.user-account.first-name
                                        ,ub.user-account.second-name
                                        )
      .
      release _user.
    end.
