...
def var vss-include-info83 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
procedure check-use-bar-code :
  define input  parameter p-b-code    like ub.bar-code.b-code no-undo .
  do
  on error  undo, return error substitute( "&1. &2&3&4", vss-include-info83, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
  on stop   undo, return error substitute( "&1. stop", vss-include-info83 )
  on endkey undo, return error substitute( "&1. endkey", vss-include-info83 )
  :
    define buffer buf_bar-code for ub.bar-code .
    find first buf_bar-code no-lock
    ...
    if not available buf_bar-code then do:
      return error substitute( "&1 (check-use-bar-code). Не найден бар-код &2", vss-include-info83, p-b-code ) .
    end.
...
:
  ...
  :
    find first buf_price-list-type-attr exclusive-lock where
          ...
          and buf_price-list-type-attr.attr-code = 'ie-gen-marg':U no-error.
  ...
  :
    ...
    find first buf_thbj-attr exclusive-lock where
          ...
          and buf_thbj-attr.prop-code = 'pr-goods':U
          and buf_thbj-attr.upper-prop-code = 'overval':U no-error.
  ...
  :
    ...
    find first buf_thbj-attr exclusive-lock where
          ...
          and buf_thbj-attr.prop-code = 'pr-goods0':U
          and buf_thbj-attr.upper-prop-code = 'overval':U no-error.
    if available buf_thbj-attr then do:
      buf_thbj-attr.property-value-character =  buf_rep-line.name2 .
    end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
    ...
    for each buf_price-list-type-attr no-lock where
        ...
        and buf_price-list-type-attr.attr-code = 'ie-gen-marg':U
    on error  undo main-block, return error substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
    on stop   undo main-block, return error substitute( "&1. stop", vss-workfile )
    on endkey undo main-block, return error substitute( "&1. endkey", vss-workfile )
    :
      ...
      .
      buf_price-list-type-attr.attr-value = 'no-margin':U.
    end.
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
    ...
    on endkey undo main-block, return error substitute( "&1. endkey", vss-workfile )
    :
      if thbjattr_thbj-attr.prop-code = 'pr-goods':U then do:
        find first buf_rep-line exclusive-lock where
                  buf_rep-line.code1   = ii
      ...
      if thbjattr_thbj-attr.prop-code = 'pr-goods0':U then do:
        find first buf_rep-line exclusive-lock where
                  buf_rep-line.code1   = ii
