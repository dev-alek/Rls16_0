...
:
 find ub.sys-ctrl No-LOCK.
  ...
  if ub.sys-ctrl.db-num = 0 then do:
    assign var-last-code = 2000 * (integer (current-value(s-ctgb-code, ub) / 2000 )  + 1)  .
    ...
    for each ub.db no-lock where ub.db.db-num > 0 :
      find first buf_code-range no-lock where buf_code-range.db-num = ub.db.db-num and buf_code-range.range-type = 'ctgb':U no-error .
