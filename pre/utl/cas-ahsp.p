...
define variable vss-workfile    as character no-undo initial "$Workfile: cas-ahsp.p $":U .
define variable vss-archive     as character no-undo initial "$Archive: utl/cas-ahsp.p $":U .
...
define frame a
  ...
  title "Проверка складского архива по поставщикам"
  .
do
on error undo, return error return-value
:
  assign
    v-log-file-name   = 'cas-ahsp.log':U
    v-error-file-name = 'cas-ahsp.txt':U
  .
...
define variable vss-include-info3 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  run gbl/d-askw.w
    (input "Вопрос"
    ,input "Проверка целостности складского архива по поставщикам" + chr(10)
    ,input "|^"
  ...
  case v-num :
    ...
    then do:
      ...
      :
        ...
        then do:
          run check-archive-ahsp in this-procedure
...
procedure check-archive-ahsp :
  ...
  :
    ...
    export stream sout "Начало проверки целостности складского архива по поставщика" .
    ...
    then do:
      ...
      export stream sout '***':u string(v-today, '99/99/9999':u) string(v-time, 'HH:MM:SS':u) .
      export stream sout "Завершена проверка складского архива по поставщикам" .
      export stream sout "Объект" p-obj-type p-obj-code .
