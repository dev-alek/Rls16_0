...
END.
ON CHOOSE OF b-inq IN FRAME Dialog-Frame
DO:
...
assign
  v-str-recid3 = trim( string( recid( ub.clients ) , "->>>>>>>>>>>9":U ) )
  v-num-entry3 = lookup( v-str-recid3 , v-rid-list )
.
if v-num-entry3 > 0 then do:
  assign
    entry( v-num-entry3, v-rid-list ) = "":U
    v-rid-list = trim( replace( v-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rid-list = v-rid-list + ( if v-rid-list = "":U then "":U else chr(44) ) + v-str-recid3
  .
end.
    end.
    else do:
  ...
  :
    assign
      v-recid = integer( entry( v-ind, v-rid-list, chr(44) ) )
      no-error
    .
    ...
    if not available buf_clients then do:
      ...
      assign
        v-obj-list = v-obj-list + chr(44) + substitute( "&1&2&3", buf_clients.obj-type, chr(44), buf_clients.obj-code )
      .
    end.
  end.
  assign
    v-obj-list = left-trim( v-obj-list, chr(44) )
  .
  run delete-object in this-procedure
    (input v-obj-list
    ,input "inquiry":U
...
assign
  v-str-recid5 = trim( string( recid( ub.clients ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry5 > 0 then do:
  ...
  .
    ...
    loc#log = br-cli:refresh() .
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-cli:select-next-row ().
        apply "VALUE-CHANGED" to br-cli in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-cli in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-cli :handle
      .
...
PROCEDURE delete-object :
  ...
  :
    ...
    .
    run gbl/d-askw.w
      (input "Удаление объекта(ов)"
      ,input "Вы хотите удалить объект(ы)." + chr(10)
        + substitute("&1", p-obj-list) + chr(10)
        + "Вы действительно хотите сделать это?" + chr(10)
      ,input "|^"
      ,input "С проверкой остатков^confirm|Без проверки остатков^confirm|Отказ"
      ,input "|"
          + "|"
          + ""
      ,input 1
      ...
      when "delete":U then do:
        ...
        then do:
          ...
          :
            ...
            if num-entries( v-check-string, chr(44) ) > 3
              and num-entries( v-passwd, chr(44) ) = 1
              and v-passwd <> "":U
            then do:
              assign
                p-obj-list  = p-obj-list + chr(44) + entry( 2, v-check-string, chr(44) ) + chr(44) + entry( 3, v-check-string, chr(44) )
                v-pswd-list = v-pswd-list + chr(44) + v-passwd
              .
            end.
          end.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-cli FOR EACH ub.clients       WHERE ( ( clients.obj-type = 'маг':U     and clients.obj-code > 0    )   OR   ( clients.obj-type = 'скл':U     and clients.obj-code > 0    ) ) NO-LOCK.
END PROCEDURE.
