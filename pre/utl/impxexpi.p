...
p-cdrg = logical(entry(8, p-parameter, chr(4)))
p-cdk = logical(entry(9, p-parameter, chr(4)))
p-thb = logical(entry(10, p-parameter, chr(4)))
p-pet = logical(entry(11, p-parameter, chr(4)))
p-db-key = entry(12, p-parameter, chr(4))
p-dir-name  = entry(13, p-parameter, chr(4))
p-glb       = logical(entry(14, p-parameter, chr(4)))
p-version   = entry(15, p-parameter, chr(4))
p-old-host-code = integer(entry(16, p-parameter, chr(4)))
p-new-host-code = integer(entry(17, p-parameter, chr(4)))
p-old-obj-type  = entry(18, p-parameter, chr(4))
p-new-obj-type  = entry(19, p-parameter, chr(4))
p-old-obj-code  = integer(entry(20, p-parameter, chr(4)))
p-new-obj-code  = integer(entry(21, p-parameter, chr(4)))
p-from-version = entry(22, p-parameter, chr(4))
no-error
.
...
case p-from-version:
  when 'v15_0000':U then do:
    ...
    v-cli-classif-name = 'th-th150_goods':U.
  end.
  when 'v14_0':U then do:
    v-classif-name = 'th-th14_goods':U.
    v-cli-classif-name = 'th-th14_goods':U.
  end.
  otherwise do:
    message
...
ii = 0.
if p-gen then do:
        run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Проверка группы данных ИНФОРМАЦИЯ О БД (НАСТРОЙКИ)" ) ).
      _configv:
  FOR EACH temp-config:
    ...
    if not available ub.config then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ИНФОРМАЦИЯ О БД (НАСТРОЙКИ)" + chr(10) + " НАСТРОЕЧНЫЙ ПАРАМЕТР(config)" +                                         " " + temp-config.param-code +                                         " Фирма " + string(temp-config.host-code) +                                         " тип объекта " + temp-config.obj-type +                                         " код объекта " + string(temp-config.obj-code) +                                         " не является валидным в данной конфигурации" ) )) .              delete temp-config.                   NEXT _configv.
    end.
    if lookup(ub.config.conf-type, 'к,п':U) > 0
    then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ИНФОРМАЦИЯ О БД (НАСТРОЙКИ)" + chr(10) + " НАСТРОЕЧНЫЙ ПАРАМЕТР(config)" +                                         " " + temp-config.param-code +                                         " является кодированным - импорт запрещен" ) )) .              delete temp-config.                   NEXT _configv.
    end.
    ...
    if temp-config.host-code > 0 then do:
      ...
      IF NOT AVAIL ub.sysconf then do:
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ИНФОРМАЦИЯ О БД (НАСТРОЙКИ)" + chr(10) + " Отсутствует фирма для НАСТРОЕЧНОГО ПАРАМЕТРА(config)" +                                           " параметр " + temp-config.param-code +                                           " Фирма " + string(temp-config.host-code) +                                           " тип объекта " + temp-config.obj-type +                                           " код объекта " + string(temp-config.obj-code) ) )) .              delete temp-config.                   NEXT _configv.
      END.
    end.
    if temp-config.obj-type <> "" or temp-config.obj-code > 0 then do:
      ...
      IF NOT AVAIL ub.clients then do:
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ИНФОРМАЦИЯ О БД (НАСТРОЙКИ)" + chr(10) + " Отсутствует объект для НАСТРОЕЧНОГО ПАРАМЕТРА(config)" +                                           " параметр " + temp-config.param-code +                                           " Фирма " + string(temp-config.host-code) +                                           " тип объекта " + temp-config.obj-type +                                           " код объекта " + string(temp-config.obj-code) ) )) .              delete temp-config.                   NEXT _configv.
      END.
      IF ub.clients.db-num <> ub.db.db-num then do:
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ИНФОРМАЦИЯ О БД (НАСТРОЙКИ)" + chr(10) + " Объект для НАСТРОЕЧНОГО ПАРАМЕТРА(config) принадлежит другой БД" +                                           " параметр " + temp-config.param-code +                                           " Фирма " + string(temp-config.host-code) +                                           " тип объекта " + temp-config.obj-type +                                           " код объекта " + string(temp-config.obj-code) ) )) .              delete temp-config.                   NEXT _configv.
      END.
    end.
    create buf_config.
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ИНФОРМАЦИЯ О БД (НАСТРОЙКИ)" + chr(10) +  " ошибка при сохранении НАСТРОЕЧНОГО ПАРАМЕТРA(config):" +                                       " параметр " + temp-config.param-code +                                       " Фирма " + string(temp-config.host-code) +                                       " тип объекта " + temp-config.obj-type +                                       " код объекта " + string(temp-config.obj-code) +                                       er-mes) )) .                NEXT _configv.
    end.
  END.
end.
if p-thb then do:
        run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Проверка группы данных ПАРАМЕТРЫ" ) ).
      _thbj-attrv:
  FOR EACH temp-thbj-attr:
    if temp-thbj-attr.obj-type <> "" or temp-thbj-attr.obj-code > 0 then do:
      ...
      IF NOT AVAIL ub.clients then do:
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ПАРАМЕТРЫ" + chr(10) + " Отсутствует объект для НАСТРОЕЧНОГО ПАРАМЕТРА(thbj-attr)" +                                          " секция " + temp-thbj-attr.upper-prop-code +                                           " параметр " + temp-thbj-attr.prop-code +                                           " тип объекта " + temp-thbj-attr.obj-type +                                           " код объекта " + string(temp-thbj-attr.obj-code) ) )) .              delete temp-thbj-attr.                   NEXT _thbj-attrv.
      END.
      IF (ub.clients.obj-type = 'маг':U
      or ub.clients.obj-type = 'скл':U)
      and ub.clients.db-num <> ub.db.db-num then do:
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ПАРАМЕТРЫ" + chr(10) + " Объект для НАСТРОЕЧНОГО ПАРАМЕТРА(thbj-attr) принадлежит другой БД" +                                          " секция " + temp-thbj-attr.upper-prop-code +                                           " параметр " + temp-thbj-attr.prop-code +                                           " тип объекта " + temp-thbj-attr.obj-type +                                           " код объекта " + string(temp-thbj-attr.obj-code) ) )) .              delete temp-thbj-attr.                   NEXT _thbj-attrv.
      END.
    end.
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ПАРАМЕТРЫ" + chr(10) +  " ошибка при сохранении НАСТРОЕЧНОГО ПАРАМЕТРA(thbj-attr):" +                                       " параметр " + temp-thbj-attr.prop-code +                                       " тип объекта " + temp-thbj-attr.obj-type +                                       " код объекта " + string(temp-thbj-attr.obj-code) +                                       er-mes) )) .                NEXT _thbj-attrv.
    end.
  END.
end.
if p-cdrg then do:
  ...
  if ub.sys-ctrl.db-num = 0 then do:
                run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Проверка группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ" ) ).
            find first temp-code-range no-lock
      where temp-code-range.range-type = 'sclc':U
         or temp-code-range.range-type = 'scgb':U
         or temp-code-range.range-type = 'pglc':U
      no-error .
    if available temp-code-range then do:
      on delete of ub.code-range override do: end.
      for each ub.code-range exclusive-lock
        where ub.code-range.range-type = 'sclc':U
           or ub.code-range.range-type = 'scgb':U
           or ub.code-range.range-type = 'pglc':U
      on error  undo, retry
      on stop   undo, retry
      on endkey undo, retry
      :
        if retry then do:
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ" + chr(10) + " Ошибка при удалении диапазонов кодов (code-range)" ) )) .
        end.
      ...
      :
        ...
        if error-status:error then do:
          er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ" + chr(10) + " ошибка при сохранении записи ДИАПАЗОН КОДОВ (code-range):" +                           " тип " + temp-code-range.range-type +                           " начало " + string(temp-code-range.first-code) +                           " окончание " + string(temp-code-range.last-code) +                           er-mes) )) .                NEXT _code-rg.
        end.
      END.
    end.
    else do:
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ" + chr(10) + " в файле импорта нет ни одной записи типа ДИАПАЗОН КОДОВ (code-range)" ) )) .
    end.
  end.
  else do:
        run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ" + chr(10) + " импорт записей типа ДИАПАЗОН КОДОВ (code-range) разрешен только в ГБД" ) )) .
  end.
end.
if p-flt then do:
...
if p-pbc then do:
  ...
  FOR EACH temp-prod-bc:
    ...
    IF AVAILABLE ub.prod-bc then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Уже есть ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
    end.
    find first ub.prod-bc no-lock where
              ub.prod-bc.b-str = temp-prod-bc.b-str
          AND ub.prod-bc.bc-on = yes no-error.
    IF AVAILABLE ub.prod-bc then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Уже есть такой включенный ДопБК(prod-bc) на другом товаре:" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод имеющегося включенного ДопБК" + string(ub.prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
    end.
    find first ub.bar-code no-lock where
               ub.bar-code.b-code = temp-prod-bc.b-code no-error.
    if not avail ub.bar-code then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Не найден баркод для ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
    end.
    FIND FIRST ub.goods no-lock where
              ub.goods.gds-code = ub.bar-code.gds-code  NO-ERROR.
    if not avail ub.bar-code then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Не найден товар по баркоду для ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
    end.
    find first ub.units no-lock where
               ub.units.unit-name = ub.goods.unit-base no-error .
    if not avail ub.units then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Не найдена основная ед. изм товара для ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code) +                         " Товар " + ub.goods.artic + chr(32) + ub.goods.prod-type + string(ub.goods.prod-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
    end.
    ...
    then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Товар по ДопБК(prod-bc) НЕ ((весовой или штучный) и ДопБК - код для весов):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code) +                         " Товар " + ub.goods.artic + chr(32) + ub.goods.prod-type + string(ub.goods.prod-code) +                         " Основн. ед. изм" + ub.goods.unit-base) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
    end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output r-bar-code
      ...
      if error-status:error then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Ошибка при поиске основного баркода товара для ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      if r-bar-code <> ub.bar-code.b-code then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Баркод для ДопБК(prod-bc) не является основным баркодом товара:" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbctv in g#library
  ...
  ,output v-is-global
      ...
      if error-status:error then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Ошибка при проверке на локальность ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      if v-is-global
      and not p-glb
      then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " ДопБК(prod-bc) не локальный:" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbctv in g#library
  ...
  ,output v-is-weight
      ...
      if error-status:error then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Ошибка при проверке весовой ли ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbctv in g#library
  ...
  ,output v-is-pgweight
      ...
      if error-status:error then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Ошибка при проверке штучный ли ДопБК для весов(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbctv in g#library
  ...
  ,output v-is-petrolium
      ...
      if error-status:error then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Ошибка при проверке на топливность ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      if not v-is-weight
      and not v-is-pgweight
      and not v-is-petrolium
      then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " ДопБК(prod-bc) не код для весов:" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      ...
      if not avail cli_units then do:
                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Не найдена ед. изм баркода для ДопБК(prod-bc):" +                           " ДопБК " + string(temp-prod-bc.b-str) +                           " Баркод " + string(temp-prod-bc.b-code) +                           " Ед.изм.баркода "  + ub.bar-code.unit-cli) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      if LOOKUP('дро':U, cli_units.type) = 0 then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Единица измерения баркода по ДопБК(prod-bc) не весовая и не дробная:" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code) +                         " Ед. изм" + ub.bar-code.unit-cli) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      if ub.bar-code.part-code <> "":U or ub.bar-code.in-code <> "":U then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Баркод для ДопБК(prod-bc) не является баркодом товара на доп.ед.изм:" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code) +                         " in-code " + ub.bar-code.in-code +                         " part-code " + ub.bar-code.part-code ) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbctv in g#library
  ...
  ,output v-is-scaleable
      ...
      if error-status:error then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Ошибка при проверке взвешиваемый ли ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      if not v-is-scaleable then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " ДопБК(prod-bc) не весовой:" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code)) )) .              delete temp-prod-bc.                   NEXT _prod-bcv.
      end.
      v-cdrg-type = 'sslc':U.
    end.
    v-pbc-rid = ?.
    ...
    then do:
      ...
      er-mes = er-mes + chr(10) + return-value .
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " ошибка при сохранении записи ДопБК(prod-bc):" +                         " ДопБК " + string(temp-prod-bc.b-str) +                         " Баркод " + string(temp-prod-bc.b-code) +                       er-mes) )) .                NEXT _prod-bcv.
    end.
  END.
  ...
  if available buf_code-range then do:
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " ошибка при получении max кода диапазона локальных штучных кодов для весов(prod-bc):" +                       er-mes) )) .
    end.
    if v-b-code <= buf_code-range.last-code then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output r-bar-code
    ...
    if error-status:error then do:
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕС И ВЗВЕШ КОДЫ" + chr(10) + " Ошибка при поиске основного баркода товара  для АТРИБУТА ТОВАРА ВЕСОВОЙ КОД НА ОБЪЕКТЕ(gds-obj-attr) принадлежит другой БД:" +                       " код товара " + string(temp-gds-obj-attr.gds-code) +                       " объект " + temp-gds-obj-attr.obj-type + string(temp-gds-obj-attr.obj-code) +                       " весовой код " + temp-gds-obj-attr.attr-value +                       er-mes) )) .              delete temp-gds-obj-attr.                   NEXT _gds-obj-attrv.
    end.
    find first ub.prod-bc no-lock where
...
if p-scl then do:
  ...
  FOR EACH temp-scales-gds :
    ...
    IF AVAILABLE ub.scales-gds then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " Уже есть ТОВАР НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code)) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    end.
    FIND FIRST ub.bar-code No-LOCK WHERE
               ub.bar-code.b-code = temp-scales-gds.b-code No-ERROR.
    IF NOT AVAIL ub.bar-code then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " Не найден БАРКОД для ТОВАРА НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code)) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    END.
    FIND FIRST ub.goods No-LOCK WHERE
               ub.goods.gds-code = ub.bar-code.gds-code NO-ERROR.
    IF NOT AVAILABLE ub.goods then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " Не найден ТОВАР для ТОВАРА НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code)) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    END.
    FIND FIRST ub.units No-LOCK WHERE
               ub.units.unit-name = ub.goods.unit-base No-ERROR.
    IF NOT AVAIL ub.units then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " Не найдена ЕДИНИЦА ИЗМЕРЕНИЯ для ТОВАРА НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code)) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    END.
    if ub.units.unit-name <> scales-unit
    and ub.units.type = 'вес':U
    then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " ЕДИНИЦА ИЗМЕРЕНИЯ ТОВАРА НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code) +                         " единица измерения товара " + ub.units.unit-name +                         " единица измерения весов " + scales-unit ) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    END.
    FIND FIRST ub.gds-prt No-LOCK WHERE
               ub.gds-prt.upper-code = ub.goods.prt-root NO-ERROR.
    IF NOT AVAIL(ub.gds-prt) then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " Не найдена ШКАЛА ПРИЗНАКОВ(пустая шкала) для ТОВАРА НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code)) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    END.
    if ub.bar-code.node-code <> ub.gds-prt.node-code OR
      ...
      ub.bar-code.unit-cli <> ub.goods.unit-base then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " Баркод не является главным баркодом товара для ТОВАРА НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code)) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    END.
    ...
    if error-status:error then dO:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных ВЕСЫ" + chr(10) + " Ошибка при создании ТОВАРА НА ВЕСАХ(scales-gds):" +                         " номер весов " + string(temp-scales-gds.scales-num) +                         " баркод " + string(temp-scales-gds.b-code) +                         return-value) )) .              delete temp-scales-gds.                   NEXT _scales-gdsv.
    end.
...
if p-cdk then do:
        run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Проверка группы данных КАССЫ" ) ).
      _cash-deskv:
  FOR EACH temp-cash-desk:
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КАССЫ" + chr(10) +  " ошибка при сохранении записи КАССА(cash-desk):" +                                       " номер " + string(temp-cash-desk.cash-num) +                                       er-mes) )) .                NEXT _cash-deskv.
    end.
  END.
end.
if p-pet then do:
  if p-pet then do:
    run p-pet-i in this-procedure.
        ...
        run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Проверка группы данных КОНФИГУРАЦИЯ АЗК" ) ).
      _placev:
  FOR EACH temp-place:
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи СКЛ.МЕСТО(place):" +                                       " номер " + string(temp-place.pl-code) +                                       er-mes) )) .                NEXT _placev.
    end.
    find first ub.place no-lock where
  ...
  for each temp-pump:
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи ТРК(pump):" +                                       " номер " + string(temp-pump.pump-code) +                                       er-mes) )) .                NEXT _pumpv.
    end.
  end.
      _nozzlev:
  for each temp-nozzle:
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи ПИСТОЛЕТ(nozzle):" +                                       " номер " + string(temp-nozzle.nozzle-code) +                                       er-mes) )) .                NEXT _nozzlev.
    end.
  end.
      _nozzlev:
  for each temp-pump-nozzle:
    FIND FIRST ub.nozzle No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.nozzle then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найден ПИСТОЛЕТ для ПРИВЯЗКИ ТРК К ПИСТОЛЕТУ(pump-nozzle):" +                          " пистолет" + string(temp-pump-nozzle.nozzle-code) + " код ТРК " + string(temp-pump-nozzle.pump-code )) )) .              delete temp-pump-nozzle.                   NEXT _nozzlev.
    end.
    FIND FIRST ub.pump No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.pump then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено ТРК для ПРИВЯЗКИ ТРК к ПИСТОЛЕТУ(pump-nozzle):" +                          " код пистолета " + string(temp-pump-nozzle.nozzle-code)  + " код ТРК " + string(temp-pump-nozzle.pump-code )) )) .              delete temp-pump-nozzle.                   NEXT _nozzlev.
    end.
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи СВЯЗКИ ТРК-ПИСТОЛЕТ(pump-nozzle):" +                                       " ПИСТОЛЕТ " + string(temp-pump-nozzle.nozzle-code) +                                       " ТРК " + string(temp-pump-nozzle.pump-code) +                                       er-mes) )) .                NEXT _nozzlev.
    end.
  end.
      _pl-gdsv:
  for each temp-pl-gds:
    ...
    if not avail ub.goods then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Нет товара для товара на скл.месте(pl-gds):" +                          " код товара " + string(temp-pl-gds.gds-code)  + " код скл.места " + string(temp-pl-gds.pl-code) ) )) .              delete temp-pl-gds.                   NEXT _pl-gdsv.
    end.
    FIND FIRST ub.place No-LOCK WHERE
    ...
    IF not AVAILABLE ub.place then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено СКЛ.МЕСТО для ПРИВЯЗКИ ТОВАРА К СКЛ.МЕСТУ(pl-gds):" +                          " код товара " + string(temp-pl-gds.gds-code)  + " код скл.места " + string(temp-pl-gds.pl-code) ) )) .              delete temp-pl-gds.                   NEXT _pl-gdsv.
    end.
    ...
    IF AVAILABLE ub.pl-gds then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Уже есть ПРИВЯЗКА ЭТОГО ТОВАРА К ЭТОМУ СКЛ.МЕСТУ(pl-gds):" +                          " код товара " + string(temp-pl-gds.gds-code)  + " код скл.места " + string(temp-pl-gds.pl-code) ) )) .              delete temp-pl-gds.                   NEXT _pl-gdsv.
    end.
    ...
    do transaction on error undo, next :
      ...
      if not available buf_pl-gds then do:
        ...
        and lookup('дро':U, buf_units.type) > 0 then do:
          run trg/plgdpmvc.p (
          ...
          or not glog
          then do:
                        run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + "Ошибка при привязке товара к резервуару(pl-gds):" +                              " код товара " + string(temp-pl-gds.gds-code)  + " код скл.места " + string(temp-pl-gds.pl-code) +                             error-status:get-message(1) + chr(10) + return-value ) )) .              delete temp-pl-gds.                   NEXT _pl-gdsv.
          end.
        END.
        else do:
          run trg/plgdpmv0.p (
  ...
  for each temp-pl-gds-pump:
    ...
    if not avail ub.goods then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Нет товара для товара на скл.месте и ТРК(pl-gds-pump):" +                          " код товара " + string(temp-pl-gds-pump.gds-code)  + " код скл.места " + string(temp-pl-gds-pump.pl-code) + " код ТРК " + string(temp-pl-gds-pump.pump-code )) )) .              delete temp-pl-gds-pump.                   NEXT _pl-gds-pumpv.
    end.
    FIND FIRST ub.place No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.place then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено СКЛ.МЕСТО для ПРИВЯЗКИ ТОВАРА К СКЛ.МЕСТУ и ТРК(pl-gds-pump):" +                          " код товара " + string(temp-pl-gds-pump.gds-code)  + " код скл.места " + string(temp-pl-gds-pump.pl-code) + " код ТРК " + string(temp-pl-gds-pump.pump-code)) )) .              delete temp-pl-gds-pump.                   NEXT _pl-gds-pumpv.
    end.
    FIND FIRST ub.pump No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.pump then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено ТРК для ПРИВЯЗКИ ТОВАРА К СКЛ.МЕСТУ и ТРК(pl-gds-pump):" +                          " код товара " + string(temp-pl-gds-pump.gds-code)  + " код скл.места " + string(temp-pl-gds-pump.pl-code) + " код ТРК " + string(temp-pl-gds-pump.pump-code )) )) .              delete temp-pl-gds-pump.                   NEXT _pl-gds-pumpv.
    end.
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи ПРИВЯЗКИ ТОВАРА К СКЛ.МЕСТУ и ТРК(pl-gds-pump):" +             " код товара " + string(temp-pl-gds-pump.gds-code)  + " код скл.места " + string(temp-pl-gds-pump.pl-code) + " код ТРК " + string(temp-pl-gds-pump.pump-code ) +                                       er-mes) )) .                NEXT _pl-gds-pumpv.
    end.
  end.
      _pl-pumpv:
  for each temp-pl-pump:
    FIND FIRST ub.place No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.place then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено СКЛ.МЕСТО для ПРИВЯЗКИ ТРК К СКЛ.МЕСТУ(pl-pump):" +                          " код скл.места " + string(temp-pl-pump.pl-code) + " код ТРК " + string(temp-pl-pump.pump-code)) )) .              delete temp-pl-pump.                   NEXT _pl-pumpv.
    end.
    FIND FIRST ub.pump No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.pump then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено ТРК для ПРИВЯЗКИ ТРК К СКЛ.МЕСТУ(pl-pump):" +                          " код скл.места " + string(temp-pl-pump.pl-code) + " код ТРК " + string(temp-pl-pump.pump-code )) )) .              delete temp-pl-pump.                   NEXT _pl-pumpv.
    end.
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи ПРИВЯЗКИ ТРК К СКЛ.МЕСТУ(pl-pump):" +                                                 " код скл.места " + string(temp-pl-pump.pl-code) + " код ТРК " + string(temp-pl-pump.pump-code ) +                                       er-mes) )) .                NEXT _pl-pumpv.
    end.
  end.
      _pl-pump-nozzlev:
  for each temp-pl-pump-nozzle:
    FIND FIRST ub.place No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.place then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено СКЛ.МЕСТО для ПРИВЯЗКИ ТРК К СКЛ.МЕСТУ и ПИСТОЛЕТУ(pl-pump-nozzle):" +                          " пистолет" + string(temp-pl-pump-nozzle.nozzle-code) + " код скл.места " + string(temp-pl-pump-nozzle.pl-code) + " код ТРК " + string(temp-pl-pump-nozzle.pump-code)) )) .              delete temp-pl-pump-nozzle.                   NEXT _pl-pump-nozzlev.
    end.
    FIND FIRST ub.pump No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.pump then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найдено ТРК для ПРИВЯЗКИ ТРК К СКЛ.МЕСТУ и ПИСТОЛЕТУ(pl-pump-nozzle):" +                          " пистолет" + string(temp-pl-pump-nozzle.nozzle-code) + " код скл.места " + string(temp-pl-pump-nozzle.pl-code) + " код ТРК " + string(temp-pl-pump-nozzle.pump-code )) )) .              delete temp-pl-pump-nozzle.                   NEXT _pl-pump-nozzlev.
    end.
    FIND FIRST ub.nozzle No-LOCK WHERE
               ...
               NO-ERROR.
    IF not AVAILABLE ub.nozzle then do:
                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Не найден ПИСТОЛЕТ для ПРИВЯЗКИ ТРК К СКЛ.МЕСТУ и ПИСТОЛЕТУ(pl-pump-nozzle):" +                          " пистолет" + string(temp-pl-pump-nozzle.nozzle-code) + " код скл.места " + string(temp-pl-pump-nozzle.pl-code) + " код ТРК " + string(temp-pl-pump-nozzle.pump-code )) )) .              delete temp-pl-pump-nozzle.                   NEXT _pl-pump-nozzlev.
    end.
    ...
    if error-status:error then do:
      er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
            run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи ПРИВЯЗКИ ТРК К СКЛ.МЕСТУ и ПИСТОЛЕТУ(pl-pump-nozzle):" +                         " пистолет" + string(temp-pl-pump-nozzle.nozzle-code) + " код скл.места " + string(temp-pl-pump-nozzle.pl-code) + " код ТРК " + string(temp-pl-pump-nozzle.pump-code ) +                                       er-mes) )) .                NEXT _pl-pump-nozzlev.
    end.
  end.
end.
if p-rht then do:
      ...
      when "15.0":U then do:
         ...
         FOR EACH temp-action-role:
            ...
            IF AVAILABLE ub.action-role then do:
                              run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Уже есть ГРУППА ПРАВ (action-role):"                                             + STRING(temp-action-role.db-num)                                             + STRING(temp-action-role.action-head-code)                                             + STRING(temp-action-role.action-role-code)                                             ) )) .              delete temp-action-role.                   NEXT _action-role.
            END.
            ...
            if error-status:error then do:
               er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
                              run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении записи ГРУППЫ ПРАВ (action-role):"                                             + STRING(temp-action-role.db-num)                                             + STRING(temp-action-role.action-head-code)                                             + STRING(temp-action-role.action-role-code)                                             ) )) .                NEXT _action-role.
            end.
         END.
         create alias restseq    for database value( ldbname( "ub":U ) ) .
         create alias restseqflt for database value( ldbname( "ubflt":U ) ) .
         ...
         FOR EACH temp-action-role-item:
            ...
            IF AVAILABLE ub.action-role-item then do:
                              run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) + " Уже есть привязка к ГРУППЕ ПРАВ (action-role-item):"                                             + STRING(temp-action-role-item.db-num)                                             + STRING(temp-action-role-item.action-head-code)                                             + STRING(temp-action-role-item.action-role-code)                                             + STRING(temp-action-role-item.action-role-item-code)                                             ) )) .              delete temp-action-role-item.                   NEXT _action-role-item.
            END.
            ...
            if error-status:error then do:
               er-mes = "".       do jj = 1 to error-status:num-messages:         er-mes = er-mes + chr(10) + error-status:get-message(JJ).       end.
                              run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Проверка группы данных КОНФИГУРАЦИЯ АЗК" + chr(10) +  " ошибка при сохранении привязки к ГРУППЕ ПРАВ (action-role-item):"                                             + STRING(temp-action-role-item.db-num)                                             + STRING(temp-action-role-item.action-head-code)                                             + STRING(temp-action-role-item.action-role-code)                                             + STRING(temp-action-role-item.action-role-item-code)                                             ) )) .                NEXT _action-role-item.
            end.
         END.
         create alias restseq    for database value( ldbname( "ub":U ) ) .
         create alias restseqflt for database value( ldbname( "ubflt":U ) ) .
         ...
         :
            ...
            :
               ...
               if not available buf_user-menu-group then do:
                  ASSIGN
                     v-user-menu-group-code = dynamic-next-value("s-user-menu-group":U, "ub":U)
                  .
...
define variable v-int1 as integer no-undo .
  ...
  on error undo, return error
  :
  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Импорт группы данных ПАРАМЕТРЫ" + " Файл: " + p-dir-name + "\":U + p-db-key + ".":U + "thb":U) )) .
  run check-iefile in this-procedure(input p-dir-name,                                      input "thb":U,                                      input "import":U,                                      output loc#log).
  ii = 0.
  if loc#log then do:
    run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Импорт группы данных ПАРАМЕТРЫ" ) ).
    input stream Instream from value(p-dir-name + "\":U + p-db-key + ".":U + "thb":U).
    _thb:
    REPEAT:
      import stream Instream unformatted ss.
      CASE ss:
          ...
          case current-table:
            when 'thbj-attr':U then do:
              ...
              import stream Instream  no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ПАРАМЕТРЫ O БД, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _thb.                   end.
              if (buf-thbj-attr.obj-type = 'маг':U
              or buf-thbj-attr.obj-type = 'скл':U )
              ...
              if buf-thbj-attr.obj-type = 'орг':U
              and p-old-host-code <> buf-thbj-attr.obj-code then do:
                delete buf-thbj-attr.
                next _thb.
              end.
              if lookup(buf-thbj-attr.upper-prop-code, "autosale,get-chk,chk-view,cd-sending,cd-inf-send," + "scale-inf,cd-type-ibm,abc-sale-day,abc-global," + "ord-global,ord-obj,fin-global,contr-in,nakl_par," + "rt-trn-doc,gds-ref,gds-ref_obj,cli-all," + "auto-task,Ass-obj") = 0 then do:
                 delete buf-thbj-attr.
                 next _thb.
               end.
              if buf-thbj-attr.obj-type = 'орг':U then do:
                assign
                buf-thbj-attr.obj-code = p-new-host-code.
              end.
              if buf-thbj-attr.obj-type = 'маг':U
              or buf-thbj-attr.obj-type = 'скл':U  then do:
                assign
              ...
              case buf-thbj-attr.upper-prop-code:
                when 'autosale':U then do:
                  case buf-thbj-attr.prop-code:
                    ...
                    when 'wrkr':U then do:
                      ...
                      if error-status:error then do:
                                                run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ПАРАМЕТРЫ O БД, СТРОКА " + string(ii) + chr(10)) + " Для значения параметра не найдено соответствие в таблице соответствий: ПАРАМЕТРА(thbj-attr)" +                                         " Секция " + string(buf-thbj-attr.upper-prop-code) +                                         " параметр " + buf-thbj-attr.prop-code +                                         " тип объекта " + buf-thbj-attr.obj-type +                                         " код объекта " + string(buf-thbj-attr.obj-code) ) )) .              delete buf-config.                   NEXT _thb.
                      end.
                      else do:
                    ...
                    when 'one-sale-per-day':U then do:
                      next _thb.
                    end.
                    when 'close-day-period':U then do:
                      next _thb.
                    end.
                  end case.
                end.
                when 'get-chk':U then do:
                  case buf-thbj-attr.prop-code:
                ...
                when 'cd-type-ibm':U then do:
                  case buf-thbj-attr.prop-code:
                    when 'specgrp':U then do:
                      next _thb.
                    end.
                  end case.
                end.
                when 'ord-obj':U then do:
                  case buf-thbj-attr.prop-code:
                    when 'ord-askp':U then do:
                      next _thb.
                    end.
                    when 'ord-obj-rc':U then do:
                      next _thb.
                    end.
                  end case.
                end.
                when 'rt-trn-doc':U then do:
                  case buf-thbj-attr.prop-code:
                    when 'wrkr':U then do:
                      run convert-thbj-attr-integer-1 in this-procedure ( input buf-thbj-attr.property-value-integer
                                                                         ,output v-int1) no-error.
                ...
                when 'gds-ref':U then do:
                  case buf-thbj-attr.prop-code:
                    when 'dfltggrp':U then do:
                      next _thb.
                    end.
                  end case.
                end.
                when 'gds-ref_obj':U then do:
                  case buf-thbj-attr.prop-code:
                    when 'dfltggrp':U then do:
                      next _thb.
                    end.
                            ...
                            IF CAN-FIND(FIRST temp-thbj-attr No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ПАРАМЕТРЫ O БД, СТРОКА " + string(ii) + chr(10)) + " Уже есть запись ПАРАМЕТРА(thbj-attr)" +                                   " Секция " + string(buf-thbj-attr.upper-prop-code) +                                   " параметр " + buf-thbj-attr.prop-code +                                   " тип объекта " + buf-thbj-attr.obj-type +                                   " код объекта " + string(buf-thbj-attr.obj-code) ) )) .              delete buf-thbj-attr.                   NEXT _thb.
              end.
...
define buffer buf_goods for ub.goods.
  ...
  on error undo, return error
  :
  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ" + " Файл: " + p-dir-name + "\":U + p-db-key + ".":U + "pbc":U) )) .
  run check-iefile in this-procedure(input p-dir-name,                                      input "pbc":U,                                      input "import":U,                                      output loc#log).
  ii = 0.
  if loc#log then do:
    run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ" ) ).
        ...
        when "prod-bc":U then do:
          ...
          case current-table:
            when "prod-bc":U then do:
              ii = ii + 1. run write-counter in p-log-handle (substitute("Считано &1 записей", ii)).
              create buf-prod-bc.
              import stream Instream buf-prod-bc.b-code                                 buf-prod-bc.b-str                                 buf-prod-bc.bc-on
              buf-prod-bc.old-gds-code
              buf-prod-bc.unit-base
              buf-prod-bc.unit-cli
              no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pbc.                   end.
              ...
              if not available buf_ext-classif then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Не найдена запись для баркода в таблице соответствия(prod-bc):" +                                  " ДопБК " + string(buf-prod-bc.b-str) +                                  " Баркод " + string(buf-prod-bc.b-code)) )) .              delete buf-prod-bc.                   NEXT _pbc.
              end.
              ...
              if error-status:error then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Ошибка при восстановлении баркода полученного из таблицы соответствия(prod-bc):" +                                  " ДопБК " + string(buf-prod-bc.b-str) +                                  " Баркод " + string(buf-prod-bc.b-code)) )) .              delete buf-prod-bc.                   NEXT _pbc.
              end.
              find first buf_goods no-lock where
                        rowid(buf_goods) = v-rowid no-error.
              if not available buf_goods then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Не найден товар, полученный из таблицы соответствия(prod-bc):" +                                  " ДопБК " + string(buf-prod-bc.b-str) +                                  " Баркод " + string(buf-prod-bc.b-code)) )) .              delete buf-prod-bc.                   NEXT _pbc.
              end.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  (input  buf_goods.gds-code
  ,input  ?
  ,output v-b-code
              ...
              if error-status:error then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Ошибка при определении главного баркода товара, полученный из таблицы соответствия(prod-bc):" +                                  " ДопБК " + string(buf-prod-bc.b-str) +                                  " Баркод " + string(buf-prod-bc.b-code)) )) .              delete buf-prod-bc.                   NEXT _pbc.
              end.
              IF CAN-FIND(FIRST temp-prod-bc No-LOCK WHERE
                            ...
                            AND temp-prod-bc.b-code = buf-prod-bc.b-code )
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Уже есть ДопБК(prod-bc):" +                                  " ДопБК " + string(buf-prod-bc.b-str) +                                  " новый Баркод " + string(v-b-code)  +                                  " старый Баркод " + string(buf-prod-bc.b-code)) )) .              delete buf-prod-bc.                   NEXT _pbc.
              end.
              create temp-prod-bc.
            ...
            when "gds-obj-attr":U then do:
              ...
              RUN gen-row-keyr IN THIS-PROCEDURE ( INPUT buf_ext-classif.uniq-key-rec
                                                  ...
                                                  ,OUTPUT v-tbl-name) no-error.
              if error-status:error then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Ошибка при восстановлении баркода полученного из таблицы соответствия(gds-obj-attr):" +                                  " Код товара " + string(buf-gds-obj-attr.gds-code) +                                  " Объект " + buf-gds-obj-attr.obj-type + string(buf-gds-obj-attr.obj-code) +                                  " Весовой код " + string(buf-gds-obj-attr.attr-value) ) )) .              delete buf-gds-obj-attr.                   NEXT _pbc.
              end.
              find first buf_goods no-lock where
                        rowid(buf_goods) = v-rowid no-error.
              if not available buf_goods then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Не найден товар, полученный из таблицы соответствия(gds-obj-attr):" +                                  " Код товара " + string(buf-gds-obj-attr.gds-code) +                                  " Объект " + buf-gds-obj-attr.obj-type + string(buf-gds-obj-attr.obj-code) +                                  " Весовой код " + string(buf-gds-obj-attr.attr-value) ) )) .              delete buf-gds-obj-attr.                   NEXT _pbc.
              end.
              IF CAN-FIND(FIRST temp-gds-obj-attr No-LOCK WHERE
                            ...
                            AND temp-gds-obj-attr.attr-code = buf-gds-obj-attr.attr-code )
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕС,ВЗВЕШ и ТОП КОДЫ, СТРОКА " + string(ii) + chr(10)) + " Уже есть атрибут товара ВЕСОВОЙ КОД НА ОБЪЕКТЕ(gds-obj-attr):" +                                  " Старый Код товара " + string(buf-gds-obj-attr.gds-code) +                                  " новый Код товара " + string(buf_goods.gds-code) +                                  " Объект " + buf-gds-obj-attr.obj-type + string(buf-gds-obj-attr.obj-code) +                                  " Весовой код " + string(buf-gds-obj-attr.attr-value) ) )) .              delete buf-gds-obj-attr.                   NEXT _pbc.
              end.
        ...
        when "scales":U or when "scales-gds":U or when "scales-grp":U then do:
          ...
          case current-table:
            ...
            when "scales-gds":U then do:
              ...
              CASE p-version:
                when "15.0" then do:
                   import stream Instream buf-scales-gds.PLU-code                                 buf-scales-gds.b-code                                 buf-scales-gds.db-num                                 buf-scales-gds.deadline                                 buf-scales-gds.obj-code                                 buf-scales-gds.obj-type                                 buf-scales-gds.scales-num                                 buf-scales-gds.to-del                                 buf-scales-gds.to-send                                 buf-scales-gds.wt-cart                                 buf-scales-gds.deaddate                                 buf-scales-gds.deadflag                                 buf-scales-gds.deadtime buf-scales-gds.old-gds-code no-error.
                end.
                otherwise do:
              ...
              if not available buf_ext-classif then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕСЫ, СТРОКА " + string(ii) + chr(10)) + " Не найдена запись для баркода в таблице соответствия(scales-gds):" +                                  " номер весов " + string(buf-scales-gds.scales-num) +                                  " PLU " + string(buf-scales-gds.PLU-code)) )) .              delete buf-scales-gds.                   NEXT _scl.
              end.
              RUN gen-row-keyr IN THIS-PROCEDURE ( INPUT buf_ext-classif.uniq-key-rec
                                                  ,input ?
                                                  ,INPUT "ub"
                                                  ...
                                                  ,OUTPUT v-tbl-name) no-error.
              if error-status:error then do:
                  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕСЫ, СТРОКА " + string(ii) + chr(10)) + " Ошибка при восстановлении баркода полученного из таблицы соответствия(scaels-gds):" +                                  " номер весов " + string(buf-scales-gds.scales-num) +                                  " PLU " + string(buf-scales-gds.PLU-code)) )) .              delete buf-scales-gds.                   NEXT _scl.
              end.
              find first buf_goods no-lock where
                        rowid(buf_goods) = v-rowid no-error.
              if not available buf_goods then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕСЫ, СТРОКА " + string(ii) + chr(10)) + " Не найден товар, полученный из таблицы соответствия(scales-gds):" +                                  " номер весов " + string(buf-scales-gds.scales-num) +                                  " PLU " + string(buf-scales-gds.PLU-code)) )) .              delete buf-scales-gds.                   NEXT _scl.
              end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output v-b-code
              ...
              if error-status:error then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕСЫ, СТРОКА " + string(ii) + chr(10)) + " Ошибка при определении главного баркода товара, полученный из таблицы соответствия(prod-bc):" +                                  " номер весов " + string(buf-scales-gds.scales-num) +                                  " PLU " + string(buf-scales-gds.PLU-code)) )) .              delete buf-scales-gds.                   NEXT _scl.
              end.
              IF CAN-FIND(FIRST temp-scales-gds No-LOCK WHERE
                                temp-scales-gds.db-num = buf-scales-gds.db-num AND
                                temp-scales-gds.scales-num = buf-scales-gds.scales-num AND
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ВЕСЫ, СТРОКА " + string(ii) + chr(10)) + " Уже есть ТОВАР НА ВЕСАХ(scales-gds):" +                                  " номер весов " + string(buf-scales-gds.scales-num) +                                  " баркод новый" + string(v-b-code) +                                  " баркод старый" + string(buf-scales-gds.b-code)) )) .              delete buf-scales-gds.                   NEXT _scl.
              end.
...
define variable v-fr-type as character no-undo .
  ...
  on error undo, return error
  :
  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Импорт группы данных КАССЫ" + " Файл: " + p-dir-name + "\":U + p-db-key + ".":U + "cdk":U) )) .
  run check-iefile in this-procedure(input p-dir-name,                                      input "cdk":U,                                      input "import":U,                                      output loc#log).
  ii = 0.
  if loc#log then do:
    run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Импорт группы данных КАССЫ" ) ).
    input stream Instream from value(p-dir-name + "\":U + p-db-key + ".":U + "cdk":U).
    _cdk:
    REPEAT:
      import stream Instream unformatted ss.
      CASE ss:
        when 'cash-desk':U then do:
          ...
          case current-table:
            when 'cash-desk':U then do:
              ...
              import stream Instream buf-cash-desk.addr-path                                 buf-cash-desk.autonomy                                 buf-cash-desk.cash-num                                 buf-cash-desk.cash-on                                 buf-cash-desk.cash-os                                 buf-cash-desk.obj-code                                 buf-cash-desk.pos-type                                 buf-cash-desk.registration-code                                 buf-cash-desk.remote                                 buf-cash-desk.serial-code                                 buf-cash-desk.version                                 buf-cash-desk.fr-type no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КАССЫ, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _cdk.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-cash-desk No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КАССЫ, СТРОКА " + string(ii) + chr(10)) + " Уже есть КАССА(cash-desk):" +                                  " номер " + string(buf-cash-desk.cash-num)) )) .              delete buf-cash-desk.                   NEXT _cdk.
              end.
...
define buffer buf_goods for ub.goods.
  ...
  on error undo, return error
  :
  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Импорт группы данных КОНФИГУРАЦИЯ АЗК" + " Файл: " + p-dir-name + "\":U + p-db-key + ".":U + "pet":U) )) .
  run check-iefile in this-procedure(input p-dir-name,                                      input "pet":U,                                      input "import":U,                                      output loc#log).
  ii = 0.
  if loc#log then do:
    run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Импорт группы данных КОНФИГУРАЦИЯ АЗК" ) ).
    input stream Instream from value(p-dir-name + "\":U + p-db-key + ".":U + "pet":U).
    _pet:
    REPEAT:
      import stream Instream unformatted ss.
      CASE ss:
        when 'place':U then do:
          ...
          case current-table:
            when 'place':U then do:
              if p-new-obj-type <> 'маг':U then do:
                next _pet.
              end.
              ii = ii + 1. run write-counter in p-log-handle (substitute("Считано &1 записей", ii)).
              create buf-place.
              import stream Instream buf-place.add-qnty                                 buf-place.is-meas                                  buf-place.loc1                                 buf-place.loc2                                  buf-place.loc3                                  buf-place.loc4                                  buf-place.max-qnty                                 buf-place.obj-code                                 buf-place.obj-type                                 buf-place.pl-code                                  buf-place.pl-name                                  buf-place.PS                                       buf-place.status_ no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-place No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть СКЛ.МЕСТО(place):" +                                  " номер " + string(buf-place.pl-code) + " на " + buf-place.obj-type + string(buf-place.obj-code)) )) .              delete buf-place.                   NEXT _pet.
              end.
              IF CAN-FIND(FIRST temp-place No-LOCK WHERE
                                temp-place.pl-code = buf-place.pl-code
                                )
        ...
        when 'pump':U then do:
          ...
          case current-table:
            when 'pump':U then do:
              ...
              ii = ii + 1. run write-counter in p-log-handle (substitute("Считано &1 записей", ii)).
              create buf-pump.
              import stream Instream buf-pump.obj-code                                  buf-pump.obj-type                                  buf-pump.PS                                 buf-pump.pump-code                                 buf-pump.status_ no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-pump No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть ТРК(pump):" +                                   " номер " + string(buf-pump.pump-code) + " на " + buf-pump.obj-type + string(buf-pump.obj-code)) )) .              delete buf-pump.                   NEXT _pet.
              end.
              IF CAN-FIND(FIRST temp-pump No-LOCK WHERE
                                temp-pump.pump-code = buf-pump.pump-code
                                )
        ...
        when 'nozzle':U then do:
          ...
          case current-table:
            when 'nozzle':U then do:
              ...
              ii = ii + 1. run write-counter in p-log-handle (substitute("Считано &1 записей", ii)).
              create buf-nozzle.
              import stream Instream buf-nozzle.nozzle-code                                 buf-nozzle.obj-code                                 buf-nozzle.obj-type                                 buf-nozzle.PS                                 buf-nozzle.status_ no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-nozzle No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть пистолет(nozzle):" +                                   " номер " + string(buf-nozzle.nozzle-code) + " на " + buf-nozzle.obj-type + string(buf-nozzle.obj-code)) )) .              delete buf-nozzle.                   NEXT _pet.
              end.
              IF CAN-FIND(FIRST temp-nozzle No-LOCK WHERE
                                temp-nozzle.nozzle-code = buf-nozzle.nozzle-code
                                )
        ...
        when 'pl-gds':U then do:
          ...
          case current-table:
            when 'pl-gds':U then do:
              ...
              ii = ii + 1. run write-counter in p-log-handle (substitute("Считано &1 записей", ii)).
              create buf-pl-gds.
              import stream Instream buf-pl-gds.gds-code                                 buf-pl-gds.max-qnty                                 buf-pl-gds.obj-code                                 buf-pl-gds.obj-type                                 buf-pl-gds.pl-code                                  buf-pl-gds.PS                                       buf-pl-gds.status_                                  buf-pl-gds.tolerance no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-pl-gds No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть ПРИВЯЗКА К СКЛ.МЕСТУ(pl-gds):" +                                   " код скл.места " + string(buf-pl-gds.pl-code) +  " код товара " + string(buf-pl-gds.gds-code) + " на " + buf-pl-gds.obj-type + string(buf-pl-gds.obj-code)) )) .              delete buf-pl-gds.                   NEXT _pet.
              end.
              create temp-pl-gds.
              ...
              if not available buf_ext-classif then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Не найдена запись для товара на скл.месте  в таблице соответствия(pl-gds):" +                                   " СклМесто " + string(temp-pl-gds.pl-code) +                                   " Товар " + string(temp-pl-gds.gds-code)) )) .              delete buf-pl-gds.                   NEXT _pet.
              end.
              RUN gen-row-keyr IN THIS-PROCEDURE ( INPUT buf_ext-classif.uniq-key-rec
                                                  ,input ?
                                                  ,INPUT "ub"
                                                  ...
                                                  ,OUTPUT v-tbl-name) no-error.
              if error-status:error then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Ошибка при восстановлении товара на скл.месте полученного из таблицы соответствия(pl-gds):" +                                   " СклМесто " + string(temp-pl-gds.pl-code) +                                   " Товар " + string(temp-pl-gds.gds-code)) )) .              delete buf-pl-gds.                   NEXT _pet.
              end.
              find first buf_goods no-lock where
                        rowid(buf_goods) = v-rowid no-error.
              if not available buf_goods then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Не найден товар, полученный из таблицы соответствия(pl-gds):" +                                   " СклМесто " + string(temp-pl-gds.pl-code) +                                   " Товар " + string(temp-pl-gds.gds-code)) )) .              delete buf-pl-gds.                   NEXT _pet.
              end.
              else do:
        ...
        when 'pl-gds-pump':U then do:
          ...
          case current-table:
            when 'pl-gds-pump':U then do:
              ...
              import stream Instream buf-pl-gds-pump.gds-code                                 buf-pl-gds-pump.obj-code                                 buf-pl-gds-pump.obj-type                                 buf-pl-gds-pump.pl-code                                  buf-pl-gds-pump.PS                                       buf-pl-gds-pump.pump-code                                 buf-pl-gds-pump.status_ no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-pl-gds-pump No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть ПРИВЯЗКА К СКЛ.МЕСТУ И ТРК(pl-gds-pump):" +                                   " код скл.места " + string(buf-pl-gds-pump.pl-code) +                                   " код товара " + string(buf-pl-gds-pump.gds-code) +                                    " ТРК " + string(buf-pl-gds-pump.pump-code) + " на " + buf-pl-gds-pump.obj-type + string(buf-pl-gds-pump.obj-code)) )) .              delete buf-pl-gds-pump.                   NEXT _pet.
              end.
              ...
              if not available buf_ext-classif then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Не найдена запись для товара на скл.месте  в таблице соответствия(pl-gds-pump):" +                                   " СклМесто " + string(temp-pl-gds-pump.pl-code) +                                   " Товар " + string(temp-pl-gds-pump.gds-code) +                                   " ТРК " + string(temp-pl-gds-pump.pump-code) ) )) .              delete buf-pl-gds-pump.                   NEXT _pet.
              end.
              RUN gen-row-keyr IN THIS-PROCEDURE ( INPUT buf_ext-classif.uniq-key-rec
                                                  ,input ?
                                                  ,INPUT "ub"
                                                  ...
                                                  ,OUTPUT v-tbl-name) no-error.
              if error-status:error then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Ошибка при восстановлении товара на скл.месте полученного из таблицы соответствия(pl-gds-pump):" +                                   " СклМесто " + string(temp-pl-gds-pump.pl-code) +                                   " Товар " + string(temp-pl-gds-pump.gds-code) +                                   " ТРК " + string(temp-pl-gds-pump.pump-code) ) )) .              delete buf-pl-gds-pump.                   NEXT _pet.
              end.
              find first buf_goods no-lock where
                        rowid(buf_goods) = v-rowid no-error.
              if not available buf_goods then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Не найден товар, полученный из таблицы соответствия(pl-gds-pump):" +                                   " СклМесто " + string(temp-pl-gds-pump.pl-code) +                                   " Товар " + string(temp-pl-gds-pump.gds-code) +                                   " ТРК " + string(temp-pl-gds-pump.pump-code) ) )) .              delete buf-pl-gds-pump.                   NEXT _pet.
              end.
              else do:
        ...
        when 'pl-pump':U then do:
          ...
          case current-table:
            when 'pl-pump':U then do:
              ...
              ii = ii + 1. run write-counter in p-log-handle (substitute("Считано &1 записей", ii)).
              create buf-pl-pump.
              import stream Instream buf-pl-pump.obj-code                                 buf-pl-pump.obj-type                                 buf-pl-pump.pl-code                                  buf-pl-pump.PS                                       buf-pl-pump.pump-code                                 buf-pl-pump.status_ no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-pl-pump No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть ПРИВЯЗКА ТРК К СКЛ.МЕСТУ(pl-pump):" +                                   " код скл.места " + string(buf-pl-pump.pl-code) +                                   " ТРК " + string(buf-pl-pump.pump-code) + " на " + buf-pl-pump.obj-type + string(buf-pl-pump.obj-code)) )) .              delete buf-pl-pump.                   NEXT _pet.
              end.
              create temp-pl-pump.
        ...
        when 'pump-nozzle':U then do:
          ...
          case current-table:
            when 'pump-nozzle':U then do:
              ...
              import stream Instream buf-pump-nozzle.is-meas                                  buf-pump-nozzle.nozzle-code                                 buf-pump-nozzle.obj-code                                    buf-pump-nozzle.obj-type                                    buf-pump-nozzle.PS                                          buf-pump-nozzle.pump-code                                   buf-pump-nozzle.status_ no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-pump-nozzle No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть ПРИВЯЗКА ТРК К ПИСТОЛЕТУ(pump-nozzle):" +                                   " пистолет " + string(buf-pump-nozzle.nozzle-code) +                                   " ТРК " + string(buf-pump-nozzle.pump-code) + " на " + buf-pump-nozzle.obj-type + string(buf-pump-nozzle.obj-code)) )) .              delete buf-pump-nozzle.                   NEXT _pet.
              end.
        ...
        when 'pl-pump-nozzle':U then do:
          ...
          case current-table:
            when 'pl-pump-nozzle':U then do:
              ...
              import stream Instream buf-pl-pump-nozzle.nozzle-code                                 buf-pl-pump-nozzle.obj-code                                  buf-pl-pump-nozzle.obj-type                                  buf-pl-pump-nozzle.pl-code                                   buf-pl-pump-nozzle.PS                                        buf-pl-pump-nozzle.pump-code                                  buf-pl-pump-nozzle.status_ no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _pet.                   end.
              ...
              .
                            IF CAN-FIND(FIRST temp-pl-pump-nozzle No-LOCK WHERE
                                ...
                                then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных КОНФИГУРАЦИЯ АЗК, СТРОКА " + string(ii) + chr(10)) + " Уже есть ПРИВЯЗКА ТРК К СКЛ.МЕСТУ и ПИСТОЛЕТУ(pl-pump-nozzle):" +                                   " код скл.места " + string(buf-pl-pump-nozzle.pl-code) +                                   " пистолет " + string(buf-pl-pump-nozzle.nozzle-code) +                                   " ТРК " + string(buf-pl-pump-nozzle.pump-code) + " на " + buf-pl-pump-nozzle.obj-type + string(buf-pl-pump-nozzle.obj-code)) )) .              delete buf-pl-pump-nozzle.                   NEXT _pet.
              end.
                           ...
                           buf-action-role.db-num = g#db-num.
                                       ...
                                       WHERE temp-action-role.db-num    = buf-action-role.db-num
                                         ...
                                         and temp-action-role.action-role-code = buf-action-role.action-role-code
                                             ...
                                             then do:
                                                                  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ПРАВА, СТРОКА " + string(ii) + chr(10)) + " Уже есть ПОЛЬЗОВАТЕЛЬ(action-role): "                                                               + STRING(buf-action-role.db-num) + " "                                                               + STRING(buf-action-role.action-head-code) + " "                                                               + STRING(buf-action-role.action-role-code)                                                               ) )) .              delete buf-action-role.                   NEXT _rht.
                           end.
                           ...
                           buf-action-role-item.db-num = g#db-num.
                                       ...
                                       WHERE temp-action-role-item.db-num                = buf-action-role-item.db-num
                                         ...
                                         and temp-action-role-item.action-role-item-code = buf-action-role-item.action-role-item-code
                                             ...
                                             then do:
                                                                  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ПРАВА, СТРОКА " + string(ii) + chr(10)) + " Уже есть ПОЛЬЗОВАТЕЛЬ(action-role-item): "                                                               + STRING(buf-action-role-item.db-num) + " "                                                               + STRING(buf-action-role-item.action-head-code) + " "                                                               + STRING(buf-action-role-item.action-role-code) + " "                                                               + STRING(buf-action-role-item.action-role-item-code)                                                               ) )) .              delete buf-action-role-item.                   NEXT _rht.
                           end.
...
procedure p-cdrg-i :
  ...
  on error undo, return error
  :
  run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", ("Импорт группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ" + " Файл: " + p-dir-name + "\":U + p-db-key + ".":U + "cdr":U) )) .
  run check-iefile in this-procedure(input p-dir-name,                                      input "cdr":U,                                      input "import":U,                                      output loc#log).
  ii = 0.
  if loc#log then do:
    run write-log-and-file in p-log-handle (                          input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "!!!&1",  "Импорт группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ" ) ).
    input stream Instream from value(p-dir-name + "\":U + p-db-key + ".":U + "cdr":U).
    _cdrg:
    REPEAT:
      import stream Instream unformatted ss.
      CASE ss:
        when "code-range":U then do:
          ...
          case current-table:
            when "code-range":U then do:
              ...
              import stream Instream  no-error.
                            if error-status:error then do:                       run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ, СТРОКА " + string(ii) + chr(10)) + error-status:get-message(error-status:num-messages)) )) .                       NEXT _cdrg.                   end.
              find first temp-code-range No-LOCK
                ...
                no-error .
              if available temp-code-range then do:
                                    run write-log-and-file in p-log-handle (                              input 1                                                              , input log-file-name                                                  , input 1                                                              , input substitute( "&1 .......", (("Импорт группы данных ДИАПАЗОНЫ ВЕСОВЫХ КОДОВ, СТРОКА " + string(ii) + chr(10)) + " Уже есть ДИАПАЗОН КОДОВ (code-range):" +                                  " тип " + buf-code-range.range-type +                                  " начало " + string(buf-code-range.first-code) +                                  " и/или " +                                  " тип " + buf-code-range.range-type +                                  " окончание " + string(buf-code-range.last-code)                                 ) )) .              delete buf-code-range.                   NEXT _cdrg.
              end.
...
define variable v-rid24 as recid no-undo .
v-found = no.
define variable vss-include-info25 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define buffer locK-batchprocess25 for ub.batchprocess.
run gbl/lock-prc.p
    (input 'lscc':U
    ,input 0
    ,input 0
...
or v-found = yes
then do:
  define variable v-exist24 as logical no-undo .
...
find first buf_ext-classif no-lock where
    ...
    and buf_ext-classif.charkey_one = 'чел':U no-error.
if not available buf_ext-classif then do:
  return error.
end.
else do:
