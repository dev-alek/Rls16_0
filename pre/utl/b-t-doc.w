...
PROCEDURE adm-disable :
    DISABLE br_table vardoc-code WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN br_table vardoc-code WITH FRAME F-Main.
...
PROCEDURE adm-display-fields :
      IF AVAILABLE trn-doc THEN
          DISPLAY trn-doc.doc-code trn-doc.doc-type trn-doc.flag_ trn-doc.status_ trn-doc.doc-date trn-doc.fact-date trn-doc.obj-code trn-doc.obj-type trn-doc.cli-code trn-doc.cli-name trn-doc.cli-type WITH BROWSE br_table
            NO-ERROR.
      DISPLAY UNLESS-HIDDEN vardoc-code
...
DO:
  find trn-doc where trn-doc.doc-code = input frame F-Main vardoc-code no-lock no-error.
  if available trn-doc then do:
     reposition br_table to recid recid(trn-doc).
...
PROCEDURE adm-open-query-cases :
  OPEN QUERY br_table FOR EACH trn-doc WHERE TRUE NO-LOCK     BY trn-doc.fact-num DESCENDING INDEXED-REPOSITION.
END PROCEDURE.
...
PROCEDURE send-key :
  ...
  CASE pc_key-name:
    WHEN "obj-code" THEN
      pc_key-value = IF AVAILABLE trn-doc THEN STRING(trn-doc.obj-code) ELSE ?.
    WHEN "doc-code" THEN
      pc_key-value = IF AVAILABLE trn-doc THEN STRING(trn-doc.doc-code) ELSE ?.
    WHEN "host-code" THEN
      pc_key-value = IF AVAILABLE trn-doc THEN STRING(trn-doc.host-code) ELSE ?.
      WHEN "":U THEN .
