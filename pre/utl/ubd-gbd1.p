...
case p-action :
  when "get-m-code":U then do:
    do v-ii0 = 1 to num-entries('ub.dis-card'):
      assign
      v-table-name0 = entry(v-ii0, 'ub.dis-card')
...
case p-action :
  when "get-m-code":U then do:
    do v-ii1 = 1 to num-entries('ub.contract'):
      assign
      v-table-name1 = entry(v-ii1, 'ub.contract')
...
case p-action :
  when "get-m-code":U then do:
    do v-ii2 = 1 to num-entries('ub.rule-by-call'):
      assign
      v-table-name2 = entry(v-ii2, 'ub.rule-by-call')
...
case p-action :
  when "get-m-code":U then do:
    do v-ii3 = 1 to num-entries('ub.fin-doc'):
      assign
      v-table-name3 = entry(v-ii3, 'ub.fin-doc')
...
case p-action :
  when "get-m-code":U then do:
    do v-ii4 = 1 to num-entries('ub.firm'):
      assign
      v-table-name4 = entry(v-ii4, 'ub.firm')
...
case p-action :
  when "get-m-code":U then do:
    do v-ii5 = 1 to num-entries('ub.person'):
      assign
      v-table-name5 = entry(v-ii5, 'ub.person')
...
case p-action :
  when "get-m-code":U then do:
    do v-ii6 = 1 to num-entries('ub.dis-rule,ub.dis-time-rule'):
      assign
      v-table-name6 = entry(v-ii6, 'ub.dis-rule,ub.dis-time-rule')
...
case p-action :
  when "get-m-code":U then do:
    do v-ii7 = 1 to num-entries('ub.bar-code,ub.place'):
      assign
      v-table-name7 = entry(v-ii7, 'ub.bar-code,ub.place')
...
define new shared temp-table temp-clients no-undo like ub.clients
field base-code as integer
field new-issue-code as integer
field deleted-sysconf as logical
.
define new shared temp-table temp-sysconf no-undo like ub.sysconf.
...
index pi proc-name
.
   p-db-num   = int (entry(1,p-parameter,chr(4))) .
   p-cli-code = int (entry(2,p-parameter,chr(4))) .
MAIN-BLOCK:
do
...
find first ub.clients no-lock where
           ...
           ub.clients.obj-type = 'орг':U no-error .
    if error-status :error then do:
        run write-log-and-file in p-log-handle (
              input 1
            , input log-file-name
