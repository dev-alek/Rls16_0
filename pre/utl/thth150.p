...
define variable vss-workfile    as character no-undo init "$Workfile: thth150.p $":U .
define variable vss-archive     as character no-undo init "$Archive: utl/thth150.p $":U .
...
:
  run gbl/d-list.w (
                INPUT "b-sel":U
                ,INPUT "Выберите операцию по сведению систем 16.0 и 15.0 "
  ...
  case v-selected-operation:
    ...
    when "ini" then do:
      assign
      v-value-list = ''
      v-attr-query-list = 'thth150-clients':U + chr(4) + 'thth150-goods':U + chr(4) + 'thth150-shop':U
      .
      ...
      if lookup(string(no), v-value-list, chr(4) ) > 0 then do:
        message
        "Начат но не закончен этап сведения v15.0 и v16.0"
    ...
    when "impxexpi" then do:
      assign
      v-attr-query-list = 'thth150-shop':U + chr(4) +
                          'thth150-clients':U + chr(4) +
                          'thth150-goods':U
