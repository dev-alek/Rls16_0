...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4':u,p-node-code,p-level-num,p-orig-name,p-new-name)
    .
end procedure.
...
define temp-table temp-gds-prt no-undo   field node-code  as integer   field upper-code as integer   field level-num  as integer   field node-name  as character format 'x(16)'   field f-name     as character format 'x(40)'   field update-node-name as logical label 'upd-node'   field update-f-name as logical label 'upd-full'   field is-process as logical   index xpk is primary unique node-code   index ie1 f-name   index ie2 level-num node-name   index ie3 update-f-name is-process   index ie4 is-process   index ie5 upper-code .
do
on error undo, return error return-value
:
