...
define  temp-table dc-list no-undo like ub.dis-card
...
define    temp-table dc-list-hist no-undo
...
define NEW SHARED temp-table temp-update-rule no-undo
field table-name as character
field table-id as integer
field field-name as character
field bh as handle
field pk as character
field gen-id as integer
field action as character
field table-name-s as character
field table-id-s as integer
field field-name-s as character
field bh-s as handle
...
procedure gen-temp-row-keyr :
  ...
  do
  on error  undo, return error substitute( "&1 (gen-temp-row-keyr). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
  ...
  :
    ...
    if p-key-rec = ?
      or p-key-rec = "":U
    then do:
      return error substitute( "&1 (gen-temp-row-keyr). Ошибка задания входных параметров. Не задан уникальный ключ.", vss-include-info8 ).
    end.
    assign
    ...
    if not available ub._file then do:
      return error substitute( "&1. Таблица &2 отсутствует в БД", vss-include-info8, v-tbl-name ).
    end.
    find ub._index
    ...
    :
      ...
      if v-where = "":U then do:
        ...
        assign
          v-where = v-where + chr(32) + "and":U
        .
      end.
      if ub._field._Data-Type ="character":U then do:
        assign
          v-where = v-where + chr(32) + substitute( "&1 = '&2'":U, ub._field._field-name, entry( v-count-fld + 1 , p-key-rec, chr(3) ) )
        .
      end.
      else do:
        assign
  ...
  if p-tbl-row = ? then do:
    return substitute( "Не найдена запись таблицы &2 по ключу &3", vss-include-info8, v-tbl-name, p-key-rec ).
  end.
  else do:
    return.
  end.
...
define input  parameter p-action as integer no-undo .
do
on error  undo, return error substitute( "&1 (tempcont_create-record). &2&3&4", vss-include-info13, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
on stop   undo, return error substitute( "&1 (tempcont_create-record). stop", vss-include-info13 )
on endkey undo, return error substitute( "&1 (tempcont_create-record). endkey", vss-include-info13 )
:
  ...
  if not p-new-tbl-handle:available then do:
    return error substitute( "&1. Переданный буфер таблицы &2 не доступен", vss-include-info13, p-tbl-name ).
  end.
  assign
  ...
  if not available buf_temp-tables then do:
    ...
    if v-ok <> true then do:
      return error substitute( "&1 (tempcont_create-record). Ошибка при создании временной таблицы &2 (1)", vss-include-info13, tt-name ) .
    end.
    assign
      v-ok = tth:temp-table-prepare( tt-name ) no-error
    .
    if v-ok <> true then do:
      return error substitute( "&1 (tempcont_create-record). Ошибка при создании временной таблицы &2 (2)", vss-include-info13, tt-name ) .
    end.
  ...
  if v-inform = ?
    ...
    or entry( 3, v-inform, ",":U ) <> "1":U
  then do:
    return error substitute( "&1 (tempcont_create-record). Таблица &2 не имеет первичного ключа в БД", vss-include-info13, p-tbl-name ).
  end.
  assign
    v-idx-field-qnty = num-entries( v-inform ) - 4
  .
  if v-idx-field-qnty < 2 then do:
    return error substitute( "&1 (tempcont_create-record). Определенный первичный индекс (&2) не содержит списка полей для таблицы &3", vss-include-info13, v-inform, p-tbl-name ).
  end.
  assign
    v-where     = "where":U
    v-word-link = "":U
  ...
  if not bh_tt:available then do:
    ...
    .
    if v-ok <> true then do:
      return error substitute( "&1 (tempcont_create-record). Ошибка при создании буфера временной таблицы.", vss-include-info13, p-tbl-name ).
    end.
    assign
      compare-log = false
    .
  ...
  if compare-log = false then do:
    ...
    .
    if v-ok <> true then do:
      return error substitute( "&1 (tempcont_create-record). BUFFER-COPY не прошел для таблицы &2", vss-include-info13, p-tbl-name ).
    end.
  end.
  assign
    v-ok = false
  ...
  .
  if v-ok <> true then do:
    return error substitute( "&1 (tempcont_create-record). buffer-release не прошел для таблицы &2", vss-include-info13, p-tbl-name ).
  end.
  assign
    v-ok = false
  .
...
:
  ...
  if not available buf_temp-tables then do:
    ...
    .
    if v-ok <> true then do:
      return error substitute( "&1 (tempcont_get-buffer-handle). Ошибка при создании временной таблицы &2 (1)", vss-include-info13, tt-name ) .
    end.
    assign
      v-ok = tth:temp-table-prepare( tt-name ) no-error
    .
    if v-ok <> true then do:
      return error substitute( "&1 (tempcont_get-buffer-handle). Ошибка при создании временной таблицы &2 (2)", vss-include-info13, tt-name ) .
    end.
    create buf_temp-tables.
    assign
    buf_temp-tables.new-table-handle = tth
...
:
  ...
  if v-param-list = "":U then do:
      run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Не заданы параметры расчета скидки или категории согласно накопительному алгоритма по ДК в задаче &1&2"                                     , p-task-num                                                                                                           , chr(10))).
      return.
  end.
  assign
  v-dc-list-mode = entry(1, v-param-list, chr(4))
  v-update-mode = entry(2, v-param-list, chr(4))
  v-dc-type-list-mode = entry(3, v-param-list, chr(4))
  v-dc-type-list = entry(4, v-param-list, chr(4))
  v-dc-type-algo-list = entry(5, v-param-list, chr(4))
  no-error
  .
  if error-status :error then do:
    run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Ошибки при получении параметров расчета&1" +                            "&2&1&3&1"                                                                          , error-status:get-message(1)                                                       , return-value )).
  end.
    run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("Пересчет скидок и категорий по дисконтным картам в соответствии с накопительными алгоритмами&1" +                              "Отбор дисконтных карт: &2&1" +                             "Режим работы: &3&1" +                             "Отбор типов дисконтных карт: &4&1" +                             "Отбор итогов для пересчета или проверки: &5"                            , chr(10)                            , (if v-dc-list-mode = "LIST" then "по списку карт" else "все карты, подлежащие расчету по алгоритму")                           , (if v-update-mode = "update" then "ПЕРЕСЧЕТ" else "ПРОВЕРКА")                           , (if v-dc-type-list-mode = "*" then "ВСЕ ТИПЫ" else ("ПО СПИСКУ" + chr(32) + v-dc-type-list ))                           , (if v-dc-type-algo-list = "" then "ВСЕ АЛГОРИТМЫ" else "ВЫБРАННЫЕ АЛГОРИТМЫ"))).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  ...
  AND v-dc-type-list = '':u  then do:
    run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Не задан список типов ДК&1", chr(10))).
    return .
  end.
  IF v-dc-list-mode = "LIST"
  AND NOT CAN-FIND(FIRST dc-list) then do:
    run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Не задан список ДК&1", chr(10))).
    return error.
  END.
...
run tempcont_clear in this-procedure no-error.
   run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("Завершен расчет&1", chr(10))).
end.
...
define buffer buf_tt0-rule-by-call for tt0-rule-by-call.
  ...
  :
    ...
    :
      ...
      if buf_temp-changes.uniq-key-rec begins 'dis-card':U then do:
            run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("&1:&5" +                              "старое значение &2 = &3&5" +                              "новое значение  &2 = &4"                             , calldscr(buf_temp-changes.uniq-key-rec)                               , (if buf_temp-changes.t_name = 'dis-card-property':U then '' else  buf_temp-changes.f_name)                             , buf_temp-changes.v_old                              , buf_temp-changes.v_new                              , chr(10))).
      end.
    ...
    if available buf_tt0-rule-by-call then do:
      ...
      if p-display then do:
        run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("&2---------------------Просмотрено карт - &1&2" +                              "---------------------Ошибок при расчете - &3&2" +                              (if v-update-mode = "update"                             then                                                     "---------------------Пересчитано - &4&2"                               else                                                                          "---------------------Значений для пересчета - &4&2") +                             "---------------------Значений не подлежащих пересчету - &5"                             , buf_tt0-rule-by-call.num-rec  + buf_tt0-rule-by-call.num-rec-new                              , chr(10)                                                                    , buf_tt0-rule-by-call.num-rec-calc-err + buf_tt0-rule-by-call.num-rec-calc-err-new                               ,  buf_tt0-rule-by-call.num-rec-value-err + buf_tt0-rule-by-call.num-rec-value-err-new                              , ((buf_tt0-rule-by-call.num-rec-ok + buf_tt0-rule-by-call.num-rec-ok-new)                                           - (buf_tt0-rule-by-call.num-rec-value-err + buf_tt0-rule-by-call.num-rec-value-err-new ))                             )).
      end.
      else do:
        run show-counter in p-log-handle .               run write-counter in p-log-handle ( input substitute("Просмотрено карт - &1", buf_tt0-rule-by-call.num-rec  + buf_tt0-rule-by-call.num-rec-new)) no-error.
      end.
    end.
  end.
end procedure.
