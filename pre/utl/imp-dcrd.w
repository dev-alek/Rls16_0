...
if v-run then do:
  ...
  then do:
    run str/diallog.w (
                        ...
                        ,input this-procedure
                        ,input 'sendclia.p':U
                        ,input (string(v-cntxt-db-num) + chr(4) +  chr(4) + "no":U + chr(4) + "O":U)
                        ,input yes
                        ,input '':U
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH X_rule-profile SHARE-LOCK,       EACH X_dis-card-type WHERE TRUE  SHARE-LOCK.
...
then do:
  ...
  for each buf_ruledict-param no-lock where
    ...
    buf_tt0-rule-call-param.order_id              = 0
...
FOR EACH dc-list:
  DELETE dc-list.
END.
run rul/ruprcall.p ( input 'dis-card-type':U
