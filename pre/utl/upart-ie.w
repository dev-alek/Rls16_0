...
END.
ON CHOOSE OF B-lkp-In IN FRAME Dialog-Frame
DO:
if not available x_parts then return .
    run str/showdoc.p
    ( input parparentproc
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-2 FOR EACH x_parts NO-LOCK,       EACH buf_goods OF x_parts NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
...
frame Dialog-Frame:title  = frame Dialog-Frame:title + ' ' + buf_trn-doc.doc-code .
...
define output parameter p-parts-recid as recid no-undo .
  case p-direction :
    ...
    otherwise do:
      reposition BROWSE-2  to recid integer(p-direction) no-error .
    end.
  end case .
  assign
...
PROCEDURE reposition-query :
  ...
  do with frame Dialog-Frame:
    apply "entry":u to browse BROWSE-2 .
  end.
END PROCEDURE.
