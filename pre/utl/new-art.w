...
DO:
  ...
  if error-status :error then do:
    apply "entry" to new-prod-code in frame Dialog-Frame.
    return no-apply.
  end.
...
END.
ON CHOOSE OF Btn_RenameAll IN FRAME Dialog-Frame
DO:
  run process-goods in this-procedure .
END.
ON RETURN OF new-artic IN FRAME Dialog-Frame
DO:
  APPLY "CHOOSE":U TO btn_Ok IN FRAME Dialog-Frame.
END.
ON RETURN OF new-prod-code IN FRAME Dialog-Frame
OR LEAVE OF new-prod-code IN FRAME Dialog-Frame
OR RETURN OF new-prod-type IN FRAME Dialog-Frame
OR LEAVE OF new-prod-type IN FRAME Dialog-Frame
DO:
...
END.
ON RETURN OF old-prod-code IN FRAME Dialog-Frame
OR LEAVE OF old-prod-code IN FRAME Dialog-Frame
OR RETURN OF old-prod-type IN FRAME Dialog-Frame
OR LEAVE OF old-prod-type IN FRAME Dialog-Frame
DO:
  define buffer buf_clients for ub.clients.
  assign
    old-prod-code
...
END.
ON CHOOSE OF sel-gds IN FRAME Dialog-Frame
DO:
...
DO:
  assign
    rid-list = ""
  .
  run ref/cli-all.w ( parParentProc, "b-sel", 'про':U, 'все':U, 'текущие':U, ?, ",,,,,,NO,,,", "news", output rid-list ) no-error.
  if error-status :error then do:
    message
...
:
  ...
  if g#news = true then do:
    assign
      v-not-two-commit = true
    .
    DISABLE sel-gds Btn_RenameAll Btn_Cancel with frame Dialog-Frame.
    HIDE sel-gds Btn_RenameAll Btn_Cancel in frame Dialog-Frame.
  end.
  else do:
  ...
  if p-old-artic <> ?
    ...
    and p-old-prod-code <> ?
  then do:
    DISABLE sel-gds Btn_RenameAll with frame Dialog-Frame.
    HIDE sel-gds Btn_RenameAll in frame Dialog-Frame.
  end.
  if g#news = true then do:
    WAIT-FOR "CHOOSE" OF Btn_OK IN FRAME Dialog-Frame.
  end.
  else do:
    WAIT-FOR "CHOOSE" OF Btn_Cancel IN FRAME Dialog-Frame.
  end.
END.
RUN disable_UI.
