...
define variable vss-workfile    as character no-undo init "$Workfile: cmprahsp.p $":U .
define variable vss-archive     as character no-undo init "$Archive: utl/cmprahsp.p $":U .
...
define temp-table temp-cli-fact-order no-undo   field temp-cli-type    like ub.stk-supp-line.cli-type   field temp-cli-code    like ub.stk-supp-line.cli-code   field temp-artic       like ub.stk-supp-line.artic   field temp-prod-type   like ub.stk-supp-line.prod-type   field temp-prod-code   like ub.stk-supp-line.prod-code   field temp-sum-type    like ub.stk-supp-line.sum-type   field temp-fact-order  like ub.stk-supp-line.fact-order   field temp-fact-date   like ub.stk-supp-line.fact-date   field temp-shift-date  like ub.stk-supp-line.shift-date   field temp-shift-num   like ub.stk-supp-line.shift-num   field temp-break-value as integer   field temp-need-delete as logical   index xpk is primary unique temp-cli-type temp-cli-code temp-artic temp-prod-type temp-prod-code temp-sum-type temp-fact-order   index xie1 temp-need-delete   index xie2 temp-cli-type temp-cli-code temp-artic temp-prod-type temp-prod-code temp-sum-type temp-break-value .
define temp-table temp-last-cli-fact-order no-undo   field temp-cli-type    like ub.stk-supp-line.cli-type   field temp-cli-code    like ub.stk-supp-line.cli-code   field temp-artic       like ub.stk-supp-line.artic   field temp-prod-type   like ub.stk-supp-line.prod-type   field temp-prod-code   like ub.stk-supp-line.prod-code   field temp-sum-type    like ub.stk-supp-line.sum-type   field temp-break-value as integer   field temp-fact-order  like ub.stk-supp-line.fact-order   index xpk is primary unique temp-cli-type temp-cli-code temp-artic temp-prod-type temp-prod-code temp-sum-type temp-break-value .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  then do:
    ...
    message
      vss-workfile vss-revision vss-description skip
      "Неправильный тип атрибута" 'ahsp-detail':U skip
      view-as alert-box error .
    undo, return error .
  end.
  run day-begin-fact-order in this-procedure
    (input  v-ahsp-detail-date
    ,output v-ahsp-detail-fact-order
    ).
...
procedure fill-temp-cli-fact-order :
  ...
  :
    ...
    on error undo, return error
    :
      if (buf_stk-supp-line.sum-type begins 'cost':U
