...
put unformatted
  '# Файл для перевода описаний полей' + chr(10)
  '# Данный файл сгенерирован автоматически при помощи программы ' + vss-workfile + chr(10)
  '# Дата  ' + string(v-today, "99/99/9999") + chr(10)
  '# Время ' + string(v-time, "hh:mm") + chr(10)
  '# Необходимо в описании поля после русского описания' +
  '# привести английский и румынский переводы, отделенные символом ` ' + chr(10)
  '# Причем надо менять тот DESCRIPTION у которого слева отсутствует знак комментария #           ' + chr(10)
  '# Пример:' + chr(10)
  '# DESCRIPTION "Главный бухгалтер`Cheif accountant`Contabil sef     " ' + chr(10)
  + chr(10)
.
output close .
...
:
  ...
  :
    ...
    then do:
      ...
      put unformatted
        'UPDATE FIELD "' + _Field._Field-Name + '" OF "' + _File._File-Name + '"' + chr(10)
        + SUBSTITUTE('# FORMAT "&1"'      , _Field._Format    ) + chr(10)
        + SUBSTITUTE('# LABEL "&1"'       , _Field._Label     ) + chr(10)
        + SUBSTITUTE('# COLUMN-LABEL "&1"', _Field._Col-Label ) + chr(10)
        + SUBSTITUTE('# DESCRIPTION "&1"' , substring(replace(_Field._Desc, chr(10), chr(32)), 1, 100) ) + chr(10)
        + '  DESCRIPTION '
        .
      export
         _Field._Desc
...
output to value(v-file-name) append .
put unformatted
  '# Всего полей для перевода ' + string(v-total-field) + chr(10)
  + chr(10)
.
output close .
message
