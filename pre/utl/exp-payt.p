...
MESSAGE "Вы уверены, что хотите начать экспорт"
        "справочника видов платежей" + "?"
VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE "Экспорт справочников" UPDATE l_start-export AS LOGICAL.
RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочника видов платежей" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ).
...
DO ON ERROR  UNDO Prepare-Block, RETURN ERROR
  ...
  FOR EACH ub.pay-type NO-LOCK :
    ...
    IF ( j_docs MODULO 100 ) = 0 THEN DO: RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочника видов платежей" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ). END.
    CREATE tt_pay-type.
    BUFFER-COPY ub.pay-type
             TO tt_pay-type.
  END.
END.
ASSIGN j_docs = 0.
...
DO TRANSACTION ON ERROR  UNDO BlockTransaction, RETURN ERROR
  ...
  OUTPUT STREAM out-str TO VALUE( v_dmp-dir + "ref-" + SUBSTRING( "pay-type", 1, 3 ) + ".txt" ).
  FOR EACH tt_pay-type :
    PROCESS EVENTS.
    ASSIGN j_docs = j_docs + 1.
    IF ( j_docs MODULO 100 ) = 0 THEN DO: RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочника видов платежей" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ). END.
    EXPORT STREAM out-str tt_pay-type.
  END.
  OUTPUT STREAM out-str CLOSE.
END.
IF SESSION :SET-WAIT-STATE( "":U ) THEN DO: END.
RUN WaitFram-Hide IN THIS-PROCEDURE.
MESSAGE "Экспорт" "справочника видов платежей" "закончен" SKIP
