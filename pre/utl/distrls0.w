...
assign
  v-str-recid4 = trim( string( recid( X_dis-time-rule ) , "->>>>>>>>>>>9":U ) )
...
ch = br-dis-time-rule:FIRST-COLUMN IN FRAME Dialog-Frame.
DO ii = 1 TO br-dis-time-rule:NUM-COLUMNS IN FRAME Dialog-Frame:
    ASSIGN
    ch:RESIZABLE = YES.
    ch = ch:NEXT-COLUMN.
END.
...
FOR EACH X_dis-time-rule NO-LOCK WHERE X_dis-time-rule.time-rule-num <= 99999 INDEXED-REPOSITION.
END PROCEDURE.
