...
PROCEDURE adm-display-fields :
      IF AVAILABLE gds-dtl THEN
          DISPLAY gds-dtl.doc-qnty gds-dtl.fact-qnty gds-dtl.price-base gds-dtl.price-rubl gds-dtl.discnt-pc gds-dtl.discnt-base gds-dtl.discnt-rubl gds-dtl.discnt-type gds-dtl.cur-base gds-dtl.prt-code gds-dtl.ov WITH BROWSE br_table
            NO-ERROR.
...
PROCEDURE adm-open-query-cases :
  OPEN QUERY br_table FOR EACH gds-dtl WHERE gds-dtl.doc-code  = doc-line.doc-code    and                                                gds-dtl.artic     = doc-line.artic       and                                                gds-dtl.prod-type = doc-line.prod-type  and                                                gds-dtl.prod-code = doc-line.prod-code  NO-LOCK     .
END PROCEDURE.
PROCEDURE adm-row-available :
  ...
  END.
  IF key-name eq ? THEN tbl-list = "doc-line":U.
  ...
  DO:
    ...
    IF row-avail-rowid NE ROWID(doc-line) THEN different-row = yes.
    IF row-avail-rowid ne ? THEN DO:
       DO:
         ...
         DO:
           ...
           THEN FIND doc-line WHERE ROWID(doc-line) = row-avail-rowid SHARE-LOCK NO-ERROR.
           ELSE DO TRANSACTION:
             FIND doc-line WHERE ROWID(doc-line) = row-avail-rowid EXCLUSIVE-LOCK NO-ERROR.
           END.
         END.
         ELSE FIND doc-line WHERE ROWID(doc-line) = row-avail-rowid  NO-LOCK NO-ERROR.
    ...
    END.
    ELSE DO:
         IF AVAILABLE doc-line THEN RELEASE doc-line.
...
PROCEDURE send-key :
  ...
  CASE pc_key-name:
    WHEN "obj-code" THEN
      pc_key-value = IF AVAILABLE gds-dtl THEN STRING(gds-dtl.obj-code) ELSE ?.
    WHEN "doc-code" THEN
      pc_key-value = IF AVAILABLE gds-dtl THEN STRING(gds-dtl.doc-code) ELSE ?.
      WHEN "":U THEN .
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "doc-line":U THEN p-rowid-list = p-rowid-list +
        ...
        ELSE "?":U.
    WHEN "gds-dtl":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE gds-dtl THEN STRING(ROWID(gds-dtl))
        ELSE "?":U.
        OTHERWISE
        DO:
