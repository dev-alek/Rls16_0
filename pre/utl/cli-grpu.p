...
PROCEDURE uniq-grp.
def input param c like ub.cli-grp.upper-code.
def input param ln like ub.cli-grp.lvl-num.
def buffer b-cli-grp for ub.cli-grp.
for each b-cli-grp where b-cli-grp.upper-code = c:
  find first ub.cli-grp where ub.cli-grp.upper-code = c and
                                ub.cli-grp.node-name begins b-cli-grp.node-name
                                and recid(ub.cli-grp) <> recid(b-cli-grp)
                                no-lock no-error.
  if available ub.cli-grp then
    assign
      b-cli-grp.node-name = b-cli-grp.node-name + "."
      upd-grp = upd-grp + 1.
  run uniq-grp (b-cli-grp.node-code, ln + 1).
  tot-grp = tot-grp + 1.
