...
DEFINE TEMP-TABLE tt_clients NO-UNDO LIKE ub.clients.
...
MESSAGE "Вы уверены, что хотите начать экспорт"
        "справочников клиентов" + "?"
VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE "Экспорт справочников" UPDATE l_start-export AS LOGICAL.
RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочников клиентов" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ).
...
DO ON ERROR  UNDO Prepare-Block, RETURN ERROR
  ...
  FOR EACH ub.clients NO-LOCK :
    ...
    IF ( j_docs MODULO 100 ) = 0 THEN DO: RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочников клиентов" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ). END.
    CREATE tt_clients.
    BUFFER-COPY ub.clients
             TO tt_clients.
...
DO TRANSACTION ON ERROR  UNDO BlockTransaction, RETURN ERROR
  ...
  OUTPUT STREAM out-str TO VALUE( v_dmp-dir + "ref-" + SUBSTRING( "clients", 1, 3 ) + ".txt" ).
  FOR EACH tt_clients :
    PROCESS EVENTS.
    ASSIGN j_docs = j_docs + 1.
    IF ( j_docs MODULO 100 ) = 0 THEN DO: RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочников клиентов" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ). END.
    EXPORT STREAM out-str tt_clients.
  END.
...
MESSAGE "Экспорт" "справочников клиентов" "закончен" SKIP
