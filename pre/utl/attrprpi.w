  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-attr-prop :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-attr-prop FOR EACH temp-attr-prop NO-LOCK WHERE temp-attr-prop.table-name = p-table-name BY temp-attr-prop.upper-prop-code BY temp-attr-prop.prop-code INDEXED-REPOSITION.
END PROCEDURE.
...
IF rs-add = "add" THEN DO:
  ...
  v-upper-node-code = (if available temp-attr-prop
   ...
   buf_temp-attr-prop.full-prop-name = temp-attr-prop.full-prop-name + f-prop-code + chr(47)
   .
END.
else do:
  assign
