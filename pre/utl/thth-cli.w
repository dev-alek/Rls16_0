...
define  new shared  temp-table clients-01 no-undo
...
src-obj-code
.
define  new shared  temp-table firm-01 no-undo
...
src-firm-code
.
define  new shared  temp-table person-01 no-undo
...
(IF X_ext-classif.uniq-key-rec BEGINS 'clients':U
 THEN (entry(2, X_ext-classif.uniq-key-rec, chr(3)) +
       entry(3, X_ext-classif.uniq-key-rec, chr(3))
       )
ELSE ''
    ) COLUMN-LABEL "КЛИЕНТ!v16.0" FORMAT "X(12)"
(X_ext-classif.charkey_one + string(X_ext-classif.KEY#_one))  COLUMN-LABEL "КЛИЕНТ!" FORMAT "X(12)"
X_ext-classif.charkey_two COLUMN-LABEL "ИНН" FORMAT "X(12)"
...
END.
ON CHOOSE OF b-imp-gds IN FRAME Dialog-Frame
DO:
...
assign
  v-str-recid18 = trim( string( recid( X_ext-classif ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry18 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-clients in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-clients in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-clients :handle
      .
...
do:
  if available X_ext-classif then v-doc-rec = recid(X_ext-classif). run openbr in this-procedure ( INPUT YES, INPUT NO, INPUT '':U) no-error. reposition br-clients to recid(v-doc-rec) no-error. APPLy 'ENTRY' to br-clients .
    apply "VALUE-CHANGED" to br-clients.
end.
ON ROW-DISPLAY OF br-clients IN frame Dialog-Frame
DO:
  ...
  case p-from-version:
    when 'v15_0000':U then do:
      ...
      if p-list-mode = 'объект':U then do:
        v-attr-code = 'thth150-shop':U.
      end.
      else do:
        v-attr-code = 'thth150-clients':U.
      end.
    ...
    when 'v14_0':U then do:
      ...
      if p-list-mode = 'объект':U then do:
        v-attr-code = 'thth14-shop':U.
      end.
      else do:
        v-attr-code = 'thth14-clients':U.
      end.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-clients FOR EACH X_ext-classif NO-LOCK OUTER-JOIN INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
define variable v-cli-type-code-h as handle no-undo .
v-cli-type-code-h = br-clients:FIRST-COLUMN IN FRAME Dialog-Frame.
DO while valid-handle(v-cli-type-code-h) :
  if v-cli-type-code-h:LABEL = "КЛИЕНТ!" then do:
    leave.
  end.
  ELSE DO:
...
b-imp:tooltip in frame Dialog-Frame = substitute("&1 &2"
                                                  ...
                                                  , p-from-version)
b-imp-gds:tooltip in frame Dialog-Frame = substitute("&1 &2"
                                                  , b-imp-gds:tooltip in frame Dialog-Frame
                                                  , p-from-version)
...
X_ext-classif.charkey_two:visible in browse br-clients = (p-list-mode <> 'объект':U)
X_ext-classif.charkey_three:resizable in browse br-clients = yes
rs-key#_three = 0
.
if p-list-mode = 'объект':U then do:
  b-imp:label in frame Dialog-Frame = "Получение данных".
end.
rs-cli-type:radio-buttons in frame Dialog-Frame = 'орг':U + chr(44) + 'орг':U + chr(44) +
                                                   'чел':U + chr(44) + 'чел':U.
rs-cli-type = 'орг':U.
display
rs-key#_three
...
b-copy when (v-cntxt-db-num = 0 and lookup("b-add", bttns) > 0 and not transaction and p-list-mode <> 'объект':U and logical(v-closed) = no)
b-sch
B-Help
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-30 as character no-undo .
      ...
      parameter-4-30 =
        (
          if (" X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1                             and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'орг':U or X_ext-classif.charkey_One = 'чел':U) " + " " + where-phrase-30) <> ""
          then  substitute('X_ext-classif.classif-subject = &1&2&1                             and X_ext-classif.classif-name = &1&3&1                             and (&6 = -1  or X_ext-classif.key#_three = &6)                             AND X_ext-classif.db-num = - 1 AND (X_ext-classif.charkey_One = &1&4&1 or X_ext-classif.charkey_One = &1&5&1) '                             , chr(34), 'clients':U, v-cli-classif-name, 'орг':U, 'чел':U, rs-key#_three )  + " " + where-phrase-30
          else "true"
        )
      parameter-5-30 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-30 =
          (" X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1                             and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'орг':U or X_ext-classif.charkey_One = 'чел':U) " + " " + where-phrase-30 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-clients:handle
  ...
  if l-filter-open-30 = false then do:
    OPEN QUERY br-clients FOR EACH X_ext-classif no-lock
      where  X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1                             and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'орг':U or X_ext-classif.charkey_One = 'чел':U)
       BY X_ext-classif.charkey_three
      INDEXED-REPOSITION
  .
      assign
  ...
  on error undo, leave
  :
    if QUERY br-clients:handle:get-buffer-handle(1) = (buffer X_ext-classif:handle) then do:
      assign
      parameter-2-30 = (if p-find-next then "true":u else "false":u )
      parameter-4-30 =
        "where ":u +  substitute('X_ext-classif.classif-subject = &1&2&1                             and X_ext-classif.classif-name = &1&3&1                             and (&6 = -1  or X_ext-classif.key#_three = &6)                             AND X_ext-classif.db-num = - 1 AND (X_ext-classif.charkey_One = &1&4&1 or X_ext-classif.charkey_One = &1&5&1) '                             , chr(34), 'clients':U, v-cli-classif-name, 'орг':U, 'чел':U, rs-key#_three )  + " ":u + where-phrase-30 + " ":u + p-find-condition + " " + ""
      parameter-5-30 = "  "
    .
      ...
      assign
      parameter-2-30 = (if p-find-next then "true":u else "false":u )
      parameter-3-30 =  "FOR EACH X_ext-classif no-lock"
      parameter-4-30 =
        (
          if (" X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1                             and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'орг':U or X_ext-classif.charkey_One = 'чел':U) " + " " + where-phrase-30) <> ""
          then  substitute('X_ext-classif.classif-subject = &1&2&1                             and X_ext-classif.classif-name = &1&3&1                             and (&6 = -1  or X_ext-classif.key#_three = &6)                             AND X_ext-classif.db-num = - 1 AND (X_ext-classif.charkey_One = &1&4&1 or X_ext-classif.charkey_One = &1&5&1) '                             , chr(34), 'clients':U, v-cli-classif-name, 'орг':U, 'чел':U, rs-key#_three )  + " " + where-phrase-30
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-32 as character no-undo .
      ...
      parameter-4-32 =
        (
          if (" X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1
                            and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'маг':U or X_ext-classif.charkey_One = 'скл':U)                             " + " " + where-phrase-32) <> ""
          then  substitute('X_ext-classif.classif-subject = &1&2&1                             and X_ext-classif.classif-name = &1&3&1                             and (&6 = -1  or X_ext-classif.key#_three = &6)                             AND X_ext-classif.db-num = - 1 AND (X_ext-classif.charkey_One = &1&4&1 or X_ext-classif.charkey_One = &1&5&1) '                             , chr(34), 'clients':U, v-cli-classif-name, 'маг':U, 'скл':U, rs-key#_three)     + " " + where-phrase-32
          else "true"
        )
      parameter-5-32 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-32 =
          (" X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1
                            and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'маг':U or X_ext-classif.charkey_One = 'скл':U)                             " + " " + where-phrase-32 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-clients:handle
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-clients FOR EACH X_ext-classif no-lock
      where  X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1
                            and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'маг':U or X_ext-classif.charkey_One = 'скл':U)
       BY X_ext-classif.charkey_three
      INDEXED-REPOSITION
  .
      assign
  ...
  :
    if QUERY br-clients:handle:get-buffer-handle(1) = (buffer X_ext-classif:handle) then do:
      ...
      parameter-4-32 =
        "where ":u +  substitute('X_ext-classif.classif-subject = &1&2&1                             and X_ext-classif.classif-name = &1&3&1                             and (&6 = -1  or X_ext-classif.key#_three = &6)                             AND X_ext-classif.db-num = - 1 AND (X_ext-classif.charkey_One = &1&4&1 or X_ext-classif.charkey_One = &1&5&1) '                             , chr(34), 'clients':U, v-cli-classif-name, 'маг':U, 'скл':U, rs-key#_three)     + " ":u + where-phrase-32 + " ":u + p-find-condition + " " + ""
      parameter-5-32 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_ext-classif no-lock"
      parameter-4-32 =
        (
          if (" X_ext-classif.classif-subject = 'clients':U                             and X_ext-classif.classif-name = v-cli-classif-name                             AND X_ext-classif.db-num = - 1
                            and (rs-key#_three = -1  or X_ext-classif.key#_three = rs-key#_three)                             AND (X_ext-classif.charkey_One = 'маг':U or X_ext-classif.charkey_One = 'скл':U)                             " + " " + where-phrase-32) <> ""
...
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
run fltfield-add in this-procedure('charkey_two', "ИНН", '',
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
...
find first buf_clients no-lock where
        buf_clients.obj-type = entry(2, X_ext-classif.uniq-key-rec, chr(3))
    and buf_clients.obj-code = integer(entry(3, X_ext-classif.uniq-key-rec, chr(3))).
v-rid-list = string(recid(buf_clients)).
end.
run ref/cli-all.w (   input parparentproc
                ,input "b-sel"
                ,input (if p-list-mode = 'объект':U then 'объект':U else X_ext-classif.charkey_one)
                ,input 'все':U
                ,input 'текущие':U
                ,input ?
                ,input ",,,,,,NO,,"
                ,input (if p-list-mode = 'объект':U then "lock-cli-type" else '')
...
if p-list-mode = 'объект':U then do:
  if not (buf_clients.obj-type = 'маг':U
      or
      buf_clients.obj-type = 'скл':U) then do:
    message
...
if p-list-mode = 'объект':U then do:
  ...
  find first sysconf_ext-classif no-lock where
            sysconf_ext-classif.classif-subject = 'clients':U
        and sysconf_ext-classif.classif-name = v-cli-classif-name
        and sysconf_ext-classif.charkey_one = 'орг':U
...
run ref/extclas1.p (
                    ...
                    ,input X_ext-classif.Key#_One
                    ,input X_ext-classif.Key#_two
                    ,input X_ext-classif.Key#_three
                    ,input X_ext-classif.charkey_One
                    ,input X_ext-classif.charkey_two
                    ,input X_ext-classif.charkey_three
...
find first buf_ext-classif no-lock where
      ...
      AND (((buf_ext-classif.charkey_One = 'орг':U or buf_ext-classif.charkey_One = 'чел':U)
           and p-list-mode = '')
           or
           ((buf_ext-classif.charkey_One = 'маг':U or buf_ext-classif.charkey_One = 'скл':U)
           and p-list-mode = 'объект':U)
           )
...
do transaction:
  for each buf_ext-classif where
      ...
      and (
            ( (buf_ext-classif.charkey_one = 'скл':U
                or buf_ext-classif.charkey_one = 'маг':U)
                and p-list-mode = 'объект':U
            )
            or
              ( (buf_ext-classif.charkey_one = 'орг':U
                or buf_ext-classif.charkey_one = 'чел':U)
                and p-list-mode = ""
              )
           )
...
apply "entry":u to sch-self-code in frame Dialog-Frame .
END PROCEDURE.
...
if p-list-mode = 'объект':U then do:
  FIND FIRST buf_ext-classif NO-LOCK WHERE
      ...
      and buf_ext-classif.classif-name = v-cli-classif-name
      AND buf_ext-classif.db-num = - 1
      and (buf_ext-classif.charkey_one = 'маг':U
          or
          buf_ext-classif.charkey_one = 'скл':U
          )
      and buf_ext-classif.key#_three = 0
  ...
  FIND FIRST buf_ext-classif NO-LOCK WHERE
      ...
      and buf_ext-classif.classif-name = v-cli-classif-name
      AND buf_ext-classif.db-num = - 1
      and (buf_ext-classif.charkey_one = 'орг':U
          or
          buf_ext-classif.charkey_one = 'чел':U
          )
      and buf_ext-classif.key#_three = 0
      NO-ERROR.
end.
...
DEF VAR iBGColor AS INTEGER NO-UNDO.
  IF X_ext-classif.uniq-key-rec = "":U THEN DO:
    ...
    ASSIGN
     X_ext-classif.charkey_three:FGCOLOR  in BROWSE br-clients = iFGColor
     X_ext-classif.charkey_three:BGCOLOR  in BROWSE br-clients = iBGColor
    .
END PROCEDURE.
FUNCTION get-cli-name RETURNS CHARACTER
