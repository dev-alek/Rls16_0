...
define variable vss-description as character no-undo init "Список сущностей правил скидок".
...
define variable v-dr-tables as character no-undo.
define variable v-dr-table-names as character no-undo.
...
message
"Хотите экспортировать ПОЛНУЮ конфигурацию правил скидок" skip
...
if v-yesno = no then do:
  run gbl/d-file.p (
      ...
      , input  no
      , input  yes
      , input  "Введите имя старого файла (для сравнения)"
      , output v-yesno
  ) no-error.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-vrmtables :handle
      .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run currsysk in g#library
  ...
  assign
  v-dr-tables = v-dr-tables + chr(44) + 'dis-rule':U +
                                chr(44) + 'dis-cfg-rule':U +
                                chr(44) + 'dis-time-rule':U
                                .
  assign
  v-dr-table-names = v-dr-table-names + chr(44) + 'правила скидок':U +
                                chr(44) + 'Связи dis-rule':U +
                                chr(44) + 'расписания':U
                                .
  assign
  v-dr-tables = trim(v-dr-tables).
  run Myenable.
...
PROCEDURE enable_UI :
  ...
  ENABLE b-quit B-sel b-export B-Help rs-language br-vrmtables
      WITH FRAME Dialog-Frame.
  VIEW FRAME Dialog-Frame.
  OPEN QUERY br-vrmtables FOR EACH buf_file NO-LOCK WHERE lookup(buf_file._file-name,        v-dr-tables) > 0.
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
CASE buf_file._file-name:
  WHEN 'dis-rule':U THEN DO:
    run utl/disruls0.w ( INPUT parparentproc
                         ,INPUT (if v-ibs then
                                'b-add':U
                                else '':U)
  ...
  WHEN 'dis-time-rule':U THEN DO:
    run utl/distrls0.w ( INPUT parparentproc
                         ,INPUT (if v-ibs then
                                'b-add':U
                                else '':U)
                         ,INPUT-OUTPUT v-rid-list) NO-ERROR.
  END.
  WHEN 'dis-cfg-rule':U THEN DO:
    run utl/discfgrs.w ( INPUT parparentproc
                         ,INPUT (if v-ibs then
                                'b-add':U
                                else '':U)
                         ,INPUT-OUTPUT v-rid-list) NO-ERROR.
...
v-tbl-fname = ENTRY(LOOKUP(p-tbl-name, v-dr-tables), v-dr-table-names).
return v-tbl-fname.
END FUNCTION.
