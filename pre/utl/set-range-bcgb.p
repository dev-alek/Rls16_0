...
mParams = trim(session:parameter,'"').
  if dynamic-current-value( "s-bcgb-code", "ub" ) > 1000000000 then return.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,output par-type
    ...
    assign
       bufcode-range.last-code  = if code-range.last-code > 1000000000 then code-range.last-code else 2000000000
       bufcode-range.first-code = 1000000000.
    assign
       code-range.last-code     = 1000000000 - 1
       code-range.stts          = "u":U
       .
    if code-range.first-code eq  1
    then
       code-range.first-code = 100000.
    dynamic-current-value( "s-bcgb-code", "ub" ) = 1000000000.
    finally:
       quit.
    end finally.
