...
PROCEDURE uniq-grp.
def input param c like ub.gds-grp.upper-code.
def input param ln like ub.gds-grp.lvl-num.
def buffer b-gds-grp for ub.gds-grp.
for each b-gds-grp where b-gds-grp.upper-code = c:
  find first ub.gds-grp where ub.gds-grp.upper-code = c and
                                ub.gds-grp.node-name begins b-gds-grp.node-name
                                and recid(ub.gds-grp) <> recid(b-gds-grp)
                                no-lock no-error.
  if available ub.gds-grp then
    assign
      b-gds-grp.node-name = b-gds-grp.node-name + "."
      upd-grp = upd-grp + 1.
  run uniq-grp (b-gds-grp.node-code, ln + 1).
  tot-grp = tot-grp + 1.
