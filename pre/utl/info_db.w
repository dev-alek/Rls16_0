...
END.
ON CHOOSE OF b-area IN FRAME Dialog-Frame
DO:
...
ON ROW-DISPLAY OF br-area IN frame Dialog-Frame
DO:
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-area :handle
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  FOR EACH _areastatus NO-LOCK:
      ...
      IF _AreaStatus._AreaStatus-Areaname  BEGINS "Primary Recovery" THEN do:
          ASSIGN
            SizeBI = SizeBI + ( DEC (_AreaStatus._AreaStatus-Totblocks) * DEC( _Area._Area-blocksize)) / 1024
            VolBI = VolBI + 1.
          NEXT.
      END.
      IF _AreaStatus._AreaStatus-Areaname  BEGINS "After Image" THEN do:
          ASSIGN
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-area FOR EACH db-area NO-LOCK.
END PROCEDURE.
PROCEDURE set-row-color :
  ...
  define variable iBGColor AS INTEGER NO-UNDO.
      ...
      ELSE
          ...
          .
    ASSIGN
      db-area.percent:FGCOLOR IN BROWSE br-area = iFGColor
      db-area.percent:BGCOLOR IN BROWSE br-area = iBGColor
    .
END PROCEDURE.
