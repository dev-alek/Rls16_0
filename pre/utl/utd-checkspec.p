...
for first buf_contract no-lock where buf_contract.host-code = buf_utd.host-code
  ...
  then do :
    ...
    AddUtdErr(buf_utd.db-num,
              ...
              "CheckMOTP",
              "ContrDate",
              (buf_contract.contract-name + chr(4) + string(buf_contract.contract-date-end)) ) .
  end .
  else do :
...
( input buf_utd.host-code
 ,input buf_utd.contract-code
 ,input buf_utd-lines.gds-code
 ,input v-price
 ,input v-vattype
 ,input v-vatpc
) no-error .
    ...
    do:
      ...
      AddUtdErr(buf_utd.db-num,
                ...
                "CheckСontract",
                "SpecifErr",
                (string(buf_utd-lines.linenum) + chr(4) + error-status :get-message(1) + chr(4) + return-value)).
      if vMarkUtd
      then do :
        for each buf_utd-marking-lines no-lock where buf_utd-marking-lines.db-num   = buf_utd-lines.db-num
                                                 and buf_utd-marking-lines.doc-id   = buf_utd-lines.doc-id
