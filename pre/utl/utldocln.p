...
procedure change-VAT-pc :
  ...
  :
    find first buf_trn-doc no-lock
      ...
      .
    if buf_trn-doc.VAT-type = 'без':U
    or buf_trn-doc.VAT-type = 'нет':U
    then do:
      message
   ...
   if (not trn-doc.internal and
      ...
      if varinvprb = "base":u then do:
        assign
          ...
          road-tax-rubl-loc = buf_doc-line.road-tax * trn-doc.base-rate / trn-doc.base-scale.
      end.
      else do:
        ASSIGN
          road-tax-rubl-loc = buf_doc-line.road-tax
          road-tax-base-loc = buf_doc-line.road-tax / trn-doc.base-rate * trn-doc.base-scale.
      end.
...
.
    ...
    run gbl/d-prompt.w
      ...
      + 'text2=Сумма в учетн. ценах без НДС (руб) \'
      + 'format=>>>>>>>>>>>>9.9999999999\'
      + 'type=decimal\'
...
.
    message
      ...
      "Новая сумма НДС в руб" vat-rubl-loc * buf_doc-line.fact-qnty skip
...
procedure change-cst-code :
  ...
  :
    ...
    :
      ...
      run gbl/d-prompt.w
        ...
        + 'text1=':U + "Партия " + str-encode(buf_parts.in-code, "", '\=':U )
          + chr(32) + str-encode(buf_parts.part-code, "", '\=':U )
          + chr(32) + string(ind) + " из " + string(v-total-parts) + '\':U
    ...
    message
      ...
      ""  ( if  l-single-db <> true
            and v-change-ind <> 0
            then "Внимание! Существуют удалённые базы данных." + chr(10)
               + "ГТД будет изменено только в текущей базе данных." + chr(10)
            else ""
          ) skip
      view-as alert-box information .
  end.
  ...
  :
    ...
    :
      ...
      run gbl/d-prompt.w
        ...
        + 'text1=':U + "Партия " + str-encode(buf_parts.in-code, "", '\=':U )
          + chr(32) + str-encode(buf_parts.part-code, "", '\=':U )
          + chr(32) + string(v-ind) + " из " + string(v-total-parts) + '\':U
        + 'text2=' + "Артикул " + str-encode(buf_parts.artic, "", '\=':U )
          + chr(32) + string(buf_parts.prod-type)
          + chr(32) + string(buf_parts.prod-code) + '\':U
        + 'format=99/99/9999\':U
  ...
  ,output v-gds-code
    ...
    message
      ...
      ""  ( if  l-single-db <> true
            and v-change-ind <> 0
            then "Внимание! Существуют удалённые базы данных." + chr(10)
               + "Срок годности будет изменено только в данной базе данных." + chr(10)
            else ""
          ) skip
      view-as alert-box information .
  end.
