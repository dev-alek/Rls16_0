...
DEFINE TEMP-TABLE tt_currency NO-UNDO LIKE ub.currency.
...
MESSAGE "Вы уверены, что хотите начать экспорт"
        "справочников валют и курсов валют" + "?"
VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO TITLE "Экспорт справочников" UPDATE l_start-export AS LOGICAL.
RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочников валют и курсов валют" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ).
...
DO ON ERROR  UNDO Prepare-Block, RETURN ERROR
  ...
  FOR EACH ub.currency NO-LOCK :
    ...
    IF ( j_docs MODULO 100 ) = 0 THEN DO: RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочников валют и курсов валют" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ). END.
    CREATE tt_currency.
    BUFFER-COPY ub.currency
             TO tt_currency.
    FOR EACH ub.curr-accnt NO-LOCK WHERE ub.curr-accnt.curr-code = ub.currency.curr-code :
      CREATE tt_curr-accnt.
      BUFFER-COPY ub.curr-accnt TO tt_curr-accnt.
    END.
    FOR EACH ub.curr-bank  NO-LOCK WHERE ub.curr-bank.curr-code  = ub.currency.curr-code :
      CREATE tt_curr-bank.
...
DO TRANSACTION ON ERROR  UNDO BlockTransaction, RETURN ERROR
  ...
  OUTPUT STREAM out-str TO VALUE( v_dmp-dir + "ref-" + SUBSTRING( "currency", 1, 3 ) + ".txt" ).
  FOR EACH tt_currency :
    PROCESS EVENTS.
    ASSIGN j_docs = j_docs + 1.
    IF ( j_docs MODULO 100 ) = 0 THEN DO: RUN WaitFram-Show IN THIS-PROCEDURE ( INPUT "Идет экспорт " + "справочников валют и курсов валют" + ", ждите..." + TRIM( STRING( j_docs, "->,>>>,>>>,>>9":U ) ) ). END.
    EXPORT STREAM out-str tt_currency.
  END.
...
MESSAGE "Экспорт" "справочников валют и курсов валют" "закончен" SKIP
