...
on stop undo, next _cmp:
   ...
   find first buf_ext-classif where
        ...
        and buf_ext-classif.classif-name = 'inn':U
   ...
   and buf_ext-classif.charkey_one <> buf_firm.inn then do:
     ...
     if error-status:error then do:
        run write-log in p-log-handle (
              input 1
            , input substitute( substitute("Ошибка при удалении ИНН (ext-classif), не совпадающего с ИНН в карточке &1&2"
                                           , 'орг':U, buf_firm.firm-code))).
       next _cmp.
     end.
     else do:
   ...
   run ref/extclas1.p ( INPUT 'ДОБАВЛЕНИЕ':U
    ...
    if not error-status:error then do:
      ...
      run write-log in p-log-handle (
            input 1
          , input substitute( substitute("Ошибка при сохранении ИНН (ext-classif) для &1&2"
                                          , 'орг':U, buf_firm.firm-code))).
   end.
end.
_prs:
...
for each buf_ext-classif no-lock where
  ...
  or not v-tbl-name = 'clients':U then do:
     run ref/extclas3.p ( input yes
                         ,input recid(buf_ext-classif)) no-error.
     if error-status:error then do:
        run write-log in p-log-handle (
              input 1
            , input substitute( substitute("Ошибка при удалении ИНН (ext-classif), с невернмы uniq-key-rec &1"
