...
DEFINE BUFFER buf_goods FOR ub.goods.
  ...
  do while valid-handle(v-uh):
    if lookup("cb-for-struct-i", v-uh:internal-entries) > 0 then do:
      ...
      ASSIGN
      e-struct:SCREEN-VALUE IN FRAME Dialog-Frame = v-struct
      v-setted = yes
      .
      if p-mode = 'ДОБАВЛЕНИЕ':U
      then do:
        case p-spr-param:
          when '15x80':U then do:
...
IF p-gds-code > 0 THEN DO:
  ...
  then do:
    ...
    ASSIGN
    e-struct:SCREEN-VALUE IN FRAME Dialog-Frame = buf_goods.struct.
  end.
END.
ASSIGN
...
END.
ASSIGN
v-num-entries = num-entries(p-attr-value, chr(4))
.
DO v-ii = 1 TO v-num-rows:
   ...
   SENSITIVE = (p-mode <> 'ПРОСМОТР':U)
   VISIBLE = TRUE
   SCREEN-VALUE = (IF v-num-entries >= v-ii
                   THEN ENTRY(v-ii, p-attr-value, chr(4))
                   ELSE '')
   .
END.
ENABLE
...
FOR each temp-widget by index_:
  ASSIGN
  v-struct = v-struct + (IF temp-widget.INDEX_ = 1 THEN '':U ELSE chr(4)) + temp-widget.HANDLE_:SCREEN-VALUE.
END.
ASSIGN
