...
assign
  v-str-recid10 = trim( string( recid( X_c-hist-nws-option ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-hst-nws-option in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-hst-nws-option in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-hst-nws-option :handle
      .
...
do:
  v-doc-rec = recid(X_c-hist-nws-option). run OpenBr in this-procedure. reposition br-hst-nws-option to recid v-doc-rec no-error. v-doc-rec = ?.               apply 'value-changed' to br-hst-nws-option.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-hst-nws-option   as character no-undo .
def var sort-clmnbr-hst-nws-option    as handle    no-undo .
def var cur-clmnbr-hst-nws-option     as handle    no-undo .
def var cur-clmn-locbr-hst-nws-option as integer   no-undo .
def var re-querybr-hst-nws-option     as logical   initial no no-undo .
on start-search, ctrl-o of br-hst-nws-option in frame Dialog-Frame do:
   run sort-brbr-hst-nws-option
...
PROCEDURE sort-brbr-hst-nws-option :
  define input parameter p-recid as recid no-undo .
  if re-querybr-hst-nws-option = no then do:
    assign
       cur-clmnbr-hst-nws-option = br-hst-nws-option:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-hst-nws-option <> ? then sort-clmnbr-hst-nws-option:column-fgcolor = 0.
    if cur-clmnbr-hst-nws-option = sort-clmnbr-hst-nws-option then do:
      assign
         sort-labelbr-hst-nws-option = ""
         sort-clmnbr-hst-nws-option = ?
      .
     end.
     else do:
       assign
         sort-labelbr-hst-nws-option = cur-clmnbr-hst-nws-option:label
         sort-clmnbr-hst-nws-option  = cur-clmnbr-hst-nws-option
         sort-clmnbr-hst-nws-option:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-hst-nws-option = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-hst-nws-option:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-hst-nws-option then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-hst-nws-option = cur-clmn-locbr-hst-nws-option + 1
    .
  end.
  case sort-labelbr-hst-nws-option:
        when X_c-hist-nws-option.table-name:label in browse br-hst-nws-option then DO:   assign     sort-column-name = "X_c-hist-nws-option.table-name"   .   run OpenBr in this-procedure .   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure .
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-hst-nws-option') then do:
          run mv-brw-defaultbr-hst-nws-option.
        end.
      if sort-labelbr-hst-nws-option <> "" then do:
        assign
          cur-clmnbr-hst-nws-option:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-hst-nws-option = ?
      .
    end.
  end case.
    if cur-clmn-locbr-hst-nws-option <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-hst-nws-option') then do:
        run ch-clmnbr-hst-nws-option in this-procedure (cur-clmn-locbr-hst-nws-option).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-hst-nws-option to recid p-recid no-error.
    apply "value-changed" to br-hst-nws-option in frame Dialog-Frame.
  end.
  apply "entry" to br-hst-nws-option in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-hst-nws-option:
if cur-clmnbr-hst-nws-option = ? then do:
   run OpenBr in this-procedure .
end.
else do:
   assign re-querybr-hst-nws-option = yes.
   run sort-brbr-hst-nws-option
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
   ...
   assign re-querybr-hst-nws-option = no.
end.
end.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
 ...
 if LOOKUP(p-mode, ('one':U + chr(4) +
                    "subject-group-db" + chr(4)  +
                    "one-db" + chr(4) +
                    "subject-group" + chr(4) +
                    "db"
                     )
...
DEF VAR cur-clmn-numbr-hst-nws-option as INT EXTENT 8 no-undo.
DEF VAR varmvibr-hst-nws-option       as INT no-undo.
DEF VAR varmvjbr-hst-nws-option       as INT no-undo.
DEF VAR varmvkbr-hst-nws-option       as INT no-undo.
DEF VAR varmvlbr-hst-nws-option       as INT no-undo.
DEF VAR move-elementbr-hst-nws-option as INT no-undo.
def var jjbr-hst-nws-option           as int no-undo.
do varmvibr-hst-nws-option = 1 to EXTENT(cur-clmn-numbr-hst-nws-option):
  ASSIGN cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = varmvibr-hst-nws-option.
END.
RUN start-mv-clmnbr-hst-nws-option.
PROCEDURE start-mv-clmnbr-hst-nws-option:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-hst-nws-option = NUM-ENTRIES('1,2,3,4,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-hst-nws-option ( cur-clmn-numbr-hst-nws-option[INTEGER(ENTRY (jjbr-hst-nws-option, '1,2,3,4,5,6,7,8'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-hst-nws-option do:
  RUN re-move-clmnbr-hst-nws-option ( 1, 8).
END.
ON ctrl-cursor-left OF BROWSE br-hst-nws-option do:
  RUN re-move-clmnbr-hst-nws-option (8, 1).
END.
PROCEDURE re-move-clmnbr-hst-nws-option:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-hst-nws-option = 1 TO EXTENT(cur-clmn-numbr-hst-nws-option):
    if cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = source-column THEN cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = -1.
  END.
  if br-hst-nws-option:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-hst-nws-option = source-column - 1 to target-column BY -1:
    DO varmvibr-hst-nws-option = 1 TO EXTENT(cur-clmn-numbr-hst-nws-option):
        if cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = varmvjbr-hst-nws-option THEN DO:
          cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-hst-nws-option = source-column + 1 to target-column:
    DO varmvibr-hst-nws-option = 1 TO EXTENT(cur-clmn-numbr-hst-nws-option):
      if cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = varmvjbr-hst-nws-option THEN DO:
        cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] - 1.
      END.
  ...
  END.
  DO varmvibr-hst-nws-option = 1 TO EXTENT(cur-clmn-numbr-hst-nws-option):
    if cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = -1 THEN cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-hst-nws-option:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-hst-nws-option = 1 TO EXTENT(cur-clmn-numbr-hst-nws-option):
    if cur-clmn-numbr-hst-nws-option[varmvibr-hst-nws-option] = cur-clmn-loc THEN move-elementbr-hst-nws-option = varmvibr-hst-nws-option.
  END.
  RUN re-move-clmnbr-hst-nws-option (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-hst-nws-option:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-hst-nws-option = 1 to EXTENT(cur-clmn-numbr-hst-nws-option):
    RUN re-move-clmnbr-hst-nws-option (cur-clmn-numbr-hst-nws-option[varmvlbr-hst-nws-option], varmvlbr-hst-nws-option).
  END.
  RUN start-mv-clmnbr-hst-nws-option.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-hst-nws-option FOR EACH X_c-hist-nws-option NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
 ...
 + "nws-to-hist" + chr(4) + "Создание ист. при приеме по СПН" + chr(4) + "get-hn-label"
 .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-hist-nws-option:handle
                                            ,input  'hist-nws-option':U
                                            ,input  "option-descr,db-num,table-name,subject-group,get-hist-form-nws,hist-from-prim,hist-to-nws,nws-to-hist"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
FUNCTION get-hn-label RETURNS CHARACTER
