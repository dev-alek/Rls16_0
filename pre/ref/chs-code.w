...
END.
ON MOUSE-SELECT-DBLCLICK OF BR-code-range IN FRAME Dialog-Frame
DO:
   IF NOT AVAILABLE tt-code-range THEN RETURN no-apply.
    RUN assign-and-find IN THIS-PROCEDURE NO-ERROR.
END.
ON RETURN OF BR-code-range IN FRAME Dialog-Frame
DO:
   IF NOT AVAILABLE tt-code-range THEN RETURN no-apply.
    RUN assign-and-find IN THIS-PROCEDURE NO-ERROR.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-code-range :handle
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  END.
  IF (p-obj-type = 'маг':U
  or p-obj-type = 'скл':U) AND p-db-num <> 0 THEN DO:
      message
      vss-workfile vss-revision vss-description skip
  ...
  END.
  if p-obj-type = 'орг':U
  or p-obj-type = 'чел':U then
  run fill-tt in this-procedure ( input (if p-obj-type = 'орг':U then 'fmgb':U else 'pngb':U)) .
...
CASE p-obj-type:
  WHEN 'орг':U OR WHEN 'чел':U THEN do:
    assign
    v-result = calc-range(
                    input p-db-num
                    ,input p-ii
                    ,input (IF p-obj-type = 'орг':U THEN 'fmgb':U ELSE 'pngb':U)
                    ) no-error .
    if v-result = ? then do:
      MESSAGE
...
define buffer buf_code-range for ub.code-range .
IF p-range-type = 'fmgb':U THEN
ASSIGN
v-seq-val = current-value(s-fmgb-code, ub).
IF p-range-type = 'pngb':U THEN
ASSIGN
...
CASE p-obj-type:
    ...
    OR
    WHEN 'чел':U THEN DO:
       FRAME Dialog-Frame:TITLE = substitute("&1 для контрагента типа &2"
    ...
    WHEN 'скл':U THEN DO:
      ...
      f-end = 99999
      .
      FRAME Dialog-Frame:TITLE = substitute("&1 для &2"
                                             , FRAME Dialog-Frame:TITLE
                                             , p-obj-type).
    END.
END CASE.
...
RS-up-down
f-cv
f-start  WHEN (p-obj-type = 'маг':U OR p-obj-type = 'скл':U)
f-end    WHEN (p-obj-type = 'маг':U OR p-obj-type = 'скл':U)
WITH FRAME Dialog-Frame.
ENABLE
f-cv
B-exit
...
RS-up-down
B-find
br-code-range WHEN (p-obj-TYPE = 'орг':U OR p-obj-type = 'чел':U)
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
IF (p-obj-type = 'маг':U
        ...
        DO ii = f-cv TO v-border BY -1:
          ...
          IF v-free THEN DO:
            ...
            f-free-code:LABEL = substitute("Первый свободный код &1 &2"
                                          ...
                                          , f-cv).
            DISPLAY
            f-free-code WITH FRAME Dialog-Frame.
            return.
          END.
        END.
      END.
