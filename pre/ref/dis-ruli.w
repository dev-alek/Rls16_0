  ...
  do:
    ...
    do:
      ...
      do:
        ...
        browse BR-gds-rule-attr:refresh().
      end.
    end.
  end.
END.
...
END.
ON LEAVE OF tt-dis-rule.CharKey_One IN FRAME Dialog-Frame
DO:
  ...
  if focus :handle <> self :handle  then do:
    return no-apply .
     ...
     RUN recalc IN THIS-PROCEDURE(
                                     ...
                                     ,INPUT "discnt-value"
                                     ,INPUT (INPUT FRAME Dialog-Frame tt-dis-rule.discnt-value )
                                     ,INPUT (INPUT FRAME Dialog-Frame tt-dis-rule.doc-qnty )
                                     ,INPUT (INPUT FRAME Dialog-Frame tt-dis-rule.tot-sum )
                                     )
   .
END.
ON LEAVE OF tt-dis-rule.CharKey_Three IN FRAME Dialog-Frame
DO:
...
then do:
  ...
  if focus :handle <> self :handle  then do:
   ...
   .
END.
ON LEAVE OF tt-dis-rule.CharKey_Two IN FRAME Dialog-Frame
DO:
...
then do:
  ...
  if focus :handle <> self :handle  then do:
   ...
   .
END.
ON LEAVE OF tt-dis-rule.dis-kat IN FRAME Dialog-Frame
DO:
...
then do:
 ...
 .
END.
ON LEAVE OF tt-dis-rule.discnt-value IN FRAME Dialog-Frame
DO:
...
END.
ON LEAVE OF tt-dis-rule.doc-qnty IN FRAME Dialog-Frame
DO:
...
.
END.
ON LEAVE OF f-price-discnt IN FRAME Dialog-Frame
DO:
...
.
END.
ON LEAVE OF f-price-netto IN FRAME Dialog-Frame
DO:
...
.
END.
ON LEAVE OF f-sum-discnt IN FRAME Dialog-Frame
DO:
...
.
END.
ON LEAVE OF f-sum-netto IN FRAME Dialog-Frame
DO:
...
then do:
  ...
  if focus :handle <> self :handle  then do:
   ...
   .
END.
ON LEAVE OF tt-dis-rule.Key#_Three IN FRAME Dialog-Frame
DO:
...
then do:
  ...
  if focus :handle <> self :handle  then do:
   ...
   .
END.
ON LEAVE OF tt-dis-rule.Key#_Two IN FRAME Dialog-Frame
DO:
...
END.
ON LEAVE OF tt-dis-rule.tot-sum IN FRAME Dialog-Frame
DO:
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-gds-rule-attr :handle
      .
...
hide br-gds-rule-attr in frame Dialog-Frame.
CASE p-display-hide:
  WHEN 1 THEN DO:
    IF p-main THEN DO:
      IF lookup("dis-kat", v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.dis-kat WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("doc-qnty":U, v-level-1) > 0 THEN DO:
        ...
        WITH FRAME Dialog-Frame.
        ENABLE
        tt-dis-rule.doc-qnty WHEN p-mode <> 'ПРОСМОТР':U
        WITH FRAME Dialog-Frame.
        IF is-good-mode  THEN DO:
          ...
          do:
           DISPLAY
            ...
            WITH FRAME Dialog-Frame.
           ENABLE
            f-sum-brutto WHEN p-mode <> 'ПРОСМОТР':U
            f-sum-discnt WHEN p-mode <> 'ПРОСМОТР':U
            f-sum-netto WHEN p-mode <> 'ПРОСМОТР':U
            WITH FRAME Dialog-Frame.
          end.
        END.
      END.
      ...
      IF lookup("tot-sum":U, v-level-1) > 0 THEN DO:
        ...
        WITH FRAME Dialog-Frame.
        ENABLE
        tt-dis-rule.tot-sum WHEN p-mode <> 'ПРОСМОТР':U
        WITH FRAME Dialog-Frame.
        IF is-good-mode THEN DO:
          DISPLAY
          f-sum-brutto
          ...
          f-sum-discnt WHEN p-mode <> 'ПРОСМОТР':U
          f-sum-netto WHEN p-mode <> 'ПРОСМОТР':U
          f-sale-qnty WHEN p-mode <> 'ПРОСМОТР':U
          WITH FRAME Dialog-Frame.
        END.
      END.
      else do:
      ...
      IF lookup("key#_two":U, v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.key#_two WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("key#_three":U, v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.key#_three WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("charkey_two":U, v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.charkey_two WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("charkey_three":U, v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.charkey_three WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("deckey_one":U, v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.deckey_one WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("deckey_two":U, v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.deckey_two WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("deckey_three":U, v-level-1) > 0 THEN DO:
         ...
         WITH FRAME Dialog-Frame.
         ENABLE
         tt-dis-rule.deckey_three WHEN p-mode <> 'ПРОСМОТР':U
         WITH FRAME Dialog-Frame.
      END.
      else do:
        hide
      ...
      IF lookup("discnt-value", v-level-1) > 0 THEN DO:
        ...
        WITH FRAME Dialog-Frame.
        ENABLE
        tt-dis-rule.discnt-value WHEN p-mode <> 'ПРОСМОТР':U
        WITH FRAME Dialog-Frame.
        IF is-good-mode
        and (v-value-type = integer('1':U)
             or
             v-value-type = integer('2':U)
             or
             v-value-type = integer('3':U))
        THEN DO:
          DISPLAY
          f-price-brutto
          f-price-discnt
          ...
          WITH FRAME Dialog-Frame.
          ENABLE
          f-price-discnt WHEN p-mode <> 'ПРОСМОТР':U
          f-price-netto  WHEN p-mode <> 'ПРОСМОТР':U
          WITH FRAME Dialog-Frame.
    ...
    ELSE DO:
      ...
      IF lookup("tot-sum", v-level-2) > 0 THEN DO:
        ...
        WITH FRAME Dialog-Frame.
        IF is-good-mode
        and (v-term-value-type = integer('1':U)
             or
             v-term-value-type = integer('2':U)
             or
             v-term-value-type = integer('3':U))
        THEN DO:
          DISPLAY
      ...
      IF lookup("discnt-value", v-level-2) > 0 THEN DO:
        ...
        THEN DO:
          ...
          f-price-netto  WHEN p-mode <> 'ПРОСМОТР':U
          WITH FRAME Dialog-Frame.
          if v-term-value-type <> integer('1':U) THEN DO:
            DISPLAY
            f-d-pcnt
            WITH FRAME Dialog-Frame.
            ENABLE
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-gds-rule-attr FOR EACH temp-gds-rule-attr .
END PROCEDURE.
...
if tt-dis-rule.is-term = no then do:
  FOR EACH buf_dis-rule NO-LOCK WHERE
    ...
    assign
    tt-bc-dis-rule.rule-num = (if p-mode = 'ДОБАВЛЕНИЕ':U
                               then (if v-is-copy
                                     then v-ii
    ...
    .
    if buf_dis-rule.tot-sum = - 1 then do:                                                                                                                                                                 CASE buf_dis-rule.value-type:                                                                                         WHEN INTEGER('1':U) THEN DO:                                                                         ASSIGN                                                                                                         tt-bc-dis-rule.d-pcnt      = buf_dis-rule.discnt-value                                                        tt-bc-dis-rule.price-discnt = tt-bc-dis-rule.price-brutto * buf_dis-rule.discnt-value / 100                      tt-bc-dis-rule.price-netto = tt-bc-dis-rule.price-brutto - tt-bc-dis-rule.price-discnt                         tt-bc-dis-rule.sum-brutto = ABS(tt-bc-dis-rule.price-brutto * buf_dis-rule.doc-qnty)                         tt-bc-dis-rule.sum-netto = ABS(tt-bc-dis-rule.price-netto * buf_dis-rule.doc-qnty)                           tt-bc-dis-rule.sum-discnt = ABS(tt-bc-dis-rule.price-discnt * buf_dis-rule.doc-qnty).                      END.                                                                                                            WHEN INTEGER('2':U) THEN DO:                                                                            ASSIGN                                                                                                          tt-bc-dis-rule.price-discnt = buf_dis-rule.discnt-value                                                       tt-bc-dis-rule.price-netto = tt-bc-dis-rule.price-brutto - tt-bc-dis-rule.price-discnt                          tt-bc-dis-rule.d-pcnt       = 100 * (1 - tt-bc-dis-rule.price-netto / tt-bc-dis-rule.price-brutto)              tt-bc-dis-rule.sum-brutto = ABS(tt-bc-dis-rule.price-brutto * buf_dis-rule.doc-qnty)                          tt-bc-dis-rule.sum-netto = ABS(tt-bc-dis-rule.price-netto * buf_dis-rule.doc-qnty)                            tt-bc-dis-rule.sum-discnt = ABS(tt-bc-dis-rule.price-discnt * buf_dis-rule.doc-qnty).                     END.                                                                                                            WHEN INTEGER('3':U) THEN DO:                                                                             ASSIGN                                                                                                          tt-bc-dis-rule.price-netto = buf_dis-rule.discnt-value                                                        tt-bc-dis-rule.price-discnt = tt-bc-dis-rule.price-brutto - tt-bc-dis-rule.price-netto                          tt-bc-dis-rule.sum-brutto = ABS(tt-bc-dis-rule.price-brutto * buf_dis-rule.doc-qnty)                          tt-bc-dis-rule.sum-netto = ABS(tt-bc-dis-rule.price-netto * buf_dis-rule.doc-qnty)                            tt-bc-dis-rule.sum-discnt = ABS(tt-bc-dis-rule.price-discnt * buf_dis-rule.doc-qnty).                     END.                                                                                                       END CASE.                                                                                                   end.                                                                                                           else do:                                                                                                                                                                                  assign  tt-bc-dis-rule.sale-qnty = truncate(buf_dis-rule.tot-sum / tt-bc-dis-rule.price-brutto, v-meas).      CASE buf_dis-rule.value-type:                                                                                         WHEN INTEGER('1':U) THEN DO:                                                                         ASSIGN                                                                                                         tt-bc-dis-rule.price-discnt = tt-bc-dis-rule.price-brutto * buf_dis-rule.discnt-value / 100                      tt-bc-dis-rule.d-pcnt      = buf_dis-rule.discnt-value                                                       tt-bc-dis-rule.price-netto = tt-bc-dis-rule.price-brutto - tt-bc-dis-rule.price-discnt                         tt-bc-dis-rule.sum-brutto = tt-bc-dis-rule.price-brutto * tt-bc-dis-rule.sale-qnty                             tt-bc-dis-rule.sum-netto = tt-bc-dis-rule.price-netto * tt-bc-dis-rule.sale-qnty                               tt-bc-dis-rule.sum-discnt = tt-bc-dis-rule.price-discnt * tt-bc-dis-rule.sale-qnty .                          END.                                                                                                            WHEN INTEGER('2':U) THEN DO:                                                                            ASSIGN                                                                                                          tt-bc-dis-rule.price-discnt = buf_dis-rule.discnt-value                                                       tt-bc-dis-rule.price-netto = tt-bc-dis-rule.price-brutto - tt-bc-dis-rule.price-discnt                          tt-bc-dis-rule.d-pcnt       = 100 * (1 - tt-bc-dis-rule.price-netto / tt-bc-dis-rule.price-brutto)              tt-bc-dis-rule.sum-brutto = tt-bc-dis-rule.price-brutto * tt-bc-dis-rule.sale-qnty                              tt-bc-dis-rule.sum-netto = tt-bc-dis-rule.price-netto * tt-bc-dis-rule.sale-qnty                                tt-bc-dis-rule.sum-discnt = tt-bc-dis-rule.price-discnt * tt-bc-dis-rule.sale-qnty .                        END.                                                                                                            WHEN INTEGER('3':U) THEN DO:                                                                             ASSIGN                                                                                                          tt-bc-dis-rule.price-netto = buf_dis-rule.discnt-value                                                        tt-bc-dis-rule.price-discnt = tt-bc-dis-rule.price-brutto - tt-bc-dis-rule.price-netto                          tt-bc-dis-rule.sum-brutto = tt-bc-dis-rule.price-brutto * tt-bc-dis-rule.sale-qnty                              tt-bc-dis-rule.sum-netto = tt-bc-dis-rule.price-netto * tt-bc-dis-rule.sale-qnty                                tt-bc-dis-rule.sum-discnt = tt-bc-dis-rule.price-discnt * tt-bc-dis-rule.sale-qnty .                        END.                                                                                                       END CASE.                                                                                                   end.
  END.
end.
else do:
  FOR EACH buf_dis-rule NO-LOCK WHERE
...
tt-dis-rule.discnt-value:LABEL = (IF v-value-type = INTEGER('1':U)
...
ELSE DO:
    ...
    DO ii = 1 TO NUM-ENTRIES(v-level-2):
      ...
      END CASE.
    END.
    if v-value-type <> integer('1':U)
    and p-b-code > 0 then do:
      tt-bc-dis-rule.d-pcnt:VISIBLE IN BROWSE br-term-dr = YES.
    end.
    assign
    tt0-term_dis-rule.discnt-value:LABEL IN BROWSE br-term-dr = (IF v-value-type = INTEGER('1':U)
...
if is-good-mode then do:
   assign
   frame Dialog-Frame:title = frame Dialog-Frame:title + substitute("в применении к товару &1, цены для бар-кода &2"
                                                                       , X_goods.gds-code
                                                                       , p-b-code) .
end.
DISPLAY
...
WITH FRAME Dialog-Frame.
  ...
  DISPLAY
  tt-dis-rule.discnt-value when (tt-dis-rule.discnt-type <> integer('13':U) and
                                tt-dis-rule.discnt-type <> integer('998':U) )
...
if v-value-type = integer('4':U) then do:
  ...
  assign
  rs-radio-integer:sensitive  = (p-mode <> 'ПРОСМОТР':U)
...
if v-value-type = integer('7':U) then do:
  assign
  t-flag:sensitive  = no
  t-flag            = yes
  t-flag:visible  = yes
...
END.
if tt-dis-rule.discnt-type = integer('13':U) then do:
  hide
  tt-dis-rule.discnt-value
  in frame Dialog-Frame .
end.
...
do:
  enable BR-gds-rule-attr with frame Dialog-Frame .
...
if v-value-type = integer('9':U) then do:
  run gbl/d-askw.w ( input "Рекомендация"
              ,input  ("Выберите тип скидки" + chr(10)
...
IF tt-dis-rule.discnt-value:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.discnt-value
.
IF tt-dis-rule.dis-kat:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.dis-kat
.
IF tt-dis-rule.doc-qnty:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.doc-qnty
.
IF tt-dis-rule.tot-sum:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.tot-sum
.
...
tt-dis-rule.charkey_one
.
IF tt-dis-rule.charkey_two:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.charkey_two
.
IF tt-dis-rule.charkey_three:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.charkey_three
.
IF tt-dis-rule.deckey_one:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.deckey_one
.
IF tt-dis-rule.deckey_two:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.deckey_two
.
IF tt-dis-rule.deckey_three:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.deckey_three
.
...
tt-dis-rule.key#_one
.
IF tt-dis-rule.key#_two:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.key#_two
.
IF tt-dis-rule.key#_three:SENSITIVE IN FRAME Dialog-Frame THEN
ASSIGN
tt-dis-rule.key#_three
.
RUN display-hide-fields IN THIS-PROCEDURE ( v-tree, NO , 1 ).
IF v-doc-qnty-tree
AND tt-dis-rule.doc-qnty:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   0 @ tt-dis-rule.doc-qnty
   WITH FRAME Dialog-Frame.
END.
IF v-tot-sum-tree
AND tt-dis-rule.tot-sum:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   0 @ tt-dis-rule.tot-sum
   WITH FRAME Dialog-Frame.
END.
IF v-dis-kat-tree
AND tt-dis-rule.dis-kat:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
...
AND tt-dis-rule.charkey_two:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   '':U @ tt-dis-rule.charkey_two
   WITH FRAME Dialog-Frame.
END.
IF v-charkey_three-tree
AND tt-dis-rule.charkey_three:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   '':U @ tt-dis-rule.charkey_three
   WITH FRAME Dialog-Frame.
END.
IF v-deckey_one-tree
AND tt-dis-rule.deckey_one:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   '':U @ tt-dis-rule.deckey_one
   WITH FRAME Dialog-Frame.
END.
IF v-deckey_two-tree
AND tt-dis-rule.deckey_two:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   '':U @ tt-dis-rule.deckey_two
   WITH FRAME Dialog-Frame.
END.
IF v-deckey_three-tree
AND tt-dis-rule.deckey_three:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   '':U @ tt-dis-rule.deckey_three
   WITH FRAME Dialog-Frame.
END.
IF v-key#_one-tree
AND tt-dis-rule.key#_one:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   0 @ tt-dis-rule.key#_one
   WITH FRAME Dialog-Frame.
END.
IF v-key#_two-tree
AND tt-dis-rule.key#_two:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
   0 @ tt-dis-rule.key#_two
   WITH FRAME Dialog-Frame.
END.
IF v-key#_three-tree
AND tt-dis-rule.key#_three:sensitive  IN FRAME Dialog-Frame THEN DO:
   DISPLAY
...
run ref/dist-rls.w (
                input parparentproc
              ,input "b-sel,b-add"
              ,input ( if p-mode = 'ДОБАВЛЕНИЕ':U then 'dis-rule':U else ("rule-num" + chr(44) + 'ИЗМЕНЕНИЕ':U))
              ,input (if p-mode = 'ДОБАВЛЕНИЕ':U then tt-dis-rule.templ-rl-root else tt-dis-rule.rule-num)
              ,input ( if p-mode = 'ДОБАВЛЕНИЕ':U then 0 else tt-dis-rule.time-templ-rl-root)
              ,input ( if p-mode = 'ДОБАВЛЕНИЕ':U then v-pos-type else '')
              ,input-output v-sts
...
.
IF tt-dis-rule.doc-qnty:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-doc-qnty = INPUT FRAME Dialog-Frame tt-dis-rule.doc-qnty
  .
END.
IF tt-dis-rule.dis-kat:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-dis-kat = INPUT FRAME Dialog-Frame tt-dis-rule.dis-kat
  .
END.
IF tt-dis-rule.tot-sum:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-tot-sum = INPUT FRAME Dialog-Frame tt-dis-rule.tot-sum
  .
END.
IF tt-dis-rule.time-rule-num:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ...
  .
END.
IF tt-dis-rule.discnt-value:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-discnt-value = INPUT FRAME Dialog-Frame tt-dis-rule.discnt-value
  .
END.
IF tt-dis-rule.charkey_one:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ...
  .
END.
IF tt-dis-rule.charkey_two:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-charkey_two = INPUT FRAME Dialog-Frame tt-dis-rule.charkey_two
  .
END.
IF tt-dis-rule.charkey_three:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-charkey_three = INPUT FRAME Dialog-Frame tt-dis-rule.charkey_three
  .
END.
IF tt-dis-rule.deckey_one:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-deckey_one = INPUT FRAME Dialog-Frame tt-dis-rule.deckey_one
  .
END.
IF tt-dis-rule.deckey_two:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-deckey_two = INPUT FRAME Dialog-Frame tt-dis-rule.deckey_two
  .
END.
IF tt-dis-rule.deckey_three:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-deckey_three = INPUT FRAME Dialog-Frame tt-dis-rule.deckey_three
  .
END.
IF tt-dis-rule.key#_one:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-key#_one = INPUT FRAME Dialog-Frame tt-dis-rule.key#_one
  .
END.
IF tt-dis-rule.key#_two:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-key#_two = INPUT FRAME Dialog-Frame tt-dis-rule.key#_two
  .
END.
IF tt-dis-rule.key#_three:SENSITIVE IN FRAME Dialog-Frame THEN DO:
  ASSIGN
  v-key#_three = INPUT FRAME Dialog-Frame tt-dis-rule.key#_three
  .
END.
...
tt-bc-dis-rule.price-discnt = INPUT FRAME Dialog-Frame f-price-discnt
tt-bc-dis-rule.price-netto = INPUT FRAME Dialog-Frame f-price-netto
tt-bc-dis-rule.sum-brutto = INPUT FRAME Dialog-Frame f-sum-brutto
tt-bc-dis-rule.sum-discnt = INPUT FRAME Dialog-Frame f-sum-discnt
tt-bc-dis-rule.sum-netto = INPUT FRAME Dialog-Frame f-sum-netto
tt-bc-dis-rule.d-pcnt = INPUT FRAME Dialog-Frame f-d-pcnt
tt-bc-dis-rule.sale-qnty = INPUT FRAME Dialog-Frame f-sale-qnty
.
...
END.
ELSE DO:
  IF tt-dis-rule.discnt-value:VISIBLE IN FRAME Dialog-Frame THEN
  ASSIGN
  tt-dis-rule.discnt-value
  .
  IF tt-dis-rule.dis-kat:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.dis-kat
    .
  IF tt-dis-rule.doc-qnty:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.doc-qnty
    .
  IF tt-dis-rule.tot-sum:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.tot-sum
    .
  ...
  IF tt-dis-rule.key#_one:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.key#_one
    .
  IF tt-dis-rule.key#_two:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.key#_two
    .
  IF tt-dis-rule.key#_three:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.key#_three
    .
  ...
  tt-dis-rule.charkey_one
    .
  IF tt-dis-rule.charkey_two:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.charkey_two
    .
  IF tt-dis-rule.charkey_three:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.charkey_three
    .
  IF tt-dis-rule.deckey_one:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.deckey_one
    .
  IF tt-dis-rule.deckey_two:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.deckey_two
    .
  IF tt-dis-rule.deckey_three:VISIBLE  IN FRAME Dialog-Frame THEN
    ASSIGN
  tt-dis-rule.deckey_three
    .
  ASSIGN
...
IF tt-dis-rule.tot-sum:VISIBLE IN FRAME Dialog-Frame = NO   THEN DO:
                                                                               ~
CASE p-main:
...
END.
DISPLAY
f-d-pcnt WHEN (v-term-value-type <> INTEGER('1':U)) AND f-d-pcnt:VISIBLE IN FRAME Dialog-Frame
f-price-discnt when f-price-discnt:VISIBLE IN FRAME Dialog-Frame
f-price-netto when f-price-netto:VISIBLE IN FRAME Dialog-Frame
f-sale-qnty WHEN f-sale-qnty:VISIBLE IN FRAME Dialog-Frame = yes
f-sum-discnt WHEN f-sum-discnt:VISIBLE IN FRAME Dialog-Frame
f-sum-netto  WHEN f-sum-netto:VISIBLE IN FRAME Dialog-Frame
f-sum-brutto WHEN f-sum-brutto:VISIBLE IN FRAME Dialog-Frame
WITH FRAME Dialog-Frame.
display
...
assign   ii = 0   v-label0 = "Сумма"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "tot-sum":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
...
assign   ii = 0   v-label0 = "Категория"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "dis-kat":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
...
assign   ii = 0   v-label0 = "Интполе1"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "key#_one":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "Интполе2"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "key#_two":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "Интполе3"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "key#_three":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "Поле1"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "charkey_one":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "Поле2"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "charkey_two":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "Поле3"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "charkey_three":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "ДПоле1"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "deckey_one":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "ДПоле2"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "deckey_two":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
...
assign   ii = 0   v-label0 = "ДПоле3"   v-label = v-label0.   for each buf_temp-drt-prop no-lock where             buf_temp-drt-prop.templ-rl-root = p-templ-rl-root         and buf_temp-drt-prop.prop-code = "Label":U         and buf_temp-drt-prop.upper-prop-code = "deckey_three":U,       first upper_temp-drt-prop no-lock where           upper_temp-drt-prop.templ-rl-root = p-templ-rl-root       and upper_temp-drt-prop.prop-code = buf_temp-drt-prop.upper-prop-code       and upper_temp-drt-prop.upper-prop-code = "Level2_UsingFields":U:     assign     v-label = buf_temp-drt-prop.property-value.     leave.   end.
  FOR EACH buf_tt0-term_dis-rule
  BY buf_tt0-term_dis-rule.time-rule-num DESCENDING:
    ASSIGN
    ii = ii + 1
