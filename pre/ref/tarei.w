...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-tare SHARE-LOCK.
...
IF AVAILABLE tt-tare THEN
  ...
  tt-tare.tare-name when p-mode <> 'ПРОСМОТР':U
  tt-tare.tare-code when p-mode = 'ДОБАВЛЕНИЕ':U
  WITH FRAME Dialog-Frame .
  VIEW FRAME Dialog-Frame .
  IF p-mode = 'ИЗМЕНЕНИЕ':U then do:
   FRAME Dialog-Frame:title = "Изменение тары".
    DISPLAY
    tt-tare.tare-name
    tt-tare.tare-code
    WITH frame Dialog-Frame.
