...
END.
ON CHOOSE OF b-set IN FRAME Dialog-Frame
DO:
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-callee :handle
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  THEN DO:
    ...
    run  gen-key-fv in this-procedure ( input p-call-id
                                      ,output v-field-list
                                      ,output v-value-list).
    if lookup(v-field-list, "obj-type", chr(3)) > 0 then do:
      assign
      v-obj-type = entry(lookup(v-field-list, "obj-type", chr(3)), v-value-list, chr(3))
      v-obj-code = integer(entry(lookup(v-field-list, "obj-code", chr(3)), v-value-list, chr(3)) )
      .
    end.
    if v-obj-type <> ''
    ...
    if buf_rule-profile.parent-feature = integer('1':U) then do:
      ...
      and (p-mode = 'ПРОСМОТР':U
          or
          (p-mode = 'ИЗМЕНЕНИЕ':U and lookup("free", bttns) > 0))
      then do:
          ...
          entry(lookup(buf_rule-profile.profile-type, p-call-id, chr(3)), v-current-call-id, chr(3)) = 'cmb':U.
          ...
          run value(v-param-form) (
                                ...
                                ,input buf2_rule-profile.profile_id
                                ,input buf_rp-by-call.once-more
                                ,input v-current-call-id
                                ,input 0
                                ,input 0
                                ,input ?
                                ,input 0
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-callee FOR EACH call_rp-rule-param,        each CALL_tt-rule-call-param.
END PROCEDURE.
PROCEDURE MyEnable :
DEFINE VARIABLE v-ii AS INTEGER NO-UNDO.
DEFINE VARIABLE v-ch0 AS widget-handle NO-UNDO.
...
b-quit
b-chg WHEN p-mode <> 'ПРОСМОТР':U and lookup("b-chg", bttns) > 0
B-Help
BR-rcp
E-rule-name
E-param-des
...
CASE p-list-mode:
  ...
  WHEN 'rp-rule-param':U THEN DO:
      ...
      BY tt-rule-call-param.param-num.
  END.
  WHEN 'rp-rule-param':U + chr(44) + 'все':U THEN DO:
      OPEN QUERY br-rcp
      FOR EACH  X_ruledict-param NO-LOCK WHERE X_ruledict-param.entry-id = v-entry-id
          ,first X_rp-rule-param WHERE
              X_rp-rule-param.profile_id = p-profile-id
...
if p-list-mode = 'rp-rule-param':U + chr(44) + 'все':U then do:
...
CASE  v-param-data-type:
  when 'integer':U then do:
    ...
    run gbl/d-integer.w (
        ...
        + 'format=' + (if term_tt-rule-call-param.param-data-type = 'L':U
                      then "yes/no"
                      else v-format) + '\':u
        + 'fillin_row=3\':u
        + 'fillin_col=4\':u
  ...
  when 'date':U then do:
    ...
    v-value-date = term_tt-rule-call-param.param-value-date.
      run gbl/d-inpday.w
        (input ?
        ,input substitute("Изменение параметра &1", term_tt-rule-call-param.param-label)
        ,input ""
        ,input ""
...
CASE entry(1, term_tt-rule-call-param.param-2-data-type, "_"):
  ...
  when "xsd" then do:
    ...
    run gbl/file2clb.p ( input 'ПРОСМОТР':U
                        ,input ""
                        ,input ?
                        ,input term_tt-rule-call-param.param-value-character
                        ,input '':U
  ...
  then do:
    ...
    run gbl/file2clb.p ( input 'ПРОСМОТР':U
                        ,input ""
                        ,input ?
                        ,input entry(1, term_tt-rule-call-param.param-value-character, "_")
                        ,input entry(2, term_tt-rule-call-param.param-value-character, "_")
                        ,input '':U
                        ,input-output v-part-num
                        ,input v-res-type
                        ,input-output v-clob-db-num
