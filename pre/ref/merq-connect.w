  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR_GOODS :handle
      .
...
define input parameter p-search as character no-undo .
  ...
  if not available tt-gds-answer then return .
  assign
    v-is-found = false
    .
case r-search:
  ...
  OPEN QUERY BR_GOODS FOR EACH tt-gds-answer
    indexed-reposition .
end procedure.
PROCEDURE proc-save :
define variable gdsMercsubsObj  as class   gdsmercsubs.
  ...
  do:
    ...
    GuidMercsubsObj = gdsmercstrObj:getguidmercs(tt-gds-answer.GUID_).
