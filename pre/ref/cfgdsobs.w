...
assign
  v-str-recid13 = trim( string( recid( X_c-fbr-gds-obj ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry13 > 0 then do:
 ...
 glog = br-c-fbr-gds-obj :refresh( )  in frame Dialog-Frame.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
    glog = br-c-fbr-gds-obj:select-next-row () in frame Dialog-Frame.
    apply "value-changed" to br-c-fbr-gds-obj in frame Dialog-Frame.
  end.
  ...
  apply "entry" to br-c-fbr-gds-obj in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-c-fbr-gds-obj :handle
      .
...
do:
  rep-rec = recid(X_c-fbr-gds-obj). run OpenBr in this-procedure  ( input yes, input no, input '':U). reposition br-c-fbr-gds-obj to recid rep-rec no-error. rep-rec = ?.               APPLY 'ENTRY' to br-c-fbr-gds-obj. APPLY 'VALUE-CHANGED' to br-c-fbr-gds-obj.
    apply "VALUE-CHANGED" to BR-c-fbr-gds-obj.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-c-fbr-gds-obj :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
def var sort-labelBR-c-fbr-gds-obj   as character no-undo .
def var sort-clmnBR-c-fbr-gds-obj    as handle    no-undo .
def var cur-clmnBR-c-fbr-gds-obj     as handle    no-undo .
def var cur-clmn-locBR-c-fbr-gds-obj as integer   no-undo .
def var re-queryBR-c-fbr-gds-obj     as logical   initial no no-undo .
on start-search, ctrl-o of BR-c-fbr-gds-obj in frame Dialog-Frame do:
   run sort-brBR-c-fbr-gds-obj
     (input (if available X_c-fbr-gds-obj
             then recid(X_c-fbr-gds-obj)
             else ?
            )
     ).
end.
PROCEDURE sort-brBR-c-fbr-gds-obj :
  define input parameter p-recid as recid no-undo .
  if re-queryBR-c-fbr-gds-obj = no then do:
    assign
       cur-clmnBR-c-fbr-gds-obj = BR-c-fbr-gds-obj:current-column in frame Dialog-Frame
    .
    if sort-clmnBR-c-fbr-gds-obj <> ? then sort-clmnBR-c-fbr-gds-obj:column-fgcolor = 0.
    if cur-clmnBR-c-fbr-gds-obj = sort-clmnBR-c-fbr-gds-obj then do:
      assign
         sort-labelBR-c-fbr-gds-obj = ""
         sort-clmnBR-c-fbr-gds-obj = ?
      .
     end.
     else do:
       assign
         sort-labelBR-c-fbr-gds-obj = cur-clmnBR-c-fbr-gds-obj:label
         sort-clmnBR-c-fbr-gds-obj  = cur-clmnBR-c-fbr-gds-obj
         sort-clmnBR-c-fbr-gds-obj:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locBR-c-fbr-gds-obj = 1
  .
  def var column-handle as handle no-undo .
  column-handle = BR-c-fbr-gds-obj:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnBR-c-fbr-gds-obj then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locBR-c-fbr-gds-obj = cur-clmn-locBR-c-fbr-gds-obj + 1
    .
  end.
  case sort-labelBR-c-fbr-gds-obj:
        when X_c-fbr-gds-obj.corr-date:label in browse BR-c-fbr-gds-obj then DO:    assign       sort-column-name = "X_c-fbr-gds-obj.corr-date"     .     run OpenBr in this-procedure  ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure  ( input yes, input no, input '':U).
      if sort-labelBR-c-fbr-gds-obj <> "" then do:
        assign
          cur-clmnBR-c-fbr-gds-obj:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locBR-c-fbr-gds-obj = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition BR-c-fbr-gds-obj to recid p-recid no-error.
    apply "value-changed" to BR-c-fbr-gds-obj in frame Dialog-Frame.
  end.
  apply "entry" to BR-c-fbr-gds-obj in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnBR-c-fbr-gds-obj:
if cur-clmnBR-c-fbr-gds-obj = ? then do:
   run OpenBr in this-procedure  ( input yes, input no, input '':U).
end.
else do:
   assign re-queryBR-c-fbr-gds-obj = yes.
   run sort-brBR-c-fbr-gds-obj
     (input (if available X_c-fbr-gds-obj
             then recid(X_c-fbr-gds-obj)
             else ?
   ...
   assign re-queryBR-c-fbr-gds-obj = no.
end.
end.
...
CASE p-mode:
    when "one":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории АТРИБУТА ТОВАРА РЕСТОРАН: объект &1&2 товар &3",
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      assign
      parameter-3-23 =
                              "FOR EACH X_c-fbr-gds-obj"
      parameter-4-23 =
        (
          if (" X_c-fbr-gds-obj.obj-code = p-obj-code AND X_c-fbr-gds-obj.obj-type = p-obj-type AND                             X_c-fbr-gds-obj.gds-code = p-gds-code" + " " + where-phrase-23) <> ""
          then  substitute('X_c-fbr-gds-obj.obj-code = &1 AND X_c-fbr-gds-obj.obj-type = &2&3&2 AND                             X_c-fbr-gds-obj.gds-code = &4', p-obj-code, chr(34), p-obj-type, p-gds-code )  + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-23 =
          (" X_c-fbr-gds-obj.obj-code = p-obj-code AND X_c-fbr-gds-obj.obj-type = p-obj-type AND                             X_c-fbr-gds-obj.gds-code = p-gds-code" + " " + where-phrase-23 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-fbr-gds-obj:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-fbr-gds-obj FOR EACH X_c-fbr-gds-obj no-lock
      where  X_c-fbr-gds-obj.obj-code = p-obj-code AND X_c-fbr-gds-obj.obj-type = p-obj-type AND                             X_c-fbr-gds-obj.gds-code = p-gds-code
  .
      assign
        l-query-was-opened = true
      .
...
v-label-param =
  "fbr-grp-code" + chr(4) + "Внутр. код группы меню" + chr(4) + "" + chr(8)
 + "fbr-obj-code" + chr(4) + "Кухня" + chr(4) + "" + chr(8)
 + "is-cd" + chr(4) + "Отсылать на кассу" + chr(4) + "" + chr(8)
 ...
 + "is-season" + chr(4) + "Использовать сезонный коэфф." + chr(4) + "" + chr(8)
 + "is-semi-finished" + chr(4) + "Полуфабрикат" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-fbr-gds-obj:handle
                                            ,input  'fbr-gds-obj':U
                                            ,input  "fbr-grp-code,fbr-obj-code,is-cd,is-menu,is-modificator,is-null-price,is-season,is-semi-finished"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
