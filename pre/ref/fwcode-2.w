...
define buffer find_code for ub.fin-code-cel-nazn .
...
define variable rr as recid no-undo .
    if available buf_fin-code then do:
        rr = recid( buf_fin-code ).
                  run ref/fci-2.w ( 'ИЗМЕНЕНИЕ':U , input-output rr , input par-host-code ).
...
define variable rr as recid no-undo .
    if available buf_fin-code then do:
        rr = recid( buf_fin-code ).
                  run ref/fci-2.w ( 'ПРОСМОТР':U , input-output rr , input par-host-code ).
     end.
END.
...
on start-search, ctrl-o of BR-docs in frame Dialog-Frame do:
   run sort-brBR-docs
     (input (if available ub.fin-code-cel-nazn
             then recid(ub.fin-code-cel-nazn)
             else ?
            )
     ).
end.
...
if not g#log then  return .
define variable rr as recid no-undo .
            run ref/fci-2.w ( 'ДОБАВЛЕНИЕ':U , input-output rr , input par-host-code ).
  v-doc-rec = rr .
  run OpenBr in this-procedure (yes, no, '':U).
...
define buffer buf2_fin-code for ub.fin-code-cel-nazn .
