...
define variable vss-description as character no-undo init "Справочник Кодов аналитического учета".
...
define buffer find_code for ub.fin-code-an-uchet .
...
DEFINE BROWSE BR-docs
  QUERY BR-docs DISPLAY
      ...
      entry (lookup (STRING(buf_fin-code.status_ ), '0,1,50,99':U), 'тек,удал,блок,удаление':U) @ t-s  COLUMN-LABEL "Статус" FORMAT "x(6)"
...
DEFINE FRAME Dialog-Frame
     ...
     sch-code AT ROW 17.42 COL 1
     p-desc AT ROW 17.42 COL 24.13
     mark-num AT ROW 1 COL 14.88 NO-LABEL
     " ПОИСК" VIEW-AS TEXT
          SIZE 7.13 BY .71 AT ROW 16.75 COL 1.63
          BGCOLOR 3 FGCOLOR 15
     SPACE(62.48) SKIP(1.20)
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_fin-reference_update':U
    ,input  'firm':U
    ,input  par-host-code
    ,input  '':U
    ,input  0
...
define variable rr as recid no-undo .
    if available buf_fin-code then do:
        rr = recid( buf_fin-code ).
                  run ref/fci-3.w ( 'ИЗМЕНЕНИЕ':U , input-output rr , input par-host-code ).
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_fin-reference_deletion':U
    ,input  'firm':U
    ,input  par-host-code
    ,input  '':U
    ,input  0
...
if not g#log then  return .
  ...
  if available buf_fin-code then do:
        find first ub.sysconf no-lock where
          (ub.sysconf.an-uchet-code-in         = buf_fin-code.fin-code and
            ub.sysconf.host-code               = buf_fin-code.host-code) or
          (ub.sysconf.an-uchet-code-in-cash    = buf_fin-code.fin-code and
            ub.sysconf.host-code               = buf_fin-code.host-code) or
          (ub.sysconf.an-uchet-code-in-payoff  = buf_fin-code.fin-code and
            ub.sysconf.host-code               = buf_fin-code.host-code) or
          (ub.sysconf.an-uchet-code-out        = buf_fin-code.fin-code and
            ub.sysconf.host-code               = buf_fin-code.host-code) or
          (ub.sysconf.an-uchet-code-out-cash   = buf_fin-code.fin-code and
            ub.sysconf.host-code               = buf_fin-code.host-code) or
          (ub.sysconf.an-uchet-code-out-payoff = buf_fin-code.fin-code and
...
END.
ON CHOOSE OF B-firm IN FRAME Dialog-Frame
DO:
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_fin-reference_export':U
    ,input  'firm':U
    ,input  par-host-code
    ,input  '':U
    ,input  0
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_fin-reference_lookup':U
    ,input  'firm':U
    ,input  par-host-code
    ,input  '':U
    ,input  0
...
define variable rr as recid no-undo .
    if available buf_fin-code then do:
        rr = recid( buf_fin-code ).
                  run ref/fci-3.w ( 'ПРОСМОТР':U , input-output rr , input par-host-code ).
     end.
END.
...
assign
  v-str-recid16 = trim( string( recid( buf_fin-code ) , "->>>>>>>>>>>9":U ) )
...
define buffer buf_file for ub.fin-code-an-uchet .
...
run current-db in this-procedure (
    ...
    output p-ret ) .
 if p-ret = no then return.
 create ub.fin-code-an-uchet.
 assign
   ub.fin-code-an-uchet.host-code  = p-host-code
   ub.fin-code-an-uchet.fin-code   = p-fin-code
   ub.fin-code-an-uchet.code-value = p-code-value
   ub.fin-code-an-uchet.descr      = p-descr
   ub.fin-code-an-uchet.status_    = p-status_
   ub.fin-code-an-uchet.level-1    = p-level-1
   ub.fin-code-an-uchet.level-2    = p-level-2
   ub.fin-code-an-uchet.level-3    = p-level-3
  no-error .
  if error-status :error then do:
      message vss-include-info17 skip
              error-status :get-message(1)
              view-as alert-box error .
      return error .
  end.
...
DEF VAR cur-clmn-numBR-docs as INT EXTENT 7 no-undo.
...
ON ctrl-cursor-right OF BROWSE BR-docs do:
  RUN re-move-clmnBR-docs ( 2, 7).
END.
ON ctrl-cursor-left OF BROWSE BR-docs do:
  RUN re-move-clmnBR-docs (7, 2).
END.
...
PROCEDURE ch-clmnBR-docs:
  ...
  END.
  RUN re-move-clmnBR-docs (cur-clmn-loc, 2).
END PROCEDURE.
PROCEDURE mv-brw-defaultBR-docs:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlBR-docs = 2 to EXTENT(cur-clmn-numBR-docs):
...
on start-search, ctrl-o of BR-docs in frame Dialog-Frame do:
   run sort-brBR-docs
     (input (if available ub.fin-code-an-uchet
             then recid(ub.fin-code-an-uchet)
             else ?
            )
     ).
end.
PROCEDURE sort-brBR-docs :
  ...
  case sort-labelBR-docs:
        when buf_fin-code.code-value:label in browse BR-docs then DO:    assign       sort-column-name = "buf_fin-code.code-value"     .     run OpenBr(yes, no, '':U).   . END.
        when buf_fin-code.descr:label in browse BR-docs then DO:    assign       sort-column-name = "buf_fin-code.descr"     .     run OpenBr(yes, no, '':U).   . END.
        when buf_fin-code.fin-code:label in browse BR-docs then DO:    assign       sort-column-name = "buf_fin-code.fin-code"     .     run OpenBr(yes, no, '':U).   . END.
        when buf_fin-code.host-code:label in browse BR-docs then DO:    assign       sort-column-name = "buf_fin-code.host-code"     .     run OpenBr(yes, no, '':U).   . END.
        when buf_fin-code.level-1:label in browse BR-docs then DO:    assign       sort-column-name = "buf_fin-code.level-1"     .     run OpenBr(yes, no, '':U).   . END.
        when buf_fin-code.level-2:label in browse BR-docs then DO:    assign       sort-column-name = "buf_fin-code.level-2"     .     run OpenBr(yes, no, '':U).   . END.
        when buf_fin-code.level-3:label in browse BR-docs then DO:    assign       sort-column-name = "buf_fin-code.level-3"     .     run OpenBr(yes, no, '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
define variable vss-include-info22 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  assign
    buf_fin-code.code-value:read-only in browse BR-docs = true
  .
  define variable p-file-label as character no-undo .
  assign
    p-file-label =  "Коды аналитического учета"
  .
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_fin-reference_add-def':U
    ,input  'firm':U
    ,input  par-host-code
    ,input  '':U
    ,input  0
...
if not g#log then  return .
define variable rr as recid no-undo .
            run ref/fci-3.w ( 'ДОБАВЛЕНИЕ':U , input-output rr , input par-host-code ).
  v-doc-rec = rr .
  run OpenBr in this-procedure (yes, no, '':U).
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-docs FOR EACH buf_fin-code NO-LOCK.
END PROCEDURE.
...
title0 = "Справочник фин.кодов " + caps(p-file-label) + chr(32).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      assign
      parameter-3-25 =
                              "FOR EACH buf_fin-code"
      parameter-4-25 =
        (
          if (" buf_fin-code.status_ = p-sts" + " " + where-phrase-25) <> ""
          then  substitute (' buf_fin-code.status_ =  &1 ', p-sts )  + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" buf_fin-code.status_ = p-sts" + " " + where-phrase-25 = "")
      .
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-docs FOR EACH buf_fin-code
      where  buf_fin-code.status_ = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  assign
    doc-rec = recid( buf_fin-code )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf_fin-code:handle) then do:
      assign
      parameter-2-25 = (if p-find-next then "true":u else "false":u )
      parameter-4-25 =
        "where ":u +  substitute (' buf_fin-code.status_ =  &1 ', p-sts )  + " ":u + where-phrase-25 + " ":u + p-find-condition + " " + ""
      parameter-5-25 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
                          ,input rowid(buf_fin-code)
                          ,input logical(parameter-2-25)
                          ,input no-lock
                          ,input (buffer buf_fin-code:handle)
                          ,input parameter-4-25
                          ,input parameter-5-25
                          ) no-error.
      .
      ...
      assign
      parameter-2-25 = (if p-find-next then "true":u else "false":u )
      parameter-3-25 =  "FOR EACH buf_fin-code"
      parameter-4-25 =
        (
          if (" buf_fin-code.status_ = p-sts" + " " + where-phrase-25) <> ""
          then  substitute (' buf_fin-code.status_ =  &1 ', p-sts )  + " " + where-phrase-25
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          if (" buf_fin-code.host-code = par-host-code and buf_fin-code.status_ = p-sts " + " " + where-phrase-27) <> ""
          then  substitute (' buf_fin-code.host-code = &2 and  buf_fin-code.status_ =  &1 ', p-sts , par-host-code)  + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-27 =
          (" buf_fin-code.host-code = par-host-code and buf_fin-code.status_ = p-sts " + " " + where-phrase-27 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-docs FOR EACH buf_fin-code
      where  buf_fin-code.host-code = par-host-code and buf_fin-code.status_ = p-sts
       USE-INDEX val
      indexed-reposition
  .
      assign
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf_fin-code:handle) then do:
      ...
      parameter-4-27 =
        "where ":u +  substitute (' buf_fin-code.host-code = &2 and  buf_fin-code.status_ =  &1 ', p-sts , par-host-code)  + " ":u + where-phrase-27 + " ":u + p-find-condition + " " + ""
      parameter-5-27 = " USE-INDEX val "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH buf_fin-code"
      parameter-4-27 =
        (
          if (" buf_fin-code.host-code = par-host-code and buf_fin-code.status_ = p-sts " + " " + where-phrase-27) <> ""
          then  substitute (' buf_fin-code.host-code = &2 and  buf_fin-code.status_ =  &1 ', p-sts , par-host-code)  + " " + where-phrase-27
...
if p-open-query then do:
  ...
  :
    ...
    .
      ...
      assign
      parameter-2-29 = (if p-find-next then "true":u else "false":u )
      parameter-3-29 =  "FOR EACH buf_fin-code"
      parameter-4-29 =
        (
          if (" TRUE " + " " + where-phrase-29) <> ""
          then  'TRUE'  + " " + where-phrase-29
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if (" buf_fin-code.host-code = par-host-code " + " " + where-phrase-31) <> ""
          then  substitute (' buf_fin-code.host-code = &1 ', par-host-code)  + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          (" buf_fin-code.host-code = par-host-code " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-docs FOR EACH buf_fin-code
      where  buf_fin-code.host-code = par-host-code
       USE-INDEX val
      indexed-reposition
  .
      assign
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf_fin-code:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute (' buf_fin-code.host-code = &1 ', par-host-code)  + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = " USE-INDEX val "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
      ...
      assign
      parameter-2-31 = (if p-find-next then "true":u else "false":u )
      parameter-3-31 =  "FOR EACH buf_fin-code"
      parameter-4-31 =
        (
          if (" buf_fin-code.host-code = par-host-code " + " " + where-phrase-31) <> ""
          then  substitute (' buf_fin-code.host-code = &1 ', par-host-code)  + " " + where-phrase-31
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_fin-reference_print':U
    ,input  'firm':U
    ,input  par-host-code
    ,input  '':U
    ,input  0
...
define buffer buf2_fin-code for ub.fin-code-an-uchet .
