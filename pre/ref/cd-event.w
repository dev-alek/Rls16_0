...
DO:
   ...
   THEN DO:
      ...
      THEN DO:
         run enable_UI.
         run post_enable_UI.
         REPOSITION BROWSE-2 to rowid rowid( buf_buf_cd-events ) no-error.
      END.
   END.
END.
...
DO:
  MESSAGE "Help for File: c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\ref\cd-event.w" VIEW-AS ALERT-BOX INFORMATION.
END.
...
assign
  v-str-recid2 = trim( string( recid( buf_cd-events ) , "->>>>>>>>>>>9":U ) )
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   THEN DO:
      ...
      ASSIGN
         frame Dialog-Frame:TITLE = SUBSTITUTE("&1, версия &2", frame Dialog-Frame:TITLE, v-version ).
      .
   END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-2 FOR EACH buf_cd-events NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
