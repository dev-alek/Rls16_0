...
assign
  v-str-recid10 = trim( string( recid( X_c-stop-list ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-c-stop-list in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-c-stop-list in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-c-stop-list :handle
      .
...
do:
  BR-c-stop-list :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
do:
   v-doc-rec = ?. if available X_c-stop-list then v-doc-rec = recid(X_c-stop-list).                run openbr in this-procedure ( input yes, input no, input '':U) no-error.                REPOSITION br-c-stop-list to recid v-doc-rec No-ERROR.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-c-stop-list FOR EACH X_c-stop-list NO-LOCK WHERE        X_c-stop-list.classif-type = 'dis-card':U      INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ENABLE
b-quit
...
case sort-column-name :
 ...
 CASE p-list-mode:
   WHEN 'все':U THEN DO:
    ASSIGN
    FRAME Dialog-Frame:TITLE = substitute("СТОПЛИСТЫ ПО ДК")
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      assign
      parameter-3-23 =
                              "FOR EACH X_c-stop-list"
      parameter-4-23 =
        (
          if (" X_c-stop-list.classif-type = 'dis-card':U                           " + " " + where-phrase-23) <> ""
          then  substitute('X_c-stop-list.classif-type = &1&2&1', chr(34), 'dis-card':U)  + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + "")
      parameter-6-23 = if sort-phrase-23 = ''
        ...
        " " + "  " +
          " " + sort-column-phrase +
        " " + " by X_c-stop-list.stop-list-code descending"
        )
                           else
        (
        " " + "  " +
    ...
    :
      assign
        l-disable-where-23 =
          (" X_c-stop-list.classif-type = 'dis-card':U                           " + " " + where-phrase-23 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-stop-list:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-stop-list FOR EACH X_c-stop-list
      where  X_c-stop-list.classif-type = 'dis-card':U
       by X_c-stop-list.stop-list-code descending
      indexed-reposition
  .
      assign
  ...
  assign
    v-doc-rec = recid( X_c-stop-list )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-c-stop-list:handle:get-buffer-handle(1) = (buffer X_c-stop-list:handle) then do:
      assign
      parameter-2-23 = (if p-find-next then "true":u else "false":u )
      parameter-4-23 =
        "where ":u +  substitute('X_c-stop-list.classif-type = &1&2&1', chr(34), 'dis-card':U)  + " ":u + where-phrase-23 + " ":u + p-find-condition + " " + ""
      parameter-5-23 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-c-stop-list:handle
                          ,input rowid(X_c-stop-list)
                          ,input logical(parameter-2-23)
                          ,input no-lock
                          ,input (buffer X_c-stop-list:handle)
                          ,input parameter-4-23
                          ,input parameter-5-23
                          ) no-error.
      .
      ...
      assign
      parameter-2-23 = (if p-find-next then "true":u else "false":u )
      parameter-3-23 =  "FOR EACH X_c-stop-list"
      parameter-4-23 =
        (
          if (" X_c-stop-list.classif-type = 'dis-card':U                           " + " " + where-phrase-23) <> ""
          then  substitute('X_c-stop-list.classif-type = &1&2&1', chr(34), 'dis-card':U)  + " " + where-phrase-23
  ...
  run waitfram-hide in this-procedure .
   ...
   when "one" then do:
    ASSIGN
    FRAME Dialog-Frame:TITLE = substitute("ИСТОРИЯ СТОПЛИСТА &1", p-stop-list-code)
      filter-label = filter-label0
      filter-point = filter-point0
      .
      if sort-column-name = '':u then do:
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_c-stop-list.classif-type = 'dis-card':U                          and X_c-stop-list.stop-list-code = p-stop-list-code                           " + " " + where-phrase-25) <> ""
          then  substitute('X_c-stop-list.classif-type = &1&2&1                          and X_c-stop-list.stop-list-code = &1&3&1 ', chr(34), 'dis-card':U, p-stop-list-code)  + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_c-stop-list.classif-type = 'dis-card':U                          and X_c-stop-list.stop-list-code = p-stop-list-code                           " + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-stop-list:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-c-stop-list FOR EACH X_c-stop-list
      where  X_c-stop-list.classif-type = 'dis-card':U                          and X_c-stop-list.stop-list-code = p-stop-list-code
       by X_c-stop-list.stop-list-code descending
      indexed-reposition
  .
      assign
  ...
  :
    if query br-c-stop-list:handle:get-buffer-handle(1) = (buffer X_c-stop-list:handle) then do:
      ...
      parameter-4-25 =
        "where ":u +  substitute('X_c-stop-list.classif-type = &1&2&1                          and X_c-stop-list.stop-list-code = &1&3&1 ', chr(34), 'dis-card':U, p-stop-list-code)  + " ":u + where-phrase-25 + " ":u + p-find-condition + " " + ""
      parameter-5-25 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-25 = (if p-find-next then "true":u else "false":u )
      parameter-3-25 =  "FOR EACH X_c-stop-list"
      parameter-4-25 =
        (
          if (" X_c-stop-list.classif-type = 'dis-card':U                          and X_c-stop-list.stop-list-code = p-stop-list-code                           " + " " + where-phrase-25) <> ""
          then  substitute('X_c-stop-list.classif-type = &1&2&1                          and X_c-stop-list.stop-list-code = &1&3&1 ', chr(34), 'dis-card':U, p-stop-list-code)  + " " + where-phrase-25
