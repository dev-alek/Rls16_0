...
define variable loc#log as logical no-undo .
  if available X_c-place-io then do:
      if can-do( p-rid-list, string( recid( X_c-place-io ) ) ) then do:
          p-rid-list = replace( p-rid-list, chr(44) + string( recid( X_c-place-io ) ), "") .
          p-rid-list = replace( p-rid-list, string( recid( X_c-place-io ) ) + chr(44), "") .
          p-rid-list = replace( p-rid-list, string( recid( X_c-place-io ) ), "") .
      end.
      else
      p-rid-list = p-rid-list + ( if p-rid-list = "" then "" else chr(44) ) + string( recid( X_c-place-io ) ) .
      loc#log = br-c-place-io:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
          loc#log = br-c-place-io:select-next-row ().
          apply "VALUE-CHANGED" to br-c-place-io in frame Dialog-Frame.
      end.
  ...
  apply "entry" to br-c-place-io in frame Dialog-Frame.
END.
...
DO:
  ...
  if available X_c-place-io then do:
    assign
      v-label-param =
        "place-io-code" + chr(4) + "Номер" + chr(4) + "" + chr(8)
      + "place-io-name" + chr(4) + "Наименование" + chr(4) + "" + chr(8)
      + "place-io-type" + chr(4) + "Тип" + chr(4) + "" + chr(8)
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-place-io :handle
      .
...
def var sort-labelbr-c-place-io   as character no-undo .
def var sort-clmnbr-c-place-io    as handle    no-undo .
def var cur-clmnbr-c-place-io     as handle    no-undo .
def var cur-clmn-locbr-c-place-io as integer   no-undo .
def var re-querybr-c-place-io     as logical   initial no no-undo .
on start-search, ctrl-o of br-c-place-io in frame Dialog-Frame do:
   run sort-brbr-c-place-io
     (input (if available X_c-place-io
             then recid(X_c-place-io)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-c-place-io :
  define input parameter p-recid as recid no-undo .
  if re-querybr-c-place-io = no then do:
    assign
       cur-clmnbr-c-place-io = br-c-place-io:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-c-place-io <> ? then sort-clmnbr-c-place-io:column-fgcolor = 0.
    if cur-clmnbr-c-place-io = sort-clmnbr-c-place-io then do:
      assign
         sort-labelbr-c-place-io = ""
         sort-clmnbr-c-place-io = ?
      .
     end.
     else do:
       assign
         sort-labelbr-c-place-io = cur-clmnbr-c-place-io:label
         sort-clmnbr-c-place-io  = cur-clmnbr-c-place-io
         sort-clmnbr-c-place-io:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-c-place-io = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-c-place-io:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-c-place-io then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-c-place-io = cur-clmn-locbr-c-place-io + 1
    .
  end.
  case sort-labelbr-c-place-io:
        when X_c-place-io.place-io-code:label in browse br-c-place-io then DO:    assign       sort-column-name = "X_c-place-io.place-io-code"     .     run OpenBr(yes, no, no).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr(yes, no, no).
      if sort-labelbr-c-place-io <> "" then do:
        assign
          cur-clmnbr-c-place-io:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-c-place-io = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-c-place-io to recid p-recid no-error.
    apply "value-changed" to br-c-place-io in frame Dialog-Frame.
  end.
  apply "entry" to br-c-place-io in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-c-place-io:
if cur-clmnbr-c-place-io = ? then do:
   run OpenBr(yes, no, no).
end.
else do:
   assign re-querybr-c-place-io = yes.
   run sort-brbr-c-place-io
     (input (if available X_c-place-io
             then recid(X_c-place-io)
             else ?
   ...
   assign re-querybr-c-place-io = no.
end.
end.
MAIN-BLOCK:
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-13 as character no-undo .
      assign
      parameter-3-13 =
                              "FOR EACH X_c-place-io"
      parameter-4-13 =
        (
          if (" X_c-place-io.obj-type = p-obj-type AND X_c-place-io.obj-code = p-obj-code  AND X_c-place-io.place-io-code  = p-place-io-code " + " " + where-phrase-13) <> ""
          then  substitute(' X_c-place-io.obj-type = &4&1&4 AND X_c-place-io.obj-code  = &2 AND X_c-place-io.place-io-code  = &3 ', p-obj-type, p-obj-code, p-place-io-code, chr(34))  + " " + where-phrase-13
          else "true"
        )
      parameter-5-13 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-13 =
          (" X_c-place-io.obj-type = p-obj-type AND X_c-place-io.obj-code = p-obj-code  AND X_c-place-io.place-io-code  = p-place-io-code " + " " + where-phrase-13 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-c-place-io:handle
  ...
  if l-filter-open-13 = false then do:
    OPEN QUERY br-c-place-io FOR EACH X_c-place-io
      where  X_c-place-io.obj-type = p-obj-type AND X_c-place-io.obj-code = p-obj-code  AND X_c-place-io.place-io-code  = p-place-io-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  assign
    v-doc-rec = recid( X_c-place-io )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if QUERY br-c-place-io:handle:get-buffer-handle(1) = (buffer X_c-place-io:handle) then do:
      assign
      parameter-2-13 = (if p-find-next then "true":u else "false":u )
      parameter-4-13 =
        "where ":u +  substitute(' X_c-place-io.obj-type = &4&1&4 AND X_c-place-io.obj-code  = &2 AND X_c-place-io.place-io-code  = &3 ', p-obj-type, p-obj-code, p-place-io-code, chr(34))  + " ":u + where-phrase-13 + " ":u + p-find-condition + " " + ""
      parameter-5-13 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input QUERY br-c-place-io:handle
                          ,input rowid(X_c-place-io)
      ...
      assign
      parameter-2-13 = (if p-find-next then "true":u else "false":u )
      parameter-3-13 =  "FOR EACH X_c-place-io"
      parameter-4-13 =
        (
          if (" X_c-place-io.obj-type = p-obj-type AND X_c-place-io.obj-code = p-obj-code  AND X_c-place-io.place-io-code  = p-place-io-code " + " " + where-phrase-13) <> ""
          then  substitute(' X_c-place-io.obj-type = &4&1&4 AND X_c-place-io.obj-code  = &2 AND X_c-place-io.place-io-code  = &3 ', p-obj-type, p-obj-code, p-place-io-code, chr(34))  + " " + where-phrase-13
