...
define variable loc#log as logical no-undo.
define variable loc-doc-rec as recid no-undo .
if not available x-abcxyz-analysis then return no-apply.
define buffer bb_abc-analysis for ub.abc-analysis.
find first bb_abc-analysis no-lock where
           bb_abc-analysis.abc-id = x-abcxyz-analysis.abc-id and
           bb_abc-analysis.db-num = x-abcxyz-analysis.db-num no-error .
           ASSIGN
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
   ...
   apply "entry" to BROWSE-ABC in frame Dialog-Frame.
END.
ON CHOOSE OF B-lookup-2 IN FRAME Dialog-Frame
DO:
define variable loc#log as logical no-undo.
define variable loc-doc-rec as recid no-undo .
if not available x-abcxyz-analysis then return no-apply.
...
find first bb_xyz-analysis no-lock where
           bb_xyz-analysis.xyz-id = x-abcxyz-analysis.xyz-id and
           bb_xyz-analysis.db-num = x-abcxyz-analysis.db-num no-error .
assign
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    if not loc#log then return no-apply .
   run ref/xyzanali.w ( INPUT parParentProc ,INPUT  'ПРОСМОТР':U ,INPUT bb_xyz-analysis.xyz-id , INPUT bb_xyz-analysis.db-num , input-output loc-doc-rec ) .
   apply "entry" to BROWSE-ABC in frame Dialog-Frame.
END.
ON CHOOSE OF B-lookup-3 IN FRAME Dialog-Frame
DO:
define variable loc#log as logical no-undo.
define variable loc-doc-rec as recid no-undo .
if not available x-abcxyz-analysis then return no-apply.
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
   ...
   if not  loc#log then return no-apply .
   run ref/abcxyzi.w ( INPUT parParentProc ,INPUT x-abcxyz-analysis.abcx-id , INPUT x-abcxyz-analysis.db-num ) .
   apply "entry" to BROWSE-ABC in frame Dialog-Frame.
END.
...
assign
  v-str-recid18 = trim( string( recid( x-abcxyz-analysis ) , "->>>>>>>>>>>9":U ) )
  v-num-entry18 = lookup( v-str-recid18 , p-rid-list )
.
if v-num-entry18 > 0 then do:
  assign
    entry( v-num-entry18, p-rid-list ) = "":U
    p-rid-list = trim( replace( p-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    p-rid-list = p-rid-list + ( if p-rid-list = "":U then "":U else chr(44) ) + v-str-recid18
  .
end.
...
DO:
   IF  p-rid-list = "" THEN DO:
      IF AVAILABLE x-abcxyz-analysis THEN p-rid-list = string(RECID(x-abcxyz-analysis)).
  END.
END.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run usrfulnm in g#library
  (input  x-abcxyz-analysis.abcx-who-create
  ,output v-user-name
  )  .
     display v-user-name  x-abcxyz-analysis.abcx-name with frame Dialog-Frame .
  end.
END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-ABC FOR EACH x-abcxyz-analysis NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE my_enable :
...
if p-open-query then do:
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY BROWSE-Abc FOR EACH x-abcxyz-analysis
      where  true
      indexed-reposition
  .
      assign
  ...
  assign
    doc-rec = recid( x-abcxyz-analysis )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query BROWSE-ABC:handle:get-buffer-handle(1) = (buffer x-abcxyz-analysis:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input query BROWSE-ABC:handle
                          ,input rowid(x-abcxyz-analysis)
                          ,input logical(parameter-2-27)
                          ,input no-lock
                          ,input (buffer x-abcxyz-analysis:handle)
                          ,input parameter-4-27
                          ,input parameter-5-27
                          ) no-error.
      .
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH x-abcxyz-analysis"
      parameter-4-27 =
        (
          if (" true   " + " " + where-phrase-27) <> ""
          then " true   " + " " + where-phrase-27
