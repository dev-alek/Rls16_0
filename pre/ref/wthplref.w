...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_wth-place-reference_work':U
    ,input  'object':U
    ,input  parhost-code
    ,input  parobj-type
    ,input  parobj-code
...
if rep-rec <> ? then do:
  ...
  apply "entry" to br-wthpl in frame Dialog-Frame.
end.
else do:
  apply "entry" to br-wthpl in frame Dialog-Frame.
  return no-apply.
...
assign
  v-str-recid16 = trim( string( recid( X_wth-place ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry16 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
            glog = br-wthpl:select-next-row ().
            apply "iteration-changed" to br-wthpl in frame Dialog-Frame.
        end.
...
END.
ON CHOOSE OF B-wthparts IN FRAME Dialog-Frame
DO:
if not available X_wth-place then do:
  message "Неправильно выбрана строка.".
  return no-apply.
...
END.
ON CHOOSE OF B-wthpobj IN FRAME Dialog-Frame
DO:
define variable v-rid-list as character no-undo .
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-wthpl :handle
      .
...
do:
  br-wthpl :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info29 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
   v-doc-rec = ?. if available X_wth-place then assign v-doc-rec = recid( X_wth-place). run openbr in this-procedure ( input yes, input no, input '':U).                reposition br-wthpl to recid v-doc-rec no-error. APPLY 'ENTRY' to br-wthpl. APPLY 'VALUe-CHANGED' to br-wthpl.
    apply "VALUE-CHANGED" to BR-wthpl.
end.
MAIN-BLOCK:
...
CASE p-list-mode:
    when 'все':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Места хранения МЦ "
...
run gbl/flt-get.p
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-wthpl FOR EACH X_wth-place
      where  TRUE
  .
      assign
        l-query-was-opened = true
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_wth-place.obj-type = parobj-type AND X_wth-place.obj-code = parobj-code " + " " + where-phrase-34) <> ""
          then  substitute('X_wth-place.obj-type = &1&2&1 AND X_wth-place.obj-code = &3 ', chr(34), parobj-type, parobj-code) + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_wth-place.obj-type = parobj-type AND X_wth-place.obj-code = parobj-code " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-wthpl:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-wthpl FOR EACH X_wth-place
      where  X_wth-place.obj-type = parobj-type AND X_wth-place.obj-code = parobj-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when 'фирма':U then do:
       find first buf_clients no-lock where
        ...
        ASSIGN frame Dialog-Frame:TITLE = "Места хранения МЦ " + (if available buf_clients
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if (" X_wth-place.host-code = parhost-code " + " " + where-phrase-36) <> ""
          then  substitute('X_wth-place.host-code = &1', parhost-code ) + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          (" X_wth-place.host-code = parhost-code " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-wthpl:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-wthpl FOR EACH X_wth-place
      where  X_wth-place.host-code = parhost-code
  .
      assign
        l-query-was-opened = true
      .
