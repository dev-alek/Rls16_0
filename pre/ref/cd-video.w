...
DO:
   ...
   THEN DO:
      ...
      THEN DO:
         run enable_UI.
         run post_enable_UI.
         REPOSITION br-link to rowid rowid( buf_cd-video-link ) no-error.
      END.
   END.
END.
...
DO:
  MESSAGE "Help for File: c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\ref\cd-video.w" VIEW-AS ALERT-BOX INFORMATION.
END.
...
assign
  v-str-recid2 = trim( string( recid( ub.cd-video-link ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry2 > 0 then do:
  ...
  .
   ...
   v-ok = br-link:select-next-row ().
   v-ok = br-link:refresh( )  in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-link :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-link FOR EACH ub.cd-video-link NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
