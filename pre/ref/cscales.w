...
DEFINE BROWSE BR-c-scales
  QUERY BR-c-scales NO-LOCK DISPLAY
      ...
      entry (lookup (X_c-scales.subject, 'scales,scales-grp,scales-gds,scales-attr':U), 'Весы,Группы товаров на весах,Товар на весах,Атрибут весов':U) COLUMN-LABEL "Предмет изменений" FORMAT "X(20)":U
...
assign
  v-str-recid13 = trim( string( recid( X_c-scales ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry13 > 0 then do:
 ...
 glog = br-c-scales :refresh( )  in frame Dialog-Frame.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
    glog = br-c-scales:select-next-row () in frame Dialog-Frame.
    apply "value-changed" to br-c-scales in frame Dialog-Frame.
  end.
  ...
  apply "entry" to br-c-scales in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-scales :handle
      .
...
do:
  v-doc-rec = recid(X_c-scales). run openbr in this-procedure ( INPUT yes, input no, input '':U). reposition br-c-scales to recid v-doc-rec no-error.
    apply "VALUE-CHANGED" to BR-c-scales.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-c-scales :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
def var sort-labelbr-c-scales   as character no-undo .
def var sort-clmnbr-c-scales    as handle    no-undo .
def var cur-clmnbr-c-scales     as handle    no-undo .
def var cur-clmn-locbr-c-scales as integer   no-undo .
def var re-querybr-c-scales     as logical   initial no no-undo .
on start-search, ctrl-o of br-c-scales in frame Dialog-Frame do:
   run sort-brbr-c-scales
     (input (if available X_c-scales
             then recid(X_c-scales)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-c-scales :
  define input parameter p-recid as recid no-undo .
  if re-querybr-c-scales = no then do:
    assign
       cur-clmnbr-c-scales = br-c-scales:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-c-scales <> ? then sort-clmnbr-c-scales:column-fgcolor = 0.
    if cur-clmnbr-c-scales = sort-clmnbr-c-scales then do:
      assign
         sort-labelbr-c-scales = ""
         sort-clmnbr-c-scales = ?
      .
     end.
     else do:
       assign
         sort-labelbr-c-scales = cur-clmnbr-c-scales:label
         sort-clmnbr-c-scales  = cur-clmnbr-c-scales
         sort-clmnbr-c-scales:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-c-scales = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-c-scales:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-c-scales then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-c-scales = cur-clmn-locbr-c-scales + 1
    .
  end.
  case sort-labelbr-c-scales:
        when X_c-scales.corr-date:label in browse br-c-scales then DO:    assign       sort-column-name = "X_c-scales.corr-date"     .     run OpenBr in this-procedure ( INPUT yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( INPUT yes, input no, input '':U).
      if sort-labelbr-c-scales <> "" then do:
        assign
          cur-clmnbr-c-scales:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-c-scales = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-c-scales to recid p-recid no-error.
    apply "value-changed" to br-c-scales in frame Dialog-Frame.
  end.
  apply "entry" to br-c-scales in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-c-scales:
if cur-clmnbr-c-scales = ? then do:
   run OpenBr in this-procedure ( INPUT yes, input no, input '':U).
end.
else do:
   assign re-querybr-c-scales = yes.
   run sort-brbr-c-scales
     (input (if available X_c-scales
             then recid(X_c-scales)
             else ?
...
CASE parref-mode:
    when 'все':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Справочник истории весов"
...
run gbl/flt-get.p
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-scales:handle
  ...
  if l-filter-open-21 = false then do:
    OPEN QUERY br-c-scales FOR EACH X_c-scales
      where  TRUE
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when "one":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории весов: БД &1 весы N&2"
                                                       ,pardb-num
                                                       ,parscales-num)
        filter-point = filter-point0 + parref-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      ...
      parameter-4-23 =
        (
          if (" X_c-scales.db-num = pardb-num AND X_c-scales.scales-num = parscales-num" + " " + where-phrase-23) <> ""
          then  substitute('X_c-scales.db-num = &1 AND X_c-scales.scales-num = &2', pardb-num, parscales-num) + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-23 =
          (" X_c-scales.db-num = pardb-num AND X_c-scales.scales-num = parscales-num" + " " + where-phrase-23 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-scales:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-scales FOR EACH X_c-scales
      where  X_c-scales.db-num = pardb-num AND X_c-scales.scales-num = parscales-num
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "one-subject":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории весов: БД &1 весы N&2 &3"
                                                       , pardb-num
                                                       , parscales-num
                                                       , p-subject
                                                       )
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_c-scales.db-num = pardb-num AND X_c-scales.scales-num = parscales-num                          AND X_c-scales.subject = p-subject " + " " + where-phrase-25) <> ""
          then  substitute('X_c-scales.db-num = &1 AND X_c-scales.scales-num = &2                          AND X_c-scales.subject = &3&4&3 ', pardb-num, parscales-num, chr(34), p-subject) + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_c-scales.db-num = pardb-num AND X_c-scales.scales-num = parscales-num                          AND X_c-scales.subject = p-subject " + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-scales:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-c-scales FOR EACH X_c-scales
      where  X_c-scales.db-num = pardb-num AND X_c-scales.scales-num = parscales-num                          AND X_c-scales.subject = p-subject
  .
      assign
        l-query-was-opened = true
      .
