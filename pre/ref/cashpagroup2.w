...
do:
   ...
   run ref/cashparamuk.w (
                            input parparentproc
                          , input 'ДОБАВЛЕНИЕ':U
                          , input iparent + chr(4) + icode + chr(4)
                          , input-output vRec).
   if vRec <> ? then  do:
         OPEN QUERY cashpg FOR EACH buf-code-func       WHERE buf-code-func.parent = iparent + chr(4) + icode NO-LOCK,       EACH  buf-code-param WHERE buf-code-param.parent =  buf-code-func.parent + chr(4) + buf-code-func.code NO-LOCK by getKeyName(buf-code-func.code) + " (" + buf-code-func.code + ")" INDEXED-REPOSITION.
...
or return of b-upd in frame f-c-p
do:
   if iMode eq 'ИЗМЕНЕНИЕ':U and v-db-num = 0
      and available buf-code-param
   then
      apply "choose" to b-upd in frame f-c-p.
end.
on ENTER of mSearch in frame f-c-p
do:
      define buffer buf-code-search1 for ub.code.
   ...
   block-code:
   for each buf-code-search1 where buf-code-search1.parent = iparent + chr(4) + icode no-lock
   ,first buf-code-search where buf-code-search.parent =  buf-code-search1.parent + chr(4) + buf-code-search1.code
   no-lock:
      if               buf-code-search1.code  begins msearch
         or getKeyName(buf-code-search1.code) begins msearch
      then do:
         vRowid1 = rowid(buf-code-search1).
         leave block-code.
      end.
            define buffer buf-code-search2 for ub.code.
      for each buf-code-search2 where buf-code-search2.parent =  buf-code-search1.parent + chr(4) + buf-code-search1.code
                                 and buf-code-search2.code begins msearch
      no-lock:
         vRowid1 = rowid(buf-code-search1).
         vRowid2 = rowid(buf-code-search2).
         leave block-code.
      end.
   end.
      if vRowid1 eq ?
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse cashpg :handle
      .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  ...
  wait-for go of frame f-c-p focus cashpg.
  if imode eq 'ВЫБОР':U
  then
...
PROCEDURE enable_UI :
  enable
    ...
    b-add
    when iMode eq 'ИЗМЕНЕНИЕ':U and v-db-num = 0
    b-del
    when iMode eq 'ИЗМЕНЕНИЕ':U and v-db-num = 0
    b-upd
    when iMode eq 'ИЗМЕНЕНИЕ':U and v-db-num = 0
 ...
 b-hist:POPUP-MENU in frame f-c-p = menu POPUP-MENU-b-hist:HANDLE.
   b-hist:MENU-MOUSE = 1.
  fselect                :visible in browse cashpg = imode eq 'ВЫБОР':U.
  OPEN QUERY cashpg FOR EACH buf-code-func       WHERE buf-code-func.parent = iparent + chr(4) + icode NO-LOCK,       EACH  buf-code-param WHERE buf-code-param.parent =  buf-code-func.parent + chr(4) + buf-code-func.code NO-LOCK by getKeyName(buf-code-func.code) + " (" + buf-code-func.code + ")" INDEXED-REPOSITION.
end procedure.
FUNCTION getKeytype RETURNS CHARACTER
   ( iCode as char):
