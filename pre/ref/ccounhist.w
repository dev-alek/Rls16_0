...
DEFINE BUFFER X_c-obj-hist FOR c-counter.
DEFINE BUFFER find_c-obj-hist FOR c-counter.
...
define input  parameter p-db-num like c-counter.corr-user-db-num no-undo .
...
define input parameter p-next as logical no-undo.
define input parameter p-user like c-counter.corr-user-name no-undo.
assign
sch-corr-date = ?.
display
0 @ sch-db-num
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if ("               X_c-obj-hist.db             = idb           and X_c-obj-hist.file-name      = ifile-name           and X_c-obj-hist.key            = ikey           and X_c-obj-hist.code           = icode  " + " " + where-phrase-31) <> ""
          then  substitute('  X_c-obj-hist.db             = &1                                        and X_c-obj-hist.file-name      = &2                                        and X_c-obj-hist.key            = &3                                        and X_c-obj-hist.code           = &4  ', idb, ifile-name, ikey, icode)   + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          ("               X_c-obj-hist.db             = idb           and X_c-obj-hist.file-name      = ifile-name           and X_c-obj-hist.key            = ikey           and X_c-obj-hist.code           = icode  " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-obj-hist:handle
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-obj-hist FOR EACH X_c-obj-hist
      where                X_c-obj-hist.db             = idb           and X_c-obj-hist.file-name      = ifile-name           and X_c-obj-hist.key            = ikey           and X_c-obj-hist.code           = icode
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-obj-hist:handle:get-buffer-handle(1) = (buffer X_c-obj-hist:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute('  X_c-obj-hist.db             = &1                                        and X_c-obj-hist.file-name      = &2                                        and X_c-obj-hist.key            = &3                                        and X_c-obj-hist.code           = &4  ', idb, ifile-name, ikey, icode)   + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = " "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
...
define variable v-label-param as character no-undo .
  v-label-param =
   "name"         + chr(4) + "Наименование счетчика " + chr(4) + "" + chr(8)
 + "CountValue"   + chr(4) + "Наименование"           + chr(4) + "" + chr(8)
 + "Rule"         + chr(4) + "Правила"                + chr(4) + "" + chr(8)
 + "sysInfo"      + chr(4) + "Системная информация"   + chr(4) + "" .
  run proc-full-temp-changes in this-procedure (
                                               input X_c-obj-hist.action = integer('1':U)
                                              ,input X_c-obj-hist.action = integer('99':U)
                                              ,input  buffer X_c-obj-hist:handle
                                              ,input  'counter':U
                                              ,input  "name,CountValue,Rule,sysInfo"
                                              ,input  v-label-param).
end.
end procedure.
