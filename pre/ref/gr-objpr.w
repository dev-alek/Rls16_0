...
define variable v-rec-id as recid no-undo .
  run ref/gr-ogr.w (input parparentproc,input 'ДОБАВЛЕНИЕ':U , input-output v-rec-id) .
  OPEN QUERY BROWSE-1grp FOR EACH buf_grp-obj-price WHERE     ( r-status = 2 OR buf_grp-obj-price.stts =  r-status ) AND     ( g-find = NO OR       LOOKUP (string(buf_grp-obj-price.gop-id) + ";"  + string(buf_grp-obj-price.gop-db-num)  ,        tt-grp-obj ) > 0 )                .
  reposition BROWSE-1grp to recid v-rec-id no-error .
  OPEN QUERY BROWSE-2db FOR EACH buf_db-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_db-grp-obj-price.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.
  OPEN QUERY BROWSE-3frm FOR EACH buf_host-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_host-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST host-name NO-LOCK WHERE       host-name.obj-code = buf_host-grp-obj-price.host-code AND       host-name.obj-type = 'орг':U     INDEXED-REPOSITION.
  OPEN QUERY BROWSE-4obj FOR EACH buf_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_obj-grp-obj-price.stts =  r-status )       NO-LOCK,            FIRST buf_obj-name NO-LOCK WHERE       buf_obj-name.obj-code = buf_obj-grp-obj-price.obj-code AND       buf_obj-name.obj-type = buf_obj-grp-obj-price.obj-type     INDEXED-REPOSITION.
  OPEN QUERY BROWSE-5all FOR EACH x_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR x_obj-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST buf_clients NO-LOCK WHERE       buf_clients.obj-code = x_obj-grp-obj-price.obj-code AND       buf_clients.obj-type = x_obj-grp-obj-price.obj-type        .
END.
...
END.
ON CHOOSE OF B-add-frm IN FRAME Dialog-Frame
DO:
   define variable g#log as logical   no-undo .
...
DO:
  g-find = false .
  F-obj = " "  .
  display F-obj  with frame Dialog-Frame .
  OPEN QUERY BROWSE-1grp FOR EACH buf_grp-obj-price WHERE     ( r-status = 2 OR buf_grp-obj-price.stts =  r-status ) AND     ( g-find = NO OR       LOOKUP (string(buf_grp-obj-price.gop-id) + ";"  + string(buf_grp-obj-price.gop-db-num)  ,        tt-grp-obj ) > 0 )                .
  OPEN QUERY BROWSE-2db FOR EACH buf_db-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_db-grp-obj-price.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.
  OPEN QUERY BROWSE-3frm FOR EACH buf_host-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_host-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST host-name NO-LOCK WHERE       host-name.obj-code = buf_host-grp-obj-price.host-code AND       host-name.obj-type = 'орг':U     INDEXED-REPOSITION.
  OPEN QUERY BROWSE-4obj FOR EACH buf_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_obj-grp-obj-price.stts =  r-status )       NO-LOCK,            FIRST buf_obj-name NO-LOCK WHERE       buf_obj-name.obj-code = buf_obj-grp-obj-price.obj-code AND       buf_obj-name.obj-type = buf_obj-grp-obj-price.obj-type     INDEXED-REPOSITION.
...
do:
  ...
  run init-5obj.
   OPEN QUERY BROWSE-2db FOR EACH buf_db-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_db-grp-obj-price.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.
END.
ON CHOOSE OF B-del-frm IN FRAME Dialog-Frame
DO:
   define variable g#log as logical   no-undo .
...
assign
  v-str-recid17 = trim( string( recid( buf_grp-obj-price ) , "->>>>>>>>>>>9":U ) )
  v-num-entry17 = lookup( v-str-recid17 , p-rec-list )
.
if v-num-entry17 > 0 then do:
  assign
    entry( v-num-entry17, p-rec-list ) = "":U
    p-rec-list = trim( replace( p-rec-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    p-rec-list = p-rec-list + ( if p-rec-list = "":U then "":U else chr(44) ) + v-str-recid17
  .
end.
...
assign
  v-str-recid19 = trim( string( recid( buf_db-grp-obj-price ) , "->>>>>>>>>>>9":U ) )
  v-num-entry19 = lookup( v-str-recid19 , v-rec-list-cli )
.
if v-num-entry19 > 0 then do:
  assign
    entry( v-num-entry19, v-rec-list-cli ) = "":U
    v-rec-list-cli = trim( replace( v-rec-list-cli , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rec-list-cli = v-rec-list-cli + ( if v-rec-list-cli = "":U then "":U else chr(44) ) + v-str-recid19
  .
end.
        g-log = browse-2db:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
          g-log = browse-2db:select-next-row ().
          apply "VALUE-CHANGED" to browse-2db in frame Dialog-Frame.
      end.
    end.
    apply "entry" to browse-2db in frame Dialog-Frame.
END.
...
assign
  v-str-recid21 = trim( string( recid( buf_db-grp-obj-price ) , "->>>>>>>>>>>9":U ) )
  v-num-entry21 = lookup( v-str-recid21 , v-rec-list-cli )
.
if v-num-entry21 > 0 then do:
  assign
    entry( v-num-entry21, v-rec-list-cli ) = "":U
    v-rec-list-cli = trim( replace( v-rec-list-cli , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rec-list-cli = v-rec-list-cli + ( if v-rec-list-cli = "":U then "":U else chr(44) ) + v-str-recid21
  .
end.
        g-log = browse-2db:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
...
assign
  v-str-recid23 = trim( string( recid( buf_db-grp-obj-price ) , "->>>>>>>>>>>9":U ) )
  v-num-entry23 = lookup( v-str-recid23 , v-rec-list-cli )
.
if v-num-entry23 > 0 then do:
  assign
    entry( v-num-entry23, v-rec-list-cli ) = "":U
    v-rec-list-cli = trim( replace( v-rec-list-cli , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rec-list-cli = v-rec-list-cli + ( if v-rec-list-cli = "":U then "":U else chr(44) ) + v-str-recid23
  .
end.
        g-log = browse-2db:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
...
DO:
    ...
    ASSIGN frame Dialog-Frame:WIDTH   = 101.50 .
    display B-history B-print B-Help B-mark-2 B-add-DB B-del-DB B-mark-3 B-add-frm B-del-frm BROWSE-2db BROWSE-3frm B-mark-4 B-add-obj B-del-obj BROWSE-4obj with FRAME Dialog-Frame no-error .
      v-full = true .
    end.
    else do:
    HIDE B-history B-print B-Help B-mark-2 B-add-DB B-del-DB B-mark-3 B-add-frm B-del-frm BROWSE-2db BROWSE-3frm B-mark-4 B-add-obj B-del-obj BROWSE-4obj IN FRAME Dialog-Frame .
    ASSIGN frame Dialog-Frame:WIDTH   = 51.50 .
    v-full = false  .
    end.
END.
...
END.
ON ROW-entry OF BROWSE-1grp IN FRAME Dialog-Frame
DO:
  if v-cntxt-db-num <> 0 then do :
    if buf_grp-obj-price.gop-db-num <> v-cntxt-db-num then do:
      return no-apply.
...
def var sort-labelBROWSE-5all   as character no-undo .
def var sort-clmnBROWSE-5all    as handle    no-undo .
def var cur-clmnBROWSE-5all     as handle    no-undo .
def var cur-clmn-locBROWSE-5all as integer   no-undo .
def var re-queryBROWSE-5all     as logical   initial no no-undo .
on start-search, ctrl-o of BROWSE-5all in frame Dialog-Frame do:
   run sort-brBROWSE-5all
     (input (if available x_obj-grp-obj-price
             then recid(x_obj-grp-obj-price)
             else ?
            )
     ).
end.
PROCEDURE sort-brBROWSE-5all :
  define input parameter p-recid as recid no-undo .
  if re-queryBROWSE-5all = no then do:
    assign
       cur-clmnBROWSE-5all = BROWSE-5all:current-column in frame Dialog-Frame
    .
    if sort-clmnBROWSE-5all <> ? then sort-clmnBROWSE-5all:column-fgcolor = 0.
    if cur-clmnBROWSE-5all = sort-clmnBROWSE-5all then do:
      assign
         sort-labelBROWSE-5all = ""
         sort-clmnBROWSE-5all = ?
      .
     end.
     else do:
       assign
         sort-labelBROWSE-5all = cur-clmnBROWSE-5all:label
         sort-clmnBROWSE-5all  = cur-clmnBROWSE-5all
         sort-clmnBROWSE-5all:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locBROWSE-5all = 1
  .
  def var column-handle as handle no-undo .
  column-handle = BROWSE-5all:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnBROWSE-5all then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locBROWSE-5all = cur-clmn-locBROWSE-5all + 1
    .
  end.
  case sort-labelBROWSE-5all:
        when 'Объект'  then DO:   assign     sort-column-name = "buf_clients.obj-code"   .   run open-5all .   . END.
        when 'Имя объекта'  then DO:   assign     sort-column-name = "buf_clients.obj-name"   .   run open-5all .   . END.
        when 'Фирма'  then DO:   assign     sort-column-name = "buf_clients.host-code"   .   run open-5all .   . END.
        when 'БД'  then DO:   assign     sort-column-name = "buf_clients.db-num"   .   run open-5all .   . END.
        when 'Статус'  then DO:   assign     sort-column-name = "x_obj-grp-obj-price.stts"   .   run open-5all .   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run open-5all .
      if sort-labelBROWSE-5all <> "" then do:
        assign
          cur-clmnBROWSE-5all:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locBROWSE-5all = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition BROWSE-5all to recid p-recid no-error.
    apply "value-changed" to BROWSE-5all in frame Dialog-Frame.
  end.
  apply "entry" to BROWSE-5all in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnBROWSE-5all:
if cur-clmnBROWSE-5all = ? then do:
   run open-5all .
end.
else do:
   assign re-queryBROWSE-5all = yes.
   run sort-brBROWSE-5all
     (input (if available x_obj-grp-obj-price
             then recid(x_obj-grp-obj-price)
             else ?
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1grp FOR EACH buf_grp-obj-price WHERE     ( r-status = 2 OR buf_grp-obj-price.stts =  r-status ) AND     ( g-find = NO OR       LOOKUP (string(buf_grp-obj-price.gop-id) + ";"  + string(buf_grp-obj-price.gop-db-num)  ,        tt-grp-obj ) > 0 )                .    OPEN QUERY BROWSE-2db FOR EACH buf_db-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_db-grp-obj-price.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.    OPEN QUERY BROWSE-3frm FOR EACH buf_host-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_host-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST host-name NO-LOCK WHERE       host-name.obj-code = buf_host-grp-obj-price.host-code AND       host-name.obj-type = 'орг':U     INDEXED-REPOSITION.    OPEN QUERY BROWSE-4obj FOR EACH buf_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR buf_obj-grp-obj-price.stts =  r-status )       NO-LOCK,            FIRST buf_obj-name NO-LOCK WHERE       buf_obj-name.obj-code = buf_obj-grp-obj-price.obj-code AND       buf_obj-name.obj-type = buf_obj-grp-obj-price.obj-type     INDEXED-REPOSITION.    OPEN QUERY BROWSE-5all FOR EACH x_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR x_obj-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST buf_clients NO-LOCK WHERE       buf_clients.obj-code = x_obj-grp-obj-price.obj-code AND       buf_clients.obj-type = x_obj-grp-obj-price.obj-type        .
END PROCEDURE.
PROCEDURE init-5obj :
...
PROCEDURE open-5all :
 case sort-column-name :
  ...
  when "buf_clients.obj-code"  then do:
    OPEN QUERY BROWSE-5all  FOR EACH x_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR x_obj-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST buf_clients NO-LOCK WHERE       buf_clients.obj-code = x_obj-grp-obj-price.obj-code AND       buf_clients.obj-type = x_obj-grp-obj-price.obj-type by buf_clients.obj-code.
  end.
  when "buf_clients.obj-name"  then do:
    OPEN QUERY BROWSE-5all  FOR EACH x_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR x_obj-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST buf_clients NO-LOCK WHERE       buf_clients.obj-code = x_obj-grp-obj-price.obj-code AND       buf_clients.obj-type = x_obj-grp-obj-price.obj-type by buf_clients.obj-name.
  end.
  when "buf_clients.host-code"  then do:
    OPEN QUERY BROWSE-5all  FOR EACH x_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR x_obj-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST buf_clients NO-LOCK WHERE       buf_clients.obj-code = x_obj-grp-obj-price.obj-code AND       buf_clients.obj-type = x_obj-grp-obj-price.obj-type by buf_clients.host-code.
  end.
  when "buf_clients.db-num"  then do:
    OPEN QUERY BROWSE-5all  FOR EACH x_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR x_obj-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST buf_clients NO-LOCK WHERE       buf_clients.obj-code = x_obj-grp-obj-price.obj-code AND       buf_clients.obj-type = x_obj-grp-obj-price.obj-type by buf_clients.db-num.
  end.
  when "x_obj-grp-obj-price.stts"  then do:
    OPEN QUERY BROWSE-5all  FOR EACH x_obj-grp-obj-price OF buf_grp-obj-price      WHERE ( r-status = 2 OR x_obj-grp-obj-price.stts =  r-status )       NO-LOCK ,            FIRST buf_clients NO-LOCK WHERE       buf_clients.obj-code = x_obj-grp-obj-price.obj-code AND       buf_clients.obj-type = x_obj-grp-obj-price.obj-type by x_obj-grp-obj-price.stts.
  end.
 end case.
END PROCEDURE.
...
define buffer buf_db-grp-obj-price for ub.db-grp-obj-price  .
  find first buf_db-grp-obj-price no-lock where
             ...
             buf_db-grp-obj-price.stts       = 0  no-error .
...
define buffer buf_host-grp-obj-price for ub.host-grp-obj-price  .
   find first buf_host-grp-obj-price no-lock where
              ...
              buf_host-grp-obj-price.stts       = 0  no-error .
...
define buffer buf_obj-grp-obj-price for ub.obj-grp-obj-price  .
  find first buf_obj-grp-obj-price no-lock where
             ...
             buf_obj-grp-obj-price.stts       = 0  no-error .
if available buf_obj-grp-obj-price then v-3 = true .
if v-1 or v-2 or  v-3  then p-stts = 0 .
                       else p-stts = 1 .
END PROCEDURE.
