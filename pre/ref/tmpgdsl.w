...
def buffer t-s for ub.tmp-sale-gds.
def buffer t-g for ub.goods.
...
(   parParentProc ,
    ...
    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     .
END.
...
DO:
  ...
  when 1 then do:
        if sch-field = "s-name-cnt" then do:
                       assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.
                      OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     .
                      end.
...
DO:
 ...
 for each t-s where t-s.tmp-code = t-code,
  ...
  if doc-rec = ? then message "Товар не найден !"  .
     ...
     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name + " , содержащие в названии " + s-name-cnt .
      OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods where       ub.goods.artic = ub.tmp-sale-gds.artic and       ub.goods.prod-type = ub.tmp-sale-gds.prod-type and       ub.goods.prod-code = ub.tmp-sale-gds.prod-code and       INDEX(ub.goods.gds-name,s-name-cnt) > 0       NO-LOCK .
     end.
return no-apply.
end.
END.
...
assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.
 ASSIGN b-list:POPUP-MENU IN FRAME Dialog-Frame   = MENU POPUP-MENU-b-list:HANDLE.
...
on start-search, ctrl-o of BROWSE-2 in frame Dialog-Frame do:
   run sort-brBROWSE-2
     (input (if available ub.tmp-sale-gds
             then recid(ub.tmp-sale-gds)
             else ?
            )
     ).
end.
PROCEDURE sort-brBROWSE-2 :
  ...
  case sort-labelBROWSE-2:
        ...
        when ub.tmp-sale-gds.tmp-value:label in browse BROWSE-2 then DO:   assign     sort-column-name = "ub.tmp-sale-gds.tmp-value"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
case sort-column-name :
  when "" then do:
        if r-sort = 3 and sch-field = "s-name-cnt" then do:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name + " , содержащие в названии " + s-name-cnt .    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods where       ub.goods.artic = ub.tmp-sale-gds.artic and       ub.goods.prod-type = ub.tmp-sale-gds.prod-type and       ub.goods.prod-code = ub.tmp-sale-gds.prod-code and       INDEX(ub.goods.gds-name,s-name-cnt) > 0       NO-LOCK .     end.    else DO:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     .    end.
  end.
  when "goods.artic" then do:
        if r-sort = 3 and sch-field = "s-name-cnt" then do:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name + " , содержащие в названии " + s-name-cnt .    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods where       ub.goods.artic = ub.tmp-sale-gds.artic and       ub.goods.prod-type = ub.tmp-sale-gds.prod-type and       ub.goods.prod-code = ub.tmp-sale-gds.prod-code and       INDEX(ub.goods.gds-name,s-name-cnt) > 0       NO-LOCK by ub.goods.artic.     end.    else DO:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     by ub.goods.artic.    end.
  end.
  when "goods.gds-name" then do:
        if r-sort = 3 and sch-field = "s-name-cnt" then do:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name + " , содержащие в названии " + s-name-cnt .    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods where       ub.goods.artic = ub.tmp-sale-gds.artic and       ub.goods.prod-type = ub.tmp-sale-gds.prod-type and       ub.goods.prod-code = ub.tmp-sale-gds.prod-code and       INDEX(ub.goods.gds-name,s-name-cnt) > 0       NO-LOCK by ub.goods.gds-name.     end.    else DO:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     by ub.goods.gds-name.    end.
  end.
  when "goods.unit-base" then do:
        if r-sort = 3 and sch-field = "s-name-cnt" then do:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name + " , содержащие в названии " + s-name-cnt .    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods where       ub.goods.artic = ub.tmp-sale-gds.artic and       ub.goods.prod-type = ub.tmp-sale-gds.prod-type and       ub.goods.prod-code = ub.tmp-sale-gds.prod-code and       INDEX(ub.goods.gds-name,s-name-cnt) > 0       NO-LOCK by ub.goods.unit-base.     end.    else DO:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     by ub.goods.unit-base.    end.
  end.
  when "tmp-sale-gds.tmp-value" then do:
        if r-sort = 3 and sch-field = "s-name-cnt" then do:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name + " , содержащие в названии " + s-name-cnt .    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods where       ub.goods.artic = ub.tmp-sale-gds.artic and       ub.goods.prod-type = ub.tmp-sale-gds.prod-type and       ub.goods.prod-code = ub.tmp-sale-gds.prod-code and       INDEX(ub.goods.gds-name,s-name-cnt) > 0       NO-LOCK by ub.tmp-sale-gds.tmp-value.     end.    else DO:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     by ub.tmp-sale-gds.tmp-value.    end.
  end.
  otherwise do:
        if r-sort = 3 and sch-field = "s-name-cnt" then do:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name + " , содержащие в названии " + s-name-cnt .    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods where       ub.goods.artic = ub.tmp-sale-gds.artic and       ub.goods.prod-type = ub.tmp-sale-gds.prod-type and       ub.goods.prod-code = ub.tmp-sale-gds.prod-code and       INDEX(ub.goods.gds-name,s-name-cnt) > 0       NO-LOCK .     end.    else DO:     assign frame Dialog-Frame:title = "Товары >> Темп продаж- " + t-name.    OPEN QUERY BROWSE-2 FOR EACH ub.tmp-sale-gds       WHERE ub.tmp-sale-gds.tmp-code = t-code NO-LOCK,       EACH ub.goods WHERE ub.goods.artic = ub.tmp-sale-gds.artic   AND  ub.goods.prod-code = ub.tmp-sale-gds.prod-code   AND ub.goods.prod-type = ub.tmp-sale-gds.prod-type NO-LOCK     .    end.
  end.
