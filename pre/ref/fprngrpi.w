...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-fbr-prn-grp SHARE-LOCK.
...
CASE par-mode:
  ...
  when 'ИЗМЕНЕНИЕ':U then do:
    ...
    .
    assign
    frame Dialog-Frame:title = frame Dialog-Frame:title + chr(32) + grp-name
    .
  end.
END CASE.
  DISPLAY
  ...
  B-exit
  B-Help
  B-grp when par-mode = 'ДОБАВЛЕНИЕ':U and par-call-mode = "printer":U
...
if error-status:error then do:
    CASE return-value:
        when "node-code":U then do:
            APPLY "ENTRY" to b-grp in frame Dialog-Frame.
        end.
