...
define buffer find_code for ub.fin-code-cor-acc .
...
define variable rr as recid no-undo .
    if available buf_fin-code then do:
        ...
        run ref/fci-1.w
        ( 'ИЗМЕНЕНИЕ':U , input-output rr , input par-host-code ).
...
define variable rr as recid no-undo .
    if available buf_fin-code then do:
        rr = recid( buf_fin-code ).
        run ref/fci-1.w ( 'ПРОСМОТР':U , input-output rr , input par-host-code ).
     end.
END.
...
assign
  v-str-recid15 = trim( string( recid( buf_fin-code ) , "->>>>>>>>>>>9":U ) )
...
DEF VAR cur-clmn-numBR-docs as INT EXTENT 9 no-undo.
...
ON ctrl-cursor-right OF BROWSE BR-docs do:
  RUN re-move-clmnBR-docs ( 2, 9).
END.
ON ctrl-cursor-left OF BROWSE BR-docs do:
  RUN re-move-clmnBR-docs (9, 2).
END.
...
on start-search, ctrl-o of BR-docs in frame Dialog-Frame do:
   run sort-brBR-docs
     (input (if available ub.fin-code-cor-acc
             then recid(ub.fin-code-cor-acc)
             else ?
            )
     ).
end.
PROCEDURE sort-brBR-docs :
  ...
  case sort-labelBR-docs:
        ...
        when 'Тип'  then DO:    assign       sort-column-name = "buf_fin-code.acc-type"     .     run OpenBr(yes, no, '':U).   . END.
...
define variable vss-include-info21 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  run openbr in this-procedure (yes, no, '':u).
  hide mark-num  B-import in frame Dialog-Frame .
  if pardoc-rec <> ? then
...
if not g#log then  return .
define variable rr as recid no-undo .
  run ref/fci-1.w ( 'ДОБАВЛЕНИЕ':U , input-output rr , input par-host-code ).
  v-doc-rec = rr .
  run openbr in this-procedure (yes, no, '':u).
...
define buffer buf2_fin-code for ub.fin-code-cor-acc .
...
FUNCTION sel-type RETURNS CHARACTER
  ...
  define variable rr as character no-undo .
    rr = string(entry (lookup (string(loc-fin-code.acc-type), '1,2,3':U), 'Активный,Пассивный,Акт-пасс':U) ) no-error .
  if error-status :error then return '' .
  RETURN rr.
END FUNCTION.
