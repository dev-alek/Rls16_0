...
END.
ON RIGHT-MOUSE-CLICK OF f-cash-d-pcnt IN FRAME Dialog-Frame
DO:
  assign
    f-cash-d-pcnt:fgcolor = 15
    f-cash-d-pcnt = 0
    l-cash-d-pcnt:visible = true.
    display f-cash-d-pcnt with frame Dialog-Frame.
    disable f-cash-d-pcnt with frame Dialog-Frame.
END.
ON RIGHT-MOUSE-CLICK OF f-category IN FRAME Dialog-Frame
DO:
  assign
    f-category:fgcolor = 15
    f-category = 0
    l-category:visible = true.
    display f-category with frame Dialog-Frame.
    disable f-category with frame Dialog-Frame.
END.
ON RIGHT-MOUSE-CLICK OF f-d-pcnt IN FRAME Dialog-Frame
DO:
  assign
    f-d-pcnt:fgcolor = 15
    f-d-pcnt = 0
    l-d-pcnt:visible = true.
    display f-d-pcnt with frame Dialog-Frame.
    disable f-d-pcnt with frame Dialog-Frame.
END.
...
DO:
   IF l-cash-d-pcnt:visible then do:
    ...
    enable f-cash-d-pcnt with frame Dialog-Frame.
...
DO:
  IF l-category:visible then do:
    ...
    enable f-category with frame Dialog-Frame.
...
DO:
   IF l-d-pcnt:visible then do:
    ...
    enable f-d-pcnt with frame Dialog-Frame.
...
if p-obj-type = '' then do:
  assign
  frame Dialog-Frame:title = substitute("&1 Фирма &2", frame Dialog-Frame:title, p-host-code).
end.
else do:
  assign
  frame Dialog-Frame:title = substitute("&1 Фирма &2 &3&4", frame Dialog-Frame:title, p-host-code, p-obj-type, p-obj-code ).
end.
DISPLAY
f-d-pcnt
f-cash-d-pcnt
...
b-quit
B-Help
l-d-pcnt       WHEN p-mode <> 'ПРОСМОТР':U
l-cash-d-pcnt  WHEN p-mode <> 'ПРОСМОТР':U
l-category     WHEN p-mode <> 'ПРОСМОТР':U
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
...
FOR EACH temp-dis-card-property where
   ...
   CASE temp-dis-card-property.node-code:
     WHEN 1 THEN DO:
       ...
       l-d-pcnt:SENSITIVE IN FRAME Dialog-Frame = NO.
     END.
     WHEN 2 THEN DO:
       APPLY "mouse-select-click" TO l-cash-d-pcnt.
       l-cash-d-pcnt:SENSITIVE IN FRAME Dialog-Frame = NO.
     END.
     WHEN 3 THEN DO:
       APPLY "mouse-select-click" TO l-category.
       l-category:SENSITIVE IN FRAME Dialog-Frame = NO.
     END.
  END CASE.
END.
END.
