  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-tax-rate-gds :handle
      .
...
on F9 of frame Dialog-Frame anywhere do:
  ...
  apply "entry" to br-tax-rate-gds in frame Dialog-Frame.
  return no-apply.
end.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-tax-rate-gds FOR EACH X_goods NO-LOCK,              FIRST X_tax-rate-gds OF X_goods NO-LOCK,              FIRST LAST_tax-rate-gds OF X_goods NO-LOCK.
END PROCEDURE.
PROCEDURE MyEnable :
CASE p-list-mode:
    when "TAX":U then do:
...
CASE p-list-mode:
    when "TAX":U then do:
        ASSIGN
        frame Dialog-Frame:TITLE = substitute("Товары по налогу с кодом &1 (&2)"
                                               ,p-tax-code
                                               ,ub.tax.tax-name)
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-19 as character no-undo .
      ...
      parameter-4-19 =
        (
          if (" TRUE " + " " + where-phrase-19) <> ""
          then " TRUE " + " " + where-phrase-19
          else "true"
        )
      parameter-5-19 = (" " + "" + " " +  substitute(' NO-LOCK,  LAST X_tax-rate-gds NO-LOCK where                                                     X_tax-rate-gds.gds-code = X_goods.gds-code AND                                                     X_tax-rate-gds.tax-code = &1 AND                                                     X_tax-rate-gds.fact-order <= &2,                                                     FIRST LAST_tax-rate-gds No-LOCK where                                                     recid(LAST_tax-rate-gds) = recid(X_tax-rate-gds) ', p-tax-code, my-fact-order))
      parameter-6-19 = if sort-phrase-19 = ''
                           then
        (
        " " + "  " +
        " " + " BY X_goods.gds-code BY X_tax-rate-gds.rate-code "
        )
                           else
        (
        " " + "  " +
    ...
    :
      assign
        l-disable-where-19 =
          (" TRUE " + " " + where-phrase-19 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-tax-rate-gds:handle
  ...
  if l-filter-open-19 = false then do:
    OPEN QUERY br-tax-rate-gds FOR EACH X_goods
      where  TRUE
     NO-LOCK,  LAST X_tax-rate-gds NO-LOCK where                                                     X_tax-rate-gds.gds-code = X_goods.gds-code AND                                                     X_tax-rate-gds.tax-code = p-tax-code AND                                                     X_tax-rate-gds.fact-order <= my-fact-order,                                                     FIRST LAST_tax-rate-gds No-LOCK where                                                     recid(LAST_tax-rate-gds) = recid(X_tax-rate-gds)
       BY X_goods.gds-code BY X_tax-rate-gds.rate-code
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when "TAX-RATE":U then do:
        ASSIGN
        frame Dialog-Frame:TITLE = substitute("Товары по ставке налога с кодом &1 (&2) - налог &3"
                                                ,p-rate-code
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-21 as character no-undo .
      ...
      parameter-5-21 = (" " + "" + " " +  substitute('NO-LOCK,  LAST X_tax-rate-gds NO-LOCK where                                                     X_tax-rate-gds.gds-code = X_goods.gds-code AND                                                     X_tax-rate-gds.tax-code = &1 AND                                                     X_tax-rate-gds.fact-order <= &2,                                                     FIRST LAST_tax-rate-gds No-LOCK where                                                     recid(LAST_tax-rate-gds) = recid(X_tax-rate-gds) AND                                                     LAST_tax-rate-gds.rate-code = &3 ', p-tax-code, my-fact-order, p-rate-code))
  ...
  if l-filter-open-21 = false then do:
    OPEN QUERY br-tax-rate-gds FOR EACH X_goods
      where  TRUE
     NO-LOCK,  LAST X_tax-rate-gds NO-LOCK where                                                     X_tax-rate-gds.gds-code = X_goods.gds-code AND                                                     X_tax-rate-gds.tax-code = p-tax-code AND                                                     X_tax-rate-gds.fact-order <= my-fact-order,                                                     FIRST LAST_tax-rate-gds No-LOCK where                                                     recid(LAST_tax-rate-gds) = recid(X_tax-rate-gds) AND                                                     LAST_tax-rate-gds.rate-code = p-rate-code
       BY X_goods.gds-code
  .
      assign
        l-query-was-opened = true
...
if available gds-list then do:
  ...
  define variable v-last22 as integer no-undo .
  find last gds-list use-index oi no-error.
  if available gds-list then do:
    v-last22 = gds-list.order-num .
  end.
  else do:
    v-last22 = 0 .
  end.
