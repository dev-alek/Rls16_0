...
DO:
  ...
  OPEN QUERY br-scales FOR EACH ub.scales-gds       WHERE scales-gds.db-num = p-db-num AND scales-gds.b-code = p-b-code  AND (scales-gds.obj-type = v-obj-type or v-obj-type = "")  AND (scales-gds.obj-code = v-obj-code or v-obj-code = 0) NO-LOCK,       EACH ub.scales WHERE (scales.db-num = scales-gds.db-num AND scales.scales-num = scales-gds.scales-num)   OR (scales.db-num = scales-gds.db-num AND  scales.master = scales-gds.scales-num) NO-LOCK,       EACH ub.bar-code WHERE bar-code.b-code = scales-gds.b-code NO-LOCK,       EACH ub.gds-obj-attr WHERE TRUE        AND gds-obj-attr.obj-type = scales-gds.obj-type  AND gds-obj-attr.obj-code = scales-gds.obj-code  AND gds-obj-attr.gds-code = bar-code.gds-code  AND gds-obj-attr.attr-code = 'scales-code':U NO-LOCK.
...
DO:
  ...
  if ub.scales.master > 0 then do:
    ...
    message "Для просмотра товаров на весах"  skip
                ...
                (if avail m-scales
                then ("N ":U + string(m-scales.scales-num) + chr(32) + m-scales.scales-name)
                else "")
    view-as alert-box.
    return no-apply.
  end.
...
do:
  v-doc-rec = ?. if available ub.scales then v-doc-rec = recid(ub.scales). run openbr in this-procedure. reposition br-scales to recid v-doc-rec.
...
PROCEDURE MyEnable :
  define variable glog as logical no-undo .
    ...
    ASSIGN frame Dialog-Frame:TITLE = substitute("БД &1 Весы с товаром &2 &3&4 &5"
                                                  , p-db-num
                                                  , ub.goods.artic
                                                  , ub.goods.prod-type
                                                  , ub.goods.prod-code
