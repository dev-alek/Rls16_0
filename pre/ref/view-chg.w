...
define SHARED temp-table temp-labels no-undo
  ...
  :
    ...
    by buf_child_temp-changes.f_parent:
      ...
      if available buf_temp-changes then do:
        ...
        buf_child_temp-changes.l_name = fill( chr(32), 3) + buf_child_temp-changes.l_name
        .
        find first buf_temp-labels where
...
END.
ON CHOOSE OF B-exit IN FRAME REDDialog-Frame
DO:
...
DO:
  DO WITH FRAME REDDialog-Frame:
    IF AVAILABLE temp-changes THEN DO:
        ...
        ASSIGN
        b-mark:sensitive IN FRAME REDDialog-Frame = YES.
        ELSE
        ASSIGN
        b-mark:sensitive IN FRAME REDDialog-Frame = no.
    END.
    ELSE DO:
    END.
  END.
...
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME REDDialog-Frame:PARENT eq ?
THEN FRAME REDDialog-Frame:PARENT = ACTIVE-WINDOW.
define variable vss-include-info6 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame REDDialog-Frame
do:
  run gbl/app_help.p
...
on choose of b-help in frame REDDialog-Frame
do:
  apply "help":u to frame REDDialog-Frame .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame REDDialog-Frame:width - 0.3
                fh            = frame REDDialog-Frame:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame REDDialog-Frame :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame REDDialog-Frame :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame REDDialog-Frame :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame REDDialog-Frame :height = v-frame-height
          .
          if frame REDDialog-Frame :scrollable = true
          then do:
            assign
              frame REDDialog-Frame :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame REDDialog-Frame :height
      v-frame-virtual-height = frame REDDialog-Frame :virtual-height
    ...
    assign
      v-field-group-handle = frame REDDialog-Frame :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame REDDialog-Frame :virtual-height = frame REDDialog-Frame :virtual-height + p-change-value
          no-error .
      ...
      assign
        frame REDDialog-Frame :height = frame REDDialog-Frame :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame REDDialog-Frame :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame REDDialog-Frame :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame REDDialog-Frame :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame REDDialog-Frame :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame REDDialog-Frame :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame REDDialog-Frame :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame REDDialog-Frame :width = v-frame-width
          .
          if frame REDDialog-Frame :scrollable = true
          then do:
            assign
              frame REDDialog-Frame :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame REDDialog-Frame :width
      v-frame-virtual-width = frame REDDialog-Frame :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame REDDialog-Frame :virtual-width = frame REDDialog-Frame :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame REDDialog-Frame :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame REDDialog-Frame :width = frame REDDialog-Frame :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame REDDialog-Frame :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame REDDialog-Frame :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame REDDialog-Frame :height - v-diasize-resize-button :height
                  - 1
                  - (frame REDDialog-Frame :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame REDDialog-Frame :width - v-diasize-resize-button :width
                  - 1
                  - (frame REDDialog-Frame :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame REDDialog-Frame
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame REDDialog-Frame :height
      v-col-delta = v-new-col - frame REDDialog-Frame :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
    ...
    then do:
      ...
      run diasize_change-width in this-procedure
        (input session :width-chars
            - frame REDDialog-Frame :width-chars
        ,input true
        ) .
      assign
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame REDDialog-Frame :width
      v-diasize-current-frame-height = frame REDDialog-Frame :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame REDDialog-Frame :height
        v-diasize-orig-frame-width  = frame REDDialog-Frame :width
        v-diasize-browse-handle     = browse BR-changes :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame REDDialog-Frame :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN Myenable.
  APPLY "ENTRY":U TO br-changes IN FRAME REDDialog-Frame.
  WAIT-FOR GO OF FRAME REDDialog-Frame.
END.
RUN disable_UI.
...
DO WITH FRAME REDDialog-Frame:
  IF AVAILABLE Temp-changes THEN DO:
    ...
    IF temp-changes.f_can_update THEN DO:
      ASSIGN
      Temp-changes.f_update = NOT (INPUT BROWSE BR-changes Temp-changes.f_update ).
  ...
  do v-ii = 1 to min(v-first-bh:num-fields, v-next-bh:num-fields):
    ...
    and fh-first:data-type = fh-next:data-type then do:
      ...
      THEN DO:
        ...
        temp-changes.f_can_update = (lookup('ИЗМЕНЕНИЕ':U, p-mode) > 0)
...
PROCEDURE MyEnable :
ASSIGN
FRAME REDDialog-Frame:TITLE = p-title
...
temp-changes.f_update:visible in browse br-changes = (lookup('ПРОСМОТР':U, p-mode) = 0)
.
DISPLAY
ED-notes
WITH FRAME REDDialog-Frame.
...
ED-notes
BR-changes
b-mark WHEN (lookup('ПРОСМОТР':U, p-mode) = 0)
WITH FRAME REDDialog-Frame.
VIEW FRAME REDDialog-Frame.
IF lookup('ПРОСМОТР':U, p-mode) > 0 THEN DO:
  HIDE b-exit
  b-mark
  IN FRAME REDDialog-Frame.
  ASSIGN
  b-quit:LABEL = "&Выход"
  b-quit:COL  = 1
  .
   ...
   DO WITH FRAME REDDialog-Frame:
      IF AVAILABLE Temp-changes THEN DO:
        ...
        if lookup( last-event :function, "MOUSE-SELECT-DBLCLICK,Return" ) = 0 then do:
          assign
          glog = BR-changes :select-next-row () in frame REDDialog-Frame
          .
          apply "VALUE-changed":U to BR-changes in frame REDDialog-Frame.
        end.
        apply "entry":U to BR-changes in frame REDDialog-Frame.
     END.
   END.
