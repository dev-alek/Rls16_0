...
DO:
  ...
  if ref-mode <> 'ПРОСМОТР':U then do:
      assign ub.fin-code-cor-acc.code-value
       ...
       .
              if lookup(fin-type-acc:screen-value,'1,2,3':U ) > 0 then
...
b-quit:label = (if  ref-mode = 'ПРОСМОТР':U then "&Выход" else b-quit:label ).
    session:data-entry-return = yes .
     if ref-mode = 'ПРОСМОТР':U then do:
        ...
        if ref-mode = 'ДОБАВЛЕНИЕ':U
            then  WAIT-FOR GO OF FRAME Dialog-Frame FOCUS ub.fin-code-cor-acc.code-value .
            else  WAIT-FOR GO OF FRAME Dialog-Frame FOCUS ub.fin-code-cor-acc.descr .
   end.
END.
...
assign
  fin-type-acc-name = 'Активный,Пассивный,Акт-пасс':U
  fin-type-acc-val  = '1,2,3':U
  l-str        = 'Активный,1,Пассивный,2,Акт-пасс,3':U
.
