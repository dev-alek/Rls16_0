...
if v-num-entry15 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-gln-cli in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-gln-cli in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-gln-cli :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-gln-cli FOR EACH X_ext-classif NO-LOCK,        FIRST X_clients NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ENABLE
b-quit
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-28 as character no-undo .
      ...
      parameter-4-28 =
        (
          if (" X_ext-classif.classif-subject = 'clients':U                         and X_ext-classif.classif-name = 'GLN':U                         AND X_ext-classif.db-num = - 1" + " " + where-phrase-28) <> ""
          then  substitute('X_ext-classif.classif-subject = &1&2&1                         and X_ext-classif.classif-name = &1&3&1                         AND X_ext-classif.db-num = - 1', chr(34), 'clients':U, 'GLN':U) + " " + where-phrase-28
          else "true"
        )
      parameter-5-28 = (" " + "" + " " + substitute(', FIRST X_clients NO-LOCK WHERE                                   X_clients.obj-type = ENTRY(2, X_eXt-classif.uniq-key-rec, chr(3)) AND                                   X_clients.obj-code = integer(ENTRY(3, X_eXt-classif.uniq-key-rec, chr(3)))'))
    ...
    :
      assign
        l-disable-where-28 =
          (" X_ext-classif.classif-subject = 'clients':U                         and X_ext-classif.classif-name = 'GLN':U                         AND X_ext-classif.db-num = - 1" + " " + where-phrase-28 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-gln-cli:handle
  ...
  if l-filter-open-28 = false then do:
    OPEN QUERY br-gln-cli FOR EACH X_ext-classif
      where  X_ext-classif.classif-subject = 'clients':U                         and X_ext-classif.classif-name = 'GLN':U                         AND X_ext-classif.db-num = - 1
...
APPLY "entry" TO br-gln-cli.
if available X_ext-classif then do:
    APPLY "VALUE-CHANGED":U to br-gln-cli.
END.
END PROCEDURE.
