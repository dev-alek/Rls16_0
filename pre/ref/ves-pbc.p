...
CASE p-mode:
    when 'ДОБАВЛЕНИЕ':U then do:
      ...
      if lookup('шту':U, buf_units.unit-name) > 0
      and lookup(sc.scales-type, 'DIGI-SM,CAS_CL5000J,CAS_CL5000,TIGER-SPCT1,TIGER-SPCT2':U) = 0 then do:
       undo _main, return error substitute("На весы типа &1 нельзя добавлять штучный товар", sc.scales-type).
      end.
define variable vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-rid4 as recid no-undo .
v-found = no.
define variable vss-include-info5 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define buffer locK-batchprocess5 for ub.batchprocess.
run gbl/lock-prc.p
    (input 'lscc':U
    ,input 0
    ,input 0
...
or v-found = yes
then do:
  define variable v-exist4 as logical no-undo .
...
then do:
  ...
  find first prod-bc exclusive-lock where
      ...
      buf_scales-gds.plu-type = (if lookup('вес':U, buf_units.type) > 0
                                       then integer('0':U)
                                       else integer('1':U)
                                       )
      sc.to-send = TRUE
      buf_scales-gds.to-del = FALSE
      buf_scales-gds.deadline = (if p-deadline = ? and p-deadflag = integer('0':U)
                                  then buf_goods.deadline
                                  else p-deadline)
      buf_scales-gds.deaddate = (if p-deaddate <> ? and p-deadflag = integer('1':U)
                                  then p-deaddate
                                  else buf_scales-gds.deaddate)
      buf_scales-gds.deadflag = (if p-deadflag <> ? then p-deadflag else buf_scales-gds.deadflag)
      buf_scales-gds.wt-cart = if lookup('вес':U, buf_units.type) > 0
                               then  (if p-wt-cart = ?
    ...
    when 'ИЗМЕНЕНИЕ':U then do:
      ...
      buf_scales-gds.deadflag = (if p-deadflag <> ? then p-deadflag else buf_scales-gds.deadflag)
      buf_scales-gds.wt-cart = (if p-wt-cart <> ? and buf_scales-gds.plu-type = integer('0':U)
                                then p-wt-cart
