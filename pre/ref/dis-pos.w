...
DEFINE BROWSE br-dis-pos
  QUERY br-dis-pos NO-LOCK DISPLAY
      entry (lookup (tt-dis-pos.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U) FORMAT "X(15)":U COLUMN-LABEL "Тип POS!(место использ)"
...
tt-dis-pos.templ-rl-root FORMAT ">>>9":U COLUMN-LABEL "Код!шабл."
tt-dis-pos.time-templ-rl-root FORMAT "->>>>9":U COLUMN-LABEL "Код!шабл.!распис"
(IF AVAILABLE X_dis-time-rule
 THEN X_dis-time-rule.des
 ELSE "") FORMAT "X(80)":U COLUMN-LABEL "Тип расписания" WIDTH 20
X_dis-rule.des FORMAT "X(255)":U column-label "Описание" WIDTH 45
...
assign
  v-str-recid3 = trim( string( tt-dis-pos.recid_ , "->>>>>>>>>>>9":U ) )
  v-num-entry3 = lookup( v-str-recid3 , p-rid-list )
.
if v-num-entry3 > 0 then do:
  assign
    entry( v-num-entry3, p-rid-list ) = "":U
    p-rid-list = trim( replace( p-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    p-rid-list = p-rid-list + ( if p-rid-list = "":U then "":U else chr(44) ) + v-str-recid3
  .
end.
    loc#log = br-dis-pos:refresh() .
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-dis-pos:select-next-row ().
        apply "VALUE-CHANGED" to br-dis-pos in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-dis-pos in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-dis-pos :handle
      .
...
define variable vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if lookup( p-mode, 'все':U + chr(44) +
                     ...
                     'скл':U + chr(44) +
                     "cd-type-list" + chr(44) +
                     "cd-type-list-without-template-pos" + chr(44) +
                     "rum") = 0 then do:
    message
...
case p-mode:
  when "rum" then do:
    for each X_dis-cfg-rule no-lock where
      ...
      if X_dis-cfg-rule.link-prop <> integer('0':U)
      and X_dis-cfg-rule.link-prop <> integer('-1':U) then next.
      if p-has-glob = 1
      and p-has-host = 0
      and p-has-obj = 0
  ...
  when "cd-type-list-without-template-pos" then do:
    for each X_dis-cfg-rule no-lock where
      ...
      find first buf_tt-dis-pos no-lock where
                ...
                no-error.
      if available buf_tt-dis-pos then next.
      if X_dis-cfg-rule.link-prop <> integer('0':U) then next.
      if p-has-glob = 1
      and p-has-host = 0
      and p-has-obj = 0
      and X_dis-cfg-rule.has-glob = 0 then next.
...
ch = br-dis-pos:FIRST-COLUMN IN FRAME Dialog-Frame.
DO ii = 1 TO br-dis-pos:NUM-COLUMNS IN FRAME Dialog-Frame:
    ASSIGN
    ch:RESIZABLE = YES.
    if p-mode = "rum"
    or p-mode = "cd-type-list-without-template-pos"
    then do:
       if ch:label = "Код!шабл."
       or ch:label = "Код!шабл.!распис"
       or ch:label = "Тип расписания"
       or ch:label = "Описание" then do:
         ch:visible = no.
       end.
    end.
    if p-mode = "cd-type-list-without-template-pos" then do:
      if ch:label = "Тип POS!(место использ)" then do:
        ch:visible = no.
      end.
    end.
    ch = ch:NEXT-COLUMN.
...
CASE p-table-name:
  WHEN 'dis-gds-rule':U THEN DO:
     ASSIGN
     FRAME Dialog-Frame:TITLE = "Возможные типы скидок на товар".
  END.
  WHEN 'dis-thbj-rule':U THEN DO:
     ASSIGN
     FRAME Dialog-Frame:TITLE = "Возможные типы общих скидок".
  END.
  WHEN 'dis-cp-rule':U THEN DO:
     ASSIGN
     FRAME Dialog-Frame:TITLE = "Возможные типы скидок на платеж".
  END.
  WHEN 'dis-dc-rule':U THEN DO:
     ASSIGN
     FRAME Dialog-Frame:TITLE = "Возможные типы скидок на отдельную ДК".
  END.
  WHEN 'dis-dct-rule':U THEN DO:
     ASSIGN
     FRAME Dialog-Frame:TITLE = "Возможные типы скидок на тип ДК".
  END.
  WHEN 'dis-grp-rule':U THEN DO:
    case p-classif-type:
     when 'sum-grp':U then do:
      ASSIGN
      FRAME Dialog-Frame:TITLE = "Возможные типы скидок на группы товаров (на кассе)".
     end.
     when 'cli-grp':U then do:
      ASSIGN
      FRAME Dialog-Frame:TITLE = "Возможные типы скидок на группы клиентов".
     end.
    end case.
  END.
  WHEN 'dis-some-rule':U THEN DO:
     ASSIGN
     FRAME Dialog-Frame:TITLE = "Возможные типы скидок на что-то там".
  END.
END CASE.
END PROCEDURE.
PROCEDURE openbr :
...
IF P-MODE = 'маг':U THEN DO:
    OPEN QUERY br-dis-pos
        ...
        X_dis-time-rule.time-rule-num = tt-dis-pos.time-templ-rl-root outer-join
           INDEXED-REPOSITION.
END.
IF P-MODE = 'скл':U THEN DO:
    OPEN QUERY br-dis-pos
        FOR EACH tt-dis-pos NO-LOCK where
            tt-dis-pos.pos-type = '-':U,
            first X_dis-rule NO-LOCK WHERE
           X_dis-rule.rule-num  = tt-dis-pos.templ-rl-root,
        FIRST X_dis-time-rule NO-LOCK where
         X_dis-time-rule.time-rule-num = tt-dis-pos.time-templ-rl-root outer-join
