...
define variable vss-include-info8 as character format "x(65)" no-undo initial "@(#)$Workfile: findocip.i $ $Revision: ff8019e24d02, 2996, rls $".
...
function clean_char return char
...
function is-fact-and-edit returns logical ():
    return (p-mode = 'ИЗМЕНЕНИЕ':U and locked_fin-doc.status_ = 'факт':U).
end.
...
define variable vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile: findocip.i $ $Revision: ff8019e24d02, 2996, rls $".
...
PROCEDURE proc-shift-name :
  ...
  if input frame Dialog-Frame tt-fin-doc.shift-name <> tt-fin-doc.shift-name then
  do:
    if input frame Dialog-Frame tt-fin-doc.shift-date <> ? then
    do:
      for each  bf_shift-obj where
        bf_shift-obj.obj-type   = tt-fin-doc.obj-type
        and  bf_shift-obj.obj-code   = tt-fin-doc.obj-code
        and  bf_shift-obj.shift-date = input frame Dialog-Frame tt-fin-doc.shift-date
        and  bf_shift-obj.shift-name = input frame Dialog-Frame tt-fin-doc.shift-name
      ...
      do:
        ...
        do:
          message
            ...
            ,tt-fin-doc.obj-type
            ,tt-fin-doc.obj-code
            ,input frame Dialog-Frame tt-fin-doc.shift-date
            ,input frame Dialog-Frame tt-fin-doc.shift-name )
        ...
        display
          tt-fin-doc.shift-name with frame Dialog-Frame.
...
PROCEDURE proc-shift-num :
  define buffer bf_shift-obj for ub.shift-obj.
  if input frame Dialog-Frame tt-fin-doc.shift-num <> tt-fin-doc.shift-num then
  do:
    if input frame Dialog-Frame tt-fin-doc.shift-date <> ? then
    do:
      find first bf_shift-obj where
        ...
        and  bf_shift-obj.obj-code   = tt-fin-doc.obj-code
        and  bf_shift-obj.shift-date = input frame Dialog-Frame tt-fin-doc.shift-date
        and  bf_shift-obj.shift-num  = input frame Dialog-Frame tt-fin-doc.shift-num  no-lock no-error.
      ...
      do:
        message
          ...
          ,tt-fin-doc.obj-code
          ,input frame Dialog-Frame tt-fin-doc.shift-date
          ,input frame Dialog-Frame tt-fin-doc.shift-num)
          view-as alert-box error.
        display
          tt-fin-doc.shift-num with frame Dialog-Frame.
        run proc-sht no-error.
        if error-status:error then
        do:
          return error.
  ...
  do:
    ...
    do:
      ...
      assign
        ...
        .
      display tt-fin-doc.doc-date with frame Dialog-Frame.
    end.
  end.
END PROCEDURE.
define variable vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile: findocip.i $ $Revision: ff8019e24d02, 2996, rls $".
...
do:
  ...
  then do:
    ASSIGN
      tt-fin-doc.doc-date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date20 :HANDLE
      tt-fin-doc.doc-date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle20 as handle no-undo .
  assign
    v-label-handle20 = tt-fin-doc.doc-date :side-label-handle in frame Dialog-Frame
  .
...
ON ANY-PRINTABLE OF tt-fin-doc.including IN FRAME Dialog-Frame
DO:
  RUN proc-uho-check IN THIS-PROCEDURE(1) NO-ERROR.
  IF ERROR-STATUS :ERROR THEN DO: RETURN NO-APPLY. END.
END.
ON BACKSPACE OF tt-fin-doc.including IN FRAME Dialog-Frame
DO:
define variable v-offset as integer no-undo .
  RUN proc-uho-check IN THIS-PROCEDURE(1) NO-ERROR.
  IF ERROR-STATUS :ERROR THEN DO: RETURN NO-APPLY. END.
  if tt-fin-doc.including :CURSOR-OFFSET > 1 then do :
    assign
    v-offset = tt-fin-doc.including:CURSOR-OFFSET
    tt-fin-doc.including:screen-value = substring(tt-fin-doc.including:screen-value, 1, tt-fin-doc.including:CURSOR-OFFSET - 2) +
                      substring(tt-fin-doc.including:screen-value, tt-fin-doc.including:CURSOR-OFFSET , length(tt-fin-doc.including:screen-value)  -  tt-fin-doc.including:CURSOR-OFFSET + 1)
    tt-fin-doc.including:CURSOR-OFFSET = v-offset - 1
    .
  end.
END.
ON DELETE-CHARACTER OF tt-fin-doc.including IN FRAME Dialog-Frame
DO:
define variable v-offset as integer no-undo .
  if tt-fin-doc.including:selection-start = ? then do:
    RUN proc-uho-check IN THIS-PROCEDURE(0) NO-ERROR.
    IF ERROR-STATUS :ERROR THEN DO: RETURN NO-APPLY. END.
    assign
    v-offset = tt-fin-doc.including:CURSOR-OFFSET
    tt-fin-doc.including:screen-value = substring(tt-fin-doc.including:screen-value, 1, tt-fin-doc.including:CURSOR-OFFSET - 1) +
                      substring(tt-fin-doc.including:screen-value, tt-fin-doc.including:CURSOR-OFFSET + 1, length(tt-fin-doc.including:screen-value)  -  tt-fin-doc.including:CURSOR-OFFSET + 1)
    tt-fin-doc.including:CURSOR-OFFSET = v-offset
    .
  end.
  else do:
    IF  index(tt-fin-doc.including:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
      IF tt-fin-doc.including:Selection-start >= index(tt-fin-doc.including:SCREEN-VALUE, "@") + 1 THEN   do:
        RETURN no-apply.
      end.
    END.
    tt-fin-doc.including:edit-clear().
  end.
END.
ON CTRL-X OF tt-fin-doc.including IN FRAME Dialog-Frame
DO:
  IF  index(tt-fin-doc.including:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
    IF tt-fin-doc.including:Selection-start >= index(tt-fin-doc.including:SCREEN-VALUE, "@") + 1 THEN   do:
      RETURN no-apply.
  ...
  END.
  tt-fin-doc.including:edit-cut().
END.
ON CTRL-V OF tt-fin-doc.including IN FRAME Dialog-Frame
DO:
  IF  index(tt-fin-doc.including:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
    IF tt-fin-doc.including:Selection-start >= index(tt-fin-doc.including:SCREEN-VALUE, "@") + 1 THEN   do:
      RETURN no-apply.
  ...
  END.
  tt-fin-doc.including:edit-paste().
END.
...
DEFINE INPUT PARAMETER p-offset AS INTEGER NO-UNDO.
  ...
  :
    IF  index(tt-fin-doc.including:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
      IF (tt-fin-doc.including:CURSOR-OFFSET >= index(tt-fin-doc.including:SCREEN-VALUE, "@") + 1
      AND p-offset = 0)
      OR (tt-fin-doc.including:CURSOR-OFFSET > index(tt-fin-doc.including:SCREEN-VALUE, "@") + 1
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  if tt-fin-doc.shift-flag = integer('1':U) then
  do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curshift in g#library
  ...
  ,output varobj-shift-name
    ...
    enable
      r-sht
      when (tt-fin-doc.shift-flag = integer('1':U) and v-limit-access = 0 and v-can-back-shift)
      with frame Dialog-Frame.
  end.
  else
  do:
...
END.
on leave of tt-fin-doc.shift-date in frame Dialog-Frame
  do:
    if input frame Dialog-Frame tt-fin-doc.shift-date <> tt-fin-doc.shift-date then
    do:
      assign
        tt-fin-doc.shift-name = ""
      ...
      display
        tt-fin-doc.shift-name
        tt-fin-doc.shift-num with frame Dialog-Frame.
      apply "entry" to tt-fin-doc.shift-name in frame Dialog-Frame.
      return no-apply.
    end.
  end.
on return of tt-fin-doc.shift-date in frame Dialog-Frame
  do:
    apply "entry" to tt-fin-doc.shift-name in frame Dialog-Frame.
    return no-apply.
  end.
on leave of tt-fin-doc.shift-num  in frame Dialog-Frame
  do:
...
on leave of tt-fin-doc.shift-name in frame Dialog-Frame
  do:
...
if tt-fin-doc.shift-flag = integer('1':U) then
do:
  if tt-fin-doc.shift-date >  input frame Dialog-Frame  tt-fin-doc.doc-date  then
  do:
    message
...
PROCEDURE disable-enable :
  ...
  CASE rs-view:
    when "full":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-cards,b-print,b-hist,b-help," +                            "prn-doc-code,doc-date,obj-type,obj-code,b-obj,b-receiver-view,str-podr-type,str-podr-code,str-podr-name," +                             v-an-uchet-tab-order +  v-sum-curr-tab-order +                            "payer-type,payer-code,b-payer,payer-name,b-payer-view,naznach-plat,including,enclosure," +                            "PS,receiver-sign2,receiver-sign3"
          .
      end.
    when "brief":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-cards,b-print,b-hist,b-help," +                             "prn-doc-code,doc-date,obj-type,obj-code,b-obj,b-receiver-view," +                              v-sum-curr-tab-order +                            "payer-type,payer-code,b-payer,payer-name,b-payer-view,naznach-plat,including,enclosure," +                            "PS,receiver-sign2,receiver-sign3"
          .
      end.
    when "contract":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-cards,b-print,b-hist,b-help," +                               "prn-doc-code,doc-date,obj-type,obj-code,b-obj,b-receiver-view," +                                v-contract-tab-order + v-sum-curr-tab-order +                                "payer-type,payer-code,b-payer,payer-name,b-payer-view,naznach-plat,including,enclosure," +                                 "PS,receiver-sign2,receiver-sign3"
          .
      end.
  END CASE.
END PROCEDURE.
...
procedure proc-update-sum-vat-chr :
  ...
  do
    ...
    do:
      assign
        entry(2, tt-fin-doc.including, "@":U) = chr(44) + chr(32)  + v-sum-vat-chr.
    end.
    display
      tt-fin-doc.including
      with frame Dialog-Frame .
...
b-payer-view
WITH FRAME Dialog-Frame.
if p-mode <> 'ПРОСМОТР':U and not is-fact-and-edit() then do:
  ENABLE
  B-exit
  B-cashbook
  f-cashbook
  ...
  tt-fin-doc.obj-code   when not v-is-auto-obj
                             ...
                             and (tt-fin-doc.shift-flag <> integer('1':U)
                                  or
                                  tt-fin-doc.status_ = 'новый':U)
  r-sht when (tt-fin-doc.shift-flag = integer('1':U) and v-limit-access = 0  and v-can-back-shift)
...
tt-fin-doc.obj-type:radio-buttons = "Маг" + chr(44) + 'маг':U + chr(44) +
                                    "Скл" + chr(44) + 'скл':U
rs-view = v-view
v-an-uchet-tab-order = "cor-acc1-value,b-cor-acc1,"  +
                       "cor-acc-value,b-cor-acc,"  +
...
.
if (p-mode = 'ПРОСМОТР':U or is-fact-and-edit()) and
...
if p-mode = 'ПРОСМОТР':U then do:
    run proc-color-widgets in this-procedure("f-an-uchet-descr,f-cel-nazn-descr,f-contract-curr-abbr,f-contract-date,f-contract-prn-code,f-contract-rate," +                         "f-contract-scale,f-contract-type,f-cor-acc1-descr,f-cor-acc-descr,base-rate,base-scale,curr-code,exch-rate,exch-scale," +                         "fact-date,fin-doc-code,payer-code,payer-type,perm-date,PS,receiver-code,receiver-type,sum-base,sum-rubl," +                         "user-name-doc,user-name-fact,user-name-perm", no, yes, ?, ?).
    run proc-color-widgets in this-procedure("F-curr-abbr,an-uchet-value,cel-nazn-value,cor-acc1-value,cor-acc-value,doc-date,enclosure," +                       "including,naznach-plat,payer-name,prn-doc-code,receiver-name,receiver-okpo,receiver-sign2,receiver-sign3," +                       "str-podr-code,str-podr-name,str-podr-type,sum-doc", no, yes, ?, ?).
end.
display
tt-fin-doc.exch-rate
tt-fin-doc.exch-scale
...
tt-fin-doc.receiver-sign3
tt-fin-doc.PS
tt-fin-doc.shift-date when tt-fin-doc.shift-flag = integer('1':U)
tt-fin-doc.shift-num  when tt-fin-doc.shift-flag = integer('1':U)
tt-fin-doc.shift-name when tt-fin-doc.shift-flag = integer('1':U)
with frame Dialog-Frame
.
find first ub.cashbook no-lock where ub.cashbook.id = tt-fin-doc.CashBookId no-error.
...
if tt-fin-doc.shift-flag <> integer('1':U)  then do:
  hide
  tt-fin-doc.shift-date
  tt-fin-doc.shift-num
  tt-fin-doc.shift-name
...
if is-fact-and-edit() then do with frame Dialog-Frame:
    assign
        tt-fin-doc.prn-doc-code.
end.
else do:
    ...
    tt-fin-doc.receiver-sign2
    tt-fin-doc.receiver-sign3
    tt-fin-doc.doc-author = (if p-mode = 'ДОБАВЛЕНИЕ':U then 'manual':U else tt-fin-doc.doc-author)
    .
