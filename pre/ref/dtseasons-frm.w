...
END.
ON CHOOSE OF b-copy-codename IN FRAME Dialog-Frame
DO:
  run setMisc1 in this-procedure (true).
END.
ON CHOOSE OF b-misc3 IN FRAME Dialog-Frame
DO:
...
END.
ON LEAVE OF f-codename IN FRAME Dialog-Frame
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   if iMode <> 'ДОБАВЛЕНИЕ':U and
      iMode <> 'ИЗМЕНЕНИЕ':U and
      iMode <> 'КОПИРОВАНИЕ':U and
      iMode <> 'ПРОСМОТР':U
   then do:
      message
      vss-workfile vss-revision vss-description skip
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  (output v-db-num
   ...
   if v-db-num <> 0 and iMode <> 'ПРОСМОТР':U then do:
      message
      vss-workfile vss-revision vss-description skip
   ...
   frame Dialog-Frame:title = frame Dialog-Frame:title + " - " + lc(iMode).
   if iMode = 'ИЗМЕНЕНИЕ':U or iMode = 'КОПИРОВАНИЕ':U or iMode = 'ПРОСМОТР':U then do:
      assign
        f-code     = iCode:buffer-field('code'):buffer-value when iMode = 'ИЗМЕНЕНИЕ':U or iMode = 'ПРОСМОТР':U
...
PROCEDURE proc-save :
   ...
   on stop undo, return error:
      if iMode = 'ДОБАВЛЕНИЕ':U or iMode = 'КОПИРОВАНИЕ':U then do:
         ...
         assign
            iCode:buffer-field("parent"):buffer-value = "DTSeasons"
