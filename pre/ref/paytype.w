...
ON GO OF FRAME d-pay-type
DO:
  p-rid-list = v-rid-list.
END.
ON CHOOSE OF b-add IN FRAME d-pay-type
DO:
define variable glog as logical no-undo .
...
run ref/paytypei.w
              (  input parparentproc
                ...
                , input-output rr ).
if rr <> ? then do:
  OPEN QUERY br-paytype FOR EACH X_pay-type NO-LOCK     BY X_pay-type.obj-name.
  reposition br-paytype to recid rr.
  log-res  = br-paytype:select-focused-row( ).
  apply "ENTRY":U to br-paytype.
end.
END.
ON CHOOSE OF b-doc IN FRAME d-pay-type
DO:
...
assign
  v-str-recid9 = trim( string( recid( X_pay-type ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry9 > 0 then do:
  ...
  .
        ...
        g-log = br-paytype:refresh() .
        if last-event:function <> "mouse-select-dblclick" then do:
            g-log = br-paytype:select-next-row ().
            apply "value-changed" to br-paytype in frame d-pay-type.
        end.
    end.
    apply "entry" to br-paytype in frame d-pay-type.
END.
ON CHOOSE OF b-print IN FRAME d-pay-type
DO:
...
DO:
  if b-mark:sensitive in frame d-pay-type then
     APPLY "CHOOSE" to b-mark.
  else do:
      if b-sel:sensitive in frame d-pay-type then
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame d-pay-type :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame d-pay-type :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame d-pay-type :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-pay-type :height = v-frame-height
          .
          if frame d-pay-type :scrollable = true
          then do:
            assign
              frame d-pay-type :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame d-pay-type :height
      v-frame-virtual-height = frame d-pay-type :virtual-height
    ...
    assign
      v-field-group-handle = frame d-pay-type :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame d-pay-type :virtual-height = frame d-pay-type :virtual-height + p-change-value
          no-error .
      ...
      assign
        frame d-pay-type :height = frame d-pay-type :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame d-pay-type :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame d-pay-type :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame d-pay-type :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame d-pay-type :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame d-pay-type :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame d-pay-type :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-pay-type :width = v-frame-width
          .
          if frame d-pay-type :scrollable = true
          then do:
            assign
              frame d-pay-type :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame d-pay-type :width
      v-frame-virtual-width = frame d-pay-type :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame d-pay-type :virtual-width = frame d-pay-type :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame d-pay-type :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame d-pay-type :width = frame d-pay-type :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame d-pay-type :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame d-pay-type :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame d-pay-type :height - v-diasize-resize-button :height
                  - 1
                  - (frame d-pay-type :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame d-pay-type :width - v-diasize-resize-button :width
                  - 1
                  - (frame d-pay-type :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame d-pay-type
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame d-pay-type :height
      v-col-delta = v-new-col - frame d-pay-type :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame d-pay-type :width
      v-diasize-current-frame-height = frame d-pay-type :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame d-pay-type :height
        v-diasize-orig-frame-width  = frame d-pay-type :width
        v-diasize-browse-handle     = browse br-paytype :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame d-pay-type :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  WAIT-FOR GO OF FRAME d-pay-type focus br-paytype.
END.
