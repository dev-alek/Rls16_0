...
DO:
  ...
  OPEN QUERY br-sert FOR EACH ub.sert       WHERE ub.sert.cli-type = i-type and ub.sert.cli-code = i-code NO-LOCK     BY sert.last-date DESCENDING.
...
DO:
  ...
  run ref/gds-sert.w (
               ...
               , input p-curr-obj-type
               , input p-curr-obj-code
               , input (if ub.db.add-goods then 'ИЗМЕНЕНИЕ':U else 'ПРОСМОТР':U)
               , input "sert":U
               , input ?
...
END.
ON CHOOSE OF b-incl IN FRAME Dialog-Frame
DO:
...
DO:
     ...
     disp v-code v-type v-name with frame Dialog-Frame.
END.
...
DO:
  ...
  case r-b-sort:
    when 2 then do:
        disable v-code v-type with frame Dialog-Frame.
        view sert-num .
        enable sert-num with frame Dialog-Frame.
        apply "entry" to sert-num.
        return no-apply.
    end.
    when 3 then do:
        disable sert-num with frame Dialog-Frame.
        hide sert-num .
    ...
    when 5 then do:
        disable v-type v-code sert-num v-days with frame Dialog-Frame.
    ...
    when 6 then do:
        disable v-type v-code sert-num with frame Dialog-Frame.
    ...
    otherwise do:
        disable sert-num v-code v-type v-days with frame Dialog-Frame.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-sert :handle
      .
...
do:
  br-sert :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info8 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
  v-doc-rec = recid(ub.sert). run openbr in this-procedure. reposition br-sert to recid(v-doc-rec). v-doc-rec = ? .
    apply "VALUE-CHANGED" to br-sert.
end.
MAIN-BLOCK:
...
APPLY "VALUE-CHANGED" TO BR-SERT in frame Dialog-Frame.
APPLY "ENTRY" TO BR-SERT in frame Dialog-Frame.
END PROCEDURE.
FUNCTION get-status RETURNS CHARACTER
