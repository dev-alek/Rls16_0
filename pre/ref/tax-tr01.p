...
def var vss-description as character no-undo init "»зменение статуса налога".
...
CASE bf-tax.status_:
  when 'тек':U then do:
      message "¬ы действительно хотите удалить (логически) запись о налоге" bf-tax.tax-name "?"
      view-as alert-box QUESTION buttons YES-NO
      update loc#log.
  end.
  when 'удал':U then do:
      message "«апись о налоге" bf-tax.tax-name "уже (логически) удалена" skip
      "¬осстановить?"
      view-as alert-box QUESTION buttons YES-NO
      update loc#log.
  end.
...
assign
bf-tax.status_ = (if bf-tax.status_ = 'удал':U
                            then 'тек':U
                            else 'удал':U).
