DEFINE BUFFER X_c-delivery-subject FOR ub.c-delivery-subject.
...
define variable vss-description AS CHAR NO-UNDO INIT "Список истории субъектов доставки":U.
...
DEFINE QUERY br-delivsubj FOR
      X_c-delivery-subject SCROLLING.
...
DEFINE BROWSE br-delivsubj
  QUERY br-delivsubj NO-LOCK DISPLAY
      mark-string(recid(X_c-delivery-subject), v-rid-list) COLUMN-LABEL "*" FORMAT "X(1)":U
      usrfulnf(X_c-delivery-subject.corr-user-name) COLUMN-LABEL "Изменил" FORMAT "X(18)":U
      X_c-delivery-subject.corr-date COLUMN-LABEL "Дата!измен" FORMAT "99/99/9999":U
      string(X_c-delivery-subject.corr-time, "HH:MM") COLUMN-LABEL "Время!измен" FORMAT "X(5)":U
      X_c-delivery-subject.deliv-subj-name FORMAT "X(50)":U
      entry (lookup (string(X_c-delivery-subject.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U)
      X_c-delivery-subject.des FORMAT "X(255)":U
      X_c-delivery-subject.deliv-subj-code COLUMN-LABEL "Вн.код группы" FORMAT ">>9":U
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    loc#log = br-delivsubj:refresh() .
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-delivsubj:select-next-row ().
        apply "VALUE-CHANGED" to br-delivsubj in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-delivsubj in frame Dialog-Frame.
END.
...
DO:
    ...
    then
    v-rid-list = string( recid( X_c-delivery-subject ) ) .
  end.
END.
ON RETURN OF br-delivsubj IN FRAME Dialog-Frame
or MOUSE-SELECT-DBLCLICK OF br-delivsubj IN FRAME Dialog-Frame
    DO:
    run proc-br-delivsubj no-error.
  if error-status:error then return no-apply.
END.
ON VALUE-CHANGED OF br-delivsubj IN FRAME Dialog-Frame
DO:
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-delivsubj :handle
      .
...
do:
  v-doc-rec = recid(X_c-delivery-subject). run OpenBr in this-procedure. reposition br-delivsubj to recid v-doc-rec no-error. v-doc-rec = ?.              apply 'value-changed' to br-delivsubj.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-delivsubj   as character no-undo .
def var sort-clmnbr-delivsubj    as handle    no-undo .
def var cur-clmnbr-delivsubj     as handle    no-undo .
def var cur-clmn-locbr-delivsubj as integer   no-undo .
def var re-querybr-delivsubj     as logical   initial no no-undo .
on start-search, ctrl-o of br-delivsubj in frame Dialog-Frame do:
   run sort-brbr-delivsubj
...
PROCEDURE sort-brbr-delivsubj :
  define input parameter p-recid as recid no-undo .
  if re-querybr-delivsubj = no then do:
    assign
       cur-clmnbr-delivsubj = br-delivsubj:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-delivsubj <> ? then sort-clmnbr-delivsubj:column-fgcolor = 0.
    if cur-clmnbr-delivsubj = sort-clmnbr-delivsubj then do:
      assign
         sort-labelbr-delivsubj = ""
         sort-clmnbr-delivsubj = ?
      .
     end.
     else do:
       assign
         sort-labelbr-delivsubj = cur-clmnbr-delivsubj:label
         sort-clmnbr-delivsubj  = cur-clmnbr-delivsubj
         sort-clmnbr-delivsubj:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-delivsubj = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-delivsubj:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-delivsubj then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-delivsubj = cur-clmn-locbr-delivsubj + 1
    .
  end.
  case sort-labelbr-delivsubj:
        when X_c-delivery-subject.deliv-subj-name:label in browse br-delivsubj then DO:   assign     sort-column-name = "X_c-delivery-subject.deliv-subj-name"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-delivsubj') then do:
          run mv-brw-defaultbr-delivsubj.
        end.
      if sort-labelbr-delivsubj <> "" then do:
        assign
          cur-clmnbr-delivsubj:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-delivsubj = ?
      .
    end.
  end case.
    if cur-clmn-locbr-delivsubj <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-delivsubj') then do:
        run ch-clmnbr-delivsubj in this-procedure (cur-clmn-locbr-delivsubj).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-delivsubj to recid p-recid no-error.
    apply "value-changed" to br-delivsubj in frame Dialog-Frame.
  end.
  apply "entry" to br-delivsubj in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-delivsubj:
if cur-clmnbr-delivsubj = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-delivsubj = yes.
   run sort-brbr-delivsubj
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
   ...
   assign re-querybr-delivsubj = no.
end.
end.
 ...
 if p-mode = "one":U then do:
  find first X_delivery-subject no-lock where
    ...
    if not available X_delivery-subject then do:
        message
        vss-workfile vss-revision vss-description skip
        "Неверное значение параметра вызова p-deliv-subj-code"
        p-deliv-subj-code
        view-as alert-box ERROR.
        return.
    end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  ...
  HIDE mark-num in frame Dialog-Frame .
  if v-doc-rec <> ? then
  REPOSITION br-delivsubj to recid v-doc-rec No-ERROR.
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
DEF VAR cur-clmn-numbr-delivsubj as INT EXTENT 8 no-undo.
DEF VAR varmvibr-delivsubj       as INT no-undo.
DEF VAR varmvjbr-delivsubj       as INT no-undo.
DEF VAR varmvkbr-delivsubj       as INT no-undo.
DEF VAR varmvlbr-delivsubj       as INT no-undo.
DEF VAR move-elementbr-delivsubj as INT no-undo.
def var jjbr-delivsubj           as int no-undo.
do varmvibr-delivsubj = 1 to EXTENT(cur-clmn-numbr-delivsubj):
  ASSIGN cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = varmvibr-delivsubj.
END.
RUN start-mv-clmnbr-delivsubj.
PROCEDURE start-mv-clmnbr-delivsubj:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-delivsubj = NUM-ENTRIES('1,2,3,4,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-delivsubj ( cur-clmn-numbr-delivsubj[INTEGER(ENTRY (jjbr-delivsubj, '1,2,3,4,5,6,7,8'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-delivsubj do:
  RUN re-move-clmnbr-delivsubj ( 1, 8).
END.
ON ctrl-cursor-left OF BROWSE br-delivsubj do:
  RUN re-move-clmnbr-delivsubj (8, 1).
END.
PROCEDURE re-move-clmnbr-delivsubj:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-delivsubj = 1 TO EXTENT(cur-clmn-numbr-delivsubj):
    if cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = source-column THEN cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = -1.
  END.
  if br-delivsubj:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-delivsubj = source-column - 1 to target-column BY -1:
    DO varmvibr-delivsubj = 1 TO EXTENT(cur-clmn-numbr-delivsubj):
        if cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = varmvjbr-delivsubj THEN DO:
          cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = cur-clmn-numbr-delivsubj[varmvibr-delivsubj] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-delivsubj = source-column + 1 to target-column:
    DO varmvibr-delivsubj = 1 TO EXTENT(cur-clmn-numbr-delivsubj):
      if cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = varmvjbr-delivsubj THEN DO:
        cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = cur-clmn-numbr-delivsubj[varmvibr-delivsubj] - 1.
      END.
  ...
  END.
  DO varmvibr-delivsubj = 1 TO EXTENT(cur-clmn-numbr-delivsubj):
    if cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = -1 THEN cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-delivsubj:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-delivsubj = 1 TO EXTENT(cur-clmn-numbr-delivsubj):
    if cur-clmn-numbr-delivsubj[varmvibr-delivsubj] = cur-clmn-loc THEN move-elementbr-delivsubj = varmvibr-delivsubj.
  END.
  RUN re-move-clmnbr-delivsubj (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-delivsubj:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-delivsubj = 1 to EXTENT(cur-clmn-numbr-delivsubj):
    RUN re-move-clmnbr-delivsubj (cur-clmn-numbr-delivsubj[varmvlbr-delivsubj], varmvlbr-delivsubj).
  END.
  RUN start-mv-clmnbr-delivsubj.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  ENABLE b-quit B-mark B-sel B-Help br-delivsubj BR-changes mark-num
      WITH FRAME Dialog-Frame.
  VIEW FRAME Dialog-Frame.
  OPEN QUERY br-delivsubj FOR EACH X_c-delivery-subject NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
END PROCEDURE.
PROCEDURE OpenBr :
OPEN QUERY br-delivsubj FOR EACH X_c-delivery-subject NO-LOCK where
                               X_c-delivery-subject.deliv-subj-code = p-deliv-subj-code
                           AND X_c-delivery-subject.corr-user-db-num = 0
                              INDEXED-REPOSITION.
APPLY "VALUE-CHANGED" TO br-delivsubj in frame Dialog-Frame.
APPLY "ENTRY" TO br-delivsubj.
END PROCEDURE.
PROCEDURE proc-br-delivsubj :
...
for each temp-changes:
    delete temp-changes.
END.
if not available X_c-delivery-subject then do:
...
v-label-param =
 ...
 + "deliv-subj-code" + chr(4) + "Внутр.код" + chr(4) + "" + chr(8)
 + "deliv-subj-name" + chr(4) + "Название субъекта доставки" + chr(4) + "" + chr(8)
 + "reg-code" + chr(4) + "Код региона" + chr(4) + "" + chr(8)
 + "sts" + chr(4) + "Статус" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-delivery-subject:handle
                                            ,input  'delivery-subject':U
                                            ,input  "des,deliv-subj-code,deliv-subj-name,reg-code,sts"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
