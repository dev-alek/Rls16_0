...
define variable v-rec-id as recid no-undo .
  run ref/gr-supra.w (input parparentproc,input 'ДОБАВЛЕНИЕ':U , input-output v-rec-id) .
  OPEN QUERY BROWSE-1grp FOR EACH buf_sum-group WHERE ( r-status = 2 OR buf_sum-group.stts =  r-status ) and   ( r-find = no or (buf_sum-group.sgr-db-num = find-db and                         buf_sum-group.sgr-id = find-id ))  .
...
if v-cntxt-db-num <> 0 then do :
  ...
  OPEN QUERY BROWSE-2cli FOR EACH buf_sum-in-sum-group OF buf_sum-group      WHERE ( r-status = 2 OR buf_sum-in-sum-group.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.
...
if v-cntxt-db-num <> 0 then do :
  ...
  run ref/gr-supra.w (input parparentproc,input 'ИЗМЕНЕНИЕ':U , input-output v-rec-id) .
  OPEN QUERY BROWSE-1grp FOR EACH buf_sum-group WHERE ( r-status = 2 OR buf_sum-group.stts =  r-status ) and   ( r-find = no or (buf_sum-group.sgr-db-num = find-db and                         buf_sum-group.sgr-id = find-id ))  .
  reposition BROWSE-1grp to recid v-rec-id no-error .
  apply "VALUE-CHANGED" to browse-1grp in frame Dialog-Frame.
END.
ON CHOOSE OF B-chg2 IN FRAME Dialog-Frame
DO:
   define variable g#log as logical   no-undo .
...
assign
  v-str-recid12 = trim( string( recid( buf_sum-group ) , "->>>>>>>>>>>9":U ) )
...
assign
  v-str-recid14 = trim( string( recid( buf_sum-in-sum-group ) , "->>>>>>>>>>>9":U ) )
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1grp FOR EACH buf_sum-group WHERE ( r-status = 2 OR buf_sum-group.stts =  r-status ) and   ( r-find = no or (buf_sum-group.sgr-db-num = find-db and                         buf_sum-group.sgr-id = find-id ))  .    OPEN QUERY BROWSE-2cli FOR EACH buf_sum-in-sum-group OF buf_sum-group      WHERE ( r-status = 2 OR buf_sum-in-sum-group.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
