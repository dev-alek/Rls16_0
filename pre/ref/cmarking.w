...
DEFINE BUFFER X_c-obj-hist FOR c-marking.
DEFINE BUFFER find_c-obj-hist FOR c-marking.
...
DEFINE BROWSE br-obj-hist
  QUERY br-obj-hist NO-LOCK DISPLAY
      ...
      X_c-obj-hist.mark COLUMN-LABEL 'Марка' FORMAT 'X(50)':U WIDTH 35
...
define input  parameter p-db-num like c-marking.corr-user-db-num no-undo .
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-6-27 = if sort-phrase-27 = ''
                           then
        (
        " " + " BY X_c-obj-hist.corr-date DESC BY X_c-obj-hist.corr-time DESC BY X_c-obj-hist.mark " +
          " " + sort-column-phrase +
        " " + " BY X_c-obj-hist.corr-date DESC BY X_c-obj-hist.corr-time DESC BY X_c-obj-hist.mark "
        )
                           else
  ...
  :
    if query br-obj-hist:handle:get-buffer-handle(1) = (buffer X_c-obj-hist:handle) then do:
      ...
      parameter-5-27 = " BY X_c-obj-hist.corr-date DESC BY X_c-obj-hist.corr-time DESC BY X_c-obj-hist.mark "
    .
...
define input parameter p-next as logical no-undo.
define input parameter p-user like c-marking.corr-user-name no-undo.
assign
sch-corr-date = ?.
display
0 @ sch-db-num
...
procedure getMarking:
  ...
  :
    ...
    v-label-param =
          "sts"   + chr(4) + "Статус" + chr(4) + "getStsName" + chr(8)
        + "last-change" + chr(4) + "Изменен" + chr(4) + "" + chr(8)
        + "online-result" + chr(4) + "Результат ГИС МТ" + chr(4) + "getOnlineResultName".
    run proc-full-temp-changes in this-procedure (
                                                 input current_c-marking.action = integer('1':U)
                                                ,input current_c-marking.action = integer('99':U)
                                                ,input  buffer current_c-marking:handle
                                                ,input  "marking"
                                                ,input  "sts,last-change,online-result"
                                                ,input  v-label-param).
  end.
end procedure.
procedure getMarkingAttr:
  ...
  :
    ...
    if not avail current_c-marking then do:
        ...
        define variable v-label-param as character no-undo .
    v-label-param =
          "attr-value"   + chr(4) + getAttributeName(current_c-marking.attr-code) + chr(4)
