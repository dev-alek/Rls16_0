...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  (input  bufp_dis-rule.obj-type
  ,input  bufp_dis-rule.obj-code
  ,output v-obj-db-num
  )  .
  end.
  ...
  on error undo _main, return error:
    if v-is-remote-dbs then do:
      ...
      assign
      v-param = string(buf_dis-rule.rule-num) + chr(4) +
                string(buf_dis-rule.sts) + chr(4) +
                v-list-db
      .
      assign
...
define buffer buf_clients for ub.clients.
  ...
  :
    if not (buf_dis-rule.obj-type = '':U
      ...
      assign
        p-list-db = string(0) + chr(44) + string(buf_Clients.db-num)
      .
    end.
    else do:
      assign
