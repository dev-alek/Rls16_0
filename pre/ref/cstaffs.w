...
assign
  v-str-recid11 = trim( string( recid( X_c-staff ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry11 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-cstaff in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-cstaff in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-cstaff :handle
      .
...
do:
  v-doc-rec = recid(X_c-staff). run OpenBr in this-procedure. reposition br-cstaff to recid v-doc-rec no-error. v-doc-rec = ?.              apply 'value-changed' to br-cstaff.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-cstaff   as character no-undo .
def var sort-clmnbr-cstaff    as handle    no-undo .
def var cur-clmnbr-cstaff     as handle    no-undo .
def var cur-clmn-locbr-cstaff as integer   no-undo .
def var re-querybr-cstaff     as logical   initial no no-undo .
on start-search, ctrl-o of br-cstaff in frame Dialog-Frame do:
   run sort-brbr-cstaff
...
PROCEDURE sort-brbr-cstaff :
  define input parameter p-recid as recid no-undo .
  if re-querybr-cstaff = no then do:
    assign
       cur-clmnbr-cstaff = br-cstaff:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-cstaff <> ? then sort-clmnbr-cstaff:column-fgcolor = 0.
    if cur-clmnbr-cstaff = sort-clmnbr-cstaff then do:
      assign
         sort-labelbr-cstaff = ""
         sort-clmnbr-cstaff = ?
      .
       ...
       assign
         sort-labelbr-cstaff = cur-clmnbr-cstaff:label
         sort-clmnbr-cstaff  = cur-clmnbr-cstaff
         sort-clmnbr-cstaff:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-cstaff = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-cstaff:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-cstaff then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-cstaff = cur-clmn-locbr-cstaff + 1
    .
  end.
  case sort-labelbr-cstaff:
        when X_c-staff.corr-date:label in browse br-cstaff then DO:   assign     sort-column-name = "X_c-staff.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-cstaff') then do:
          run mv-brw-defaultbr-cstaff.
        end.
      if sort-labelbr-cstaff <> "" then do:
        assign
          cur-clmnbr-cstaff:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-cstaff = ?
      .
    end.
  end case.
    if cur-clmn-locbr-cstaff <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-cstaff') then do:
        run ch-clmnbr-cstaff in this-procedure (cur-clmn-locbr-cstaff).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-cstaff to recid p-recid no-error.
    apply "value-changed" to br-cstaff in frame Dialog-Frame.
  end.
  apply "entry" to br-cstaff in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-cstaff:
if cur-clmnbr-cstaff = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-cstaff = yes.
   run sort-brbr-cstaff
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
...
DEF VAR cur-clmn-numbr-cstaff as INT EXTENT 6 no-undo.
DEF VAR varmvibr-cstaff       as INT no-undo.
DEF VAR varmvjbr-cstaff       as INT no-undo.
DEF VAR varmvkbr-cstaff       as INT no-undo.
DEF VAR varmvlbr-cstaff       as INT no-undo.
DEF VAR move-elementbr-cstaff as INT no-undo.
def var jjbr-cstaff           as int no-undo.
do varmvibr-cstaff = 1 to EXTENT(cur-clmn-numbr-cstaff):
  ASSIGN cur-clmn-numbr-cstaff[varmvibr-cstaff] = varmvibr-cstaff.
END.
RUN start-mv-clmnbr-cstaff.
PROCEDURE start-mv-clmnbr-cstaff:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-cstaff = NUM-ENTRIES('1,2,3,4,5,6') TO 1 BY -1:
     RUN re-move-clmnbr-cstaff ( cur-clmn-numbr-cstaff[INTEGER(ENTRY (jjbr-cstaff, '1,2,3,4,5,6'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-cstaff do:
  RUN re-move-clmnbr-cstaff ( 1, 6).
END.
ON ctrl-cursor-left OF BROWSE br-cstaff do:
  RUN re-move-clmnbr-cstaff (6, 1).
END.
PROCEDURE re-move-clmnbr-cstaff:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-cstaff = 1 TO EXTENT(cur-clmn-numbr-cstaff):
    if cur-clmn-numbr-cstaff[varmvibr-cstaff] = source-column THEN cur-clmn-numbr-cstaff[varmvibr-cstaff] = -1.
  END.
  if br-cstaff:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-cstaff = source-column - 1 to target-column BY -1:
    DO varmvibr-cstaff = 1 TO EXTENT(cur-clmn-numbr-cstaff):
        if cur-clmn-numbr-cstaff[varmvibr-cstaff] = varmvjbr-cstaff THEN DO:
          cur-clmn-numbr-cstaff[varmvibr-cstaff] = cur-clmn-numbr-cstaff[varmvibr-cstaff] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-cstaff = source-column + 1 to target-column:
    DO varmvibr-cstaff = 1 TO EXTENT(cur-clmn-numbr-cstaff):
      if cur-clmn-numbr-cstaff[varmvibr-cstaff] = varmvjbr-cstaff THEN DO:
        cur-clmn-numbr-cstaff[varmvibr-cstaff] = cur-clmn-numbr-cstaff[varmvibr-cstaff] - 1.
      END.
  ...
  END.
  DO varmvibr-cstaff = 1 TO EXTENT(cur-clmn-numbr-cstaff):
    if cur-clmn-numbr-cstaff[varmvibr-cstaff] = -1 THEN cur-clmn-numbr-cstaff[varmvibr-cstaff] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-cstaff:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-cstaff = 1 TO EXTENT(cur-clmn-numbr-cstaff):
    if cur-clmn-numbr-cstaff[varmvibr-cstaff] = cur-clmn-loc THEN move-elementbr-cstaff = varmvibr-cstaff.
  END.
  RUN re-move-clmnbr-cstaff (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-cstaff:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-cstaff = 1 to EXTENT(cur-clmn-numbr-cstaff):
    RUN re-move-clmnbr-cstaff (cur-clmn-numbr-cstaff[varmvlbr-cstaff], varmvlbr-cstaff).
  END.
  RUN start-mv-clmnbr-cstaff.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-cstaff FOR EACH X_c-staff NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
