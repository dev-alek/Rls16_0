...
DEFINE BROWSE BR-dis-dct-rule
  QUERY BR-dis-dct-rule NO-LOCK DISPLAY
      mark-string(recid(X_dis-dct-rule), v-rid-list) COLUMN-LABEL '*' FORMAT "X(1)"
X_dis-card-type.emitent-host-code COLUMN-LABEL 'Эмитент' format ">>>>9"
X_dis-card-type.type COLUMN-LABEL 'Тип'
X_dis-dct-rule.templ-rl-root COLUMN-LABEL 'Тип правила' FORMAT ">>>>>>>>9"
cd-type-name(X_dis-dct-rule.pos-type) COLUMN-LABEL 'Место использ.' FORMAT "X(10)"
X_dis-dct-rule.obj-type COLUMN-LABEL 'Тип объ' FORMAT "X(3)"
X_dis-dct-rule.obj-code COLUMN-LABEL 'Код объ' FORMAT ">>>>9"
dis-dct-rule-name(X_dis-dct-rule.discnt-role) COLUMN-LABEL 'Тип скидки' FORMAT "X(70)"
X_dis-dct-rule.rule-num COLUMN-LABEL '№ правила' FORMAT ">>>>>>>>9"
X_dis-dct-rule.rl-root COLUMN-LABEL '№ корн.!правила' FORMAT ">>>>>>>>9"
...
DEFINE BROWSE BR-tt0-dis-dct-rule
  QUERY BR-tt0-dis-dct-rule NO-LOCK DISPLAY
      mark-string(recid(tt0-dis-dct-rule), v-rid-list) COLUMN-LABEL '*' FORMAT "X(1)"
tt0-dis-card-type.emitent-host-code COLUMN-LABEL 'Эмитент' format ">>>>9"
tt0-dis-card-type.type COLUMN-LABEL 'Тип'
tt0-dis-dct-rule.templ-rl-root COLUMN-LABEL 'Тип правила' FORMAT ">>>>>>>>9"
cd-type-name(tt0-dis-dct-rule.pos-type) COLUMN-LABEL 'Место использ.' FORMAT "X(10)"
tt0-dis-dct-rule.obj-type COLUMN-LABEL 'Тип объ' FORMAT "X(3)"
tt0-dis-dct-rule.obj-code COLUMN-LABEL 'Код объ' FORMAT ">>>>9"
dis-dct-rule-name(tt0-dis-dct-rule.discnt-role) COLUMN-LABEL 'Тип скидки' FORMAT "X(70)"
tt0-dis-dct-rule.rule-num COLUMN-LABEL '№ правила' FORMAT ">>>>>>>>9"
tt0-dis-dct-rule.rl-root COLUMN-LABEL '№ корн.!правила' FORMAT ">>>>>>>>9"
    WITH NO-ROW-MARKERS SEPARATORS SIZE 98 BY 20 FIT-LAST-COLUMN.
...
ON F5 OF FRAME Dialog-Frame
DO:
   if br-tt0-dis-dct-rule:visible in frame Dialog-Frame then do:
...
apply "entry" to br-dis-dct-rule in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-dis-dct-rule :handle
      .
...
def var sort-labelbr-dis-dct-rule   as character no-undo .
def var sort-clmnbr-dis-dct-rule    as handle    no-undo .
def var cur-clmnbr-dis-dct-rule     as handle    no-undo .
def var cur-clmn-locbr-dis-dct-rule as integer   no-undo .
def var re-querybr-dis-dct-rule     as logical   initial no no-undo .
on start-search, ctrl-o of br-dis-dct-rule in frame Dialog-Frame do:
   run sort-brbr-dis-dct-rule
     (input (if available X_dis-dct-rule
             then recid(X_dis-dct-rule)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-dis-dct-rule :
  define input parameter p-recid as recid no-undo .
  if re-querybr-dis-dct-rule = no then do:
    assign
       cur-clmnbr-dis-dct-rule = br-dis-dct-rule:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-dis-dct-rule <> ? then sort-clmnbr-dis-dct-rule:column-fgcolor = 0.
    if cur-clmnbr-dis-dct-rule = sort-clmnbr-dis-dct-rule then do:
      assign
         sort-labelbr-dis-dct-rule = ""
         sort-clmnbr-dis-dct-rule = ?
      .
     end.
     else do:
       assign
         sort-labelbr-dis-dct-rule = cur-clmnbr-dis-dct-rule:label
         sort-clmnbr-dis-dct-rule  = cur-clmnbr-dis-dct-rule
         sort-clmnbr-dis-dct-rule:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-dis-dct-rule = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-dis-dct-rule:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-dis-dct-rule then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-dis-dct-rule = cur-clmn-locbr-dis-dct-rule + 1
    .
  end.
  case sort-labelbr-dis-dct-rule:
        when '*'  then DO:    assign       sort-column-name = "mark-string(recid(X_dis-dct-rule), v-rid-list)"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Эмитент'  then DO:    assign       sort-column-name = "X_dis-card-type.emitent-host-code"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Тип'  then DO:    assign       sort-column-name = "X_dis-card-type.type"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Тип правила'  then DO:    assign       sort-column-name = "X_dis-dct-rule.templ-rl-root"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Место использ.'  then DO:    assign       sort-column-name = "cd-type-name(X_dis-dct-rule.pos-type)"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Тип объ'  then DO:    assign       sort-column-name = "X_dis-dct-rule.obj-type"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Код объ'  then DO:    assign       sort-column-name = "X_dis-dct-rule.obj-code"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Тип скидки'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1dis-dct-rule-name&1, X_dis-dct-rule.discnt-role)', chr(34))     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when '№ правила'  then DO:    assign       sort-column-name = "X_dis-dct-rule.rule-num"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when '№ корн.!правила'  then DO:    assign       sort-column-name = "X_dis-dct-rule.rl-root"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input no).
      if sort-labelbr-dis-dct-rule <> "" then do:
        assign
          cur-clmnbr-dis-dct-rule:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-dis-dct-rule = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-dis-dct-rule to recid p-recid no-error.
    apply "value-changed" to br-dis-dct-rule in frame Dialog-Frame.
  end.
  apply "entry" to br-dis-dct-rule in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-dis-dct-rule:
if cur-clmnbr-dis-dct-rule = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input no).
end.
else do:
   assign re-querybr-dis-dct-rule = yes.
   run sort-brbr-dis-dct-rule
     (input (if available X_dis-dct-rule
             then recid(X_dis-dct-rule)
             else ?
   ...
   assign re-querybr-dis-dct-rule = no.
end.
end.
define variable vss-include-info15 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-dis-dct-rule :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
define variable vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  IF lookup(p-list-mode, ('все':U + chr(4) +
                          ...
                          "rule-num":U + chr(4) +
                          "rl-root":U + chr(4) +
                          'dis-card-type':U + chr(4) +
                           ('dis-card-type':U + chr(44) + "temp")) , chr(4)) = 0
  THEN DO:
     MESSAGE
     ...
     UNDO, RETURN ERROR.
  END.
  if p-list-mode = ('dis-card-type':U + chr(44) + "temp") then do:
    temp-mode = yes.
  end.
  ...
  IF LOOKUP( "discnt-role", p-list-mode) > 0 THEN DO:
    FIND FIRST par_dis-cfg-rule NO-LOCK WHERE
          ...
          AND par_dis-cfg-rule.TABLE-name = 'dis-dct-rule':U NO-ERROR.
    ...
    END.
  END.
  if lookup('dis-card-type':U, p-list-mode) > 0
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-dis-dct-rule FOR EACH X_dis-dct-rule NO-LOCK ,            FIRST X_dis-card-type NO-LOCK WHERE           X_dis-card-type.type = X_dis-dct-rule.type        AND X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code INDEXED-REPOSITION.    OPEN QUERY BR-tt0-dis-dct-rule FOR EACH tt0-dis-dct-rule NO-LOCK ,            FIRST tt0-dis-card-type NO-LOCK WHERE           tt0-dis-card-type.type = tt0-dis-dct-rule.type        AND tt0-dis-card-type.emitent-host-code = tt0-dis-dct-rule.emitent-host-code INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
define variable v-h as handle no-undo .
v-h = BR-dis-dct-rule:FIRST-COLUMN IN FRAME Dialog-Frame.
DO while valid-handle(v-h) :
  ...
  END.
END.
v-h = BR-tt0-dis-dct-rule:FIRST-COLUMN IN FRAME Dialog-Frame.
DO while valid-handle(v-h) :
  if v-h:LABEL = 'Тип скидки' then do:
    v-h:RESIZABLE = YES.
    leave.
...
if temp-mode then do:
    ...
    BR-tt0-dis-dct-rule
    with frame Dialog-Frame.
    hide br-dis-dct-rule in frame Dialog-Frame.
    VIEW FRAME Dialog-Frame.
    RUn Openbrtt in this-procedure ( input yes, input no, input '':U).
end.
else do:
    ...
    BR-dis-dct-rule
    with frame Dialog-Frame.
    hide br-tt0-dis-dct-rule in frame Dialog-Frame.
    VIEW FRAME Dialog-Frame.
    RUn Openbr in this-procedure ( input yes, input no, input '':U).
end.
END PROCEDURE.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-19 as character no-undo .
      ...
      parameter-4-19 =
        (
          if ("  " + " " + where-phrase-19) <> ""
          then "  " + " " + where-phrase-19
          else "true"
        )
      parameter-5-19 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-19 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  on error undo, leave
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      assign
      parameter-2-19 = (if p-find-next then "true":u else "false":u )
      parameter-4-19 =
        "where ":u + "  " + " ":u + where-phrase-19 + " ":u + p-find-condition + " " + ""
      parameter-5-19 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
                          ,input rowid(X_dis-dct-rule)
                          ,input logical(parameter-2-19)
                          ,input no-lock
                          ,input (buffer X_dis-dct-rule:handle)
                          ,input parameter-4-19
                          ,input parameter-5-19
                          ) no-error.
      .
      ...
      assign
      parameter-2-19 = (if p-find-next then "true":u else "false":u )
      parameter-3-19 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-19 =
        (
          if ("  " + " " + where-phrase-19) <> ""
          then "  " + " " + where-phrase-19
      ...
      parameter-5-19 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-19 = if sort-phrase-19 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-21 as character no-undo .
      ...
      parameter-4-21 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code " + " " + where-phrase-21) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3 ', chr(34), p-curr-obj-type, p-curr-obj-code) + " " + where-phrase-21
          else "true"
        )
      parameter-5-21 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-21 =
          (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code " + " " + where-phrase-21 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-21 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-21 =
        "where ":u +  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3 ', chr(34), p-curr-obj-type, p-curr-obj-code) + " ":u + where-phrase-21 + " ":u + p-find-condition + " " + ""
      parameter-5-21 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-21 = (if p-find-next then "true":u else "false":u )
      parameter-3-21 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-21 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code " + " " + where-phrase-21) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3 ', chr(34), p-curr-obj-type, p-curr-obj-code) + " " + where-phrase-21
      ...
      parameter-5-21 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-21 = if sort-phrase-21 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      ...
      parameter-4-23 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.pos-type = p-pos-type " + " " + where-phrase-23) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.pos-type = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-pos-type) + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-23 =
          (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.pos-type = p-pos-type " + " " + where-phrase-23 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.pos-type = p-pos-type
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-23 =
        "where ":u +  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.pos-type = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-pos-type) + " ":u + where-phrase-23 + " ":u + p-find-condition + " " + ""
      parameter-5-23 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-23 = (if p-find-next then "true":u else "false":u )
      parameter-3-23 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-23 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.pos-type = p-pos-type " + " " + where-phrase-23) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.pos-type = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-pos-type) + " " + where-phrase-23
      ...
      parameter-5-23 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-23 = if sort-phrase-23 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-25) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.templ-rl-root = &4 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.templ-rl-root = p-templ-rl-root
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-25 =
        "where ":u +  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.templ-rl-root = &4 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " ":u + where-phrase-25 + " ":u + p-find-condition + " " + ""
      parameter-5-25 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-25 = (if p-find-next then "true":u else "false":u )
      parameter-3-25 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-25 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-25) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.templ-rl-root = &4 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " " + where-phrase-25
      ...
      parameter-5-25 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-25 = if sort-phrase-25 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.discnt-role = p-discnt-role " + " " + where-phrase-27) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.discnt-role = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-27 =
          (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.discnt-role = p-discnt-role " + " " + where-phrase-27 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.discnt-role = p-discnt-role
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-27 =
        "where ":u +  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.discnt-role = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " ":u + where-phrase-27 + " ":u + p-find-condition + " " + ""
      parameter-5-27 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-27 =
        (
          if (" X_dis-dct-rule.obj-type = p-curr-obj-type and X_dis-dct-rule.obj-code = p-curr-obj-code                       AND X_dis-dct-rule.discnt-role = p-discnt-role " + " " + where-phrase-27) <> ""
          then  substitute('X_dis-dct-rule.obj-type = &1&2&1 and X_dis-dct-rule.obj-code = &3                       AND X_dis-dct-rule.discnt-role = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " " + where-phrase-27
      ...
      parameter-5-27 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-27 = if sort-phrase-27 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-29 as character no-undo .
      ...
      parameter-4-29 =
        (
          if (" X_dis-dct-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-29) <> ""
          then  substitute('X_dis-dct-rule.templ-rl-root = &1', p-templ-rl-root ) + " " + where-phrase-29
          else "true"
        )
      parameter-5-29 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-29 =
          (" X_dis-dct-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-29 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-29 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.templ-rl-root = p-templ-rl-root
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-29 =
        "where ":u +  substitute('X_dis-dct-rule.templ-rl-root = &1', p-templ-rl-root ) + " ":u + where-phrase-29 + " ":u + p-find-condition + " " + ""
      parameter-5-29 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-29 = (if p-find-next then "true":u else "false":u )
      parameter-3-29 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-29 =
        (
          if (" X_dis-dct-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-29) <> ""
          then  substitute('X_dis-dct-rule.templ-rl-root = &1', p-templ-rl-root ) + " " + where-phrase-29
      ...
      parameter-5-29 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-29 = if sort-phrase-29 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if (" X_dis-dct-rule.rule-num = p-rule-num " + " " + where-phrase-31) <> ""
          then  substitute('X_dis-dct-rule.rule-num = &1', p-rule-num ) + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-31 =
          (" X_dis-dct-rule.rule-num = p-rule-num " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.rule-num = p-rule-num
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute('X_dis-dct-rule.rule-num = &1', p-rule-num ) + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-31 = (if p-find-next then "true":u else "false":u )
      parameter-3-31 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-31 =
        (
          if (" X_dis-dct-rule.rule-num = p-rule-num " + " " + where-phrase-31) <> ""
          then  substitute('X_dis-dct-rule.rule-num = &1', p-rule-num ) + " " + where-phrase-31
      ...
      parameter-5-31 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-31 = if sort-phrase-31 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-33 as character no-undo .
      ...
      parameter-4-33 =
        (
          if (" X_dis-dct-rule.rl-root = p-rule-num " + " " + where-phrase-33) <> ""
          then  substitute('X_dis-dct-rule.rl-root = &1', p-rule-num ) + " " + where-phrase-33
          else "true"
        )
      parameter-5-33 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-33 =
          (" X_dis-dct-rule.rl-root = p-rule-num " + " " + where-phrase-33 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-33 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.rl-root = p-rule-num
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-33 =
        "where ":u +  substitute('X_dis-dct-rule.rl-root = &1', p-rule-num ) + " ":u + where-phrase-33 + " ":u + p-find-condition + " " + ""
      parameter-5-33 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-33 = (if p-find-next then "true":u else "false":u )
      parameter-3-33 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-33 =
        (
          if (" X_dis-dct-rule.rl-root = p-rule-num " + " " + where-phrase-33) <> ""
          then  substitute('X_dis-dct-rule.rl-root = &1', p-rule-num ) + " " + where-phrase-33
      ...
      parameter-5-33 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-33 = if sort-phrase-33 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-35 as character no-undo .
      ...
      parameter-4-35 =
        (
          if (" X_dis-dct-rule.pos-type = p-pos-type " + " " + where-phrase-35) <> ""
          then  substitute('X_dis-dct-rule.pos-type = &1&2&1', chr(34), p-pos-type ) + " " + where-phrase-35
          else "true"
        )
      parameter-5-35 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-35 =
          (" X_dis-dct-rule.pos-type = p-pos-type " + " " + where-phrase-35 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-35 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.pos-type = p-pos-type
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-35 =
        "where ":u +  substitute('X_dis-dct-rule.pos-type = &1&2&1', chr(34), p-pos-type ) + " ":u + where-phrase-35 + " ":u + p-find-condition + " " + ""
      parameter-5-35 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-35 = (if p-find-next then "true":u else "false":u )
      parameter-3-35 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-35 =
        (
          if (" X_dis-dct-rule.pos-type = p-pos-type " + " " + where-phrase-35) <> ""
          then  substitute('X_dis-dct-rule.pos-type = &1&2&1', chr(34), p-pos-type ) + " " + where-phrase-35
      ...
      parameter-5-35 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-35 = if sort-phrase-35 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-37 as character no-undo .
      ...
      parameter-4-37 =
        (
          if (" X_dis-dct-rule.type = p-type and X_dis-dct-rule.emitent-host-code = p-emitent-host-code " + " " + where-phrase-37) <> ""
          then  substitute('X_dis-dct-rule.type = &1&2&1 and X_dis-dct-rule.emitent-host-code = &3 ', chr(34), p-type, p-emitent-host-code) + " " + where-phrase-37
          else "true"
        )
      parameter-5-37 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-37 =
          (" X_dis-dct-rule.type = p-type and X_dis-dct-rule.emitent-host-code = p-emitent-host-code " + " " + where-phrase-37 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-37 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.type = p-type and X_dis-dct-rule.emitent-host-code = p-emitent-host-code
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-37 =
        "where ":u +  substitute('X_dis-dct-rule.type = &1&2&1 and X_dis-dct-rule.emitent-host-code = &3 ', chr(34), p-type, p-emitent-host-code) + " ":u + where-phrase-37 + " ":u + p-find-condition + " " + ""
      parameter-5-37 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-37 = (if p-find-next then "true":u else "false":u )
      parameter-3-37 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-37 =
        (
          if (" X_dis-dct-rule.type = p-type and X_dis-dct-rule.emitent-host-code = p-emitent-host-code " + " " + where-phrase-37) <> ""
          then  substitute('X_dis-dct-rule.type = &1&2&1 and X_dis-dct-rule.emitent-host-code = &3 ', chr(34), p-type, p-emitent-host-code) + " " + where-phrase-37
      ...
      parameter-5-37 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-37 = if sort-phrase-37 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-39 as character no-undo .
      ...
      parameter-4-39 =
        (
          if (" X_dis-dct-rule.discnt-role = p-discnt-role " + " " + where-phrase-39) <> ""
          then  substitute('X_dis-dct-rule.discnt-role = &1&2&1', chr(34), p-discnt-role ) + " " + where-phrase-39
          else "true"
        )
      parameter-5-39 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-39 =
          (" X_dis-dct-rule.discnt-role = p-discnt-role " + " " + where-phrase-39 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-dct-rule:HANDLE
  ...
  if l-filter-open-39 = false then do:
    OPEN QUERY br-dis-dct-rule FOR EACH X_dis-dct-rule no-lock
      where  X_dis-dct-rule.discnt-role = p-discnt-role
    , FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-dct-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-dct-rule:handle) then do:
      ...
      parameter-4-39 =
        "where ":u +  substitute('X_dis-dct-rule.discnt-role = &1&2&1', chr(34), p-discnt-role ) + " ":u + where-phrase-39 + " ":u + p-find-condition + " " + ""
      parameter-5-39 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-39 = (if p-find-next then "true":u else "false":u )
      parameter-3-39 =  "FOR EACH X_dis-dct-rule no-lock"
      parameter-4-39 =
        (
          if (" X_dis-dct-rule.discnt-role = p-discnt-role " + " " + where-phrase-39) <> ""
          then  substitute('X_dis-dct-rule.discnt-role = &1&2&1', chr(34), p-discnt-role ) + " " + where-phrase-39
      ...
      parameter-5-39 = (" " + "" + " " + ", FIRST X_dis-card-type no-lock where X_dis-card-type.type = X_dis-dct-rule.type and                                                                      X_dis-card-type.emitent-host-code = X_dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-39 = if sort-phrase-39 = ''
                           then
        (
        " " + "  " +
...
filter-point = filter-point0 + p-list-mode.
CASE p-list-mode :
  WHEN 'dis-card-type':U + chr(44) + "temp" THEN DO:
      ASSIGN
      frame Dialog-Frame:TITLE =
                                    substitute("&1 применяемые типом ДК &2 эмитент &3"
                                    , title0
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-42 as character no-undo .
      ...
      parameter-4-42 =
        (
          if (" tt0-dis-dct-rule.type = p-type and tt0-dis-dct-rule.emitent-host-code = p-emitent-host-code " + " " + where-phrase-42) <> ""
          then  substitute('tt0-dis-dct-rule.type = &1&2&1 and tt0-dis-dct-rule.emitent-host-code = &3 ', chr(34), p-type, p-emitent-host-code) + " " + where-phrase-42
          else "true"
        )
      parameter-5-42 = (" " + "" + " " + ", FIRST tt0-dis-card-type no-lock where tt0-dis-card-type.type = tt0-dis-dct-rule.type and                                                                      tt0-dis-card-type.emitent-host-code = tt0-dis-dct-rule.emitent-host-code")
    ...
    :
      assign
        l-disable-where-42 =
          (" tt0-dis-dct-rule.type = p-type and tt0-dis-dct-rule.emitent-host-code = p-emitent-host-code " + " " + where-phrase-42 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-tt0-dis-dct-rule:handle
  ...
  if l-filter-open-42 = false then do:
    OPEN QUERY br-tt0-dis-dct-rule FOR EACH tt0-dis-dct-rule no-lock
      where  tt0-dis-dct-rule.type = p-type and tt0-dis-dct-rule.emitent-host-code = p-emitent-host-code
    , FIRST tt0-dis-card-type no-lock where tt0-dis-card-type.type = tt0-dis-dct-rule.type and                                                                      tt0-dis-card-type.emitent-host-code = tt0-dis-dct-rule.emitent-host-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  on error undo, leave
  :
    if QUERY br-tt0-dis-dct-rule:handle:get-buffer-handle(1) = (buffer tt0-dis-dct-rule:handle) then do:
      assign
      parameter-2-42 = (if p-find-next then "true":u else "false":u )
      parameter-4-42 =
        "where ":u +  substitute('tt0-dis-dct-rule.type = &1&2&1 and tt0-dis-dct-rule.emitent-host-code = &3 ', chr(34), p-type, p-emitent-host-code) + " ":u + where-phrase-42 + " ":u + p-find-condition + " " + ""
      parameter-5-42 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input QUERY br-tt0-dis-dct-rule:handle
                          ,input rowid(tt0-dis-dct-rule)
                          ,input logical(parameter-2-42)
                          ,input no-lock
                          ,input (buffer tt0-dis-dct-rule:handle)
                          ,input parameter-4-42
                          ,input parameter-5-42
                          ) no-error.
      .
      ...
      assign
      parameter-2-42 = (if p-find-next then "true":u else "false":u )
      parameter-3-42 =  "FOR EACH tt0-dis-dct-rule no-lock"
      parameter-4-42 =
        (
          if (" tt0-dis-dct-rule.type = p-type and tt0-dis-dct-rule.emitent-host-code = p-emitent-host-code " + " " + where-phrase-42) <> ""
          then  substitute('tt0-dis-dct-rule.type = &1&2&1 and tt0-dis-dct-rule.emitent-host-code = &3 ', chr(34), p-type, p-emitent-host-code) + " " + where-phrase-42
      ...
      parameter-5-42 = (" " + "" + " " + ", FIRST tt0-dis-card-type no-lock where tt0-dis-card-type.type = tt0-dis-dct-rule.type and                                                                      tt0-dis-card-type.emitent-host-code = tt0-dis-dct-rule.emitent-host-code" + " " + p-find-condition)
      parameter-6-42 = if sort-phrase-42 = ''
                           then
        (
        " " + "  " +
...
run waitfram-hide in this-procedure.
APPLY "VALUE-CHANGED" TO br-tt0-dis-dct-rule in frame Dialog-Frame.
...
assign
  v-str-recid44 = trim( string( recid( X_dis-dct-rule ) , "->>>>>>>>>>>9":U ) )
  v-num-entry44 = lookup( v-str-recid44 , v-rid-list )
.
if v-num-entry44 > 0 then do:
  assign
    entry( v-num-entry44, v-rid-list ) = "":U
    v-rid-list = trim( replace( v-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rid-list = v-rid-list + ( if v-rid-list = "":U then "":U else chr(44) ) + v-str-recid44
  .
end.
  loc#log = br-dis-dct-rule:refresh() IN FRAME Dialog-Frame .
...
X_dis-card-type.type COLUMN-LABEL 'Тип'
X_dis-dct-rule.templ-rl-root COLUMN-LABEL 'Тип правила' FORMAT ">>>>>>>>9"
v-pos-type COLUMN-LABEL 'Место использ.' FORMAT "X(10)"
X_dis-dct-rule.obj-type COLUMN-LABEL 'Тип объ' FORMAT "X(3)"
X_dis-dct-rule.obj-code COLUMN-LABEL 'Код объ' FORMAT ">>>>>>>>9"
...
DO WHILE available X_dis-dct-rule :
  ...
  mark-string(recid(X_dis-dct-rule), v-rid-list) @ v-mark
  X_dis-card-type.emitent-host-code
  X_dis-card-type.type
  X_dis-dct-rule.templ-rl-root
  cd-type-name(X_dis-dct-rule.pos-type) @ v-pos-type
  X_dis-dct-rule.obj-type
  X_dis-dct-rule.obj-code
  dis-dct-rule-name(X_dis-dct-rule.discnt-role) @ v-d-name
  X_dis-dct-rule.rule-num
  X_dis-dct-rule.rl-root
  with frame List .
...
RETURN entry (lookup (p-pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U).
END FUNCTION.
FUNCTION dis-dct-rule-name RETURNS CHARACTER
  ( input p-dis-dct-rule-code as character ) :
return entry (lookup (p-dis-dct-rule-code, 'calc-d-pcnt,calc-cash-d-pcnt,calc-categ,dis-tot-flag,def-categ,def-pcnt,def-cash-pcnt':u) + 1, ',' + 'Расчет %скидки ДК на товар,Расчет %скидки ДК на итог,Расчет категории ДК,Участие в итогах по ДК,Категория ДК по умолчанию,% скидки ДК на товар по умолч.,% скидки ДК на итог по умолч.':u).
END FUNCTION.
