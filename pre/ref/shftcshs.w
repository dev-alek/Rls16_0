...
DO:
    IF NOT l-shift-on AND v-shft > 0 THEN DO:
      assign
      sb-shift-cash.sale-date:read-only in browse BR-docs = NOT
              sb-shift-cash.sale-date:read-only in browse BR-docs .
      APPLY "ENTRY" to browse BR-docs.
      IF   sb-shift-cash.sale-date:read-only in browse BR-docs = FALSE THEN do:
          APPLY "ENTRY" to sb-shift-cash.sale-date in browse BR-docs.
      end.
      else do:
          .
      end.
...
do:
  ...
  if t-shift-cash.status_ <> 'зкр':U  then do:
    run cur-time in this-procedure ( output v-today, output v-time).
    assign
    t-shift-cash.status_ =  'зкр':U
    t-shift-cash.closed = 'режим-менеджера':U
...
do:
  assign v-rec = recid(sb-shift-cash). run openbr in this-procedure  ( input yes, input no, input '':U). reposition br-docs to recid v-rec no-error.               APPLY 'ENTRY' to br-docs. APPLY 'VALUE-CHANGED' to br-docs.
...
define variable vss-include-info19 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  IF (p-list-mode = 'касс':U
  OR p-list-mode = 'маг':U + 'продажа':U) THEN DO:
define variable vss-include-info20 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type20 as character no-undo .
define variable v-value-character20 as character no-undo .
...
define variable v-value-logical21 as INTEGER no-undo .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  END.
  assign
  sb-shift-cash.sale-date:read-only in browse BR-docs = true.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-docs FOR EACH sb-shift-cash NO-LOCK.
END PROCEDURE.
PROCEDURE MyEnable :
...
CASE p-list-mode:
    when 'все':U then do:
        ASSIGN
        frame Dialog-Frame:TITLE = "ВСЕ СМЕНЫ ПО ВСЕМ КАССАМ "
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-26 as character no-undo .
      assign
      parameter-3-26 =
                              "FOR EACH sb-shift-cash"
      parameter-4-26 =
        (
          if (" " + " " + where-phrase-26) <> ""
          then " " + " " + where-phrase-26
          else "true"
        )
      parameter-5-26 = (" " + "" + " " + "")
  ...
  if l-filter-open-26 = false then do:
    OPEN QUERY br-docs FOR EACH sb-shift-cash
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when 'маг':U + 'продажа':U then do:
        FIND first ub.shop No-LOCK WHERE ub.shop.obj-code = shop-code No-ERROR.
        ASSIGN
        frame Dialog-Frame:TITLE = "СМЕНЫ по магазину N " +
                                            string(shop-code, ">>>>9") +
                                           " за " +
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-28 as character no-undo .
      ...
      parameter-4-28 =
        (
          if (" sb-shift-cash.obj-type = 'маг':U              AND sb-shift-cash.obj-code = shop-code              AND sb-shift-cash.sale-date = sale-date " + " " + where-phrase-28) <> ""
          then  substitute('sb-shift-cash.obj-type = &1&2&1              AND sb-shift-cash.obj-code = &3              AND sb-shift-cash.sale-date = &4 ', chr(34), 'маг':U, shop-code, sale-date) + " " + where-phrase-28
          else "true"
        )
      parameter-5-28 = (" " + "" + " " + "")
      parameter-6-28 = if sort-phrase-28 = ''
        ...
        " " + "  " +
          " " + sort-column-phrase +
        " " + " BY sb-shift-cash.shift-date Descending By sb-shift-cash.shift-num descending By sb-shift-cash.cash-num  "
        )
                           else
        (
        " " + "  " +
    ...
    :
      assign
        l-disable-where-28 =
          (" sb-shift-cash.obj-type = 'маг':U              AND sb-shift-cash.obj-code = shop-code              AND sb-shift-cash.sale-date = sale-date " + " " + where-phrase-28 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-docs:handle
  ...
  if l-filter-open-28 = false then do:
    OPEN QUERY br-docs FOR EACH sb-shift-cash
      where  sb-shift-cash.obj-type = 'маг':U              AND sb-shift-cash.obj-code = shop-code              AND sb-shift-cash.sale-date = sale-date
       BY sb-shift-cash.shift-date Descending By sb-shift-cash.shift-num descending By sb-shift-cash.cash-num
      indexed-reposition
  .
      assign
  ...
  run waitfram-hide in this-procedure .
    ...
    when 'касс':U then do:
        ...
        find first ub.shop No-LOCK WHERE ub.shop.obj-code = shop-code No-ERROR.
        ASSIGN
        frame Dialog-Frame:TITLE = "СМЕНЫ ПО КАССЕ N " +
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-30 as character no-undo .
      ...
      parameter-4-30 =
        (
          if ("sb-shift-cash.obj-type = 'маг':U              AND sb-shift-cash.obj-code = shop-code              AND sb-shift-cash.cash-num = cashnum " + " " + where-phrase-30) <> ""
          then  substitute('sb-shift-cash.obj-type = &1&2&1              AND sb-shift-cash.obj-code = &3              AND sb-shift-cash.cash-num = &4 ', chr(34), 'маг':U, shop-code, cashnum) + " " + where-phrase-30
          else "true"
        )
      parameter-5-30 = (" " + "" + " " + "")
      parameter-6-30 = if sort-phrase-30 = ''
        ...
        " " + "  " +
          " " + sort-column-phrase +
        " " + " BY sb-shift-cash.shift-date Descending By sb-shift-cash.shift-num descending "
        )
                           else
        (
        " " + "  " +
    ...
    :
      assign
        l-disable-where-30 =
          ("sb-shift-cash.obj-type = 'маг':U              AND sb-shift-cash.obj-code = shop-code              AND sb-shift-cash.cash-num = cashnum " + " " + where-phrase-30 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-docs:handle
  ...
  if l-filter-open-30 = false then do:
    OPEN QUERY br-docs FOR EACH sb-shift-cash
      where sb-shift-cash.obj-type = 'маг':U              AND sb-shift-cash.obj-code = shop-code              AND sb-shift-cash.cash-num = cashnum
       BY sb-shift-cash.shift-date Descending By sb-shift-cash.shift-num descending
      indexed-reposition
  .
      assign
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curshift in g#library
  ...
  IF v-shift-date <> p-shift-date THEN DO:
    ASSIGN
    v-change-fields = v-change-fields + chr(44) + "shift-date".
  END.
  if v-shift-num <> p-shift-num THEN DO:
    ASSIGN
    v-change-fields = v-change-fields + chr(44) +  "shift-num".
  END.
  if v-shift-name <> p-shift-name THEN DO:
    ASSIGN
    v-change-fields = v-change-fields + chr(44) +  "shift-name".
  END.
  assign
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
    ...
    for each buf_chk-doc where
      ...
      and buf_chk-doc.obj-type = 'маг':U
