...
DEFINE BROWSE BR-c-cash-desk
  QUERY BR-c-cash-desk DISPLAY
      ...
      entry (lookup (X_c-cash-desk.subject, 'cash-desk,cash-desk-attr':U), 'Касса,Аттр.кассы':U) COLUMN-LABEL "Предмет изменений" FORMAT "X(12)":U
...
assign
  v-str-recid13 = trim( string( recid( X_c-cash-desk ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry13 > 0 then do:
 ...
 glog = br-c-cash-desk :refresh( )  in frame Dialog-Frame.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
    glog = br-c-cash-desk:select-next-row () in frame Dialog-Frame.
    apply "value-changed" to br-c-cash-desk in frame Dialog-Frame.
  end.
  ...
  apply "entry" to br-c-cash-desk in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-cash-desk :handle
      .
...
do:
  v-doc-rec = recid(X_c-cash-desk). run openbr in this-procedure. reposition br-c-cash-desk to recid v-doc-rec no-error.               APPLY 'ENTRY' to br-c-cash-desk. APPLY 'VALUE-CHANGED' to br-c-cash-desk.
    apply "VALUE-CHANGED" to BR-c-cash-desk.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-c-cash-desk :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
def var sort-labelbr-c-cash-desk   as character no-undo .
def var sort-clmnbr-c-cash-desk    as handle    no-undo .
def var cur-clmnbr-c-cash-desk     as handle    no-undo .
def var cur-clmn-locbr-c-cash-desk as integer   no-undo .
def var re-querybr-c-cash-desk     as logical   initial no no-undo .
on start-search, ctrl-o of br-c-cash-desk in frame Dialog-Frame do:
   run sort-brbr-c-cash-desk
     (input (if available X_c-cash-desk
             then recid(X_c-cash-desk)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-c-cash-desk :
  define input parameter p-recid as recid no-undo .
  if re-querybr-c-cash-desk = no then do:
    assign
       cur-clmnbr-c-cash-desk = br-c-cash-desk:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-c-cash-desk <> ? then sort-clmnbr-c-cash-desk:column-fgcolor = 0.
    if cur-clmnbr-c-cash-desk = sort-clmnbr-c-cash-desk then do:
      assign
         sort-labelbr-c-cash-desk = ""
         sort-clmnbr-c-cash-desk = ?
      .
     end.
     else do:
       assign
         sort-labelbr-c-cash-desk = cur-clmnbr-c-cash-desk:label
         sort-clmnbr-c-cash-desk  = cur-clmnbr-c-cash-desk
         sort-clmnbr-c-cash-desk:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-c-cash-desk = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-c-cash-desk:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-c-cash-desk then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-c-cash-desk = cur-clmn-locbr-c-cash-desk + 1
    .
  end.
  case sort-labelbr-c-cash-desk:
        when X_c-cash-desk.corr-date:label in browse br-c-cash-desk then DO:    assign       sort-column-name = "X_c-cash-desk.corr-date"     .     run OpenBr in this-procedure.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure.
      if sort-labelbr-c-cash-desk <> "" then do:
        assign
          cur-clmnbr-c-cash-desk:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-c-cash-desk = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-c-cash-desk to recid p-recid no-error.
    apply "value-changed" to br-c-cash-desk in frame Dialog-Frame.
  end.
  apply "entry" to br-c-cash-desk in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-c-cash-desk:
if cur-clmnbr-c-cash-desk = ? then do:
   run OpenBr in this-procedure.
end.
else do:
   assign re-querybr-c-cash-desk = yes.
   run sort-brbr-c-cash-desk
     (input (if available X_c-cash-desk
             then recid(X_c-cash-desk)
             else ?
   ...
   assign re-querybr-c-cash-desk = no.
end.
end.
...
CASE parref-mode:
    when 'все':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Справочник истории касс"
...
run gbl/flt-get.p
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-cash-desk:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-cash-desk FOR EACH X_c-cash-desk
      where  TRUE
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when "one":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории касс: БД &1 &2&3 &4 касса N&5",
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_c-cash-desk.obj-code = parobj-code AND X_c-cash-desk.db-num = pardb-num AND                             X_c-cash-desk.pos-type = parpos-type and X_c-cash-desk.cash-num = parcash-num" + " " + where-phrase-25) <> ""
          then  substitute('X_c-cash-desk.obj-code = &1 AND X_c-cash-desk.db-num = &2 AND                             X_c-cash-desk.pos-type = &3&4&3 and X_c-cash-desk.cash-num = &5', parobj-code, pardb-num, chr(34), parpos-type, parcash-num) + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_c-cash-desk.obj-code = parobj-code AND X_c-cash-desk.db-num = pardb-num AND                             X_c-cash-desk.pos-type = parpos-type and X_c-cash-desk.cash-num = parcash-num" + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-cash-desk:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-c-cash-desk FOR EACH X_c-cash-desk
      where  X_c-cash-desk.obj-code = parobj-code AND X_c-cash-desk.db-num = pardb-num AND                             X_c-cash-desk.pos-type = parpos-type and X_c-cash-desk.cash-num = parcash-num
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "one-subject":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории касс: БД &1 &2&3 &4 касса N&5 &6"
                                                       , pardb-num
                                                       , parobj-type
                                                       , parobj-code
                                                       , parpos-type
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          if (" X_c-cash-desk.obj-code = parobj-code AND X_c-cash-desk.db-num = pardb-num AND                             X_c-cash-desk.pos-type = parpos-type and X_c-cash-desk.cash-num = parcash-num                          AND X_c-cash-desk.subject = p-subject " + " " + where-phrase-27) <> ""
          then  substitute('X_c-cash-desk.obj-code = &1 AND X_c-cash-desk.db-num = &2 AND                             X_c-cash-desk.pos-type = &3&4&3 and X_c-cash-desk.cash-num = &5                          AND X_c-cash-desk.subject = &3&6&3 ', parobj-code, pardb-num, chr(34), parpos-type, parcash-num, p-subject )  + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-27 =
          (" X_c-cash-desk.obj-code = parobj-code AND X_c-cash-desk.db-num = pardb-num AND                             X_c-cash-desk.pos-type = parpos-type and X_c-cash-desk.cash-num = parcash-num                          AND X_c-cash-desk.subject = p-subject " + " " + where-phrase-27 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-cash-desk:handle
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-c-cash-desk FOR EACH X_c-cash-desk
      where  X_c-cash-desk.obj-code = parobj-code AND X_c-cash-desk.db-num = pardb-num AND                             X_c-cash-desk.pos-type = parpos-type and X_c-cash-desk.cash-num = parcash-num                          AND X_c-cash-desk.subject = p-subject
  .
      assign
        l-query-was-opened = true
      .
