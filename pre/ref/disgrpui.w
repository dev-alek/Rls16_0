...
v-mode =  ('dis-grp-rule':U + "=" + p-discnt-role).
run ref/dis-ruls.w (
            input parparentproc
            ,input p-host-code
            ,input p-obj-type
...
:
  if v-rid-list <> "":U then do:
    ...
    find first buf_dis-cfg-rule no-lock where
        ...
        and buf_Dis-cfg-rule.table-name = 'dis-grp-rule':U
...
entry (lookup (temp-grpdisc.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U) FORMAT "X(20)":U COLUMN-LABEL "Место использ."
...
define variable v-deleted as logical no-undo .
  ...
  substitute("Вы уверены, что хотите удалить скидку &1 (место использ. &2)&7на фирме &3 &4&5 для группы &6?"
           ,v-rule-label
           ,entry (lookup (temp-grpdisc.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U)
...
DO:
    IF v-cntxt-db-num > 0
  AND (temp-grpdisc.obj-type = 'орг':U
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-dis-grp :handle
      .
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_grp-discount_global_work':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_grp-discount_firm_work':U
    ,input  'firm':U
    ,input  p-host-code
    ,input  '':U
    ,input  0
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_grp-discount_object_work':U
    ,input  'object':U
    ,input  p-host-code
    ,input  p-obj-type
    ,input  p-obj-code
...
PROCEDURE enable_UI :
  ...
  ENABLE b-quit b-ins b-lkp b-chg b-del b-help v-grp-name cd-grp-code
      WITH FRAME Dialog-Frame.
  VIEW FRAME Dialog-Frame.
  OPEN QUERY br-dis-grp FOR EACH temp-grpdisc NO-LOCK.
END PROCEDURE.
PROCEDURE init-proc :
define variable v-rule-label as character no-undo .
...
v-h = br-dis-grp:FIRST-COLUMN IN FRAME Dialog-Frame.
...
delete object v-tth15 no-error.
