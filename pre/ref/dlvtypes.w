...
DEFINE BROWSE br-delivtype
  QUERY br-delivtype NO-LOCK DISPLAY
      ...
      entry (lookup (STRING(X_delivery-type.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
...
if loc-doc-rec <> ? then do:
  ...
  if error-status:error then do:                           find first pos_delivery-type no-lock where                                   recid(pos_delivery-type) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ТИПА ДОСТАВКИ" skip                            string(if avail pos_delivery-type                                     then  substitute("Вн код типа доставки: &1"                                                     , pos_delivery-type.deliv-type-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
end.
apply "entry" to br-delivtype in frame Dialog-Frame.
apply "value-changed" to br-delivtype in frame Dialog-Frame.
END.
...
apply "value-changed" to br-delivtype in frame Dialog-Frame.
END.
ON CHOOSE OF B-condkeep IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE v-rid-list AS CHARACTER NO-UNDO.
...
DO:
  ...
  if error-status:error then return no-apply.
END.
ON CHOOSE OF B-delivsubj IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE v-rid-list AS CHARACTER NO-UNDO.
  define variable v-sts as integer no-undo init ?.
  IF NOT AVAILABLE X_delivery-type THEN RETURN no-apply.
...
assign
  v-str-recid10 = trim( string( recid( X_delivery-type ) , "->>>>>>>>>>>9":U ) )
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-delivtype :handle
      .
...
def var sort-labelbr-delivtype   as character no-undo .
def var sort-clmnbr-delivtype    as handle    no-undo .
def var cur-clmnbr-delivtype     as handle    no-undo .
def var cur-clmn-locbr-delivtype as integer   no-undo .
def var re-querybr-delivtype     as logical   initial no no-undo .
on start-search, ctrl-o of br-delivtype in frame Dialog-Frame do:
   run sort-brbr-delivtype
     (input (if available X_delivery-type
             then recid(X_delivery-type)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-delivtype :
  define input parameter p-recid as recid no-undo .
  if re-querybr-delivtype = no then do:
    assign
       cur-clmnbr-delivtype = br-delivtype:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-delivtype <> ? then sort-clmnbr-delivtype:column-fgcolor = 0.
    if cur-clmnbr-delivtype = sort-clmnbr-delivtype then do:
      assign
         sort-labelbr-delivtype = ""
         sort-clmnbr-delivtype = ?
      .
     end.
     else do:
       assign
         sort-labelbr-delivtype = cur-clmnbr-delivtype:label
         sort-clmnbr-delivtype  = cur-clmnbr-delivtype
         sort-clmnbr-delivtype:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-delivtype = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-delivtype:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-delivtype then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-delivtype = cur-clmn-locbr-delivtype + 1
    .
  end.
  case sort-labelbr-delivtype:
        when X_delivery-type.deliv-type-code:label in browse br-delivtype then DO:   assign     sort-column-name = "X_delivery-type.deliv-type-code"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-delivtype') then do:
          run mv-brw-defaultbr-delivtype.
        end.
      if sort-labelbr-delivtype <> "" then do:
        assign
          cur-clmnbr-delivtype:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-delivtype = ?
      .
    end.
  end case.
    if cur-clmn-locbr-delivtype <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-delivtype') then do:
        run ch-clmnbr-delivtype in this-procedure (cur-clmn-locbr-delivtype).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-delivtype to recid p-recid no-error.
    apply "value-changed" to br-delivtype in frame Dialog-Frame.
  end.
  apply "entry" to br-delivtype in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-delivtype:
if cur-clmnbr-delivtype = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-delivtype = yes.
   run sort-brbr-delivtype
     (input (if available X_delivery-type
             then recid(X_delivery-type)
             else ?
   ...
   assign re-querybr-delivtype = no.
end.
end.
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-delivtype :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
DEF VAR cur-clmn-numbr-delivtype as INT EXTENT 5 no-undo.
DEF VAR varmvibr-delivtype       as INT no-undo.
DEF VAR varmvjbr-delivtype       as INT no-undo.
DEF VAR varmvkbr-delivtype       as INT no-undo.
DEF VAR varmvlbr-delivtype       as INT no-undo.
DEF VAR move-elementbr-delivtype as INT no-undo.
def var jjbr-delivtype           as int no-undo.
do varmvibr-delivtype = 1 to EXTENT(cur-clmn-numbr-delivtype):
  ASSIGN cur-clmn-numbr-delivtype[varmvibr-delivtype] = varmvibr-delivtype.
END.
RUN start-mv-clmnbr-delivtype.
PROCEDURE start-mv-clmnbr-delivtype:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-delivtype = NUM-ENTRIES('1,2,3,4,5') TO 1 BY -1:
     RUN re-move-clmnbr-delivtype ( cur-clmn-numbr-delivtype[INTEGER(ENTRY (jjbr-delivtype, '1,2,3,4,5'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-delivtype do:
  RUN re-move-clmnbr-delivtype ( 1, 5).
END.
ON ctrl-cursor-left OF BROWSE br-delivtype do:
  RUN re-move-clmnbr-delivtype (5, 1).
END.
PROCEDURE re-move-clmnbr-delivtype:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-delivtype = 1 TO EXTENT(cur-clmn-numbr-delivtype):
    if cur-clmn-numbr-delivtype[varmvibr-delivtype] = source-column THEN cur-clmn-numbr-delivtype[varmvibr-delivtype] = -1.
  END.
  if br-delivtype:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-delivtype = source-column - 1 to target-column BY -1:
    DO varmvibr-delivtype = 1 TO EXTENT(cur-clmn-numbr-delivtype):
        if cur-clmn-numbr-delivtype[varmvibr-delivtype] = varmvjbr-delivtype THEN DO:
          cur-clmn-numbr-delivtype[varmvibr-delivtype] = cur-clmn-numbr-delivtype[varmvibr-delivtype] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-delivtype = source-column + 1 to target-column:
    DO varmvibr-delivtype = 1 TO EXTENT(cur-clmn-numbr-delivtype):
      if cur-clmn-numbr-delivtype[varmvibr-delivtype] = varmvjbr-delivtype THEN DO:
        cur-clmn-numbr-delivtype[varmvibr-delivtype] = cur-clmn-numbr-delivtype[varmvibr-delivtype] - 1.
      END.
  ...
  END.
  DO varmvibr-delivtype = 1 TO EXTENT(cur-clmn-numbr-delivtype):
    if cur-clmn-numbr-delivtype[varmvibr-delivtype] = -1 THEN cur-clmn-numbr-delivtype[varmvibr-delivtype] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-delivtype:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-delivtype = 1 TO EXTENT(cur-clmn-numbr-delivtype):
    if cur-clmn-numbr-delivtype[varmvibr-delivtype] = cur-clmn-loc THEN move-elementbr-delivtype = varmvibr-delivtype.
  END.
  RUN re-move-clmnbr-delivtype (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-delivtype:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-delivtype = 1 to EXTENT(cur-clmn-numbr-delivtype):
    RUN re-move-clmnbr-delivtype (cur-clmn-numbr-delivtype[varmvlbr-delivtype], varmvlbr-delivtype).
  END.
  RUN start-mv-clmnbr-delivtype.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-delivtype FOR EACH X_delivery-type NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
if not loc#log then return error.
  ...
  if error-status:error then do:                           find first pos_delivery-type no-lock where                                   recid(pos_delivery-type) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ТИПА ДОСТАВКИ" skip                            string(if avail pos_delivery-type                                     then  substitute("Вн код типа доставки: &1"                                                     , pos_delivery-type.deliv-type-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
  if available X_delivery-type then do:
    loc#log = br-delivtype:select-focused-row( ) IN FRAME Dialog-Frame.
  end.
  apply "ENTRY" to br-delivtype.
end.
END PROCEDURE.
