...
DEFINE VARIABLE ii AS INTEGER NO-UNDO.
DEFINE VARIABLE v-ok AS LOGICAL NO-UNDO.
s-grp-name:LIST-ITEM-pairs IN FRAME Dialog-Frame = cpgrpnam.
...
ELSE do:
   ASSIGN
    v-dops = ENTRY(2, p-attr-value, chr(4))
    v-dopi = INTEGER(ENTRY(2, p-attr-value, chr(4)))
    NO-ERROR
    .
    IF ERROR-STATUS:ERROR  THEN DO:
       MESSAGE
...
END.
ASSIGN
p-attr-value = v-dop + chr(4) + STRING(f-grp-code)
p-ok = yes
.
END PROCEDURE.
