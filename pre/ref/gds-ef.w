...
assign
  v-str-recid12 = trim( string( recid( Buf_ext-classif ) , "->>>>>>>>>>>9":U ) )
  v-num-entry12 = lookup( v-str-recid12 , p-rid-list )
.
if v-num-entry12 > 0 then do:
  assign
    entry( v-num-entry12, p-rid-list ) = "":U
    p-rid-list = trim( replace( p-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    p-rid-list = p-rid-list + ( if p-rid-list = "":U then "":U else chr(44) ) + v-str-recid12
  .
end.
    loc#log = BROWSE-ext-goods:refresh() .
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = BROWSE-ext-goods:select-next-row ().
        apply "VALUE-CHANGED" to BROWSE-ext-goods in frame Dialog-Frame.
    end.
  ...
  apply "display" to BROWSE-ext-goods in frame Dialog-Frame.
END.
...
DO:
   IF  v-rid-list = ""
   or b-mark:sensitive in frame Dialog-Frame = no
   THEN DO:
...
PROCEDURE sort-brBROWSE-ext-goods :
  ...
  case sort-labelBROWSE-ext-goods:
        when '*'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1mark-string&1, recid(buf_ext-classif), &1&2&1)', chr(34), p-rid-list)     .     run OpenBr.   . END.
        when 'Артикул'  then DO:    assign       sort-column-name = "Buf_goods.artic"     .     run OpenBr.   . END.
        when 'Код'  then DO:    assign       sort-column-name = "Buf_goods.gds-code"     .     run OpenBr.   . END.
        when 'Название'  then DO:    assign       sort-column-name = "Buf_goods.gds-name"     .     run OpenBr.   . END.
        when 'Код EASYFUEL'  then DO:    assign       sort-column-name = "Buf_ext-classif.Key#_One"     .     run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
do:
  BROWSE-ext-goods :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  ...
  hide mark-num in frame Dialog-Frame .
  if v-doc-rec <> ? then
  reposition BROWSE-ext-goods to recid v-doc-rec no-error.
  apply "row-display" to BROWSE-ext-goods in frame Dialog-Frame.
  WAIT-FOR GO OF FRAME Dialog-Frame focus BROWSE-ext-goods.
END.
...
:
  ...
  if p-mode = 'ДОБАВЛЕНИЕ':U then do:
    run ref/extclas1.p ( INPUT 'ДОБАВЛЕНИЕ':U
                        ...
                        ,INPUT 'goods':U
                        ,INPUT 'exp-easyfuel-talon-gds-code':U
                        ,input (-1)
                        ,input p-accor-code
                        ,input 0
                        ,input 0
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-ext-goods FOR EACH Buf_ext-classif       WHERE Buf_ext-classif.classif-subject = 'goods':U         AND Buf_ext-classif.classif-name = 'exp-easyfuel-talon-gds-code':U NO-LOCK,               first Buf_goods where              Buf_goods.gds-code = int(entry(2,       ub.Buf_ext-classif.uniq-key-rec,        chr(3))) NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-21 as character no-undo .
      ...
      parameter-4-21 =
        (
          if ("  Buf_ext-classif.classif-subject = 'goods':U  AND                      Buf_ext-classif.classif-name = 'exp-easyfuel-talon-gds-code':U  " + " " + where-phrase-21) <> ""
          then substitute(' Buf_ext-classif.classif-subject = &1&2&1  AND                  Buf_ext-classif.classif-name = &1&2&1 '  , chr(34), 'goods':U ,  'exp-easyfuel-talon-gds-code':U) + " " + where-phrase-21
          else "true"
        )
      parameter-5-21 = (" " + "" + " " + substitute( ', first buf_goods no-lock where                                      buf_goods.gds-code = integer(entry(2,buf_ext-classif.uniq-key-rec,&1&2&1 )) '                                     , chr(34), chr(3) ))
    ...
    :
      assign
        l-disable-where-21 =
          ("  Buf_ext-classif.classif-subject = 'goods':U  AND                      Buf_ext-classif.classif-name = 'exp-easyfuel-talon-gds-code':U  " + " " + where-phrase-21 = "")
      .
  ...
  if l-filter-open-21 = false then do:
    OPEN QUERY BROWSE-ext-goods FOR EACH Buf_ext-classif
      where   Buf_ext-classif.classif-subject = 'goods':U  AND                      Buf_ext-classif.classif-name = 'exp-easyfuel-talon-gds-code':U
    , first buf_goods no-lock where                                    buf_goods.gds-code = integer(entry(2,buf_ext-classif.uniq-key-rec, chr(3)))
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query BROWSE-ext-goods:handle:get-buffer-handle(1) = (buffer Buf_ext-classif:handle) then do:
      ...
      parameter-4-21 =
        "where ":u + substitute(' Buf_ext-classif.classif-subject = &1&2&1  AND                  Buf_ext-classif.classif-name = &1&2&1 '  , chr(34), 'goods':U ,  'exp-easyfuel-talon-gds-code':U) + " ":u + where-phrase-21 + " ":u + p-find-condition + " " + ""
      parameter-5-21 = " "
    .
      ...
      assign
      parameter-2-21 = (if p-find-next then "true":u else "false":u )
      parameter-3-21 =  "FOR EACH Buf_ext-classif"
      parameter-4-21 =
        (
          if ("  Buf_ext-classif.classif-subject = 'goods':U  AND                      Buf_ext-classif.classif-name = 'exp-easyfuel-talon-gds-code':U  " + " " + where-phrase-21) <> ""
          then substitute(' Buf_ext-classif.classif-subject = &1&2&1  AND                  Buf_ext-classif.classif-name = &1&2&1 '  , chr(34), 'goods':U ,  'exp-easyfuel-talon-gds-code':U) + " " + where-phrase-21
      ...
      parameter-5-21 = (" " + "" + " " + substitute( ', first buf_goods no-lock where                                      buf_goods.gds-code = integer(entry(2,buf_ext-classif.uniq-key-rec,&1&2&1 )) '                                     , chr(34), chr(3) ) + " " + p-find-condition)
      parameter-6-21 = if sort-phrase-21 = ''
                           then
        (
        " " + " " +
...
:
  ...
  run openbr in this-procedure .
  REPOSITION BROWSE-ext-goods to recid loc-doc-rec No-error.
  if available buf_ext-classif then do:
    loc#log = BROWSE-ext-goods:select-focused-row( ) IN FRAME Dialog-Frame.
    loc#log = BROWSE-ext-goods:refresh() .
  end.
  APPLY "display" TO BROWSE-ext-goods.
end.
END PROCEDURE.
...
DO WHILE available buf_ext-classif :
  GET prev BROWSE-ext-goods.
END.
GET next BROWSE-ext-goods.
DO WHILE available buf_ext-classif :
...
define variable doc-rec as recid no-undo.
  ...
  find first  Buff_ext-classif no-lock where
          ...
          and Buff_ext-classif.classif-subject   = 'goods':U
          AND Buff_ext-classif.classif-name  = 'exp-easyfuel-talon-gds-code':U
          and can-find(first  buf_goods no-lock where
                              buf_goods.gds-code =  int(entry(2,Buff_ext-classif.uniq-key-rec, chr(3))) and
