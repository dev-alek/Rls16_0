...
assign
  v-str-recid10 = trim( string( recid( X_c-wth-place ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-wp in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-wp in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-wp :handle
      .
...
do:
  v-doc-rec = recid(X_wth-place). run openbr in this-procedure. reposition br-wp to recid v-doc-rec no-error.               apply 'value-changed' to br-wp.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-wp   as character no-undo .
def var sort-clmnbr-wp    as handle    no-undo .
def var cur-clmnbr-wp     as handle    no-undo .
def var cur-clmn-locbr-wp as integer   no-undo .
def var re-querybr-wp     as logical   initial no no-undo .
on start-search, ctrl-o of br-wp in frame Dialog-Frame do:
   run sort-brbr-wp
...
PROCEDURE sort-brbr-wp :
  define input parameter p-recid as recid no-undo .
  if re-querybr-wp = no then do:
    assign
       cur-clmnbr-wp = br-wp:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-wp <> ? then sort-clmnbr-wp:column-fgcolor = 0.
    if cur-clmnbr-wp = sort-clmnbr-wp then do:
      assign
         sort-labelbr-wp = ""
         sort-clmnbr-wp = ?
      .
       ...
       assign
         sort-labelbr-wp = cur-clmnbr-wp:label
         sort-clmnbr-wp  = cur-clmnbr-wp
         sort-clmnbr-wp:column-fgcolor = 4
       .
     end.
   end.
  assign
  ...
  column-handle = br-wp:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-wp then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-wp = cur-clmn-locbr-wp + 1
    .
  end.
  case sort-labelbr-wp:
        when X_c-wth-place.w-p-name:label in browse br-wp then DO:   assign     sort-column-name = "X_c-wth-place.w-p-name"   .   run OpenBr.   . END.
        when X_c-wth-place.corr-date:label in browse br-wp then DO:   assign     sort-column-name = "X_c-wth-place.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-wp') then do:
          run mv-brw-defaultbr-wp.
        end.
      if sort-labelbr-wp <> "" then do:
        assign
          cur-clmnbr-wp:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-wp = ?
    ...
    if cur-clmn-locbr-wp <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-wp') then do:
        run ch-clmnbr-wp in this-procedure (cur-clmn-locbr-wp).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-wp to recid p-recid no-error.
    apply "value-changed" to br-wp in frame Dialog-Frame.
  end.
  apply "entry" to br-wp in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-wp:
if cur-clmnbr-wp = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-wp = yes.
   run sort-brbr-wp
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
...
DEF VAR cur-clmn-numbr-wp as INT EXTENT 7 no-undo.
DEF VAR varmvibr-wp       as INT no-undo.
DEF VAR varmvjbr-wp       as INT no-undo.
DEF VAR varmvkbr-wp       as INT no-undo.
DEF VAR varmvlbr-wp       as INT no-undo.
DEF VAR move-elementbr-wp as INT no-undo.
def var jjbr-wp           as int no-undo.
do varmvibr-wp = 1 to EXTENT(cur-clmn-numbr-wp):
  ASSIGN cur-clmn-numbr-wp[varmvibr-wp] = varmvibr-wp.
END.
RUN start-mv-clmnbr-wp.
PROCEDURE start-mv-clmnbr-wp:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-wp = NUM-ENTRIES('1,2,3,4,5,6,7') TO 1 BY -1:
     RUN re-move-clmnbr-wp ( cur-clmn-numbr-wp[INTEGER(ENTRY (jjbr-wp, '1,2,3,4,5,6,7'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-wp do:
  RUN re-move-clmnbr-wp ( 1, 7).
END.
ON ctrl-cursor-left OF BROWSE br-wp do:
  RUN re-move-clmnbr-wp (7, 1).
END.
PROCEDURE re-move-clmnbr-wp:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-wp = 1 TO EXTENT(cur-clmn-numbr-wp):
    if cur-clmn-numbr-wp[varmvibr-wp] = source-column THEN cur-clmn-numbr-wp[varmvibr-wp] = -1.
  END.
  if br-wp:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-wp = source-column - 1 to target-column BY -1:
    DO varmvibr-wp = 1 TO EXTENT(cur-clmn-numbr-wp):
        if cur-clmn-numbr-wp[varmvibr-wp] = varmvjbr-wp THEN DO:
          cur-clmn-numbr-wp[varmvibr-wp] = cur-clmn-numbr-wp[varmvibr-wp] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-wp = source-column + 1 to target-column:
    DO varmvibr-wp = 1 TO EXTENT(cur-clmn-numbr-wp):
      if cur-clmn-numbr-wp[varmvibr-wp] = varmvjbr-wp THEN DO:
        cur-clmn-numbr-wp[varmvibr-wp] = cur-clmn-numbr-wp[varmvibr-wp] - 1.
      END.
  ...
  END.
  DO varmvibr-wp = 1 TO EXTENT(cur-clmn-numbr-wp):
    if cur-clmn-numbr-wp[varmvibr-wp] = -1 THEN cur-clmn-numbr-wp[varmvibr-wp] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-wp:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-wp = 1 TO EXTENT(cur-clmn-numbr-wp):
    if cur-clmn-numbr-wp[varmvibr-wp] = cur-clmn-loc THEN move-elementbr-wp = varmvibr-wp.
  END.
  RUN re-move-clmnbr-wp (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-wp:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-wp = 1 to EXTENT(cur-clmn-numbr-wp):
    RUN re-move-clmnbr-wp (cur-clmn-numbr-wp[varmvlbr-wp], varmvlbr-wp).
  END.
  RUN start-mv-clmnbr-wp.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-wp FOR EACH X_c-wth-place NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
  "w-p-code" + chr(4) + "Код МХ МЦ" + chr(4) + "" + chr(8)
 + "w-p-name" + chr(4) + "Название МХ МЦ" + chr(4) + "" + chr(8)
 + "status_" + chr(4) + "Статус" + chr(4) + "" + chr(8)
 + "PS" + chr(4) + "Примечания" + chr(4) + "" + chr(8)
 + "cash-desk" + chr(4) + "№ кассы" + chr(4) + "" + chr(8)
 + "main-cash-desk" + chr(4) + "Главная касса" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-wth-place:handle
                                            ,input  'wth-place':U
                                            ,input  "w-p-code,w-p-name,status_,PS,cash-desk,main-cash-desk"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
