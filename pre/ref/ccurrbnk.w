DEFINE BUFFER X_c-curr-bank FOR ub.c-curr-bank.
...
define variable vss-description AS CHAR NO-UNDO INIT "Список истории курса валюты ЦБ":U.
...
DEFINE QUERY br-ccur FOR
      X_c-curr-bank SCROLLING.
DEFINE QUERY BR-changes FOR
      temp-changes SCROLLING.
DEFINE BROWSE br-ccur
  QUERY br-ccur NO-LOCK DISPLAY
      mark-string(RECID(X_c-curr-bank), v-rid-list) COLUMN-LABEL "*" FORMAT "X(1)":U
      usrfulnf(X_c-curr-bank.corr-user-name) COLUMN-LABEL "Изменил" FORMAT "X(18)":U
      X_c-curr-bank.corr-date COLUMN-LABEL "Дата!измен" FORMAT "99/99/9999":U
      string(X_c-curr-bank.corr-time, "HH:MM") COLUMN-LABEL "Время!измен" FORMAT "X(5)":U
      X_c-curr-bank.curr-code COLUMN-LABEL "Код валюты" FORMAT ">>9":U
      X_c-curr-bank.exch-date COLUMN-LABEL "Дата курса" FORMAT "99/99/99":U
...
DO:
    ...
    then
    v-rid-list = string( recid( X_c-curr-bank ) ) .
  end.
END.
...
do:
  v-doc-rec = recid(X_c-curr-bank). run OpenBr in this-procedure. reposition br-ccur to recid v-doc-rec no-error. v-doc-rec = ?.               apply 'value-changed' to br-ccur.
...
on start-search, ctrl-o of br-ccur in frame Dialog-Frame do:
   run sort-brbr-ccur
     (input (if available X_c-curr-bank
             then recid(X_c-curr-bank)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-ccur :
  ...
  case sort-labelbr-ccur:
        when X_c-curr-bank.exch-date:label in browse br-ccur then DO:   assign     sort-column-name = "X_c-curr-bank.exch-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
 ...
 if LOOKUP(p-mode, 'one':U + chr(4) + 'все':U,
                chr(4)) = 0
     then dO:
    message
    vss-workfile vss-revision vss-description skip
 ...
 if p-mode = "one":U then do:
  find first X_curr-bank no-lock where
             X_curr-bank.curr-code = p-curr-code
       AND   X_curr-bank.exch-date = p-exch-date
                no-error.
    if not available X_curr-bank then do:
        message
        vss-workfile vss-revision vss-description skip
 ...
 if p-mode = 'все':U then do:
  find first X_curr-bank no-lock where
                X_curr-bank.curr-code = p-curr-code no-error.
    if not available X_curr-bank then do:
        message
        vss-workfile vss-revision vss-description skip
...
DEF VAR cur-clmn-numbr-ccur as INT EXTENT 6 no-undo.
...
def var old-session as logical no-undo.
   ...
   DO jjbr-ccur = NUM-ENTRIES('1,2,3,4,5,6') TO 1 BY -1:
     RUN re-move-clmnbr-ccur ( cur-clmn-numbr-ccur[INTEGER(ENTRY (jjbr-ccur, '1,2,3,4,5,6'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-ccur do:
  RUN re-move-clmnbr-ccur ( 1, 6).
END.
ON ctrl-cursor-left OF BROWSE br-ccur do:
  RUN re-move-clmnbr-ccur (6, 1).
END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ccur FOR EACH X_c-curr-bank NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
CASE p-mode:
  when 'все':U then do:
    OPEN QUERY br-ccur FOR EACH X_c-curr-bank NO-LOCK where
                                  X_c-curr-bank.curr-code = p-curr-code  INDEXED-REPOSITION.
  end.
  when 'one':U then do:
    OPEN QUERY br-ccur FOR EACH X_c-curr-bank NO-LOCK where
                                  X_c-curr-bank.curr-code = p-curr-code
                              AND X_c-curr-bank.exch-date = p-exch-date
                              AND X_c-curr-bank.corr-user-db-num = 0
                                  INDEXED-REPOSITION.
  end.
END CASE.
APPLY "VALUE-CHANGED" TO br-ccur in frame Dialog-Frame.
...
define buffer new_c-curr-bank for ub.c-curr-bank.
define buffer current_curr-bank for ub.curr-bank.
define variable v-chg-fields as character no-undo.
...
for each temp-changes:
    delete temp-changes.
END.
if not available X_c-curr-bank then do:
...
v-label-param =
 ...
 + "exch-date" + chr(4) + "Дата курса" + chr(4) + "" + chr(8)
 + "exch-rate" + chr(4) + "Курс валюты" + chr(4) + "" + chr(8)
 + "exch-scale" + chr(4) + "Масштаб" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-curr-bank:handle
                                            ,input  'curr-bank':U
                                            ,input  "curr-code,exch-date,exch-rate,exch-scale"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
