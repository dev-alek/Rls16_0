...
END FUNCTION.
function getstatus return char
   (iStatus as integer  ):
...
function getproceditEx return character
   (ibuffertt  as handle,
...
function getprocedit return character
   (ibuffertt  as handle):
...
function endLevel returns logical():
   return MaxLevel ne 0 and num-entries(iparent,chr(4))  eq  MaxLevel  .
end.
define variable startlevel as integer no-undo.
startlevel = num-entries(iparent,chr(4)) .
...
on choose of menu-item mHistOne
do:
...
on choose of menu-item mHistChiled
do:
   define variable v-rid-list as character no-undo.
   if avail Code then
   do:
...
ON go OF FRAME f-c-p
do:
end.
ON choose OF b-exit IN FRAME f-c-p
do:
   define variable vView as logical no-undo.
   run PrevLevel(output vView).
   if vView
   then do:
      OPEN QUERY BROWSE-Code FOR EACH code       where code.parent = mParent NO-LOCK INDEXED-REPOSITION.
      apply "ENTRY" to BROWSE-Code.
   end.
   else
      APPLY "GO" TO FRAME f-c-p.
   return.
end.
ON choose OF b-add IN FRAME f-c-p
do:
   ...
   then assign
      ...
      .
         ...
         vparent = mparent
         vcode    = entry(num-entries(vparent,chr(4)),vparent,chr(4))
         entry(num-entries(vparent,chr(4)),vparent,chr(4)) = ""
...
ON choose OF b-look IN FRAME f-c-p
do:
   define variable vRec as recid no-undo.
   define variable v-ok as logical no-undo.
   define variable vProcNextLevel as character no-undo init "ref/codefrm.w".
...
ON choose OF b-chiled IN FRAME f-c-p
do:
  run nextlevel.
end.
ON MOUSE-SELECT-DBLCLICK OF BROWSE-Code IN FRAME f-c-p
OR RETURN OF b-chiled IN FRAME f-c-p
DO:
  apply "choose" to b-chiled in frame f-c-p.
END.
ON choose OF b-del IN FRAME f-c-p
do:
...
ON choose OF b-sel IN FRAME f-c-p
do:
   if not avail code then return.
   setSelect(buffer code:handle).
   BROWSE-Code:refresh ().
end.
ON choose OF b-upd IN FRAME f-c-p
do:
   define variable vRec as recid no-undo.
   if not avail code then return.
   define variable v-ok as logical no-undo.
...
on ENTER of mSearch in frame f-c-p
do:
      define buffer buf-code-search for ub.code.
   assign
     msearch
   .
   define variable vRec as recid no-undo init ?.
   block-code:
   for each buf-code-search where (buf-code-search.parent = mParent and buf-code-search.code     begins msearch)
                              or (buf-code-search.parent = mParent and buf-code-search.CodeName begins msearch)
   no-lock:
      vRec = recid(buf-code-search).
      leave block-code.
   end.
      if vRec eq ?
   then do:
...
if valid-handle(active-window) and frame f-c-p:PARENT eq ?
  then frame f-c-p:PARENT = active-window.
define variable vss-include-info7 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame f-c-p
do:
  run gbl/app_help.p
...
on choose of b-help in frame f-c-p
do:
  apply "help":u to frame f-c-p .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame f-c-p:width - 0.3
                fh            = frame f-c-p:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame f-c-p :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame f-c-p :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame f-c-p :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame f-c-p :height = v-frame-height
          .
          if frame f-c-p :scrollable = true
          then do:
            assign
              frame f-c-p :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame f-c-p :height
      v-frame-virtual-height = frame f-c-p :virtual-height
    ...
    assign
      v-field-group-handle = frame f-c-p :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame f-c-p :virtual-height = frame f-c-p :virtual-height + p-change-value
          no-error .
      ...
      assign
        frame f-c-p :height = frame f-c-p :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame f-c-p :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame f-c-p :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame f-c-p :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame f-c-p :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame f-c-p :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame f-c-p :width = v-frame-width
          .
          if frame f-c-p :scrollable = true
          then do:
            assign
              frame f-c-p :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame f-c-p :width
      v-frame-virtual-width = frame f-c-p :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame f-c-p :virtual-width = frame f-c-p :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame f-c-p :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame f-c-p :width = frame f-c-p :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame f-c-p :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame f-c-p :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame f-c-p :height - v-diasize-resize-button :height
                  - 1
                  - (frame f-c-p :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame f-c-p :width - v-diasize-resize-button :width
                  - 1
                  - (frame f-c-p :border-right-pixels / session :pixels-per-column)
    .
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame f-c-p :height
      v-col-delta = v-new-col - frame f-c-p :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame f-c-p :width
      v-diasize-current-frame-height = frame f-c-p :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame f-c-p :height
        v-diasize-orig-frame-width  = frame f-c-p :width
        v-diasize-browse-handle     = browse BROWSE-Code :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame f-c-p :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
on window-close of frame f-c-p
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  ...
  wait-for go of frame f-c-p focus BROWSE-Code.
  if imode eq 'ВЫБОР':U
  then
...
PROCEDURE enable_UI :
  enable
    ...
    b-chiled
    b-sel
    when imode eq 'ВЫБОР':U
    b-add
    when imode eq 'ИЗМЕНЕНИЕ':U
    b-del
    when imode eq 'ИЗМЕНЕНИЕ':U
    b-upd
    when imode eq 'ИЗМЕНЕНИЕ':U
    ...
    b-hist
    with frame f-c-p.
   b-hist:POPUP-MENU in frame f-c-p = menu POPUP-MENU-b-hist:HANDLE.
   b-hist:MENU-MOUSE = 1.
  fselect                :visible in browse BROWSE-Code = imode eq 'ВЫБОР':U.
  OPEN QUERY BROWSE-Code FOR EACH code       where code.parent = mParent NO-LOCK INDEXED-REPOSITION.
end procedure.
PROCEDURE NextLevel:
   if available  code
   then do:
      ...
      then do:
         ...
         then
            assign
               mparent  = code.parent + chr(4) + code.code
               mparent  = left-trim(mparent,chr(4))
            .
         OPEN QUERY BROWSE-Code FOR EACH code       where code.parent = mParent NO-LOCK INDEXED-REPOSITION.
      end.
   end.
...
PROCEDURE PrevLevel:
   ...
   then do:
      assign
         entry(num-entries(mparent,chr(4)),mparent,chr(4)) = ""
         mparent = substring (mparent,1,length(mParent) - 1)
      no-error.
      if mparent ne "" and num-entries(mparent,chr(4)) le  startlevel
      then do:
         if imode eq 'ВЫБОР':U
         then do:
            for each tmprecid:
