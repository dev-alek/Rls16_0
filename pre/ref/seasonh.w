...
DEFINE BROWSE BR-docs
  QUERY BR-docs DISPLAY
     ...
     buf_c-season.corr-date   COLUMN-LABEL 'Дата изменения'
     string(buf_c-season.corr-time,'hh:mm:ss')   COLUMN-LABEL 'Время изменения '
     buf_c-season.corr-user-db-num   COLUMN-LABEL 'Изменение с БД №' format ">>>>9"
     buf_c-season.corr-user-name   COLUMN-LABEL 'Изменил'
...
buf_c-season.corr-date:read-only in browse br-docs = true .
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH ub.c-season SHARE-LOCK.
  ...
  OPEN QUERY BR-docs FOR EACH buf_c-season no-lock.
END PROCEDURE.
...
when "List_":U then do:     create temp-changes.     assign     temp-changes.f_name = "List_":U     temp-changes.l_name = "Список"     temp-changes.v_old = string(buf_c-season.List_)     temp-changes.v_new = (if available new_c-season                               then string(new_c-season.List_)                               else string(current_season.List_))     .   end.
when "sea-code":U then do:     create temp-changes.     assign     temp-changes.f_name = "sea-code":U     temp-changes.l_name = "Код сезона"     temp-changes.v_old = string(buf_c-season.sea-code)     temp-changes.v_new = (if available new_c-season                               then string(new_c-season.sea-code)                               else string(current_season.sea-code))     .   end.
when "db-num":U then do:     create temp-changes.     assign     temp-changes.f_name = "db-num":U     temp-changes.l_name = "БД"     temp-changes.v_old = string(buf_c-season.db-num)     temp-changes.v_new = (if available new_c-season                               then string(new_c-season.db-num)                               else string(current_season.db-num))     .   end.
when "sea-month-1":U then do:     create temp-changes.     assign     temp-changes.f_name = "sea-month-1":U     temp-changes.l_name = "Месяц с"     temp-changes.v_old = string(buf_c-season.sea-month-1)     temp-changes.v_new = (if available new_c-season                               then string(new_c-season.sea-month-1)                               else string(current_season.sea-month-1))     .   end.
when "sea-month-2":U then do:     create temp-changes.     assign     temp-changes.f_name = "sea-month-2":U     temp-changes.l_name = "Месяц по"     temp-changes.v_old = string(buf_c-season.sea-month-2)     temp-changes.v_new = (if available new_c-season                               then string(new_c-season.sea-month-2)                               else string(current_season.sea-month-2))     .   end.
when "sea-name":U then do:     create temp-changes.     assign     temp-changes.f_name = "sea-name":U     temp-changes.l_name = "Наименование сезона/коллекции"     temp-changes.v_old = string(buf_c-season.sea-name)     temp-changes.v_new = (if available new_c-season                               then string(new_c-season.sea-name)                               else string(current_season.sea-name))     .   end.
when "stts":U then do:     create temp-changes.     assign     temp-changes.f_name = "stts":U     temp-changes.l_name = "Статус (0-текущий)"     temp-changes.v_old = string(buf_c-season.stts)     temp-changes.v_new = (if available new_c-season                               then string(new_c-season.stts)                               else string(current_season.stts))     .   end.
END CASE.
end.
Open QUery br-changes for each temp-changes.
END PROCEDURE.
