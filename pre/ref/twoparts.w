...
END.
ON CHOOSE OF B-case IN FRAME Dialog-Frame
DO:
define variable glog as logical no-undo .
  ...
  case paction:
    ...
    when "fuse":U then do:
      if mark-list = "" then do:
        ...
        if INDEX(X_parts.part-code, '#':U) = 0 then do:
...
END.
ON CHOOSE OF B-part IN FRAME Dialog-Frame
DO:
...
assign
  v-str-recid17 = trim( string( recid( X_parts ) , "->>>>>>>>>>>9":U ) )
  v-num-entry17 = lookup( v-str-recid17 , mark-list )
.
if v-num-entry17 > 0 then do:
  assign
    entry( v-num-entry17, mark-list ) = "":U
    mark-list = trim( replace( mark-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    mark-list = mark-list + ( if mark-list = "":U then "":U else chr(44) ) + v-str-recid17
  .
end.
  display
  string(lookup (string (recid (X_parts)), mark-list ) > 0, "*/":U)  @ mark
  with browse BR-parts.
END PROCEDURE.
PROCEDURE MyEnable :
  assign
  rs-action = paction.
  case paction:
    when "split":U then do:
      b-case:label in frame Dialog-Frame = "&Коробка".
    end.
    when "fuse":U then do:
  ...
  hide b-mark in frame Dialog-Frame .
  VIEW FRAME Dialog-Frame.
END PROCEDURE.
PROCEDURE OpenBr :
...
CASE p-mode:
    when 'объект':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Партии товаров с двумя ед.изм. на объекте " + p-obj-type + string(p-obj-code)
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-19 as character no-undo .
      assign
      parameter-3-19 =
                              "FOR EACH X_parts NO-LOCK"
      parameter-4-19 =
        (
          if (" X_parts.artic = partic AND X_parts.prod-type = pprod-type AND X_parts.prod-code = pprod-code AND X_parts.obj-type = p-obj-type AND X_parts.obj-code = p-obj-code AND X_parts.cli-qnty > 1 AND X_parts.out-code = 'free-zone':U " + " " + where-phrase-19) <> ""
          then  substitute('X_parts.artic = &1&2&1 AND X_parts.prod-type = &1&3&1 AND X_parts.prod-code = &4 AND X_parts.obj-type = &1&5&1 AND X_parts.obj-code = &6 AND X_parts.cli-qnty > 1 AND X_parts.out-code = &1&7&1 '                                  , chr(34), partic, pprod-type, pprod-code, p-obj-type, p-obj-code, 'free-zone':U) + " " + where-phrase-19
          else "true"
        )
      parameter-5-19 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-19 =
          (" X_parts.artic = partic AND X_parts.prod-type = pprod-type AND X_parts.prod-code = pprod-code AND X_parts.obj-type = p-obj-type AND X_parts.obj-code = p-obj-code AND X_parts.cli-qnty > 1 AND X_parts.out-code = 'free-zone':U " + " " + where-phrase-19 = "")
      .
  ...
  if l-filter-open-19 = false then do:
    OPEN QUERY br-parts FOR EACH X_parts NO-LOCK
      where  X_parts.artic = partic AND X_parts.prod-type = pprod-type AND X_parts.prod-code = pprod-code AND X_parts.obj-type = p-obj-type AND X_parts.obj-code = p-obj-code AND X_parts.cli-qnty > 1 AND X_parts.out-code = 'free-zone':U
      indexed-reposition
  .
      assign
        l-query-was-opened = true
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-21 as character no-undo .
      ...
      parameter-4-21 =
        (
          if (" X_parts.artic = partic AND X_parts.prod-type = pprod-type AND X_parts.prod-code = pprod-code AND X_parts.obj-type = p-obj-type AND X_parts.obj-code = p-obj-code AND X_parts.cli-qnty = 1 AND X_parts.out-code = 'free-zone':U " + " " + where-phrase-21) <> ""
          then  substitute('X_parts.artic = &1&2&1 AND X_parts.prod-type = &1&3&1 AND X_parts.prod-code = &4 AND X_parts.obj-type = &1&5&1 AND X_parts.obj-code = &6 AND X_parts.cli-qnty = 1 AND X_parts.out-code = &1&7&1 '                                 , chr(34), partic, pprod-type, pprod-code, p-obj-type, p-obj-code, 'free-zone':U) + " " + where-phrase-21
          else "true"
        )
      parameter-5-21 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-21 =
          (" X_parts.artic = partic AND X_parts.prod-type = pprod-type AND X_parts.prod-code = pprod-code AND X_parts.obj-type = p-obj-type AND X_parts.obj-code = p-obj-code AND X_parts.cli-qnty = 1 AND X_parts.out-code = 'free-zone':U " + " " + where-phrase-21 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-parts:handle
