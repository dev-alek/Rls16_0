...
DEFINE BROWSE BR-dis-grp-rule
  QUERY BR-dis-grp-rule NO-LOCK DISPLAY
      mark-string(recid(X_dis-grp-rule), v-rid-list) COLUMN-LABEL '*' FORMAT "X(1)"
X_dis-grp-rule.node-code COLUMN-LABEL 'Код группы'
full-grp-name(X_dis-grp-rule.classif-type, X_dis-grp-rule.node-code, X_dis-grp-rule.host-code, X_dis-grp-rule.obj-type, X_dis-grp-rule.obj-code) COLUMN-LABEL 'Название группы' FORMAT "X(255)" WIDTH 40
X_dis-grp-rule.templ-rl-root COLUMN-LABEL 'Тип правила' FORMAT ">>>>>>>>9"
entry (lookup (X_dis-grp-rule.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U) COLUMN-LABEL 'Место использ.' FORMAT "X(20)"
X_dis-grp-rule.obj-type COLUMN-LABEL 'Тип объ' FORMAT "X(3)"
X_dis-grp-rule.obj-code COLUMN-LABEL 'Код объ' FORMAT ">>>>9"
(if p-classif-type = 'sum-grp':U then entry (lookup (X_dis-grp-rule.discnt-role, 'gds-grp-pcnt,gds-grp-pcnt-kat,gds-grp-abs,gds-grp-qnty,gds-grp-sum':u) + 1, ',' + '% скидка на группу товара,% скидка на группу товара для кат.клиентов,Abs скидка на группу товара,% Скидка на группу товара по кол-ву,% Скидка на группу товара на сумму':u) else entry (lookup (X_dis-grp-rule.discnt-role, 'cli-grp-pcnt':u) + 1, ',' + '% скидка на группу клиентов':u)) COLUMN-LABEL 'Тип скидки' FORMAT "X(70)"
X_dis-grp-rule.rule-num COLUMN-LABEL '№ правила' FORMAT ">>>>>>>>9"
X_dis-grp-rule.rl-root COLUMN-LABEL '№ корн.правила' FORMAT ">>>>>>>>9"
...
apply "entry" to br-dis-grp-rule in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-dis-grp-rule :handle
      .
...
do:
   v-doc-rec = recid(X_dis-grp-rule).  Run OpenBR in this-procedure ( input yes, input no, input '':U, input p-classif-type).  REPOSITION br-dis-grp-rule to recid v-doc-rec No-ERROR.               apply 'value-changed' to br-dis-grp-rule.
...
on SHIFT-F9 of frame Dialog-Frame anywhere do:
  if not available X_sum-grp then
    return no-apply.
  gds-rec = recid (X_sum-grp).
  run ref/gds-form.w ( input parparentproc
                      ,input 'ПРОСМОТР':U
                      ,input ?
                      ,input ?
...
def var sort-labelbr-dis-grp-rule   as character no-undo .
def var sort-clmnbr-dis-grp-rule    as handle    no-undo .
def var cur-clmnbr-dis-grp-rule     as handle    no-undo .
def var cur-clmn-locbr-dis-grp-rule as integer   no-undo .
def var re-querybr-dis-grp-rule     as logical   initial no no-undo .
on start-search, ctrl-o of br-dis-grp-rule in frame Dialog-Frame do:
   run sort-brbr-dis-grp-rule
     (input (if available X_dis-grp-rule
             then recid(X_dis-grp-rule)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-dis-grp-rule :
  define input parameter p-recid as recid no-undo .
  if re-querybr-dis-grp-rule = no then do:
    assign
       cur-clmnbr-dis-grp-rule = br-dis-grp-rule:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-dis-grp-rule <> ? then sort-clmnbr-dis-grp-rule:column-fgcolor = 0.
    if cur-clmnbr-dis-grp-rule = sort-clmnbr-dis-grp-rule then do:
      assign
         sort-labelbr-dis-grp-rule = ""
         sort-clmnbr-dis-grp-rule = ?
      .
     end.
     else do:
       assign
         sort-labelbr-dis-grp-rule = cur-clmnbr-dis-grp-rule:label
         sort-clmnbr-dis-grp-rule  = cur-clmnbr-dis-grp-rule
         sort-clmnbr-dis-grp-rule:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-dis-grp-rule = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-dis-grp-rule:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-dis-grp-rule then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-dis-grp-rule = cur-clmn-locbr-dis-grp-rule + 1
    .
  end.
  case sort-labelbr-dis-grp-rule:
        when '*'  then DO:    assign       sort-column-name = "mark-string(recid(X_dis-grp-rule), v-rid-list)"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Тип классификатора'  then DO:    assign       sort-column-name = "entry (lookup (X_dis-grp-rule.classif-type, 'sum-grp,cli-grp':u) + 1, ',' + 'Группы товаров (на кассе),Группа клиентов':u)"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Код группы'  then DO:    assign       sort-column-name = "X_dis-grp-rule.node-code"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Тип правила'  then DO:    assign       sort-column-name = "X_dis-grp-rule.templ-rl-root"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Место использ.'  then DO:    assign       sort-column-name = "entry (lookup (X_dis-grp-rule.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U)"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Тип объ'  then DO:    assign       sort-column-name = "X_dis-grp-rule.obj-type"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Код объ'  then DO:    assign       sort-column-name = "X_dis-grp-rule.obj-code"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Тип скидки'  then DO:    assign       sort-column-name = "(if p-classif-type = 'sum-grp':U then entry (lookup (X_dis-grp-rule.discnt-role, 'gds-grp-pcnt,gds-grp-pcnt-kat,gds-grp-abs,gds-grp-qnty,gds-grp-sum':u) + 1, ',' + '% скидка на группу товара,% скидка на группу товара для кат.клиентов,Abs скидка на группу товара,% Скидка на группу товара по кол-ву,% Скидка на группу товара на сумму':u) else entry (lookup (X_dis-grp-rule.discnt-role, 'cli-grp-pcnt':u) + 1, ',' + '% скидка на группу клиентов':u))"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when '№ правила'  then DO:    assign       sort-column-name = "X_dis-grp-rule.rule-num"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when 'Название группы'  then DO:    assign       sort-column-name = "full-grp-name(X_dis-grp-rule.classif-type, X_dis-grp-rule.node-code, X_dis-grp-rule.host-code, X_dis-grp-rule.obj-type, X_dis-grp-rule.obj-code)"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
        when '№ корн.правила'  then DO:    assign       sort-column-name = "X_dis-grp-rule.rl-root"     .     run OpenBr in this-procedure ( input yes, input no, input no, input p-classif-type).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input no, p-classif-type).
      if sort-labelbr-dis-grp-rule <> "" then do:
        assign
          cur-clmnbr-dis-grp-rule:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-dis-grp-rule = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-dis-grp-rule to recid p-recid no-error.
    apply "value-changed" to br-dis-grp-rule in frame Dialog-Frame.
  end.
  apply "entry" to br-dis-grp-rule in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-dis-grp-rule:
if cur-clmnbr-dis-grp-rule = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input no, p-classif-type).
end.
else do:
   assign re-querybr-dis-grp-rule = yes.
   run sort-brbr-dis-grp-rule
     (input (if available X_dis-grp-rule
             then recid(X_dis-grp-rule)
             else ?
   ...
   assign re-querybr-dis-grp-rule = no.
end.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-dis-grp-rule :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
define variable vss-include-info19 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  IF LOOKUP(p-classif-type, 'sum-grp,cli-grp':u) = 0 THEN DO:
      MESSAGE
  ...
  IF LOOKUP( "discnt-role", p-list-mode) > 0 THEN DO:
    FIND FIRST par_dis-cfg-rule NO-LOCK WHERE
          ...
          AND par_dis-cfg-rule.TABLE-name = 'dis-grp-rule':U NO-ERROR.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-dis-grp-rule FOR EACH X_dis-grp-rule NO-LOCK  INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
DEFINE variable v-h as handle no-undo .
define variable v-ii as integer no-undo .
v-h = BR-dis-grp-rule:FIRST-COLUMN IN FRAME Dialog-Frame.
DO while valid-handle(v-h)
and  v-ii < BR-dis-grp-rule:num-COLUMNs IN FRAME Dialog-Frame
:
  v-ii = v-ii + 1.
  if v-h:LABEL = 'Тип скидки'
  OR v-h:LABEL = 'Название группы'
      then do:
    v-h:RESIZABLE = YES.
  end.
  ELSE DO:
...
ASSIGN
title0 = entry (lookup (p-classif-type, 'sum-grp,cli-grp':u) + 1, ',' + 'Группы товаров (на кассе),Группа клиентов':u)
.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-22 as character no-undo .
      assign
      parameter-3-22 =
                              "FOR EACH X_dis-grp-rule"
      parameter-4-22 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type " + " " + where-phrase-22) <> ""
          then " X_dis-grp-rule.classif-type = p-classif-type " + " " + where-phrase-22
          else "true"
        )
      parameter-5-22 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-22 =
          (" X_dis-grp-rule.classif-type = p-classif-type " + " " + where-phrase-22 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-22 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  assign
    v-doc-rec = recid( X_dis-grp-rule )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      assign
      parameter-2-22 = (if p-find-next then "true":u else "false":u )
      parameter-4-22 =
        "where ":u + " X_dis-grp-rule.classif-type = p-classif-type " + " ":u + where-phrase-22 + " ":u + p-find-condition + " " + ""
      parameter-5-22 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
                          ,input rowid(X_dis-grp-rule)
                          ,input logical(parameter-2-22)
                          ,input no-lock
                          ,input (buffer X_dis-grp-rule:handle)
                          ,input parameter-4-22
                          ,input parameter-5-22
                          ) no-error.
      .
      ...
      assign
      parameter-2-22 = (if p-find-next then "true":u else "false":u )
      parameter-3-22 =  "FOR EACH X_dis-grp-rule"
      parameter-4-22 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type " + " " + where-phrase-22) <> ""
          then " X_dis-grp-rule.classif-type = p-classif-type " + " " + where-phrase-22
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-24 as character no-undo .
      ...
      parameter-4-24 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type and                       X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code " + " " + where-phrase-24) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1 and                       X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4 ', chr(34),  p-classif-type, p-curr-obj-type, p-curr-obj-code) + " " + where-phrase-24
          else "true"
        )
      parameter-5-24 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-24 =
          (" X_dis-grp-rule.classif-type = p-classif-type and                       X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code " + " " + where-phrase-24 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-24 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type and                       X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-24 =
        "where ":u +  substitute('X_dis-grp-rule.classif-type = &1&2&1 and                       X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4 ', chr(34),  p-classif-type, p-curr-obj-type, p-curr-obj-code) + " ":u + where-phrase-24 + " ":u + p-find-condition + " " + ""
      parameter-5-24 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-24 = (if p-find-next then "true":u else "false":u )
      parameter-3-24 =  "FOR EACH X_dis-grp-rule"
      parameter-4-24 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type and                       X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code " + " " + where-phrase-24) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1 and                       X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4 ', chr(34),  p-classif-type, p-curr-obj-type, p-curr-obj-code) + " " + where-phrase-24
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-26 as character no-undo .
      ...
      parameter-4-26 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                         AND X_dis-grp-rule.pos-type = p-pos-type " + " " + where-phrase-26) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                         and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                         AND X_dis-grp-rule.pos-type = &1&5&1 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-pos-type) + " " + where-phrase-26
          else "true"
        )
      parameter-5-26 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-26 =
          (" X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                         AND X_dis-grp-rule.pos-type = p-pos-type " + " " + where-phrase-26 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-26 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                         AND X_dis-grp-rule.pos-type = p-pos-type
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-26 =
        "where ":u +  substitute('X_dis-grp-rule.classif-type = &1&2&1                         and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                         AND X_dis-grp-rule.pos-type = &1&5&1 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-pos-type) + " ":u + where-phrase-26 + " ":u + p-find-condition + " " + ""
      parameter-5-26 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-26 = (if p-find-next then "true":u else "false":u )
      parameter-3-26 =  "FOR EACH X_dis-grp-rule"
      parameter-4-26 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                         AND X_dis-grp-rule.pos-type = p-pos-type " + " " + where-phrase-26) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                         and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                         AND X_dis-grp-rule.pos-type = &1&5&1 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-pos-type) + " " + where-phrase-26
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-28 as character no-undo .
      ...
      parameter-4-28 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-28) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                       AND X_dis-grp-rule.templ-rl-root = &5 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " " + where-phrase-28
          else "true"
        )
      parameter-5-28 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-28 =
          (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-28 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-28 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.templ-rl-root = p-templ-rl-root
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-28 =
        "where ":u +  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                       AND X_dis-grp-rule.templ-rl-root = &5 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " ":u + where-phrase-28 + " ":u + p-find-condition + " " + ""
      parameter-5-28 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-28 = (if p-find-next then "true":u else "false":u )
      parameter-3-28 =  "FOR EACH X_dis-grp-rule"
      parameter-4-28 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-28) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                       AND X_dis-grp-rule.templ-rl-root = &5 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " " + where-phrase-28
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-30 as character no-undo .
      ...
      parameter-4-30 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.discnt-role = p-discnt-role " + " " + where-phrase-30) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                       AND X_dis-grp-rule.discnt-role = &1&5&1 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " " + where-phrase-30
          else "true"
        )
      parameter-5-30 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-30 =
          (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.discnt-role = p-discnt-role " + " " + where-phrase-30 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-30 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.discnt-role = p-discnt-role
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-30 =
        "where ":u +  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                       AND X_dis-grp-rule.discnt-role = &1&5&1 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " ":u + where-phrase-30 + " ":u + p-find-condition + " " + ""
      parameter-5-30 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-30 = (if p-find-next then "true":u else "false":u )
      parameter-3-30 =  "FOR EACH X_dis-grp-rule"
      parameter-4-30 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.obj-type = p-curr-obj-type and X_dis-grp-rule.obj-code = p-curr-obj-code                       AND X_dis-grp-rule.discnt-role = p-discnt-role " + " " + where-phrase-30) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.obj-type = &1&3&1 and X_dis-grp-rule.obj-code = &4                       AND X_dis-grp-rule.discnt-role = &1&5&1 ', chr(34), p-classif-type, p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " " + where-phrase-30
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-32 as character no-undo .
      ...
      parameter-4-32 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-32) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.templ-rl-root = &3 ', chr(34), p-classif-type, p-templ-rl-root) + " " + where-phrase-32
          else "true"
        )
      parameter-5-32 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-32 =
          (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-32 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.templ-rl-root = p-templ-rl-root
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-32 =
        "where ":u +  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.templ-rl-root = &3 ', chr(34), p-classif-type, p-templ-rl-root) + " ":u + where-phrase-32 + " ":u + p-find-condition + " " + ""
      parameter-5-32 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_dis-grp-rule"
      parameter-4-32 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                       and X_dis-grp-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-32) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                       and X_dis-grp-rule.templ-rl-root = &3 ', chr(34), p-classif-type, p-templ-rl-root) + " " + where-phrase-32
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_dis-grp-rule.rule-num = p-rule-num " + " " + where-phrase-34) <> ""
          then  substitute('X_dis-grp-rule.rule-num = &1', p-rule-num ) + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_dis-grp-rule.rule-num = p-rule-num " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.rule-num = p-rule-num
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_dis-grp-rule.rule-num = &1', p-rule-num ) + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_dis-grp-rule"
      parameter-4-34 =
        (
          if (" X_dis-grp-rule.rule-num = p-rule-num " + " " + where-phrase-34) <> ""
          then  substitute('X_dis-grp-rule.rule-num = &1', p-rule-num ) + " " + where-phrase-34
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if (" X_dis-grp-rule.rl-root = p-rule-num " + " " + where-phrase-36) <> ""
          then  substitute('X_dis-grp-rule.rl-root = &1', p-rule-num ) + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          (" X_dis-grp-rule.rl-root = p-rule-num " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.rl-root = p-rule-num
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-36 =
        "where ":u +  substitute('X_dis-grp-rule.rl-root = &1', p-rule-num ) + " ":u + where-phrase-36 + " ":u + p-find-condition + " " + ""
      parameter-5-36 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_dis-grp-rule"
      parameter-4-36 =
        (
          if (" X_dis-grp-rule.rl-root = p-rule-num " + " " + where-phrase-36) <> ""
          then  substitute('X_dis-grp-rule.rl-root = &1', p-rule-num ) + " " + where-phrase-36
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-38 as character no-undo .
      ...
      parameter-4-38 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.pos-type = p-pos-type " + " " + where-phrase-38) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                         and X_dis-grp-rule.pos-type = &1&3&1 ', chr(34), p-classif-type, p-pos-type) + " " + where-phrase-38
          else "true"
        )
      parameter-5-38 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-38 =
          (" X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.pos-type = p-pos-type " + " " + where-phrase-38 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-38 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.pos-type = p-pos-type
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-38 =
        "where ":u +  substitute('X_dis-grp-rule.classif-type = &1&2&1                         and X_dis-grp-rule.pos-type = &1&3&1 ', chr(34), p-classif-type, p-pos-type) + " ":u + where-phrase-38 + " ":u + p-find-condition + " " + ""
      parameter-5-38 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-38 = (if p-find-next then "true":u else "false":u )
      parameter-3-38 =  "FOR EACH X_dis-grp-rule"
      parameter-4-38 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                         and X_dis-grp-rule.pos-type = p-pos-type " + " " + where-phrase-38) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                         and X_dis-grp-rule.pos-type = &1&3&1 ', chr(34), p-classif-type, p-pos-type) + " " + where-phrase-38
  ...
  WHEN "discnt-role":U THEN DO:
        ...
        frame Dialog-Frame:TITLE =   substitute("&1 тип &2"
                                      , title0
                                      , entry (lookup (X_dis-grp-rule.discnt-role, 'gds-grp-pcnt,gds-grp-pcnt-kat,gds-grp-abs,gds-grp-qnty,gds-grp-sum':u) + 1, ',' + '% скидка на группу товара,% скидка на группу товара для кат.клиентов,Abs скидка на группу товара,% Скидка на группу товара по кол-ву,% Скидка на группу товара на сумму':u))
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-40 as character no-undo .
      ...
      parameter-4-40 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                           and X_dis-grp-rule.discnt-role = p-discnt-role " + " " + where-phrase-40) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                           and X_dis-grp-rule.discnt-role = &1&3&1 ', chr(34), p-classif-type, p-discnt-role) + " " + where-phrase-40
          else "true"
        )
      parameter-5-40 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-40 =
          (" X_dis-grp-rule.classif-type = p-classif-type                           and X_dis-grp-rule.discnt-role = p-discnt-role " + " " + where-phrase-40 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-grp-rule:handle
  ...
  if l-filter-open-40 = false then do:
    OPEN QUERY br-dis-grp-rule FOR EACH X_dis-grp-rule
      where  X_dis-grp-rule.classif-type = p-classif-type                           and X_dis-grp-rule.discnt-role = p-discnt-role
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-dis-grp-rule:handle:get-buffer-handle(1) = (buffer X_dis-grp-rule:handle) then do:
      ...
      parameter-4-40 =
        "where ":u +  substitute('X_dis-grp-rule.classif-type = &1&2&1                           and X_dis-grp-rule.discnt-role = &1&3&1 ', chr(34), p-classif-type, p-discnt-role) + " ":u + where-phrase-40 + " ":u + p-find-condition + " " + ""
      parameter-5-40 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-40 = (if p-find-next then "true":u else "false":u )
      parameter-3-40 =  "FOR EACH X_dis-grp-rule"
      parameter-4-40 =
        (
          if (" X_dis-grp-rule.classif-type = p-classif-type                           and X_dis-grp-rule.discnt-role = p-discnt-role " + " " + where-phrase-40) <> ""
          then  substitute('X_dis-grp-rule.classif-type = &1&2&1                           and X_dis-grp-rule.discnt-role = &1&3&1 ', chr(34), p-classif-type, p-discnt-role) + " " + where-phrase-40
...
define variable v-str-recid42 as character no-undo .
define variable v-num-entry42 as integer   no-undo .
assign
  v-str-recid42 = trim( string( recid( X_dis-grp-rule ) , "->>>>>>>>>>>9":U ) )
  v-num-entry42 = lookup( v-str-recid42 , v-rid-list )
.
if v-num-entry42 > 0 then do:
  assign
    entry( v-num-entry42, v-rid-list ) = "":U
    v-rid-list = trim( replace( v-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rid-list = v-rid-list + ( if v-rid-list = "":U then "":U else chr(44) ) + v-str-recid42
  .
end.
  loc#log = br-dis-grp-rule:refresh() IN FRAME Dialog-Frame .
...
X_dis-grp-rule.node-code COLUMN-LABEL 'Код группы' FORMAT ">>>>9"
X_dis-grp-rule.templ-rl-root COLUMN-LABEL 'Тип правила' FORMAT ">>>>>>>>9"
v-pos-type COLUMN-LABEL 'Место использ.'
X_dis-grp-rule.obj-type COLUMN-LABEL 'Тип объ' FORMAT "X(3)"
X_dis-grp-rule.obj-code COLUMN-LABEL 'Код объ' FORMAT ">>>>>>>>9"
...
DO WHILE available X_dis-grp-rule :
  ...
  mark-string(recid(X_dis-grp-rule), v-rid-list) @ v-mark
  entry (lookup (p-classif-type, 'sum-grp,cli-grp':u) + 1, ',' + 'Группы товаров (на кассе),Группа клиентов':u)
  X_dis-grp-rule.node-code
  X_dis-grp-rule.templ-rl-root
  entry (lookup (X_dis-grp-rule.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U) @ v-pos-type
  X_dis-grp-rule.obj-type
  X_dis-grp-rule.obj-code
  (if p-classif-type = 'sum-grp':U then entry (lookup (X_dis-grp-rule.discnt-role, 'gds-grp-pcnt,gds-grp-pcnt-kat,gds-grp-abs,gds-grp-qnty,gds-grp-sum':u) + 1, ',' + '% скидка на группу товара,% скидка на группу товара для кат.клиентов,Abs скидка на группу товара,% Скидка на группу товара по кол-ву,% Скидка на группу товара на сумму':u) else entry (lookup (X_dis-grp-rule.discnt-role, 'cli-grp-pcnt':u) + 1, ',' + '% скидка на группу клиентов':u)) @ v-d-name
  X_dis-grp-rule.rule-num
  X_dis-grp-rule.rl-root
  with frame List .
