...
assign
  v-str-recid4 = trim( string( recid( X_rule-profile ) , "->>>>>>>>>>>9":U ) )
  v-num-entry4 = lookup( v-str-recid4 , v-ref-list )
.
if v-num-entry4 > 0 then do:
  assign
    entry( v-num-entry4, v-ref-list ) = "":U
    v-ref-list = trim( replace( v-ref-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-ref-list = v-ref-list + ( if v-ref-list = "":U then "":U else chr(44) ) + v-str-recid4
  .
end.
  loc#log = br-rule-profile:refresh() .
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      loc#log = br-rule-profile:select-next-row ().
      apply "iteration-changed" to br-rule-profile in frame Dialog-Frame.
  end.
  if num-entries( v-ref-list ) = 0
  then
      hide mark-num in frame Dialog-Frame.
  else
      disp num-entries( v-ref-list ) @ mark-num with frame Dialog-Frame.
end.
...
do:
  br-rule-profile :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
   if available X_rule-profile then ri = recid(X_rule-profile).     RUn OpenBr in this-procedure .     reposition br-rule-profile to recid ri no-error.
...
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
 ...
 if num-entries(p-list-mode, chr(4)) > 1 then do:
   v-obj-mode = entry(2, p-list-mode, chr(4)) = 'маг':U
               or
               entry(2, p-list-mode, chr(4)) = 'скл':U.
   p-list-mode = entry(1, p-list-mode, chr(4) ).
 end.
 IF LOOKUP(p-list-mode, 'dis-card-type,goods,clients,gds-grp,cli-grp,chk-doc_IBS-TH,chk-doc_IBS-TH-MOB,edoc,thref,pdf,rep,ord,cmb,fdoc':U) = 0 THEN DO:
   message
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-rule-profile FOR EACH X_rule-profile NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
