...
END.
ON CHOOSE OF b-add IN FRAME d-cash-pay
DO:
  define variable glog as logical no-undo .
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_payments-types_input-deletion-updating':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
...
END.
ON CHOOSE OF B-attr IN FRAME d-cash-pay
DO:
...
do:
  ...
  attr-option = "":U.
END.
ON CHOOSE OF b-chg IN FRAME d-cash-pay
DO:
define variable glog as logical no-undo .
...
END.
ON CHOOSE OF B-del IN FRAME d-cash-pay
DO:
...
apply "ENTRY" to br-cashpay.
END.
ON CHOOSE OF B-disc IN FRAME d-cash-pay
DO:
  if not available X_cash-pay THEN return no-apply.
  DEFINE VARIABLE glog AS LOGICAL NO-UNDO.
...
END.
ON CHOOSE OF b-hist IN FRAME d-cash-pay
DO:
  define variable v-rid-list as character no-undo.
 if not available X_cash-pay then return no-apply.
      run ref/ccashpay.w (
    ...
    apply "entry" to br-cashpay.
END.
ON CHOOSE OF b-lkp IN FRAME d-cash-pay
DO:
    if not available X_cash-pay THEN
        return no-apply.
    rr = recid( X_cash-pay ).
...
assign
  v-str-recid17 = trim( string( recid( X_cash-pay ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry17 > 0 then do:
 ...
 glog = br-cashpay  :refresh( ) in frame d-cash-pay.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
          glog = br-cashpay:select-next-row () in frame d-cash-pay.
          apply "iteration-changed" to br-cashpay in frame d-cash-pay.
  end.
  if num-entries (v-rid-list) = 0 then
      hide mark-num in frame d-cash-pay.
  else
  disp num-entries (v-rid-list) @ mark-num
  with frame d-cash-pay.
  apply "entry" to br-cashpay in frame d-cash-pay.
END.
ON CHOOSE OF b-print IN FRAME d-cash-pay
DO:
...
END.
ON CHOOSE OF B-sch IN FRAME d-cash-pay
DO:
  run proc-b-sch IN THIS-PROCEDURE NO-ERROR.
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
ON CHOOSE OF b-sel IN FRAME d-cash-pay
DO:
...
DO:
    if can-do( bttns, "b-sel" ) THEN do:
      apply "choose" to b-sel in frame d-cash-pay .
      return no-apply .
    end.
END.
...
DO:
    ...
    END.
END.
ON RETURN OF f-obj-name IN FRAME d-cash-pay
OR ctrl-J OF f-obj-name IN FRAME d-cash-pay DO:
 DEFINE VARIABLE v-next AS LOGICAL NO-UNDO.
 IF LAST-EVENT:LABEL = "CTRL-J" THEN DO:
   v-next = YES.
 END.
  assign f-obj-name.
  run proc-find in THIS-PROCEDURE ( INPUT v-next, input frame d-cash-pay f-obj-name) no-error.
END.
...
DO:
  ...
  .
  APPLY "CHOOSE" TO b-attr IN FRAME d-cash-pay.
END.
ON CHOOSE OF MENU-ITEM m_lookup-disc
DO:
  assign
  dcpr-option = 'ПРОСМОТР':U
  .
  APPLY "CHOOSE" TO b-disc IN FRAME d-cash-pay.
END.
...
DO:
  assign
  dcpr-option = 'ИЗМЕНЕНИЕ':U
  .
  APPLY "CHOOSE" TO b-disc IN FRAME d-cash-pay.
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME d-cash-pay:PARENT eq ?
THEN FRAME d-cash-pay:PARENT = ACTIVE-WINDOW.
define variable vss-include-info19 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame d-cash-pay
do:
  run gbl/app_help.p
...
on choose of b-help in frame d-cash-pay
do:
  apply "help":u to frame d-cash-pay .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame d-cash-pay:width - 0.3
                fh            = frame d-cash-pay:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame d-cash-pay :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame d-cash-pay :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame d-cash-pay :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-cash-pay :height = v-frame-height
          .
          if frame d-cash-pay :scrollable = true
          then do:
            assign
              frame d-cash-pay :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame d-cash-pay :height
      v-frame-virtual-height = frame d-cash-pay :virtual-height
    ...
    assign
      v-field-group-handle = frame d-cash-pay :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame d-cash-pay :virtual-height = frame d-cash-pay :virtual-height + p-change-value
          no-error .
      ...
      assign
        frame d-cash-pay :height = frame d-cash-pay :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame d-cash-pay :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame d-cash-pay :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame d-cash-pay :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame d-cash-pay :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame d-cash-pay :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame d-cash-pay :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-cash-pay :width = v-frame-width
          .
          if frame d-cash-pay :scrollable = true
          then do:
            assign
              frame d-cash-pay :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame d-cash-pay :width
      v-frame-virtual-width = frame d-cash-pay :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame d-cash-pay :virtual-width = frame d-cash-pay :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame d-cash-pay :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame d-cash-pay :width = frame d-cash-pay :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame d-cash-pay :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame d-cash-pay :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame d-cash-pay :height - v-diasize-resize-button :height
                  - 1
                  - (frame d-cash-pay :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame d-cash-pay :width - v-diasize-resize-button :width
                  - 1
                  - (frame d-cash-pay :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame d-cash-pay
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame d-cash-pay :height
      v-col-delta = v-new-col - frame d-cash-pay :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame d-cash-pay :width
      v-diasize-current-frame-height = frame d-cash-pay :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame d-cash-pay :height
        v-diasize-orig-frame-width  = frame d-cash-pay :width
        v-diasize-browse-handle     = browse br-cashpay :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame d-cash-pay :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
on f5 of frame d-cash-pay anywhere
do:
   v-rep-rec = ?. if available X_cash-pay then v-rep-rec = recid(X_cash-pay).                run openbr in this-procedure ( input yes, input no, input '':U) no-error.                REPOSITION br-cashpay to recid v-rep-rec No-ERROR.
    apply "VALUE-CHANGED" to br-cashpay.
end.
define variable vss-include-info23 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on INS of frame d-cash-pay anywhere do:
  if b-mark :sensitive then DO: apply "CHOOSE":U to b-mark in frame d-cash-pay. END.
  return no-apply.
end.
define variable vss-include-info24 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F3 of frame d-cash-pay anywhere do:
  if b-lkp :sensitive then DO: apply "CHOOSE":U to b-lkp in frame d-cash-pay. END.
  return no-apply.
end.
define variable vss-include-info25 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on CTRL-N, CTRL-Т of frame d-cash-pay anywhere do:
  if b-add :sensitive then DO: apply "CHOOSE":U to b-add in frame d-cash-pay. END.
  return no-apply.
end.
define variable vss-include-info26 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F4 of frame d-cash-pay anywhere do:
  if b-chg :sensitive then DO: apply "CHOOSE":U to b-chg in frame d-cash-pay. END.
  return no-apply.
end.
define variable vss-include-info27 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F8 of frame d-cash-pay anywhere do:
  if b-del :sensitive then DO: apply "CHOOSE":U to b-del in frame d-cash-pay. END.
  return no-apply.
end.
define variable vss-include-info28 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame d-cash-pay anywhere do:
  if b-sel :sensitive then DO: apply "CHOOSE":U to b-sel in frame d-cash-pay. END.
  return no-apply.
end.
define variable vss-include-info29 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame d-cash-pay anywhere do:
  if b-quit :sensitive then DO: apply "CHOOSE":U to b-quit in frame d-cash-pay. END.
  return no-apply.
end.
define variable vss-include-info30 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on CTRL-P, CTRL-З of frame d-cash-pay anywhere do:
  if b-print :sensitive then DO: apply "CHOOSE":U to b-print in frame d-cash-pay. END.
  return no-apply.
end.
...
define input parameter p-filter-name as character no-undo .
  do with frame d-cash-pay:
    if p-filter-name > "" then do:
      assign
        frame d-cash-pay:title
          = frame d-cash-pay:title + "   ФИЛЬТР: " + p-filter-name.
      .
      assign
...
ON WINDOW-CLOSE OF FRAME d-cash-pay APPLY "END-ERROR":U TO SELF.
ON END-ERROR, ENDKEY OF FRAME d-cash-pay DO:
   run gbl/markqwa.p (
...
define variable vss-include-info32 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  run mainmenu_getcntxt in parparentproc
    ...
    if num-entries (v-rid-list) = 0 then
        ...
        with frame d-cash-pay.
    apply 'entry':U to br-cashpay.
    WAIT-FOR GO OF FRAME d-cash-pay.
END.
RUN disable_UI.
...
PROCEDURE enable_UI :
ASSIGN
b-attr:MENU-MOUSE IN frame d-cash-pay = 1
B-disc:MENU-MOUSE IN frame d-cash-pay = 1
.
ENABLE
br-cashpay
b-quit
...
case p-list-mode:
  when 'все':U then do:
    ASSIGN
    frame d-cash-pay:title = substitute("&1: ВСЕ", title0)
...
if p-open-query then do:
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-cashpay FOR EACH X_cash-pay
      where  true
       BY X_cash-pay.cdpay-code
      INDEXED-REPOSITION
  .
  ...
  assign
    v-rep-rec = recid( X_cash-pay )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if QUERY br-cashpay:handle:get-buffer-handle(1) = (buffer X_cash-pay:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input QUERY br-cashpay:handle
                          ,input rowid(X_cash-pay)
                          ,input logical(parameter-2-34)
                          ,input no-lock
                          ,input (buffer X_cash-pay:handle)
                          ,input parameter-4-34
                          ,input parameter-5-34
                          ) no-error.
      .
      assign
        v-rep-rec = integer(return-value)
        v-fltopend-rowid[1] = ?
      .
    end.
    else do:
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_cash-pay"
      parameter-4-34 =
        (
          if (" true " + " " + where-phrase-34) <> ""
          then " true " + " " + where-phrase-34
  ...
  when "with-return" then do:
    ASSIGN
    frame d-cash-pay:title = substitute("&1 для которых разрешен возрат", title0)
    filter-label = SUBSTITUTE("&1"
                              , frame d-cash-pay:title
                              )
    .
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if (" X_cash-pay.has-return = 1 " + " " + where-phrase-36) <> ""
          then " X_cash-pay.has-return = 1 " + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          (" X_cash-pay.has-return = 1 " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-cashpay:handle
  ...
  :
    if QUERY br-cashpay:handle:get-buffer-handle(1) = (buffer X_cash-pay:handle) then do:
      ...
      parameter-4-36 =
        "where ":u + " X_cash-pay.has-return = 1 " + " ":u + where-phrase-36 + " ":u + p-find-condition + " " + ""
      parameter-5-36 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_cash-pay"
      parameter-4-36 =
        (
          if (" X_cash-pay.has-return = 1 " + " " + where-phrase-36) <> ""
          then " X_cash-pay.has-return = 1 " + " " + where-phrase-36
...
APPLY "entry" TO br-cashpay.
if available X_cash-pay then do:
    APPLY "VALUE-CHANGED":U to br-cashpay.
    END.
END PROCEDURE.
...
assign
p-obj-name = replace(p-obj-name, chr(34), "":U)
p-obj-name = replace(p-obj-name, chr(39), chr(39) + chr(39))
p-obj-name = chr(34) + p-obj-name + chr(34).
run OpenBr in this-procedure
    (input false
    ,input p-next
    ,input substitute(" and X_cash-pay.obj-name  begins &1 " , p-obj-name)
    ).
apply "entry":u to f-obj-name in frame d-cash-pay .
END PROCEDURE.
FUNCTION get-grp-name RETURNS CHARACTER
