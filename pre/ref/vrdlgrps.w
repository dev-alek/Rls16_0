...
DEFINE BUFFER X_var-deliv-gr-per-val FOR ub.var-deliv-gr-per-val.
...
define variable vss-description AS CHAR NO-UNDO INIT "Список ВАРИАНТОВ ДОСТАВКИ ПО ГРУППАМ СРОКОВ ГОДНОСТИ":U.
...
define variable filter-label as character no-undo init "Список вариантов доставки по группам сроков годности" .
define variable filter-label0 as character no-undo init "Список вариантов доставки по группам сроков годности" .
define variable filter-point as character no-undo init "vrdlgrps" .
define variable filter-point0 as character no-undo init "vrdlgrps" .
define variable v-rid-list as character no-undo .
DEFINE VARIABLE v-term-delivery LIKE ub.variant-delivery.term-delivery NO-UNDO.
DEFINE VARIABLE v-gr-per-from LIKE ub.group-period-validity.gr-per-from NO-UNDO.
DEFINE VARIABLE v-gr-per-to  LIKE ub.group-period-validity.gr-per-to NO-UNDO.
define buffer pos_var-deliv-gr-per-val for ub.var-deliv-gr-per-val.
FUNCTION get-gr-per-val RETURNS CHARACTER
  ( INPUT p-gr-per-val-code AS integer, OUTPUT p-gr-per-from AS INTEGER, OUTPUT p-gr-per-to AS INTEGER )  FORWARD.
FUNCTION get-subject RETURNS CHARACTER
  ( INPUT p-deliv-subj-code AS INTEGER)  FORWARD.
FUNCTION get-term-delivery RETURNS INTEGER
  ( INPUT p-deliv-type-code AS INTEGER,
    INPUT p-deliv-subj-code AS INTEGER,
    INPUT p-deliv-obj-type AS CHARACTER,
    INPUT p-deliv-obj-code AS INTEGER)  FORWARD.
...
DEFINE QUERY br-vardeliv FOR
      X_var-deliv-gr-per-val SCROLLING.
DEFINE BROWSE br-vardeliv
  QUERY br-vardeliv NO-LOCK DISPLAY
      mark-string(recid( X_var-deliv-gr-per-val), v-rid-list) COLUMN-LABEL "*" FORMAT "X(1)":U
      entry (lookup (STRING(X_var-deliv-gr-per-val.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
      X_var-deliv-gr-per-val.deliv-type-code COLUMN-LABEL "Вн.код!типа!доставки" FORMAT ">>9":U
      get-type(X_var-deliv-gr-per-val.deliv-type-code) COLUMN-LABEL "Тип доставки" FORMAT "X(25)":U
      X_var-deliv-gr-per-val.deliv-subj-code COLUMN-LABEL "Вн.код!субъекта!доставки" FORMAT ">>9":U
      get-subject(X_var-deliv-gr-per-val.deliv-subj-code) COLUMN-LABEL "Субъект доставки" FORMAT "X(25)":U
      X_var-deliv-gr-per-val.obj-type + string(X_var-deliv-gr-per-val.obj-code) COLUMN-LABEL "Объект!доставки" FORMAT "X(8)":U
      X_var-deliv-gr-per-val.gr-per-val-code COLUMN-LABEL "Код группы!сроков!хранения" FORMAT ">>9":U
            WIDTH 8
      get-gr-per-val(X_var-deliv-gr-per-val.gr-per-val-code, output v-gr-per-from, output v-gr-per-to) COLUMN-LABEL "Группа сроков годности" FORMAT "X(25)":U
      get-term-delivery(X_var-deliv-gr-per-val.deliv-type-code,
X_var-deliv-gr-per-val.deliv-subj-code,
X_var-deliv-gr-per-val.obj-type,
X_var-deliv-gr-per-val.obj-code ) COLUMN-LABEL "Срок!доставки" FORMAT ">>>9":U
      string(v-gr-per-from) + "-":U + string(v-gr-per-to) COLUMN-LABEL "Срок!годности" FORMAT "X(10)":U
      X_var-deliv-gr-per-val.des FORMAT "X(100)":U WIDTH 50
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_delivery-storage_work':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
...
run ref/vrdlgrpi.w
              (
                ...
                ,input 'ДОБАВЛЕНИЕ':U
                ,input (if p-mode = "delivery-type-subject" or p-mode = "variant-delivery"
                        then p-deliv-type-code
                        else 0)
                ,input (if p-mode = "delivery-type-subject" or p-mode = "variant-delivery"
                        then p-deliv-subj-code
                        else 0)
                ,input (if p-mode = 'объект':U or p-mode = "group-period-validity":U
                        or p-mode = "variant-delivery"
                ...
                ,input (if p-mode = "group-period-validity"
...
if loc-doc-rec <> ? then do:
  ...
  if error-status:error then do:                           find first pos_var-deliv-gr-per-val no-lock where                                   recid(pos_var-deliv-gr-per-val) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ВАРИАНТ ДОСТАВКИ ПО ГРУППАМ СРОКОВ ГОДНОСТИ" skip                            string(if avail pos_var-deliv-gr-per-val                                     then  substitute("Вн код типа доставки: &1 вн код субъекта доставки: &2 объект доставки: &3&4 вн уод группы сроков годности &5"                                                     , pos_var-deliv-gr-per-val.deliv-type-code                                                     , pos_var-deliv-gr-per-val.deliv-subj-code                                                     , pos_var-deliv-gr-per-val.obj-type                                                     , pos_var-deliv-gr-per-val.obj-code                                                     , pos_var-deliv-gr-per-val.gr-per-val-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
end.
apply "entry" to br-vardeliv in frame Dialog-Frame.
apply "value-changed" to br-vardeliv in frame Dialog-Frame.
END.
ON CHOOSE OF B-chg IN FRAME Dialog-Frame
DO:
  DEFINE variable loc#log as logical no-undo.
define variable loc-doc-rec as recid no-undo .
if not available X_var-deliv-gr-per-val then return no-apply.
...
if not loc#log then return no-apply.
assign
loc-doc-rec = recid(X_var-deliv-gr-per-val).
run ref/vrdlgrpi.w
              (
                 input parParentProc
                ,input p-curr-obj-type
                ,input p-curr-obj-code
                ,input 'ИЗМЕНЕНИЕ':U
                ,input X_var-deliv-gr-per-val.deliv-type-code
                ,input X_var-deliv-gr-per-val.deliv-subj-code
                ,input X_var-deliv-gr-per-val.obj-type
                ,input X_var-deliv-gr-per-val.obj-code
                ,input X_var-deliv-gr-per-val.gr-per-val-code
                ,input-output loc-doc-rec
                            ) no-error
.
if loc-doc-rec <> ? then do:
...
DO:
    ...
    then
    v-rid-list = string( recid( X_var-deliv-gr-per-val ) ) .
  end.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-vardeliv :handle
      .
...
def var sort-labelbr-vardeliv   as character no-undo .
def var sort-clmnbr-vardeliv    as handle    no-undo .
def var cur-clmnbr-vardeliv     as handle    no-undo .
def var cur-clmn-locbr-vardeliv as integer   no-undo .
def var re-querybr-vardeliv     as logical   initial no no-undo .
on start-search, ctrl-o of br-vardeliv in frame Dialog-Frame do:
   run sort-brbr-vardeliv
     (input (if available X_var-deliv-gr-per-val
             then recid(X_var-deliv-gr-per-val)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-vardeliv :
  define input parameter p-recid as recid no-undo .
  if re-querybr-vardeliv = no then do:
    assign
       cur-clmnbr-vardeliv = br-vardeliv:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-vardeliv <> ? then sort-clmnbr-vardeliv:column-fgcolor = 0.
    if cur-clmnbr-vardeliv = sort-clmnbr-vardeliv then do:
      assign
         sort-labelbr-vardeliv = ""
         sort-clmnbr-vardeliv = ?
      .
     end.
     else do:
       assign
         sort-labelbr-vardeliv = cur-clmnbr-vardeliv:label
         sort-clmnbr-vardeliv  = cur-clmnbr-vardeliv
         sort-clmnbr-vardeliv:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-vardeliv = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-vardeliv:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-vardeliv then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-vardeliv = cur-clmn-locbr-vardeliv + 1
    .
  end.
  case sort-labelbr-vardeliv:
        when X_var-deliv-gr-per-val.deliv-type-code:label in browse br-vardeliv then DO:    assign       sort-column-name = "X_var-deliv-gr-per-val.deliv-type-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_var-deliv-gr-per-val.deliv-subj-code:label in browse br-vardeliv then DO:    assign       sort-column-name = "X_var-deliv-gr-per-val.deliv-subj-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-vardeliv') then do:
          run mv-brw-defaultbr-vardeliv.
        end.
      if sort-labelbr-vardeliv <> "" then do:
        assign
          cur-clmnbr-vardeliv:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-vardeliv = ?
      .
    end.
  end case.
    if cur-clmn-locbr-vardeliv <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-vardeliv') then do:
        run ch-clmnbr-vardeliv in this-procedure (cur-clmn-locbr-vardeliv).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-vardeliv to recid p-recid no-error.
    apply "value-changed" to br-vardeliv in frame Dialog-Frame.
  end.
  apply "entry" to br-vardeliv in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-vardeliv:
if cur-clmnbr-vardeliv = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U).
end.
else do:
   assign re-querybr-vardeliv = yes.
   run sort-brbr-vardeliv
     (input (if available X_var-deliv-gr-per-val
             then recid(X_var-deliv-gr-per-val)
             else ?
   ...
   assign re-querybr-vardeliv = no.
end.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
DEF VAR cur-clmn-numbr-vardeliv as INT EXTENT 9 no-undo.
DEF VAR varmvibr-vardeliv       as INT no-undo.
DEF VAR varmvjbr-vardeliv       as INT no-undo.
DEF VAR varmvkbr-vardeliv       as INT no-undo.
DEF VAR varmvlbr-vardeliv       as INT no-undo.
DEF VAR move-elementbr-vardeliv as INT no-undo.
def var jjbr-vardeliv           as int no-undo.
do varmvibr-vardeliv = 1 to EXTENT(cur-clmn-numbr-vardeliv):
  ASSIGN cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = varmvibr-vardeliv.
END.
RUN start-mv-clmnbr-vardeliv.
PROCEDURE start-mv-clmnbr-vardeliv:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-vardeliv = NUM-ENTRIES('1,2,3,4,5,6,7,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-vardeliv ( cur-clmn-numbr-vardeliv[INTEGER(ENTRY (jjbr-vardeliv, '1,2,3,4,5,6,7,8,9'))] , 1).
   END.
       END.
       IF  p-mode = 'delivery-type-subject'  THEN DO:
   DO jjbr-vardeliv = NUM-ENTRIES('1,2,5,6,7,8,9,3,4') TO 1 BY -1:
     RUN re-move-clmnbr-vardeliv ( cur-clmn-numbr-vardeliv[INTEGER(ENTRY (jjbr-vardeliv, '1,2,5,6,7,8,9,3,4'))] , 1).
   END.
       END.
       IF  p-mode = 'объект':U  THEN DO:
   DO jjbr-vardeliv = NUM-ENTRIES('1,2,3,4,5,6,8,9,7') TO 1 BY -1:
     RUN re-move-clmnbr-vardeliv ( cur-clmn-numbr-vardeliv[INTEGER(ENTRY (jjbr-vardeliv, '1,2,3,4,5,6,8,9,7'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-vardeliv do:
  RUN re-move-clmnbr-vardeliv ( 1, 9).
END.
ON ctrl-cursor-left OF BROWSE br-vardeliv do:
  RUN re-move-clmnbr-vardeliv (9, 1).
END.
PROCEDURE re-move-clmnbr-vardeliv:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-vardeliv = 1 TO EXTENT(cur-clmn-numbr-vardeliv):
    if cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = source-column THEN cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = -1.
  END.
  if br-vardeliv:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-vardeliv = source-column - 1 to target-column BY -1:
    DO varmvibr-vardeliv = 1 TO EXTENT(cur-clmn-numbr-vardeliv):
        if cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = varmvjbr-vardeliv THEN DO:
          cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = cur-clmn-numbr-vardeliv[varmvibr-vardeliv] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-vardeliv = source-column + 1 to target-column:
    DO varmvibr-vardeliv = 1 TO EXTENT(cur-clmn-numbr-vardeliv):
      if cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = varmvjbr-vardeliv THEN DO:
        cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = cur-clmn-numbr-vardeliv[varmvibr-vardeliv] - 1.
      END.
  ...
  END.
  DO varmvibr-vardeliv = 1 TO EXTENT(cur-clmn-numbr-vardeliv):
    if cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = -1 THEN cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-vardeliv:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-vardeliv = 1 TO EXTENT(cur-clmn-numbr-vardeliv):
    if cur-clmn-numbr-vardeliv[varmvibr-vardeliv] = cur-clmn-loc THEN move-elementbr-vardeliv = varmvibr-vardeliv.
  END.
  RUN re-move-clmnbr-vardeliv (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-vardeliv:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-vardeliv = 1 to EXTENT(cur-clmn-numbr-vardeliv):
    RUN re-move-clmnbr-vardeliv (cur-clmn-numbr-vardeliv[varmvlbr-vardeliv], varmvlbr-vardeliv).
  END.
  RUN start-mv-clmnbr-vardeliv.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
do:
  br-vardeliv :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
define variable vss-include-info29 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
 ...
 if LOOKUP(p-mode, ('все':U + chr(4) +
                    "delivery-type-subject":U + chr(4) +
                    'объект':U + chr(4) +
                    "variant-delivery":U + chr(4) +
                    "group-period-validity":U ) ,
                chr(4)) = 0
     then dO:
    message
    vss-workfile vss-revision vss-description skip
 ...
 THEN DO:
   ...
   END.
 END.
IF p-mode = "delivery-type-subject":U
or p-deliv-subj-code <> 0
THEN DO:
    ...
    IF NOT AVAILABLE X_delivery-subject THEN DO:
        MESSAGE
        vss-workfile vss-revision vss-description skip
        "Неверное значение параметров вызова p-deliv-subj-code" skip
        p-deliv-subj-code
        view-as alert-box ERROR.
        return error .
   END.
 END.
 if p-mode = 'объект':U
 or p-mode = "group-period-validity":U
 or p-deliv-obj-type <> "":U
 ...
 if p-mode = "variant-delivery":U then do:
     FIND FIRST X_variant-delivery NO-LOCK WHERE
              X_variant-delivery.deliv-type-code = p-deliv-type-code
           AND X_variant-delivery.deliv-subj-code = p-deliv-subj-code
           AND X_variant-delivery.obj-type = p-deliv-obj-type
           AND X_variant-delivery.obj-code = p-deliv-obj-code        NO-ERROR.
    IF NOT AVAILABLE X_variant-delivery THEN DO:
        MESSAGE
        vss-workfile vss-revision vss-description skip
        "Неверное значение параметров вызова p-deliv-subj-code"
        "и/или p-deliv-type-code" skip
        "и/или p-deliv-obj-type p-deliv-obj-code" skip
        p-DELIV-type-CODE  p-DELIV-subj-CODE p-deliv-obj-type p-deliv-obj-code
        view-as alert-box ERROR.
        return error .
   END.
 end.
 if p-mode = "group-period-validity":U then do:
    ...
    IF NOT AVAILABLE X_group-period-validity THEN DO:
        MESSAGE
        vss-workfile vss-revision vss-description skip
        "Неверное значение параметров вызова p-gr-per-val-code" skip
        p-gr-per-val-code
        view-as alert-box ERROR.
        return error .
   END.
...
PROCEDURE enable_UI :
  ...
  ENABLE b-quit B-mark B-sel B-add b-lkp B-chg B-del B-print B-hist B-sch
         B-Help RS-sts br-vardeliv mark-num
      WITH FRAME Dialog-Frame.
  VIEW FRAME Dialog-Frame.
  OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
title0 = "Список вариантов доставки по группам сроков годности" + chr(32).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-32 as character no-undo .
      assign
      parameter-3-32 =
                              "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-32 =
        (
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where  TRUE
      indexed-reposition
  .
      assign
  ...
  assign
    v-doc-rec = recid( X_var-deliv-gr-per-val )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input QUERY br-vardeliv:handle
                          ,input rowid(X_var-deliv-gr-per-val)
                          ,input logical(parameter-2-32)
                          ,input no-lock
                          ,input (buffer X_var-deliv-gr-per-val:handle)
                          ,input parameter-4-32
                          ,input parameter-5-32
                          ) no-error.
      .
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-32 =
        (
          if (" TRUE " + " " + where-phrase-32) <> ""
          then " TRUE " + " " + where-phrase-32
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_var-deliv-gr-per-val.sts = &1', p-sts ) + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where  X_var-deliv-gr-per-val.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_var-deliv-gr-per-val.sts = &1', p-sts ) + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-34 =
        (
          if (" X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_var-deliv-gr-per-val.sts = &1', p-sts ) + " " + where-phrase-34
  ...
  run waitfram-hide in this-procedure .
    ...
    WHEN "delivery-type-subject" THEN DO:
       ...
       frame Dialog-Frame:TITLE = title0 +
                                   ...
                                   chr(32) + (if p-sts = ? then "":U else  entry (lookup (STRING(p-sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U)).
      IF p-sts = ? THEN DO:
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if ("               X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code                              " + " " + where-phrase-36) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1              AND X_var-deliv-gr-per-val.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)  + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          ("               X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code                              " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-36 =
        "where ":u +  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1              AND X_var-deliv-gr-per-val.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)  + " ":u + where-phrase-36 + " ":u + p-find-condition + " " + ""
      parameter-5-36 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-36 =
        (
          if ("               X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code                              " + " " + where-phrase-36) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1              AND X_var-deliv-gr-per-val.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)  + " " + where-phrase-36
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-38 as character no-undo .
      ...
      parameter-4-38 =
        (
          if ("               X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code              AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-38) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1              AND X_var-deliv-gr-per-val.deliv-subj-code  = &2             AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-type-code, p-deliv-subj-code , p-sts) + " " + where-phrase-38
          else "true"
        )
      parameter-5-38 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-38 =
          ("               X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code              AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-38 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-38 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code              AND X_var-deliv-gr-per-val.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-38 =
        "where ":u +  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1              AND X_var-deliv-gr-per-val.deliv-subj-code  = &2             AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-type-code, p-deliv-subj-code , p-sts) + " ":u + where-phrase-38 + " ":u + p-find-condition + " " + ""
      parameter-5-38 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-38 = (if p-find-next then "true":u else "false":u )
      parameter-3-38 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-38 =
        (
          if ("               X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code              AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code              AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-38) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1              AND X_var-deliv-gr-per-val.deliv-subj-code  = &2             AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-type-code, p-deliv-subj-code , p-sts) + " " + where-phrase-38
  ...
  run waitfram-hide in this-procedure .
      END.
    END.
    WHEN "variant-delivery" THEN DO:
       assign
       filter-point = filter-point0 + p-mode
       filter-label = substitute("&1 Один тип доставки, один субъект доставки, Один объект доставки", filter-label0)
       .
       ASSIGN
       frame Dialog-Frame:TITLE = title0 +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-40 as character no-undo .
      ...
      parameter-4-40 =
        (
          if ("                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code               AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code                               " + " " + where-phrase-40) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1               AND X_var-deliv-gr-per-val.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)  + " " + where-phrase-40
          else "true"
        )
      parameter-5-40 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-40 =
          ("                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code               AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code                               " + " " + where-phrase-40 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-40 =
        "where ":u +  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1               AND X_var-deliv-gr-per-val.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)  + " ":u + where-phrase-40 + " ":u + p-find-condition + " " + ""
      parameter-5-40 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-40 = (if p-find-next then "true":u else "false":u )
      parameter-3-40 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-40 =
        (
          if ("                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code               AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code                               " + " " + where-phrase-40) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1               AND X_var-deliv-gr-per-val.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)  + " " + where-phrase-40
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-42 as character no-undo .
      ...
      parameter-4-42 =
        (
          if ("                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code               AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code               AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-42) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1               AND X_var-deliv-gr-per-val.deliv-subj-code  = &2               AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-type-code, p-deliv-subj-code, p-sts) + " " + where-phrase-42
          else "true"
        )
      parameter-5-42 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-42 =
          ("                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code               AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code               AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-42 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-42 =
        "where ":u +  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1               AND X_var-deliv-gr-per-val.deliv-subj-code  = &2               AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-type-code, p-deliv-subj-code, p-sts) + " ":u + where-phrase-42 + " ":u + p-find-condition + " " + ""
      parameter-5-42 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-42 = (if p-find-next then "true":u else "false":u )
      parameter-3-42 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-42 =
        (
          if ("                X_var-deliv-gr-per-val.deliv-type-code  = p-deliv-type-code               AND X_var-deliv-gr-per-val.deliv-subj-code  = p-deliv-subj-code               AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-42) <> ""
          then  substitute(' X_var-deliv-gr-per-val.deliv-type-code  = &1               AND X_var-deliv-gr-per-val.deliv-subj-code  = &2               AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-type-code, p-deliv-subj-code, p-sts) + " " + where-phrase-42
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-44 as character no-undo .
      ...
      parameter-4-44 =
        (
          if ("               X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code                              " + " " + where-phrase-44) <> ""
          then  substitute(' X_var-deliv-gr-per-val.obj-type  = &1              AND X_var-deliv-gr-per-val.obj-code  = &2 ', p-deliv-obj-type, p-deliv-obj-code)  + " " + where-phrase-44
          else "true"
        )
      parameter-5-44 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-44 =
          ("               X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code                              " + " " + where-phrase-44 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-44 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-44 =
        "where ":u +  substitute(' X_var-deliv-gr-per-val.obj-type  = &1              AND X_var-deliv-gr-per-val.obj-code  = &2 ', p-deliv-obj-type, p-deliv-obj-code)  + " ":u + where-phrase-44 + " ":u + p-find-condition + " " + ""
      parameter-5-44 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-44 = (if p-find-next then "true":u else "false":u )
      parameter-3-44 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-44 =
        (
          if ("               X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code                              " + " " + where-phrase-44) <> ""
          then  substitute(' X_var-deliv-gr-per-val.obj-type  = &1              AND X_var-deliv-gr-per-val.obj-code  = &2 ', p-deliv-obj-type, p-deliv-obj-code)  + " " + where-phrase-44
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-46 as character no-undo .
      ...
      parameter-4-46 =
        (
          if ("               X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-46) <> ""
          then  substitute(' X_var-deliv-gr-per-val.obj-type  = &1             AND X_var-deliv-gr-per-val.obj-code  = &2              AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-sts ) + " " + where-phrase-46
          else "true"
        )
      parameter-5-46 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-46 =
          ("               X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-46 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-46 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND X_var-deliv-gr-per-val.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-46 =
        "where ":u +  substitute(' X_var-deliv-gr-per-val.obj-type  = &1             AND X_var-deliv-gr-per-val.obj-code  = &2              AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-sts ) + " ":u + where-phrase-46 + " ":u + p-find-condition + " " + ""
      parameter-5-46 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-46 = (if p-find-next then "true":u else "false":u )
      parameter-3-46 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-46 =
        (
          if ("               X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type              AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-46) <> ""
          then  substitute(' X_var-deliv-gr-per-val.obj-type  = &1             AND X_var-deliv-gr-per-val.obj-code  = &2              AND X_var-deliv-gr-per-val.sts = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-sts ) + " " + where-phrase-46
  ...
  run waitfram-hide in this-procedure .
      END.
  END.
  WHEN "group-period-validity" THEN DO:
      assign
      filter-point = filter-point0 + p-mode
      filter-label = substitute("&1 Один объект доставки, одна группа сроков хранения", filter-label0)
      .
      ASSIGN
      frame Dialog-Frame:TITLE = title0 +
                                  substitute(" Объект доставки: &1&2 Группа сроков хранения: &3"
                                   , X_clients.obj-type
                                   , X_clients.obj-code
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-48 as character no-undo .
      ...
      parameter-4-48 =
        (
          if ("                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code                          " + " " + where-phrase-48) <> ""
          then  substitute('X_var-deliv-gr-per-val.obj-type  = &1               AND X_var-deliv-gr-per-val.obj-code  = &2              AND  X_var-deliv-gr-per-val.gr-per-val-code  = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-gr-per-val-code)  + " " + where-phrase-48
          else "true"
        )
      parameter-5-48 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-48 =
          ("                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code                          " + " " + where-phrase-48 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-48 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where                 X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-48 =
        "where ":u +  substitute('X_var-deliv-gr-per-val.obj-type  = &1               AND X_var-deliv-gr-per-val.obj-code  = &2              AND  X_var-deliv-gr-per-val.gr-per-val-code  = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-gr-per-val-code)  + " ":u + where-phrase-48 + " ":u + p-find-condition + " " + ""
      parameter-5-48 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-48 = (if p-find-next then "true":u else "false":u )
      parameter-3-48 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-48 =
        (
          if ("                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code                          " + " " + where-phrase-48) <> ""
          then  substitute('X_var-deliv-gr-per-val.obj-type  = &1               AND X_var-deliv-gr-per-val.obj-code  = &2              AND  X_var-deliv-gr-per-val.gr-per-val-code  = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-gr-per-val-code)  + " " + where-phrase-48
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-50 as character no-undo .
      ...
      parameter-4-50 =
        (
          if ("                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code           AND  X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-50) <> ""
          then  substitute(' X_var-deliv-gr-per-val.obj-type  = &1               AND X_var-deliv-gr-per-val.obj-code  = &2              AND  X_var-deliv-gr-per-val.gr-per-val-code  = &3           AND  X_var-deliv-gr-per-val.sts = &4 ', p-deliv-obj-type, p-deliv-obj-code, p-gr-per-val-code, p-sts) + " " + where-phrase-50
          else "true"
        )
      parameter-5-50 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-50 =
          ("                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code           AND  X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-50 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-50 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_var-deliv-gr-per-val
      where                 X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code           AND  X_var-deliv-gr-per-val.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_var-deliv-gr-per-val:handle) then do:
      ...
      parameter-4-50 =
        "where ":u +  substitute(' X_var-deliv-gr-per-val.obj-type  = &1               AND X_var-deliv-gr-per-val.obj-code  = &2              AND  X_var-deliv-gr-per-val.gr-per-val-code  = &3           AND  X_var-deliv-gr-per-val.sts = &4 ', p-deliv-obj-type, p-deliv-obj-code, p-gr-per-val-code, p-sts) + " ":u + where-phrase-50 + " ":u + p-find-condition + " " + ""
      parameter-5-50 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-50 = (if p-find-next then "true":u else "false":u )
      parameter-3-50 =  "FOR EACH X_var-deliv-gr-per-val"
      parameter-4-50 =
        (
          if ("                X_var-deliv-gr-per-val.obj-type  = p-deliv-obj-type               AND X_var-deliv-gr-per-val.obj-code  = p-deliv-obj-code              AND  X_var-deliv-gr-per-val.gr-per-val-code  = p-gr-per-val-code           AND  X_var-deliv-gr-per-val.sts = p-sts " + " " + where-phrase-50) <> ""
          then  substitute(' X_var-deliv-gr-per-val.obj-type  = &1               AND X_var-deliv-gr-per-val.obj-code  = &2              AND  X_var-deliv-gr-per-val.gr-per-val-code  = &3           AND  X_var-deliv-gr-per-val.sts = &4 ', p-deliv-obj-type, p-deliv-obj-code, p-gr-per-val-code, p-sts) + " " + where-phrase-50
...
define variable loc#log as logical no-undo.
define variable v-sts like ub.var-deliv-gr-per-val.sts no-undo .
DEFINE VARIABLE loc-doc-rec AS RECID NO-UNDO.
if not available X_var-deliv-gr-per-val then return error.
do
on error undo, return error
on stop undo, return error
:
...
if not loc#log then return error.
  ...
  .
  run ref/dlvtysu2.p (
                  input recid(X_var-deliv-gr-per-val)
                  ,input-output v-sts
                 ) no-error .
  ...
  if error-status:error then do:                           find first pos_var-deliv-gr-per-val no-lock where                                   recid(pos_var-deliv-gr-per-val) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ВАРИАНТ ДОСТАВКИ ПО ГРУППАМ СРОКОВ ГОДНОСТИ" skip                            string(if avail pos_var-deliv-gr-per-val                                     then  substitute("Вн код типа доставки: &1 вн код субъекта доставки: &2 объект доставки: &3&4 вн уод группы сроков годности &5"                                                     , pos_var-deliv-gr-per-val.deliv-type-code                                                     , pos_var-deliv-gr-per-val.deliv-subj-code                                                     , pos_var-deliv-gr-per-val.obj-type                                                     , pos_var-deliv-gr-per-val.obj-code                                                     , pos_var-deliv-gr-per-val.gr-per-val-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
  if available X_var-deliv-gr-per-val then do:
    loc#log = br-vardeliv:select-focused-row( ) IN FRAME Dialog-Frame.
  end.
  apply "ENTRY" to br-vardeliv.
end.
END PROCEDURE.
...
DEFINE FRAME var-deliv-gr-per-val-list
v-sts-chr FORMAT "X(8)" COLUMN-LABEL "Статус"
X_var-deliv-gr-per-val.deliv-type-code COLUMN-LABEL "Вн.код!тип доставки"
v-deliv-type-name COLUmn-LABEL "Тип доставки" format "X(25)"
X_var-deliv-gr-per-val.deliv-subj-code COLUMN-LABEL "Вн.код!субъекта!доставки"
v-deliv-subj-name COLUmn-LABEL "Субъект доставки" format "X(25)"
v-obj COLUMn-LABEL "Объект!доставки" format "X(8)"
X_var-deliv-gr-per-val.des FORMAT "X(100)"
...
FORM with FRAME var-deliv-gr-per-val-list  .
run waitfram-show in this-procedure ("Ждите...").
v-doc-rec = recid(X_var-deliv-gr-per-val).
DO WHILE available X_var-deliv-gr-per-val :
  GET prev br-vardeliv.
END.
GET next br-vardeliv.
DO WHILE available X_var-deliv-gr-per-val :
  Display STREAM PrnLibStream
  entry (lookup (STRING(p-sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) @ v-sts-chr
  X_var-deliv-gr-per-val.deliv-type-code
  get-type(X_var-deliv-gr-per-val.deliv-type-code) @ v-deliv-type-name
  X_var-deliv-gr-per-val.deliv-subj-code
  get-subject(X_var-deliv-gr-per-val.deliv-subj-code) @ v-deliv-subj-name
  (X_var-deliv-gr-per-val.obj-type + string(X_var-deliv-gr-per-val.obj-code)) @ v-obj
  X_var-deliv-gr-per-val.des
  with FRAME var-deliv-gr-per-val-list .
...
HIDE  STREAM PrnLibStream FRAME var-deliv-gr-per-val-List.
...
assign
  tbl = 'var-deliv-gr-per-val'
  join-tbl = 'X_var-deliv-gr-per-val'
  fld = ""
  lab = ""
  spr = ""
  dim = '0'
...
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
run fltfield-add in this-procedure('obj-type*obj-code', 'Объект доставки', 'cli',
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
...
END PROCEDURE.
FUNCTION get-gr-per-val RETURNS CHARACTER
  ( INPUT p-gr-per-val-code AS integer, OUTPUT p-gr-per-from AS INTEGER, OUTPUT p-gr-per-to AS INTEGER ) :
DEFINE BUFFER loc_group-period-validity FOR ub.group-period-validity.
FIND FIRST loc_group-period-validity NO-LOCK WHERE
          loc_group-period-validity.gr-per-val-code = p-gr-per-val-code NO-ERROR.
IF NOT AVAILABLE loc_group-period-validity THEN dO:
    ASSIGN
    p-gr-per-from = ?
    p-gr-per-to = ?
    .
    RETURN loc_group-period-validity.gr-per-val-name.
END.
ASSIGN
    p-gr-per-from = loc_group-period-validity.gr-per-from
    p-gr-per-to = loc_group-period-validity.gr-per-to
    .
  RETURN "Неизв.группа сроков годности".
END FUNCTION.
FUNCTION get-subject RETURNS CHARACTER
  ( INPUT p-deliv-subj-code AS INTEGER) :
...
IF AVAILABLE loc_delivery-subject THEN RETURN loc_delivery-subject.deliv-subj-name.
  RETURN "Неизв.субъект доставки".
END FUNCTION.
FUNCTION get-term-delivery RETURNS INTEGER
  ( INPUT p-deliv-type-code AS INTEGER,
    INPUT p-deliv-subj-code AS INTEGER,
    INPUT p-deliv-obj-type AS CHARACTER,
    INPUT p-deliv-obj-code AS INTEGER) :
DEFINE BUFFER loc_variant-delivery  FOR ub.variant-delivery.
FIND FIRST loc_variant-delivery NO-LOCK WHERE
            loc_variant-delivery.deliv-type-code = p-deliv-type-code
        AND loc_variant-delivery.deliv-subj-code = p-deliv-subj-code
        AND loc_variant-delivery.obj-type = p-deliv-obj-type
        AND loc_variant-delivery.obj-code = p-deliv-obj-code NO-ERROR.
 IF NOT AVAILABLE loc_variant-delivery THEN DO:
     RETURN ?.
 END.
  RETURN loc_variant-delivery.term-delivery.
END FUNCTION.
FUNCTION get-type RETURNS CHARACTER
( INPUT p-deliv-type-code AS INTEGER) :
