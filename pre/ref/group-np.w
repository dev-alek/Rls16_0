...
define variable vss-description as character no-undo init "Выбор группы НП".
...
DEFINE FRAME Dialog-Frame
     ...
     cb-office-type AT ROW 2.67 COL 11.25 COLON-ALIGNED WIDGET-ID 6
     SPACE(3.19) SKIP(1.40)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE
         TITLE "Выбор группы НП" WIDGET-ID 100.
ASSIGN
...
END PROCEDURE.
PROCEDURE proc-load :
define variable v-list-names as character no-undo init "I,II,III,IV".
define variable v-list as character no-undo.
define variable i as integer no-undo.
define variable v-val as character no-undo.
define variable v-text as character no-undo.
do i = 1 to num-entries('I,II,III,IV':U):
    v-val = entry(i, 'I,II,III,IV':U).
    v-text = entry(i, v-list-names).
    v-list = v-list + "," + v-text + "," + v-val.
end.
v-list = trim(v-list, ",").
cb-office-type:list-item-pairs in frame Dialog-Frame = v-list.
if p-mode = 'ДОБАВЛЕНИЕ':U OR p-mode = 'ИЗМЕНЕНИЕ':U then do:
    enable cb-office-type with frame Dialog-Frame.
end.
p-setted = false.
cb-office-type = p-attr-value.
display cb-office-type with frame Dialog-Frame.
end procedure.
PROCEDURE proc-save :
