...
DEFINE BUFFER X_c-obj-hist FOR c-cashbook-head.
DEFINE BUFFER find_c-obj-hist FOR c-cashbook-head.
...
DEFINE BROWSE br-obj-hist
  QUERY br-obj-hist NO-LOCK DISPLAY
      ...
      X_c-obj-hist.id COLUMN-LABEL 'Код!КК' FORMAT '>>9':U
...
define input  parameter p-db-num like c-cashbook-head.corr-user-db-num no-undo .
...
define input parameter p-next as logical no-undo.
define input parameter p-user like c-cashbook-head.corr-user-name no-undo.
assign
sch-corr-date = ?.
display
0 @ sch-db-num
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if ("           X_c-obj-hist.id         = pid  or           X_c-obj-hist.cashbookid = pid  " + " " + where-phrase-31) <> ""
          then  substitute('  X_c-obj-hist.id  = &1  or           X_c-obj-hist.cashbookid  = &1 ', pid)   + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          ("           X_c-obj-hist.id         = pid  or           X_c-obj-hist.cashbookid = pid  " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-obj-hist:handle
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-obj-hist FOR EACH X_c-obj-hist
      where            X_c-obj-hist.id         = pid  or           X_c-obj-hist.cashbookid = pid
       by X_c-obj-hist.chip-num
      indexed-reposition
  .
      assign
  ...
  :
    if query br-obj-hist:handle:get-buffer-handle(1) = (buffer X_c-obj-hist:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute('  X_c-obj-hist.id  = &1  or           X_c-obj-hist.cashbookid  = &1 ', pid)   + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = " "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
...
define variable v-label-param as character no-undo .
 ...
 + "CashBookName" + chr(4) + "Наименование"                          + chr(4) + "" + chr(8)
 + "RuleOsn"      + chr(4) + "Правило заполнения графы Основание"    + chr(4) + "" + chr(8)
 + "RulePril"     + chr(4) + "Правило заполнения графы Приложение"   + chr(4) + "" + chr(8)
 + "Status_"      + chr(4) + "Статус"                                + chr(4) + "" + chr(8)
 + "Credit"       + chr(4) + "Значение для заполнения поля «кредит»" + chr(4) + "" + chr(8)
 + "FlagSepCash"  + chr(4) + "Отдельный ПКО для каждой кассы"        + chr(4) + "" + chr(8)
 + "FlagSepFull"  + chr(4) + "Раздельно НП и ТНП"                    + chr(4) + "" + chr(8)
 + "cli-type"     + chr(4) + "Тип контрагента"                       + chr(4) + "" + chr(8)
 + "cli-code"     + chr(4) + "Код контрагента"                       + chr(4) + "" + chr(8)
 + "takenfrom"    + chr(4) + "Принято от"                            + chr(4) + "" + chr(8)
 + "Debit"        + chr(4) + "Значение для заполнения поля «дебет»"  + chr(4) + "" .
  run proc-full-temp-changes in this-procedure (
                                               input current_c-cashbook.action = integer('1':U)
                                              ,input current_c-cashbook.action = integer('99':U)
                                              ,input  buffer current_c-cashbook:handle
                                              ,input  'CashBook':U
                                              ,input  "ext-code,CashBookName,RuleOsn,RulePril,Status_,Credit,FlagSepCash,FlagSepFull,Debit,cli-type,cli-code,takenfrom"
                                              ,input  v-label-param).
end.
end procedure.
  ...
  v-label-param =
   "RuleValue"     + chr(4) +
       ( if  current_c-cashbookrule.Code eq "PkoMask" then "Маска ПКО"
                               ...
                               + chr(4) + "" .
  run proc-full-temp-changes in this-procedure (
                                               input current_c-cashbookrule.action = integer('1':U)
                                              ,input current_c-cashbookrule.action = integer('99':U)
                                              ,input  buffer current_c-cashbookrule:handle
                                              ,input  'CashBookRule':U
                                              ,input  "RuleValue"
                                              ,input  v-label-param).
end.
end procedure.
...
define variable v-label-param as character no-undo .
  ...
  run proc-full-temp-changes in this-procedure (
                                               input current_c-goods-attr-any.action = integer('1':U)
                                              ,input current_c-goods-attr-any.action = integer('99':U)
