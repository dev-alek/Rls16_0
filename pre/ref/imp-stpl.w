...
define variable vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  ELSE DO:
      find last buf_stop-list no-lock where
                buf_stop-list.classif-type = 'dis-card':U no-error .
      if not available buf_stop-list then do:
      end.
      ELSE DO:
         if buf_stop-list.status_ <> 'факт':U then do:
         ...
         p-stop-list-code = STRING(next-value(s-stop-list, ub), "999999999").
      END.
  END.
  RUN Myenable.
...
find first buf_clients no-lock where
      ...
      and buf_clients.stts = integer('0':U) no-error.
assign
...
.
 run str/diallog.w (
              ...
              ,input (file-name                        + chr(4) +
                      string(f-stop-list-code, "999999999") + chr(4) +
                      p-mode + chr(4) +
                      string(t-fact)
                 )
              ,input no
...
then do:
  ...
  and buf_stop-list.status_ = 'факт':U then do:
    run str/diallog.w (
                  input parparentproc
                  ,input this-procedure
