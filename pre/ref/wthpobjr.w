...
DO:
  define variable doc-rec as recid no-undo .
    ...
    apply "entry" to br-wthpobj in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-wthpobj :handle
      .
...
do:
  br-wthpobj :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
   v-doc-rec = ?. if available X_wth-pobj then assign v-doc-rec = recid( X_wth-pobj). run openbr in this-procedure ( input yes, input no, input '':U).                reposition br-wthpobj to recid v-doc-rec no-error. APPLY 'ENTRY' to br-wthpobj. APPLY 'VALUe-CHANGED' to br-wthpobj.
    apply "VALUE-CHANGED" to BR-wthpobj.
end.
MAIN-BLOCK:
...
DEF VAR cur-clmn-numbr-wthpobj as INT EXTENT 9 no-undo.
DEF VAR varmvibr-wthpobj       as INT no-undo.
DEF VAR varmvjbr-wthpobj       as INT no-undo.
DEF VAR varmvkbr-wthpobj       as INT no-undo.
DEF VAR varmvlbr-wthpobj       as INT no-undo.
DEF VAR move-elementbr-wthpobj as INT no-undo.
def var jjbr-wthpobj           as int no-undo.
do varmvibr-wthpobj = 1 to EXTENT(cur-clmn-numbr-wthpobj):
  ASSIGN cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = varmvibr-wthpobj.
END.
RUN start-mv-clmnbr-wthpobj.
PROCEDURE start-mv-clmnbr-wthpobj:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-list-mode = 'Место хранения МЦ':U  THEN DO:
   DO jjbr-wthpobj = NUM-ENTRIES('1,2,5,6,7,9,8,3,4') TO 1 BY -1:
     RUN re-move-clmnbr-wthpobj ( cur-clmn-numbr-wthpobj[INTEGER(ENTRY (jjbr-wthpobj, '1,2,5,6,7,9,8,3,4'))] ,  + 1).
   END.
       END.
       IF  p-list-mode = 'МЦ':U  THEN DO:
   DO jjbr-wthpobj = NUM-ENTRIES('1,2,3,4,9,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-wthpobj ( cur-clmn-numbr-wthpobj[INTEGER(ENTRY (jjbr-wthpobj, '1,2,3,4,9,5,6,7,8'))] ,  + 1).
   END.
       END.
       IF  p-list-mode = 'МЦ':U + chr(47) + 'объект':U  THEN DO:
   DO jjbr-wthpobj = NUM-ENTRIES('1,2,5,6,7,9,8,3,4') TO 1 BY -1:
     RUN re-move-clmnbr-wthpobj ( cur-clmn-numbr-wthpobj[INTEGER(ENTRY (jjbr-wthpobj, '1,2,5,6,7,9,8,3,4'))] ,  + 1).
   END.
       END.
...
END.
ON ctrl-cursor-right OF BROWSE br-wthpobj do:
  RUN re-move-clmnbr-wthpobj (  + 1, 9).
END.
ON ctrl-cursor-left OF BROWSE br-wthpobj do:
  RUN re-move-clmnbr-wthpobj (9,  + 1).
END.
PROCEDURE re-move-clmnbr-wthpobj:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-wthpobj = 1 TO EXTENT(cur-clmn-numbr-wthpobj):
    if cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = source-column THEN cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = -1.
  END.
  if br-wthpobj:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-wthpobj = source-column - 1 to target-column BY -1:
    DO varmvibr-wthpobj = 1 TO EXTENT(cur-clmn-numbr-wthpobj):
        if cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = varmvjbr-wthpobj THEN DO:
          cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = cur-clmn-numbr-wthpobj[varmvibr-wthpobj] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-wthpobj = source-column + 1 to target-column:
    DO varmvibr-wthpobj = 1 TO EXTENT(cur-clmn-numbr-wthpobj):
      if cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = varmvjbr-wthpobj THEN DO:
        cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = cur-clmn-numbr-wthpobj[varmvibr-wthpobj] - 1.
      END.
  ...
  END.
  DO varmvibr-wthpobj = 1 TO EXTENT(cur-clmn-numbr-wthpobj):
    if cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = -1 THEN cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-wthpobj:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <=  + 1 then do:
    return .
  end.
  DO varmvibr-wthpobj = 1 TO EXTENT(cur-clmn-numbr-wthpobj):
    if cur-clmn-numbr-wthpobj[varmvibr-wthpobj] = cur-clmn-loc THEN move-elementbr-wthpobj = varmvibr-wthpobj.
  END.
  RUN re-move-clmnbr-wthpobj (cur-clmn-loc,  + 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-wthpobj:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-wthpobj =  + 1 to EXTENT(cur-clmn-numbr-wthpobj):
    RUN re-move-clmnbr-wthpobj (cur-clmn-numbr-wthpobj[varmvlbr-wthpobj], varmvlbr-wthpobj).
  END.
  RUN start-mv-clmnbr-wthpobj.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
CASE p-list-mode:
    when 'Место хранения МЦ':U then do:
        ASSIGN
        frame Dialog-Frame:TITLE = "Остатки МЦ на месте хранения " + string(pw-p-code)
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-20 as character no-undo .
      ...
      parameter-4-20 =
        (
          if (" X_wth-pobj.w-p-code = pw-p-code                            AND X_wth-pobj.obj-type = p-obj-type                            and X_wth-pobj.obj-code = p-obj-code" + " " + where-phrase-20) <> ""
          then  substitute('X_wth-pobj.w-p-code = &1                            AND X_wth-pobj.obj-type = &2&3&2                            and X_wth-pobj.obj-code = &4 ', pw-p-code, chr(34), p-obj-type, p-obj-code) + " " + where-phrase-20
          else "true"
        )
      parameter-5-20 = (" " + "" + " " + ", FIRST X_wealth No-LOCK WHERE X_wealth.wth-code = X_wth-pobj.wth-code ,first X_wth-place WHERE X_wth-place.w-p-code = X_wth-pobj.w-p-code and X_wth-place.obj-code = X_wth-pobj.obj-code and X_wth-place.obj-type = X_wth-pobj.obj-type")
    ...
    :
      assign
        l-disable-where-20 =
          (" X_wth-pobj.w-p-code = pw-p-code                            AND X_wth-pobj.obj-type = p-obj-type                            and X_wth-pobj.obj-code = p-obj-code" + " " + where-phrase-20 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-wthpobj:handle
  ...
  if l-filter-open-20 = false then do:
    OPEN QUERY br-wthpobj FOR EACH X_wth-pobj
      where  X_wth-pobj.w-p-code = pw-p-code                            AND X_wth-pobj.obj-type = p-obj-type                            and X_wth-pobj.obj-code = p-obj-code
    , FIRST X_wealth No-LOCK WHERE X_wealth.wth-code = X_wth-pobj.wth-code ,first X_wth-place WHERE X_wth-place.w-p-code = X_wth-pobj.w-p-code and X_wth-place.obj-code = X_wth-pobj.obj-code and X_wth-place.obj-type = X_wth-pobj.obj-type
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when ('МЦ':U + chr(47) + 'объект':U) then do:
        ASSIGN frame Dialog-Frame:TITLE = "Остатки МЦ " + string(pwth-code) + " на местах хранения объекта " + p-obj-type + chr(32) + string(p-obj-code)
        filter-point = "Остатки МЦ на месте хранения" + p-list-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-22 as character no-undo .
      ...
      parameter-4-22 =
        (
          if (" X_wth-pobj.wth-code = pwth-code                             AND X_wth-pobj.obj-type = p-obj-type                             and X_wth-pobj.obj-code = p-obj-code" + " " + where-phrase-22) <> ""
          then  substitute('X_wth-pobj.wth-code = &1                             AND X_wth-pobj.obj-type = &2&3&2                             and X_wth-pobj.obj-code = &4', pwth-code, chr(34), p-obj-type, p-obj-code) + " " + where-phrase-22
          else "true"
        )
      parameter-5-22 = (" " + "" + " " + ", FIRST X_wealth No-LOCK WHERE X_wealth.wth-code = X_wth-pobj.wth-code ,first X_wth-place WHERE X_wth-place.w-p-code = X_wth-pobj.w-p-code and X_wth-place.obj-code = X_wth-pobj.obj-code and X_wth-place.obj-type = X_wth-pobj.obj-type")
    ...
    :
      assign
        l-disable-where-22 =
          (" X_wth-pobj.wth-code = pwth-code                             AND X_wth-pobj.obj-type = p-obj-type                             and X_wth-pobj.obj-code = p-obj-code" + " " + where-phrase-22 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-wthpobj:handle
  ...
  if l-filter-open-22 = false then do:
    OPEN QUERY br-wthpobj FOR EACH X_wth-pobj
      where  X_wth-pobj.wth-code = pwth-code                             AND X_wth-pobj.obj-type = p-obj-type                             and X_wth-pobj.obj-code = p-obj-code
    , FIRST X_wealth No-LOCK WHERE X_wealth.wth-code = X_wth-pobj.wth-code ,first X_wth-place WHERE X_wth-place.w-p-code = X_wth-pobj.w-p-code and X_wth-place.obj-code = X_wth-pobj.obj-code and X_wth-place.obj-type = X_wth-pobj.obj-type
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when 'МЦ':U then do:
        ASSIGN
        frame Dialog-Frame:TITLE = "Остатки МЦ " + string(pwth-code) + " на местах хранения"
        filter-point = "Остатки МЦ на месте хранения" + p-list-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-24 as character no-undo .
      ...
      parameter-4-24 =
        (
          if (" X_wth-pobj.wth-code = pwth-code " + " " + where-phrase-24) <> ""
          then  substitute('X_wth-pobj.wth-code = &1', pwth-code ) + " " + where-phrase-24
          else "true"
        )
      parameter-5-24 = (" " + "" + " " + ", FIRST X_wealth No-LOCK WHERE X_wealth.wth-code = X_wth-pobj.wth-code ,first X_wth-place WHERE X_wth-place.w-p-code = X_wth-pobj.w-p-code and X_wth-place.obj-code = X_wth-pobj.obj-code and X_wth-place.obj-type = X_wth-pobj.obj-type")
    ...
    :
      assign
        l-disable-where-24 =
          (" X_wth-pobj.wth-code = pwth-code " + " " + where-phrase-24 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-wthpobj:handle
  ...
  if l-filter-open-24 = false then do:
    OPEN QUERY br-wthpobj FOR EACH X_wth-pobj
      where  X_wth-pobj.wth-code = pwth-code
    , FIRST X_wealth No-LOCK WHERE X_wealth.wth-code = X_wth-pobj.wth-code ,first X_wth-place WHERE X_wth-place.w-p-code = X_wth-pobj.w-p-code and X_wth-place.obj-code = X_wth-pobj.obj-code and X_wth-place.obj-type = X_wth-pobj.obj-type
  .
      assign
        l-query-was-opened = true
...
apply "value-changed" to br-wthpobj in frame Dialog-Frame.
END PROCEDURE.
PROCEDURE PrintProc :
