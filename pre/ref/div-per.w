...
ON CHOOSE OF b-del IN FRAME get-rep
DO:
  IF AVAILABLE temp-date THEN DELETE  temp-date.
OPEN QUERY BROWSE-date FOR EACH temp-date.
END.
ON CHOOSE OF B-more IN FRAME get-rep
DO:
...
END.
ON CHOOSE OF b-quit IN FRAME get-rep
DO:
...
DO:
    APPLY "CHOOSE" TO b-ok IN FRAME get-rep.
    RETURN NO-APPLY.
END.
...
on delete-character of date-b in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of date-b in frame get-rep
do:
...
on ctrl-b of date-b in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of date-b in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of date-b in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date2
    ...
    .
  if date-b :POPUP-MENU in frame get-rep = ?
  then do:
    ASSIGN
      date-b :POPUP-MENU in frame get-rep = MENU m-ed-date2 :HANDLE
      date-b :MENU-MOUSE in frame get-rep = 3
    .
  end.
  define variable v-label-handle2 as handle no-undo .
  assign
    v-label-handle2 = date-b :side-label-handle in frame get-rep
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date2-1 in menu m-ed-date2 DO:
    apply "ctrl-b":U to date-b in frame get-rep .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date2-2 in menu m-ed-date2 DO:
    apply "ctrl-d":U to date-b in frame get-rep .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date2-3 in menu m-ed-date2 DO:
    apply "ctrl-e":U to date-b in frame get-rep .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date2-4 in menu m-ed-date2 DO:
    apply "ctrl-f":U to date-b in frame get-rep .
  END.
define variable vss-include-info3 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on ' ' of date-e in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on delete-character of date-e in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of date-e in frame get-rep
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of date-e in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of date-e in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of date-e in frame get-rep
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date4
    ...
    .
  if date-e :POPUP-MENU in frame get-rep = ?
  then do:
    ASSIGN
      date-e :POPUP-MENU in frame get-rep = MENU m-ed-date4 :HANDLE
      date-e :MENU-MOUSE in frame get-rep = 3
    .
  end.
  define variable v-label-handle4 as handle no-undo .
  assign
    v-label-handle4 = date-e :side-label-handle in frame get-rep
  .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame get-rep :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame get-rep :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame get-rep :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame get-rep :height = v-frame-height
          .
          if frame get-rep :scrollable = true
          then do:
            assign
              frame get-rep :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame get-rep :height
      v-frame-virtual-height = frame get-rep :virtual-height
    ...
    assign
      v-field-group-handle = frame get-rep :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame get-rep :virtual-height = frame get-rep :virtual-height + p-change-value
          no-error .
      ...
      assign
        frame get-rep :height = frame get-rep :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame get-rep :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame get-rep :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame get-rep :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame get-rep :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame get-rep :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame get-rep :width = v-frame-width
          .
          if frame get-rep :scrollable = true
          then do:
            assign
              frame get-rep :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame get-rep :width
      v-frame-virtual-width = frame get-rep :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame get-rep :virtual-width = frame get-rep :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame get-rep :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame get-rep :width = frame get-rep :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame get-rep :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame get-rep :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame get-rep :height - v-diasize-resize-button :height
                  - 1
                  - (frame get-rep :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame get-rep :width - v-diasize-resize-button :width
                  - 1
                  - (frame get-rep :border-right-pixels / session :pixels-per-column)
    .
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame get-rep :height
      v-col-delta = v-new-col - frame get-rep :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame get-rep :width
      v-diasize-current-frame-height = frame get-rep :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame get-rep :height
        v-diasize-orig-frame-width  = frame get-rep :width
        v-diasize-browse-handle     = browse BROWSE-date :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame get-rep :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  assign
      ...
      RADIO-TYPE = 0
      .
  APPLY "VALUE-CHANGED" TO RADIO-TYPE IN FRAME get-rep .
  run enable_ui.
  apply "entry" to date-b in frame get-rep.
  wait-for go of frame get-rep.
end.
run disable_ui.
