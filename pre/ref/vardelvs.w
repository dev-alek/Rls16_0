...
DEFINE BROWSE br-vardeliv
  QUERY br-vardeliv NO-LOCK DISPLAY
      ...
      entry (lookup (STRING(X_variant-delivery.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
...
if loc-doc-rec <> ? then do:
  ...
  if error-status:error then do:                           find first pos_variant-delivery no-lock where                                   recid(pos_variant-delivery) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ВАРИАНТ ДОСТАВКИ" skip                            string(if avail pos_variant-delivery                                     then  substitute("Вн код типа доставки: &1 вн код субъекта доставки: &2 объект доставки: &3&4"                                                     , pos_variant-delivery.deliv-type-code                                                     , pos_variant-delivery.deliv-subj-code                                                     , pos_variant-delivery.obj-type                                                     , pos_variant-delivery.obj-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
end.
...
assign
  v-str-recid14 = trim( string( recid( X_variant-delivery ) , "->>>>>>>>>>>9":U ) )
...
END.
ON CHOOSE OF B-vrdlgrps IN FRAME Dialog-Frame
DO:
...
on start-search, ctrl-o of br-vardeliv in frame Dialog-Frame do:
   run sort-brbr-vardeliv
     (input (if available X_variant-delivery
             then recid(X_variant-delivery)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-vardeliv :
  ...
  case sort-labelbr-vardeliv:
        when X_variant-delivery.deliv-type-code:label in browse br-vardeliv then DO:    assign       sort-column-name = "X_variant-delivery.deliv-type-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_variant-delivery.deliv-subj-code:label in browse br-vardeliv then DO:    assign       sort-column-name = "X_variant-delivery.deliv-subj-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
define variable vss-include-info29 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
 ...
 if LOOKUP(p-mode, ('все':U + chr(4) + "delivery-type-subject":U + chr(4) + 'объект':U) ,
                chr(4)) = 0
     then dO:
    message
    vss-workfile vss-revision vss-description skip
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-vardeliv FOR EACH X_variant-delivery NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
title0 = "Список вариантов доставки" + chr(32).
...
if p-open-query then do:
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_variant-delivery
      where  TRUE
      indexed-reposition
  .
      assign
  ...
  on error undo, leave
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_variant-delivery:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input QUERY br-vardeliv:handle
                          ,input rowid(X_variant-delivery)
                          ,input logical(parameter-2-32)
                          ,input no-lock
                          ,input (buffer X_variant-delivery:handle)
                          ,input parameter-4-32
                          ,input parameter-5-32
                          ) no-error.
      .
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_variant-delivery"
      parameter-4-32 =
        (
          if (" TRUE " + " " + where-phrase-32) <> ""
          then " TRUE " + " " + where-phrase-32
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_variant-delivery.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_variant-delivery.sts = &1', p-sts ) + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_variant-delivery.sts = p-sts " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_variant-delivery
      where  X_variant-delivery.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_variant-delivery:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_variant-delivery.sts = &1', p-sts ) + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_variant-delivery"
      parameter-4-34 =
        (
          if (" X_variant-delivery.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_variant-delivery.sts = &1', p-sts ) + " " + where-phrase-34
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if ("                X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code                               " + " " + where-phrase-36) <> ""
          then  substitute(' X_variant-delivery.deliv-type-code  = &1               AND X_variant-delivery.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)
                            + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          ("                X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code                               " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_variant-delivery
      where                 X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_variant-delivery:handle) then do:
      ...
      parameter-4-36 =
        "where ":u +  substitute(' X_variant-delivery.deliv-type-code  = &1               AND X_variant-delivery.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)
    ...
    .
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_variant-delivery"
      parameter-4-36 =
        (
          if ("                X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code                               " + " " + where-phrase-36) <> ""
          then  substitute(' X_variant-delivery.deliv-type-code  = &1               AND X_variant-delivery.deliv-subj-code  = &2 ', p-deliv-type-code, p-deliv-subj-code)
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-38 as character no-undo .
      ...
      parameter-4-38 =
        (
          if ("                X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code               AND X_variant-delivery.sts = p-sts " + " " + where-phrase-38) <> ""
          then  substitute('X_variant-delivery.deliv-type-code  = &1               AND X_variant-delivery.deliv-subj-code  = &2               AND X_variant-delivery.sts = &3 ', p-deliv-type-code, p-deliv-subj-code, p-sts) + " " + where-phrase-38
          else "true"
        )
      parameter-5-38 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-38 =
          ("                X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code               AND X_variant-delivery.sts = p-sts " + " " + where-phrase-38 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-38 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_variant-delivery
      where                 X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code               AND X_variant-delivery.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_variant-delivery:handle) then do:
      ...
      parameter-4-38 =
        "where ":u +  substitute('X_variant-delivery.deliv-type-code  = &1               AND X_variant-delivery.deliv-subj-code  = &2               AND X_variant-delivery.sts = &3 ', p-deliv-type-code, p-deliv-subj-code, p-sts) + " ":u + where-phrase-38 + " ":u + p-find-condition + " " + ""
      parameter-5-38 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-38 = (if p-find-next then "true":u else "false":u )
      parameter-3-38 =  "FOR EACH X_variant-delivery"
      parameter-4-38 =
        (
          if ("                X_variant-delivery.deliv-type-code  = p-deliv-type-code               AND X_variant-delivery.deliv-subj-code  = p-deliv-subj-code               AND X_variant-delivery.sts = p-sts " + " " + where-phrase-38) <> ""
          then  substitute('X_variant-delivery.deliv-type-code  = &1               AND X_variant-delivery.deliv-subj-code  = &2               AND X_variant-delivery.sts = &3 ', p-deliv-type-code, p-deliv-subj-code, p-sts) + " " + where-phrase-38
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-40 as character no-undo .
      ...
      parameter-4-40 =
        (
          if ("              X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code                             " + " " + where-phrase-40) <> ""
          then  substitute(' X_variant-delivery.obj-type  = &1             AND X_variant-delivery.obj-code  = &2 ', p-deliv-obj-type, p-deliv-obj-code)  + " " + where-phrase-40
          else "true"
        )
      parameter-5-40 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-40 =
          ("              X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code                             " + " " + where-phrase-40 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-40 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_variant-delivery
      where               X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_variant-delivery:handle) then do:
      ...
      parameter-4-40 =
        "where ":u +  substitute(' X_variant-delivery.obj-type  = &1             AND X_variant-delivery.obj-code  = &2 ', p-deliv-obj-type, p-deliv-obj-code)  + " ":u + where-phrase-40 + " ":u + p-find-condition + " " + ""
      parameter-5-40 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-40 = (if p-find-next then "true":u else "false":u )
      parameter-3-40 =  "FOR EACH X_variant-delivery"
      parameter-4-40 =
        (
          if ("              X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code                             " + " " + where-phrase-40) <> ""
          then  substitute(' X_variant-delivery.obj-type  = &1             AND X_variant-delivery.obj-code  = &2 ', p-deliv-obj-type, p-deliv-obj-code)  + " " + where-phrase-40
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-42 as character no-undo .
      ...
      parameter-4-42 =
        (
          if ("              X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code             AND X_variant-delivery.sts = p-sts " + " " + where-phrase-42) <> ""
          then  substitute(' X_variant-delivery.obj-type  = &1             AND X_variant-delivery.obj-code  = &2             AND X_variant-delivery.sts = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-sts) + " " + where-phrase-42
          else "true"
        )
      parameter-5-42 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-42 =
          ("              X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code             AND X_variant-delivery.sts = p-sts " + " " + where-phrase-42 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-vardeliv:handle
  ...
  if l-filter-open-42 = false then do:
    OPEN QUERY br-vardeliv FOR EACH X_variant-delivery
      where               X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code             AND X_variant-delivery.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-vardeliv:handle:get-buffer-handle(1) = (buffer X_variant-delivery:handle) then do:
      ...
      parameter-4-42 =
        "where ":u +  substitute(' X_variant-delivery.obj-type  = &1             AND X_variant-delivery.obj-code  = &2             AND X_variant-delivery.sts = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-sts) + " ":u + where-phrase-42 + " ":u + p-find-condition + " " + ""
      parameter-5-42 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-42 = (if p-find-next then "true":u else "false":u )
      parameter-3-42 =  "FOR EACH X_variant-delivery"
      parameter-4-42 =
        (
          if ("              X_variant-delivery.obj-type  = p-deliv-obj-type             AND X_variant-delivery.obj-code  = p-deliv-obj-code             AND X_variant-delivery.sts = p-sts " + " " + where-phrase-42) <> ""
          then  substitute(' X_variant-delivery.obj-type  = &1             AND X_variant-delivery.obj-code  = &2             AND X_variant-delivery.sts = &3 ', p-deliv-obj-type, p-deliv-obj-code, p-sts) + " " + where-phrase-42
...
DO ON STOP    UNDO Filter-Block, LEAVE Filter-Block
  ...
  run gbl/filter.w ( INPUT parparentproc
                   , INPUT (filter-point + chr(4) + filter-label0)
                   , INPUT tbl
                   , INPUT join-tbl
                   , INPUT fld
                   , INPUT lab
