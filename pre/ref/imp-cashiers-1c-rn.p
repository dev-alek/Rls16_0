...
do:
  find first buf_clients EXCLUSIVE-LOCK where buf_clients.obj-code = integer(p-Cashiers:code_)
    and buf_clients.obj-type = 'чел':U and buf_clients.grp-code = 4 no-error .
...
do:
  ...
  do:
    ...
    if p-Cashiers:role-code = 2 then v-role = 'C':U .
    else v-role = "cli-all" .
    run ref/person1.p (
