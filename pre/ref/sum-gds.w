...
(   input parParentProc ,
    ...
    if ref-list <> "" then do:
      ...
      if error-status:error then do:
       ...
       OPEN QUERY BROWSE-2 FOR EACH goods-attr       WHERE goods-attr.attr-code = p-attr-code and goods-attr.attr-value = p-attr-value NO-LOCK,       EACH goods WHERE goods.gds-code = goods-attr.gds-code NO-LOCK     .
    end.
END.
...
on start-search, ctrl-o of BROWSE-2 in frame Dialog-Frame do:
   run sort-brBROWSE-2
     (input (if available goods-attr
             then recid(goods-attr)
             else ?
            )
     ).
end.
...
case sort-column-name :
  when "goods.artic" then do:
        assign frame Dialog-Frame:title = "Товары >> " + p-name.    OPEN QUERY BROWSE-2 FOR EACH goods-attr       WHERE goods-attr.attr-code = p-attr-code and goods-attr.attr-value = p-attr-value NO-LOCK,       EACH goods WHERE goods.gds-code = goods-attr.gds-code NO-LOCK     by goods.artic.    .
  end.
  when "goods.gds-name" then do:
        assign frame Dialog-Frame:title = "Товары >> " + p-name.    OPEN QUERY BROWSE-2 FOR EACH goods-attr       WHERE goods-attr.attr-code = p-attr-code and goods-attr.attr-value = p-attr-value NO-LOCK,       EACH goods WHERE goods.gds-code = goods-attr.gds-code NO-LOCK     by goods.gds-name.    .
  end.
  otherwise do:
        assign frame Dialog-Frame:title = "Товары >> " + p-name.    OPEN QUERY BROWSE-2 FOR EACH goods-attr       WHERE goods-attr.attr-code = p-attr-code and goods-attr.attr-value = p-attr-value NO-LOCK,       EACH goods WHERE goods.gds-code = goods-attr.gds-code NO-LOCK     .    .
  end.
end case.
