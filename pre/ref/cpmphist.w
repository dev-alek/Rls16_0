DEFINE BUFFER find_c-pmp-hist FOR ub.c-pmp-hist.
DEFINE BUFFER X_c-pmp-hist FOR ub.c-pmp-hist.
...
define input parameter p-pump-code like ub.c-pmp-hist.pump-code no-undo .
define input parameter p-pl-code  like ub.c-pmp-hist.pl-code no-undo .
define input parameter p-gds-code like ub.c-pmp-hist.gds-code no-undo .
define input parameter p-nozzle-code like ub.c-pmp-hist.nozzle-code no-undo .
...
define variable vss-description AS CHAR NO-UNDO INIT "Список полной истории ТРК":U.
...
define variable filter-label as character no-undo init "История ТРК" .
define variable filter-label0 as character no-undo init "История ТРК" .
define variable filter-point0 as character no-undo init "cpmphist" .
define variable filter-point as character no-undo init "cpmphist" .
...
DEFINE VARIABLE sch-pump-code AS INTEGER FORMAT ">>>>>>9":U INITIAL 0
     LABEL "№ ТРК"
     VIEW-AS FILL-IN
     SIZE 8 BY 1 TOOLTIP "Поиск первой записи - <ВВОД>; поиск следующей - <CTRL-J>" NO-UNDO.
DEFINE QUERY BR-changes FOR
      temp-changes SCROLLING.
DEFINE QUERY br-pmp-hist FOR
      X_c-pmp-hist SCROLLING.
...
DEFINE BROWSE br-pmp-hist
  QUERY br-pmp-hist NO-LOCK DISPLAY
      mark-string(recid(X_c-pmp-hist), v-rid-list) COLUMN-LABEL "*" FORMAT "X(1)":U
      X_c-pmp-hist.pump-code COLUMN-LABEL "№ ТРК" FORMAT ">9":U
      X_c-pmp-hist.corr-date FORMAT "99/99/9999":U
      string(X_c-pmp-hist.corr-time, "HH:MM:SS":U) COLUMN-LABEL "Время изм." FORMAT "X(8)":U
      usrfulnf(X_c-pmp-hist.corr-user-name) FORMAT "X(18)":U
      get-action(X_c-pmp-hist.action) COLUMN-LABEL "Действие" FORMAT "X(10)":U
      X_c-pmp-hist.corr-user-db-num FORMAT ">>>>9":U
      get-subject(X_c-pmp-hist.subject) COLUMN-LABEL "Предмет изменений" FORMAT "X(15)":U
      X_c-pmp-hist.obj-type + string(X_c-pmp-hist.obj-code) COLUMN-LABEL "Объект" FORMAT "X(8)":U
  ENABLE
      X_c-pmp-hist.corr-date
...
DEFINE FRAME Dialog-Frame
     ...
     br-pmp-hist AT ROW 2 COL 1
     sch-corr-date AT ROW 13.63 COL 85.75 COLON-ALIGNED
     sch-pump-code AT ROW 13.67 COL 56.13 COLON-ALIGNED
...
DO:
  ...
  DEFINE BUFFER buf_c-pump-nozzle FOR ub.c-pl-pump-nozzle.
    IF AVAILABLE X_c-pmp-hist THEN DO:
    CASE X_c-pmp-hist.subject:
      WHEN 'place':U THEN DO:
        find first buf_c-pump no-lock where
               buf_c-pump.obj-type = p-obj-type
           AND buf_c-pump.obj-code = p-obj-code
           AND buf_c-pump.pump-code = X_c-pmp-hist.pump-code
           AND buf_c-pump.chip-num = X_c-pmp-hist.chip-num
           AND buf_c-pump.corr-user-db-num = X_c-pmp-hist.corr-user-db-num
           no-error .
           ASSIGN v-rec = RECID(buf_c-pump).
            run str/c-pumpa.w (
                           INPUT 'ПРОСМОТР':U
...
assign
  v-str-recid14 = trim( string( recid( X_c-pmp-hist ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry14 > 0 then do:
  ...
  .
    ...
    loc#log = br-pmp-hist:refresh() .
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-pmp-hist:select-next-row ().
        apply "VALUE-CHANGED" to br-pmp-hist in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-pmp-hist in frame Dialog-Frame.
END.
...
DO:
  if ( available X_c-pmp-hist ) then do:
    ...
    then
    v-rid-list = string( recid( X_c-pmp-hist ) ) .
  end.
END.
ON MOUSE-SELECT-DBLCLICK OF br-pmp-hist IN FRAME Dialog-Frame
DO:
     run proc-br-pmp-hist in this-procedure no-error.
  if error-status:error then return no-apply.
END.
ON RETURN OF br-pmp-hist IN FRAME Dialog-Frame
DO:
    run proc-br-pmp-hist in this-procedure no-error.
  if error-status:error then return no-apply.
END.
ON VALUE-CHANGED OF br-pmp-hist IN FRAME Dialog-Frame
DO:
...
DO:
  ...
  if error-status:error then return no-apply.
END.
ON CTRL-J OF sch-pump-code IN FRAME Dialog-Frame
DO:
  run proc-find-pump-code in this-procedure(yes, input frame Dialog-Frame sch-pump-code) no-error.
  if error-status:error then return no-apply.
END.
ON RETURN OF sch-pump-code IN FRAME Dialog-Frame
DO:
  run proc-find-pump-code in this-procedure(no, input frame Dialog-Frame sch-pump-code) no-error.
  if error-status:error then return no-apply.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-pmp-hist :handle
      .
...
do:
   v-doc-rec = recid(X_c-pmp-hist).  RUn OpenBR in this-procedure ( input yes, input no, input '':U).  REPOSITION br-pmp-hist to recid v-doc-rec No-ERROR.                apply 'value-changed' to br-pmp-hist.
...
do:
  ...
  END.
def var sort-labelbr-pmp-hist   as character no-undo .
def var sort-clmnbr-pmp-hist    as handle    no-undo .
def var cur-clmnbr-pmp-hist     as handle    no-undo .
def var cur-clmn-locbr-pmp-hist as integer   no-undo .
def var re-querybr-pmp-hist     as logical   initial no no-undo .
on start-search, ctrl-o of br-pmp-hist in frame Dialog-Frame do:
   run sort-brbr-pmp-hist
     (input (if available X_c-pmp-hist
             then recid(X_c-pmp-hist)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-pmp-hist :
  define input parameter p-recid as recid no-undo .
  if re-querybr-pmp-hist = no then do:
    assign
       cur-clmnbr-pmp-hist = br-pmp-hist:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-pmp-hist <> ? then sort-clmnbr-pmp-hist:column-fgcolor = 0.
    if cur-clmnbr-pmp-hist = sort-clmnbr-pmp-hist then do:
      assign
         sort-labelbr-pmp-hist = ""
         sort-clmnbr-pmp-hist = ?
      .
     end.
     else do:
       assign
         sort-labelbr-pmp-hist = cur-clmnbr-pmp-hist:label
         sort-clmnbr-pmp-hist  = cur-clmnbr-pmp-hist
         sort-clmnbr-pmp-hist:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-pmp-hist = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-pmp-hist:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-pmp-hist then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-pmp-hist = cur-clmn-locbr-pmp-hist + 1
    .
  end.
  case sort-labelbr-pmp-hist:
        when X_c-pmp-hist.pump-code:label in browse br-pmp-hist then DO:    assign       sort-column-name = "X_c-pmp-hist.pump-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-pmp-hist.corr-date:label in browse br-pmp-hist then DO:    assign       sort-column-name = "X_c-pmp-hist.corr-date"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-pmp-hist.corr-user-db-num:label in browse br-pmp-hist then DO:    assign       sort-column-name = "X_c-pmp-hist.corr-user-db-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-pmp-hist') then do:
          run mv-brw-defaultbr-pmp-hist.
        end.
      if sort-labelbr-pmp-hist <> "" then do:
        assign
          cur-clmnbr-pmp-hist:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-pmp-hist = ?
      .
    end.
  end case.
    if cur-clmn-locbr-pmp-hist <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-pmp-hist') then do:
        run ch-clmnbr-pmp-hist in this-procedure (cur-clmn-locbr-pmp-hist).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-pmp-hist to recid p-recid no-error.
    apply "value-changed" to br-pmp-hist in frame Dialog-Frame.
  end.
  apply "entry" to br-pmp-hist in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-pmp-hist:
if cur-clmnbr-pmp-hist = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U).
end.
else do:
   assign re-querybr-pmp-hist = yes.
   run sort-brbr-pmp-hist
     (input (if available X_c-pmp-hist
             then recid(X_c-pmp-hist)
             else ?
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
 ...
 or p-mode = "subject":U
 then do:
  find first X_pump no-lock where
             X_pump.obj-type = p-obj-type
         AND  X_pump.obj-code = p-obj-code
         AND  X_pump.pump-code = p-pump-code no-error.
    if not available X_pump then do:
        message
        vss-workfile vss-revision vss-description skip
        "Неверное значение параметра вызова p-pump-code" p-pump-code
        view-as alert-box ERROR.
        return.
    end.
  end.
  if p-mode = "subject"  then do:
    if p-subject = 'pl-gds-pump':U
    then do:
  ...
  if v-rid-list <> "" then do:
      FIND FIRST find_c-pmp-hist No-LOCK where
                 recid(find_c-pmp-hist) = integer(entry(1, v-rid-list)) No-ERROR.
      if not avail find_c-pmp-hist then do:
        message
        vss-workfile vss-revision vss-description skip
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  ...
  REPOSITION br-pmp-hist to recid integer(entry(1, v-rid-list)) No-ERROR.
define variable vss-include-info25 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
DEF VAR cur-clmn-numbr-pmp-hist as INT EXTENT 9 no-undo.
DEF VAR varmvibr-pmp-hist       as INT no-undo.
DEF VAR varmvjbr-pmp-hist       as INT no-undo.
DEF VAR varmvkbr-pmp-hist       as INT no-undo.
DEF VAR varmvlbr-pmp-hist       as INT no-undo.
DEF VAR move-elementbr-pmp-hist as INT no-undo.
def var jjbr-pmp-hist           as int no-undo.
do varmvibr-pmp-hist = 1 to EXTENT(cur-clmn-numbr-pmp-hist):
  ASSIGN cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = varmvibr-pmp-hist.
END.
RUN start-mv-clmnbr-pmp-hist.
PROCEDURE start-mv-clmnbr-pmp-hist:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-pmp-hist = NUM-ENTRIES('9,1,2,3,4,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-pmp-hist ( cur-clmn-numbr-pmp-hist[INTEGER(ENTRY (jjbr-pmp-hist, '9,1,2,3,4,5,6,7,8'))] , 1).
   END.
       END.
       IF  p-mode <> 'все':U  THEN DO:
   DO jjbr-pmp-hist = NUM-ENTRIES('1,2,3,4,5,6,7,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-pmp-hist ( cur-clmn-numbr-pmp-hist[INTEGER(ENTRY (jjbr-pmp-hist, '1,2,3,4,5,6,7,8,9'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-pmp-hist do:
  RUN re-move-clmnbr-pmp-hist ( 1, 9).
END.
ON ctrl-cursor-left OF BROWSE br-pmp-hist do:
  RUN re-move-clmnbr-pmp-hist (9, 1).
END.
PROCEDURE re-move-clmnbr-pmp-hist:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-pmp-hist = 1 TO EXTENT(cur-clmn-numbr-pmp-hist):
    if cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = source-column THEN cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = -1.
  END.
  if br-pmp-hist:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-pmp-hist = source-column - 1 to target-column BY -1:
    DO varmvibr-pmp-hist = 1 TO EXTENT(cur-clmn-numbr-pmp-hist):
        if cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = varmvjbr-pmp-hist THEN DO:
          cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-pmp-hist = source-column + 1 to target-column:
    DO varmvibr-pmp-hist = 1 TO EXTENT(cur-clmn-numbr-pmp-hist):
      if cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = varmvjbr-pmp-hist THEN DO:
        cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] - 1.
      END.
  ...
  END.
  DO varmvibr-pmp-hist = 1 TO EXTENT(cur-clmn-numbr-pmp-hist):
    if cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = -1 THEN cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-pmp-hist:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-pmp-hist = 1 TO EXTENT(cur-clmn-numbr-pmp-hist):
    if cur-clmn-numbr-pmp-hist[varmvibr-pmp-hist] = cur-clmn-loc THEN move-elementbr-pmp-hist = varmvibr-pmp-hist.
  END.
  RUN re-move-clmnbr-pmp-hist (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-pmp-hist:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-pmp-hist = 1 to EXTENT(cur-clmn-numbr-pmp-hist):
    RUN re-move-clmnbr-pmp-hist (cur-clmn-numbr-pmp-hist[varmvlbr-pmp-hist], varmvlbr-pmp-hist).
  END.
  RUN start-mv-clmnbr-pmp-hist.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
END PROCEDURE.
PROCEDURE enable_UI :
  DISPLAY sch-corr-date sch-pump-code sch-corr-user-name mark-num
      WITH FRAME Dialog-Frame.
  ENABLE b-quit B-mark b-sel B-print B-sch B-Help B-lookup br-pmp-hist
         sch-corr-date sch-pump-code sch-corr-user-name BR-changes mark-num
      WITH FRAME Dialog-Frame.
  VIEW FRAME Dialog-Frame.
END PROCEDURE.
PROCEDURE MyEnable :
assign
  br-pmp-hist:num-locked-columns in frame Dialog-Frame = 1
  X_c-pmp-hist.corr-date:read-only in browse br-pmp-hist = yes
  br-changes:title = "":U
  ...
  br-pmp-hist
  sch-corr-date
  sch-pump-code when p-mode = 'все':U
  sch-corr-user-name
  BR-changes mark-num
  WITH FRAME Dialog-Frame .
  VIEW FRAME Dialog-Frame .
...
if p-open-query then do:
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where  TRUE
       use-index idate
      indexed-reposition
  .
  ...
  assign
    v-doc-rec = recid( X_c-pmp-hist )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input query br-pmp-hist:handle
                          ,input rowid(X_c-pmp-hist)
                          ,input logical(parameter-2-27)
                          ,input no-lock
                          ,input (buffer X_c-pmp-hist:handle)
                          ,input parameter-4-27
                          ,input parameter-5-27
                          ) no-error.
      .
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH X_c-pmp-hist"
      parameter-4-27 =
        (
          if (" TRUE " + " " + where-phrase-27) <> ""
          then " TRUE " + " " + where-phrase-27
  ...
  WHEN 'объект':U THEN DO:
    ...
    if p-open-query then do:
      ASSIGN
      frame Dialog-Frame:TITLE = title0 + substitute(" ТРК на объекте: &1&2"
                                                        ,p-obj-type, p-obj-code).
    end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-29 as character no-undo .
      ...
      parameter-4-29 =
        (
          if ("                     X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                       " + " " + where-phrase-29) <> ""
          then  substitute(' X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3 ', chr(34), p-obj-type, p-obj-code )   + " " + where-phrase-29
          else "true"
        )
      parameter-5-29 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-29 =
          ("                     X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                       " + " " + where-phrase-29 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-29 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where                      X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      ...
      parameter-4-29 =
        "where ":u +  substitute(' X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3 ', chr(34), p-obj-type, p-obj-code )   + " ":u + where-phrase-29 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-29 = (if p-find-next then "true":u else "false":u )
      parameter-3-29 =  "FOR EACH X_c-pmp-hist"
      parameter-4-29 =
        (
          if ("                     X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                       " + " " + where-phrase-29) <> ""
          then  substitute(' X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3 ', chr(34), p-obj-type, p-obj-code )   + " " + where-phrase-29
  ...
  WHEN "one":u THEN DO:
    assign
    filter-point = filter-point0 + p-mode
    filter-label = substitute("&1 Одна ТРК", filter-label0)
    .
    if p-open-query then do:
      ASSIGN frame Dialog-Frame:TITLE = title0 + substitute(" ТРК №&1 Объект &2&3"
                                                              ,p-pump-code ,p-obj-type, p-obj-code ).
    end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code AND X_c-pmp-hist.pump-code  = p-pump-code " + " " + where-phrase-31) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3 AND X_c-pmp-hist.pump-code  = &4 '                           ,chr(34)                           , p-obj-type                           , p-obj-code                           ,p-pump-code) + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code AND X_c-pmp-hist.pump-code  = p-pump-code " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where  X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code AND X_c-pmp-hist.pump-code  = p-pump-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3 AND X_c-pmp-hist.pump-code  = &4 '                           ,chr(34)                           , p-obj-type                           , p-obj-code                           ,p-pump-code) + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-31 = (if p-find-next then "true":u else "false":u )
      parameter-3-31 =  "FOR EACH X_c-pmp-hist"
      parameter-4-31 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code AND X_c-pmp-hist.pump-code  = p-pump-code " + " " + where-phrase-31) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3 AND X_c-pmp-hist.pump-code  = &4 '                           ,chr(34)                           , p-obj-type                           , p-obj-code                           ,p-pump-code) + " " + where-phrase-31
  ...
  WHEN "subject":u THEN DO:
    assign
    filter-point = filter-point0 + p-mode
    v-subject-chr = entry (lookup (p-subject, 'pl-gds-pump,pl-pump,pl-pump-nozzle,pump-nozzle,pump,pump-attr':U), 'Товар на ТРК,Резервуар/ТРК,Резервуар/ТРК/Пистолет,ТРК/Пистолет,ТРК,Атрибут ТРК':U)
    filter-label = substitute("&1 Один объект, Предмет изменений", filter-label0)
    .
    ...
    CASE p-subject:
      when 'pump':U then do:
        if p-open-query then do:
          assign
          frame Dialog-Frame:title = v-title + substitute(" &1", p-pump-code).
        end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-33 as character no-undo .
      ...
      parameter-4-33 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject                         " + " " + where-phrase-33) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pump-code  = &4 and X_c-pmp-hist.subject = &1&5&1 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject)  + " " + where-phrase-33
          else "true"
        )
      parameter-5-33 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-33 =
          (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject                         " + " " + where-phrase-33 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-33 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where  X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      ...
      parameter-4-33 =
        "where ":u +  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pump-code  = &4 and X_c-pmp-hist.subject = &1&5&1 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject)  + " ":u + where-phrase-33 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-33 = (if p-find-next then "true":u else "false":u )
      parameter-3-33 =  "FOR EACH X_c-pmp-hist"
      parameter-4-33 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject                         " + " " + where-phrase-33) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pump-code  = &4 and X_c-pmp-hist.subject = &1&5&1 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject)  + " " + where-phrase-33
  ...
  run waitfram-hide in this-procedure .
      ...
      when 'pl-pump':U then do:
        if p-open-query then do:
          assign
          frame Dialog-Frame:title = v-title + substitute(" &1/&2"
                                                           , p-pl-code, p-pump-code).
        end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-35 as character no-undo .
      ...
      parameter-4-35 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code                          " + " " + where-phrase-35) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4 and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.pump-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code )  + " " + where-phrase-35
          else "true"
        )
      parameter-5-35 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-35 =
          (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code                          " + " " + where-phrase-35 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-35 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where  X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      ...
      parameter-4-35 =
        "where ":u +  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4 and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.pump-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code )  + " ":u + where-phrase-35 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-35 = (if p-find-next then "true":u else "false":u )
      parameter-3-35 =  "FOR EACH X_c-pmp-hist"
      parameter-4-35 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code                          " + " " + where-phrase-35) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4 and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.pump-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code )  + " " + where-phrase-35
  ...
  run waitfram-hide in this-procedure .
      ...
      when 'pl-gds-pump':U then do:
        if p-open-query then do:
          assign
          frame Dialog-Frame:title = v-title + substitute(" Резервуар/Товар &1/&2", p-pl-code, p-gds-code).
        end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-37 as character no-undo .
      ...
      parameter-4-37 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.gds-code  = p-gds-code                            And X_c-pmp-hist.pump-code  = p-pump-code                          " + " " + where-phrase-37) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.gds-code  = &6                            And X_c-pmp-hist.pump-code  = &6', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-gds-code, p-pump-code)   + " " + where-phrase-37
          else "true"
        )
      parameter-5-37 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-37 =
          (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.gds-code  = p-gds-code                            And X_c-pmp-hist.pump-code  = p-pump-code                          " + " " + where-phrase-37 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-37 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where  X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.gds-code  = p-gds-code                            And X_c-pmp-hist.pump-code  = p-pump-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      ...
      parameter-4-37 =
        "where ":u +  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.gds-code  = &6                            And X_c-pmp-hist.pump-code  = &6', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-gds-code, p-pump-code)   + " ":u + where-phrase-37 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-37 = (if p-find-next then "true":u else "false":u )
      parameter-3-37 =  "FOR EACH X_c-pmp-hist"
      parameter-4-37 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.gds-code  = p-gds-code                            And X_c-pmp-hist.pump-code  = p-pump-code                          " + " " + where-phrase-37) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.gds-code  = &6                            And X_c-pmp-hist.pump-code  = &6', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-gds-code, p-pump-code)   + " " + where-phrase-37
  ...
  run waitfram-hide in this-procedure .
      ...
      when 'pump-nozzle':U then do:
        if p-open-query then do:
          assign
          frame Dialog-Frame:title = v-title + substitute(" &1/&2", p-pump-code, p-nozzle-code).
        end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-39 as character no-undo .
      ...
      parameter-4-39 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-39) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pump-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.nozzle-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject, p-nozzle-code) + " " + where-phrase-39
          else "true"
        )
      parameter-5-39 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-39 =
          (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-39 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-39 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where  X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.nozzle-code  = p-nozzle-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      ...
      parameter-4-39 =
        "where ":u +  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pump-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.nozzle-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject, p-nozzle-code) + " ":u + where-phrase-39 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-39 = (if p-find-next then "true":u else "false":u )
      parameter-3-39 =  "FOR EACH X_c-pmp-hist"
      parameter-4-39 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pump-code  = p-pump-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-39) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pump-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.nozzle-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject, p-nozzle-code) + " " + where-phrase-39
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-41 as character no-undo .
      ...
      parameter-4-41 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code                            And X_c-pmp-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-41) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.pump-code  = &6                            And X_c-pmp-hist.nozzle-code  = &7 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code, p-nozzle-code)  + " " + where-phrase-41
          else "true"
        )
      parameter-5-41 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-41 =
          (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code                            And X_c-pmp-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-41 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-pmp-hist:handle
  ...
  if l-filter-open-41 = false then do:
    OPEN QUERY br-pmp-hist FOR EACH X_c-pmp-hist
      where  X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code                            And X_c-pmp-hist.nozzle-code  = p-nozzle-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-pmp-hist:handle:get-buffer-handle(1) = (buffer X_c-pmp-hist:handle) then do:
      ...
      parameter-4-41 =
        "where ":u +  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.pump-code  = &6                            And X_c-pmp-hist.nozzle-code  = &7 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code, p-nozzle-code)  + " ":u + where-phrase-41 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-41 = (if p-find-next then "true":u else "false":u )
      parameter-3-41 =  "FOR EACH X_c-pmp-hist"
      parameter-4-41 =
        (
          if (" X_c-pmp-hist.obj-type = p-obj-type and X_c-pmp-hist.obj-code = p-obj-code                            And X_c-pmp-hist.pl-code  = p-pl-code  and X_c-pmp-hist.subject = p-subject                           And X_c-pmp-hist.pump-code  = p-pump-code                            And X_c-pmp-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-41) <> ""
          then  substitute('X_c-pmp-hist.obj-type = &1&2&1 and X_c-pmp-hist.obj-code = &3                            And X_c-pmp-hist.pl-code  = &4  and X_c-pmp-hist.subject = &1&5&1                           And X_c-pmp-hist.pump-code  = &6                            And X_c-pmp-hist.nozzle-code  = &7 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code, p-nozzle-code)  + " " + where-phrase-41
...
REPOSITION br-pmp-hist to recid v-doc-rec No-ERROR.
if not p-open-query and v-fltopend-rowid[1] <> ? then
query br-pmp-hist:handle:reposition-to-rowid(v-fltopend-rowid) No-ERROR.
run waitfram-hide in this-procedure .
APPLY "VALUE-CHANGED" TO br-pmp-hist in frame Dialog-Frame.
APPLY "ENTRY" TO br-pmp-hist.
END PROCEDURE.
...
X_c-pmp-hist.pump-code COLUMN-LABEL "№ ТРК"
...
assign
v-doc-rec = recid( X_c-pmp-hist ).
DO WHILE available X_c-pmp-hist :
      GET prev br-pmp-hist.
END.
...
run waitfram-show in this-procedure ("Ждите...").
  GET next br-pmp-hist.
    DO WHILE available X_c-pmp-hist :
      Display STREAM PrnLibStream
      X_c-pmp-hist.pump-code
      get-action(X_c-pmp-hist.action) @ v-action-chr
      get-subject(X_c-pmp-hist.subject) @ v-subject-chr
      X_c-pmp-hist.corr-date
      string(X_c-pmp-hist.corr-time, "HH:MM:SS":U) @ v-upd-time
      usrfulnf(X_c-pmp-hist.corr-user-name) @ v-for-user-name
      X_c-pmp-hist.corr-user-db-num
      X_c-pmp-hist.obj-type + string(X_c-pmp-hist.obj-code) @ v-obj
...
run fltfield-add in this-procedure('pump-code', '№ ТРК', '',
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
...
v-date-chr = string(day(p-date)) + chr(47) +
       ...
       run OpenBr in this-procedure
        (input false
        ,input true
        ,input substitute("and X_c-pmp-hist.corr-date = &1 "
          , v-date-chr)
        ).
      apply "entry":u to sch-corr-date in frame Dialog-Frame.
END PROCEDURE.
PROCEDURE proc-find-pump-code :
define input parameter p-next as logical no-undo.
define input parameter p-pump-code like ub.c-pmp-hist.pump-code no-undo.
...
with frame Dialog-Frame.
assign
v-pump-code = string(p-pump-code).
run OpenBr in this-procedure
    (input false
    ,input p-next
    ,input substitute("and X_c-pmp-hist.pump-code = &1 "
      , v-pump-code)
    ).
apply "entry":u to sch-pump-code in frame Dialog-Frame .
END PROCEDURE.
PROCEDURE proc-find-user :
define input parameter p-next as logical no-undo.
define input parameter p-user like ub.c-pmp-hist.corr-user-name no-undo.
assign
sch-corr-date = ?.
display
sch-corr-date
...
run OpenBr in this-procedure
    (input false
    ,input p-next
    ,input substitute("and X_c-pmp-hist.corr-user-name = &1 "
      , p-user)
    ).
...
for each temp-changes:
    delete temp-changes.
END.
if not available X_c-pmp-hist then do:
  Open QUery br-changes for each temp-changes.
  return.
end.
run ref/cpmphisv.p (
                   input X_c-pmp-hist.obj-type
                  ,input X_c-pmp-hist.obj-code
                  ,input X_c-pmp-hist.pump-code
                  ,input X_c-pmp-hist.chip-num
                  ,input X_c-pmp-hist.corr-user-db-num
                  ,input X_c-pmp-hist.subject
                  ,input X_c-pmp-hist.action
                  ,input no
...
FUNCTION get-subject RETURNS CHARACTER
  ...
  RETURN entry (lookup (p-subject, 'pl-gds-pump,pl-pump,pl-pump-nozzle,pump-nozzle,pump,pump-attr':U), 'Товар на ТРК,Резервуар/ТРК,Резервуар/ТРК/Пистолет,ТРК/Пистолет,ТРК,Атрибут ТРК':U).
END FUNCTION.
