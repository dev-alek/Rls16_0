...
mViewDop = num-entries (iparent,chr(4)) eq 2.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  (output v-db-num
   ...
   assign
      vparentpar = iparent
      mParentCode   = entry(num-entries (vparentpar,chr(4)),vparentpar,chr(4))
   ...
   if imode = 'ИЗМЕНЕНИЕ':U or imode = 'ПРОСМОТР':U then
   do:
      find first b3-code where
...
PROCEDURE enable_UI :
   ...
   ENABLE B-save when iMode ne 'ПРОСМОТР':U
   b-quit B-Help
   mParentCode when iMode eq 'ИЗМЕНЕНИЕ':U
   mCode    when iMode eq 'ИЗМЕНЕНИЕ':U
   mZNACH   when iMode eq 'ИЗМЕНЕНИЕ':U
   fStatus  when iMode eq 'ИЗМЕНЕНИЕ':U
   mDecript when iMode eq 'ИЗМЕНЕНИЕ':U
      WITH FRAME Dialog-Frame.
   VIEW FRAME Dialog-Frame.
   if mParentCode eq  "" and iMode eq 'ИЗМЕНЕНИЕ':U
...
PROCEDURE proc-save :
   ...
   do on error undo, return error
      ...
      entry(num-entries (iparent,chr(4)),iparent,chr(4)) = mParentCode.
      if mCode eq ?
      then do:
