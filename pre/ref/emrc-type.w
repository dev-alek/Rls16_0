...
v-list = "," + v-list.
cb-emrc-type:list-item-pairs in frame Dialog-Frame = v-list.
if p-mode = 'ДОБАВЛЕНИЕ':U OR p-mode = 'ИЗМЕНЕНИЕ':U then do:
    enable cb-emrc-type with frame Dialog-Frame.
end.
p-setted = false.
cb-emrc-type = p-attr-value.
display cb-emrc-type with frame Dialog-Frame.
end procedure.
PROCEDURE proc-save :
   ...
   find first ub.Code no-lock where ub.Code.parent = "emc" + chr(4) + cb-emrc-type and
      ub.Code.status_ = 0 no-error .
   ...
   do:
      ...
      if not available (ub.Code) then
      do:
         find first buf_Code no-lock where buf_Code.parent = "emc" + chr(4) + cb-emrc-type and
            buf_Code.status_ = 0 and buf_Code.code > iso-date(today) no-error .
