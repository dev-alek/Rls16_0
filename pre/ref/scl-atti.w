...
DEFINE VARIABLE jj AS INTEGER NO-UNDO.
  ...
  if NOT glog then return no-apply.
    ...
    if error-status:error or not loc#log then do:
       message "Ошибка при удалении атрибута весов!"         "Обратитесь к администратору системы" skip error-status:get-message(1) skip         return-value skip view-as alert-box ERROR.
       return no-apply.
    end.
    delete temp-hattr.
    updated = yes.
...
DO:
END.
ON CHOOSE OF MENU-ITEM m_tara-weight
DO:
  add-option = 'tare-weight':U.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-SCL-ATTRS :handle
      .
...
PROCEDURE enable_UI :
  ...
  ENABLE b-quit b-ins b-chg b-del b-help scl-db-num scl-scales-type
      ...
      WITH FRAME Dialog-Frame.
  VIEW FRAME Dialog-Frame.
  OPEN QUERY BR-SCL-ATTRS FOR EACH Temp-hattr NO-LOCK.
END PROCEDURE.
PROCEDURE init-proc :
...
if mode <> 'ПРОСМОТР':U then do:
  do ii = 1 to num-entries('tare-weight':u):
    run scl-attr-code (
      input  entry(ii, 'tare-weight':u)
      ,output attr-type
      ,output attr-format
      ,output attr-label
      ,output attr-user-can-edit
    ...
    do jj = 1 to num-entries(attr-other, chr(47)):
      if entry(1, entry(jj, attr-other, chr(47)), "=":U) = "scl":U then do:
        assign
        v-scl = string(entry(2, entry(jj, attr-other, chr(47)), "=":U))
        v-scl-list = v-scl-list + (if ii = 1 then "":U else chr(47)) + v-scl
        .
      end.
    end.
  end.
    assign
    MENU-ITEM m_tara-weight:private-data in menu MENU-b-ins = 'tare-weight':U
    .
    assign
    ii = LOOKUP(MENU-ITEM m_tara-weight:private-data in menu MENU-b-ins, 'tare-weight':u)
    v-scl = entry(ii, v-scl-list, chr(47))
...
case p-add:
  ...
  when no then do:
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        message "Ошибка при определении названия и типа атрибута весов!"         "Обратитесь к администратору системы" skip error-status:get-message(1) skip         return-value skip view-as alert-box ERROR.
        return error.
    END.
    ...
    IF ERROR-STATUS:ERROR THEN DO:
        message "Ошибка при определении значения атрибута весов!"         "Обратитесь к администратору системы" skip error-status:get-message(1) skip         return-value skip view-as alert-box ERROR.
        return error.
    END.
  end.
END CASE.
IF attr-user-can-edit Then DO:
  ...
  END.
      ...
      message "Ошибка при изменении значения атрибута весов!"         "Обратитесь к администратору системы" skip error-status:get-message(1) skip         return-value skip view-as alert-box ERROR.
  end.
End.
