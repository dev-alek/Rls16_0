...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  ,output v-obj-db-num
    ...
    DO ii = 1 TO NUM-ENTRIES(p-attr-value, chr(4)):
        CREATE tt-cash-desk.
        ASSIGN
        rs-is-use = (IF integer(ENTRY(1, ENTRY(ii, p-attr-value, chr(4)))) < 0
                      THEN '-'
                      ELSE '*')
        tt-cash-desk.cash-num = ABS(integer(ENTRY(1, ENTRY(ii, p-attr-value, chr(4)))))
        tt-cash-desk.db-num = v-obj-db-num
        tt-cash-desk.obj-code = p-obj-code
        tt-cash-desk.pos-type = ENTRY(2, ENTRY(ii, p-attr-value, chr(4)))
        .
   END.
END.
END PROCEDURE.
...
PROCEDURE proc-cash-desk :
  ...
  do ii = 1 to num-entries(v-rid-list):
    ...
    if available buf_cash-desk then do:
      if buf_cash-desk.autonomy = integer('1':U) then do:
        message
...
AND rs-cash-desk = 'N':U THEN DO:
    FOR EACH tt-cash-desk:
        ASSIGN
        v-dop = v-dop + (IF v-dop = '':U THEN '':U ELSE chr(4)) +
                (IF rs-is-use = '-' THEN '-' ELSE '':U) + string(tt-cash-desk.cash-num) + chr(44) + tt-cash-desk.pos-type.
    END.
    if v-dop = '':U then do:
      message
