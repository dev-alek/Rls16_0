...
DEFINE BROWSE br-c-fgg-hist
  QUERY br-c-fgg-hist DISPLAY
      ...
      entry (lookup (X_cfgg-hist.subject, 'fbr-gds-grp,fbr-gds-grp-attr':U), 'Группа блюд,Атр-т группы блюд':U) COLUMN-LABEL "Предмет изменений" FORMAT "X(32)":U
      ...
      string(X_cfgg-hist.corr-time, "HH:MM") COLUMN-LABEL "Время!корр" FORMAT "X(5)":U
            WIDTH 6
      if X_cfgg-hist.subject <> 'fbr-gds-grp':U and X_cfgg-hist.obj-code > 0 then
...
assign
  v-str-recid13 = trim( string( recid( X_cfgg-hist ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry13 > 0 then do:
  ...
  g#log = br-c-fgg-hist :refresh( )  in frame Dialog-Frame.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
          g#log = br-c-fgg-hist:select-next-row () in frame Dialog-Frame.
          apply "value-changed" to br-c-fgg-hist in frame Dialog-Frame.
  end.
  ...
  apply "entry" to br-c-fgg-hist in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-fgg-hist :handle
      .
...
do:
  v-doc-rec = recid(X_cfgg-hist). run openbr in this-procedure. reposition br-c-fgg-hist to recid v-doc-rec no-error.
    apply "VALUE-CHANGED" to br-c-fgg-hist.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-c-fgg-hist :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
def var sort-labelbr-c-fgg-hist   as character no-undo .
def var sort-clmnbr-c-fgg-hist    as handle    no-undo .
def var cur-clmnbr-c-fgg-hist     as handle    no-undo .
def var cur-clmn-locbr-c-fgg-hist as integer   no-undo .
def var re-querybr-c-fgg-hist     as logical   initial no no-undo .
on start-search, ctrl-o of br-c-fgg-hist in frame Dialog-Frame do:
   run sort-brbr-c-fgg-hist
     (input (if available X_cfgg-hist
             then recid(X_cfgg-hist)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-c-fgg-hist :
  define input parameter p-recid as recid no-undo .
  if re-querybr-c-fgg-hist = no then do:
    assign
       cur-clmnbr-c-fgg-hist = br-c-fgg-hist:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-c-fgg-hist <> ? then sort-clmnbr-c-fgg-hist:column-fgcolor = 0.
    if cur-clmnbr-c-fgg-hist = sort-clmnbr-c-fgg-hist then do:
      assign
         sort-labelbr-c-fgg-hist = ""
         sort-clmnbr-c-fgg-hist = ?
      .
     end.
     else do:
       assign
         sort-labelbr-c-fgg-hist = cur-clmnbr-c-fgg-hist:label
         sort-clmnbr-c-fgg-hist  = cur-clmnbr-c-fgg-hist
         sort-clmnbr-c-fgg-hist:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-c-fgg-hist = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-c-fgg-hist:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-c-fgg-hist then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-c-fgg-hist = cur-clmn-locbr-c-fgg-hist + 1
    .
  end.
  case sort-labelbr-c-fgg-hist:
        when X_cfgg-hist.corr-date:label in browse br-c-fgg-hist then DO:    assign       sort-column-name = "X_cfgg-hist.corr-date"     .     run OpenBr in this-procedure .   . END.
        when X_cfgg-hist.corr-user-db-num:label in browse br-c-fgg-hist then DO:    assign       sort-column-name = "X_cfgg-hist.corr-user-db-num"     .     run OpenBr in this-procedure .   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure .
      if sort-labelbr-c-fgg-hist <> "" then do:
        assign
          cur-clmnbr-c-fgg-hist:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-c-fgg-hist = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-c-fgg-hist to recid p-recid no-error.
    apply "value-changed" to br-c-fgg-hist in frame Dialog-Frame.
  end.
  apply "entry" to br-c-fgg-hist in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-c-fgg-hist:
if cur-clmnbr-c-fgg-hist = ? then do:
   run OpenBr in this-procedure .
end.
else do:
   assign re-querybr-c-fgg-hist = yes.
   run sort-brbr-c-fgg-hist
     (input (if available X_cfgg-hist
             then recid(X_cfgg-hist)
             else ?
   ...
   assign re-querybr-c-fgg-hist = no.
end.
end.
MAIN-BLOCK:
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   if lookup(p-mode, "fbr-gds-grp-attr") > 0 then do:
      if lookup(p-attr-code, '':u) = 0 then do:
          message
          vss-workfile vss-revision vss-description skip
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-c-fgg-hist FOR EACH X_cfgg-hist NO-LOCK.
END PROCEDURE.
PROCEDURE MyEnable :
...
CASE p-mode:
    when 'все':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Справочник истории групп блюд"
...
run gbl/flt-get.p
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-fgg-hist:handle
  ...
  if l-filter-open-21 = false then do:
    OPEN QUERY br-c-fgg-hist FOR EACH X_cfgg-hist
      where  TRUE
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when 'объект':U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории групп блюд: Объект &1&2",
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      ...
      parameter-4-23 =
        (
          if (" X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code " + " " + where-phrase-23) <> ""
          then  substitute('X_cfgg-hist.obj-type = &1&2&1 and X_cfgg-hist.obj-code  = &3', chr(34), p-obj-type, p-obj-code)   + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-23 =
          (" X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code " + " " + where-phrase-23 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-fgg-hist:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-fgg-hist FOR EACH X_cfgg-hist
      where  X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "one":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории групп блюд: Объект &1&2 Вн. № группы &3",
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code and                              X_cfgg-hist.node-code = p-node-code " + " " + where-phrase-25) <> ""
          then  substitute('X_cfgg-hist.obj-type = &1&2&1 and X_cfgg-hist.obj-code  = &3 and                                X_cfgg-hist.node-code = &4', chr(34), p-obj-type, p-obj-code, p-node-code)   + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code and                              X_cfgg-hist.node-code = p-node-code " + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-fgg-hist:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-c-fgg-hist FOR EACH X_cfgg-hist
      where  X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code and                              X_cfgg-hist.node-code = p-node-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "fbr-gds-grp-attr":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории групп блюд: Объект &1&2 Вн. № группы &3 атрибут &4",
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          if (" X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code and                              X_cfgg-hist.node-code = p-node-code and X_cfgg-hist.subject = 'fbr-gds-grp-attr':U " + " " + where-phrase-27) <> ""
          then  substitute('X_cfgg-hist.obj-type = &1&2&1 and X_cfgg-hist.obj-code  = &3 and                                X_cfgg-hist.node-code = &4 and X_cfgg-hist.subject = &1&5&1'                                , chr(34), p-obj-type, p-obj-code, p-node-code, 'fbr-gds-grp-attr':U)   + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-27 =
          (" X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code and                              X_cfgg-hist.node-code = p-node-code and X_cfgg-hist.subject = 'fbr-gds-grp-attr':U " + " " + where-phrase-27 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-fgg-hist:handle
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-c-fgg-hist FOR EACH X_cfgg-hist
      where  X_cfgg-hist.obj-type = p-obj-type and X_cfgg-hist.obj-code  = p-obj-code and                              X_cfgg-hist.node-code = p-node-code and X_cfgg-hist.subject = 'fbr-gds-grp-attr':U
  .
      assign
        l-query-was-opened = true
      .
