...
ON CHOOSE OF B-exit IN FRAME Dialog-Frame
  ...
  END.
ON LEAVE OF f-ext-obj-guid IN FRAME Dialog-Frame
  DO:
  ...
  END.
ON LEAVE OF f-ext-obj-guidobj IN FRAME Dialog-Frame
  DO:
    define variable Msg as character no-undo .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  IF lookup(p-mode, 'ИЗМЕНЕНИЕ':U + chr(44) + 'ДОБАВЛЕНИЕ':U + chr(44) + 'ПРОСМОТР':U ) = 0  THEN
  DO:
    MESSAGE
...
PROCEDURE MyEnable :
  ASSIGN
    rs-ext-obj-type:RADIO-BUTTONS IN FRAME Dialog-Frame = 'маг':U + chr(44) + 'маг':U + chr(44) +
    'скл':U + chr(44) + 'скл':U + chr(44) +
    "Нет деления на типы объектов" + chr(44) + "" +
    (if p-ext-obj-type = 'скл':U
    then  (chr(44) +
    "+100000 (для DKlink)" + chr(44) + "+100000")
    else '')
    .
  if p-esys-type = integer('10':U) then
  do:
    if p-ext-guid <> "" then
    do:
      do ii = 1 to NUM-ENTRIES (p-ext-guid,chr(6)):
        if ii = 1 then v-ext-guidobj = ENTRY (ii,p-ext-guid,chr(6)) .
        else v-ext-guid = ENTRY (ii,p-ext-guid,chr(6)) .
      end.
    end.
    ASSIGN
      rs-ext-obj-type           = ''
    ...
    do:
      DISABLE
        ...
        f-ext-obj-code
        WITH FRAME Dialog-Frame.
      if p-ext-obj-type = 'маг':U or p-ext-obj-type = 'скл':U then
      do:
        ENABLE
          f-ext-obj-guid
          WITH FRAME Dialog-Frame.
      ...
      do:
        if p-ext-obj-type = 'орг':U and p-ext-obj-code = STRING (v-cntxt-host-code-obj) then
        do:
          ENABLE
            f-ext-obj-guidobj
            WITH FRAME Dialog-Frame.
...
PROCEDURE proc-save :
  ...
  ASSIGN
    ...
    p-ext-guid     = f-ext-obj-guidobj + chr(6) + f-ext-obj-guid
    p-ok           = YES
    .
END PROCEDURE.
