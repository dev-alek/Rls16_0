  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-ex-mark :handle
      .
...
do:
  br-c-ex-mark :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info9 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
  if available x_c-ex-mark then do:   assign     v-curr-rowid = rowid(x_c-ex-mark)   .   end.   run open-br in this-procedure .   reposition br-c-ex-mark to rowid v-curr-rowid no-error .   apply 'entry' to br-c-ex-mark in frame Dialog-Frame.
    apply "VALUE-CHANGED" to br-c-ex-mark.
end.
MAIN-BLOCK:
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-c-ex-mark FOR EACH x_c-ex-mark NO-LOCK INDEXED-REPOSITION.    OPEN QUERY br-changes FOR EACH temp-changes.
END PROCEDURE.
PROCEDURE my-enable :
  ...
  assign
    frame Dialog-Frame:title = substitute( "История марки БД: &1 внутренний код: &2" , buf_ex-mark.db-num , buf_ex-mark.mark-code )
  .
  enable
    b-exit
    b-help
...
PROCEDURE open-br :
  ...
  indexed-reposition.
  apply "value-changed" to br-c-ex-mark in frame Dialog-Frame.
  apply "entry" to br-c-ex-mark in frame Dialog-Frame.
...
PROCEDURE proc-view-changes :
  ...
  v-label-param =
    "mark-name" + chr(4) + "Код марки" + chr(4) + "" + chr(8)
  + "mark-type" + chr(4) + "Тип марки" + chr(4) + "exmrklib_get-type-name" + chr(8)
  + "stts"      + chr(4) + "Статус"    + chr(4) + "exmrklib_get-status-name"  .
  run proc-full-temp-changes in this-procedure ( input  buffer X_c-ex-mark:handle
                                               , input  'ex-mark':U
                                               , input  "mark-name,mark-type,stts":U
                                               , input  v-label-param
                                               ) .
  open query br-changes for each temp-changes.
END PROCEDURE.
