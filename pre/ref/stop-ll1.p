...
:
  ...
  if lookup(string(p-flag), '1,2,3,4':U) = 0 then do:
    assign
  ...
  if p-mode = 'ДОБАВЛЕНИЕ':U then do:
    ...
    find last buf_stop-list-line no-lock where
            buf_stop-list-line.stop-list-code = buf_stop-list.stop-list-code
        and buf_stop-list-line.classif-type = 'dis-card':U use-index pi no-error.
    if available buf_stop-list-line then do:
    ...
    buf_stop-list-line.line-message = entry (lookup (string(p-flag), '1,2,3,4':U) + 1, ',':U + 'стоп-карта,стоп-клиент,стоп-карта;стоп-клиент,удал-карта':U)
    buf_stop-list-line.resource_id = (if p-flag = integer('1':U)
  ...
  if p-mode = 'ИЗМЕНЕНИЕ':U then do:
    ...
    if buf_stop-list-line.key#_one = integer('1':U) then do:
      run gen-key-rec in this-procedure ( input 'dis-card':U
                                          ,input (buffer buf_dis-card:handle)
                                          ,output v-card-resource-id).
    end.
