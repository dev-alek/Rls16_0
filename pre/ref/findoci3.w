...
run ref/cli-all.w ( parParentProc
  ...
  , ?
  , ?
  , (if available X_payer then recid(X_payer) else ?)
  , ?
  , "without-obj":U
  , output ref-list) .
if ref-list = "" then
...
find first X_payer no-lock where
  recid(X_payer) = recid(buf_clients).
assign
  tt-fin-doc.payer-type = buf_clients.obj-type
  tt-fin-doc.payer-code = buf_clients.obj-code
  tt-fin-doc.payer-name = buf_clients.obj-name
  .
display
  tt-fin-doc.payer-type
  ...
  with frame Dialog-Frame.
define variable v-recid-schet as recid no-undo . define variable v-recid-bank as recid no-undo . CASE X_payer.obj-type:   when 'чел':U then do:              find first X_payer-person no-lock where               X_payer-person.psn-code = X_payer.obj-code .   end.   when 'орг':U then do:     find first X_payer-firm no-lock where               X_payer-firm.firm-code = X_payer.obj-code .   end. END CASE.   assign   tt-fin-doc.payer-type =  X_payer.obj-type   tt-fin-doc.payer-code = X_payer.obj-code   tt-fin-doc.payer-name = X_payer.obj-name   tt-fin-doc.payer-inn = (if X_payer.obj-type = 'чел':U                             then X_payer-person.inn                             else X_payer-firm.inn                             )   tt-fin-doc.payer-kpp = (if X_payer.obj-type = 'чел':U                             then X_payer-person.kpp                             else X_payer-firm.kpp                             )   .   run get-single-schet in this-procedure (                                           input p-host-code                                           ,input X_payer.obj-type                                           ,input X_payer.obj-code                                           ,input tt-fin-doc.curr-code                                           ,output v-recid-schet                                           ,output v-recid-bank).   if v-recid-schet <> ? then do:     find first X_payer-fin-schet no-lock where                 recid(X_payer-fin-schet) = v-recid-schet.     find first X_payer-fin-bank no-lock where                 recid(X_payer-fin-bank) = v-recid-bank.     assign     tt-fin-doc.payer-code-schet = X_payer-fin-schet.code-schet     tt-fin-doc.payer-bank-name = X_payer-fin-bank.bank-name       tt-fin-doc.payer-bank-city = X_payer-fin-bank.bank-city       tt-fin-doc.payer-dop1       = X_payer-fin-schet.dop1     tt-fin-doc.payer-dop2       = X_payer-fin-schet.dop2     tt-fin-doc.payer-bik = X_payer-fin-bank.bik     tt-fin-doc.payer-r-schet = X_payer-fin-schet.r-schet     tt-fin-doc.payer-c-schet = X_payer-fin-schet.c-schet     .   end.   else do:     RELEASE X_payer-fin-schet.     RELEASE X_payer-fin-bank.     assign     tt-fin-doc.payer-code-schet = 0     tt-fin-doc.payer-bank-name = "":U     tt-fin-doc.payer-bank-city = "":U     tt-fin-doc.payer-dop1      = "":U     tt-fin-doc.payer-dop2      = "":U     tt-fin-doc.payer-bik = "":U     tt-fin-doc.payer-r-schet = "":U     tt-fin-doc.payer-c-schet = "":U     .   end. display tt-fin-doc.payer-type tt-fin-doc.payer-code tt-fin-doc.payer-name tt-fin-doc.payer-inn tt-fin-doc.payer-kpp tt-fin-doc.payer-bank-name tt-fin-doc.payer-bank-city tt-fin-doc.payer-bik tt-fin-doc.payer-r-schet tt-fin-doc.payer-c-schet with frame Dialog-Frame.
CASE X_payer.obj-type:   when 'чел':U then do:              find first X_payer-person no-lock where               X_payer-person.psn-code = X_payer.obj-code .   end.   when 'орг':U then do:     find first X_payer-firm no-lock where               X_payer-firm.firm-code = X_payer.obj-code .   end. END CASE. assign tt-fin-doc.payer-sign1  = if X_payer.obj-type = 'орг':U                              then X_payer-firm.director                             else X_payer.obj-name . display tt-fin-doc.payer-sign1 with frame Dialog-Frame.
find first ub.CashBook no-lock where ub.CashBook.id = tt-fin-doc.cashbookId no-error .
if ub.CashBook.cli-code = tt-fin-doc.payer-code and ub.CashBook.cli-type = tt-fin-doc.payer-type then do:
assign
...
do:
  if getCliKassa(tt-fin-doc.payer-type, tt-fin-doc.payer-code, "Vnecli", tt-fin-doc.cashbookId) then do:
assign
  tt-fin-doc.cor-acc1-value = ub.CashBook.OsnAcct
  .
...
assign
  ...
  if getCliKassa(tt-fin-doc.payer-type, tt-fin-doc.payer-code, "Avanscli", tt-fin-doc.cashbookId) then do:
assign
  tt-fin-doc.cor-acc1-value = ub.CashBook.OsnAcct
  .
find first ub.CashBookRule no-lock where ub.CashBookRule.CashBookID = tt-fin-doc.cashbookId
...
run ref/finschts.w (
  ...
  ,input "b-sel,b-add":U
  ,input "cmp-host":U
  ,input X_payer.obj-type
  ,input X_payer.obj-code
  ,input ?
  ,input tt-fin-doc.host-code
  ,input 0
  ,input-output v-status_
...
if   input frame Dialog-Frame tt-fin-doc.payer-code <> 0 then
do:
...
END.
ON VALUE-CHANGED OF tt-fin-doc.payer-type IN FRAME Dialog-Frame
  DO:
    assign
      tt-fin-doc.payer-type.
    if   input frame Dialog-Frame tt-fin-doc.payer-code <> 0 then
...
PROCEDURE check-payer :
  define buffer buf_clients for ub.clients.
  find first buf_clients no-lock where
    buf_clients.obj-code = input frame Dialog-Frame tt-fin-doc.payer-code
    and buf_clients.obj-type = input frame Dialog-Frame tt-fin-doc.payer-type no-error.
  if not available buf_clients then
  do:
    if input frame Dialog-Frame tt-fin-doc.payer-code <> ?  then
      message "Неправильный код или тип " 'ПЛАТЕЛЬЩИКА'.
    apply "entry" to tt-fin-doc.payer-code in frame Dialog-Frame.
    return error.
  end.
  find first X_payer no-lock where recid(X_payer) = recid(buf_clients).
  assign
    tt-fin-doc.payer-type = buf_clients.obj-type
    tt-fin-doc.payer-code = buf_clients.obj-code
    tt-fin-doc.payer-name = buf_clients.obj-name
...
run proc-update-sum-vat-chr in this-procedure (input-output v-start).
END.
ON CHOOSE OF B-cards IN FRAME Dialog-Frame
  DO:
...
PROCEDURE disable-enable :
  ...
  CASE rs-view:
    when "full":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-cards,b-print,b-hist,b-help," + "prn-doc-code,vid-plat,doc-date,obj-type,obj-code,b-obj," +                             v-an-uchet-tab-order +                             "payer-type,payer-code,b-payer,payer-name,b-payer-schet,b-payer-view," +                              v-sum-curr-tab-order + "b-receiver-schet,b-receiver-view," +                              "naznach-plat,PS,payer-sign1,payer-sign2"
          .
      end.
    when "brief":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-cards,b-print,b-hist,b-help," +                              "prn-doc-code,vid-plat,doc-date," +                             "obj-type,obj-code,b-obj,payer-type,payer-code,b-payer,payer-name,b-payer-schet,b-payer-view," +                              v-sum-curr-tab-order + "b-receiver-schet,b-receiver-view," +                              "naznach-plat,PS,payer-sign1,payer-sign2"
          .
      end.
    when "contract":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-cards,b-print,b-hist,b-help," +                              "prn-doc-code,vid-plat,doc-date,obj-type,obj-code,b-obj," +                              v-contract-tab-order +                              "payer-type,payer-code,b-payer,payer-name,b-payer-schet,b-payer-view," +                              v-sum-curr-tab-order + "b-receiver-schet,b-receiver-view," +                              "naznach-plat,PS,payer-sign1,payer-sign2"
          .
      end.
  END CASE.
END PROCEDURE.
...
.
  if getCliKassa(tt-fin-doc.payer-type, tt-fin-doc.payer-code, "Vnecli", tt-fin-doc.cashbookId) then paramVne = "vne" .
  else if getCliKassa(tt-fin-doc.payer-type, tt-fin-doc.payer-code, "Avanscli", tt-fin-doc.cashbookId) then paramVne = "avans" .
  else paramVne = "" .
  run proc-update-sum-vat-chr in this-procedure (input-output v-start).
END PROCEDURE.
...
tt-fin-doc.payer-type:radio-buttons = "Орг" + chr(44) + 'орг':U + chr(44) +
                                         "Чел" + chr(44) + 'чел':U
...
assign
  f-curr-abbr = X_Currency.curr-abbr.
if p-mode = 'ДОБАВЛЕНИЕ':U and tt-fin-doc.payer-type = "":U  then do:
  assign
...
END.
if p-mode = 'ПРОСМОТР':U then do:
    run proc-color-widgets in this-procedure("f-an-uchet-descr,f-cel-nazn-descr,f-contract-curr-abbr,f-contract-date,f-contract-prn-code,f-contract-rate," +                         "f-contract-scale,f-contract-type,f-cor-acc-descr,base-rate,base-scale,curr-code,exch-rate,exch-scale,contract-rate,contract-scale," +                         "fact-date,fin-doc-code,payer-code,payer-type,pay-date,perm-date,PS,receiver-code,receiver-type,sum-base,sum-rubl,sum-contr," +                         "user-name-doc,user-name-fact,user-name-perm.user-name-pl", no, yes, ?, ?).
    run proc-color-widgets in this-procedure("F-curr-abbr,an-uchet-value,cel-nazn-value,cor-acc-value,doc-date," +                       "naznach-plat,payer-name,payer-inn,payer-kpp,payer-bank-name,payer-bank-city,payer-bik,payer-r-schet,payer-c-schet," +                       "prn-doc-code,receiver-name,receiver-inn,receiver-kpp,receiver-bank-name,receiver-bank-city,receiver-bik,receiver-r-schet,receiver-c-schet,payer-sign1,payer-sign2" +                       "sum-doc", no, yes, ?, ?).
end.
display
tt-fin-doc.exch-rate
tt-fin-doc.exch-scale
