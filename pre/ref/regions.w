...
assign
  v-str-recid7 = trim( string( recid( buf_regions ) , "->>>>>>>>>>>9":U ) )
  v-num-entry7 = lookup( v-str-recid7 , v-regions-mark-list )
.
if v-num-entry7 > 0 then do:
  assign
    entry( v-num-entry7, v-regions-mark-list ) = "":U
    v-regions-mark-list = trim( replace( v-regions-mark-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-regions-mark-list = v-regions-mark-list + ( if v-regions-mark-list = "":U then "":U else chr(44) ) + v-str-recid7
  .
end.
      v-log = br-region:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then  do:
        v-log = br-region:select-next-row ().
        apply "iteration-changed" to br-region in frame Dialog-Frame.
      end.
      if num-entries( v-regions-mark-list ) = 0 then
          hide mark-num in frame Dialog-Frame.
      else
          disp num-entries( v-regions-mark-list ) @ mark-num with frame Dialog-Frame.
    end.
    apply "entry" to br-region in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-region :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-region FOR EACH buf_regions NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
...
put stream prnlibstream "РЕГИОНЫ РФ":U at center-field(1,126, length("РЕГИОНЫ РФ":U)) skip(2).
assign
...
reposition br-region to rowid v-curr-doc-rowid no-error.
apply "display" to br-region in frame Dialog-Frame.
...
FUNCTION mark-str-regions RETURNS CHARACTER
  ...
  RETURN (if ( lookup( string( recid( buf_regions ) ) , v-regions-mark-list ) ) > 0 then "*":u else '':u ).
END FUNCTION.
