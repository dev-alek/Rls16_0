...
if p-obj-type <> 'маг':U
and p-obj-type <> 'скл':U then do:
  v-mess = substitute("Складские места могут быть только для &1 и &2", 'маг':U, 'скл':U).
  run err-mes in this-procedure ( input-output v-mess).
  ...
  ,output par-type
         ...
         then do:
          find last c-place where c-place.obj-type = p-obj-type
                              ...
                              and c-place.pl-code  < 10000000000
                              no-lock no-error.
          ...
          p-pl-code =  p-pl-code +  1.
           ...
           run gen-b-code in this-procedure ( input 'bcgb':U, output p-pl-code) no-error.
            if error-status:error then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curshift in g#library
  ...
  if error-status:error then do:
    v-mess = substitute("Ошибка при сохранении СКЛАДСКОГО МЕСТА&1&2&1&3", chr(10), return-value, error-status:get-message(1)).
    run err-mes in this-procedure ( input-output v-mess).
