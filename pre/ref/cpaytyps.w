...
assign
  v-str-recid10 = trim( string( recid( X_c-pay-type ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-cpay-type in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-cpay-type in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-cpay-type :handle
      .
...
do:
  v-doc-rec = recid(X_pay-type). run openbr in this-procedure. reposition br-cpay-type to recid v-doc-rec no-error.               apply 'value-changed' to br-cpay-type.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-cpay-type   as character no-undo .
def var sort-clmnbr-cpay-type    as handle    no-undo .
def var cur-clmnbr-cpay-type     as handle    no-undo .
def var cur-clmn-locbr-cpay-type as integer   no-undo .
def var re-querybr-cpay-type     as logical   initial no no-undo .
on start-search, ctrl-o of br-cpay-type in frame Dialog-Frame do:
   run sort-brbr-cpay-type
...
PROCEDURE sort-brbr-cpay-type :
  define input parameter p-recid as recid no-undo .
  if re-querybr-cpay-type = no then do:
    assign
       cur-clmnbr-cpay-type = br-cpay-type:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-cpay-type <> ? then sort-clmnbr-cpay-type:column-fgcolor = 0.
    if cur-clmnbr-cpay-type = sort-clmnbr-cpay-type then do:
      assign
         sort-labelbr-cpay-type = ""
         sort-clmnbr-cpay-type = ?
      .
     end.
     else do:
       assign
         sort-labelbr-cpay-type = cur-clmnbr-cpay-type:label
         sort-clmnbr-cpay-type  = cur-clmnbr-cpay-type
         sort-clmnbr-cpay-type:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-cpay-type = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-cpay-type:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-cpay-type then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-cpay-type = cur-clmn-locbr-cpay-type + 1
    .
  end.
  case sort-labelbr-cpay-type:
        when X_c-pay-type.obj-name:label in browse br-cpay-type then DO:   assign     sort-column-name = "X_c-pay-type.obj-name"   .   run OpenBr.   . END.
        when X_c-pay-type.corr-date:label in browse br-cpay-type then DO:   assign     sort-column-name = "X_c-pay-type.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-cpay-type') then do:
          run mv-brw-defaultbr-cpay-type.
        end.
      if sort-labelbr-cpay-type <> "" then do:
        assign
          cur-clmnbr-cpay-type:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-cpay-type = ?
      .
    end.
  end case.
    if cur-clmn-locbr-cpay-type <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-cpay-type') then do:
        run ch-clmnbr-cpay-type in this-procedure (cur-clmn-locbr-cpay-type).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-cpay-type to recid p-recid no-error.
    apply "value-changed" to br-cpay-type in frame Dialog-Frame.
  end.
  apply "entry" to br-cpay-type in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-cpay-type:
if cur-clmnbr-cpay-type = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-cpay-type = yes.
   run sort-brbr-cpay-type
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
   ...
   assign re-querybr-cpay-type = no.
end.
end.
...
DEF VAR cur-clmn-numbr-cpay-type as INT EXTENT 7 no-undo.
DEF VAR varmvibr-cpay-type       as INT no-undo.
DEF VAR varmvjbr-cpay-type       as INT no-undo.
DEF VAR varmvkbr-cpay-type       as INT no-undo.
DEF VAR varmvlbr-cpay-type       as INT no-undo.
DEF VAR move-elementbr-cpay-type as INT no-undo.
def var jjbr-cpay-type           as int no-undo.
do varmvibr-cpay-type = 1 to EXTENT(cur-clmn-numbr-cpay-type):
  ASSIGN cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = varmvibr-cpay-type.
END.
RUN start-mv-clmnbr-cpay-type.
PROCEDURE start-mv-clmnbr-cpay-type:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-cpay-type = NUM-ENTRIES('1,2,3,4,5,6,7') TO 1 BY -1:
     RUN re-move-clmnbr-cpay-type ( cur-clmn-numbr-cpay-type[INTEGER(ENTRY (jjbr-cpay-type, '1,2,3,4,5,6,7'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-cpay-type do:
  RUN re-move-clmnbr-cpay-type ( 1, 7).
END.
ON ctrl-cursor-left OF BROWSE br-cpay-type do:
  RUN re-move-clmnbr-cpay-type (7, 1).
END.
PROCEDURE re-move-clmnbr-cpay-type:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-cpay-type = 1 TO EXTENT(cur-clmn-numbr-cpay-type):
    if cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = source-column THEN cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = -1.
  END.
  if br-cpay-type:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-cpay-type = source-column - 1 to target-column BY -1:
    DO varmvibr-cpay-type = 1 TO EXTENT(cur-clmn-numbr-cpay-type):
        if cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = varmvjbr-cpay-type THEN DO:
          cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = cur-clmn-numbr-cpay-type[varmvibr-cpay-type] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-cpay-type = source-column + 1 to target-column:
    DO varmvibr-cpay-type = 1 TO EXTENT(cur-clmn-numbr-cpay-type):
      if cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = varmvjbr-cpay-type THEN DO:
        cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = cur-clmn-numbr-cpay-type[varmvibr-cpay-type] - 1.
      END.
  ...
  END.
  DO varmvibr-cpay-type = 1 TO EXTENT(cur-clmn-numbr-cpay-type):
    if cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = -1 THEN cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-cpay-type:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-cpay-type = 1 TO EXTENT(cur-clmn-numbr-cpay-type):
    if cur-clmn-numbr-cpay-type[varmvibr-cpay-type] = cur-clmn-loc THEN move-elementbr-cpay-type = varmvibr-cpay-type.
  END.
  RUN re-move-clmnbr-cpay-type (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-cpay-type:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-cpay-type = 1 to EXTENT(cur-clmn-numbr-cpay-type):
    RUN re-move-clmnbr-cpay-type (cur-clmn-numbr-cpay-type[varmvlbr-cpay-type], varmvlbr-cpay-type).
  END.
  RUN start-mv-clmnbr-cpay-type.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-cpay-type FOR EACH X_c-pay-type NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
 ...
 run proc-full-temp-changes in this-procedure (
                                             input  (X_c-pay-type.action = integer('1':U))
                                            ,input  (X_c-pay-type.action = integer('99':U))
                                            ,input  buffer X_c-pay-type:handle
                                            ,input  'pay-type':U
                                            ,input  "obj-name,obj-code"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
FUNCTION get-action RETURNS CHARACTER
