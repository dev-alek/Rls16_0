DEFINE BUFFER X_c-currency FOR ub.c-currency.
...
define variable vss-description AS CHAR NO-UNDO INIT "Список истории валюты":U.
...
DEFINE QUERY br-ccur FOR
      X_c-currency SCROLLING.
DEFINE QUERY BR-changes FOR
      temp-changes SCROLLING.
DEFINE BROWSE br-ccur
  QUERY br-ccur NO-LOCK DISPLAY
      mark-string(recid(X_c-currency), v-rid-list) COLUMN-LABEL "*" FORMAT "X(1)":U
      usrfulnf(X_c-currency.corr-user-name) COLUMN-LABEL "Изменил" FORMAT "X(18)":U
      X_c-currency.corr-date COLUMN-LABEL "Дата!измен" FORMAT "99/99/9999":U
      string(X_c-currency.corr-time, "HH:MM") COLUMN-LABEL "Время!измен" FORMAT "X(5)":U
      X_c-currency.curr-abbr FORMAT "X(3)":U
      X_c-currency.curr-name FORMAT "X(40)":U
      X_c-currency.curr-code COLUMN-LABEL "Код валюты" FORMAT ">>9":U
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-ccur in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-ccur in frame Dialog-Frame.
END.
...
DO:
    ...
    then
    v-rid-list = string( recid( X_c-currency ) ) .
  end.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-ccur :handle
      .
...
do:
  v-doc-rec = recid(X_c-currency). run OpenBr in this-procedure. reposition br-ccur to recid v-doc-rec no-error. v-doc-rec = ?.              apply 'value-changed' to br-ccur.
    apply "VALUE-CHANGED" to br-ccur.
end.
def var sort-labelbr-ccur   as character no-undo .
def var sort-clmnbr-ccur    as handle    no-undo .
def var cur-clmnbr-ccur     as handle    no-undo .
def var cur-clmn-locbr-ccur as integer   no-undo .
def var re-querybr-ccur     as logical   initial no no-undo .
on start-search, ctrl-o of br-ccur in frame Dialog-Frame do:
   run sort-brbr-ccur
     (input (if available X_c-currency
             then recid(X_c-currency)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-ccur :
  define input parameter p-recid as recid no-undo .
  if re-querybr-ccur = no then do:
    assign
       cur-clmnbr-ccur = br-ccur:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-ccur <> ? then sort-clmnbr-ccur:column-fgcolor = 0.
    if cur-clmnbr-ccur = sort-clmnbr-ccur then do:
      assign
         sort-labelbr-ccur = ""
         sort-clmnbr-ccur = ?
      .
       ...
       assign
         sort-labelbr-ccur = cur-clmnbr-ccur:label
         sort-clmnbr-ccur  = cur-clmnbr-ccur
         sort-clmnbr-ccur:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-ccur = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-ccur:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-ccur then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-ccur = cur-clmn-locbr-ccur + 1
    .
  end.
  case sort-labelbr-ccur:
        when X_c-currency.curr-abbr:label in browse br-ccur then DO:   assign     sort-column-name = "X_c-currency.curr-abbr"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-ccur') then do:
          run mv-brw-defaultbr-ccur.
        end.
      if sort-labelbr-ccur <> "" then do:
        assign
          cur-clmnbr-ccur:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-ccur = ?
    ...
    if cur-clmn-locbr-ccur <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-ccur') then do:
        run ch-clmnbr-ccur in this-procedure (cur-clmn-locbr-ccur).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-ccur to recid p-recid no-error.
    apply "value-changed" to br-ccur in frame Dialog-Frame.
  end.
  apply "entry" to br-ccur in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-ccur:
if cur-clmnbr-ccur = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-ccur = yes.
   run sort-brbr-ccur
     (input (if available X_c-currency
             then recid(X_c-currency)
             else ?
   ...
   assign re-querybr-ccur = no.
end.
end.
define variable vss-include-info15 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-ccur :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
DEF VAR cur-clmn-numbr-ccur as INT EXTENT 8 no-undo.
DEF VAR varmvibr-ccur       as INT no-undo.
DEF VAR varmvjbr-ccur       as INT no-undo.
DEF VAR varmvkbr-ccur       as INT no-undo.
DEF VAR varmvlbr-ccur       as INT no-undo.
DEF VAR move-elementbr-ccur as INT no-undo.
def var jjbr-ccur           as int no-undo.
do varmvibr-ccur = 1 to EXTENT(cur-clmn-numbr-ccur):
  ASSIGN cur-clmn-numbr-ccur[varmvibr-ccur] = varmvibr-ccur.
END.
RUN start-mv-clmnbr-ccur.
PROCEDURE start-mv-clmnbr-ccur:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-ccur = NUM-ENTRIES('1,2,3,4,5,6,7') TO 1 BY -1:
     RUN re-move-clmnbr-ccur ( cur-clmn-numbr-ccur[INTEGER(ENTRY (jjbr-ccur, '1,2,3,4,5,6,7'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-ccur do:
  RUN re-move-clmnbr-ccur ( 1, 8).
END.
ON ctrl-cursor-left OF BROWSE br-ccur do:
  RUN re-move-clmnbr-ccur (8, 1).
END.
PROCEDURE re-move-clmnbr-ccur:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-ccur = 1 TO EXTENT(cur-clmn-numbr-ccur):
    if cur-clmn-numbr-ccur[varmvibr-ccur] = source-column THEN cur-clmn-numbr-ccur[varmvibr-ccur] = -1.
  END.
  if br-ccur:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-ccur = source-column - 1 to target-column BY -1:
    DO varmvibr-ccur = 1 TO EXTENT(cur-clmn-numbr-ccur):
        if cur-clmn-numbr-ccur[varmvibr-ccur] = varmvjbr-ccur THEN DO:
          cur-clmn-numbr-ccur[varmvibr-ccur] = cur-clmn-numbr-ccur[varmvibr-ccur] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-ccur = source-column + 1 to target-column:
    DO varmvibr-ccur = 1 TO EXTENT(cur-clmn-numbr-ccur):
      if cur-clmn-numbr-ccur[varmvibr-ccur] = varmvjbr-ccur THEN DO:
        cur-clmn-numbr-ccur[varmvibr-ccur] = cur-clmn-numbr-ccur[varmvibr-ccur] - 1.
      END.
  ...
  END.
  DO varmvibr-ccur = 1 TO EXTENT(cur-clmn-numbr-ccur):
    if cur-clmn-numbr-ccur[varmvibr-ccur] = -1 THEN cur-clmn-numbr-ccur[varmvibr-ccur] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-ccur:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-ccur = 1 TO EXTENT(cur-clmn-numbr-ccur):
    if cur-clmn-numbr-ccur[varmvibr-ccur] = cur-clmn-loc THEN move-elementbr-ccur = varmvibr-ccur.
  END.
  RUN re-move-clmnbr-ccur (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-ccur:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-ccur = 1 to EXTENT(cur-clmn-numbr-ccur):
    RUN re-move-clmnbr-ccur (cur-clmn-numbr-ccur[varmvlbr-ccur], varmvlbr-ccur).
  END.
  RUN start-mv-clmnbr-ccur.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ccur FOR EACH X_c-currency NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
END PROCEDURE.
PROCEDURE OpenBr :
OPEN QUERY br-ccur FOR EACH X_c-currency NO-LOCK where
                               X_c-currency.curr-code = p-curr-code
                           AND X_c-currency.corr-user-db-num = 0
                              INDEXED-REPOSITION.
APPLY "VALUE-CHANGED" TO br-ccur in frame Dialog-Frame.
...
define buffer new_c-currency for ub.c-currency.
define buffer current_currency for ub.currency.
define variable v-chg-fields as character no-undo.
...
for each temp-changes:
    delete temp-changes.
END.
if not available X_c-currency then do:
...
v-label-param =
 ...
 + "curr-abbr" + chr(4) + "Аббревиатура" + chr(4) + "" + chr(8)
 + "curr-name" + chr(4) + "Название валюты" + chr(4) + "" + chr(8)
 + "curr-name-five" + chr(4) + "пять..." + chr(4) + "" + chr(8)
 + "curr-name-one" + chr(4) + "один/одна..." + chr(4) + "" + chr(8)
 + "curr-name-three" + chr(4) + "три..." + chr(4) + "" + chr(8)
 + "curr-eng-name" + chr(4) + "Название (англ.)+Букв.код" + chr(4) + "" + chr(8)
 + "curr-eng-name-five" + chr(4) + "five..." + chr(4) + "" + chr(8)
 + "curr-eng-name-one" + chr(4) + "one..." + chr(4) + "" + chr(8)
 + "curr-eng-name-three" + chr(4) + "three..." + chr(4) + "" + chr(8)
 + "okv-code" + chr(4) + "Код ОКВ" + chr(4) + "" + chr(8)
 + "part-abbr" + chr(4) + "Сокращение дробной части" + chr(4) + "" + chr(8)
 + "part-name" + chr(4) + "Название дробной части" + chr(4) + "" + chr(8)
 + "part-name-five" + chr(4) + "пять..." + chr(4) + "" + chr(8)
 + "part-name-one" + chr(4) + "один/одна..." + chr(4) + "" + chr(8)
 + "part-name-three" + chr(4) + "три..." + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-currency:handle
                                            ,input  'currency':U
                                            ,input  "curr-code,curr-abbr,curr-name,curr-name-five,curr-name-one,curr-name-three,curr-eng-name,curr-eng-name-five,curr-eng-name-one,curr-eng-name-three,okv-code,part-abbr,part-name,part-name-five,part-name-one,part-name-three"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
