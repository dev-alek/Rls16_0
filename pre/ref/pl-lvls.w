...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,output glog
        ...
        END.
    END.
ON CHOOSE OF b-zone IN FRAME Dialog-Frame
DO:
    ...
    OPEN QUERY BROWSE-2 FOR EACH buf_pl-level       WHERE buf_pl-level.obj-type = p-obj-type  AND buf_pl-level.obj-code = p-obj-code  AND buf_pl-level.pl-code = p-pl-code NO-LOCK,  FIRST buf_pl-level-mm OUTER-JOIN where buf_pl-level-mm.obj-type = p-obj-type  and buf_pl-level-mm.obj-code = p-obj-code  and buf_pl-level-mm.pl-code = p-pl-code  and buf_pl-level-mm.min-level <= buf_pl-level.pl-level  and buf_pl-level-mm.max-level >= buf_pl-level.pl-level NO-LOCK,  first buf_pl-level-attr where buf_pl-level-attr.obj-type = p-obj-type  and buf_pl-level-attr.obj-code = p-obj-code  and buf_pl-level-attr.pl-code = p-pl-code  and buf_pl-level-attr.pl-level = buf_pl-level.pl-level  and buf_pl-level-attr.attr-code = "tarir-delta" no-lock,  first buf2_pl-level-attr OUTER-JOIN where buf2_pl-level-attr.obj-type = p-obj-type  and buf2_pl-level-attr.obj-code = p-obj-code  and buf2_pl-level-attr.pl-code = p-pl-code  and buf2_pl-level-attr.pl-level = buf_pl-level.pl-level  and buf2_pl-level-attr.attr-code = "deltaV" no-lock INDEXED-REPOSITION.
END.
...
repeat:
  ...
  put stream vTxt unformatted
    string(buf_place.loc1) chr(9)
    string(vLevel) chr(9)
    string(round(vQnty * 1000,0)) chr(9)
    string(round((if vTarirDelta > 0 then vTarirDelta else vTarirDeltaMain),3)) chr(9)
