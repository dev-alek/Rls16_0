...
ON GO OF FRAME d-reasons-suspicious-check
  DO:
    p-code = X_ext-classif.Code no-error.
  END.
ON CHOOSE OF b-sel IN FRAME d-reasons-suspicious-check
  DO:
...
DO:
    ...
    if t-del
    then do :
      OPEN QUERY br-reasons-suspicious-check FOR EACH X_ext-classif NO-LOCK where X_ext-classif.parent = 'reasons-suspicious-check':U
...
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME d-reasons-suspicious-check:PARENT eq ?
  THEN FRAME d-reasons-suspicious-check:PARENT = ACTIVE-WINDOW.
ON WINDOW-CLOSE OF FRAME d-reasons-suspicious-check
...
define variable vss-include-info8 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  WAIT-FOR GO OF FRAME d-reasons-suspicious-check focus br-reasons-suspicious-check.
END.
...
PROCEDURE enable_UI :
  ENABLE
    ...
    b-exit
    b-sel WHEN can-do( bttns, 'ВЫБОР':U )
    WITH FRAME d-reasons-suspicious-check.
    enable
      t-del
    WITH FRAME d-reasons-suspicious-check.
  OPEN QUERY br-reasons-suspicious-check FOR EACH X_ext-classif NO-LOCK where X_ext-classif.parent = 'reasons-suspicious-check':U
