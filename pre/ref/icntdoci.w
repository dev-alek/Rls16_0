...
tt-icnt-line.state-el-cnt                 COLUMN-LABEL 'Показания!электронного!счетчика' FORMAT "->>>,>>>,>>>,>>9.999"
tt-icnt-line.state-mh-cnt                 COLUMN-LABEL 'Показания!механического!счетчика' FORMAT "->>>,>>>,>>>,>>9.999"
(tt-icnt-line.state-el-cnt - tt-icnt-line.state-mh-cnt)  @ v-delta-line COLUMN-LABEL 'Разница' FORMAT "->>>,>>>,>>>,>>9.999"
tt-icnt-line.meas-el-cnt                 COLUMN-LABEL 'Измерение!электронного!счетчика' FORMAT "->>>,>>>,>>>,>>9.999"
buf_goods.gds-name                 COLUMN-LABEL 'Название товара'
tt-icnt-line.pl-code                 COLUMN-LABEL 'Резервуар' FORMAT "999999999999"
ENABLE
...
then do:
  ...
  p-next-prev = "quit".
END.
ON CHOOSE OF B-read IN FRAME Dialog-Frame
DO:
...
PROCEDURE sort-brbr-line :
  ...
  case sort-labelbr-line:
        ...
        when 'Показания!электронного!счетчика'  then DO:   OPEN QUERY br-line                                             FOR EACH tt-icnt-line WHERE tt-icnt-line.doc-code = tt-icnt-doc.doc-code NO-LOCK,              FIRST buf_goods OUTER-JOIN NO-LOCK WHERE buf_goods.gds-code  = tt-icnt-line.gds-code BY tt-icnt-line.state-el-cnt .   . END.
        when 'Показания!механического!счетчика'  then DO:   OPEN QUERY br-line                                             FOR EACH tt-icnt-line WHERE tt-icnt-line.doc-code = tt-icnt-doc.doc-code NO-LOCK,              FIRST buf_goods OUTER-JOIN NO-LOCK WHERE buf_goods.gds-code  = tt-icnt-line.gds-code BY tt-icnt-line.state-mh-cnt .   . END.
        ...
        when 'Измерение!электронного!счетчика'  then DO:   OPEN QUERY br-line                                             FOR EACH tt-icnt-line WHERE tt-icnt-line.doc-code = tt-icnt-doc.doc-code NO-LOCK,              FIRST buf_goods OUTER-JOIN NO-LOCK WHERE buf_goods.gds-code  = tt-icnt-line.gds-code BY tt-icnt-line.meas-el-cnt .   . END.
...
IF p-mode = 'ДОБАВЛЕНИЕ':U then do:
  find first buf_rvs-doc where
      ...
      and  buf_rvs-doc.rvs-type <> 'проверка':U no-lock no-error.
  ...
  find first old_icnt-doc where old_icnt-doc.obj-type =  p-obj-type and
                                ...
                                old_icnt-doc.status_  <> 'факт':U    no-lock no-error.
...
frame Dialog-Frame:title = substitute("(&1) :   ДОКУМЕНТ ИНВЕНТАРИЗАЦИИ СЧЕТЧИКОВ ТРК - &2 № &3 - &4"
...
b-read WHEN (p-mode <> 'ПРОСМОТР':U AND tt-icnt-doc.STATUS_ = 'новый':U)
WITH frame Dialog-Frame.
...
if not glog then do:
    ASSIGN tt-icnt-line.state-el-cnt:READ-ONLY in browse br-line = YES.
end.
if p-mode = 'ПРОСМОТР':U then do:
   ASSIGN
   tt-icnt-line.state-el-cnt:READ-ONLY in browse br-line = YES
