  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-ext-artic :handle
      .
...
do:
  br-ext-artic :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
  RUN open-br IN THIS-PROCEDURE ( input ? ) .
    apply "VALUE-CHANGED" to br-ext-artic.
end.
MAIN-BLOCK:
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ext-artic FOR EACH buf_ext-artic NO-LOCK,       EACH buf_clients WHERE ub.buf_ext-artic.cli-type = buf_clients.obj-type   AND buf_ext-artic.cli-code = buf_clients.obj-code NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
...
:
  ...
  apply "value-changed":U to br-ext-artic in frame Dialog-Frame.
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_ext-artic_add-def':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
...
do:
  ...
  if buf_ext-artic.status_ = 'удал':U
  then do:
    message
      substitute("Нельзя редактировать запись в статусе '&1'" , 'удал':U)
    view-as alert-box information.
    return error.
  end.
  assign
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_ext-artic_deletion':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
  ...
  run ref/extartd.p ( input del_ext-artic.cli-type
                    ...
                    , input if del_ext-artic.status_ = 'удал':U then 'тек':U else 'удал':U
                    ) no-error .
  if error-status :error then do:
    message
      return-value skip
...
on error undo, return error return-value
:
  if b-lkp :sensitive in frame Dialog-Frame then do:
...
:
  ...
  then do:
    ...
    then do:
      assign
        b-del :label in frame Dialog-Frame = "В&осстанов":U
      .
    end.
    else do:
      assign
        b-del :label in frame Dialog-Frame = "&Удалить":U
      .
    end.
  end.
end.
