...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_place-io-reference_add-def':U
    ,input  'object':U
  ...
  run ref/pl-io-fm.w ( input parparentproc, input p-obj-type, input p-obj-code, input 'ДОБАВЛЕНИЕ':U, input-output v-rep-rec).
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_place-io-reference_update':U
    ,input  'object':U
    ,input  v-chk-act-host-code
    ,input  p-obj-type
    ,input  p-obj-code
  ...
  if NOT glog then return no-apply.
  run ref/pl-io-fm.w ( input parparentproc, input p-obj-type, input p-obj-code, input 'ИЗМЕНЕНИЕ':U,input-output v-rep-rec).
  if v-rep-rec <> ? then do:
    RUn OpenBr.
    apply "entry" to br-pl in frame d-pl-list.
  end.
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_place-io-reference_deletion':U
    ,input  'object':U
    ,input  v-chk-act-host-code
    ,input  p-obj-type
    ,input  p-obj-code
...
assign
  v-str-recid13 = trim( string( recid( X_place-io ) , "->>>>>>>>>>>9":U ) )
  v-num-entry13 = lookup( v-str-recid13 , rid-list )
.
if v-num-entry13 > 0 then do:
  assign
    entry( v-num-entry13, rid-list ) = "":U
    rid-list = trim( replace( rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    rid-list = rid-list + ( if rid-list = "":U then "":U else chr(44) ) + v-str-recid13
  .
end.
    br-pl:refresh().
...
DO:
  ...
  if RADIO-obj = 1 then do:
    ...
    if can-do ("b-add", bttns) then  ENABLE b-add  b-chg  b-del with frame d-pl-list.
  end.
  else do:
    assign p-mode = 'все':U .
    DISABLE b-add  b-chg  b-del with frame d-pl-list.
  end.
  RUN OpenBr.
  apply "entry" to br-pl .
END.
...
DO:
  ...
  case RADIO-type :
    ...
    when 2 then assign p-type = 'приемки':U .
...
do:
  v-doc-rec = recid(X_place-io). run openbr in this-procedure. reposition br-pl to recid(v-doc-rec). v-doc-rec = ? .
...
on start-search, ctrl-o of br-pl in frame d-pl-list do:
   run sort-brbr-pl
     (input (if available X_place-io
             then recid(X_place-io)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-pl :
  ...
  case sort-labelbr-pl:
        when X_place-io.place-io-type:label in browse br-pl then DO:    assign       sort-column-name = "X_place-io.place-io-type"     .     run OpenBr.   . END.
        when X_place-io.place-io-code:label in browse br-pl then DO:    assign       sort-column-name = "X_place-io.place-io-code"     .     run OpenBr.   . END.
        when X_place-io.place-io-name:label in browse br-pl then DO:    assign       sort-column-name = "X_place-io.place-io-name"     .     run OpenBr.   . END.
        when X_place-io.status_:label in browse br-pl then DO:    assign       sort-column-name = "X_place-io.status_"     .     run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
define variable vss-include-info20 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  case p-type :
    ...
    when 'приемки':U  then assign RADIO-type = 2 .
    when 'отгрузки':U then assign RADIO-type = 1 .
  end.
  if p-mode = 'объект':U then assign RADIO-obj = 1 .
  else                           assign RADIO-obj = 2 .
  DISPLAY RADIO-obj RADIO-type  WITH FRAME d-pl-list.
  RUN enable_UI.
  RUN OpenBR.
...
PROCEDURE enable_UI :
assign
  X_place-io.place-io-name:read-only in browse br-pl = true.
  ...
  CASE p-mode :
    when 'объект':U then do:
      ...
      if p-type = 'all' then do:
        ASSIGN frame d-pl-list:TITLE = "Места приемки/отгрузки " + buf_clients.obj-name    filter-point = "Места приемки/отгрузки" + p-mode.
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      ...
      parameter-4-23 =
        (
          if (" X_place-io.obj-type = p-obj-type AND X_place-io.obj-code = p-obj-code " + " " + where-phrase-23) <> ""
          then  substitute(' X_place-io.obj-type = &3&1&3 AND X_place-io.obj-code  = &2 ', p-obj-type, p-obj-code, chr(34))  + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-23 =
          (" X_place-io.obj-type = p-obj-type AND X_place-io.obj-code = p-obj-code " + " " + where-phrase-23 = "")
      .
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-pl FOR EACH X_place-io
      where  X_place-io.obj-type = p-obj-type AND X_place-io.obj-code = p-obj-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
        ...
        if p-type = 'приемки':U then ASSIGN frame d-pl-list:TITLE = "Места приемки " + buf_clients.obj-name   filter-point = "Места приемки " + p-mode .
        else                         ASSIGN frame d-pl-list:TITLE = "Места отгрузки " + buf_clients.obj-name  filter-point = "Места отгрузки " + p-mode .
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_place-io.obj-type = p-obj-type AND X_place-io.obj-code = p-obj-code and X_place-io.place-io-type = p-type " + " " + where-phrase-25) <> ""
          then  substitute(' X_place-io.obj-type = &4&1&4 AND X_place-io.obj-code  = &2 and X_place-io.place-io-type = &4&3&4 ', p-obj-type, p-obj-code, p-type, chr(34))  + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_place-io.obj-type = p-obj-type AND X_place-io.obj-code = p-obj-code and X_place-io.place-io-type = p-type " + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-pl:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-pl FOR EACH X_place-io
      where  X_place-io.obj-type = p-obj-type AND X_place-io.obj-code = p-obj-code and X_place-io.place-io-type = p-type
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when 'все':U then do:
      if p-type = 'all' then do:
        ASSIGN frame d-pl-list:TITLE = "Места приемки/отгрузки "    filter-point = "Места приемки/отгрузки" + p-mode .
...
run gbl/flt-get.p
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          ...
          then  substitute(' TRUE ')  + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + "")
  ...
  run waitfram-hide in this-procedure .
        ...
        if p-type = 'приемки':U then ASSIGN frame d-pl-list:TITLE = "Места приемки "   filter-point = "Места приемки " + p-mode .
        else                         ASSIGN frame d-pl-list:TITLE = "Места отгрузки "  filter-point = "Места отгрузки " + p-mode .
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-29 as character no-undo .
      ...
      parameter-4-29 =
        (
          if (" X_place-io.place-io-type = p-type " + " " + where-phrase-29) <> ""
          then  substitute(' X_place-io.place-io-type = &2&1&2 ', p-type, chr(34))  + " " + where-phrase-29
          else "true"
        )
      parameter-5-29 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-29 =
          (" X_place-io.place-io-type = p-type " + " " + where-phrase-29 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-pl:handle
  ...
  if l-filter-open-29 = false then do:
    OPEN QUERY br-pl FOR EACH X_place-io
      where  X_place-io.place-io-type = p-type
      indexed-reposition
  .
      assign
        l-query-was-opened = true
