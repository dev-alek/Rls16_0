...
define variable v-rec-id as recid no-undo .
  run ref/gr-bupra.w (input parparentproc,input 'ДОБАВЛЕНИЕ':U , input-output v-rec-id) .
  OPEN QUERY BROWSE-1grp FOR EACH buf_buyer-group WHERE     ( r-status = 2 OR buf_buyer-group.stts =  r-status )      AND    ( g-find = NO OR      LOOKUP (STRING (buf_buyer-group.bgr-id) + ";"  + string(buf_buyer-group.bgr-db-num)  ,        tt-buyer-group ) > 0 )      and      ( r-find = no or (buf_buyer-group.bgr-db-num = find-db and      buf_buyer-group.bgr-id = find-id ))       BY buf_buyer-group.oborot BY buf_buyer-group.NAME .
...
if v-cntxt-db-num <> 0 then do :
  ...
  run ref/gr-buprb.p (
      ...
      output v-rec-id) .
  OPEN QUERY BROWSE-2cli FOR EACH buf_buyer-in-buyer-group OF buf_buyer-group      WHERE ( r-status = 2 OR buf_buyer-in-buyer-group.stts =  r-status )       NO-LOCK ,              EACH buf_clients WHERE buf_clients.obj-code = buf_buyer-in-buyer-group.bbg-obj-code                          AND buf_clients.obj-type = buf_buyer-in-buyer-group.bbg-obj-type                              NO-LOCK INDEXED-REPOSITION.
...
v-rec-id = RECID( buf_buyer-group ).
  run ref/gr-bupra.w (input parparentproc,input 'ИЗМЕНЕНИЕ':U , input-output v-rec-id) .
  OPEN QUERY BROWSE-1grp FOR EACH buf_buyer-group WHERE     ( r-status = 2 OR buf_buyer-group.stts =  r-status )      AND    ( g-find = NO OR      LOOKUP (STRING (buf_buyer-group.bgr-id) + ";"  + string(buf_buyer-group.bgr-db-num)  ,        tt-buyer-group ) > 0 )      and      ( r-find = no or (buf_buyer-group.bgr-db-num = find-db and      buf_buyer-group.bgr-id = find-id ))       BY buf_buyer-group.oborot BY buf_buyer-group.NAME .
  reposition BROWSE-1grp to recid v-rec-id no-error .
  apply "VALUE-CHANGED" to browse-1grp in frame Dialog-Frame.
END.
...
assign
  v-str-recid11 = trim( string( recid( buf_buyer-group ) , "->>>>>>>>>>>9":U ) )
  v-num-entry11 = lookup( v-str-recid11 , p-rec-list )
.
if v-num-entry11 > 0 then do:
  assign
    entry( v-num-entry11, p-rec-list ) = "":U
    p-rec-list = trim( replace( p-rec-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    p-rec-list = p-rec-list + ( if p-rec-list = "":U then "":U else chr(44) ) + v-str-recid11
  .
end.
...
assign
  v-str-recid13 = trim( string( recid( buf_buyer-in-buyer-group ) , "->>>>>>>>>>>9":U ) )
  v-num-entry13 = lookup( v-str-recid13 , v-rec-list-cli )
.
if v-num-entry13 > 0 then do:
  assign
    entry( v-num-entry13, v-rec-list-cli ) = "":U
    v-rec-list-cli = trim( replace( v-rec-list-cli , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rec-list-cli = v-rec-list-cli + ( if v-rec-list-cli = "":U then "":U else chr(44) ) + v-str-recid13
  .
end.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1grp FOR EACH buf_buyer-group WHERE     ( r-status = 2 OR buf_buyer-group.stts =  r-status )      AND    ( g-find = NO OR      LOOKUP (STRING (buf_buyer-group.bgr-id) + ";"  + string(buf_buyer-group.bgr-db-num)  ,        tt-buyer-group ) > 0 )      and      ( r-find = no or (buf_buyer-group.bgr-db-num = find-db and      buf_buyer-group.bgr-id = find-id ))       BY buf_buyer-group.oborot BY buf_buyer-group.NAME .    OPEN QUERY BROWSE-2cli FOR EACH buf_buyer-in-buyer-group OF buf_buyer-group      WHERE ( r-status = 2 OR buf_buyer-in-buyer-group.stts =  r-status )       NO-LOCK ,              EACH buf_clients WHERE buf_clients.obj-code = buf_buyer-in-buyer-group.bbg-obj-code                          AND buf_clients.obj-type = buf_buyer-in-buyer-group.bbg-obj-type                              NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE m-tt :
