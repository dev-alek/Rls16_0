...
define input param d-str as char no-undo.
define input param r-price as decimal no-undo.
define input param c-point as char no-undo.
...
define variable v-bc-price as logical no-undo .
...
ON WINDOW-CLOSE OF FRAME b-bc-rcnz
DO:
  rid = ?.
  APPLY "END-ERROR":U TO SELF.
END.
ON CHOOSE OF b-on IN FRAME b-bc-rcnz
DO:
...
define buffer bc-on   for ub.bar-code.
  ...
  if lookup('шту':U, ub.units.type) > 0 then do:
...
do:
  ...
  run enable_ui.
  apply "entry" to br-dpl in frame b-bc-rcnz.
  reposition br-dpl to recid v-rep-rec no-error.
END.
ON CHOOSE OF b-quit IN FRAME b-bc-rcnz
DO:
  rid = ?.
END.
ON CHOOSE OF b-sel IN FRAME b-bc-rcnz
DO:
  if c-point = "choose" then rid = recid (ub.prod-bc).
END.
ON MOUSE-SELECT-DBLCLICK OF br-dpl IN FRAME b-bc-rcnz
DO:
  apply "choose" to b-sel in frame b-bc-rcnz.
  return no-apply.
END.
...
if available ub.parts then
  disp ub.parts.part-code ub.parts.in-code with frame b-bc-rcnz.
else
  disp "" @ ub.parts.part-code "" @ ub.parts.in-code with frame b-bc-rcnz.
find ub.gds-prt where
     ub.bar-code.node-code = ub.gds-prt.node-code no-lock.
disp ub.gds-prt.f-name with frame b-bc-rcnz.
END.
define variable vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame b-bc-rcnz
do:
  run gbl/app_help.p
...
on choose of b-help in frame b-bc-rcnz
do:
  apply "help":u to frame b-bc-rcnz .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame b-bc-rcnz:width - 0.3
                fh            = frame b-bc-rcnz:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame b-bc-rcnz :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame b-bc-rcnz :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame b-bc-rcnz :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame b-bc-rcnz :height = v-frame-height
          .
          if frame b-bc-rcnz :scrollable = true
          then do:
            assign
              frame b-bc-rcnz :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame b-bc-rcnz :height
      v-frame-virtual-height = frame b-bc-rcnz :virtual-height
    ...
    assign
      v-field-group-handle = frame b-bc-rcnz :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame b-bc-rcnz :virtual-height = frame b-bc-rcnz :virtual-height + p-change-value
          no-error .
      ...
      assign
        frame b-bc-rcnz :height = frame b-bc-rcnz :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame b-bc-rcnz :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame b-bc-rcnz :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame b-bc-rcnz :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame b-bc-rcnz :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame b-bc-rcnz :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame b-bc-rcnz :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame b-bc-rcnz :width = v-frame-width
          .
          if frame b-bc-rcnz :scrollable = true
          then do:
            assign
              frame b-bc-rcnz :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame b-bc-rcnz :width
      v-frame-virtual-width = frame b-bc-rcnz :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame b-bc-rcnz :virtual-width = frame b-bc-rcnz :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame b-bc-rcnz :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame b-bc-rcnz :width = frame b-bc-rcnz :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame b-bc-rcnz :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame b-bc-rcnz :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame b-bc-rcnz :height - v-diasize-resize-button :height
                  - 1
                  - (frame b-bc-rcnz :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame b-bc-rcnz :width - v-diasize-resize-button :width
                  - 1
                  - (frame b-bc-rcnz :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame b-bc-rcnz
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame b-bc-rcnz :height
      v-col-delta = v-new-col - frame b-bc-rcnz :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame b-bc-rcnz :width
      v-diasize-current-frame-height = frame b-bc-rcnz :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame b-bc-rcnz :height
        v-diasize-orig-frame-width  = frame b-bc-rcnz :width
        v-diasize-browse-handle     = browse br-dpl :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame b-bc-rcnz :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
do:
  br-dpl :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME b-bc-rcnz:PARENT eq ?
THEN FRAME b-bc-rcnz:PARENT = ACTIVE-WINDOW.
MAIN-BLOCK:
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
  duplicate-str = d-str.
  RUN enable_UI.
  if c-point <> 'ИЗМЕНЕНИЕ':U then
    disable b-on with frame b-bc-rcnz.
  ...
  if r-price <> ? and
    ...
    req-price = r-price.
    disp req-price with frame b-bc-rcnz.
  end.
  else hide req-price in frame b-bc-rcnz.
  assign ub.goods.gds-name:resizable in browse br-dpl = yes
         ub.goods.gds-name:width-chars in browse br-dpl = 35  .
  apply "value-changed" to br-dpl in frame b-bc-rcnz.
  reposition br-dpl to recid rid no-error.
  WAIT-FOR GO OF FRAME b-bc-rcnz focus br-dpl.
END.
RUN disable_UI.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-dpl FOR EACH ub.prod-bc       WHERE prod-bc.b-str = duplicate-str NO-LOCK,       EACH ub.bar-code OF ub.prod-bc NO-LOCK,       EACH ub.goods OF ub.bar-code NO-LOCK     BY ub.goods.artic.
END PROCEDURE.
