...
assign
  v-str-recid13 = trim( string( recid( X_c-action-role ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry13 > 0 then do:
  ...
  .
        ...
        glog = br-c-action-role :refresh( )  in frame Dialog-Frame.
        if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then
        do:
            glog = br-c-action-role:select-next-row () in frame Dialog-Frame.
            apply "value-changed" to br-c-action-role in frame Dialog-Frame.
        end.
        ...
        apply "entry" to br-c-action-role in frame Dialog-Frame.
    END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-action-role :handle
      .
...
do:
  v-doc-rec = recid(X_c-action-role). run openbr in this-procedure. reposition br-c-action-role to recid v-doc-rec no-error.               APPLY 'ENTRY' to br-c-action-role. APPLY 'VALUE-CHANGED' to br-c-action-role.
    apply "VALUE-CHANGED" to BR-c-action-role.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-c-action-role :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
def var sort-labelbr-c-action-role   as character no-undo .
def var sort-clmnbr-c-action-role    as handle    no-undo .
def var cur-clmnbr-c-action-role     as handle    no-undo .
def var cur-clmn-locbr-c-action-role as integer   no-undo .
def var re-querybr-c-action-role     as logical   initial no no-undo .
on start-search, ctrl-o of br-c-action-role in frame Dialog-Frame do:
   run sort-brbr-c-action-role
     (input (if available X_c-action-role
             then recid(X_c-action-role)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-c-action-role :
  define input parameter p-recid as recid no-undo .
  if re-querybr-c-action-role = no then do:
    assign
       cur-clmnbr-c-action-role = br-c-action-role:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-c-action-role <> ? then sort-clmnbr-c-action-role:column-fgcolor = 0.
    if cur-clmnbr-c-action-role = sort-clmnbr-c-action-role then do:
      assign
         sort-labelbr-c-action-role = ""
         sort-clmnbr-c-action-role = ?
      .
     end.
     else do:
       assign
         sort-labelbr-c-action-role = cur-clmnbr-c-action-role:label
         sort-clmnbr-c-action-role  = cur-clmnbr-c-action-role
         sort-clmnbr-c-action-role:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-c-action-role = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-c-action-role:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-c-action-role then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-c-action-role = cur-clmn-locbr-c-action-role + 1
    .
  end.
  case sort-labelbr-c-action-role:
        when X_c-action-role.corr-date:label in browse br-c-action-role then DO:    assign       sort-column-name = "X_c-action-role.corr-date"     .     run OpenBr in this-procedure.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure.
      if sort-labelbr-c-action-role <> "" then do:
        assign
          cur-clmnbr-c-action-role:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-c-action-role = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-c-action-role to recid p-recid no-error.
    apply "value-changed" to br-c-action-role in frame Dialog-Frame.
  end.
  apply "entry" to br-c-action-role in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-c-action-role:
if cur-clmnbr-c-action-role = ? then do:
   run OpenBr in this-procedure.
end.
else do:
   assign re-querybr-c-action-role = yes.
   run sort-brbr-c-action-role
     (input (if available X_c-action-role
             then recid(X_c-action-role)
             else ?
   ...
   assign re-querybr-c-action-role = no.
end.
end.
...
define variable l-query-was-opened as logical   no-undo .
    ...
    CASE parref-mode:
        when 'все':U then
            do:
                ASSIGN
                    frame Dialog-Frame:TITLE = "Справочник групп прав"
...
run gbl/flt-get.p
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-action-role:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-action-role FOR EACH X_c-action-role
      where  TRUE
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
        ...
        when "one":U then
            do:
                ASSIGN
                    frame Dialog-Frame:TITLE = substitute("Справочник групп прав: БД &1 код права &2 код роли &3",
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_c-action-role.db-num = pardb-num AND                             X_c-action-role.action-head-code = parhead-code and X_c-action-role.action-role-code = parrole-code" + " " + where-phrase-25) <> ""
          then  substitute('X_c-action-role.db-num = &1 AND                             X_c-action-role.action-head-code = &3 and X_c-action-role.action-role-code = &4', pardb-num, chr(34), parhead-code, parrole-code) + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_c-action-role.db-num = pardb-num AND                             X_c-action-role.action-head-code = parhead-code and X_c-action-role.action-role-code = parrole-code" + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-action-role:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-c-action-role FOR EACH X_c-action-role
      where  X_c-action-role.db-num = pardb-num AND                             X_c-action-role.action-head-code = parhead-code and X_c-action-role.action-role-code = parrole-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
        ...
        when "one-subject":U then
            do:
                ASSIGN
                    frame Dialog-Frame:TITLE = substitute("Справочник групп прав: БД &1 &2 &3 &4"
                                                       , pardb-num
                                                       , parhead-code
                                                       , parrole-code
                                                       , p-subject
...
FUNCTION get-action RETURNS CHARACTER
    ...
    assign
        dops = entry (lookup (trim(string(p-action)), '99,1,2,3,4,9,51,79':U), 'Удаление,Создание,Изменение,Коррекция,Восстановление,Смена_кода,Смена_артик,Выключ.':U) no-error.
    RETURN dops.
END FUNCTION.
FUNCTION get-subject RETURNS CHARACTER
    ...
    define variable dops as character no-undo.
    case p-subject:
        when 'action-role':U then
            do:
                assign
                    dops = "роль" no-error.
            end.
        when 'action-role-item':U then
            do:
                assign
                    dops = "право" no-error.
            end.
