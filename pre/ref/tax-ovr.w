...
.
define variable vss-include-info64 as character format "x(65)" no-undo initial "@(#)$Workfile$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output varinvprb
  )  .
   find first in-vatp_doc-attr no-lock
...
assign
  ...
  price-base-with-tax-loc = buf_parts.price-base
.
define variable vss-include-info84 as character format "x(65)" no-undo initial "@(#)$Workfile$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output varinvprb
  )  .
  if buf_parts.out-code = 'free-zone':U     or
...
END.
ON CHOOSE OF bt-close IN FRAME frm
DO:
END.
ON CHOOSE OF bt-export IN FRAME frm
DO:
   ...
   .
END.
ON CHOOSE OF bt-form-goods-list IN FRAME frm
DO:
    ...
    OPEN QUERY br-goods FOR EACH gds-list,        first buf_goods where buf_goods.gds-code = gds-list.gds-code.
END.
ON CHOOSE OF bt-form-ovr-obj IN FRAME frm
DO:
    if not available obj-list then do:
      message "Не выбран объект в списке" view-as alert-box.
      return no-apply.
...
END.
ON CHOOSE OF bt-form-ovr-obj-2 IN FRAME frm
DO:
    message
...
DO:
  ...
  .
  apply "choose" to bt-export in frame frm.
END.
ON CHOOSE OF MENU-ITEM m_obj-list
DO:
  assign
...
on choose of b-help in frame frm
do:
  apply "help":u to frame frm .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame frm:width - 0.3
                fh            = frame frm:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
define variable vss-include-info89 as character format "x(65)" no-undo initial "@(#)$Workfile$ Библиотека изменения размеров окна".
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame frm :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame frm :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame frm :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame frm :height = v-frame-height
          .
          if frame frm :scrollable = true
          then do:
            assign
              frame frm :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame frm :height
      v-frame-virtual-height = frame frm :virtual-height
    ...
    assign
      v-field-group-handle = frame frm :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame frm :virtual-height = frame frm :virtual-height + p-change-value
          no-error .
        if error-status :error
      ...
      assign
        frame frm :height = frame frm :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame frm :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame frm :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame frm :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame frm :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame frm :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame frm :width = v-frame-width
          .
          if frame frm :scrollable = true
          then do:
            assign
              frame frm :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame frm :width
      v-frame-virtual-width = frame frm :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame frm :virtual-width = frame frm :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame frm :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame frm :width = frame frm :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame frm :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame frm :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame frm :height - v-diasize-resize-button :height
                  - 1
                  - (frame frm :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame frm :width - v-diasize-resize-button :width
                  - 1
                  - (frame frm :border-right-pixels / session :pixels-per-column)
    .
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame frm :height
      v-col-delta = v-new-col - frame frm :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame frm :width
      v-diasize-current-frame-height = frame frm :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame frm :height
        v-diasize-orig-frame-width  = frame frm :width
        v-diasize-browse-handle     = browse br-goods :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame frm :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME frm:PARENT eq ?
THEN FRAME frm:PARENT = ACTIVE-WINDOW.
MAIN-BLOCK:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curobjdt in g#library
  (input  buf_tax-rate-value.obj-type
...
then do:
    ...
    skip(1) "Текущая дата на объекте:" chr(9) chr(9) chr(9) string( v-obj-current-date, "99/99/9999" )
    skip    "Дата начала действия ставки налога:" chr(9) string( buf_tax-rate-value.fact-date, "99/99/9999" )
...
PROCEDURE enable_UI :
  ENABLE bt-close bt-form-goods-list bt-form-ovr-obj bt-form-ovr-obj-2 b-help
      ...
      WITH FRAME frm.
  VIEW FRAME frm.
  OPEN QUERY br-goods FOR EACH gds-list,        first buf_goods where buf_goods.gds-code = gds-list.gds-code.    OPEN QUERY br-objects FOR EACH obj-list.
END PROCEDURE.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
    ...
    then do:
        assign
            v-end-message = v-end-message + chr(10) + string(buf_obj-list.obj-type) + string(buf_obj-list.obj-code)
            + chr(9) + "Нет товаров для ДНЦ"
        .
    ...
    then do:
        assign
            v-end-message = v-end-message + chr(10) + string(buf_obj-list.obj-type) + string(buf_obj-list.obj-code)
            + chr(9) + "Ошибка при создании документа"
        .
        next create-on-object.
    end.
    else do:
    ...
    assign
        v-end-message = v-end-message + chr(10) + string(buf_obj-list.obj-type) + string(buf_obj-list.obj-code)
        + chr(9) + string(buf_price-doc-forming.pdf-id) + chr(9) + string(v-counter) + " товаров" + chr(10)
    .
end.
run waitfram-hide .
message
...
on error undo, return error
:
define input parameter p-save-option as character no-undo.
