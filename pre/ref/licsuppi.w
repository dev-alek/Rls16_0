...
END.
ON CHOOSE OF b-supp IN FRAME Dialog-Frame
DO:
...
assign
  ...
  v-num-entry11 = lookup( v-str-recid11 , rid-list-supp )
.
if v-num-entry11 > 0 then do:
  assign
    entry( v-num-entry11, rid-list-supp ) = "":U
    rid-list-supp = trim( replace( rid-list-supp , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    rid-list-supp = rid-list-supp + ( if rid-list-supp = "":U then "":U else chr(44) ) + v-str-recid11
  .
end.
     if t-all
      then do:
...
PROCEDURE create-lic :
    ...
    define variable v-code as integer no-undo .
     find first buf_clients
          ...
          no-error
          .
     v-code = next-value(s-alc-supp-lic, ub).
