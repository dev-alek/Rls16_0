...
assign
  v-str-recid10 = trim( string( recid( X_c-tare ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-ctare in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-ctare in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-ctare :handle
      .
...
do:
  v-doc-rec = recid(X_tare). run openbr in this-procedure. reposition br-ctare to recid v-doc-rec no-error.               apply 'value-changed' to br-ctare.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-ctare   as character no-undo .
def var sort-clmnbr-ctare    as handle    no-undo .
def var cur-clmnbr-ctare     as handle    no-undo .
def var cur-clmn-locbr-ctare as integer   no-undo .
def var re-querybr-ctare     as logical   initial no no-undo .
on start-search, ctrl-o of br-ctare in frame Dialog-Frame do:
   run sort-brbr-ctare
...
PROCEDURE sort-brbr-ctare :
  define input parameter p-recid as recid no-undo .
  if re-querybr-ctare = no then do:
    assign
       cur-clmnbr-ctare = br-ctare:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-ctare <> ? then sort-clmnbr-ctare:column-fgcolor = 0.
    if cur-clmnbr-ctare = sort-clmnbr-ctare then do:
      assign
         sort-labelbr-ctare = ""
         sort-clmnbr-ctare = ?
      .
       ...
       assign
         sort-labelbr-ctare = cur-clmnbr-ctare:label
         sort-clmnbr-ctare  = cur-clmnbr-ctare
         sort-clmnbr-ctare:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-ctare = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-ctare:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-ctare then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-ctare = cur-clmn-locbr-ctare + 1
    .
  end.
  case sort-labelbr-ctare:
        when X_c-tare.tare-code:label in browse br-ctare then DO:   assign     sort-column-name = "X_c-tare.tare-code"   .   run OpenBr.   . END.
        when X_c-tare.corr-date:label in browse br-ctare then DO:   assign     sort-column-name = "X_c-tare.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-ctare') then do:
          run mv-brw-defaultbr-ctare.
        end.
      if sort-labelbr-ctare <> "" then do:
        assign
          cur-clmnbr-ctare:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-ctare = ?
      .
    end.
  end case.
    if cur-clmn-locbr-ctare <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-ctare') then do:
        run ch-clmnbr-ctare in this-procedure (cur-clmn-locbr-ctare).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-ctare to recid p-recid no-error.
    apply "value-changed" to br-ctare in frame Dialog-Frame.
  end.
  apply "entry" to br-ctare in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-ctare:
if cur-clmnbr-ctare = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-ctare = yes.
   run sort-brbr-ctare
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
...
DEF VAR cur-clmn-numbr-ctare as INT EXTENT 6 no-undo.
DEF VAR varmvibr-ctare       as INT no-undo.
DEF VAR varmvjbr-ctare       as INT no-undo.
DEF VAR varmvkbr-ctare       as INT no-undo.
DEF VAR varmvlbr-ctare       as INT no-undo.
DEF VAR move-elementbr-ctare as INT no-undo.
def var jjbr-ctare           as int no-undo.
do varmvibr-ctare = 1 to EXTENT(cur-clmn-numbr-ctare):
  ASSIGN cur-clmn-numbr-ctare[varmvibr-ctare] = varmvibr-ctare.
END.
RUN start-mv-clmnbr-ctare.
PROCEDURE start-mv-clmnbr-ctare:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-ctare = NUM-ENTRIES('1,2,3,4,5,6') TO 1 BY -1:
     RUN re-move-clmnbr-ctare ( cur-clmn-numbr-ctare[INTEGER(ENTRY (jjbr-ctare, '1,2,3,4,5,6'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-ctare do:
  RUN re-move-clmnbr-ctare ( 1, 6).
END.
ON ctrl-cursor-left OF BROWSE br-ctare do:
  RUN re-move-clmnbr-ctare (6, 1).
END.
PROCEDURE re-move-clmnbr-ctare:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-ctare = 1 TO EXTENT(cur-clmn-numbr-ctare):
    if cur-clmn-numbr-ctare[varmvibr-ctare] = source-column THEN cur-clmn-numbr-ctare[varmvibr-ctare] = -1.
  END.
  if br-ctare:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-ctare = source-column - 1 to target-column BY -1:
    DO varmvibr-ctare = 1 TO EXTENT(cur-clmn-numbr-ctare):
        if cur-clmn-numbr-ctare[varmvibr-ctare] = varmvjbr-ctare THEN DO:
          cur-clmn-numbr-ctare[varmvibr-ctare] = cur-clmn-numbr-ctare[varmvibr-ctare] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-ctare = source-column + 1 to target-column:
    DO varmvibr-ctare = 1 TO EXTENT(cur-clmn-numbr-ctare):
      if cur-clmn-numbr-ctare[varmvibr-ctare] = varmvjbr-ctare THEN DO:
        cur-clmn-numbr-ctare[varmvibr-ctare] = cur-clmn-numbr-ctare[varmvibr-ctare] - 1.
      END.
  ...
  END.
  DO varmvibr-ctare = 1 TO EXTENT(cur-clmn-numbr-ctare):
    if cur-clmn-numbr-ctare[varmvibr-ctare] = -1 THEN cur-clmn-numbr-ctare[varmvibr-ctare] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-ctare:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-ctare = 1 TO EXTENT(cur-clmn-numbr-ctare):
    if cur-clmn-numbr-ctare[varmvibr-ctare] = cur-clmn-loc THEN move-elementbr-ctare = varmvibr-ctare.
  END.
  RUN re-move-clmnbr-ctare (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-ctare:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-ctare = 1 to EXTENT(cur-clmn-numbr-ctare):
    RUN re-move-clmnbr-ctare (cur-clmn-numbr-ctare[varmvlbr-ctare], varmvlbr-ctare).
  END.
  RUN start-mv-clmnbr-ctare.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ctare FOR EACH X_c-tare NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
  "tare-code" + chr(4) + "Код" + chr(4) + "" + chr(8)
  + "stts" + chr(4) + "Статус" + chr(4) + "" + chr(8)
  + "tare-name" + chr(4) + "Длинное название" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-tare:handle
                                            ,input  'tare':U
                                            ,input  "tare-code,stts,tare-name"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
