...
DEFINE BROWSE BR-c-cash-pay
  QUERY BR-c-cash-pay DISPLAY
      ...
      entry (lookup (X_c-cash-pay.subject, 'cash-pay,cash-pay-attr,dis-cp-rule':U), 'Касс.платеж,Аттр.касс.пл-жа,Скидки на платеж':U) COLUMN-LABEL "Предмет изменений" FORMAT "X(12)":U
...
assign
  v-str-recid12 = trim( string( recid( X_c-cash-pay ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry12 > 0 then do:
 ...
 glog = br-c-cash-pay :refresh( )  in frame Dialog-Frame.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
          glog = br-c-cash-pay:select-next-row () in frame Dialog-Frame.
          apply "value-changed" to br-c-cash-pay in frame Dialog-Frame.
  end.
  ...
  apply "entry" to br-c-cash-pay in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-cash-pay :handle
      .
...
do:
  rep-rec = recid(X_c-cash-pay). run openbr in this-procedure  ( input yes, input no, input '':U). reposition br-c-cash-pay to recid rep-rec no-error.                APPLY 'ENTRY' to br-c-cash-pay. APPLY 'VALUE-CHANGED' to br-c-cash-pay.
    apply "VALUE-CHANGED" to BR-c-cash-pay.
end.
...
CASE parref-mode:
    when 'все':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Справочник истории кассовых платежей"
...
run gbl/flt-get.p
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-cash-pay:handle
  ...
  if l-filter-open-21 = false then do:
    OPEN QUERY br-c-cash-pay FOR EACH X_c-cash-pay
      where  TRUE
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when "one":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории кассовых платежей: код платежа &1 код валюты &2",
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      ...
      parameter-4-23 =
        (
          if (" X_c-cash-pay.curr-code = p-curr-code AND X_c-cash-pay.cdpay-code = p-cdpay-code " + " " + where-phrase-23) <> ""
          then  substitute('X_c-cash-pay.curr-code = &1 AND X_c-cash-pay.cdpay-code = &2 ', p-curr-code, p-cdpay-code )  + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-23 =
          (" X_c-cash-pay.curr-code = p-curr-code AND X_c-cash-pay.cdpay-code = p-cdpay-code " + " " + where-phrase-23 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-cash-pay:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-cash-pay FOR EACH X_c-cash-pay
      where  X_c-cash-pay.curr-code = p-curr-code AND X_c-cash-pay.cdpay-code = p-cdpay-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "one-subject":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории кассовых платежей: код платежа &1 код валюты &2; &3"
                                                       , p-cdpay-code
                                                       , p-curr-code
                                                       , entry (lookup (p-subject, 'cash-pay,cash-pay-attr,dis-cp-rule':U), 'Касс.платеж,Аттр.касс.пл-жа,Скидки на платеж':U)
                                                       )
        filter-point = filter-point0 + parref-mode
        filter-label = substitute("&1 - один тип платежа", filter-label0 )
        .
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_c-cash-pay.curr-code = p-curr-code AND X_c-cash-pay.cdpay-code = p-cdpay-code AND                             X_c-cash-pay.subject = p-subject " + " " + where-phrase-25) <> ""
          then  substitute('X_c-cash-pay.curr-code = &1 AND X_c-cash-pay.cdpay-code = &2 AND                             X_c-cash-pay.subject = &3&4&3', p-curr-code, p-cdpay-code, chr(34), p-subject)  + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_c-cash-pay.curr-code = p-curr-code AND X_c-cash-pay.cdpay-code = p-cdpay-code AND                             X_c-cash-pay.subject = p-subject " + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-cash-pay:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-c-cash-pay FOR EACH X_c-cash-pay
      where  X_c-cash-pay.curr-code = p-curr-code AND X_c-cash-pay.cdpay-code = p-cdpay-code AND                             X_c-cash-pay.subject = p-subject
  .
      assign
        l-query-was-opened = true
      .
