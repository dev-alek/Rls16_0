...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-5 as character no-undo .
      ...
      parameter-4-5 =
        (
          if (" X_clients.obj-type = 'маг':U                     and X_clients.db-num = g#db-num                     and X_clients.obj-name begins NameOrCode " + " " + where-phrase-5) <> ""
          then  substitute('X_clients.obj-type = &1&2&1                     and X_clients.db-num = &3                     and X_clients.obj-name begins &1&4&1 ', chr(34), 'маг':U, g#db-num, NameOrCode) + " " + where-phrase-5
          else "true"
        )
    ...
    :
      assign
        l-disable-where-5 =
          (" X_clients.obj-type = 'маг':U                     and X_clients.db-num = g#db-num                     and X_clients.obj-name begins NameOrCode " + " " + where-phrase-5 = "")
      .
  ...
  if l-filter-open-5 = false then do:
    OPEN QUERY CLi-ListB FOR EACH X_clients no-lock
      where  X_clients.obj-type = 'маг':U                     and X_clients.db-num = g#db-num                     and X_clients.obj-name begins NameOrCode
  ...
  :
    if query cli-listB:handle:get-buffer-handle(1) = (buffer X_clients:handle) then do:
      ...
      parameter-4-5 =
        "where ":u +  substitute('X_clients.obj-type = &1&2&1                     and X_clients.db-num = &3                     and X_clients.obj-name begins &1&4&1 ', chr(34), 'маг':U, g#db-num, NameOrCode) + " ":u + where-phrase-5 + " ":u + p-find-condition + " " + ""
      parameter-5-5 = ""
    .
