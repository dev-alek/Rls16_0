...
END FUNCTION.
DEFINE shared TEMP-TABLE tt-chk-type NO-UNDO
...
DO:
   ...
   assign
      v-focused-row      = br-chk-type:focused-row in frame f-chk-type
      v-repositioned-row = current-result-row("br-chk-type")
      .
   OPEN QUERY br-chk-type FOR EACH tt-work-chk-type NO-LOCK.
   if v-focused-row > br-chk-type:height - 2
   then do:
      assign
   ...
   br-chk-type:set-repositioned-row(v-focused-row, "ALWAYS") in frame f-chk-type.
   reposition br-chk-type to row v-repositioned-row.
   apply "entry" to br-chk-type in frame f-chk-type.
END.
ON CHOOSE OF b-sel IN FRAME f-chk-type
DO:
...
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME f-chk-type:PARENT eq ?
  THEN FRAME f-chk-type:PARENT = ACTIVE-WINDOW.
ON WINDOW-CLOSE OF FRAME f-chk-type
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdbnum in g#library
  ...
  WAIT-FOR GO OF FRAME f-chk-type focus br-chk-type.
END.
...
PROCEDURE FillTT :
   ...
   if not can-find(first tt-chk-type) then do:
      do vI = 1 to num-entries("1,6,8,11,12,13,40,69,96,14,15,16,17,36,101,106,108,111,112,113,169,196,114,115,116,117,136,201,206,208,301,306,2,3,4,5,7,43,44"):
         vCode = integer(entry(vI, "1,6,8,11,12,13,40,69,96,14,15,16,17,36,101,106,108,111,112,113,169,196,114,115,116,117,136,201,206,208,301,306,2,3,4,5,7,43,44")).
         vName = entry(vI, "Продажа,Возврат,Аннуляция,Инвентаризация,Z-отчет,Закрытие_смены,Открытие_смены,Списание,ВзврСпис,СбросТрнзкц,Перелив,ПеревТрнзкц,ТехПролив,РазблТрнзкц,_Продажа,_Возврат,_Аннуляция,_Инвентаризация,_Z-отчет,_Закрытие_смены,_Списание,_ВзврСпис,_СбросТрнзкц,_Перелив,_ПеревТрнзкц,_ТехПролив,_РазблТрнзкц,>Продажа,>Возврат,>Аннуляция,>>Продажа,>>Возврат,Инкассация,Касс_фонд,Перевод_опл,Расход_кассы,Декл_ден_ящ,Приход_Корр,Расход_Корр").
