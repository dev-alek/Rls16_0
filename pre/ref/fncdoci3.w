...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH locked_c-fin-doc SHARE-LOCK,       EACH tt-fin-doc WHERE TRUE  SHARE-LOCK.
...
.
tt-c-fin-doc.payer-inn:label in frame Dialog-Frame = "ИНН".
tt-c-fin-doc.receiver-inn:label in frame Dialog-Frame = "ИНН".
tt-c-fin-doc.payer-kpp:label in frame Dialog-Frame = "КПП".
tt-c-fin-doc.receiver-kpp:label in frame Dialog-Frame = "КПП".
...
tt-c-fin-doc.obj-type:radio-buttons = "Маг" + chr(44) + 'маг':U + chr(44) +
                                      "Скл" + chr(44) + 'скл':U
tt-c-fin-doc.vid-plat:list-items = 'почтой,телеграфом,электронно,':U
rs-view = v-view
.
...
if tt-c-fin-doc.cor-acc <> 0 then do:
  ...
  or (p-mode = 'ИЗМЕНЕНИЕ':U and X_fin-code-cor-acc.code-value <> tt-c-fin-doc.cor-acc-value ) then do:
    assign
...
find first X_fin-code-an-uchet no-lock where
  ...
  or (p-mode = 'ИЗМЕНЕНИЕ':U and X_fin-code-an-uchet.code-value <> tt-c-fin-doc.an-uchet-value ) then do:
    assign
...
if tt-c-fin-doc.cel-nazn-code <> 0 then do:
  ...
  or (p-mode = 'ИЗМЕНЕНИЕ':U and X_fin-code-cel-nazn.code-value <> tt-c-fin-doc.cel-nazn-value ) then do:
    assign
