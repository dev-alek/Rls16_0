...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-docs FOR EACH buf_c-turnover-group no-lock.
END PROCEDURE.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-13 as character no-undo .
      assign
      parameter-3-13 =
                              "FOR EACH buf_c-turnover-group"
      parameter-4-13 =
        (
          if (" buf_c-turnover-group.tog-id = par-tog-id and buf_c-turnover-group.tog-db-num = par-tog-db-num " + " " + where-phrase-13) <> ""
          then  substitute ( ' buf_c-turnover-group.tog-id = &1 and buf_c-turnover-group.tog-db-num = &2 ', par-tog-id , par-tog-db-num )  + " " + where-phrase-13
          else "true"
        )
      parameter-5-13 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-13 =
          (" buf_c-turnover-group.tog-id = par-tog-id and buf_c-turnover-group.tog-db-num = par-tog-db-num " + " " + where-phrase-13 = "")
      .
  ...
  if l-filter-open-13 = false then do:
    OPEN QUERY br-docs FOR EACH buf_c-turnover-group
      where  buf_c-turnover-group.tog-id = par-tog-id and buf_c-turnover-group.tog-db-num = par-tog-db-num
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  assign
    doc-rec = recid( buf_c-turnover-group )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf_c-turnover-group:handle) then do:
      assign
      parameter-2-13 = (if p-find-next then "true":u else "false":u )
      parameter-4-13 =
        "where ":u +  substitute ( ' buf_c-turnover-group.tog-id = &1 and buf_c-turnover-group.tog-db-num = &2 ', par-tog-id , par-tog-db-num )  + " ":u + where-phrase-13 + " ":u + p-find-condition + " " + ""
      parameter-5-13 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
                          ,input rowid(buf_c-turnover-group)
                          ,input logical(parameter-2-13)
                          ,input no-lock
                          ,input (buffer buf_c-turnover-group:handle)
                          ,input parameter-4-13
                          ,input parameter-5-13
                          ) no-error.
      .
      ...
      assign
      parameter-2-13 = (if p-find-next then "true":u else "false":u )
      parameter-3-13 =  "FOR EACH buf_c-turnover-group"
      parameter-4-13 =
        (
          if (" buf_c-turnover-group.tog-id = par-tog-id and buf_c-turnover-group.tog-db-num = par-tog-db-num " + " " + where-phrase-13) <> ""
          then  substitute ( ' buf_c-turnover-group.tog-id = &1 and buf_c-turnover-group.tog-db-num = &2 ', par-tog-id , par-tog-db-num )  + " " + where-phrase-13
...
CASE entry(ii, v-chg-fields):
 when "db-num-chg":U then do:     create temp-changes.       assign         temp-changes.f_name = "db-num-chg":U         temp-changes.l_name = "БД изм."         temp-changes.v_new  = string(buf_c-turnover-group.db-num-chg)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.db-num-chg)                                  else "Создание шапки" )         .   end.
when "name":U then do:     create temp-changes.       assign         temp-changes.f_name = "name":U         temp-changes.l_name = "Наименование"         temp-changes.v_new  = string(buf_c-turnover-group.name)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.name)                                  else "Создание шапки" )         .   end.
 when "tog-db-num":U then do:     create temp-changes.       assign         temp-changes.f_name = "tog-db-num":U         temp-changes.l_name = "ДБ "         temp-changes.v_new  = string(buf_c-turnover-group.tog-db-num)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.tog-db-num)                                  else "Создание шапки" )         .   end.
 when "tog-id":U then do:     create temp-changes.       assign         temp-changes.f_name = "tog-id":U         temp-changes.l_name = "№ группы"         temp-changes.v_new  = string(buf_c-turnover-group.tog-id)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.tog-id)                                  else "Создание шапки" )         .   end.
 when "stts":U then do:     create temp-changes.       assign         temp-changes.f_name = "stts":U         temp-changes.l_name = "Статус"         temp-changes.v_new  = string(buf_c-turnover-group.stts)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.stts)                                  else "Создание шапки" )         .   end.
 when "sys-date":U then do:     create temp-changes.       assign         temp-changes.f_name = "sys-date":U         temp-changes.l_name = "Дата"         temp-changes.v_new  = string(buf_c-turnover-group.sys-date)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.sys-date)                                  else "Создание шапки" )         .   end.
 when "sys-time-chr":U then do:     create temp-changes.       assign         temp-changes.f_name = "sys-time-chr":U         temp-changes.l_name = "Время"         temp-changes.v_new  = string(buf_c-turnover-group.sys-time-chr)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.sys-time-chr)                                  else "Создание шапки" )         .   end.
 when "sys-time":U then do:     create temp-changes.       assign         temp-changes.f_name = "sys-time":U         temp-changes.l_name = "Время (int)"         temp-changes.v_new  = string(buf_c-turnover-group.sys-time)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.sys-time)                                  else "Создание шапки" )         .   end.
 when "who":U then do:     create temp-changes.       assign         temp-changes.f_name = "who":U         temp-changes.l_name = "Кто"         temp-changes.v_new  = string(buf_c-turnover-group.who)         temp-changes.v_old  = (if available old_c-turnover-group                                  then string(old_c-turnover-group.who)                                  else "Создание шапки" )         .   end.
END CASE.
end.
end.
...
for each ub.c-tnv-in-turnover-group where
  ...
  END.
   do ii = 1 to num-entries(v-chg-fields):
  CASE entry(ii, v-chg-fields):
when "ttg-summa":U then do:     create temp-changes.       assign         temp-changes.f_name ="oborot"  + string(ub.c-tnv-in-turnover-group.ttg-summa ) + "ttg-summa":U         temp-changes.l_name ="Изм.Оборот-"   + string(ub.c-tnv-in-turnover-group.ttg-summa ) + " " + "Оборот "         temp-changes.v_new = string(ub.c-tnv-in-turnover-group.ttg-summa)         temp-changes.v_old =  (if available old_c-tnv-in-turnover-group                                  then string(old_c-tnv-in-turnover-group.ttg-summa)                                  else "" )       .   end.
when "tog-db-num":U then do:     create temp-changes.       assign         temp-changes.f_name ="oborot"  + string(ub.c-tnv-in-turnover-group.ttg-summa ) + "tog-db-num":U         temp-changes.l_name ="Изм.Оборот-"   + string(ub.c-tnv-in-turnover-group.ttg-summa ) + " " + "БД ТПЛ"         temp-changes.v_new = string(ub.c-tnv-in-turnover-group.tog-db-num)         temp-changes.v_old =  (if available old_c-tnv-in-turnover-group                                  then string(old_c-tnv-in-turnover-group.tog-db-num)                                  else "" )       .   end.
when "tog-id":U then do:     create temp-changes.       assign         temp-changes.f_name ="oborot"  + string(ub.c-tnv-in-turnover-group.ttg-summa ) + "tog-id":U         temp-changes.l_name ="Изм.Оборот-"   + string(ub.c-tnv-in-turnover-group.ttg-summa ) + " " + "№ ТПЛ"         temp-changes.v_new = string(ub.c-tnv-in-turnover-group.tog-id)         temp-changes.v_old =  (if available old_c-tnv-in-turnover-group                                  then string(old_c-tnv-in-turnover-group.tog-id)                                  else "" )       .   end.
when "stts":U then do:     if ub.c-tnv-in-turnover-group.stts = 1 then do:     create temp-changes.       assign         temp-changes.f_name ="oborot"   + string(ub.c-tnv-in-turnover-group.ttg-summa ) + "stts":U         temp-changes.l_name ="Изм.Оборот-"   + string(ub.c-tnv-in-turnover-group.ttg-summa ) + " " + "Статус строки"         temp-changes.v_new = "удален"         temp-changes.v_old =  (if available old_c-tnv-in-turnover-group                                  then "текущий"                                 else "" )       .      end.     if ub.c-tnv-in-turnover-group.stts = 0 then do:     create temp-changes.       assign         temp-changes.f_name ="oborot"   + string(ub.c-tnv-in-turnover-group.ttg-summa ) + "stts":U         temp-changes.l_name ="Изм.Оборот-"   + string(ub.c-tnv-in-turnover-group.ttg-summa ) + " " + "Статус строки"         temp-changes.v_new = "текущий"         temp-changes.v_old =  (if available old_c-tnv-in-turnover-group                                  then "удален"                                 else "" )       .      end.   end.
            end case.
        end.
end.
Open QUery br-changes for each temp-changes.
