...
define buffer bf_lvl-name for ub.lvl-name.
  ...
  if can-find(first bf_gds-prt no-lock where
      ...
      put stream PrnLibStream unformatted
      fill(chr(32), p-lvl-num * 2)
      "Уровень" chr(32) p-lvl-num chr(58) chr(32)
      ...
      for each bf_gds-prt no-lock where
        ...
        if v-current-pos + 14 >  136 then do:
          assign
          v-current-pos = p-lvl-num * 2
          .
          put stream PrnLibStream unformatted
        ...
        put stream PrnLibStream unformatted
        string(chr(124) + substr(bf_gds-prt.node-name, 1, 10), "X(11)") chr(32) chr(32) chr(32)
        .
        assign
