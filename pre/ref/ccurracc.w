...
assign
  v-str-recid10 = trim( string( recid( X_c-curr-accnt ) , "->>>>>>>>>>>9":U ) )
...
do:
  v-doc-rec = recid(X_c-curr-accnt). run OpenBr in this-procedure. reposition br-ccur to recid v-doc-rec no-error. v-doc-rec = ?.              apply 'value-changed' to br-ccur.
...
on start-search, ctrl-o of br-ccur in frame Dialog-Frame do:
   run sort-brbr-ccur
     (input (if available X_c-curr-accnt
             then recid(X_c-curr-accnt)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-ccur :
  ...
  case sort-labelbr-ccur:
        when X_c-curr-accnt.exch-date:label in browse br-ccur then DO:   assign     sort-column-name = "X_c-curr-accnt.exch-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ccur FOR EACH X_c-curr-accnt NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
