...
:
  find first src_stop-list share-lock where
        ...
        and src_stop-list.classif-type = 'dis-card':U no-error.
  ...
  find first trg_stop-list exclusive-lock where
              trg_stop-list.classif-type = 'dis-card':U
  ...
  for each buf2_stop-list-line no-lock where
      ...
      and buf2_stop-list-line.classif-type = 'dis-card':U
  on error  undo main-block, return error substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1))
  on stop   undo main-block, return error substitute( "&1. stop", vss-workfile )
  on endkey undo main-block, return error substitute( "&1. endkey", vss-workfile )
  :
  ...
  on endkey undo main-block, return error substitute( "&1. endkey", vss-workfile )
  :
    if buf2_stop-list-line.key#_one = integer('2':U)
    or buf2_stop-list-line.key#_one = integer('3':U) then do:
       ...
       for each buf2_dis-card no-lock where
        ...
        :
         ...
         find first buf3_stop-list-line no-lock where
                ...
                and buf3_stop-list-line.classif-type = 'dis-card':U
