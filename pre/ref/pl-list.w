...
do:
  ...
  END.
ON value-changed OF rs-stat in frame d-pl-list
  DO:
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    do on stop undo _deletion, return no-apply:
      ...
      if  buf_place.status_ <> 'удал':U then
      do:
        ...
        do:
          ...
          do:
            buf_place.status_ = "".
            delete buf_place.
            br-pl:delete-current-row().
          end.
          else
          do:
            buf_place.status_ = 'удал':U.
...
ON CHOOSE OF B-hist IN FRAME d-pl-list
  ...
  END.
ON CHOOSE OF b-level IN FRAME d-pl-list
  DO:
    define variable v-recid as recid no-undo .
...
assign
  v-str-recid12 = trim( string( recid( X_place ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry12 > 0 then do:
  ...
  .
      ...
      if num-entries( v-rid-list ) = 0 then
        hide mark-num in frame d-pl-list.
      else
        disp num-entries( v-rid-list ) @ mark-num with frame d-pl-list.
    end.
    apply "entry" to br-pl in frame d-pl-list.
  END.
...
ON CHOOSE OF b-print IN FRAME d-pl-list
  ...
  END.
ON CHOOSE OF b-rest IN FRAME d-pl-list
  DO:
    if not avail X_place then return no-apply.
    define variable loc-rid-list as character no-undo .
    run ref/pl-gdss.w (
...
ON CHOOSE OF b-sel IN FRAME d-pl-list
  ...
  END.
ON CHOOSE OF b-quit IN FRAME d-pl-list
  DO:
      p-rid-list = "cancel" .
  END.
...
ON VALUE-CHANGED OF br-pl IN FRAME d-pl-list
  DO:
    ...
    do:
      display
        X_place.PS with frame d-pl-list.
    end.
  END.
ON ENTRY OF X_place.PS IN FRAME d-pl-list
  DO:
    v-doc-rec = recid (X_place).
  END.
ON LEAVE OF X_place.PS IN FRAME d-pl-list
  DO:
    define buffer buf_place for ub.place.
    find buf_place where recid (buf_place) = recid(X_place) no-error.
    if available buf_place and input frame d-pl-list X_place.PS <> buf_place.PS then
    do:
      assign X_place.PS.
    end.
  END.
...
do:
  v-doc-rec = recid(X_place). run openbr in this-procedure (  input yes, input no, input '':U). reposition br-pl to recid(v-doc-rec). v-doc-rec = ? .
...
PROCEDURE sort-brbr-pl :
  ...
  case sort-labelbr-pl:
        when X_place.pl-code:label in browse br-pl then DO:    assign       sort-column-name = "X_place.pl-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_place.pl-name:label in browse br-pl then DO:    assign       sort-column-name = "X_place.pl-name"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_place.loc1:label in browse br-pl then DO:    assign       sort-column-name = "X_place.loc1"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_place.loc2:label in browse br-pl then DO:    assign       sort-column-name = "X_place.loc2"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_place.loc3:label in browse br-pl then DO:    assign       sort-column-name = "X_place.loc3"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_place.loc4:label in browse br-pl then DO:    assign       sort-column-name = "X_place.loc4"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
ON WINDOW-CLOSE OF FRAME d-pl-list
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  then do :
    assign
      v-is-only-np = (entry(2, p-mode, chr(4)) = "only-np")
      v-is-np-list = (entry(2, p-mode, chr(4)) = "np-list")
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  if error-status :error or par-type  <> 'L':U or v_is-ptrl <> 'yes':U then
  do:
    assign
      v_is-ptrl = 'no':U.
  end.
  ...
  display rs-stat with frame d-pl-list.
  enable rs-stat with frame d-pl-list.
  RUN enable_UI in this-procedure .
  RUN OpenBR in this-procedure (  input yes, input no, input '':U).
define variable vss-include-info30 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
DEF VAR cur-clmn-numbr-pl as INT EXTENT 12 no-undo.
...
def var old-session as logical no-undo.
   ...
   DO jjbr-pl = NUM-ENTRIES('1,2,3,4,5,6,7,10,11,12,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-pl ( cur-clmn-numbr-pl[INTEGER(ENTRY (jjbr-pl, '1,2,3,4,5,6,7,10,11,12,8,9'))] , 1 ).
   END.
       END.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-pl = NUM-ENTRIES('1,8,9,2,3,4,5,6,7,10,11,12') TO 1 BY -1:
     RUN re-move-clmnbr-pl ( cur-clmn-numbr-pl[INTEGER(ENTRY (jjbr-pl, '1,8,9,2,3,4,5,6,7,10,11,12'))] , 1 ).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-pl do:
  RUN re-move-clmnbr-pl ( 1 , 12).
END.
ON ctrl-cursor-left OF BROWSE br-pl do:
  RUN re-move-clmnbr-pl (12, 1 ).
END.
...
PROCEDURE ch-clmnbr-pl:
  ...
  END.
  RUN re-move-clmnbr-pl (cur-clmn-loc, 1 ).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-pl:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-pl = 1  to EXTENT(cur-clmn-numbr-pl):
...
PROCEDURE enable_UI :
  assign
    X_place.pl-name:read-only in browse br-pl = true.
  ENABLE
    b-quit
    b-rest
...
PROCEDURE OpenBr :
  ...
  define buffer buf_place for ub.place.
  assign FRAME d-pl-list   rs-stat.
  run waitfram-show in this-procedure ( input "Ждите...").
  define variable sort-column-phrase as character no-undo .
  for each tt-place-attr :
    delete tt-place-attr.
  end.
  if p-mode = 'объект':U  then
  do:
    for each buf_place where buf_place.obj-code = p-obj-code and buf_place.obj-type = p-obj-type and (if rs-stat = 'все':U then true else  buf_place.status_ = (if rs-stat =  'текущие':U then "" else 'удал':U)) no-lock :
  ...
  do:
    for each buf_place where  (if rs-stat = 'все':U then true else  buf_place.status_ = (if rs-stat =  'текущие':U then "" else 'удал':U)) no-lock :
      find first tt-place-attr exclusive-lock where tt-place-attr.pl-code = buf_place.pl-code no-error.
      if not available tt-place-attr then
      do :
        create tt-place-attr.
  ...
  CASE p-mode:
    when 'объект':U then
      do:
        ...
        ASSIGN
          frame d-pl-list:TITLE = "Складские места " + clients.obj-name
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-32 as character no-undo .
      ...
      parameter-4-32 =
        (
          if (" X_place.obj-type = p-obj-type AND X_place.obj-code = p-obj-code " + " " + where-phrase-32) <> ""
          then  substitute('X_place.obj-type = &1&2&1 AND X_place.obj-code = &3 ', chr(34), p-obj-type, p-obj-code) + " " + where-phrase-32
          else "true"
        )
      parameter-5-32 = (" " + "" + " " + substitute(', first tt-place-attr no-lock where tt-place-attr.pl-code = X_place.pl-code' ))
    ...
    :
      assign
        l-disable-where-32 =
          (" X_place.obj-type = p-obj-type AND X_place.obj-code = p-obj-code " + " " + where-phrase-32 = "")
      .
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-pl FOR EACH X_place
      where  X_place.obj-type = p-obj-type AND X_place.obj-code = p-obj-code
    , first tt-place-attr no-lock where tt-place-attr.pl-code = X_place.pl-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when 'все':U then
      do:
        ASSIGN
          frame d-pl-list:TITLE = "Складские места "
          filter-point              = filter-point0 + p-mode
          filter-label              = substitute("&1", filter-label0)
          .
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-5-34 = (" " + "" + " " + substitute(', first tt-place-attr no-lock where tt-place-attr.pl-code = X_place.pl-code' ))
