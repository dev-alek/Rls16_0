...
assign
  v-str-recid11 = trim( string( recid( X_c-fin-doc-attr ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry11 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-attrs in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-attrs in frame Dialog-Frame.
END.
...
do:
  v-doc-rec = recid(X_fin-doc-attr). run openbr in this-procedure. reposition br-attrs to recid v-doc-rec no-error.
    apply "VALUE-CHANGED" to br-attrs.
end.
def var sort-labelbr-attrs   as character no-undo .
def var sort-clmnbr-attrs    as handle    no-undo .
def var cur-clmnbr-attrs     as handle    no-undo .
def var cur-clmn-locbr-attrs as integer   no-undo .
def var re-querybr-attrs     as logical   initial no no-undo .
on start-search, ctrl-o of br-attrs in frame Dialog-Frame do:
   run sort-brbr-attrs
     (input (if available X_c-fin-doc-attr
             then recid(X_c-fin-doc-attr)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-attrs :
  define input parameter p-recid as recid no-undo .
  if re-querybr-attrs = no then do:
    assign
       cur-clmnbr-attrs = br-attrs:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-attrs <> ? then sort-clmnbr-attrs:column-fgcolor = 0.
    if cur-clmnbr-attrs = sort-clmnbr-attrs then do:
      assign
         sort-labelbr-attrs = ""
         sort-clmnbr-attrs = ?
      .
       ...
       assign
         sort-labelbr-attrs = cur-clmnbr-attrs:label
         sort-clmnbr-attrs  = cur-clmnbr-attrs
         sort-clmnbr-attrs:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-attrs = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-attrs:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-attrs then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-attrs = cur-clmn-locbr-attrs + 1
    .
  end.
  case sort-labelbr-attrs:
        when X_c-fin-doc-attr.corr-date:label in browse br-attrs then DO:   assign     sort-column-name = "X_c-fin-doc-attr.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-attrs') then do:
          run mv-brw-defaultbr-attrs.
        end.
      if sort-labelbr-attrs <> "" then do:
        assign
          cur-clmnbr-attrs:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-attrs = ?
      .
    end.
  end case.
    if cur-clmn-locbr-attrs <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-attrs') then do:
        run ch-clmnbr-attrs in this-procedure (cur-clmn-locbr-attrs).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-attrs to recid p-recid no-error.
    apply "value-changed" to br-attrs in frame Dialog-Frame.
  end.
  apply "entry" to br-attrs in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-attrs:
if cur-clmnbr-attrs = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-attrs = yes.
   run sort-brbr-attrs
     (input (if available X_c-fin-doc-attr
             then recid(X_c-fin-doc-attr)
             else ?
   ...
   assign re-querybr-attrs = no.
end.
end.
define variable vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-attrs :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
DEF VAR cur-clmn-numbr-attrs as INT EXTENT 4 no-undo.
DEF VAR varmvibr-attrs       as INT no-undo.
DEF VAR varmvjbr-attrs       as INT no-undo.
DEF VAR varmvkbr-attrs       as INT no-undo.
DEF VAR varmvlbr-attrs       as INT no-undo.
DEF VAR move-elementbr-attrs as INT no-undo.
def var jjbr-attrs           as int no-undo.
do varmvibr-attrs = 1 to EXTENT(cur-clmn-numbr-attrs):
  ASSIGN cur-clmn-numbr-attrs[varmvibr-attrs] = varmvibr-attrs.
END.
RUN start-mv-clmnbr-attrs.
PROCEDURE start-mv-clmnbr-attrs:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-attrs = NUM-ENTRIES('1,2,3,4') TO 1 BY -1:
     RUN re-move-clmnbr-attrs ( cur-clmn-numbr-attrs[INTEGER(ENTRY (jjbr-attrs, '1,2,3,4'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-attrs do:
  RUN re-move-clmnbr-attrs ( 1, 4).
END.
ON ctrl-cursor-left OF BROWSE br-attrs do:
  RUN re-move-clmnbr-attrs (4, 1).
END.
PROCEDURE re-move-clmnbr-attrs:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-attrs = 1 TO EXTENT(cur-clmn-numbr-attrs):
    if cur-clmn-numbr-attrs[varmvibr-attrs] = source-column THEN cur-clmn-numbr-attrs[varmvibr-attrs] = -1.
  END.
  if br-attrs:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-attrs = source-column - 1 to target-column BY -1:
    DO varmvibr-attrs = 1 TO EXTENT(cur-clmn-numbr-attrs):
        if cur-clmn-numbr-attrs[varmvibr-attrs] = varmvjbr-attrs THEN DO:
          cur-clmn-numbr-attrs[varmvibr-attrs] = cur-clmn-numbr-attrs[varmvibr-attrs] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-attrs = source-column + 1 to target-column:
    DO varmvibr-attrs = 1 TO EXTENT(cur-clmn-numbr-attrs):
      if cur-clmn-numbr-attrs[varmvibr-attrs] = varmvjbr-attrs THEN DO:
        cur-clmn-numbr-attrs[varmvibr-attrs] = cur-clmn-numbr-attrs[varmvibr-attrs] - 1.
      END.
  ...
  END.
  DO varmvibr-attrs = 1 TO EXTENT(cur-clmn-numbr-attrs):
    if cur-clmn-numbr-attrs[varmvibr-attrs] = -1 THEN cur-clmn-numbr-attrs[varmvibr-attrs] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-attrs:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-attrs = 1 TO EXTENT(cur-clmn-numbr-attrs):
    if cur-clmn-numbr-attrs[varmvibr-attrs] = cur-clmn-loc THEN move-elementbr-attrs = varmvibr-attrs.
  END.
  RUN re-move-clmnbr-attrs (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-attrs:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-attrs = 1 to EXTENT(cur-clmn-numbr-attrs):
    RUN re-move-clmnbr-attrs (cur-clmn-numbr-attrs[varmvlbr-attrs], varmvlbr-attrs).
  END.
  RUN start-mv-clmnbr-attrs.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-attrs FOR EACH X_c-fin-doc-attr NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
...
if not available X_c-fin-doc-attr then do:
  ...
  assign
  v-description = "Атрибут" + chr(32) + v-label
  .
