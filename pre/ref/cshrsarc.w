...
END.
ON CHOOSE OF MENU-ITEM m_cshr-del
DO:
  assign
...
END.
ON CHOOSE OF MENU-ITEM m_cshr-sum
DO:
    assign
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-cshr-sum :handle
      .
...
do:
  BR-cshr-del :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    FIND FIRST ub.person WHERE ub.person.psn-code = p-psn-code NO-LOCK .
    FIND ub.clients WHERE ub.clients.obj-type = 'чел':U AND
                                       ub.clients.obj-code = ub.person.psn-code NO-LOCK .
    FRAME Dialog-Frame:title = substitute("Архивы по кассиру ( код &1 )   &2"
                                          , p-cashier-code
                                          , clients.obj-name).
    RUN MYenable in this-procedure .
    apply "entry" to BR-cshr-sum in FRAME Dialog-Frame.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-cshr-del FOR EACH del_cshr-month       WHERE del_cshr-month.cshr-code = p-cashier-code AND ( del_cshr-month.cashier-psn-code = p-psn-code or del_cshr-month.cashier-psn-code = 0)  AND ( del_cshr-month.out-count <> 0  OR del_cshr-month.ret-count <> 0 )  SHARE-LOCK INDEXED-REPOSITION.    OPEN QUERY BR-cshr-sum FOR EACH sum_cshr-month       WHERE sum_cshr-month.cshr-code = p-cashier-code and ( sum_cshr-month.cashier-psn-code = p-psn-code or sum_cshr-month.cashier-psn-code = 0) SHARE-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
for-all-totchk   COLUMN-LABEL "Всего!чеков" FORMAT ">>>>>>9"
sum_cshr-month.out-totchk COLUMN-LABEL "Чеков!продаж" FORMAT ">>>>>>9"
for-all-totsum  COLUMN-LABEL "Выручка" FORMAT "->>,>>>,>>>,>>9.99"
...
case p-print-option:
  when "cshr-del":U then do:
    ...
    SPACE(25) ( frame Dialog-Frame:title ) format "x(90)" skip
    ...
    apply "entry" to br-cshr-del in frame Dialog-Frame.
  end.
