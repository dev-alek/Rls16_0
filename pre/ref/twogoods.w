...
define variable v-doc-rec as recid no-undo .
  ...
  reposition  BR-gds to recid v-doc-rec no-error.
  end.
END.
ON CHOOSE OF B-reshparts IN FRAME Dialog-Frame
DO:
define variable v-doc-rec as recid no-undo .
    if avail X_gds-obj then do:
    v-doc-rec = recid(X_gds-obj).
...
do:
  v-doc-rec = recid(X_gds-obj). Run Openbr in this-procedure ( input yes, input no, input '':U). reposition br-gds to recid v-doc-rec no-error. v-doc-rec = ?.
...
DEF VAR cur-clmn-numbr-gds as INT EXTENT 9 no-undo.
...
def var old-session as logical no-undo.
   ...
   DO jjbr-gds = NUM-ENTRIES('1,2,3,4,5,6,7,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-gds ( cur-clmn-numbr-gds[INTEGER(ENTRY (jjbr-gds, '1,2,3,4,5,6,7,8,9'))] ,  + 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-gds do:
  RUN re-move-clmnbr-gds (  + 1, 9).
END.
ON ctrl-cursor-left OF BROWSE br-gds do:
  RUN re-move-clmnbr-gds (9,  + 1).
END.
...
CASE p-mode:
    when 'объект':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Товары с двумя ед.изм. на объекте " + loc-store-type + string(loc-store-code)
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-14 as character no-undo .
      assign
      parameter-3-14 =
                              "FOR EACH X_gds-obj NO-LOCK"
      parameter-4-14 =
        (
          if (" X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0 " + " " + where-phrase-14) <> ""
          then  substitute('X_gds-obj.obj-type = &1&2&1 AND X_gds-obj.obj-code = &3 and X_gds-obj.fact-qnty <> 0 '                                    , chr(34), loc-store-type, loc-store-code) + " " + where-phrase-14
          else "true"
        )
      parameter-5-14 = (" " + "" + " " + substitute(', FIRST X_goods NO-LOCK WHERE X_goods.gds-code = X_gds-obj.gds-code,              FIRST X_units WHERE X_units.unit-name = X_goods.unit-base AND LOOKUP(&1&2&1, X_units.type) > 0,             FIRST X_parts No-LOCK WHERE X_parts.artic = X_gds-obj.artic AND X_parts.prod-type = X_gds-obj.prod-type AND             X_parts.prod-code = X_gds-obj.prod-code AND X_parts.obj-type = X_gds-obj.obj-type AND             X_parts.obj-code = X_gds-obj.obj-code AND X_parts.out-code = &1&3&1  ', chr(34), '2ед':U, 'free-zone':U))
    ...
    :
      assign
        l-disable-where-14 =
          (" X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0 " + " " + where-phrase-14 = "")
      .
  ...
  if l-filter-open-14 = false then do:
    OPEN QUERY br-gds FOR EACH X_gds-obj NO-LOCK
      where  X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0
    , FIRST X_goods NO-LOCK WHERE X_goods.gds-code = X_gds-obj.gds-code,              FIRST X_units WHERE X_units.unit-name = X_goods.unit-base AND LOOKUP('2ед':U, X_units.type) > 0,             FIRST X_parts No-LOCK WHERE X_parts.artic = X_gds-obj.artic AND X_parts.prod-type = X_gds-obj.prod-type AND             X_parts.prod-code = X_gds-obj.prod-code AND X_parts.obj-type = X_gds-obj.obj-type AND             X_parts.obj-code = X_gds-obj.obj-code AND X_parts.out-code = 'free-zone':U
  .
      assign
        l-query-was-opened = true
      .
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-16 as character no-undo .
      ...
      parameter-4-16 =
        (
          if (" X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0 " + " " + where-phrase-16) <> ""
          then  substitute('X_gds-obj.obj-type = &1&2&1 AND X_gds-obj.obj-code = &3 and X_gds-obj.fact-qnty <> 0 '                                   , chr(34), loc-store-type, loc-store-code) + " " + where-phrase-16
          else "true"
        )
      parameter-5-16 = (" " + "" + " " + substitute(', FIRST X_goods NO-LOCK WHERE X_goods.gds-code = X_gds-obj.gds-code,              FIRST X_units WHERE X_units.unit-name = X_goods.unit-base AND LOOKUP(&1&2&1, X_units.type) > 0,             FIRST X_parts No-LOCK WHERE X_parts.artic = X_gds-obj.artic AND X_parts.prod-type = X_gds-obj.prod-type AND             X_parts.prod-code = X_gds-obj.prod-code AND X_parts.obj-type = X_gds-obj.obj-type AND             X_parts.obj-code = X_gds-obj.obj-code AND X_parts.out-code = &1&3&1 AND X_parts.cli-qnty > 1', chr(34), '2ед':U, 'free-zone':U))
    ...
    :
      assign
        l-disable-where-16 =
          (" X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0 " + " " + where-phrase-16 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-gds:handle
  ...
  if l-filter-open-16 = false then do:
    OPEN QUERY br-gds FOR EACH X_gds-obj NO-LOCK
      where  X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0
    , FIRST X_goods NO-LOCK WHERE X_goods.gds-code = X_gds-obj.gds-code,              FIRST X_units WHERE X_units.unit-name = X_goods.unit-base AND LOOKUP('2ед':U, X_units.type) > 0,             FIRST X_parts No-LOCK WHERE X_parts.artic = X_gds-obj.artic AND X_parts.prod-type = X_gds-obj.prod-type AND             X_parts.prod-code = X_gds-obj.prod-code AND X_parts.obj-type = X_gds-obj.obj-type AND             X_parts.obj-code = X_gds-obj.obj-code AND X_parts.out-code = 'free-zone':U AND X_parts.cli-qnty > 1
  .
      assign
        l-query-was-opened = true
      .
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-18 as character no-undo .
      ...
      parameter-4-18 =
        (
          if (" X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0 " + " " + where-phrase-18) <> ""
          then  substitute('X_gds-obj.obj-type = &1&2&1 AND X_gds-obj.obj-code = &3 and X_gds-obj.fact-qnty <> 0 '                                 , chr(34), loc-store-type, loc-store-code) + " " + where-phrase-18
          else "true"
        )
      parameter-5-18 = (" " + "" + " " + substitute(', FIRST X_goods NO-LOCK WHERE X_goods.gds-code = X_gds-obj.gds-code,              FIRST X_units WHERE X_units.unit-name = X_goods.unit-base AND LOOKUP(&1&2&1, X_units.type) > 0,             FIRST X_parts No-LOCK WHERE X_parts.artic = X_gds-obj.artic AND X_parts.prod-type = X_gds-obj.prod-type AND             X_parts.prod-code = X_gds-obj.prod-code AND X_parts.obj-type = X_gds-obj.obj-type AND             X_parts.obj-code = X_gds-obj.obj-code AND X_parts.out-code = &1&3&1 AND X_parts.cli-qnty = 1', chr(34), '2ед':U, 'free-zone':U))
    ...
    :
      assign
        l-disable-where-18 =
          (" X_gds-obj.obj-type = loc-store-type AND X_gds-obj.obj-code = loc-store-code and X_gds-obj.fact-qnty <> 0 " + " " + where-phrase-18 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-gds:handle
...
DEFINE BUFFER LOC-PARTS FOR ub.parts.
  find FIRST loc-parts No-LOCK WHERE
             ...
             loc-parts.out-code = 'free-zone':U and
             LOC-PARTS.CLI-QNTY > 1 no-error.
  RETURN avail(loc-parts).
END FUNCTION.
FUNCTION is-stuck RETURNS LOGICAL
  ( buffer loc-gds-obj for X_gds-obj ) :
define buffer loc-parts for ub.parts.
