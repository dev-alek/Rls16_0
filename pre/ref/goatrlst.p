...
DEFINE VARIABLE var-object as character no-undo init 'gds-obj-attr':U.
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'goatrlst.txt').
end.
  return .
end.
for each obj-list
    ...
    for each temp-attr no-lock
      ...
      if v-check <> "":U then do:
        ...
        or not v-correct  then do:
          assign v-mes = substitute("товар с кодом &1, &2&3: ошибка при проверке корректности задаваемого значения атрибута товара на объекте &4&5:&6 &7&5"  +                    "Обратитесь к администратору системы"                     , gds-list.gds-code                    , parobj-type                            , parobj-code                            , temp-attr.attr-value                    , chr(10)                        , error-status:get-message(1)                    , return-value ).
      ...
      CASE temp-attr.action:
        when yes then do:
          run gdsoattr-write in this-procedure(
           ...
           if error-status:error  then do:
             if entry(1, return-value , chr(4) ) = 'lock':U then do:
               undo _main, return error entry(2, return-value, chr(4)).
             end.
             else do:
              assign v-mes = substitute("товар с кодом &1, &2&3: ошибка при записи атрибута товара на объекте &4&5:&6 &7&5"  +                    "Обратитесь к администратору системы"                     , gds-list.gds-code                    , parobj-type                            , parobj-code                            , temp-attr.attr-value                    , chr(10)                        , error-status:get-message(1)                    , return-value ).
              undo _main, return error v-mes.
             end.
           end.
        end.
          ...
          run gdsoattr-delete in this-procedure(
           ...
           if error-status:error  then do:
             if entry(1, return-value , chr(4) ) = 'lock':U then do:
              ...
              assign v-mes = substitute("товар с кодом &1, &2&3: ошибка при удалении атрибута товара на объекте &4&5:&6 &7&5"  +                    "Обратитесь к администратору системы"                     , gds-list.gds-code                    , parobj-type                            , parobj-code                            , temp-attr.attr-value                    , chr(10)                        , error-status:get-message(1)                    , return-value ).
              undo _main, return error v-mes.
             end.
           end.
         end.
