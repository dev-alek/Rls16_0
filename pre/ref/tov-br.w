...
return entry (lookup (string(loc-table.type), '0,1,50,99':U), 'тек,удал,блок,удаление':U) .
end function.
...
buf_turnover-buyer-main.sum-doc-rubl-itog   COLUMN-LABEL "В продаж.ценах (РУБ)" FORMAT "->>>,>>>,>>>,>>9.99"
buf_turnover-buyer-main.sum-acc-base-itog   COLUMN-LABEL "В учетных ценах(баз.в.)"  FORMAT "->>>,>>>,>>>,>>9.99"
buf_turnover-buyer-main.sum-acc-rubl-itog   COLUMN-LABEL "В учетных ценах(РУБ)" FORMAT "->>>,>>>,>>>,>>9.99"
...
DEFINE BROWSE BROWSE-1ob
  QUERY BROWSE-1ob NO-LOCK DISPLAY
      ...
      buf_turnover-buyer.sum-doc-rubl  COLUMN-LABEL "В прод.ценах(РУБ)"  FORMAT "->>>,>>>,>>>,>>9.99"
      buf_turnover-buyer.sum-acc-base  COLUMN-LABEL "В учетных ценах(баз.в.)"  FORMAT "->>>,>>>,>>>,>>9.99"
      buf_turnover-buyer.sum-acc-rubl  COLUMN-LABEL "В учетных ценах(РУБ)"  FORMAT "->>>,>>>,>>>,>>9.99"
...
buf_turnover-buyer.sum-doc-rubl-itog   COLUMN-LABEL "В прод.ценах(РУБ)!нараст.итог" FORMAT "->>>,>>>,>>>,>>9.99"
buf_turnover-buyer.sum-acc-base-itog   COLUMN-LABEL "В учетных ценах(баз.в.)!нараст.итог"  FORMAT "->>>,>>>,>>>,>>9.99"
buf_turnover-buyer.sum-acc-rubl-itog   COLUMN-LABEL "В учетных ценах(РУБ)!нараст.итог"  FORMAT "->>>,>>>,>>>,>>9.99"
...
DEFINE BROWSE BROWSE-2ob-gds
  QUERY BROWSE-2ob-gds NO-LOCK DISPLAY
      ...
      buf_turnover-buyer-gds.sum-doc-rubl  COLUMN-LABEL "В прод.ценах(РУБ)" FORMAT "->>>,>>>,>>9.99"
      buf_turnover-buyer-gds.sum-acc-base  COLUMN-LABEL "В учетных ценах(баз.в.)" FORMAT "->>>,>>>,>>9.99"
      buf_turnover-buyer-gds.sum-acc-rubl  COLUMN-LABEL "В учетных ценах(РУБ)" FORMAT "->>>,>>>,>>9.99"
...
if available buf_turnover-buyer-main then do:
  ...
  run ref/tov-bri.w ( Parparentproc, 'ДОБАВЛЕНИЕ':U , root_clients.obj-type ,  root_clients.obj-code ,  input-output v-rec-id ) .
  ...
  run refresh-itogo.
  OPEN QUERY BROWSE-0ob FOR EACH buf_turnover-buyer-main WHERE     ( r-obj-choose = 2 OR       lookup((buf_turnover-buyer-main.obj-type + string(buf_turnover-buyer-main.obj-code)) ,        v-obj-list ) > 0 ) AND     buf_turnover-buyer-main.cli-type = root_clients.obj-type AND     buf_turnover-buyer-main.cli-code = root_clients.obj-code NO-LOCK ,            EACH buf_clients WHERE buf_clients.obj-code = buf_turnover-buyer-main.obj-code                        AND buf_clients.obj-type = buf_turnover-buyer-main.obj-type                                                             NO-LOCK .
...
if available buf_turnover-buyer-main then do:
  ...
  run ref/tov-bri.w ( Parparentproc, 'ИЗМЕНЕНИЕ':U , root_clients.obj-type ,  root_clients.obj-code ,  input-output v-rec-id ) .
  define buffer bbuf_turnover-buyer for ub.turnover-buyer  .
  find first bbuf_turnover-buyer no-lock where recid(bbuf_turnover-buyer) =  v-rec-id no-error .
  if available  bbuf_turnover-buyer then do:
     run ref/calctur1.p ( bbuf_turnover-buyer.cli-type , bbuf_turnover-buyer.cli-code ,
...
if not available buf_turnover-buyer then return.
v-rec-id = recid(buf_turnover-buyer) .
run ref/tov-bri.w (Parparentproc, 'ПРОСМОТР':U , root_clients.obj-type ,  root_clients.obj-code , input-output v-rec-id )  .
reposition BROWSE-1ob to recid v-rec-id no-error .
END.
...
assign
  v-str-recid6 = trim( string( recid( buf_turnover-buyer-main ) , "->>>>>>>>>>>9":U ) )
  v-num-entry6 = lookup( v-str-recid6 , p-rec-list )
.
if v-num-entry6 > 0 then do:
  assign
    entry( v-num-entry6, p-rec-list ) = "":U
    p-rec-list = trim( replace( p-rec-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    p-rec-list = p-rec-list + ( if p-rec-list = "":U then "":U else chr(44) ) + v-str-recid6
  .
end.
        g-log = browse-0ob:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
          g-log = browse-0ob:select-next-row ().
          apply "VALUE-CHANGED" to browse-0ob in frame Dialog-Frame.
      end.
    end.
    apply "display" to browse-0ob in frame Dialog-Frame.
END.
...
DO:
  ...
  OPEN QUERY BROWSE-2ob-gds FOR EACH buf_turnover-buyer-gds      WHERE ( r-type-add = 2 OR buf_turnover-buyer-gds.type =  r-type-add ) AND     buf_turnover-buyer-gds.obj-type = buf_turnover-buyer-main.obj-type AND     buf_turnover-buyer-gds.obj-code = buf_turnover-buyer-main.obj-code AND     buf_turnover-buyer-gds.cli-type = buf_turnover-buyer-main.cli-type AND     buf_turnover-buyer-gds.cli-code = buf_turnover-buyer-main.cli-code     NO-LOCK     ,            FIRST buf_goods NO-LOCK WHERE buf_goods.gds-code = buf_turnover-buyer-gds.gds-code                 .
END.
ON VALUE-CHANGED OF BROWSE-1ob IN FRAME Dialog-Frame
DO:
  OPEN QUERY BROWSE-2ob-gds FOR EACH buf_turnover-buyer-gds      WHERE ( r-type-add = 2 OR buf_turnover-buyer-gds.type =  r-type-add ) AND     buf_turnover-buyer-gds.obj-type = buf_turnover-buyer-main.obj-type AND     buf_turnover-buyer-gds.obj-code = buf_turnover-buyer-main.obj-code AND     buf_turnover-buyer-gds.cli-type = buf_turnover-buyer-main.cli-type AND     buf_turnover-buyer-gds.cli-code = buf_turnover-buyer-main.cli-code     NO-LOCK     ,            FIRST buf_goods NO-LOCK WHERE buf_goods.gds-code = buf_turnover-buyer-gds.gds-code                 .
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BROWSE-0ob :handle
      .
...
IF NOT root_clients.turnover-buyer-gds THEN do:
    HIDE BROWSE-2ob-gds IN FRAME Dialog-Frame.
    BROWSE-1ob:WIDTH IN FRAME Dialog-Frame = 98 .
END.
  disable
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-0ob FOR EACH buf_turnover-buyer-main WHERE     ( r-obj-choose = 2 OR       lookup((buf_turnover-buyer-main.obj-type + string(buf_turnover-buyer-main.obj-code)) ,        v-obj-list ) > 0 ) AND     buf_turnover-buyer-main.cli-type = root_clients.obj-type AND     buf_turnover-buyer-main.cli-code = root_clients.obj-code NO-LOCK ,            EACH buf_clients WHERE buf_clients.obj-code = buf_turnover-buyer-main.obj-code                        AND buf_clients.obj-type = buf_turnover-buyer-main.obj-type                                                             NO-LOCK .    OPEN QUERY BROWSE-1ob FOR EACH buf_turnover-buyer NO-LOCK WHERE     ( r-type-add = 2 OR buf_turnover-buyer.type =  r-type-add ) AND buf_turnover-buyer.obj-type = buf_turnover-buyer-main.obj-type AND buf_turnover-buyer.obj-code = buf_turnover-buyer-main.obj-code AND buf_turnover-buyer.cli-type = buf_turnover-buyer-main.cli-type    AND buf_turnover-buyer.cli-code = buf_turnover-buyer-main.cli-code  USE-INDEX clients  .    OPEN QUERY BROWSE-2ob-gds FOR EACH buf_turnover-buyer-gds      WHERE ( r-type-add = 2 OR buf_turnover-buyer-gds.type =  r-type-add ) AND     buf_turnover-buyer-gds.obj-type = buf_turnover-buyer-main.obj-type AND     buf_turnover-buyer-gds.obj-code = buf_turnover-buyer-main.obj-code AND     buf_turnover-buyer-gds.cli-type = buf_turnover-buyer-main.cli-type AND     buf_turnover-buyer-gds.cli-code = buf_turnover-buyer-main.cli-code     NO-LOCK     ,            FIRST buf_goods NO-LOCK WHERE buf_goods.gds-code = buf_turnover-buyer-gds.gds-code                 .
END PROCEDURE.
...
ITOGO-sum-doc-rubl:label in frame Dialog-Frame  = "Итого в ценах реализации(РУБ)" .
display
  ITOGO-sum-doc-rubl
  ITOGO-sum-doc-base
  ITOGO-sum-rash-base
