...
define variable vss-description as character no-undo init "Редактирование правил скидок - категорийная скидка, задаваемая через ТПЛ".
...
DEFINE FRAME Dialog-Frame
     ...
     tt-dis-rule.dis-kat AT ROW 5.92 COL 8 COLON-ALIGNED WIDGET-ID 20
          LABEL "Катег." FORMAT ">>>9"
          VIEW-AS FILL-IN
          SIZE 6 BY 1 TOOLTIP "Для категорийных клиентов"
     tt-dis-rule.CharKey_One AT ROW 5.92 COL 36.5 COLON-ALIGNED WIDGET-ID 8
          LABEL "Поле1"
          VIEW-AS FILL-IN
          SIZE 12 BY 1
     ...
     B-time-rule-lookup AT ROW 7.92 COL 89 WIDGET-ID 22
...
PROCEDURE enable_UI :
  ...
  IF AVAILABLE tt-dis-rule THEN
    DISPLAY tt-dis-rule.des tt-dis-rule.value-type tt-dis-rule.dis-kat
  ...
  ENABLE B-exit b-quit B-hist B-Help tt-dis-rule.des tt-dis-rule.value-type
         f-pos-type tt-dis-rule.dis-kat tt-dis-rule.CharKey_One B-plt
         tt-dis-rule.time-rule-num B-dis-time-rule B-exit-1 B-quit-1 B-add
         B-del b-plt-lookup B-time-rule-lookup BR-term-dr tt-dis-rule.rule-num
         F-region
      WITH FRAME Dialog-Frame.
...
ELSE DO:
  ...
  tt-dis-rule.charkey_one
    .
  if tt-dis-rule.charkey_one = "":U then
  do:
    message "Не указано правило "
        view-as alert-box .
    return error .
  end .
  ASSIGN
  tt-dis-rule.discnt-value = 0
  tt-dis-rule.dis-kat = tt0-term_dis-rule.dis-kat
  .
END.
