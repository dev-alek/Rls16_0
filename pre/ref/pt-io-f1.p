...
if p-point-type <> 'доставки':U
and p-point-type <> 'отгрузки':U then do:
...
:
  if p-mode = 'ДОБАВЛЕНИЕ':U then do:
    ...
    buf_point-io.point-code = next-value( s-point-io, ub )
    buf_point-io.status_    = 'тек':U
    .
  end.
  else do:
...
if p-silent then do:
  ...
  p-mess = substitute("Пункт доставки/отгрузки: код &1 БД &2&3:&4"
                      , (if p-mode = 'ДОБАВЛЕНИЕ':U or not available buf_point-io then p-point-code else buf_point-io.point-code)
                      , (if p-mode = 'ДОБАВЛЕНИЕ':U or not available buf_point-io then p-db-num else buf_point-io.db-num)
                      , chr(10)
                      , p-mess)
  .
end.
