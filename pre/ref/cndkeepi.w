  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = tt-condition-keeping.des :handle
      .
...
END.
  ...
  or p-mode = 'ПРОСМОТР':U then do:
    ...
    buffer-copy locked_condition-keeping to tt-condition-keeping.
      ...
      ASSIGN varsts = entry (lookup (STRING(tt-condition-keeping.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U).
  RUN Myenable.
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-condition-keeping SHARE-LOCK.
...
B-Help
tt-condition-keeping.cond-keep-name when p-mode <> 'ПРОСМОТР':U
tt-condition-keeping.des
tt-condition-keeping.h-mode-from  when p-mode <> 'ПРОСМОТР':U
tt-condition-keeping.h-mode-to    when p-mode <> 'ПРОСМОТР':U
tt-condition-keeping.t-mode-from  when p-mode <> 'ПРОСМОТР':U
tt-condition-keeping.t-mode-to    when p-mode <> 'ПРОСМОТР':U
WITH FRAME Dialog-Frame.
if p-mode = 'ПРОСМОТР':U then do:
  assign
    tt-condition-keeping.des:read-only in frame Dialog-Frame = yes.
end.
VIEW FRAME Dialog-Frame.
END PROCEDURE.
PROCEDURE Proc-save :
...
FIND FIRST bf_condition-keeping WHERE bf_condition-keeping.sts            =  0 AND
                                      bf_condition-keeping.cond-keep-name =  INPUT FRAME Dialog-Frame tt-condition-keeping.cond-keep-name AND
...
IF AVAILABLE bf_condition-keeping THEN DO:
  ...
  RETURN ERROR.
END.
IF INPUT FRAME Dialog-Frame tt-condition-keeping.h-mode-to < INPUT FRAME Dialog-Frame tt-condition-keeping.h-mode-from THEN DO:
    MESSAGE "Влажность 'до' меньше влажности 'от'." VIEW-AS ALERT-BOX.
    RETURN ERROR.
END.
IF INPUT FRAME Dialog-Frame tt-condition-keeping.t-mode-to < INPUT FRAME Dialog-Frame tt-condition-keeping.t-mode-from THEN DO:
