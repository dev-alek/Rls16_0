...
DEFINE BUFFER X_c-obj-hist FOR c-goods-attr-any.
DEFINE BUFFER find_c-obj-hist FOR c-goods-attr-any.
...
define input  parameter p-db-num like c-goods-attr-any.corr-user-db-num no-undo .
...
define input parameter p-next as logical no-undo.
define input parameter p-user like c-goods-attr-any.corr-user-name no-undo.
assign
sch-corr-date = ?.
display
0 @ sch-db-num
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if ("           X_c-obj-hist.Bush                = 'operserv'          and X_c-obj-hist.attr-code          = 'operservid'          and X_c-obj-hist.attr-value         = string(pid)  " + " " + where-phrase-31) <> ""
          then  substitute('  X_c-obj-hist.Bush                = &1operserv&1                                         and X_c-obj-hist.attr-code          = &1operservid&1          and X_c-obj-hist.attr-value         = &1&2&1 ', chr(34), pid)   + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          ("           X_c-obj-hist.Bush                = 'operserv'          and X_c-obj-hist.attr-code          = 'operservid'          and X_c-obj-hist.attr-value         = string(pid)  " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-obj-hist:handle
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-obj-hist FOR EACH X_c-obj-hist
      where            X_c-obj-hist.Bush                = 'operserv'          and X_c-obj-hist.attr-code          = 'operservid'          and X_c-obj-hist.attr-value         = string(pid)
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-obj-hist:handle:get-buffer-handle(1) = (buffer X_c-obj-hist:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute('  X_c-obj-hist.Bush                = &1operserv&1                                         and X_c-obj-hist.attr-code          = &1operservid&1          and X_c-obj-hist.attr-value         = &1&2&1 ', chr(34), pid)   + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = " "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
