...
assign
  v-long-obj-code = (if b-clients.obj-type = 'орг':U
                     then exp(10, 9)
                     else 0
                    )
                  + b-clients.obj-code
...
do:
  ...
  RUN verify-file((Path-To-Dir-Pictures + "cli\"),
                  "Не найден подкаталог " + Path-To-Dir-Pictures + "cli\",
                  no,
                  output loc#log) no-error.
  if error-status:error or not loc#log then return error.
  run adm/shattri.p (
  ...
  v-image-order = "jpg,bmp".
   assign
   Path-To-Pictures = Path-To-Dir-Pictures + "cli\" + string( b-clients.obj-code )
   .
