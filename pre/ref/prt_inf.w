...
PROCEDURE adm-disable :
    DISABLE b-exit b-help WITH FRAME D-Dialog.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN b-exit b-help WITH FRAME D-Dialog.
...
ASSIGN frame D-Dialog:TITLE = "Товар " + bf-goods.gds-name + ". Признак: " + ub.gds-prt.node-name + ".".
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "TT-prt-obj":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE TT-prt-obj THEN STRING(ROWID(TT-prt-obj))
        ELSE "?":U.
        OTHERWISE
        DO:
