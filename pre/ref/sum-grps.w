...
ON END-ERROR OF FRAME d-sum-grp
OR ENDKEY OF FRAME d-sum-grp DO:
   run gbl/markqwa.p (
...
END.
ON GO OF FRAME d-sum-grp
DO:
  p-rid-list = v-rid-list.
  run proc-send in this-procedure no-error.
END.
ON CHOOSE OF b-add IN FRAME d-sum-grp
DO:
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_group-goods-cash-desk_add-def':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
...
if rr <> ? then  do:
  ...
  if error-status:error then return no-apply.
  OPEN QUERY br-sumgrps FOR EACH X_sum-grp NO-LOCK     BY X_sum-grp.grp-code INDEXED-REPOSITION.
  reposition br-sumgrps to recid rr.
  log-res  = br-sumgrps:select-focused-row( ).
  apply "ENTRY":U to br-sumgrps.
end.
END.
ON CHOOSE OF b-chg IN FRAME d-sum-grp
DO:
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_group-goods-cash-desk_update':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
...
reposition br-sumgrps to recid rr .
END.
ON CHOOSE OF b-del IN FRAME d-sum-grp
DO:
DEFINE VARIABLE V-OLD-NAME LIKE UB.SUM-GRP.GRP-NAME NO-UNDO.
if not available X_sum-grp THEN  return no-apply.
...
END.
ON CHOOSE OF b-disc IN FRAME d-sum-grp
DO:
 if not available X_sum-grp THEN return no-apply.
  DEFINE VARIABLE glog AS LOGICAL NO-UNDO.
...
END.
ON CHOOSE OF b-goods IN FRAME d-sum-grp
DO:
 if not available X_sum-grp THEN return no-apply.
  DEFINE VARIABLE glog AS LOGICAL NO-UNDO.
  if dgrpr-option = "":U then do:
  ...
  if dgrpr-option = 'sum-grp-gl':U then do:
  ...
  if dgrpr-option = 'sum-grp':U then do:
...
END.
ON CHOOSE OF b-hist IN FRAME d-sum-grp
DO:
...
assign
  v-str-recid12 = trim( string( recid( X_sum-grp ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry12 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
            ...
            apply "iteration-changed" to br-sumgrps in frame d-sum-grp.
      end.
    if num-entries( v-rid-list ) = 0 then
        hide mark-num in frame d-sum-grp.
    else
        disp num-entries( v-rid-list ) @ mark-num with frame d-sum-grp.
  end.
  apply "entry" to br-sumgrps in frame d-sum-grp.
END.
ON CHOOSE OF b-print IN FRAME d-sum-grp
DO:
  ...
  DEFINE FRAME List
      ...
      X_sum-grp.grp-code column-label 'код':U format ">>9"
      sym2 column-label ":" format "x(1)"
      X_sum-grp.grp-name column-label 'название':U format "x(50)"
...
END.
ON CHOOSE OF b-sel IN FRAME d-sum-grp
DO:
...
DO:
    ...
    apply "choose" to b-mark in frame d-sum-grp .
    else if can-do( bttns, "b-sel" ) then
        apply "choose" to b-sel in frame d-sum-grp .
    else dO:
    end.
END.
...
DO:
  assign
  dgrpr-option = 'ПРОСМОТР':U
  .
  APPLY "CHOOSE" TO b-disc IN FRAME d-sum-grp.
END.
ON CHOOSE OF MENU-ITEM m_update-disc
DO:
    assign
   dgrpr-option = 'ИЗМЕНЕНИЕ':U
   .
   APPLY "CHOOSE" TO b-disc IN FRAME d-sum-grp.
END.
ON CHOOSE OF MENU-ITEM m_lookup-goods
DO:
  assign
  dgrpr-option = 'sum-grp-gl':U
  .
  APPLY "CHOOSE" TO b-goods IN FRAME d-sum-grp.
END.
ON CHOOSE OF MENU-ITEM m_update-goods
DO:
    assign
   dgrpr-option = 'sum-grp':U
   .
   APPLY "CHOOSE" TO b-goods IN FRAME d-sum-grp.
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME d-sum-grp:PARENT eq ?
THEN FRAME d-sum-grp:PARENT = ACTIVE-WINDOW.
define variable vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame d-sum-grp
do:
  run gbl/app_help.p
...
on choose of b-help in frame d-sum-grp
do:
  apply "help":u to frame d-sum-grp .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame d-sum-grp:width - 0.3
                fh            = frame d-sum-grp:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame d-sum-grp :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame d-sum-grp :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame d-sum-grp :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-sum-grp :height = v-frame-height
          .
          if frame d-sum-grp :scrollable = true
          then do:
            assign
              frame d-sum-grp :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame d-sum-grp :height
      v-frame-virtual-height = frame d-sum-grp :virtual-height
    ...
    assign
      v-field-group-handle = frame d-sum-grp :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame d-sum-grp :virtual-height = frame d-sum-grp :virtual-height + p-change-value
          no-error .
        if error-status :error
      ...
      assign
        frame d-sum-grp :height = frame d-sum-grp :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame d-sum-grp :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame d-sum-grp :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame d-sum-grp :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame d-sum-grp :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame d-sum-grp :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame d-sum-grp :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-sum-grp :width = v-frame-width
          .
          if frame d-sum-grp :scrollable = true
          then do:
            assign
              frame d-sum-grp :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame d-sum-grp :width
      v-frame-virtual-width = frame d-sum-grp :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame d-sum-grp :virtual-width = frame d-sum-grp :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame d-sum-grp :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame d-sum-grp :width = frame d-sum-grp :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame d-sum-grp :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame d-sum-grp :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame d-sum-grp :height - v-diasize-resize-button :height
                  - 1
                  - (frame d-sum-grp :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame d-sum-grp :width - v-diasize-resize-button :width
                  - 1
                  - (frame d-sum-grp :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame d-sum-grp
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame d-sum-grp :height
      v-col-delta = v-new-col - frame d-sum-grp :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame d-sum-grp :width
      v-diasize-current-frame-height = frame d-sum-grp :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame d-sum-grp :height
        v-diasize-orig-frame-width  = frame d-sum-grp :width
        v-diasize-browse-handle     = browse br-sumgrps :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame d-sum-grp :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
on INS of frame d-sum-grp anywhere do:
  if b-mark :sensitive then DO: apply "CHOOSE":U to b-mark in frame d-sum-grp. END.
  return no-apply.
end.
define variable vss-include-info17 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on CTRL-N, CTRL-Т of frame d-sum-grp anywhere do:
  if b-add :sensitive then DO: apply "CHOOSE":U to b-add in frame d-sum-grp. END.
  return no-apply.
end.
define variable vss-include-info18 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F4 of frame d-sum-grp anywhere do:
  if b-chg :sensitive then DO: apply "CHOOSE":U to b-chg in frame d-sum-grp. END.
  return no-apply.
end.
define variable vss-include-info19 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F8 of frame d-sum-grp anywhere do:
  if b-del :sensitive then DO: apply "CHOOSE":U to b-del in frame d-sum-grp. END.
  return no-apply.
end.
define variable vss-include-info20 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame d-sum-grp anywhere do:
  if b-sel :sensitive then DO: apply "CHOOSE":U to b-sel in frame d-sum-grp. END.
  return no-apply.
end.
define variable vss-include-info21 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame d-sum-grp anywhere do:
  if b-quit :sensitive then DO: apply "CHOOSE":U to b-quit in frame d-sum-grp. END.
  return no-apply.
end.
define variable vss-include-info22 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on CTRL-P, CTRL-З of frame d-sum-grp anywhere do:
  if b-print :sensitive then DO: apply "CHOOSE":U to b-print in frame d-sum-grp. END.
  return no-apply.
end.
ON WINDOW-CLOSE OF FRAME d-sum-grp APPLY "END-ERROR":U TO SELF.
MAIN-BLOCK:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  send-ref = (IF error-status:error or dops <> "yes" then no else yes).
   ...
   WAIT-FOR GO OF FRAME d-sum-grp.
END.
RUN disable_UI.
...
PROCEDURE proc-send :
define variable glog as logical no-undo .
if can-find(first ub.cash-desk where ub.cash-desk.pos-type = 'IBM':U) AND
