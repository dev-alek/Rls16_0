...
assign
  v-str-recid15 = trim( string( recid( X_c-sum-grp ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry15 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-csum-grp in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-csum-grp in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-csum-grp :handle
      .
...
do:
  v-doc-rec = recid(X_sum-grp). run openbr in this-procedure. reposition br-csum-grp to recid v-doc-rec no-error.               apply 'value-changed' to br-csum-grp.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-csum-grp   as character no-undo .
def var sort-clmnbr-csum-grp    as handle    no-undo .
def var cur-clmnbr-csum-grp     as handle    no-undo .
def var cur-clmn-locbr-csum-grp as integer   no-undo .
def var re-querybr-csum-grp     as logical   initial no no-undo .
on start-search, ctrl-o of br-csum-grp in frame Dialog-Frame do:
   run sort-brbr-csum-grp
...
PROCEDURE sort-brbr-csum-grp :
  define input parameter p-recid as recid no-undo .
  if re-querybr-csum-grp = no then do:
    assign
       cur-clmnbr-csum-grp = br-csum-grp:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-csum-grp <> ? then sort-clmnbr-csum-grp:column-fgcolor = 0.
    if cur-clmnbr-csum-grp = sort-clmnbr-csum-grp then do:
      assign
         sort-labelbr-csum-grp = ""
         sort-clmnbr-csum-grp = ?
      .
     end.
     else do:
       assign
         sort-labelbr-csum-grp = cur-clmnbr-csum-grp:label
         sort-clmnbr-csum-grp  = cur-clmnbr-csum-grp
         sort-clmnbr-csum-grp:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-csum-grp = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-csum-grp:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-csum-grp then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-csum-grp = cur-clmn-locbr-csum-grp + 1
    .
  end.
  case sort-labelbr-csum-grp:
        when X_c-sum-grp.grp-name:label in browse br-csum-grp then DO:   assign     sort-column-name = "X_c-sum-grp.grp-name"   .   run OpenBr.   . END.
        when X_c-sum-grp.corr-date:label in browse br-csum-grp then DO:   assign     sort-column-name = "X_c-sum-grp.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-csum-grp') then do:
          run mv-brw-defaultbr-csum-grp.
        end.
      if sort-labelbr-csum-grp <> "" then do:
        assign
          cur-clmnbr-csum-grp:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-csum-grp = ?
      .
    end.
  end case.
    if cur-clmn-locbr-csum-grp <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-csum-grp') then do:
        run ch-clmnbr-csum-grp in this-procedure (cur-clmn-locbr-csum-grp).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-csum-grp to recid p-recid no-error.
    apply "value-changed" to br-csum-grp in frame Dialog-Frame.
  end.
  apply "entry" to br-csum-grp in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-csum-grp:
if cur-clmnbr-csum-grp = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-csum-grp = yes.
   run sort-brbr-csum-grp
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
...
DEF VAR cur-clmn-numbr-csum-grp as INT EXTENT 6 no-undo.
DEF VAR varmvibr-csum-grp       as INT no-undo.
DEF VAR varmvjbr-csum-grp       as INT no-undo.
DEF VAR varmvkbr-csum-grp       as INT no-undo.
DEF VAR varmvlbr-csum-grp       as INT no-undo.
DEF VAR move-elementbr-csum-grp as INT no-undo.
def var jjbr-csum-grp           as int no-undo.
do varmvibr-csum-grp = 1 to EXTENT(cur-clmn-numbr-csum-grp):
  ASSIGN cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = varmvibr-csum-grp.
END.
RUN start-mv-clmnbr-csum-grp.
PROCEDURE start-mv-clmnbr-csum-grp:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-csum-grp = NUM-ENTRIES('1,2,3,4,5,6') TO 1 BY -1:
     RUN re-move-clmnbr-csum-grp ( cur-clmn-numbr-csum-grp[INTEGER(ENTRY (jjbr-csum-grp, '1,2,3,4,5,6'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-csum-grp do:
  RUN re-move-clmnbr-csum-grp ( 1, 6).
END.
ON ctrl-cursor-left OF BROWSE br-csum-grp do:
  RUN re-move-clmnbr-csum-grp (6, 1).
END.
PROCEDURE re-move-clmnbr-csum-grp:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-csum-grp = 1 TO EXTENT(cur-clmn-numbr-csum-grp):
    if cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = source-column THEN cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = -1.
  END.
  if br-csum-grp:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-csum-grp = source-column - 1 to target-column BY -1:
    DO varmvibr-csum-grp = 1 TO EXTENT(cur-clmn-numbr-csum-grp):
        if cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = varmvjbr-csum-grp THEN DO:
          cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = cur-clmn-numbr-csum-grp[varmvibr-csum-grp] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-csum-grp = source-column + 1 to target-column:
    DO varmvibr-csum-grp = 1 TO EXTENT(cur-clmn-numbr-csum-grp):
      if cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = varmvjbr-csum-grp THEN DO:
        cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = cur-clmn-numbr-csum-grp[varmvibr-csum-grp] - 1.
      END.
  ...
  END.
  DO varmvibr-csum-grp = 1 TO EXTENT(cur-clmn-numbr-csum-grp):
    if cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = -1 THEN cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-csum-grp:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-csum-grp = 1 TO EXTENT(cur-clmn-numbr-csum-grp):
    if cur-clmn-numbr-csum-grp[varmvibr-csum-grp] = cur-clmn-loc THEN move-elementbr-csum-grp = varmvibr-csum-grp.
  END.
  RUN re-move-clmnbr-csum-grp (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-csum-grp:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-csum-grp = 1 to EXTENT(cur-clmn-numbr-csum-grp):
    RUN re-move-clmnbr-csum-grp (cur-clmn-numbr-csum-grp[varmvlbr-csum-grp], varmvlbr-csum-grp).
  END.
  RUN start-mv-clmnbr-csum-grp.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-csum-grp FOR EACH X_c-sum-grp NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
...
CASE X_c-sum-grp.subject:
  when 'sum-grp':U then do:
    ...
    + "grp-name" + chr(4) + "Наименование группы" + chr(4) + ""  .
    run proc-full-temp-changes in this-procedure (
                                                input  (X_c-sum-grp.action = integer('1':U))
                                                ,input  (X_c-sum-grp.action = integer('99':U))
                                                ,input  buffer X_c-sum-grp:handle
                                                ,input  'sum-grp':U
                                                ,input  "grp-code,grp-name"
                                                ,input  v-label-param).
  end.
  when 'dis-grp-rule':U then do:
        find first buf_c-dis-grp-rule no-lock where
              buf_c-dis-grp-rule.classif-type = 'sum-grp':U
...
FUNCTION get-subject RETURNS CHARACTER
  ( p-subject as character ) :
    RETURN entry (lookup (p-subject, 'sum-grp,dis-grp-rule':U), 'Группа товаров,Скидки по группе':U).
END FUNCTION.
