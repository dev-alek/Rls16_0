...
ON GO OF FRAME d-bas-cds
DO:
  p-rec-list = v-rec-list.
END.
ON WINDOW-CLOSE OF FRAME d-bas-cds
DO:
  APPLY "END-ERROR":U TO SELF.
END.
ON CHOOSE OF b-add IN FRAME d-bas-cds
DO:
...
assign
  v-num-entry3 = lookup(string( bas-code.rid ), v-rec-list ).
if v-num-entry3 > 0 then do:
  assign
    entry(v-num-entry3, v-rec-list) = "":U
    v-rec-list = replace( v-rec-list, chr(44) + chr(44), chr(44)) .
end.
else do:
  assign
  v-rec-list = v-rec-list + ( if v-rec-list = "":U then "":U else chr(44) ) + string( bas-code.rid) .
end.
br-cds :refresh ().
if last-event :function <> "mouse-select-dblclick" then
  br-cds :select-next-row ().
apply "entry" to br-cds in frame d-bas-cds.
END.
ON CHOOSE OF b-sel IN FRAME d-bas-cds
DO:
if v-rec-list = "" and
...
DO:
apply "choose" to b-mark in frame d-bas-cds.
END.
...
on CTRL-N, CTRL-Т of frame d-bas-cds anywhere do:
  if b-add :sensitive then DO: apply "CHOOSE":U to b-add in frame d-bas-cds. END.
  return no-apply.
end.
define variable vss-include-info5 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame d-bas-cds anywhere do:
  if b-sel :sensitive then DO: apply "CHOOSE":U to b-sel in frame d-bas-cds. END.
  return no-apply.
end.
define variable vss-include-info6 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on INS of frame d-bas-cds anywhere do:
  if b-mark :sensitive then DO: apply "CHOOSE":U to b-mark in frame d-bas-cds. END.
  return no-apply.
end.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME d-bas-cds:PARENT eq ?
THEN FRAME d-bas-cds:PARENT = ACTIVE-WINDOW.
define variable vss-include-info7 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame d-bas-cds
do:
  run gbl/app_help.p
...
on choose of b-help in frame d-bas-cds
do:
  apply "help":u to frame d-bas-cds .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame d-bas-cds:width - 0.3
                fh            = frame d-bas-cds:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame d-bas-cds :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame d-bas-cds :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame d-bas-cds :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-bas-cds :height = v-frame-height
          .
          if frame d-bas-cds :scrollable = true
          then do:
            assign
              frame d-bas-cds :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame d-bas-cds :height
      v-frame-virtual-height = frame d-bas-cds :virtual-height
    ...
    assign
      v-field-group-handle = frame d-bas-cds :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame d-bas-cds :virtual-height = frame d-bas-cds :virtual-height + p-change-value
          no-error .
      ...
      assign
        frame d-bas-cds :height = frame d-bas-cds :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame d-bas-cds :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame d-bas-cds :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame d-bas-cds :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame d-bas-cds :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame d-bas-cds :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame d-bas-cds :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame d-bas-cds :width = v-frame-width
          .
          if frame d-bas-cds :scrollable = true
          then do:
            assign
              frame d-bas-cds :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame d-bas-cds :width
      v-frame-virtual-width = frame d-bas-cds :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame d-bas-cds :virtual-width = frame d-bas-cds :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame d-bas-cds :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame d-bas-cds :width = frame d-bas-cds :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame d-bas-cds :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame d-bas-cds :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame d-bas-cds :height - v-diasize-resize-button :height
                  - 1
                  - (frame d-bas-cds :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame d-bas-cds :width - v-diasize-resize-button :width
                  - 1
                  - (frame d-bas-cds :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame d-bas-cds
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame d-bas-cds :height
      v-col-delta = v-new-col - frame d-bas-cds :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame d-bas-cds :width
      v-diasize-current-frame-height = frame d-bas-cds :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame d-bas-cds :height
        v-diasize-orig-frame-width  = frame d-bas-cds :width
        v-diasize-browse-handle     = browse br-cds :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame d-bas-cds :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
define variable vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  RUN UI-on.
  WAIT-FOR GO OF FRAME d-bas-cds.
END.
RUN disable_UI.
PROCEDURE cre-bas :
...
find first buf2_parts no-lock where
      buf2_parts.out-code  = 'free-zone':U and
...
case mode:
  when "gds-current" then do:
    frame d-bas-cds :title = "Имеющиеся основные цены:   " +
  ...
  when "scl-gds-all" then do:
    frame d-bas-cds :title = "Все основные коды по признакам:   " +
                                 "Товар: " + ub.goods.artic + "  " + ub.goods.gds-name
                                 .
    for each ub.bar-code no-lock where
             ub.bar-code.gds-code  = ub.goods.gds-code and
  ...
  when "par-gds-all" then do:
    frame d-bas-cds :title = "Все основные коды по партиям:   " +
                                 "Товар: " + ub.goods.artic + "  " + ub.goods.gds-name
                                 .
    for each ub.bar-code no-lock where
             ub.bar-code.gds-code  = ub.goods.gds-code and
  ...
  when "par-gds-free" then do:
    frame d-bas-cds :title = "Основные коды по партиям свободной зоны и незакрытых ПН:   " +
  ...
  when "gds-all" then do:
    frame d-bas-cds :title = "Все основные коды по товару:   " +
                                 "Товар: " + ub.goods.artic + "  " + ub.goods.gds-name
                                 .
    for each ub.bar-code no-lock where
             ub.bar-code.gds-code  = ub.goods.gds-code and
...
frame d-bas-cds :title = frame d-bas-cds :title +
