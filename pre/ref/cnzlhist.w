...
assign
  v-str-recid14 = trim( string( recid( X_c-nzl-hist ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry14 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-nzl-hist in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-nzl-hist in frame Dialog-Frame.
END.
...
DO:
  ...
  if error-status:error then return no-apply.
END.
ON CTRL-J OF sch-nozzle-code IN FRAME Dialog-Frame
DO:
  run proc-find-nozzle-code in this-procedure(yes, input frame Dialog-Frame sch-nozzle-code) no-error.
  if error-status:error then return no-apply.
END.
ON RETURN OF sch-nozzle-code IN FRAME Dialog-Frame
DO:
  run proc-find-nozzle-code in this-procedure(no, input frame Dialog-Frame sch-nozzle-code) no-error.
  if error-status:error then return no-apply.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-nzl-hist :handle
      .
...
do:
   v-doc-rec = recid(X_c-nzl-hist).  RUn OpenBR in this-procedure ( input yes, input no, '':U).  REPOSITION br-nzl-hist to recid v-doc-rec No-ERROR.               apply 'value-changed' to br-nzl-hist.
...
do:
  ...
  END.
def var sort-labelbr-nzl-hist   as character no-undo .
def var sort-clmnbr-nzl-hist    as handle    no-undo .
def var cur-clmnbr-nzl-hist     as handle    no-undo .
def var cur-clmn-locbr-nzl-hist as integer   no-undo .
def var re-querybr-nzl-hist     as logical   initial no no-undo .
on start-search, ctrl-o of br-nzl-hist in frame Dialog-Frame do:
   run sort-brbr-nzl-hist
     (input (if available X_c-nzl-hist
             then recid(X_c-nzl-hist)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-nzl-hist :
  define input parameter p-recid as recid no-undo .
  if re-querybr-nzl-hist = no then do:
    assign
       cur-clmnbr-nzl-hist = br-nzl-hist:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-nzl-hist <> ? then sort-clmnbr-nzl-hist:column-fgcolor = 0.
    if cur-clmnbr-nzl-hist = sort-clmnbr-nzl-hist then do:
      assign
         sort-labelbr-nzl-hist = ""
         sort-clmnbr-nzl-hist = ?
      .
     end.
     else do:
       assign
         sort-labelbr-nzl-hist = cur-clmnbr-nzl-hist:label
         sort-clmnbr-nzl-hist  = cur-clmnbr-nzl-hist
         sort-clmnbr-nzl-hist:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-nzl-hist = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-nzl-hist:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-nzl-hist then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-nzl-hist = cur-clmn-locbr-nzl-hist + 1
    .
  end.
  case sort-labelbr-nzl-hist:
        when X_c-nzl-hist.nozzle-code:label in browse br-nzl-hist then DO:    assign       sort-column-name = "X_c-nzl-hist.nozzle-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-nzl-hist.corr-date:label in browse br-nzl-hist then DO:    assign       sort-column-name = "X_c-nzl-hist.corr-date"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-nzl-hist.corr-user-db-num:label in browse br-nzl-hist then DO:    assign       sort-column-name = "X_c-nzl-hist.corr-user-db-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-nzl-hist') then do:
          run mv-brw-defaultbr-nzl-hist.
        end.
      if sort-labelbr-nzl-hist <> "" then do:
        assign
          cur-clmnbr-nzl-hist:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-nzl-hist = ?
      .
    end.
  end case.
    if cur-clmn-locbr-nzl-hist <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-nzl-hist') then do:
        run ch-clmnbr-nzl-hist in this-procedure (cur-clmn-locbr-nzl-hist).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-nzl-hist to recid p-recid no-error.
    apply "value-changed" to br-nzl-hist in frame Dialog-Frame.
  end.
  apply "entry" to br-nzl-hist in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-nzl-hist:
if cur-clmnbr-nzl-hist = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U).
end.
else do:
   assign re-querybr-nzl-hist = yes.
   run sort-brbr-nzl-hist
     (input (if available X_c-nzl-hist
             then recid(X_c-nzl-hist)
             else ?
...
DEF VAR cur-clmn-numbr-nzl-hist as INT EXTENT 9 no-undo.
DEF VAR varmvibr-nzl-hist       as INT no-undo.
DEF VAR varmvjbr-nzl-hist       as INT no-undo.
DEF VAR varmvkbr-nzl-hist       as INT no-undo.
DEF VAR varmvlbr-nzl-hist       as INT no-undo.
DEF VAR move-elementbr-nzl-hist as INT no-undo.
def var jjbr-nzl-hist           as int no-undo.
do varmvibr-nzl-hist = 1 to EXTENT(cur-clmn-numbr-nzl-hist):
  ASSIGN cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = varmvibr-nzl-hist.
END.
RUN start-mv-clmnbr-nzl-hist.
PROCEDURE start-mv-clmnbr-nzl-hist:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-nzl-hist = NUM-ENTRIES('9,1,2,3,4,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-nzl-hist ( cur-clmn-numbr-nzl-hist[INTEGER(ENTRY (jjbr-nzl-hist, '9,1,2,3,4,5,6,7,8'))] , 1).
   END.
       END.
       IF  p-mode <> 'все':U  THEN DO:
   DO jjbr-nzl-hist = NUM-ENTRIES('1,2,3,4,5,6,7,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-nzl-hist ( cur-clmn-numbr-nzl-hist[INTEGER(ENTRY (jjbr-nzl-hist, '1,2,3,4,5,6,7,8,9'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-nzl-hist do:
  RUN re-move-clmnbr-nzl-hist ( 1, 9).
END.
ON ctrl-cursor-left OF BROWSE br-nzl-hist do:
  RUN re-move-clmnbr-nzl-hist (9, 1).
END.
PROCEDURE re-move-clmnbr-nzl-hist:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-nzl-hist = 1 TO EXTENT(cur-clmn-numbr-nzl-hist):
    if cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = source-column THEN cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = -1.
  END.
  if br-nzl-hist:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-nzl-hist = source-column - 1 to target-column BY -1:
    DO varmvibr-nzl-hist = 1 TO EXTENT(cur-clmn-numbr-nzl-hist):
        if cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = varmvjbr-nzl-hist THEN DO:
          cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-nzl-hist = source-column + 1 to target-column:
    DO varmvibr-nzl-hist = 1 TO EXTENT(cur-clmn-numbr-nzl-hist):
      if cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = varmvjbr-nzl-hist THEN DO:
        cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] - 1.
      END.
  ...
  END.
  DO varmvibr-nzl-hist = 1 TO EXTENT(cur-clmn-numbr-nzl-hist):
    if cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = -1 THEN cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-nzl-hist:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-nzl-hist = 1 TO EXTENT(cur-clmn-numbr-nzl-hist):
    if cur-clmn-numbr-nzl-hist[varmvibr-nzl-hist] = cur-clmn-loc THEN move-elementbr-nzl-hist = varmvibr-nzl-hist.
  END.
  RUN re-move-clmnbr-nzl-hist (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-nzl-hist:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-nzl-hist = 1 to EXTENT(cur-clmn-numbr-nzl-hist):
    RUN re-move-clmnbr-nzl-hist (cur-clmn-numbr-nzl-hist[varmvlbr-nzl-hist], varmvlbr-nzl-hist).
  END.
  RUN start-mv-clmnbr-nzl-hist.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE MyEnable :
assign
  br-nzl-hist:num-locked-columns in frame Dialog-Frame = 1
  ...
  br-nzl-hist
  sch-corr-date
  sch-nozzle-code when p-mode = 'все':U
  sch-corr-user-name
  BR-changes mark-num
  WITH FRAME Dialog-Frame .
  VIEW FRAME Dialog-Frame .
...
title0 = "История пистолета" + chr(32).
...
if p-open-query then do:
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-nzl-hist:handle
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-nzl-hist FOR EACH X_c-nzl-hist
      where  TRUE
       use-index idate
      indexed-reposition
  .
  ...
  on error undo, leave
  :
    if query br-nzl-hist:handle:get-buffer-handle(1) = (buffer X_c-nzl-hist:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input query br-nzl-hist:handle
                          ,input rowid(X_c-nzl-hist)
                          ,input logical(parameter-2-27)
                          ,input no-lock
                          ,input (buffer X_c-nzl-hist:handle)
                          ,input parameter-4-27
                          ,input parameter-5-27
                          ) no-error.
      .
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH X_c-nzl-hist"
      parameter-4-27 =
        (
          if (" TRUE " + " " + where-phrase-27) <> ""
          then " TRUE " + " " + where-phrase-27
  ...
  WHEN 'объект':U THEN DO:
    ...
    if p-open-query then do:
      ASSIGN
      frame Dialog-Frame:TITLE = title0 + substitute(" Пистолеты на объекте: &1&2"
                                                        ,p-obj-type, p-obj-code).
    end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-29 as character no-undo .
      ...
      parameter-4-29 =
        (
          if ("                     X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                       " + " " + where-phrase-29) <> ""
          then  substitute(' X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3 ', chr(34), p-obj-type, p-obj-code )  + " " + where-phrase-29
          else "true"
        )
      parameter-5-29 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-29 =
          ("                     X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                       " + " " + where-phrase-29 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-nzl-hist:handle
  ...
  if l-filter-open-29 = false then do:
    OPEN QUERY br-nzl-hist FOR EACH X_c-nzl-hist
      where                      X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-nzl-hist:handle:get-buffer-handle(1) = (buffer X_c-nzl-hist:handle) then do:
      ...
      parameter-4-29 =
        "where ":u +  substitute(' X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3 ', chr(34), p-obj-type, p-obj-code )  + " ":u + where-phrase-29 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-29 = (if p-find-next then "true":u else "false":u )
      parameter-3-29 =  "FOR EACH X_c-nzl-hist"
      parameter-4-29 =
        (
          if ("                     X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                       " + " " + where-phrase-29) <> ""
          then  substitute(' X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3 ', chr(34), p-obj-type, p-obj-code )  + " " + where-phrase-29
  ...
  WHEN "one":u THEN DO:
    ...
    if p-open-query then do:
      ASSIGN frame Dialog-Frame:TITLE = title0 + substitute(" Пистолет №&1 Объект &2&3"
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code AND X_c-nzl-hist.nozzle-code  = p-nozzle-code " + " " + where-phrase-31) <> ""
          then  substitute(' X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3  AND X_c-nzl-hist.nozzle-code  = &4 ', chr(34), p-obj-type, p-obj-code, p-nozzle-code )  + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code AND X_c-nzl-hist.nozzle-code  = p-nozzle-code " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-nzl-hist:handle
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-nzl-hist FOR EACH X_c-nzl-hist
      where  X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code AND X_c-nzl-hist.nozzle-code  = p-nozzle-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-nzl-hist:handle:get-buffer-handle(1) = (buffer X_c-nzl-hist:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute(' X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3  AND X_c-nzl-hist.nozzle-code  = &4 ', chr(34), p-obj-type, p-obj-code, p-nozzle-code )  + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-31 = (if p-find-next then "true":u else "false":u )
      parameter-3-31 =  "FOR EACH X_c-nzl-hist"
      parameter-4-31 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code AND X_c-nzl-hist.nozzle-code  = p-nozzle-code " + " " + where-phrase-31) <> ""
          then  substitute(' X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3  AND X_c-nzl-hist.nozzle-code  = &4 ', chr(34), p-obj-type, p-obj-code, p-nozzle-code )  + " " + where-phrase-31
  ...
  WHEN "subject":u THEN DO:
    assign
    filter-point = filter-point0 + p-mode
    v-subject-chr = entry (lookup (p-subject, 'nozzle,pl-pump-nozzle,pump-nozzle,nozzle-attr':U), 'Пистолет,Резервуар/ТРК/Пистолет,ТРК/Пистолет,Атрибут Пистолета':U)
    ...
    CASE p-subject:
      when 'nozzle':U then do:
         if p-open-query then do:
          assign
          frame Dialog-Frame:title = v-title + substitute(" &1", p-nozzle-code).
        end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-33 as character no-undo .
      ...
      parameter-4-33 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code  and X_c-nzl-hist.subject = p-subject                         " + " " + where-phrase-33) <> ""
          then  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.nozzle-code  = &4 and X_c-nzl-hist.subject = &1&5&1 ', chr(34), p-obj-type, p-obj-code, p-nozzle-code, p-subject)  + " " + where-phrase-33
          else "true"
        )
      parameter-5-33 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-33 =
          (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code  and X_c-nzl-hist.subject = p-subject                         " + " " + where-phrase-33 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-nzl-hist:handle
  ...
  if l-filter-open-33 = false then do:
    OPEN QUERY br-nzl-hist FOR EACH X_c-nzl-hist
      where  X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code  and X_c-nzl-hist.subject = p-subject
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-nzl-hist:handle:get-buffer-handle(1) = (buffer X_c-nzl-hist:handle) then do:
      ...
      parameter-4-33 =
        "where ":u +  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.nozzle-code  = &4 and X_c-nzl-hist.subject = &1&5&1 ', chr(34), p-obj-type, p-obj-code, p-nozzle-code, p-subject)  + " ":u + where-phrase-33 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-33 = (if p-find-next then "true":u else "false":u )
      parameter-3-33 =  "FOR EACH X_c-nzl-hist"
      parameter-4-33 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code  and X_c-nzl-hist.subject = p-subject                         " + " " + where-phrase-33) <> ""
          then  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.nozzle-code  = &4 and X_c-nzl-hist.subject = &1&5&1 ', chr(34), p-obj-type, p-obj-code, p-nozzle-code, p-subject)  + " " + where-phrase-33
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-35 as character no-undo .
      ...
      parameter-4-35 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pump-code  = p-pump-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-35) <> ""
          then  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.pump-code  = &4  and X_c-nzl-hist.subject = &1&5&1                           And X_c-nzl-hist.nozzle-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject, p-nozzle-code)  + " " + where-phrase-35
          else "true"
        )
      parameter-5-35 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-35 =
          (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pump-code  = p-pump-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-35 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-nzl-hist:handle
  ...
  if l-filter-open-35 = false then do:
    OPEN QUERY br-nzl-hist FOR EACH X_c-nzl-hist
      where  X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pump-code  = p-pump-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.nozzle-code  = p-nozzle-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-nzl-hist:handle:get-buffer-handle(1) = (buffer X_c-nzl-hist:handle) then do:
      ...
      parameter-4-35 =
        "where ":u +  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.pump-code  = &4  and X_c-nzl-hist.subject = &1&5&1                           And X_c-nzl-hist.nozzle-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject, p-nozzle-code)  + " ":u + where-phrase-35 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-35 = (if p-find-next then "true":u else "false":u )
      parameter-3-35 =  "FOR EACH X_c-nzl-hist"
      parameter-4-35 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pump-code  = p-pump-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-35) <> ""
          then  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.pump-code  = &4  and X_c-nzl-hist.subject = &1&5&1                           And X_c-nzl-hist.nozzle-code  = &6 ', chr(34), p-obj-type, p-obj-code, p-pump-code, p-subject, p-nozzle-code)  + " " + where-phrase-35
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-37 as character no-undo .
      ...
      parameter-4-37 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pl-code  = p-pl-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.pump-code  = p-pump-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-37) <> ""
          then  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.pl-code  = &4  and X_c-nzl-hist.subject = &1&5&1                           And X_c-nzl-hist.pump-code  = &6                            And X_c-nzl-hist.nozzle-code  = &7 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code, p-nozzle-code )   + " " + where-phrase-37
          else "true"
        )
      parameter-5-37 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-37 =
          (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pl-code  = p-pl-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.pump-code  = p-pump-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-37 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-nzl-hist:handle
  ...
  if l-filter-open-37 = false then do:
    OPEN QUERY br-nzl-hist FOR EACH X_c-nzl-hist
      where  X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pl-code  = p-pl-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.pump-code  = p-pump-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code
       use-index idate
      indexed-reposition
  .
      assign
  ...
  :
    if query br-nzl-hist:handle:get-buffer-handle(1) = (buffer X_c-nzl-hist:handle) then do:
      ...
      parameter-4-37 =
        "where ":u +  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.pl-code  = &4  and X_c-nzl-hist.subject = &1&5&1                           And X_c-nzl-hist.pump-code  = &6                            And X_c-nzl-hist.nozzle-code  = &7 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code, p-nozzle-code )   + " ":u + where-phrase-37 + " ":u + p-find-condition + " " + ""
    ...
    .
      ...
      assign
      parameter-2-37 = (if p-find-next then "true":u else "false":u )
      parameter-3-37 =  "FOR EACH X_c-nzl-hist"
      parameter-4-37 =
        (
          if (" X_c-nzl-hist.obj-type = p-obj-type and X_c-nzl-hist.obj-code = p-obj-code                            And X_c-nzl-hist.pl-code  = p-pl-code  and X_c-nzl-hist.subject = p-subject                           And X_c-nzl-hist.pump-code  = p-pump-code                            And X_c-nzl-hist.nozzle-code  = p-nozzle-code                          " + " " + where-phrase-37) <> ""
          then  substitute('X_c-nzl-hist.obj-type = &1&2&1 and X_c-nzl-hist.obj-code = &3                            And X_c-nzl-hist.pl-code  = &4  and X_c-nzl-hist.subject = &1&5&1                           And X_c-nzl-hist.pump-code  = &6                            And X_c-nzl-hist.nozzle-code  = &7 ', chr(34), p-obj-type, p-obj-code, p-pl-code, p-subject, p-pump-code, p-nozzle-code )   + " " + where-phrase-37
...
apply "entry":u to sch-nozzle-code in frame Dialog-Frame .
END PROCEDURE.
...
FUNCTION get-subject RETURNS CHARACTER
  ...
  RETURN entry (lookup (p-subject, 'nozzle,pl-pump-nozzle,pump-nozzle,nozzle-attr':U), 'Пистолет,Резервуар/ТРК/Пистолет,ТРК/Пистолет,Атрибут Пистолета':U).
END FUNCTION.
