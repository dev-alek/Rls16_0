define input  parameter p-db-num-order as int64 no-undo.
define input  parameter p-doc-code-order as int64 no-undo.
...
vLabel = "История по Заказу " + string(p-doc-code-order) + " по ДБ " + string(p-db-num-order).
define temp-table X_c-obj-hist like c-order-head use-index chip-num use-index corr-user-db-num use-index user-name use-index sub
...
define input  parameter p-db-num like c-order-head.corr-user-db-num no-undo .
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-6-27 = if sort-phrase-27 = ''
                           then
        (
        " " + " BY X_c-obj-hist.corr-date DESC BY X_c-obj-hist.corr-time DESC  " +
          " " + sort-column-phrase +
        " " + " BY X_c-obj-hist.corr-date DESC BY X_c-obj-hist.corr-time DESC  "
        )
                           else
  ...
  :
    if query br-obj-hist:handle:get-buffer-handle(1) = (buffer X_c-obj-hist:handle) then do:
      ...
      parameter-5-27 = " BY X_c-obj-hist.corr-date DESC BY X_c-obj-hist.corr-time DESC  "
    .
...
define input parameter p-next as logical no-undo.
define input parameter p-user like c-order-head.corr-user-name no-undo.
assign
sch-corr-date = ?.
display
0 @ sch-db-num
...
function fLabel returns character :
   return if p-mode eq "one"  then "История по Заказу " + string(p-doc-code-order) + " по ДБ " + string(p-db-num-order) else "История по Заказам".
end.
...
(  p-open-query     as logical    ,
  ...
  p-find-condition as character ):
   ...
   then do:
      if p-chip-num eq ?
      then do:
         for each c-order-head where c-order-head.db-num   = p-db-num-order
                                    and c-order-head.doc-code = p-doc-code-order
                                    and c-order-head.corr-user-db-num = v-corr-user-db-num
         no-lock:
            create X_c-obj-hist.
            buffer-copy c-order-head to X_c-obj-hist.
            if c-order-head.subject <> "order-doc" and
               c-order-head.subject <> "*" then
            do:
              run getRecordsByChipNum in this-procedure ((buffer X_c-obj-hist:handle), c-order-head.subject).
            end.
         end.
      end.
      else do:
         for each c-order-head where
                  c-order-head.db-num           = p-db-num-order
              and c-order-head.doc-code         = p-doc-code-order
              and c-order-head.corr-user-db-num = v-corr-user-db-num
              and c-order-head.chip-num         = p-chip-num
             no-lock:
...
procedure getRecordsByChipNum:
  define input parameter iXObjHist as handle no-undo.
  ...
  vEachWhere  = substitute(
    ...
    ,vHistTable
    ,c-order-head.db-num
    ,c-order-head.doc-code
    ,c-order-head.corr-user-db-num
    ,c-order-head.chip-num)
  .
  create query vQuery.
  ...
  do while not vQuery:query-off-end:
    if iXObjHist = ? then do:
      ...
      vXObjHist:buffer-copy(buffer c-order-head:handle).
    end.
...
procedure order-doc-proc :
  ...
  define buffer buf_c-order-doc for ub.c-order-doc .
  define buffer b_File         for ub._File.
  define buffer b_Field        for ub._Field.
  do
  on error undo, return error return-value
  :
    find first buf_c-order-doc no-lock where
               buf_c-order-doc.corr-user-db-num = X_c-obj-hist.corr-user-db-num
           and buf_c-order-doc.chip-num         = X_c-obj-hist.chip-num no-error.
    if not avail buf_c-order-doc then
    do:
      return error  "Неверная ссылка на c-order-doc в таблице c-order-head" .
    end.
    for first b_File no-lock where
              b_File._File-name = 'order-doc',
        each b_Field no-lock where
             b_Field._File-recid = recid(b_file):
      if can-do('"', b_Field._Field-name) then next.
      vListNameFields = substitute("&1&2&3",vListNameFields, if vListNameFields = "" then "" else ",", b_Field._Field-name).
      vIndexLabel = lookup(b_Field._Field-name, '"').
      vIndexMethod = lookup(b_Field._Field-name, 'sts,getStsOrder').
      vListLabelParam = substitute("&1&2&3&6&4&6&5",
                        vListLabelParam,
                        if vListLabelParam = "" then "" else chr(8),
                        b_Field._Field-name,
                        if vIndexLabel > 0 then entry(vIndexLabel + 1,'"') else b_Field._Label,
                        if vIndexMethod > 0 then entry(vIndexMethod + 1,'sts,getStsOrder') else "",
                        chr(4)).
    end.
    run proc-full-temp-changes in this-procedure
      (input buf_c-order-doc.action = integer('1':U)
      ,input buf_c-order-doc.action = integer('99':U)
      ,input buffer buf_c-order-doc:handle
      ,input "order-doc"
...
procedure order-doc-attr-proc :
  ...
  define variable vListNameFields as character no-undo.
  define variable vListLabelParam as character no-undo.
  define buffer buf_c-order-doc-attr for ub.c-order-doc-attr .
  define buffer b_File         for ub._File.
  define buffer b_Field        for ub._Field.
  do
  on error undo, return error return-value
  :
    find first buf_c-order-doc-attr no-lock where
               buf_c-order-doc-attr.corr-user-db-num = X_c-obj-hist.corr-user-db-num
           and buf_c-order-doc-attr.chip-num         = X_c-obj-hist.chip-num no-error.
    if not avail buf_c-order-doc-attr then
    do:
      return error  "Неверная ссылка на c-order-doc-attr в таблице c-order-head" .
    end.
    for first b_File no-lock where
              b_File._File-name = 'order-doc-attr',
        each b_Field no-lock where
             b_Field._File-recid = recid(b_file):
      if can-do('db-num,doc-code', b_Field._Field-name) then next.
      vListNameFields = substitute("&1&2&3",vListNameFields, if vListNameFields = "" then "" else ",", b_Field._Field-name).
      vIndexLabel = lookup(b_Field._Field-name, '"').
      vIndexMethod = lookup(b_Field._Field-name, '"').
      vListLabelParam = substitute("&1&2&3&6&4&6&5",
                        vListLabelParam,
                        if vListLabelParam = "" then "" else chr(8),
                        b_Field._Field-name,
                        if vIndexLabel > 0 then entry(vIndexLabel + 1,'"') else b_Field._Label,
                        if vIndexMethod > 0 then entry(vIndexMethod + 1,'"') else "",
                        chr(4)).
    end.
    run proc-full-temp-changes in this-procedure
      (input buf_c-order-doc-attr.action = integer('1':U)
      ,input buf_c-order-doc-attr.action = integer('99':U)
      ,input buffer buf_c-order-doc-attr:handle
      ,input "order-doc-attr"
      ,input vListNameFields
      ,input vListLabelParam).
  end.
end.
procedure order-line-proc :
  ...
  define variable vListNameFields as character no-undo.
  define variable vListLabelParam as character no-undo.
  define buffer buf_c-order-line for ub.c-order-line .
  define buffer b_File         for ub._File.
  define buffer b_Field        for ub._Field.
  do
  on error undo, return error return-value
  :
    find first buf_c-order-line no-lock where
               buf_c-order-line.corr-user-db-num = X_c-obj-hist.corr-user-db-num
           and buf_c-order-line.chip-num         = X_c-obj-hist.chip-num no-error.
    if not avail buf_c-order-line then
    do:
      return error  "Неверная ссылка на c-order-line в таблице c-order-head" .
    end.
    for first b_File no-lock where
              b_File._File-name = 'order-line',
        each b_Field no-lock where
             b_Field._File-recid = recid(b_file):
      if can-do('db-num,doc-code', b_Field._Field-name) then next.
      vListNameFields = substitute("&1&2&3",vListNameFields, if vListNameFields = "" then "" else ",", b_Field._Field-name).
    ...
    run proc-full-temp-changes in this-procedure
      (input buf_c-order-line.action = integer('1':U)
      ,input buf_c-order-line.action = integer('99':U)
      ,input buffer buf_c-order-line:handle
      ,input "order-line"
      ,input vListNameFields
      ,input vListLabelParam).
  end.
...
procedure order-line-attr-proc :
  ...
  define variable vListNameFields as character no-undo.
  define variable vListLabelParam as character no-undo.
  define buffer buf_c-order-line-attr for ub.c-order-line-attr .
  define buffer b_File         for ub._File.
  define buffer b_Field        for ub._Field.
  do
  on error undo, return error return-value
  :
    find first buf_c-order-line-attr no-lock where
               buf_c-order-line-attr.corr-user-db-num = X_c-obj-hist.corr-user-db-num
           and buf_c-order-line-attr.chip-num         = X_c-obj-hist.chip-num no-error.
    if not avail buf_c-order-line-attr then
    do:
      return error  "Неверная ссылка на c-order-line-attr в таблице c-order-head" .
    end.
    for first b_File no-lock where
              b_File._File-name = 'order-line-attr',
        each b_Field no-lock where
             b_Field._File-recid = recid(b_file):
      if can-do('db-num,doc-code', b_Field._Field-name) then next.
      vListNameFields = substitute("&1&2&3",vListNameFields, if vListNameFields = "" then "" else ",", b_Field._Field-name).
    ...
    run proc-full-temp-changes in this-procedure
      (input buf_c-order-line-attr.action = integer('1':U)
      ,input buf_c-order-line-attr.action = integer('99':U)
      ,input buffer buf_c-order-line-attr:handle
      ,input "order-line-attr"
      ,input vListNameFields
      ,input vListLabelParam).
  end.
end.
