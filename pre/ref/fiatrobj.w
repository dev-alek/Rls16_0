...
DO:
  IF dec( tt-shst.proch-ras:screen-value IN BROWSE b-shst) <> tt-shst.proch-ras
  or dec( tt-shst.proch:screen-value IN BROWSE b-shst) <> tt-shst.proch
  or dec( tt-shst.ostatok-start:screen-value IN BROWSE b-shst) <> tt-shst.Ostatok-start
  or dec( tt-shst.plan-pri:screen-value IN BROWSE b-shst) <> tt-shst.plan-pri
  THEN
    assign
      tt-shst.upd      = true
    .
  if dec( tt-shst.ostatok-start:screen-value IN BROWSE b-shst) <> tt-shst.Ostatok-start
  THEN
    assign
      tt-shst.upd1      = true
    .
  if dec( tt-shst.plan-pri:screen-value IN BROWSE b-shst) <> tt-shst.plan-pri
  THEN
    assign
      tt-shst.upd2      = true
    .
  if dec( tt-shst.proch:screen-value IN BROWSE b-shst) <> tt-shst.proch
  THEN
    assign
      tt-shst.upd3      = true
    .
  ...
  THEN
    ...
    .
END.
ON LEAVE OF tt-shst.proch-ras in browse b-shst
DO:
define variable sss  as character no-undo .
  IF dec(tt-shst.proch-ras:screen-value IN BROWSE b-shst) > 0  THEN do:
        MESSAGE "Расход дожен быть < 0 !!! "  .
        sss =  string(dec(tt-shst.proch-ras:screen-value IN BROWSE b-shst) * ( - 1 )) .
        tt-shst.proch-ras:screen-value IN BROWSE b-shst = sss.
        RETURN NO-APPLY.
  end.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse b-shst :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY b-shst FOR EACH tt-shst indexed-reposition.
END PROCEDURE.
...
if parhost-code = ? then do:
 for each buf_shop no-lock,
   first buf_clients where buf_clients.obj-type = 'маг':U       and
...
IF pARhost-code = ? THEN DO:
  FOR EACH buf_thbj-attr EXCLUSIVE-LOCK WHERE
          buf_thbj-attr.upper-prop-code = 'fin-plan':U
  ON error undo, RETURN error:
  ...
  for each tt-shst  where tt-shst.upd      = true
   ...
   if tt-shst.upd1      = true THEN DO:
     find FIRST thbjattr_thbj-attr WHERE
                thbjattr_thbj-attr.obj-type = tt-shst.obj-type
         AND    thbjattr_thbj-attr.obj-code = tt-shst.obj-code
         AND    thbjattr_thbj-attr.upper-prop-code = 'fin-plan':U
         AND    thbjattr_thbj-attr.prop-code = 'fin-ostatok-start':U.
     ASSIGN
   ...
   if tt-shst.upd2      = true THEN DO:
       find FIRST thbjattr_thbj-attr WHERE
           ...
           AND    thbjattr_thbj-attr.obj-code = tt-shst.obj-code
           AND    thbjattr_thbj-attr.upper-prop-code = 'fin-plan':U
           AND    thbjattr_thbj-attr.prop-code = 'fin-plan-pri':U.
       ASSIGN
   ...
   if tt-shst.upd3      = true THEN DO:
       find FIRST thbjattr_thbj-attr WHERE
           ...
           AND    thbjattr_thbj-attr.obj-code = tt-shst.obj-code
           AND    thbjattr_thbj-attr.upper-prop-code = 'fin-plan':U
           AND    thbjattr_thbj-attr.prop-code = 'fin-proch':U.
       ASSIGN
   ...
   if tt-shst.upd4      = true THEN DO:
       find FIRST thbjattr_thbj-attr WHERE
           ...
           AND    thbjattr_thbj-attr.obj-code = tt-shst.obj-code
           AND    thbjattr_thbj-attr.upper-prop-code = 'fin-plan':U
           AND    thbjattr_thbj-attr.prop-code = 'fin-proch-ras':U.
       ASSIGN
