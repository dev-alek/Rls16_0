...
define variable loc#log as logical no-undo .
  if available X_c-point-io then do:
      if can-do( p-rid-list, string( recid( X_c-point-io ) ) ) then do:
          p-rid-list = replace( p-rid-list, chr(44) + string( recid( X_c-point-io ) ), "") .
          p-rid-list = replace( p-rid-list, string( recid( X_c-point-io ) ) + chr(44), "") .
          p-rid-list = replace( p-rid-list, string( recid( X_c-point-io ) ), "") .
      end.
      else
      p-rid-list = p-rid-list + ( if p-rid-list = "" then "" else chr(44) ) + string( recid( X_c-point-io ) ) .
      loc#log = br-c-point-io:refresh() .
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
          loc#log = br-c-point-io:select-next-row ().
          apply "VALUE-CHANGED" to br-c-point-io in frame Dialog-Frame.
      end.
  ...
  apply "entry" to br-c-point-io in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-point-io :handle
      .
...
def var sort-labelbr-c-point-io   as character no-undo .
def var sort-clmnbr-c-point-io    as handle    no-undo .
def var cur-clmnbr-c-point-io     as handle    no-undo .
def var cur-clmn-locbr-c-point-io as integer   no-undo .
def var re-querybr-c-point-io     as logical   initial no no-undo .
on start-search, ctrl-o of br-c-point-io in frame Dialog-Frame do:
   run sort-brbr-c-point-io
     (input (if available X_c-point-io
             then recid(X_c-point-io)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-c-point-io :
  define input parameter p-recid as recid no-undo .
  if re-querybr-c-point-io = no then do:
    assign
       cur-clmnbr-c-point-io = br-c-point-io:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-c-point-io <> ? then sort-clmnbr-c-point-io:column-fgcolor = 0.
    if cur-clmnbr-c-point-io = sort-clmnbr-c-point-io then do:
      assign
         sort-labelbr-c-point-io = ""
         sort-clmnbr-c-point-io = ?
      .
     end.
     else do:
       assign
         sort-labelbr-c-point-io = cur-clmnbr-c-point-io:label
         sort-clmnbr-c-point-io  = cur-clmnbr-c-point-io
         sort-clmnbr-c-point-io:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-c-point-io = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-c-point-io:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-c-point-io then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-c-point-io = cur-clmn-locbr-c-point-io + 1
    .
  end.
  case sort-labelbr-c-point-io:
        when X_c-point-io.point-code:label in browse br-c-point-io then DO:    assign       sort-column-name = "X_c-point-io.point-code"     .     run OpenBr in this-procedure (input yes, input no, input no).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure (input yes, input no, input no).
      if sort-labelbr-c-point-io <> "" then do:
        assign
          cur-clmnbr-c-point-io:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-c-point-io = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-c-point-io to recid p-recid no-error.
    apply "value-changed" to br-c-point-io in frame Dialog-Frame.
  end.
  apply "entry" to br-c-point-io in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-c-point-io:
if cur-clmnbr-c-point-io = ? then do:
   run OpenBr in this-procedure (input yes, input no, input no).
end.
else do:
   assign re-querybr-c-point-io = yes.
   run sort-brbr-c-point-io
     (input (if available X_c-point-io
             then recid(X_c-point-io)
             else ?
   ...
   assign re-querybr-c-point-io = no.
end.
end.
MAIN-BLOCK:
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-13 as character no-undo .
      assign
      parameter-3-13 =
                              "FOR EACH X_c-point-io"
      parameter-4-13 =
        (
          if (" X_c-point-io.db-num = p-db-num AND X_c-point-io.point-code  = p-point-code " + " " + where-phrase-13) <> ""
          then  substitute(' X_c-point-io.db-num = &1 AND X_c-point-io.point-code  = &2 ', p-db-num, p-point-code)  + " " + where-phrase-13
          else "true"
        )
      parameter-5-13 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-13 =
          (" X_c-point-io.db-num = p-db-num AND X_c-point-io.point-code  = p-point-code " + " " + where-phrase-13 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-point-io:handle
  ...
  if l-filter-open-13 = false then do:
    OPEN QUERY br-c-point-io FOR EACH X_c-point-io
      where  X_c-point-io.db-num = p-db-num AND X_c-point-io.point-code  = p-point-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  assign
    v-doc-rec = recid( X_c-point-io )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-c-point-io:handle:get-buffer-handle(1) = (buffer X_c-point-io:handle) then do:
      assign
      parameter-2-13 = (if p-find-next then "true":u else "false":u )
      parameter-4-13 =
        "where ":u +  substitute(' X_c-point-io.db-num = &1 AND X_c-point-io.point-code  = &2 ', p-db-num, p-point-code)  + " ":u + where-phrase-13 + " ":u + p-find-condition + " " + ""
      parameter-5-13 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-c-point-io:handle
                          ,input rowid(X_c-point-io)
      ...
      assign
      parameter-2-13 = (if p-find-next then "true":u else "false":u )
      parameter-3-13 =  "FOR EACH X_c-point-io"
      parameter-4-13 =
        (
          if (" X_c-point-io.db-num = p-db-num AND X_c-point-io.point-code  = p-point-code " + " " + where-phrase-13) <> ""
          then  substitute(' X_c-point-io.db-num = &1 AND X_c-point-io.point-code  = &2 ', p-db-num, p-point-code)  + " " + where-phrase-13
...
else  REPOSITION br-c-point-io to row 7 No-ERROR.
run proc-view-changes in this-procedure no-error.
END PROCEDURE.
...
v-label-param =
 ...
 + "point-code" + chr(4) + "Номер" + chr(4) + "" + chr(8)
 + "point-name" + chr(4) + "Наименование" + chr(4) + "" + chr(8)
 + "point-type" + chr(4) + "Тип" + chr(4) + "" + chr(8)
 + "ps" + chr(4) + "Примечание" + chr(4) + "" + chr(8)
 + "is-default" + chr(4) + "По умолчанию" + chr(4) + "" + chr(8)
 + "address" + chr(4) + "Адрес" + chr(4) + "" + chr(8)
 + "dist" + chr(4) + "Километраж" + chr(4) + "" + chr(8)
 + "deliv-subj-code" + chr(4) + "Код субъекта доставки" + chr(4) + "" + chr(8)
 + "status_" + chr(4) + "Статус" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-point-io:handle
                                            ,input  'point-io':U
                                            ,input  "db-num,cli-type,cli-code,point-code,point-name,point-type,ps,is-defailt,address,dist,deliv-subj-code,status_"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
FUNCTION mark-string RETURNS CHARACTER
