...
DEFINE BROWSE br-condkeep
  QUERY br-condkeep NO-LOCK DISPLAY
      ...
      entry (lookup (string(X_c-condition-keeping.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
...
assign
  v-str-recid10 = trim( string( recid( X_c-condition-keeping ) , "->>>>>>>>>>>9":U ) )
...
do:
  v-doc-rec = recid(X_condition-keeping). run openbr in this-procedure. reposition br-condkeep to recid v-doc-rec no-error.               apply 'value-changed' to br-condkeep.
...
PROCEDURE sort-brbr-condkeep :
  ...
  case sort-labelbr-condkeep:
        when X_c-condition-keeping.cond-keep-name:label in browse br-condkeep then DO:   assign     sort-column-name = "X_c-condition-keeping.cond-keep-name"   .   run OpenBr.   . END.
        when X_c-condition-keeping.corr-date:label in browse br-condkeep then DO:   assign     sort-column-name = "X_c-condition-keeping.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
DEF VAR cur-clmn-numbr-condkeep as INT EXTENT 8 no-undo.
...
def var old-session as logical no-undo.
   ...
   DO jjbr-condkeep = NUM-ENTRIES('1,2,3,4,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-condkeep ( cur-clmn-numbr-condkeep[INTEGER(ENTRY (jjbr-condkeep, '1,2,3,4,5,6,7,8'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-condkeep do:
  RUN re-move-clmnbr-condkeep ( 1, 8).
END.
ON ctrl-cursor-left OF BROWSE br-condkeep do:
  RUN re-move-clmnbr-condkeep (8, 1).
END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-condkeep FOR EACH X_c-condition-keeping NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
  "sts" + chr(4) + "Статус" + chr(4) + "display-int-status" + chr(8)
 + "cond-keep-name" + chr(4) + "Название условий хранения" + chr(4) + "" + chr(8)
 + "cond-keep-code" + chr(4) + "Внутр.код" + chr(4) + "" + chr(8)
 + "des" + chr(4) + "Описание" + chr(4) + "" + chr(8)
 + "h-mode-from" + chr(4) + "Влажность от" + chr(4) + "" + chr(8)
 + "h-mode-to" + chr(4) + "Влажность до" + chr(4) + "" + chr(8)
 + "t-mode-from" + chr(4) + "Темп-ра от" + chr(4) + "" + chr(8)
 + "t-mode-to" + chr(4) + "Темп-ра до" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-condition-keeping:handle
                                            ,input  'condition-keeping':U
                                            ,input  "sts,cond-keep-name,cond-keep-code,des,h-mode-from,h-mode-to,t-mode-from,t-mode-to"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
