...
assign
  v-str-recid14 = trim( string( recid( X_c-fin-bank ) , "->>>>>>>>>>>9":U ) )
...
do:
  v-doc-rec = recid(X_c-fin-bank). run OpenBr in this-procedure ( input yes, input no, input '':U). reposition br-bank to recid v-doc-rec no-error. v-doc-rec = ?.
...
on start-search, ctrl-o of BR-bank in frame Dialog-Frame do:
   run sort-brBR-bank
     (input (if available X_c-fin-bank
             then recid(X_c-fin-bank)
             else ?
            )
     ).
end.
PROCEDURE sort-brBR-bank :
  ...
  case sort-labelBR-bank:
        when X_c-fin-bank.bank-name:label in browse BR-bank then DO:    assign       sort-column-name = "X_c-fin-bank.bank-name"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-fin-bank.bik:label in browse BR-bank then DO:    assign       sort-column-name = "X_c-fin-bank.bik"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-fin-bank.corr-date:label in browse BR-bank then DO:    assign       sort-column-name = "X_c-fin-bank.corr-date"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
DEF VAR cur-clmn-numbr-bank as INT EXTENT 9 no-undo.
...
def var old-session as logical no-undo.
   ...
   DO jjbr-bank = NUM-ENTRIES('1,2,3,4,5,6,7,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-bank ( cur-clmn-numbr-bank[INTEGER(ENTRY (jjbr-bank, '1,2,3,4,5,6,7,8,9'))] , 1).
   END.
       END.
       IF  p-mode = 'фирма':U  THEN DO:
   DO jjbr-bank = NUM-ENTRIES('1,3,4,5,6,7,8,9,2') TO 1 BY -1:
     RUN re-move-clmnbr-bank ( cur-clmn-numbr-bank[INTEGER(ENTRY (jjbr-bank, '1,3,4,5,6,7,8,9,2'))] , 1).
   END.
       END.
       IF  p-mode = 'one':U  THEN DO:
   DO jjbr-bank = NUM-ENTRIES('1,4,5,6,7,8,9,2,3') TO 1 BY -1:
     RUN re-move-clmnbr-bank ( cur-clmn-numbr-bank[INTEGER(ENTRY (jjbr-bank, '1,4,5,6,7,8,9,2,3'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-bank do:
  RUN re-move-clmnbr-bank ( 1, 9).
END.
ON ctrl-cursor-left OF BROWSE br-bank do:
  RUN re-move-clmnbr-bank (9, 1).
END.
...
title0 = "Список истории изменения банков" + chr(32).
...
if p-open-query then do:
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-bank FOR EACH X_c-fin-bank
      where  TRUE
      indexed-reposition
  .
      assign
  ...
  assign
    v-doc-rec = recid( X_c-fin-bank )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if QUERY br-bank:handle:get-buffer-handle(1) = (buffer X_c-fin-bank:handle) then do:
      assign
    ...
    .
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH X_c-fin-bank"
      parameter-4-27 =
        (
          if (" TRUE " + " " + where-phrase-27) <> ""
          then " TRUE " + " " + where-phrase-27
  ...
  run waitfram-hide in this-procedure .
    ...
    WHEN "one":u THEN DO:
      ...
      .
      if p-open-query then do:
        ASSIGN frame Dialog-Frame:TITLE = title0 + substitute(" Банк (&1): Фирма (&2) &3",
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-29 as character no-undo .
      ...
      parameter-4-29 =
        (
          if ("           X_c-fin-bank.host-code  = p-host-code AND X_c-fin-bank.code-bank = p-code-bank                         " + " " + where-phrase-29) <> ""
          then  substitute('X_c-fin-bank.host-code  = &1 AND X_c-fin-bank.code-bank = &2', p-host-code, p-code-bank)  + " " + where-phrase-29
          else "true"
        )
      parameter-5-29 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-29 =
          ("           X_c-fin-bank.host-code  = p-host-code AND X_c-fin-bank.code-bank = p-code-bank                         " + " " + where-phrase-29 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-bank:handle
  ...
  if l-filter-open-29 = false then do:
    OPEN QUERY br-bank FOR EACH X_c-fin-bank
      where            X_c-fin-bank.host-code  = p-host-code AND X_c-fin-bank.code-bank = p-code-bank
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-bank:handle:get-buffer-handle(1) = (buffer X_c-fin-bank:handle) then do:
      ...
      parameter-4-29 =
        "where ":u +  substitute('X_c-fin-bank.host-code  = &1 AND X_c-fin-bank.code-bank = &2', p-host-code, p-code-bank)  + " ":u + where-phrase-29 + " ":u + p-find-condition + " " + ""
      parameter-5-29 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-29 = (if p-find-next then "true":u else "false":u )
      parameter-3-29 =  "FOR EACH X_c-fin-bank"
      parameter-4-29 =
        (
          if ("           X_c-fin-bank.host-code  = p-host-code AND X_c-fin-bank.code-bank = p-code-bank                         " + " " + where-phrase-29) <> ""
          then  substitute('X_c-fin-bank.host-code  = &1 AND X_c-fin-bank.code-bank = &2', p-host-code, p-code-bank)  + " " + where-phrase-29
...
v-label-param =
  "addres" + chr(4) + "Адрес юридический" + chr(4) + "" + chr(8)
 + "addres1" + chr(4) + "Адрес почтовый" + chr(4) + "" + chr(8)
 + "bank-name" + chr(4) + "Название" + chr(4) + "" + chr(8)
 + "bank-city" + chr(4) + "Город" + chr(4) + "" + chr(8)
 + "cl-bank" + chr(4) + "Система Клиент-Банк" + chr(4) + "" + chr(8)
 + "bik" + chr(4) + "БИК" + chr(4) + "" + chr(8)
 + "code-bank" + chr(4) + "Код банка" + chr(4) + "" + chr(8)
 + "cor-acc" + chr(4) + "№ Корр.счета" + chr(4) + "" + chr(8)
 + "rkc" + chr(4) + "РКЦ" + chr(4) + "" + chr(8)
 ...
 + "licenz" + chr(4) + "Лицензия" + chr(4) + "" + chr(8)
 + "okato" + chr(4) + "ОКАТО" + chr(4) + "" + chr(8)
 + "okonx" + chr(4) + "ОКОНХ" + chr(4) + "" + chr(8)
 + "okpo" + chr(4) + "ОКПО" + chr(4) + "" + chr(8)
 + "otdel" + chr(4) + "Отделение" + chr(4) + "" + chr(8)
 + "phone" + chr(4) + "Телефон" + chr(4) + "" + chr(8)
 + "PS" + chr(4) + "Примечания" + chr(4) + "" + chr(8)
 + "short-name" + chr(4) + "Укороченное название" + chr(4) + "" + chr(8)
 + "status_" + chr(4) + "Статус" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-fin-bank:handle
                                            ,input  'fin-bank':U
                                            ,input  "addres,addres1,bank-name,bank-city,cl-bank,bik,code-bank,cor-acc,rkc,e-mail,fax,host-code,inn,kpp,licenz,okato,okonx,okpo,otdel,phone,PS,short-name,status_"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
