...
DO:
  ...
  for first goods no-lock where goods.gds-code = ext-classif.key#_one:
    gdsrec = recid(goods).
    run ref/gds-form.w (parparentproc, 'ПРОСМОТР':U, v-cntxt-obj-type, v-cntxt-obj-code, input this-procedure:handle, input-output gdsrec).
  end.
...
DO:
   find first ub.ext-classif no-lock  where
                                        ...
                                        and  ub.ext-classif.classif-subject = 'goods':U
                                        and  ub.ext-classif.classif-name = 'exp-esys-gds-code':U no-error.
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-28 as character no-undo .
      ...
      parameter-4-28 =
        (
          if (" ext-classif.classif-subject = 'goods':U                         and ext-classif.classif-name = 'exp-esys-gds-code':U" + " " + where-phrase-28) <> ""
          then  substitute('ext-classif.classif-subject = &1&2&1                         and ext-classif.classif-name = &1&3&1 ', chr(34), 'goods':U, 'exp-esys-gds-code':U) + " " + where-phrase-28
          else "true"
        )
      parameter-5-28 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-28 =
          (" ext-classif.classif-subject = 'goods':U                         and ext-classif.classif-name = 'exp-esys-gds-code':U" + " " + where-phrase-28 = "")
      .
  ...
  if l-filter-open-28 = false then do:
    OPEN QUERY br-esys-gds-query FOR EACH ext-classif
      where  ext-classif.classif-subject = 'goods':U                         and ext-classif.classif-name = 'exp-esys-gds-code':U
      INDEXED-REPOSITION
  .
      assign
        l-query-was-opened = true
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-30 as character no-undo .
      ...
      parameter-4-30 =
        (
          if (" ext-classif.classif-subject = 'goods':U                           and ext-classif.classif-name = 'exp-esys-gds-code':U                           AND ext-classif.key#_two = p-esys-id" + " " + where-phrase-30) <> ""
          then  substitute('ext-classif.classif-subject = &1&2&1                           and ext-classif.classif-name = &1&4&1                           AND ext-classif.key#_two = &3', chr(34), 'goods':U, p-esys-id, 'exp-esys-gds-code':U) + " " + where-phrase-30
          else "true"
        )
      parameter-5-30 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-30 =
          (" ext-classif.classif-subject = 'goods':U                           and ext-classif.classif-name = 'exp-esys-gds-code':U                           AND ext-classif.key#_two = p-esys-id" + " " + where-phrase-30 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-esys-gds-query:handle
  ...
  if l-filter-open-30 = false then do:
    OPEN QUERY br-esys-gds-query FOR EACH ext-classif
      where  ext-classif.classif-subject = 'goods':U                           and ext-classif.classif-name = 'exp-esys-gds-code':U                           AND ext-classif.key#_two = p-esys-id
      INDEXED-REPOSITION
  .
      assign
        l-query-was-opened = true
...
PROCEDURE proc-b-print :
    ...
    report:table-header("Внешняя система|Код внешней системы|Код товара|Код товара во внешней системе|Наименование товара","40","4").
    for each b-ext-classif no-lock where b-ext-classif.classif-subject = 'goods':U and b-ext-classif.classif-name = 'exp-esys-gds-code':U by b-ext-classif.key#_two:
