...
if v-value = "no"  then do:
  ...
  if p-mode = 'ДОБАВЛЕНИЕ':U then do:
    ...
    run grplib-get-full-name in this-procedure (
                                               ...
                                               ,output v-full-name).
    assign
    v-full-name = v-full-name + chr(47) + p-node-name + chr(47)
    .
  end.
  CASE p-calc-method:
    ...
    otherwise do:
     run str/pr-listv.p (
                     input 'Учетная,Учет-резерв,Накладная,Накл-безНДС,Учет-безНДС,Учет+накл,Уч+накл-НДС,Не-считать,Производит,Произв-НДС,ПорогПр-НДС,ПорогПр+НДС,Спецификация':U
                   , "":U
                   , output v-list) .
  ...
  CASE p-round-method:
    ...
    otherwise do:
      if LOOKUP(p-round-method, '9-окончание,9-99окончание,Без-дробных,Произвольно,Вверх,Коэффициент,Отключено':U) = 0 then do:
        assign
  ...
  END CASE.
  if lookup(p-round-method, 'Произвольно,Вверх,Коэффициент,9-99окончание':U) > 0 and
  p-base = 0 then do:
    assign
  ...
  if error-status:error or v-error-message <> "":U then do:
    assign
    v-error-message = substitute("Группа &1&2&3", p-node-name, chr(10), v-error-message).
    run err-mess in this-procedure ( input-output v-error-message).
    undo main-block, return error (if p-silent then v-error-message else "node-name").
  end.
  ...
  if p-fill-tax-from-upper then do:
    run ref/dtaxgrps.p (
                   input (if p-mode = 'ДОБАВЛЕНИЕ':U then 0 else p-node-code)
                  ,input p-upper-code
                  ,input 0
                  ,input "":U
                  ,input 0
