...
END.
ON CHOOSE OF B-act IN FRAME Dialog-Frame
DO:
...
assign
  v-str-recid1 = trim( string( recid( x-criterion-analysis ) , "->>>>>>>>>>>9":U ) )
  v-num-entry1 = lookup( v-str-recid1 , p-rid-list )
.
if v-num-entry1 > 0 then do:
  assign
    entry( v-num-entry1, p-rid-list ) = "":U
    p-rid-list = trim( replace( p-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    p-rid-list = p-rid-list + ( if p-rid-list = "":U then "":U else chr(44) ) + v-str-recid1
  .
end.
    loc#log = BROWSE-5:refresh() .
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = BROWSE-5:select-next-row ().
        apply "VALUE-CHANGED" to BROWSE-5 in frame Dialog-Frame.
    end.
  ...
  apply "entry" to BROWSE-5 in frame Dialog-Frame.
END.
ON CHOOSE OF B-not-act IN FRAME Dialog-Frame
DO:
find current x-criterion-analysis exclusive-lock no-error .
if available x-criterion-analysis then do:
...
DO:
  APPLY "CHOOSE":U TO B-sel in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BROWSE-5 :handle
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  x-criterion-analysis.cral-id:READ-ONLY IN browse BROWSE-5      = TRUE.
  run enable_ui.
  run my_enable .
  wait-for go of frame Dialog-Frame focus BROWSE-5 .
end.
run disable_ui.
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH x-criterion-analysis SHARE-LOCK.
  ...
  OPEN QUERY BROWSE-5 FOR EACH x-criterion-analysis NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE my_enable :
DISPLAY mark-num
      WITH FRAME Dialog-Frame.
