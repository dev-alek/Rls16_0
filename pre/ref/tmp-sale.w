...
DO:
    run ref/tmpsalei.w
      ( 'ДОБАВЛЕНИЕ':U, input-output rr ).
    if rr <> ? then
        do:
            OPEN QUERY br-tmp-sale FOR EACH ub.tmp-sale NO-LOCK     BY ub.tmp-sale.tmp-code INDEXED-REPOSITION.
            reposition br-tmp-sale to recid rr.
            log-res  = br-tmp-sale:select-focused-row( ).
            apply "ENTRY":U to br-tmp-sale.
            apply "home"  to br-tmp-sale.
        end.
END.
ON CHOOSE OF b-chg IN FRAME d-type-tmp
DO:
    if not available ub.tmp-sale THEN
        return no-apply.
    rr = recid( ub.tmp-sale ).
    run ref/tmpsalei.w
    ( 'ИЗМЕНЕНИЕ':U, input-output rr ).
    OPEN QUERY br-tmp-sale FOR EACH ub.tmp-sale NO-LOCK     BY ub.tmp-sale.tmp-code INDEXED-REPOSITION.
    reposition br-tmp-sale to recid rr .
END.
ON CHOOSE OF b-exit IN FRAME d-type-tmp
DO:
  rid-list = "" .
END.
...
assign
  v-str-recid6 = trim( string( recid( ub.tmp-sale ) , "->>>>>>>>>>>9":U ) )
  v-num-entry6 = lookup( v-str-recid6 , rid-list )
.
if v-num-entry6 > 0 then do:
  assign
    entry( v-num-entry6, rid-list ) = "":U
    rid-list = trim( replace( rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    rid-list = rid-list + ( if rid-list = "":U then "":U else chr(44) ) + v-str-recid6
  .
end.
   g#log = br-tmp-sale  :refresh( ) in frame d-type-tmp.
    if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
            g#log = br-tmp-sale:select-next-row () in frame d-type-tmp.
            apply "iteration-changed" to br-tmp-sale in frame d-type-tmp.
    end.
    ...
    apply "entry" to br-tmp-sale in frame d-type-tmp.
END.
...
DO:
    ...
    DEFINE FRAME List
        ...
        ub.tmp-sale.tmp-code column-label 'код':U format "9999"
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-tmp-sale :handle
      .
...
PROCEDURE enable_UI :
    ...
    if available ub.tmp-sale then
        log-res  = br-tmp-sale:select-focused-row( ) in frame d-type-tmp.
     OPEN QUERY br-tmp-sale FOR EACH ub.tmp-sale NO-LOCK     BY ub.tmp-sale.tmp-code INDEXED-REPOSITION.
END PROCEDURE.
