...
cb-mark-type:delete(1) in frame Dialog-Frame.
do vi = 1 to MarkType:GetItemByLab(vi):
    objType  = ObjSrv:Env:Marking:Types:CurrProp.
    cb-mark-type:add-last(objType:Label_,objType:NameProp) in frame Dialog-Frame.
end.
if p-mode = 'ДОБАВЛЕНИЕ':U OR p-mode = 'ИЗМЕНЕНИЕ':U then do:
    enable cb-mark-type with frame Dialog-Frame.
end.
p-setted = false.
cb-mark-type = p-attr-value.
display cb-mark-type with frame Dialog-Frame.
end procedure.
PROCEDURE proc-save :
