...
define variable v-param-type16 as character no-undo .
define variable v-value-date16 as date no-undo .
define variable v-value-decimal16 as decimal no-undo .
define variable v-value-integer16 as INTEGER no-undo .
define variable v-value-logical16 AS LOGICAL no-undo .
define variable v-tth16 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  p-obj-type
    ,input  p-obj-code
    ...
    ,input  'dflt-cd':U
    ,output p-pos-type
    ,output v-value-date16
    ,output v-value-decimal16
    ,output v-value-integer16
    ,output v-value-logical16
    ,output v-param-type16
    ,INPUT-OUTPUT table-handle v-tth16
    )  .
delete object v-tth16 no-error.
    end.
    find first buf_dis-rule no-lock where
...
DEFINE BROWSE BR-dis-gds-rule
  QUERY BR-dis-gds-rule NO-LOCK DISPLAY
      mark-string(recid(X_dis-gds-rule), v-rid-list) COLUMN-LABEL '*' FORMAT "X(1)"
X_dis-gds-rule.gds-code COLUMN-LABEL 'Код товара'
X_goods.gds-name COLUMN-LABEL 'Название' FORMAT "X(25)"
X_goods.artic COLUMN-LABEL 'Артикул'
(X_goods.prod-type + string(X_goods.prod-code)) COLUMN-LABEL 'Пр-ль' FORMAT "X(12)"
X_dis-gds-rule.nonunique COLUMN-LABEL 'Детализ.' FORMAT "X(9)"
X_dis-gds-rule.templ-rl-root COLUMN-LABEL 'Тип правила' FORMAT ">>>>>>>>9"
entry (lookup (X_dis-gds-rule.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U) COLUMN-LABEL 'Место использ.' FORMAT "X(10)"
X_dis-gds-rule.obj-type COLUMN-LABEL 'Тип объ' FORMAT "X(3)"
X_dis-gds-rule.obj-code COLUMN-LABEL 'Код объ' FORMAT ">>>>>>>>9"
entry (lookup (X_dis-gds-rule.discnt-role, 'std-disc,abs-disc,pcnt-tot,temp-disc,pcnt-kat,pcnt-qnty,pcnt-date,without-disc,without-gds-disc,dis-tot-flag,max-disc,bonus-qnty':u) + 1, ',' + 'Стандартная скидка,Абсолютная скидка,% скидка c суммы,Временная скидка,Категорийная скидка,Количественная скидка,Скидка по дате,Запрет на участие в бонусных программах\участие в скидке на итог,Запрет скидки на товар,Участие в итогах по ДК,Порог max скидки на товар,Начисление бонусов на кол-во товара':u) COLUMN-LABEL 'Тип скидки' FORMAT "X(70)"
X_dis-gds-rule.rule-num COLUMN-LABEL '№ правила' FORMAT ">>>>>>>>9"
X_dis-gds-rule.rule-num COLUMN-LABEL '№ корн.!правила' FORMAT ">>>>>>>>9"
...
or p-curr-obj-type = 'скл':U then do:
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-dis-gds-rule :handle
      .
...
do:
   v-doc-rec = recid(X_dis-gds-rule).  Run OpenBR in this-procedure ( input yes, input no, input '':U).  REPOSITION br-dis-gds-rule to recid v-doc-rec No-ERROR.               apply 'value-changed' to br-dis-gds-rule.
...
on F9 of frame Dialog-Frame anywhere do:
  ...
  apply "entry" to br-dis-gds-rule in frame Dialog-Frame.
  return no-apply.
end.
on SHIFT-F9 of frame Dialog-Frame anywhere do:
  if not available X_goods then
    return no-apply.
...
def var sort-labelbr-dis-gds-rule   as character no-undo .
def var sort-clmnbr-dis-gds-rule    as handle    no-undo .
def var cur-clmnbr-dis-gds-rule     as handle    no-undo .
def var cur-clmn-locbr-dis-gds-rule as integer   no-undo .
def var re-querybr-dis-gds-rule     as logical   initial no no-undo .
on start-search, ctrl-o of br-dis-gds-rule in frame Dialog-Frame do:
   run sort-brbr-dis-gds-rule
     (input (if available X_dis-gds-rule
             then recid(X_dis-gds-rule)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-dis-gds-rule :
  define input parameter p-recid as recid no-undo .
  if re-querybr-dis-gds-rule = no then do:
    assign
       cur-clmnbr-dis-gds-rule = br-dis-gds-rule:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-dis-gds-rule <> ? then sort-clmnbr-dis-gds-rule:column-fgcolor = 0.
    if cur-clmnbr-dis-gds-rule = sort-clmnbr-dis-gds-rule then do:
      assign
         sort-labelbr-dis-gds-rule = ""
         sort-clmnbr-dis-gds-rule = ?
      .
     end.
     else do:
       assign
         sort-labelbr-dis-gds-rule = cur-clmnbr-dis-gds-rule:label
         sort-clmnbr-dis-gds-rule  = cur-clmnbr-dis-gds-rule
         sort-clmnbr-dis-gds-rule:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-dis-gds-rule = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-dis-gds-rule:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-dis-gds-rule then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-dis-gds-rule = cur-clmn-locbr-dis-gds-rule + 1
    .
  end.
  case sort-labelbr-dis-gds-rule:
        when '*'  then DO:    assign       sort-column-name = "mark-string(recid(X_dis-gds-rule), v-rid-list)"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Код товара'  then DO:    assign       sort-column-name = "X_dis-gds-rule.gds-code"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Название'  then DO:    assign       sort-column-name = "X_goods.gds-name"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Артикул'  then DO:    assign       sort-column-name = "X_goods.artic"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Пр-ль'  then DO:    assign       sort-column-name = "(X_goods.prod-type + string(X_goods.prod-code))"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Тип правила'  then DO:    assign       sort-column-name = "X_dis-gds-rule.templ-rl-root"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Место использ.'  then DO:    assign       sort-column-name = "entry (lookup (X_dis-gds-rule.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U)"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Тип объ'  then DO:    assign       sort-column-name = "X_dis-gds-rule.obj-type"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Код объ'  then DO:    assign       sort-column-name = "X_dis-gds-rule.obj-code"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Тип скидки'  then DO:    assign       sort-column-name = "entry (lookup (X_dis-gds-rule.discnt-role, 'std-disc,abs-disc,pcnt-tot,temp-disc,pcnt-kat,pcnt-qnty,pcnt-date,without-disc,without-gds-disc,dis-tot-flag,max-disc,bonus-qnty':u) + 1, ',' + 'Стандартная скидка,Абсолютная скидка,% скидка c суммы,Временная скидка,Категорийная скидка,Количественная скидка,Скидка по дате,Запрет на участие в бонусных программах\участие в скидке на итог,Запрет скидки на товар,Участие в итогах по ДК,Порог max скидки на товар,Начисление бонусов на кол-во товара':u)"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when '№ правила'  then DO:    assign       sort-column-name = "X_dis-gds-rule.rule-num"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when '№ корн.!правила'  then DO:    assign       sort-column-name = "X_dis-gds-rule.rule-num"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
        when 'Детализ.'  then DO:    assign       sort-column-name = "X_dis-gds-rule.nonunique"     .     run OpenBr in this-procedure ( input yes, input no, input no).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input no).
      if sort-labelbr-dis-gds-rule <> "" then do:
        assign
          cur-clmnbr-dis-gds-rule:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-dis-gds-rule = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-dis-gds-rule to recid p-recid no-error.
    apply "value-changed" to br-dis-gds-rule in frame Dialog-Frame.
  end.
  apply "entry" to br-dis-gds-rule in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-dis-gds-rule:
if cur-clmnbr-dis-gds-rule = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input no).
end.
else do:
   assign re-querybr-dis-gds-rule = yes.
   run sort-brbr-dis-gds-rule
     (input (if available X_dis-gds-rule
             then recid(X_dis-gds-rule)
             else ?
   ...
   assign re-querybr-dis-gds-rule = no.
end.
end.
define variable vss-include-info25 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-dis-gds-rule :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
define variable vss-include-info26 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  IF LOOKUP( "discnt-role", p-list-mode) > 0 THEN DO:
    FIND FIRST par_dis-cfg-rule NO-LOCK WHERE
          ...
          AND par_dis-cfg-rule.TABLE-name = 'dis-gds-rule':U NO-ERROR.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-dis-gds-rule FOR EACH X_dis-gds-rule NO-LOCK ,            FIRST X_goods NO-LOCK WHERE           X_goods.gds-code = X_dis-gds-rule.gds-code INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
define variable v-h as handle no-undo .
define variable v-ii as integer no-undo .
v-h = BR-dis-gds-rule:FIRST-COLUMN IN FRAME Dialog-Frame.
DO while valid-handle(v-h)
and  v-ii < br-dis-gds-rule:num-COLUMNs IN FRAME Dialog-Frame:
  v-ii = v-ii + 1.
  if v-h:LABEL = 'Тип правила'
  or v-h:LABEL = 'Тип скидки'
  then do:
    v-h:RESIZABLE = YES.
  end.
  ELSE DO:
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-29 as character no-undo .
      ...
      parameter-4-29 =
        (
          if (" X_dis-gds-rule.rule-num > 0 " + " " + where-phrase-29) <> ""
          then " X_dis-gds-rule.rule-num > 0 " + " " + where-phrase-29
          else "true"
        )
      parameter-5-29 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-29 =
          (" X_dis-gds-rule.rule-num > 0 " + " " + where-phrase-29 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-29 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where  X_dis-gds-rule.rule-num > 0
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  assign
    v-doc-rec = recid( X_dis-gds-rule )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      assign
      parameter-2-29 = (if p-find-next then "true":u else "false":u )
      parameter-4-29 =
        "where ":u + " X_dis-gds-rule.rule-num > 0 " + " ":u + where-phrase-29 + " ":u + p-find-condition + " " + ""
      parameter-5-29 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
                          ,input rowid(X_dis-gds-rule)
                          ,input logical(parameter-2-29)
                          ,input no-lock
                          ,input (buffer X_dis-gds-rule:handle)
                          ,input parameter-4-29
                          ,input parameter-5-29
                          ) no-error.
      .
      ...
      assign
      parameter-2-29 = (if p-find-next then "true":u else "false":u )
      parameter-3-29 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-29 =
        (
          if (" X_dis-gds-rule.rule-num > 0 " + " " + where-phrase-29) <> ""
          then " X_dis-gds-rule.rule-num > 0 " + " " + where-phrase-29
      ...
      parameter-5-29 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-29 = if sort-phrase-29 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if (" X_dis-gds-rule.rule-num > 0                     AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code " + " " + where-phrase-31) <> ""
          then  substitute('X_dis-gds-rule.rule-num > 0                     AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3 ', chr(34), p-curr-obj-type, p-curr-obj-code) + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-31 =
          (" X_dis-gds-rule.rule-num > 0                     AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where  X_dis-gds-rule.rule-num > 0                     AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-31 =
        "where ":u +  substitute('X_dis-gds-rule.rule-num > 0                     AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3 ', chr(34), p-curr-obj-type, p-curr-obj-code) + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-31 = (if p-find-next then "true":u else "false":u )
      parameter-3-31 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-31 =
        (
          if (" X_dis-gds-rule.rule-num > 0                     AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code " + " " + where-phrase-31) <> ""
          then  substitute('X_dis-gds-rule.rule-num > 0                     AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3 ', chr(34), p-curr-obj-type, p-curr-obj-code) + " " + where-phrase-31
      ...
      parameter-5-31 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-31 = if sort-phrase-31 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-33 as character no-undo .
      ...
      parameter-4-33 =
        (
          if (" X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.pos-type = p-pos-type " + " " + where-phrase-33) <> ""
          then  substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &2                       AND X_dis-gds-rule.pos-type = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-pos-type) + " " + where-phrase-33
          else "true"
        )
      parameter-5-33 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-33 =
          (" X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.pos-type = p-pos-type " + " " + where-phrase-33 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-33 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.pos-type = p-pos-type
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-33 =
        "where ":u +  substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &2                       AND X_dis-gds-rule.pos-type = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-pos-type) + " ":u + where-phrase-33 + " ":u + p-find-condition + " " + ""
      parameter-5-33 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-33 = (if p-find-next then "true":u else "false":u )
      parameter-3-33 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-33 =
        (
          if (" X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.pos-type = p-pos-type " + " " + where-phrase-33) <> ""
          then  substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &2                       AND X_dis-gds-rule.pos-type = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-pos-type) + " " + where-phrase-33
      ...
      parameter-5-33 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-33 = if sort-phrase-33 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-35 as character no-undo .
      ...
      parameter-4-35 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-35) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3                       AND X_dis-gds-rule.templ-rl-root = &4 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " " + where-phrase-35
          else "true"
        )
      parameter-5-35 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-35 =
          ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-35 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-35 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where   X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-35 =
        "where ":u +   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3                       AND X_dis-gds-rule.templ-rl-root = &4 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " ":u + where-phrase-35 + " ":u + p-find-condition + " " + ""
      parameter-5-35 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-35 = (if p-find-next then "true":u else "false":u )
      parameter-3-35 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-35 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-35) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3                       AND X_dis-gds-rule.templ-rl-root = &4 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-templ-rl-root) + " " + where-phrase-35
      ...
      parameter-5-35 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-35 = if sort-phrase-35 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-37 as character no-undo .
      ...
      parameter-4-37 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.discnt-role = p-discnt-role " + " " + where-phrase-37) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3                       AND X_dis-gds-rule.discnt-role = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " " + where-phrase-37
          else "true"
        )
      parameter-5-37 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-37 =
          ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.discnt-role = p-discnt-role " + " " + where-phrase-37 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-37 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where   X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.discnt-role = p-discnt-role
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-37 =
        "where ":u +   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3                       AND X_dis-gds-rule.discnt-role = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " ":u + where-phrase-37 + " ":u + p-find-condition + " " + ""
      parameter-5-37 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-37 = (if p-find-next then "true":u else "false":u )
      parameter-3-37 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-37 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = p-curr-obj-type and X_dis-gds-rule.obj-code = p-curr-obj-code                       AND X_dis-gds-rule.discnt-role = p-discnt-role " + " " + where-phrase-37) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.obj-type = &1&2&1 and X_dis-gds-rule.obj-code = &3                       AND X_dis-gds-rule.discnt-role = &1&4&1 ', chr(34), p-curr-obj-type, p-curr-obj-code, p-discnt-role) + " " + where-phrase-37
      ...
      parameter-5-37 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-37 = if sort-phrase-37 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-39 as character no-undo .
      ...
      parameter-4-39 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-39) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.templ-rl-root = &1', p-templ-rl-root ) + " " + where-phrase-39
          else "true"
        )
      parameter-5-39 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-39 =
          ("  X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-39 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-39 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where   X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-39 =
        "where ":u +   substitute('X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.templ-rl-root = &1', p-templ-rl-root ) + " ":u + where-phrase-39 + " ":u + p-find-condition + " " + ""
      parameter-5-39 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-39 = (if p-find-next then "true":u else "false":u )
      parameter-3-39 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-39 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.templ-rl-root = p-templ-rl-root " + " " + where-phrase-39) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.templ-rl-root = &1', p-templ-rl-root ) + " " + where-phrase-39
      ...
      parameter-5-39 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-39 = if sort-phrase-39 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-41 as character no-undo .
      ...
      parameter-4-41 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.rule-num = p-rule-num " + " " + where-phrase-41) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.rule-num = &1', p-rule-num ) + " " + where-phrase-41
          else "true"
        )
      parameter-5-41 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-41 =
          ("  X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.rule-num = p-rule-num " + " " + where-phrase-41 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-41 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where   X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.rule-num = p-rule-num
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-41 =
        "where ":u +   substitute('X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.rule-num = &1', p-rule-num ) + " ":u + where-phrase-41 + " ":u + p-find-condition + " " + ""
      parameter-5-41 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-41 = (if p-find-next then "true":u else "false":u )
      parameter-3-41 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-41 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.rule-num = p-rule-num " + " " + where-phrase-41) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                         AND X_dis-gds-rule.rule-num = &1', p-rule-num ) + " " + where-phrase-41
      ...
      parameter-5-41 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-41 = if sort-phrase-41 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-43 as character no-undo .
      ...
      parameter-4-43 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.rl-root = p-rule-num " + " " + where-phrase-43) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.rl-root = &1', p-rule-num ) + " " + where-phrase-43
          else "true"
        )
      parameter-5-43 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-43 =
          ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.rl-root = p-rule-num " + " " + where-phrase-43 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-43 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where   X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.rl-root = p-rule-num
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-43 =
        "where ":u +   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.rl-root = &1', p-rule-num ) + " ":u + where-phrase-43 + " ":u + p-find-condition + " " + ""
      parameter-5-43 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-43 = (if p-find-next then "true":u else "false":u )
      parameter-3-43 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-43 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.rl-root = p-rule-num " + " " + where-phrase-43) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                       AND X_dis-gds-rule.rl-root = &1', p-rule-num ) + " " + where-phrase-43
      ...
      parameter-5-43 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-43 = if sort-phrase-43 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-45 as character no-undo .
      ...
      parameter-4-45 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.pos-type = p-pos-type " + " " + where-phrase-45) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.pos-type = &1&2&1', chr(34), p-pos-type ) + " " + where-phrase-45
          else "true"
        )
      parameter-5-45 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-45 =
          ("  X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.pos-type = p-pos-type " + " " + where-phrase-45 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-45 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where   X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.pos-type = p-pos-type
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-45 =
        "where ":u +   substitute('X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.pos-type = &1&2&1', chr(34), p-pos-type ) + " ":u + where-phrase-45 + " ":u + p-find-condition + " " + ""
      parameter-5-45 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-45 = (if p-find-next then "true":u else "false":u )
      parameter-3-45 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-45 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.pos-type = p-pos-type " + " " + where-phrase-45) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.pos-type = &1&2&1', chr(34), p-pos-type ) + " " + where-phrase-45
      ...
      parameter-5-45 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-45 = if sort-phrase-45 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-47 as character no-undo .
      ...
      parameter-4-47 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.discnt-role = p-discnt-role " + " " + where-phrase-47) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.discnt-role = &1&2&1', chr(34), p-discnt-role ) + " " + where-phrase-47
          else "true"
        )
      parameter-5-47 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code")
    ...
    :
      assign
        l-disable-where-47 =
          ("  X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.discnt-role = p-discnt-role " + " " + where-phrase-47 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-dis-gds-rule:HANDLE
  ...
  if l-filter-open-47 = false then do:
    OPEN QUERY br-dis-gds-rule FOR EACH X_dis-gds-rule no-lock
      where   X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.discnt-role = p-discnt-role
    , FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code
      indexed-reposition
  .
      assign
  ...
  :
    if QUERY br-dis-gds-rule:HANDLE:get-buffer-handle(1) = (buffer X_dis-gds-rule:handle) then do:
      ...
      parameter-4-47 =
        "where ":u +   substitute('X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.discnt-role = &1&2&1', chr(34), p-discnt-role ) + " ":u + where-phrase-47 + " ":u + p-find-condition + " " + ""
      parameter-5-47 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-47 = (if p-find-next then "true":u else "false":u )
      parameter-3-47 =  "FOR EACH X_dis-gds-rule no-lock"
      parameter-4-47 =
        (
          if ("  X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.discnt-role = p-discnt-role " + " " + where-phrase-47) <> ""
          then   substitute('X_dis-gds-rule.rule-num > 0                           AND X_dis-gds-rule.discnt-role = &1&2&1', chr(34), p-discnt-role ) + " " + where-phrase-47
      ...
      parameter-5-47 = (" " + "" + " " + ", FIRST X_goods no-lock where X_goods.gds-code = X_dis-gds-rule.gds-code" + " " + p-find-condition)
      parameter-6-47 = if sort-phrase-47 = ''
                           then
        (
        " " + "  " +
...
define variable v-str-recid49 as character no-undo .
define variable v-num-entry49 as integer   no-undo .
assign
  v-str-recid49 = trim( string( recid( X_dis-gds-rule ) , "->>>>>>>>>>>9":U ) )
  v-num-entry49 = lookup( v-str-recid49 , v-rid-list )
.
if v-num-entry49 > 0 then do:
  assign
    entry( v-num-entry49, v-rid-list ) = "":U
    v-rid-list = trim( replace( v-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rid-list = v-rid-list + ( if v-rid-list = "":U then "":U else chr(44) ) + v-str-recid49
  .
end.
  loc#log = br-dis-gds-rule:refresh() IN FRAME Dialog-Frame .
...
X_dis-gds-rule.templ-rl-root COLUMN-LABEL 'Тип правила' FORMAT ">>>>>>>>9"
v-pos-type COLUMN-LABEL 'Место использ.' FORmat "X(20)"
X_dis-gds-rule.obj-type COLUMN-LABEL 'Тип объ' FORMAT "X(3)"
X_dis-gds-rule.obj-code COLUMN-LABEL 'Код объ' FORMAT ">>>>>>>>9"
v-d-name COLUMN-LABEL 'Тип скидки' FORMAT "X(60)"
X_dis-gds-rule.rule-num COLUMN-LABEL '№ правила' FORMAT ">>>>>>>>9"
X_dis-gds-rule.rule-num COLUMN-LABEL '№ корн.!правила' FORMAT ">>>>>>>>9"
HEADER
...
DO WHILE available X_dis-gds-rule :
  ...
  mark-string(recid(X_dis-gds-rule), v-rid-list) @ v-mark
  X_dis-gds-rule.gds-code
  X_goods.gds-name
  X_goods.artic
  (X_goods.prod-type + string(X_goods.prod-code)) @ v-prod
  X_dis-gds-rule.templ-rl-root
  entry (lookup (X_dis-gds-rule.pos-type, 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,InfoKiosk,pricecheck-Servis+,Emulator-NKT-IBM,MARIA,-,bo,Autotank':U), 'IBM,IBM-XML,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,R-KEEPER,InfoKiosk,Прайс-чекер Servis+,Emulator-NKT-IBM,MARIA,Накладная,Бэкофис,Autotank':U) @ v-pos-type
  X_dis-gds-rule.obj-type
  X_dis-gds-rule.obj-code
  entry (lookup (p-discnt-role, 'std-disc,abs-disc,pcnt-tot,temp-disc,pcnt-kat,pcnt-qnty,pcnt-date,without-disc,without-gds-disc,dis-tot-flag,max-disc,bonus-qnty':u) + 1, ',' + 'Стандартная скидка,Абсолютная скидка,% скидка c суммы,Временная скидка,Категорийная скидка,Количественная скидка,Скидка по дате,Запрет на участие в бонусных программах\участие в скидке на итог,Запрет скидки на товар,Участие в итогах по ДК,Порог max скидки на товар,Начисление бонусов на кол-во товара':u) @ v-d-name
  X_dis-gds-rule.rule-num
  X_dis-gds-rule.rule-num
  with frame List .
