...
DEFINE BROWSE br-tares
  QUERY br-tares NO-LOCK DISPLAY
      ...
      entry (lookup (STRING(X_tare.stts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
  ENABLE
      X_tare.tare-name
...
if loc-doc-rec <> ? THEN DO:
  ...
  if error-status:error then do:                           find first pos_tare no-lock where                                   recid(pos_tare) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ТАРЫ" skip                            string(if avail pos_tare                                     then  substitute("Код тары: &1"                                                     , pos_tare.tare-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
END.
apply "entry" to br-tares in frame Dialog-Frame.
apply "value-changed" to br-tares in frame Dialog-Frame.
END.
...
assign
  v-str-recid14 = trim( string( recid( X_tare ) , "->>>>>>>>>>>9":U ) )
...
DO:
  ASSIGN
  rs-stts
  p-stts = (IF rs-stts = 'все':U THEN ? ELSE INTEGER(rs-stts))
  .
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-tares :handle
      .
...
def var sort-labelbr-tares   as character no-undo .
def var sort-clmnbr-tares    as handle    no-undo .
def var cur-clmnbr-tares     as handle    no-undo .
def var cur-clmn-locbr-tares as integer   no-undo .
def var re-querybr-tares     as logical   initial no no-undo .
on start-search, ctrl-o of br-tares in frame Dialog-Frame do:
   run sort-brbr-tares
     (input (if available X_tare
             then recid(X_tare)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-tares :
  define input parameter p-recid as recid no-undo .
  if re-querybr-tares = no then do:
    assign
       cur-clmnbr-tares = br-tares:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-tares <> ? then sort-clmnbr-tares:column-fgcolor = 0.
    if cur-clmnbr-tares = sort-clmnbr-tares then do:
      assign
         sort-labelbr-tares = ""
         sort-clmnbr-tares = ?
      .
       ...
       assign
         sort-labelbr-tares = cur-clmnbr-tares:label
         sort-clmnbr-tares  = cur-clmnbr-tares
         sort-clmnbr-tares:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-tares = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-tares:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-tares then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-tares = cur-clmn-locbr-tares + 1
    .
  end.
  case sort-labelbr-tares:
        when X_tare.tare-code:label in browse br-tares then DO:   assign     sort-column-name = "X_tare.tare-code"   .   run OpenBr ( input yes, input no, input no).   . END.
        when X_tare.tare-name:label in browse br-tares then DO:   assign     sort-column-name = "X_tare.tare-name"   .   run OpenBr ( input yes, input no, input no).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr ( input yes, input no, input no).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-tares') then do:
          run mv-brw-defaultbr-tares.
        end.
      if sort-labelbr-tares <> "" then do:
        assign
          cur-clmnbr-tares:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-tares = ?
      .
    end.
  end case.
    if cur-clmn-locbr-tares <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-tares') then do:
        run ch-clmnbr-tares in this-procedure (cur-clmn-locbr-tares).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-tares to recid p-recid no-error.
    apply "value-changed" to br-tares in frame Dialog-Frame.
  end.
  apply "entry" to br-tares in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-tares:
if cur-clmnbr-tares = ? then do:
   run OpenBr ( input yes, input no, input no).
end.
else do:
   assign re-querybr-tares = yes.
   run sort-brbr-tares
     (input (if available X_tare
             then recid(X_tare)
             else ?
...
do:
  br-tares :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
DEF VAR cur-clmn-numbr-tares as INT EXTENT 4 no-undo.
DEF VAR varmvibr-tares       as INT no-undo.
DEF VAR varmvjbr-tares       as INT no-undo.
DEF VAR varmvkbr-tares       as INT no-undo.
DEF VAR varmvlbr-tares       as INT no-undo.
DEF VAR move-elementbr-tares as INT no-undo.
def var jjbr-tares           as int no-undo.
do varmvibr-tares = 1 to EXTENT(cur-clmn-numbr-tares):
  ASSIGN cur-clmn-numbr-tares[varmvibr-tares] = varmvibr-tares.
END.
RUN start-mv-clmnbr-tares.
PROCEDURE start-mv-clmnbr-tares:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-tares = NUM-ENTRIES('1,2,3,4') TO 1 BY -1:
     RUN re-move-clmnbr-tares ( cur-clmn-numbr-tares[INTEGER(ENTRY (jjbr-tares, '1,2,3,4'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-tares do:
  RUN re-move-clmnbr-tares ( 1, 4).
END.
ON ctrl-cursor-left OF BROWSE br-tares do:
  RUN re-move-clmnbr-tares (4, 1).
END.
PROCEDURE re-move-clmnbr-tares:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-tares = 1 TO EXTENT(cur-clmn-numbr-tares):
    if cur-clmn-numbr-tares[varmvibr-tares] = source-column THEN cur-clmn-numbr-tares[varmvibr-tares] = -1.
  END.
  if br-tares:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-tares = source-column - 1 to target-column BY -1:
    DO varmvibr-tares = 1 TO EXTENT(cur-clmn-numbr-tares):
        if cur-clmn-numbr-tares[varmvibr-tares] = varmvjbr-tares THEN DO:
          cur-clmn-numbr-tares[varmvibr-tares] = cur-clmn-numbr-tares[varmvibr-tares] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-tares = source-column + 1 to target-column:
    DO varmvibr-tares = 1 TO EXTENT(cur-clmn-numbr-tares):
      if cur-clmn-numbr-tares[varmvibr-tares] = varmvjbr-tares THEN DO:
        cur-clmn-numbr-tares[varmvibr-tares] = cur-clmn-numbr-tares[varmvibr-tares] - 1.
      END.
  ...
  END.
  DO varmvibr-tares = 1 TO EXTENT(cur-clmn-numbr-tares):
    if cur-clmn-numbr-tares[varmvibr-tares] = -1 THEN cur-clmn-numbr-tares[varmvibr-tares] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-tares:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-tares = 1 TO EXTENT(cur-clmn-numbr-tares):
    if cur-clmn-numbr-tares[varmvibr-tares] = cur-clmn-loc THEN move-elementbr-tares = varmvibr-tares.
  END.
  RUN re-move-clmnbr-tares (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-tares:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-tares = 1 to EXTENT(cur-clmn-numbr-tares):
    RUN re-move-clmnbr-tares (cur-clmn-numbr-tares[varmvlbr-tares], varmvlbr-tares).
  END.
  RUN start-mv-clmnbr-tares.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-tares FOR EACH X_tare NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyENable :
ASSIGN
rs-stts:RADIO-BUTTONS IN FRAME Dialog-Frame
                       = "Текущие&+" + chr(44) +  '0':U + chr(44) +
                       "Все&!" + chr(44) + 'все':U + chr(44) +
                        "Удаленные&-" + chr(44) + '1':U
rs-stts = (IF p-stts = ? THEN 'все':U ELSE string(p-stts))
...
if p-open-query then do:
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-tares FOR  EACH  X_tare no-lock
      where  true
      indexed-reposition
  .
      assign
  ...
  on error undo, leave
  :
    if query br-tares:handle:get-buffer-handle(1) = (buffer X_tare:handle) then do:
      assign
    ...
    .
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_tare"
      parameter-4-32 =
        (
          if (" true " + " " + where-phrase-32) <> ""
          then " true " + " " + where-phrase-32
  ...
  END.
  ELSE DO:
    frame Dialog-Frame:TITLE = title00 +  chr(32) + entry (lookup (STRING(p-stts), '0,1,50,99':U), 'тек,удал,блок,удаление':U).
    filter-point-name = title00 +  chr(32) + entry (lookup (STRING(p-stts), '0,1,50,99':U), 'тек,удал,блок,удаление':U).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_tare.stts = p-stts " + " " + where-phrase-34) <> ""
          then  substitute('X_tare.stts = &1', p-stts)  + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_tare.stts = p-stts " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-tares:handle
  ...
  :
    if query br-tares:handle:get-buffer-handle(1) = (buffer X_tare:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_tare.stts = &1', p-stts)  + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_tare"
      parameter-4-34 =
        (
          if (" X_tare.stts = p-stts " + " " + where-phrase-34) <> ""
          then  substitute('X_tare.stts = &1', p-stts)  + " " + where-phrase-34
...
if not loc#log then return error.
  ...
  if error-status:error then do:                           find first pos_tare no-lock where                                   recid(pos_tare) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ТАРЫ" skip                            string(if avail pos_tare                                     then  substitute("Код тары: &1"                                                     , pos_tare.tare-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
  if available X_tare then do:
    loc#log = br-tares:select-focused-row( ) IN FRAME Dialog-Frame.
  end.
  apply "ENTRY" to br-tares.
end.
END PROCEDURE.
