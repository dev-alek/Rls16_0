...
DEFINE BROWSE br-condkeep
  QUERY br-condkeep NO-LOCK DISPLAY
      ...
      entry (lookup (STRING(X_condition-keeping.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
...
if loc-doc-rec <> ? THEN DO:
    ...
    if error-status:error then do:                           find first pos_condition-keeping no-lock where                                   recid(pos_condition-keeping) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи УСЛОВИЙ ХРАНЕНИЯ" skip                            string(if avail pos_condition-keeping                                     then  substitute("Вн код условий хранения: &1"                                                     , pos_condition-keeping.cond-keep-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
END.
apply "entry" to br-condkeep in frame Dialog-Frame.
...
assign
  v-str-recid10 = trim( string( recid( X_condition-keeping ) , "->>>>>>>>>>>9":U ) )
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-condkeep :handle
      .
...
def var sort-labelbr-condkeep   as character no-undo .
def var sort-clmnbr-condkeep    as handle    no-undo .
def var cur-clmnbr-condkeep     as handle    no-undo .
def var cur-clmn-locbr-condkeep as integer   no-undo .
def var re-querybr-condkeep     as logical   initial no no-undo .
on start-search, ctrl-o of br-condkeep in frame Dialog-Frame do:
   run sort-brbr-condkeep
     (input (if available X_condition-keeping
             then recid(X_condition-keeping)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-condkeep :
  define input parameter p-recid as recid no-undo .
  if re-querybr-condkeep = no then do:
    assign
       cur-clmnbr-condkeep = br-condkeep:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-condkeep <> ? then sort-clmnbr-condkeep:column-fgcolor = 0.
    if cur-clmnbr-condkeep = sort-clmnbr-condkeep then do:
      assign
         sort-labelbr-condkeep = ""
         sort-clmnbr-condkeep = ?
      .
     end.
     else do:
       assign
         sort-labelbr-condkeep = cur-clmnbr-condkeep:label
         sort-clmnbr-condkeep  = cur-clmnbr-condkeep
         sort-clmnbr-condkeep:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-condkeep = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-condkeep:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-condkeep then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-condkeep = cur-clmn-locbr-condkeep + 1
    .
  end.
  case sort-labelbr-condkeep:
        when X_condition-keeping.cond-keep-code:label in browse br-condkeep then DO:   assign     sort-column-name = "X_condition-keeping.cond-keep-code"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-condkeep') then do:
          run mv-brw-defaultbr-condkeep.
        end.
      if sort-labelbr-condkeep <> "" then do:
        assign
          cur-clmnbr-condkeep:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-condkeep = ?
      .
    end.
  end case.
    if cur-clmn-locbr-condkeep <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-condkeep') then do:
        run ch-clmnbr-condkeep in this-procedure (cur-clmn-locbr-condkeep).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-condkeep to recid p-recid no-error.
    apply "value-changed" to br-condkeep in frame Dialog-Frame.
  end.
  apply "entry" to br-condkeep in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-condkeep:
if cur-clmnbr-condkeep = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-condkeep = yes.
   run sort-brbr-condkeep
     (input (if available X_condition-keeping
             then recid(X_condition-keeping)
             else ?
   ...
   assign re-querybr-condkeep = no.
end.
end.
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-condkeep :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
DEF VAR cur-clmn-numbr-condkeep as INT EXTENT 9 no-undo.
DEF VAR varmvibr-condkeep       as INT no-undo.
DEF VAR varmvjbr-condkeep       as INT no-undo.
DEF VAR varmvkbr-condkeep       as INT no-undo.
DEF VAR varmvlbr-condkeep       as INT no-undo.
DEF VAR move-elementbr-condkeep as INT no-undo.
def var jjbr-condkeep           as int no-undo.
do varmvibr-condkeep = 1 to EXTENT(cur-clmn-numbr-condkeep):
  ASSIGN cur-clmn-numbr-condkeep[varmvibr-condkeep] = varmvibr-condkeep.
END.
RUN start-mv-clmnbr-condkeep.
PROCEDURE start-mv-clmnbr-condkeep:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-condkeep = NUM-ENTRIES('1,2,3,4,5,6,7,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-condkeep ( cur-clmn-numbr-condkeep[INTEGER(ENTRY (jjbr-condkeep, '1,2,3,4,5,6,7,8,9'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-condkeep do:
  RUN re-move-clmnbr-condkeep ( 1, 9).
END.
ON ctrl-cursor-left OF BROWSE br-condkeep do:
  RUN re-move-clmnbr-condkeep (9, 1).
END.
PROCEDURE re-move-clmnbr-condkeep:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-condkeep = 1 TO EXTENT(cur-clmn-numbr-condkeep):
    if cur-clmn-numbr-condkeep[varmvibr-condkeep] = source-column THEN cur-clmn-numbr-condkeep[varmvibr-condkeep] = -1.
  END.
  if br-condkeep:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-condkeep = source-column - 1 to target-column BY -1:
    DO varmvibr-condkeep = 1 TO EXTENT(cur-clmn-numbr-condkeep):
        if cur-clmn-numbr-condkeep[varmvibr-condkeep] = varmvjbr-condkeep THEN DO:
          cur-clmn-numbr-condkeep[varmvibr-condkeep] = cur-clmn-numbr-condkeep[varmvibr-condkeep] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-condkeep = source-column + 1 to target-column:
    DO varmvibr-condkeep = 1 TO EXTENT(cur-clmn-numbr-condkeep):
      if cur-clmn-numbr-condkeep[varmvibr-condkeep] = varmvjbr-condkeep THEN DO:
        cur-clmn-numbr-condkeep[varmvibr-condkeep] = cur-clmn-numbr-condkeep[varmvibr-condkeep] - 1.
      END.
  ...
  END.
  DO varmvibr-condkeep = 1 TO EXTENT(cur-clmn-numbr-condkeep):
    if cur-clmn-numbr-condkeep[varmvibr-condkeep] = -1 THEN cur-clmn-numbr-condkeep[varmvibr-condkeep] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-condkeep:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-condkeep = 1 TO EXTENT(cur-clmn-numbr-condkeep):
    if cur-clmn-numbr-condkeep[varmvibr-condkeep] = cur-clmn-loc THEN move-elementbr-condkeep = varmvibr-condkeep.
  END.
  RUN re-move-clmnbr-condkeep (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-condkeep:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-condkeep = 1 to EXTENT(cur-clmn-numbr-condkeep):
    RUN re-move-clmnbr-condkeep (cur-clmn-numbr-condkeep[varmvlbr-condkeep], varmvlbr-condkeep).
  END.
  RUN start-mv-clmnbr-condkeep.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-condkeep FOR EACH X_condition-keeping NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
IF p-sts = ? THEN DO:
    frame Dialog-Frame:TITLE = title0 + chr(32) + "все".
...
if not loc#log then return error.
  ...
  if error-status:error then do:                           find first pos_condition-keeping no-lock where                                   recid(pos_condition-keeping) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи УСЛОВИЙ ХРАНЕНИЯ" skip                            string(if avail pos_condition-keeping                                     then  substitute("Вн код условий хранения: &1"                                                     , pos_condition-keeping.cond-keep-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
  if available X_condition-keeping then do:
    loc#log = br-condkeep:select-focused-row( ) IN FRAME Dialog-Frame.
  end.
  apply "ENTRY" to br-condkeep.
end.
END PROCEDURE.
...
DO WHILE available X_condition-keeping :
  Display STREAM PrnLibStream
   X_condition-keeping.cond-keep-name
  entry (lookup (STRING(X_condition-keeping.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) @ v-sts-chr
