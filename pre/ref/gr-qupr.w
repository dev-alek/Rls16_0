...
define variable v-rec-id as recid no-undo .
  run ref/gr-qupra.w (input parparentproc,input 'ДОБАВЛЕНИЕ':U , input-output v-rec-id) .
  OPEN QUERY BROWSE-1grp FOR EACH buf_qnty-group WHERE   ( r-status = 2 OR buf_qnty-group.stts =  r-status ) and   ( r-find = no or (buf_qnty-group.qgr-db-num = find-db and                     buf_qnty-group.qgr-id = find-id )).
...
if buf_qnty-group.stts <> 0 then do:
  ...
  OPEN QUERY BROWSE-2cli FOR EACH buf_qnty-in-qnty-group OF buf_qnty-group      WHERE ( r-status = 2 OR buf_qnty-in-qnty-group.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.
...
if v-cntxt-db-num <> 0 then do :
  ...
  run ref/gr-qupra.w (input parparentproc,input 'ИЗМЕНЕНИЕ':U , input-output v-rec-id) .
  OPEN QUERY BROWSE-1grp FOR EACH buf_qnty-group WHERE   ( r-status = 2 OR buf_qnty-group.stts =  r-status ) and   ( r-find = no or (buf_qnty-group.qgr-db-num = find-db and                     buf_qnty-group.qgr-id = find-id )).
  reposition BROWSE-1grp to recid v-rec-id no-error .
  apply "VALUE-CHANGED" to browse-1grp in frame Dialog-Frame.
END.
...
assign
  v-str-recid12 = trim( string( recid( buf_qnty-group ) , "->>>>>>>>>>>9":U ) )
...
assign
  v-str-recid14 = trim( string( recid( buf_qnty-in-qnty-group ) , "->>>>>>>>>>>9":U ) )
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1grp FOR EACH buf_qnty-group WHERE   ( r-status = 2 OR buf_qnty-group.stts =  r-status ) and   ( r-find = no or (buf_qnty-group.qgr-db-num = find-db and                     buf_qnty-group.qgr-id = find-id )).    OPEN QUERY BROWSE-2cli FOR EACH buf_qnty-in-qnty-group OF buf_qnty-group      WHERE ( r-status = 2 OR buf_qnty-in-qnty-group.stts =  r-status )       NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
