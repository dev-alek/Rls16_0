...
ON GO OF FRAME d-rvd-reason
  DO:
    p-rid-list = v-rid-list.
  END.
ON CHOOSE OF b-add IN FRAME d-rvd-reason
  DO:
    ...
    do:
      if t-del
      then do :
        OPEN QUERY br-rvd-reason FOR EACH X_ext-classif NO-LOCK where X_ext-classif.classif-subject = 'rvd-reason':U
                                                                and X_ext-classif.classif-name = 'rvd-reason':U
  ...
  END.
ON CHOOSE OF b-del IN FRAME d-rvd-reason
  DO:
  ...
  END.
ON CHOOSE OF b-sel IN FRAME d-rvd-reason
  DO:
    if ( available X_ext-classif )
...
assign
  v-str-recid9 = trim( string( recid( X_ext-classif ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry9 > 0 then do:
  ...
  .
      ...
      g-log = br-rvd-reason:refresh() .
      if last-event:function <> "mouse-select-dblclick" then
      do:
        g-log = br-rvd-reason:select-next-row ().
        apply "value-changed" to br-rvd-reason in frame d-rvd-reason.
      end.
    end.
    apply "entry" to br-rvd-reason in frame d-rvd-reason.
  END.
ON CHOOSE OF b-upd IN FRAME d-rvd-reason
  DO:
    define variable glog as logical no-undo .
...
define variable vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  WAIT-FOR GO OF FRAME d-rvd-reason focus br-rvd-reason.
END.
