...
DEFINE BUFFER find_bar-code FOR ub.bar-code.
DEFINE BUFFER find_inkas FOR ub.inkas.
define input parameter parparentproc as widget-handle no-undo .
define input parameter r-g as recid no-undo.
...
define variable vss-description AS CHAR NO-UNDO INIT "Список чеков по товару":U.
...
define variable filter-label as character no-undo init "Список чеков по товару" .
define variable filter-label0 as character no-undo init "Список чеков по товару" .
define variable filter-point0 as character no-undo.
define variable filter-point as character no-undo.
assign
filter-point0 = ('ТОВАР':U + chr(44) + 'чеки':U)
.
...
DEFINE MENU m-print
       ...
       RULE
       MENU-ITEM m-one-time     LABEL "Игнорировать повторение строк чеков с таким же бар-кодом"
              TOGGLE-BOX
...
DEFINE VARIABLE mark-num AS CHARACTER FORMAT "X(256)":U
     ...
     SIZE 6 BY 1
     FGCOLOR 4  NO-UNDO.
DEFINE VARIABLE sch-bcode AS INTEGER FORMAT ">>>>>>>>9":U INITIAL 0
     LABEL "бар-коду"
     VIEW-AS FILL-IN
...
DEFINE BROWSE BR-docs
  QUERY BR-docs DISPLAY
      bar-code.b-code FORMAT "999999999":U
  ...
  ENABLE
      c-doc.cashier
    WITH NO-ROW-MARKERS SEPARATORS SIZE 98 BY 14.
DEFINE FRAME Dialog-Frame
     b-quit AT ROW 1 COL 1
     B-mark AT ROW 1 COL 11
     b-sel AT ROW 1 COL 21
     b-lkp AT ROW 1 COL 31
     b-onlybcode AT ROW 1 COL 41
     ...
     BR-docs AT ROW 3.33 COL 1
     ED-notes AT ROW 17.67 COL 1 NO-LABEL
     sch-bcode AT ROW 19.8 COL 17.6 COLON-ALIGNED
     sch-code AT ROW 20.8 COL 17.6 COLON-ALIGNED WIDGET-ID 2
     sch-date AT ROW 20.83 COL 48.3 COLON-ALIGNED WIDGET-ID 4
     sch-price AT ROW 20.83 COL 77.5 COLON-ALIGNED WIDGET-ID 6
     mark-num AT ROW 1 COL 12.5 COLON-ALIGNED NO-LABEL
     "Сортировка" VIEW-AS TEXT
          SIZE 12.8 BY .8 AT ROW 2.3 COL 30
          FGCOLOR 4
     "ПОИСК ПО" VIEW-AS TEXT
          SIZE 9.3 BY 1 AT ROW 19.8 COL 1.5
          FGCOLOR 4
     SPACE(88.20) SKIP(1.19)
...
if v-num-entry16 > 0 then do:
  ...
  .
    ...
    apply "entry" to br-docs in frame Dialog-Frame.
END.
ON CHOOSE OF b-onlybcode IN FRAME Dialog-Frame
DO:
DEFINE VARIABLE rid-list as character no-undo .
if not avail bar-code then return no-apply.
   run ref/gds-chk.w (
                   input parparentproc
                  ,input bar-code.b-code
                  ,input bttns
                  ,input "gds-chks":U + chr(44) + par-mode
                  ,input ?
                  ,input parobj-type
                  ,input parobj-code
                  ,input parout-code
   ...
   if print-type = "list":U or print-type = "gds":U then do:
      if (par-mode = 'объект':U or par-mode = 'все':U) and index(frame Dialog-Frame:title,"ФИЛЬТР" ) = 0 then do:
        CASE print-type:
          ...
          when "gds":U then do:
               message "Вы хотите напечатать строки чеков при невключенном фильтре!" skip
               "Эта процедура может занять долгое время! Продолжать?" view-as alert-box
              WARNING buttons YES-NO update glog.
              if NOT glog then return no-apply.
          end.
...
assign
v-int = integer(last-event:label) no-error .
if not error-status:error then do:
    sch-bcode:screen-value = sch-bcode:screen-value + last-event:label.
    apply "entry" to sch-bcode in frame Dialog-Frame.
end.
END.
...
DO:
  ...
  Run OpenBr in this-procedure ( input yes, input no, input '':U).
END.
ON CTRL-J OF sch-bcode IN FRAME Dialog-Frame
DO:
  run proc-find-bcode in this-procedure ( input yes, input string(input frame Dialog-Frame sch-bcode)) no-error.
  if error-status:error then return no-apply.
END.
ON RETURN OF sch-bcode IN FRAME Dialog-Frame
DO:
  run proc-find-bcode in this-procedure ( input no, input string(input frame Dialog-Frame sch-bcode)) no-error.
  if error-status:error then return no-apply.
END.
...
do:
  v-doc-rec = recid(bar-code). run OpenBr in this-procedure ( input yes, input no, input '':U). reposition br-docs to recid v-doc-rec no-error. v-doc-rec = ?.               apply 'value-changed' TO BR-DOCS.
...
PROCEDURE sort-brBR-docs :
  ...
  case sort-labelBR-docs:
        when BAR-CODE.B-CODE:label in browse BR-docs then DO:    assign       sort-column-name = "BAR-CODE.B-CODE"     .     run OpenBr in this-procedure ( input yes, input no, input '').   . END.
...
DEF VAR cur-clmn-numbr-docs as INT EXTENT 29 no-undo.
...
ON ctrl-cursor-right OF BROWSE br-docs do:
  RUN re-move-clmnbr-docs ( 10, 29).
END.
ON ctrl-cursor-left OF BROWSE br-docs do:
  RUN re-move-clmnbr-docs (29, 10).
END.
...
define variable vss-include-info30 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  find FIRST buf_goods where
             recid(buf_goods) = r-g NO-LOCK.
  find FIRST buf_gds-prt where
             buf_gds-prt.upper-code = buf_goods.prt-root no-lock.
  if entry(1, par-mode) = "gds-chk":u then do:
    assign
    v-cycle = yes
    par-mode = substr(par-mode, 9)
    .
  end.
...
CASE par-mode:
    ...
    if pardoc-rec <> ? then do:
      FIND FIRST find_bar-code No-LOCK where
                 recid(find_bar-code) = pardoc-rec No-ERROR.
      if not avail find_bar-code then do:
        message
...
END PROCEDURE.
PROCEDURE enable_UI :
  DISPLAY Cb-chk-type RS-sort ED-notes sch-bcode sch-code sch-date sch-price
          mark-num
      WITH FRAME Dialog-Frame.
  ENABLE b-quit B-mark b-sel b-lkp b-onlybcode B-del B-sale B-print B-sch
         B-Help Cb-chk-type RS-sort BR-docs ED-notes sch-bcode sch-code
...
p-chk-type = integer(cb-chk-type)
br-docs:NUM-LOCKED-COLUMNS IN FRAME Dialog-Frame = 9
...
title0 = "Список чеков по товару" + chr(32) +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-34) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + ", EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code")
    ...
    :
      assign
        l-disable-where-34 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-34 = "")
      .
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-docs FOR EACH bar-code
      where  bar-code.gds-code = buf_goods.gds-code
    , EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
                          ,input rowid(bar-code)
                          ,input logical(parameter-2-34)
                          ,input no-lock
                          ,input (buffer bar-code:handle)
                          ,input parameter-4-34
                          ,input parameter-5-34
                          ) no-error.
      .
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH bar-code"
      parameter-4-34 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-34) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-34
      ...
      parameter-5-34 = (" " + "" + " " + ", EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code" + " " + p-find-condition)
      parameter-6-34 = if sort-phrase-34 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-37 as character no-undo .
      ...
      parameter-4-37 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-37) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-37
          else "true"
        )
      parameter-5-37 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3)'                                   ,parobj-type                                   ,parobj-code                                   ,p-chk-type ))
    ...
    :
      assign
        l-disable-where-37 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-37 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-37 = false then do:
    OPEN QUERY br-docs FOR EACH bar-code
      where  bar-code.gds-code = buf_goods.gds-code
    , EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = parobj-type AND c-doc.obj-code = parobj-code and (p-chk-type = 0 or c-doc.chk-type = p-chk-type)
  .
      assign
        l-query-was-opened = true
      .
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-37 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-37 + " ":u + p-find-condition + " " + ""
      parameter-5-37 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-37 = (if p-find-next then "true":u else "false":u )
      parameter-3-37 =  "FOR EACH bar-code"
      parameter-4-37 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-37) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-37
      ...
      parameter-5-37 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3)'                                   ,parobj-type                                   ,parobj-code                                   ,p-chk-type ) + " " + p-find-condition)
      parameter-6-37 = if sort-phrase-37 = ''
                           then
        (
        " " + "  " +
    ...
    .
      ...
      WHEN "d-card":U    THEN DO:
        ...
        filter-label = substitute("&1 по одной карте", filter-label0)
        .
        if p-open-query then do:
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-39 as character no-undo .
      ...
      parameter-4-39 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-39) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-39
          else "true"
        )
      parameter-5-39 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code,  FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.d-card = "&1" and (&2 = 0 or c-doc.chk-type = &2)'                                   ,pard-card                                   ,p-chk-type ))
    ...
    :
      assign
        l-disable-where-39 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-39 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-39 = false then do:
    OPEN QUERY br-docs FOR EACH bar-code
      where  bar-code.gds-code = buf_goods.gds-code
    , EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code,  FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.d-card = pard-card and (p-chk-type = 0 or c-doc.chk-type = p-chk-type)
  .
      assign
        l-query-was-opened = true
      .
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-39 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-39 + " ":u + p-find-condition + " " + ""
      parameter-5-39 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-39 = (if p-find-next then "true":u else "false":u )
      parameter-3-39 =  "FOR EACH bar-code"
      parameter-4-39 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-39) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-39
      ...
      parameter-5-39 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code,  FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.d-card = "&1" and (&2 = 0 or c-doc.chk-type = &2)'                                   ,pard-card                                   ,p-chk-type ) + " " + p-find-condition)
      parameter-6-39 = if sort-phrase-39 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-42 as character no-undo .
      ...
      parameter-4-42 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-42) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-42
          else "true"
        )
      parameter-5-42 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = "&1", FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.d-card = "&2" AND c-doc.obj-type = "&3" and c-doc.obj-code = &4 and (&5 = 0 or c-doc.chk-type = &5)'                                      ,parout-code                                      ,pard-card                                       ,parobj-code                                      ,parobj-type                                      ,p-chk-type))
    ...
    :
      assign
        l-disable-where-42 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-42 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-42 = false then do:
    OPEN QUERY br-docs FOR EACH bar-code
      where  bar-code.gds-code = buf_goods.gds-code
    , EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = parout-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.d-card = pard-card AND c-doc.obj-code = parobj-code and (p-chk-type = 0 or c-doc.chk-type = p-chk-type)
  .
      assign
        l-query-was-opened = true
      .
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-42 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-42 + " ":u + p-find-condition + " " + ""
      parameter-5-42 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-42 = (if p-find-next then "true":u else "false":u )
      parameter-3-42 =  "FOR EACH bar-code"
      parameter-4-42 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-42) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-42
    ...
    .
      ...
      define buffer buf_inkas for ub.inkas.
        ...
        ASSIGN
        filter-label = substitute("&1 Один объект, одна продажа", filter-label0)
        filter-point = filter-point0 + par-mode
        .
        if p-open-query then do:
          assign
          frame Dialog-Frame:TITLE = substitute("&1 Чеки по отчету &2 Объект &3&4 &5"
                                                      ,title0
                                                      ,parout-code
                                                      ,parobj-type
                                                      ,parobj-code
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-44 as character no-undo .
      ...
      parameter-4-44 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-44) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-44
          else "true"
        )
      parameter-5-44 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = "&1" , FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   and ( &2 = 0 or c-doc.chk-type = &2)'                                   ,parout-code                                   ,p-chk-type ))
    ...
    :
      assign
        l-disable-where-44 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-44 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-44 = false then do:
    OPEN QUERY br-docs FOR EACH bar-code
      where  bar-code.gds-code = buf_goods.gds-code
    , EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = parout-code , FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   and (p-chk-type = 0 or c-doc.chk-type = p-chk-type)
  .
      assign
        l-query-was-opened = true
      .
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-44 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-44 + " ":u + p-find-condition + " " + ""
      parameter-5-44 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-44 = (if p-find-next then "true":u else "false":u )
      parameter-3-44 =  "FOR EACH bar-code"
      parameter-4-44 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-44) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-44
      ...
      parameter-5-44 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = "&1" , FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   and ( &2 = 0 or c-doc.chk-type = &2)'                                   ,parout-code                                   ,p-chk-type ) + " " + p-find-condition)
      parameter-6-44 = if sort-phrase-44 = ''
                           then
        (
        " " + "  " +
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-print-host-code
        ...
        filter-label = substitute("&1 Один объект, неучтенные чеки", filter-label0)
        .
        if p-open-query then do:
          assign
          frame Dialog-Frame:TITLE = substitute("&1 НЕ включенные в отчеты чеки Объект &2&3 &4"
                                                      ,title0
                                                      ,parobj-type
                                                      ,parobj-code
                                                      , (if p-chk-type = 0 then '':u else entry (lookup (string(p-chk-type), '1,6,8,69,96,14,15,16,36,17,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) + 1, ',' + 'Продажа,Возврат,Аннуляция,Списание,ВзврСпис,СбросТрнзкц,Перелив,ПеревТрнзкц,РазблТрнзкц,ТехПролив,Инвентаризация,Z-отчет,Закрытие_смены,Открытие_смены,_Продажа,_Возврат,_Аннуляция,_Списание,_ВзврСпис,_СбросТрнзкц,_Перелив,_ПеревТрнзкц,_ТехПролив,_Инвентаризация,_Z-отчет,_РазблТрнзкц,_Закрытие_смены,>Продажа,>Возврат,>Аннуляция,>>Продажа,>>Возврат,Инкассация,Касс_фонд,Перевод_опл,Расход_кассы,Декл_ден_ящ,Приход_Корр,Расход_Корр':U))
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-47 as character no-undo .
      ...
      parameter-4-47 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-47) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-47
          else "true"
        )
      parameter-5-47 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = ?, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3)'                                                  ,parobj-type                                                  ,parobj-code                                                  ,p-chk-type ))
    ...
    :
      assign
        l-disable-where-47 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-47 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-47 = false then do:
    OPEN QUERY br-docs FOR EACH bar-code
      where  bar-code.gds-code = buf_goods.gds-code
    , EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = ?, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = parobj-type AND c-doc.obj-code = parobj-code and (p-chk-type = 0 or c-doc.chk-type = p-chk-type)
  .
      assign
        l-query-was-opened = true
      .
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-47 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-47 + " ":u + p-find-condition + " " + ""
      parameter-5-47 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-47 = (if p-find-next then "true":u else "false":u )
      parameter-3-47 =  "FOR EACH bar-code"
      parameter-4-47 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-47) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-47
      ...
      parameter-5-47 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = ?, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3)'                                                  ,parobj-type                                                  ,parobj-code                                                  ,p-chk-type ) + " " + p-find-condition)
      parameter-6-47 = if sort-phrase-47 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-49 as character no-undo .
      ...
      parameter-4-49 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-49) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-49
          else "true"
        )
      parameter-5-49 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = ?, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3)'                                                  ,parobj-type                                                  ,parobj-code                                                  ,p-chk-type ))
      parameter-6-49 = if sort-phrase-49 = ''
                           then
        (
        " " + "  " +
          " " + sort-column-phrase +
        " " + " by chk-gds.chk-date  descending "
        )
                           else
        (
        " " + "  " +
    ...
    :
      assign
        l-disable-where-49 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-49 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-49 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-49 + " ":u + p-find-condition + " " + ""
      parameter-5-49 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-49 = (if p-find-next then "true":u else "false":u )
      parameter-3-49 =  "FOR EACH bar-code"
      parameter-4-49 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-49) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-49
      ...
      parameter-5-49 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = ?, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3)'                                                  ,parobj-type                                                  ,parobj-code                                                  ,p-chk-type ) + " " + p-find-condition)
      parameter-6-49 = if sort-phrase-49 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-52 as character no-undo .
      ...
      parameter-4-52 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-52) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-52
          else "true"
        )
      parameter-5-52 = (" " + "" + " " + substitute( ', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                  AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3) '                                                 , parobj-type                                                 , parobj-code                                                 , p-chk-type ))
    ...
    :
      assign
        l-disable-where-52 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-52 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-52 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-52 + " ":u + p-find-condition + " " + ""
      parameter-5-52 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-52 = (if p-find-next then "true":u else "false":u )
      parameter-3-52 =  "FOR EACH bar-code"
      parameter-4-52 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-52) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-52
      ...
      parameter-5-52 = (" " + "" + " " + substitute( ', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                  AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2 and (&3 = 0 or c-doc.chk-type = &3) '                                                 , parobj-type                                                 , parobj-code                                                 , p-chk-type ) + " " + p-find-condition)
      parameter-6-52 = if sort-phrase-52 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-54 as character no-undo .
      ...
      parameter-4-54 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-54) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-54
          else "true"
        )
      parameter-5-54 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code,  FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                  AND c-doc.d-card = "&1" and (&2 = 0 or c-doc.chk-type = &2)'                                               , pard-card                                               , p-chk-type ))
    ...
    :
      assign
        l-disable-where-54 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-54 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-54 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-54 + " ":u + p-find-condition + " " + ""
      parameter-5-54 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-54 = (if p-find-next then "true":u else "false":u )
      parameter-3-54 =  "FOR EACH bar-code"
      parameter-4-54 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-54) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-54
      ...
      parameter-5-54 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code,  FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                  AND c-doc.d-card = "&1" and (&2 = 0 or c-doc.chk-type = &2)'                                               , pard-card                                               , p-chk-type ) + " " + p-find-condition)
      parameter-6-54 = if sort-phrase-54 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-57 as character no-undo .
      ...
      parameter-4-57 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-57) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-57
          else "true"
        )
      parameter-5-57 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = "&1", FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                             AND c-doc.d-card = "&2" AND c-doc.obj-type = "&3" and c-doc.obj-code = &4 and (&5 = 0 or c-doc.chk-type = &5)'                                                 ,parout-code                                                 ,pard-card                                                  ,parobj-type                                                 ,parobj-code                                                 ,p-chk-type  ))
    ...
    :
      assign
        l-disable-where-57 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-57 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-57 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-57 + " ":u + p-find-condition + " " + ""
      parameter-5-57 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-57 = (if p-find-next then "true":u else "false":u )
      parameter-3-57 =  "FOR EACH bar-code"
      parameter-4-57 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-57) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-57
      ...
      parameter-5-57 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = "&1", FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                             AND c-doc.d-card = "&2" AND c-doc.obj-type = "&3" and c-doc.obj-code = &4 and (&5 = 0 or c-doc.chk-type = &5)'                                                 ,parout-code                                                 ,pard-card                                                  ,parobj-type                                                 ,parobj-code                                                 ,p-chk-type  ) + " " + p-find-condition)
      parameter-6-57 = if sort-phrase-57 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-59 as character no-undo .
      ...
      parameter-4-59 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-59) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-59
          else "true"
        )
      parameter-5-59 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = "&1" ,                                   FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   and (&2 = 0 or c-doc.chk-type = &2p-chk-type)'                                   ,parout-code                                   ,p-chk-type ))
    ...
    :
      assign
        l-disable-where-59 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-59 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-59 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-59 + " ":u + p-find-condition + " " + ""
      parameter-5-59 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-59 = (if p-find-next then "true":u else "false":u )
      parameter-3-59 =  "FOR EACH bar-code"
      parameter-4-59 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-59) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-59
      ...
      parameter-5-59 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = "&1" ,                                   FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                   and (&2 = 0 or c-doc.chk-type = &2p-chk-type)'                                   ,parout-code                                   ,p-chk-type ) + " " + p-find-condition)
      parameter-6-59 = if sort-phrase-59 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-62 as character no-undo .
      ...
      parameter-4-62 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-62) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-62
          else "true"
        )
      parameter-5-62 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = ?, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                  AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2                                  and (&3 = 0 or c-doc.chk-type = &3)'                                  ,parobj-type                                  ,parobj-code                                  ,p-chk-type ))
    ...
    :
      assign
        l-disable-where-62 =
          (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-62 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer chk-gds:handle) then do:
      ...
      parameter-4-62 =
        "where ":u +  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " ":u + where-phrase-62 + " ":u + p-find-condition + " " + ""
      parameter-5-62 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-62 = (if p-find-next then "true":u else "false":u )
      parameter-3-62 =  "FOR EACH bar-code"
      parameter-4-62 =
        (
          if (" bar-code.gds-code = buf_goods.gds-code " + " " + where-phrase-62) <> ""
          then  substitute(' bar-code.gds-code = &1 ', buf_goods.gds-code) + " " + where-phrase-62
      ...
      parameter-5-62 = (" " + "" + " " + substitute(', EACH chk-gds No-LOCK WHERE chk-gds.b-code = bar-code.b-code and chk-gds.out-code = ?, FIRST c-doc No-LOCK WHERE c-doc.doc-code = chk-gds.doc-code                                  AND c-doc.obj-type = "&1" AND c-doc.obj-code = &2                                  and (&3 = 0 or c-doc.chk-type = &3)'                                  ,parobj-type                                  ,parobj-code                                  ,p-chk-type ) + " " + p-find-condition)
      parameter-6-62 = if sort-phrase-62 = ''
                           then
        (
        " " + "  " +
...
chk-gds.b-code      column-label "Бар-код"
...
v-b-code = chk-gds.b-code
.
DO WHILE available chk-gds :
  if v-one-time = no OR
    v-doc-code <> chk-gds.doc-code
    OR v-b-code <> chk-gds.b-code
    then do:
...
CASE del-type:
   when "list":U then do:
       ...
       message
       "Вы хотите исключить ВСЕ чеки с данным товаром из продажи!" skip
     ...
     ELSE DO:
       message
       "Вы действительно хотите исключить ВСЕ чеки с данным товаром по текущему списку из продажи?!" skip
       view-as alert-box WARNING buttons YES-NO update glog.
       if NOT glog then return error.
     END.
   ...
   when "one":U then do:
    ...
    v-doc-rec = recid (c-doc).
       ...
       deleted = yes.
       RUN OpenBr in this-procedure ( input yes, input no, input '':U).
       APPLY "page-UP"   to br-docs in frame Dialog-Frame.
       APPLY "page-down"   to br-docs in frame Dialog-Frame.
    end.
  end.
END CASE.
end.
...
assign
  ...
  dim = '0'
  .
run fltfield-add in this-procedure('b-code', '', '',
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
run fltfield-add in this-procedure('in-code', 'Номер ПН', '',
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
run fltfield-add in this-procedure('unit-cli', 'Ед.изм', '',
input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
...
END.
END PROCEDURE.
PROCEDURE proc-find-bcode :
define input parameter par-next as logical no-undo.
define input parameter parb-code as character no-undo .
display
"  /  /":U @ sch-date
0 @ sch-price
"" @ sch-code
...
run OpenBr in this-procedure (
     input false
    ,input par-next
    ,input substitute("and bar-code.b-code = &1 "
      , parb-code)
    ).
apply "entry":u to sch-bcode in frame Dialog-Frame .
END PROCEDURE.
