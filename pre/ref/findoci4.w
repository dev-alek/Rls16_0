...
define variable vss-workfile    as character no-undo init "$Workfile: findoci4.w $":U .
define variable vss-archive     as character no-undo init "$Archive: ref/findoci4.w $":U .
define variable vss-description as character no-undo init "Карточка редактирования платежного поручения".
...
DEFINE FRAME Dialog-Frame
     ...
     tt-fin-doc.payer-type AT ROW 8 COL 35 NO-LABEL
          VIEW-AS FILL-IN
          SIZE 4 BY .77
     tt-fin-doc.payer-code AT ROW 8 COL 37.5 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN
          SIZE 6 BY .77
     tt-fin-doc.payer-name AT ROW 8 COL 44.1 COLON-ALIGNED NO-LABEL FORMAT "X(130)"
          VIEW-AS FILL-IN
          SIZE 53 BY .77
          FGCOLOR 4
     ...
     tt-fin-doc.payer-bank-city AT ROW 8.77 COL 59 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN
          SIZE 26.5 BY 1
     B-payer-view AT ROW 8.77 COL 86.8
...
DEFINE FRAME Dialog-Frame
     ...
     tt-fin-doc.sum-contr AT ROW 13.77 COL 72.3 COLON-ALIGNED
          ...
          VIEW-AS FILL-IN
          SIZE 25 BY 1
     B-receiver AT ROW 14.5 COL 55.3
     ...
     tt-fin-doc.receiver-kpp AT ROW 14.77 COL 22.5 COLON-ALIGNED
          ...
          SIZE 10 BY .77
          FGCOLOR 4
     tt-fin-doc.receiver-type AT ROW 14.77 COL 35 NO-LABEL
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS
                    "Item 1", "1":U,
"Item 1", "2":U
          SIZE 11.4 BY .77
          FGCOLOR 4
     tt-fin-doc.receiver-code AT ROW 14.77 COL 44 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN
          SIZE 9 BY .77
     tt-fin-doc.receiver-name AT ROW 14.77 COL 56.6 COLON-ALIGNED NO-LABEL FORMAT "X(130)"
          VIEW-AS FILL-IN
          SIZE 40.6 BY .77
          FGCOLOR 4
...
END.
ON CHOOSE OF b-other IN FRAME Dialog-Frame
DO:
  run ref/fndoci4d.w (
 INPUT     parParentProc
...
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile: findocip.i $ $Revision: ff8019e24d02, 2996, rls $".
...
do:
  ...
  then do:
    ASSIGN
      tt-fin-doc.doc-date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date21 :HANDLE
      tt-fin-doc.doc-date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle21 as handle no-undo .
  assign
    v-label-handle21 = tt-fin-doc.doc-date :side-label-handle in frame Dialog-Frame
  .
...
ON ANY-PRINTABLE OF tt-fin-doc.naznach-plat IN FRAME Dialog-Frame
DO:
  RUN proc-uho-check IN THIS-PROCEDURE(1) NO-ERROR.
  IF ERROR-STATUS :ERROR THEN DO: RETURN NO-APPLY. END.
END.
ON BACKSPACE OF tt-fin-doc.naznach-plat IN FRAME Dialog-Frame
DO:
define variable v-offset as integer no-undo .
  RUN proc-uho-check IN THIS-PROCEDURE(1) NO-ERROR.
  IF ERROR-STATUS :ERROR THEN DO: RETURN NO-APPLY. END.
  if tt-fin-doc.naznach-plat :CURSOR-OFFSET > 1 then do :
    assign
    v-offset = tt-fin-doc.naznach-plat:CURSOR-OFFSET
    tt-fin-doc.naznach-plat:screen-value = substring(tt-fin-doc.naznach-plat:screen-value, 1, tt-fin-doc.naznach-plat:CURSOR-OFFSET - 2) +
                      substring(tt-fin-doc.naznach-plat:screen-value, tt-fin-doc.naznach-plat:CURSOR-OFFSET , length(tt-fin-doc.naznach-plat:screen-value)  -  tt-fin-doc.naznach-plat:CURSOR-OFFSET + 1)
    tt-fin-doc.naznach-plat:CURSOR-OFFSET = v-offset - 1
    .
  end.
END.
ON DELETE-CHARACTER OF tt-fin-doc.naznach-plat IN FRAME Dialog-Frame
DO:
define variable v-offset as integer no-undo .
  if tt-fin-doc.naznach-plat:selection-start = ? then do:
    RUN proc-uho-check IN THIS-PROCEDURE(0) NO-ERROR.
    IF ERROR-STATUS :ERROR THEN DO: RETURN NO-APPLY. END.
    assign
    v-offset = tt-fin-doc.naznach-plat:CURSOR-OFFSET
    tt-fin-doc.naznach-plat:screen-value = substring(tt-fin-doc.naznach-plat:screen-value, 1, tt-fin-doc.naznach-plat:CURSOR-OFFSET - 1) +
                      substring(tt-fin-doc.naznach-plat:screen-value, tt-fin-doc.naznach-plat:CURSOR-OFFSET + 1, length(tt-fin-doc.naznach-plat:screen-value)  -  tt-fin-doc.naznach-plat:CURSOR-OFFSET + 1)
    tt-fin-doc.naznach-plat:CURSOR-OFFSET = v-offset
    .
  end.
  else do:
    IF  index(tt-fin-doc.naznach-plat:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
      IF tt-fin-doc.naznach-plat:Selection-start >= index(tt-fin-doc.naznach-plat:SCREEN-VALUE, "@") + 1 THEN   do:
        RETURN no-apply.
      end.
    END.
    tt-fin-doc.naznach-plat:edit-clear().
  end.
END.
ON CTRL-X OF tt-fin-doc.naznach-plat IN FRAME Dialog-Frame
DO:
  IF  index(tt-fin-doc.naznach-plat:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
    IF tt-fin-doc.naznach-plat:Selection-start >= index(tt-fin-doc.naznach-plat:SCREEN-VALUE, "@") + 1 THEN   do:
      RETURN no-apply.
  ...
  END.
  tt-fin-doc.naznach-plat:edit-cut().
END.
ON CTRL-V OF tt-fin-doc.naznach-plat IN FRAME Dialog-Frame
DO:
  IF  index(tt-fin-doc.naznach-plat:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
    IF tt-fin-doc.naznach-plat:Selection-start >= index(tt-fin-doc.naznach-plat:SCREEN-VALUE, "@") + 1 THEN   do:
      RETURN no-apply.
  ...
  END.
  tt-fin-doc.naznach-plat:edit-paste().
END.
...
DEFINE INPUT PARAMETER p-offset AS INTEGER NO-UNDO.
  ...
  :
    IF  index(tt-fin-doc.naznach-plat:SCREEN-VALUE IN FRAME Dialog-Frame, "@") > 0 then do:
      IF (tt-fin-doc.naznach-plat:CURSOR-OFFSET >= index(tt-fin-doc.naznach-plat:SCREEN-VALUE, "@") + 1
      AND p-offset = 0)
      OR (tt-fin-doc.naznach-plat:CURSOR-OFFSET > index(tt-fin-doc.naznach-plat:SCREEN-VALUE, "@") + 1
...
ON CHOOSE OF B-calc IN FRAME Dialog-Frame
  DO:
    ...
    display
      ...
      with frame Dialog-Frame .
  END.
ON CHOOSE OF B-payer-schet IN FRAME Dialog-Frame
  DO:
...
if available X_payer-fin-schet then
  assign
    v-rid-list = string(recid(X_payer-fin-schet))
    v-status_  = X_payer-fin-schet.status_
    .
run ref/finschts.w (
  INPUT parParentProc
  ,p-curr-host-code
...
if v-rid-list = "" then
do:
  apply "entry" to b-payer-schet in frame Dialog-Frame.
  return no-apply.
end.
...
do:
  message
    "Валюта выбранного Вами счета" 'ПЛАТЕЛЬЩИКА' "не совпадает с валютой платежа"
...
do:
  message
    "Статус выбранного Вами счета" 'ПЛАТЕЛЬЩИКА' buf_fin-schet.status_ " - нельзя работать с таким счетом"
    view-as alert-box error .
  return no-apply.
end.
FIND FIRST X_payer-fin-schet WHERE
  recid (X_payer-fin-schet) = ref-rec NO-LOCK  .
find first X_payer-fin-bank no-lock where
  X_payer-fin-bank.host-code = tt-fin-doc.host-code
  AND X_payer-fin-bank.code-bank = X_payer-fin-schet.code-bank .
assign
  tt-fin-doc.payer-bank-name  = X_payer-fin-bank.bank-name
  tt-fin-doc.payer-bank-city  = X_payer-fin-bank.bank-city
  tt-fin-doc.payer-dop1       = X_payer-fin-schet.dop1
  tt-fin-doc.payer-dop2       = X_payer-fin-schet.dop2
  tt-fin-doc.payer-bik        = X_payer-fin-bank.bik
  tt-fin-doc.payer-c-schet    = X_payer-fin-schet.c-schet
  tt-fin-doc.payer-r-schet    = X_payer-fin-schet.r-schet
  tt-fin-doc.payer-code-schet = X_payer-fin-schet.code-schet
  .
display
...
display
  ...
  with frame Dialog-Frame.
define variable v-recid-schet as recid no-undo . define variable v-recid-bank as recid no-undo . CASE X_receiver.obj-type:   when 'чел':U then do:              find first X_receiver-person no-lock where               X_receiver-person.psn-code = X_receiver.obj-code .   end.   when 'орг':U then do:     find first X_receiver-firm no-lock where               X_receiver-firm.firm-code = X_receiver.obj-code .   end. END CASE.   assign   tt-fin-doc.receiver-type =  X_receiver.obj-type   tt-fin-doc.receiver-code = X_receiver.obj-code   tt-fin-doc.receiver-name = X_receiver.obj-name   tt-fin-doc.receiver-inn = (if X_receiver.obj-type = 'чел':U                             then X_receiver-person.inn                             else X_receiver-firm.inn                             )   tt-fin-doc.receiver-kpp = (if X_receiver.obj-type = 'чел':U                             then X_receiver-person.kpp                             else X_receiver-firm.kpp                             )   .   run get-single-schet in this-procedure (                                           input p-host-code                                           ,input X_receiver.obj-type                                           ,input X_receiver.obj-code                                           ,input tt-fin-doc.curr-code                                           ,output v-recid-schet                                           ,output v-recid-bank).   if v-recid-schet <> ? then do:     find first X_receiver-fin-schet no-lock where                 recid(X_receiver-fin-schet) = v-recid-schet.     find first X_receiver-fin-bank no-lock where                 recid(X_receiver-fin-bank) = v-recid-bank.     assign     tt-fin-doc.receiver-code-schet = X_receiver-fin-schet.code-schet     tt-fin-doc.receiver-bank-name = X_receiver-fin-bank.bank-name       tt-fin-doc.receiver-bank-city = X_receiver-fin-bank.bank-city       tt-fin-doc.receiver-dop1       = X_receiver-fin-schet.dop1     tt-fin-doc.receiver-dop2       = X_receiver-fin-schet.dop2     tt-fin-doc.receiver-bik = X_receiver-fin-bank.bik     tt-fin-doc.receiver-r-schet = X_receiver-fin-schet.r-schet     tt-fin-doc.receiver-c-schet = X_receiver-fin-schet.c-schet     .   end.   else do:     RELEASE X_receiver-fin-schet.     RELEASE X_receiver-fin-bank.     assign     tt-fin-doc.receiver-code-schet = 0     tt-fin-doc.receiver-bank-name = "":U     tt-fin-doc.receiver-bank-city = "":U     tt-fin-doc.receiver-dop1      = "":U     tt-fin-doc.receiver-dop2      = "":U     tt-fin-doc.receiver-bik = "":U     tt-fin-doc.receiver-r-schet = "":U     tt-fin-doc.receiver-c-schet = "":U     .   end. display tt-fin-doc.receiver-type tt-fin-doc.receiver-code tt-fin-doc.receiver-name tt-fin-doc.receiver-inn tt-fin-doc.receiver-kpp tt-fin-doc.receiver-bank-name tt-fin-doc.receiver-bank-city tt-fin-doc.receiver-bik tt-fin-doc.receiver-r-schet tt-fin-doc.receiver-c-schet with frame Dialog-Frame.
find first ub.CashBook no-lock where ub.CashBook.id = tt-fin-doc.cashbookId no-error .
...
END.
ON CHOOSE OF B-receiver-schet IN FRAME Dialog-Frame
  DO:
...
define variable v-status_  like ub.fin-schet.status_ no-undo init 'тек':U.
define buffer buf_fin-schet for ub.fin-schet.
if not available X_receiver then
do:
  message
...
if available X_receiver-fin-schet then
  assign
    v-rid-list = string(recid(X_receiver-fin-schet))
    v-status_  = X_receiver-fin-schet.status_
    .
run ref/finschts.w (
  INPUT parParentProc
  ,p-curr-host-code
  ,input "b-sel,b-add":U
  ,input "cmp-host":U
  ,input X_receiver.obj-type
  ,input X_receiver.obj-code
  ,input ?
  ,input tt-fin-doc.host-code
  ,input 0
  ,input-output v-status_
...
if v-rid-list = "" then
do:
  apply "entry" to b-receiver-schet in frame Dialog-Frame.
  return no-apply.
end.
ref-rec = integer( v-rid-list ).
FIND FIRST buf_fin-schet WHERE
...
do:
  message
    "Валюта выбранного Вами счета" 'ПОЛУЧАТЕЛЯ' "не совпадает с валютой платежа"
    view-as alert-box error .
  return no-apply.
end.
if buf_fin-schet.status_ <> 'тек':U then
do:
  message
    "Статус выбранного Вами счета" 'ПОЛУЧАТЕЛЯ' buf_fin-schet.status_ " - нельзя работать с таким счетом"
    view-as alert-box error .
  return no-apply.
end.
FIND FIRST X_receiver-fin-schet WHERE
  recid (X_receiver-fin-schet) = ref-rec NO-LOCK .
find first X_receiver-fin-bank no-lock where
  X_receiver-fin-bank.host-code = tt-fin-doc.host-code
  AND X_receiver-fin-bank.code-bank = X_receiver-fin-schet.code-bank .
assign
  tt-fin-doc.receiver-bank-name  = X_receiver-fin-bank.bank-name
  tt-fin-doc.receiver-bank-city  = X_receiver-fin-bank.bank-city
  tt-fin-doc.receiver-dop1       = X_receiver-fin-schet.dop1
  tt-fin-doc.receiver-dop2       = X_receiver-fin-schet.dop2
  tt-fin-doc.receiver-bik        = X_receiver-fin-bank.bik
  tt-fin-doc.receiver-c-schet    = X_receiver-fin-schet.c-schet
  tt-fin-doc.receiver-r-schet    = X_receiver-fin-schet.r-schet
  tt-fin-doc.receiver-code-schet = X_receiver-fin-schet.code-schet
  .
display
...
PROCEDURE disable-enable :
  ...
  CASE rs-view:
    when "full":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-print,b-hist,b-help," +                             "prn-doc-code,vid-plat,doc-date,obj-type,obj-code,b-obj," +                             v-an-uchet-tab-order +  "b-payer-schet,b-payer-view," + v-sum-curr-tab-order +                             "receiver-type,receiver-code,b-receiver,receiver-name,b-receiver-schet,b-receiver-view," +                              "naznach-plat,PS,payer-sign1,payer-sign2"
          .
      end.
    when "brief":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-print,b-hist,b-help," +                              "prn-doc-code,vid-plat,doc-date," +                             "obj-type,obj-code,b-obj,b-payer-schet,b-payer-view," + v-sum-curr-tab-order +                             "receiver-type,receiver-code,b-receiver,receiver-name,b-receiver-schet,b-receiver-view," +                              "naznach-plat,PS,payer-sign1,payer-sign2"
          .
      end.
    when "contract":u then
      do:
        assign
          v-tab-order = "b-exit,b-quit,b-tax,b-print,b-hist,b-help," +                             "prn-doc-code,vid-plat,doc-date,obj-type,obj-code,b-obj," +                              v-contract-tab-order +  "b-payer-schet,b-payer-view," + v-sum-curr-tab-order +                             "receiver-type,receiver-code,b-receiver,receiver-name,b-receiver-schet,b-receiver-view," +                              "naznach-plat,PS,payer-sign1,payer-sign2"
          .
      end.
  END CASE.
END PROCEDURE.
...
procedure proc-update-sum-vat-chr :
  ...
  do
        ...
        do:
          ...
          do:
            v-each-vat-chr = v-each-vat-chr +
              substitute (" &1% = &2;"
              , string(tt0-fin-doc-tax.vat-pc)
              , string(truncate(tt0-fin-doc-tax.sum-vat-line-doc, 0)) + chr(32) +  "руб." + chr(32) +
              (if tt0-fin-doc-tax.sum-vat-line-doc <> truncate(tt0-fin-doc-tax.sum-vat-line-doc, 0)
              then (string(100 * round(tt0-fin-doc-tax.sum-vat-line-doc - truncate(tt0-fin-doc-tax.sum-vat-line-doc, 0), 2))  + chr(32) + "коп.")
              else "":U)).
          end.
          else
          do:
    ...
    do:
      assign
        v-sum-vat-chr = "в том числе НДС" + chr(32) +
                      string(v-sum-vat)
        .
    end.
    if p-start  = 0 then
    ...
    do:
      assign
        entry(2, tt-fin-doc.naznach-plat, "@":U) = chr(44) + chr(32)  + v-sum-vat-chr.
    end.
    display
      tt-fin-doc.naznach-plat
      with frame Dialog-Frame .
...
PROCEDURE enable_UI :
  ...
  IF AVAILABLE tt-fin-doc THEN
    DISPLAY tt-fin-doc.prn-doc-code tt-fin-doc.vid-plat tt-fin-doc.fin-doc-code
          ...
          tt-fin-doc.receiver-type tt-fin-doc.receiver-code
          ...
          tt-fin-doc.payer-sign2
      WITH FRAME Dialog-Frame.
  ENABLE B-exit b-quit b-other B-tax B-print B-hist B-Help RS-view
         ...
         tt-fin-doc.payer-name B-payer-schet tt-fin-doc.payer-bank-name
         tt-fin-doc.payer-bank-city B-payer-view tt-fin-doc.payer-c-schet
         tt-fin-doc.sum-doc f-rest-con-sum tt-fin-doc.curr-code B-currency
         B-calc tt-fin-doc.exch-rate tt-fin-doc.exch-scale tt-fin-doc.sum-rubl
         tt-fin-doc.sum-base tt-fin-doc.contract-rate tt-fin-doc.contract-scale
         tt-fin-doc.sum-contr B-receiver tt-fin-doc.receiver-type
         tt-fin-doc.receiver-code tt-fin-doc.receiver-name B-receiver-schet
         tt-fin-doc.receiver-bank-city B-receiver-view tt-fin-doc.naznach-plat
...
define variable g-log as logical no-undo.
tt-fin-doc.payer-inn:label in frame Dialog-Frame = "ИНН".
tt-fin-doc.receiver-inn:label in frame Dialog-Frame = "ИНН".
tt-fin-doc.payer-kpp:label in frame Dialog-Frame = "КПП".
tt-fin-doc.receiver-kpp:label in frame Dialog-Frame = "КПП".
assign
...
tt-fin-doc.obj-type:radio-buttons = "Маг" + chr(44) + 'маг':U + chr(44) +
                                    "Скл" + chr(44) + 'скл':U
tt-fin-doc.vid-plat:list-items = 'почтой,телеграфом,электронно,':U
rs-view = v-view
...
assign
  f-curr-abbr = X_Currency.curr-abbr.
if p-mode = 'ДОБАВЛЕНИЕ':U and tt-fin-doc.receiver-type = "":U then do:
  assign
...
END.
if p-mode = 'ПРОСМОТР':U then do:
    run proc-color-widgets in this-procedure("f-an-uchet-descr,f-cel-nazn-descr,f-contract-curr-abbr,f-contract-date,f-contract-prn-code,f-contract-rate," +                         "f-contract-scale,f-contract-type,f-cor-acc-descr,base-rate,base-scale,curr-code,exch-rate,exch-scale,contract-rate,contract-scale," +                         "fact-date,fin-doc-code,receiver-code,receiver-type,pay-date,perm-date,PS,payer-code,payer-type,sum-base,sum-rubl,sum-contr," +                         "user-name-doc,user-name-fact,user-name-perm.user-name-pl", no, yes, ?, ?).
    run proc-color-widgets in this-procedure("F-curr-abbr,an-uchet-value,cel-nazn-value,cor-acc-value,doc-date," +                       "naznach-plat,receiver-name,receiver-inn,receiver-kpp,receiver-bank-name,receiver-bank-city,receiver-bik,receiver-r-schet,receiver-c-schet," +                       "prn-doc-code,payer-name,payer-inn,payer-kpp,payer-bank-name,payer-bank-city,payer-bik,payer-r-schet,payer-c-schet,payer-sign1,payer-sign2" +                       "sum-doc", no, yes, ?, ?).
end.
display
tt-fin-doc.exch-rate
tt-fin-doc.exch-scale
