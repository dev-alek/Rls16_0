...
assign
v-add-chg = 'ДОБАВЛЕНИЕ':U.
run proc-b-add-chg in this-procedure (input 'ДОБАВЛЕНИЕ':U) no-error.
...
assign
v-add-chg = 'ИЗМЕНЕНИЕ':U.
run proc-b-add-chg in this-procedure (input 'ИЗМЕНЕНИЕ':U) no-error.
if error-status:error then do:
    assign
    v-add-chg = "":U.
    return no-apply.
...
END.
ON CHOOSE OF B-not-ok IN FRAME Dialog-Frame
DO:
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-fin-doc-tax :handle
      .
...
if "b-add,b-chg,b-del," + v-change-tab-order + "b-quit,b-exit" <> '' then do:
  if self:type = "TOGGLE-BOX" then
  self:BGCOLOR = ?.
  assign
  ii = lookup(self:name, "b-add,b-chg,b-del," + v-change-tab-order + "b-quit,b-exit").
  assign
  ii = ii + 1
  v-next-widget-name = entry(ii, "b-add,b-chg,b-del," + v-change-tab-order + "b-quit,b-exit")
  no-error .
  if error-status:error then do:
    assign
    ii = 1
...
if "b-add,b-chg,b-del," + v-change-tab-order + "b-quit,b-exit" <> '' then do:
  ...
  assign
  ii = (if ii = 1
        then  num-entries("b-add,b-chg,b-del," + v-change-tab-order + "b-quit,b-exit")
        else ii - 1
        )
  v-next-widget-name = entry(ii, "b-add,b-chg,b-del," + v-change-tab-order + "b-quit,b-exit")
  .
...
do:
  BR-fin-doc-tax :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
B-chg  when lookup('ПРОСМОТР':U, p-mode, chr(4)) = 0
...
with frame Dialog-Frame.
assign
v-change-tab-order = "f-sum-doc,T-with-vat,T-vatpc,f-vat-pc,T-vatsum,f-sum-vat,T-with-slt,T-sltpc,f-slt-pc,T-sltsum,f-sum-slt,b-ok,b-not-ok,".
...
hide
f-slt-pc in frame Dialog-Frame
f-sum-doc
f-sum-vat
f-sum-slt
f-vat-pc
...
b-not-ok
in frame Dialog-Frame.
DISPLAY BR-fin-doc-tax WITH FRAME Dialog-Frame .
enable
b-exit
b-add
b-chg
...
find first buf_tt-fin-doc-tax no-lock where
           buf_tt-fin-doc-tax.line-num = (if v-add-chg = 'ДОБАВЛЕНИЕ':U
                                          then (last-line + 1)
