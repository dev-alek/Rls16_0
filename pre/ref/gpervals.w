...
DEFINE BROWSE BR-gperval
  QUERY BR-gperval NO-LOCK DISPLAY
      ...
      entry (lookup (STRING(X_group-period-validity.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
...
if loc-doc-rec <> ? THEN DO:
    ...
    if error-status:error then do:                           find first pos_group-period-validity no-lock where                                   recid(pos_group-period-validity) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ГРУППЫ СРОКОВ ГОДНОСТИ" skip                            string(if avail pos_group-period-validity                                     then  substitute("Вн код группы сроков годности: &1"                                                     , pos_group-period-validity.gr-per-val-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
END.
...
assign
  v-str-recid10 = trim( string( recid( X_group-period-validity ) , "->>>>>>>>>>>9":U ) )
...
END.
ON CHOOSE OF B-var-deliv-gr-per-val IN FRAME Dialog-Frame
DO:
      DEFINE VARIABLE v-rid-list AS CHARACTER NO-UNDO.
...
on start-search, ctrl-o of BR-gperval in frame Dialog-Frame do:
   run sort-brBR-gperval
     (input (if available X_group-period-validity
             then recid(X_group-period-validity)
             else ?
            )
     ).
end.
PROCEDURE sort-brBR-gperval :
  ...
  case sort-labelBR-gperval:
        when X_group-period-validity.gr-per-val-code:label in browse BR-gperval then DO:   assign     sort-column-name = "X_group-period-validity.gr-per-val-code"   .   run OpenBr in this-procedure .   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
do:
  BR-gperval :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-gperval FOR EACH X_group-period-validity NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyENable :
ASSIGN
...
if not loc#log then return error.
  ...
  if error-status:error then do:                           find first pos_group-period-validity no-lock where                                   recid(pos_group-period-validity) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ГРУППЫ СРОКОВ ГОДНОСТИ" skip                            string(if avail pos_group-period-validity                                     then  substitute("Вн код группы сроков годности: &1"                                                     , pos_group-period-validity.gr-per-val-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
  if available X_group-period-validity then do:
    loc#log = br-gperval:select-focused-row( ) IN FRAME Dialog-Frame.
  end.
  apply "ENTRY" to br-gperval.
end.
END PROCEDURE.
