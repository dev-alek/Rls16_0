...
assign
  v-str-recid10 = trim( string( recid( X_c-units ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-cunits in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-cunits in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-cunits :handle
      .
...
do:
  v-doc-rec = recid(X_units). run openbr in this-procedure. reposition br-cunits to recid v-doc-rec no-error.               apply 'value-changed' to br-cunits.
    apply "VALUE-CHANGED" to BR-changes.
end.
def var sort-labelbr-cunits   as character no-undo .
def var sort-clmnbr-cunits    as handle    no-undo .
def var cur-clmnbr-cunits     as handle    no-undo .
def var cur-clmn-locbr-cunits as integer   no-undo .
def var re-querybr-cunits     as logical   initial no no-undo .
on start-search, ctrl-o of br-cunits in frame Dialog-Frame do:
   run sort-brbr-cunits
...
PROCEDURE sort-brbr-cunits :
  define input parameter p-recid as recid no-undo .
  if re-querybr-cunits = no then do:
    assign
       cur-clmnbr-cunits = br-cunits:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-cunits <> ? then sort-clmnbr-cunits:column-fgcolor = 0.
    if cur-clmnbr-cunits = sort-clmnbr-cunits then do:
      assign
         sort-labelbr-cunits = ""
         sort-clmnbr-cunits = ?
      .
       ...
       assign
         sort-labelbr-cunits = cur-clmnbr-cunits:label
         sort-clmnbr-cunits  = cur-clmnbr-cunits
         sort-clmnbr-cunits:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-cunits = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-cunits:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-cunits then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-cunits = cur-clmn-locbr-cunits + 1
    .
  end.
  case sort-labelbr-cunits:
        when X_c-units.unit-name:label in browse br-cunits then DO:   assign     sort-column-name = "X_c-units.unit-name"   .   run OpenBr.   . END.
        when X_c-units.corr-date:label in browse br-cunits then DO:   assign     sort-column-name = "X_c-units.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-cunits') then do:
          run mv-brw-defaultbr-cunits.
        end.
      if sort-labelbr-cunits <> "" then do:
        assign
          cur-clmnbr-cunits:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-cunits = ?
      .
    end.
  end case.
    if cur-clmn-locbr-cunits <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-cunits') then do:
        run ch-clmnbr-cunits in this-procedure (cur-clmn-locbr-cunits).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-cunits to recid p-recid no-error.
    apply "value-changed" to br-cunits in frame Dialog-Frame.
  end.
  apply "entry" to br-cunits in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-cunits:
if cur-clmnbr-cunits = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-cunits = yes.
   run sort-brbr-cunits
     (input (if available temp-changes
             then recid(temp-changes)
             else ?
...
DEF VAR cur-clmn-numbr-cunits as INT EXTENT 6 no-undo.
DEF VAR varmvibr-cunits       as INT no-undo.
DEF VAR varmvjbr-cunits       as INT no-undo.
DEF VAR varmvkbr-cunits       as INT no-undo.
DEF VAR varmvlbr-cunits       as INT no-undo.
DEF VAR move-elementbr-cunits as INT no-undo.
def var jjbr-cunits           as int no-undo.
do varmvibr-cunits = 1 to EXTENT(cur-clmn-numbr-cunits):
  ASSIGN cur-clmn-numbr-cunits[varmvibr-cunits] = varmvibr-cunits.
END.
RUN start-mv-clmnbr-cunits.
PROCEDURE start-mv-clmnbr-cunits:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-cunits = NUM-ENTRIES('1,2,3,4,5,6') TO 1 BY -1:
     RUN re-move-clmnbr-cunits ( cur-clmn-numbr-cunits[INTEGER(ENTRY (jjbr-cunits, '1,2,3,4,5,6'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-cunits do:
  RUN re-move-clmnbr-cunits ( 1, 6).
END.
ON ctrl-cursor-left OF BROWSE br-cunits do:
  RUN re-move-clmnbr-cunits (6, 1).
END.
PROCEDURE re-move-clmnbr-cunits:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-cunits = 1 TO EXTENT(cur-clmn-numbr-cunits):
    if cur-clmn-numbr-cunits[varmvibr-cunits] = source-column THEN cur-clmn-numbr-cunits[varmvibr-cunits] = -1.
  END.
  if br-cunits:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-cunits = source-column - 1 to target-column BY -1:
    DO varmvibr-cunits = 1 TO EXTENT(cur-clmn-numbr-cunits):
        if cur-clmn-numbr-cunits[varmvibr-cunits] = varmvjbr-cunits THEN DO:
          cur-clmn-numbr-cunits[varmvibr-cunits] = cur-clmn-numbr-cunits[varmvibr-cunits] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-cunits = source-column + 1 to target-column:
    DO varmvibr-cunits = 1 TO EXTENT(cur-clmn-numbr-cunits):
      if cur-clmn-numbr-cunits[varmvibr-cunits] = varmvjbr-cunits THEN DO:
        cur-clmn-numbr-cunits[varmvibr-cunits] = cur-clmn-numbr-cunits[varmvibr-cunits] - 1.
      END.
  ...
  END.
  DO varmvibr-cunits = 1 TO EXTENT(cur-clmn-numbr-cunits):
    if cur-clmn-numbr-cunits[varmvibr-cunits] = -1 THEN cur-clmn-numbr-cunits[varmvibr-cunits] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-cunits:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-cunits = 1 TO EXTENT(cur-clmn-numbr-cunits):
    if cur-clmn-numbr-cunits[varmvibr-cunits] = cur-clmn-loc THEN move-elementbr-cunits = varmvibr-cunits.
  END.
  RUN re-move-clmnbr-cunits (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-cunits:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-cunits = 1 to EXTENT(cur-clmn-numbr-cunits):
    RUN re-move-clmnbr-cunits (cur-clmn-numbr-cunits[varmvlbr-cunits], varmvlbr-cunits).
  END.
  RUN start-mv-clmnbr-cunits.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-cunits FOR EACH X_c-units NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
  "unit-name" + chr(4) + "Название" + chr(4) + "" + chr(8)
 + "OKEI" + chr(4) + "ОКЕИ" + chr(4) + "" + chr(8)
 + "type" + chr(4) + "Тип" + chr(4) + "" + chr(8)
 + "long-name" + chr(4) + "Длинное названи" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-units:handle
                                            ,input  'units':U
                                            ,input  "unit-name,OKEI,type,long-name"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
