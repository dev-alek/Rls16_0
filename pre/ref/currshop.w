...
if glog then do:
  run str/diallog.w (
        ...
        , input (p-obj-type + chr(4) + string(p-obj-code) + chr(4) + "U":U)
        , input no
        , input "":U
...
END.
END.
ON CHOOSE OF B-repeat IN FRAME Dialog-Frame
DO:
  define variable glog as logical no-undo .
  assign
  glog = no.
 ...
 OPEN QUERY br-currency FOR EACH X_currency       WHERE X_currency.curr-code > 0 NO-LOCK INDEXED-REPOSITION.
 OPEN QUERY BR-curr-shop FOR EACH X_curr-shop       WHERE X_curr-shop.obj-type = p-obj-type  AND X_curr-shop.obj-code = p-obj-code  AND X_curr-shop.curr-code = X_currency.curr-code NO-LOCK     BY X_curr-shop.exch-date DESCENDING      BY X_curr-shop.exch-time DESCENDING INDEXED-REPOSITION.    OPEN QUERY br-currency FOR EACH X_currency       WHERE X_currency.curr-code > 0 NO-LOCK INDEXED-REPOSITION.
apply "value-changed":U to br-currency IN FRAME Dialog-Frame.
END.
...
DO:
  ...
  OPEN QUERY BR-curr-shop FOR EACH X_curr-shop       WHERE X_curr-shop.obj-type = p-obj-type  AND X_curr-shop.obj-code = p-obj-code  AND X_curr-shop.curr-code = X_currency.curr-code NO-LOCK     BY X_curr-shop.exch-date DESCENDING      BY X_curr-shop.exch-time DESCENDING INDEXED-REPOSITION.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-curr-shop :handle
      .
