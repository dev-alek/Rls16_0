...
define variable vss-workfile    as character no-undo init "$Workfile: cndkeep2.p $":U .
define variable vss-archive     as character no-undo init "$Archive: ref/cndkeep2.p $":U .
...
CASE par-sts:
  WHEN integer('0':U) then do:
    if integer('0':U) = bf-condition-keeping.sts  then do:
  ...
  WHEN integer('1':U) then do:
    if integer('1':U) = bf-condition-keeping.sts  then do:
