...
DEFINE BROWSE BR-c-tax-hist
  QUERY BR-c-tax-hist DISPLAY
      ...
      X_c-tax-hist.corr-user-db-num COLUMN-LABEL "БД" FORMAT ">>>>9":U
            WIDTH 6
      entry (lookup (X_c-tax-hist.subject, 'tax,tax-rate,tax-rate-value,tax-units':U), 'Налог,Ставка налога,Знач.ставки налога,Налоги на тип ед.изм.':U) COLUMN-LABEL "Предмет изменений" FORMAT "X(12)":U
      ...
      string(X_c-tax-hist.corr-time, "HH:MM") COLUMN-LABEL "Время!корр" FORMAT "X(5)":U
            WIDTH 6
      if X_c-tax-hist.subject = 'tax-rate':U or X_c-tax-hist.subject = 'tax-rate-value':U then string(X_c-tax-hist.rate-code) else '':U COLUMN-LABEL "Код!ставки" FORMAT "X(3)":U
            WIDTH 7
      if X_c-tax-hist.subject = 'tax-rate-value':U and X_c-tax-hist.host-code > 0 then string(X_c-tax-hist.host-code) else '':U COLUMN-LABEL "Фирма" FORMAT "X(5)":U
            WIDTH 6
      if X_c-tax-hist.subject = 'tax-rate-value':U and X_c-tax-hist.obj-code > 0 then
(X_c-tax-hist.obj-type + string(X_c-tax-hist.obj-code)) else '':U COLUMN-LABEL "Объект" FORMAT "X(9)":U
            WIDTH 10
      if X_c-tax-hist.subject = 'tax-rate-value':U then
get-fact-date-str(X_c-tax-hist.fact-order) else '':U COLUMN-LABEL "Дата факт" FORMAT "X(10)":U
            WIDTH 11
      if X_c-tax-hist.subject = 'tax-rate-value':U then string(X_c-tax-hist.status_) else '':U COLUMN-LABEL "Статус" FORMAT "X(10)":U
            WIDTH 11
...
assign
  v-str-recid15 = trim( string( recid( X_c-tax-hist ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry15 > 0 then do:
 ...
 g#log = br-c-tax-hist :refresh( )  in frame Dialog-Frame.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
          g#log = br-c-tax-hist:select-next-row () in frame Dialog-Frame.
          apply "value-changed" to br-c-tax-hist in frame Dialog-Frame.
  end.
  ...
  apply "entry" to br-c-tax-hist in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-c-tax-hist :handle
      .
...
do:
  v-doc-rec = recid(X_c-tax-hist). run OpenBr in this-procedure ( input yes, input no, input '':U). reposition br-c-tax-hist to recid v-doc-rec no-error.
    apply "VALUE-CHANGED" to BR-c-tax-hist.
end.
define variable vss-include-info20 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-c-tax-hist :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
CASE parref-mode:
    when 'все':U then do:
        ASSIGN frame Dialog-Frame:TITLE = "Справочник истории категорий и ставок налогов"
...
run gbl/flt-get.p
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-tax-hist:handle
  ...
  if l-filter-open-23 = false then do:
    OPEN QUERY br-c-tax-hist FOR EACH X_c-tax-hist
      where  TRUE
  .
      assign
        l-query-was-opened = true
  ...
  run waitfram-hide in this-procedure .
    ...
    when "tax":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории категорий и ставок налогов: код налога &1",
                                                       p-tax-code)
        filter-point = filter-point0 + parref-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-25 as character no-undo .
      ...
      parameter-4-25 =
        (
          if (" X_c-tax-hist.tax-code = p-tax-code " + " " + where-phrase-25) <> ""
          then  substitute('X_c-tax-hist.tax-code = &1', p-tax-code ) + " " + where-phrase-25
          else "true"
        )
      parameter-5-25 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-25 =
          (" X_c-tax-hist.tax-code = p-tax-code " + " " + where-phrase-25 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-tax-hist:handle
  ...
  if l-filter-open-25 = false then do:
    OPEN QUERY br-c-tax-hist FOR EACH X_c-tax-hist
      where  X_c-tax-hist.tax-code = p-tax-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "tax-rate":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории категорий и ставок налогов: код налога &1 код ставки &2"
                                                       , p-tax-code
                                                       , p-rate-code
                                                       )
        filter-point = filter-point0 + parref-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          if (" X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = 'tax-rate':U " + " " + where-phrase-27) <> ""
          then  substitute('X_c-tax-hist.tax-code = &1 AND                             X_c-tax-hist.rate-code = &2 AND                             X_c-tax-hist.subject = &3&4&3', p-tax-code, p-rate-code, chr(34), 'tax-rate':U)  + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-27 =
          (" X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = 'tax-rate':U " + " " + where-phrase-27 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-tax-hist:handle
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-c-tax-hist FOR EACH X_c-tax-hist
      where  X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = 'tax-rate':U
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "tax-rate-value":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории категорий и ставок налогов: код налога &1 код ставки &2: значения ставок"
                                                       , p-tax-code
                                                       , p-rate-code
                                                       )
        filter-point = filter-point0 + parref-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-29 as character no-undo .
      ...
      parameter-4-29 =
        (
          if (" X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = 'tax-rate-value':U " + " " + where-phrase-29) <> ""
          then  substitute('X_c-tax-hist.tax-code = &1 AND                             X_c-tax-hist.rate-code = &2 AND                             X_c-tax-hist.subject = &3&4&3', p-tax-code, p-rate-code , chr(34), 'tax-rate-value':U) + " " + where-phrase-29
          else "true"
        )
      parameter-5-29 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-29 =
          (" X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = 'tax-rate-value':U " + " " + where-phrase-29 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-tax-hist:handle
  ...
  if l-filter-open-29 = false then do:
    OPEN QUERY br-c-tax-hist FOR EACH X_c-tax-hist
      where  X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = 'tax-rate-value':U
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "subject":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории категорий и ставок налогов: код налога &1 код ставки &2 предмет изменений &3"
                                                       , p-tax-code
                                                       , p-rate-code
                                                       , entry (lookup (p-subject, 'tax,tax-rate,tax-rate-value,tax-units':U), 'Налог,Ставка налога,Знач.ставки налога,Налоги на тип ед.изм.':U)
                                                       )
        filter-point = filter-point0 + parref-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      ...
      parameter-4-31 =
        (
          if (" X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = p-subject " + " " + where-phrase-31) <> ""
          then  substitute('X_c-tax-hist.tax-code = &1 AND                             X_c-tax-hist.rate-code = &2 AND                             X_c-tax-hist.subject = &3&4&3 ', p-tax-code, p-rate-code, p-subject) + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          (" X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = p-subject " + " " + where-phrase-31 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-tax-hist:handle
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-c-tax-hist FOR EACH X_c-tax-hist
      where  X_c-tax-hist.tax-code = p-tax-code AND                             X_c-tax-hist.rate-code = p-rate-code AND                             X_c-tax-hist.subject = p-subject
  .
      assign
        l-query-was-opened = true
      .
