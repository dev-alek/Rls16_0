...
assign
  v-str-recid10 = trim( string( recid( X_c-country ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry10 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-ccountry in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-ccountry in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-ccountry :handle
      .
...
do:
  v-doc-rec = recid(X_country). run openbr in this-procedure. reposition br-ccountry to recid v-doc-rec no-error.              apply 'value-changed' to br-ccountry.
    apply "VALUE-CHANGED" to br-ccountry.
end.
def var sort-labelbr-ccountry   as character no-undo .
def var sort-clmnbr-ccountry    as handle    no-undo .
def var cur-clmnbr-ccountry     as handle    no-undo .
def var cur-clmn-locbr-ccountry as integer   no-undo .
def var re-querybr-ccountry     as logical   initial no no-undo .
on start-search, ctrl-o of br-ccountry in frame Dialog-Frame do:
   run sort-brbr-ccountry
     (input (if available X_c-country
             then recid(X_c-country)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-ccountry :
  define input parameter p-recid as recid no-undo .
  if re-querybr-ccountry = no then do:
    assign
       cur-clmnbr-ccountry = br-ccountry:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-ccountry <> ? then sort-clmnbr-ccountry:column-fgcolor = 0.
    if cur-clmnbr-ccountry = sort-clmnbr-ccountry then do:
      assign
         sort-labelbr-ccountry = ""
         sort-clmnbr-ccountry = ?
      .
     end.
     else do:
       assign
         sort-labelbr-ccountry = cur-clmnbr-ccountry:label
         sort-clmnbr-ccountry  = cur-clmnbr-ccountry
         sort-clmnbr-ccountry:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-ccountry = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-ccountry:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-ccountry then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-ccountry = cur-clmn-locbr-ccountry + 1
    .
  end.
  case sort-labelbr-ccountry:
        when X_c-country.short-name:label in browse br-ccountry then DO:   assign     sort-column-name = "X_c-country.short-name"   .   run OpenBr.   . END.
        when X_c-country.corr-date:label in browse br-ccountry then DO:   assign     sort-column-name = "X_c-country.corr-date"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr.
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-ccountry') then do:
          run mv-brw-defaultbr-ccountry.
        end.
      if sort-labelbr-ccountry <> "" then do:
        assign
          cur-clmnbr-ccountry:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-ccountry = ?
      .
    end.
  end case.
    if cur-clmn-locbr-ccountry <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-ccountry') then do:
        run ch-clmnbr-ccountry in this-procedure (cur-clmn-locbr-ccountry).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-ccountry to recid p-recid no-error.
    apply "value-changed" to br-ccountry in frame Dialog-Frame.
  end.
  apply "entry" to br-ccountry in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-ccountry:
if cur-clmnbr-ccountry = ? then do:
   run OpenBr.
end.
else do:
   assign re-querybr-ccountry = yes.
   run sort-brbr-ccountry
     (input (if available X_c-country
             then recid(X_c-country)
             else ?
   ...
   assign re-querybr-ccountry = no.
end.
end.
define variable vss-include-info15 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-ccountry :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
DEF VAR cur-clmn-numbr-ccountry as INT EXTENT 7 no-undo.
DEF VAR varmvibr-ccountry       as INT no-undo.
DEF VAR varmvjbr-ccountry       as INT no-undo.
DEF VAR varmvkbr-ccountry       as INT no-undo.
DEF VAR varmvlbr-ccountry       as INT no-undo.
DEF VAR move-elementbr-ccountry as INT no-undo.
def var jjbr-ccountry           as int no-undo.
do varmvibr-ccountry = 1 to EXTENT(cur-clmn-numbr-ccountry):
  ASSIGN cur-clmn-numbr-ccountry[varmvibr-ccountry] = varmvibr-ccountry.
END.
RUN start-mv-clmnbr-ccountry.
PROCEDURE start-mv-clmnbr-ccountry:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'one'  THEN DO:
   DO jjbr-ccountry = NUM-ENTRIES('1,2,3,4,5,6,7') TO 1 BY -1:
     RUN re-move-clmnbr-ccountry ( cur-clmn-numbr-ccountry[INTEGER(ENTRY (jjbr-ccountry, '1,2,3,4,5,6,7'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-ccountry do:
  RUN re-move-clmnbr-ccountry ( 1, 7).
END.
ON ctrl-cursor-left OF BROWSE br-ccountry do:
  RUN re-move-clmnbr-ccountry (7, 1).
END.
PROCEDURE re-move-clmnbr-ccountry:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-ccountry = 1 TO EXTENT(cur-clmn-numbr-ccountry):
    if cur-clmn-numbr-ccountry[varmvibr-ccountry] = source-column THEN cur-clmn-numbr-ccountry[varmvibr-ccountry] = -1.
  END.
  if br-ccountry:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-ccountry = source-column - 1 to target-column BY -1:
    DO varmvibr-ccountry = 1 TO EXTENT(cur-clmn-numbr-ccountry):
        if cur-clmn-numbr-ccountry[varmvibr-ccountry] = varmvjbr-ccountry THEN DO:
          cur-clmn-numbr-ccountry[varmvibr-ccountry] = cur-clmn-numbr-ccountry[varmvibr-ccountry] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-ccountry = source-column + 1 to target-column:
    DO varmvibr-ccountry = 1 TO EXTENT(cur-clmn-numbr-ccountry):
      if cur-clmn-numbr-ccountry[varmvibr-ccountry] = varmvjbr-ccountry THEN DO:
        cur-clmn-numbr-ccountry[varmvibr-ccountry] = cur-clmn-numbr-ccountry[varmvibr-ccountry] - 1.
      END.
  ...
  END.
  DO varmvibr-ccountry = 1 TO EXTENT(cur-clmn-numbr-ccountry):
    if cur-clmn-numbr-ccountry[varmvibr-ccountry] = -1 THEN cur-clmn-numbr-ccountry[varmvibr-ccountry] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-ccountry:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-ccountry = 1 TO EXTENT(cur-clmn-numbr-ccountry):
    if cur-clmn-numbr-ccountry[varmvibr-ccountry] = cur-clmn-loc THEN move-elementbr-ccountry = varmvibr-ccountry.
  END.
  RUN re-move-clmnbr-ccountry (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-ccountry:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-ccountry = 1 to EXTENT(cur-clmn-numbr-ccountry):
    RUN re-move-clmnbr-ccountry (cur-clmn-numbr-ccountry[varmvlbr-ccountry], varmvlbr-ccountry).
  END.
  RUN start-mv-clmnbr-ccountry.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-ccountry FOR EACH X_c-country NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
  "long-name" + chr(4) + "Название" + chr(4) + "" + chr(8)
 + "num-code" + chr(4) + "Цифровой код" + chr(4) + "" + chr(8)
 + "alpha1" + chr(4) + "Код страны" + chr(4) + "" + chr(8)
 + "alpha2" + chr(4) + "Код страны-2" + chr(4) + "" + chr(8)
 + "short-name" + chr(4) + "Короткое название" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-country:handle
                                            ,input  'country':U
                                            ,input  "long-name,num-code,alpha1,alpha2,short-name"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
