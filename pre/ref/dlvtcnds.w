...
DEFINE BROWSE br-delivtypecndk
  QUERY br-delivtypecndk NO-LOCK DISPLAY
      ...
      entry (lookup (STRING(X_deliv-type-cond-keep.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
...
if loc-doc-rec <> ? then  do:
  ...
  if error-status:error then do:                           find first pos_deliv-type-cond-keep no-lock where                                   recid(pos_deliv-type-cond-keep) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ВОЗМОЖНОСТИ ДОСТАВКИ ПО УСЛОВИЯМ ХРАНЕНИЯ" skip                            string(if avail pos_deliv-type-cond-keep                                     then  substitute("Вн код типа доставки: &1 вн код услоий хранения: &2"                                                     , pos_deliv-type-cond-keep.deliv-type-code                                                     , pos_deliv-type-cond-keep.cond-keep-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
end.
apply "entry" to br-delivtypecndk in frame Dialog-Frame.
apply "value-changed" to br-delivtypecndk in frame Dialog-Frame.
END.
...
assign
  v-str-recid14 = trim( string( recid( X_deliv-type-cond-keep ) , "->>>>>>>>>>>9":U ) )
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-delivtypecndk :handle
      .
...
def var sort-labelbr-delivtypecndk   as character no-undo .
def var sort-clmnbr-delivtypecndk    as handle    no-undo .
def var cur-clmnbr-delivtypecndk     as handle    no-undo .
def var cur-clmn-locbr-delivtypecndk as integer   no-undo .
def var re-querybr-delivtypecndk     as logical   initial no no-undo .
on start-search, ctrl-o of br-delivtypecndk in frame Dialog-Frame do:
   run sort-brbr-delivtypecndk
     (input (if available X_deliv-type-cond-keep
             then recid(X_deliv-type-cond-keep)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-delivtypecndk :
  define input parameter p-recid as recid no-undo .
  if re-querybr-delivtypecndk = no then do:
    assign
       cur-clmnbr-delivtypecndk = br-delivtypecndk:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-delivtypecndk <> ? then sort-clmnbr-delivtypecndk:column-fgcolor = 0.
    if cur-clmnbr-delivtypecndk = sort-clmnbr-delivtypecndk then do:
      assign
         sort-labelbr-delivtypecndk = ""
         sort-clmnbr-delivtypecndk = ?
      .
     end.
     else do:
       assign
         sort-labelbr-delivtypecndk = cur-clmnbr-delivtypecndk:label
         sort-clmnbr-delivtypecndk  = cur-clmnbr-delivtypecndk
         sort-clmnbr-delivtypecndk:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-delivtypecndk = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-delivtypecndk:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-delivtypecndk then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-delivtypecndk = cur-clmn-locbr-delivtypecndk + 1
    .
  end.
  case sort-labelbr-delivtypecndk:
        when X_deliv-type-cond-keep.deliv-type-code:label in browse br-delivtypecndk then DO:    assign       sort-column-name = "X_deliv-type-cond-keep.deliv-type-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-delivtypecndk') then do:
          run mv-brw-defaultbr-delivtypecndk.
        end.
      if sort-labelbr-delivtypecndk <> "" then do:
        assign
          cur-clmnbr-delivtypecndk:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-delivtypecndk = ?
      .
    end.
  end case.
    if cur-clmn-locbr-delivtypecndk <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-delivtypecndk') then do:
        run ch-clmnbr-delivtypecndk in this-procedure (cur-clmn-locbr-delivtypecndk).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-delivtypecndk to recid p-recid no-error.
    apply "value-changed" to br-delivtypecndk in frame Dialog-Frame.
  end.
  apply "entry" to br-delivtypecndk in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-delivtypecndk:
if cur-clmnbr-delivtypecndk = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U).
end.
else do:
   assign re-querybr-delivtypecndk = yes.
   run sort-brbr-delivtypecndk
     (input (if available X_deliv-type-cond-keep
             then recid(X_deliv-type-cond-keep)
             else ?
   ...
   assign re-querybr-delivtypecndk = no.
end.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-delivtypecndk :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
define variable vss-include-info28 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
 ...
 if LOOKUP(p-mode, ('все':U + chr(4) + "delivery-type" + chr(4) + "condition-keeping") ,
                chr(4)) = 0
     then dO:
    message
    vss-workfile vss-revision vss-description skip
...
DEF VAR cur-clmn-numbr-delivtypecndk as INT EXTENT 7 no-undo.
DEF VAR varmvibr-delivtypecndk       as INT no-undo.
DEF VAR varmvjbr-delivtypecndk       as INT no-undo.
DEF VAR varmvkbr-delivtypecndk       as INT no-undo.
DEF VAR varmvlbr-delivtypecndk       as INT no-undo.
DEF VAR move-elementbr-delivtypecndk as INT no-undo.
def var jjbr-delivtypecndk           as int no-undo.
do varmvibr-delivtypecndk = 1 to EXTENT(cur-clmn-numbr-delivtypecndk):
  ASSIGN cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = varmvibr-delivtypecndk.
END.
RUN start-mv-clmnbr-delivtypecndk.
PROCEDURE start-mv-clmnbr-delivtypecndk:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-delivtypecndk = NUM-ENTRIES('1,2,3,4,5,6,7') TO 1 BY -1:
     RUN re-move-clmnbr-delivtypecndk ( cur-clmn-numbr-delivtypecndk[INTEGER(ENTRY (jjbr-delivtypecndk, '1,2,3,4,5,6,7'))] , 1).
   END.
       END.
       IF  p-mode = 'delivery-type'  THEN DO:
   DO jjbr-delivtypecndk = NUM-ENTRIES('1,4,5,6,7,2,3') TO 1 BY -1:
     RUN re-move-clmnbr-delivtypecndk ( cur-clmn-numbr-delivtypecndk[INTEGER(ENTRY (jjbr-delivtypecndk, '1,4,5,6,7,2,3'))] , 1).
   END.
       END.
       IF  p-mode = 'condition-keeping'  THEN DO:
   DO jjbr-delivtypecndk = NUM-ENTRIES('1,2,3,6,7,4,5') TO 1 BY -1:
     RUN re-move-clmnbr-delivtypecndk ( cur-clmn-numbr-delivtypecndk[INTEGER(ENTRY (jjbr-delivtypecndk, '1,2,3,6,7,4,5'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-delivtypecndk do:
  RUN re-move-clmnbr-delivtypecndk ( 1, 7).
END.
ON ctrl-cursor-left OF BROWSE br-delivtypecndk do:
  RUN re-move-clmnbr-delivtypecndk (7, 1).
END.
PROCEDURE re-move-clmnbr-delivtypecndk:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-delivtypecndk = 1 TO EXTENT(cur-clmn-numbr-delivtypecndk):
    if cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = source-column THEN cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = -1.
  END.
  if br-delivtypecndk:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-delivtypecndk = source-column - 1 to target-column BY -1:
    DO varmvibr-delivtypecndk = 1 TO EXTENT(cur-clmn-numbr-delivtypecndk):
        if cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = varmvjbr-delivtypecndk THEN DO:
          cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-delivtypecndk = source-column + 1 to target-column:
    DO varmvibr-delivtypecndk = 1 TO EXTENT(cur-clmn-numbr-delivtypecndk):
      if cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = varmvjbr-delivtypecndk THEN DO:
        cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] - 1.
      END.
  ...
  END.
  DO varmvibr-delivtypecndk = 1 TO EXTENT(cur-clmn-numbr-delivtypecndk):
    if cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = -1 THEN cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-delivtypecndk:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-delivtypecndk = 1 TO EXTENT(cur-clmn-numbr-delivtypecndk):
    if cur-clmn-numbr-delivtypecndk[varmvibr-delivtypecndk] = cur-clmn-loc THEN move-elementbr-delivtypecndk = varmvibr-delivtypecndk.
  END.
  RUN re-move-clmnbr-delivtypecndk (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-delivtypecndk:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-delivtypecndk = 1 to EXTENT(cur-clmn-numbr-delivtypecndk):
    RUN re-move-clmnbr-delivtypecndk (cur-clmn-numbr-delivtypecndk[varmvlbr-delivtypecndk], varmvlbr-delivtypecndk).
  END.
  RUN start-mv-clmnbr-delivtypecndk.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-delivtypecndk FOR EACH X_deliv-type-cond-keep NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ASSIGN
...
title0 = "Список возможностей доставки по условиям хранения" + chr(32).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-32 as character no-undo .
      assign
      parameter-3-32 =
                              "FOR EACH X_deliv-type-cond-keep"
      parameter-4-32 =
        (
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-delivtypecndk:handle
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-delivtypecndk FOR EACH X_deliv-type-cond-keep
      where  TRUE
      indexed-reposition
  .
      assign
  ...
  on error undo, leave
  :
    if QUERY br-delivtypecndk:handle:get-buffer-handle(1) = (buffer X_deliv-type-cond-keep:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input QUERY br-delivtypecndk:handle
                          ,input rowid(X_deliv-type-cond-keep)
                          ,input logical(parameter-2-32)
                          ,input no-lock
                          ,input (buffer X_deliv-type-cond-keep:handle)
                          ,input parameter-4-32
                          ,input parameter-5-32
                          ) no-error.
      .
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_deliv-type-cond-keep"
      parameter-4-32 =
        (
          if (" TRUE " + " " + where-phrase-32) <> ""
          then " TRUE " + " " + where-phrase-32
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_deliv-type-cond-keep.sts = &1', p-sts ) + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-delivtypecndk:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-delivtypecndk FOR EACH X_deliv-type-cond-keep
      where  X_deliv-type-cond-keep.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-delivtypecndk:handle:get-buffer-handle(1) = (buffer X_deliv-type-cond-keep:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_deliv-type-cond-keep.sts = &1', p-sts ) + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_deliv-type-cond-keep"
      parameter-4-34 =
        (
          if (" X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_deliv-type-cond-keep.sts = &1', p-sts ) + " " + where-phrase-34
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if ("           X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code                          " + " " + where-phrase-36) <> ""
          then  substitute('X_deliv-type-cond-keep.deliv-type-code  = &1', p-deliv-type-code)  + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          ("           X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code                          " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-delivtypecndk:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-delivtypecndk FOR EACH X_deliv-type-cond-keep
      where            X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-delivtypecndk:handle:get-buffer-handle(1) = (buffer X_deliv-type-cond-keep:handle) then do:
      ...
      parameter-4-36 =
        "where ":u +  substitute('X_deliv-type-cond-keep.deliv-type-code  = &1', p-deliv-type-code)  + " ":u + where-phrase-36 + " ":u + p-find-condition + " " + ""
      parameter-5-36 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_deliv-type-cond-keep"
      parameter-4-36 =
        (
          if ("           X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code                          " + " " + where-phrase-36) <> ""
          then  substitute('X_deliv-type-cond-keep.deliv-type-code  = &1', p-deliv-type-code)  + " " + where-phrase-36
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-38 as character no-undo .
      ...
      parameter-4-38 =
        (
          if ("             X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code              AND X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-38) <> ""
          then  substitute(' X_deliv-type-cond-keep.deliv-type-code  = &1              AND X_deliv-type-cond-keep.sts = &2 ', p-deliv-type-code, p-sts) + " " + where-phrase-38
          else "true"
        )
      parameter-5-38 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-38 =
          ("             X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code              AND X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-38 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-delivtypecndk:handle
  ...
  if l-filter-open-38 = false then do:
    OPEN QUERY br-delivtypecndk FOR EACH X_deliv-type-cond-keep
      where              X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code              AND X_deliv-type-cond-keep.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-delivtypecndk:handle:get-buffer-handle(1) = (buffer X_deliv-type-cond-keep:handle) then do:
      ...
      parameter-4-38 =
        "where ":u +  substitute(' X_deliv-type-cond-keep.deliv-type-code  = &1              AND X_deliv-type-cond-keep.sts = &2 ', p-deliv-type-code, p-sts) + " ":u + where-phrase-38 + " ":u + p-find-condition + " " + ""
      parameter-5-38 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-38 = (if p-find-next then "true":u else "false":u )
      parameter-3-38 =  "FOR EACH X_deliv-type-cond-keep"
      parameter-4-38 =
        (
          if ("             X_deliv-type-cond-keep.deliv-type-code  = p-deliv-type-code              AND X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-38) <> ""
          then  substitute(' X_deliv-type-cond-keep.deliv-type-code  = &1              AND X_deliv-type-cond-keep.sts = &2 ', p-deliv-type-code, p-sts) + " " + where-phrase-38
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-40 as character no-undo .
      ...
      parameter-4-40 =
        (
          if ("         X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code                        " + " " + where-phrase-40) <> ""
          then  substitute(' X_deliv-type-cond-keep.cond-keep-code  = &1', p-cond-keep-code ) + " " + where-phrase-40
          else "true"
        )
      parameter-5-40 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-40 =
          ("         X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code                        " + " " + where-phrase-40 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-delivtypecndk:handle
  ...
  if l-filter-open-40 = false then do:
    OPEN QUERY br-delivtypecndk FOR EACH X_deliv-type-cond-keep
      where          X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-delivtypecndk:handle:get-buffer-handle(1) = (buffer X_deliv-type-cond-keep:handle) then do:
      ...
      parameter-4-40 =
        "where ":u +  substitute(' X_deliv-type-cond-keep.cond-keep-code  = &1', p-cond-keep-code ) + " ":u + where-phrase-40 + " ":u + p-find-condition + " " + ""
      parameter-5-40 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-40 = (if p-find-next then "true":u else "false":u )
      parameter-3-40 =  "FOR EACH X_deliv-type-cond-keep"
      parameter-4-40 =
        (
          if ("         X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code                        " + " " + where-phrase-40) <> ""
          then  substitute(' X_deliv-type-cond-keep.cond-keep-code  = &1', p-cond-keep-code ) + " " + where-phrase-40
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-42 as character no-undo .
      ...
      parameter-4-42 =
        (
          if ("            X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code               AND X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-42) <> ""
          then  substitute(' X_deliv-type-cond-keep.cond-keep-code  = &1              AND X_deliv-type-cond-keep.sts = &2 ', p-cond-keep-code, p-sts ) + " " + where-phrase-42
          else "true"
        )
      parameter-5-42 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-42 =
          ("            X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code               AND X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-42 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-delivtypecndk:handle
  ...
  if l-filter-open-42 = false then do:
    OPEN QUERY br-delivtypecndk FOR EACH X_deliv-type-cond-keep
      where             X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code               AND X_deliv-type-cond-keep.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-delivtypecndk:handle:get-buffer-handle(1) = (buffer X_deliv-type-cond-keep:handle) then do:
      ...
      parameter-4-42 =
        "where ":u +  substitute(' X_deliv-type-cond-keep.cond-keep-code  = &1              AND X_deliv-type-cond-keep.sts = &2 ', p-cond-keep-code, p-sts ) + " ":u + where-phrase-42 + " ":u + p-find-condition + " " + ""
      parameter-5-42 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-42 = (if p-find-next then "true":u else "false":u )
      parameter-3-42 =  "FOR EACH X_deliv-type-cond-keep"
      parameter-4-42 =
        (
          if ("            X_deliv-type-cond-keep.cond-keep-code  = p-cond-keep-code               AND X_deliv-type-cond-keep.sts = p-sts " + " " + where-phrase-42) <> ""
          then  substitute(' X_deliv-type-cond-keep.cond-keep-code  = &1              AND X_deliv-type-cond-keep.sts = &2 ', p-cond-keep-code, p-sts ) + " " + where-phrase-42
...
if not loc#log then return error.
  ...
  if error-status:error then do:                           find first pos_deliv-type-cond-keep no-lock where                                   recid(pos_deliv-type-cond-keep) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ВОЗМОЖНОСТИ ДОСТАВКИ ПО УСЛОВИЯМ ХРАНЕНИЯ" skip                            string(if avail pos_deliv-type-cond-keep                                     then  substitute("Вн код типа доставки: &1 вн код услоий хранения: &2"                                                     , pos_deliv-type-cond-keep.deliv-type-code                                                     , pos_deliv-type-cond-keep.cond-keep-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
  if available X_deliv-type-cond-keep then do:
    loc#log = br-delivtypecndk:select-focused-row( ) IN FRAME Dialog-Frame.
  end.
  apply "ENTRY" to br-delivtypecndk.
end.
END PROCEDURE.
