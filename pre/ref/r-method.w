...
define variable v-base-str as character no-undo.
  ...
  END.
  assign
   v-base-str = (if f-base:visible in frame Dialog-Frame
                    then string(f-base, "->>>>9.99":U)
                    else "":U)
                    .
  assign
  p-value = s-round-method + chr(32) + v-base-str
  .
END.
...
assign
v-round-method = entry(1, p-value, chr(32))
v-base = decimal(entry(2, p-value, chr(32)))
no-error.
if error-status:error and num-entries(p-value, chr(32)) > 0 then do:
    message
...
DISPLAY S-round-method f-base
  ...
  ENABLE b-quit B-exit B-Help S-round-method f-base
      WITH FRAME Dialog-Frame.
  VIEW FRAME Dialog-Frame.
s-round-method:screen-value in frame Dialog-Frame = v-round-method.
f-base = v-base.
