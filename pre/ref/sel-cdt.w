...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-cash-desk SHARE-LOCK.
...
DO v-ii = 1 TO NUM-ENTRIES('IBM-XML,Autotank,IBM,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,MARIA,-,bo':U):
   ...
   FIND FIRST buf_dis-cfg-rule NO-LOCK WHERE
       ...
       AND  buf_dis-cfg-rule.pos-type  = ENTRY(v-ii, 'IBM-XML,Autotank,IBM,IPC-Servis+,OMRON-NEW,OMRON,NCR-GM,MAGIA-XML,NCR-AS@R,IBS-TH,IBS-TH-MOB,r-keeper,MARIA,-,bo':U) NO-ERROR.
   iF NOT AVAILABLE buf_dis-cfg-rule THEN NEXT _v-ii.
   END.
   ASSIGN
    v-list-items = v-list-items +  (IF v-list-items = '':U THEN '':U ELSE chr(44)) +
...
END.
assign
cb-dflt-cd:list-item-pairs in frame Dialog-Frame = v-list-items.
DISPLAY CB-dflt-cd
WITH FRAME Dialog-Frame.
ENABLE
B-exit
