  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-prn-grp :handle
      .
...
CASE par-mode:
  when "printer":U then do:
    ASSIGN frame Dialog-Frame:TITLE = title0 + " Принтер: "+  string(p-prn-num).
    ...
    when "group":U then do:
      ASSIGN frame Dialog-Frame:TITLE = title0 + " Группа товаров: "+  string(p-node-code).
      OPEN QUERY BR-prn-grp
      FOR EACH buf_fbr-prn-grp NO-LOCK where
...
APPLY "ENTRY" to br-prn-grp in frame Dialog-Frame.
END PROCEDURE.
PROCEDURE proc-chg :
define variable v-rec as recid no-undo.
define variable loc#log as logical no-undo .
