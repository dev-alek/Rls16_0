...
define variable vss-include-info3 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  CASE Dc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str3 = string( p-code, "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str3 = string( p-code, "9999999" )
      .
    end.
    OTHERWISE DO:
  ...
  if varcont3 = yes then do:
    if integer( substring( tmp-str3, 1, length( Dc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = Dc-pfx + substring( tmp-str3, length( Dc-pfx ) + 1, length( tmp-str3 ) - length( Dc-pfx ) )
...
.
assign
file-name            = entry(1, p-parameter, chr(4))
p-stop-list-code     = entry(2, p-parameter, chr(4))
p-mode               = entry(3, p-parameter, chr(4))
v-close              = logical(entry(4, p-parameter, chr(4)))
no-error
.
if error-status:error then do:
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'in-stpl1.txt').
end.
                        return.
end.
if NOT g#db-num = 0 then do:
  run write-log-and-file in p-log-handle (
      ...
      , input log-file-name
      , input 1
      , input substitute("Импорт стоплистов по ДК возможен только в ГБД")).
  assign
  v-view-log = yes.
...
if error-status:error  = ? then do:
  run write-log-and-file in p-log-handle (
      ...
      , input log-file-name
      , input 1
      , input substitute("Не найден файл для импорта стоплиста по ДК &1", file-name)).
  assign
  v-view-log = yes.
  ...
  for each buf_dis-gds-rule no-lock where
    ...
    if available buf_dis-rule then do:
       for each term_dis-rule no-lock where
          ...
          temp-discounts.value-type = term_dis-rule.value-type
          .
...
output to temp-discounts.txt.
for each temp-discounts:
 export temp-discounts.
end.
output close.
if p-mode = 'ИЗМЕНЕНИЕ':U then do:
  find first buf0_stop-list exclusive-lock where
            buf0_stop-list.classif-type = 'dis-card':U
          and buf0_stop-list.stop-list-code = p-stop-list-code  no-error .
  if not available buf0_stop-list then do:
    run write-log-and-file in p-log-handle (
          input 1
        , input log-file-name
        , input 1
        , input substitute("Не найден стоплист &1", p-stop-list-code)).
    v-view-log = yes.
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  ...
  v-status_ = '':U
  .
  create buf0_stop-list.
  assign
  buf0_stop-list.classif-type = 'dis-card':U
  buf0_stop-list.stop-list-code = p-stop-list-code
  buf0_stop-list.doc-date = v-doc-date
  buf0_stop-list.status_ = 'новый':U
  .
  if error-status:error then do:
    run write-log-and-file in p-log-handle (
          input 1
        , input log-file-name
        , input 1
        , input substitute("Ошибка при записи шапки стоплиста &1&2" +
                            "&3&2&4"
                            ,p-stop-list-code
                            ,chr(10)
                            ,error-status:get-message(1)
...
if not v-stop then do:
  ...
  on error undo _stroka2, next _stroka2 :
    ...
    ON ERROR UNDO _stroka2, NEXT _stroka2:
      if temp-imp.del-status-int = 1 then do:
        v-status = 'удал':U.
        run ref/dcardi02.p (
                          input parparentproc
        ...
        if error-status:error then do:
          my-mess =  substitute("!!!Ошибка при изменении статуса ДК &1 для клиента &2&3&4" +
                                  "&5&4&6"
                                  ,buf_dis-card.d-card
                                  ,buf_dis-card.cli-type
                                  ,buf_dis-card.cli-code
      ...
      if temp-imp.cli-stop-list-flag = 1 then do:
        ...
        find last buf_stop-list-line no-lock where
              ...
              and buf_stop-list-line.stop-list-code = p-stop-list-code  no-error.
        ...
        :
          find first buf_stop-list-line no-lock where
                    buf_stop-list-line.classif-type = 'dis-card':U
                and buf_stop-list-line.stop-list-code = p-stop-list-code
          ...
          if not available buf_stop-list-line then do:
            ...
            assign
            buf_stop-list-line.classif-type = 'dis-card':U
            buf_stop-list-line.stop-list-code = p-stop-list-code
      ...
      run discprop-write in this-procedure (
                                              ...
                                              ,input 4
                                              ,input ?
                                              ,input substitute("petrol-&1"
                                                                ,(if temp-imp.product-code > 0
                                                                  then ext-product-code.gds-code else 0)
                                                                  )
                                              ,input '':U
                                              ,input ?
                                              ,input (- temp-imp.lim-kr)
                                              ,input 0
                                              ,input no
                                              ,input 'stop-list':U
                                              ,input p-stop-list-code
      ...
      if error-status:error then do:
        my-mess =  substitute("!!!Ошибка при внесении изменений по лимиту ДК &1 из стоплиста &2&3" +
                                "&4&3&5"
                                ,buf_dis-card.d-card
                                ,p-stop-list-code
                                ,chr(10)
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute("Сохранение стоплиста завершено: из &1 записей успешно сохранено &2", file-name, num-rec-write, num-rec-write-ok )).
if v-close
