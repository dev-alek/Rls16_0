...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH tt-country SHARE-LOCK.
...
tt-country.alpha1    WHEN p-mode = 'ДОБАВЛЕНИЕ':U
tt-country.alpha2    WHEN p-mode <> 'ПРОСМОТР':U
tt-country.num-code  WHEN p-mode = 'ДОБАВЛЕНИЕ':U
tt-country.short-name  WHEN p-mode <> 'ПРОСМОТР':U
tt-country.long-name  WHEN p-mode <> 'ПРОСМОТР':U
WITH FRAME Dialog-Frame.
VIEW FRAME Dialog-Frame.
CASE p-mode:
  ...
  WHEN 'ДОБАВЛЕНИЕ':U THEN DO:
    FRAME Dialog-Frame:TITLE = "Ввод СТРАНЫ" .
  END.
WHEN 'ИЗМЕНЕНИЕ':U THEN DO:
    FRAME Dialog-Frame:TITLE = "Редактирование СТРАНЫ".
  END.
END CASE.
END PROCEDURE.
PROCEDURE proc-save :
