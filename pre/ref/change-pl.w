...
ON WINDOW-CLOSE OF FRAME d-place
  DO:
...
assign
  v-str-recid13 = trim( string( recid( tt-place ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry13 > 0 then do:
  ...
  .
      ...
      loc#log = br-pl:refresh() .
      reposition br-pl to recid row_place no-error.
      if last-event:function <> "MOUSE-SELECT-DBLCLICK" then
      do:
        loc#log = br-pl:select-next-row ().
        apply "VALUE-CHANGED" to br-pl in frame d-place.
      end.
    end.
    apply "entry" to br-pl in frame d-place.
  END.
ON CHOOSE OF b-sel IN FRAME d-place
  DO:
...
assign
  v-str-recid15 = trim( string( recid( tt-place ) , "->>>>>>>>>>>9":U ) )
...
ON CHOOSE OF bt-not-sel-desel-all IN FRAME d-place
  DO:
    ...
    loc#log = br-pl:refresh() .
  END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME d-place:PARENT eq ?
  THEN FRAME d-place:PARENT = ACTIVE-WINDOW.
MAIN-BLOCK:
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
define variable vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-pl :SET-REPOSITIONED-ROW(9, "CONDITIONAL") .
end.
  RUN enable_UI.
  WAIT-FOR GO OF FRAME d-place.
END.
RUN disable_UI.
...
PROCEDURE enable_UI :
  ENABLE b-exit b-sel b-mark br-pl bt-not-sel-desel-all bt-not-sel-all
    WITH FRAME d-place.
  VIEW FRAME d-place.
  OPEN QUERY br-pl FOR EACH                           obj-list NO-LOCK,                                  EACH tt-place WHERE                           tt-place.obj-code = obj-list.obj-code and                           tt-place.obj-type = obj-list.obj-type                           NO-LOCK.
END PROCEDURE.
FUNCTION obj-name RETURNS CHARACTER
