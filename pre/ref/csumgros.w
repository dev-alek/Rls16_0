...
assign
  v-str-recid19 = trim( string( recid( X_c-sum-grp-obj ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry19 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-sum-grp-obj in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-sum-grp-obj in frame Dialog-Frame.
END.
...
DO:
  ...
  if error-status:error then return no-apply.
END.
ON CTRL-J OF sch-grp-code IN FRAME Dialog-Frame
DO:
  run proc-find-grp-code in this-procedure(yes, input frame Dialog-Frame sch-grp-code) no-error.
  if error-status:error then return no-apply.
END.
ON RETURN OF sch-grp-code IN FRAME Dialog-Frame
DO:
  run proc-find-grp-code in this-procedure(no, input frame Dialog-Frame sch-grp-code) no-error.
  if error-status:error then return no-apply.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-sum-grp-obj :handle
      .
...
do:
   v-doc-rec = recid(X_c-sum-grp-obj).  RUn OpenBR in this-procedure ( input yes, input no, input '':U).  REPOSITION br-sum-grp-obj to recid v-doc-rec No-ERROR.               apply 'value-changed' to br-sum-grp-obj.
...
do:
  ...
  then do:
    ASSIGN
      sch-corr-date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date26 :HANDLE
      sch-corr-date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle26 as handle no-undo .
  assign
    v-label-handle26 = sch-corr-date :side-label-handle in frame Dialog-Frame
  .
  ...
  END.
def var sort-labelbr-sum-grp-obj   as character no-undo .
def var sort-clmnbr-sum-grp-obj    as handle    no-undo .
def var cur-clmnbr-sum-grp-obj     as handle    no-undo .
def var cur-clmn-locbr-sum-grp-obj as integer   no-undo .
def var re-querybr-sum-grp-obj     as logical   initial no no-undo .
on start-search, ctrl-o of br-sum-grp-obj in frame Dialog-Frame do:
   run sort-brbr-sum-grp-obj
     (input (if available X_c-sum-grp-obj
             then recid(X_c-sum-grp-obj)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-sum-grp-obj :
  define input parameter p-recid as recid no-undo .
  if re-querybr-sum-grp-obj = no then do:
    assign
       cur-clmnbr-sum-grp-obj = br-sum-grp-obj:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-sum-grp-obj <> ? then sort-clmnbr-sum-grp-obj:column-fgcolor = 0.
    if cur-clmnbr-sum-grp-obj = sort-clmnbr-sum-grp-obj then do:
      assign
         sort-labelbr-sum-grp-obj = ""
         sort-clmnbr-sum-grp-obj = ?
      .
     end.
     else do:
       assign
         sort-labelbr-sum-grp-obj = cur-clmnbr-sum-grp-obj:label
         sort-clmnbr-sum-grp-obj  = cur-clmnbr-sum-grp-obj
         sort-clmnbr-sum-grp-obj:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-sum-grp-obj = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-sum-grp-obj:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-sum-grp-obj then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-sum-grp-obj = cur-clmn-locbr-sum-grp-obj + 1
    .
  end.
  case sort-labelbr-sum-grp-obj:
        when X_c-sum-grp-obj.grp-code:label in browse br-sum-grp-obj then DO:    assign       sort-column-name = "X_c-sum-grp-obj.grp-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-sum-grp-obj.corr-date:label in browse br-sum-grp-obj then DO:    assign       sort-column-name = "X_c-sum-grp-obj.corr-date"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-sum-grp-obj.corr-user-db-num:label in browse br-sum-grp-obj then DO:    assign       sort-column-name = "X_c-sum-grp-obj.corr-user-db-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-sum-grp-obj') then do:
          run mv-brw-defaultbr-sum-grp-obj.
        end.
      if sort-labelbr-sum-grp-obj <> "" then do:
        assign
          cur-clmnbr-sum-grp-obj:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-sum-grp-obj = ?
      .
    end.
  end case.
    if cur-clmn-locbr-sum-grp-obj <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-sum-grp-obj') then do:
        run ch-clmnbr-sum-grp-obj in this-procedure (cur-clmn-locbr-sum-grp-obj).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-sum-grp-obj to recid p-recid no-error.
    apply "value-changed" to br-sum-grp-obj in frame Dialog-Frame.
  end.
  apply "entry" to br-sum-grp-obj in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-sum-grp-obj:
if cur-clmnbr-sum-grp-obj = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U).
end.
else do:
   assign re-querybr-sum-grp-obj = yes.
   run sort-brbr-sum-grp-obj
     (input (if available X_c-sum-grp-obj
             then recid(X_c-sum-grp-obj)
             else ?
   ...
   assign re-querybr-sum-grp-obj = no.
end.
end.
...
DEF VAR cur-clmn-numbr-sum-grp-obj as INT EXTENT 8 no-undo.
DEF VAR varmvibr-sum-grp-obj       as INT no-undo.
DEF VAR varmvjbr-sum-grp-obj       as INT no-undo.
DEF VAR varmvkbr-sum-grp-obj       as INT no-undo.
DEF VAR varmvlbr-sum-grp-obj       as INT no-undo.
DEF VAR move-elementbr-sum-grp-obj as INT no-undo.
def var jjbr-sum-grp-obj           as int no-undo.
do varmvibr-sum-grp-obj = 1 to EXTENT(cur-clmn-numbr-sum-grp-obj):
  ASSIGN cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = varmvibr-sum-grp-obj.
END.
RUN start-mv-clmnbr-sum-grp-obj.
PROCEDURE start-mv-clmnbr-sum-grp-obj:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-sum-grp-obj = NUM-ENTRIES('1,2,3,4,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-sum-grp-obj ( cur-clmn-numbr-sum-grp-obj[INTEGER(ENTRY (jjbr-sum-grp-obj, '1,2,3,4,5,6,7,8'))] , 1).
   END.
       END.
       IF  p-mode <> 'все':U  THEN DO:
   DO jjbr-sum-grp-obj = NUM-ENTRIES('1,2,3,4,5,6,7,8') TO 1 BY -1:
...
END.
ON ctrl-cursor-right OF BROWSE br-sum-grp-obj do:
  RUN re-move-clmnbr-sum-grp-obj ( 1, 8).
END.
ON ctrl-cursor-left OF BROWSE br-sum-grp-obj do:
  RUN re-move-clmnbr-sum-grp-obj (8, 1).
END.
PROCEDURE re-move-clmnbr-sum-grp-obj:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-sum-grp-obj = 1 TO EXTENT(cur-clmn-numbr-sum-grp-obj):
    if cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = source-column THEN cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = -1.
  END.
  if br-sum-grp-obj:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-sum-grp-obj = source-column - 1 to target-column BY -1:
    DO varmvibr-sum-grp-obj = 1 TO EXTENT(cur-clmn-numbr-sum-grp-obj):
        if cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = varmvjbr-sum-grp-obj THEN DO:
          cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-sum-grp-obj = source-column + 1 to target-column:
    DO varmvibr-sum-grp-obj = 1 TO EXTENT(cur-clmn-numbr-sum-grp-obj):
      if cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = varmvjbr-sum-grp-obj THEN DO:
        cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] - 1.
      END.
  ...
  END.
  DO varmvibr-sum-grp-obj = 1 TO EXTENT(cur-clmn-numbr-sum-grp-obj):
    if cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = -1 THEN cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-sum-grp-obj:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-sum-grp-obj = 1 TO EXTENT(cur-clmn-numbr-sum-grp-obj):
    if cur-clmn-numbr-sum-grp-obj[varmvibr-sum-grp-obj] = cur-clmn-loc THEN move-elementbr-sum-grp-obj = varmvibr-sum-grp-obj.
  END.
  RUN re-move-clmnbr-sum-grp-obj (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-sum-grp-obj:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-sum-grp-obj = 1 to EXTENT(cur-clmn-numbr-sum-grp-obj):
    RUN re-move-clmnbr-sum-grp-obj (cur-clmn-numbr-sum-grp-obj[varmvlbr-sum-grp-obj], varmvlbr-sum-grp-obj).
  END.
  RUN start-mv-clmnbr-sum-grp-obj.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE MyEnable :
assign
  br-sum-grp-obj:num-locked-columns in frame Dialog-Frame = 1
  ...
  br-sum-grp-obj
  sch-corr-date
  sch-grp-code when p-mode = 'все':U
  sch-corr-user-name
  BR-changes mark-num
  WITH FRAME Dialog-Frame .
  VIEW FRAME Dialog-Frame .
...
title0 = "История групп товаров на кассах объекта" + chr(32).
...
if p-open-query then do:
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-sum-grp-obj:handle
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-sum-grp-obj FOR EACH X_c-sum-grp-obj
      where  TRUE
      indexed-reposition
  .
      assign
  ...
  on error undo, leave
  :
    if QUERY br-sum-grp-obj:handle:get-buffer-handle(1) = (buffer X_c-sum-grp-obj:handle) then do:
      assign
    ...
    .
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_c-sum-grp-obj"
      parameter-4-32 =
        (
          if (" TRUE " + " " + where-phrase-32) <> ""
          then " TRUE " + " " + where-phrase-32
  ...
  WHEN 'объект':U THEN DO:
    ...
    if p-open-query then do:
      ASSIGN
      frame Dialog-Frame:TITLE = title0 + substitute(" Объект: &1&2"
                                                        ,p-obj-type, p-obj-code).
    end.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if ("                     X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code                       " + " " + where-phrase-34) <> ""
          then  substitute('X_c-sum-grp-obj.obj-type = &1&2&1 and X_c-sum-grp-obj.obj-code = &3 '                           , chr(34), p-obj-type, p-obj-code)  + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          ("                     X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code                       " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-sum-grp-obj:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-sum-grp-obj FOR EACH X_c-sum-grp-obj
      where                      X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-sum-grp-obj:handle:get-buffer-handle(1) = (buffer X_c-sum-grp-obj:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_c-sum-grp-obj.obj-type = &1&2&1 and X_c-sum-grp-obj.obj-code = &3 '                           , chr(34), p-obj-type, p-obj-code)  + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_c-sum-grp-obj"
      parameter-4-34 =
        (
          if ("                     X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code                       " + " " + where-phrase-34) <> ""
          then  substitute('X_c-sum-grp-obj.obj-type = &1&2&1 and X_c-sum-grp-obj.obj-code = &3 '                           , chr(34), p-obj-type, p-obj-code)  + " " + where-phrase-34
  ...
  WHEN "one":u THEN DO:
    ...
    .
    if p-open-query then do:
      ASSIGN frame Dialog-Frame:TITLE = title0 + substitute(" Объект &1&2 Группа &3"
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if (" X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code AND X_c-sum-grp-obj.grp-code  = p-grp-code " + " " + where-phrase-36) <> ""
          then  substitute('X_c-sum-grp-obj.obj-type = &1&2&1 and X_c-sum-grp-obj.obj-code = &3 AND X_c-sum-grp-obj.grp-code  = &4 '                             , chr(34), p-obj-type, p-obj-code,  p-grp-code) + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-36 =
          (" X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code AND X_c-sum-grp-obj.grp-code  = p-grp-code " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-sum-grp-obj:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-sum-grp-obj FOR EACH X_c-sum-grp-obj
      where  X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code AND X_c-sum-grp-obj.grp-code  = p-grp-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-sum-grp-obj:handle:get-buffer-handle(1) = (buffer X_c-sum-grp-obj:handle) then do:
      ...
      parameter-4-36 =
        "where ":u +  substitute('X_c-sum-grp-obj.obj-type = &1&2&1 and X_c-sum-grp-obj.obj-code = &3 AND X_c-sum-grp-obj.grp-code  = &4 '                             , chr(34), p-obj-type, p-obj-code,  p-grp-code) + " ":u + where-phrase-36 + " ":u + p-find-condition + " " + ""
      parameter-5-36 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_c-sum-grp-obj"
      parameter-4-36 =
        (
          if (" X_c-sum-grp-obj.obj-type = p-obj-type and X_c-sum-grp-obj.obj-code = p-obj-code AND X_c-sum-grp-obj.grp-code  = p-grp-code " + " " + where-phrase-36) <> ""
          then  substitute('X_c-sum-grp-obj.obj-type = &1&2&1 and X_c-sum-grp-obj.obj-code = &3 AND X_c-sum-grp-obj.grp-code  = &4 '                             , chr(34), p-obj-type, p-obj-code,  p-grp-code) + " " + where-phrase-36
...
apply "entry":u to sch-grp-code in frame Dialog-Frame .
END PROCEDURE.
...
CASE X_c-sum-grp-obj.subject:
  when  'sum-grp-obj':U then do:
    v-label-param =
      "grp-code" + chr(4) + "Код группы" + chr(4) + "" + chr(8)
    + "grp-name" + chr(4) + "Наименование группы" + chr(4) + "" + chr(8)
    + "obj-code" + chr(4) + "Код объекта" + chr(4) + "" + chr(8)
    + "obj-type" + chr(4) + "Тип объекта" + chr(4) + ""  .
    run proc-full-temp-changes in this-procedure (
                                                 input  (X_c-sum-grp-obj.action = integer('1':U))
                                                ,input  (X_c-sum-grp-obj.action = integer('99':U))
                                                ,input  buffer X_c-sum-grp-obj:handle
                                                ,input  'sum-grp-obj':U
                                                ,input  "grp-code,grp-name,obj-code,obj-type"
                                                ,input  v-label-param).
  end.
  when 'dis-grp-rule':U then do:
      find first buf_c-dis-grp-rule no-lock where
              buf_c-dis-grp-rule.classif-type = 'sum-grp-obj':U
   ...
   if not available buf_c-dis-grp-rule then do:
    ...
    + "discnt-role" + chr(4) + "Тип скидки" + chr(4) + "disgrpru-get-disc-role-label"  + chr(8)
    + "obj-type" + chr(4) + "Тип объекта" + chr(4) + "" + chr(8)
    + "obj-code" + chr(4) + "Код" + chr(4) + ""
    .
    run proc-full-temp-changes in this-procedure (
                                                input  (X_c-sum-grp-obj.action = integer('1':U))
                                                ,input  (X_c-sum-grp-obj.action = integer('99':U))
                                                ...
                                                ,input  'dis-grp-rule':U
                                                ,input  "rule-num,pos-type,templ-rl-root,discnt-role,obj-code,obj-type"
                                                ,input  v-label-param).
  end.
END CASE.
Open QUery br-changes for each temp-changes.
...
FUNCTION get-subject RETURNS CHARACTER
  ...
  RETURN entry (lookup (p-subject, 'sum-grp-obj,dis-grp-rule':U), 'Группы товаров (на кассе объекта),Скидки по группе':U).
END FUNCTION.
