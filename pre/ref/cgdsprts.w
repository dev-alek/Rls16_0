...
assign
  v-str-recid12 = trim( string( recid( X_c-gds-prt ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry12 > 0 then do:
  ...
  g#log = br-c-gds-prt :refresh( )  in frame Dialog-Frame.
  if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
          g#log = br-c-gds-prt:select-next-row () in frame Dialog-Frame.
          apply "value-changed" to br-c-gds-prt in frame Dialog-Frame.
  end.
  ...
  apply "entry" to br-c-gds-prt in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-gds-prt :handle
      .
...
do:
  v-doc-rec = recid(X_c-gds-prt). run openbr in this-procedure ( input yes, input no, input '':U). reposition br-c-gds-prt to recid v-doc-rec no-error.
              APPLY 'VALUE-CHANGED' to br-c-gds-prt.
    apply "VALUE-CHANGED" to BR-c-gds-prt.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-c-gds-prt :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
def var sort-labelbr-c-gds-prt   as character no-undo .
def var sort-clmnbr-c-gds-prt    as handle    no-undo .
def var cur-clmnbr-c-gds-prt     as handle    no-undo .
def var cur-clmn-locbr-c-gds-prt as integer   no-undo .
def var re-querybr-c-gds-prt     as logical   initial no no-undo .
on start-search, ctrl-o of br-c-gds-prt in frame Dialog-Frame do:
   run sort-brbr-c-gds-prt
     (input (if available X_c-gds-prt
             then recid(X_c-gds-prt)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-c-gds-prt :
  define input parameter p-recid as recid no-undo .
  if re-querybr-c-gds-prt = no then do:
    assign
       cur-clmnbr-c-gds-prt = br-c-gds-prt:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-c-gds-prt <> ? then sort-clmnbr-c-gds-prt:column-fgcolor = 0.
    if cur-clmnbr-c-gds-prt = sort-clmnbr-c-gds-prt then do:
      assign
         sort-labelbr-c-gds-prt = ""
         sort-clmnbr-c-gds-prt = ?
      .
     end.
     else do:
       assign
         sort-labelbr-c-gds-prt = cur-clmnbr-c-gds-prt:label
         sort-clmnbr-c-gds-prt  = cur-clmnbr-c-gds-prt
         sort-clmnbr-c-gds-prt:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-c-gds-prt = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-c-gds-prt:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-c-gds-prt then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-c-gds-prt = cur-clmn-locbr-c-gds-prt + 1
    .
  end.
  case sort-labelbr-c-gds-prt:
        when X_c-gds-prt.corr-date:label in browse br-c-gds-prt then DO:    assign       sort-column-name = "X_c-gds-prt.corr-date"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when X_c-gds-prt.corr-user-db-num:label in browse br-c-gds-prt then DO:    assign       sort-column-name = "X_c-gds-prt.corr-user-db-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U).
      if sort-labelbr-c-gds-prt <> "" then do:
        assign
          cur-clmnbr-c-gds-prt:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-c-gds-prt = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-c-gds-prt to recid p-recid no-error.
    apply "value-changed" to br-c-gds-prt in frame Dialog-Frame.
  end.
  apply "entry" to br-c-gds-prt in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-c-gds-prt:
if cur-clmnbr-c-gds-prt = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U).
end.
else do:
   assign re-querybr-c-gds-prt = yes.
   run sort-brbr-c-gds-prt
     (input (if available X_c-gds-prt
             then recid(X_c-gds-prt)
             else ?
   ...
   assign re-querybr-c-gds-prt = no.
end.
end.
MAIN-BLOCK:
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-c-gds-prt FOR EACH X_c-gds-prt NO-LOCK.
END PROCEDURE.
PROCEDURE MyEnable :
...
CASE p-mode:
    when "one":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории шкал товаров: вн № узла шкалы &1",
                                                       p-node-code)
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-20 as character no-undo .
      assign
      parameter-3-20 =
                              "FOR EACH X_c-gds-prt"
      parameter-4-20 =
        (
          if (" X_c-gds-prt.node-code = p-node-code " + " " + where-phrase-20) <> ""
          then  substitute('X_c-gds-prt.node-code = &1', p-node-code)  + " " + where-phrase-20
          else "true"
        )
      parameter-5-20 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-20 =
          (" X_c-gds-prt.node-code = p-node-code " + " " + where-phrase-20 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-gds-prt:handle
  ...
  if l-filter-open-20 = false then do:
    OPEN QUERY br-c-gds-prt FOR EACH X_c-gds-prt
      where  X_c-gds-prt.node-code = p-node-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  run waitfram-hide in this-procedure .
    ...
    when "prt-root":U then do:
        ASSIGN frame Dialog-Frame:TITLE = substitute("Справочник истории шкал товаров: Шкала &1",
                                                       X_gds-prt.node-name)
        filter-point = filter-point0 + p-mode
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-22 as character no-undo .
      ...
      parameter-4-22 =
        (
          if (" X_c-gds-prt.prt-root = p-node-code " + " " + where-phrase-22) <> ""
          then  substitute('X_c-gds-prt.prt-root = &1', p-node-code)  + " " + where-phrase-22
          else "true"
        )
      parameter-5-22 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-22 =
          (" X_c-gds-prt.prt-root = p-node-code " + " " + where-phrase-22 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-c-gds-prt:handle
  ...
  if l-filter-open-22 = false then do:
    OPEN QUERY br-c-gds-prt FOR EACH X_c-gds-prt
      where  X_c-gds-prt.prt-root = p-node-code
  .
      assign
        l-query-was-opened = true
      .
...
v-full-name-new = "":U
.
if X_c-gds-prt.action = integer('1':U) then do:
  assign
  v-is-created = yes
  v-chg-fields = get-all-fields ("gds-prt")
  .
end.
if X_c-gds-prt.action = integer('99':U) then do:
  assign
  v-is-deleted = yes
  v-chg-fields = get-all-fields ("gds-prt")
  .
...
do ii = 1 to num-entries(v-chg-fields):
  ...
  jj = lookup(v-field-name, "prt-num,prt-root,is-term,lvl-num,node-code,node-name,root,upper-code").
  if jj = 0 then next _ii.
  assign
  v-field-label = entry(jj, "Порядковый № в группе признаков одного уровня,Вн № выш.узла корневого признака шкалы,Терминальная,Уровень,Вн №,Наименование,Корневой?,Вн № выш.узла")
  .
  create temp-changes.
  assign
  temp-changes.f_name = v-field-name
