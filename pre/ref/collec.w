...
run ref/seasonii.w ( parParentProc , 'ДОБАВЛЕНИЕ':U, input-output rr ).
if rr <> ? then
    do:
        OPEN QUERY br-coll FOR EACH ub.season       WHERE ub.season.sea-month-1 = 0  AND ub.season.sea-month-2 = 0 NO-LOCK.
...
assign
  v-str-recid8 = trim( string( recid( ub.season ) , "->>>>>>>>>>>9":U ) )
  v-num-entry8 = lookup( v-str-recid8 , rid-list )
.
if v-num-entry8 > 0 then do:
  assign
    entry( v-num-entry8, rid-list ) = "":U
    rid-list = trim( replace( rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    rid-list = rid-list + ( if rid-list = "":U then "":U else chr(44) ) + v-str-recid8
  .
end.
      v-log = br-coll :refresh( )  in frame d-type-tmp.
    if not can-do ("MOUSE-SELECT-DBLCLICK,Return", last-event:function) then do:
            v-log = br-coll:select-next-row () in frame d-type-tmp.
            apply "iteration-changed" to br-coll in frame d-type-tmp.
    end.
...
DO:
    ...
    DEFINE FRAME List
        ...
        ub.season.sea-code column-label 'код':U format ">>>>>>>>>>>>9"
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-coll :handle
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
    ...
    ub.season.sea-name:RESIZABLE in  browse br-coll   = true .
    if num-entries (rid-list) = 0 then
        hide mark-num in frame d-type-tmp.
    else
        disp
...
PROCEDURE enable_UI :
    ENABLE
        ...
        WITH FRAME  d-type-tmp.
    if available ub.season then
        log-res  = br-coll:select-focused-row( ) in frame d-type-tmp.
      OPEN QUERY br-coll FOR EACH ub.season       WHERE ub.season.sea-month-1 = 0  AND ub.season.sea-month-2 = 0 NO-LOCK.
     END PROCEDURE.
FUNCTION mon-name RETURNS CHARACTER
(input n-mon as int) :
