...
DEFINE BROWSE br-delivtype
  QUERY br-delivtype NO-LOCK DISPLAY
      ...
      entry (lookup (string(X_c-delivery-type.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U)
...
assign
  v-str-recid10 = trim( string( recid( X_c-delivery-type ) , "->>>>>>>>>>>9":U ) )
...
do:
  v-doc-rec = recid(X_c-delivery-type). run OpenBr in this-procedure. reposition BR-delivtype to recid v-doc-rec no-error. v-doc-rec = ?.               apply 'value-changed' to br-delivtype.
...
PROCEDURE sort-brbr-delivtype :
  ...
  case sort-labelbr-delivtype:
        when X_c-delivery-type.deliv-type-name:label in browse br-delivtype then DO:   assign     sort-column-name = "X_c-delivery-type.deliv-type-name"   .   run OpenBr.   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
DEF VAR cur-clmn-numbr-delivtype as INT EXTENT 8 no-undo.
...
def var old-session as logical no-undo.
   ...
   DO jjbr-delivtype = NUM-ENTRIES('1,2,3,4,5,6,7,8') TO 1 BY -1:
     RUN re-move-clmnbr-delivtype ( cur-clmn-numbr-delivtype[INTEGER(ENTRY (jjbr-delivtype, '1,2,3,4,5,6,7,8'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-delivtype do:
  RUN re-move-clmnbr-delivtype ( 1, 8).
END.
ON ctrl-cursor-left OF BROWSE br-delivtype do:
  RUN re-move-clmnbr-delivtype (8, 1).
END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-delivtype FOR EACH X_c-delivery-type NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
v-label-param =
 ...
 + "deliv-type-code" + chr(4) + "Внутр.код" + chr(4) + "" + chr(8)
 + "deliv-type-name" + chr(4) + "Название типа доставки" + chr(4) + "" + chr(8)
 + "sts" + chr(4) + "Статус" + chr(4) + ""  .
 run proc-full-temp-changes in this-procedure (
                                             input  buffer X_c-delivery-type:handle
                                            ,input  'delivery-type':U
                                            ,input  "des,deliv-type-code,deliv-type-name,sts"
                                            ,input  v-label-param).
Open QUery br-changes for each temp-changes.
END PROCEDURE.
