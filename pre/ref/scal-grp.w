...
get-full-name(X_scales-grp.node-code) COLUMN-LABEL "Название группы" FORMAT "X(255)":U  WIDTH 80
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_scales-goods-groups_adding-deletion':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-scal-grp :handle
      .
...
do:
  br-scal-grp :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info8 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
  v-doc-rec = recid(X_scales-grp). run Openbr in this-procedure. reposition br-scal-grp to recid v-doc-rec NO-ERROR. v-doc-rec = ?.
    apply "VALUE-CHANGED" to br-scal-grp.
end.
MAIN-BLOCK:
...
define variable vss-include-info9 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  run mainmenu_getcntxt in parparentproc
    ...
    CASE p-mode:
      WHEN 'gds-grp':U
      OR
      WHEN ('db':U + chr(44) + 'gds-grp':U) THEN DO:
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-scal-grp FOR EACH X_scales-grp       WHERE X_scales-grp.node-code = n-code NO-LOCK,              EACH X_gds-grp WHERE X_gds-grp.node-code = X_scales-grp.node-code NO-LOCK,              EACH X_scales WHERE X_scales.scales-num = X_scales-grp.scales-num            AND X_scales.db-num = X_scales-grp.db-num     NO-LOCK     BY X_scales-grp.scales-num.
END PROCEDURE.
PROCEDURE MyEnable :
define variable v-h as handle no-undo .
v-h = br-scal-grp:FIRST-COLUMN IN FRAME Dialog-Frame.
    DO while valid-handle(v-h) :
      if v-h:LABEL = "Название группы" then do:
        v-h:resizable = yes.
        leave.
      end.
      ELSE DO:
...
b-add WHEN (LOOKUP('b-add', bttns) > 0
       AND (p-mode = 'db':U + chr(44) + 'gds-grp':U
            or
            p-mode = 'scales':U)
...
CASE p-mode:
  WHEN ('db':U + chr(44) + 'gds-grp':U) THEN DO:
    frame Dialog-Frame:title = substitute("Привязка группы товаров &1 к весам для БД &2"
                                          , full-grpname
                                          , p-db-num).
  ...
  END.
  WHEN 'db':U THEN DO:
    frame Dialog-Frame:title = substitute("Привязка групп товаров к весам для БД &1", p-db-num).
    OPEN QUERY br-scal-grp FOR EACH X_scales-grp
  ...
  END.
  WHEN 'все':U  THEN DO:
    frame Dialog-Frame:title = substitute("Привязка групп товаров к весам").
  ...
  WHEN 'gds-grp':U  THEN DO:
    frame Dialog-Frame:title = substitute("Привязка группы товаров &1 к весам", full-grpname).
    OPEN QUERY br-scal-grp FOR EACH X_scales-grp
  ...
  END.
  WHEN 'scales':U  THEN DO:
    frame Dialog-Frame:title = substitute("Привязка групп товаров к весам: БД &1 весы № &2", p-db-num, p-scales-num).
      OPEN QUERY br-scal-grp FOR EACH X_scales-grp
          WHERE X_scales-grp.db-num >= p-db-num
...
define buffer buf_scales-grp for ub.scales-grp.
assign
jj = br-scal-grp:FOCUSED-ROW IN FRAME Dialog-Frame
rec-list = "" .
CASE p-mode:
  WHEN ('db':U + chr(44) + 'gds-grp':U) THEN DO:
