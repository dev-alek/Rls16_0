...
DEFINE BROWSE br-delivsubject
  QUERY br-delivsubject NO-LOCK DISPLAY
      ...
      entry (lookup (STRING(X_delivery-subject.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U) COLUMN-LABEL "Статус"
...
if loc-doc-rec <> ? THEN DO:
  ...
  if error-status:error then do:                           find first pos_delivery-subject no-lock where                                   recid(pos_delivery-subject) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи СУБЪЕКТА ДОСТАВКИ" skip                            string(if avail pos_delivery-subject                                     then  substitute("Вн код субъекта доставки: &1"                                                     , pos_delivery-subject.deliv-subj-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
END.
apply "entry" to br-delivsubject in frame Dialog-Frame.
apply "value-changed" to br-delivsubject in frame Dialog-Frame.
END.
...
DO:
  ...
  if error-status:error then return no-apply.
END.
ON CHOOSE OF B-delivtype IN FRAME Dialog-Frame
DO:
  DEFINE VARIABLE v-rid-list AS CHARACTER NO-UNDO.
...
assign
  v-str-recid14 = trim( string( recid( X_delivery-subject ) , "->>>>>>>>>>>9":U ) )
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-delivsubject :handle
      .
...
def var sort-labelbr-delivsubject   as character no-undo .
def var sort-clmnbr-delivsubject    as handle    no-undo .
def var cur-clmnbr-delivsubject     as handle    no-undo .
def var cur-clmn-locbr-delivsubject as integer   no-undo .
def var re-querybr-delivsubject     as logical   initial no no-undo .
on start-search, ctrl-o of br-delivsubject in frame Dialog-Frame do:
   run sort-brbr-delivsubject
     (input (if available X_delivery-subject
             then recid(X_delivery-subject)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-delivsubject :
  define input parameter p-recid as recid no-undo .
  if re-querybr-delivsubject = no then do:
    assign
       cur-clmnbr-delivsubject = br-delivsubject:current-column in frame Dialog-Frame
    .
    if sort-clmnbr-delivsubject <> ? then sort-clmnbr-delivsubject:column-fgcolor = 0.
    if cur-clmnbr-delivsubject = sort-clmnbr-delivsubject then do:
      assign
         sort-labelbr-delivsubject = ""
         sort-clmnbr-delivsubject = ?
      .
     end.
     else do:
       assign
         sort-labelbr-delivsubject = cur-clmnbr-delivsubject:label
         sort-clmnbr-delivsubject  = cur-clmnbr-delivsubject
         sort-clmnbr-delivsubject:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-delivsubject = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-delivsubject:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-delivsubject then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-delivsubject = cur-clmn-locbr-delivsubject + 1
    .
  end.
  case sort-labelbr-delivsubject:
        when X_delivery-subject.deliv-subj-code:label in browse br-delivsubject then DO:    assign       sort-column-name = "X_delivery-subject.deliv-subj-code"     .     run OpenBr ( input yes, input no, input no).   . END.
        when X_delivery-subject.deliv-subj-name:label in browse br-delivsubject then DO:    assign       sort-column-name = "X_delivery-subject.deliv-subj-name"     .     run OpenBr ( input yes, input no, input no).   . END.
        when X_delivery-subject.reg-code:label in browse br-delivsubject then DO:    assign       sort-column-name = "X_delivery-subject.reg-code"     .     run OpenBr ( input yes, input no, input no).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr ( input yes, input no, input no).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-delivsubject') then do:
          run mv-brw-defaultbr-delivsubject.
        end.
      if sort-labelbr-delivsubject <> "" then do:
        assign
          cur-clmnbr-delivsubject:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-delivsubject = ?
      .
    end.
  end case.
    if cur-clmn-locbr-delivsubject <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnbr-delivsubject') then do:
        run ch-clmnbr-delivsubject in this-procedure (cur-clmn-locbr-delivsubject).
      end.
    end.
  if p-recid <> ? then do:
    reposition br-delivsubject to recid p-recid no-error.
    apply "value-changed" to br-delivsubject in frame Dialog-Frame.
  end.
  apply "entry" to br-delivsubject in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-delivsubject:
if cur-clmnbr-delivsubject = ? then do:
   run OpenBr ( input yes, input no, input no).
end.
else do:
   assign re-querybr-delivsubject = yes.
   run sort-brbr-delivsubject
     (input (if available X_delivery-subject
             then recid(X_delivery-subject)
             else ?
   ...
   assign re-querybr-delivsubject = no.
end.
end.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  br-delivsubject :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
DEF VAR cur-clmn-numbr-delivsubject as INT EXTENT 5 no-undo.
DEF VAR varmvibr-delivsubject       as INT no-undo.
DEF VAR varmvjbr-delivsubject       as INT no-undo.
DEF VAR varmvkbr-delivsubject       as INT no-undo.
DEF VAR varmvlbr-delivsubject       as INT no-undo.
DEF VAR move-elementbr-delivsubject as INT no-undo.
def var jjbr-delivsubject           as int no-undo.
do varmvibr-delivsubject = 1 to EXTENT(cur-clmn-numbr-delivsubject):
  ASSIGN cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = varmvibr-delivsubject.
END.
RUN start-mv-clmnbr-delivsubject.
PROCEDURE start-mv-clmnbr-delivsubject:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  p-mode = 'все':U  THEN DO:
   DO jjbr-delivsubject = NUM-ENTRIES('1,2,3,4,5') TO 1 BY -1:
     RUN re-move-clmnbr-delivsubject ( cur-clmn-numbr-delivsubject[INTEGER(ENTRY (jjbr-delivsubject, '1,2,3,4,5'))] , 1).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-delivsubject do:
  RUN re-move-clmnbr-delivsubject ( 1, 5).
END.
ON ctrl-cursor-left OF BROWSE br-delivsubject do:
  RUN re-move-clmnbr-delivsubject (5, 1).
END.
PROCEDURE re-move-clmnbr-delivsubject:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-delivsubject = 1 TO EXTENT(cur-clmn-numbr-delivsubject):
    if cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = source-column THEN cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = -1.
  END.
  if br-delivsubject:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-delivsubject = source-column - 1 to target-column BY -1:
    DO varmvibr-delivsubject = 1 TO EXTENT(cur-clmn-numbr-delivsubject):
        if cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = varmvjbr-delivsubject THEN DO:
          cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = cur-clmn-numbr-delivsubject[varmvibr-delivsubject] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-delivsubject = source-column + 1 to target-column:
    DO varmvibr-delivsubject = 1 TO EXTENT(cur-clmn-numbr-delivsubject):
      if cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = varmvjbr-delivsubject THEN DO:
        cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = cur-clmn-numbr-delivsubject[varmvibr-delivsubject] - 1.
      END.
  ...
  END.
  DO varmvibr-delivsubject = 1 TO EXTENT(cur-clmn-numbr-delivsubject):
    if cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = -1 THEN cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-delivsubject:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 1 then do:
    return .
  end.
  DO varmvibr-delivsubject = 1 TO EXTENT(cur-clmn-numbr-delivsubject):
    if cur-clmn-numbr-delivsubject[varmvibr-delivsubject] = cur-clmn-loc THEN move-elementbr-delivsubject = varmvibr-delivsubject.
  END.
  RUN re-move-clmnbr-delivsubject (cur-clmn-loc, 1).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-delivsubject:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-delivsubject = 1 to EXTENT(cur-clmn-numbr-delivsubject):
    RUN re-move-clmnbr-delivsubject (cur-clmn-numbr-delivsubject[varmvlbr-delivsubject], varmvlbr-delivsubject).
  END.
  RUN start-mv-clmnbr-delivsubject.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-delivsubject FOR EACH X_delivery-subject NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyENable :
ASSIGN
...
define variable l-open-query as logical   no-undo .
  IF p-sts = ? THEN DO:
      ...
      frame Dialog-Frame:TITLE = title0 +  chr(32) + entry (lookup (STRING(X_delivery-subject.sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U).
      ASSIGN
...
if p-open-query then do:
  ...
  then do:
    ...
    :
      ...
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-delivsubject:handle
  ...
  if l-filter-open-32 = false then do:
    OPEN QUERY br-delivsubject FOR  EACH  X_delivery-subject no-lock
      where  true
      indexed-reposition
  .
      assign
  ...
  on error undo, leave
  :
    if query br-delivsubject:handle:get-buffer-handle(1) = (buffer X_delivery-subject:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input query br-delivsubject:handle
                          ,input rowid(X_delivery-subject)
                          ,input logical(parameter-2-32)
                          ,input no-lock
                          ,input (buffer X_delivery-subject:handle)
                          ,input parameter-4-32
                          ,input parameter-5-32
                          ) no-error.
      .
      ...
      assign
      parameter-2-32 = (if p-find-next then "true":u else "false":u )
      parameter-3-32 =  "FOR EACH X_delivery-subject"
      parameter-4-32 =
        (
          if (" true " + " " + where-phrase-32) <> ""
          then " true " + " " + where-phrase-32
  ...
  END.
  ELSE DO:
    frame Dialog-Frame:TITLE = title00 +  chr(32) + entry (lookup (STRING(p-sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U).
    filter-point-name = title00 +  chr(32) + entry (lookup (STRING(p-sts), '0,1,50,99':U), 'тек,удал,блок,удаление':U).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-34 as character no-undo .
      ...
      parameter-4-34 =
        (
          if (" X_delivery-subject.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_delivery-subject.sts = &1', p-sts)  + " " + where-phrase-34
          else "true"
        )
      parameter-5-34 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-34 =
          (" X_delivery-subject.sts = p-sts " + " " + where-phrase-34 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-delivsubject:handle
  ...
  if l-filter-open-34 = false then do:
    OPEN QUERY br-delivsubject FOR  EACH  X_delivery-subject no-lock
      where  X_delivery-subject.sts = p-sts
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-delivsubject:handle:get-buffer-handle(1) = (buffer X_delivery-subject:handle) then do:
      ...
      parameter-4-34 =
        "where ":u +  substitute('X_delivery-subject.sts = &1', p-sts)  + " ":u + where-phrase-34 + " ":u + p-find-condition + " " + ""
      parameter-5-34 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-34 = (if p-find-next then "true":u else "false":u )
      parameter-3-34 =  "FOR EACH X_delivery-subject"
      parameter-4-34 =
        (
          if (" X_delivery-subject.sts = p-sts " + " " + where-phrase-34) <> ""
          then  substitute('X_delivery-subject.sts = &1', p-sts)  + " " + where-phrase-34
...
if not loc#log then return error.
  ...
  if error-status:error then do:                           find first pos_delivery-subject no-lock where                                   recid(pos_delivery-subject) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи СУБЪЕКТА ДОСТАВКИ" skip                            string(if avail pos_delivery-subject                                     then  substitute("Вн код субъекта доставки: &1"                                                     , pos_delivery-subject.deliv-subj-code)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
  if available X_delivery-subject then do:
    loc#log = br-delivsubject:select-focused-row( ) IN FRAME Dialog-Frame.
  end.
  apply "ENTRY" to br-delivsubject.
end.
END PROCEDURE.
