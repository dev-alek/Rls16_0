...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1',bttns)
    .
end procedure.
...
def new shared  temp-table cntrlist no-undo like ub.country
  field to-del as logical
  index obj  is primary unique alpha1
  index iname      short-name
  .
define variable vss-include-info2 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define  new shared   temp-table cntrlist-hist no-undo
...
DO:
  case input frame Dialog-Frame a-n-c :
    when "alpha1" then do:
      apply "entry" to br-countries in frame Dialog-Frame.
      hide loc-name in frame Dialog-Frame.
      loc-alpha-1 = "".
    end.
    when "name" then do:
      enable loc-name with frame Dialog-Frame.
      disp loc-name with frame Dialog-Frame.
      hide loc-alpha-1 in frame Dialog-Frame.
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_country-reference_work':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
  ...
  then do:
    ...
    if v-country-recid <> ?
    then do:
      OPEN QUERY BR-countries FOR EACH X_country NO-LOCK     BY X_country.short-name.
...
assign
  v-str-recid7 = trim( string( recid( X_country ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry7 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-countries in frame Dialog-Frame.
    end.
...
DO:
  ...
  if avail buf_country then do:
   ...
   return no-apply.
END.
ON MOUSE-SELECT-DBLCLICK OF loc-name IN FRAME Dialog-Frame
DO:
...
DO:
  ...
  return no-apply.
END.
ON CHOOSE OF MENU-ITEM m_batchwork-routing
DO:
  rum-option = 'batchwork-routing':U.
  RUN proc-b-rum IN THIS-PROCEDURE ( INPUT rum-option) NO-ERROR.
  IF ERROR-STATUS:ERROR THEN DO:
      rum-option = "".
      RETURN NO-APPLY.
...
DO:
  ...
  rum-option = "".
END.
ON VALUE-CHANGED OF t-pck IN FRAME Dialog-Frame
DO:
  ASSIGN
  t-pck
  b-mark:SENSITIVE IN FRAME Dialog-Frame = (t-pck OR LOOKUP("b-mark", bttns) > 0)
  .
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-countries :handle
      .
...
do:
  BR-countries :SET-REPOSITIONED-ROW(10, "CONDITIONAL") .
end.
on any-printable of br-countries in frame Dialog-Frame
do:
  ...
  then do:
    ...
    disp loc-alpha-1 with frame Dialog-Frame.
    v-rec = recid (buf_country).
    reposition br-countries to recid v-rec no-error.
  end.
end.
on backspace of br-countries in frame Dialog-Frame
do:
  ...
  find first buf_country no-lock
    ...
    no-error .
  display loc-alpha-1 with frame Dialog-Frame.
  v-rec = recid (buf_country).
  reposition br-countries to recid v-rec no-error.
end.
MAIN-BLOCK:
...
if p-rum-option = 'batchwork-export':U then do:
  v-radio-button-parameter = 'batchwork-export':U.
end.
else do:
  v-radio-button-parameter = 'batchwork-routing':U  .
end.
if v-rid-list = '' then do:
  message
...
find cntrlist
  where cntrlist.alpha1 = buf_country.alpha1
  no-error .
if available cntrlist then do:
  assign
    cntrlist.to-del = no
  .
end.
  ...
  create cntrlist .
  buffer-copy buf_country to cntrlist
  assign
    cntrlist.to-del = no
  .
  assign
    lns-cnt = lns-cnt + 1
    line-rec = recid (cntrlist)
  .
end.
  end.
end.
