...
PROCEDURE adm-disable :
    DISABLE RECT-1 my-qnty choice RS-method text-5 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 my-qnty choice RS-method text-5 WITH FRAME F-Main.
...
run My-var.
if X-selectGood = 4 and not can-find(first gds-list no-lock) then do:
    g#log = no.
...
for each buf_chk-doc where buf_chk-doc.chk-date >= X-date-start and
    ...
    FOR EACH buf_chk-gds No-LOCK WHERE
        ...
        if choice = yes then do:
            ...
            buf_chk-doc.chk-date FORMAT "99/99/9999" CHR(9)
            buf_chk-doc.pay-desk CHR(9)
            buf_chk-doc.cashier CHR(9)
            string(buf_chk-gds.b-code) FORMAT "X(10)" CHR(9)
            buf_chk-gds.doc-qnty CHR(9)
            (if X-selectgood = 1 then buf_goods.artic else gds-list.artic ) format "x(16)" CHR(9)
            buf_chk-gds.price-base CHR(9)
            buf_chk-gds.discnt CHR(9)
            (buf_chk-gds.price-base - buf_chk-gds.discnt) * buf_chk-gds.doc-qnty CHR(9)
            (if X-selectgood = 1
