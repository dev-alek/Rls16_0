...
PROCEDURE adm-disable :
    DISABLE RECT-1 RS_wdays WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 RS_wdays WITH FRAME F-Main.
...
PROCEDURE local-initialize :
  ...
  assign
  RS_wdays:radio-buttons in frame F-Main =
  entry(2, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота") + chr(44) + string(2 ) + chr(44) +
entry(3, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота") + chr(44) + string(3 ) + chr(44) +
entry(4, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота") + chr(44) + string(4 ) + chr(44) +
entry(5, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота") + chr(44) + string(5 ) + chr(44) +
entry(6, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота") + chr(44) + string(6 ) + chr(44) +
entry(7, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота") + chr(44) + string(7 ) + chr(44) +
entry(1, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота") + chr(44) + string(1 ).
  assign
  RS_wdays  = 2
  .
END PROCEDURE.
...
FOR EACH obj-list NO-LOCK,
    FIRST buf_inkas where
        ...
        buf_inkas.status_ = 'факт':U NO-LOCK:
  glog = (if glog = no then yes else glog) .
END.
if NOT glog then do:
    message "Не было никаких закрытых продаж на выбранных объектах в течение заданного Вами периода времени." view-as alert-box.
    return.
end.
assign
...
assign
sheetf.Excel-COlumn-Lable = string( "Дата", "x(10)" ) + chr(44) +
                            string("День недели", "X(11)") + chr(44)
...
for each obj-list no-lock:
  ...
  sheetf.Excel-COlumn-Lable = sheetf.Excel-Column-Lable +
                             string(replace(cli_shop.obj-name, chr(44), chr(32)), "x(15)" ) + chr(44) +
                             fill (chr(32), 12) + chr(44) +
                             fill (chr(32), 12) + chr(44)
...
assign
sheetf.Excel-Column-Lable = sheetf.Excel-Column-Lable +
                          string("Итог дня", "X(8)") +  chr(44) +
                          fill( chr(32), 15 ) + chr(44) +
                          string("Итог недели", "X(11)") + chr(44) +
                          fill(chr(32), 15 )
sheetf.sizes = sheetf.sizes +
              "8" + chr(44) +
              "15" + chr(44) +
...
for each obj-list no-lock:
  ...
  assign
  sheetf.Excel-COlumn-Lable = sheetf.Excel-Column-Lable +
                             string("Чеки", "x(6)" ) + chr(44) +
                             string("Сред. покупка", "X(12)") + chr(44) +
                             string("Выручка", "X(12)") + chr(44)
  sheetf.MergeCellsH = Sheetf.MergeCellsH + (if Sheetf.MergeCellsH = "":U then "":U else chr(44)) +
...
assign
sheetf.Excel-Column-Lable = sheetf.Excel-Column-Lable +
                            string("Чеки", "X(8)") + chr(44) +
                            string("Выручка", "X(15)") +  chr(44) +
                            string("Чеки", "X(11)") + chr(44) +
                            string("Выручка", "X(15)")
sheetf.MergeCellsH = Sheetf.MergeCellsH + (if Sheetf.MergeCellsH = "":U then "":U else chr(44)) +
                       string(v-col-num)+ ":":U + string(v-col-num + 1) + chr(44) +
...
run rep/extitle.p (1).
assign
Sheetf.Bas-Params = string(kol_obj) + chr(4) + string(Sheetf.Excel-Row-Heder + 2  )
.
FOR EACH obj-list NO-LOCK,
...
do v-print-date = my-x-date-start to my-x-date-end:
  ...
  string(v-print-date, "99/99/99":U) CHR(9)
  get-weekday-name-short(weekday(v-print-date)) CHR(9)
  .
  for each obj-list no-lock:
    find first temp-sum no-lock where
               temp-sum.obj-type = obj-list.obj-type
    ...
    StrBuf =
              (if avail temp-sum
              then (string(temp-sum.tot-chk) + CHR(9) +
                    string(ROUND((temp-sum.tot-sum / temp-sum.tot-chk), 2)) + CHR(9) +
                    string(temp-sum.tot-sum) + CHR(9))
              else  (CHR(9) +
                     CHR(9) +
                     CHR(9))
             )
  ...
  if weekday(v-print-date) = v-week-end then do:
    if Make-Excel then  put   stream ForExcel unformatted
    (if accum-tot-chk <> 0 then string(accum-tot-chk) else "":U) CHR(9)
    (if accum-tot-chk <> 0 then string(accum-tot-sum) else "":U) CHR(9)
    (if accum-week-tot-chk <> 0 then string(accum-week-tot-chk) else "":U) CHR(9)
...
ReportHeader =  ("Началом отчетной недели считается" + chr(32) +
...
x-date-end   = my-x-date-end    .
 IF ( my#x-date-start <> my-x-date-start
  ...
  IF RETURN-VALUE NE "DESIGN" THEN DO:
    RUN get-link-handle IN adm-broker-hdl ( THIS-PROCEDURE, 'State':U , OUTPUT source-str ).
    State-source = WIDGET-HANDLE ( source-str ).
    IF not VALID-HANDLE ( State-source ) THEN
      Message "Не определен линк " 'State':U view-as alert-box error.
  END.
...
FUNCTION get-weekday-name RETURNS CHARACTER
  ...
  RETURN entry(p-weekday, "Воскресенье,Понедельник,Вторник,Среда,Четверг,Пятница,Суббота").
END FUNCTION.
FUNCTION get-weekday-name-short RETURNS CHARACTER
  ( input p-weekday as integer ) :
  RETURN entry(p-weekday, "вс,пн,вт,ср,чт,пн,сб").
END FUNCTION.
