...
define variable vss-description as character no-undo init "Œ“◊≈“ œŒ œŒ ”œ ¿Ã œŒ—“ŒﬂÕÕ€’  À»≈Õ“Œ¬ ( — ƒ»—. ¿–“¿Ã») ‰Îˇ Lui Vuitton - Ò·Ó ‰‡ÌÌ˚ı".
...
define SHARED temp-table temp-cards no-undo
...
cli-code
.
define SHARED temp-table temp-gds no-undo
field d-card like ub.dis-card.d-card
field cli-type like ub.dis-card.cli-type
field cli-code like ub.dis-card.cli-code
...
.
FOR EACH obj-list :
  if obj-list.obj-type = 'Ï‡„':U then do:
  ...
  if can-find( FIRST ub.chk-doc WHERE
    ...
    FOR EACH ub.chk-doc NO-LOCK WHERE
      ...
      if lookup(string(ub.chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,69,101,106,108,169,196,114,115,116,117,111,112,136,113,101,106,108,208,2,3,4,5,7,43,44':U) > 0 then next _chk-doc.
      PROCESS EVENTS .
...
CASE p-list:
  ...
  when "LIST":U then do:
    case prodmode:
      ...
      when 2 then do:
        assign
        v-first-level = substr(ub.goods.grp-name, 1, index(ub.goods.grp-name, chr(47)))
        .
