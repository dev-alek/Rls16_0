...
PROCEDURE adm-disable :
    DISABLE b-cp E-cp T-discnt-dtl f-pay-card FILL-IN-1 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN b-cp E-cp T-discnt-dtl f-pay-card FILL-IN-1 WITH FRAME F-Main.
...
PROCEDURE view-page :
  ...
  RUN broker-view-page IN adm-broker-hdl (INPUT THIS-PROCEDURE,
      INPUT p-page#).
  END PROCEDURE.
ON CHOOSE OF b-cp IN FRAME F-Main
DO:
...
ASSIGN
e-cp:SCREEN-VALUE IN FRAME F-Main = ''.
FOR EACH buf_tt-cash-pay:
   ...
   IF AVAILABLE buf_cash-pay THEN DO:
       v-rid-list = v-rid-list + (IF v-rid-list = '' THEN '' ELSE chr(44)) + STRING(RECID(buf_cash-pay)).
   END.
END.
  ...
  do v-ii = 1 to num-entries(v-rid-list):
    ...
    if available buf_cash-pay then do:
       ...
       glog = e-cp:INSERT-STRING ( display-cp(
                                                 ...
                                                 , buf_tt-cash-pay.obj-name )) in frame F-Main .
       glog = e-cp:INSERT-STRING ( chr(10) ) in frame F-Main .
