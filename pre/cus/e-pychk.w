...
DEFINE BROWSE BR-cash-pay
  QUERY BR-cash-pay DISPLAY
      ...
      trim(tt-cash-pay.grp-code, chr(4)) COLUMN-LABEL "№!группы" FORMAT "X(2)"
...
DEFINE BROWSE BR-cpay-group
  QUERY BR-cpay-group DISPLAY
      trim(tt-cash-group.grp-code, chr(4)) column-label "№"
...
PROCEDURE adm-disable :
    DISABLE T-cdpay-group BR-cpay-group B-add B-del BR-cash-pay T-rv WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN T-cdpay-group BR-cpay-group B-add B-del BR-cash-pay T-rv WITH FRAME F-Main.
...
if available buf_tt-cash-group then do:
    assign
    jj = integer(trim(buf_tt-cash-group.grp-code, chr(4)))
    .
end.
...
run proc-add-cash-group in this-procedure (chr(4) + string(jj + 1), "Группа" + chr(32) + string(jj + 1),  v-cash-pay-list) no-error.
if error-status:error then do:
...
define buffer buf_tt-cash-pay for tt-cash-pay.
  ...
  ii = integer(trim(buf_tt-cash-group.grp-code, chr(4))).
  for each buf_tt-cash-pay where
            buf_tt-cash-pay.grp-code = buf_tt-cash-group.grp-code :
         buf_tt-cash-pay.grp-code = chr(4) + "0":U.
  end.
  delete buf_tt-cash-group.
  for each buf_tt-cash-pay where
            integer(trim(buf_tt-cash-pay.grp-code, chr(4))) > ii:
      assign
      buf_tt-cash-pay.grp-code = chr(4) + string(integer(trim(buf_tt-cash-pay.grp-code, chr(4))) - 1      )
      .
  end.
    for each buf2_tt-cash-group where
            integer(trim(buf2_tt-cash-group.grp-code, chr(4))) > ii:
      assign
      buf2_tt-cash-group.grp-code = chr(4) + string(integer(trim(buf2_tt-cash-group.grp-code, chr(4))) - 1      )
      .
  end.
run set-cash-group in this-procedure(no, yes) no-error.
OPEN QUERY BR-cash-pay FOR EACH tt-cash-pay NO-LOCK.
OPEN QUERY BR-cpay-group FOR EACH tt-cash-group.
END.
ON LEAVE OF BR-cpay-group IN FRAME F-Main
DO:
  ASSIGN
  TT-CASH-GROUP.OBJ-NAME = TT-CASH-GROUP.OBJ-NAME:SCREEN-VALUE IN browse br-cpay-group.
END.
ON VALUE-CHANGED OF T-cdpay-group IN FRAME F-Main
DO:
  assign
  t-cdpay-group.
  CASE t-cdpay-group:
...
if not error-status:error then do:
    do ii = 1 to num-entries (v-uf-list_, chr(4)):
        assign
        v-grp-int-code = integer(entry(1, entry(ii, v-uf-list_, chr(4)), "=":U))
        v-grp-name = entry(2, entry(ii, v-uf-list_, chr(4)), "=":U)
        v-cash-pay-list = entry(ii, v-uf-Naim, chr(4))
        .
        run proc-add-cash-group in this-procedure (chr(4) + string(v-grp-int-code), v-grp-name, v-cash-pay-list) no-error.
...
OPEN QUERY BR-cash-pay FOR EACH tt-cash-pay NO-LOCK.
OPEN QUERY BR-cpay-group FOR EACH tt-cash-group.
apply "VALUE-CHANGED" to T-cdpay-group in frame F-Main.
END PROCEDURE.
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "tt-cash-group":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE tt-cash-group THEN STRING(ROWID(tt-cash-group))
        ELSE "?":U.
    WHEN "tt-cash-pay":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE tt-cash-pay THEN STRING(ROWID(tt-cash-pay))
        ELSE "?":U.
        OTHERWISE
        DO:
...
for each buf_tt-cash-group:
    assign
    ii = 0
    v-uf-list_ = v-uf-list_ + (if v-uf-list_ = "":U then "":U else chr(4)) +
                     trim(buf_tt-cash-group.grp-code, chr(4)) + "=":U +
                     buf_tt-cash-group.obj-name
    v-uf-naim = v-uf-Naim + (if v-uf-Naim = "":U then "":U else chr(4))
    .
    for each buf_tt-cash-pay where
                buf_tt-cash-pay.grp-code = buf_tt-cash-group.grp-code:
           ii = ii + 1.
