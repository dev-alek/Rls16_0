...
DEFINE BROWSE br-docs
  QUERY br-docs NO-LOCK DISPLAY
      mark-string(recid( buf-OR_ord-doc), rid-list)  COLUMN-LABEL '*' FORMAT "X(1)"
      buf-OR_ord-doc.status_  COLUMN-LABEL 'Статус' Format "X(6)"
      buf-OR_ord-doc.flag_  COLUMN-LABEL 'OK' format "+/-"
      buf-OR_ord-doc.doc-code  COLUMN-LABEL '№ док-та'
      buf-OR_ord-doc.doc-date  COLUMN-LABEL 'Создан' FORMAT "99/99/99"
      buf-OR_ord-doc.fact-date  COLUMN-LABEL 'Факт' FORMAT "99/99/99"
      buf-OR_ord-doc.obj-code  COLUMN-LABEL 'Объект' format ">>>>9"
      buf-OR_ord-doc.obj-type  COLUMN-LABEL 'Тип' format "x(3)"
      buf-OR_ord-doc.cli-type + string (buf-OR_ord-doc.cli-code)  COLUMN-LABEL 'РЦ'
      buf-OR_ord-doc.qnty COLUMN-LABEL 'Количество'
      if trim (buf-OR_ord-doc.ps) <> "" then "+" else "" @ c-ps  COLUMN-LABEL "П" format "x(1)"
      f-nameo(recid(buf-OR_ord-doc)) COLUMN-LABEL 'Наименование' Format "X(26)"
  ENABLE
       buf-OR_ord-doc.status_
...
DEFINE BROWSE br-zapr
  QUERY br-zapr NO-LOCK DISPLAY
      ...
      buf-OR_trn-doc.tot-rubl  COLUMN-LABEL "Сумма руб." FORMAT ">>>>>>>>>9.99"
...
define variable aobj-code as integer   no-undo .
 if not available buf-or_ord-doc then return.
   if not (
   ( buf-OR_ord-doc.status_ = 'новый':U and buf-OR_ord-doc.flag_ = false ) or
   ( buf-OR_ord-doc.status_ = 'запрос':U and buf-OR_ord-doc.flag_ = true  )
   ) then do:
...
if buf-OR_ord-doc.status_ = 'запрос':U then do:
  ...
  if   buf-OR_ord-doc.status_ = 'новый':U  and
...
DO:
 ...
 message "Закрыть накладную ?"
  ...
  if g-log = false then return no-apply.
   ...
   OPEN QUERY br-zapr FOR EACH buf-OR_ord-doc-rcv WHERE                   buf-OR_ord-doc-rcv.doc-code = buf-OR_ord-doc.doc-code NO-LOCK,         each ub.ord-chain no-lock where             ub.ord-chain.doc-code = buf-OR_ord-doc-rcv.rcv-code and             ub.ord-chain.doc-type = 'rcv'                  and             ub.ord-chain.rel-doc-type = 'trn'          ,          EACH buf-OR_trn-doc  where        buf-OR_trn-doc.doc-code = ub.ord-chain.rel-doc-code NO-LOCK.
END.
ON CHOOSE OF b-del IN FRAME Dialog-Frame
DO:
  if not available buf-OR_ord-doc then return.
do on stop undo, return no-apply :
  find current buf-OR_ord-doc exclusive-lock .
end.
  if not( buf-OR_ord-doc.status_ = 'новый':U and buf-OR_ord-doc.flag_ = false ) then do:
    if buf-OR_ord-doc.cli-code = p-obj-code and
       buf-OR_ord-doc.cli-type = p-obj-type and
       buf-OR_ord-doc.status_  = 'запрос':U
       then do:
...
assign
  v-str-recid17 = trim( string( recid( buf-OR_ord-doc ) , "->>>>>>>>>>>9":U ) )
...
END.
ON CHOOSE OF b-PS IN FRAME Dialog-Frame
DO:
 if not available buf-OR_ord-doc then return .
...
define variable rr as recid no-undo .
 ...
 message "Вы действительно хотите поставить статус ОТКАЗАТЬ на заказ ?"
  ...
  find current buf-or_ord-doc exclusive-lock no-error .
  if available buf-or_ord-doc then do:
     buf-or_ord-doc.status_ =  'отказ':U .
  end.
  run openbr in this-procedure (yes, no, '':u).
  reposition br-docs to recid rr no-error.
  apply "value-changed" to br-docs in frame dialog-frame.
  apply "entry" to br-docs in frame dialog-frame.
END.
ON CHOOSE OF b-rep IN FRAME Dialog-Frame
DO:
...
define MENU m-rep
    .
assign b-rep:popup-menu in frame Dialog-Frame  = menu m-rep:handle.
...
do:
  ...
  then do:
    ASSIGN
      sch-date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date23 :HANDLE
      sch-date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle23 as handle no-undo .
  assign
    v-label-handle23 = sch-date :side-label-handle in frame Dialog-Frame
  .
...
do:
  ...
  then do:
    ASSIGN
      sch-fact :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date25 :HANDLE
      sch-fact :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle25 as handle no-undo .
  assign
    v-label-handle25 = sch-fact :side-label-handle in frame Dialog-Frame
  .
...
DEF VAR cur-clmn-numbr-docs as INT EXTENT 10 no-undo.
...
ON ctrl-cursor-right OF BROWSE br-docs do:
  RUN re-move-clmnbr-docs ( 5, 10).
END.
ON ctrl-cursor-left OF BROWSE br-docs do:
  RUN re-move-clmnbr-docs (10, 5).
END.
...
on start-search, ctrl-o of br-docs in frame Dialog-Frame do:
   run sort-brbr-docs
     (input (if available buf-OR_ord-doc
             then recid(buf-OR_ord-doc)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-docs :
  ...
  case sort-labelbr-docs:
        when '*'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1mark-string&1, recid(buf-or_ord-doc), &1&2&1)', chr(34), rid-list)     .     run OpenBr(yes, no, '':U).   . END.
        when 'Статус'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.status_"     .     run OpenBr(yes, no, '':U).   . END.
        when 'OK'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.flag_"     .     run OpenBr(yes, no, '':U).   . END.
        when '№ док-та'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.doc-code"     .     run OpenBr(yes, no, '':U).   . END.
        when 'Создан'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.doc-date"     .     run OpenBr(yes, no, '':U).   . END.
        when 'Факт'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.fact-date"     .     run OpenBr(yes, no, '':U).   . END.
        when 'Объект'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.obj-code"     .     run OpenBr(yes, no, '':U).   . END.
        when 'Тип'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.obj-type"     .     run OpenBr(yes, no, '':U).   . END.
        when 'РЦ'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.cli-type + string (buf-OR_ord-doc.cli-code)"     .     run OpenBr(yes, no, '':U).   . END.
        when 'Количество'  then DO:    assign       sort-column-name = "buf-OR_ord-doc.qnty"     .     run OpenBr(yes, no, '':U).   . END.
        when '>'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1f-direct&1, recid(buf-or_ord-doc))', chr(34))     .     run OpenBr(yes, no, '':U).   . END.
        when 'Наименование'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1f-nameo&1, recid(buf-or_ord-doc))', chr(34))     .     run OpenBr(yes, no, '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   buf-OR_ord-doc.status_:read-only in browse br-docs = true .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-docs FOR EACH buf-OR_ord-doc NO-LOCK.    OPEN QUERY br-zapr FOR EACH buf-OR_ord-doc-rcv WHERE                   buf-OR_ord-doc-rcv.doc-code = buf-OR_ord-doc.doc-code NO-LOCK,         each ub.ord-chain no-lock where             ub.ord-chain.doc-code = buf-OR_ord-doc-rcv.rcv-code and             ub.ord-chain.doc-type = 'rcv'                  and             ub.ord-chain.rel-doc-type = 'trn'          ,          EACH buf-OR_trn-doc  where        buf-OR_trn-doc.doc-code = ub.ord-chain.rel-doc-code NO-LOCK.
END PROCEDURE.
PROCEDURE main-ord :
 do
 on error undo, return error substitute("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2))
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-31 as character no-undo .
      assign
      parameter-3-31 =
                              "FOR EACH buf-or_ord-doc"
      parameter-4-31 =
        (
          if (" buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type " + " " + where-phrase-31) <> ""
          then  substitute(' buf-OR_ord-doc.host-code = &2 and                         buf-OR_ord-doc.obj-code = &3  and                                       buf-OR_ord-doc.obj-type = &1&4&1  and                                    buf-OR_ord-doc.doc-type = &1&5&1' , chr(34) , par-host-code , p-obj-code, p-obj-type , p-doc-type)  + " " + where-phrase-31
          else "true"
        )
      parameter-5-31 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-31 =
          (" buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type " + " " + where-phrase-31 = "")
      .
  ...
  if l-filter-open-31 = false then do:
    OPEN QUERY br-docs FOR EACH buf-OR_ord-doc
      where  buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type
       USE-INDEX by-obj
  .
      assign
        l-query-was-opened = true
  ...
  assign
    doc-rec = recid( buf-OR_ord-doc )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf-or_ord-doc:handle) then do:
      assign
      parameter-2-31 = (if p-find-next then "true":u else "false":u )
      parameter-4-31 =
        "where ":u +  substitute(' buf-OR_ord-doc.host-code = &2 and                         buf-OR_ord-doc.obj-code = &3  and                                       buf-OR_ord-doc.obj-type = &1&4&1  and                                    buf-OR_ord-doc.doc-type = &1&5&1' , chr(34) , par-host-code , p-obj-code, p-obj-type , p-doc-type)  + " ":u + where-phrase-31 + " ":u + p-find-condition + " " + ""
      parameter-5-31 = " USE-INDEX by-obj "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
                          ,input rowid(buf-OR_ord-doc)
                          ,input logical(parameter-2-31)
                          ,input no-lock
                          ,input (buffer buf-OR_ord-doc:handle)
                          ,input parameter-4-31
                          ,input parameter-5-31
                          ) no-error.
      .
      ...
      assign
      parameter-2-31 = (if p-find-next then "true":u else "false":u )
      parameter-3-31 =  "FOR EACH buf-or_ord-doc"
      parameter-4-31 =
        (
          if (" buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type " + " " + where-phrase-31) <> ""
          then  substitute(' buf-OR_ord-doc.host-code = &2 and                         buf-OR_ord-doc.obj-code = &3  and                                       buf-OR_ord-doc.obj-type = &1&4&1  and                                    buf-OR_ord-doc.doc-type = &1&5&1' , chr(34) , par-host-code , p-obj-code, p-obj-type , p-doc-type)  + " " + where-phrase-31
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-33 as character no-undo .
      ...
      parameter-4-33 =
        (
          if (" buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_ " + " " + where-phrase-33) <> ""
          then  substitute(' buf-OR_ord-doc.host-code = &2 and                         buf-OR_ord-doc.obj-code = &3     and                          buf-OR_ord-doc.obj-type = &1&4&1 and                          buf-OR_ord-doc.doc-type = &1&5&1 and                          buf-OR_ord-doc.status_  = &1&6&1 ' , chr(34) , par-host-code , p-obj-code, p-obj-type , p-doc-type , p-status_ )  + " " + where-phrase-33
          else "true"
        )
      parameter-5-33 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-33 =
          (" buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_ " + " " + where-phrase-33 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-33 = false then do:
    OPEN QUERY br-docs FOR EACH buf-OR_ord-doc
      where  buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_
       USE-INDEX by-obj-type
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf-or_ord-doc:handle) then do:
      ...
      parameter-4-33 =
        "where ":u +  substitute(' buf-OR_ord-doc.host-code = &2 and                         buf-OR_ord-doc.obj-code = &3     and                          buf-OR_ord-doc.obj-type = &1&4&1 and                          buf-OR_ord-doc.doc-type = &1&5&1 and                          buf-OR_ord-doc.status_  = &1&6&1 ' , chr(34) , par-host-code , p-obj-code, p-obj-type , p-doc-type , p-status_ )  + " ":u + where-phrase-33 + " ":u + p-find-condition + " " + ""
      parameter-5-33 = " USE-INDEX by-obj-type "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
      ...
      assign
      parameter-2-33 = (if p-find-next then "true":u else "false":u )
      parameter-3-33 =  "FOR EACH buf-or_ord-doc"
      parameter-4-33 =
        (
          if (" buf-OR_ord-doc.host-code = par-host-code and buf-OR_ord-doc.obj-code = p-obj-code and buf-OR_ord-doc.obj-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_ " + " " + where-phrase-33) <> ""
          then  substitute(' buf-OR_ord-doc.host-code = &2 and                         buf-OR_ord-doc.obj-code = &3     and                          buf-OR_ord-doc.obj-type = &1&4&1 and                          buf-OR_ord-doc.doc-type = &1&5&1 and                          buf-OR_ord-doc.status_  = &1&6&1 ' , chr(34) , par-host-code , p-obj-code, p-obj-type , p-doc-type , p-status_ )  + " " + where-phrase-33
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-35 as character no-undo .
      ...
      parameter-4-35 =
        (
          if (" buf-or_ord-doc.cons-code = chr(-1) and buf-or_ord-doc.cli-code = p-obj-code and buf-or_ord-doc.cli-type = p-obj-type and buf-or_ord-doc.doc-type = p-doc-type " + " " + where-phrase-35) <> ""
          then  substitute(' buf-OR_ord-doc.cons-code = chr(-1) and                         buf-OR_ord-doc.cli-code = &2     and                          buf-OR_ord-doc.cli-type = &1&3&1 and                          buf-OR_ord-doc.doc-type = &1&4&1 ' , chr(34) , p-obj-code, p-obj-type , p-doc-type )  + " " + where-phrase-35
          else "true"
        )
      parameter-5-35 = (" " + "" + " " + "")
      parameter-6-35 = if sort-phrase-35 = ''
                           then
        (
        " " + " USE-INDEX by-cons-cli-doc " +
          " " + sort-column-phrase +
        " " + " by buf-or_ord-doc.doc-date desc by buf-or_ord-doc.fact-num desc "
        )
                           else
        (
        " " + " USE-INDEX by-cons-cli-doc " +
    ...
    :
      assign
        l-disable-where-35 =
          (" buf-or_ord-doc.cons-code = chr(-1) and buf-or_ord-doc.cli-code = p-obj-code and buf-or_ord-doc.cli-type = p-obj-type and buf-or_ord-doc.doc-type = p-doc-type " + " " + where-phrase-35 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-35 = false then do:
    OPEN QUERY br-docs FOR EACH buf-OR_ord-doc
      where  buf-or_ord-doc.cons-code = chr(-1) and buf-or_ord-doc.cli-code = p-obj-code and buf-or_ord-doc.cli-type = p-obj-type and buf-or_ord-doc.doc-type = p-doc-type
       USE-INDEX by-cons-cli-doc
       by buf-or_ord-doc.doc-date desc by buf-or_ord-doc.fact-num desc
  .
      assign
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf-or_ord-doc:handle) then do:
      ...
      parameter-4-35 =
        "where ":u +  substitute(' buf-OR_ord-doc.cons-code = chr(-1) and                         buf-OR_ord-doc.cli-code = &2     and                          buf-OR_ord-doc.cli-type = &1&3&1 and                          buf-OR_ord-doc.doc-type = &1&4&1 ' , chr(34) , p-obj-code, p-obj-type , p-doc-type )  + " ":u + where-phrase-35 + " ":u + p-find-condition + " " + ""
      parameter-5-35 = " USE-INDEX by-cons-cli-doc "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
      ...
      assign
      parameter-2-35 = (if p-find-next then "true":u else "false":u )
      parameter-3-35 =  "FOR EACH buf-or_ord-doc"
      parameter-4-35 =
        (
          if (" buf-or_ord-doc.cons-code = chr(-1) and buf-or_ord-doc.cli-code = p-obj-code and buf-or_ord-doc.cli-type = p-obj-type and buf-or_ord-doc.doc-type = p-doc-type " + " " + where-phrase-35) <> ""
          then  substitute(' buf-OR_ord-doc.cons-code = chr(-1) and                         buf-OR_ord-doc.cli-code = &2     and                          buf-OR_ord-doc.cli-type = &1&3&1 and                          buf-OR_ord-doc.doc-type = &1&4&1 ' , chr(34) , p-obj-code, p-obj-type , p-doc-type )  + " " + where-phrase-35
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-37 as character no-undo .
      ...
      parameter-4-37 =
        (
          if (" buf-or_ord-doc.cons-code = chr(-1) and buf-OR_ord-doc.cli-code = p-obj-code and buf-OR_ord-doc.cli-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_ " + " " + where-phrase-37) <> ""
          then  substitute(' buf-OR_ord-doc.cons-code = chr(-1) and                         buf-OR_ord-doc.cli-code = &2     and                          buf-OR_ord-doc.cli-type = &1&3&1 and                          buf-OR_ord-doc.doc-type = &1&4&1 and                          buf-OR_ord-doc.status_  = &1&5&1 ' , chr(34) , p-obj-code, p-obj-type , p-doc-type , p-status_ )  + " " + where-phrase-37
          else "true"
        )
      parameter-5-37 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-37 =
          (" buf-or_ord-doc.cons-code = chr(-1) and buf-OR_ord-doc.cli-code = p-obj-code and buf-OR_ord-doc.cli-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_ " + " " + where-phrase-37 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-37 = false then do:
    OPEN QUERY br-docs FOR EACH buf-OR_ord-doc
      where  buf-or_ord-doc.cons-code = chr(-1) and buf-OR_ord-doc.cli-code = p-obj-code and buf-OR_ord-doc.cli-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_
       USE-INDEX by-cons-cli-doc
       by buf-or_ord-doc.doc-date desc by buf-or_ord-doc.fact-num desc
  .
      assign
  ...
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf-or_ord-doc:handle) then do:
      ...
      parameter-4-37 =
        "where ":u +  substitute(' buf-OR_ord-doc.cons-code = chr(-1) and                         buf-OR_ord-doc.cli-code = &2     and                          buf-OR_ord-doc.cli-type = &1&3&1 and                          buf-OR_ord-doc.doc-type = &1&4&1 and                          buf-OR_ord-doc.status_  = &1&5&1 ' , chr(34) , p-obj-code, p-obj-type , p-doc-type , p-status_ )  + " ":u + where-phrase-37 + " ":u + p-find-condition + " " + ""
      parameter-5-37 = " USE-INDEX by-cons-cli-doc "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-docs:handle
      ...
      assign
      parameter-2-37 = (if p-find-next then "true":u else "false":u )
      parameter-3-37 =  "FOR EACH buf-or_ord-doc"
      parameter-4-37 =
        (
          if (" buf-or_ord-doc.cons-code = chr(-1) and buf-OR_ord-doc.cli-code = p-obj-code and buf-OR_ord-doc.cli-type = p-obj-type and buf-OR_ord-doc.doc-type = p-doc-type  and buf-OR_ord-doc.status_= p-status_ " + " " + where-phrase-37) <> ""
          then  substitute(' buf-OR_ord-doc.cons-code = chr(-1) and                         buf-OR_ord-doc.cli-code = &2     and                          buf-OR_ord-doc.cli-type = &1&3&1 and                          buf-OR_ord-doc.doc-type = &1&4&1 and                          buf-OR_ord-doc.status_  = &1&5&1 ' , chr(34) , p-obj-code, p-obj-type , p-doc-type , p-status_ )  + " " + where-phrase-37
...
if par-mode begins "rc" and
   buf-or_ord-doc.status_ = 'новый':U
    then do:
...
if not g-log then  return .
if  buf-OR_ord-doc.status_ = 'факт':U  then do:
