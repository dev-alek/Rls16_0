...
assign
    tdoc-date = (if ub.ord-doc.status_ <> 'факт':U then ub.ord-doc.doc-date else ub.ord-doc.fact-date)
...
Case ub.ord-doc.cli-type :
 ...
 End case.
 IF ub.ord-doc.doc-type = 'при':U Then
  Assign
...
PUT STREAM OutStream
    ...
    space(0) "" format "X(29)" AT 50 + 38                               "| " AT 42 + 76
                            (if ub.ord-doc.status_ <> 'факт':U then tdoc-date else ?) format "99/99/9999" "|" AT 60 + 76 skip
    ...
    space(56) string( "З А К А З ") format "x(10)"
                                    ...
                                    + " | "  + (if ub.ord-doc.status_ <> 'факт':U then string( "(" + CAPS(ub.ord-doc.status_) + ")" ) else ""))
                                    + " " + entry(1, ub.ord-doc.cli-out-doc, chr(4))
                                           format "X(116)" skip
                                         space(79) Line format "X(33)" skip
...
for each ub.ord-line where ub.ord-line.doc-code = ub.ord-doc.doc-code no-lock ,
        first ub.goods where ub.goods.prod-type = ub.ord-line.prod-type and
                          ...
                          ub.goods.artic = ub.ord-line.artic no-lock break by (if sort-gr then  ub.goods.grp-name else ub.goods.artic) by ub.goods.artic :
