...
PROCEDURE adm-disable :
    DISABLE RECT-7 RECT-8 COMBO-Firm SortType sort-grp WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 RECT-8 COMBO-Firm SortType sort-grp WITH FRAME F-Main.
...
for each ub.sysconf where ub.sysconf.avrg-price = yes no-lock :
 find first ub.clients where ub.clients.obj-code = ub.sysconf.host-code and ub.clients.obj-type = 'орг':U no-error .
...
END.
ASSIGN COMBO-Firm:LIST-ITEMS IN FRAME F-Main = temp-string.
ASSIGN COMBO-Firm:SCREEN-VALUE = str.
ENABLE COMBO-Firm WITH FRAME F-Main.
...
END PROCEDURE.
PROCEDURE my-var :
  Assign frame F-Main  COMBO-Firm SortType sort-grp.
  Assign
    STR-obj-type = ''
    STR-obj-code = ''
    STR-obj-name = ''
