...
v-size-col2  = decimal (entry(2, v-uf-List_ ,chr(4))) no-error.
v-size-col3   = decimal (entry(3, v-uf-List_ ,chr(4))) no-error.
...
if v-cntxt-db-num <> 0 then p-mode = 'ПРОСМОТР':U .
...
DO:
  parnext-prev = ?.
END.
ON CHOOSE OF B-rcv IN FRAME Dialog-Frame
DO:
  if not available tt_ord-line then return no-apply.
  run cus/ord-sosr.w
     ( input parparentproc,
     ...
     case tt_ord-line.type-str :
        when "ord" then do:
           ...
           if tt_ord-line.qnty <> tt_ord-line.qnty-rcv
              then tt_ord-line.qnty-rcv:fgcolor in browse BROWSE-3      = 12.
              else tt_ord-line.qnty-rcv:fgcolor = ?.
           if tt_ord-line.qnty <> tt_ord-line.qnty-trn
              then tt_ord-line.qnty-trn:fgcolor in browse BROWSE-3      = 12.
...
p-doc-code = buf_ord-doc.doc-code .
 if not ( buf_ord-doc.doc-type = 'ОП':U or
      buf_ord-doc.doc-type = 'ПО':U or
...
assign
  tt_ord-line.gds-code :bgcolor in browse BROWSE-3 = p-color
  tt_ord-line.artic    :bgcolor in browse BROWSE-3 = p-color
  buf_goods.gds-name   :bgcolor in browse BROWSE-3 = p-color
  buf_goods.unit-base  :bgcolor in browse BROWSE-3 = p-color
  tt_ord-line.qnty:bgcolor in browse BROWSE-3 = p-color
  tt_ord-line.qnty-rcv:bgcolor in browse BROWSE-3 = p-color
  tt_ord-line.qnty-trn:bgcolor in browse BROWSE-3 = p-color
.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-3 FOR EACH tt_ord-line NO-LOCK,              EACH buf_goods WHERE buf_goods.gds-code = tt_ord-line.gds-code NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE fill-widgets :
END PROCEDURE.
PROCEDURE init-proc :
  buf_goods.gds-name  :resizable in browse BROWSE-3 = true .
  tt_ord-line.artic   :resizable in browse BROWSE-3 = true .
  tt_ord-line.qnty-rcv:resizable in browse BROWSE-3 = true .
  buf_goods.gds-name  :width     in browse BROWSE-3   = v-size-col2 .
  tt_ord-line.artic   :width     in browse BROWSE-3   = v-size-col1 .
  tt_ord-line.qnty-rcv  :width     in browse BROWSE-3   = v-size-col3 .
  frame Dialog-Frame:title = "Состояние заказа № " + buf_ord-doc.doc-code .
  if p-mode <> 'ИЗМЕНЕНИЕ':U then do:
     B-exit:label in frame Dialog-Frame  = "Вы&ход"  .
     hide B-quit in frame Dialog-Frame .
  END.
 hide b-print  in frame Dialog-Frame .
end procedure.
PROCEDURE init-tt :
...
assign
   v-list-new =  string(decimal( tt_ord-line.artic  :width in browse BROWSE-3)) +  chr(4)
               + string(decimal( buf_goods.gds-name :width  in browse BROWSE-3))  +  chr(4)
               + string(decimal( tt_ord-line.qnty-rcv :width  in browse BROWSE-3))  +  chr(4) .
