...
ExpData1 = new Route-data_( input parparentproc, input this-procedure:handle, input this-procedure:handle, input this-procedure:handle) .
...
:
    ...
    for each buf_sysconf no-lock,
       ...
       where X_ext-classif.classif-subject = 'clients':U
         and X_ext-classif.classif-name = 'exite-edi':U
         and X_ext-classif.db-num = - 1
         and X_ext-classif.key#_one = X_ext-system.esys-id
         and ENTRY(2, X_eXt-classif.uniq-key-rec, chr(3)) = 'орг':U
         and integer(ENTRY(3, X_eXt-classif.uniq-key-rec, chr(3))) = buf_sysconf.host-code
    :
        v-frm-gln = get-gln( input 'орг':U
                            ,input buf_sysconf.host-code) no-error.
        find first buf_clients no-lock where buf_clients.obj-type = 'орг':U and buf_clients.obj-code = buf_sysconf.host-code .
        ...
        for each X_clients no-lock
           ...
           where XX_ext-classif.classif-subject = 'clients':U
             and XX_ext-classif.classif-name = 'exite-edi':U
             and XX_ext-classif.db-num = - 1
             and XX_ext-classif.key#_one = X_ext-system.esys-id
             and ENTRY(2, XX_eXt-classif.uniq-key-rec, chr(3)) = X_clients.obj-type
             and integer(ENTRY(3, XX_eXt-classif.uniq-key-rec, chr(3))) = X_clients.obj-code
        :
