...
PROCEDURE adm-disable :
    DISABLE RECT-5 RECT-7 T-new OrdDate T-req T-per T-ship T-fact WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-5 RECT-7 T-new OrdDate T-req T-per T-ship T-fact WITH FRAME F-Main.
...
PROCEDURE local-initialize :
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .
   T-fact:screen-value in frame F-Main = 'yes'.
   T-new:screen-value in frame F-Main = 'yes'.
   T-per:screen-value in frame F-Main = 'yes'.
   T-req:screen-value in frame F-Main = 'yes'.
   T-ship:screen-value in frame F-Main = 'yes'.
END PROCEDURE.
PROCEDURE my-report :
run cus/r-ordrc2.p
 ( input t-fact,
...
assign frame F-Main T-fact T-new T-per T-req T-ship OrdDate.
...
ReportHeader = "Статусы: " .
 if T-new:screen-value  in frame F-Main = 'yes' then ReportHeader =  ReportHeader  + T-new:label + chr(10) .
 if T-req:screen-value  in frame F-Main = 'yes'  then ReportHeader =  ReportHeader + T-req:label + chr(10) .
 if T-per:screen-value  in frame F-Main = 'yes'  then ReportHeader =  ReportHeader + T-per:label + chr(10) .
 if T-ship:screen-value in frame F-Main = 'yes'  then ReportHeader =  ReportHeader + T-ship:label + chr(10) .
 if T-fact:screen-value in frame F-Main = 'yes'  then ReportHeader =  ReportHeader + T-fact:label + chr(10) .
ReportHeader = ReportHeader + chr(10) + "Формировать отчет по " + t-OrdDate .
END PROCEDURE.
