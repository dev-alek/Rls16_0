...
FOR EACH ub.doc-line WHERE ub.doc-line.doc-code = doc-num NO-LOCK:
  ...
  FOR EACH ub.gds-dtl WHERE
    ...
    assign
    cur-discnt = (if v-curr-r-b = 'rubl':U then ub.gds-dtl.discnt-rubl else ub.gds-dtl.discnt-base)
    .
    CASE method :
    ...
    sj-goods.brutto-sum = sj-goods.brutto-sum + ( cur-quant * (if v-curr-r-b = 'rubl':U then ub.gds-dtl.price-rubl else ub.gds-dtl.price-base))
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  FOR EACH buf_inkas WHERE
    ...
    if buf_sale-doc.in-inkas then do:
      ...
      offc = t-doc.office
      .
      if buf_sale-doc.chr-office = 'у':U
...
sj-goods.u-pcnt column-label  "Торг.!нацен. %"                              format "->>9.99%"
sj-goods.n-u-sum column-label "Эффективность"                               format "->>>>>>>>9.99"
