...
DEFINE VARIABLE v_supp-type AS CHARACTER NO-UNDO VIEW-AS COMBO-BOX INNER-LINES 4 LIST-ITEMS 'орг':U, 'чел':U, 'скл':U, 'маг':U SIZE-CHARS  6.50 BY 1.00 FORMAT "x(3)":U.
...
DEFINE NEW GLOBAL SHARED VAR adm-broker-hdl    AS HANDLE  NO-UNDO.
      adm-object-hdl = FRAME fr-D-otv-xr-0:HANDLE.
RUN ensure-broker.
...
PROCEDURE adm-disable :
    DISABLE v_title j_supp-code v_supp-type Btn_Supplier v_scf-code  v_pay-title WITH FRAME fr-D-otv-xr-0.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN v_title j_supp-code v_supp-type Btn_Supplier v_scf-code  v_pay-title WITH FRAME fr-D-otv-xr-0.
...
PROCEDURE set-editors :
    ...
    ASSIGN curr-widget = FRAME fr-D-otv-xr-0:CURRENT-ITERATION.
...
ASSIGN FRAME fr-D-otv-xr-0 :SCROLLABLE = NO
       FRAME fr-D-otv-xr-0 :HIDDEN     = YES.
ASSIGN j_supp-code  :TOOLTIP IN FRAME fr-D-otv-xr-0 = "Код поставщика"
       v_supp-type  :TOOLTIP IN FRAME fr-D-otv-xr-0 = "Тип поставщика"
       v_supp-name  :TOOLTIP IN FRAME fr-D-otv-xr-0 = "Наименование поставщика"
       Btn_Supplier :TOOLTIP IN FRAME fr-D-otv-xr-0 = "Вызов справочника клиентов"
       type-scale   :TOOLTIP IN FRAME fr-D-otv-xr-0 = "Печатать с детализацией по признакам".
ON CHOOSE OF Btn_Supplier IN FRAME fr-D-otv-xr-0 DO:
  ...
  END.                  ELSE DO:
    ASSIGN v_supp-name = "":U
           ...
           v_supp-type = 'орг':U.
  END.
  DISPLAY  j_supp-code v_supp-type v_supp-name WITH FRAME fr-D-otv-xr-0.
END.
ON LEAVE OF v_supp-type IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN v_supp-type.
  ...
  DISPLAY v_supp-name WITH FRAME fr-D-otv-xr-0.
END.
ON VALUE-CHANGED OF v_supp-type IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN v_supp-type.
  FIND supplier NO-LOCK WHERE supplier.obj-type = v_supp-type AND supplier.obj-code = j_supp-code NO-ERROR.
  ASSIGN  v_supp-name = ( IF AVAILABLE supplier THEN supplier.obj-name ELSE "":U ).
  DISPLAY v_supp-name WITH FRAME fr-D-otv-xr-0.
END.
ON LEAVE OF j_supp-code IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN j_supp-code.
  FIND supplier NO-LOCK WHERE supplier.obj-type = v_supp-type AND supplier.obj-code = j_supp-code NO-ERROR.
  ASSIGN  v_supp-name = ( IF AVAILABLE supplier THEN supplier.obj-name ELSE "":U ).
  DISPLAY v_supp-name WITH FRAME fr-D-otv-xr-0.
END.
ON VALUE-CHANGED OF v_title IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN v_title.
END.
ON LEAVE OF v_title IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN v_title.
END.
ON LEAVE OF v_scf-code IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN v_scf-code.
  IF INPUT FRAME fr-D-otv-xr-0 v_pay-title = "счет" THEN DO:
    ASSIGN  v_pay-code = v_scf-code.
    DISPLAY v_pay-code WITH FRAME fr-D-otv-xr-0.
  END.
END.
ON VALUE-CHANGED OF v_pay-title IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN v_pay-title.
  IF           v_pay-title = "счет"    THEN DO:
    ASSIGN  v_pay-code = ( INPUT FRAME fr-D-otv-xr-0 v_scf-code ).
    ASSIGN  v_pay-code :BGCOLOR IN FRAME fr-D-otv-xr-0 = 8
            v_pay-code :FGCOLOR IN FRAME fr-D-otv-xr-0 = 4.
    DISABLE v_pay-code WITH FRAME fr-D-otv-xr-0.
    DISPLAY v_pay-code WITH FRAME fr-D-otv-xr-0.
  END. ELSE IF v_pay-title = "вручную" THEN DO:
    IF v_pay-code :SENSITIVE IN FRAME fr-D-otv-xr-0 THEN DO: ASSIGN v_pay-code. END.
    ASSIGN  v_pay-code :BGCOLOR IN FRAME fr-D-otv-xr-0 = 15
            v_pay-code :FGCOLOR IN FRAME fr-D-otv-xr-0 = 0.
    ENABLE  v_pay-code WITH FRAME fr-D-otv-xr-0.
    APPLY "ENTRY":U TO v_pay-code IN FRAME fr-D-otv-xr-0.
  ...
  END.
END.
ON LEAVE OF v_pay-title IN FRAME fr-D-otv-xr-0 DO:
  ASSIGN v_pay-title.
END.
ASSIGN  v_supp-type = 'орг':U
        v_header    = " З А Г О Л О В О К ".
ASSIGN  v_pay-code :BGCOLOR IN FRAME fr-D-otv-xr-0 = 8
        v_pay-code :FGCOLOR IN FRAME fr-D-otv-xr-0 = 4.
DISPLAY v_supp-type v_header WITH FRAME fr-D-otv-xr-0.
PROCEDURE disable_UI :
  HIDE FRAME fr-D-otv-xr-0 NO-PAUSE.
...
run create_obj-list in this-procedure
 ( input v-cntxt-obj-type ,
   ...
   .
  ASSIGN  v_supp-type = 'орг':U.
  DISPLAY v_supp-type WITH FRAME fr-D-otv-xr-0.
END PROCEDURE.
PROCEDURE my-report :
  ASSIGN FRAME fr-D-otv-xr-0 v_supp-type j_supp-code v_scf-code v_pay-code  .
...
END PROCEDURE.
PROCEDURE my-var :
  ASSIGN FRAME fr-D-otv-xr-0 v_supp-type j_supp-code type-scale.
  IF           v_title = "компенсация"  THEN DO:
  ...
  ASSIGN Sheetf.Excel-Column-Lable = "Наименование товара,Ед.,Количество,Цена за,Ст-ть товаров,в т.ч.,Ставка,Сумма," +
                                     "Ст-ть товаров,Страна,Номер ГТД," + chr(10) +
