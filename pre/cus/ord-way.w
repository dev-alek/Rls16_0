...
do while next-prev <> ?:
  if not available ub.ord-doc then do:
    ...
    find first shar-buf_ord-doc no-lock
        ...
        no-error .
       bf-handle = buffer shar-buf_ord-doc:handle in frame Dialog-Frame .
   run cus/lkp-zakz.w
     ( input parparentproc ,
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run usrfulnm in g#library
  (input  ub.ord-doc.creid
  ,output v-user-name
  )  .
  end.
  display v-user-name  with fram Dialog-Frame .
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   .
   if p-type = 1 then do:
      frame Dialog-Frame:title = "Заказаные товары в пути до даты поставки " + string (p-LOC-DATE-SHIP , "99/99/9999" ) +
      if p-doc-type = 'ФП':U then
   ...
   if p-type = 2 then do:
      frame Dialog-Frame:title = "Заказаные товары в пути на период продаж с " + string (p-DATE-sale-1 , "99/99/9999" )  +
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH ub.goods       WHERE ub.goods.artic = v-artic  AND ub.goods.prod-code = v-prod-code  AND ub.goods.prod-type = v-prod-type NO-LOCK.
  ...
  v-log =  session:SET-WAIT-STATE("GENERAL") . OPEN QUERY br-docs FOR EACH ub.ord-line NO-LOCK where ub.ord-line.artic     = p-artic and ub.ord-line.prod-type = p-prod-type and ub.ord-line.prod-code = p-prod-code   ,              EACH ub.ord-doc OF ub.ord-line NO-LOCK       WHERE ub.ord-doc.doc-type <> 'ПО':U AND           ( ub.ord-doc.status_ = 'закрыто':U              OR              ub.ord-doc.status_ = 'поставка':U )              and              (            if p-type = 1 then                ( ub.ord-doc.ship-date <=  p-LOC-DATE-SHIP )                else                ( ub.ord-doc.DATE-sale-1 <= p-date-sale-2 and                  ub.ord-doc.DATE-sale-2 >= p-date-sale-1)                  )                  and      ( if p-doc-type = 'ФП':U then        (ub.ord-doc.host-code = v-cntxt-host-code-obj)        else        (ub.ord-doc.obj-code = p-obj-code  and         ub.ord-doc.obj-type = p-obj-type        )        ). v-log =  session:SET-WAIT-STATE("") .
END PROCEDURE.
