...
DO:
  IF AVAILABLE tt-table THEN DO:
      ...
      if tt-table.new-id > 34 then tt-table.new-id = 34.
      OPEN QUERY BROWSE-1 FOR EACH tt-table .
      reposition BROWSE-1 to recid v-recid no-error .
      BROWSE-1 :SET-REPOSITIONED-ROW(10, "CONDITIONAL") .
  END.
END.
...
for each tt-table break by new-id :
    v-i = v-i + 1 .
    v-str = v-str + string(tt-table.id) + ',' + string(v-i) + chr(4) .
end.
v-str = trim(v-str,chr(4)) .
