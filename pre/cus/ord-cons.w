...
def buffer t-d-b for ub.ord-cons.
def buffer cns-doc for ub.ord-cons.
...
DEFINE BROWSE br-docs
  QUERY br-docs NO-LOCK DISPLAY
      IF (shar-buf_ord-cons.status_ = 'факт':U or shar-buf_ord-cons.status_ = 'закрыто':U)
...
DO:
  ...
  OPEN QUERY br-docs FOR EACH shar-buf_ord-cons       WHERE shar-buf_ord-cons.host-code = v-cntxt-host-code-obj and (p-status = "all":U  or (shar-buf_ord-cons.status_ = p-status) ) NO-LOCK     BY shar-buf_ord-cons.doc-date DESCENDING        BY shar-buf_ord-cons.cons-code DESCENDING.
END.
ON CHOOSE OF b-chg IN FRAME Dialog-Frame
DO:
find current shar-buf_ord-cons no-lock no-error .       if not avail shar-buf_ord-cons then do:                        message                                          "Документ не выбран ! " view-as  alert-box .        return no-apply.                                     end.
  if available shar-buf_ord-cons and (shar-buf_ord-cons.status_ <> 'новый':U
                     and shar-buf_ord-cons.status_ <> 'распределение':U
                     and shar-buf_ord-cons.status_ <> 'закрыто':U) then do:
     message "Изменять документ в статусе  '" shar-buf_ord-cons.status_ "' нельзя! " view-as alert-box .
     return no-apply.
  end.
  if available shar-buf_ord-cons and (shar-buf_ord-cons.status_ = 'новый':U
                       OR shar-buf_ord-cons.status_ = 'распределение':U
                       OR shar-buf_ord-cons.status_ = 'закрыто':U
                       ) then do:
      run cus/or-plan.w ( parParentProc , input shar-buf_ord-cons.cons-code , 'ИЗМЕНЕНИЕ':U, list-mode) no-error .
...
find current shar-buf_ord-cons no-lock no-error .       if not avail shar-buf_ord-cons then do:                        message                                          "Документ не выбран ! " view-as  alert-box .        return no-apply.                                     end.
  if available shar-buf_ord-cons and shar-buf_ord-cons.status_ <> 'новый':U then do:
               ...
               message "Удалить совокупную заявку №"  shar-buf_ord-cons.cons-code view-as alert-box
                    ...
                    if g#log then do:
                        for each loc_ord-doc where loc_ord-doc.cons-code = shar-buf_ord-cons.cons-code  OR
                                 loc_ord-doc.cons-code = shar-buf_ord-cons.cons-code + 'отказ':U
                                 exclusive-lock  :
                           assign
                               old-state = loc_ord-doc.status_
                               loc_ord-doc.cons-code = ""
                               loc_ord-doc.status_ = 'согласование':U
                           .
                           run str/callnews.p     ( input "ord-doc"      , input (buffer loc_ord-doc:handle)     ) no-error .   if error-status:error then do:     assign loc_ord-doc.flag_ = false  loc_ord-doc.status_ = old-state .     message                                                      vss-workfile vss-revision vss-description skip             "Ошибка при передаче "                                     (if loc_ord-doc.doc-type  =  'ОФ':U then " заявки " else "заказа" )            " в новости" skip                                          "Документ" loc_ord-doc.doc-code skip                             view-as alert-box .                                        return no-apply.                                       end.
                        end.
...
DO:
 find current shar-buf_ord-cons no-lock no-error .       if not avail shar-buf_ord-cons then do:                        message                                          "Документ не выбран ! " view-as  alert-box .        return no-apply.                                     end.
    if available shar-buf_ord-cons then do:
      run cus/or-plan.w ( parParentProc , input shar-buf_ord-cons.cons-code  ,input 'ПРОСМОТР':U , list-mode) .
  end.
END.
...
END.
ON CHOOSE OF b-post IN FRAME Dialog-Frame
DO:
...
END.
ON CHOOSE OF b-za IN FRAME Dialog-Frame
DO:
...
DO:
  ...
  display  editor_pS with frame Dialog-Frame.
END.
ON CHOOSE OF MENU-ITEM m_print_goods
DO:
find current shar-buf_ord-cons no-lock no-error .       if not avail shar-buf_ord-cons then do:                        message                                          "Документ не выбран ! " view-as  alert-box .        return no-apply.                                     end.
    run cus/r-cnsgds.p
      ( input parparentproc
...
END.
ON CHOOSE OF MENU-ITEM m_print_full_goods
DO:
find current shar-buf_ord-cons no-lock no-error .       if not avail shar-buf_ord-cons then do:                        message                                          "Документ не выбран ! " view-as  alert-box .        return no-apply.                                     end.
    run cus/r-cons2.p
      ( input parparentproc
...
END.
ON CHOOSE OF MENU-ITEM m_print_petrol
DO:
find current shar-buf_ord-cons no-lock no-error .       if not avail shar-buf_ord-cons then do:                        message                                          "Документ не выбран ! " view-as  alert-box .        return no-apply.                                     end.
 run cus/r-cons.p
   ( input parparentproc
...
do:
  run enable_UI in this-procedure .  run init-p in this-procedure .
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH ub.ord-cons SHARE-LOCK.
...
END PROCEDURE.
PROCEDURE init-p :
frame Dialog-Frame:title =  "Совокупные заявки , Фирма: " + p-g#host-name  +
if p-status = "all" then "" else ", Статус: " + p-status.
If list-mode <> "obj":U  then do:
   if not( p-status = 'новый':U  OR  p-status = "all" )  then disable  b-add b-del b-chg with frame Dialog-Frame .
   if ( p-status = 'распределение':U
        or  p-status = 'закрыто':U
         )  then do:
        enable   b-chg with frame Dialog-Frame .
        disable  b-add b-del with frame Dialog-Frame .
      end.
   end.
   else do:
      disable b-close b-add b-del b-sel b-mark with frame Dialog-Frame .
   end.
ASSIGN B-print:POPUP-MENU IN FRAME Dialog-Frame = MENU POPUP-MENU-B-print:HANDLE.
ASSIGN B-print:MENU-MOUSE = 1.
disable b-open with frame Dialog-Frame .
END PROCEDURE.
PROCEDURE OpenBr :
...
run gbl/flt-get.p
  ...
  if l-filter-open-18 = false then do:
    OPEN QUERY br-docs FOR EACH shar-buf_ord-cons
      where  TRUE
       USE-INDEX pi
      indexed-reposition
  .
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-20 as character no-undo .
      ...
      parameter-4-20 =
        (
          if (" shar-buf_ord-cons.status_ = p-status " + " " + where-phrase-20) <> ""
          then  substitute(' shar-buf_ord-cons.status_ =  &1&2&1 ' , chr(34) , p-status )  + " " + where-phrase-20
          else "true"
        )
      parameter-5-20 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-20 =
          (" shar-buf_ord-cons.status_ = p-status " + " " + where-phrase-20 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-docs:handle
  ...
  if l-filter-open-20 = false then do:
    OPEN QUERY br-docs FOR EACH shar-buf_ord-cons
      where  shar-buf_ord-cons.status_ = p-status
       USE-INDEX pi
      indexed-reposition
  .
      assign
