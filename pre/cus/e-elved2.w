...
PROCEDURE adm-disable :
    DISABLE RECT-client B-cli f-cli-type-code f-cli-name WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-client B-cli f-cli-type-code f-cli-name WITH FRAME F-Main.
...
if can-find( first dcards ) then do:
  ...
  space(40) str1 format "X(136)" skip(0).
  ...
  by dcards.obj-code:
    if first-of (dcards.d-card) then do:
      ...
      by buf_dis-card-property.sum-id :
        ...
        if buf_dis-card-property.node-code = 2 then do:
          assign
          v-car-name = buf_dis-card-property.property-value-character.
        end.
        if buf_dis-card-property.node-code = 1 then do:
          assign
    ...
    substitute("&1 &2"
              ,string(dcards.chk-date, "99.99.99")
              ,string(dcards.chk-time, "HH:MM")) CHR(9)
    buf_temp-obj.obj-name CHR(9)
    dcards.pump CHR(9)
    buf_temp-goods.gds-name CHR(9)
    dcards.doc-qnty CHR(9)
    dcards.price-real CHR(9)
    dcards.SUM-netto
    skip.
  ...
  by pet_dcards.price-real:
    ...
    CHR(9)
    buf_temp-goods.gds-name CHR(9)
    pet_dcards.doc-qnty CHR(9)
    pet_dcards.price-real CHR(9)
