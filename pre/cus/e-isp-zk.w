...
define variable vss-description as character no-undo init "Отчет по исполнению заказов (закладка № 2)".
...
DEFINE VARIABLE r-status_ AS INTEGER INITIAL 2
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS
          "Все", 1,
"только ФАКТ", 2
     SIZE 27.63 BY 1.83 NO-UNDO.
...
DEFINE VARIABLE RAD-Post AS INTEGER
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS
          "От всех поставщиков", 1,
"От выбранных поставщиков", 2
     SIZE 27.63 BY 2 NO-UNDO.
DEFINE VARIABLE RAD-Post-2 AS INTEGER
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS
          "По поставкам", 1,
"По накладным", 2
     SIZE 27.63 BY 1.96 NO-UNDO.
...
DEFINE VARIABLE v-ext-art AS LOGICAL INITIAL no
     LABEL "Артикул Поставщика"
     VIEW-AS TOGGLE-BOX
     SIZE 21.5 BY .83 TOOLTIP "печатать Артикул ПОСТАВЩИКА или ПРОИЗВОДИТЕЛЯ" NO-UNDO.
DEFINE FRAME F-Main
     ...
     r-status_ AT ROW 5.67 COL 2 NO-LABEL WIDGET-ID 2
     Ed-otkl-tmh AT ROW 6.96 COL 57.5 COLON-ALIGNED NO-LABEL
     Ed-otkl-tmm AT ROW 6.96 COL 63.38 COLON-ALIGNED NO-LABEL
     RAD-Post-2 AT ROW 10.33 COL 1.88 NO-LABEL
     v-ext-art AT ROW 13.58 COL 2.5 WIDGET-ID 10
     tg-zay AT ROW 14.67 COL 2.5
     "Выбор заказов по статусам:" VIEW-AS TEXT
          SIZE 27.5 BY .67 AT ROW 4.75 COL 2.38 WIDGET-ID 8
          FGCOLOR 4
     ":" VIEW-AS TEXT
          SIZE .75 BY .96 TOOLTIP ":" AT ROW 6.96 COL 64.5
     "Выбор заказов по ПОСТАВЩИКАМ:" VIEW-AS TEXT
          SIZE 27 BY .67 AT ROW 1.33 COL 2
          FGCOLOR 4
     "Сравнение:" VIEW-AS TEXT
          SIZE 19.38 BY .67 AT ROW 9.42 COL 7.13
          FGCOLOR 4
     "Выбор товаров:" VIEW-AS TEXT
          SIZE 19.38 BY .67 AT ROW 1.25 COL 37.75
          FGCOLOR 4
     RECT-7 AT ROW 1.08 COL 1.75
     RECT-9 AT ROW 9.17 COL 1.75
     RECT-8 AT ROW 13 COL 1.75
...
PROCEDURE adm-disable :
    DISABLE RECT-7 RECT-9 RECT-8 RECT-5 RAD-otkl RAD-Post Ed-otkl-prc Ed-otkl-prc-2 r-status_ Ed-otkl-tmh Ed-otkl-tmm RAD-Post-2 v-ext-art tg-zay WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 RECT-9 RECT-8 RECT-5 RAD-otkl RAD-Post Ed-otkl-prc Ed-otkl-prc-2 r-status_ Ed-otkl-tmh Ed-otkl-tmm RAD-Post-2 v-ext-art tg-zay WITH FRAME F-Main.
...
ON VALUE-CHANGED OF r-status_ IN FRAME F-Main
DO:
  Assign RAD-Post.
  Case RAD-Post :
    when 2 then
    do:
        ...
        if cli-lst = "" then do:
          Assign  RAD-Post = 1.
          Display RAD-Post with frame F-Main .
        end.
    end.
  End case.
END.
...
PROCEDURE my-report :
 ...
 run cus/r-isp-zk.p ( RAD-Post, cli-lst, tg-zay, RAD-otkl, Ed-otkl-prc, Ed-otkl-tm, RAD-Post-2 , r-status_ , v-ext-art) .
END PROCEDURE.
PROCEDURE my-var :
  assign frame F-Main   RAD-Post RAD-Post-2 RAD-otkl Ed-otkl-prc Ed-otkl-tmm  Ed-otkl-tmh tg-zay Ed-otkl-prc-2  r-status_  v-ext-art.
  str1 = "Выбор заказов: " .
  case RAD-Post :
    when 1 then assign str1 = str1 + "все" .
    when 2 then assign str1 = str1 + "от выбранных поставщиков" .
  end.
  case r-status_ :
    when 1 then assign str1 = str1 + "Статусы - все" .
    when 2 then assign str1 = str1 + "Закрытые на ФАКТ" .
  end.
  str2 = "Выбор товаров: " .
  case RAD-otkl :
    when 1 then assign str2 = str2 + "все" .
    when 2 then assign str2 = str2 + "отклонения по кол-ву больше " + String(Ed-otkl-prc) + "  %" .
    when 3 then assign str2 = str2 + "отклонения по цене больше " + String(Ed-otkl-prc-2) + "  %" .
    when 4 then assign str2 = str2 + "отклонения по времени больше " + String(Ed-otkl-tmh) + " ч. " + String(Ed-otkl-tmm) + " м. " .
    when 5 then assign str2 = str2 + "все отклонения" .
  end.
  case RAD-post-2 :
    when 1 then assign str3 = "Сравнивать по поставкам" .
    when 2 then assign str3 = "Сравнивать по накладным" .
  end.
 ReportNAme   = "Отчет по исполнению заказов" .
if  RAD-Post-2 = 2 then do:
 sheetf.Excel-Column-Lable =
 "Код        "         + "," +
 ( if v-ext-art then "Артикул Поставщика"    else  "Артикул "  )  + "," +
 "Наименование товара "  + "," +
 "Ед. изм"                + "," +
 "Кол-во по заказу"        + "," +
 "Дата заказа"             + "," +
