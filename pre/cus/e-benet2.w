...
PROCEDURE adm-disable :
    DISABLE RECT-8 RECT-6 RECT-5 SortType Classify ShowZero Tog-Qnty ShowOrders BUTTON-grp month-1 month-2 week-1 week-12 week-2 week-22 week-3 week-32 week-4 week-42 week-5 week-52 tog-voz L-Grp-name WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-8 RECT-6 RECT-5 SortType Classify ShowZero Tog-Qnty ShowOrders BUTTON-grp month-1 month-2 week-1 week-12 week-2 week-22 week-3 week-32 week-4 week-42 week-5 week-52 tog-voz L-Grp-name WITH FRAME F-Main.
...
ON CHOOSE OF BUTTON-grp IN FRAME F-Main
DO:
  run ref/cli-grps.w
  ...
  Display l-grp-name with frame F-Main .
END.
...
END.
ON VALUE-CHANGED OF ShowOrders IN FRAME F-Main
DO:
  assign showorders.
...
END.
ON VALUE-CHANGED OF Tog-Qnty IN FRAME F-Main
DO:
 define variable ret# as log no-undo.
  Assign tog-QNTY.
  if tog-QNTY =TRUE
        Then do:
            display  bsAmount  with frame F-Main .
            enable   bsAmount  with frame F-Main .
            Ret# = Sorttype:Disable(Entry(1,Sorttype:radio-buttons)).
            Ret# = Sorttype:Disable(Entry(3,Sorttype:radio-buttons)).
        end.
         Else do:
            display    bsAmount with frame F-Main .
            disable    bsAmount with frame F-Main .
...
on ' ' of month-1, month-2, week-1, week-12, week-2, week-22, week-3, week-32, week-4, week-42, week-5, week-52 in frame F-Main
do:
...
on delete-character of month-1, month-2, week-1, week-12, week-2, week-22, week-3, week-32, week-4, week-42, week-5, week-52 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of month-1, month-2, week-1, week-12, week-2, week-22, week-3, week-32, week-4, week-42, week-5, week-52 in frame F-Main
do:
...
on ctrl-b of month-1, month-2, week-1, week-12, week-2, week-22, week-3, week-32, week-4, week-42, week-5, week-52 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of month-1, month-2, week-1, week-12, week-2, week-22, week-3, week-32, week-4, week-42, week-5, week-52 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of month-1, month-2, week-1, week-12, week-2, week-22, week-3, week-32, week-4, week-42, week-5, week-52 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
PROCEDURE local-initialize :
  ...
  hide TOG-lavel   in frame F-Main .
  hide var-Lavel in frame F-Main .
END PROCEDURE.
PROCEDURE my-report :
if classify = "no-classify":U then do:
   run cus/r-benet2.p
      (input v-cntxt-obj-code, input v-cntxt-obj-type, input base-type , input base-code , input Classify  , input SortType  , input false     , input ShowZero  , input False     , input false     , input false     , input tog-lavel , input var-lavel , input fo0  , input fo02 , input fo1  , input fo12 , input fo2  , input fo22 , input fo3  , input fo32 , input fo4  , input fo42 , input fo5  , input fo52 , input Tog-Qnty, input bsamount, input v-cntxt-host-code-obj, input tog-voz, input ShowOrders, input Number-Orders) .
End.
Else do:
   run cus/r-bene22.p
   (input v-cntxt-obj-code, input v-cntxt-obj-type, input base-type , input base-code , input Classify  , input SortType  , input false     , input ShowZero  , input False     , input false     , input false     , input tog-lavel , input var-lavel , input fo0  , input fo02 , input fo1  , input fo12 , input fo2  , input fo22 , input fo3  , input fo32 , input fo4  , input fo42 , input fo5  , input fo52 , input Tog-Qnty, input bsamount, input v-cntxt-host-code-obj, input tog-voz, input ShowOrders, input Number-Orders) .
...
sheetf.Sizes = "" .
assign frame F-Main  ShowZero tog-qnty bsamount ShowOrders
...
ReportHeader = ReportHeader +
               (if tog-voz  then  chr(10) +  "Включая  " + 'возврат внешний':U  else " "    ).
...
ReportHeader = ReportHeader +
 ...
 Str-ECL-empty = ''.
    For each Temp#obj-list break by Temp#obj-list.grp-name:
        ...
        Str-ret = Str-ret + SUBSTRING(Entry(Num-entries(right-trim(Temp#obj-list.grp-name, chr(47)), chr(47)),Temp#obj-list.grp-name, chr(47)),1,3) + '. - ':U + Temp#obj-list.grp-name + chr(10).
        Str-ECL = Str-ECL + SUBSTRING(Entry(Num-entries(right-trim(Temp#obj-list.grp-name, chr(47)), chr(47)),Temp#obj-list.grp-name, chr(47)),1,3) + '.,':U.
...
if date(month-1:screen-value in frame F-Main) <> ? then
 assign frame F-Main
month-1
month-2
week-1
...
week-52
.
display month-1 month-2 week-1 week-12 week-2 week-22 week-3 week-32 week-4 week-42 week-5 week-52 with frame F-Main.
  if tog-QNTY =TRUE
        Then do:
            Ret# = Sorttype:Disable(Entry(1,Sorttype:radio-buttons)).
            Ret# = Sorttype:Disable(Entry(3,Sorttype:radio-buttons)).
