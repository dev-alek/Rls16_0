...
define buffer buf_goods for ub.goods.
  ...
  on error undo, return error
  :
     run waitfram-show in this-procedure ("Обрабатывается товар" + chr(32) + p-artic + chr(32) + p-prod-type + string(p-prod-code) ).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run termnode in g#library
  ...
  ,output v-term-node
        ...
        v-before = yes
        .
        if buf_temp-prt-obj.fact-qnty > 0 then do:   find first buf_temp-prt-in-sale where             buf_temp-prt-in-sale.gds-code = p-gds-code         AND  buf_temp-prt-in-sale.prt-code = buf_temp-prt-obj.prt-code         AND buf_temp-prt-in-sale.obj    = v-obj         AND buf_temp-prt-in-sale.cr < (cr0  + 1) no-error.   if not available buf_temp-prt-in-sale then do:     assign     v-evening = V-BEFORE        v-first-processing = yes     .     find first buf_temp-prt-in-sale where               buf_temp-prt-in-sale.cr = (cr0 + 1) use-index icreate no-error.     if not available buf_temp-prt-in-sale then do:         create buf_temp-prt-in-sale.         assign         buf_temp-prt-in-sale.cr = cr0 + 1         cr0 = cr0 + 1         .     end.     else do:       assign       cr0 = cr0 + 1       .     end.   end.   else do:     assign      v-evening = no     v-first-processing = no     .   end.    assign    buf_temp-prt-in-sale.evening  = if v-evening                                   then (buf_temp-prt-obj.fact-qnty > 0)                                   else (if v-first-processing then no else buf_temp-prt-in-sale.evening )   buf_temp-prt-in-sale.gds-code = p-gds-code   buf_temp-prt-in-sale.prt-code = buf_temp-prt-obj.prt-code   buf_temp-prt-in-sale.obj      = v-obj   buf_temp-prt-in-sale.fact-qnty = buf_temp-prt-obj.fact-qnty   .    end .
        case buf_trn-doc.doc-type :
          when 'при':U or
          when 'возврат':U
          then do:
