...
define variable doc-code    like ub.trn-doc.doc-code   no-undo.
     ...
     assign
        ...
        End case.
  case  PayType :
      when 2 Then type-arh =   'csdt':U.
      when 3 Then type-arh =   'sadt':U.
      when 1 Then type-arh =   'cgdt':U.
  End case.
...
PROCEDURE report-execute :
  ...
  run rep/runexcel.p
      (string( session:temp-directory) + "rpt" + string( g#report-num ) + ".txt").
END PROCEDURE.
PROCEDURE foreach :
 R = R + 1.
If Integer(10) = 0 Then Temp1  = 100  .
    ...
    run ob-line ( input   x-store-code   ,  input   x-store-type   ,  input   gds-zap-artic       ,  input   gds-zap-prod-code,      input   gds-zap-prod-type   ,
      input   fact-order-1,
      input   fact-order-2,
      input   type-arh,  input   '##,##':U, input   "" , input   xtog-obj ,  input   1 ,
      output v#r-qnty[1] ,
      output v#r-sum[1]).
...
if not firstline then  run display-title.
    ...
    if xtog-obj and    x-selectobject <> "currency":u   then  do:
          if Make-Excel then  put   stream ForExcel unformatted  "ПО ОБЪЕКТУ : " + caps(objname)  format "x(170)" skip.
          end.
           break_group = true.
           break_group1 = true.
   end procedure.
procedure print-footer :
 if (q4 - q3) <> 0 then do:
        ...
        'ОСТАЛЬНЫЕ'                                                             CHR(9)
        excel-qnty(decimal( ((q4 - q3) / q4 ) * 100    ))                   CHR(9)
        excel-qnty(decimal( q4 - q3                    ))             CHR(9)
        excel-sum(decimal ( coast4 - coast3            ))            CHR(9)
        excel-qnty(decimal( (q4 - q3) / (integer(x-date-end - x-date-start) + 1 )    ))           CHR(9)
        excel-sum(decimal ( (coast4 - coast3) / (integer(x-date-end - x-date-start) + 1 ) ))         skip.
 end.
        if Make-Excel then  put   stream ForExcel unformatted
        '          ИТОГО'                                                   CHR(9)
        'по всем товарам'                                                   CHR(9)
        100                                                                 CHR(9)
        excel-qnty(decimal(q4                                                    ))      CHR(9)
        excel-sum (decimal(coast4                                                ))     CHR(9)
        excel-qnty( decimal( q4 / (integer(x-date-end - x-date-start) + 1 )       ))       CHR(9)
...
if tog-ras then DO:
  ...
  Assign temp#sum-type.sum-type = type-arh + 'ee':U       temp#sum-type.xi = 1      .
End.
if tog-voz then DO:
  Create temp#sum-type no-error.
  Assign temp#sum-type.sum-type = type-arh + 're':U   temp#sum-type.xi = 1      .
End.
if tog-kass then DO:
  Create temp#sum-type no-error.
  Assign temp#sum-type.sum-type = type-arh + 'es':U       temp#sum-type.xi = 1      .
  Create temp#sum-type no-error.
  Assign temp#sum-type.sum-type = type-arh + 'rs':U   temp#sum-type.xi = 1      .
  End.
 End procedure.
PROCEDURE Eqq :
...
define variable i as int init 0  no-undo.
    ...
    Break by (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U )
    by
    ...
    if xClassify = "grp-goods":U Then DO:
       Accumulate Tmp#bs.r-qnty (TOTAL BY (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U )  ).
       Accumulate Tmp#bs.r-sum (TOTAL BY (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U )  ).
      If FIRST-OF( (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U ) ) Then DO:
             if Make-Excel then  put   stream ForExcel unformatted    'Группа ' CHR(9) TMP#bs.grp-name skip.
            END.
   End.
       if Make-Excel then  put   stream ForExcel unformatted
        TMP#bs.artic                  CHR(9)
        TMP#bs.gds-name               CHR(9)
        excel-qnty(decimal(( TMP#bs.r-qnty / Q4 ) * 100 )) CHR(9)
        excel-qnty(decimal(TMP#bs.r-qnty                )) CHR(9)
        excel-sum(decimal (TMP#bs.r-sum                 )) CHR(9)
        excel-qnty(decimal(TMP#bs.avr-qnty              )) CHR(9)
        excel-sum(decimal (TMP#bs.avr-sum               )) CHR(9)
        if tog-1  then
         (excel-qnty(decimal(TMP#bs.r-qnty2 )) + CHR(9) +
          excel-qnty(decimal(TMP#bs.r-sum2  )) + CHR(9) +
          excel-sum(decimal (TMP#bs.r-qnty3 )) + CHR(9) +
          excel-qnty(decimal(TMP#bs.r-sum3  )) + CHR(9) +
          excel-sum(decimal (TMP#bs.r-qnty4 )) + CHR(9) +
          excel-qnty(decimal(TMP#bs.r-sum4  )) + CHR(9) +
          excel-qnty(decimal(TMP#bs.r-qnty5 )) + CHR(9) +
          excel-sum(decimal (TMP#bs.r-sum5  )) + CHR(9) +
          excel-qnty(decimal(TMP#bs.r-qnty6 )) + CHR(9) +
          excel-sum(decimal (TMP#bs.r-sum6  )) + CHR(9) )
        else ""
        skip.
     if xClassify = "grp-goods":U Then DO:
        IF LAST-OF( (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U )  )  THEN DO:
            if Make-Excel then  put   stream ForExcel unformatted
            'ИТОГО по группе :'  CHR(9)
            TMP#bs.grp-name      CHR(9)
            excel-qnty(decimal( ((ACCUM TOTAL BY (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U ) Tmp#bs.r-qnty) / Q4 ) * 100                                )) CHR(9)
            excel-qnty(decimal( ACCUM   TOTAL BY (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U ) Tmp#bs.r-qnty                                              )) CHR(9)
            excel-sum(decimal ( ACCUM   TOTAL BY (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U ) Tmp#bs.r-sum                                               )) CHR(9)
            excel-qnty(decimal( (ACCUM  TOTAL BY (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U ) Tmp#bs.r-qnty) / (Integer(x-date-end - x-date-start) + 1 ) )) CHR(9)
            excel-sum(decimal ( (ACCUM  TOTAL BY (if xClassify = "grp-goods":U Then TMP#bs.grp-name  Else "1":U ) Tmp#bs.r-sum) /  (Integer(x-date-end - x-date-start) + 1 ) )) CHR(9)
            skip.
        End.
     End.
    End.
