...
PROCEDURE adm-disable :
    DISABLE RECT-SLT T-SLT-sum WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-SLT T-SLT-sum WITH FRAME F-Main.
...
ON VALUE-CHANGED OF T-SLT-sum IN FRAME F-Main
DO:
  assign
  T-SLT-sum.
  CASE T-SLT-sum:
...
CASE x-SelectGood:
  when 5 then do:
    ...
    v-to-del = no
    .
    FIND FIRST buf_gds-prt No-LOCK WHERE               buf_gds-prt.upper-code = gds-list.prt-root.      if not buf_gds-prt.node-name = '_Пустая шкала':U then do:        message       "Принципал не может быть отождествлен со шкальным товаром"       "Товар-" gds-list.artic gds-list.prod-type gds-list.prod-code skip       gds-list.gds-name       view-as alert-box ERROR.       v-to-del = yes.     end.     if not v-to-del then do:       find first buf_units No-LOCK WHERE                 buf_units.unit-name = gds-list.unit-base .       if lookup('сер':U, buf_units.type ) > 0 then do:         message         "Принципал не может быть отождествлен с серийным товаром"         "Товар-" gds-list.artic gds-list.prod-type gds-list.prod-code skip         gds-list.gds-name         view-as alert-box ERROR.         v-to-del = yes.       end.     end.
    if v-to-del then do:
      return.
    end.
    assign
    ...
    str1 =  "Акт выполненных работ по агентскому договору" + chr(32) +
            (if available gds-list
              then ("(принципиал" + chr(32) + gds-list.gds-name + ")":U)
              else "( принципиал не определен) ")
    str2 =    "За период с" + chr(32) + string(X-date-start, "99/99/9999") + chr(32)  +
              "по" + chr(32) + string(X-date-end, "99/99/9999")
    str3 = "":U
    v-report-header = ReportName + chr(10) +
                      str1 + chr(10) +
                      str2 + chr(10) +
                      str4
                      .
...
PROCEDURE My-var :
Assign
frame F-Main T-SLT-sum
frame F-Main f-slt-pc
.
Assign
 STR-obj-type = ''
 STR-obj-code = ''
...
End.
Reportname = "Отчет по сумме кассовых услуг, оказанных агентом" + chr(32) + "принципиалу".
ReportHeader =
                (if available gds-list
                ...
                chr(10) +
                (if T-SLT-sum
                                then ("(в том числе с НсП" + chr(32) +  string(f-slt-pc, ">9.99%") + ")")
                                else "":U)
                .
END PROCEDURE.
