  ...
  FOR EACH buf_chk-doc No-LOCK WHERE
     ...
     each buf_chk-gds-pay no-lock where
      ...
      and buf_chk-gds-pay.algo-num = "1.8"
          :
    if lookup(string(buf_chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) > 0 then next _chk-gds-pay.
    if v-doc-code <> buf_chk-doc.doc-code then do:
      v-num-units = ''.
      v-doc-code = buf_chk-doc.doc-code.
    end.
    if replace(replace(replace(replace(buf_chk-doc.office, 'у':U, ''), 'т':U, ''), 'смн-ош':U, ''), chr(44), '') <> '' then next _chk-gds-pay.
    ...
    buf_temp-acush.sum = buf_temp-acush.sum + buf_chk-gds-pay.tot-r-b * (if v-curr-r-b = 'rubl':U then 1 else  buf_chk-gds-pay.eff-base-rate)
    buf_temp-acush.num-units = buf_temp-acush.num-units +
                               (if lookup(string(buf_chk-gds-pay.cpline-num), v-num-units) = 0 then 1 else 0) +
                               (if num-entries(buf_chk-gds-pay.line-type, chr(4)) > 2
                                then
                                integer(entry(3, buf_chk-gds-pay.line-type, chr(4))) - 1
                                else 0)
    v-num-units = v-num-units + chr(44) + string(buf_chk-gds-pay.cpline-num)
    .
  end.
end.
...
str3 = string( "(Все суммы в рублях)" )
.
...
with FRAME BottomFrame width 136
...
do v-ii = 1 to num-entries(rep-sort):
  ...
  do v-jj = 1 to num-entries(cplot):
    ...
    for each buf_temp-acush where
      ...
      (if available buf_goods
      then buf_goods.gds-name
      else substitute("Топливо - код товара &1", buf_temp-acush.gds-code)) CHR(9)
      (if available buf_cash-pay
      then (buf_cash-pay.obj-name + buf_temp-acush.prefix)
      else substitute("Тип касс.платежа &1/&2", buf_temp-acush.cdpay-code, buf_temp-acush.prefix) ) CHR(9)
      round(buf_temp-acush.qnty, 3) CHR(9)
      round(buf_temp-acush.sum, 2) CHR(9)
      buf_temp-acush.num-units
      skip.
    end.
  end.
