...
procedure lhistprex-print-cli-list-hist-excel :
...
define buffer buf_cli-list-hist for cli-list-hist.
  do
  on error undo, return error
  :
    find first buf_cli-list-hist no-lock where buf_cli-list-hist.id = 0 no-error .
    if p-excel then do:
      ...
      run rep/extitlee.p (input p-sheet-num
                    , input  substitute("История создания списка &1 &2"
                                ,
''
                                ,(if available buf_cli-list-hist
                                then buf_cli-list-hist.des
                                else "БЕЗЫМЯННЫЙ"))
                    ) .
    end.
    if p-text then do:
    ...
    for each buf_cli-list-hist where buf_cli-list-hist.id > 0
    by buf_cli-list-hist.id
    :
      if p-text then do:
        put stream PrnLibStream unformatted
        (if buf_cli-list-hist.line = 0
        then string(buf_cli-list-hist.id, ">>>>>>>>9")
        else fill(chr(32) , 9)
        )  chr(32)
        (if buf_cli-list-hist.item_ <> '':U
        then string(buf_cli-list-hist.hist-mode, "X(8)")
        else fill( chr(32), 8)) chr(32)
        string(buf_cli-list-hist.num-add, ">>>>>>>>9") chr(32) chr(32) chr(32) chr(32)
        string(buf_cli-list-hist.num-recs, ">>>>>>>>9")  chr(32)
        (if page-size(PrnLibStream) > 43
        then string(buf_cli-list-hist.des, "X(" + string(136 - 43) + ")")
        else string(buf_cli-list-hist.des, "X(" + string(198 - 43) + ")"))
        skip.
      end.
      if p-excel then do:
        ...
        CHR(9)
        (if buf_cli-list-hist.item_ <> '':U
        then buf_cli-list-hist.hist-mode
        else '':U)  CHR(9)
        (if buf_cli-list-hist.item_ <> '':U
        then string(buf_cli-list-hist.num-add, "->>>>>>>>9")
        else '':U)  CHR(9)
        (if buf_cli-list-hist.item_ <> '':U
        then string(buf_cli-list-hist.num-recs, ">>>>>>>>9")
        else '':U)  CHR(9)
        buf_cli-list-hist.des
        skip.
      end.
    end.
  end.
...
Assign l-col-type="CHARACTER" l-col-len=8 l-col-format= "X(8)"     l-col-lable="Объект".
  ...
  define variable c-for-curr-obj as widget-handle no-undo.
  ...
  if use-column[1] = true then DO:
        ...
        CREATE EDITOR L-1 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-curr-obj IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
...
Assign l-col-type="CHARACTER" l-col-len=14 l-col-format= "X(14)"       l-col-lable="ПН".
  ...
  define variable c-for-in-code as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[2] = true then DO:
        ...
        CREATE EDITOR L-2 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-in-code IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="DATE"   l-col-len=10  l-col-format= "99/99/9999"  l-col-lable="Дата ПН".
  ...
  define variable c-for-in-date as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[3] = true then DO:
        ...
        CREATE EDITOR L-3 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-in-date IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="CHARACTER"   l-col-len=8  l-col-format= "X(8)"  l-col-lable="Объект прихода".
  ...
  define variable c-for-ini-obj as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[4] = true then DO:
        ...
        CREATE EDITOR L-4 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-ini-obj IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="CHARACTER"   l-col-len=20  l-col-format= "X(20)"  l-col-lable="Условия   поставки".
  ...
  define variable c-for-terms as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[5] = true then DO:
        ...
        CREATE EDITOR L-5 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-terms IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="CHARACTER"   l-col-len=14  l-col-format= "X(14)"  l-col-lable="Партия".
  ...
  define variable c-for-part-code as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[6] = true then DO:
        ...
        CREATE EDITOR L-6 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-part-code IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
.
assign
v-artic-cols = v-artic-cols + (if v-artic-cols = "":U then "":U else chr(44)) + string(v-r-col-num).
...
Assign l-col-type="CHARACTER"   l-col-len=3  l-col-format= "X(3)"  l-col-lable="Ед.изм".
  ...
  define variable c-for-unit as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[9] = true then DO:
        ...
        CREATE EDITOR L-9 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-unit IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="DECIMAL"   l-col-len=18  l-col-format= "->>,>>>,>>>,>>9.99"  l-col-lable="Цена по ТТН".
  ...
  define variable c-for-price-cli-in-brutto as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[10] = true then DO:
        ...
        CREATE EDITOR L-10 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-price-cli-in-brutto IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="DECIMAL"   l-col-len=5  l-col-format= ">9.9"  l-col-lable="НДС %".
  ...
  define variable c-for-VAT-pc as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[11] = true then DO:
        ...
        CREATE EDITOR L-11 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-VAT-pc IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="DECIMAL"   l-col-len=18  l-col-format= "->>,>>>,>>>,>>9.99"  l-col-lable="Цена поставки с налогами".
  ...
  define variable c-for-price-cli-in as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[13] = true then DO:
        ...
        CREATE EDITOR L-13 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-price-cli-in IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="DECIMAL"   l-col-len=15  l-col-format= "->>,>>>,>>9.999"  l-col-lable="Внешний  приход".
  ...
  define variable c-for-qnty-all as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[14] = true then DO:
        ...
        CREATE EDITOR L-14 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-qnty-all IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="INTEGER"   l-col-len=15  l-col-format= "->>,>>>,>>9.999"  l-col-lable="Приход за  период".
  ...
  define variable c-for-qnty-in as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[15] = true then DO:
        ...
        CREATE EDITOR L-15 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-qnty-in IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="INTEGER"   l-col-len=18  l-col-format= "->>,>>>,>>>,>>9.99"  l-col-lable="Сумма прихода".
  ...
  define variable c-for-price-cli-in-sum as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[16] = true then DO:
        ...
        CREATE EDITOR L-16 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-price-cli-in-sum IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="INTEGER"   l-col-len=15  l-col-format= "->>,>>>,>>9.999"  l-col-lable="Расход  за  период".
  ...
  define variable c-for-qnty-out as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[17] = true then DO:
        ...
        CREATE EDITOR L-17 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-qnty-out IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="INTEGER"   l-col-len=18  l-col-format= "->>,>>>,>>>,>>9.99"  l-col-lable="Сумма расхода".
  ...
  define variable c-for-price-cli-out-sum as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[18] = true then DO:
        ...
        CREATE EDITOR L-18 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-price-cli-out-sum IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
Assign l-col-type="INTEGER"   l-col-len=15  l-col-format= "->>,>>>,>>9.999"  l-col-lable="Остаток".
  ...
  define variable c-for-qnty-rest as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[19] = true then DO:
        ...
        CREATE EDITOR L-19 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-qnty-rest IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Obval:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
assign
sheetf.colformat = sheetf.colformat + chr(4)
.
...
FOR EACH temp-goods No-LOCK
  ...
  IF (line-counter(PrnLibStream)  modulo page-size(PrnLibStream)= 0) AND
     (line-counter(PrnLibStream) >= page-size(PrnLibStream)) then DO:
      display STREAM PrnLibStream    with frame top-frame .
  End.
  ...
  End.
    ...
    first-supp = no.
  if use-column[2]
  then  C-for-in-code:screen-value = string('Поставщик: ', entry(1, c-for-in-code:private-data, chr(4))).
  if use-column[8]
  then  C-for-gds-name:screen-value = string(suppnamebuf1, entry(1, c-for-gds-name:private-data, chr(4))).
  IF (line-counter(PrnLibStream)  modulo page-size(PrnLibStream)= 0) AND
     (line-counter(PrnLibStream) >= page-size(PrnLibStream)) then DO:
      display STREAM PrnLibStream    with frame top-frame .
  End.
    DISPLAY stream  PrnLibStream with frame Obval.                                      DOWN 1 stream PrnLibStream with frame Obval.
  IF (line-counter(PrnLibStream)  modulo page-size(PrnLibStream)= 0) AND
     (line-counter(PrnLibStream) >= page-size(PrnLibStream)) then DO:
      display STREAM PrnLibStream    with frame top-frame .
  End.
  if use-column[1]
  then  C-FOR-CURR-OBJ:screen-value = string(fill8).
  if use-column[2]
  then  C-for-IN-CODE:screen-value = string(fill14).
  if use-column[3]
  then  C-for-IN-DATE:screen-value = string(fill10).
  if use-column[4]
  then  C-for-ini-obj:screen-value = string(fill8).
  if use-column[5]
  then  C-for-terms:screen-value = string(fill20).
  if use-column[6]
  then  C-for-part-code:screen-value = string(fill14).
  if use-column[7]
  then  C-for-artic:screen-value = string(fill16).
  if use-column[8]
  then  C-for-gds-name:screen-value = string(fill22).
  if use-column[9]
  then  C-for-unit:screen-value = string(fill3).
  if use-column[10]
  then  C-for-price-cli-in-brutto:screen-value = string(fill18).
  if use-column[11]
  then  C-for-VAT-pc:screen-value = string(fill5).
  if use-column[12]
  then  C-for-SLT-pc:screen-value = string(fill5).
  if use-column[13]
  then  C-for-price-cli-in:screen-value = string(fill18).
  if use-column[14]
  then  C-for-qnty-all:screen-value = string(fill15).
  if use-column[15]
  then  C-for-qnty-in:screen-value = string(fill15).
  if use-column[16]
  then  C-for-price-cli-in-sum:screen-value = string(fill18).
  if use-column[17]
  then  C-for-qnty-out:screen-value = string(fill15).
  if use-column[18]
  then  C-for-price-cli-out-sum:screen-value = string(fill18).
  if use-column[19]
  then  C-for-qnty-rest:screen-value = string(fill15).
        DISPLAY stream  PrnLibStream with frame Obval.       DOWN 1 stream PrnLibStream with frame Obval.
    if Make-Excel then  put   stream ForExcel unformatted
  if use-column[1]
  then (if 1 < last-col-num then CHR(9) else "")
  else ""
  if use-column[2]
  then (reg-output(
                    string('Поставщик: ', entry(1, c-for-in-code:private-data, chr(4)))
                   ,c-for-in-code:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[6]
  then (if 6 < last-col-num then CHR(9) else "")
  else ""
  if use-column[7]
  ...
  if use-column[8]
  then (reg-output(
                    string(suppnamebuf1, entry(1, c-for-gds-name:private-data, chr(4)))
  ...
  if use-column[9]
  then (if 9 < last-col-num then CHR(9) else "")
  else ""
  if use-column[10]
  then (if 10 < last-col-num then CHR(9) else "")
  else ""
  if use-column[11]
  then (if 11 < last-col-num then CHR(9) else "")
  else ""
  if use-column[12]
  then (if 12 < last-col-num then CHR(9) else "")
  else ""
  if use-column[13]
  then (if 13 < last-col-num then CHR(9) else "")
  else ""
  if use-column[14]
  then (if 14 < last-col-num then CHR(9) else "")
  else ""
  if use-column[15]
  then (if 15 < last-col-num then CHR(9) else "")
  else ""
  if use-column[16]
  then (if 16 < last-col-num then CHR(9) else "")
  else ""
  if use-column[17]
  then (if 17 < last-col-num then CHR(9) else "")
  else ""
  if use-column[18]
  then (if 18 < last-col-num then CHR(9) else "")
  else ""
  if use-column[19]
  then (if 19 < last-col-num then CHR(9) else "")
  else ""
    skip.
    if Make-Excel then  put   stream ForExcel unformatted
  if use-column[1]
  then (string(fill8) + (if 1 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[2]
  then (string(fill14) + (if 2 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[3]
  then (string(fill10) + (if 3 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[4]
  then (string(fill8) + (if 4 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[5]
  then (string(fill20) + (if 5 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[6]
  then (string(fill14) + (if 6 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[7]
  then (string(fill16) + (if 7 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[8]
  then (string(fill22) + (if 8 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[9]
  then (string(fill3) + (if 9 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[10]
  then (string(fill18) + (if 10 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[11]
  then (string(fill5) + (if 11 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[12]
  then (string(fill5) + (if 12 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[13]
  ...
  if use-column[18]
  then (string(fill18) + (if 18 < last-col-num then CHR(9) else ""))
  else ""
  if use-column[19]
  ...
  if use-column[2]
  then  C-for-in-code:screen-value = string(('Валюта: ' + temp-goods.curr-name), entry(1, c-for-in-code:private-data, chr(4))).
  IF (line-counter(PrnLibStream)  modulo page-size(PrnLibStream)= 0) AND
     (line-counter(PrnLibStream) >= page-size(PrnLibStream)) then DO:
      display STREAM PrnLibStream    with frame top-frame .
  End.
  ...
  if use-column[2]
  then (reg-output(
                    string(('Валюта: ' + temp-goods.curr-name), entry(1, c-for-in-code:private-data, chr(4)))
                   ,c-for-in-code:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[8]
  then (if 8 < last-col-num then CHR(9) else "")
  else ""
  if use-column[9]
  then (if 9 < last-col-num then CHR(9) else "")
  else ""
  ...
  END.
  if use-column[1]
  then  C-for-curr-obj:screen-value = string((if temp-goods.obj-code > 0 then (temp-goods.obj-type + string(temp-goods.obj-code)) else ''), entry(1, c-for-curr-obj:private-data, chr(4))).
    if pnum-obj > 1 and temp-goods.obj-code > 0 then do:
  if use-column[2]
  then  C-for-in-code:screen-value = string('------>', entry(1, c-for-in-code:private-data, chr(4))).
    first-good = no.
  end.
  else do:
  if use-column[2]
  then  C-for-in-code:screen-value = string(temp-goods.in-code, entry(1, c-for-in-code:private-data, chr(4))).
  if use-column[3]
  then  C-for-in-date:screen-value = string(temp-goods.in-date, entry(1, c-for-in-date:private-data, chr(4))).
  if use-column[4]
  then  C-for-ini-obj:screen-value = string((temp-goods.obj-in-type + string(temp-goods.obj-in-code)), entry(1, c-for-ini-obj:private-data, chr(4))).
  if use-column[5]
  then  C-for-terms:screen-value = string(('НДС ' + temp-goods.vat-type + chr(32) + 'НП ' + temp-goods.slt-type), entry(1, c-for-terms:private-data, chr(4))).
  if use-column[6]
  then  C-for-part-code:screen-value = string(temp-goods.part-code, entry(1, c-for-part-code:private-data, chr(4))).
  if use-column[7]
  then  C-for-artic:screen-value = string(temp-goods.artic, entry(1, c-for-artic:private-data, chr(4))).
  if use-column[8]
  then  C-for-gds-name:screen-value = string(temp-goods.gds-name, entry(1, c-for-gds-name:private-data, chr(4))).
  if use-column[9]
  then  C-for-unit:screen-value = string(temp-goods.unit, entry(1, c-for-unit:private-data, chr(4))).
  if use-column[10]
  then  C-for-price-cli-in-brutto:screen-value = string(temp-goods.price-cli-in-brutto, entry(1, c-for-price-cli-in-brutto:private-data, chr(4))).
  if use-column[11]
  then  C-for-VAT-pc:screen-value = string(temp-goods.vat-pc, entry(1, c-for-VAT-pc:private-data, chr(4))).
  if use-column[12]
  then  C-for-SLT-pc:screen-value = string(temp-goods.SLT-pc, entry(1, c-for-SLT-pc:private-data, chr(4))).
  if use-column[13]
  then  C-for-price-cli-in:screen-value = string(temp-goods.price-cli-in, entry(1, c-for-price-cli-in:private-data, chr(4))).
  if use-column[14]
  then  C-for-qnty-all:screen-value = string(temp-goods.qnty-all, entry(1, c-for-qnty-all:private-data, chr(4))).
  end.
  if use-column[15]
  then  C-for-qnty-in:screen-value = string(temp-goods.qnty-in, entry(1, c-for-qnty-in:private-data, chr(4))).
  if use-column[16]
  then  C-for-price-cli-in-sum:screen-value = string(temp-goods.price-cli-in-sum, entry(1, c-for-price-cli-in-sum:private-data, chr(4))).
  if use-column[17]
  then  C-for-qnty-out:screen-value = string((- temp-goods.qnty-out), entry(1, c-for-qnty-out:private-data, chr(4))).
  if use-column[18]
  then  C-for-price-cli-out-sum:screen-value = string((- temp-goods.price-cli-out-sum), entry(1, c-for-price-cli-out-sum:private-data, chr(4))).
  IF (line-counter(PrnLibStream)  modulo page-size(PrnLibStream)= 0) AND
     (line-counter(PrnLibStream) >= page-size(PrnLibStream)) then DO:
      display STREAM PrnLibStream    with frame top-frame .
  End.
  ...
  if use-column[1]
  then (reg-output(
                    string((if temp-goods.obj-code > 0 then (temp-goods.obj-type + string(temp-goods.obj-code)) else ''), entry(1, c-for-curr-obj:private-data, chr(4)))
                   ,c-for-curr-obj:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[2]
  then (reg-output(
                    string('------>', entry(1, c-for-in-code:private-data, chr(4)))
                   ,c-for-in-code:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[2]
  then (reg-output(
                    string(temp-goods.in-code, entry(1, c-for-in-code:private-data, chr(4)))
                   ,c-for-in-code:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[3]
  then (reg-output(
                    string(temp-goods.in-date, entry(1, c-for-in-date:private-data, chr(4)))
                   ,c-for-in-date:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[4]
  then (reg-output(
                    string((temp-goods.obj-in-type + string(temp-goods.obj-in-code)), entry(1, c-for-ini-obj:private-data, chr(4)))
                   ,c-for-ini-obj:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[5]
  then (reg-output(
                    string(('НДС ' + temp-goods.vat-type + chr(32) + 'НП ' + temp-goods.slt-type), entry(1, c-for-terms:private-data, chr(4)))
                   ,c-for-terms:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[6]
  then (reg-output(
                    string(temp-goods.part-code, entry(1, c-for-part-code:private-data, chr(4)))
                   ,c-for-part-code:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[7]
  then (reg-output(
                    string((chr(4) + temp-goods.artic), entry(1, c-for-artic:private-data, chr(4)))
  ...
  if use-column[8]
  then (reg-output(
                    string(temp-goods.gds-name, entry(1, c-for-gds-name:private-data, chr(4)))
                   ,c-for-gds-name:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[9]
  then (reg-output(
                    string(temp-goods.unit, entry(1, c-for-unit:private-data, chr(4)))
                   ,c-for-unit:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[10]
  then (reg-output(
                    string(temp-goods.price-cli-in-brutto, entry(1, c-for-price-cli-in-brutto:private-data, chr(4)))
                   ,c-for-price-cli-in-brutto:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[11]
  then (reg-output(
                    string(temp-goods.vat-pc, entry(1, c-for-VAT-pc:private-data, chr(4)))
                   ,c-for-VAT-pc:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[12]
  then (reg-output(
                    string(temp-goods.SLT-pc, entry(1, c-for-SLT-pc:private-data, chr(4)))
  ...
  if use-column[13]
  then (reg-output(
                    string(temp-goods.price-cli-in, entry(1, c-for-price-cli-in:private-data, chr(4)))
                   ,c-for-price-cli-in:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[14]
  then (reg-output(
                    string(temp-goods.qnty-all, entry(1, c-for-qnty-all:private-data, chr(4)))
                   ,c-for-qnty-all:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[15]
  then (reg-output(
                    string(temp-goods.qnty-in, entry(1, c-for-qnty-in:private-data, chr(4)))
                   ,c-for-qnty-in:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[16]
  then (reg-output(
                    string(temp-goods.price-cli-in-sum, entry(1, c-for-price-cli-in-sum:private-data, chr(4)))
                   ,c-for-price-cli-in-sum:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[17]
  then (reg-output(
                    string((- temp-goods.qnty-out), entry(1, c-for-qnty-out:private-data, chr(4)))
                   ,c-for-qnty-out:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[18]
  then (reg-output(
                    string((- temp-goods.price-cli-out-sum), entry(1, c-for-price-cli-out-sum:private-data, chr(4)))
                   ,c-for-price-cli-out-sum:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  End.
  if use-column[2]
  then  C-for-in-code:screen-value = string(('Итого в ' + temp-goods.curr-name), entry(1, c-for-in-code:private-data, chr(4))).
  if use-column[14]
  then  C-for-qnty-all:screen-value = string(accum-qnty-all-by-exch-code, entry(1, c-for-qnty-all:private-data, chr(4))).
  if use-column[15]
  then  C-for-qnty-in:screen-value = string(accum-in-qnty-by-exch-code, entry(1, c-for-qnty-in:private-data, chr(4))).
  if use-column[16]
  then  C-for-price-cli-in-sum:screen-value = string(accum-in-by-exch-code, entry(1, c-for-price-cli-in-sum:private-data, chr(4))).
  if use-column[17]
  then  C-for-qnty-out:screen-value = string((- accum-out-qnty-by-exch-code), entry(1, c-for-qnty-out:private-data, chr(4))).
  if use-column[18]
  then  C-for-price-cli-out-sum:screen-value = string((- accum-out-by-exch-code), entry(1, c-for-price-cli-out-sum:private-data, chr(4))).
  IF (line-counter(PrnLibStream)  modulo page-size(PrnLibStream)= 0) AND
     (line-counter(PrnLibStream) >= page-size(PrnLibStream)) then DO:
      display STREAM PrnLibStream    with frame top-frame .
  End.
  ...
  if use-column[2]
  then (reg-output(
                    string(('Итого в ' + temp-goods.curr-name), entry(1, c-for-in-code:private-data, chr(4)))
                   ,c-for-in-code:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[14]
  then (reg-output(
                    string(accum-qnty-all-by-exch-code, entry(1, c-for-qnty-all:private-data, chr(4)))
                   ,c-for-qnty-all:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[15]
  then (reg-output(
                    string(accum-in-qnty-by-exch-code, entry(1, c-for-qnty-in:private-data, chr(4)))
                   ,c-for-qnty-in:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[16]
  then (reg-output(
                    string(accum-in-by-exch-code, entry(1, c-for-price-cli-in-sum:private-data, chr(4)))
                   ,c-for-price-cli-in-sum:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[17]
  then (reg-output(
                    string((- accum-out-qnty-by-exch-code), entry(1, c-for-qnty-out:private-data, chr(4)))
                   ,c-for-qnty-out:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[18]
  then (reg-output(
                    string((- accum-out-by-exch-code), entry(1, c-for-price-cli-out-sum:private-data, chr(4)))
                   ,c-for-price-cli-out-sum:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  End.
  if use-column[5]
  then  C-for-terms:screen-value = string('Итого по поставщику:', entry(1, c-for-terms:private-data, chr(4))).
  if use-column[14]
  then  C-for-qnty-all:screen-value = string(accum-qnty-all-by-supp-code, entry(1, c-for-qnty-all:private-data, chr(4))).
  if use-column[15]
  then  C-for-qnty-in:screen-value = string(accum-in-qnty-by-supp-code, entry(1, c-for-qnty-in:private-data, chr(4))).
  if use-column[16]
  then  C-for-price-cli-in-sum:screen-value = string(accum-in-by-supp-code, entry(1, c-for-price-cli-in-sum:private-data, chr(4))).
  if use-column[17]
  then  C-for-qnty-out:screen-value = string((- accum-out-qnty-by-supp-code), entry(1, c-for-qnty-out:private-data, chr(4))).
  if use-column[18]
  then  C-for-price-cli-out-sum:screen-value = string((- accum-out-by-supp-code), entry(1, c-for-price-cli-out-sum:private-data, chr(4))).
  IF (line-counter(PrnLibStream)  modulo page-size(PrnLibStream)= 0) AND
     (line-counter(PrnLibStream) >= page-size(PrnLibStream)) then DO:
      display STREAM PrnLibStream    with frame top-frame .
  End.
  ...
  if use-column[5]
  then (reg-output(
                    string('Итого по поставщику:', entry(1, c-for-terms:private-data, chr(4)))
                   ,c-for-terms:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[14]
  then (reg-output(
                    string(accum-qnty-all-by-supp-code, entry(1, c-for-qnty-all:private-data, chr(4)))
                   ,c-for-qnty-all:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[15]
  then (reg-output(
                    string(accum-in-qnty-by-supp-code, entry(1, c-for-qnty-in:private-data, chr(4)))
                   ,c-for-qnty-in:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[16]
  then (reg-output(
                    string(accum-in-by-supp-code, entry(1, c-for-price-cli-in-sum:private-data, chr(4)))
                   ,c-for-price-cli-in-sum:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[17]
  then (reg-output(
                    string((- accum-out-qnty-by-supp-code), entry(1, c-for-qnty-out:private-data, chr(4)))
                   ,c-for-qnty-out:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
  ...
  if use-column[18]
  then (reg-output(
                    string((- accum-out-by-supp-code), entry(1, c-for-price-cli-out-sum:private-data, chr(4)))
                   ,c-for-price-cli-out-sum:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
