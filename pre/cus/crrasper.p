  ...
  for each  old_doc-ord-line no-lock  where
    ...
    if buf_goods.stts <> 0 then do:
          if LENGTH(new_trn-doc.Ps) < 31900 then do:
              new_trn-doc.Ps = trim(new_trn-doc.Ps) + chr(10) + 'товар удален '  + old_doc-ord-line.artic.
          end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjcr in g#library
  (input  new_trn-doc.obj-type
  ,input  new_trn-doc.obj-code
  ,input  old_doc-ord-line.artic
  ,input  old_doc-ord-line.prod-type
  ,input  old_doc-ord-line.prod-code
  ,buffer ub.gds-obj
  ) no-error .
    ...
    if ub.gds-obj.price-sale  = 0  or ub.gds-obj.price-sale  = ?  then do:
          if LENGTH(new_trn-doc.Ps) < 31900 then do:
              new_trn-doc.Ps = trim(new_trn-doc.Ps) + chr(10) + 'нет цены у '  + old_doc-ord-line.artic.
          end.
          put stream str unformatted
              substitute("&1,&2" , buf_goods.gds-code,  old_doc-ord-line.qnty )
              skip.
    ...
    if old_doc-ord-line.price-rubl <> ub.gds-obj.price-sale and old_doc-ord-line.price-rubl <> 0 then do:
      if length(old_trn-ord-doc.ps) < 31900 then do:
          old_trn-ord-doc.Ps = trim(old_trn-ord-doc.Ps) + chr(10) + old_doc-ord-line.artic  + " Цена взята с РЦ , была: " + string (old_doc-ord-line.price-rubl) .
      end.
    end.
...
if not error-status :error then do:
 ...
 if  old_trn-ord-doc.status_ = 'запрос':U or error-status :error then do:
