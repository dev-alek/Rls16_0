...
define  shared temp-table tt-cst no-undo
...
prod-code
.
define   shared temp-table tt-cst-ext  no-undo
field cst-code         like ub.parts.cst-code
field artic            like ub.goods.artic
field prod-type        like ub.goods.prod-type
field prod-code        like ub.goods.prod-code
...
prod-code
.
define   shared temp-table tt-cst-year  no-undo
...
if error-status:error then return error.
run waitfram-show in this-procedure ("Остатки по товарам на объектах фирмы...").
my-accum = 0.
_gds-obj:
...
for each temp-parts :
  ...
  IF my-accum MODULO 50  = 0 then do:
    run waitfram-show in this-procedure ("Остатки по товарам на объектах фирмы..." + " Обработано " + string(my-accum) + " партий остатков ").
  end.
  ...
  if not avail tt-cst then do:
    ...
    tt-cst.name_artic_unit = string(ub.goods.gds-name,
                                    ("X(" + string(41 - 5 - length(ub.goods.artic)) + ")") )
                                          +  chr(32) +
                              ub.goods.artic + chr(32) +
...
run waitfram-show in this-procedure ("Получение информации по расходам...").
my-accum = 0.
FOR EACH obj-list NO-LOCK:
  FOR EACH for-doc No-LOCK WHERE
    ...
    if LOOKUP(for-doc.ext-doc-type, 'iv,rv,ev':U) > 0 then NEXT.
    assign
    doc-num = for-doc.doc-code
    is-out = if LOOKUP(for-doc.ext-doc-type, 'ie,re,rs,vt,im':U) > 0  then 1 else -1
    is-sale = if lookup(for-doc.ext-doc-type, 'es,rs':U) > 0
              then yes else no
    is-ext_expense = if lookup(for-doc.ext-doc-type, 'ee':U) > 0
                     then yes else no
    is-other_expense = if lookup(for-doc.ext-doc-type, 'ee':U + chr(44) + 'ie,es,rs':U) = 0
                     then yes else no
    .
...
FOR  each for-line NO-LOCk WHERE for-line.doc-code = doc-num:
  my-accum = my-accum + 1.
  IF my-accum MODULO 50  = 0 then do:
    run waitfram-show in this-procedure ("Получение информации по расходам..." + " Обработано " + string(my-accum) + " строк документов ").
  end.
  _parts:
  FOR EACH ub.parts NO-LOCK WHERE
    ...
    IF ub.parts.is-supp = no  then NEXT _parts.
    if can-find(first tt-file-ignore where
    ...
    if not avail tt-cst then do:
      ...
      tt-cst.nationality = ub.goods.nationality
      tt-cst.name_artic_unit = string(ub.goods.gds-name,
                                      ("X(" + string(41 - 5 - length(goods.artic)) + ")") )
                                           +  chr(32) +
                               ub.goods.artic + chr(32) +
                               string(ub.goods.unit-base, "X(3)")
      tt-cst.tnved = ub.goods.tnved
...
END.
  END.
END.
run waitfram-show in this-procedure ("Список ГТД, исключенных из отчета...").
my-accum = 0.
for each obj-list No-LOCK:
_zero:
  for each for-doc No-LOCK WHERE
    ...
    my-accum = my-accum + 1.
    IF my-accum MODULO 50  = 0 then do:
      run waitfram-show in this-procedure ("Список ГТД, исключенных из отчета..." + " Обработано " + string(my-accum) + " приходных партий ").
    end.
    find first tt-cst where
               tt-cst.cst-code = ub.parts.cst-code AND
