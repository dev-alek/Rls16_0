...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjpr in g#library
  ...
  ,output v-obj-AssMin
  ,output v-obj-igt
  ,output tmp#zakaz.min-stock
  ,output tmp#zakaz.max-stock
  ,output tmp#zakaz.service-order
  ,output tmp#zakaz.min-order
  )  .
 end.
 when 2 then do:
  ...
  for each buf_season no-lock where
    ...
    then do:
      find first buf_gds-season-attr no-lock where buf_gds-season-attr.sea-code = buf_gds-season.sea-code
        ...
        and buf_gds-season-attr.attr-code = 'gdssea-season-coef':U
          no-error .
...
find first ubflt.usr-flt where
    ...
    if available ubflt.usr-flt then do:
                    tog-det-prizn = logical(entry(5, ubflt.usr-flt.list_, chr(4))) .
    end.
