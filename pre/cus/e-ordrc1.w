...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-5 RECT-7 Tog-obj SortType Classify OrdDate T-1 v-prc WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-5 RECT-7 Tog-obj SortType Classify OrdDate T-1 v-prc WITH FRAME F-Main.
...
assign frame F-Main tog-obj Classify sorttype OrdDate T-1 v-prc.
...
ReportHeader = ReportHeader  + chr(10) +
               ...
               "Формировать отчет по " + t-OrdDate + chr(10) +
               ( if t-1 then chr(10) + "Процент поставлено/разрешено менее " + string(v-prc) + "%"
                        else " "
                        ) .
END PROCEDURE.
