...
define variable vss-workfile    as character no-undo init "$Workfile: crhldr.p $":U .
define variable vss-archive     as character no-undo init "$Archive: cus/crhldr.p $":U .
define variable vss-description as character no-undo init "Создание МФ расхода ".
...
:
  ...
  if error-status :error
  then do:
    undo, return error substitute( "&1. &2&3&4", vss-include-info19, return-value, chr(10), error-status :get-message (1)).
  end.
end.
end procedure.
...
define buffer old_trn-doc  for  ub.ord-doc.
define buffer old_doc-line for  ub.ord-line.
...
find first cli_clients no-lock where
           cli_clients.obj-code = old_trn-doc.obj-code  and
           cli_clients.obj-type = old_trn-doc.obj-type  no-error .
find first obj_clients no-lock where
           obj_clients.obj-code = old_trn-doc.cli-code  and
           obj_clients.obj-type = old_trn-doc.cli-type  no-error .
  create  tt-trn-doc.
  buffer-copy  old_trn-doc  to    tt-trn-doc
    assign
     ...
     tt-trn-doc.PS             = "Сформирована по заказу № " +  old_trn-doc.doc-code + " " + old_trn-doc.doc-type + " " + old_trn-doc.PS
     tt-trn-doc.creid          = v-cntxt-userid
     tt-trn-doc.flag_          = false
     tt-trn-doc.ext-doc-type   = v-ext-doc-type
     tt-trn-doc.discnt-type    = 'процент':U
     ...
     tt-trn-doc.host-code            = obj_clients.host-code
     tt-trn-doc.obj-code             = old_trn-doc.cli-code
     tt-trn-doc.obj-type             = old_trn-doc.cli-type
     tt-trn-doc.hold-obj-code        = old_trn-doc.obj-code
     tt-trn-doc.hold-obj-type        = old_trn-doc.obj-type
     tt-trn-doc.cli-code             = cli_clients.host-code
     tt-trn-doc.cli-type             = 'орг':U
     tt-trn-doc.contract-code        = old_trn-doc.contract-code
  .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run baserate in g#library
  ...
  ,output tt-trn-doc.base-scale
   ...
   if lookup (string(ub.sysconf.purch-code), '1,2,3':U) = 0 then do:
     if not is-rt then
...
.
  ...
  assign
      new_trn-doc.hold-obj-code = old_trn-doc.obj-code
      new_trn-doc.hold-obj-type = old_trn-doc.obj-type
      new_trn-doc.cli-code      = cli_clients.host-code
      new_trn-doc.cli-type      = 'орг':U
      ...
      new_trn-doc.contract-code        = old_trn-doc.contract-code
...
define variable v-err2 as logical   no-undo .
  ...
  for each  old_doc-line no-lock  where
           ...
           find first buf_goods no-lock where
                      buf_goods.artic     =  old_doc-line.artic and
                      buf_goods.prod-type =  old_doc-line.prod-type and
                      buf_goods.prod-code =  old_doc-line.prod-code
                      no-error .
    if buf_goods.stts <> 0 then do:
          if LENGTH(new_trn-doc.Ps) < 31900 then do:
              new_trn-doc.Ps = trim(new_trn-doc.Ps) + chr(10) + 'товар удален '  + old_doc-line.artic.
          end.
          put stream str unformatted
              substitute("&1,&2" , buf_goods.gds-code,  old_doc-line.qnty )
              skip.
          v-err2 = true  .
          next.
    end.
        v-event-code = substitute("mf_&1" , new_trn-doc.ext-doc-type) .
...
if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run goassizt in g#library2
  ...
  ,output v-mess
          ...
          if v-Ok = false then do:
              if LENGTH(new_trn-doc.Ps) < 31900 then do:
                  new_trn-doc.Ps = trim(new_trn-doc.Ps) + chr(10) +  v-mess.
              end.
              put stream str unformatted
        ...
        v-event-code = substitute("cli_mf_&1" , new_trn-doc.ext-doc-type) .
define variable vss-include-info23 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run goassizt in g#library2
  (input  v-event-code
  ,input  buf_goods.gds-code
  ,input  new_trn-doc.hold-obj-type
  ,input  new_trn-doc.hold-obj-code
  ,input  false
  ,output v-Ok
  ,output v-mess
  ) no-error.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjcr in g#library
  (input  new_trn-doc.obj-type
  ,input  new_trn-doc.obj-code
  ,input  old_doc-line.artic
  ,input  old_doc-line.prod-type
  ,input  old_doc-line.prod-code
  ,buffer ub.gds-obj
  ) no-error .
    ...
    find first ub.gds-obj no-lock where
        ...
        ub.gds-obj.artic     = old_doc-line.artic      and
        ub.gds-obj.prod-type = old_doc-line.prod-type  and
        ub.gds-obj.prod-code = old_doc-line.prod-code  no-error .
        if error-status :error then message error-status :get-message(1) .
    if ub.gds-obj.price-sale  = 0  or ub.gds-obj.price-sale  = ?  then do:
          if LENGTH(new_trn-doc.Ps) < 31900 then do:
              new_trn-doc.Ps = trim(new_trn-doc.Ps) + chr(10) + 'нет цены у '  + old_doc-line.artic.
          end.
          put stream str unformatted
              substitute("&1,&2" , buf_goods.gds-code,  old_doc-line.qnty )
              skip.
    ...
    if old_doc-line.price-rubl <> ub.gds-obj.price-sale and old_doc-line.price-rubl <> 0 then do:
      if length(old_trn-doc.ps) < 31900 then do:
           find current old_trn-doc exclusive-lock no-error .
          old_trn-doc.Ps = trim(old_trn-doc.Ps) + chr(10) + old_doc-line.artic  + " Цена взята с РЦ , была: " + string (old_doc-line.price-rubl) .
      end.
    end.
  find first corr_ord-line exclusive-lock where recid(corr_ord-line) = recid(old_doc-line).
  assign
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run rootnode in g#library
  ...
  ,output v-root-node
      ...
      create old_gds-dtl.
      BUFFER-COPY old_doc-line to old_gds-dtl
        assign
          old_gds-dtl.doc-qnty = old_doc-line.qnty
          old_gds-dtl.prt-code = v-root-node
          old_gds-dtl.ov = v-no-ask
        .
      create tt-doc-line .
      BUFFER-COPY old_doc-line to tt-doc-line
        assign
          tt-doc-line.doc-qnty       = old_doc-line.qnty
          tt-doc-line.fact-qnty      = old_doc-line.qnty
     ...
     for each   old_gds-dtl where
      ...
      buffer-copy  old_gds-dtl to  tt-gds-dtl.
        assign
          ...
          tt-gds-dtl.fact-qnty      = old_doc-line.qnty
          tt-gds-dtl.doc-qnty       = old_doc-line.qnty
      .
     end.
  end.
...
for each tt-doc-line :
 ...
 run waitfram-show in this-procedure ("Создание ЗАПР " + caps('рас':U)) .
...
run waitfram-show in this-procedure ("Создание НАКЛ- " + caps('рас':U)) .
run clos-trn in this-procedure (new_trn-doc.doc-code) no-error .
if not error-status :error then do:
   create buf_ord-doc-rcv.
   buffer-copy old_trn-doc to buf_ord-doc-rcv
   assign
 ...
 run create-chain in this-procedure (
      ...
      ,''
      ,'' ).
   run cus/ordorcls.p (input parparentproc, input recid(old_trn-doc) , input false ) no-error .
    if  old_trn-doc.STATUS_ = 'запрос':U or error-status :error then do:
        run waitfram-hide in this-procedure .
        UNDO MAIN-BLOCK, return error substitute("Нельзя закрыть заказ! &1" , return-value , error-status :get-message(1)    ) .
    end.
end.
else do:
    run waitfram-hide in this-procedure .
