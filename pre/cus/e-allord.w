...
PROCEDURE adm-disable :
    DISABLE date-ship date-p-1 date-p-2 g#type BUTTON-1 EDITOR-1 T-grp radio-column B-neword T-artic B-newcol radio-gds-obj tog-goods-from-am tog-det-post tog-det-prizn FILL-IN-1 FILL-IN-2 FILL-IN-3 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN date-ship date-p-1 date-p-2 g#type BUTTON-1 EDITOR-1 T-grp radio-column B-neword T-artic B-newcol radio-gds-obj tog-goods-from-am tog-det-post tog-det-prizn FILL-IN-1 FILL-IN-2 FILL-IN-3 WITH FRAME F-Main.
...
DO:
  ...
  editor-1 = e-method .
  display editor-1 with frame F-Main .
...
if integer(entry(2,(entry(3,ubflt.usr-flt.list_)), "=" )) = 1 and radio-column:screen-value = "no" then do :
    enable tog-det-prizn with frame F-Main.
end.
else do :
    hide tog-det-prizn .
end.
...
DO:
    if radio-column:screen-value = "yes" then do :
        enable b-neword with frame F-Main .
      ...
      find first ubflt.usr-flt  no-lock where
        ...
        enable b-newcol radio-gds-obj fill-in-3 with frame F-Main .
        hide b-neword .
        if radio-gds-obj:screen-value = "no" then do :
          enable tog-det-post with frame F-Main.
        end.
      if (integer(entry(2,(entry(3,ubflt.usr-flt.list_)), "=" )) = 1) then do :
          enable tog-det-prizn with frame F-Main.
      end.
    end.
    run save-usr-flt.
...
DO:
  ...
  run save-usr-flt.
END.
ON VALUE-CHANGED OF tog-det-post IN FRAME F-Main
DO:
  run save-usr-flt.
END.
ON VALUE-CHANGED OF tog-goods-from-am IN FRAME F-Main
DO:
  run save-usr-flt.
END.
ON VALUE-CHANGED OF tog-det-prizn IN FRAME F-Main
DO:
  run save-usr-flt.
END.
...
on ' ' of date-ship in frame F-Main
do:
...
on delete-character of date-ship in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of date-ship in frame F-Main
do:
...
on ctrl-b of date-ship in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of date-ship in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of date-ship in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date15
    ...
    .
  if date-ship :POPUP-MENU in frame F-Main = ?
  then do:
    ASSIGN
      date-ship :POPUP-MENU in frame F-Main = MENU m-ed-date15 :HANDLE
      date-ship :MENU-MOUSE in frame F-Main = 3
    .
  end.
  define variable v-label-handle15 as handle no-undo .
  assign
    v-label-handle15 = date-ship :side-label-handle in frame F-Main
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date15-1 in menu m-ed-date15 DO:
    apply "ctrl-b":U to date-ship in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date15-2 in menu m-ed-date15 DO:
    apply "ctrl-d":U to date-ship in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date15-3 in menu m-ed-date15 DO:
    apply "ctrl-e":U to date-ship in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date15-4 in menu m-ed-date15 DO:
    apply "ctrl-f":U to date-ship in frame F-Main .
  END.
define variable vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on ' ' of date-p-1 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on delete-character of date-p-1 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of date-p-1 in frame F-Main
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of date-p-1 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of date-p-1 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of date-p-1 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date17
    ...
    .
  if date-p-1 :POPUP-MENU in frame F-Main = ?
  then do:
    ASSIGN
      date-p-1 :POPUP-MENU in frame F-Main = MENU m-ed-date17 :HANDLE
      date-p-1 :MENU-MOUSE in frame F-Main = 3
    .
  end.
  define variable v-label-handle17 as handle no-undo .
  assign
    v-label-handle17 = date-p-1 :side-label-handle in frame F-Main
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date17-1 in menu m-ed-date17 DO:
    apply "ctrl-b":U to date-p-1 in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date17-2 in menu m-ed-date17 DO:
    apply "ctrl-d":U to date-p-1 in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date17-3 in menu m-ed-date17 DO:
    apply "ctrl-e":U to date-p-1 in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date17-4 in menu m-ed-date17 DO:
    apply "ctrl-f":U to date-p-1 in frame F-Main .
  END.
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on ' ' of date-p-2 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on delete-character of date-p-2 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of date-p-2 in frame F-Main
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of date-p-2 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of date-p-2 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of date-p-2 in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date19
    ...
    .
  if date-p-2 :POPUP-MENU in frame F-Main = ?
  then do:
    ASSIGN
      date-p-2 :POPUP-MENU in frame F-Main = MENU m-ed-date19 :HANDLE
      date-p-2 :MENU-MOUSE in frame F-Main = 3
    .
  end.
  define variable v-label-handle19 as handle no-undo .
  assign
    v-label-handle19 = date-p-2 :side-label-handle in frame F-Main
  .
...
END PROCEDURE.
PROCEDURE my-var :
ASSIGN frame F-Main g#type T-grp T-artic
...
PROCEDURE my_init :
    ...
    apply "choose" to button-1 in frame F-Main .
    g#type:radio-buttons = "Объект-Поставщик,ОП,Объект-Фирма,ОФ,Фирма-Поставщик,ФП,Объект-Объект,ОО".
    run value-order-type in parent-handle (output g#type) .
    assign
    ...
    if available ubflt.usr-flt then do:
      DO v-ii = 1 TO NUM-ENTRIES( ubflt.usr-flt.list_, chr(4)):
        CASE v-ii:
            WHEN 1 THEN DO:
              radio-column:screen-value      = ENTRY(v-ii, ubflt.usr-flt.list_, chr(4)).
            END.
            WHEN 2 THEN DO:
              radio-gds-obj:screen-value     = ENTRY(v-ii, ubflt.usr-flt.list_, chr(4)).
            END.
            WHEN 3 THEN DO:
              tog-det-post:screen-value      = ENTRY(v-ii, ubflt.usr-flt.list_, chr(4)).
            END.
            WHEN 4 THEN DO:
              tog-goods-from-am:screen-value = ENTRY(v-ii, ubflt.usr-flt.list_, chr(4)).
            END.
            WHEN 5 THEN DO:
              tog-det-prizn:screen-value     = ENTRY(v-ii, ubflt.usr-flt.list_, chr(4)).
            END.
        end case.
      end.
    end.
    apply "value-changed" to radio-column in frame F-Main.
DISPLAY date-p-1 date-p-2 date-ship EDITOR-1 g#type  FILL-IN-1 FILL-IN-2 FILL-IN-3 tog-det-post tog-det-prizn WITH FRAME F-Main.
if radio-gds-obj:screen-value = "yes" then do :
    hide tog-det-post .
end.
else do :
...
find first ubflt.usr-flt where
        ...
        assign
            ubflt.usr-flt.list_ = string(radio-column)  + chr(4)
                                + string(radio-gds-obj) + chr(4)
                                + string(tog-det-post)  + chr(4)
                                + string(tog-goods-from-am) + chr(4)
