...
PROCEDURE adm-disable :
    DISABLE RECT-checks B-type f-region-code f-org-code f-shop-code F-substring f-type-name WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-checks B-type f-region-code f-org-code f-shop-code F-substring f-type-name WITH FRAME F-Main.
...
and num-entries(v-uf-List_, chr(4)) = 5 then do:
  assign
  f-type-name = entry(1, v-uf-List_, chr(4))
  f-region-code = integer(entry(2, v-uf-List_, chr(4)))
  f-org-code = integer(entry(3, v-uf-List_, chr(4)))
  f-shop-code = integer(entry(4, v-uf-List_, chr(4)))
  f-substring = integer(entry(5, v-uf-List_, chr(4)))
  NO-ERROR
  .
...
:
  assign
  Sheetf.ColFOrmat = '6=dd/mm/yyyy;7=0;8=0' + chr(4) + '7=@;8=@'
  .
  ...
  "Код региона"  + chr(44) +
  "№ Организации"  + chr(44) +
  "№ торгового предприятия Организации"  + chr(44) +
  "№ кассы"  + chr(44) +
  "№ чека"  + chr(44) +
  "Дата чека"  + chr(44) +
  "Время чека"  + chr(44) +
  "Социальный номер"  + chr(44) +
  "Стоимость купленных товаров"  + chr(44) +
  "Скидка"  + chr(44) +
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run r-b-curr in g#library
  ...
  _chk-gds:
    FOR EACH buf_chk-doc No-LOCK WHERE
      ...
      if lookup(string(buf_chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) > 0
      ...
      f-region-code  FORMAT "99":U chr(32)
      f-org-code  FORMAT "9999":U chr(32)
      f-shop-code  FORMAT "9999":U chr(32)
      buf_chk-doc.pay-desk FORMAT "99" chr(32)
      buf_chk-doc.chk-num FORMAT  "999999" chr(32)
      string(buf_chk-doc.chk-date, "99/99/9999") chr(32)
      string(buf_chk-doc.chk-time, "hh:mm")      chr(32)
      substring(STRING(buf_chk-doc.src-d-card), f-substring, 12) FORMAT "x(12)" chr(32)
      .
      if buf_chk-doc.tot-doc < 0 then
      PUT STREAM PrnLibStream UNFORMATTED
      buf_chk-doc.tot-doc FORMAT "-99999.99" chr(32)
      buf_chk-doc.discnt FORMAT "-99999.99" chr(32)
      "000000.00"
      SKIP
      .
      else
      PUT STREAM PrnLibStream UNFORMATTED
      buf_chk-doc.tot-doc FORMAT "999999.99" chr(32)
      buf_chk-doc.discnt FORMAT "999999.99" chr(32)
      "000000.00"
      SKIP
      .
      if Make-Excel then  put   stream ForExcel unformatted
      f-region-code  FORMAT "99":U CHR(9)
      f-org-code  FORMAT "9999":U CHR(9)
      f-shop-code  FORMAT "9999":U CHR(9)
      buf_chk-doc.pay-desk FORMAT "99" CHR(9)
      buf_chk-doc.chk-num FORMAT  "999999" CHR(9)
      string(buf_chk-doc.chk-date, "99/99/9999") CHR(9)
      (chr(4) + string(buf_chk-doc.chk-time, "hh:mm"))      CHR(9)
      (chr(4) + substring(STRING(buf_chk-doc.src-d-card), f-substring, 12)) FORMAT "x(13)" CHR(9)
      buf_chk-doc.tot-doc FORMAT "-999999.99" CHR(9)
      buf_chk-doc.discnt FORMAT "-999999.99" CHR(9)
...
PROCEDURE My-var :
 ...
 or f-type-name = "":U then do:
  ...
  (if f-type-name  = "":U
   then ("Введите тип карты, соответствующий СКМ"  + chr(10))
   else "":U)
  (if f-org-code  = 0
   then ("Введите номер Организации"  + chr(10))
   else "":U)
  (if f-shop-code  = 0
   then ("Введите номер торгового предприятия"  + chr(10))
   else "":U)
  (if f-region-code  = 0
   then ("Введите код региона"  + chr(10))
   else "":U)
   view-as alert-box error .
   return error .
 end.
 assign
 v-uf-list_ = f-type-name + chr(4) +
             string(f-region-code, "9999":U) +  chr(4) +
             string(f-org-code, "9999":U) +  chr(4) +
             string(f-shop-code, "9999":U) + chr(4) +
             string(f-substring)
.
...
End.
ReportHeader =
               "Тип карты в системе:" + chr(32) + f-type-name + chr(10) +
               "Номер Организации:" + chr(32) + string(f-org-code, "9999":U) + chr(10) +
               "Номер торгового предприятия Организации:" + chr(32) + string(f-shop-code, "9999":U)
                .
END PROCEDURE.
