...
if not( ( buf_ord-doc.status_ = 'запрос':U and buf_ord-doc.flag_ = true  ) or
        ( buf_ord-doc.status_ = 'новый':U and buf_ord-doc.flag_ = true  )  ) then do:
   RELEASE buf_ord-doc.
   message "Открыть можно только заказ в статусе запр+ или новый+ " view-as alert-box information .
   undo, return .
end.
if  buf_ord-doc.status_ = 'запрос':U and buf_ord-doc.flag_ = true  then do:
...
if  buf_ord-doc.status_ = 'новый':U and buf_ord-doc.flag_ = true  then do:
