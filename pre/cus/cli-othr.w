...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-2 FOR EACH ub.cli-gds       WHERE cli-gds.artic = xartic  AND cli-gds.prod-code = xprod-code  AND cli-gds.prod-type = xprod-type  and cli-gds.price-cli > 0  AND NOT (cli-gds.cli-code =  xcli-code And cli-gds.cli-type =  xcli-type)   NO-LOCK,       EACH ub.clients WHERE clients.obj-code = cli-gds.cli-code   AND clients.obj-type = cli-gds.cli-type       AND (clients.sup-cons = TRUE  OR clients.sup-gds = TRUE  OR clients.sup-serv = TRUE) NO-LOCK     BY ub.cli-gds.price-cli.
END PROCEDURE.
