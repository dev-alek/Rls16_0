...
then do:
  ...
  display v-dir-name with frame Dialog-Frame.
    .
END.
...
CASE p-mode:
  when 'shd':U then do:
      ...
      FIND FIRST buf_schedule-attr NO-LOCK WHERE
                 buf_schedule-attr.cre-db-num = p-cre-db-num
             and buf_schedule-attr.task-type  = p-task-type
             and buf_schedule-attr.attr-code = ('schd-free-id':U + chr(4) + 'dctxt-e') NO-ERROR.
      IF AVAILABLE  buf_schedule-attr
          AND buf_schedule-attr.task-num <> p-task-num
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH X_rule-profile SHARE-LOCK,       EACH ub.rule WHERE TRUE  SHARE-LOCK,       EACH X_dis-card-type WHERE TRUE  SHARE-LOCK.
...
file-name-1 = ENTRY(1, v-file-name, chr(63))
file-name-2 = (IF NUM-ENTRIES(v-file-name, chr(63)) > 1
               AND rs-file-rule = "seq"
               THEN ENTRY(2, v-file-name, chr(63))
               ELSE '':U)
.
END PROCEDURE.
...
define buffer buf_schedule-attr for ub.schedule-attr.
  CASE p-mode:
    when 'shd':U then do:
      if p-task-num > 0 then do:
        ...
        for each buf_schedule no-lock where
            ...
            first buf_schedule-attr no-lock where
              ...
              AND buf_schedule-attr.attr-code = ('schd-free-id':U + chr(4) + 'dctxt-e') :
      ...
      v-param-list = string(12) + chr(4) + 'ini' + chr(4) + "seq" + chr(4) + "f?.txt" + chr(4).
    END.
    WHEN 'run' THEN DO:
      assign
      v-param-list = string(12) + chr(4) + 'ini' + chr(4) + "seq" + chr(4) + "f?.txt" + chr(4).
  ...
  END CASE.
  ASSIGN
  v-local-rule-profile-id = integer(entry(1, v-param-list, chr(4)))
  v-dir = entry(2, v-param-list, chr(4))
  v-file-rule = entry(3, v-param-list, chr(4))
  v-file-name = entry(4, v-param-list, chr(4))
  v-emitent-host-code = 0
  v-dc-type = entry(1, entry(5, v-param-list, chr(4)))
...
CASE p-mode:
    WHEN 'run':U THEN DO:
        ASSIGN
        rs-dir:RADIO-BUTTONS IN FRAME Dialog-Frame
                             = "берется из Ini-файла ([schedule-free] dctxt-e_out=)" + chr(44) +
                               "ini":U + chr(44) +
                               "задать директорию экспорта" + chr(44) +
                               "other".
    END.
    WHEN 'shd':U THEN DO:
        ASSIGN
        rs-dir:RADIO-BUTTONS = "берется из Ini-файла ([schedule-free] dctxt-e_out=)" + chr(44) +
                               "ini":U + chr(44) +
                               "привязываются  к строке расписания" + chr(44) +
                               "other".
    END.
END CASE.
DISPLAY
...
rs-file-rule
file-name-1
file-name-2 WHEN file-name-2:VISIBLE IN FRAME Dialog-Frame
.
...
CASE rs-file-rule:
    WHEN "seq":U THEN DO:
      ASSIGN
      v-file-name = file-name-1 + chr(63) + file-name-2.
    END.
    ...
    END.
END CASE.
v-dop-file-name = replace(v-file-name, chr(63), '':U).
do ii = 1 to length('\/:*?"<>|':U):
  if index(v-dop-file-name, substring('\/:*?"<>|':U, ii, 1)) > 0 then do:
    message
    substitute("Введенное имя файла содержит недопустимый символ &1", substring('\/:*?"<>|':U, ii, 1))
    view-as alert-box error .
    return error.
  end.
end.
...
END.
ASSIGN
v-param-list = ( string(v-rule-profile-id) + chr(4) +
                IF rs-dir = 'ini' THEN 'ini' ELSE v-dir-name) + chr(4) +
               rs-file-rule + chr(4) +
                v-file-name + chr(4) +
                X_dis-card-type.type:SCREEN-VALUE + chr(4) +
                v-other-param.
     ...
     and  buf_rule-call-param.order_id = X_rule-by-call.order_id :
      ...
      CASE buf_rule-call-param.param-data-type:
        WHEN 'character':U THEN DO:
           ASSIGN
           buf_tt0-rule-call-param.param-value-character = ENTRY(buf_tt0-rule-call-param.param-num, p-other-params, chr(4))
           NO-ERROR.
        END.
        WHEN 'date':U THEN DO:
           ASSIGN
           buf_tt0-rule-call-param.param-value-date = IF entry(buf_tt0-rule-call-param.param-num, p-other-params, chr(4)) = chr(63)
                                                      THEN ?
                                                      ELSE DATE(ENTRY(buf_tt0-rule-call-param.param-num, p-other-params, chr(4)))
           NO-ERROR.
        END.
        WHEN 'decimal':U THEN DO:
           ASSIGN
           buf_tt0-rule-call-param.param-value-decimal = decimal(ENTRY(buf_tt0-rule-call-param.param-num, p-other-params, chr(4)))
           NO-ERROR.
        END.
        WHEN 'integer':U THEN DO:
           ASSIGN
           buf_tt0-rule-call-param.param-value-integer = integer(ENTRY(buf_tt0-rule-call-param.param-num, p-other-params, chr(4)))
           NO-ERROR.
        END.
        WHEN 'logical':U THEN DO:
           ASSIGN
           buf_tt0-rule-call-param.param-value-logical = logical(ENTRY(buf_tt0-rule-call-param.param-num, p-other-params, chr(4)))
           NO-ERROR.
        END.
      END CASE.
      IF ERROR-STATUS:ERROR THEN DO:
