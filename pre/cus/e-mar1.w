...
PROCEDURE adm-disable :
    DISABLE RECT-supp RECT-method T-zero RS-sort WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-supp RECT-method T-zero RS-sort WITH FRAME F-Main.
...
DO:
  assign RS-sort.
END.
ON VALUE-CHANGED OF T-zero IN FRAME F-Main
DO:
  assign T-zero.
END.
...
"50" + chr(44) +
"7"
sheetf.colformat = "2=0":U + chr(4) + "2=@"
str2 = " "
.
Assign
 STR-obj-type = ''
...
ReportHeader =  "Сортировка: " + radio-label(string(RS-sort), Rs-sort:radio-buttons) + chr(10) +
                (if not T-zero
                 then  "Не показывать товар с нулевым значением <дни в продаже>"
                 else  ("Показывать товар с нулевым значением <дни в продаже>" + chr(10) +
