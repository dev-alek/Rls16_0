...
field measure-qnty like ub.rvs-line.state-measure-qnty
     ...
     if last-of(ub.clients.obj-code) then do:
       ...
       do kk = 1 to max-str :
            ...
            if Make-Excel then  put   stream ForExcel unformatted
              ...
              format-excel-text(ub.clients.obj-name) CHR(9)
            .
            end.
            else do:
            if Make-Excel then  put   stream ForExcel unformatted
         ...
         for each  bf_ord-cons-gds no-lock  where bf_ord-cons-gds.cons-code = bf_ord-cons.cons-code,    first ub.goods where bf_ord-cons-gds.artic = ub.goods.artic                                               and bf_ord-cons-gds.prod-type = ub.goods.prod-type                                   and bf_ord-cons-gds.prod-code = ub.goods.prod-code no-lock :
            find first   temp-tt where temp-tt.obj-type = ub.clients.obj-type + " " + string(ub.clients.obj-code) and
                    ...
                    if Make-Excel then  put   stream ForExcel unformatted
                      excel-qnty-null(temp-tt.max-qnty)    CHR(9)
                      excel-qnty-null(temp-tt.measure-qnty) CHR(9)
                      excel-qnty-null(temp-tt.free-qnty)   CHR(9)
                      ...
                      excel-qnty-null(temp-tt.qnty-rcv)    CHR(9)
                      format-excel-text(temp-tt.rcv-code)    CHR(9)
                    .
                    end.
                    else do:
                      if Make-Excel then  put   stream ForExcel unformatted
...
if Make-Excel then  put   stream ForExcel unformatted  "Итого"    CHR(9)
 ...
 if Make-Excel then  put   stream ForExcel unformatted chr(10) chr(10).
 ...
 If l-nn = 1 then Do:
    run calc-var in this-procedure (input ub.clients.obj-type,
      ...
      assign
          ...
          temp-tt.measure-qnty = l-ost
          temp-tt.free-qnty   = temp-tt.max-qnty  - temp-tt.measure-qnty
