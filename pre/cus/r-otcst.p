...
define NEW shared temp-table tt-cst no-undo
...
prod-code
.
define  NEW shared temp-table tt-cst-ext  no-undo
field cst-code         like ub.parts.cst-code
field artic            like ub.goods.artic
field prod-type        like ub.goods.prod-type
field prod-code        like ub.goods.prod-code
...
prod-code
.
define  NEW shared temp-table tt-cst-year  no-undo
...
define variable v-line      as character no-undo format "X(198)" .
...
define variable v-line1     as character no-undo format "X(198)" .
define variable v-line2     as character no-undo format "X(198)" .
define variable v-line3     as character no-undo format "X(198)" .
...
sym01 space(0) for-ii                  format ">>>>9"            COLUMN-LABEL "  1  "  space(0)
sym02 space(0) tt-cst.cst-code         format "x(6)"             COLUMN-LABEL "  2   "  space(0)
sym03 space(0) tt-cst.name_artic_unit  format "x(41)" COLUMN-LABEL "                    3"  space(0)
sym04 space(0) tt-cst.nationality      format "X(12)"            COLUMN-LABEL "     4"  space(0)
sym05 space(0) tt-cst.tnved            format "X(10)"            COLUMN-LABEL "    5"  space(0)
sym06 space(0) for-num-place_brutto    format "X(21)"            COLUMn-LABEL "          6"  space(0)
sym07 space(0) tt-cst.qnty-income      format "->>>>>>>9.<<<"     COLUMN-LABEL "      7"  space(0)
sym08 space(0) tt-cst.qnty-start       format "->>>>>>>9.<<<"     COLUMN-LABEL "      8"  space(0)
sym09 space(0) tt-cst.qnty-sale        format "->>>>>>>9.<<<"     COLUMN-LABEL "      9"  space(0)
sym10 space(0) tt-cst.qnty-ext_expence format "->>>>>>>9.<<<"     COLUMN-LABEL "     10" space(0)
sym11 space(0) vardes                  format "x(28)"            COLUMN-LABEL "     11    :         12      " space(0)
sym12 space(0) tt-cst.qnty-rest        format "->>>>>>>9.<<<"     column-label "     13" space(0)
sym13 space(0) tt-cst.ps               format "x(10)"            column-label "    14" space(0)
sym14 space(0)
with width 200 down stream-io use-text  NO-BOX .
define frame zero-frm
sym01 space(0) for-ii                format ">>>>9"            COLUMN-LABEL "  1  "  space(0)
sym02 space(0) tt-cst-year.cst-code  format "x(31)"            COLUMN-LABEL "          2          "  space(0)
sym03 space(0)
with width 200 down stream-io use-text  NO-BOX .
...
assign
v-header-name = "Отчет о товарах, помеченных под таможенный режим магазина беспошлинной торговли" + chr(10) + "и реализованных в магазине беспошлинной торговли ЗАО 'Порт-Альянc' лицензия от 06.09.99 № 52" + chr(10) + "по состоянию на " + string(x-Date-End + 1, "99/99/9999") + " за период с " + string(x-date-start, "99/99/9999") + " по " + string(x-date-end, "99/99/9999")
v-print-time  = cur-time-string().
put stream PrnLibStream unformatted
"Отчет о товарах, помеченных под таможенный режим магазина беспошлинной торговли" skip(1)
"и реализованных в магазине беспошлинной торговли ЗАО 'Порт-Альянc' лицензия от 06.09.99 № 52" skip(1)
"по состоянию на " + string(x-Date-End + 1, "99/99/9999") + " за период с " + string(x-date-start, "99/99/9999") + " по " + string(x-date-end, "99/99/9999") skip(1)
...
v-line2 at 1
with frame topframe
width 200 page-top no-labels no-box .
view stream PrnLibStream frame topframe .
form header
v-line skip
"ГТД  за " varyear-cst "год "
"Стр." at 180 string( page-number(PrnLibStream), ">>>9" )
"Продолжение на следующей странице " at 30 skip
with frame bottomframe
width 200 page-bottom no-labels no-box .
...
for each tt-cst-year No-LOCK WHERE
  ...
  if first-of(tt-cst-year.in-year) then do:
    ...
    "ГТД" @ for-ii
    "за"  @ tt-cst.cst-code
    (string(tt-cst-year.in-year) + chr(32) + "год") @ tt-cst.name_artic_unit
    with frame doc-line-frm.
  ...
  DISPLAY stream PrnLibStream
  sym01 sym02 sym03 sym04 sym05 sym06 sym07 sym08 sym09 sym10 sym11 sym12 sym13 sym14
  for-ii
  "--//--" @ tt-cst.cst-code
  tt-cst.name_artic_unit
  tt-cst.nationality
  tt-cst.tnved
  string(tt-cst.num-place, "->>>>>>>9.<<<") + chr(32) + string(tt-cst.brutto, ">>>>9.999")
  ...
  (if avail tt-cst-ext
   then (string(tt-cst-ext.qnty, "->>>>>>>9.<<<") + chr(32) + tt-cst-ext.des)
   else "") @ vardes
  tt-cst.qnty-rest
  tt-cst.PS
  with frame doc-line-frm.
...
assign
v-header-name = "                Список ГТД, которые исключены из Отчета, как ГТД," + chr(10) + " по которым остаток по всем наименованиям товаров, помещенных под таможенный " + chr(10) + " режим МБТ и реализованных в магазине беспошлинной торговли ЗАО 'Порт-Альянс'"
.
put stream PrnLibStream unformatted
"                Список ГТД, которые исключены из Отчета, как ГТД," skip(1)
" по которым остаток по всем наименованиям товаров, помещенных под таможенный " skip(1)
" режим МБТ и реализованных в магазине беспошлинной торговли ЗАО 'Порт-Альянс'" skip(1)
"лицензия от 06.09.99 N 52 по состоянию на " + string(X-date-end + 1, "99/99/9999") + " равен 0" skip(1)
SPACE(50) v-print-time skip(1).
put stream PrnLibStream unformatted
v-line skip
":     :                     :" skip
...
FOR EACH tt-cst-year where
  ...
  IF first-of(tt-cst-year.in-year) then do:
    ...
    DISPLAY stream PrnLibStream
    "ГТД" @ for-ii
    ("за " + string(tt-cst-year.in-year, "9999") + chr(32) + "год") @ tt-cst-year.cst-code
    with frame zero-frm.
    DOWN 2 stream PrnLibStream
    with FRAME zero-frm.
  end.
