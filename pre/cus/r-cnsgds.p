...
field brutto-qnty like ub.rvs-line.state-brutto-qnty
field free-qnty   like ub.rvs-line.state-brutto-qnty
field temp-sale   as decimal
     ...
     if first-of(ub.clients.obj-code) then do:
       old-l-ord-code = "".
            if Make-Excel then  put   stream ForExcel unformatted
              format-excel-text(ub.clients.obj-type + " " + string(ub.clients.obj-code))  CHR(9)
              format-excel-text(ub.clients.obj-name) chr(10)
            .
         for each  bf_ord-cons-gds no-lock  where bf_ord-cons-gds.cons-code = bf_ord-cons.cons-code,    first ub.goods where bf_ord-cons-gds.artic = ub.goods.artic                                           and bf_ord-cons-gds.prod-type = ub.goods.prod-type                               and bf_ord-cons-gds.prod-code = ub.goods.prod-code no-lock :
            ...
            for each   temp-tt where temp-tt.obj-type = ub.clients.obj-type + " " + string(ub.clients.obj-code) and
                    ...
                    if first (temp-tt.gds-code) then do:
                        if Make-Excel then  put   stream ForExcel unformatted ub.goods.artic CHR(9)
                    ...
                    if Make-Excel then  put   stream ForExcel unformatted
                     ...
                     if Make-Excel then  put   stream ForExcel unformatted
                      excel-qnty-null(temp-tt.qnty-of  )   CHR(9)
                      format-excel-text(temp-tt.time-of)   CHR(9)
                      excel-qnty-null (temp-tt.qnty-fp  )  CHR(9)
                      format-excel-text(temp-tt.cli-cod)   CHR(9)
                      format-excel-text(temp-tt.cli-name)  CHR(9)
                      format-excel-text(temp-tt.time-rcv)  CHR(9)
                      excel-qnty-null(temp-tt.qnty-rcv)    chr(10)
                    .
...
if Make-Excel then  put   stream ForExcel unformatted  "ИТОГО"    CHR(9)
                        CHR(9)
                        .
          assign
              s-max-qnty    = 0
...
assign
 ...
 If l-nn = 1 then Do:
    run calc-var(input ub.clients.obj-type,
                 ...
                 output l-temp-sale).
      assign
          temp-tt.max-qnty    = l-max
          temp-tt.brutto-qnty = l-ost
          temp-tt.free-qnty   = temp-tt.max-qnty  - temp-tt.brutto-qnty
          temp-tt.temp-sale   = l-temp-sale
...
if Make-Excel then  put   stream ForExcel unformatted  "Итого по объекту"    CHR(9)
             ub.clients.obj-name      CHR(9)
                        .
          assign
              m-max-qnty    = 0
              m-brutto-qnty = 0
...
if Make-Excel then  put   stream ForExcel unformatted                                     CHR(9)
             "Итого по :" + ub.goods.gds-name      CHR(9)
                        .
          assign
              m-max-qnty    = 0
              m-brutto-qnty = 0
