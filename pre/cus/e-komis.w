...
PROCEDURE adm-disable :
    DISABLE RECT-parts RECT-purpose RECT-columns RECT-supp RECT-by RS-purpose T-prod T-price-without-tax-cost T-sum-without-tax-cost button-supp T-sum-VAT-cost E-supp T-price-without-tax-sale T-sum-without-tax-sale T-sum-VAT-sale RS-By T-sum-SLT-sale T-sum-with-tax-sale T-parts WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-parts RECT-purpose RECT-columns RECT-supp RECT-by RS-purpose T-prod T-price-without-tax-cost T-sum-without-tax-cost button-supp T-sum-VAT-cost E-supp T-price-without-tax-sale T-sum-without-tax-sale T-sum-VAT-sale RS-By T-sum-SLT-sale T-sum-with-tax-sale T-parts WITH FRAME F-Main.
...
ON CHOOSE OF button-supp IN FRAME F-Main
DO:
  CASE rs-purpose:
    when  1 then do:
...
DO:
    ASSIGN T-parts.
END.
ON VALUE-CHANGED OF T-price-without-slt-sale IN FRAME F-Main
DO:
    ASSIGN T-price-without-slt-sale.
END.
ON VALUE-CHANGED OF T-price-without-tax-cost IN FRAME F-Main
DO:
    ASSIGN T-price-without-tax-cost.
END.
ON VALUE-CHANGED OF T-price-without-tax-sale IN FRAME F-Main
DO:
    ASSIGN T-price-without-tax-sale.
END.
ON VALUE-CHANGED OF T-prod IN FRAME F-Main
DO:
  ASSIGN T-prod.
END.
ON VALUE-CHANGED OF T-sum-SLT-sale IN FRAME F-Main
DO:
    ASSIGN T-sum-SLT-sale.
END.
ON VALUE-CHANGED OF T-sum-VAT-cost IN FRAME F-Main
DO:
    ASSIGN T-sum-VAT-cost.
END.
ON VALUE-CHANGED OF T-sum-VAT-sale IN FRAME F-Main
DO:
    ASSIGN T-sum-VAT-sale.
END.
ON VALUE-CHANGED OF T-sum-with-tax-cost IN FRAME F-Main
DO:
    ASSIGN T-sum-with-tax-cost.
END.
ON VALUE-CHANGED OF T-sum-with-tax-sale IN FRAME F-Main
DO:
    ASSIGN T-sum-with-tax-sale.
END.
ON VALUE-CHANGED OF T-sum-without-tax-cost IN FRAME F-Main
DO:
    ASSIGN T-sum-without-tax-cost.
END.
ON VALUE-CHANGED OF T-sum-without-tax-sale IN FRAME F-Main
DO:
    ASSIGN T-sum-without-tax-sale.
END.
...
For each cli-list no-lock:
 Assign
 STR-supp-name = STR-supp-name + cli-list.obj-name + chr(10)
 STR-supp = STR-supp +  cli-list.obj-type + '#' + string(cli-list.obj-code)  + ',' .
End.
END PROCEDURE.
PROCEDURE local-initialize :
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .
  APPLY "VALUE-CHANGED" to RS-purpose in frame F-Main.
END PROCEDURE.
PROCEDURE My-report :
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostname in g#library
  ...
  ,output v-host-name
    ...
    ReportHeader = "":U
    ReportName = "Отчет"
    str1 =  "о продаже товаров принятых на комиссию" + chr(32) +
            v-host-name + chr(32) +
            "от фирмы" + chr(32) +
    ...
    str2 =    "за период с" + chr(32) + string(X-date-start, "99/99/9999") + chr(32)  +
              "по" + chr(32) + string(X-date-end, "99/99/9999") + chr(32) +
...
frame F-Main RS-by
frame F-Main RS-purpose
frame F-Main T-price-without-slt-sale
frame F-Main T-price-without-tax-cost
frame F-Main T-price-without-tax-sale
frame F-Main T-sum-SLT-sale
frame F-Main T-sum-VAT-cost
frame F-Main T-sum-VAT-sale
frame F-Main T-sum-without-tax-cost
frame F-Main T-sum-without-tax-sale
frame F-Main T-sum-with-tax-cost
frame F-Main T-sum-with-tax-sale
frame F-Main T-prod
.
Assign
 STR-obj-type = ''
