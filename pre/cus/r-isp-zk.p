   ...
   if v-button-stop then  DO:
        ...
        for each buf_ord-doc-rcv no-lock where buf_ord-doc-rcv.doc-code  = buf_ord-doc.doc-code :
          if p-post-2 = 2 then do:
            ...
            if not available buf_trn-doc then next TT .
            if  p-otkl = 4 or p-otkl = 5 then do:
        ...
        assign
          ...
          StatOtkl = 0
        .
        if p-otkl = 1 or p-otkl = 4 or p-otkl = 5 then  assign StatOtkl = 1 .
        for each Temp-i no-lock :
          assign NumPost = NumPost + 1 .
          case p-otkl :
            when 2 then do:
              if ABSOLUTE((( v-val-post - Temp-i.i-val-nakl ) * 100 / v-val-post )) > p-proc then assign StatOtkl = 1 .
            end.
            when 3 then do:
              if ABSOLUTE(((v-cost-post - Temp-i.i-cost-nakl) * 100 / v-cost-post )) > p-proc then assign StatOtkl = 1 .
            end.
            when 5 then do:
              if ABSOLUTE((( v-val-post - Temp-i.i-val-nakl ) * 100 / v-val-post )) > p-proc then assign StatOtkl = 1 .
              if ABSOLUTE((( v-cost-post - Temp-i.i-cost-nakl ) * 100 / v-cost-post )) > p-proc then assign StatOtkl = 1 .
 ...
 .
       ...
       if available ub.ext-artic then do:
        ...
        if PrnObj = no then do:
          assign
            PrnObj = yes
            v-goods-name = string( " Объект: " + obj-list.obj-name)
          .
          if p-post-2 = 2 then do:
