...
for each kass where
    ...
    if first-of ( kass.pay-desk ) then do:
        put stream outstream unformatted
    ...
    run macr_excel_dec  (        kass.sum-rubl-ost , num#str# , 2   ) . assign    num#col# = 3 .
        for each prsn where prsn.pay-desk = kass.pay-desk and
            ...
            if first-of(prsn.cashier) then do:
              put stream outstream unformatted
                  sym1                          format "x(1)" space(0)
                ...
                for each tt-temp where
                        ...
                        :
                          case tt-temp.doc-type :
                              when string('3':U)  then do:
                                v-1 = v-1 + tt-temp.doc-code + "," .
                              end.
                              when string('2':U)  then do:
                                v-2 = v-2  + tt-temp.doc-code + "," .
                              end.
                              when string('4':U)  then do:
  ...
  End.
    ...
    run macr_excel_char ("Перевод оплаты"  , num#str# , num#col#   ) . run macr_cell_size ( 15 , ? , num#str# , num#col# , ?, ? ) .
     put  stream macr_excel unformatted
       substitute('select("r&1c&2:r&3c&4 ")' , num#str# , 4 , num#str# ,  5 ) + chr(10)  +
       'BORDER(2,,,,,,0,0,0,0,0) '  + chr(10) +
       'ALIGNMENT(7 , , 4 , 4 ,)'  + chr(10)
       .
     put  stream macr_excel unformatted
       substitute('select("r&1c&2:r&3c&4 ")' , num#str# , 6 , num#str# ,  7 ) + chr(10)  +
       'BORDER(2,,,,,,0,0,0,0,0) '  + chr(10) +
       'ALIGNMENT(7 , , 4 , 4 ,)'  + chr(10)
       .
     put  stream macr_excel unformatted
       substitute('select("r&1c&2:r&3c&4 ")' , num#str# , 8 , num#str# ,  11 ) + chr(10)  +
       'BORDER(2,,,,,,0,0,0,0,0) '  + chr(10) +
       'ALIGNMENT(7 , , 4 , 4 ,)'  + chr(10)
       .
    put  stream macr_excel unformatted
...
find first buf_psn-clients no-lock  where
           ...
           buf_psn-clients.obj-type = 'чел':U no-error .
          if error-status :error then
          fio =  "Кассир № " + string( buf_chk-doc.cashier ).
          else fio  = buf_psn-clients.obj-name .
            jj = jj + 1.
IF ( JJ modulo v-account = 0 )  then DO:
 Assign
    v-kol-spice = (50 - LENGTH(obj-list.obj-name)) / 2
    RecordsString = fill(' ',v-kol-spice) + string(obj-list.obj-name)
    .
          if  log-manager:logfile-name ne ?
          then DO:
...
procedure sum-doc :
  ...
  define buffer buf_chk-pay for ub.chk-pay  .
   ...
   p-aft-vid = "" .
   p-sum-rubl = 0 .
   IF p-doc-type <> '4':U THEN DO:
      p-sum-rubl = 0 .
      for each buf2_chk-pay no-lock where buf2_chk-pay.doc-code = p-doc-code  :
               p-sum-rubl = p-sum-rubl + buf2_chk-pay.tot-sum * buf2_chk-pay.bank-rate / buf2_chk-pay.bank-scale .
      end.
   END.
   ELSE DO:
      find first  buf2_chk-pay no-lock where buf2_chk-pay.doc-code = p-doc-code   no-error .
