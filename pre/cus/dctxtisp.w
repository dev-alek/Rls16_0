...
CASE p-mode:
  when 'shd':U then do:
      ...
      FIND FIRST buf_schedule-attr NO-LOCK WHERE
                 buf_schedule-attr.cre-db-num = p-cre-db-num
             and buf_schedule-attr.task-type  = p-task-type
             and buf_schedule-attr.attr-code = ('schd-free-id':U + chr(4) + 'dctxt-i') NO-ERROR.
  ...
  CASE p-mode:
    when 'shd':U then do:
      if p-task-num > 0 then do:
        ...
        for each buf_schedule no-lock where
            ...
            first buf_schedule-attr no-lock where
              ...
              AND buf_schedule-attr.attr-code = ('schd-free-id':U + chr(4) + 'dctxt-i') :
      ...
      if v-param-list = '':U THEN DO:
         v-param-list = string(12) + chr(4) + 'ini' + chr(4) + "f?.txt" + chr(4) + chr(4) + chr(4) +
                chr(4) + chr(4) + string(TODAY) + chr(4) + chr(63).
      END.
    END.
    WHEN 'run' THEN DO:
      assign
...
CASE p-mode:
    WHEN 'run':U THEN DO:
        ...
        rs-dir:RADIO-BUTTONS IN FRAME Dialog-Frame
                             = "берется из Ini-файла ([schedule-free] dctxt-i_in=)" + chr(44) +
                               "ini":U + chr(44) +
                               "задать директорию импорта" + chr(44) +
                               "other".
    END.
    WHEN 'shd':U THEN DO:
        ASSIGN
        rs-dir:RADIO-BUTTONS = "берется из Ini-файла ([schedule-free] dctxt-i_in=)" + chr(44) +
                               "ini":U + chr(44) +
...
END.
RUN verify-file in this-procedure
                                  ( input (trim(v-dir-name, chr(92)) + '\save\')
...
if error-status:error or not glog then return error return-value .
assign
file-info :file-name = (trim(v-dir-name, chr(92)) + '\save\')
.
if index( file-info:file-type, "W" ) = 0  then do:
  message
  'Вы не имеете права писать в директорию для сохранения копий обработанных файлов: '
  trim(v-dir-name, chr(92)) + '\save\' view-as alert-box error.
  return error.
end.
v-dop-file-name = replace(replace(file-name-1, chr(63), '':U), "*", '':U).
do ii = 1 to length('\/:*?"<>|':U):
...
v-param-list = (string(v-rule-profile-id) + chr(4) +
                ...
                file-name-1 + chr(4) +
                string(X_dis-card-type.emitent-host-code) + chr(4) +
                X_dis-card-type.type + chr(4) +
                v-other-param
.
