...
if p-par = "'8':U"   then  v-shap = "Сумма аннуляции" .
if p-par = "'6':U" then v-shap = "Сумма возврата" .
...
FORM with frame plan-menu .
    ...
    v-ind = v-ind + 1.
  find ub.clients      where ub.clients.obj-type     = 'орг':U            and ub.clients.obj-code      = parhost-code no-lock .
...
find first buf_sale-clients no-lock  where
           ...
           buf_sale-clients.obj-type = 'чел':U no-error .
v-vid = "" .
 ...
 if first-of (buf_chk-pay.curr-code) then do:
    find first ub.cash-pay no-lock where
       ...
       if available ub.cash-pay then
          v-vid = v-vid +  ub.cash-pay.obj-name +  ","  .
 end.
end.
v-vid =  trim (v-vid,",") .
