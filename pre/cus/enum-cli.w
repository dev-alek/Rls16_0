...
DO:
  ...
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
ON CHOOSE OF b-sconf IN FRAME Dialog-Frame
DO:
...
assign
  v-str-recid14 = trim( string( recid( X_ext-classif ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry14 > 0 then do:
  ...
  .
    ...
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        ...
        apply "VALUE-CHANGED" to br-enum-cli in frame Dialog-Frame.
    end.
  ...
  apply "entry" to br-enum-cli in frame Dialog-Frame.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-enum-cli :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-enum-cli FOR EACH X_ext-classif NO-LOCK,        FIRST X_clients NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
ENABLE
b-quit
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      ...
      parameter-4-27 =
        (
          if (" X_ext-classif.classif-subject = 'clients':U                           and X_ext-classif.classif-name = 'code_firm_ext':U                           AND X_ext-classif.db-num = - 1" + " " + where-phrase-27) <> ""
          then  substitute('X_ext-classif.classif-subject = &1&2&1                           and X_ext-classif.classif-name = &1&3&1                           AND X_ext-classif.db-num = - 1', chr(34), 'clients':U, 'code_firm_ext':U) + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + substitute(', FIRST X_clients NO-LOCK WHERE                                   X_clients.obj-type = ENTRY(2, X_eXt-classif.uniq-key-rec, chr(3)) AND                                   X_clients.obj-code = integer(ENTRY(3, X_eXt-classif.uniq-key-rec, chr(3)))'))
    ...
    :
      assign
        l-disable-where-27 =
          (" X_ext-classif.classif-subject = 'clients':U                           and X_ext-classif.classif-name = 'code_firm_ext':U                           AND X_ext-classif.db-num = - 1" + " " + where-phrase-27 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-enum-cli:handle
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY br-enum-cli FOR EACH X_ext-classif
      where  X_ext-classif.classif-subject = 'clients':U                           and X_ext-classif.classif-name = 'code_firm_ext':U                           AND X_ext-classif.db-num = - 1
    , FIRST X_clients NO-LOCK WHERE                                   X_clients.obj-type = ENTRY(2, X_eXt-classif.uniq-key-rec, chr(3)) AND                                   X_clients.obj-code = integer(ENTRY(3, X_eXt-classif.uniq-key-rec, chr(3)))
      INDEXED-REPOSITION
  .
      assign
        l-query-was-opened = true
...
APPLY "entry" TO br-enum-cli.
if available X_ext-classif then do:
    APPLY "VALUE-CHANGED":U to br-enum-cli.
END.
END PROCEDURE.
