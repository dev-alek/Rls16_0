...
put  stream macr_excel unformatted
     substitute('patterns(1,,&1,true)', 42 ) + chr(10)  .
run head-post in this-procedure .
put  stream macr_excel unformatted
     substitute('select("r&1c&2:r&3c&4 ")' , num#str#   , 1 , num#str# ,  max-col ) + chr(10)  +
     'BORDER( 2 , 2 , 2 , 2 , 2 , ,0,0,0,0,0) '  + chr(10) +
     'ALIGNMENT(3 , , 4 , 4 ,)'  + chr(10)     .
    put  stream macr_excel unformatted 'Split(,4)' + chr(10)     .
    put  stream macr_excel unformatted 'FREEZE.PANES(true,,true)'+ chr(10)     .
num#col# =  0.
...
if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run goassizt in g#library2
  ...
  ,input  export-ras.obj-type
  ,input  export-ras.obj-code
  ,input  false
  ,output v-ok
  ,output v-mess
  ) no-error.
...
if v-erase = true then do:
    run view-exept-gds ( substitute("Не все товары попали в заказ !&1Просмотреть товары, не вошедшие в заказ ?", chr(10))) .
end.
if p-val-gds-obj then do:
    for each  export-ras ,
        ...
        assign
          v-stroka = num#str#
        .
        if g#type = 'ОФ':U then run spis-post in this-procedure .
        ...
        if g#type <> 'ФП':U or kol-obj = 1 then
        run spis-post in this-procedure .
        if last-of(export-ras.gds-code) and g#type = 'ФП':U and kol-obj > 1  then do:
                num#str# = num#str# + 1 .
    ...
    for each  export-ras ,
        ...
        num#col# = new-n(34). run macr_excel_char_with_format in this-procedure ( v-b-str , num#str# , num#col#  ).
        if g#type = 'ОФ':U and p-det-post then run spis-post in this-procedure .
        for each tmp#zakaz-prn where tmp#zakaz-prn.artic     = export-ras.artic     and
                                     tmp#zakaz-prn.prod-type = export-ras.prod-type and
                                     tmp#zakaz-prn.prod-code = export-ras.prod-code and
                                     tmp#zakaz-prn.obj-type  = export-ras.obj-type  and
        ...
        if last-of(export-ras.obj-code) and g#type = 'ФП':U and kol-obj > 1 then do:
                num#str# = num#str# + 1 .
...
find first ubflt.usr-flt  no-lock where
         ...
         if available ubflt.usr-flt then do :
            DO v-ii = 1 TO NUM-ENTRIES( ubflt.usr-flt.list_, chr(4)):
              CASE v-ii:
                  WHEN 1 THEN DO:
                    p-val-rad     = logical(entry(v-ii, ubflt.usr-flt.list_, chr(4))) .
                  END.
                  WHEN 2 THEN DO:
                    p-val-gds-obj = logical(entry(v-ii, ubflt.usr-flt.list_, chr(4))) .
                  END.
                  WHEN 3 THEN DO:
                    p-det-post    = logical(entry(v-ii, ubflt.usr-flt.list_, chr(4))) .
                  END.
                  WHEN 4 THEN DO:
                    p-only-am     = logical(entry(v-ii, ubflt.usr-flt.list_, chr(4))) .
                  END.
                  WHEN 5 THEN DO:
                    tog-det-prizn = logical(entry(v-ii, ubflt.usr-flt.list_, chr(4))) .
                  END.
              end case.
            end.
         end.
...
if p-val-col <> "" then do:
  assign
    print-col-1  = if p-val-rad then yes else logical(entry( 1 , p-val-col, chr(4)))
    print-col-2  = if p-val-rad then yes else logical(entry( 2 , p-val-col, chr(4)))
    print-col-3  = if p-val-rad then yes else logical(entry( 3 , p-val-col, chr(4)))
    print-col-4  = if p-val-rad then yes else logical(entry( 4 , p-val-col, chr(4)))
    print-col-5  = if p-val-rad then yes else logical(entry( 5 , p-val-col, chr(4)))
    print-col-6  = if p-val-rad then yes else logical(entry( 6 , p-val-col, chr(4)))
    print-col-7  = if p-val-rad then yes else logical(entry( 7 , p-val-col, chr(4)))
    print-col-8  = if p-val-rad then yes else logical(entry( 8 , p-val-col, chr(4)))
    print-col-9  = if p-val-rad then yes else logical(entry( 9 , p-val-col, chr(4)))
    print-col-10 = if p-val-rad then yes else logical(entry( 10, p-val-col, chr(4)))
    print-col-11 = if p-val-rad then yes else logical(entry( 11, p-val-col, chr(4)))
    print-col-12 = if p-val-rad then yes else logical(entry( 12, p-val-col, chr(4)))
    print-col-13 = if p-val-rad then yes else logical(entry( 13, p-val-col, chr(4)))
    print-col-14 = if p-val-rad then yes else logical(entry( 14, p-val-col, chr(4)))
    print-col-15 = if p-val-rad then yes else logical(entry( 15, p-val-col, chr(4)))
    print-col-16 = if p-val-rad then yes else logical(entry( 16, p-val-col, chr(4)))
    print-col-17 = if p-val-rad then yes else logical(entry( 17, p-val-col, chr(4)))
    print-col-18 = if p-val-rad then yes else logical(entry( 18, p-val-col, chr(4)))
    print-col-19 = if p-val-rad then yes else logical(entry( 19, p-val-col, chr(4)))
    print-col-20 = if p-val-rad then yes else logical(entry( 20, p-val-col, chr(4)))
    print-col-21 = if p-val-rad then yes else logical(entry( 21, p-val-col, chr(4)))
    print-col-22 = if p-val-rad then yes else logical(entry( 22, p-val-col, chr(4)))
    print-col-23 = if p-val-rad then yes else logical(entry( 23, p-val-col, chr(4)))
    print-col-24 = if p-val-rad then yes else logical(entry( 24, p-val-col, chr(4)))
    print-col-25 = if p-val-rad then yes else logical(entry( 25, p-val-col, chr(4)))
    print-col-26 = if p-val-rad then yes else logical(entry( 26, p-val-col, chr(4)))
    print-col-27 = if p-val-rad then yes else logical(entry( 27, p-val-col, chr(4)))
    print-col-28 = if p-val-rad then yes else logical(entry( 28, p-val-col, chr(4)))
    print-col-29 = if p-val-rad then yes else logical(entry( 29, p-val-col, chr(4)))
    print-col-30 = if p-val-rad then yes else logical(entry( 30, p-val-col, chr(4)))
    print-col-31 = if p-val-rad then yes else logical(entry( 31, p-val-col, chr(4)))
    print-col-32 = if p-val-rad then yes else logical(entry( 32, p-val-col, chr(4)))
    print-col-33 = if p-val-rad then yes else logical(entry( 33, p-val-col, chr(4)))
    print-col-34 = if p-val-rad then yes else logical(entry( 34, p-val-col, chr(4)))
  no-error .
end.
else do:
  if p-val-rad then do:
  assign
      print-col-1  = yes
      print-col-2  = yes
      print-col-3  = yes
...
i-kol = num-entries (v-uf-List_, chr(4)) .
    repeat Iv = 1 to i-kol :
      st = entry(Iv,v-uf-List_, chr(4)).
      create tt-table.
      assign
...
if p-val-col <> "" then do:
  repeat i = 1 to num-entries(p-val-col, chr(4)) :
     if entry( i , p-val-col, chr(4) ) = "yes" then do :
...
v-nn = num-entries ( e-method ,"chr(10)")  .
    repeat iii = 1 to v-nn :
        pp-str = entry (iii, e-method , "chr(10)").
...
if G#type = 'ОФ':U then do:
  ...
  num#col# = new-n(28). run macr_excel_char in this-procedure ('орг':U                  , num#str# , num#col#  ).
...
if G#type <> 'ОФ':U then do:
    ...
    if not print-col-31 then put  stream macr_excel unformatted  'COLUMN.WIDTH(0,,,,)'  skip.
        put  stream macr_excel unformatted
            substitute('patterns(1,,&1,true)', 3 ) + chr(10)  .
        put  stream macr_excel unformatted
            substitute('format.font(,,,,,,&1)' , 2 ) + chr(10) .
end.
if G#type = 'ОФ':U then do:
