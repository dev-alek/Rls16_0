...
if session :set-wait-state( "compiler" ) then.
  ...
  if sort-grp then do:
    for each temp-goods break by temp-goods.grp-name by if SortType = 1 then  temp-goods.b-code Else (if SortType = 2 then  temp-goods.artic Else  temp-goods.gds-name) :
    ...
    for each temp-goods break by if SortType = 1 then  temp-goods.b-code Else (if SortType = 2 then  temp-goods.artic Else  temp-goods.gds-name) :
      run PrintLine in this-procedure .
    end.
  end.
      ...
      for each temp-obj :
        find last buf_doc-line no-lock
          where buf_doc-line.artic      = temp-goods.artic
            ...
            and (buf_doc-line.ext-doc-type = 'ie':U or buf_doc-line.ext-doc-type = 'iv':U )
          no-error .
