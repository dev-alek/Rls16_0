...
PROCEDURE enable_UI :
  ...
  v-log =  session:SET-WAIT-STATE("GENERAL") . OPEN QUERY br-docs FOR EACH dd_ord-line NO-LOCK   ,              first ub.ord-line NO-LOCK where ub.ord-line.doc-code = p-doc-code and                                 ub.ord-line.artic     = dd_ord-line.artic and                                 ub.ord-line.prod-type = dd_ord-line.prod-type and                                 ub.ord-line.prod-code = dd_ord-line.prod-code   ,             first ub.goods no-lock where                         ub.ord-line.artic     = ub.goods.artic and                         ub.ord-line.prod-type = ub.goods.prod-type and                         ub.ord-line.prod-code = ub.goods.prod-code   ,               first ub.ord-doc OF dd_ord-line NO-LOCK       WHERE ( ub.ord-doc.status_ = 'закрыто':U              OR              ub.ord-doc.status_ = 'поставка':U )              and              ub.ord-doc.doc-type <> 'ПО':U AND              (              if p-type = 1 then                ( ub.ord-doc.ship-date <=  p-LOC-DATE-SHIP )                else                ( ub.ord-doc.DATE-sale-1 <= p-date-sale-2 and                  ub.ord-doc.DATE-sale-2 >= p-date-sale-1)                  )                  and      ( if p-doc-type = 'ФП':U then        (ub.ord-doc.host-code = v-cntxt-host-code-obj)        else        (ub.ord-doc.obj-code = p-obj-code  and         ub.ord-doc.obj-type = p-obj-type )      ) .      v-log =  session:SET-WAIT-STATE("") .
END PROCEDURE.
