...
PROCEDURE adm-disable :
    DISABLE RECT-client selectcli t-zero WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-client selectcli t-zero WITH FRAME F-Main.
...
DO:
 ...
 END CASE.
 display selectcli with frame F-Main.
END.
...
dcards.sum column-label "Реализация, руб" format "->,>>>,>>>,>>9.99"
...
"Наименование организации" + chr(44) +
"Реализация руб"
sheetf.sizes =
"105" + chr(44) +
"16"
...
ReportHeader = SUBSTITUTE("&1", IF t-zero = NO THEN "Нулевые обороты не выводятся" ELSE '':U) + chr(10) +
    "Клиенты: " +
                           radio-label(string(selectcli), selectcli:radio-buttons) + chr(10)
                 .
END PROCEDURE.
