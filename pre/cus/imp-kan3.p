...
def var vss-include-info77 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
procedure check-use-bar-code :
  define input  parameter p-b-code    like ub.bar-code.b-code no-undo .
  do
  on error  undo, return error substitute( "&1. &2&3&4", vss-include-info77, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
  on stop   undo, return error substitute( "&1. stop", vss-include-info77 )
  on endkey undo, return error substitute( "&1. endkey", vss-include-info77 )
  :
    define buffer buf_bar-code for ub.bar-code .
    find first buf_bar-code no-lock
    ...
    if not available buf_bar-code then do:
      return error substitute( "&1 (check-use-bar-code). Не найден бар-код &2", vss-include-info77, p-b-code ) .
    end.
...
if buf_price-doc-forming.stts <> int('0':U) then do:
  message
...
find first  buf_price-list-type no-lock where
            buf_price-list-type.plt-id     = buf_price-doc-forming.plt-id and
            buf_price-list-type.plt-db-num = buf_price-doc-forming.plt-db-num no-error .
            if error-status :error or buf_price-list-type.stts = int('1':U)  then do:
              message
