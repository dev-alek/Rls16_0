...
on error undo, return error
:
    if num-entries(p-parameter, chr(4) ) <> 9 then do:
    ...
    p-rfile-name = entry(5, p-parameter, chr(4))
    p-lfile-name = entry(6, p-parameter, chr(4))
    p-del-after-get = logical(entry(7, p-parameter, chr(4)))
    v-cb-proc = entry(8, p-parameter, chr(4))
    log-file-name = entry(9, p-parameter, chr(4))
    .
    if p-rfile-name = '' then do:
      if v-cb-proc = ''
      or lookup(v-cb-proc, p-parent-handle:internal-entries) = 0 then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 &2 &3 Неверно заданы параметры&4Файл для скачивания не задан и cb-процедура для скачивания тож не определена"                                     ,vss-workfile                                      ,vss-revision                                      ,vss-description                                     ,chr(10))).
        undo, return error ''.
      end.
    end.
      ...
      then do:
          ...
          if p-rfile-name = '' then do:
            ...
            or v-rfile-name = '' then do:
              leave _do.
                  ...
                  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Получаю файл &1 (в &2)... ", v-rfile-name, v-lfile-name)).
        if not FtpGetFile( input hFTPsession
          ...
          if p-del-after-get then do:
                          run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Удаляю файл &1... ", v-rfile-name)).
