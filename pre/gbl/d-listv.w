...
DO:
  if rid-list = "" then  do:
    ...
    if not b-mark:sensitive in frame Dialog-Frame then do:
      rid-list = string( recid( temp-sel ) ) .
    end.
  end.
  Run fill-parameter(input pdelim,
                     ...
                     output psel-values) .
END.
ON CHOOSE OF B-value IN FRAME Dialog-Frame
DO:
define variable vvalue as character no-undo.
define buffer buf_temp-sel for temp-sel.
        run gbl/d-prompt.w (
        'title=':u + temp-sel.attr-label + '\':u
      + 'format=' + (if temp-sel.attr-format = 'L':U then "yes/no" else temp-sel.attr-format) + '\':u
...
.
  OPEN QUERY BR-list FOR EACH temp-sel NO-LOCK.
END.
...
If lookup(temp-sel.attr-code, ppresel-codes) > 0 then do:
  rid-list = rid-list + (if rid-list = "":U then "":U else chr(44) ) + string(recid(temp-sel)).
end.
end.
END PROCEDURE.
PROCEDURE MyEnable :
...
BR-list
WITH FRAME Dialog-Frame.
frame Dialog-Frame:title = ptitle.
  VIEW FRAME Dialog-Frame.
  OPEN QUERY BR-list FOR EACH temp-sel NO-LOCK.
...
assign
  v-str-recid4 = trim( string( recid( temp-sel ) , "->>>>>>>>>>>9":U ) )
  v-num-entry4 = lookup( v-str-recid4 , rid-list )
.
if v-num-entry4 > 0 then do:
  assign
    entry( v-num-entry4, rid-list ) = "":U
    rid-list = trim( replace( rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    rid-list = rid-list + ( if rid-list = "":U then "":U else chr(44) ) + v-str-recid4
  .
end.
    loc#log = br-list:refresh() in frame Dialog-Frame.
    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-List:select-next-row ().
        apply "iteration-changed" to br-List in frame Dialog-Frame.
    end.
