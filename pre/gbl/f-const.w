...
procedure progs-name :
  ...
  :
    case p-action-code :
            ...
            otherwise do:
        return error substitute( "&1. Неизвестный тип операции: &2", vss-include-info4, p-action-code ).
      end.
    end case.
  end.
  return.
 ...
 define output parameter p-rec-string  as character no-undo.
  ...
  :
    ...
    if p-key-rec = ?
      or p-key-rec = "":U
    then do:
      return error substitute( "&1 (get-row-keyr-string). Ошибка задания входных параметров. Не задан уникальный ключ.", vss-include-info4 ).
    end.
    assign
    ...
    if not available ub._file then do:
      return error substitute( "&1. Таблица &2 отсутствует в БД", vss-include-info4, v-tbl-name ).
    end.
    assign
...
DEFINE FRAME DIALOG-1
     ...
     b-help  AT ROW 1.25 COL 25   SPACE(0.03)
...
do:
  ...
  then do:
    ASSIGN
      in-date :POPUP-MENU in frame DIALOG-1 = MENU m-ed-date11 :HANDLE
      in-date :MENU-MOUSE in frame DIALOG-1 = 3
    .
  end.
  define variable v-label-handle11 as handle no-undo .
  assign
    v-label-handle11 = in-date :side-label-handle in frame DIALOG-1
  .
...
DO ON ERROR    UNDO MAIN-BLOCK, return error
  ...
  case type:
    ...
    then do:
      ...
      then do:
        assign
          str = input frame DIALOG-1 comb
          str_rus = input comb
        .
      end.
      else do:
        assign
          str = input frame DIALOG-1 in-char
    ...
    when "date"
    then do:
      if input frame DIALOG-1 in-date = "":U
      or input frame DIALOG-1 in-date = ?
      or input frame DIALOG-1 in-date = "?"
      then do:
        assign
          str = chr(63)
        ...
        if toggle-date :checked = true then do:
          ...
          assign
            v-diff-value = input frame DIALOG-1 in-date - today
          .
          if v-diff-value = 0
          then do:
            assign
          ...
          define variable v-date as date      no-undo .
          assign
            v-date = input frame DIALOG-1 in-date
          .
          if v-date = ?
          then do:
            assign
    ...
    then do:
      assign
        str = string(input frame DIALOG-1 in-dec)
    ...
    then do:
      ...
      then do:
        assign
          str = string( input frame DIALOG-1 in-int )
        .
      end.
      else do:
        if    comb:visible in frame DIALOG-1
    ...
    when "logical" then do:
      assign
        str = (input frame DIALOG-1 in-log)
...
PROCEDURE enable_UI :
  DISPLAY in-log comb in-date toggle-date in-dec in-char in-int
      WITH FRAME DIALOG-1.
  ENABLE in-log comb in-date toggle-date in-dec in-char in-int Btn_OK Btn_Cancel b-help
      WITH FRAME DIALOG-1.
END PROCEDURE.
PROCEDURE UI_on :
   run InitForm.
