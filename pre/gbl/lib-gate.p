...
if not valid-object(v-route-data_) then do:
    v-route-data_ = new Route-data_( input parparentproc, input p-parent-handle, input p-log-handle, input p-gc-handle, input this-procedure:handle) .
...
:
  if p-file-name <> '' then do:
    ...
    for each temp-gate:
      ...
      temp-gate.uniq-gate-rec chr(32)
      temp-gate.schema-name chr(32)
    ...
    for each temp-xml-tables:
      put stream outstream unformatted
      temp-xml-tables.order chr(32)
      temp-xml-tables.tbl-name chr(32)
      temp-xml-tables.gate-name chr(32)
      skip.
    end.
    if valid-object(v-route-data_) then do:
      put stream outstream unformatted
      "route-data_" chr(32) v-route-data_:name_ skip.
    end.
