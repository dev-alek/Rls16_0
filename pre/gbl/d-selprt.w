...
define temp-table temp-prt-root no-undo     field node-code  as integer label 'Код'     field prt-level  as integer label 'Уровней'     field node-name like ub.gds-prt.node-name format 'x(40)' label 'Название' column-label 'Название'     index pk is primary unique node-code     index ie1 node-name     .
...
do:
  BROWSE-1 :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1 FOR EACH temp-prt-root .
END PROCEDURE.
