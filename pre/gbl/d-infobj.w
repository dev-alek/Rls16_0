  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse b-widget-value :handle
      .
...
PROCEDURE create-attrib-by-list :
  ...
  :
    ...
    if not available widget-attributes then do:
      ...
      .
      run gbl/prgsattr.p
        (input  widget-attributes.attrib-name
        ,output widget-attributes.attrib-type
        ) .
    end.
    if can-query(h-widget, v-attr-name) then do:
      ...
      if widget-attributes.attrib-type <> "" then do:
        run get-widget-value
          (input  h-widget
          ,input  widget-attributes.attrib-name
          ,output widget-attributes.attrib-value
          ) no-error .
      end.
    end.
...
PROCEDURE init-widget-attributes :
  ...
  open query b-widget-value for each widget-attributes by attrib-name .
  if v-sel-attr <> "" then do:
    ...
    if available buf_widget-attributes then do:
      if available buf_widget-attributes then do:
        reposition b-widget-value to rowid rowid(buf_widget-attributes) .
      end.
    end.
  end.
END PROCEDURE.
