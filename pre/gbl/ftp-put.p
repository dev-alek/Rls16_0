...
on error undo, return error
:
    if num-entries(p-parameter, chr(4) ) <> 8 then do:
    ...
    p-del-after-put = logical(entry(7, p-parameter, chr(4)))
    log-file-name = entry(8, p-parameter, chr(4))
    .
    ...
    if not ConnectWinInet ( output hInternetSession  ) then do:
      ...
      then do:
                run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Передаю файл &1 (в &2)... ", p-lfile-name, p-rfile-name)).
        if not FtpPutFile( input hFTPsession
          ...
          if p-del-after-put then do:
                        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Удаляю файл &1... ", p-lfile-name)).
