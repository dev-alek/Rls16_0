...
END.
ON CHOOSE OF B-overwrite IN FRAME Dialog-Frame
...
WITH FRAME Dialog-Frame.
ASSIGN
B-overwrite:PRIVATE-DATA IN FRAME Dialog-Frame = string(1)
B-overwrite-all :PRIVATE-DATA IN FRAME Dialog-Frame = string(2)
B-skip:PRIVATE-DATA IN FRAME Dialog-Frame = string(3)
b-quit:PRIVATE-DATA IN FRAME Dialog-Frame = string(6)
B-overwrite-all-older:PRIVATE-DATA IN FRAME Dialog-Frame = string(5)
B-skip-all:PRIVATE-DATA IN FRAME Dialog-Frame = string(4)
.
define variable fh as widget-handle no-undo .
define variable hh as widget-handle no-undo .
assign
...
if hh:private-data = string(p-cancel-button) then do:
  ASSIGN
  FRAME Dialog-Frame:cancel-button = hh.
end.
