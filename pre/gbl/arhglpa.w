...
DO:
  MESSAGE i-apusharh:private-data  VIEW-AS ALERT-BOX INFORMATION.
END.
...
FOR EACH glb-thbjattr_thbj-attr:
  IF glb-thbjattr_thbj-attr.prop-code = 'apusharh':U THEN DO:
     apusharh = glb-thbjattr_thbj-attr.property-value-logical.
     apusharh:private-data in frame Dialog-Frame  = "recid=" + string(recid(glb-thbjattr_thbj-attr)).
     display apusharh with frame Dialog-Frame .
  END.
...
define variable v-found as decimal   no-undo .
  if p-mode = 'ИЗМЕНЕНИЕ':U then do:
    find first buf_thbj-attr exclusive-lock where
        ...
        and   buf_thbj-attr.upper-prop-code = 'arh-global':U
  ...
  if p-mode <> 'ИЗМЕНЕНИЕ':U then do:
     disable apusharh  with frame Dialog-Frame.
     B-exit:label = "Вы&ход"  .
