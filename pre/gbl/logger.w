...
DO:
  ...
  RETURN NO-APPLY.
END.
ON CHOOSE OF b-clear IN FRAME DEFAULT-FRAME
DO:
...
DO:
  APPLY 'CLOSE':u to this-procedure .
END.
ON CHOOSE OF b-export IN FRAME DEFAULT-FRAME
DO:
  run export-log-info in this-procedure .
END.
ON CHOOSE OF b-import IN FRAME DEFAULT-FRAME
DO:
  ...
  then do:
    ...
    :
      ...
      run process-event in p-callback
        (input  buf_temp-event.event-name
        ,input  buf_temp-event.event-revision
        ,input  v-call-point
        ,input  buf_temp-event.event-date
        ,input  buf_temp-event.event-time
        ,input  buf_temp-event.event-time-int
        ) .
    end.
...
PROCEDURE local-open-query :
  ...
  on error undo, return error return-value
  :
    open query BROWSE-2 for each temp-event .
    run display-dependent in this-procedure .
  end.
END PROCEDURE.
PROCEDURE logevent :
