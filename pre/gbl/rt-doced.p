...
define variable vss-description as character no-undo init "–адиотерминал. «арегистрировать номер документа".
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run usrfulnm in g#library
  (input  buf_batchprocess.user_id
  ,output v-user-nik
  ) no-error .
          ...
          assign
            p-status        = '3':u
            p-error-message = substitute('ƒокумент &1 редактируетс€ пользователем &2 с &3 в режием &4'
                                        ,p-unique-doc-code
                                        ,v-user-nik
      ...
      assign
        buf_batchprocess.bp_type        = 'rt-doc':U
    ...
    then do:
      ...
      then do:
        assign
          p-status        = '1':u
          p-error-message = substitute('ƒокумент &1 не редактируетс€'
                                      ,p-unique-doc-code
                                      )
        .
        return.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run usrfulnm in g#library
  ...
  ,output v-user-nik
        ...
        assign
          p-status        = '1':u
          p-error-message = substitute('ƒокумент &1 редактируетс€ пользователем &2 с &3'
                                      ,p-unique-doc-code
                                      ,v-user-nik
                                      ,string(buf_batchprocess.bp_sysdate, '99/99/9999':u)
                                      )
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run usrfulnm in g#library
  ...
  ,output v-user-nik
    ...
    otherwise do:
      undo, return error substitute("Ќеизвестное значение параметра p-action &1", p-action)
        .
    end.
  end.
end.
