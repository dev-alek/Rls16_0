...
END.
ON CHOOSE OF btn-exit IN FRAME DIALOG-1
DO:
   p-rec = ?.
...
END.
ON CHOOSE OF btn-update IN FRAME DIALOG-1
DO:
...
define variable vss-include-info5 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if num-entries(c-point, chr(4)) > 2 then do:
    c-title = entry(3, c-point, chr(4) ).
    c-point = entry(1, c-point, chr(4) ) + chr(4) +  entry(2, c-point, chr(4) ).
  end.
  else do:
    c-title = entry(1, c-point, chr(4) ).
  end.
  assign frame DIALOG-1:title = substitute("Ш А Б Л О Н Ы   П Е Ч А Т И    (&1)", c-title).
  ...
  apply "value-changed" to browser-1 in frame DIALOG-1.
  WAIT-FOR GO OF FRAME DIALOG-1 focus browser-1.
END.
RUN disable_UI.
...
PROCEDURE enable_UI :
  ...
  ENABLE Btn_Cancel Btn_OK btn-exit b-help BROWSER-1 BR-sel-fields btn-add
         btn-del btn-update
      WITH FRAME DIALOG-1.
  OPEN QUERY BROWSER-1 FOR EACH ubflt.filter       WHERE ubflt.filter.call-point = c-point NO-LOCK.
END PROCEDURE.
...
APPLY "ENTRY" to BROWSER-1 in frame DIALOG-1.
END PROCEDURE.
