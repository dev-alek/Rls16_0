...
define variable vss-description as character no-undo init "—читать строку из *.ini файла".
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3',p-ini-filename,p-section,p-key)
    .
end procedure.
...
:
  ...
  run GetPrivateProfileStringA in hpApi
    ...
    ,input  p-key
    ,input  ""
    ,input  get-pointer-value(v-key-value)
    ,input  v-key-size
    ,input  p-ini-filename
