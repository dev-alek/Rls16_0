...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2':u,p-lock-gds-type,p-sub-type)
    .
end procedure.
...
on error undo, return error return-value
:
  if  p-lock-gds-type <> 'grar':U
  and p-lock-gds-type <> 'grgc':U
  then do:
    message
  ...
  if  p-sub-type <> 'disable':U
  and p-sub-type <> 'enable':U
  then do:
    message
      vss-workfile vss-revision vss-description skip
      "Ошибка задания входных параметров" skip
  ...
  run gbl/lock-swi.p
    (input p-lock-gds-type
    ,input p-sub-type
    ,input 3600
    ,input "Переименование артикула"
    ,input 0
    ,input 0
    ,input 0
    ,input ""
