...
do
    ...
    if p-resource-type = 'data':U
        or ((p-resource-type = 'report':U
        or p-resource-type = 'report-xml':U)
        and p-bh <> ? )
        then
    do:
        if p-resource-type = 'data':U then
        do:
        ...
        when 'ДОБАВЛЕНИЕ':U then
            do:
                ...
                if available self_clob-bind
                    and (self_clob-bind.resource-type = 'data':U
                    or
                    self_clob-bind.resource-type = 'report':U
                    or
                    self_clob-bind.resource-type = 'report-xml':U
                    or
                    self_clob-bind.resource-type = 'list':U
                    or
                    self_clob-bind.resource-type = 'list-macro':U
                    )
                    then
                do:
                    assign
                ...
                do:
                    ...
                    assign
                        buf_clob-data.int64-id      = next-value( s-clob-int64, ub)
                ...
                assign
                    self_clob-bind.uniq-key-rec  = v-uniq-key-rec
                    self_clob-bind.field-name    = (if p-resource-type = 'list':U
                    ...
                    p-clob-db-num                = self_clob-bind.db-num
                    p-int64-id                   = self_clob-bind.int64-id
                    buf_clob-data.file-name_     = (if p-resource-type = 'list':U
                                or p-resource-type = 'list-macro':U
        ...
        when 'удаление':U then
            do:
                ...
                if self_clob-bind.db-num <> g#db-num
                    and not (p-resource-type = 'data':U
                    or p-resource-type = 'report':U
                    or p-resource-type = 'report-xml':U)
                    then
                do:
