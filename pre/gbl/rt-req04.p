...
define variable vss-workfile    as character no-undo init "$Workfile: rt-req04.p $":U .
define variable vss-archive     as character no-undo init "$Archive: gbl/rt-req04.p $":U .
define variable vss-description as character no-undo init "Обрабока запроса радиотерминала 04. Приемка товара. Настройка".
...
:
  ...
  then do:
    put stream sout unformatted 'status:0' + chr(10) .
    put stream sout unformatted 'message:' + chr(10) .
  end.
  else do:
    put stream sout unformatted 'status:1' + chr(10) .
    put stream sout unformatted substitute('message:&1', rtencode(v-error-message)) + chr(10) .
  end.
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    then do:
      assign
        p-data-valid    = false
        p-error-message = substitute( "Неверное значение параметра cop_check : &1" , p-cop-check )
      .
      return .
    end.
    ...
    then do:
      assign
        p-data-valid    = false
        p-error-message = substitute( "Неверное значение параметра prod_artic_search : &1" , p-prod-artic-search )
      .
      return .
    end.
    assign
