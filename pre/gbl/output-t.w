...
DEFINE VARIABLE  v-output-type AS CHARACTER NO-UNDO.
  IF t-text:VISIBLE IN FRAME Dialog-Frame THEN DO:
    ASSIGN
    t-text.
    IF t-text THEN DO:
        v-output-type = v-output-type +
                         (IF v-output-type = '' THEN '' ELSE chr(44)) +
                        'text':U.
    END.
  END.
  IF t-excel:VISIBLE IN FRAME Dialog-Frame THEN DO:
    ASSIGN
    t-excel.
    IF t-excel THEN DO:
        v-output-type = v-output-type +
    ...
    END.
  END.
  IF t-xml:VISIBLE IN FRAME Dialog-Frame THEN DO:
    ASSIGN
    t-xml.
    IF t-xml THEN DO:
        v-output-type = v-output-type +
    ...
    END.
  END.
  IF t-screen:VISIBLE IN FRAME Dialog-Frame THEN DO:
    ASSIGN
    t-screen.
    IF t-screen THEN DO:
        v-output-type = v-output-type +
    ...
    END.
  END.
  IF t-pdf:VISIBLE IN FRAME Dialog-Frame THEN DO:
    ASSIGN
    t-pdf.
    IF t-pdf THEN DO:
        v-output-type = v-output-type +
...
DO v-ii = 1 TO NUM-ENTRIES(p-mode):
   ...
   CASE v-entry:
     WHEN 'text':U THEN DO:
       t-text:VISIBLE IN FRAME Dialog-Frame = YES.
       t-text = LOOKUP('text':U, p-output-type) > 0.
       ENABLE t-text
       WITH FRAME Dialog-Frame.
     END.
     WHEN 'excel':U THEN DO:
       t-excel:VISIBLE IN FRAME Dialog-Frame = YES.
       t-excel = LOOKUP('excel':U, p-output-type) > 0.
       ENABLE t-excel
       WITH FRAME Dialog-Frame.
     END.
     WHEN 'xml':U THEN DO:
       t-xml:VISIBLE IN FRAME Dialog-Frame = YES.
       t-xml = LOOKUP('xml':U, p-output-type) > 0.
       ENABLE t-xml
       WITH FRAME Dialog-Frame.
     END.
     WHEN 'screen':U THEN DO:
       t-screen:VISIBLE IN FRAME Dialog-Frame = YES.
       t-screen = LOOKUP('screen':U, p-output-type) > 0.
       ENABLE t-screen
       WITH FRAME Dialog-Frame.
     END.
     WHEN 'pdf':U THEN DO:
       t-pdf:VISIBLE IN FRAME Dialog-Frame = YES.
       t-pdf = LOOKUP('pdf':U, p-output-type) > 0.
       ENABLE t-pdf
       WITH FRAME Dialog-Frame.
     END.
  END CASE.
