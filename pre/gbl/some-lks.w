...
entry (lookup (X_who-lk.lk-type, 'update-delete':U) + 1, ',' + 'Изменение/Удаление':U) COLUMN-LABEL "Блокирование на" FORMAT "X(20)"
...
assign
  v-str-recid8 = trim( string( recid( X_who-lk ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry8 > 0 then do:
  ...
  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      ...
      apply "VALUE-CHANGED" to br-who-lk in frame Dialog-Frame.
  end.
...
apply "entry" to br-who-lk in frame Dialog-Frame.
END.
...
DO:
  if available X_who-lk then do:
     e-ps:screen-value in frame Dialog-Frame = X_who-lk.ps.
  end.
  else do:
     e-ps:screen-value in frame Dialog-Frame = "".
  end.
END.
define variable vss-include-info9 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on f5 of frame Dialog-Frame anywhere
do:
   v-doc-rec = recid(X_who-lk).    RUn OpenBR in this-procedure.  REPOSITION br-who-lk to recid v-doc-rec No-ERROR.   apply 'value-changed' to br-who-lk.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-who-lk :handle
      .
...
define variable vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if lookup( p-list-mode , 'все':U + chr(4) + "resource-id", chr(4) ) = 0 then do:
    message
...
CASE p-list-mode:
   when 'все':U then do:
      frame Dialog-Frame :title = "Все использования".
      OPEN QUERY br-who-lk FOR EACH X_who-lk NO-LOCK INDEXED-REPOSITION.
   end.
   when "resource-id" then do:
      frame Dialog-Frame :title = substitute("Использование &1", v-resource-descr).
