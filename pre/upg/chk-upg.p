...
:
  ...
  if error-status :error then do:
    return error string( vss-workfile + chr(32) + "Ошибка при определении текущего времени" ) .
  end.
      ...
      :
        ...
        if not available buf_BatchProcess then do:
          ...
          if available buf_BatchProcess then do:
            ...
            run nws/cr-route.p ( input 'send-cmd':U, input v-cmd, input ?, input string( buf_db.db-num ) ).
