...
:
  ...
  if error-status :error then do:
    assign
      v-msg = vss-workfile + chr(32)
              + substitute( "Ошибка выполнения upgrade в БД &1. Шаг &2", p-curr-db-num, p-step-num ) + chr(10)
              + v-msg + chr(10)
    ...
    if p-curr-db-num <> 0 then do:
      assign
        v-cmd = "command":U + chr(1)
                + "upgrade":U + chr(1)
                + string( p-action ) + chr(1)
                + string( p-step-num ) + chr(1)
                + string( p-curr-db-num ) + chr(1)
                + "Err":U + chr(1)
                + str-encode ( v-msg, "":U, chr(10) )
      .
      run nws/cr-route.p ( input 'send-cmd':U, input v-cmd, input ?, input "0":U ).
    end.
  end.
  else do:
    run write-to-log( v-msg ) .
    if p-step-num = 0 then do:
      run upg/upg-clbp.p no-error .
      if error-status :error then do:
        run write-to-log( vss-workfile + chr(32) + "Ошибка при удалении записей о времени запуска Upgrade ! (1)" ).
      end.
    end.
    else do:
