...
:
    ...
    assign
        ...
        buf_temp_sheet1_line-data.sheet-name    = "M11":U
    ...
    put stream excel-line unformatted
        ...
        chr(9)   "DTA":U
        chr(9)   buf_temp_sheet1_line-data.account
        chr(9)   buf_temp_sheet1_line-data.analytics
        chr(9)   buf_temp_sheet1_line-data.wealthname
        chr(9)   buf_temp_sheet1_line-data.article
        chr(9)   buf_temp_sheet1_line-data.unitcode
        chr(9)   buf_temp_sheet1_line-data.unitname
        chr(9)   buf_temp_sheet1_line-data.quantity1
        chr(9)   buf_temp_sheet1_line-data.quantity2
        chr(9)   buf_temp_sheet1_line-data.costrubl
        chr(9)   buf_temp_sheet1_line-data.noVATrubl
        chr(9)   buf_temp_sheet1_line-data.order
        chr(10)
    .
    .
end.
...
HEADER
  ...
  ": счет, :код анали-:                                   :                :   :                                   :             :             :      руб. коп.     :      руб. коп.      : складской:" SKIP
...
DO ON ERROR   UNDO Main-Block, LEAVE Main-Block
  ...
  RUN prn-lib-open-stream IN THIS-PROCEDURE ( INPUT p-parent-proc, INPUT 43, INPUT YES, INPUT NO ).
  FIND buf_own NO-LOCK WHERE
       buf_own.obj-type = 'орг':U            AND
  ...
  PUT STREAM PrnLibStream UNFORMATTED
    ": " + STRING( buf_doc.doc-date, "99.99.9999":U ) +
                ":" + ( IF buf_doc.ext-doc-type = 'we':U OR
                           buf_doc.ext-doc-type = 'wm':U  THEN "  списание " ELSE
                      ( IF buf_doc.ext-doc-type = 'ev':U THEN "расх.внутр."
                                                                         ELSE "           ":U ) ) +
  ...
  ASSIGN total-qty1  = 0
         ...
         j-order#    = 0.
  RUN r-f_m11xl-init.
  RUN r-f_m11xl-write-cell-data (INPUT "h_doccode":U, INPUT  buf_doc.doc-code) .
  RUN r-f_m11xl-write-cell-data (INPUT "h_orgname":U, INPUT  IF AVAILABLE buf_own THEN  TRIM( buf_own.obj-name )
                           ELSE "" ) .
  RUN r-f_m11xl-write-cell-data (INPUT "h_docdate":U, INPUT STRING( buf_doc.doc-date, "99.99.9999":U )).
  RUN r-f_m11xl-write-cell-data (INPUT "h_doctype":U, INPUT ( IF buf_doc.ext-doc-type = 'we':U OR
                           buf_doc.ext-doc-type = 'wm':U  THEN "списание" ELSE
                      ( IF buf_doc.ext-doc-type = 'ev':U THEN "расх.внутр."
                                                                         ELSE "":U ) )).
  RUN r-f_m11xl-write-cell-data (INPUT "h_objname1":U, INPUT TRIM ( buf_obj.obj-name ) ).
  RUN r-f_m11xl-write-cell-data (INPUT "h_objname2":U, INPUT TRIM ( buf_cli.obj-name ) ).
  RUN r-f_m11xl-write-cell-data (INPUT "h_storeman":U, INPUT TRIM ( IF AVAILABLE buf_wrkr THEN ( buf_wrkr.obj-name ) ELSE "" ) ).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run basecode in g#library
  ...
  RUN r-f_m11xl-write-cell-data (INPUT "f_wordord":U, INPUT TRIM ( word-ord ) + " наименований" ).
  RUN r-f_m11xl-write-cell-data (INPUT "f_wordsum":U, INPUT TRIM ( word-sum ) ).
  RUN r-f_m11xl-write-cell-data (INPUT "f_wordVAT":U, INPUT TRIM ( word-VAT ) ).
  RUN r-f_m11xl-write-cell-data (INPUT "M11_it_noVATrubl":U, INPUT TRIM ( STRING ( total-noVAT ))).
  RUN r-f_m11xl-write-cell-data (INPUT "M11_it_quantity1":U, INPUT TRIM ( STRING ( total-qty1 ))).
  RUN r-f_m11xl-write-cell-data (INPUT "M11_it_quantity2":U, INPUT TRIM ( STRING ( total-qty2 ))).
...
PROCEDURE get-vars :
  ...
  DO ON ERROR UNDO, RETURN :
    ...
    ASSIGN p-unit-name     = ( IF AVAILABLE buf-unit THEN buf-unit.long-name ELSE "":U ).
        RUN clcprtsl_calc-line IN THIS-PROCEDURE ( INPUT p-line-rec_id ) NO-ERROR.                  IF ERROR-STATUS :ERROR THEN DO: UNDO, RETURN. END.
    FIND tt-allsum-line WHERE tt-allsum-line.sum-type = 'основная_сумма':U NO-ERROR.                  IF NOT AVAILABLE tt-allsum-line THEN DO: UNDO, RETURN ERROR. END.
    ASSIGN p-quantity-fact = tt-allsum-line.fact-qnty.
    IF CostPrice
      THEN DO:
        IF printrubl
          THEN DO:
            ASSIGN  p-cost     = ( tt-allsum-line.sum-dsc-rubl-acc / tt-allsum-line.fact-qnty )
                    p-no-VAT   = ( tt-allsum-line.sum-dsc-rubl-acc - tt-allsum-line.vat-rubl-acc )
                    VAT        = VAT + tt-allsum-line.vat-rubl-acc.
          END.
          ELSE DO:
            ASSIGN  p-cost     = ( tt-allsum-line.sum-dsc-base-acc / tt-allsum-line.fact-qnty )
                    p-no-VAT   = ( tt-allsum-line.sum-dsc-base-acc - tt-allsum-line.vat-base-acc )
                    VAT        = VAT + tt-allsum-line.vat-base-acc.
          END.
      END.
      ELSE DO:
        IF printrubl
          THEN DO:
            ASSIGN  p-cost     = ( tt-allsum-line.sum-dsc-rubl-doc / tt-allsum-line.fact-qnty )
                    p-no-VAT   = ( tt-allsum-line.sum-dsc-rubl-doc - tt-allsum-line.vat-rubl-doc )
                    VAT        = VAT + tt-allsum-line.vat-rubl-doc.
          END.
          ELSE DO:
            ASSIGN  p-cost     = ( tt-allsum-line.sum-dsc-base-doc / tt-allsum-line.fact-qnty )
                    p-no-VAT   = ( tt-allsum-line.sum-dsc-base-doc - tt-allsum-line.vat-base-doc )
                     VAT       = VAT + tt-allsum-line.vat-base-doc.
          END.
    END.
  END.
END PROCEDURE.
