...
:
    ...
    assign
        ...
        buf_temp_line-data.data-key       = "DTA":U
    ...
    put stream excel-line unformatted
        ...
        CHR(9)   buf_temp_line-data.data-key
        CHR(9)   buf_temp_line-data.d_znumber
        CHR(9)   buf_temp_line-data.d_empty2
        CHR(9)   buf_temp_line-data.d_empty3
        CHR(9)   buf_temp_line-data.d_empty4
        CHR(9)   buf_temp_line-data.d_empty5
        CHR(9)   buf_temp_line-data.d_empty6
        CHR(9)   buf_temp_line-data.d_summ-sale-7
        CHR(9)   buf_temp_line-data.d_summreturn
        CHR(9)   buf_temp_line-data.d_person
        CHR(9)   buf_temp_line-data.d_empty10
        chr(10)
    .
end.
end procedure.
...
define temp-table temp-str no-undo
   ...
   field chk-shift-open-time as logical
  INDEX pi  IS PRIMARY
        chk-date
        chk-time
        .
...
define variable v-is-cash-list as character no-undo.
...
if v-curr-r-b = 'base':U and v-base-code <> 0 then printrubl = no .
 for each temp-str :
    delete temp-str.
 end.
...
"|          |        |        |ков на нули    |               |               |               |               |                  |       |" skip
"+----------+--------+--------+---------------+---------------+---------------+---------------+---------------+------------------+-------+" skip
  with width 232 down stream-io no-box no-underline no-labels .
...
define variable chk-shift-open-time as logical no-undo init yes.
for each ub.inkas no-lock where ub.inkas.obj-code = tt-cash-desk.obj-code
and ub.inkas.obj-type =  'маг':U and ub.inkas.shift-date = x-date-start and ub.inkas.shift-num = x-shift-alone,
 each buf_chk-doc
    where buf_chk-doc.out-code    = ub.inkas.inkas-code and buf_chk-doc.pay-desk = tt-cash-desk.cash-num
      no-lock use-index sale:
        ...
        for first buf_shift-cash
           where buf_shift-cash.obj-type    = 'маг':U
      ...
      if temp-str.chk-time-2 = 0 then do:
          assign temp-str.chk-time-2 = buf_chk-doc.chk-time.
      end.
      if temp-str.chk-shift-open-time = no then do:
          assign temp-str.chk-time-1 = buf_chk-doc.chk-time.
      end.
      if lookup(string(buf_chk-doc.chk-type), '1,69,14,15,16,36':U) > 0 then do :
      ...
      if lookup(string(buf_chk-doc.chk-type), '6,96':U) > 0 then do :
      ...
      find first buf_sale-clients
            where buf_sale-clients.obj-code = buf_chk-doc.cashier-psn-code
              and buf_sale-clients.obj-type = 'чел':U
              no-lock
              no-error.
...
release temp-str.
    ...
    assign
      sheet-list = sheet-list + (if sheet-list = '' then '' else chr(44)) + substitute("ККМ&1", tt-cash-desk.cash-num)
      sheet-list-copy-from = sheet-list-copy-from + (if sheet-list-copy-from = '' then '' else chr(44)) + "Template"
    .
  end.
end.
...
if not available temp-str then do:
    ...
    if p-batch > 0 then do:      run write-log-and-file in p-log-handle (                 input 1                                          , input p-log-file-name                              , input 1                                          , input "Не было чеков за выбранный период!!").    end.    else do:       run write-to-log in p-log-handle ( input "Не было чеков за выбранный период!!").    end.
    run cb_write-report-error in p-parent-handle ( input p-rebh
                                                  ,input p-report-id
                                                  ,input ?
                                                  ,input '3':U
                                                  ,input "Не было чеков за выбранный период!!").
    RETURN.
  end.
  else do:
    if p-batch > 0 then do:      run write-log-and-file in p-log-handle (                 input 1                                          , input p-log-file-name                              , input 1                                          , input "Не было чеков за выбранный период!!").    end.    else do:       run write-to-log in p-log-handle ( input "Не было чеков за выбранный период!!").    end.
...
for each tt-cash-desk
  ...
  if session:set-wait-state("compiler") then.
   ...
   FIND FIRST buf_cash-desk WHERE buf_cash-desk.cash-num = tt-cash-desk.cash-num
                            ...
                            NO-ERROR
                            .
   FIND first This_Object  WHERE This_Object.obj-type = 'маг':U
                       AND This_Object.obj-code = tt-cash-desk.obj-code
                     NO-LOCK.
   FIND first ub.clients  WHERE ub.clients.obj-type   = 'орг':U
   ...
   assign
      ...
      v-kkm-programm  = (if tt-cash-desk.pos-type = 'IBM-XML':U
                         or tt-cash-desk.pos-type = 'IBM':U
...
DO:
  ...
  display stream Out-Stream
          ...
          sym11
  with FRAME km-frame.
  DOWN stream Out-Stream 1 with FRAME km-frame
  .
...
do on error undo, return error return-value  :
  ...
  do:
   ...
   do:
     ...
     do:
       do v-i = 1 to v-ii + 1:
         ...
         if length(v-txt-1) + 1 + length(entry((v-i + 1), v-organization, " ")) > 78 then
         do:
           leave.
         end.
       end.
   ...
   PUT STREAM Out-Stream
       ...
       space(5) UndLine      format "X(100)" "по ОКРО" format "X(7)" AT 113 "|" AT 120 t-okpo format "X(16)" "|" AT 137 skip
       space(35) "организация, адрес, номер телефона" format "X(80)"                                     "+----------------+"     AT 120 skip
       space(5) v-object format "X(100)" "ИНН| " AT 117                                t-INN format "X(15)"  "|" AT 137 skip
...
do on error undo, return error return-value  :
   ...
   define variable o-cassir            AS integer no-undo.
   define variable v-cassir            AS character no-undo.
   define variable o-cassir-op         AS integer no-undo.
   define variable v-cassir-op         AS character no-undo.
   define buffer   buf_shop            FOR ub.shop.
   define buffer   buf_store           FOR ub.store.
   ...
   define buffer   buf_shift-staff     FOR ub.shift-staff.
    ...
    where buf_shift-staff.obj-type    = 'маг':U
        ...
        where buf_arh-fin-doc-schet-nal-obj.host-code         = This_Object.host-code
          ...
          find first buf_fin-doc
                  ...
                  if available buf_fin-doc then do :
                    ...
                    case o-director:
                      when '1':U then do:
                        if This_Object.obj-type = 'маг':U then do:
                        ...
                        if This_Object.obj-type = 'скл':U then do:
