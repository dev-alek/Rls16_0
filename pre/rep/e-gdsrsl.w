...
define variable SuppName  as character no-undo view-as editor scrollbar-vertical size-chars 51.50 by 3.00
  tooltip "Список выбранных Поставщиков" initial 'все':U .
...
.
assign
  SuppName :read-only in frame F-Main = yes
.
...
PROCEDURE adm-disable :
    DISABLE r-s-suppl SuppName Tog-Suppl r-s-bill# Btn_Columns WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN r-s-suppl SuppName Tog-Suppl r-s-bill# Btn_Columns WITH FRAME F-Main.
...
on choose of Btn_Columns in frame F-Main
do:
  ...
  then do:
    ...
    assign
      v-part-2 = string( input frame F-Main Tog-Suppl, "yes/no":U )
      v-part-3 = string( input frame F-Main r-s-bill#, "9":U      )
    .
  end.
  run rep/c-xtypes.w
    ( input "r-gdsrsl":U
...
on value-changed of r-s-bill# in frame F-Main
do:
  assign
    r-s-bill#
  .
end.
on value-changed of r-s-suppl in frame F-Main
do:
      ...
      then do:
        ...
        :
          ...
          assign
            ...
            SuppName             = SuppName + bf_suppl.obj-name + chr(10)
          .
        end.
        display
          SuppName
...
procedure local-initialize :
  ...
  .
  apply "VALUE-CHANGED":U to r-s-suppl in frame F-Main .
  display
    SuppName
    Tog-Suppl
    r-s-bill#
