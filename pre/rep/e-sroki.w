...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-5 fi-days-amount rs-classificator rs-sort-type tg-empty-disabled T-free v-godendo WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-5 fi-days-amount rs-classificator rs-sort-type tg-empty-disabled T-free v-godendo WITH FRAME F-Main.
...
ON LEAVE OF fi-days-amount IN FRAME F-Main
DO:
  ...
  if fi-days-amount = 0 then display 'ВСЕ' @ v-godendo with frame F-Main.
  else
  display v-godendo with frame F-Main.
END.
