...
define variable vss-description as character no-undo init "Оборотная ведомость по поставшикам (закладка № 2)".
...
DEFINE VARIABLE Showsale AS LOGICAL INITIAL no
     ...
     VIEW-AS TOGGLE-BOX
     SIZE 21 BY .83 TOOLTIP "Показать суммы в ценах документа" NO-UNDO.
DEFINE VARIABLE ShowZero AS LOGICAL INITIAL no
     LABEL "Нулевые остатки":L
     VIEW-AS TOGGLE-BOX
     SIZE 28.63 BY 1 TOOLTIP "Показать товары с нулевым оборотом и нулевыми остатками" NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Showcost AT ROW 6.58 COL 49
     Classify AT ROW 6.79 COL 2.75 NO-LABEL
     Showsale AT ROW 7.54 COL 49.13
     ...
     type-stor AT ROW 8.63 COL 49.38 NO-LABEL
     t-in AT ROW 12.04 COL 2.75
     SumsOnly AT ROW 14.25 COL 49.25
     ShowParts AT ROW 15.42 COL 49.25
     ShowZero-2 AT ROW 15.5 COL 2.75
     ShowZero AT ROW 16.42 COL 2.75
     Cli-art AT ROW 16.58 COL 49.25
     "Выбор поставщика:" VIEW-AS TEXT
          SIZE 19.38 BY .67 AT ROW 1.25 COL 7.13
          FGCOLOR 4
     "Сортировка :" VIEW-AS TEXT
          SIZE 13.63 BY .75 AT ROW 1.29 COL 52.75
          FGCOLOR 4
     "Показать :" VIEW-AS TEXT
          SIZE 11.5 BY .75 AT ROW 5.71 COL 52.75
          FGCOLOR 4
...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-7 RECT-5 RECT-8 RECT-9 RADPost PostName SortType Tog-obj Showcost Classify Showsale type-stor t-in SumsOnly ShowParts ShowZero-2 Cli-art WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-7 RECT-5 RECT-8 RECT-9 RADPost PostName SortType Tog-obj Showcost Classify Showsale type-stor t-in SumsOnly ShowParts ShowZero-2 Cli-art WITH FRAME F-Main.
...
ASSIGN
       ShowParts:HIDDEN IN FRAME F-Main           = TRUE.
...
DO:
        ...
        do:
           run ref/cli-all.w
            ...
            if post-grp_recids = "" then do:
                 Assign  Postname = 'все':U radpost = 1.
                 Display PostName radpost with frame F-Main .
            end.
            else do:
                Assign  Postname = ''.
  ...
  End case.
  END.
ON VALUE-CHANGED OF Showsale IN FRAME F-Main
DO:
assign Showsale.
  if showsale then DO:
                  type-stor = 1 .
                  type-stor:screen-value = '1' .
                  disable type-stor with frame F-Main.
                  End.
              else enable type-stor with frame F-Main.
END.
...
DO:
  ...
  if ShowZero-2 = no then do:
     ShowZero = no .
     display ShowZero with frame F-Main .
     disable ShowZero with frame F-Main .
  end.
...
END.
ON VALUE-CHANGED OF Tog-obj IN FRAME F-Main
DO:
define variable g#log as logical   no-undo .
 assign Tog-obj.
  if tog-obj = false then do:
       g#log = Classify:disable(radio-label("post", Classify:radio-buttons)) .
       g#log = Classify:disable(radio-label("post/grp-goods", Classify:radio-buttons)) .
       g#log = Classify:disable(radio-label("grp-goods/post", Classify:radio-buttons)) .
  End.
  Else do:
       g#log = Classify:enable (radio-label("post", Classify:radio-buttons)) .
       g#log = Classify:enable (radio-label("post/grp-goods", Classify:radio-buttons)) .
       g#log = Classify:enable (radio-label("grp-goods/post", Classify:radio-buttons)) .
    display Classify SumsOnly type-stor ShowZero  with frame F-Main.
    enable Classify SumsOnly type-stor ShowZero  with frame F-Main.
  End.
END.
...
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main type-stor
Cli-art  PostName  RADPost   ShowParts showcost showsale
ShowZero ShowZero-2 Classify  SortType  SumsOnly Tog-obj
Tog-lavel  var-lavel Tog-lavel-2  Var-lavel-2 t-in .
Show-cost = showcost.
Show-sale = showsale.
ReportNAme = "Оборотная ведомость по поставщикам " .
...
ReportHeader =  ReportHeader  + chr(10) + Caps(Entry(((type-stor * 2) - 1), type-stor:RADIO-BUTTONS)) .
 sheetf.Excel-Column-Lable = "Код,Артикул,Название товара ,Ед.изм" +
       ",Остаток на  начало" +  (if Show-Cost then "," else "" )  + (if  Show-Sale then "," else "" )  +
       ...
       ",Остаток на конец" +  (if Show-Cost then "," else "" )  + (if  Show-Sale then "," else "" )  +
        chr(10) .
 ...
 sheetf.Sizes  = "10,16,60,7,13,13,13,13,13,13,13,13,13," +
                           ...
                           (if Show-Sale then "13,13,13,13,13,13,13,13,13," else "") .
