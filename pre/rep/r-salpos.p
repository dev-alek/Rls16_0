...
if session :set-wait-state( "compiler" ) then.
  ...
  for each store no-lock where store.host-code = v-cntxt-host-code-obj :
    find first clients no-lock where clients.obj-type = 'скл':U and clients.obj-code = store.obj-code no-error .
  ...
  for each shop no-lock where shop.host-code = v-cntxt-host-code-obj :
    find first clients no-lock where clients.obj-type = 'маг':U and clients.obj-code = shop.obj-code no-error .
    if available clients then do:
      create temp-obj .
      assign  temp-obj.obj-name = clients.obj-name    temp-obj.obj-type = clients.obj-type    temp-obj.obj-code = clients.obj-code   .
    end.
...
End.
  ...
  HEADER
      ...
      string( "Страница " + string( PAGE-NUMBER( PrnLibStream )  , ">>9") ) AT 100 format "X(15)" SKIP Line format "X(149)" AT 1
  with width 235 down stream-io.
  run prn-lib-open-stream  in this-procedure (input parParentProc,input 43,input yes,input no).
  FORM HEADER
      Line format "X(149)" AT 1 SKIP   "Продолжение - на следующей странице" AT 30 SKIP
  ...
  PUT STREAM PrnLibStream Line format "X(149)".
