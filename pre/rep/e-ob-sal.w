...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-7 RECT-8 contr-code BUTTON-contr is-post is-real snum is-fo is-fin itog-only is-date WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-7 RECT-8 contr-code BUTTON-contr is-post is-real snum is-fo is-fin itog-only is-date WITH FRAME F-Main.
...
ON CHOOSE OF BUTTON-contr IN FRAME F-Main
DO:
  ...
  display contr-code snum with frame F-Main.
  if not itog-only then do :
    enable itog-contract with frame F-Main.
  end.
END.
ON LEAVE OF contr-code IN FRAME F-Main
DO:
  assign contr-code.
  ...
  if snum = "" and contr-code = 0 then do :
    disable itog-contract with frame F-Main.
  end.
END.
ON RETURN OF contr-code IN FRAME F-Main
DO:
  assign contr-code.
  if contr-code <> 0 then do :
    run  FindRec in this-procedure  ( input 0 ) .
...
END.
ON VALUE-CHANGED OF itog-contract IN FRAME F-Main
DO:
  assign itog-contract .
  if itog-contract then do:
    disable itog-only with frame F-Main.
  end.
  else do:
    enable itog-only with frame F-Main.
  end.
END.
ON VALUE-CHANGED OF itog-only IN FRAME F-Main
DO:
  assign itog-only .
...
then do:
  ...
  assign  frame F-Main  is-real is-post is-fin is-fo .
  display is-real is-post is-fin is-fo with frame F-Main.
  enable itog-contract with frame F-Main.
...
END PROCEDURE.
PROCEDURE my-var :
  Assign frame F-Main itog-only itog-contract is-date is-fin is-fo is-post is-real .
  ...
  if contr-code > 0  then  assign ReportNAme = "Оборотно-сальдовая ведомость по договору " + snum .
