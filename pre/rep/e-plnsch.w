...
def var vss-description as character no-undo init "Отчет Планируемые платежи (закладка № 2)".
...
PROCEDURE adm-disable :
    DISABLE RECT-6 itog-comp itog-contract WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 itog-comp itog-contract WITH FRAME F-Main.
...
END PROCEDURE.
PROCEDURE my-report :
run rep/r-plnsch.p ( itog-comp, itog-contract ) .
END PROCEDURE.
PROCEDURE my-var :
  Assign frame F-Main itog-comp itog-contract .
  find first clients no-lock where clients.obj-type = 'орг':U and clients.obj-code = v-cntxt-host-code-obj .
  assign
    ReportNAme = "Планируемые платежи на: " + string(x-Date-Alone,"99/99/9999") + "г." .
