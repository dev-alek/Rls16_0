        ...
        FOR EACH ub.doc-line WHERE ub.doc-line.doc-code = ub.trn-doc.doc-code NO-LOCK :
            ...
            if available prod_sum then do:
                FOR EACH gds-dtl WHERE
                    ...
                    ACCUMULATE (if v-curr-r-b = 'base':U
...
if can-find( first prod_sum where prod_sum.tot-sum <> 0 ) then do:
    ...
    run prn-lib-open-stream  in this-procedure (
     ...
     sheetf.Excel-COlumn-Lable = string( "ФИРМА", "x(19)" ) + chr(44)
     ...
     FOR EACH objects BY objects.obj-type BY objects.obj-code :
         ...
         sheetf.Excel-COlumn-Lable = sheetf.Excel-COlumn-Lable + string( replace( objects.obj-name, chr(44), "":U), "x(15)" ) + chr(44)
         .
     END .
     assign
     ...
     FOR EACH prod_sum BREAK BY prod_sum.name-attr
        ...
        XLStrBuf = string( string(prod_sum.obj-name, "X(19)" ) + CHR(9) )
        .
    ...
    XLStrBuf = string("ИТОГО:", "X(18)") + CHR(9)
    .
    ...
    XLStrBuf = XLStrBuf +
             string( ACCUM TOTAL prod_sum.tot-sum) + CHR(9)
     .
    EXPORT stream PrnLibStream StrBuf .
    PUT stream PrnLibStream " " SKIP.
    if Make-Excel then  put   stream ForExcel unformatted
