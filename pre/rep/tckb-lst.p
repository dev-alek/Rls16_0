...
define variable vss-workfile    as character no-undo init "$Workfile: tckb-lst.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/tckb-lst.p $":U .
...
run rep/tickets.w (input parparentproc, input p-obj-type, input p-obj-code, input Action, input 'bb-list' ).
if TicketName = "" then
    RETURN.
...
define variable v-value12 as character no-undo .
if NOT can-find(first bb-list) then
    RETURN.
for each bb-list
by bb-list.artic
by bb-list.prod-type
by bb-list.prod-code
:
  if bb-list.b-str = "":U then next.
  find ub.goods no-lock
    where ub.goods.prod-type = bb-list.prod-type
      and ub.goods.prod-code = bb-list.prod-code
      and ub.goods.artic = bb-list.artic
  .
  find ub.gds-prt no-lock
    where ub.gds-prt.upper-code = ub.goods.prt-root
  .
  if bb-list.qnty = ? then do:
    run gbl/d-prompt.w (
      'title=':u +  substitute("ЕАН &1" , bb-list.b-str )  + '\':u
    ...
    + 'max-chars=5\':u
    + 'readonly=no\':u
    , input-output v-value12
    ).
    if return-value = 'false':u
        then bb-list.qnty .
        else bb-list.qnty = int(v-value12) .
  end.
  assign
    rootnode_code = ub.gds-prt.node-code
    list-qnty = bb-list.qnty
  .
  ...
  find first ub.bar-code no-lock where
            ub.bar-code.b-code = bb-list.b-code no-error .
  if available ub.bar-code then do:
    if bb-list.b-str <> "":U
    and not bb-list.loc-ean then do:
      define buffer buf_prod-bc for ub.prod-bc.
      find first buf_prod-bc where buf_prod-bc.b-code =
                  bb-list.b-code
              AND buf_prod-bc.b-str = bb-list.b-str no-error .
...
run rep/ticket.p (
    ...
    if bb-list.b-str <> "":U
    and bb-list.loc-ean then do:
      ListProdbc = bb-list.b-str.
      do on error undo, return error :
        if prn-prt = false     and bar-code.node-code <> rootnode_code   then do:     next.   end.   if prn-prt = true then do:     find first gds-prt no-lock       where gds-prt.node-code = bar-code.node-code     .     if gds-prt.is-term = false then do:       next.     end.   end.
run rep/ticket.p (
               buffer ub.goods
...
define variable v-value14 as character no-undo .
if NOT can-find(first bb-list) then
    RETURN.
for each bb-list
by bb-list.b-code
:
  ...
  if bb-list.qnty = ? then do:
    ...
    + 'max-chars=5\':u
    + 'readonly=no\':u
    , input-output v-value14
    ).
    if return-value = 'false':u
        then bb-list.qnty .
        else bb-list.qnty = int(v-value14) .
  end.
  assign
    rootnode_code = ub.gds-prt.node-code
    list-qnty = bb-list.qnty
...
define variable v-value16 as character no-undo .
if NOT can-find(first bb-list) then
    RETURN.
for each bb-list
by bb-list.order-num
:
  if bb-list.b-str = "":U then next.
  find ub.goods no-lock
    where ub.goods.prod-type = bb-list.prod-type
  ...
  if bb-list.qnty = ? then do:
    ...
    + 'max-chars=5\':u
    + 'readonly=no\':u
    , input-output v-value16
    ).
    if return-value = 'false':u
        then bb-list.qnty .
        else bb-list.qnty = int(v-value16) .
  end.
  assign
    rootnode_code = ub.gds-prt.node-code
    list-qnty = bb-list.qnty
