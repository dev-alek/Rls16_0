...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-dispet.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-7 v-hours v-minutes t-excel t-xml b-esys WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 v-hours v-minutes t-excel t-xml b-esys WITH FRAME F-Main.
...
ON CHOOSE OF b-esys IN FRAME F-Main
DO:
...
END.
 IF p-interface THEN DO:
      run bge/oxmlexts.p (
            input my-handle
          , input 2
          , input substitute("esys-type = &1", '6':U)
   ...
   CASE place-call:
     WHEN 'schedule':U THEN DO:
       ...
       f-text-1
               ...
               f-esys-name
       IN FRAME F-Main.
       t-excel = LOOKUP('excel':U, v-output-type) > 0.
       t-xml = LOOKUP('xml':U, v-output-type) > 0.
       display
       t-excel
       t-xml
       WITH FRAME F-Main.
       ENABLE
       t-excel WHEN params-only-mode <> 'ПРОСМОТР':U
       t-xml WHEN params-only-mode <> 'ПРОСМОТР':U
       WITH FRAME F-Main.
     END.
           WHEN 52 THEN DO:
             HIDE
             ...
             WITH FRAME F-Main.
             ENABLE
             b-esys WHEN params-only-mode <> 'ПРОСМОТР':U
             WITH FRAME F-Main.
           END.
       END CASE.
     END.
...
case p-action:
  when "get" then do:
    ...
    do while v-index-id >= 1 :
      ...
      find first X-init_obj-list where
                X-init_obj-list.obj-type = 'маг':U
  ...
  when "set" then do:
    ...
    t-xml
    v-loc-output-type = ","
    ENTRY(1, v-loc-output-type) = (IF t-excel THEN 'excel':U ELSE '')
    ENTRY(2, v-loc-output-type) = (IF t-xml THEN 'xml':U ELSE '')
    v-loc-output-type = TRIM(v-loc-output-type, chr(44))
    .
    RUN rcps_set-value IN parent-handle (
                                     input "p-output-type"
                                    ,INPUT 0
                                    ,input v-loc-output-type
                                    ,input ?
                                    ,input 0.0
                                    ,input 0
                                    ,input no
    ...
    case v-profile-id:
      when 50 THEN DO:
        ...
        ReportHeader = substitute("&1Направление вывода: &2 &3"
                                  , chr(10)
                                  , (if t-excel then 'excel':U else '')
                                  , (if t-xml then 'xml':U else '')).
      end.
      when 52 then do:
        assign
        frame F-Main
