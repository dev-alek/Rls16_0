...
define variable vss-include-info3 as character no-undo format "x(65)":U
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  ...
  ,output dexcise
    ...
    if ptrlprop-expptrl = 'volume':U then do:
      assign
        v-qnty-type = "base":U
    ...
    find first bf_doc-line-sum no-lock
      where bf_doc-line-sum.doc-code  = bf_trn-doc.doc-code
        ...
        and bf_doc-line-sum.sum-type  = substitute( "&1&2&3&2&4", "mterr":U, chr(4), v-qnty-type, bf_rvs-line.pl-code )
      no-error .
    ...
    find first bf_doc-line-sum no-lock
      where bf_doc-line-sum.doc-code  = bf_trn-doc.doc-code
        and bf_doc-line-sum.gds-code  = bf_rvs-line.gds-code
        and bf_doc-line-sum.sum-type  = substitute( "&1&2&3&2&4", 'wst':U, chr(4), v-qnty-type, bf_rvs-line.pl-code )
      no-error .
    if available bf_doc-line-sum then do:
      assign
    ...
    put stream s-out unformatted
      ...
      '            :        :                     :            :        :                     :            :        :            :        :' +
      string( OutQty( dLoss-qnty,  yes ),                   "x(12)":U ) + ":" +
      string( OutSum( dLoss-sum,   yes ),                   "x(7)":U  ) + ":" +
      string( OutQty( dNorm-qnty,  yes ),                   "x(12)":U ) + ":" +
      string( OutSum( dNorm-sum,   yes ),                   "x(8)":U  ) + ":" +
    ...
    run r-orsvxl-write-line-data in this-procedure
      ...
      , input OutDec( dLoss-qnty,  yes )
      , input OutDec( dLoss-sum,   yes )
      , input OutDec( dNorm-qnty,  yes )
      , input OutDec( dNorm-sum,   yes )
