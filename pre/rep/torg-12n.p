...
DEFINE FRAME f-doc
        ...
        ub.goods.gds-name COLUMN-LABEL "Наименование товара! ! ! ! " format "X(30)" space(0)
...
DEFINE FRAME f-doc-m
        ...
        ub.goods.artic COLUMN-LABEL "Артикул! ! ! ! " format "X(17)" space(0)
        sym19 column-label ":!:!:!:!:" format "X(1)" space(0)
        ub.goods.gds-name COLUMN-LABEL "Наименование товара! ! ! ! " format "X(52)" space(0)
        sym3 column-label ":!:!:!:!:" format "X(1)" space(0)
        tb-code COLUMN-LABEL "Код товара! ! ! ! " format "X(13)" space(0)
        sym4 column-label ":!:!:!:!:" format "X(1)" space(0)
        ub.goods.unit-base COLUMN-LABEL "Наим!ед.!изм.! ! " format "X(4)" space(0)
...
assign
    ...
    v-gds-name-length   = ( if v-torgconf-outt12 = yes then 52 else 30 )
.
if v-torgconf-outnum = yes
then do:
    assign
...
then do:
    ...
    assign v-doc-date-string =  ( if t-doc.status_ <> 'факт':U or v-print-doc = "yes"
...
PUT STREAM Out-Stream
    space(10) string( "Всего на сумму: " + trim( string( ( v-sum-stoim + v-sum-SLT-gds ), "->>>,>>>,>>>,>>>,>>9.99") ) + " (" +
                                trim( ( if Invers then ub.currency.curr-abbr else ( if PrintRubl then "РУБ" else "баз.вал" ) ) ) + ")" ) format "X(180)"
.
...
if page-number( out-stream ) > prevpage then
    ...
    if ( NOT can-do( '_Пустая шкала':U, ub.gds-prt.node-name ) ) AND ( NOT Invers )
    then do:
            if PrintScale then
                do:
                    if v-torgconf-outt12 = yes
...
initial "@(#)$Workfile$ $Revision$".
    ...
    then do:
        ...
        :
            assign
                gds-str = gds-str2
                gds-str1 = breakstr(gds-str, 52, input-output gds-str1, input-output gds-str2)
            .
            if v-torgconf-outt12 = yes
            then do:
                display stream out-stream
...
:
    FIND ub.currency WHERE ub.currency.curr-code = t-doc.exch-code NO-LOCK.
    assign val-str = ( if Invers then ub.currency.curr-abbr else ( if PrintRubl then "рублях" else "баз.вал" ) ).
    ...
    PUT STREAM Out-Stream
        ...
        space(5) ( if t-doc.doc-type <> 'при':U then
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input t-doc.doc-code ,
    ...
    put stream out-stream
        space(5) "дата" format "X(4)" AT 175 "| " AT 180 v-torgconf-vdoc-date "|" AT 198 skip
        space(5) "Вид операции" format "X(12)" at 167 "| " at 180
                        (if t-doc.ext-doc-type = 'ep':U
                        then ( if lookup( "Mari":U, p-mode ) = 0
                            then "возврат пост-ку"
                            else "":U )
                        else ( if t-doc.doc-type = 'при':U and not Invers
                            then " приход"
