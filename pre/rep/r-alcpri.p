...
define temp-table temp_p-fmt_string-part no-undo
    ...
    index pi is primary unique
        str-key
.
define variable v-p-fmt-41-str-key    as integer      no-undo.
...
:
    ...
    :
        ...
        assign
            v-p-fmt-41-str-key = v-p-fmt-41-str-key + 1
        .
        assign
            buf_temp_p-fmt_string-part.str-key      = v-p-fmt-41-str-key
...
define frame f-doc
  ...
  temp-str.gds-name column-label "Наименование! " format "X(40)" space(0)
  sym3 column-label ":!:!" format "X(1)" space (0)
  temp-str.doc-code column-label "Документ!прихода" format "X(14)" space(0)
...
header
  ...
  v-line format "X(153)" at 1
with width 160 down stream-io .
form header
        v-line format "X(153)" at 1 SKIP
...
End.
  ...
  find first g#post-f no-lock
    where g#post-f.obj-code = buf_trn-doc.cli-code
      and g#post-f.obj-type = buf_trn-doc.cli-type
      no-error .
  if p-RADpost = 1 or available g#post-f then do:
  ...
  if error-status:error then do:
    message substitute ("Не найдена запись по типу &1 для товара &2 &3 &4 в документе &5. ", 'основная_сумма':U, buf_doc-line.artic, buf_doc-line.prod-type, buf_doc-line.prod-code, buf_doc-line.doc-code) view-as alert-box error .
  end.
  create temp-str.
  assign
...
End.
  ...
  for each buf_trn-doc no-lock
      ...
      each buf_doc-line no-lock
        where buf_doc-line.doc-code    = buf_trn-doc.doc-code
          and buf_doc-line.artic       = temp-gds.artic
          and buf_doc-line.prod-type   = temp-gds.prod-type
          and buf_doc-line.prod-code   = temp-gds.prod-code
  :
  assign
    v-counter = v-counter + 1
  .
...
End.
  ...
  assign
    ...
    temp-str.prod-type    = buf_doc-line.prod-type
    temp-str.prod-code    = buf_doc-line.prod-code
    temp-str.gds-code     = temp-gds.gds-code
...
End.
  ...
  assign
    ...
    temp-str.prod-type    = buf_doc-line.prod-type
    temp-str.prod-code    = buf_doc-line.prod-code
    temp-str.gds-code     = gds-list.gds-code
...
:
  ...
  put stream out-stream
    v-rep-title at center-field( 1 , 153 , length(v-rep-title) ) format "X(50)" skip
    v-rep-date  at center-field( 1 , 153 , length(v-rep-date) ) format "X(50)" skip(1)
  .
  assign
    ...
    "Дата поставки"  + chr(44) +
    "Количество" + chr(44) +
    "Цена" + chr(44) +
    "Сумма без НДС" + chr(44) +
    "Сумма с НДС"
    sheetf.sizes =
    "20"  + chr(44) +
    "40"  + chr(44) +
...
:
  ...
  if Make-Excel then  put   stream ForExcel unformatted
    temp-str.gds-code                           CHR(9)
    temp-str.gds-name                           CHR(9)
    temp-str.doc-code                           CHR(9)
    temp-str.fact-date    format "99.99.99"     CHR(9)
    temp-str.fact-qnty                          CHR(9)
    temp-str.price        format ">>>>>>>>9.99" CHR(9)
    temp-str.sum-noNDS    format ">>>>>>>>9.99" CHR(9)
    temp-str.sum-withNDS  format ">>>>>>>>9.99" CHR(9)
  skip.
end.
end procedure.
...
:
    ...
    put stream out-stream v-line at 1 format "X(153)" skip .
    display stream out-stream
    ...
    put stream out-stream v-line at 1 format "X(153)" skip .
    if Make-Excel then  put   stream ForExcel unformatted
      temp-str.cli-code     CHR(9)
      temp-str.cli-name     CHR(9)
      " "                   CHR(9)
      " "                   CHR(9)
      " "                   CHR(9)
      " "                   CHR(9)
...
:
    ...
    if p-sub-total-qnty > 0 then do:
        ...
        down stream out-stream with frame f-doc.
        if Make-Excel then  put   stream ForExcel unformatted
          "Итого по товарам"                            CHR(9)
          " "                                           CHR(9)
          " "                                           CHR(9)
          " "                                           CHR(9)
          p-sub-total-qnty                              CHR(9)
          " "                                           CHR(9)
          p-sub-total-sum-noNDS   format ">>>>>>>>9.99" CHR(9)
          p-sub-total-sum-withNDS format ">>>>>>>>9.99" CHR(9)
        skip(1).
    end.
end.
end procedure.
...
:
  ...
  put stream out-stream v-line at 1 format "X(153)".
  display stream out-stream
    sym1 sym2 sym8 sym9
  ...
  if Make-Excel then  put   stream ForExcel unformatted
  skip(2)
    "Итого по поставщикам"                    CHR(9)
    " "                                       CHR(9)
    " "                                       CHR(9)
    " "                                       CHR(9)
    " "                                       CHR(9)
    " "                                       CHR(9)
    p-total-sum-noNDS   format ">>>>>>>>9.99" CHR(9)
    p-total-sum-withNDS format ">>>>>>>>9.99" CHR(9)
  skip.
end.
end procedure.
