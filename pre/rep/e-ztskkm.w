...
define variable vss-description as character no-undo init "Отчет по анализу длительности пересменка (Закрытие технологической смены на ККМ)".
...
DEFINE VARIABLE type-pos AS CHARACTER FORMAT "X(256)":U
     ...
     DROP-DOWN-LIST
     SIZE 29 BY 1 BGCOLOR 15 NO-UNDO.
DEFINE VARIABLE porog-zn AS INTEGER FORMAT "->,>>>,>>9" INITIAL 0
     LABEL "Ввод порогового значения простоя реализации"
     VIEW-AS FILL-IN
     SIZE 14 BY 1
     BGCOLOR 15  NO-UNDO.
DEFINE VARIABLE porog-zn-sv AS INTEGER FORMAT "->,>>>,>>9" INITIAL 0
     LABEL "Предел отображения чеков закрытия смены к сверке"
     VIEW-AS FILL-IN
     SIZE 14 BY 1
     BGCOLOR 15  NO-UNDO.
DEFINE FRAME F-Main
     porog-zn AT ROW 1.95 COL 50 COLON-ALIGNED WIDGET-ID 2
     porog-zn-sv AT ROW 3.38 COL 50 COLON-ALIGNED WIDGET-ID 2
     type-pos AT ROW 4.81 COL 50 COLON-ALIGNED WIDGET-ID 4
...
PROCEDURE adm-disable :
    DISABLE porog-zn porog-zn-sv type-pos WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN porog-zn porog-zn-sv type-pos WITH FRAME F-Main.
