...
FOR EACH obj-list WHERE obj-list.obj-type = 'маг':U NO-LOCK :
    ...
    shops_.obj-name = REPLACE(REPLACE(ub.clients.obj-name, chr(44), ""), p-XL-delim, "")
    .
    ...
    FOR EACH ub.inkas No-LOCK WHERE
        ...
        ACCUMULATE
        ub.trn-doc.tot-fact ( SUB-TOTAL BY ub.inkas.doc-date )
        ub.trn-doc.tot-calc ( SUB-TOTAL BY ub.inkas.doc-date )
        ub.trn-doc.tot-ov ( SUB-TOTAL BY ub.inkas.doc-date )
        ub.trn-doc.tot-sale ( SUB-TOTAL BY ub.inkas.doc-date )
        ub.trn-doc.discnt-rubl ( SUB-TOTAL BY ub.inkas.doc-date )
        .
        if available ret-doc then
        ACCUMULATE
        ret-doc.tot-fact ( SUB-TOTAL BY ub.inkas.doc-date )
        ret-doc.tot-calc ( SUB-TOTAL BY ub.inkas.doc-date )
        ret-doc.tot-ov  ( SUB-TOTAL BY ub.inkas.doc-date )
        ret-doc.tot-sale ( SUB-TOTAL BY ub.inkas.doc-date )
        ret-doc.discnt-rubl ( SUB-TOTAL BY ub.inkas.doc-date )
        .
   ...
   if can-find( first day_sum where day_sum.tot-base <> 0 ) then do:
      ...
      FOR EACH shops_ BY shops_.obj-code :
         ...
         sheetf.EXcel-Column-Lable = sheetf.EXcel-Column-Lable +
                  string( "Маг.  N " + string ( shops_.obj-code, ">>>>>>9" ) +
                  chr(44) , "x(16)" )
         sheetf.Sizes = sheetf.SIzes + "16" + chr(44)
         .
      END .
      assign
      StrBuf = StrBuf + "     ИТОГО" + fill( " ", 5 ) + p-XL-delim
      sheetf.EXcel-Column-Lable = sheetf.EXcel-Column-Lable + "     ИТОГО" + fill( " ", 5 ) + chr(44)
      sheetf.Sizes = sheetf.SIzes + "16" + chr(44)
      .
      EXPORT stream PrnLibStream StrBuf p-XL-delim.
      ASSIGN
      StrBuf = p-XL-delim + fill( " ", 6 ) + p-XL-delim
      sheetf.EXcel-Column-Lable = sheetf.EXcel-Column-Lable + chr(10) + chr(44)
      .
      FOR EACH shops_ BY shops_.obj-code :
         assign
         StrBuf = StrBuf + string( string( shops_.obj-name, "x(15)" ) + p-XL-delim , "x(16)" )
         sheetf.EXcel-Column-Lable = sheetf.EXcel-Column-Lable + string( replace(shops_.obj-name, chr(44), "":U), "x(15)" ) + chr(44)
         .
      END .
      assign
      StrBuf = StrBuf + fill( " ", 15 ) + p-XL-delim
      sheetf.EXcel-Column-Lable = EXcel-Column-Lable + chr(44)
      str3 = " "
      .
      ...
      FOR EACH day_sum BREAK BY day_sum.day BY day_sum.obj-code :
         ...
         XlStrBuf = string( day_sum.day ) + " " + CHR(9)
         .
      ...
      XLStrBuf = XLStrBuf +
               string( ACCUM TOTAL day_sum.tot-base )+ CHR(9)
      .
      EXPORT stream PrnLibStream StrBuf .
      PUT stream PrnLibStream " " SKIP.
      if Make-Excel then  put   stream ForExcel unformatted
      ...
      XLStrBuf = XLStrBuf +
               string( ACCUM TOTAL day_sum.discnt ) + CHR(9)
      .
      EXPORT stream PrnLibStream StrBuf .
      PUT stream PrnLibStream " " SKIP.
      if Make-Excel then  put   stream ForExcel unformatted
...
ReportHeader = "За " + StrBuf  + " " + ENTRY(NUm-entries(vStrBuf, " "), vStrBuf, " ") +
               chr(10) + ( if NotInc then "( сформирован НЕ ПО ВСЕМ ЧЕКАМ )" else " " ).
END PROCEDURE.
