...
define variable vss-description as character no-undo initial "сменный отчет - генерация таблицы для групп ТНП (ЮКОС лист 3)":U .
...
then do:
  ...
  assign t-3.grp-code  = 0
         t-3.serv-name = "":U
         t-3.grp-name  = ( if X-selectgood = 2 then "ИТОГО ПО ВСЕМ ВЫБРАННЫМ ГРУППАМ" else "ИТОГО ПО ВСЕМ ГРУППАМ" )
  .
  return .
end.
if X-selectgood = 2
