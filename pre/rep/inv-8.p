...
define variable vss-workfile    as character no-undo initial "$Workfile: inv-8.p $":U .
define variable vss-archive     as character no-undo initial "$Archive: rep/inv-8.p $":U .
...
define variable sum-qntyFact    as decimal initial 0     no-undo .
define variable sum-qntyBuh     as decimal initial 0     no-undo .
define variable sum-weightFact  as decimal initial 0     no-undo .
define variable sum-weightBuh   as decimal initial 0     no-undo .
define variable sum1-qntyFact   as decimal initial 0     no-undo .
define variable sum1-qntyBuh    as decimal initial 0     no-undo .
define variable sum1-weightFact as decimal initial 0     no-undo .
define variable sum1-weightBuh  as decimal initial 0     no-undo .
define variable sum2-qntyFact   as decimal initial 0     no-undo .
define variable sum2-qntyBuh    as decimal initial 0     no-undo .
define variable sum2-weightFact as decimal initial 0     no-undo .
define variable sum2-weightBuh  as decimal initial 0     no-undo .
define variable weightitem      as decimal initial 0     no-undo .
define variable PgQnty          as decimal               no-undo .
define variable PgWeight        as decimal               no-undo .
define variable PgQntyBuh       as decimal               no-undo .
define variable PgWeightBuh     as decimal               no-undo .
...
DEFINE FRAME invent-gold
      ...
      Lines_Counter        column-label "N!по!поряк!ку!":C5                   format ">>>>9" space(0)
      ...
      temp-str.weightitem  column-label " наличие!---------------! Масса !---------------!Одной ед. изм.":C15   format "->>>,>>>,>>9.99" space(0)
      sym8                 column-label " !-! !-!:"                           format "X(1)" space(0)
      sum1-weightfact      column-label "!---------------! !---------------! общ. ":C15   format "->>>,>>>,>>9.99" space(0)
      sym9                 column-label ":!:!:!:!:"                           format "X(1)" space(0)
      temp-str.qntyBuh     column-label " Числится !--------------! ! Количество ! ":C14 format "->>>>>>>9.<<<" space(0)
      sym10                column-label " !-!:!:!:"                           format "X(1)" space(0)
      weightitem           column-label " по данным !---------------! Масса !---------------!Одной ед. изм.":C15   format "->>>,>>>,>>9.99" space(0)
      sym11                column-label " !-! !-!:"                           format "X(1)" space(0)
      sum1-weightbuh       column-label " учета !---------------! !---------------! общ. ":C15   format "->>>,>>>,>>9.99" space(0)
...
for each buf_doc-line where buf_doc-line.doc-code = trn-doc.doc-code
  ...
  find first buf_goods where buf_goods.prod-type = buf_doc-line.prod-type
                       ...
                       no-error
                       .
  find first ub.units where ub.units.unit-name = buf_goods.unit-base
                      no-lock
                      no-error
                      .
...
End.
  ...
  assign temp-str.b-code     = if not available buf_prod-bc then string( v-b-code )
         ...
         temp-str.WeightItem = buf_goods.wt-base
  ...
  .
   ...
   display stream Out-Stream
         ...
         String("Всего по акту"
                ...
                + "               "
                + sym8
                + String(sum1-weightFact ,   "->>>>>>>>>9.999"    )
                + sym9
                + String(sum1-qntyBuh ,   "->>>>>>>>9.999"     )
                + sym10
                + "               "
                + sym11
                + String(sum1-weightBuh , "->>>>>>>>>9.999" )
...
do on error undo, return error return-value  :
   ...
   run rep/wp-qnty.p ( sum1-weightFact, output PropisSumall ) .
   if PropisQnty = ''
   Then PropisQnty = 'Ноль'.
   run inv8xl-write-cell-data in this-procedure (
         input "it_s_Num":U
   ...
   PUT STREAM Out-Stream
       "Итого по акту:" Skip
         ...
         "в) масса драгоценных металлов фактически: "  AT 18                               SKIP
         "лигатурная "                                 AT 25 "(" STRING(sum1-weightFact) ")" f-wp-qnty (decimal(sum1-weightFact)) FORMAT "x(90)"  SKIP
         "чистая "                                     AT 25                               SKIP
