...
procedure mdtcxl-init :
    ...
    run mdtcxl-write-cell-data in this-procedure (
          input "Sheet1_valutCode":U
        , input "0":U
    ).
    run mdtcxl-write-cell-data in this-procedure (
          input "Sheet1_columnList":U
        , input "gds_name,start_qnty,all_rcv_qnty,all_spent_qnty,end_qnty":U
    ).
    run mdtcxl-write-cell-data in this-procedure (
          input "Sheet1_columnType":U
        , input "S,D,D,D,D":U
    ).
    run mdtcxl-write-cell-data in this-procedure (
          input "Sheet1_subtotalList":U
        , input "":U
    ).
    run mdtcxl-write-cell-data in this-procedure (
          input "Sheet1_subtotalType":U
        , input "":U
    ).
end procedure.
procedure mdtcxl-sheet1-write-line-data :
    ...
    put stream excel-line UNFORMATTED
        "Sheet1":U
        CHR(9) "DTA":U
        CHR(9) subst("&1, &2", p-gds-name, p-unit-base)
        CHR(9) p-start-qnty
        CHR(9) p-all-rcv-qnty
        CHR(9) p-all-spent-qnty
        CHR(9) p-end-qnty
        chr(10)
    .
end procedure.
...
define temp-table tt-line no-undo
    ...
    field all-rcv-qnty as decimal
    field all-spent-qnty as decimal
...
with width 150 down stream-io use-text NO-BOX NO-LABELS
.
run prepare-rep.
run fill-goods.
run fill-info.
...
procedure calc:
    ...
    define variable v-has-ob        as logical no-undo.
    ...
    for each tt-line:
        ...
        find last lc_stk-line
            ...
            and lc_stk-line.cat-id = '##,##':U
            and lc_stk-line.sum-type = 'cost':U
        ...
        for each lc_ot-line no-lock
            ...
            and lc_ot-line.cat-id = '##,##':U
            and lc_ot-line.sum-type = 'cost':U
            ...
            :
                ...
                case lc_ot-line.ext-doc-type:
                   ...
                   otherwise do:
                        if lc_ot-line.ext-doc-type = 'vt':U and  lc_ot-line.fact-qnty > 0 then
                        tt-line.all-rcv-qnty = tt-line.all-rcv-qnty + lc_ot-line.fact-qnty.
                        else if lc_ot-line.ext-doc-type = 'vt':U and lc_ot-line.fact-qnty <= 0 then
                        tt-line.all-spent-qnty = tt-line.all-spent-qnty - lc_ot-line.fact-qnty.
                   end.
               end.
        end.
...
procedure format-rep:
    ...
    for each obj-list:
        ...
        run mdtcxl-write-cell-data(
            "Sheet1_it_start_qnty":U,
            v-sum-start-qnty
        ).
        run mdtcxl-write-cell-data(
            "Sheet1_it_all_rcv_qnty":U,
            v-sum-all-rcv-qnty
        ).
        run mdtcxl-write-cell-data(
            "Sheet1_it_all_spent_qnty":U,
            v-sum-all-spent-qnty
        ).
        run mdtcxl-write-cell-data(
            "Sheet1_it_end_qnty":U,
            v-sum-end-qnty
        ).
        put stream Out-Stream unformatted fill("-", 150) format "x(150)" skip.
        disp stream Out-Stream
            sym1
