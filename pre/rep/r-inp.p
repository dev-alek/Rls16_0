...
:
    ...
    put stream excel-line unformatted
        ...
        chr(9)   buf_temp_line-data.nazen
        chr(10)
    .
end.
end procedure.
...
define variable vss-include-info38 as character format "x(65)" no-undo initial "@(#)$Workfile:currsysk.i $ $Revision: $".
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  if v-torgconf-outdate = yes then assign v-doc-date-string = fill( " ", 10 )  .
    ...
    assign
        v-doc-date-string = string( buf_trn-doc.doc-date, "99.99.9999" ) + ( if buf_trn-doc.status_ = 'факт':U then " (факт " + string( buf_trn-doc.fact-date, "99.99.9999" ) + ")" else "" )
    .
  ...
  if can-do('факт':U, buf_trn-doc.status_ ) or ( can-do('накл':U, buf_trn-doc.status_) and buf_trn-doc.flag_ ) then assign is-akt = no .
  ...
  do on error undo, return error return-value :
    if can-do('факт':U, buf_trn-doc.status_ ) or ( can-do('накл':U, buf_trn-doc.status_) and buf_trn-doc.flag_ ) then do:
      ...
      if v-torgconf-outprim = no then do:
        ...
        run r-inpxl-write-cell-data in this-procedure (
            input "h_PS":U
          , input trim(replace((If (buf_trn-doc.Ps Begins "@") Then buf_trn-doc.Ps Else " "), chr(10), " "),"@ ")
        ).
      end.
      ...
      run r-inpxl-write-cell-data in this-procedure (
          input "h_cliFrom":U
        , input (if buf_trn-doc.obj-type = 'скл':U then "на склад" else "в магазин") + Our_Object.obj-name
      ).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prtat in g#library
  ...
  ,output empty-scale
    ...
    if empty-scale = no and PrintScale = yes then do:
      display stream out_stream sym1  Lines_Counter sym2 tb-code sym3 buf_goods.artic sym4 gds_name sym5 buf_goods.unit-base sym6 sym7 sym8 sym9 with frame akt.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output b-code
      ...
      display stream out_stream sym1 Lines_Counter sym2 tb-code sym3 buf_goods.artic sym4 gds_name sym5 buf_goods.unit-base sym6  Price sym7 qnty sym8 (Price - qnty) @ stoim sym9  with frame akt.
      down stream out_stream with frame akt .
      run r-inpxl-write-line-data in this-procedure (
                  input Lines_Counter
                , input tb-code
