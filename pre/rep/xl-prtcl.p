...
if prn-cost-price then
    do:  assign  sheetf.excel-column-lable =
            "Артикул,Наименование товара, Кол-во, Ед. изм., " +
              ...
              string( "Цена реализации (" + (if var-report-r-b = "rubl" then "РУБ" else "баз.вал" ) + ")" )  + "," +
              "Коэфф надбавки" + ","
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curobjdt in g#library
  ...
  run ostatok  (    input  ? ,    input  ? ,    input  false   ,    input  v-today ,    input  ""        ,
    input  ?          ,    input  ?          ,    input  'crsa':U ,    input  '##,##':U ,    input  false     ,
    ...
    if prn-cost-price then
        do:
            if Make-Excel then  put   stream ForExcel unformatted
                ...
                excel-format-dec-to-char(qnty)               CHR(9)
                gds-list.unit-base                           CHR(9)
                excel-format-dec-to-char(cost-base)          CHR(9)
                excel-format-dec-to-char(price)              CHR(9)
                round( price /
        ...
        do:
            if Make-Excel then  put   stream ForExcel unformatted
                ...
                excel-format-dec-to-char(qnty)               CHR(9)
                gds-list.unit-base                           CHR(9)
                excel-format-dec-to-char(price)              chr(10)
                .
        end.
end .
...
if Make-Excel then  put   stream ForExcel unformatted chr(10) chr(10) CHR(9) "Утверждаю," chr(10)  chr(10)
 CHR(9) "Финансовый директор" chr(10)
 CHR(9) v-host-name CHR(9)  "___________" CHR(9) '"__"_________20___г.'  chr(10)
 CHR(9) CHR(9) CHR(9) CHR(9) CHR(9) "М.П."  chr(10).
