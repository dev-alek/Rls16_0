...
procedure end-proc :
 ...
 :
  ...
  define temp-table temp-gds-obj no-undo
...
End.
   if v-button-stop then  DO:
        ...
        if buf_doc-line.ext-doc-type = 'ie':U or
          ...
          if not available temp-goods then do:
            ...
            assign
              ...
              temp-goods.p-spis    = 0
            .
            if buf_doc-line.ext-doc-type = 'ie':U  then assign temp-goods.p-ves = buf_doc-line.wt-brutto / buf_doc-line.fact-qnty .
  ...
  for each temp-gds-obj :
    for each buf_doc-line no-lock
      where buf_doc-line.obj-type  = temp-gds-obj.obj-type
        ...
        and buf_doc-line.fact-order >= v-fact-order-start
        and buf_doc-line.fact-order < v-fact-order-end
        and ( buf_doc-line.ext-doc-type  = 'ep':U       or
              buf_doc-line.ext-doc-type  = 'vt':U                or
              buf_doc-line.ext-doc-type  = 'vp':U           or
              buf_doc-line.ext-doc-type  = 'we':U          or
              buf_doc-line.ext-doc-type  = 'es':U     or
              buf_doc-line.ext-doc-type  = 'ee':U          or
              buf_doc-line.ext-doc-type  = 'rs':U or
              buf_doc-line.ext-doc-type  = 're':U )
      :
      for each buf_parts no-lock
        where buf_parts.out-code  = buf_doc-line.doc-code
          and buf_parts.obj-type  = buf_doc-line.obj-type
...
End.
   if v-button-stop then  DO:
        ...
        case buf_doc-line.ext-doc-type :
          when 'ep':U  then assign  temp-goods.p-ost  = temp-goods.p-ost  - buf_parts.fact-qnty .
          when 'vt':U           then assign  temp-goods.p-spis = temp-goods.p-spis - buf_parts.fact-qnty .
          when 'vp':U      then assign  temp-goods.p-spis = temp-goods.p-spis - buf_parts.fact-qnty .
          when 'we':U     then assign  temp-goods.p-spis = temp-goods.p-spis + buf_parts.fact-qnty .
          when 'es':U     or
          when 'ee':U     then assign  temp-goods.p-prod = temp-goods.p-prod + buf_parts.fact-qnty .
          when 'rs':U or
          when 're':U then assign  temp-goods.p-prod = temp-goods.p-prod - buf_parts.fact-qnty .
        end.
      end.
    end.
  end.
...
output stream OutStream to value( string( session:temp-directory +
  ...
  for each temp-goods break by temp-goods.gtd-date by temp-goods.gtd-code by temp-goods.gds-name by temp-goods.artic :
    ...
    if first-of(temp-goods.gtd-date ) then do:
      ...
      put stream outstream      "|"  "|" at 8  str format "X(26)"   "|"   at 35  "|"   at 50
         "|"   at 83     "|"   at 96     "|"   at 112    "|"   at 128
         "|"   at 144     "|"   at 162     "|"   at 171    "|"   at 180    "|"   at 198  skip
      .
    end.
    run  PrintString in this-procedure .
  end.
  PUT stream OutStream
    "|"    Line format "X(196)"   "|"    skip
...
procedure PrintString :
  ...
  :
    ...
    put stream outstream
      "|"   ii                                          format ">>>>>9"
      "|"   temp-goods.gtd-code                         format "X(26)"
      "|"   temp-goods.artic                            format "X(14)"
      "|"   temp-goods.gds-name                         format "X(32)"
      "|"   temp-goods.nat                              format "X(12)"
      "|"   temp-goods.p-ost                            format "->>,>>>,>>9.999"
      "|"   (temp-goods.p-ost * temp-goods.p-ves)       format "->>,>>>,>>9.999"
      "|"   temp-goods.p-prod                           format "->>,>>>,>>9.999"
      "|"   temp-goods.p-spis                           format "->>,>>>,>>9.999"
      "|"   at 162
      "|"   at 171
      "|"   at 180  ost-end   format "->>,>>>,>>9.999"
      "|"   at 198  skip
    .
    assign
      v-row = v-row + 1
...
procedure PutColumnTitul :
  ...
  on error undo, return error return-value
  :
    put stream outstream skip Line format "X(198)" skip
        "|"                  "№ п/п"                       format "X(6)"
        "| "   at 8  "№ ГТД"                       format "X(6)"
        "| "   at 35  "Артикул"                     format "X(10)"
        "| "   at 50 "Краткое наименование товара" format "X(30)"
        "| "   at 83  "Статус "                     format "X(10)"
        "|"    at 96  "   Кол-во в"                 format "X(15)"
        "| "   at 112  "Вес брутто"                  format "X(11)"
        "|"    at 128  "Реализованное"               format "X(15)"
        "|"    at 144  "Товары, в отнош. кот-х тамож. режим"  format "X(35)"
        "|"    at 180 "Остаток товаров"             format "X(15)"
        "|"    at 198 skip
        "| "
        "| "   at 8
        "| "   at 35
        "| "   at 50
        "| "   at 83  "товаров"                              format "X(10)"
        "|"    at 96  " ед. измер."                          format "X(15)"
        "| "   at 112
        "|"    at 128  "кол-во"                                format "X(15)"
        "|"    at 144  "маг. беспошл. торг. изменен на иной"   format "X(35)"
        "|"    at 180 "кол-во"                                format "X(15)"
        "|"    at 198 skip
        "| "
        "| "   at 8
        "| "   at 35
        ...
        "| "   at 112
        "|"    at 128
        "|"    at 144  "Кол-во в"                  format "X(11)"
        "|"    at 162  "тамож"                     format "X(8)"
        "|"    at 171  "№ ГТД"                     format "X(8)"
        "|"    at 180 "в ед. изм."                     format "X(15)"
        "|"    at 198 skip
        "| "
        "| "   at 8
        "| "   at 35
        ...
        "| "   at 112
        "|"    at 128
        "|"    at 144  "ед. измер."                format "X(15)"
        "|"    at 162  "режим"                     format "X(8)"
        "|"    at 171
        "|"    at 180
        "|"    at 198 skip
        "|"    Line format "X(196)"   "|"    skip
