        ...
        do iNum = 1 to v-InfoSectionsTotal:SectionNum:
         ...
         run aktpq-xl-write-cell-data in this-procedure (
          ...
          if v-availrvsline then do:
            run aktpq-xl-write-cell-data in this-procedure (
              input "beforelevtotal":U + string (ii)
              , input v-state-level-total
              ).
            run aktpq-xl-write-cell-data in this-procedure (
              input "beforemeasqnty":U + string (ii)
              , input v-state-measure-qnty
              ).
            assign
              v-beforemeasqnty = v-state-measure-qnty.
            run aktpq-xl-write-cell-data in this-procedure (
              input "beforestatetemp":U + string (ii)
              , input v-state-temperature
              ).
            run aktpq-xl-write-cell-data in this-procedure (
              input "beforestatedens":U + string (ii)
              , input v-state-measure-cli-qnty / v-state-measure-qnty * 1000
              ).
            run aktpq-xl-write-cell-data in this-procedure (
              input "beforestateweight":U + string (ii)
              , input v-state-measure-cli-qnty
              ).
            assign
              v-beforestateweight = v-state-measure-cli-qnty.
          end.
          find first buf_rvs-doc no-lock where buf_rvs-doc.rvs-type = 'после_док':U and buf_rvs-doc.out-code = buf_trn-doc.doc-code no-error.
          assign
            v-state-level-total = 0
            v-state-measure-qnty = 0
            v-state-measure-cli-qnty = 0
          ...
          if v-availrvsline then do:
            run aktpq-xl-write-cell-data in this-procedure (
              input "afterlevtotal":U + string (ii)
              , input v-state-level-total
              ).
            run aktpq-xl-write-cell-data in this-procedure (
              input "aftermeasqnty":U + string (ii)
              , input  v-state-measure-qnty
              ).
            run aktpq-xl-write-cell-data in this-procedure (
              input "afterstatetemp":U + string (ii)
              , input v-state-temperature
              ).
            run aktpq-xl-write-cell-data in this-procedure (
              input "afterstatedens":U + string (ii)
              , input v-state-measure-cli-qnty / v-state-measure-qnty * 1000
              ).
            run aktpq-xl-write-cell-data in this-procedure (
              input "afterstateweight":U + string (ii)
              , input v-state-measure-cli-qnty
              ).
            assign
          ...
          run aktpq-xl-write-cell-data in this-procedure (
            input "natural_loss":U + string (ii)
            , input string (v-natural_loss)
            ).
          if v-diff_mass > 0 then do:
            run aktpq-xl-write-cell-data in this-procedure (
              input "surpluse":U + string (ii)
...
output stream out-stream to value( string( session:temp-directory +
    ...
    if available buf_clients then do:
      ...
      find first ub.clients where ub.clients.obj-code = buf_clients.host-code and ub.clients.obj-type = 'орг':U no-error.
      run aktpq-xl-write-cell-data in this-procedure (
            input "firmname":U
    ...
    run aktpq-xl-write-cell-data in this-procedure (
          input "factmonth":U
        , input entry( month( buf_trn-doc.fact-date ) , 'января,февраля,марта,апреля,мая,июня,июля,августа,сентября,октября,ноября,декабря':U )
    ).
    run aktpq-xl-write-cell-data in this-procedure (
          input "factyear":U
    ...
    find first buf_clients where buf_clients.obj-code = buf_trn-doc.wrkr and buf_clients.obj-type = 'чел':U no-error.
    if available buf_clients then do:
      run aktpq-xl-write-cell-data in this-procedure (
            input "oper":U
          , input buf_clients.obj-name
    ...
    find first buf_clients where buf_clients.obj-code = buf_trn-doc.boss and buf_clients.obj-type = 'чел':U no-error.
    if available buf_clients then do:
      run aktpq-xl-write-cell-data in this-procedure (
            input "chiefoper":U
          , input buf_clients.obj-name
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_trn-doc.doc-code ,
    ...
    if v-trndocattr <> "" then do:
      ...
      run aktpq-xl-write-cell-data in this-procedure (
            input "ddovmonth":U
          , input entry( month( date(v-trndocattr) ) , 'января,февраля,марта,апреля,мая,июня,июля,августа,сентября,октября,ноября,декабря':U )
      ).
      run aktpq-xl-write-cell-data in this-procedure (
            input "ddovyear":U
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_trn-doc.doc-code ,
                       ...
                       output v-trndocattr ,
                       output v-type )  .
    v-trndocattr = replace (v-trndocattr, chr(4), ",").
    run aktpq-xl-write-cell-data in this-procedure (
          input "sealscondition":U
        , input v-trndocattr
    ).
    v-trndocattr = "".
    run aktpq-xl-write-cell-data in this-procedure (
...
procedure cr-end-general:
    ...
    if v-time-start-min <> 0 and v-time-start-min <> ? then do:
      ...
      .
      run aktpq-xl-write-cell-data in this-procedure (
            input "acceptbeginmin":U
          , input v-time-start-min mod 60
      ).
      run aktpq-xl-write-cell-data in this-procedure (
            input "acceptbeginhour":U
    ...
    if v-time-end-max <> 0 and v-time-end-max <> ? then do:
      ...
      .
      run aktpq-xl-write-cell-data in this-procedure (
            input "acceptendmin":U
          , input v-time-end-max mod 60
      ).
      run aktpq-xl-write-cell-data in this-procedure (
            input "acceptendhour":U
