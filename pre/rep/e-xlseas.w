...
PROCEDURE adm-disable :
    DISABLE RECT-1 button-prod E-prod WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 button-prod E-prod WITH FRAME F-Main.
...
FOR EACH obj-list :
    ...
    objects.obj-name = REPLACE(REPLACE(ub.clients.obj-name, chr(44), ""), p-XL-delim, "")
    .
    ...
    if can-find( FIRST ub.trn-doc WHERE
                     ...
                     ( ub.trn-doc.doc-type = 'рас':U OR trn-doc.doc-type = 'возврат':U ) AND
                       ub.trn-doc.status_ = 'факт':U AND
                       ub.trn-doc.fact-date >= vStartPoint AND
                       ub.trn-doc.fact-date <= vEndPoint AND
                       ub.trn-doc.ext-doc-type <> 'ep':U AND
                       ub.trn-doc.discnt-type <> 'касс':U ) then
        FOR EACH ub.trn-doc WHERE
                 ub.trn-doc.obj-type = obj-list.obj-type AND
                 ub.trn-doc.obj-code = obj-list.obj-code AND
                 ub.trn-doc.internal = no AND
               ( ub.trn-doc.doc-type = 'рас':U OR ub.trn-doc.doc-type = 'возврат':U ) AND
                 ub.trn-doc.status_ = 'факт':U AND
                 ub.trn-doc.fact-date >= vStartPoint AND
                 ub.trn-doc.fact-date <= vEndPoint AND
                 ub.trn-doc.ext-doc-type <> 'ep':U AND
                 ub.trn-doc.discnt-type <> 'касс':U NO-LOCK :
            PROCESS EVENTS .
                ...
                if available ub.goods then do:
                    ...
                    if available grp_sum then do:
                        ...
                        grp_sum.tot-sum = grp_sum.tot-sum +
                                         ( if can-do( 'рас':U, ub.trn-doc.doc-type )
                                          then ( ACCUM TOTAL
                                                (if v-curr-r-b = 'base':U
                                                then ub.gds-dtl.price-base
                                                else ub.gds-dtl.price-rubl )
...
if can-find( first grp_sum where grp_sum.tot-sum <> 0 ) then do:
    ...
    sheetf.Excel-COlumn-Lable = string( string( "Группа товаров", "x(49)" ) + chr(44) )
    sheetf.sizes = "49" + chr(44)
    .
    FOR EACH objects BY objects.obj-type BY objects.obj-code :
        ...
        sheetf.Excel-Column-Lable = sheetf.Excel-COlumn-Lable + string( caps( trim( objects.obj-type ) ) + ".  N " +
                 string ( objects.obj-code, ">>>>>>9" ) + chr(44) , "x(16)" )
    ...
    FOR EACH objects BY objects.obj-type BY objects.obj-code :
        ...
        sheetf.Excel-COlumn-Lable = sheetf.Excel-COlumn-Lable +  string( replace(objects.obj-name, chr(44), "":U), "x(16)" ) + chr(44)
        .
    END .
    assign
    ...
    FOR EACH grp_sum BREAK BY grp_sum.grp-name
      ...
      XLStrBuf = string( string( grp_sum.grp-name, "X(49)" ) + CHR(9))
      .
   ...
   XLStrBuf = string("ИТОГО:", "X(49)") + CHR(9)
   .
