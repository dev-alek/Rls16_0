  ...
  if itog-contract then do:
    for each temp-doc break by temp-doc.cli-type by temp-doc.cli-code by temp-doc.contr :
      if first-of(temp-doc.cli-code) and itog-comp = no then do:
        ...
        PUT STREAM PrnLibStream string("| Контрагент: " + temp-cli.cli-name) format "X(89)" "|" at 120 skip .
        PUT STREAM PrnLibStream "| Начальный остаток по контрагенту: " format "X(64)"  "|" at 95 temp-cli.sum format "->>>,>>>,>>>,>>>,>>9.99" "|" at 120 skip .
      ...
      if first-of(temp-doc.contr) and itog-comp = no  then do:
        ...
        PUT STREAM PrnLibStream string("| Договор: " + temp-contr.contr-name) format "X(89)" "|" at 120  skip .
        PUT STREAM PrnLibStream "| Начальный остаток по договору: " format "X(64)"  "|" at 95 temp-contr.sum format "->>>,>>>,>>>,>>>,>>9.99" "|" at 120 skip .
        assign v-sum1 = temp-contr.sum .
      end.
      run prn-line in this-procedure .
      if last-of(temp-doc.contr)  then do:
        PUT STREAM PrnLibStream string("| Всего по договору: " + temp-contr.contr-name) format "X(64)" "|" at 95 v-sum1 format "->>>,>>>,>>>,>>>,>>9.99" "|" at 120 skip .
      end.
      if last-of(temp-doc.cli-code)  then do:
        PUT STREAM PrnLibStream Line format "X(113)" skip .
        PUT STREAM PrnLibStream string("| Всего по контрагенту: " + temp-cli.cli-name) format "X(64)" "|" at 95 v-sum format "->>>,>>>,>>>,>>>,>>9.99"  "|" at 120 skip  .
        PUT STREAM PrnLibStream Line format "X(113)" skip .
      end.
    end.
  end.
...
procedure prn-line :
  do on error undo, return error return-value :
    ...
    if temp-doc.contr-type = 'рас':U then do:
      ...
      if temp-doc.typ = 0 then
        ...
        if temp-doc.styp = 'ппп':U or temp-doc.styp = 'пко':U or temp-doc.styp = 'апп':U  then
    ...
    if itog-comp = no then do:
      ...
      put stream PrnLibStream
        "|"  At 1   temp-doc.dat     Format "99/99/9999"
        "|"  At 13   temp-doc.styp    Format "X(3)"
        "|"  At 17   temp-doc.num     Format "X(20)"
        "|"  At 47   temp-doc.nakl    Format "X(20)"
        "|"  At 70   temp-doc.sum     Format "->>>,>>>,>>>,>>>,>>9.99"
        "|"  At 95   v-sm             Format "->>>,>>>,>>>,>>>,>>9.99"
        "|"  At 120
      skip .
    end.
  end.
...
procedure is-page :
  ...
  :
    if line-counter( PrnLibStream ) + 2 > page-size( PrnLibStream ) then do:
      put stream PrnLibStream  skip Line format "X(113)" skip "продолжение - на следующей странице" AT 30 SKIP .
...
procedure PrintTitul :
  ...
  :
    ...
    put stream PrnLibStream
      skip  Line format "X(113)"  skip
      "|"           " Дата"     format "X(5)"
      "|" at 13  "Тип"       format "X(3)"
      "|" at 17  " № док-та" format "X(10)"
      "|" at 47  " № наклад" format "X(10)"
      "|" at 70  string(" Сумма (" + s-val + ")")  format "X(15)"
      "|" at 95  string(" Сальдо (" + s-val + ")") format "X(17)"
      "|" at 120
      skip  Line format "X(113)"  skip
    .
  end.
