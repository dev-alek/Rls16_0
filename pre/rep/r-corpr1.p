 ...
 .
  ...
  for each obj-list no-lock :
    ...
    for each buf_trn-doc no-lock
      ...
      :
      if buf_trn-doc.ext-doc-type <> 'pc':U then next .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output gds-prop.b-code
          ...
          if is_rem = ? then do:
            ...
            if (units.type = "дро,2ед"  or  units.type = "дро,доп" ) then
...
if session :set-wait-state( "compiler" ) then.
  ...
  assign  val-str =  if x-SET_val_TYPE = 1 then "рублях" else base-type .
  ...
  put stream PrnLibStream
    ...
    space(5) string( "ИНН " + t-inn + " " + CAPS( clients.obj-name ) + " (" + string(clients.obj-code) + ")" + t-addres + t-phone) format "X(160)"
  ...
  .
  if is_rem = yes then find first clients no-lock where clients.obj-type = 'орг':U     and clients.obj-code = v-cntxt-host-code-obj  .
  ...
  put stream PrnLibStream
    space(5) string( "Грузоотправитель: " + "ИНН " + t-inn + " " + CAPS( clients.obj-name ) + " (" + string(clients.obj-code) + ") "
  ...
  else                 find first clients no-lock where clients.obj-type = 'орг':U     and clients.obj-code = v-cntxt-host-code-obj .
    run fmtcli-get-bank in this-procedure (
          input v-cntxt-host-code-obj
        , input clients.obj-type
        , input clients.obj-type
  ...
  put stream PrnLibStream
    ...
    " (" trim( (  if x-SET_val_TYPE = 1 then "РУБ" else base-type ) ) format "X(6)" ")"
  .
  ...
  if v-tax-sum <> 0 then do:
    put stream PrnLibStream space(21) v-tax-name + ":" + fill(" ", 1) + trim(string(v-tax-sum, "->>>,>>>,>>>,>>>,>>9.99"))
                       + " (" + trim( ( if x-SET_val_TYPE = 1 then "РУБ" else base-type ) ) + ")" format "X(160)"  .
  end.
    define variable v-doc-places    as character    no-undo.
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input tdoc-code ,
                        input 'QntyPlace':U ,
                       output v-doc-places ,
                       output v-attr-type )  .
    if v-doc-places = "":U
  ...
  put stream PrnLibStream  skip
    space(30) string( "НДС: " + trim( string( all-VAT-gds, "->>>,>>>,>>>,>>>,>>9.99") ) +
                                " (" + trim( ( if x-SET_val_TYPE = 1 then "РУБ" else base-type ) ) + ")" ) format "X(160)" skip(2)
