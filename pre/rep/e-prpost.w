...
PROCEDURE adm-disable :
    DISABLE RECT-9 RECT-5 RECT-6 RECT-7 RECT-8 RADPost PostName SortType Tog-obj Classify date1Rash date2Rash Showcost Show-cost-Vat Showsale type-stor Cli-art SumsOnly Str WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-9 RECT-5 RECT-6 RECT-7 RECT-8 RADPost PostName SortType Tog-obj Classify date1Rash date2Rash Showcost Show-cost-Vat Showsale type-stor Cli-art SumsOnly Str WITH FRAME F-Main.
...
assign Showsale.
  if showsale then DO:
                  ...
                  enable Show-sale-Vat  with frame F-Main.
                  End.
              else do:
                                     Show-sale-Vat = no.
                   enable  type-stor with frame F-Main.
                   disable Show-Sale-vat  with frame F-Main .
                    display Show-Sale-vat  with frame F-Main.
                   end.
END.
...
DO:
  ...
  if tog-obj = false then do:
    assign
      ...
      tog-Lavel   = false
      .
      display Classify SumsOnly type-stor   var-Lavel-2 var-Lavel tog-Lavel-2 tog-Lavel with frame F-Main.
      disable Classify SumsOnly type-stor  var-Lavel-2 var-Lavel tog-Lavel-2 tog-Lavel  with frame F-Main.
  End.
  Else do:
    display Classify SumsOnly type-stor with frame F-Main.
    enable Classify SumsOnly type-stor  with frame F-Main.
  End.
END.
...
run create_obj-list in this-procedure
 ( input v-cntxt-obj-type ,
   input v-cntxt-obj-code )
   .
str:screen-value in frame F-Main       = 'Период для РН'.
date1Rash:screen-value in frame F-Main = string(x-Date-Start,"99/99/99").
date2Rash:screen-value in frame F-Main = string(x-Date-End,"99/99/99")   .
END PROCEDURE.
PROCEDURE my-report :
