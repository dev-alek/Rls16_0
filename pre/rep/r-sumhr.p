        ...
        FOR EACH obj-list No-LOCK:
          If byobject then v-obj-code = obj-list.obj-code.
           ...
           for EACH chk-doc WHERE
             ...
             :
             if lookup(string(chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) > 0 then next _chk-doc2.
      ...
      when yes then do:
        FOR EACH obj-list NO-LOCK:
          ...
          for EACH chk-doc WHERE
            ...
            if lookup(string(chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) > 0 then next _chk-doc4.
            if rs-dis-card = 1 then do:
              FIND FIRST ub.dis-card No-LOCK WHERE
                ub.dis-card.d-card = ub.chk-doc.d-card no-error .
                if not available ub.dis-card then next.
...
assign
grp-h.sum[ii-sec + 1]     = grp-h.sum[ii-sec + 1]  + (if v-curr-r-b = 'base':U then chk-pay.tot-base else chk-pay.tot-rubl)
grp-h.num-chk[ii-sec + 1] = grp-h.num-chk[ii-sec + 1] + 1
.
if v-obj-code > 0 then
assign
tot_grp-h.sum[ii-sec + 1]     = tot_grp-h.sum[ii-sec + 1]  + (if v-curr-r-b = 'base':U then chk-pay.tot-base else chk-pay.tot-rubl)
tot_grp-h.num-chk[ii-sec + 1] = tot_grp-h.num-chk[ii-sec + 1] + 1
.
PROCESS EVENTS .
