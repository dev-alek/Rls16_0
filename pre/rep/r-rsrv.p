  ...
  do:
   ...
   v-button-stop = false .
    ...
    assign
        ...
        v-r-rsrv-single-line   = fill( "-", 195 )
        v-r-rsrv-double-line   = fill( "=", 195 )
...
output stream out-stream to value( string( session:temp-directory +
        ...
        form header
            space(2) v-r-rsrv-single-line format "X(195)"
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run docextnm in g#library
  ...
  ,output v-ext-name
        ...
        .
IF ( v-r-rsrv-line-counter modulo v-account = 0 )  then DO:
 Assign
    ...
    .
          ...
          then DO:
           ...
           then do:
            DISPLAY
              v-r-rsrv-line-counter @ RecordsDone
...
:
    ...
    put stream out-stream
        skip
        space(2)
            v-r-rsrv-single-line   format "X(195)"
        skip space(2)  "|"
            "Код"                   at center-field(2 + 1, 2 + 11, 3)
            "|"                     at 2 + 11
            "Артикул"               at center-field(2 + 11, 2 + 28, 7)
            "|"                     at 2 + 28
            "Производитель"         at center-field(2 + 28, 2 + 42, 13)
            "|"                     at 2 + 42
            "Наименование товара"   at 2 + 42 + 2
            "|"                     at 2 + 82
            "Количество"            at 2 + 82 + 2
            "|"                     at 2 + 96
            "Учетная"               at 2 + 96 + 2
            "|"                     at 2 + 110
            "Сумма"                 at 2 + 110 + 2
            "|"                     at 2 + 125
            "Свободное"             at 2 + 125 + 2
            "|"                     at 2 + 139
            "Номер"                 at 2 + 139 + 2
            "|"                     at 2 + 156
            "Дата"                  at 2 + 156 + 2
            "|"                     at 2 + 168
            "Контрагент"            at 2 + 168 + 2
            "|"                     at 2 + 182
            "Дней"                  at 2 + 182 + 2
            "|"                     at 2 + 195
        skip space(2)  "|"
            "товара"                at center-field(2 + 1, 2 + 11, 6)
            "|"                     at 2 + 11
            "|"                     at 2 + 28
            "|"                     at 2 + 42
            "|"                     at 2 + 82
            ...
            "в уч.ц."               at 2 + 110 + 2
            "|"                     at 2 + 125
            "количество"            at 2 + 125 + 2
            "|"                     at 2 + 139
            "накладной"             at 2 + 139 + 2
            "|"                     at 2 + 156
            "накладной"             at 2 + 156 + 2
            "|"                     at 2 + 168
            "|"                     at 2 + 182
            "в резерве"             at 2 + 182 + 2
            "|"                     at 2 + 195
        skip space(2)
            "|" v-r-rsrv-single-line format "X(193)" "|"
    .
end.
end procedure.
...
:
    ...
    then do:
        put stream out-stream
            skip space(2)
                "|" v-r-rsrv-double-line format "X(193)" "|"
        .
    end.
    put stream out-stream
        skip space(2)
            "|"
            substitute( "Объект: &1 &2", p-obj-type, p-obj-code )  format "X(193)"
            "|"             at 2 + 195
        skip space(2)
            "|" v-r-rsrv-double-line format "X(193)" "|"
    .
...
:
    put stream out-stream
        skip space(2) "|     Итого по классификатору"
            "|"                     at 2 + 82
