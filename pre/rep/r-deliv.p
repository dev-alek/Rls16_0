...
define variable in-time-in      as character no-undo.
define variable gdscode         as integer   no-undo.
define variable goods-flag      as integer   no-undo init 0.
define variable chk-find        as integer   no-undo.
...
.
function in-time-period returns logical
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input trn-doc.doc-code ,
        ...
        if p-value = "" or p-value = "?" or p-value = ? then do:
            p-value = string(trn-doc.doc-date).
            nodate = yes.
        end.
...
stat = 'разрешен':U + "," + 'запрос':U + "," + 'накл':U + "," + 'факт':U + "," + 'закрыто':U.
if X-SelectObject <> 'все':U then do:
    for each obj-list
    :
        if customer-name = 'все':U then do:
            for each g#customer
            :
                do ii = 1 to 5:
                    for each trn-doc where trn-doc.obj-type = obj-list.obj-type
                                       and trn-doc.obj-code = obj-list.obj-code
                                       and trn-doc.ext-doc-type = 'ee':U
                                       and trn-doc.cli-type = g#customer.obj-type
                                       and trn-doc.cli-code = g#customer.obj-code
                                       and trn-doc.status_ = entry(ii, stat, ",")
                                       no-lock
                    :
...
Report:table-columns("60,60,60,60,150,120,110,90,90,70,70,60,110,140").
Report:table-types = "String,String,String,String,String,String,String,String,String,Number,Number,Number,String,String".
Report:table-header("Дата доставки|Время доставки|Номер документа|Статус документа|Адрес доставки|Наименование клиента|Контактное лицо|Контактный телефон|Подразделение|Сумма по накладной с учетом скидок|Сумма доставки|Количество мест|Описание груза|Примечание","60","4").
