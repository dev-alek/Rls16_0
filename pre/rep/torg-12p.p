...
define frame f-doc
        ...
        goods.gds-name COLUMN-LABEL "Наименование товара! ! ! ! " format "X(29)" space(0)
...
.
assign
    val-str = ( if PrintRubl then "рублях" else "баз.вал" )
.
assign tdoc-code = t-doc.doc-code.
assign tdoc-date =  ( if t-doc.status_ <> 'факт':U or v-print-doc = "yes"
                    then t-doc.doc-date
...
put stream out-stream
    ...
    space(5) "дата" format "X(4)" at 175 "| " at 180 (if t-doc.status_ = 'факт':U then t-doc.fact-date else ? ) format "99/99/9999" "|" at 198 skip
...
put stream out-stream
    space(10) "  Всего на сумму:        "
         ...
         " ("
         ( if PrintRubl then "РУБ" else "баз.вал" ) format "X(6)"
         ")"
.
...
put stream out-stream
    ...
    space(30) string( "НДС: " + trim( string( (v-itogo-sum-vat), "->>>,>>>,>>>,>>>,>>9.99") ) +
                                " (" + trim( ( if PrintRubl then "РУБ" else "баз.вал" ) ) + ")" ) format "X(160)" skip
.
...
if FullGdsName
then do:
    gds-str1 = breakstr(goods.gds-name, 29, input-output gds-str1, input-output gds-str2).
    assign v-char-counter = 0.
    do while gds-str2 <> "" :
        assign
            gds-str = gds-str2
            gds-str1 = breakstr(gds-str, 29, input-output gds-str1, input-output gds-str2)
