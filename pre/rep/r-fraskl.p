...
for each fbr-doc no-lock   where
    ...
    for each fbr-line no-lock  where fbr-line.doc-code  = fbr-doc.doc-code ,
        first fbr-recipe no-lock where fbr-recipe.recipe-code = fbr-line.recipe-code and
                                    ...
                                    (   fbr-recipe.recipe-type = 'производство':U or
                                       fbr-recipe.recipe-type = 'комплектация':U )
     :
    ...
    if  fbr-line.is-comp = true    then do:
...
on error undo, return error return-value
 :
define input parameter x-doc-code like fbr-line.doc-code no-undo .
define input parameter x-recipe like fbr-line.recipe-code no-undo .
define input parameter x-id as integer no-undo .
