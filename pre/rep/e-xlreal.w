...
DEFINE NEW shared VAR vStartPoint        as      date       NO-UNDO.
DEFINE NEW shared VAR vEndPoint          as      date       NO-UNDO.
DEFINE NEW shared VAR vStrBuf          as      char       NO-UNDO.
DEFINE NEW shared VAR vEndLastDay      as  integer     no-undo.
...
FOR EACH ub.inkas WHERE
    ...
    FOR EACH ub.gds-dtl NO-LOCK WHERE ub.gds-dtl.doc-code = ub.trn-doc.doc-code
        ...
        if NOT available day_sum then do:
            ...
            day_sum.sale-sum = ub.gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.price-base else ub.gds-dtl.price-rubl)
            day_sum.discnt-sum = gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.discnt-base else ub.gds-dtl.discnt-rubl)
            .
         ...
         day_sum.sale-sum = day_sum.sale-sum + ub.gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.price-base else gds-dtl.price-rubl)
         day_sum.discnt-sum = day_sum.discnt-sum + ub.gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.discnt-base else gds-dtl.discnt-rubl)
         .
      END.
      if available ret-doc then do:
        FOR EACH ub.gds-dtl NO-LOCK WHERE ub.gds-dtl.doc-code = ret-doc.doc-code
            ...
            if NOT available day_sum then do:
                ...
                day_sum.prod-sum = - ub.gds-dtl.doc-qnty * ub.gds-dtl.cur-base
                day_sum.sale-sum = - ub.gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.price-base else ub.gds-dtl.price-rubl)
                day_sum.discnt-sum = - ub.gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.discnt-base else ub.gds-dtl.discnt-rubl)
                .
                FOR EACH ub.chk-gds where
                        ub.chk-gds.out-code = ub.inkas.inkas-code,
                    FIRST ub.bar-code No-LOCK where
            ...
            day_sum.prod-qnty = day_sum.prod-qnty -  ub.gds-dtl.doc-qnty
            day_sum.prod-sum = day_sum.prod-sum -  ub.gds-dtl.doc-qnty * ub.gds-dtl.cur-base
            day_sum.sale-sum = day_sum.sale-sum -  ub.gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.price-base else ub.gds-dtl.price-rubl)
            day_sum.discnt-sum = day_sum.discnt-sum - ub.gds-dtl.doc-qnty * (if v-curr-r-b = 'base':U then  ub.gds-dtl.discnt-base else ub.gds-dtl.discnt-rubl)
            .
        END.
      end.
  ...
  if can-find( first day_sum where day_sum.prod-sum <> 0 ) then do:
      DO ii = 1 TO vEndLastDay :
        ...
        sheetf.Excel-COlumn-Lable = string( "   Дата", "x(15)" ) + chr(44)
        sheetf.sizes = "10" + chr(44)
        .
        FOR EACH prods BY prods.prod-attr :
          assign
          StrBuf = StrBuf + fill( " ", 12 ) + p-XL-delim +
                   string( " " + prods.prod-name, "x(17)" ) + p-XL-delim
          sheetf.Excel-Column-Lable = sheetf.Excel-Column-Lable + string(replace(prods.prod-name, chr(44), "":U), "x(17)" ) + chr(44) +
                               fill (" ", 17) + chr(44)
          sheetf.sizes = sheetf.sizes + "17" + chr(44) + "17" + chr(44)
          .
        END .
     assign
     StrBuf = StrBuf + fill( " ", 5 ) + "  " + fill( " ", 5 ) + p-XL-delim +
              ...
              " " + fill( " ", 7 ) + p-XL-delim
     sheetf.Excel-Column-Lable = sheetf.Excel-Column-Lable + fill( " ", 6 ) + "ИТОГО" + fill( " ", 6 )  + chr(44) +
                          fill( " ", 17 ) + chr(44) +
                          fill( " ", 8 ) + chr(44)
     sheetf.sizes = sheetf.sizes + "17" + chr(44) + "17" + chr(44) + "8" + chr(44)
     str2 = " "
     str3 = " "
     .
     ...
     sheetf.Excel-Column-Lable = sheetf.Excel-Column-Lable + chr(10) + fill( " ", 5 ) + "(День" + fill ( " ", 6) + chr(44)
     .
     FOR EACH prods :
        assign
        StrBuf = StrBuf + fill( " ", 5 ) + "шт" + fill( " ", 5 ) + p-XL-delim +
                 fill( " ", 4 ) + "Стоимость" + fill( " ", 4 ) + p-XL-delim
        sheetf.Excel-COlumn-Lable = sheetf.Excel-Column-Lable + string("Количество", "X(12)") + chr(44) +
                             fill( " ", 4 ) + "Стоимость" + fill( " ", 4 ) + chr(44)
        .
     END .
     assign
     StrBuf = StrBuf + fill( " ", 5 ) + "шт" + fill( " ", 5 ) + p-XL-delim +
     ...
     sheetf.Excel-Column-Lable = sheetf.Excel-COlumn-Lable + string("Количество", "X(12)") + chr(44) +
                          fill( " ", 4 ) + "Стоимость" + fill( " ", 4 ) + chr(44) +
                          " " + "Кол-во" + fill( " ", 1 ) + chr(44)
     .
     EXPORT stream PrnLibStream StrBuf .
     assign
     StrBuf = p-XL-delim + fill( " ", 15 ) + p-XL-delim
     sheetf.Excel-Column-Lable =  sheetf.Excel-Column-Lable + chr(10) + string("месяца)", "X(15)") + chr(44)
     .
     FOR EACH prods :
      assign
       ...
       then "РУБ"
               ...
               + fill( " ", 8 ) + p-XL-delim
       sheetf.Excel-Column-Lable = sheetf.Excel-Column-Lable + fill( " ", 5 ) + "шт" + fill( " ", 5 ) + chr(44) +
                            fill( " ", 6 ) +
       (if v-curr-r-b = 'rubl':U
       then "РУБ"
       else  string( caps( trim( base-type ) ), "x(3)" ))
    ...
    + fill( " ", 8 ) + chr(44) +
     ...
     FOR EACH day_sum
         ...
         XLStrBuf = string( day_sum.day_, ">>>>9" ) + fill( " ", 10 ) +  CHR(9)
         .
         ACCUMULATE
       ...
       XLStrBuf = string( "Итого( нетто )", "x(15)" ) + CHR(9)
       .
     ...
     XLStrBuf = XLStrBuf +
              string( string( ACCUM TOTAL day_sum.prod-qnty, "->>>,>>9.<<<" ), "x(12)" )
                   + CHR(9) +
              string( ( ACCUM TOTAL day_sum.prod-sum ) -
                         ( ACCUM TOTAL day_sum.discnt-sum ) , "->,>>>,>>>,>>9.99" ) + CHR(9) +
              string( ACCUM TOTAL chk-day.chk-qnty, ">>>>>>>9" ) + CHR(9)
     .
     EXPORT stream PrnLibStream StrBuf .
     PUT stream PrnLibStream "" skip .
     if Make-Excel then  put   stream ForExcel unformatted
     ...
     XLStrBuf = string( "Ошибки по кассе", "x(15)" ) + CHR(9)
     .
     FOR EACH day_sum BREAK BY day_sum.prod-attr :
        ACCUMULATE
        day_sum.discnt-sum ( SUB-TOTAL BY day_sum.prod-attr )
        ...
        StrBuf = StrBuf + fill( " ", 12 ) + p-XL-delim +
                 string( ACCUM SUB-TOTAL BY day_sum.prod-attr day_sum.discnt-sum,
                         "->,>>>,>>>,>>9.99" ) + p-XL-delim
        XLStrBuf = XlStrBuf + fill( " ", 12 ) + CHR(9) +
                 string( ACCUM SUB-TOTAL BY day_sum.prod-attr day_sum.discnt-sum,
                         "->,>>>,>>>,>>9.99" ) + CHR(9)
        .
     END.
     ...
     XLStrBuf = XLStrBuf + fill( " ", 12 ) + CHR(9) +
              string( ACCUM TOTAL day_sum.discnt-sum, "->,>>>,>>>,>>9.99" ) + CHR(9)
     StrBuf = StrBuf + fill( " ", 8 ) + CHR(9)
     .
     EXPORT stream PrnLibStream StrBuf .
     PUT stream PrnLibStream "" SKIP.
     if Make-Excel then  put   stream ForExcel unformatted
     ...
     XLStrBuf = string( "   ИТОГО", "x(15)" ) + CHR(9)
     .
     FOR EACH day_sum BREAK BY day_sum.prod-attr :
        ACCUMULATE
        day_sum.prod-qnty ( SUB-TOTAL BY day_sum.prod-attr )
     ...
     XLStrBuf = XLStrBuf +
              string( string( ACCUM TOTAL day_sum.prod-qnty, "->>>,>>9.<<<" ), "x(12)" ) +
              CHR(9) +
              string( ( ACCUM TOTAL day_sum.prod-sum ), "->,>>>,>>>,>>9.99" ) + CHR(9) +
              string( ACCUM TOTAL chk-day.chk-qnty, ">>>>>>>9" ) + CHR(9)
     .
     EXPORT stream PrnLibStream StrBuf .
     PUT stream PrnLibStream "" SKIP .
...
run gbl/numtomon.p ( month( vStartPoint ), output StrBuf ) .
run no-benqi(OUTPUT NotInc).
ReportHeader = "За " + StrBuf  + " " + ENTRY(NUm-entries(vStrBuf, " "), vStrBuf, " ")  + chr(10) +
