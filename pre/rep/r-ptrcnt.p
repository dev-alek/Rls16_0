...
output stream out-stream to value( string( session:temp-directory +
  ...
  :
      assign
        ...
        v-clmn-format     = v-clmn-format + substitute("&1=@;&2=@;", v-column-count, v-column-count + 1)
        v-column-count    = v-column-count + 1
        v-clmn-label-1    = v-clmn-label-1 + if first-of( tt-rep-head.gds-code ) then tt-rep-head.ptrl-name + chr(44) + chr(44) else chr(44) + chr(44)
        v-clmn-label-2    = v-clmn-label-2 + substitute( "ТРК &1" , tt-rep-head.pump-code ) + chr(44) + chr(44)
        v-clmn-label-3    = v-clmn-label-3 + "Измеряемые счетчики" + chr(44) + "Измеряемые обороты" + chr(44)
        v-clmn-sizes      = v-clmn-sizes + "20"  + chr(44) + "20"  + chr(44)
      .
    if last-of( tt-rep-head.gds-code ) then do :
      assign
        v-column-count    = v-column-count + 1
        v-line-1          = v-line-1 + substitute(",&1:&2", v-start-gds-count ,  v-column-count )
        v-clmn-format     = v-clmn-format + substitute("&1=@;", v-column-count )
        v-start-gds-count = v-column-count
        v-line-v          = v-line-v + substitute("&1=2:3/" , v-column-count )
        v-clmn-label-1    = v-clmn-label-1 + chr(44)
        v-clmn-label-2    = v-clmn-label-2 + "Сумма измеряемых оборотов" + chr(44)
        v-clmn-label-3    = v-clmn-label-3 + chr(44)
        v-clmn-sizes      = v-clmn-sizes + "20"  + chr(44)
      .
    end.
  end.
  ...
  do v-i = 1 to v-column-count :
    assign
      v-clmn-label-4 = v-clmn-label-4 + string(v-i) + chr(44)
    .
  end.
  assign
    v-line-1      = trim( v-line-1 , chr(44) )
    v-line-2      = trim( v-line-2 , chr(44) )
  ...
  assign
    ...
    sheetf.Excel-Column-Lable = "Смена / Дата"  + chr(44) + "Номер документа сверки" + chr(44) + v-clmn-label-1
                                + chr(10)
                                                + chr(44)                            + chr(44) + v-clmn-label-2
                                + chr(10)
                                                + chr(44)                            + chr(44) + v-clmn-label-3
                                + chr(10)
                                                                                                           + v-clmn-label-4
    sheetf.colformat          = "1=@;2=@" + chr(4) + v-clmn-format + chr(4) + "Отчет"
    sheetf.Sizes              = "20" + chr(44) + "20" + chr(44) + v-clmn-sizes
  .
  ...
  :
    if first-of(tt-rep-line.rvs-code) then do :
      if Make-Excel then  put   stream ForExcel unformatted
        substitute("№&1 &2" , tt-rep-line.shift-num , string( tt-rep-line.shift-date , "99.99.99" ) )  CHR(9)
        tt-rep-line.rvs-code CHR(9)
      .
    end.
    if Make-Excel then  put   stream ForExcel unformatted
       tt-rep-line.meas-el-cnt   CHR(9)
       tt-rep-line.meas-mh-qnty  CHR(9)
    .
    accumulate tt-rep-line.meas-mh-qnty ( TOTAL by tt-rep-line.gds-code ) .
    if last-of(tt-rep-line.gds-code) then do:
      if Make-Excel then  put   stream ForExcel unformatted accum total by tt-rep-line.gds-code tt-rep-line.meas-mh-qnty.
      if not last-of(tt-rep-line.rvs-code) then if Make-Excel then  put   stream ForExcel unformatted CHR(9).
    end.
...
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_is-petrl in g#lib-trn
    ...
    if not available tt-rep-line then do:
      ...
      if not available tt-rep-head then do :
        ...
        assign
          ...
          tt-rep-head.ptrl-name = replace( buf_goods.gds-name , chr(44) , " " )
        .
      end.
    end.
    else do :
  ...
  for each buf_rvs-doc no-lock
      where buf_rvs-doc.obj-type    = obj-list.obj-type
        ...
        and buf_rvs-doc.shift-date <= x-Date-End
        and buf_rvs-doc.status_     = 'факт':U ,
      first buf_rvs-line-pump no-lock
      where buf_rvs-line-pump.rvs-code = buf_rvs-doc.rvs-code
  :
    if buf_rvs-doc.shift-date = x-Date-Start and buf_rvs-doc.shift-num  < x-Shift-Start then next .
    if buf_rvs-doc.shift-date = x-Date-End   and buf_rvs-doc.shift-num  > x-Shift-End   then next .
