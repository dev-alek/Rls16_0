...
procedure print-header :
  ...
  assign
    sheetf.MergeCellsH = ""
    Sheetf.MergeCellsV = ""
    Sheetf.Excel-Column-Lable = "Артикул-цвет-размер" + chr(44) +
                                "Наименование"        + chr(44) +
                                "Цена розничная"      + chr(44) +
                                "Количество"          + chr(44) +
                                "Цена за ед."         + chr(44) +
                                "Стоимость"           + chr(44) +
                                "Штрих коды"
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  ...
  ,output v-excise
     ...
     run proc-cur-rate( input buf_trn-doc.obj-type, input buf_trn-doc.obj-code, output Rubl_Coeff ).
        ...
        v-cost-mpl  = if PrintRubl then v-price-sale  else ( v-price-sale / Rubl_Coeff )
        .
        do while available gds-prt:
            if available gds-prt
          then do:
            assign
                v-prt-name = "-" + string( gds-prt.node-name  ) + v-prt-name
            .
          end.
            assign
