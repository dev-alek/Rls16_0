...
define variable vss-include-info32 as character format "x(65)" no-undo initial "@(#)$Workfile:currsysk.i $ $Revision: $".
...
run adm/shattri.p (
  ...
  for each thbjattr_thbj-attr :
    ...
    if t-doc.obj-type = 'маг':U then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run basecode in g#library
  ...
  if v-torgconf-self-schet-exists then do:
    PUT STREAM Out_Stream
      space(9)  string ( "Поставщик: ИНН " + v-torgconf-self-host-inn + " , КПП " + v-torgconf-self-host-kpp + " " + v-torgconf-self-host-name ) format "X(100)" SKIP
  ...
  if line-counter( Out_stream ) + 8 > page-size( Out_stream ) then page stream Out_stream .
    ...
    put stream Out_stream space(5)  "В том числе: " skip .
      if PrintRubl then do:
        for each temp-nalog :
          put stream Out_stream space(15) string( "НДС " + string(temp-nalog.vat-prc) + "% : " + string( temp-nalog.vat-sum , "->>>,>>>,>>9.99" ) + " " + "РУБ" ) format "x(35)"
                             "от суммы "  string( temp-nalog.sum , "->>>,>>>,>>9.99" ) + " " + "РУБ"   format "x(20)"   skip .
        end.
      end.
      else do:
       for each temp-nalog :
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output b-code
        ...
        if not CostPrice then do:
          if PrintRubl then assign Price = buf_gds-dtl.price-rubl .
          else              assign Price = buf_gds-dtl.price-base .
        end.
        display stream out_stream
