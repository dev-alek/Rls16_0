...
define variable vss-description as character no-undo init "Почасовая статистика розничных продаж по ВЕЛИЧИНЕ СУММ ПРОДАЖ".
...
DEFINE NEW SHARED VARIABLE loc#log as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE loc#db-num as integer NO-UNDO.
DEFINE NEW SHARED VARIABLE loc#host-code as integer NO-UNDO.
DEFINE NEW SHARED VARIABLE loc#store-code as integer NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-0 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-1 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-2 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-3 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-4 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-5 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-6 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-7 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-8 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-9 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-10 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-11 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-12 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-13 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-14 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-15 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-16 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-17 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-18 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-19 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-20 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-21 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-22 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vH-23 as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE vXL    as LOGICAL NO-UNDO.
DEFINE NEW SHARED VARIABLE WH-start as CHARACTER NO-UNDO.
DEFINE NEW SHARED VARIABLE WH-end  as CHARACTER NO-UNDO.
...
DEFINE NEW SHARED temp-table grp-h no-undo
    field obj-code like ub.clients.obj-code
 ...
 .
DEFINE NEW SHARED temp-table sum-vals no-undo
    field sum1   like ub.chk-doc.netto
...
"Чеки", "CHECK":U,
"Оплаты", "PAY":U
     SIZE 25.63 BY 2.25 NO-UNDO.
DEFINE VARIABLE Rets AS LOGICAL INITIAL no
     LABEL "Обрабатывать возвраты"
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME F-Main
     ...
     RS-OPTION AT ROW 12.63 COL 1.88 NO-LABEL
     B-vals AT ROW 12.67 COL 28.5
     Tree AT ROW 15 COL 1.38
     t-byobject AT ROW 15 COL 47
     Rets AT ROW 16.04 COL 1.38
...
PROCEDURE adm-disable :
    DISABLE T-dis-card BR-dis-card-type RS-OPTION B-vals Tree t-byobject Rets WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN T-dis-card BR-dis-card-type RS-OPTION B-vals Tree t-byobject Rets WITH FRAME F-Main.
...
ON CHOOSE OF B-vals IN FRAME F-Main
DO:
  run str/sum-vals.w .
END.
  ...
  CASE RS-dis-card:
    when 1 then do:
        ...
        if varrid-list = "" then do:
            ...
            rs-dis-card = 0.
            OPEN QUERY BR-dis-card-type FOR EACH temp-dis-card-type NO-LOCK.
            DISPLAY
            rs-dis-card
            with frame F-Main.
            HIDE
...
DO:
    assign RS-option.
    if RS-option <> "LINE":U then disable Tree with FRAME F-Main.
    else enable Tree with FRAME F-Main.
END.
ON VALUE-CHANGED OF T-dis-card IN FRAME F-Main
DO:
  assign
  t-dis-card.
  CASE t-dis-card:
...
END PROCEDURE.
PROCEDURE enable_UI :
  DISPLAY T-dis-card RS-dis-card RS-OPTION Tree t-byobject Rets
      WITH FRAME F-Main.
  ENABLE T-dis-card BR-dis-card-type RS-OPTION B-vals Tree t-byobject Rets
      WITH FRAME F-Main.
  OPEN QUERY BR-dis-card-type FOR EACH temp-dis-card-type NO-LOCK.
END PROCEDURE.
PROCEDURE local-initialize :
  ...
  APPLY "VALUE-CHANGED" to T-dis-card in frame F-Main.
  APPLY "VALUE-CHANGED" to RS-dis-card in frame F-Main.
...
define variable glog as logical no-undo .
assign
FRAME F-Main RS-OPTION
FRAME F-Main Tree
FRAME F-Main Rets
.
IF NOT CAN-FIND(FIRST SUM-VALS) THEN DO:
   message "Вы не выбрали ни одного диапазона сумм!"
   view-as alert-box ERROR .
   RETURN NO-APPLY.
END.
...
glog = ?.
if num-cols > 10 AND num-cols < 14  and NOT vXL then do:
    run gbl/d-askw.w (input "Формат вывода",
                 input ("Количество интересующих Вас интервалов времени больше 10 :" + chr(10) +
...
else if num-cols >=14 AND NOT vXl then do:
    run gbl/d-askw.w (input "Формат вывода",
                 input ("Количество интересующих Вас интервалов времени больше 13 :" + chr(10) +
...
then do:
   ...
   run waitfram-show in this-procedure ( "Подождите ..." ).
    IF RETS THEN FOR each sum-vals NO-LOCK WHERE sum1 >= 0 :
        create for-sum-vals.
        assign
        for-sum-vals.sum1 = - sum-vals.sum2
        for-sum-vals.sum2 = - sum-vals.sum1.
     END.
     if vxl then do:
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
      ...
      IF tmp-var1 = "" then p-XL-delim = ";".
       ...
       If RS-Option = "LINE":U OR RS-OPTION = "PAY":U then do:
        run rep/r-svhr.p (
                         input RS-option
                        ,input (if x-SelectObject <> "currency":U and t-byobject then yes else no)
                        ,input RETS
                        ,input t-dis-card
                        ,input rs-dis-card
                      ) no-error.
      ...
      if can-find( first grp-h ) then  do:
          ...
          END.
           ...
           else run prn-lib-prn-file in this-procedure (
                                                      input parParentProc
                                                      ,input 8
                                                      ).
           FOR EACH grp-h :
       ...
       message "В указанный Вами период времени" skip
               ...
               "не было строк чеков в указанном диапазоне сумм."
       view-as alert-box INFORMATION .
        ...
        run waitfram-hide in this-procedure .
          if can-find( first grp-h ) then do:
           ...
           message "В указанный Вами период времени" skip
                   "на интересующих Вас объектах" skip
                   "не было чеков в указанном диапазоне сумм."
                   view-as alert-box INFORMATION .
        end.
    end.
END PROCEDURE.
...
End.
ReportNAme = "Почасовая статистика розничных продаж  ПО ВЕЛИЧИНЕ СУММ ПРОДАЖ" .
ReportHeader =  radio-label(string(Rs-Option), RS-Option:radio-buttons) + ", "
                            + Tree:label + " " + Rets:label +
                            (IF t-byobject THEN t-byobject:LABEL IN FRAME F-Main ELSE '':U) + ", " +
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "temp-dis-card-type":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE temp-dis-card-type THEN STRING(ROWID(temp-dis-card-type))
        ELSE "?":U.
        OTHERWISE
        DO:
