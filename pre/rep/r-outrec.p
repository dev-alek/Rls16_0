...
:
    ...
    put stream excel-line unformatted
        ...
        chr(9)   buf_temp_line-data.wtbrutto
        chr(9)   buf_temp_line-data.numplace
...
procedure print-itog :
  do on error undo, return error return-value :
    ...
    if PrintRubl then do:
      display stream out_stream
        sym1 sym2 sym3 sym4 "ИТОГО: " @ gds_name sym5 sym6 sym7 all-qnty @ qnty sym8 all-stoim @ stoim  sym10 varall-wt-brutto @ varwt-brutto sym11 varall-num-place @ varnum-place sym12 sym13
      with frame rubl.
      down stream out_stream with frame rubl .
    end.
    else do:
    ...
    put stream Out_stream skip(1) space(5) "Всего " format "x(8)" all-qnty format ">,>>>,>>9.<<<"
      space(2) "единицы" format "x(8)" space(2) Lines_Counter format ">,>>>,>>9" space(2) "наименований" format "x(13)"
                                              "на сумму"  all-stoim  format ">,>>>,>>>,>>>,>>9.99"  " "  ( if not PrintRubl then base-type else "РУБ" )
      skip(1).
    run outretxl-write-cell-data in this-procedure (
          input "f_lineAmount":U
        , input string( "Всего " + string(all-qnty,">,>>>,>>9.<<<") + " единицы " + string(Lines_Counter,">,>>>,>>9") + " наименований  на сумму " + string(all-stoim,">,>>>,>>>,>>>,>>9.99") + " " +  ( if not PrintRubl then base-type else "РУБ" ) )
    ).
    if v-torgconf-outdisc = no and not CostPrice then do:
      put stream Out_stream space(5) "Скидка " format "x(8)"
        ...
        space(2) ( if not PrintRubl then base-type else "РУБ" )                    format "x(4)" skip(1)
      .
