...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-p-pli.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-7 RAD-type-rep RADPost PostName WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 RAD-type-rep RADPost PostName WITH FRAME F-Main.
...
DO:
  ...
  Case RAdPost :
      ...
      END.
  when 1 then DO:
            For each ub.clients where ub.clients.obj-type = 'орг':U or ub.clients.obj-type = 'чел':U:
...
PROCEDURE local-apply-layout :
  ...
  apply "VALUE-CHANGED" to RADPost IN FRAME F-Main .
  apply "VALUE-CHANGED" to RAD-type-rep IN FRAME F-Main .
END PROCEDURE.
PROCEDURE my-report :
 run rep/r-p-pli.p(
         RAD-type-rep
...
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main PostName  RADPost  .
ASSIGN
