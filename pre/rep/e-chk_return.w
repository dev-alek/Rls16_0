...
PROCEDURE adm-disable :
    DISABLE RECT-13 TOG-itog SEL-oplat WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-13 TOG-itog SEL-oplat WITH FRAME F-Main.
...
DO:
  assign sel-oplat .
END.
ON VALUE-CHANGED OF TOG-itog IN FRAME F-Main
DO:
  assign tog-itog.
assign
  v-itog = tog-itog
...
v-oplat = "" + chr(44) + '-1':U .
for each ub.cash-pay no-lock where ub.cash-pay.status_ = 'тек':U:
    v-oplat = v-oplat + chr(44) + ub.cash-pay.obj-name + chr(44) + string(ub.cash-pay.cdpay-code) .
end.
ASSIGN
        SEL-oplat:LIST-ITEM-PAIRS  in frame F-Main = v-oplat .
END PROCEDURE.
