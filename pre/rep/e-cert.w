...
PROCEDURE adm-disable :
    DISABLE cl-code cl-type B-cli r-s-type WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN cl-code cl-type B-cli r-s-type WITH FRAME F-Main.
...
DO:
  run ref/cli-all.w (my-handle,  "'ПРОСМОТР':U,b-sel", ?, ?, ?, ?, ?, ?, output rid-list).
...
DO:
  ...
  case r-s-type:
    when 1 then do:
        disable cur-date q-days with frame F-Main.
        hide cur-date q-days .
    end.
    when 2 then do:
        disable q-days with frame F-Main.
        hide q-days .
        enable cur-date with frame F-Main.
        disp cur-date with frame F-Main.
    end.
    when 3 then do:
        disable cur-date with frame F-Main.
        hide cur-date .
        enable q-days with frame F-Main.
        disp q-days with frame F-Main.
    end.
   end case.
END.
