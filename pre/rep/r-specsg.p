...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
    ...
    assign
        v-income            = ( if buf_trn-doc.doc-type = 'при':U
    ...
    put stream out-stream
        ...
        skip (1) space( 22 )
            ...
            ( if buf_trn-doc.status_ <> 'факт':U then "   Статус: " + caps(buf_trn-doc.status_) else " " )
                                                                        format "X(25)"
        skip space( 22 ) "Поставщик (отправитель): "
            (if v-income = yes then v-cli-name else v-obj-name )    format "X(60)"
        skip space( 22 ) "Покупатель (получатель): "
...
put stream out-stream
    ...
    skip space(1)  "|"
        ...
        "|"                     at 1 + 81
        "Количество"            at center-field(1 + 81, 1 + 93, 10)
        "|"                     at 1 + 93
        "Цена"                  at center-field(1 + 93, 1 + 108, 4)
        "|"                     at 1 + 108
        "Сумма"                 at center-field(1 + 108, 1 + 124, 10)
        "|"                     at 1 + 124
        "Дата"                  at center-field(1 + 124, 1 + 135, 4)
        "|"                     at 1 + 135
    skip space(1)
        "|" v-single-line format "X(133)" "|"
.
