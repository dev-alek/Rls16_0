...
PROCEDURE adm-disable :
    DISABLE r-tax T-without-vat E-tax-list t-break-by-cp t-no-covered-techrfsl t-ext-doc-type-subtotals t-excel t-TEXT f-tax-rate WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN r-tax T-without-vat E-tax-list t-break-by-cp t-no-covered-techrfsl t-ext-doc-type-subtotals t-excel t-TEXT f-tax-rate WITH FRAME F-Main.
...
DO:
  ...
  v-tax-rate-list = ''
   ...
   DO v-ind = 1 to v-num
    ...
    then do:
        ASSIGN
        e-tax-list = SUBSTITUTE("&1&2&3", e-tax-list, (if e-tax-list = '' then '' else chr(10)), buf_tax-rate.rate-name)
        v-tax-rate-list = v-tax-rate-list + (if v-tax-rate-list = '' then '' else chr(44)) + string(buf_tax-rate.rate-code)
        .
     END.
   END.
   DISPLAY e-tax-list with frame F-Main.
END.
...
CASE p-action :
  WHEN 'get' THEN DO:
    IF v-profile-id = 69  THEN DO:
       ...
       RUN rcps_get-value IN parent-handle (
                                         ...
                                         ,output v-value-decimal
                                         ,output v-value-integer
                                         ,output t-ext-doc-type-subtotals
                                         ) no-error .
       v-index-id = 0.
       RUN rcps_get-value IN parent-handle (
                                          input "p-price-type"
       ...
       assign
       t-without-vat:screen-value in frame F-Main =  string(t-without-vat)
       t-break-by-cp:screen-value in frame F-Main =  string(t-break-by-cp)
       t-no-covered-techrfsl:screen-value in frame F-Main =  string(t-no-covered-techrfsl)
       t-ext-doc-type-subtotals:screen-value in frame F-Main =  string(t-ext-doc-type-subtotals)
       t-excel:screen-value in frame F-Main =  string(lookup('excel':U, v-output-type) > 0)
       t-text:screen-value in frame F-Main = string(lookup('text':U, v-output-type) > 0)
       .
    END.
  ...
  WHEN 'set' THEN DO:
    IF v-profile-id = 69 THEN DO:
      ...
      t-excel
      v-output-type = ","
      ENTRY(1, v-output-type) = (IF t-excel THEN 'excel':U ELSE '')
      ENTRY(2, v-output-type) = (IF t-text THEN 'text':U ELSE '')
      v-output-type = TRIM(v-output-type, chr(44))
      .
      if not t-excel
      ...
      RUN rcps_set-value IN parent-handle (
                                      ...
                                      ,input ?
                                      ,input 0.0
                                      ,input x-SET_PAY_TYPE
                                      ,input no
                                      ) no-error .
      v-val-type = if X-SET_val_TYPE = 2 then 'base':U else 'rubl':U.
      RUN rcps_set-value IN parent-handle (
                                        input "p-r-b"
                                      ,INPUT 0
                                      ,input v-val-type
