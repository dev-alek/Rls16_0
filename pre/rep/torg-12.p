...
define temp-table temp_p-fmt_string-part no-undo
    ...
    index pi is primary unique
        str-key
.
define variable v-p-fmt-30-str-key    as integer      no-undo.
...
:
    ...
    :
        ...
        assign
            v-p-fmt-30-str-key = v-p-fmt-30-str-key + 1
        .
        assign
            buf_temp_p-fmt_string-part.str-key      = v-p-fmt-30-str-key
...
:
  ...
  if error-status :error
  then do:
    undo, return error substitute( "&1. &2&3&4", vss-include-info43, return-value, chr(10), error-status :get-message (1)).
  end.
end.
end procedure.
...
assign
    v-torg-12-gds-name-length       = ( if v-torgconf-outt12 = yes then 73 else 48 )
.
define frame f-doc
        sym1 column-label ":!:!:" format "X(1)" space(0)
        v-doc-line-counter COLUMN-LABEL "N!п/п! " format ">>>>9" space(0)
        sym2 column-label ":!:!:" format "X(1)" space(0)
        ub.goods.gds-name COLUMN-LABEL "Наименование товара! ! " format "X(48)" space(0)
...
define frame f-doc-m
        ...
        v-doc-line-counter COLUMN-LABEL "N!п/п! " format ">>>>9" space(0)
        sym2 column-label ":!:!:" format "X(1)" space(0)
        ub.goods.gds-name COLUMN-LABEL "Наименование товара! ! " format "X(73)" space(0)
        sym3 column-label ":!:!:" format "X(1)" space(0)
        tb-code COLUMN-LABEL "Код товара! ! " format "X(17)" space(0)
        sym4 column-label ":!:!:" format "X(1)" space(0)
        ub.goods.unit-base COLUMN-LABEL "Наим!ед.!изм." format "X(4)" space(0)
...
"--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------" skip
   with width 235  down stream-io no-underline no-labels no-box.
...
if page-number( out-stream ) > prevpage then
    ...
    put stream out-stream
        space(10) "  Всего на сумму:        "
            ...
            " ("
            trim( ( if Invers then currency.curr-abbr else ( if PrintRubl then "РУБ" else "баз.вал" ) ) ) format "X(7)"
            ")"
    .
        ...
        if lookup( "TOPAUKC":U, p-mode ) <> 0 then do:
          put stream out-stream
                              string( ", скидка: "
                                  ...
                                  + " (" + trim( ( if PrintRubl then "РУБ" else "баз.вал" ) ) + ")"
                              ) format "X(100)"
          .
        end.
        else do:
    ...
    put stream out-stream
        ...
        string( "Приложение (паспорта, сертификаты, и т.д.) на " + string( v-underline, "X(42)" ) + " листах" ) format "X(95)" "|" at 97
            string( "По доверенности N " + string( p-torgconf-N-warrant, "X(39)" ) + " от " + p-torgconf-date-char  ) format "X(100)" at 99 skip
    ...
    put stream out-stream
        string( " _________________  " + v-torgconf-ogr-name) format "X(47)" " |":U at 96  v-underline  format "X(101)"skip
    ...
    put stream out-stream
        string(  " _________________  " + p-torgconf-wrkr-name ) format "X(48)" "|" at 97  "Груз получил грузополучатель:    " v-loadtplace Format "x(17)" " ___________________ " v-loadtname Format "X(26)" skip
        ...
        then do:
            ...
            then do:
                ...
                display stream out-stream
                    ...
                    v-gds-name @ ub.goods.gds-name
                    sym1 sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9 sym10
                    sym11 sym12 sym13 sym14 sym15 sym16  sym18
                    with frame f-doc .
                down stream out-stream  with frame f-doc .
            end.
            ...
            then do:
              ...
              then do:
                  display stream out-stream
                      ...
                      sym1 sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9 sym10
                      sym11 sym12 sym13 sym14 sym15 sym16  sym20 sym21
                  with frame f-doc-m-bb .
                  down stream out-stream 1 with frame f-doc-m-bb .
              end.
              else do:
                  display stream out-stream
                      temp_gds-name.gdnString @ ub.goods.gds-name
                      sym1 sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9 sym10
                      sym11 sym12 sym13 sym14 sym15 sym16  sym18  sym20 sym21
                  with frame f-doc-bb .
                  down stream out-stream 1 with frame f-doc-bb .
              end.
            end.
    ...
    if line-counter( Out-Stream ) + 1 > page-size( Out-Stream ) then
        ...
        find first ub.ext-artic no-lock
             where ub.ext-artic.gds-code = bar-code.gds-code
               ...
               and ub.ext-artic.status_  = 'тек':U no-error.
        if available ub.ext-artic then v-ext-artic = ub.ext-artic.ext-artic.
        if v-ext-doc-type = 'ep':U or ( lookup( "TOPAUKC":U, p-mode ) <> 0 and lookup( "GTD":U, p-mode ) <> 0 )
        then do:
                ...
                :
                    ...
                    then do:
                        ...
                        display stream out-stream
                            ...
                            v-tax                       @ stoim
                            sym1 sym10 sym11 sym12 sym13 sym14 sym15 sym16  sym18
                        with frame f-doc.
                        down stream out-stream  with frame f-doc.
                    end.
                end.
    ...
    then do:
        ...
        then do:
          ...
          display stream out-stream
              ...
              v-country
              sym1 sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9 sym10
              sym11 sym12 sym13 sym14 sym15 sym16  sym18 sym22  sym20 sym21
          with frame f-doc-bb.
          down stream out-stream  with frame f-doc-bb.
        end.
        else do:
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
    ...
    view stream out-stream frame BottomFrame .
        assign
        val-str = ( if Invers then ub.currency.curr-abbr else ( if PrintRubl then "рублях" else "баз.вал" ) )
    .
 ...
 run gbl/trdcat-v.p  ( input buf_trn-doc.doc-code
   ...
   then do:
    ...
    then do:
        ...
        put stream out-stream
               space(5) string( "Примечание: " + replace( buf_trn-doc.PS, chr(10), " " ))   format "X(163)"
        .
    end.
    if lookup( "TOPAUKC":U, p-mode ) <> 0 then do:
      put stream out-stream
