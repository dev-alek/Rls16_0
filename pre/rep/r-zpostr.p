...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run baserate in g#library
  ...
  ,output v-base-scale
    ...
    DEFINE temp-table temp-sum no-undo
        ...
        .
   os-delete value(string( session:temp-directory ) + "rpt" + string( g#report-num ) + ".t-t")   .
    ...
    if tog-obj then do:
      for each temp-gds
        break by temp-gds.obj-type
              ...
              by if xSortType = "sort-code" then  temp-gds.sb-code  Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
...
if first-of(temp-gds.grp-name) then do:
  ...
  assign CurrGrpName = "" .
  do ind = 1 to lvel :
    assign CurrGrpName = CurrGrpName + entry ( ind, temp-gds.grp-name, chr(47) )  + chr(47) .
  ...
  do ij = old-lvel to ind by -1 :
    ...
    if ij <= xvar-lavel then do:
      ...
      run PrintItogGroup in this-procedure ( ItogStr, tt-grp-tree.fact-qnty, tt-grp-tree.free-qnty, tt-grp-tree.zak-sum, tt-grp-tree.naz-sum, tt-grp-tree.wait-qnty ) .
    end.
    delete tt-grp-tree .
  end.
  ...
  do ij = ind to lvel :
    create tt-grp-tree .
    if ij > ind then do:
      assign CurrGrpName = CurrGrpName + entry ( ij, temp-gds.grp-name, chr(47) )  + chr(47) .
    end.
    assign
      tt-grp-tree.num  = ij + 3
      tt-grp-tree.full = CurrGrpName
      tt-grp-tree.name = entry ( ij, temp-gds.grp-name, chr(47) )
...
if first-of(temp-gds.cgrp-name) then do:
  assign
    lvel = num-entries( right-trim(temp-gds.cgrp-name, chr(47)), chr(47) )
  .
  assign CurrGrpName = "" .
  do ind = 1 to lvel :
    assign CurrGrpName = CurrGrpName + entry ( ind, temp-gds.cgrp-name, chr(47) )  + chr(47) .
  ...
  do ij = ind to lvel :
    create tt-grp-tree .
    if ij > ind then do:
      assign CurrGrpName = CurrGrpName + entry ( ij, temp-gds.cgrp-name, chr(47) )  + chr(47) .
    end.
    assign
      tt-grp-tree.num  = ij + 3
      tt-grp-tree.full = CurrGrpName
      tt-grp-tree.name = entry ( ij, temp-gds.cgrp-name, chr(47) )
      tt-grp-tree.fact-qnty = 0
      tt-grp-tree.free-qnty = 0
      tt-grp-tree.zak-sum   = 0
      tt-grp-tree.naz-sum   = 0
