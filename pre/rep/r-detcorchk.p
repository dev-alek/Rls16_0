...
define buffer buf_clients for ub.clients .
for first buf_staff NO-LOCK where buf_staff.staff-code = p-cashier-code and buf_staff.role = 'C':U,
first buf_person no-lock where
          buf_person.psn-code = buf_staff.psn-code,
          first buf_clients no-lock where buf_clients.obj-code = buf_person.psn-code and buf_clients.obj-type = 'чел':U:
...
define variable v-rastype-name as character no-undo .
  case p-rastype-code:
    when integer('43':U) then
      do:
        v-rastype-name = "Приход" .
      end.
    when integer('44':U) then
      do:
