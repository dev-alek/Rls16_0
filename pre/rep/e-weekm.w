...
PROCEDURE adm-disable :
    DISABLE RECT-1 RS-method WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 RS-method WITH FRAME F-Main.
   ...
   FOR EACH ub.doc-line WHERE ub.doc-line.doc-code = doc-num NO-LOCK,
        ...
        if ( jj  modulo 10 ) = 0  then
            run waitfram-show in this-procedure (input ( "Обработано строк накладных : " +
                                        string(  jj ) ) ).
           IF X-SelectGood = 4  AND NOT (can-find(FIRST gds-list WHERE
        ...
        assign
        s-price = (if v-curr-r-b = 'rubl':U then ub.gds-dtl.price-rubl else ub.gds-dtl.price-base)
