...
:
  ...
  if error-status :error
  then do:
    undo, return error substitute( "&1. &2&3&4", vss-include-info6, return-value, chr(10), error-status :get-message (1)).
  end.
end.
end procedure.
...
for each parts no-lock
  ...
  tt-rep1.name-pol = p-torgconf-accept-position + " " + p-torgconf-accept-fname .
    ...
    run clntattr-value in this-procedure ( input 'орг':U, input v-cntxt-host-code-obj, input 'egrip-num':U , output v-host-egrip-num , output v-par-type).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  (input  t-doc.cli-type
  ,input  t-doc.cli-code
  ,output v-cntxt-host-code-obj-cli
  )  .
    ...
    find first alc-sale-lic
      WHERE alc-sale-lic.cli-type = 'орг':U
...
put stream OutStr-html unformatted
  ...
  .
for each tt-rep1 :
if tt-rep1.ext-doc-type = 'ee':U then do:
  put stream OutStr-html unformatted
    ' <body>' skip
