...
str3 = str3-1 + chr(32) + str3-2 + chr(32) + str3-3
str4 = string( "Cуммы указаны в " + (if PrintRubl then "руб" else base-type) )
.
...
PUT STREAM PrnLibStream string( reportname + chr(32) + str2)
                                               AT 69 format "X(195)"
                                           SKIP(1)
                                           str3-1
                                               AT 10 format "X(195)"
    ...
    DO WHILE available ub.parts :
        ...
        t-doc.doc-type                                                  CHR(9)
        t-doc.status_                                                   CHR(9)
        (if t-doc.flag_ then "+":U else "-":U)                          CHR(9)
        t-doc.doc-code                                                  CHR(9)
        (substring ((string (t-doc.doc-date)), 1, 5))                   CHR(9)
        t-doc.fact-date                                                 CHR(9)
        (if t-doc.internal then "+":U else "-":U)                       CHR(9)
        t-doc.cli-name                                                  CHR(9)
        ub.parts.fact-qnty                                                 CHR(9)
        price0                                                          CHR(9)
        stoim0                                                          CHR(9)
        (if ub.parts.part-code = "" then "------" else ub.parts.part-code)    CHR(9)
...
tot-free-qnty                                                   CHR(9)
                                                                CHR(9)
(if PrintRubl then tot-free-sum0-rubl else tot-free-sum0-base)  CHR(9)
                                                                CHR(9)
skip.
    ...
    each  ub.parts-attr where
     ...
     EACH t-doc WHERE t-doc.doc-code = ub.parts.out-code
                ...
                and (
                  ...
                  or t-doc.ext-doc-type = 'rs':U
                  or t-doc.ext-doc-type = 're':U )
             NO-LOCK :
      assign
...
PROCEDURE enable_UI :
  ...
  ENABLE rect-in br-docs b-print b-help b-quit
      WITH FRAME v-cust.
  VIEW FRAME v-cust.
  OPEN QUERY br-docs FOR EACH ub.parts WHERE             ub.parts.artic = suppl-parts.artic         AND ub.parts.prod-type = suppl-parts.prod-type         AND ub.parts.prod-code = suppl-parts.prod-code         AND ub.parts.status_ = yes NO-LOCK,     each  ub.parts-attr where           ub.parts-attr.gds-code = suppl-parts.gds-code and             ub.parts-attr.in-code = ub.parts.in-code and                 ub.parts-attr.part-code = ub.parts.part-code    and             ub.parts-attr.income-in-code = suppl-parts.out-code           no-lock ,                          EACH t-doc WHERE t-doc.doc-code = ub.parts.out-code                   and t-doc.fact-date >= from-date                   and t-doc.fact-date <= to-date                   and ( t-doc.ext-doc-type = 'ie':U                    or t-doc.ext-doc-type = 'ee':U                     or t-doc.ext-doc-type = 'es':U                   or t-doc.ext-doc-type = 'rs':U                   or t-doc.ext-doc-type = 're':U                   or t-doc.ext-doc-type = 'pc':U                      or t-doc.ext-doc-type = 'ap':U )               NO-LOCK BY t-doc.fact-date BY t-doc.fact-num.
