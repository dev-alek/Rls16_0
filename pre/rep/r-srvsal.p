...
with width 198 down stream-io use-text NO-BOX.
...
if not found then do:
  ...
  message "Не было никакой реализации услуг на выбранных объектах в течение заданного Вами периода времени." view-as alert-box information .
  return.
end.
    ...
    for each buf_gds-dtl no-lock where
      ...
      then do:
        if p-rs-by = 2 then do:
          ...
          assign
          gds_temp-serv.tot-doc   = gds_temp-serv.tot-doc + (if v-curr-r-b = 'base':U
                                                            then buf_gds-dtl.price-base
                                                            else buf_gds-dtl.price-rubl)
                                                            * buf_gds-dtl.fact-qnty * buf_sale-doc.dir
          gds_temp-serv.netto     = gds_temp-serv.netto   + (if v-curr-r-b = 'base':U
        ...
        if p-rs-by = 1 then do:
          ...
          assign
          gdsobj_temp-serv.tot-doc   = gdsobj_temp-serv.tot-doc + (if v-curr-r-b = 'base':U
                                                            then buf_gds-dtl.price-base
                                                            else buf_gds-dtl.price-rubl)
                                                            * buf_gds-dtl.fact-qnty * buf_sale-doc.dir
          gdsobj_temp-serv.netto     = gdsobj_temp-serv.netto   + (if v-curr-r-b = 'base':U
                                                            then (buf_gds-dtl.price-base - buf_gds-dtl.discnt-base)
                                                            else (buf_gds-dtl.price-rubl - buf_gds-dtl.discnt-rubl)
                                                            )
                                                            * buf_gds-dtl.fact-qnty * buf_sale-doc.dir
      ...
      assign
      gdstot_temp-serv.tot-doc   = gdstot_temp-serv.tot-doc + (if v-curr-r-b = 'base':U
                                                        then buf_gds-dtl.price-base
                                                        else buf_gds-dtl.price-rubl)
                                                        * buf_gds-dtl.fact-qnty * buf_sale-doc.dir
      gdstot_temp-serv.netto     = gdstot_temp-serv.netto   + (if v-curr-r-b = 'base':U
                                                        then (buf_gds-dtl.price-base - buf_gds-dtl.discnt-base)
                                                        else (buf_gds-dtl.price-rubl - buf_gds-dtl.discnt-rubl)
                                                        )
                                                        * buf_gds-dtl.fact-qnty * buf_sale-doc.dir
...
with FRAME NBottomFrame width 232
