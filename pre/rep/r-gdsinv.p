...
do:
  ...
  if v-sort = 1 then ReportHeader = "Сортировка по имени".
    ...
    when 1 then
      for each obj-list no-lock ,
          each buf-doc where buf-doc.obj-code = obj-list.obj-code
              and buf-doc.obj-type = obj-list.obj-type  and buf-doc.doc-type = 'инв':U and buf-doc.ext-doc-type = 'vt':U and buf-doc.status_ = 'факт':U
          ...
          for first buf-gds field (gds-name grp-name grp-code unit-base) where
            ...
            find first buf-clients where buf-clients.obj-code =  buf-doc.agnt and buf-clients.obj-type = 'чел':U no-error.
            create tt-line.
            assign
...
for tt-line field ( artic
    ...
    if Make-Excel then  put   stream ForExcel unformatted
              ...
              tt-line.unit-base CHR(9)
              tt-line.fact-date CHR(9)
              tt-line.doc-code  CHR(9)
              tt-line.agnt      CHR(9)
              tt-line.fact-qnty CHR(9)
    skip.
    display stream PrnLibStream sym1  tt-line.artic
                                ...
                                sym8  skip
    with frame gdsinvent .
    down stream PrnLibStream with frame gdsinvent .
end.
end.
procedure print-obj.
  define input parameter p-grp as character.
  form with frame gdsinvent.
  display stream PrnLibStream sym1 "Объект -" @ tt-line.artic p-grp @ tt-line.gds-name sym8 with frame gdsinvent.
  down stream PrnLibStream with frame gdsinvent .
  underline stream PrnLibStream tt-line.artic tt-line.gds-name with frame gdsinvent .
end.
procedure print-grp.
  define input parameter p-grp as character.
  form with frame gdsinvent.
  display stream PrnLibStream sym1 "Группа - " + p-grp @ tt-line.gds-name sym8 with frame gdsinvent.
  down stream PrnLibStream with frame gdsinvent .
