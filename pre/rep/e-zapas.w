...
PROCEDURE adm-disable :
    DISABLE RECT-5 RECT-6 Classify SortType ShowZero Long-name PartsDet v-photo WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-5 RECT-6 Classify SortType ShowZero Long-name PartsDet v-photo WITH FRAME F-Main.
    ...
    if Classify  Begins "prod":U OR
       Classify  Begins "grp-goods":U then
        ...
        do:
            ...
            enable PartsDet with frame F-Main .
        end.
END.
...
DO:
  ASSIGN long-name .
END.
ON VALUE-CHANGED OF PartsDet IN FRAME F-Main
DO:
...
if v-value-mark and PartsDet then do:
    enable v-alc-marks with frame F-Main .
end.
else do:
    DISABLE v-alc-marks with frame F-Main .
end.
END.
...
DO:
  ...
  if not SumsOnly then do:
      enable PartsDet with frame F-Main .
      enable v-photo  with frame F-Main .
  end.
  else do :
      PartsDet = FALSE .
      display PartsDet with frame F-Main .
      disable PartsDet with frame F-Main .
      v-photo = FALSE .
      display v-photo with frame F-Main .
      disable v-photo with frame F-Main .
  end.
END.
ON VALUE-CHANGED OF v-alc-marks IN FRAME F-Main
DO:
  ASSIGN v-alc-marks .
END.
ON VALUE-CHANGED OF v-photo IN FRAME F-Main
DO:
  ASSIGN v-photo .
END.
...
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main SumsOnly ShowZero  Classify SortType long-name PartsDet v-alc-marks.
