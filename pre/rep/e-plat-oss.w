...
PROCEDURE adm-disable :
    DISABLE RECT-1 RECT-3 v-oper-oss v-sel-oss rs-classified WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 RECT-3 v-oper-oss v-sel-oss rs-classified WITH FRAME F-Main.
...
ON VALUE-CHANGED OF v-oper-oss IN FRAME F-Main
DO:
...
PROCEDURE local-initialize :
  ...
  END.
  v-ch = "".
  FOR EACH ext-classif WHERE ext-classif.classif-subject = 'oss-ref':U NO-LOCK:
      v-ch = v-ch + ENTRY(1, ext-classif.charkey_two,chr(4)) + ",".
  END.
