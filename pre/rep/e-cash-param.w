...
PROCEDURE adm-disable :
    DISABLE rect-3 rect-4 rect-6 rect-8 SelectParam choose-device SelectSource SelectDiff WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN rect-3 rect-4 rect-6 rect-8 SelectParam choose-device SelectSource SelectDiff WITH FRAME F-Main.
...
ON CHOOSE OF b-sel-cash IN FRAME F-Main
  DO:
    ...
    do ii = 1 to num-entries(v-rid-list):
      ...
      if available buf_cash-desk and buf_cash-desk.autonomy <> integer('1':U) then
      do:
        CREATE tt-cash-list.
...
ON VALUE-CHANGED OF SelectDiff IN FRAME F-Main
  DO:
    ...
    case SelectDiff:
      when "all" then
        do:
          disable b-sel-cash with frame F-Main .
          for each obj-list:
            for each cash-desk where cash-desk.obj-code = obj-list.obj-code and cash-desk.is-del = false and cash-desk.autonomy <> integer('1':U):
      ...
      otherwise
      do:
        enable b-sel-cash with frame F-Main .
        run ref/cashlist.w (input my-handle
          ,INPUT "b-sel,b-mark"
          ,INPUT 'все':U
          ,INPUT v-cntxt-db-num
...
ON VALUE-CHANGED OF SelectParam IN FRAME F-Main
  DO:
    ...
    case SelectParam:
      when "choose":u then
        do:
          enable b-sel-param with frame F-Main .
          ...
          do:
            ...
            SelectParam = "all" .
            display SelectParam with frame F-Main .
            disable b-sel-param with frame F-Main .
          end.
        end.
      otherwise
      do:
...
PROCEDURE inifields :
  ...
  choose-device:list-item-pairs in frame F-Main = "ВСЕ,-1" .
