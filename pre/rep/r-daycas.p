...
if session :set-wait-state( "compiler" ) then.
  ...
  run macr_excel_char("1. Выручка всего, руб" , v-row, 1) .
  ...
  run macr_excel_char1("   в т.ч. кредитные карты, руб" , v-row, 1) .
  ...
  assign v-row = v-row + 1 .
  run macr_excel_char("2. Реализация (розница), руб" , v-row, 1) .
  run macr_cell_format1 ( 'Arial Cyr',9, yes, yes, ?, v-row, 1, v-row, 1) .
  assign v-col = 2 .
  for each temp-mag :
    run macr_excel_sum1  ( Temp-mag.sum-viruch, v-row, v-col,  2) .
  ...
  assign v-row = v-row + 1 .
  run macr_excel_char1("   Наценка (розница), руб" , v-row, 1) .
  run macr_cell_format1 ( 'Arial Cyr',9, no, yes, ?, v-row, 1, v-row, 1) .
  assign v-col = 2 .
  for each temp-mag :
...
procedure PutColumnTitulExcel :
  ...
  :
    ...
    if p-period-type = 'yesterday':U then do:
    ...
    run macr_cell_format1 ( 'Arial Cyr',10, yes, no, ?, v-row, 1, v-row, v-col - 1) .
...
procedure macr_excel_sum1 :
 ...
 if p-typ = 0 then assign ss = string( Round( p-val, 0), ">>>,>>>,>>>,>>>,>>>,>>>,>>>" ) .
  ...
  if p-typ = 0 then put stream macr_excel unformatted 'format.number("#,##0")' + chr(10) .
  else              put stream macr_excel unformatted 'format.number("#,##0.00")' + chr(10) .
