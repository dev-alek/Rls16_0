...
IF ERROR-STATUS :ERROR THEN DO:
  ...
  RETURN.
END.
ASSIGN v_param-pay-hide = ( IF v_r-b = "rubl" THEN "1":U ELSE "1,2":U ).
run rep/d-report.w (            INPUT parParentProc,
                            INPUT "rep/e-corpr.w",
                            INPUT "Объединенные документы возврата поставщику и прихода для смены типа приобретения",
                            INPUT 2,
                            INPUT "":U,
                            INPUT "1,2,3":U,
                            INPUT "":U,
                            INPUT v_param-pay-hide,
                            INPUT "":U,
                            INPUT NO                                          ).
