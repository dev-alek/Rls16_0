...
define variable vss-description as character no-undo init "Анализ цен поставщиков (закладка № 2)".
...
DEFINE VARIABLE PostName AS CHARACTER
     ...
     SIZE 49.75 BY 11.83 TOOLTIP "Список выбранных Поставщиков"
     FONT 4 NO-UNDO.
...
DEFINE RECTANGLE RECT-7
     ...
     SIZE 68.25 BY 15.29.
DEFINE VARIABLE T-null AS LOGICAL INITIAL no
     LABEL "Показать нулевые цены"
     VIEW-AS TOGGLE-BOX
     SIZE 25.38 BY .83 NO-UNDO.
DEFINE FRAME F-Main
     PostName AT ROW 2.96 COL 19.38 NO-LABEL
     RADPost AT ROW 3 COL 3.25 NO-LABEL
     T-null AT ROW 15.42 COL 4
...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-p-pl.w should only be RUN PERSISTENT."
...
PROCEDURE adm-disable :
    DISABLE RECT-7 PostName RADPost T-null WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 PostName RADPost T-null WITH FRAME F-Main.
        ...
        do:
            ...
            if post-grp_recids = "" then do:
                ...
                DO ii = 1 TO v-nn :
                    ...
                    if cli-post.obj-type = 'орг':U
                    or cli-post.obj-type = 'чел':U then do:
...
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main PostName  RADPost t-null  .
