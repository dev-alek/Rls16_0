...
define variable vss-workfile    as character no-undo init "$Workfile: r-bener3.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-bener3.p $":U .
...
DEFINE FRAME PayCodes-rubl
    ...
    HEADER  date_string format "X(35)" AT 5
                    ...
                    Line format "X(82)" AT 1
    with width 160 down stream-io use-text .
DEFINE FRAME ZUM-PayCodes-rubl
        sym1 column-label ":" format "X(1)"
        benefits.pay-name column-label "Вид оплаты" format "X(41)"
        sym6 column-label ":" format "X(1)"
        benefits.tot-r-b column-label "Сумма (вал.продаж)"  format "->,>>>,>>>,>>>,>>9.99"
...
assign
allday-r-bsum = (if v-curr-r-b = 'base':U then allday-basesum else allday-rublsum).
run waitfram-hide in this-procedure .
...
if choice
then do:
  FORM with frame Benefit-rubl .
end.
else do:
  FORM with FRAME PayCodes-rubl .
end.
DatePrinted = FALSE .
FOR EACH obj-list WHERE
         obj-list.obj-type = 'маг':U BREAK BY obj-list.obj-code :
  FIND FIRST clients WHERE
            clients.obj-type = obj-list.obj-type  AND
            clients.obj-code = obj-list.obj-code  NO-LOCK .
  if choice then do:
    DOWN stream PrnLibStream 1 with frame Benefit-rubl .
  end.
  else do:
    DOWN stream PrnLibStream 1 with FRAME PayCodes-rubl .
  end.
  ...
  if choice then do:
    ...
    BY ub.inkas-pay-desk.curr-code :
      if first-of( ub.inkas.doc-date ) then  do:
        ...
        Day-Tot-R-b  = (if v-curr-r-b = 'base':U then Day-Tot-Base else Day-Tot-rubl)
  ...
  then
  assign
  ALLOBJ-ALlDAy-Num-CHk = ALLOBJ-ALlDay-Num-CHk +   accum total Day-Num-Chk
  ALLOBJ-ALlDAy-Num-CHk-nf = ALLOBJ-ALlDay-Num-CHk-nf +  accum total Day-Num-Chk-nf
  .
  else
  assign
...
if choice AND ( ObjAmount > 1 ) then do:
  FORM with frame ZUM-PayCodes-rubl .
  FOR EACH obj-list WHERE
  ...
  BY ub.inkas-pay-desk.curr-code :
    ...
    if last-of( ub.inkas-pay-desk.curr-code ) AND
      ...
      ( ACCUM SUB-TOTAL BY ub.inkas-pay-desk.curr-code ub.inkas-pay-desk.tot-rubl ) @ benefits.tot-rubl
      with frame ZUM-PayCodes-rubl .
      DOWN stream PrnLibStream 1 with FRAME ZUM-PayCodes-rubl .
     end.
  END.
end.
