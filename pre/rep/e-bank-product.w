...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-bank-product.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-1  selectagent WITH FRAME fMain.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1  selectagent WITH FRAME fMain.
...
DO:
  ...
  CASE selectagent:
    when "selective":U then do:
      run ref/bpa.p(INPUT my-handle, INPUT 'ВЫБОР':U, OUTPUT v-rid-list-oss) no-error.
      IF  v-rid-list-oss <> "":U
      AND v-rid-list-oss <> ?
      THEN DO:
      end.
  ...
  END CASE.
  display selectagent with frame fMain.
END.
