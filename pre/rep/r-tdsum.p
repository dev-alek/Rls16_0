...
procedure tdsxl-init :
    ...
    run tdsxl-write-cell-data in this-procedure (
          input "Сводка_subtotalList":U
        , input "":U
    ).
    run tdsxl-write-cell-data in this-procedure (
          input "Сводка_subtotalType":U
        , input "":U
    ).
end procedure.
procedure tdsxl-sheet1-write-line-data :
    ...
    put stream excel-line UNFORMATTED
        ...
        CHR(9) p-impl-day
        CHR(9) p-impl-month
        CHR(9) p-price-impl-now
        CHR(9) p-price-impl-change
        CHR(9) 0
        CHR(9) 0
        CHR(9) 0
        CHR(9) 0
...
define temp-table tt-line no-undo
    ...
    field qnty-start-month as decimal
    field qnty-start-day as decimal
    field price-end-shift as decimal
    field price-shift-change as decimal
...
output STREAM Out-Stream to value( string( session:temp-directory +
                                     ...
                                     page-size  value(43) .
    put stream Out-Stream UNFORMATTED "Отчет доступен только в формате Excel.".
end.
procedure close-rep:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  (input  tt-line.gds-code
  ,input  ?
  ,output v-b-code
  ) no-error .
define variable vss-include-info15 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  (input  tt-line.obj-type
  ,input  tt-line.obj-code
  ,input  v-b-code
  ,input  0
  ,input  v-factord-end
  ,output v-doc-num
...
procedure calc-month:
    ...
    for each lc_ot-line no-lock
        ...
        :
            if lc_ot-line.ext-doc-type = 'rs':U then do:
                if tt-line.is-petrol then
            ...
            else if lc_ot-line.ext-doc-type = 'es':U then do:
                if tt-line.is-petrol then
                    tt-line.qnty-start-month = tt-line.qnty-start-month - lc_ot-line.fact-qnty.
                else
                    tt-line.sum-month = tt-line.sum-month - lc_ot-line.sum-rubl.
...
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_is-petrl in g#lib-trn
  (
     input tt-line.artic
  ,  input tt-line.prod-type
  ,  input tt-line.prod-code
  , output tt-line.is-petrol
  , output v-is-pieces
  ) .
    end.
end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostname in g#library
  ...
  ,output v-org-name
    ...
    run tdsxl-write-cell-data("org_name":U, v-org-name).
    run tdsxl-write-cell-data("date_day":U, day(today)).
    run tdsxl-write-cell-data("date_month":U, month(today)).
    run tdsxl-write-cell-data("date_year":U, year(today)).
    for each obj-list,
        each tt-line
            where tt-line.obj-type = obj-list.obj-type
            and tt-line.obj-code = obj-list.obj-code
