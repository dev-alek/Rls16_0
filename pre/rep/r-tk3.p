...
define variable v-orgname as character no-undo .
define variable v-name as character no-undo.
define variable v-weight as character no-undo.
define variable v-ps as character no-undo.
define variable v-ps1 as character no-undo.
...
define variable v-show-t2 as logical no-undo.
...
procedure get-factord:
    ...
    if p-type = 1 and p-shift-num > 0 then do:
        run ostatok(
            p-obj-code,
            p-obj-type,
            true,
...
define temp-table tt-line no-undo
    ...
    field gds-unit as character
    field brutto as decimal
    field waste-pc as decimal
    field netto as decimal
    field netto-1 as decimal
    field final-weight as decimal
.
run main no-error.
...
procedure main:
    ...
    run prepare-info no-error. if error-status:ERROR then return error subst("&1 &2 &3", return-value, ERROR-STATUS:get-message(1), ERROR-STATUS:get-message(2)).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostname in g#library
  ...
  ,output v-orgname
    ...
    assign
        ...
        v-weight = string(ub.recipe.portion-weight, "->>>,>>>,>>9.999") + " кг"
    .
    run gds-attr-value(
        ub.goods.gds-code,
        'calories':U,
    ...
    first ub.goods no-lock
        ...
        assign
            ...
            tt-line.gds-unit = ub.goods.unit-base
            tt-line.brutto = ub.recipe-gds.brutto-qnty
            tt-line.netto = ub.recipe-gds.qnty
            tt-line.final-weight = tt-line.netto
            tt-line.waste-pc = ub.recipe-gds.coeff-waste
...
procedure write-data:
    ...
    hw:insert-attribute ("fat", string(if v-fat = ? then 0 else v-fat, "->>>,>>>,>>9.999")).
    hw:insert-attribute ("protein", string(if v-protein = ? then 0 else v-protein, "->>>,>>>,>>9.999")).
    hw:insert-attribute ("carbohydrate", string(if v-carbohydrate = ? then 0 else v-carbohydrate, "->>>,>>>,>>9.999")).
    hw:insert-attribute ("calories", string(if v-calories = ? then 0 else v-calories, "->>>,>>>,>>9.999")).
    for each tt-line no-lock:
        ...
        hw:insert-attribute ("brutto", string(tt-line.brutto, "->>>,>>>,>>9.999")).
        hw:insert-attribute ("netto", string(tt-line.netto, "->>>,>>>,>>9.999")).
        hw:insert-attribute ("netto-1",  if tt-line.netto-1 = ? then "" else string(tt-line.netto-1, "->>>,>>>,>>9.999") ).
        hw:insert-attribute ("waste-pc", string(tt-line.waste-pc, "->>9.999")).
        hw:insert-attribute ("final-weight", string(tt-line.final-weight, "->>>,>>>,>>9.999")).
        hw:end-element ("line").
    end.
    hw:end-element ("card").
    hw:end-element ("rep").
