...
PROCEDURE adm-disable :
    DISABLE RECT-10 RECT-12 RECT-14 RECT-15 RECT-13 ie Classify ep ee re es rs Itog we vt iv ev rv em wm im ost-1 ost-2 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-10 RECT-12 RECT-14 RECT-15 RECT-13 ie Classify ep ee re es rs Itog we vt iv ev rv em wm im ost-1 ost-2 WITH FRAME F-Main.
...
run cur-time in this-procedure ( output v-today
  ...
  if x-SelectGood = 1 Then DO:
            run rep/r-2-prt1.p
                   (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input Itog,   input tog-zero,  input ost-1,    input ost-2   ).
  End.
  Else DO:
            run rep/r-2-prt3.p
                   (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input Itog,   input tog-zero,  input ost-1,    input ost-2   ).
...
if itog then ReportHeader = (ReportHeader)+  chr(10) + 'только итоги'.
 ReportHeader = (ReportHeader) +  chr(10) + '*** ТОЛЬКО товары со шкалами и со вложенностью до 2го уровня *** '.
