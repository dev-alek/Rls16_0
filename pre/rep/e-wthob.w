...
define temp-table temp_twowin_itemsSelected no-undo
    ...
    index im
        itm-key
.
define variable v-twowin13-itm-key    as integer      no-undo.
...
:
    assign
        v-twowin13-itm-key = v-twowin13-itm-key + 1
    .
    create temp_twowin_items.
    assign
        temp_twowin_items.itm-key      = v-twowin13-itm-key
...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-wthob.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-7 cb-ob-tal cb-ob-liter cb-ob-rubl bt-doc-type sl-doc-type WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 cb-ob-tal cb-ob-liter cb-ob-rubl bt-doc-type sl-doc-type WITH FRAME F-Main.
...
:
    ...
    then do:
        ...
        assign
            sl-doc-type :list-item-pairs in frame F-Main = v-ext-doc-type-list
        .
    end.
end.
END PROCEDURE.
...
:
    ...
    .
    if sl-doc-type :list-item-pairs in frame F-Main = "< Все >,''":U
    then do:
        assign
...
:
    ...
    .
    assign
        v-list-edt-full  = 'приход внешний':U
                + ",":U + 'расход внешний':U
                + ",":U + 'приход внутр. в зону погашения':U
                + ",":U + 'расход внутр. из зоны погашения':U
                + ",":U + 'возврат внутр. в зону погашения':U
                + ",":U + 'приход внутр. в своб. зону':U
                + ",":U + 'расход внутр. из своб. зоны':U
                + ",":U + 'возврат внутр. в своб зону':U
                + ",":U + 'погашение через кассу':U
                + ",":U + 'погашение за реализованное топливо':U
                + ",":U + 'возврат от покупателя':U
                + ",":U + 'уничтожение в свободной зоне':U
                + ",":U + 'уничтожение в зоне погашения':U
                + ",":U + 'обмен':U
    .
