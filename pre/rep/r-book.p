...
:
    ...
    put stream excel-line unformatted
        ...
        CHR(9)   buf_temp_line-data.docDateCode
        CHR(9)   buf_temp_line-data.payDate
        CHR(9)   buf_temp_line-data.inDate
        CHR(9)   buf_temp_line-data.cliName
        CHR(9)   buf_temp_line-data.cliInn
        CHR(9)   buf_temp_line-data.cliKPP
        CHR(9)   buf_temp_line-data.countryGds
        CHR(9)   buf_temp_line-data.sumRubl
        CHR(9)   buf_temp_line-data.sumVAT20
        CHR(9)   buf_temp_line-data.VAT20
        CHR(9)   buf_temp_line-data.sumVAT10
        CHR(9)   buf_temp_line-data.VAT10
        CHR(9)   buf_temp_line-data.sumVAT0
        CHR(9)   buf_temp_line-data.sumVATno
        chr(10)
    .
end.
end procedure.
...
if session :set-wait-state( "compiler" ) then.
  ...
  find first clients no-lock where clients.obj-code = v-cntxt-host-code-obj and clients.obj-type = 'орг':U .
  find first firm no-lock where firm.firm-code = v-cntxt-host-code-obj .
  run PrintTitul in this-procedure .
  run bookxl-write-cell-data in this-procedure ( input "h_organization":U, input clients.obj-name ).
  run bookxl-write-cell-data in this-procedure ( input "h_inn":U         , input firm.inn + "/" + firm.kpp ).
  run bookxl-write-cell-data in this-procedure ( input "h_startDate":U   , input string(x-date-start,"99/99/9999") ).
  run bookxl-write-cell-data in this-procedure ( input "h_endDate":U     , input string(x-date-end,"99/99/9999") ).
  ...
  for each buf_schet-fact-doc no-lock
    where buf_schet-fact-doc.host-code  = v-cntxt-host-code-obj
      ...
      AND buf_schet-fact-doc.doc-type   = 'при':U
  :
...
End.
   if v-button-stop then  DO:
    ...
    put stream PrnLibStream
      "|"  at 1   Counter1       format ">>>9"
      "|"  at 6   buf_schet-fact-doc.doc-date format "99/99/99"
      "|"  at 20   buf_schet-fact-doc.pay-date format "99/99/99"
      "|"  at 29   buf_schet-fact-doc.in-date  format "99/99/99"
      "|"  at 38   buf_schet-fact-doc.cli-name format "X(38)"
      "|"  at 77   buf_schet-fact-doc.cli-inn  format "X(15)"
      "|"  at 93   buf_schet-fact-doc.country  format "X(15)"
      "|"  at 109   buf_schet-fact-doc.sum-rubl        format "->>>>>>>>>9.99"
      "|"  at 124   buf_schet-fact-doc.sum-VAT-20-rubl format "->>>>>>>>>9.99"
      "|"  at 139  buf_schet-fact-doc.VAT-20-rubl     format "->>>>>9.99"
      "|"  at 150  buf_schet-fact-doc.sum-VAT-10-rubl format "->>>>>>>>>9.99"
      "|"  at 165  buf_schet-fact-doc.VAT-10-rubl     format "->>>>>9.99"
      "|"  at 176  buf_schet-fact-doc.sum-VAT-0-rubl  format "->>>>>9.99"
      "|"  at 187  buf_schet-fact-doc.sum-VAT-no-rubl format "->>>>>9.99"
      "|"  at 198
      skip
      "|"  at 1
      "|"  at 6   buf_schet-fact-doc.book-code format "X(12)"
      "|"  at 20
      "|"  at 29
      "|"  at 38
      "|"  at 77   buf_schet-fact-doc.cli-kpp  format "X(15)"
      "|"  at 93   buf_schet-fact-doc.gtd  format "X(15)"
      "|"  at 109
      "|"  at 124
      "|"  at 139
      "|"  at 150
  ...
  run bookxl-write-cell-data in this-procedure ( input "it_sum":U ,      input string( v-sum1 ) ).
  run bookxl-write-cell-data in this-procedure ( input "it_sumVAT20":U , input string( v-sum2 ) ).
  run bookxl-write-cell-data in this-procedure ( input "it_VAT20":U ,    input string( v-sum3 ) ).
  run bookxl-write-cell-data in this-procedure ( input "it_sumVAT10":U , input string( v-sum4 ) ).
  run bookxl-write-cell-data in this-procedure ( input "it_VAT10":U ,    input string( v-sum5 ) ).
  run bookxl-write-cell-data in this-procedure ( input "it_sumVAT0":U ,  input string( v-sum6 ) ).
  run bookxl-write-cell-data in this-procedure ( input "it_sumVATno":U , input string( v-sum7 ) ).
  put stream PrnLibStream  Line format "X(198)"
      "|"  at 1
       "ВСЕГО" format "X(29)" at 38
      "|"  at 109   v-sum1 format "->>>>>>>>>9.99"
      "|"  at 124   v-sum2 format "->>>>>>>>>9.99"
      "|"  at 139  v-sum3 format "->>>>>9.99"
      "|"  at 150  v-sum4 format "->>>>>>>>>9.99"
      "|"  at 165  v-sum5 format "->>>>>9.99"
      "|"  at 176  v-sum6 format "->>>>>9.99"
      "|"  at 187  v-sum7 format "->>>>>9.99"
      "|"  at 198
      skip
      Line format "X(198)"   skip   "    Гл. бухгалтер" format "X(30)"
  .
...
procedure is-page :
  do on error undo, return error return-value :
    if line-counter( PrnLibStream ) + 3 > page-size( PrnLibStream ) then do:
      put stream PrnLibStream  skip Line format "X(198)" skip "продолжение - на следующей странице" AT 30 SKIP .
...
procedure PrintTitul :
  ...
  :
    ...
    .
    put stream PrnLibStream   string( "Страница" ) AT 145 PAGE-NUMBER( PrnLibStream ) AT 155 FORMAT ">>9" SKIP .
    put stream PrnLibStream  Line format "X(198)"  skip
      "|"  at 1   " № "                        format "X(4)"
      "|"  at 6   "Дата и номер"               format "X(12)"
      "|"  at 20   "Дата"                       format "X(8)"
      "|"  at 29   "Дата"                       format "X(8)"
      "|"  at 38   " Наименование поставщика"   format "X(25)"
      "|"  at 77   "ИНН/КПП"     format "X(10)"
      "|"  at 93   "Страна"                     format "X(14)"
      "|"  at 109   "Всего"                      format "X(10)"
      "|"  at 124   "В том числе покупки, облагаемые налогом по ставке"                format "X(50)"
      "|"  at 198
      skip
      "|"  at 1   " п/п"                       format "X(4)"
      "|"  at 6   "счета-фактуры"              format "X(12)"
      "|"  at 20   "оплаты"                     format "X(8)"
      "|"  at 29   "оприход."                   format "X(8)"
      "|"  at 38
      "|"  at 77   "поставщика"                 format "X(10)"
      "|"  at 93   "происхождения"              format "X(14)"
      "|"  at 109   "покупок,"                   format "X(10)"
      "|"  at 124   Line format string("X(" + string( 198 - 124 - 1 ) + ")")
      "|"  at 198
      skip
      "|"  at 1
      "|"  at 6   "продавца"  format "X(12)"
      "|"  at 20   "счета-"     format "X(8)"
      "|"  at 29   "товара"     format "X(8)"
      "|"  at 38
      "|"  at 77
      "|"  at 93   "товара."     format "X(8)"
      "|"  at 109   "включая НДС"                      format "X(11)"
      "|"  at 124   "      18 %"    format "X(12)"
      "|"  at 150  "      10 %"    format "X(12)"
      "|"  at 176  "    0 %"    format "X(9)"
      "|"  at 187  "покупки"    format "X(10)"
      "|"  at 198
      skip
      "|"  at 1
      "|"  at 6
      "|"  at 20   "фактуры"     format "X(8)"
      "|"  at 29
      "|"  at 38
      "|"  at 77
      "|"  at 93  "Номер ГТД"      format "X(14)"
      "|"  at 109
      "|"  at 124  Line format string("X(" + string( 187 - 124 - 1 ) + ")")
      "|"  at 187 "освобожд."  format "X(10)"
      "|"  at 198
      skip
      "|"  at 1
      "|"  at 6
      "|"  at 20   "продавца"     format "X(8)"
      "|"  at 29
      "|"  at 38
      "|"  at 77
      "|"  at 93
      "|"  at 109
      "|"  at 124   "стоим. без НДС"   format "X(14)"
      "|"  at 139  "сумма НДС"        format "X(9)"
      "|"  at 150  "стоим. без НДС"   format "X(14)"
      "|"  at 165  "сумма НДС"        format "X(9)"
      "|"  at 176
      "|"  at 187 "от налога"  format "X(9)"
      "|"  at 198
      skip  Line format "X(198)"  skip .
    .
  end.
end procedure.
