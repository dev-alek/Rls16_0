...
:
  ...
  :
    ...
    if ub.shift-obj.status_ <> 'зкр':U then do:
            run write-to-log in this-procedure ( input substitute("На объекте &1 смена &2 с датой начала &3&4"  +                                   "еще не закрыта!&4Отчет сделать нельзя!"                                  , obj-list.obj-name                                 ,ub.shift-obj.shift-num                                 ,string(ub.shift-obj.shift-date,"99/99/9999")                                 , chr(10))).
      return.
    end.
...
if not available temp-shift-obj Then DO:
    run write-to-log in this-procedure ( input substitute("Нет смены &1 с датой начала &2&3" +                               "Исправьте запрашиваемые данные!"                               , X-shift-start                               ,string(X-date-start,"99/99/9999")                               , chr(10) )).
  return.
End.
assign
