...
def new  shared   var F-ostatok-start   as   char  no-undo.
def new  shared   var F-ostatok-End     as   char  no-undo.
def new  shared  var ostatok-start     as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
def new  shared  var ostatok-End       as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
...
def new  shared  var Inv2              as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
...
def new  shared  var p-ostatok-start     as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
def new  shared  var p-ostatok-End       as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
...
.
def new  shared  var o-ostatok-start     as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
def new  shared  var o-ostatok-End       as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
...
def new shared var B1-ostatok-start            as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
def new shared var B1-ostatok-End              as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
...
def new shared var B2-ostatok-start            as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
def new shared var B2-ostatok-End              as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
...
def new shared var Bi-ostatok-start            as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
def new shared var Bi-ostatok-End              as   decimal EXTENT 10 Format "->>>>>>>>>9.<<<" no-undo.
...
if  xtog-lavel-2  then  xlavel        =  xvar-lavel-2 .
 if postname = 'все':U or postname = "" or postname = ?   then do:
...
view stream outstream frame bottomframe .
  ...
  if xtogobj = false then do:
    ...
    for each all-obj-list :
        ...
        find first obj-list .
        PUT stream  OutStream  UNFORMATTED "ОБЪЕКТ (" + obj-list.obj-type + string(obj-list.obj-code) + ") " + obj-list.obj-name at 1 format "x(150)" skip.
