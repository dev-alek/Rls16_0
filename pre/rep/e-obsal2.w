...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-7 RECT-8 contr-code BUTTON-contr is-real snum is-fo itog-contract is-fin itog-only is-date WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-7 RECT-8 contr-code BUTTON-contr is-real snum is-fo itog-contract is-fin itog-only is-date WITH FRAME F-Main.
...
DO:
  run str/cont-all.w (input parParentProc, input v-cntxt-host-code-obj, input "b-sel", input 'фирма':U, input ?,
                  input ?, input ?, input ?, input "current":u, input 'рас':U, input-output contr-list).
...
then do:
  ...
  assign  frame F-Main  is-real is-fin is-fo .
  display is-real is-fin is-fo with frame F-Main.
...
END PROCEDURE.
PROCEDURE my-var :
  Assign frame F-Main itog-only itog-contract is-date is-fin is-fo is-real .
