...
PROCEDURE adm-disable :
    DISABLE RECT-2 RECT-client RECT-4 selectcard T-zeros T-legacy T-subsid T-imp T-obj-detal up-levelt WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-2 RECT-client RECT-4 selectcard T-zeros T-legacy T-subsid T-imp T-obj-detal up-levelt WITH FRAME F-Main.
...
END.
ON VALUE-CHANGED OF T-obj-detal IN FRAME F-Main
DO:
    assign T-obj-detal.
    assign T-zeros.
    if T-obj-detal = yes then
    do:
        v-activate-v-tmp-t-zeros = 1.
    ...
    do:
        if v-activate-v-tmp-t-zeros = 1 then T-zeros = v-tmp-T-zeros.
        T-zeros:screen-value in frame F-Main = string(T-zeros).
        assign T-zeros.
        enable
            T-zeros
...
DO:
    ...
    do:
        assign T-subsid = no.
        display T-subsid with frame F-Main.
    end.
END.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  IF ERROR-STATUS:ERROR OR v-conf-type <> 'L':U THEN
    v-dcoveris = "no".
  if v-dcoveris <> "yes" then
    disable
      t-legacy
