...
.
DEFINE NEW SHARED TEMP-TABLE times NO-UNDO
...
PROCEDURE adm-disable :
    DISABLE RECT-2 RECT-3 RECT-client selectcard TotalOnly T-zeros T-legacy T-subsid T-imp T-time up-levelt WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-2 RECT-3 RECT-client selectcard TotalOnly T-zeros T-legacy T-subsid T-imp T-time up-levelt WITH FRAME F-Main.
...
DO:
 ...
 END CASE.
 display selectcard with frame F-Main.
END.
ON VALUE-CHANGED OF T-imp IN FRAME F-Main
DO:
  assign T-imp.
  display
    T-imp
...
END.
ON VALUE-CHANGED OF T-zeros IN FRAME F-Main
DO:
    assign TotalOnly .
END.
ON VALUE-CHANGED OF TotalOnly IN FRAME F-Main
DO:
    assign TotalOnly .
    if TotalOnly then do:
        ENABLE UpLevel with frame F-Main .
        apply "entry" to UpLevel in frame F-Main .
    end.
    else do:
        UpLevel = 0 .
        DISPLAY UpLevel with frame F-Main .
        DISABLE UpLevel with frame F-Main .
    end.
END.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  ...
  Up-Levelt  = Up-Levelt +
  ( if v-curr-r-b = 'base':U then " (вал.)" else " (руб.)" ) .
  DIsplay Up-levelt
  WITH frame F-Main.
...
assign
frame F-Main selectcard
frame F-Main TotalOnly
frame F-Main UpLevel
frame F-Main T-time
frame F-Main T-Zeros
.
CASE X-selectgood:
  when 3 then do:
...
CASE DcardMode:
    when "ALL":U then do:
        run rep/e-xldcd.p (
                           ...
                           ,input T-legacy
                           ,input t-subsid
                           ,input ( if X-SelectGood = 1 then "TRUE" else (IF num-g# > 1 then "LIST" else "ONE"))
                           ,input T-imp
                           )
                .
    end.
  ...
  if X-SelectGood = 1 then do:
    ...
    CASE X-selectgood:
      ...
      when 5 then do:
        ...
        gds-list.artic chr(32) gds-list.prod-type gds-list.prod-code chr(32) gds-list.gds-name
        skip(0) .
      end.
    END CASE.
    ...
    if first-of( dcards.d-card ) then do:
      ...
      if NOT TotalOnly then do:
        if dcards.b-code <> 0 then do:
          ...
          IF AVAIL gds-prt AND NOT gds-prt.node-name = '_Пустая шкала':U
          then
...
ReportHeader = "Покупатели: " +
                           radio-label(string(selectcard), selectcard:radio-buttons) + chr(10) +
               (if TotalOnly then totalOnly:label else "") + chr(10) +
               (If Uplevel > 0
                then string((UpLevel:label + " " + string(Uplevel)) )
                else "") +
                (IF T-time then string(chr(10) + "Выборочно по времени") else "") +
                (IF T-legacy then string(chr(10) + "С учетом перевыпуска карт") else "") +
                (IF T-subsid then string(chr(10) + "С учетом дополнительных карт") else "")
                 .
END PROCEDURE.
