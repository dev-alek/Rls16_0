...
End.
  ...
  for each temp-parts :
    if xShowZero = no and temp-parts.fact-qnty = 0 and temp-parts.free-qnty = 0 then next .
    ...
    if x-SET_val_TYPE = 1 then do:
      if v-curr-r-b = 'base':U then v-cur-base = v-cur-base * v-base-rate / v-base-scale .
      assign
        temp-gds.zak-sum  = temp-gds.zak-sum  + temp-parts.price-rubl * temp-parts.fact-qnty
      .
    end.
    else do:
      if v-curr-r-b <> 'base':U then v-cur-base = v-cur-base * v-base-scale / v-base-rate .
      assign
  ...
  run CalcWaitQnty (input 'ie':U ) .
  run CalcWaitQnty (input 'iv':U ) .
  run CalcWaitQnty (input 're':U ) .
  run CalcWaitQnty (input 'rs':U ) .
  run CalcWaitQnty (input 'im':U ) .
...
output stream OutStream to value( string( session:temp-directory +
  ...
  CASE xClassify :
    when "no-classify":U  then DO:
      if tog-obj then do:
        for each temp-gds  break by temp-gds.obj-type by temp-gds.obj-code  by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        ...
        for each temp-gds  break by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
          run PrintLine        in this-procedure .
        end.
      end.
    End.
...
procedure Run1 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds  break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.grp-name by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
      ...
      for each temp-gds  break by temp-gds.grp-name by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.grp-name) then run PrintName ( temp-gds.grp-name ) .
        run PrintLine        in this-procedure .
        if last-of(temp-gds.grp-name) then do:
          run PrintItog ("Всего по группе " + temp-gds.grp-name, all1-fact-qnty, all1-free-qnty, all1-zak-sum, all1-naz-sum, all1-wait-qnty, 1).
...
procedure Run2 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.prod-type by temp-gds.prod-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
      ...
      for each temp-gds break by temp-gds.prod-type by temp-gds.prod-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.prod-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.prod-type and  buf_clients.obj-code = temp-gds.prod-code .
          run PrintName ( buf_clients.obj-name ) .
        end.
...
procedure Run3 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.prod-type by temp-gds.prod-code by temp-gds.grp-name by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
        if first-of(temp-gds.prod-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.prod-type and  buf_clients.obj-code = temp-gds.prod-code .
      ...
      for each temp-gds break by temp-gds.prod-type by temp-gds.prod-code by temp-gds.grp-name by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.prod-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.prod-type and  buf_clients.obj-code = temp-gds.prod-code .
          run PrintName ( "Производитель " + buf_clients.obj-name ) .
        end.
...
procedure Run4 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.grp-name by temp-gds.prod-type by temp-gds.prod-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
        if first-of(temp-gds.grp-name) then  run PrintName ( "Группа " + temp-gds.grp-name ) .
        if first-of(temp-gds.prod-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.prod-type and  buf_clients.obj-code = temp-gds.prod-code .
      ...
      for each temp-gds break by temp-gds.grp-name by temp-gds.prod-type by temp-gds.prod-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.grp-name) then  run PrintName ( "Группа " + temp-gds.grp-name ) .
        if first-of(temp-gds.prod-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.prod-type and  buf_clients.obj-code = temp-gds.prod-code .
          run PrintName ( "Производитель " + buf_clients.obj-name ) .
...
procedure Run5 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.post-type by temp-gds.post-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
      ...
      for each temp-gds break by temp-gds.post-type by temp-gds.post-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.post-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.post-type and  buf_clients.obj-code = temp-gds.post-code .
          run PrintName ( buf_clients.obj-name ) .
        end.
...
procedure Run6 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.post-type by temp-gds.post-code by temp-gds.grp-name by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
        if first-of(temp-gds.post-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.post-type and  buf_clients.obj-code = temp-gds.post-code .
      ...
      for each temp-gds break by temp-gds.post-type by temp-gds.post-code by temp-gds.grp-name by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.post-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.post-type and  buf_clients.obj-code = temp-gds.post-code .
          run PrintName ( "Поставщик " + buf_clients.obj-name ) .
        end.
...
procedure Run7 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.grp-name by temp-gds.post-type by temp-gds.post-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
        if first-of(temp-gds.grp-name) then  run PrintName ( "Группа " + temp-gds.grp-name ) .
        if first-of(temp-gds.post-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.post-type and  buf_clients.obj-code = temp-gds.post-code .
      ...
      for each temp-gds break by temp-gds.grp-name by temp-gds.post-type by temp-gds.post-code by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.grp-name) then  run PrintName ( "Группа " + temp-gds.grp-name ) .
        if first-of(temp-gds.post-code) then do:
          find first buf_clients no-lock where buf_clients.obj-type = temp-gds.post-type and  buf_clients.obj-code = temp-gds.post-code .
          run PrintName ( "Поставщик " + buf_clients.obj-name ) .
...
procedure Run8 :
  do on error undo, return error return-value :
    if tog-obj then do:
      for each temp-gds break by temp-gds.obj-type by temp-gds.obj-code by temp-gds.vat-pc by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.obj-code) then run PrintName ( temp-gds.obj-name ) .
      ...
      for each temp-gds break by temp-gds.vat-pc by if xSortType = "sort-code" then  temp-gds.sb-code Else (if xSortType = "sort-artic" then  temp-gds.artic Else  temp-gds.gds-name) :
        if first-of(temp-gds.vat-pc) then run PrintName ( "НДС " + string(temp-gds.vat-pc) + " %" ) .
        run PrintLine        in this-procedure .
        if last-of(temp-gds.vat-pc) then run PrintItog ("Всего по НДС " + string(temp-gds.vat-pc) + " %", all1-fact-qnty, all1-free-qnty, all1-zak-sum, all1-naz-sum, all1-wait-qnty, 1).
      end.
  ...
  do on error undo, return error return-value :
    ...
    for each buf_doc-line no-lock
      ...
      for each buf_parts no-lock
        ...
        :
        if ( p-ext-doc-type = 'vt':U or p-ext-doc-type = 'vp':U ) and buf_parts.fact-qnty <= 0 then next .
        if tog-obj = true then do:
          find first temp-gds
...
procedure ColumnTitle :
  do on error undo, return error return-value :
    put stream outstream  skip cur-time-print() format "x(35)"   "Цены указаны в "  (if x-SET_val_TYPE = 1 then "РУБ" else x-base-type )  string( "Страница" ) AT 100 PAGE-NUMBER( outstream ) FORMAT ">>>9" SKIP .
  ...
  do on error undo, return error return-value :
    ...
    if xSumsOnly = no then do:
      ...
      if use-column[1]  = yes then do:
        ...
        if Make-Excel then  put   stream ForExcel unformatted  string(num-line)   CHR(9) .
      end.
      if use-column[2]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.sb-code     format "X(10)" .
        if Make-Excel then  put   stream ForExcel unformatted  temp-gds.sb-code   CHR(9) .
      end.
      if use-column[3]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.artic       format "X(16)" .
        if Make-Excel then  put   stream ForExcel unformatted  temp-gds.artic   CHR(9) .
      end.
      if use-column[4]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.gds-name    format "X(40)" .
        if Make-Excel then  put   stream ForExcel unformatted  temp-gds.gds-name   CHR(9) .
      end.
      if use-column[5]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.unit-base   format "X(3)" .
        if Make-Excel then  put   stream ForExcel unformatted   temp-gds.unit-base  CHR(9) .
      end.
      if use-column[6]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.fact-qnty   format "->>>>>>>>9.999" .
        if Make-Excel then  put   stream ForExcel unformatted  excel-qnty ( temp-gds.fact-qnty )   CHR(9) .
      end.
      if use-column[7]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.free-qnty   format "->>>>>>>>9.999".
        if Make-Excel then  put   stream ForExcel unformatted  excel-qnty ( temp-gds.free-qnty )   CHR(9) .
      end.
      if use-column[8]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.zak-price   format "->>>>>>>>>>9.99" .
        if Make-Excel then  put   stream ForExcel unformatted  excel-sum ( temp-gds.zak-price )  CHR(9) .
      end.
      if use-column[9]  = yes then do:
        PUT stream OutStream  "|"   temp-gds.prod-price  format "->>>>>>>>>>9.99" .
        if Make-Excel then  put   stream ForExcel unformatted  excel-sum ( temp-gds.prod-price )  CHR(9) .
      end.
      if use-column[10] = yes then do:
        PUT stream OutStream  "|"   temp-gds.zak-sum     format "->>>>>>>>>>9.99" .
        if Make-Excel then  put   stream ForExcel unformatted  excel-sum ( temp-gds.zak-sum )  CHR(9) .
      end.
      if use-column[11] = yes then do:
        PUT stream OutStream  "|"   temp-gds.naz-sum     format "->>>>>>>>>>9.99" .
        if Make-Excel then  put   stream ForExcel unformatted  excel-sum ( temp-gds.naz-sum )  CHR(9) .
      end.
      if use-column[12] = yes then do:
        PUT stream OutStream  "|"   temp-gds.naz-prc     format "->>>>9.99" .
        if Make-Excel then  put   stream ForExcel unformatted  excel-sum ( temp-gds.naz-prc )  CHR(9) .
      end.
      if use-column[13] = yes then do:
        PUT stream OutStream  "|"   temp-gds.wait-qnty   format "->>>>>>>9.999" .
       if Make-Excel then  put   stream ForExcel unformatted  excel-qnty ( temp-gds.wait-qnty )   CHR(9) .
      end.
...
procedure PrintName :
  do on error undo, return error return-value :
    ...
    if Make-Excel then  put   stream ForExcel unformatted   str  chr(10) .
  end.
end procedure.
procedure PrintItog :
  do on error undo, return error return-value :
  ...
  define input  parameter  level        as integer   no-undo .
    ...
    do i = 1 to 5 :
      if i = 3 and use-column[3] then  if Make-Excel then  put   stream ForExcel unformatted   str .
      if use-column[i]  = yes then   if Make-Excel then  put   stream ForExcel unformatted   CHR(9)  .
    end.
    if use-column[6]  = yes then do:
      PUT stream OutStream  "|"   sum-fact-qnty   format "->>>>>>>>9.999" .
     if Make-Excel then  put   stream ForExcel unformatted  excel-qnty ( sum-fact-qnty )   CHR(9) .
    end.
    if use-column[7]  = yes then do:
      PUT stream OutStream  "|"   sum-free-qnty   format "->>>>>>>>9.999".
     if Make-Excel then  put   stream ForExcel unformatted  excel-qnty ( sum-free-qnty )   CHR(9) .
    end.
    if use-column[8]  = yes then do:
    ...
    if use-column[10] = yes then do:
     ...
     if Make-Excel then  put   stream ForExcel unformatted  excel-sum ( sum-zak-sum )  CHR(9) .
    end.
    if use-column[11] = yes then do:
      PUT stream OutStream  "|"   sum-naz-sum     format "->>>>>>>>>>9.99" .
     if Make-Excel then  put   stream ForExcel unformatted  excel-sum ( sum-naz-sum )  CHR(9) .
    end.
    if use-column[12] = yes then do:
    ...
    if use-column[13] = yes then do:
     ...
     if Make-Excel then  put   stream ForExcel unformatted  excel-qnty ( sum-wait-qnty )   CHR(9) .
    end.
    if Make-Excel then  put   stream ForExcel unformatted chr(10) .
    PUT stream OutStream "|" skip  .
    run is-page .
