...
define temp-table temp_onewin_itemsSelected no-undo
    ...
    index im
        itm-key
.
define variable v-onewin13-itm-key    as integer      no-undo.
...
:
    ...
    if available buf_temp_onewin_items then do:
      v-onewin13-itm-key = buf_temp_onewin_items.itm-key.
    end.
    else do:
      v-onewin13-itm-key = 0.
    end.
    assign
        v-onewin13-itm-key = v-onewin13-itm-key + 1
    .
    create buf_temp_onewin_items.
    assign
    buf_temp_onewin_items.itm-key      = v-onewin13-itm-key
...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-wrsttl.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-7 ed-grp-obj rs-cli-grp bt-cli-grp ed-grp-obj-out rs-cli-grp-out bt-cli-grp-out bt-cli-out ed-wth rs-wth bt-wth tg-price-detail WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 ed-grp-obj rs-cli-grp bt-cli-grp ed-grp-obj-out rs-cli-grp-out bt-cli-grp-out bt-cli-out ed-wth rs-wth bt-wth tg-price-detail WITH FRAME F-Main.
...
DO:
  ...
  case rs-wth :
    when 1 then do:
      ...
      :
        assign
          v-wth-list = v-wth-list + string(buf_wealth.wth-code) + chr(44)
        .
      end.
      display
        ed-wth
...
PROCEDURE convert-wth-code :
  ...
  :
    ...
    :
      ...
      then do:
        assign
          p-recid-list = p-recid-list + string( recid(buf_wealth) ) + chr(44)
        .
      end.
    end.
    assign
      p-recid-list = trim( p-recid-list, chr(44) )
    .
  end.
END PROCEDURE.
PROCEDURE convert-wth-recid :
  ...
  :
    ...
    :
      ...
      then do:
        assign
          p-code-list = p-code-list + string( buf_wealth.wth-code ) + chr(44)
        .
      end.
    end.
    assign
      p-code-list = trim( p-code-list, chr(44) )
    .
  end.
END PROCEDURE.
...
:
   ...
   find first thbjattr_thbj-attr
        where thbjattr_thbj-attr.obj-code  = 0
          ...
          and thbjattr_thbj-attr.prop-code = 'cligrplist':U
   ...
   .
    run uf-get in this-procedure ( input 'wrsttl1':U
    ...
    then do:
      ...
      then do:
        ...
        :
          assign
            v-grp-code = integer( entry( v-i, v-cli-grp-list-tmp, chr(44) ) )
          .
          find first buf_cli-grp no-lock
            where buf_cli-grp.node-code = v-grp-code
          no-error .
          if available buf_cli-grp and ( lookup( string(v-grp-code) , v-cli-grp-all , chr(44)) > 0)
          then do:
            assign
              ed-grp-obj = ed-grp-obj + buf_cli-grp.node-name + chr(10)
              v-cli-grp-list = v-cli-grp-list + string(v-grp-code) + chr(44)
            .
          end.
        end.
        assign
          v-cli-grp-list = trim( v-cli-grp-list , chr(44) )
        .
      end.
      if v-rs-wth = no
      then do:
        ...
        :
          find first buf_wealth no-lock
            where buf_wealth.wth-code = integer( entry( v-i, v-wth-list, chr(44) ) )
          no-error .
          if available buf_wealth
          then do:
            assign
              ed-wth = ed-wth + buf_wealth.wth-name + chr(10)
            .
          end.
        end.
      end.
      ...
      apply "value-changed" to rs-cli-grp in frame F-Main.
      apply "value-changed" to rs-wth in frame F-Main.
    end.
    run uf-get in this-procedure ( input 'wrsttl2':U
                                 , input  v-cntxt-userid
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          :
            assign
              v-grp-code = integer( entry( v-i, v-cli-grp-out-list-tmp, chr(44) ) )
            .
            find first buf_cli-grp no-lock
              where buf_cli-grp.node-code = v-grp-code
            no-error .
            ...
            then do:
              assign
                ed-grp-obj-out = ed-grp-obj-out + buf_cli-grp.node-name + chr(10)
                v-cli-grp-out-list = v-cli-grp-out-list + string(v-grp-code) + chr(44)
              .
            end.
          end.
          assign
            v-cli-grp-out-list = trim( v-cli-grp-out-list, chr(44) )
          .
        end.
        when 3
        then do:
          ...
          :
              find first buf_cli-grp no-lock
                where buf_cli-grp.node-code = integer(entry(v-i, v-cli-grp-all, chr(44)))
              no-error.
              ...
              then do:
                ...
                :
                  ...
                  then do:
                    ...
                    then do:
                      assign
                        ed-grp-obj-out = ed-grp-obj-out + buf_tt-object.obj-name + chr(10)
                      .
                    end.
                  end.
                end.
      ...
      apply "value-changed" to rs-cli-grp-out in frame F-Main.
    end.
    assign
...
:
   ...
   run uf-set in this-procedure ( input 'wrsttl1':U
                                , input v-cntxt-userid
                                , input v-wth-list
   ...
   run uf-set in this-procedure ( input 'wrsttl2':U
                                , input v-cntxt-userid
...
PROCEDURE my-var :
  ...
  .
  assign
    ReportHeader =  ( if rs-cli-grp-out = 3 then "Объекты погашения: " else "Группы объектов погашения: " ) + chr(10) + ed-grp-obj-out + chr(10) + chr(10) +
                    "Группы объектов реализации: " + chr(10) + ed-grp-obj + chr(10) + chr(10) +
                    "Материальные ценности: " + chr(10) + ed-wth + chr(10)
  .
END PROCEDURE.
...
:
   ...
   :
      find first buf_cli-grp
           where buf_cli-grp.node-code = INTEGER(ENTRY(v-count, v-cli-grp-all, chr(44)))
           no-lock
           no-error
           .
      IF AVAILABLE buf_cli-grp
      THEN DO:
        run onewin_add-item in this-procedure   ( input buf_cli-grp.node-code
                                                ...
                                                , INPUT ""
                                                , input (LOOKUP(STRING(buf_cli-grp.node-code), v-cli-grp-list, chr(44)) <> 0)
                                                ) .
      END.
   END.
      ...
      then do:
        assign
          v-cli-grp-list = IF v-cli-grp-list = "":U THEN temp_onewin_itemsSelected.itmExtKey
                                                    ELSE v-cli-grp-list + chr(44) + temp_onewin_itemsSelected.itmExtKey
          ed-grp-obj = ed-grp-obj + buf_cli-grp.node-name + chr(10)
        .
      end.
   END.
      ...
      then do:
        assign
          v-cli-grp-out-list = IF v-cli-grp-out-list = "":U THEN temp_onewin_itemsSelected.itmExtKey
                               ELSE v-cli-grp-out-list + chr(44) + temp_onewin_itemsSelected.itmExtKey
          ed-grp-obj-out     = ed-grp-obj-out + buf_cli-grp.node-name + chr(10)
        .
      end.
   END.
