...
:
  ...
  Find Last from-stk-line no-lock where
            ...
            from-stk-line.cat-id      = '##,##':U       and
            from-stk-line.sum-type    = 'crsa':U          and
  ...
  find last to-stk-line no-lock where
            ...
            to-stk-line.cat-id      = '##,##':U       and
            to-stk-line.sum-type    = 'crsa':U          and
...
v-grp-name = replace (ub.goods.grp-name," /","/") .
  ...
  for each ub.ot-line no-lock where
           ...
           ub.ot-line.cat-id      = '##,##':U       and
           ub.ot-line.sum-type    = 'cost':U
  :
    if not ( ub.ot-line.ext-doc-type = 'ie':U     or
             ub.ot-line.ext-doc-type = 're':U or
             ub.ot-line.ext-doc-type = 'iv':U     or
             ub.ot-line.ext-doc-type = 'rv':U or
             ub.ot-line.ext-doc-type = 'im':U    )
    then do:
      next .
    end.
  ...
  for each tt-stk-line
  :
    case substring( tt-stk-line.sum-type, length( 'sadt':U ) + 1 ) :
      when 'vt':U      or
      when 'vp':U
      then do:
