...
define input parameter p-break-name         as logical          no-undo.
...
def var vss-workfile    as character no-undo init "$Workfile: torg-13x.p $":U .
def var vss-archive     as character no-undo init "$Archive: rep/torg-13x.p $":U .
...
if not tdoc-prt then
    ...
    output stream OutStr-html to value(v-report-name-html) append convert target 'UTF-8'.
          ...
          '    <style type="text/css">' skip
                '      table ' + chr(123) + ' border-collapse: collapse; font-size:9pt; font-family:Calibri; table-layout: fixed; width: 540px; hight:  padding: 2px; margin: 2px;  ' + chr(125) skip
...
do:
    ...
    do:
        put stream OutStr-html unformatted
            ...
            '<TD STYLE="border: 1px solid black;text-align: center;">'+ (if t-doc.doc-type = 'при':U then " приход"
            else ( if t-doc.doc-type = 'возврат':U    then "возврат"
            else " расход" ))  + '</TD>'
            '</TR>'skip
                '<TR>'skip
                '<TD HEIGHT="17" style="text-align: left;border: none"></TD>'skip
                ...
                '<TD HEIGHT="17" style="text-align: left;border: none"></TD>'skip
                '<TD style="text-align: left;border: none"></TD>'skip
                '<TD style="text-align: left;border: none; font-weight: bold;">Накладная</TD>'skip
                ...
                '<TD HEIGHT="17" style="text-align: left;border: none"></TD>'skip
                '<TD style="text-align: left;border: none"></TD>'skip
                '<TD colspan = "5" style="text-align: left;border: none; font-weight: bold;">НА ВНУТРЕННЕЕ ПЕРЕМЕЩЕНИЕ, ПЕРЕДАЧУ ТОВАРОВ, ТАРЫ</TD>'skip
                '<TD style="text-align: left;border: none"></TD>'skip
                '</TR>'skip
            '<TR>'skip
            '<TD HEIGHT="17" colspan = "4" style="text-align: left;border: 1px solid black;font-weight: bold;">Отправитель</TD>'skip
...
do:
         put stream OutStr-html unformatted
        '<TR>'skip
        '<TD HEIGHT="17" colspan = "4" style="text-align: left;border: 1px solid black;">' +  v-otprav + '</TD>'skip
    ...
    do:
        do:
            ...
            '         <th  style="background-color:#ffffcc; text-align: center">Код товара</th>' skip
                '         <th  style="background-color:#ffffcc; text-align: center">Наименование ед. изм.</th>' skip
...
if p-print-prod = yes
  and sort-gr = yes then do:
  p-level = 2.
  g-level = 3.
  end.
if sort-gr = yes and p-print-prod = no then do:
    g-level = 2.
    p-level = 1.
    end.
if sort-name = yes
 ...
 if PrintRubl then
   ...
   do:
     put stream OutStr-html unformatted
           ...
           '         <td colspan = "3"  style="display: yes; text-align: left; font-weight: bold;"> Итого:</td>'  skip
     ...
     put stream OutStr-html unformatted
           '       <tr >' skip
           '         <td   style="display: yes; text-align: left; font-weight: bold;"></td>'  skip
           '         <td colspan = "3"  style="display: yes; text-align: left; font-weight: bold;">Всего по накладной:</td>'  skip
     ...
     '<TD colspan = "5" style="text-align: left;border: none"> ' + s1 + '</TD>'skip
     '</TR>'skip
     '<TR style="height: 30pt;">'skip
     '<TD style="text-align: left;border: none"></TD>'skip
     '<TD style="text-align: left;border:none;"></TD>'skip
