...
assign Sheetf.Excel-Column-Lable = "Наименование нефтепродукта" + chr(44) +
                                   "№ ТРК"                      + chr(44) +
                                   "Дата Время       час мин."  + chr(44) +
                                   "Объем (л)"                  + chr(44) +
                                   "Примечание"
       Sheetf.Sizes              = "40,5,15,11,127".
assign str1 = "":U
       str2 = "":U
       str3 = "":U
       str4 = "":U.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostname in g#library
  ...
  assign v-obj-name  = trim( obj-list.obj-name ) + fill( "_", 40 )
         v-host-nmhd = trim( v-host-name       ) + fill( "_", 40 ).
  assign ReportHeader =
    fill( " ":U, 97 ) + string( v-host-nmhd, "x(40)":U )             + chr(10) +
    fill( " ":U, 97 ) + string( v-obj-name,  "x(40)":U )             + chr(10) +
                                                                                             chr(10) +
                                                                                             chr(10) +
    fill( " ":U, 64            ) + "А К Т"                                                 + chr(10) +
    fill( " ":U, 54            ) + "прокачки топлива через ТРК"                            + chr(10) +
                                                                                             chr(10) +
                                                                                             chr(10) +
    fill( " ":U, 56            ) + string( v_today,     "x(24)":U            )             + chr(10) +
                                                                                             chr(10) +
                                                                                             chr(10) +
    '           Мы, нижеподписавшиеся, представители "' + v-host-name + '" составили'      + chr(10) +
    "настоящий Акт о том, что при проведении технических работ, проверке точности отпуска дозы (ненужное зачеркнуть) " + chr(10) +
    "через ТРК было прокачено в мерник и слито (возвращено) в резервуар в соответствии с видом топлива:"  + chr(10) +
                                                                                                            chr(10) .
  ...
  break by tt_tsf.obj-type
    ...
    if first-of( tt_tsf.gds-code ) then do:
      if Make-Excel then  put   stream ForExcel unformatted string( tt_tsf.gds-name, "x(40)":U ) CHR(9).
    end.
    else do:
      if Make-Excel then  put   stream ForExcel unformatted string( "           ":U, "x(40)":U ) CHR(9).
    end.
    if Make-Excel then  put   stream ForExcel unformatted string( tt_tsf.pump-code, "z9":U          ) CHR(9)
                string( tt_tsf.chk-date,  "99/99/99":U    ) + ", ":U +
                string( tt_tsf.chk-time,  "HH:MM":U       ) CHR(9)
                string( tt_tsf.doc-qnty,  "zzzzzzz9.99":U ) CHR(9)
                string( tt_tsf.ps,        "x(61)"         ) chr(10).
  end.
...
procedure print-header :
  put stream PrnLibStream unformatted                                                        skip( 1 )
    string( v-host-nmhd, "x(40)":U ) format "x(40)":U at 97 skip
    string( v-obj-name,  "x(40)":U ) format "x(40)":U at 97 skip( 2 )
              "А К Т"                                                       at 65            skip
...
procedure spill-fuel :
  ...
  for each obj-list no-lock where
    ...
    for each bf_chk-doc no-lock where
      ...
      if bf_chk-doc.chk-type <> integer( '17':U ) then do: next. end.
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input bf-spi_trn-doc.doc-code ,
        ...
        if available buf_sale-doc or v-tech-pass then do:
          ...
          if available bf-spi_trn-doc and
            ...
            assign
              tt_tsf.ps = substring(replace(replace(bf-spi_trn-doc.ps, chr(13), "":u), chr(10), "":u), 1, 61).
          end.
        end.
...
procedure print-footer :
  ...
  if Make-Excel then  put   stream ForExcel unformatted                                                                                     chr(10)
                                                                                                  chr(10)
              "Председатель комиссии:"                                                            chr(10)
              fill( " ":U, 49 ) +
              "_______________________    (                                                    )" chr(10)
                                                                                                  chr(10)
                                                                                                  chr(10)
              "Члены комиссии:"                                                                   chr(10)
              fill( " ":U, 50 ) +
              "_______________________    (                                                    )" chr(10)
                                                                                                  chr(10)
              fill( " ":U, 50 ) +
