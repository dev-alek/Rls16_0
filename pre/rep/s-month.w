...
define variable vss-description as character no-undo init "Окно выбора месяца для помесячных отчетов".
...
DEFINE  shared VAR vStartPoint        as      date       NO-UNDO.
DEFINE  shared VAR vEndPoint          as      date       NO-UNDO.
DEFINE  shared VAR vStrBuf          as      char       NO-UNDO.
DEFINE  shared VAR vEndLastDay      as  integer     no-undo.
...
PROCEDURE adm-disable :
    DISABLE RECT-TimePeriod TimePeriod WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-TimePeriod TimePeriod WITH FRAME F-Main.
...
define variable DatasBuf   as      character  no-undo.
    ...
    TimePeriod:list-items in frame F-Main = right-trim ( datasbuf, "," ) .
    stat = TimePeriod:scroll-to-item( TimePeriod:num-items in frame F-Main )
        in frame F-Main .
  stat = TimePeriod :scroll-to-item( TimePeriod:num-items in frame F-Main) in frame F-Main .
  TimePeriod:screen-value in frame F-Main = entry( TimePeriod:num-items, TimePeriod:list-items ) .
  apply "value-changed" to TimePeriod in FRAME F-Main.
END PROCEDURE.
