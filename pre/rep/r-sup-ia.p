...
assign v-account = ( if integer( 10 ) = 0 then 100 else integer( 10 ) ).
  ...
  for each obj-list no-lock :
    ...
    if p-SelectGood = 1 then
    do:
      for each buf_goods no-lock :
        run suppl-cycl in this-procedure .
      end.
    end.
    else do:
...
then do:
  ...
  do:
    run PrintProvOtchet in this-procedure .
  end.
end.
PROCEDURE suppl-cycl :
  for each buf_parts no-lock
    where  buf_parts.obj-type  = obj-list.obj-type
       ...
       and (buf_parts.doc-type  = 'рас':U  or
            buf_parts.doc-type  = 'возврат':U )
       ,
...
End.
   if v-button-stop then  DO:
    ...
    .
    if buf_trn-doc.ext-doc-type = 'ep':U then
      assign
  ...
  for each gds-cycl no-lock
    ...
    if FIRST-OF( gds-cycl.out-doc-type ) then do:
      if gds-cycl.type = 1 then do:
        if gds-cycl.out-doc-type = 'возврат':U then  PUT stream OutStream sym1 format "X(1)" " Возврат: " format "X(100)" sym2 AT 135 .
        else                                       PUT stream OutStream sym1 format "X(1)" " Расход: "  format "X(100)" sym2 AT 135 .
      end.
    end.
    if gds-cycl.out-doc-type = 'возврат':U then do:
      assign
    ...
    if LAST-OF( gds-cycl.out-doc-type ) then do:
      if gds-cycl.type = 1 then do:
        DISPLAY stream OutStream
          sym1 (if gds-cycl.out-doc-type = 'возврат':U then " Возврат" else " Расход") @ gds-cycl.artic
