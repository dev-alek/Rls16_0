...
:
    ...
    run r-f_t1xl-write-cell-data in this-procedure (
          input "sheetList":U
        , input "Приложение_4":U
    ).
    if printrubl
    then do:
        run r-f_t1xl-write-cell-data in this-procedure (
              input "Приложение_4_valutCode":U
            , input "0":U
        ).
    end.
    else do:
    ...
    run r-f_t1xl-write-cell-data in this-procedure (
          input "Приложение_4_subtotalList":U
        , input "":U
    ).
    run r-f_t1xl-write-cell-data in this-procedure (
          input "Приложение_4_subtotalType":U
        , input "":U
    ).
end.
end procedure.
...
output stream out-stream to value( string( session:temp-directory +
    ...
    run r-f_t1xl-write-cell-data in this-procedure (
          input "h_placeAmount1":U
        , input v-doc-places
    ).
    run r-f_t1xl-write-cell-data in this-procedure (
          input "h_placeAmount2":U
    ...
    find first clients where clients.obj-code = buf_trn-doc.boss and clients.obj-type = 'чел':U no-lock no-error.
    ...
    if buf_trn-doc.ext-doc-type = 'rv':U or buf_trn-doc.ext-doc-type = 'ev':U then do:
    ...
    then do:
      ...
      run r-f_t1xl-write-cell-data in this-procedure (
            input "h_massNetto1":U
          , input string (v-cargo-mass-netto)
      ).
      run r-f_t1xl-write-cell-data in this-procedure (
            input "h_massBruttoNetto":U
...
:
    ...
    assign
        s1 = breakstr( v-torgconf-organization, 150, input-output s1, input-output s2)
    .
    run r-f_t1xl-write-cell-data in this-procedure (
        input "h_orgFrom":U
      , input s1
    ...
    run r-f_t1xl-write-cell-data in this-procedure (
     ...
     v-temp-phone  =  "".
    assign
        s1 = breakstr( v-torgconf-torg12-cargo-value, 150, input-output s1, input-output s2)
    .
    run r-f_t1xl-write-cell-data in this-procedure (
