...
PROCEDURE adm-disable :
    DISABLE RECT-11 RECT-12 RS-method RS-cass cas-num WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-11 RECT-12 RS-method RS-cass cas-num WITH FRAME F-Main.
...
assign RS-Cass.
  if RS-cass = 'все':U then do:
    ...
    display cas-num rs-method with frame F-Main.
  end.
END.
...
ASSIGN
RS-cass:RADIO-BUTTONS IN FRAME F-Main =  "все" + chr(44) + 'все':U + chr(44) +
                                 "Выборочно" + chr(44) + "currency":U.
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'apply-layout':U ) .
END PROCEDURE.
PROCEDURE local-initialize :
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .
    apply "value-changed" to rs-method in frame F-Main.
    apply "value-changed" to rs-cass in frame F-Main.
END PROCEDURE.
PROCEDURE My-Report :
ASSIGN
...
if not found then do:
    ...
    message "Не было продаж на выбранных объектах в течение заданного Вами периода времени." view-as alert-box information .
    return.
end.
...
ReportHeader =  radio-label(string(Rs-Method), RS-Method:radio-buttons) + chr(10) +
                            "Кассы: " +
                            radio-label(string(Rs-cass), RS-cass:radio-buttons) + chr(10) +
...
FOR EACH benefits NO-LOCK BREAk BY benefits.b-code  BY benefits.out-code:
        IF FIRST-OF(benefits.out-code) then do:
            ...
            for-tot-r-b = if v-curr-r-b = 'base':U
  ...
  CASE (X-Radio-Task > 1):
    WHEN YES THEN DO:
      ...
      FOR EACH ub.chk-doc WHERE
        ...
        IF NOT ub.chk-doc.office = 'т':U  then NEXT.
        IF X-Radio-Task = 3 AND
...
IF NOT chk-doc.office = 'т':U  then NEXT.
...
IF LASt-of(chk-gds.b-code) then do:
        ...
        IF NOT avail benefits then do:
            ...
            benefits.main_ =  IF LOOKUP('топ':U, units.type) > 0 and ub.goods.gds-type = 'у':U then no else yes
