...
define temp-table temp_twowin_itemsSelected no-undo
    ...
    index im
        itm-key
.
define variable v-twowin15-itm-key    as integer      no-undo.
...
:
    assign
        v-twowin15-itm-key = v-twowin15-itm-key + 1
    .
    create temp_twowin_items.
    assign
        temp_twowin_items.itm-key      = v-twowin15-itm-key
...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-wthrd.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-7 RECT-8 RECT-9 RECT-10 is-detal rs-date is-range bt-doc-type sl-doc-type rs-wth-pl bt-wth-pl ed-wth-pl WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 RECT-8 RECT-9 RECT-10 is-detal rs-date is-range bt-doc-type sl-doc-type rs-wth-pl bt-wth-pl ed-wth-pl WITH FRAME F-Main.
...
ON CHOOSE OF bt-doc-type IN FRAME F-Main
DO:
    run select-doc-type in this-procedure.
END.
ON CHOOSE OF bt-wth-pl IN FRAME F-Main
DO:
  assign rs-wth-pl = 2 .
  run select-wth-pl in this-procedure.
  display  rs-wth-pl  ed-wth-pl with frame F-Main.
END.
ON RETURN OF f-dtFrom IN FRAME F-Main
DO:
  APPLY 'entry':U TO f-dtEnd.
  RETURN NO-APPLY.
END.
ON VALUE-CHANGED OF is-range IN FRAME F-Main
DO:
  IF SELF:CHECKED THEN DO:
      ENABLE f-dtFrom f-dtEnd with FRAME F-Main.
      f-dtFrom:BGCOLOR = 15.
      f-dtEnd:BGCOLOR = 15.
  ...
  END.
  ELSE DO:
      DISABLE f-dtFrom f-dtEnd with FRAME F-Main.
      f-dtFrom:BGCOLOR = ?.
      f-dtEnd:BGCOLOR = ?.
  END.
END.
...
DO:
  ...
  case rs-wth-pl:
    when 1 then do:
      ...
      display ed-wth-pl with frame F-Main.
      v-wth-pl-list = ''.
    end.
    when 2 then do:
...
define buffer buf_wth-place for ub.wth-place .
  ...
  do on error undo, return error :
    ...
    if error-status :error = false then do:
      ...
      if v-rs-wth-pl = no then do:
        ...
        do v-i = 1 to num-entries(v-wth-pl-lst) :
          find first buf_wth-place where RECID(buf_wth-place) = integer( entry( v-i, v-wth-pl-lst, chr(44) ) ) no-lock no-error .
          if available buf_wth-place then do:
            assign
              ed-wth-pl = ed-wth-pl + buf_wth-place.w-p-name + chr(10)
              v-wth-pl-list = IF v-wth-pl-list = "":U THEN entry( v-i, v-wth-pl-lst, chr(44) ) ELSE v-wth-pl-list + chr(44) + entry( v-i, v-wth-pl-lst, chr(44) )
            .
          end.
        end.
      end.
    ...
    display is-detal rs-date rs-wth-pl ed-wth-pl with frame F-Main.
  end.
END PROCEDURE.
PROCEDURE my-report :
    define variable v-counter               as integer      no-undo.
    define variable v-ext-doc-type-list     as character    no-undo.
assign frame F-Main is-range .
f-dtFrom = date(f-dtFrom:screen-value in frame F-Main ) no-error.
...
END PROCEDURE.
PROCEDURE my-var :
 assign frame F-Main  rs-wth-pl rs-date is-detal is-range .
def var v-sl-doc-type as char  no-undo.
define variable ii as integer   no-undo .
do ii = 1 to num-entries(sl-doc-type:list-item-pairs):
  if ii modulo 2  = 1 then do:
    v-sl-doc-type = v-sl-doc-type + entry(ii,sl-doc-type:list-item-pairs)  + chr(10).
  end.
end.
  assign  ReportHeader =   chr(10) + "Отбор документов по:" + chr(10) + entry(rs-date,"Фактической дате,Дате счета-фактуры") + chr(10)
                           + chr(10) + "Виды документов:" + chr(10) + v-sl-doc-type + chr(10)
                           + chr(10)+ "Места хранения:" + chr(10) + ed-wth-pl + chr(10)
                           + chr(10) + "Детализация по МЦ:" + chr(10) + string(is-detal,"Да/Нет") + chr(10)
                           + chr(10) + "Учитывать срок годности:" + chr(10) + string(is-range,"Да/Нет") + chr(10)
                           + chr(10) + (if is-range then substitute("Период возможного начала срока годности: &1 - &2",f-dtFrom:screen-value,f-dtEnd:screen-value) else '')
                           .
END PROCEDURE.
PROCEDURE select-doc-type :
    ...
    define variable ii as integer   no-undo .
  do with frame F-Main on error undo, return error :
    assign v-list-edt  = v-list-wdedt.
    do ii = 1 to num-entries( v-list-wdedt ) :
      if ii > 1 then assign v-list-edt-full = v-list-edt-full + ",":U .
      assign v-list-edt-full = v-list-edt-full + entry( lookup ( entry( ii, v-list-wdedt ), 'ie,ee,ii,ei,ij,ej,fj,jj,pj,oj,we,ci,ce,iy,rj,ip,ep,rp,ff,ef,rf,pc,ps,pz,df,dp,dc,de,xc':u ), 'приход внешний,расход внешний,приход внутренний,расход внутренний,приход внутри объекта,расход внутри объекта,приход внутриобъектн. в своб. зону,расход внутриобъектн. из своб. зоны,приход внутриобъектн. в зону погаш.,расход внутриобъектн. из зоны погаш.,списание,приход внешний через кассы,возврат покупателю через кассы,инвентаризация,возврат внутренний,приход внутр. в зону погашения,расход внутр. из зоны погашения,возврат внутр. в зону погашения,приход внутр. в своб. зону,расход внутр. из своб. зоны,возврат внутр. в своб зону,погашение через кассу,погашение за реализованное топливо,возврат от покупателя,уничтожение в свободной зоне,уничтожение в зоне погашения,уничтожение в зоне клиента,декларация,обмен':u ) .
    end.
...
PROCEDURE select-wth-pl :
  ...
  v-wth-pl-list = ''.
    run ref/wthplref.w
        ...
        ,input v-cntxt-obj-type
        ,input v-cntxt-obj-code
        ,input if v-cntxt-db-num = 0 then 'фирма':U else 'объект':U
