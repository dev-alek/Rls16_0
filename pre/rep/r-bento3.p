...
FOR EACH obj-list WHERE
  ...
  if choice then do:
    ...
    DOWN stream PrnLibStream 1 with FRAME PayCodes-tot .
  end.
...
if choice AND ( ObjAmount > 1 ) then do:
  FORM with frame ZUM-PayCodes-tot .
  FOR EACH obj-list WHERE
  ...
  BY ub.inkas-pay-desk.curr-code :
    ...
    if last-of( ub.inkas-pay-desk.curr-code ) AND
      ...
      DOWN stream PrnLibStream 1 with frame ZUM-PayCodes-tot.
    end.
  END.
end.
