...
for each tt-supp no-lock :
  if p-det-tov then do :
    ...
    tt-supp.name                      CHR(9)
    tt-supp.type + " " tt-supp.code   CHR(9)
                                      CHR(9)
    round(tt-supp.sum,2)              CHR(9)
    round(tt-supp.sum-vat,2)          CHR(9)
                                      CHR(9)
                                      CHR(9)
                                      CHR(9)
    round(tt-supp.sum-bonus,2)        CHR(9)
    SKIP.
    if not p-itog-prod then do :
      for each tt-gds where tt-gds.supp-type = tt-supp.type  and
        ...
        tt-gds.code       CHR(9)
        tt-gds.artic      CHR(9)
        round(tt-gds.sum,2)        CHR(9)
        round(tt-gds.sum-vat,2)    CHR(9)
        tt-gds.pct-rate   CHR(9)
        tt-gds.sum-rate   CHR(9)
        v-method          CHR(9)
        round(tt-gds.sum-bonus,2)  CHR(9)
        SKIP.
      end.
    end.
  end.
  ...
  if p-itog-prod then do :
    for each tt-prod where tt-prod.supp-type = tt-supp.type and
      ...
      if p-det-tov then do :
        if Make-Excel then  put   stream ForExcel unformatted
        "   " + tt-prod.name              CHR(9)
        tt-prod.type + " " tt-prod.code   CHR(9)
                                          CHR(9)
        round(tt-prod.sum,2)              CHR(9)
        round(tt-prod.sum-vat,2)          CHR(9)
                                          CHR(9)
                                          CHR(9)
                                          CHR(9)
        round(tt-prod.sum-bonus,2)        CHR(9)
        SKIP.
  ...
  for each buf_clients no-lock :
    ...
    for each   buf_contract where buf_contract.contract-date-beg <= x-date-start          and
                                 (buf_contract.contract-date-end >= x-date-start or
                                  ...
                                  buf_contract.status_            = 'тек':U      and
                                  buf_contract.host-code          = buf2_clients.host-code    and
                                  buf_contract.doc-type           = 'при':U     and
                                  buf_contract.cli-type           = buf_clients.obj-type  and
                                  buf_contract.cli-code           = buf_clients.obj-code  no-lock :
      ...
      if available buf_contract-specif then do :
        find first tt-gds where tt-gds.supp-type    = buf_clients.obj-type  and
                                tt-gds.supp-code    = buf_clients.obj-code  and
        ...
        if not available tt-gds then do :
          ...
          create tt-gds.
          assign  tt-gds.supp-type    = buf_clients.obj-type
                  tt-gds.supp-code    = buf_clients.obj-code
        ...
        find first tt-supp where tt-supp.type = buf_clients.obj-type and
                                 tt-supp.code = buf_clients.obj-code no-lock no-error.
        if not available tt-supp then do :
          create tt-supp.
          assign  tt-supp.type      = buf_clients.obj-type
                  tt-supp.code      = buf_clients.obj-code
                  tt-supp.name      = buf_clients.obj-name
                  tt-supp.sum       = 0
                  tt-supp.sum-vat   = 0
                  tt-supp.sum-bonus = 0
          .
        ...
        for last buf_stk-supp-line where buf_stk-supp-line.artic        = buf_gds-obj.artic                 and
                                         ...
                                         buf_stk-supp-line.cli-type     = buf_clients.obj-type              and
                                         buf_stk-supp-line.cli-code     = buf_clients.obj-code              and
                                         buf_stk-supp-line.sum-type     = 'sadt':U + 'ie':U  and
        ...
        for last buf_stk-supp-line where buf_stk-supp-line.artic        = buf_gds-obj.artic                 and
                                         ...
                                         buf_stk-supp-line.cli-type     = buf_clients.obj-type              and
                                         buf_stk-supp-line.cli-code     = buf_clients.obj-code              and
                                         buf_stk-supp-line.sum-type     = 'sadt':U + 'ep':U  and
                                         buf_stk-supp-line.fact-order  <= v-end-fact-order                  no-lock :
          assign tt-gds.sum       = tt-gds.sum + buf_stk-supp-line.sum-rubl
                 tt-gds.sum-vat   = tt-gds.sum-vat + buf_stk-supp-line.sum-rubl - buf_stk-supp-line.vat-rubl
                 tt-prod.sum      = tt-prod.sum + buf_stk-supp-line.sum-rubl
  ...
  Else for each G#CUSTOMER no-lock :
    ...
    for each   buf_contract where buf_contract.contract-date-beg <= x-date-start          and
                                 (buf_contract.contract-date-end >= x-date-start or
                                  ...
                                  buf_contract.cli-code           = G#CUSTOMER.obj-code  no-lock :
      find last  buf_contract-specif where buf_contract-specif.host-code    = buf_contract.host-code    and
                                           buf_contract-specif.contract-num = buf_contract.contract-code and
                                           buf_contract-specif.gds-code     = buf_gds-obj.gds-code      no-lock no-error.
      if available buf_contract-specif then do :
        find first tt-gds where tt-gds.supp-type    = G#CUSTOMER.obj-type  and
                                tt-gds.supp-code    = G#CUSTOMER.obj-code  and
                                tt-gds.prod-type    = buf_gds-obj.prod-type and
                                tt-gds.prod-code    = buf_gds-obj.prod-code and
                                tt-gds.code         = buf_gds-obj.gds-code  no-lock no-error.
        if not available tt-gds then do :
          find first buf_goods where buf_goods.gds-code = buf_gds-obj.gds-code no-lock.
          create tt-gds.
          assign  tt-gds.supp-type    = G#CUSTOMER.obj-type
                  tt-gds.supp-code    = G#CUSTOMER.obj-code
                  tt-gds.prod-type    = buf_gds-obj.prod-type
                  tt-gds.prod-code    = buf_gds-obj.prod-code
                  tt-gds.code         = buf_gds-obj.gds-code
                  tt-gds.name         = buf_goods.gds-name
        ...
        find first tt-supp where tt-supp.type = G#CUSTOMER.obj-type and
                                 tt-supp.code = G#CUSTOMER.obj-code no-lock no-error.
        if not available tt-supp then do :
          create tt-supp.
          assign  tt-supp.type      = G#CUSTOMER.obj-type
                  tt-supp.code      = G#CUSTOMER.obj-code
                  tt-supp.name      = G#CUSTOMER.obj-name
                  tt-supp.sum       = 0
                  tt-supp.sum-vat   = 0
                  tt-supp.sum-bonus = 0
          .
        ...
        for last buf_stk-supp-line where buf_stk-supp-line.artic        = buf_gds-obj.artic                 and
                                         ...
                                         buf_stk-supp-line.obj-type     = obj-list.obj-type                 and
                                         buf_stk-supp-line.obj-code     = obj-list.obj-code                 and
                                         buf_stk-supp-line.cli-type     = G#CUSTOMER.obj-type              and
                                         buf_stk-supp-line.cli-code     = G#CUSTOMER.obj-code              and
                                         buf_stk-supp-line.sum-type     = 'sadt':U + 'ie':U  and
                                         buf_stk-supp-line.fact-order  <= v-end-fact-order                  no-lock :
          assign tt-gds.sum       = tt-gds.sum + buf_stk-supp-line.sum-rubl
                 tt-gds.sum-vat   = tt-gds.sum-vat + buf_stk-supp-line.sum-rubl - buf_stk-supp-line.vat-rubl
