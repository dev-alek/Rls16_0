...
def var vss-workfile    as character no-undo init "$Workfile: rbblstg.p $":U .
def var vss-archive     as character no-undo init "$Archive: rep/rbblstg.p $":U .
...
if not can-find(first bb-list) then do:
  message
    "Список пустой"
...
FOR EACH bb-list USE-INDEX art NO-LOCK:
  assign
  v-ind = v-ind + 1
  .
  ...
  FIND buf_clients WHERE
     buf_clients.obj-type = bb-list.prod-type
 AND buf_clients.obj-code = bb-list.prod-code NO-LOCK .
  CREATE gds-zap .
  assign
  gds-zap.unit-base  = bb-list.unit-base
  gds-zap.prt-root     = bb-list.prt-root
  gds-zap.gds-name = bb-list.gds-name
  gds-zap.prod-type  = bb-list.prod-type
  gds-zap.prod-code = bb-list.prod-code
  gds-zap.artic          = bb-list.artic
  gds-zap.prod-name = buf_clients.obj-name
  gds-zap.grp-name = bb-list.grp-name
  gds-zap.order-num  = bb-list.order-num
  .
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  FIND buf_units WHERE
      buf_units.unit-name = bb-list.unit-base NO-LOCK.
  ...
  gds-zap.b-str     = (if gds-zap.weight-bc <> ? and gds-zap.weight-bc <> '':U then '':U else bb-list.b-str).
END.
