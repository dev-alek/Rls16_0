...
DEFINE FRAME f-doc
        ...
        goods.gds-name COLUMN-LABEL "Наименование товара ! ! ! ! " format "X(26)" space(1)
...
assign val-str = ( if Invers then currency.curr-abbr else ( if PrintRubl then "рублях" else "баз.вал" ) ).
...
PUT STREAM Out-Stream
    ...
    space(5) string( "ИНН " + t-inn + " " + CAPS( clients.obj-name ) + " (" + string(clients.obj-code) + ")"
                              + t-addres + t-phone)
                              + (if t-doc.internal = yes and t-doc.doc-type = 'при':U
...
PUT STREAM Out-Stream
    space(5) string( (if t-doc.doc-type = 'при':U AND NOT Invers then "Грузоотправитель: " else "Грузополучатель: ")
                              + "ИНН " + t-inn + " " + CAPS( clients.obj-name ) + " (" + string(clients.obj-code) + ") "
...
put stream out-stream
    ...
    space(5) "Вид операции" format "X(12)" AT 167 "| " AT 180
                    ( if t-doc.doc-type = 'при':U AND NOT Invers then " приход" else " расход" ) format "X(16)" "|" AT 198 skip
    space(5) v-single-line format  "X(19)" AT 180 skip
...
FOR  EACH doc-line where doc-line.doc-code = t-doc.doc-code NO-LOCK,
    FIRST goods WHERE goods.prod-type = doc-line.prod-type AND
                         ...
                         BREAK BY if sort-gr then  goods.grp-name Else goods.artic BY doc-line.artic   :
     FullNameGds = Trim(goods.gds-name) + " " + Trim(goods.PS) .
    gds-str1 = breakstr(FullNameGds,  26, input-output gds-str1, input-output gds-str2).
    assign j = 0.
    DO WHILE gds-str2 <> "" :
        assign gds-str = gds-str2.
        gds-str1 = breakstr(gds-str, 26, input-output gds-str1, input-output gds-str2).
        assign j = j + 1.
    END.
                ...
                define variable old-qnty like  tqnty no-undo.
                    ...
                    if units.type = "дро,2ед" then DO:
                          run partrqst in this-procedure
                            ...
                            Assign
                              qnty-pl = v-total-parts-fact-cli-qnty
                              tqnty    = qnty-pl
                              .
                            End.
  ...
  ,output  varprice-no-vat-slt-base
                ...
                do:
                    ...
                    assign
                        ...
                        .
                      if t-doc.doc-type = 'при':U and CostPrice then
                          do:
assign
...
assign
  ...
  price-rubl-without-tax-sale = price-rubl-with-tax-sale - vat-rubl-sale - slt-rubl-sale - road-tax-rubl-sale.
    ...
    if sort-gr = true  and first-of (if sort-gr then  goods.grp-name Else goods.artic) THEN DO:
      DOWN stream Out-Stream 1 with FRAME f-doc .
...
PUT STREAM Out-Stream
    space(10) string( "Всего на сумму: " + trim( string( ( ( accum total stoim ) + (accum total SLT-gds) ), "->>>,>>>,>>>,>>>,>>9.99") ) + " (" +
                                trim( ( if Invers then currency.curr-abbr else ( if PrintRubl then "РУБ" else "баз.вал" ) ) ) + ")" ) format "X(180)"
.
