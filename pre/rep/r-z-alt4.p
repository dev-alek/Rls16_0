         ...
         RUN waitfram-show( "Обработано строк : " + string( n-nm )) .
             For each obj-list no-lock :
                for each ub.gds-obj no-lock where
                    ...
                    Assign
                      ...
                      f-cost-sum   = f-cost-sum + determined(gds-obj.fact-base)
             ...
             For each alt-obj-list no-lock :
                for each ub.gds-obj no-lock where
                    ...
                    Assign
                    f-qnty-alt       = f-qnty-alt     + ub.gds-obj.fact-qnty
                    f-cost-sum-alt   = f-cost-sum-alt + determined(gds-obj.fact-base)
        ...
        by ub.gds-prt.f-name with FRAME Zapas :
            ...
            For each alt-obj-list no-lock :
                for each ub.prt-obj no-lock where
                    ...
                    :
                    assign
                      p-qnty-alt      = p-qnty-alt       + ub.prt-obj.fact-qnty
                      p-sale-sum-alt  = p-sale-sum-alt   + ub.prt-obj.fact-qnty * ub.prt-obj.price-sale
                   .
                end.
            end.
            assign
        ...
        for each obj-list :
           for each ub.gds-obj no-lock where ub.gds-obj.obj-type =  obj-list.obj-type and
                ...
                if available gds-list then do:
                    if gds-list.gds-type   <> 'т':U or (p-show-qnty = true  and ub.gds-obj.fact-qnty <= 0 ) then do:
                          delete gds-list .
                    end.
                    else do:
                       if classify = 4 then
        ...
        for each obj-list :
           for each ub.gds-obj no-lock where ub.gds-obj.obj-type =  obj-list.obj-type and
                ...
                if not available gds-list then do:
                    if (p-show-qnty = true  and ub.gds-obj.fact-qnty > 0) or p-show-qnty = false then do:
