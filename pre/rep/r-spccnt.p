...
END. ELSE DO:
   ASSIGN
      i-gl-Host-Code      = v-host-code
      i-gl-Contract-Code  = buf_trn-doc.contract-code
      .
END.
...
output stream out-stream to value( string( session:temp-directory +
                                     ...
                                     page-size  value(62) .
    form header
        space(1) v-single-line format "X(135)" skip
...
if session :set-wait-state( "compiler" ) then.
    put stream out-stream
            buf_clients.obj-name format "X(60)" at 1 + ( ( 1 + 135 - 1) / 2 ) - ( length( buf_clients.obj-name ) / 2 )
        skip (1) space( 22 )
        ...
        skip space( 22 )
            ...
            ( if buf_trn-doc.status_ <> 'факт':U then "   Статус: " + caps(buf_trn-doc.status_) else " " )  format "X(25)"
        skip space( 22 )
            "   Номер договора:  "   buf_contract.contract-prn-code    format "X(14)"
            "   Дата: "    string(buf_contract.contract-date)         format "X(10)"
        skip space( 22 ) "Поставщик (отправитель): "     v-cli-name   format "X(60)"
        skip space( 22 ) "Покупатель (получатель): "     v-obj-name   format "X(60)"
        skip
    .
      ...
      if sort-gr = yes then do:
        if sort-name = yes then do:
          ...
          :
            ...
            if last( buf_goods.gds-name ) and line-counter( Out-Stream ) + 12 + 1 > page-size( Out-Stream ) then do:
              run PrintTitul .
            end.
          ...
          :
            ...
            if last( buf_doc-line.line-num ) and line-counter( Out-Stream ) + 12 + 1 > page-size( Out-Stream ) then do:
              run PrintTitul .
            end.
            run print-line in this-procedure (input buf_trn-doc.doc-code, input buf_doc-line.artic, input buf_doc-line.prod-type, input buf_doc-line.prod-code).
          end.
...
procedure PrintTitul :
  do on error undo, return error return-value :
    if line-counter( Out-Stream ) + 2 < page-size( Out-Stream ) then do:
      put stream out-stream skip space(1) "|" v-single-line format "X(133)" "|" .
    end.
...
procedure print-header :
  do on error undo, return error:
    ...
    put stream out-stream
    skip
    space(1)       v-single-line   format "X(135)"
    skip space(1)  "|"
        "Код"                   at center-field(1 + 1, 1 + 11, 3)
        "|"                     at 1 + 11
        "Артикул"               at center-field(1 + 11, 1 + 28, 7)
        "|"                     at 1 + 28
        "Производитель"         at center-field(1 + 28, 1 + 42, 13)
        "|"                     at 1 + 42
        "Наименование товара"   at 1 + 42 + 2
        "|"                     at 1 + 81
        "Цена "                 at center-field(1 + 81, 1 + 93, 10)
        "|"                     at 1 + 93
        "Цена"                  at center-field(1 + 93, 1 + 105, 10)
        "|"                     at 1 + 105
        "Допуст."               at center-field(1 + 105, 1 + 115, 8)
        "|"                     at 1 + 115
        "Реальный"              at center-field(1 + 115, 1 + 125, 8)
        "|"                     at 1 + 125
        "Отклон."               at center-field(1 + 125, 1 + 135, 8)
        "|"                     at 1 + 135
    skip space(1)  "|"
        "|"                     at 1 + 11
        "|"                     at 1 + 28
        "|"                     at 1 + 42
        "|"                     at 1 + 81
        "накладной"             at center-field(1 + 81, 1 + 93, 10)
        "|"                     at 1 + 93
        "специф."               at center-field(1 + 93, 1 + 105, 10)
        "|"                     at 1 + 105
        "% откл."               at center-field(1 + 105, 1 + 115, 8)
        "|"                     at 1 + 115
        "% откл."               at center-field(1 + 115, 1 + 125, 8)
        "|"                     at 1 + 125
        "|"                     at 1 + 135
    skip space(1)
        "|" v-single-line format "X(133)" "|"
    .
  end.
end procedure.
    ...
    :
      ...
      find first  buf_contract-specif-attr no-lock
        where buf_contract-specif-attr.contract-num = buf_trn-doc.contract-code
          ...
          and buf_contract-specif-attr.attr-code    = 'prc-min':U
      no-error .
      ...
      if available buf_contract-specif then do:
        put stream out-stream
            skip space(1)  "|"
                ...
                "|"   at 1 + 28
                substitute( "&1 &2", buf_goods.prod-type, buf_goods.prod-code )  format "X(13)"
                "|"   at 1 + 42
...
procedure print-group-line :
  ...
  do for buf_goods on error undo, return error :
    ...
    put stream out-stream  skip space(1) "|   ***  Группа:  "  + buf_goods.grp-name format "X(110)" "|" at 1 + 135 .
end.
end procedure.
procedure print-prod :
  do on error undo, return error :
    ...
    put stream out-stream  skip space(1) "| *** Производитель: "  + buf_clients.obj-name format "X(110)" "|" at 1 + 135 .
  end.
end procedure.
procedure print-total-result :
  do on error undo, return error :
    put stream out-stream  skip space(1)  v-single-line format "X(135)" .
  end.
end procedure.
