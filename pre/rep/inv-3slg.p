...
output STREAM Out-Stream to value( string( session:temp-directory +
  ...
  PUT stream Out-Stream UNFORMATTED  space(50) string( "ИНВЕНТАРИЗАЦИЯ " + string( tdoc-code , "X(16)") + " от " + string( tdoc-date, "99/99/9999")
     + (if trn-doc.status_ <> 'факт':U then string( "(" + CAPS(trn-doc.status_) + ")" ) else "")) format "X(100)" skip .
...
End.
  ...
  if v-sort-prod = "yes" then do:
    if sort-group = yes then do:
      if s-pole < 3 then do:
        for each temp-str no-lock break by temp-str.prod-type by temp-str.prod-code by temp-str.grp-name by if s-pole = 1 then  temp-str.gds-name Else temp-str.artic :
      ...
      if s-pole < 3 then do:
        for each temp-str no-lock break by temp-str.prod-type by temp-str.prod-code by if s-pole = 1 then  temp-str.gds-name Else temp-str.artic :
          if first-of( temp-str.prod-code) then run print-prod in this-procedure .
          run print-line in this-procedure .
        end.
      end.
    ...
    if sort-group = yes then do:
      if s-pole < 3 then do:
        for each temp-str no-lock break by temp-str.grp-name by if s-pole = 1 then  temp-str.gds-name Else temp-str.artic :
          if first-of( temp-str.grp-name) then run print-grp in this-procedure .
          run print-line in this-procedure .
        end.
      end.
      ...
      if s-pole < 3 then do:
        for each temp-str no-lock break by if s-pole = 1 then  temp-str.gds-name Else temp-str.artic :
          run print-line in this-procedure .
        end.
      end.
      else do:
  ...
  PUT stream Out-Stream Line  FORMAT "X(164)"  skip  .
...
procedure print-grp :
  do  on error undo, return error return-value  :
    ...
    PUT stream Out-Stream UNFORMATTED String("_______________Группа : " + TRIM(CAPS(temp-str.grp-name)) + Line)  FORMAT "X(164)"  skip  .
  end.
end procedure.
procedure print-prod :
  do  on error undo, return error return-value  :
    ...
    .
    DOWN stream Out-Stream 1 with FRAME invent .
    PUT stream Out-Stream UNFORMATTED String("________Производитель : " + TRIM(CAPS(buf_clients.obj-name)) + Line)  FORMAT "X(164)"  skip  .
  end.
end procedure.
procedure print-line :
  ...
  DOWN stream Out-Stream 1 with FRAME invent .
  if print-graft = false THEN  Put stream Out-Stream Line format "X(164)" SKIP.
  end.
end procedure.
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input trn-doc.doc-code ,
    ...
    if trn-doc.status_ = 'факт':U then do:
      if lookup( 'bd':U, v-attr-value ) = 0 or
         lookup( 'ad':U, v-attr-value ) = 0  then run utl/uaddsum.p (trn-doc.doc-code, no, ?, ?) no-error  .
      ...
      if lookup( 'ad':U, v-attr-value ) = 0  then assign is-after = no .
    End.
  end.
end procedure.
