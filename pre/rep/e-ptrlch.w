...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-ptrlch.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-5 RECT-6 RECT-7 tog-trans-cancell tog-rcpt-overflow tog-tech-refuel rs-grp-tech-refuell tog-trans-transfer tog-unlock-trans tog-total-tech-chk v-header v-grp-tech-refuell WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-5 RECT-6 RECT-7 tog-trans-cancell tog-rcpt-overflow tog-tech-refuel rs-grp-tech-refuell tog-trans-transfer tog-unlock-trans tog-total-tech-chk v-header v-grp-tech-refuell WITH FRAME F-Main.
...
DO:
    ...
    .
END.
ON VALUE-CHANGED OF tog-rcpt-overflow IN FRAME F-Main
DO:
    assign
        tog-rcpt-overflow
    .
...
END.
ON VALUE-CHANGED OF tog-tech-refuel IN FRAME F-Main
DO:
    assign
        tog-tech-refuel
    .
    run sensitive-tog-total-tech-chk.
END.
ON VALUE-CHANGED OF tog-total-tech-chk IN FRAME F-Main
DO:
    assign
        tog-total-tech-chk
    .
    run sensitive-tog-total-tech-chk.
END.
ON VALUE-CHANGED OF tog-trans-cancell IN FRAME F-Main
DO:
    assign
        tog-trans-cancell
    .
    run sensitive-tog-total-tech-chk.
END.
ON VALUE-CHANGED OF tog-trans-transfer IN FRAME F-Main
DO:
    assign
        tog-trans-transfer
    .
    run sensitive-tog-total-tech-chk.
END.
ON VALUE-CHANGED OF tog-unlock-trans IN FRAME F-Main
DO:
    assign
        tog-unlock-trans
    .
...
define variable v-list as character no-undo.
    ...
    display v-header with frame F-Main.
    assign
    ...
    assign
        tog-trans-cancell:screen-value in frame F-Main = string(tog-trans-cancell)
        tog-rcpt-overflow:screen-value in frame F-Main = string(tog-rcpt-overflow)
        tog-tech-refuel:screen-value in frame F-Main = string(tog-tech-refuel)
        tog-trans-transfer:screen-value in frame F-Main = string(tog-trans-transfer)
        tog-unlock-trans:screen-value in frame F-Main = string(tog-unlock-trans)
        tog-total-tech-chk:screen-value in frame F-Main = string(tog-total-tech-chk)
        rs-grp-tech-refuell:screen-value in frame F-Main = string(rs-grp-tech-refuell)
    .
END PROCEDURE.
PROCEDURE my-report :
    if tog-trans-cancell = yes then
    do:
        v-param-list = v-param-list + string(integer('14':U)).
    end.
    if tog-rcpt-overflow = yes then
    do:
        if v-param-list <> "" then
        do:
            v-param-list = v-param-list + chr(44) + string(integer('15':U)).
        end.
        else
        do:
            v-param-list = v-param-list + string(integer('15':U)).
        end.
    end.
    if tog-tech-refuel = yes then
    do:
        if v-param-list <> "" then
        do:
            v-param-list = v-param-list + chr(44) + string(integer('17':U)).
        end.
        else
        do:
            v-param-list = v-param-list + string(integer('17':U)).
        end.
    end.
    if tog-trans-transfer = yes then
    do:
        if v-param-list <> "" then
        do:
            v-param-list = v-param-list + chr(44) + string(integer('16':U)).
        end.
        else
        do:
            v-param-list = v-param-list + string(integer('16':U)).
        end.
    end.
    if tog-total-tech-chk = yes then
    do:
        if v-param-list <> "" then
        do:
            v-param-list = v-param-list + chr(44) + '0':U.
        end.
        else
        do:
...
PROCEDURE sensitive-tog-total-tech-chk :
    if
    tog-trans-cancell  = no
    and
    tog-rcpt-overflow  = no
    and
    tog-tech-refuel    = no
    and
    tog-trans-transfer = no
    and
    tog-unlock-trans   = no
    then
        do:
            tog-total-tech-chk:screen-value in frame F-Main = 'no':U.
            disable
                tog-total-tech-chk
            with frame F-Main.
        end.
