...
run get-quest-print in p-mainmenu-handle ( output g#quest-print ).
    ...
    if buf_wth-doc.doc-type <> 'при':U and buf_wth-doc.doc-type <> 'рас':U and buf_wth-doc.doc-type <> 'возврат':U then return.
...
if valid-handle( g#wthcalib ) <> yes then do:       run str/wthcalib.p persistent no-error.       if error-status :error or valid-handle( g#wthcalib ) <> yes then do:         message "Error starting wthcalib.p"    skip( 0 )                 g#wthcalib                     skip( 0 )                 g#wthcalib   :type             skip( 0 )                 g#wthcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run wthcalib_wthat-val in g#wthcalib (  input buf_wth-doc.doc-code ,
    ...
    if buf_wth-doc.obj-type <> 'маг':U then do:
      find first buf_clients no-lock where buf_clients.obj-type = 'орг':U and buf_clients.obj-code = buf_wth-doc.host-code .
...
output stream PrnLibStream to value( string( session:temp-directory +
                                     ...
                                     page-size  value(62) .
    form header
        space(1) v-single-line format "X(132)" skip    'Продолжение - на следующей странице' at 90 skip
        with frame BottomFrame width 136 page-bottom no-labels no-box .
    view stream PrnLibStream frame BottomFrame .
if session :set-wait-state( "compiler" ) then.
    put stream PrnLibStream         "Форма НН-2-ДО"  at 90  if buf_wth-doc.doc-type = 'рас':U then "" else " (возврат)"  skip
    ...
    .
    if buf_wth-doc.doc-type = 'рас':U then put stream PrnLibStream     space( 35 ) "НА ОТПУСК ТАЛОНОВ НА НЕФТЕПРОДУКТЫ (литровые)" skip .
    ...
    put stream PrnLibStream     space( 55 ) v-doc-date  format "X(12)" skip
         ...
         if buf_wth-doc.doc-type = 'рас':U then  " Кому " else " От кого "  format "X(9)" v-cli-name format "X(60)" " через кого " v-receiver  format "X(40)" skip
    ...
    put stream PrnLibStream   skip  space(1)
      string("Всего " + (if buf_wth-doc.doc-type = 'рас':U then  "отпущено " else "возвращено ") + str-qnty + " талонов" )  format "X(130)"  skip
      space(1) string("на сумму " + s1 )  format "X(130)"  skip
    .
    if buf_wth-doc.doc-type = 'рас':U then put stream PrnLibStream  skip(1) space(12)  string("Отпуск разрешил " + v-main-boss )  format "X(50)"  string("Главный бухгалтер " + v-main-buh )  format "X(50)" .
    else  put stream PrnLibStream  skip(1) space(12)                                      string("Возврат разрешил " + v-main-boss )   format "X(50)"  string("Главный бухгалтер " + v-main-buh )  format "X(50)" .
    run print-note in this-procedure .
    if line-counter( PrnLibStream ) + 2 + 2 > page-size( PrnLibStream ) then do:
      page stream PrnLibStream .
    end.
    if buf_wth-doc.doc-type = 'рас':U then put stream PrnLibStream  skip (2)  space(35)  "РАСШИФРОВКА ОТПУЩЕННЫХ ТАЛОНОВ НА НЕФТЕПРОДУКТЫ ПО"   skip .
...
procedure PrintTitul :
  do on error undo, return error return-value :
    if line-counter( PrnLibStream ) + 2 < page-size( PrnLibStream ) then do:
      put stream PrnLibStream skip space(1) "|" v-single-line format "X(130)" "|" .
    end.
    page stream PrnLibStream .
    run print-header in this-procedure .
  end.
...
procedure print-header :
  do on error undo, return error:
    ...
    skip
    space(1)       v-single-line   format "X(132)"
    skip space(1)  "|"
        "Номенклатурный №"      at center-field(1 + 1, 1 + 20, 17)
        "|"                     at 1 + 20
        "Наименование нефтепродуктов"   at center-field(1 + 20, 1 + 70, 30)
        "|"                     at 1 + 70
        "Ед.изм."               at center-field(1 + 70, 1 + 78, 7)
        "|"                     at 1 + 78
        "Кол-во"                at center-field(1 + 78, 1 + 95, 6)
        "|"                     at 1 + 95
        "Цена за ед."           at center-field(1 + 95, 1 + 112, 10)
        "|"                     at 1 + 112
        "Сумма"                 at center-field(1 + 112, 1 + 132, 11)
        "|"                     at 1 + 132
    skip space(1)
...
procedure print-header2 :
  do on error undo, return error:
    ...
    space(1)       v-single-line   format "X(132)"
    skip space(1)  "|"
        "Наименование нефтепродуктов"      at center-field(2, 40, 35)
        "|"                     at 40
        "Купюры"                at center-field(40, 80, 8)
        "|"                     at 80
        "Серия"                 at center-field(80, 98, 5)
        "|"                     at 98
        "№ талона"              at center-field(98, 118, 8)
        "|"                     at 118
        "Кол-во талонов"        at center-field(118, 133, 14)
        "|"                     at 133
    skip space(1)
        "|" v-single-line format "X(130)" "|"
    .
...
procedure print-group-line :
  ...
  if line-counter( PrnLibStream ) + 2 + 2 + 1 > page-size( PrnLibStream ) then do:
    page stream PrnLibStream.
    run print-header in this-procedure .
  end.
  ...
  put stream PrnLibStream  skip space(1) "|   ***  Группа:  "  + p-grp-name format "X(110)" "|" at 1 + 132 .
end procedure.
procedure print-total-result :
  do on error undo, return error :
    put stream PrnLibStream  skip space(1)  v-single-line format "X(132)" .
  end.
end procedure.
procedure print-note :
  do on error undo, return error :
    if buf_wth-doc.doc-type = 'рас':U then put stream PrnLibStream  skip(1) space(12)  string("Отпустил " + v-deliver)   format "X(50)"  string("Получил " + v-receiver)  format "X(50)" .
    else  put stream PrnLibStream  skip(1) space(12)                                      string("Возвратил " + v-receiver)   format "X(50)"  string("Получил " + v-deliver)  format "X(50)" .
  end.
end procedure.
