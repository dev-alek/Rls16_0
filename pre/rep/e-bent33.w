...
PROCEDURE adm-disable :
    DISABLE RECT-5 SumsOnly WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-5 SumsOnly WITH FRAME F-Main.
...
FOR EACH benefits:
  ...
  FOR EACH obj-list No-LOCK:
    ...
    FOR EACH chk-doc No-LOCK
      ...
      :
      if lookup(string(chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) > 0 then next _chk-gds.
  ...
  END.
  assign
    sheetf.Excel-Column-Lable = "Артикул" + chr(44) + "Количество"  + chr(44) + "Цена исх" + chr(44) + "Цена со скидкой"  + chr(44) + "Сумма скидки"
    sheetf.sizes = "16" + chr(44) + "15"  + chr(44) + "15"  + chr(44) + "15"  + chr(44) + "15"
  ...
  FOR EACH benefits NO-LOCK BREAK BY BENEFITS.OBJ-CODE BY BENEFITS.DCPC1 BY BENEFITS.ARTIC  BY BENEFITS.PRICE-BASE:
    IF FIRST-OF(BENEFITS.OBj-CODE) THEN DO:
      FIND FIRST clients NO-LOCK WHERE clients.obj-type = 'маг':U AND clients.obj-code = benefits.obj-code No-ERROR.
      if Make-Excel then  put   stream ForExcel unformatted (IF AVAIL clients then clients.obj-name  else ("Магазин N " + string(benefits.obj-code)) )  SKIP.
    END.
    IF FIRST-OF(BENEFITS.dcpc1) THEN DO:
      if Make-Excel then  put   stream ForExcel unformatted  ("Скидка " + string(BENEFITS.dcpc1, "->>9.9%"))  SKIP.
    END.
    if SumsOnly = no then do :
      if Make-Excel then  put   stream ForExcel unformatted
      BENEFITS.artic CHR(9)
      excel-qnty ( BENEFITS.qnty ) CHR(9)
      excel-sum  ( BENEFITS.PRICE-BASE ) CHR(9)
      excel-sum  ( BENEFITS.PRICE-NETTO ) CHR(9)
    ...
    IF LAST-OF(BENEFITS.dcpc1) THEN DO:
      if Make-Excel then  put   stream ForExcel unformatted
      ("ИТОГО: (" + string(BENEFITS.dcpc1, "->>9.9%") + ")") CHR(9)
      excel-qnty ( ACCUM TOTAL BY BENEFITS.dcpc1 BENEFITS.qnty ) CHR(9)
      excel-sum  ( ACCUM TOTAL BY BENEFITS.dcpc1 (BENEFITS.qnty * BENEFITS.PRICE-BASE) ) CHR(9)
      excel-sum  ( ACCUM TOTAL BY BENEFITS.dcpc1 (BENEFITS.qnty * BENEFITS.PRICE-NETTo) ) CHR(9)
      excel-sum  ( ACCUM TOTAL BY BENEFITS.dcpc1 (BENEFITS.qnty * BENEFITS.DISCNT) )
      SKIP.
    END.
    IF Multi-obj AND LAST-OF(BENEFITS.OBJ-CODE) THEN DO:
      if Make-Excel then  put   stream ForExcel unformatted
      ("ИТОГО ПО МАГАЗИНУ " + string(BENEFITS.OBJ-CODE)) CHR(9)
      excel-qnty ( ACCUM TOTAL BY BENEFITS.obj-code BENEFITS.qnty ) CHR(9)
      excel-sum  ( ACCUM TOTAL BY BENEFITS.obj-code (BENEFITS.qnty * BENEFITS.PRICE-BASE) ) CHR(9)
      excel-sum  ( ACCUM TOTAL BY BENEFITS.obj-code (BENEFITS.qnty * BENEFITS.PRICE-NETTo) ) CHR(9)
    ...
    IF LAST(BENEFITS.OBJ-CODE) THEN DO:
      if Make-Excel then  put   stream ForExcel unformatted
      "ИТОГО ПО ВСЕМ " CHR(9)
      excel-qnty ( ACCUM TOTAL BENEFITS.qnty ) CHR(9)
      excel-sum  ( ACCUM TOTAL (BENEFITS.qnty * BENEFITS.PRICE-BASE) ) CHR(9)
      excel-sum  ( ACCUM TOTAL (BENEFITS.qnty * BENEFITS.PRICE-NETTo) ) CHR(9)
