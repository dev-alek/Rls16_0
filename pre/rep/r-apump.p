...
define variable v-line        as character no-undo format "X(105)" .
define variable l-line        as character no-undo format "X(75)" .
...
assign
  ...
  l-line = fill("-", 75 )
.
define variable v-line1        as character no-undo format "X(105)" .
define variable v-line2        as character no-undo format "X(105)" .
define variable l-line1        as character no-undo format "X(75)" .
define variable l-line2        as character no-undo format "X(75)" .
assign
  v-line1 = v-line
  v-line2 = v-line
  l-line1 = v-line
  l-line2 = v-line
.
define variable sym1          as character no-undo format "x(1)":u  init ":":u .
define variable sym2          as character no-undo format "x(1)":u  init ":":u .
define variable sym3          as character no-undo format "x(1)":u  init ":":u .
define variable sym4          as character no-undo format "x(1)":u  init ":":u .
define variable sym5          as character no-undo format "x(1)":u  init ":":u .
define variable sym6          as character no-undo format "x(1)":u  init ":":u .
define variable sym7          as character no-undo format "x(1)":u  init ":":u .
define variable sym8          as character no-undo format "x(1)":u  init ":":u .
...
form header
  ...
  "стр." + string( page-number(PrnLibStream), ">>>9" )  at 95
  with frame topframe
  width 106 page-top  no-labels   no-box stream-io use-text.
...
form header
  ...
  width 106 page-bottom no-labels no-box stream-io use-text.
...
with width 106  down stream-io use-text  no-labels no-box.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  run waitfram-show in this-procedure ("Печать документа. Обработано строк: " + string(v-ind) ) .
      ...
      v-column-5  = buf_icnt-line.state-el-cnt .
      v-column-6  = buf_icnt-line.state-el-cnt - buf_icnt-line.state-mh-cnt.
