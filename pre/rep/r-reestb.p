...
PROCEDURE foreach :
  ...
  for each tdedt where tdedt.id  <> 'rs':U + ',' + 'es':U  no-lock :
      if tdedt.id = 'ot':U Then find-str = 'cgsr':U + ','.
...
End.
 ...
 Assign
      ...
      IF ub.ot-tot.sum-type = 'cgsr':U  THEN DO:
            ...
            if  ub.ot-tot.ext-doc-type = 'vt':U then do :
              ...
              if avail ub.trn-doc then
                ...
                (first cost-ot-tot-inv where
                       ...
                       cost-ot-tot-inv.sum-type = 'cssr':U no-lock )
                        Then
                        Assign
                          SumWithNDS-coast    = 0
                          VAT-Sum-coast       = 0
                ...
                (first sale-ot-tot-inv where
                       ...
                       sale-ot-tot-inv.sum-type = 'sasr':U no-lock )
                        Then
                        Assign
                          discnt-sum   =      0
                          ov-sum       =      0
            ...
            End.
      End.
      IF ub.ot-tot.sum-type = 'sasr':U  THEN DO:
          Find Last ub.trn-doc where ub.trn-doc.doc-code = ub.ot-tot.doc-code no-lock no-error.
          ...
          SumWithoutNDS  = SumWithNDS - VAT-Sum.
            FIND LAST crsa-ot-tot where
                ...
                crsa-ot-tot.sum-type = 'cgsr':U    and
       ...
       End.
      IF ub.ot-tot.sum-type = 'cssr':U  THEN DO:
          Assign
       ...
       End.
     IF ub.ot-tot.sum-type BEGINS 'cssr':U and ub.ot-tot.sum-type <> 'cssr':U Then DO:
            IF CostSum Then DO:
...
if available acc#taxslt then
  ...
  End.
              End.
          End.
     IF ub.ot-tot.sum-type BEGINS 'sasr':U and ub.ot-tot.sum-type <> 'sasr':U Then DO:
              IF entry (2 , ub.ot-tot.cat-id) = '##':U Then DO:
create tmp#taxVAT.
assign tmp#taxVAT.type    = ot-tot.sum-type
      tmp#taxVAT.pc      = entry (1 , ot-tot.cat-id)
...
if available acc#taxslt then
  ...
  End.
     End.
      IF ub.ot-tot.sum-type BEGINS 'cgsr':U and ub.ot-tot.sum-type <> 'cgsr':U Then DO:
                IF entry (2 , ub.ot-tot.cat-id) = '##':U Then DO:
create tmp#taxVAT.
assign tmp#taxVAT.type    = ot-tot.sum-type
      tmp#taxVAT.pc      = entry (1 , ot-tot.cat-id)
  ...
  End.
      End.
          ...
          if last-of ( ub.ot-tot.doc-code ) then do:
              if (ub.ot-tot.sum-type     =  'cgsr':U  and
...
PROCEDURE Break-2str :
  ...
  if vat-slt   then run break-vat  in this-procedure ( 'sasr':U) .
  if vat-slt-s then run break-vat-sub in this-procedure  ( 'sasr':U) .
  if vat-slt   then run break-vat  in this-procedure ( 'cgsr':U) .
  if vat-slt-s then run break-vat-sub in this-procedure  ( 'cgsr':U) .
END PROCEDURE.
...
Display stream OutStream
    ...
    if vat-slt then   run break-vat  in this-procedure ( 'cssr':U) .
    if vat-slt-s then   run break-vat-sub in this-procedure  ( 'cssr':U) .
END PROCEDURE.
PROCEDURE Break-Akt :
