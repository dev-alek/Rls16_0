...
define variable i_obi-sup-num as integer no-undo.
define variable c_ord-num as character no-undo.
define variable d_order-date as date no-undo.
define variable c_doc-num as character no-undo.
define variable d_doc-date as date no-undo.
define variable i_mag-num as integer no-undo.
define variable i_pallet as integer no-undo.
define variable c_consignee as character no-undo.
define variable c_consignor as character no-undo.
define variable c_picker as character no-undo.
define variable i_cur-list as integer no-undo.
define variable i_lists as integer no-undo.
define buffer buf_trn-doc for ub.trn-doc.
...
field obi-gds-art as character
field gds-name as character
field sup-art as character
field units as character
field brutto as decimal
field EAN as character
field qnty as decimal
index ind_num is primary unique num.
find first buf_trn-doc where recid(buf_trn-doc) = p_recid no-lock.
run get_header in this-procedure.
run get_tt_list in this-procedure.
run get_footer in this-procedure.
run create_empty_report in this-procedure.
run fill_report in this-procedure.
...
c_doc-num = v-torgconf-vdoc-code.
d_doc-date = buf_trn-doc.doc-date.
...
for each buf_doc-line where buf_doc-line.doc-code = buf_trn-doc.doc-code no-lock:
    ...
    assign
    tt_list.num = ii
    tt_list.qnty = buf_doc-line.fact-qnty.
    ...
    assign
    tt_list.units = buf_goods.unit-base
    tt_list.brutto = buf_goods.wt-base
    tt_list.gds-name = buf_goods.gds-name
    tt_list.sup-art = string(buf_goods.gds-code).
    find first buf_ext-artic where buf_ext-artic.cli-type = 'орг':U
                               and buf_ext-artic.cli-code = buf_trn-doc.cli-code
                               and buf_ext-artic.gds-code = buf_goods.gds-code
                               and buf_ext-artic.status_ <> 'удал':U no-lock no-error.
    if available(buf_ext-artic) then tt_list.obi-gds-art = buf_ext-artic.ext-artic.
    ...
    tt_list.EAN = trim(tt_list.EAN,";").
end.
end procedure.
procedure get_footer:
find first buf_clients where buf_clients.obj-type = 'чел':U
...
define variable c_row as character no-undo.
i_cur-list = 1.
