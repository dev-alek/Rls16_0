...
define variable vss-include-info15 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    ...
    assign
        log-file-name = v-log-file
    .
    if log-file-name <> "":U
    then do:
        if search( v-log-file ) = ?
        then do:
...
:
    ...
    assign
        ...
        buf_temp_sheet1_line-data.data-key   = "LD":U
    ...
    put stream excel-line unformatted
        ...
        chr(9)   buf_temp_sheet1_line-data.firm
        chr(9)   buf_temp_sheet1_line-data.region
        chr(9)   buf_temp_sheet1_line-data.azs_name
        chr(9)   buf_temp_sheet1_line-data.gds_type
        chr(9)   buf_temp_sheet1_line-data.gds_code
        chr(9)   string( buf_temp_sheet1_line-data.data_sell, "99.99.9999" )
        chr(9)   string( buf_temp_sheet1_line-data.nqty )
        chr(9)   string( buf_temp_sheet1_line-data.price )
        chr(9)   string( buf_temp_sheet1_line-data.sum_sell )
        chr(9)   buf_temp_sheet1_line-data.price_type
        chr(10)
    .
end.
end procedure.
...
:
  ...
  if x-TOG-Shift = false then do:
    ...
    run rnpazsxl-write-cell-data in this-procedure ( input "h_num":U   , input "Порядок смен - без смен " ).
  end.
  else do:
    put stream  out-stream
    space (65) "Порядок смен с "X-Shift-Start " по " X-Shift-End skip.
    run rnpazsxl-write-cell-data in this-procedure ( input "h_num":U   , input substitute("Порядок смен с &1  по  &2", X-Shift-Start, X-Shift-End  ) ).
  end.
  run rnpazsxl-write-cell-data in this-procedure ( input "h_docName" , input "Данные о реализации НП на АЗС за период (ТамбовНП)" ).
  run rnpazsxl-write-cell-data in this-procedure ( input "h_date":U  , input substitute("ЗА ПЕРИОД с  &1  по &2", string(x-Date-Start, "99/99/9999"), string(x-Date-End, "99/99/9999"))).
  run rnpazsxl-write-cell-data in this-procedure ( input "h_obj":U   , input substitute("&1", v-obj-list  ) ).
  run rnpazsxl-write-cell-data in this-procedure ( input "h_printdate":U, input cur-time-print() ).
end.
end procedure.
procedure incom :
do
...
:
  for each ub.units no-lock
    where lookup( 'топ':U, ub.units.type) > 0
...
assign
  ...
  tt-petrol.gds-price = tt-petrol.tot-sum
.
END PROCEDURE.
