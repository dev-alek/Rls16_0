...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  find Our_Host    where Our_Host.obj-type    = 'орг':U               and Our_Host.obj-code    = buf_trn-doc.host-code no-lock.
...
procedure print-titul :
  do on error undo, return error return-value :
    if can-do('факт':U, buf_trn-doc.status_ ) or ( can-do('накл':U, buf_trn-doc.status_) and buf_trn-doc.flag_ ) then do:
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_trn-doc.doc-code ,
      ...
      put stream Out_stream "Приходный  акт  N  " at 40 format "X(20)" v-doc-code format "X(8)"
              ...
              (if buf_trn-doc.obj-type = 'скл':U then "на склад" else "в магазин") format "X(10)" Our_Object.obj-name format "X(40)" skip
      ...
      form header
           Line format "X(136)" at 1 skip "Продолжение - на следующей странице" at 30 skip
           with frame Akt-Bottomframe width 160 page-bottom no-labels no-box.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  assign mk-code = "" .
  if buf_trn-doc.obj-type = 'маг':U then do:
