...
np = 0 .
 if not can-find
   (  first trn-doc  no-lock  where
      ...
      then next.
vv0 = "+-----+--------------------------------+----------------------------------------+-------------------------------------+".
vv1 = ":     :    Заборный лист  ( накладная)   :            Сумма, руб.коп              :                                     :".
vv2 = ": n/n :------------------+-------------:----------------------+-----------------:            Примечание               :".
...
space ( 5) string (  CAPS (  This_Object.obj-name ) + "  ( " + string ( This_Object.obj-code) + ")" )   "|          по ОКПО |       |" at 89  skip
 ...
 PUT STREAM OutStream UNFORMATTED
  ...
  PUT STREAM OutStream UNFORMATTED
    ...
    run macr_cell_size  in this-procedure  (  15 , ? , num#str# , num#col# , ?, ? ) .
    num#col# = 4.
    run macr_excel_char in this-procedure  (  "Сумма по учетным ценам кухни, руб"   , num#str# , num#col#   ) .
    run macr_cell_size  in this-procedure  (  15 , ? , num#str# , num#col# , ?, ? ) .
    num#col# = 5.
    run macr_excel_char in this-procedure  (  "Сумма по ценам продажи, руб"   , num#str# , num#col#   ) .
    run macr_cell_size  in this-procedure  (  15 , ? , num#str# , num#col# , ?, ? ) .
    num#col# = 6.
...
find first buf_fbr-gds-obj  where
  ...
  if not available tt-allsum-line then do:
     ...
     if trn-doc.doc-type = 'при':U and trn-doc.internal = false  then
      assign
