...
for each main-buf_fbr-doc no-lock where main-buf_fbr-doc.out-code = buf_fbr-pln.doc-code
    ...
    .
        for each ub.fbr-recipe no-lock where
                ...
                ub.fbr-recipe.recipe-type = 'производство':U
                :
...
create temp-doc.
 if v-ves then
          assign
              temp-doc.ves  =  ub.fbr-recipe.qnty  / n-porc
          .
       else
          assign
              temp-doc.ves  =  1
...
assign
  ...
  temp-doc.qnty-bl     = ub.fbr-line.fact-qnty / ub.fbr-recipe.qnty
  temp-doc.netto       = if ub.fbr-recipe.qnty <> 0 and ub.fbr-recipe.qnty <> ?  then ub.fbr-recipe.qnty else 1
  temp-doc.qnty-line   = ub.fbr-line.fact-qnty
  temp-doc.all-stoim     = round( ub.fbr-line.price-rubl  * temp-doc.ves  , 2 )
  temp-doc.price-sale-1  = round( ub.fbr-line.price-sale  * temp-doc.ves  , 2 )
  temp-doc.Vat         = round( ub.fbr-line.price-sum-vat-rubl / ( temp-doc.porcii * temp-doc.qnty-bl ), 2 )
...
.
  if v-sec-i modulo  2 = 0 then v-sec = v-sec + 1 .
end.
end.
...
find ub.clients      where ub.clients.obj-type     = 'орг':U            and ub.clients.obj-code      = v-cntxt-host-code-obj no-lock no-error .
...
PUT STREAM OutStream UNFORMATTED string('Общая стоимость сырьевого набора на 1 блюдо')  format "x(57)" .   for each temp-doc where temp-doc.sec = ll-sec :        PUT STREAM OutStream UNFORMATTED ":" string(temp-doc.all-stoim) format "x(32)" .   end.    PUT STREAM OutStream skip.
PUT STREAM OutStream UNFORMATTED string('НДС,' + string(pp-r))  format "x(57)" .   for each temp-doc where temp-doc.sec = ll-sec :        PUT STREAM OutStream UNFORMATTED ":" string(temp-doc.vat) format "x(32)" .   end.    PUT STREAM OutStream skip.
PUT STREAM OutStream UNFORMATTED string('Наценка,' + string(pp-r))  format "x(57)" .   for each temp-doc where temp-doc.sec = ll-sec :        PUT STREAM OutStream UNFORMATTED ":" string(temp-doc.discn) format "x(32)" .   end.    PUT STREAM OutStream skip.
PUT STREAM OutStream UNFORMATTED string('Цена продажи 1 блюда,' + string(pp-r))  format "x(57)" .   for each temp-doc where temp-doc.sec = ll-sec :        PUT STREAM OutStream UNFORMATTED ":" string(temp-doc.price-sale-1) format "x(32)" .   end.    PUT STREAM OutStream skip.
PUT STREAM OutStream UNFORMATTED string('Выход одного блюда в готовом виде,грамм')  format "x(57)" .   for each temp-doc where temp-doc.sec = ll-sec :        PUT STREAM OutStream UNFORMATTED ":" string(temp-doc.qnty-inp) format "x(32)" .   end.    PUT STREAM OutStream skip.
PUT STREAM OutStream UNFORMATTED  vv0 skip.
  end.
end procedure.
