...
def var vss-description as character no-undo init "Отчет по межфирменным операциям - динамика инвентаризации  (закладка № 2)".
...
PROCEDURE adm-disable :
    DISABLE RECT-5 dat WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-5 dat WITH FRAME F-Main.
...
PROCEDURE my-report :
 ...
 run rep/r-hold8.p  ( input dat ) .
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main dat  .
ReportNAme = "Динамика инвентаризации за "  + String(dat) + " год." .
