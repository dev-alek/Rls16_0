...
define variable  v-curcode-rec              as integer   no-undo .
    define frame f-doc
        ...
        v-gds-name          column-label "Наименование товара! ! ! ! "      format "X(48)"      space(0)
    ...
    define frame f-doc-m
        ...
        v-doc-line-counter  column-label "N!п/п! ! ! "                      format ">>>>9"              space(0)
        sym2                column-label ":!:!:!:!:"                        format "X(1)"               space(0)
        v-gds-name          column-label "Наименование товара! ! ! ! "      format "X(70)"    space(0)
        sym3                column-label ":!:!:!:!:"                        format "X(1)"               space(0)
        v-gds-code          column-label "Код товара! ! ! ! "               format "999999999"          space(0)
        sym4                column-label ":!:!:!:!:"                        format "X(1)"               space(0)
        v-unit-base         column-label "Наим!ед.!изм.! ! "                format "X(4)"               space(0)
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run rbisbase in g#library
  (output v-rb-is-base
    ...
    then do:
        ...
        assign v-doc-date-string =  ( if buf_trn-doc.status_ <> 'факт':U or v-print-doc-date = yes
...
output stream out-stream to value( string( session:temp-directory +
    ...
    assign
        v-valute-abbr = ( if PrintRubl then "рублях" else "баз.вал" )
    .
            ...
            skip space(5)
                substitute( "Примечание: &1"
                    , ( if not( buf_trn-doc.PS begins "@" )
                        then replace( buf_trn-doc.PS, chr(10), " " )
                        else "" ) )     format "X(163)"
        .
    end.
    ...
    :
        ...
        then do:
            ...
            run clcprtsl_calc-ttable in this-procedure (
                ...
                , input ( if p-rb-is-base = yes then "base":U else "rubl":U )
                , input 0
                , input 0
                , input 0
                , input 0
    ...
    :
        ...
        then do:
            display stream out-stream
                sym1 sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9 sym10 sym11 sym12 sym13 sym14 sym15 sym16
                v-doc-line-counter
            ...
            display stream out-stream
                sym1 sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9 sym10 sym11 sym12 sym13 sym14 sym15 sym16 sym17 sym18
                v-doc-line-counter
                buf_temp_place_parts.gds-name           @ v-gds-name
                buf_temp_place_parts.gds-code           @ v-gds-code
                buf_temp_place_parts.unit-base          @ v-unit-base
...
:
    ...
    put stream out-stream
        space(10) "  Всего на сумму:        "
            ...
            " ("
            trim( if PrintRubl then "руб" else "баз.вал" )              format "X(6)"
            ")"
    .
