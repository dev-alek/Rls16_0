...
sheetf.sizes = "".
assign
Sheetf.Excel-Column-Lable = "Код"                        + chr(44) +
                            "Артикул"                    + chr(44) +
                            "Название"                   + chr(44) +
...
for each tt-gds-parts use-index print-order:
  ...
  tt-gds-parts.gds-code    CHR(9)
  tt-gds-parts.artic       CHR(9)
  tt-gds-parts.gds-name    CHR(9)
  tt-gds-parts.prod-name   CHR(9)
  tt-gds-parts.doc-qnty    CHR(9)
  tt-gds-parts.price       CHR(9)
  tt-gds-parts.sum-brutto  CHR(9)
  tt-gds-parts.sum-discnt  CHR(9)
  tt-gds-parts.prc-discnt  CHR(9)
  tt-gds-parts.sum-netto   CHR(9)
  tt-gds-parts.num-doc     CHR(9)
  if tt-gds-parts.date-doc  = ? then "" else string(tt-gds-parts.date-doc, "99/99/9999" )  CHR(9)
  tt-gds-parts.cst-code    CHR(9)
  skip.
  assign
...
for each obj-list :
  ...
  for each bf_chk-doc where
    ...
    if lookup(string(bf_chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) > 0 then next _chk-doc.
    find first bf_inkas where bf_inkas.inkas-code = bf_chk-doc.out-code no-lock no-error.
    if not available bf_inkas or bf_inkas.status_ <> 'факт':U then next _chk-doc.
    ...
    for each bf_chk-gds where
       ...
       if not available tt-gds then do:
         ...
         assign
           ...
           tt-gds.price       = (if v-curr-r-b = 'base':U then round(bf_chk-gds.price-base * v-rate, 2) else bf_chk-gds.price-base)
           tt-gds.discnt      = (if v-curr-r-b = 'base':U then round(bf_chk-gds.discnt     * v-rate, 2) else bf_chk-gds.discnt)
           .
       end.
       assign
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input bf-in_trn-doc.doc-code ,
                        input 'nids':U ,
                       output v-attr-value-nids ,
                       output v-attr-type )  .
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input bf-in_trn-doc.doc-code ,
                        input 'dids':U ,
                       output v-attr-value-dids ,
                       output v-attr-type )  .
      end.
