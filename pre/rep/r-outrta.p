...
:
    ...
    put stream excel-line unformatted
        ...
        chr(9)   buf_temp_line-data.part-code
...
procedure print-titul :
  ...
  if is-chek then do:
    ...
    If V-torgconf-outprim = No or t-doc.internal Then Do:
      ...
      run outretxl-write-cell-data in this-procedure (
          input "h_PS":U
        , input Replace(( If ( Not Can-do( 'факт':U, T-doc.Status_ ) Or Logres ) And Not ( T-doc.Ps Begins "@" ) Then  T-doc.Ps Else " " ), chr(10), " ")
      ).
    End.
  ...
  do on error undo, return error return-value :
    ...
    put stream Out_stream skip(1) space(5) "Всего " format "x(8)" all-qnty format ">,>>>,>>9.<<<"
      space(2) "единицы" format "x(8)" space(2) Lines_Counter format ">,>>>,>>9" space(2) "наименований" format "x(13)"
                                              "на сумму"  all-stoim  format ">,>>>,>>>,>>>,>>9.99"  " "  ( if not PrintRubl then currency.curr-abbr else "РУБ" )
      skip(1).
    run outretxl-write-cell-data in this-procedure (
          input "f_lineAmount":U
        , input string( "Всего " + string(all-qnty,">,>>>,>>9.<<<") + " единицы " + string(Lines_Counter,">,>>>,>>9") + " наименований  на сумму " + string(all-stoim,">,>>>,>>>,>>>,>>9.99") + " " +  ( if not PrintRubl then currency.curr-abbr else "РУБ" ) )
    ).
    ...
    if v-torgconf-outdisc = no and not CostPrice then do:
      put stream Out_stream space(5) "Скидка " format "x(8)"
        ...
        space(2) ( if not PrintRubl then currency.curr-abbr else "РУБ" )                    format "x(4)" skip(1)
      .
