...
End.
   if v-button-stop then  DO:
      ...
      if buf_doc-line.ext-doc-type = 'vt':U      or
         buf_doc-line.ext-doc-type = 'vp':U then do:
        assign
          v-qnty = - v-qnty
        ...
        if buf_doc-line.ext-doc-type = 'rs':U or buf_doc-line.ext-doc-type = 're':U  then do:
...
run macr_excel_char(String(ii),                      v-row, 1) .
    ...
    run macr_excel_sum ( p-prod,                         v-row, 12,  2) .
    run macr_excel_sum ( p-nds,                          v-row, 13,  2) .
    run macr_excel_sum ( p-np,                           v-row, 14,  2) .
    run macr_excel_sum ( p-prod-nal,                     v-row, 15,  2) .
    run macr_excel_sum ( temp-goods.p-val,               v-row, 16,  3) .
    run macr_excel_sum ( p-sum-prod,                     v-row, 17,  2) .
    run macr_excel_sum ( temp-goods.p-sum-nds,           v-row, 18,  2) .
    run macr_excel_sum ( p-sum-prod-nds,                 v-row, 19,  2) .
    run macr_excel_sum ( temp-goods.p-sum-np,            v-row, 20,  2) .
    run macr_excel_sum ( temp-goods.p-sum-prod-nal,      v-row, 21,  2) .
    run macr_excel_sum ( p-nazen,                        v-row, 22,  2) .
    assign  v-row = v-row + 1  .
  end.
