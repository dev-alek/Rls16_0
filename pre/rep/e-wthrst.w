...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-wthrst.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-7 v-free-zone v-put-zone ed-wth rs-wth bt-wth WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 v-free-zone v-put-zone ed-wth rs-wth bt-wth WITH FRAME F-Main.
...
define buffer buf_wealth  for ub.wealth .
   ...
   when 1 then do:
      ...
      assign
         v-wth-list = trim( v-wth-list, chr(44) )
      .
      display
         ed-wth
      with frame F-Main.
...
on error undo, return error return-value
:
   do v-i = 1 to num-entries( p-code-list , chr(44) )
   :
      find first buf_wth-par
           where buf_wth-par.wth-code = integer( ENTRY(1, entry( v-i, p-code-list, chr(44)), chr(4) ))
             and buf_wth-par.par-code = integer( ENTRY(2, entry( v-i, p-code-list, chr(44)), chr(4) ))
           no-lock
         no-error
         .
      ...
      then do:
      assign
         p-recid-list = p-recid-list + string( recid(buf_wth-par) ) + chr(44)
      .
      end.
   end.
   assign
...
on error undo, return error return-value
:
   do v-i = 1 to num-entries( p-recid-list, chr(44) )
   :
...
:
   ...
   then do:
      ...
      assign
         rs-wth = 2
      .
      do v-i = 1 to num-entries(v-wth-list, chr(44) )
      :
         find first buf_wth-par
              where buf_wth-par.wth-code = integer( ENTRY(1, entry( v-i, v-wth-list, chr(44)), chr(4) ))
                and buf_wth-par.par-code = integer( ENTRY(2, entry( v-i, v-wth-list, chr(44)), chr(4) ))
              no-lock
              no-error
              .
         if available buf_wth-par
         then do:
            assign
               ed-wth = ed-wth + STRING(buf_wth-par.par-val, ">>>>>9") + " " + buf_wth-par.par-unit + chr(10)
            .
         end.
      end.
   end.
...
:
   ...
   run rep/r-wthrst.p ( input my-handle
                      ...
                      , input v-put-zone
                      , input (rs-wth = 1)
                      , input ((x-SelectObject = "4":U) OR (x-SelectObject = "все"))
                      ) .
end.
END PROCEDURE.
PROCEDURE my-var :
  assign
    ReportHeader =  ReportHeader + "Группы объектов: " + chr(10) + chr(10) +
...
:
  ...
  assign
    ed-wth = "":u
  .
  do v-i = 1 to num-entries( v-wth-recid-list, chr(44) )
  :
    find first buf_wth-par no-lock
