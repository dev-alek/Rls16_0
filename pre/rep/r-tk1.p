...
.
 ...
 v-max = ( integer (length(vvv) / 60 ) ) .   if (  length(vvv) modulo 60  ) > 0 then  v-max-dop = 1 . else v-max-dop = 0  .  if vvv <> "" then do:      create temp-str .           temp-str.str-1 = substring( vvv , 1 , 60) .      temp-str.pr = false  .      repeat v = 1 to v-max + v-max-dop    :         if substring( vvv , (60 * v ) + 1 , 60) <> "" then do :               create temp-str .                     temp-str.str-1 = substring(vvv , (60 * v) + 1 , 60) .                temp-str.pr = false  .               if temp-str.str-1 = ? then temp-str.str-1 = " " .               temp-str.str-1 = REPLACE (temp-str.str-1 , chr(10) ,chr(32) ) .               end.      end.  end.
 create temp-str .
...
if error-status :error then return error .
 ...
 if lookup('шту':U, buf_units.type) > 0  or lookup('сер':U, buf_units.type) > 0 then do:
