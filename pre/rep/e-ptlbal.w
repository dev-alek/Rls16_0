...
PROCEDURE adm-disable :
    DISABLE tog-weight tog-with-tot-day WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN tog-weight tog-with-tot-day WITH FRAME F-Main.
...
PROCEDURE local-initialize :
  ...
  if not error-status:error
    and num-entries(v-uf-List_, chr(4)) = 2
  then do:
    assign
      tog-weight       :screen-value in frame F-Main = entry(1, v-uf-List_, chr(4))
      tog-with-tot-day :screen-value in frame F-Main = entry(2, v-uf-List_, chr(4))
    .
  end.
  else do:
    assign
      tog-weight :screen-value in frame F-Main       = "no":U
      tog-with-tot-day :screen-value in frame F-Main = "no":U
    .
  end.
...
PROCEDURE my-disable-with-tot-day :
  if x-tog-shift = true then do:
    hide tog-with-tot-day in frame F-Main .
  end.
  else do:
    enable tog-with-tot-day with frame F-Main .
  end.
END PROCEDURE.
PROCEDURE my-report :
...
define variable reportheader as char no-undo.
  assign frame F-Main
    ...
    .
  assign
    v-uf-list_ = string( tog-weight )         + chr(4)
