...
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  run mainmenu_getcntxt in parparentproc
    ...
    ,output v-cntxt-is-admin
      ...
      v-task-num = integer(entry(2, p-doc-code, chr(4)))
      v-dirs0 =  entry(3, p-doc-code, chr(4))
      v-profile-id = p-profile-id
      .
      assign
      v-codex-id-list = string(p-codex-id)
      v-ruleset-id-list[1] = string(3) + chr(44) +
                              string(1) + chr(44) +
                              string(2) + chr(44) +
                              string(4)
      v-ruleset-id-list[2] = ''
...
define variable vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  run mainmenu_getcntxt in parparentproc
    ...
    ,output v-cntxt-is-admin
      ...
      assign
      v-codex-id-list = string(p-codex-id)
      v-ruleset-id-list[1] = string(5) + chr(44) +
                             string(6)
      v-ruleset-id-list[2] = ''
      v-cont-handle = this-procedure:handle
      .
      ...
      on endkey undo _main, return error substitute( "&1. endkey", vss-workfile ) :
        v-proc-name = "rul/" + string(buf_rule-by-call.rule_id, '999999999') + '.p'.
          run write-log  in p-log-handle (
                                          input 0
                                        , "&DLine").
                    if log-file-name <> '':U then run write-log-and-file in p-log-handle (      input 1                                                        , input log-file-name                                            , input 1                                                        , input substitute("...(&2) &1", buf_rule-by-call.algo-des, buf_rule-by-call.rule_id)).
        v-found = yes.
        ...
        then do:
          case v-ruleset-id:
            ...
            when integer(5) then do:
            end.
            when integer(6) then do:
            end.
            otherwise do:
        ...
        if not error-status:error then do:
          if v-ruleset-id = integer(1)
          then do:
            run write-log  in p-log-handle (
                                            input 0
                                          , "&DLine").
                        if log-file-name <> '':U then run write-log-and-file in p-log-handle (      input 1                                                        , input log-file-name                                            , input 1                                                        , input substitute("...(&1) - Закончен", buf_rule-by-call.rule_id)).
          end.
        end.
  ...
  run update-rule-call-params in this-procedure ( input 'rep':U
                                                 ,input p-uniq-key-rec ).
...
for each buf_rule-call-param where
 ...
 :
  ...
  if buf_rule-call-param.param-mode <> 'input':U then next.
  ...
  p-rpbh::parameter-value = if buf_rule-call-param.param-data-type = 'character':U
                        then buf_rule-call-param.param-value-character
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if buf_rule-call-param.param-data-type = 'date':U
                        then string(buf_rule-call-param.param-value-date, "99/99/9999")
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if buf_rule-call-param.param-data-type = 'decimal':U
                        then string(buf_rule-call-param.param-value-decimal)
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if buf_rule-call-param.param-data-type = 'logical':U
                        then string(buf_rule-call-param.param-value-logical)
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if buf_rule-call-param.param-data-type = 'integer':U
...
:
  ...
  p-rpbh::parameter-value = if p-param-data-type = 'character':U
                        then p-param-value-character
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if p-param-data-type = 'date':U
                        then string(p-param-value-date, "99/99/9999")
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if p-param-data-type = 'decimal':U
                        then string(p-param-value-decimal)
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if p-param-data-type = 'logical':U
                        then string(p-param-value-logical)
                        else p-rpbh::parameter-value
  p-rpbh::parameter-value = if p-param-data-type = 'integer':U
...
define input parameter p-destination-details as character no-undo .
do
on error undo, return error
:
...
if error-status:error or p-batch-rep-1 = ? then do:
    if log-file-name <> '':U then run write-log-and-file in p-log-handle (      input 1                                                        , input log-file-name                                            , input 1                                                        , input return-value).
  if p-allow-current-dir then do:
    file-info:file-name = ".".
    assign
    p-batch-rep-1 = file-info:full-pathname.
        if log-file-name <> '':U then run write-log-and-file in p-log-handle (      input 1                                                        , input log-file-name                                            , input 1                                                        , input substitute("Отчеты, выводимые в текстовом виде и в EXCEL, будут сохраняться в рабочей директории")).
    v-flag-1 = yes.
  end.
  else do:
    undo, return error .
...
if not v-flag-1 then do:
  p-batch-rep-1 = prepare-path2(p-batch-rep-1) + chr(92).
  ...
  or not glog then do:
    ...
    if p-allow-current-dir then do:
      ...
      assign
      p-batch-rep-1 = file-info:full-pathname.
            if log-file-name <> '':U then run write-log-and-file in p-log-handle (      input 1                                                        , input log-file-name                                            , input 1                                                        , input substitute("Отчеты, выводимые в текстовом виде и EXCEL будут сохраняться в рабочей директории")).
    end.
    else do:
      undo, return error .
    end.
...
if error-status:error or p-batch-rep-2 = ? then do:
  ...
  if p-allow-current-dir then do:
    ...
    p-batch-rep-2 = ".".
    v-flag-2 = yes.
        if log-file-name <> '':U then run write-log-and-file in p-log-handle (      input 1                                                        , input log-file-name                                            , input 1                                                        , input substitute("Отчеты, выводимые в XML, будут сохраняться в рабочей директории")).
  end.
  else do:
    undo, return error .
  end.
...
if v-flag-2 then do:
  p-batch-rep-2 = prepare-path2(p-batch-rep-2) + chr(92).
  RUN verify-file in this-procedure
                                    ( input p-batch-rep-2
...
on error undo, return error
:
  do v-ii = 1 to num-entries( p-dirs0, chr(124)):
    v-process-dir-name = entry(v-ii, p-dirs0, chr(124)).
    if v-process-dir-name = ?
    or v-process-dir-name = '' then do:
      v-process-dir-name = ".".
    end.
    else do:
      v-process-dir-name = right-trim(v-process-dir-name, chr(92)).
      v-process-dir-name = v-process-dir-name  + chr(92).
    end.
    run gbl/dir-canw.p ( input v-process-dir-name) no-error.
    if error-status:error then do:
            if log-file-name <> '':U then run write-log-and-file in p-log-handle (      input 1                                                        , input log-file-name                                            , input 1                                                        , input substitute("!!!! Нет возможности вывести отчет в директорию &1&2&3&2&4"                                 , v-process-dir-name                                 , chr(10)                                 , error-status:get-message(1)                                  , return-value )).
      undo, return error .
    end.
    p-dirs = p-dirs + (if v-ii = 1 then '' else chr(124)) + v-process-dir-name.
  end.
end.
end procedure.
