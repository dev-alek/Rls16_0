...
define variable vss-workfile    as character no-undo init "$Workfile: e-sj2.p $":u .
define variable vss-archive     as character no-undo init "$Archive: rep/e-sj2.p $":u .
...
if v-curr-r-b = 'base':U then do:
  ...
  with FRAME BottomFramebasesj-base-t width  232 PAGE-BOTTOM NO-LABELS NO-BOX .
  VIEW STREAM PrnLibStream FRAME BottomFramebasesj-base-t .
end.
else do:
  FORM HEADER
  line format "X(196)"
  AT 1 SKIP
  string( "Продолжение - на следующей странице" ) FORMAT "X(35)" AT 30 SKIP
  with FRAME BottomFramerublsj-base-t width 232 PAGE-BOTTOM NO-LABELS NO-BOX .
    VIEW STREAM PrnLibStream FRAME BottomFramerublsj-base-t .
end.
CASE my-Set_Val_Type :
  when 2 then
      FORM with FRAME sj-base-t .
  when 3 then
      FORM with FRAME sj-full-t .
END CASE .
assign
p-frame-width = frame sj-base-t:width.
CASE SHRS-by :
...
if first( sj-goods.obj-attr ) then do:
  if v-curr-r-b = 'base':U then do:
                        if my-set_val_type = 2  then
                            DOWN STREAM PrnLibStream 1 with FRAME sj-base-t .
                        else
                            DOWN STREAM PrnLibStream 1 with FRAME sj-full-t .
  end.
  else do:
                            DOWN STREAM PrnLibStream 1 with FRAME sj-base-t .
  end.
...
if v-curr-r-b = 'base':U then do:
                        ...
                        sj-goods.artic
                        sj-goods.name
                        with FRAME sj-base-t .
                        else
                        UNDERLINE STREAM PrnLibStream
                        sj-goods.artic
                        sj-goods.name
                        with FRAME sj-full-t .
end.
else do:
                        UNDERLINE STREAM PrnLibStream
                        sj-goods.artic
      ...
      if first-of( sj-goods.grp-name  ) then do:
        if v-curr-r-b = 'base':U then do:
          if my-Set_Val_Type = 2 then do:
            if frame sj-base-t:line = 0 then do:
              down 1 stream PrnLibStream
              with frame sj-base-t.
            end.
          end.
          else do:
            if frame sj-full-t:line = 0 then do:
              down 1 stream PrnLibStream
              with frame sj-full-t.
            end.
          end.
...
define variable vss-include-info29 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    ...
    sj-adv.qnty  (SUB-TOTAL BY ( sj-adv.price  ))
    sj-adv.qnty-2 (SUB-TOTAL BY ( sj-adv.price  ))
    sj-adv.qnty-3 (SUB-TOTAL BY ( sj-adv.price  ))
    sj-adv.brutto-sum  (SUB-TOTAL BY (  sj-adv.price  ))
    sj-adv.discnt-sum  (SUB-TOTAL BY ( sj-adv.price  ))
    sj-adv.netto-sum  (SUB-TOTAL BY (  sj-adv.price  ) )
    sj-adv.brutto-sum-r  (SUB-TOTAL BY (  sj-adv.price  ))
    sj-adv.netto-sum-r  (SUB-TOTAL BY (  sj-adv.price  ) )
    sj-adv.num-lines ( SUB-TOTAL BY (  sj-adv.price  ))
    sj-adv.num-docs (SUB-TOTAL BY (  sj-adv.price  ))
    ...
    .
IF FIRST-OF(sj-adv.price) then do:
      chk-gds-lines = 0.
end.
...
if sj-goods.twounit = 0 then do:
  if last-of (sj-adv.price) then  do:
    if not SHOnly_tot then do:
      namebuf1 = get-strokes(buffer sj-goods, 18, 21,
                             input-output namebuf1,
                             input-output namebuf2,
    ...
    CASE my-Set_Val_Type :
      when 2 then  do:
        ...
        namebuf1 @ sj-goods.name
        prodbuf1 @ sj-goods.prod-name
        ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.qnty @ sj-adv.qnty
        ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.qnty-2 @ sj-adv.qnty-2
        ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.qnty-3 @ sj-adv.qnty-3
        sj-adv.price
        ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.brutto-sum @ sj-adv.brutto-sum
        ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.discnt-sum @ sj-adv.discnt-sum
        (IF (ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.qnty ) <> 0 THEN
        round( ( ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.discnt-sum ) /
        ( ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.brutto-sum ) *
          100 , 1 )   ELSE 0) @ pcnt
        ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.netto-sum @ sj-adv.netto-sum
        with FRAME sj-base-t .
        if not SHOnly_tot then
        DOWN STREAM PrnLibStream 1 with FRAME sj-base-t .
      end.
      when 3 then do:
        ...
        ACCUM SUB-TOTAL BY  ( sj-adv.price  )  sj-adv.brutto-sum @
        sj-adv.brutto-sum
        ACCUM SUB-TOTAL BY  ( sj-adv.price  )  sj-adv.brutto-sum-r @
        sj-adv.brutto-sum-r
        ACCUM SUB-TOTAL BY  ( sj-adv.price )  sj-adv.discnt-sum @
        sj-adv.discnt-sum
        (IF (ACCUM SUB-TOTAL BY  ( sj-adv.price  )  sj-adv.qnty) <> 0 THEN
        round( ( ACCUM SUB-TOTAL BY  ( sj-adv.price  )  sj-adv.discnt-sum ) /
              ( ACCUM SUB-TOTAL BY  ( sj-adv.price  )  sj-adv.brutto-sum ) *
                      100 , 1 )   ELSE 0) @ pcnt
        ACCUM SUB-TOTAL BY  ( sj-adv.price  )  sj-adv.netto-sum @ sj-adv.netto-sum
        ACCUM SUB-TOTAL BY  ( sj-adv.price  )  sj-adv.netto-sum-r @ sj-adv.netto-sum-r
        with FRAME sj-full-t .
        if not SHOnly_tot then
        DOWN STREAM PrnLibStream 1 with FRAME sj-full-t.
      end.
  ...
  if last-of( sj-goods.prod-name ) AND prodtot_flag then do:
    CASE my-Set_Val_Type :
      when 2 then do:
        ...
        ACCUM SUB-TOTAL BY sj-goods.prod-name sj-adv.qnty-2    @ sj-adv.qnty-2
        ACCUM SUB-TOTAL BY sj-goods.prod-name sj-adv.qnty-3    @ sj-adv.qnty-3
    ...
    IF AllObjsTotalsBy  then do:
      ...
      CASE my-set_val_type :
        when 2 then do:
          ...
          ACCUM SUB-TOTAL BY sj-goods.grp-name sj-adv.qnty-2    @ sj-adv.qnty-2
          ACCUM SUB-TOTAL BY sj-goods.grp-name sj-adv.qnty-3    @ sj-adv.qnty-3
...
if sj-goods.twounit = 0 then do:
  if last-of ( sj-adv.price ) AND SHRs-by <> 0 then do:
    namebuf1 = get-strokes(buffer sj-goods, 18, 21,
                           input-output namebuf1,
                           input-output namebuf2,
                           input-output prodbuf1,
    ...
    when 2 then  do:
      ...
      ACCUM SUB-TOTAL BY sj-adv.price sj-adv.brutto-sum @ sj-adv.brutto-sum
      ACCUM SUB-TOTAL BY sj-adv.price sj-adv.discnt-sum @ sj-adv.discnt-sum
      (IF (ACCUM SUB-TOTAL BY sj-adv.price sj-adv.qnty) <> 0
      THEN
      round( ( ACCUM SUB-TOTAL BY sj-adv.price sj-adv.discnt-sum ) /
      ( ACCUM SUB-TOTAL BY sj-adv.price sj-adv.brutto-sum ) * 100 , 1 )
      else 0)  @ pcnt
      ACCUM SUB-TOTAL BY sj-adv.price sj-adv.netto-sum @ sj-adv.netto-sum
      with FRAME sj-base-t .
      DOWN STREAM PrnLibStream 1 with FRAME sj-base-t .
     ...
     when 3 then do:
      ...
      sj-adv.price
      ACCUM SUB-TOTAL BY  sj-adv.price  sj-adv.brutto-sum @ sj-adv.brutto-sum
      ACCUM SUB-TOTAL BY  sj-adv.price  sj-adv.brutto-sum-r @ sj-adv.brutto-sum-r
      ACCUM SUB-TOTAL BY  sj-adv.price  sj-adv.discnt-sum @ sj-adv.discnt-sum
      (IF (ACCUM SUB-TOTAL BY  sj-adv.price  sj-adv.qnty) <> 0
      THEN
      round( ( ACCUM SUB-TOTAL BY  sj-adv.price  sj-adv.discnt-sum ) /
  ...
  if last-of( sj-goods.saleman ) AND SHBySalers then do:
    ...
    CASE my-Set_val_type :
      when 2 then  do:
        ...
        ACCUM SUB-TOTAL BY sj-goods.saleman sj-adv.qnty-2    @ sj-adv.qnty-2
        ACCUM SUB-TOTAL BY sj-goods.saleman sj-adv.qnty-3    @ sj-adv.qnty-3
