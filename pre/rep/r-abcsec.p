...
IF ( temp-abc.nn modulo v-account = 0 )  then DO:
 Assign
    v-kol-spice = (50 - LENGTH('Пересчет ABC для подгрупп')) / 2
    RecordsString = fill(' ',v-kol-spice) + string('Пересчет ABC для подгрупп')
    .
 Assign
    v-kol-spice = (50 - LENGTH(temp-abc.name)) / 2
    RecordsString2 = fill(' ',v-kol-spice) + string(temp-abc.name)
    .
          if  log-manager:logfile-name ne ?
          then DO:
