...
End.
  ...
  for each buf_doc-line no-lock
    where buf_doc-line.obj-type     = buf_gds-obj.obj-type
      ...
      and buf_doc-line.ext-doc-type = 'es':U
    ...
    .
    if p-sale-rubl < p-cost-rubl then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output temp-DiscSales.b-code
        ...
        assign
          ...
          temp-DiscSales.grp-name  = entry ( num-entries( right-trim(buf_goods.grp-name, chr(47)), chr(47) ) , buf_goods.grp-name, chr(47) )
...
procedure GrpSumTree :
  v-level      = num-entries( right-trim(temp-DiscSales.full-grp-name, chr(47)), chr(47) ) - 1 .
  assign CurrGrpName = "" .
  do ind = 1 to v-level :
    assign CurrGrpName = CurrGrpName + entry ( ind, temp-DiscSales.full-grp-name, chr(47) )  + chr(47).
    find first temp-grp-sum
  ...
  do ii = ind to v-level :
    create temp-grp-sum .
    if ii > ind then do:
      assign CurrGrpName = CurrGrpName + entry ( ii, temp-DiscSales.full-grp-name, chr(47) )  + chr(47).
    end.
    assign
      temp-grp-sum.num = ii
