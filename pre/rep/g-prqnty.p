...
define variable vss-workfile    as character no-undo init "$Workfile: g-prqnty.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/g-prqnty.p $":U .
...
find first buf_qnty-group no-lock where recid(buf_qnty-group) = p-recid-grp no-error .
if error-status :error then return .
run rep/d-report.w
( input  parParentProc ,
  input  "rep/e-prqnty.w"  ,
  input  "Прайс-лист по количественной группе " + caps(buf_qnty-group.name) + fill(" ",200) + "|" +
          string(p-recid-grp) + "|2"   ,
  input  1,
  input  "1":U,
  input  "2,3":U,
  input  "",
  input  "",
  input  "all,1,14,15,16,17,18,19," +
         "10,X-SCHET-NAME=Выбор валюты"  ,
  input  no ).
