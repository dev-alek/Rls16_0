...
DEFINE temp-table temp-DiscSales no-undo
  ...
  run day-begin-fact-order in this-procedure ( input x-date,              output v-fact-order ).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output ii
    ...
    do ii = 1 to 8 :
      case ii :
        when 1 then assign str-find = 'sadt':U + 'ee':U .
        when 2 then assign str-find = 'sadt':U + 'es':U .
        when 3 then assign str-find = 'sadt':U + 're':U .
        when 4 then assign str-find = 'sadt':U + 'rs':U .
        when 5 then assign str-find = 'csdt':U + 'ee':U .
        when 6 then assign str-find = 'csdt':U + 'es':U .
        when 7 then assign str-find = 'csdt':U + 're':U .
        when 8 then assign str-find = 'csdt':U + 'rs':U .
      end.
      find last buf_stk-line no-lock
        where buf_stk-line.obj-type  = buf_gds-obj.obj-type
          and buf_stk-line.obj-code  = buf_gds-obj.obj-code
