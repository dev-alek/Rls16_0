...
BREAK
    ...
    BY (goods.gds-name) :
      ...
      if return-value <> "" then NEXT.
        ...
        If String(Entry(2,"goods.grp-name",".")) = "Grp-name"
          Then  Assign
                  S-bar-code   = " Итого по "
...
no-lock
BREAK
      BY ((substring(clients.obj-name,1,10) + string(goods.prod-code)))
    BY (goods.gds-name) :
      run item-goods ( "2" , "goods" ) .
      if return-value <> "" then NEXT.
        If Last-of((substring(clients.obj-name,1,10) + string(goods.prod-code))) Then Do:
        If String(Entry(2,"(substring(clients.obj-name,1,10) + string(goods.prod-code))",".")) = "Grp-name"
          Then  Assign
                  S-bar-code   = " Итого по "
                  Gds-zap-artic = Substring(
                                      B1-name
...
procedure lavel1 :
              case select-good :
                  when 1  then do:
    define buffer buf_obj-list30 for obj-list.
  for each buf_obj-list30 no-lock :
      if xtog-obj and  not (buf_obj-list30.obj-type = x-store-type  and
                            buf_obj-list30.obj-code = x-store-code )
      then next.
        for  each gds-obj   where
                            gds-obj.obj-type = buf_obj-list30.obj-type    and
                            gds-obj.obj-code = buf_obj-list30.obj-code
                            and ( (v-show-all-goods = true ) or   ( gds-obj.last-doc = ? or gds-obj.last-doc >= x-date-start or gds-obj.fact-qnty <> 0 or gds-obj.avrg-qnty <> 0 or gds-obj.fact-sale <> 0 or gds-obj.fact-base <> 0 ) )
                            no-lock ,
        first goods where gds-obj.gds-code  = goods.gds-code
                            no-lock :
  ...
  run foreach in this-procedure .
   ...
   if last-of( temp-gds-list.grp-name ) then do:
      ...
      run clear-b2 in this-procedure .
                  ...
                  when 2  then do:
    define buffer buf_obj-list31 for obj-list.
  for each buf_obj-list31 no-lock :
      if xtog-obj and  not (buf_obj-list31.obj-type = x-store-type  and
                            buf_obj-list31.obj-code = x-store-code )
      then next.
        for  each gds-obj   where
                            gds-obj.obj-type = buf_obj-list31.obj-type    and
                            gds-obj.obj-code = buf_obj-list31.obj-code
                            and ( (v-show-all-goods = true ) or   ( gds-obj.last-doc = ? or gds-obj.last-doc >= x-date-start or gds-obj.fact-qnty <> 0 or gds-obj.avrg-qnty <> 0 or gds-obj.fact-sale <> 0 or gds-obj.fact-base <> 0 ) )
                              no-lock ,
        first  tmp#grp
  ...
  run foreach in this-procedure .
   ...
   if last-of( temp-gds-list.grp-name ) then do:
      ...
      run clear-b2 in this-procedure .
                  ...
                  when 3 then do:
    define buffer buf_obj-list32 for obj-list.
  for each buf_obj-list32 no-lock :
      if xtog-obj and  not (buf_obj-list32.obj-type = x-store-type  and
                            buf_obj-list32.obj-code = x-store-code )
      then next.
        for  each gds-obj   where
                  gds-obj.obj-type = buf_obj-list32.obj-type    and
                  gds-obj.obj-code = buf_obj-list32.obj-code
                  and ( (v-show-all-goods = true ) or   ( gds-obj.last-doc = ? or gds-obj.last-doc >= x-date-start or gds-obj.fact-qnty <> 0 or gds-obj.avrg-qnty <> 0 or gds-obj.fact-sale <> 0 or gds-obj.fact-base <> 0 ) )
                    no-lock ,
        first g#cli
  ...
  run foreach in this-procedure .
   ...
   if last-of( temp-gds-list.grp-name ) then do:
      ...
      run clear-b2 in this-procedure .
                  ...
                  otherwise do:
    define buffer buf_obj-list33 for obj-list.
  for each buf_obj-list33 no-lock :
      if xtog-obj and  not (buf_obj-list33.obj-type = x-store-type  and
                            buf_obj-list33.obj-code = x-store-code )
      then next.
        for  each gds-obj   where
                            gds-obj.obj-type = buf_obj-list33.obj-type    and
                            gds-obj.obj-code = buf_obj-list33.obj-code
                            and ( (v-show-all-goods = true ) or   ( gds-obj.last-doc = ? or gds-obj.last-doc >= x-date-start or gds-obj.fact-qnty <> 0 or gds-obj.avrg-qnty <> 0 or gds-obj.fact-sale <> 0 or gds-obj.fact-base <> 0 ) )
                            no-lock ,
        first gds-list where gds-obj.gds-code  = gds-list.gds-code
                            no-lock :
