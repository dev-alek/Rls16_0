...
assign v-account = ( if integer( 1 ) = 0 then 100 else integer( 1 ) ).
  ...
  if p-rs-date = 1 then do:
    ...
    do ii = 0 to (x-date-end - x-date-start)  :
      ...
      for each buf_wth-doc-attr no-lock where buf_wth-doc-attr.attr-code =  'wthdsf':U  and buf_wth-doc-attr.attr-value = str :
...
procedure PutColumnTitulExcel :
  ...
  :
    ...
    if p-rs-wth-pl = 2 then do v-i = 1 to num-entries(p-wth-pl-list) :
      find first buf_wth-place where RECID(buf_wth-place) = integer( entry( v-i, p-wth-pl-list, chr(44) ) ) no-lock no-error .
...
End.
   if v-button-stop then  DO:
    ...
    for each buf_wth-line no-lock where buf_wth-line.doc-code = buf_wth-doc.doc-code :
      ...
      .
      if p-is-detal and ( buf_wth-line.sum-gds-rubl <> 0 or buf_wth-line.fact-sum <> 0 ) then do:
      ...
      for each buf_wth-parts no-lock  where  buf_wth-parts.out-code = buf_wth-doc.doc-code
        ...
        if buf_wth-parts.beg-dt >= p-dtFrom and buf_wth-parts.beg-dt <= p-dtEnd then do:
          assign
    ...
    if v-sum <> 0 or v-qnty <> 0 then do:
...
if valid-handle( g#wthcalib ) <> yes then do:       run str/wthcalib.p persistent no-error.       if error-status :error or valid-handle( g#wthcalib ) <> yes then do:         message "Error starting wthcalib.p"    skip( 0 )                 g#wthcalib                     skip( 0 )                 g#wthcalib   :type             skip( 0 )                 g#wthcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run wthcalib_wthat-val in g#wthcalib (  input buf_wth-doc.doc-code ,
                        input 'wthnsf':U ,
                       output temp-doc.num-sf ,
                       output v-attr-type )  .
if valid-handle( g#wthcalib ) <> yes then do:       run str/wthcalib.p persistent no-error.       if error-status :error or valid-handle( g#wthcalib ) <> yes then do:         message "Error starting wthcalib.p"    skip( 0 )                 g#wthcalib                     skip( 0 )                 g#wthcalib   :type             skip( 0 )                 g#wthcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run wthcalib_wthat-val in g#wthcalib (  input buf_wth-doc.doc-code ,
                        input 'wthdsf':U ,
                       output temp-doc.dat-sf ,
                       output v-attr-type )  .
      if p-is-detal then do:
