...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-wthobr.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-1 RECT-2 RECT-3 ed-supp t-calc rs-supp rs-wth-type ed-wth rs-wth cb-wth-detail WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 RECT-2 RECT-3 ed-supp t-calc rs-supp rs-wth-type ed-wth rs-wth cb-wth-detail WITH FRAME F-Main.
...
DO:
  IF SELF:SCREEN-VALUE = '3' THEN DO:
      ENABLE f-dtFrom f-dtTo WITH FRAME F-Main.
      apply "entry":U to f-dtFrom.
  END.
  ELSE DO:
      DISABLE f-dtFrom f-dtTo WITH FRAME F-Main.
      hide f-dtFrom f-dtTo in FRAME F-Main.
  END.
END.
...
PROCEDURE local-apply-layout :
  ...
  assign
    v-list-item-str = "Без детализации,1," +
                      "По МЦ,2," +
                      "По номиналам,3," +
                      "По сериям,4":U
    cb-wth-detail:list-item-pairs in frame F-Main = v-list-item-str
  .
  assign
    ed-supp = "Все":U
    ed-wth  = "Все":U
  ...
  then do:
    ...
    :
      ...
      then do:
        assign
          v-supp-name-list = v-supp-name-list + buf_clients.obj-name + chr(10)
        .
      end.
    end.
    assign
        ...
        when 1 then do:
          ...
          do v-i = 1 to num-entries( v-wth-recid-list ) :
            ...
            if available buf_wealth then do:
              assign
                v-wth-list = v-wth-list + buf_wealth.wth-name + chr(10)
              .
            end.
          end.
        end.
...
PROCEDURE my-var :
  ...
  .
    assign
    ReportHeader = chr(10) + "Выбор даты: ":U
  .
  if t-calc = 1 then     assign
    ...
    ReportHeader = ReportHeader + "По сроку годности".
                                           ...
                                           ,f-dtTo).
  assign
    ReportHeader = ReportHeader +  chr(10) + "Контрагенты : ":U
  .
  if rs-supp <> 1 then do:
    assign
      ReportHeader = ReportHeader + chr(10)
    .
    do v-i = 1 to num-entries( ed-supp , chr(10) ):
      assign
        ReportHeader = ReportHeader + fill( ' ' , 3 ) + entry( v-i , ed-supp , chr(10) ) + chr(10)
      .
    end.
  end.
  else do:
    assign
      ReportHeader = ReportHeader + ed-supp + chr(10) + chr(10)
    .
  end.
  assign
  ...
  if rs-wth = 1 then do:
    ...
    assign
      ReportHeader = ReportHeader + chr(10) + fill( ' ' , 3 ) + entry( 1 , ed-wth , chr(10) ) + chr(10)
    .
    do v-i = 2 to num-entries( ed-wth , chr(10) ) :
      assign
        ReportHeader = ReportHeader + fill( ' ' , 3 ) + fill( ' ' , 3 ) + entry( v-i , ed-wth , chr(10) ) + chr(10)
      .
    end.
  end.
  assign
    ReportHeader = ReportHeader + chr(10) + "Уровень детализации: ":U + entry( cb-wth-detail , "Без детализации,По МЦ,По номиналам,По сериям":u)
  .
END PROCEDURE.
    ...
    if not available cli-list then do:
      ...
      for each cli-list :
        ...
        then do:
          assign
            v-clients-recid-list = v-clients-recid-list + string(recid(buf_clients)) + ','
            v-supp-name-list     = v-supp-name-list + cli-list.obj-name + chr(10)
          .
        end.
      end.
      assign
