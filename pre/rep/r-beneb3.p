...
define variable vss-workfile    as character no-undo init "$Workfile: r-beneb3.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-beneb3.p $":U .
...
DEFINE FRAME PayCodes-base
    ...
    HEADER  date_string format "X(35)" AT 5
                    ...
                    Line format "X(82)" AT 1
    with width 160 down stream-io use-text .
DEFINE FRAME ZUM-PayCodes-base
        sym1 column-label ":" format "X(1)"
        benefits.pay-name column-label "Вид оплаты" format "X(41)"
        sym6 column-label ":" format "X(1)"
        benefits.tot-r-b column-label "Сумма (вал.продаж)"  format "->,>>>,>>>,>>>,>>9.99"
...
FOR EACH obj-list WHERE
  ...
  if choice then do:
    ...
    DOWN stream PrnLibStream 1 with FRAME PayCodes-base .
  end.
...
if choice AND ( ObjAmount > 1 ) then do:
  FORM with frame ZUM-PayCodes-base .
  FOR EACH obj-list WHERE
  ...
  BY ub.inkas-pay-desk.curr-code :
    ...
    if last-of( ub.inkas-pay-desk.curr-code ) AND
      ...
      with frame ZUM-PayCodes-base .
      DOWN stream PrnLibStream 1 with FRAME ZUM-PayCodes-base .
     end.
  END.
end.
