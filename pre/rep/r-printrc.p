...
'  </head>' skip
  ...
  do:
    empty temp-table temp_db-list .
    do jj = 1 to num-entries (p-db-list,chr(44)):
      create temp_db-list .
      temp_db-list.db-num = integer(entry (jj,p-db-list, chr(44))) .
    end.
  end.
  for each temp_db-list,
    ...
    by buf_upgrade.UpgDate desc by buf_upgrade.UpgTime desc:
      ...
      put stream OutStr-html unformatted
        ...
        '<td text_wrap="true" style="align: center;">' + string(entry(1,buf_upgrade.version-num,chr(4))) + '</td>' skip
        .
      ...
      find first buf_code where
                 buf_code.parent = substitute("Versions&1&2",chr(4),buf_db.db-num)
