...
:
    ...
    put stream excel-line unformatted
        ...
        CHR(9) buf_temp_line-data.d_count
        CHR(9) buf_temp_line-data.d_grpname
        CHR(9) buf_temp_line-data.d_ostbegin
        CHR(9) buf_temp_line-data.d_pripost
        CHR(9) buf_temp_line-data.d_priperem
        CHR(9) buf_temp_line-data.d_priprvo
        CHR(9) buf_temp_line-data.d_privozv
        CHR(9) buf_temp_line-data.d_prielse
        CHR(9) buf_temp_line-data.d_rasprod
        CHR(9) buf_temp_line-data.d_rasvozv
        CHR(9) buf_temp_line-data.d_rasperem
        CHR(9) buf_temp_line-data.d_rasvnesh
        CHR(9) buf_temp_line-data.d_rasprvo
        CHR(9) buf_temp_line-data.d_rasspis
        CHR(9) buf_temp_line-data.d_raselse
        CHR(9) buf_temp_line-data.d_peresort
        CHR(9) buf_temp_line-data.d_izlish
        CHR(9) buf_temp_line-data.d_nedost
        CHR(9) buf_temp_line-data.d_ostend
        chr(10)
    .
end.
end procedure.
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_ot-line.doc-code ,
             ...
             if v-tech-pass or can-find(first buf_sale-doc where buf_sale-doc.doc-code = buf_ot-line.doc-code and buf_sale-doc.doc-kind = 'itr':U)
                            then do :
    ...
    when 'iv':U or when 'rv':U
                                                    then assign temp-grp-obj.priperem   = temp-grp-obj.priperem  + ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
    when 'im':U       then assign temp-grp-obj.priprvo    = temp-grp-obj.priprvo   + ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
    when 're':U  then assign temp-grp-obj.privozv    = temp-grp-obj.privozv   + ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
    when 'ap':U or when 'mp':U
                       then do :
  ...
  when 'rs':U then assign temp-grp-obj.rasprod = temp-grp-obj.rasprod  - ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
    when 'ep':U   then assign temp-grp-obj.rasvozv    = temp-grp-obj.rasvozv   + ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
    when 'ev':U      then assign temp-grp-obj.rasperem   = temp-grp-obj.rasperem  + ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
    when 'wm':U       then assign temp-grp-obj.rasprvo    = temp-grp-obj.rasprvo   + ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
           when 'we':U then do :
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_ot-line.doc-code ,
                        input 'techpass':U ,
                       output v-value ,
                       output v-type ) no-error .
                              assign
                                v-tech-pass = yes when v-value = "yes".
                             if v-tech-pass or can-find(first buf_sale-doc where buf_sale-doc.doc-code = buf_ot-line.doc-code and buf_sale-doc.doc-kind = 'trf':U)
                             then do :
                                assign temp-grp-obj.raselse    = temp-grp-obj.raselse   + ABS(buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl).
                             end .
                             else do :
           ...
           when 'vp':U       then assign temp-grp-obj.peresort   = temp-grp-obj.peresort  + (buf_ot-line.sum-rubl - buf_ot-line.VAT-rubl) .
                       when 'vt':U then do :
            if buf_ot-line.sum-rubl < 0  then do :
