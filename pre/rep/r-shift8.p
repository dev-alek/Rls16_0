...
if 29 + 1 + (v-ii * (23 + 2)) + 29 + 1 > 232 then do:
...
:
  ...
  if v-text then do:
    put stream PrnLibstream unformatted
    string(buf_cash-pay.obj-name, "X(29)") ":"
    .
  end.
  if Make-Excel then  put   stream ForExcel unformatted
  buf_cash-pay.obj-name CHR(9)
  .
  for each gds-treal-8 where
        gds-treal-8.gds-code > 0
    and gds-treal-8.cpay-code = 0
    ...
    if available cp-gds-treal-8 then do:
      if v-text then do:
        put stream PrnLibstream unformatted
        cp-gds-treal-8.qnty1 format  "->>>,>>9.99" ':':U
        cp-gds-treal-8.netto-rubl format "->>>,>>9.99" ':':U
        .
      end.
      if Make-Excel then  put   stream ForExcel unformatted
      cp-gds-treal-8.qnty1      CHR(9)
      cp-gds-treal-8.netto-rubl CHR(9)
      .
    end.
    else do:
      if v-text then do:
  ...
  if v-text then do:
    put stream PrnLibstream unformatted
    cp-treal-8.qnty1      format "->>,>>>,>>9.99" ':':U
    cp-treal-8.netto-rubl format "->>,>>>,>>9.99"
    skip
    .
  end.
  if Make-Excel then  put   stream ForExcel unformatted
  cp-treal-8.qnty1      CHR(9)
  cp-treal-8.netto-rubl  CHR(9)
  skip
  .
end.
if v-text then do:                       put stream PrnLibstream unformatted        fill('-':U, 29 + 1)        .                                        do v-jj = 1 to v-ii:                       put stream PrnLibstream unformatted         fill('-':U, 23 + 2)         .                                         end.                                         put stream PrnLibstream unformatted             fill('-':U, 29 + 1)       skip.                                   end.
if v-text then do:
  put stream PrnLibstream unformatted
...
by treal-8.cli-code:
   if last-of (treal-8.cli-code) then do:
     ...
     run on-same-page in this-procedure ( input 1 + 1 ).
     ...
     find first buf_clients no-lock where
      ...
      if v-text then do:
        put stream PrnLibstream unformatted
        string(buf_clients.obj-name, "X(29)") ":"
        .
      end.
      if Make-Excel then  put   stream ForExcel unformatted
      ...
      for each gds-treal-8 where
        ...
        if available cli-gds-treal-8 then do:
          if v-text then do:
            put stream PrnLibstream unformatted
            cli-gds-treal-8.qnty1      format "->>>,>>9.99" ':':U
            cli-gds-treal-8.netto-rubl format "->>>,>>9.99" ':':U
            .
          end.
          if Make-Excel then  put   stream ForExcel unformatted
          cli-gds-treal-8.qnty1      CHR(9)
          cli-gds-treal-8.netto-rubl CHR(9)
          .
        end.
        else do:
          if v-text then do:
      ...
      if available cli-treal-8 then do:
        if v-text then do:
          put stream PrnLibstream unformatted
          cli-treal-8.qnty1 format  "->>,>>>,>>9.99" ':':U
          cli-treal-8.netto-rubl format "->>,>>>,>>9.99"
          skip
          .
        end.
        if Make-Excel then  put   stream ForExcel unformatted
        cli-treal-8.qnty1  CHR(9)
        cli-treal-8.netto-rubl CHR(9)
        skip
        .
      end.
      else do:
...
v-prev-page = page-number( PrnLibstream).
run on-same-page in this-procedure ( input 1 + 2 ).
v-curr-page = page-number( PrnLibstream).
if v-curr-page > v-prev-page then do:
...
for each gds-treal-8 where
  ...
  if v-text then do:
    put stream PrnLibstream unformatted
    gds-treal-8.qnty1      format "->>>,>>9.99" ':':U
    gds-treal-8.netto-rubl format "->>>,>>9.99"  ':':U
    .
  end.
  if Make-Excel then  put   stream ForExcel unformatted
  gds-treal-8.qnty1  CHR(9)
  gds-treal-8.netto-rubl CHR(9)
  .
  assign
...
if v-text then do:
  put stream PrnLibstream unformatted
  accum-qnty1 format  "->>,>>>,>>9.99" ':':U
  accum-netto-rubl format "->>,>>>,>>9.99"
  skip.
  if Make-Excel then  put   stream ForExcel unformatted
  accum-qnty1  CHR(9)
  accum-netto-rubl
...
define input parameter p-col1-name as character no-undo .
  ...
  :
    ...
    for each gds-treal-8 where
      ...
      if v-text then do:
        put stream PrnLibstream unformatted
        string(buf_goods.chk-name, "X(23)")  ":"
        .
      end.
    end.
    if v-text then do:
