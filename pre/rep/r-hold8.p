...
define variable vss-workfile    as character no-undo init "$Workfile: r-hold8.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-hold8.p $":U .
...
assign v-account = ( if integer( 25 ) = 0 then 100 else integer( 25 ) ).
  ...
  for each buf_hold-time where buf_hold-time.cat-code = 2  no-lock  :
...
End.
   if v-button-stop then  DO:
      ...
      case x-SelectGood :
        when 4 or when 5 or when 6 then do:
