...
if v-curr-r-b = 'base':U then do:
  ...
  with FRAME BottomFramebasesj-base width  232 PAGE-BOTTOM NO-LABELS NO-BOX .
  VIEW STREAM PrnLibStream FRAME BottomFramebasesj-base .
end.
else do:
  FORM HEADER
  line format "X(136)"
  AT 1 SKIP
  string( "Продолжение - на следующей странице" ) FORMAT "X(35)" AT 30 SKIP
  with FRAME BottomFramerublsj-base width 232 PAGE-BOTTOM NO-LABELS NO-BOX .
    VIEW STREAM PrnLibStream FRAME BottomFramerublsj-base .
end.
CASE my-Set_Val_Type :
  when 2 then
      FORM with FRAME sj-base .
  when 3 then
      FORM with FRAME sj-full .
END CASE .
assign
p-frame-width = frame sj-base:width.
CASE SHRS-by :
      ...
      if first-of( sj-goods.grp-name  ) then do:
        if v-curr-r-b = 'base':U then do:
          if my-Set_Val_Type = 2 then do:
            if frame sj-base:line = 0 then do:
              down 1 stream PrnLibStream
              with frame sj-base.
            end.
          end.
          else do:
            if frame sj-full:line = 0 then do:
              down 1 stream PrnLibStream
              with frame sj-full.
            end.
          end.
...
define variable vss-include-info30 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    ...
    CASE my-set_val_type :
      when 2 then do:
        ...
        PUT STREAM PrnLibStream  LINE format ("X(" + string(138) + ")") SKIP.
        DISPLAY STREAM PrnLibStream
...
END CASE .
HIDE STREAM PrnLibStream FRAME BottomFramesj-base .
if Print-List-hist
...
define variable vss-include-info47 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    ...
    if last(  sj-adv.price  ) then do:
      if v-curr-r-b = 'base':U and my-Set_val_type = 3 then  do:
        ...
        if OneLinePrinted then
        PUT STREAM PrnLibStream Line format ("X(" + string(136) + ")")  SKIP.
      end.
      if ( line-counter(PrnLibStream) + 7 ) > page-size(PrnLibStream) then page stream PrnLibStream .
      CASE my-Set_val_type :
        when 2 then do:
