...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-wthcom.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-7 ed-grp-obj rs-cli-grp bt-cli-grp ed-wth rs-wth bt-wth WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 ed-grp-obj rs-cli-grp bt-cli-grp ed-wth rs-wth bt-wth WITH FRAME F-Main.
   ...
   .
    run uf-get (
      ...
      do v-i = 1 to num-entries(v-cli-grp-lst)
        :
         IF LOOKUP(entry( v-i, v-cli-grp-lst, chr(44) ), v-cli-grp-all) = 0
         THEN DO:
            NEXT _flt-grp-load.
         END.
         find first buf_cli-grp
              where buf_cli-grp.node-code = integer( entry( v-i, v-cli-grp-lst, chr(44) ) )
              no-lock
              no-error
              .
          ...
          then do:
            ...
            v-cli-grp-list = IF v-cli-grp-list = "":U THEN entry( v-i, v-cli-grp-lst, chr(44) )
                                                      ELSE v-cli-grp-list + chr(44) + entry( v-i, v-cli-grp-lst, chr(44) )
            .
          end.
        end.
      end.
      ...
      do v-i = 1 to num-entries(v-wth-lst)
        :
         find first buf_wealth
              where buf_wealth.wth-code = integer( entry( v-i, v-wth-lst, chr(44) ) )
              no-lock
              no-error
              .
          if available buf_wealth
          then do:
            ...
            v-wth-list = IF v-wth-list = "":U THEN entry( v-i, v-wth-lst, chr(44) )
                                              ELSE v-wth-list + chr(44) + entry( v-i, v-wth-lst, chr(44) )
            .
          end.
        end.
      end.
...
PROCEDURE my-var :
  assign
    ReportHeader =  ReportHeader + "Группы объектов: " + chr(10) + ed-grp-obj + chr(10) + chr(10) +
                    "Материальные ценности: " + chr(10) + ed-wth
.
END PROCEDURE.
