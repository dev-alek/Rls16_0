...
define variable v-shift-name           as character format "X(2)" no-undo.
define variable v-shift-name-min       as character format "X(2)" no-undo.
define variable v-shift-name-max       as character format "X(2)" no-undo.
define variable v-shift-num-min        as integer   no-undo.
define variable v-shift-num-max        as integer   no-undo.
define variable v-multy-shift          as logical   no-undo.
define variable v-shift-multydate      as logical   no-undo.
define variable v-date-start-multydate as date      no-undo.
define variable shift-alone            as logical   no-undo.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  ,output v-shift-on
    ...
    if error-status :error
      then
    do:
               if p-batch > 0 then do:      run write-log-and-file in p-log-handle (                 input 1                                          , input p-log-file-name                              , input 1                                          , input substitute("&1 &2 &3&4" +                                         "Невозможно определить тип сменный/не сменный&4" +                                        "для заданного объекта.&4" +                                        "Объект: &5&6&4&7&4&8"                                         ,vss-workfile                                         ,vss-revision                                         ,vss-description                                        ,chr(10)                                        ,obj-list.obj-type                                         ,obj-list.obj-code                                        , return-value                                          ,error-status:get-message(1) )).    end.    else do:       run write-to-log in p-log-handle ( input substitute("&1 &2 &3&4" +                                         "Невозможно определить тип сменный/не сменный&4" +                                        "для заданного объекта.&4" +                                        "Объект: &5&6&4&7&4&8"                                         ,vss-workfile                                         ,vss-revision                                         ,vss-description                                        ,chr(10)                                        ,obj-list.obj-type                                         ,obj-list.obj-code                                        , return-value                                          ,error-status:get-message(1) )).    end.
      undo, return error .
    end.
    if v-shift-on = no
      then
    do:
                if p-batch > 0 then do:      run write-log-and-file in p-log-handle (                 input 1                                          , input p-log-file-name                              , input 1                                          , input substitute("Неверно задан тип объекта &1&2&3"  +                                       "Объект не сменный."                                       ,obj-list.obj-type                                       ,obj-list.obj-code                                       , chr(10) )).    end.    else do:       run write-to-log in p-log-handle ( input substitute("Неверно задан тип объекта &1&2&3"  +                                       "Объект не сменный."                                       ,obj-list.obj-type                                       ,obj-list.obj-code                                       , chr(10) )).    end.
      undo, return error .
    end.
  end.
...
do:
  for each buf_cashbook where buf_cashbook.Status_ = 0 no-lock:
    if p-cashbook <> "" then
      p-cashbook = p-cashbook + chr(6) + string(buf_cashbook.id) .
    else  p-cashbook = string(buf_cashbook.id) .
  end.
end.
do ii = 1 to num-entries (p-cashbook,chr(6)):
  ...
  define variable v-ok as logical no-undo .
      ...
      do:
         ...
         then do:
            ...
            do:
               run utl/fin-doc-nom.p(parparentproc,int64(entry(ii,p-cashbook,chr(6))),x-Date-Start).
            end.
         end.
      end.
      else
  run utl/fin-doc-nom.p(parparentproc,int64(entry(ii,p-cashbook,chr(6))),x-Date-Start).
  o-head-position = mCashBook:getSinglRule(integer(entry(ii,p-cashbook,chr(6))), v-obj-type, v-obj-code, "ManagerPosition") .
  o-director      = mCashBook:getSinglRule(integer(entry(ii,p-cashbook,chr(6))), v-obj-type, v-obj-code, "ManagerFIO") .
  o-snr-accnt     = mCashBook:getSinglRule(integer(entry(ii,p-cashbook,chr(6))), v-obj-type, v-obj-code, "BuhFIO") .
  ...
  v-file-name-rep-html = session:temp-directory + string(p-report-id) + "_" + string (entry(ii,p-cashbook,chr(6))) + ".html".
  ...
  find first buf_cashbook no-lock where buf_cashbook.id = integer((entry(ii,p-cashbook,chr(6)))) no-error .
  v-num-page = 0 .
  ...
  do:
    put stream OutStr-html unformatted
      '<TABLE fit_to_page="true" orientation="portrait" CELLSPACING="0" BORDER="0" name="Титульный лист КК ' + string(entry(ii,p-cashbook,chr(6))) + '">'skip
      .
    put stream OutStr-html unformatted
      '<thead>' skip
  ...
  do v-date-start = date_ to (v-date-end) :
    ...
    do:
      find first buf_day-shift-obj
        where buf_day-shift-obj.shift-date = v-date-start
    ...
    do:
      ...
      do:
        ...
        do:
          if v-shift-num-min = 0 and buf_day-shift-obj.shift-num > 0 then
          do:
            v-shift-num-min = 0.
            v-shift-num-max = 0.
            v-shift-name-min = "".
    ...
    assign
      ...
      .
    run report-exec in this-procedure (input v-date-start, input integer(entry(ii,p-cashbook,chr(6))) ).
    find first temp-fin-doc no-lock where temp-fin-doc.cashbook_id = integer(entry(ii,p-cashbook,chr(6))) no-error .
    if available (temp-fin-doc) then
    do:
      v-num-page = v-num-page + 1 .
      put stream OutStr-html unformatted
        '<TABLE fit_to_page="true" orientation="portrait" CELLSPACING="0" BORDER="0" name="KK_' + string(entry(ii,p-cashbook,chr(6))) + "_за_" + string (v-date-start,"99.99.99") + '">'skip
        .
      put stream OutStr-html unformatted
        '<thead>' skip
        '<tr class="set_columns">' skip
      ...
      for each buf_temp-fin-doc where buf_temp-fin-doc.cashbook_id = temp-fin-doc.cashbook_id by buf_temp-fin-doc.fin-doc-type:
        if buf_temp-fin-doc.fin-doc-type = 'пко':U then v-payer = buf_temp-fin-doc.payer.
        ...
        put stream OutStr-html unformatted
          ...
          '<td text_wrap="true" style="text-align: center;">' + if buf_temp-fin-doc.fin-doc-type = 'пко':U then string(buf_temp-fin-doc.sum-rubl, "->>>>>>>>9.99")  + '</td>' else "         -" + '</td>' skip
          '<td text_wrap="true" style="text-align: center;">' + if buf_temp-fin-doc.fin-doc-type = 'рко':U then string(buf_temp-fin-doc.sum-rubl, "->>>>>>>>9.99")  + '</td>' else "         -" + '</td>' skip
          '</tr>' skip
          .
      end.
      ...
      for each buf_temp-fin-doc where buf_temp-fin-doc.cashbook_id = temp-fin-doc.cashbook_id:
        if buf_temp-fin-doc.fin-doc-type = 'пко':U then v-kolvo-pko = v-kolvo-pko + 1 .
  ...
  do:
    put stream OutStr-html unformatted
      '<TABLE fit_to_page="true" orientation="portrait" CELLSPACING="0" BORDER="0" name="ПЛ_КК ' + string(entry(ii,p-cashbook,chr(6))) + '">'skip
      .
    put stream OutStr-html unformatted
      '<thead>' skip
      '<tr>' skip
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
    ...
    for each buf_arh-fin-doc-schet-nal-obj no-lock
      ...
      and buf_arh-fin-doc-schet-nal-obj.cashbookid        = integer(entry(ii,p-cashbook,chr(6)))
      ...
      and p-date = ( if x-tog-shift then buf_arh-fin-doc-schet-nal-obj.shift-date else buf_arh-fin-doc-schet-nal-obj.fact-date )
      use-index pi :
      if x-TOG-Shift then
      do:
      ...
      do :
        ...
        do:
          ...
          assign
            ...
            .
          if temp-fin-doc.fin-doc-type = 'пко':U then temp-fin-doc.sum-income = temp-fin-doc.sum-income + temp-fin-doc.sum-rubl .
