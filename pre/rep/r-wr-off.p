...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_trn-doc.doc-code ,
      ...
      if not (v-tech-pass or can-find(first ub.sale-doc where ub.sale-doc.doc-code = buf_trn-doc.doc-code
                                          and ub.sale-doc.doc-kind = 'trf':U)) then do :
...
for tt-write-off where recid (tt-write-off) = p-line-rec_id no-lock :
    if Make-Excel then  put   stream ForExcel unformatted
        tt-write-off.object      CHR(9)
        tt-write-off.wr-off-num  CHR(9)
        tt-write-off.pri-num     CHR(9)
        tt-write-off.prt-date    CHR(9)
        tt-write-off.qnty        CHR(9)
        tt-write-off.sum-rubl    CHR(9)
        tt-write-off.vat-rubl    CHR(9)
        tt-write-off.vat         CHR(9)
    skip.
...
find first sheetf where sheet-num = 1 .
    ...
    Sheetf.MergeCellsH = ""
    Sheetf.MergeCellsV = ""
    Sheetf.Excel-Column-Lable = "Наименование объекта" + chr(44) +
                        "Номер документа списания " + chr(44) +
                        "Номер приходной накладной" + chr(44) +
                        "Дата поступления партии" + chr(44) +
                        "Количество" + chr(44) +
                        "Сумма учетных цен без НДС" + chr(44) +
                        "Сумма НДС (учетные цены)" + chr(44) +
                        "Ставка НДС"
