...
define variable v-clients-boss-name as   character             no-undo.
...
define variable v-diff-qnty-kg      as   decimal               no-undo.
define variable v-error-meas-kg     as   decimal               no-undo.
define variable v-shortage-surplus-kg as decimal               no-undo.
define variable v-diff-qnty-kg-15C  as   decimal               no-undo.
define variable v-error-meas-kg-15C as   decimal               no-undo.
define variable v-shortage-surplus-kg-15C as decimal           no-undo.
...
define variable v-norm-natur-loss   as   decimal               no-undo.
define variable v-norm-natur-loss-15C as decimal               no-undo.
...
define variable v-tank-density-pomi-dll as decimal             no-undo.
define variable v-tank-vol-pomi-dll     as decimal             no-undo.
define variable v-tank-weight-pomi-dll  as decimal             no-undo.
    define buffer buf_trn-doc       for ub.trn-doc.
...
output stream out-stream to value( string( session:temp-directory +
        ...
        find first clients where clients.obj-code = buf_trn-doc.boss and
                                 clients.obj-type = 'чел':U no-lock no-error.
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_trn-doc.doc-code ,
        ...
        run apn-xl-write-cell-data in this-procedure (
              input "driver_name2":U
            , input v-fio
        ).
        run apn-xl-write-cell-data in this-procedure (
              input "inspection_cert":U
        ...
        if available buf_sr-izmerenia then do:
          ...
          if buf_sr-izmerenia.sr-type-id = 3 or buf_sr-izmerenia.sr-type-id = 4 then do:
            ...
            run apn-xl-write-cell-data in this-procedure (
                  input "passport_plotn":U
                , input v-passport-plotn
            ).
            run apn-xl-write-cell-data in this-procedure (
                  input "date_pov_plotn":U
                , input fnc-DD-MM-YYYY(v-date-pov-plotn)
            ).
          end.
      end.
        ...
        do:
            ...
            no-lock:
                v-clients-boss-name = trim(buf_clients.obj-name).
                ...
                no-lock:
                    v-clients-boss-name =
                        ...
                        else caps(substring(trim(buf_person.name1), 1, 1)) + ". ") +
                        (if trim(buf_person.name1) = "" or trim(buf_person.name2) = "" then ""
                        else caps(substring(trim(buf_person.name2), 1, 1)) + ".").
                end.
            end.
...
if (valid-handle(g#lib-calc) <> true) then do:   run str/lib-calc.p persistent no-error .   if error-status :error or (valid-handle(g#lib-calc) <> true) then do:     message       "Error starting lib-calc.p" skip       g#lib-calc skip       g#lib-calc :type skip       g#lib-calc :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-calc_chkqtpl in g#lib-calc
  ...
  , output varinv-set
        ...
        run apn-xl-write-cell-data in this-procedure (
              input "norm_natur_losses":U
            , input fnc-convert-dot-to-colon((v-norm-natur-loss), "->>>>>>>>9.999")
        ).
        run apn-xl-write-cell-data in this-procedure (
              input "sum_errm_and_norml_kg":U
        ...
        run apn-xl-write-cell-data in this-procedure (
              input "norm_natur_losses_15C":U
            , input fnc-convert-dot-to-colon(v-norm-natur-loss-15C, "->>>>>>>>9.999")
        ).
        run apn-xl-write-cell-data in this-procedure (
              input "sum_errm_and_norm_15C":U
            , input fnc-convert-dot-to-colon(
        ...
        run apn-xl-write-cell-data in this-procedure (
              input "shor_surplus_kg_15C":U
            , input fnc-convert-dot-to-colon(v-shortage-surplus-kg-15C, "->>>>>>>>9.999")
        ).
        run apn-xl-write-cell-data in this-procedure (
              input "diff_norm_nat_los_15C":U
            , input fnc-convert-dot-to-colon(
                                            (v-norm-natur-loss-15C)
                                            , "->>>>>>>>9.999")
        ).
        run apn-xl-write-cell-data in this-procedure (
              input "driver_name4":U
            , input v-fio
        ).
        run apn-xl-write-cell-data in this-procedure (
              input "host_name3":U
...
procedure loc-get-set-attr :
  ...
  :
    define buffer buf_doc-line-attr for ub.doc-line-attr .
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "car-num"     no-error.
    if p-mode-attr = "get-attr":U then do:
      if available buf_doc-attr then do:       assign         v-car-num = buf_doc-attr.attr-value       .     end.
    end.
    else do:
    end.
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "inspection-cert"     no-error.
    if p-mode-attr = "get-attr":U then do:
      if available buf_doc-attr then do:       assign         v-inspection-cert = buf_doc-attr.attr-value       .     end.
    end.
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "date-cert"     no-error.
    if p-mode-attr = "get-attr":U then do:
      if available buf_doc-attr then do:       assign         v-date-cert = date(buf_doc-attr.attr-value)       .     end.
    end.
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "autoent"     no-error.
    if p-mode-attr = "get-attr":U then do:
            assign
    ...
    if p-mode-attr = "get-attr":U then do:
      ...
      no-error.
        ...
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "ptb-item-pour"     no-error.
    if p-mode-attr = "get-attr":U then do:
            if available buf_doc-attr then do:       assign         v-item-pour = buf_doc-attr.attr-value       .     end.
    end.
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "time-income"     no-error.
    if p-mode-attr = "get-attr":U then do:
      if available buf_doc-attr then do:       assign         v-time-income = buf_doc-attr.attr-value       .     end.
    end.
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "fio-driver"     no-error.
    if p-mode-attr = "get-attr":U then do:
            if available buf_doc-attr then do:       assign         v-fio = buf_doc-attr.attr-value       .     end.
    end.
    else do:
    end.
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "ptbobj"     no-error.
    if p-mode-attr = "get-attr":U then do:
            assign
        v-ptbotype = entry (1, buf_doc-attr.attr-value, ";")
      no-error.
    end.
    else do:
    end.
    ...
    if p-mode-attr = "get-attr":U then do:
            assign
        v-ptbocode = entry (2, buf_doc-attr.attr-value, ";")
      no-error.
    end.
    else do:
    end.
        find first buf_doc-attr     where buf_doc-attr.doc-code  = v-doc-code       and buf_doc-attr.attr-code = "time-pour"     no-error.
    if p-mode-attr = "get-attr":U then do:
            assign
        v-time-pour = buf_doc-attr.attr-value
      no-error.
    end.
    else do:
    end.
            assign         v-car-vol = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):CarVol)      .
            assign         v-dens-temp = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):DensTemp)      .
            assign         v-tank-vol-pomi = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):TankVolPomi)      .
            assign         v-tank-density-pomi = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):TankDensityPomi)      .
            assign         v-place-si = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):PlaceSi)      .
            assign         v-tests = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):Tests)      .
            assign         v-certif-fuel = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):CertifFuel)      .
            assign         v-norm-doc = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):NormDoc)      .
            assign         v-num-passport = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):NumPassport)      .
            assign         v-validity-certif = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):ValidityCertif)      .
            assign         v-num-plotn = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):NumPlotn)      .
            assign         v-passport-plotn = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):PassportPlotn)      .
            assign         v-date-pov-plotn = v-InfoSectionsTotal:GetInfoSectionProp(iNum):DatePovPlotn      .
            assign         v-date-start = v-InfoSectionsTotal:GetInfoSectionProp(iNum):DateStart      .
            assign         v-time-start = v-InfoSectionsTotal:GetInfoSectionProp(iNum):TimeStart      .
            assign         v-date-end = v-InfoSectionsTotal:GetInfoSectionProp(iNum):DateEnd      .
            assign         v-time-end = v-InfoSectionsTotal:GetInfoSectionProp(iNum):TimeEnd      .
            assign         v-tank-vol = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):TankVol)      .
            assign         v-tank-temp = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):TankTemp)      .
            assign         v-tank-water = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):TankWater)      .
            assign         v-tank-density = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):TankDensity)      .
            assign         v-tank-weight = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):TankWeight)      .
            assign         v-mouth = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):Mouth)      .
            assign         v-a-b-tarir = string(v-InfoSectionsTotal:GetInfoSectionProp(iNum):ABTarir)      .
    return .
  end.
end procedure.
procedure proc-calc-library-pomi:
    define input parameter p-car-vol as decimal no-undo.
    define input parameter p-tank-vol as decimal no-undo.
    define input parameter p-a-b-tarir as decimal no-undo.
    ...
    define input parameter p-place-si as integer no-undo.
...
procedure proc-get-address:
    ...
    p-cli-obj-type = trim(p-cli-obj-type).
    case p-cli-obj-type:
        when 'орг':U then
        do:
