...
:
    ...
    put stream excel-line unformatted
        ...
        CHR(9)   "DTA":U
        CHR(9)   buf_temp_line-data.d_LineNum
        CHR(9)   buf_temp_line-data.d_SectionName
        CHR(9)   buf_temp_line-data.d_ShiftNum
        CHR(9)   buf_temp_line-data.d_ChkNum
        CHR(9)   buf_temp_line-data.d_Summ
        CHR(9)   buf_temp_line-data.d_menager
        chr(10)
    .
end.
end procedure.
...
define variable v-obj-code        as integer no-undo init -1.
  if is-doc then do :
    DEFINE FRAME km-frame-akt
          sym1                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.num-pos        column-label "Номер! по !пор-!ядку":C5  format ">>>>>9"          space(0)
          sym2                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.section-name   column-label " ! Наименование ! отдела, секции ! ":C30 format "X(30)"           space(0)
          sym3                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.shift-num      column-label " ! Номер ! смены ! ":C9  format "9999999"         space(0)
          Sym4                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.chk-num        column-label " ! Номер ! чека !":C7  format "9999999"         space(0)
          sym5                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.chk-tot        column-label " ! сумма чека ! руб. коп. ! ":C15 format "->>>,>>>,>>9.99" space(0)
          sym6                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.person         column-label " Должность, ! фамилия, и., о. лица, ! разрешившего возврат ! денег по чеку ":C27 format "X(27)"           space(0)
          sym7                    column-label ":!:!:!:":U  format "X(1)"            space(0)
     HEADER
    ...
    .
      DEFINE FRAME km-frame-rep
          ...
          temp-str.section-name   column-label " ! Наименование ! отдела, секции ! ":C30 format "X(30)"           space(0)
          sym3                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.shift-date     column-label " ! Дата  ! смены ! ":C10 format "99/99/9999"      space(0)
          sym4                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.shift-num      column-label " ! Номер ! смены ! ":C7  format "9999999"         space(0)
          Sym5                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.cash-num       column-label " ! Номер ! кассы ! ":C7  format "9999999"         space(0)
          Sym6                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.chk-num        column-label " ! Номер ! чека !":C7  format "9999999"         space(0)
          sym7                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.chk-tot        column-label " ! сумма чека ! руб. коп. ! ":C15 format "->>>,>>>,>>9.99" space(0)
          sym8                    column-label ":!:!:!:":U  format "X(1)"            space(0)
          temp-str.person         column-label " Должность, ! фамилия, и., о. лица, ! разрешившего возврат ! денег по чеку ":C30 format "x(30)"           space(0)
          sym9                    column-label ":!:!:!:":U  format "X(1)"            space(0)
        HEADER
    ...
    assign
      sheetf.sheet-num = 1
      sheetf.Excel-Column-Lable = "Номер по порядку"                         + chr(44) +
                                  "Наименование отдела\секции"               + chr(44) +
                                  "Дата смены"                               + chr(44) +
                                  "Номер смены"                              + chr(44) +
                                  "Номер кассы"                              + chr(44) +
                                  "Номер чека"                               + chr(44) +
                                  "Сумма чека руб. коп."                     + chr(44) +
...
foreach:
for each buf_chk-doc where buf_chk-doc.obj-type  = 'маг':U
...
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_is-petrl in g#lib-trn
  ...
  temp-str.shift-num     = buf_chk-doc.shift-num
...
if not available temp-str then do:
    if p-batch > 0 then do:
    if p-batch > 0 then do:      run write-log-and-file in p-log-handle (                 input 1                                          , input p-log-file-name                              , input 1                                          , input "Не было возвратов за выбранный период!!").    end.    else do:       run write-to-log in p-log-handle ( input "Не было возвратов за выбранный период!!").    end.
    run cb_write-report-error in p-parent-handle ( input p-rebh
                                                  ,input p-report-id
                                                  ,input ?
                                                  ,input '3':U
                                                  ,input "Не было возвратов за выбранный период!!").
    RETURN.
  end.
  else do:
    if is-doc then do:
   ...
   .
    FIND FIRST buf_cash-desk
        WHERE buf_cash-desk.cash-num = tt-cash-desk.cash-num
          ...
          NO-LOCK NO-ERROR .
    FIND    This_Object
      WHERE This_Object.obj-type = 'маг':U
...
if is-doc then do:
  run km3xl-close in this-procedure .
    ...
    if Make-Excel then  put   stream ForExcel unformatted
      "Итого:"  CHR(9) CHR(9) CHR(9) CHR(9) CHR(9) CHR(9)
      sum2-shift
    .
    Put stream Out-Stream Line format "X(123)" SKIP.
  Display stream Out-Stream
          "    ИТОГО" @ temp-str.chk-num
          sum2-shift  @ temp-str.chk-tot
          sym7 sym8
...
DO:
  ...
  display stream Out-Stream
          ...
          sym7
  with FRAME km-frame-akt.
  DOWN stream Out-Stream 1 with FRAME km-frame-akt
  .
...
DO:
  ...
  then  PAGE STREAM Out-Stream.
    display stream Out-Stream
            ...
            sym9
    with FRAME km-frame-rep.
    DOWN stream Out-Stream 1 with FRAME km-frame-rep
    .
    if Make-Excel then  put   stream ForExcel unformatted
      Lines_Counter                CHR(9)
      temp-str.section-name        CHR(9)
      temp-str.shift-date          CHR(9)
      temp-str.shift-num           CHR(9)
      temp-str.cash-num            CHR(9)
      temp-str.chk-num             CHR(9)
      temp-str.chk-tot             CHR(9)
      temp-str.person              CHR(9) chr(10)
    .
end.
    end.
  end.
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
   ...
   for each thbjattr_thbj-attr :
       if thbjattr_thbj-attr.prop-code = 'outR':U      then v-outR      = string( thbjattr_thbj-attr.property-value-character) .
   end.
   if v-outR = "no_print"
      then do:
         assign
...
define variable vss-include-info33 as character format "x(65)" no-undo initial "@(#)$Workfile:currsysk.i $ $Revision: $".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run currsysk in g#library
  ...
  if check-entry-with-mask( v-sys-key, "Pskov":U, chr(44) ) = true then do:
  ...
  FIND FIRST buf_cash-desk WHERE buf_cash-desk.cash-num = tt-cash-desk.cash-num
   ...
   PUT STREAM Out-Stream
       ...
       space(4) Line           format "X(73)" "по ОКРО" format "X(7)" AT 79 "|" AT 86 t-okpo format "X(16)" "|" AT 103 skip
       space(34) "организация" format "X(50)" "+----------------+" AT 86 skip
       space(4) v-object format "X(75)" "ИНН|" AT 83  t-INN format "X(15)"  "|" AT 103 skip
...
do on error undo, return error return-value  :
   ...
   v-out-str[1] = "на сумму " + PropisSumall + " руб. " + STRING(v-kop,"99") + " коп.".
   ...
   PUT STREAM Out-Stream
       ...
       space(5) v-out-str[1] FORMAT "X(103)" skip
       space(5) v-out-str[2] FORMAT "X(103)" skip
