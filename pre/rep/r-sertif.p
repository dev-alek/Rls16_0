             ...
             if available buf_chk-discnt then do :
                find first buf_clients no-lock
                    where buf_clients.obj-code = buf_chk-doc.cashier-psn-code
                      and buf_clients.obj-type = 'чел':U no-error.
                create tt-sertif.
                assign
  ...
  for each obj-list no-lock :
    ...
    if not available sheetf then do :
      ...
      if Make-Excel then  put   stream ForExcel unformatted
        ...
        "Итого:"             CHR(9)
        v-tot-chk-sum        CHR(9)
        v-tot-discount       CHR(9)
                             CHR(9)
        v-tot-sum-netto      CHR(9)
        skip.
        if Make-Excel then  put   stream ForExcel unformatted
        skip.
        if Make-Excel then  put   stream ForExcel unformatted
...
find first sheetf where sheet-num = p-page-num .
    assign
    Sheetf.MergeCellsH = ""
    Sheetf.MergeCellsV = ""
    Sheetf.Excel-Column-Lable = "№ ККМ" + chr(44) +
                         "№ чека по ККМ" + chr(44) +
                         "Дата" + chr(44) +
                         "№ чека в Trade House" + chr(44) +
                         "Кассир" + chr(44) +
                         "Сумма по чеку" + chr(44) +
                         "Сертификат(Скидка)" + chr(44) +
                         "Прочие скидки" + chr(44) +
                         "Сумма оплат (нетто)"
...
if Make-Excel then  put   stream ForExcel unformatted
   tt-sertif.cash-num        CHR(9)
   tt-sertif.cash-chk-num    CHR(9)
   tt-sertif.chk-date        CHR(9)
   tt-sertif.th-chk-num      CHR(9)
   tt-sertif.cashier         CHR(9)
   tt-sertif.chk-sum         CHR(9)
   tt-sertif.discount        CHR(9)
   tt-sertif.other-discount  CHR(9)
   tt-sertif.sum-netto       CHR(9)
   skip.
end.
