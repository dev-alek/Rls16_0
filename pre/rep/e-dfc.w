...
define variable vss-description as character no-undo init "Печать Документа назначения цены (закладка № 2)".
...
PROCEDURE adm-disable :
    DISABLE RECT-5 RECT-6 Classify SortType WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-5 RECT-6 Classify SortType WITH FRAME F-Main.
...
ASSIGN
       TOG-prt:HIDDEN IN FRAME F-Main           = TRUE.
...
if num-entries (reportname , "|" ) < 3 then message "Ошибка программирования".
v-tmp  = entry ( 2 , reportname , "|" ) .
v-type = entry ( 3 , reportname , "|" ) .
  run rep/r-dfc.p
      ( integer (v-tmp)  ,
        p-curr-code ,
        classify   ,
...
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main Classify SortType.
...
define variable v-radio-customer as integer no-undo .
define variable v-radio-schet    as integer no-undo .
define variable v-curr-code      as integer no-undo .
define variable v-radio-period   as character no-undo .
define variable v-keep-spis      as character no-undo .
...
procedure get-var-from-1-report-page :
 ...
 on error undo, return error return-value
 :
define output parameter p-customer as integer no-undo .
define output parameter p-schet    as integer no-undo .
define output parameter p-curr-code as integer no-undo .
define output parameter p-period    as character no-undo .
define output parameter p-keep-spis  as character no-undo .
