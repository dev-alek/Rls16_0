...
PROCEDURE adm-disable :
    DISABLE RECT-12 RECT-10 Classify ie ee ep es re rs Itog we tog-zero vt iv R-coll ev rv E-coll em wm im ot WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-12 RECT-10 Classify ie ee ep es re rs Itog we tog-zero vt iv R-coll ev rv E-coll em wm im ot WITH FRAME F-Main.
...
DO:
  ...
  IF r-coll = 1  THEN DO:
    ...
    for each season no-lock where
       ...
       e-coll = e-coll + season.sea-name + chr(10)  .
    end.
  END.
  ELSE DO:
    run ref/collec.w
    ...
    if v-recid = "" or v-recid = ? then do:
       r-coll = 1.
       display r-coll  with frame F-Main .
        e-coll = "Все: " + chr(10)  .
        for each season no-lock where
                season.sea-month-1 = 0 and
                season.sea-month-2 = 0 :
  ...
  END.
  DISPLAY e-coll WITH FRAME F-Main .
END.
...
v-par-val = "Коллекции"
Classify:RADIO-BUTTONS in frame F-Main  = substitute("&1,5,&2,6,&3,7", v-par-val , v-par-val + '/Группы товаров' , 'Группы товаров/' + v-par-val ) .
R-coll:RADIO-BUTTONS   in frame F-Main  = substitute("&1,1,&2,2", "Все " + v-par-val ,  'Выборочно'  ) .
...
run cur-time in this-procedure ( output v-today
      ...
      if x-SelectGood = 1 Then DO:
          If x-SET_val_TYPE = 1
            then DO :
                run rep/r-oprts5.p (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input Itog, input tog-zero , input table tt-season ) .
                End.
            Else do:
                run rep/r-oprts6.p (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input Itog, input tog-zero , input table tt-season ) .
                End.
      End.
      Else DO:
          If x-SET_val_TYPE = 1
            then do:
                run rep/r-oprts7.p (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input Itog, input tog-zero , input table tt-season ) .
                End.
            Else do:
                run rep/r-oprts8.p (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input Itog, input tog-zero , input table tt-season ) .
                End.
      End.
END PROCEDURE.
PROCEDURE my-var :
