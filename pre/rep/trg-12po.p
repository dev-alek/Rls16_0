...
define frame f-doc-m
        ...
        gds-prop.gds-name COLUMN-LABEL "Наименование товара! ! ! ! " format "X(52)" space(0)
    ...
    header
        ...
        string( "Документ N: " + v-torgconf-doc-code + " от " + v-torgconf-doc-date ) at 40 format "X(50)"
            ( if t-doc.status_ <> 'факт':U then string( "Статус документа: " + t-doc.status_ + " " + string( t-doc.flag_, "+/-" ) )
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input t-doc.doc-code ,
  ...
  put stream out-stream  "дата" format "X(4)" at 175 "| " at 180 "|" at 198 skip
    ...
    space(45) string( "ТОВАРНАЯ НАКЛАДНАЯ | " + string( v-torgconf-doc-code, "X(16)") + " | " + v-torgconf-doc-date + " | " +
                      (if t-doc.status_ <> 'факт':U then string( "(" + CAPS(t-doc.status_) + ")" ) else "") ) format "X(100)" skip
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input t-doc.doc-code ,
  ...
  put stream out-stream  skip
    space(30) string( "НДС: " + trim( string( all-VAT-gds, "->>>,>>>,>>>,>>>,>>9.99") ) +
                                " (" + trim( ( if PrintRubl then "руб" else "баз.вал" ) ) + ")" ) format "X(160)" skip(2)
...
procedure print-line :
  do on error undo, return error :
    if FullGdsName then do:
      gds-str1 = breakstr(gds-prop.gds-name, 52, input-output gds-str1, input-output gds-str2).
