...
PROCEDURE adm-disable :
    DISABLE RECT-8 RECT-9 RECT-5 RECT-6 RECT-10 Tog-obj SortType Classify tog-wt tog-dens ShowZero-2 ShowCost ShowCostNDS ShowCost-2 EDITOR-1 ShowSale ShowSaleNDS ShowSale-2 ShowSaleNDS-2 ShowSaleSLT ShowMediator Tog-Excel B-columns WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-8 RECT-9 RECT-5 RECT-6 RECT-10 Tog-obj SortType Classify tog-wt tog-dens ShowZero-2 ShowCost ShowCostNDS ShowCost-2 EDITOR-1 ShowSale ShowSaleNDS ShowSale-2 ShowSaleNDS-2 ShowSaleSLT ShowMediator Tog-Excel B-columns WITH FRAME F-Main.
...
ON CHOOSE OF B-columns IN FRAME F-Main
DO:
...
END.
ON VALUE-CHANGED OF Long-name IN FRAME F-Main
DO:
  ASSIGN long-name .
END.
ON VALUE-CHANGED OF ShowCost IN FRAME F-Main
DO:
  assign ShowCost.
  if ShowCost then
   enable   ShowCost-2 with frame F-Main .
  else  do:
  ShowCost-2 = false .
  disable ShowCost-2  with frame F-Main .
  end.
display  ShowCost-2 with frame F-Main.
END.
...
END.
ON VALUE-CHANGED OF ShowZero-3 IN FRAME F-Main
DO:
  assign showzero-3.
      if showzero-3 = no then do:
         display EDITOR-1 with frame F-Main .
      end.
      else hide EDITOR-1 in frame F-Main .
END.
ON VALUE-CHANGED OF tog-dens IN FRAME F-Main
DO:
  assign tog-dens.
  display  tog-dens with frame F-Main.
  run excel-yes in this-procedure.
END.
ON VALUE-CHANGED OF Tog-Excel IN FRAME F-Main
DO:
    ASSIGN Tog-Excel .
  IF Tog-Excel THEN ENABLE Long-name WITH FRAME F-Main.
      ELSE
    DISABLE Long-name WITH FRAME F-Main.
END.
...
END.
ON VALUE-CHANGED OF Tog-tree IN FRAME F-Main
DO:
...
IF tog-excel = NO THEN
   DISABLE long-name with frame F-Main.
ELSE enable long-name with frame F-Main.
...
define buffer   buf_usr-flt for ubflt.usr-flt  .
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .
   Tog-obj:screen-value in frame F-Main = string(True).
          ...
          repeat l-ind = 1 to v-nn :
            if index ( entry(l-ind,buf_usr-flt.list_),"=") = 0 THEN DO :
                if int(entry(l-ind,buf_usr-flt.list_)) > 0 and
                   int(entry(l-ind,buf_usr-flt.list_)) <= 28
                   then
...
do:
 ...
 if not g#log then
    disable ShowCost ShowCostNDS with frame F-Main.
...
do:
 ...
 if not g#log then
    disable ShowSAle ShowSaleNDS with frame F-Main.
...
do:
 ...
 if not g#log then
    disable ShowSAle-2 ShowSaleNDS-2 ShowSaleSlt with frame F-Main.
display editor-1 with frame F-Main.
END PROCEDURE.
PROCEDURE my-report :
...
END PROCEDURE.
PROCEDURE my-var :
Assign frame F-Main SumsOnly ShowZero tog-obj Classify SortType
...
ASSIGN ReportHeader = "Классификация : " + t-Class +
                        ( IF Tog-Lavel = YES THEN "    Итоги с уровня  " + STRING( Var-Lavel ) ELSE " ":U ) + chr(10) +
                        "Сортировка " + t-Sort + chr(10) + "Показать : " +
                        ...
                        ( IF ShowMediator  = YES THEN " Суммы в ценах посредника"        ELSE " ":U ) + chr(10) +
                        ( IF Tog-Tree      = YES THEN " группировка ввиде дерева"        ELSE " ":U ) + chr(10) +
                        ...
                        ( IF RServ <> 'все':U THEN chr(10) + "Только " + RServ ELSE " ":U ).
ReportHeader = ReportHeader + (if Rserv <> 'все':U     then  chr(10) + "Только " + Rserv  else " "  ) .
