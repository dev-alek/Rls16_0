...
PROCEDURE adm-disable :
    DISABLE RECT-totals RECT-cashiers RECT-checks WithCashiers SelectCashiers WithGoods ByOperations WithoutObjects WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-totals RECT-cashiers RECT-checks WithCashiers SelectCashiers WithGoods ByOperations WithoutObjects WITH FRAME F-Main.
...
define buffer buf_staff for ub.staff.
  ...
  if SelectCashiers = 'все':U then do:
    create tt-cshr.
    assign
    tt-cshr.psn-code = - 1
    tt-cshr.cashier = 0
...
ASSIGN
SelectCashiers:RADIO-BUTTONS IN FRAME F-Main =  "все" + chr(44) + 'все':U + chr(44) +
                                 "Выборочно" + chr(44) + "currency":U.
...
FOR EACH obj-list :
  ...
  FOR EACH ub.chk-doc WHERE
    ...
    if NOT can-do( 'т,у,сумма':U, ub.chk-doc.office )  then NEXT _chk-doc.
    if SelectCashiers <> 'все':U then do:
    ...
    if chk-doc.netto < 0 then  do:
      ...
      AND can-find( first ub.chk-gds where
        ...
        str-buf = ub.chk-doc.obj-type + chr(32) + string( ub.chk-doc.obj-code ) .
        _chk-gds:
...
PROCEDURE My-var :
 assign
 FRAME F-Main SelectCashiers
 FRAME F-Main ByGoods
 FRAME F-Main BySums
 FRAME F-Main ByOperations
 FRAME F-Main WithoutObjects
 FRAME F-Main WithCashiers
 .
Assign
 STR-obj-type = ''
 STR-obj-code = ''
...
ReportHeader =
               "Кассиры : " +
               radio-label(string(SelectCashiers), SelectCashiers:radio-buttons) + chr(10) +
               ...
               (if BySums then " Суммовые" else "" ) + chr(10) +
               "Итоги по объектам : " +
               radio-label(string(WithoutObjects), WithoutObjects:radio-buttons) + chr(10) +
               "Итоги по кассирам : " +
               radio-label(string(WithCashiers), WithCashiers:radio-buttons) + chr(10) +
               "Итоги по товарам  : " +
