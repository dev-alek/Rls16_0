...
define variable vss-workfile    as character no-undo initial "$Workfile: r-outretu.p $":U .
define variable vss-archive     as character no-undo initial "$Archive: rep/r-outretu.p $":U .
define variable vss-description as character no-undo initial "Документ расхода и списания короткий(с упаковками)":U .
...
define temp-table temp_line-data no-undo
    ...
    field qntyup       as character
    field qntycart     as character
...
for buf_temp_cell-data on error undo, return error :
    ...
    run outretxl-write-cell-data in this-procedure (
          input "columnList":U
        , input "ID,gdscode,artic,Name,EI,price,qnty,qntyup,qntycart,sum":U
    ).
    run outretxl-write-cell-data in this-procedure (
          input "columnType":U
        , input "I,I,S,S,S,D,D,D,D,D":U
    ).
    run outretxl-write-cell-data in this-procedure (
...
define input parameter p-qntyup         as character        no-undo.
define input parameter p-qntycart       as character        no-undo.
...
:
    ...
    assign
        ...
        buf_temp_line-data.qntyup       = p-qntyup
        buf_temp_line-data.qntycart     = p-qntycart
    ...
    put stream excel-line unformatted
        ...
        chr(9)   buf_temp_line-data.qntyup
        chr(9)   buf_temp_line-data.qntycart
...
procedure outretxl-run-excel :
  ...
  do for buf_temp-param on error undo, return error :
    create buf_temp-param.
    assign
        v-template-file-name    = search( "exe/nou_97.xlt" )
...
define variable vss-include-info34 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  define variable qnty-up      as  decimal     no-undo .
  define variable qnty-cart    like ub.goods.qnty-cart .
  ...
  define variable all-qnty-up      as  decimal     no-undo.
  ...
  define variable v-accept-fname    as  character          no-undo .
  define variable v-pass-fname      as  character          no-undo .
  define frame val
        ...
        gds_name column-label "Наименование! товара ":C34 format "X(33)" space(0)
        ...
        qnty-up column-label "Кол-во ! уп-вок" format "->>,>>9.<<<" space(0)
        sym9 column-label ":!:" format "X(1)" space(0)
        qnty-cart column-label "Кол-во ! в уп-ке" format ">>,>>9.<<<" space(0)
  ...
  define frame rubl
        ...
        buf_goods.artic column-label "Артикул! ":C16 format "X(16)" space(0)
        sym4     column-label ":!:" format "X(1)" space(0)
        gds_name column-label "Наименование! товара ":C32 format "X(31)" space(0)
        sym5     column-label ":!:" format "X(1)" space(0)
        buf_goods.unit-base column-label "Ед.!изм" format "X(3)" space(0)
        sym6    column-label ":!:" format "X(1)" space(0)
        ...
        qnty-cart column-label "Кол-во!в уп-ке" format ">>,>>9.<<<" space(0)
        sym10 column-label ":!:" format "X(1)" space(0)
        stoim   column-label "Стоимость!(РУБ) " format "->>>,>>>,>>>,>>9.99" space(0)
        sym11   column-label ":!:" format "X(1)" space(0)
    header
        string( "Дата печати : " + string(TODAY,"99.99.9999") +  " , " + string(TIME, "HH:MM") ) at 5 format "X(35)"
        string( "Накладная " + " N " + v-doc-code + " от " + v-doc-date-string ) at 45 format "X(70)"
...
run adm/shattri.p (
  ...
  find first Our_Host   no-lock where Our_Host.obj-type = 'орг':U           and Our_Host.obj-code = v-torgconf-self-host-code .
  ...
  else  assign v-doc-date-string = string( t-doc.doc-date, "99.99.9999" ) + ( if t-doc.status_ = 'факт':U then " (факт " + string( t-doc.fact-date, "99.99.9999" ) + ")" else "" ) .
output stream Out_stream to value( string( session:temp-directory +
                                     "rpt" + string( g#report-num ) ) )
                                     page-size 62 .
  run outretxl-init in this-procedure .
  form header  Line format "X(137)" at 1  skip "Продолжение - на следующей странице" at 30   skip
...
do:
  ...
  if can-do( 'накл':U , t-doc.status_ ) and ( not t-doc.flag_ ) then put stream Out_stream "----------   Т Е С Т О В А Я   П Е Ч А Т Ь   ----------"  at 31 format "X(60)" skip(1) .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prtat in g#library
  ...
  ,output empty-scale
    ...
    .
    if empty-scale = no and PrintScale = yes then do:
      ...
      with frame rubl.
      display stream out_stream    with frame rubl.
  ...
  ,output empty-scale
    ...
    if empty-scale = no and PrintScale = yes then do:
      display stream out_stream
        sym1  Lines_Counter sym2 tb-code sym3 buf_goods.artic sym4 gds_name sym5 buf_goods.unit-base sym6 sym7 sym8 qnty-up sym9 qnty-cart sym10 sym11
      with frame val.
      display stream out_stream    with frame val.
...
procedure print-titul :
  ...
  if is-chek then do:
    ...
    If V-torgconf-outprim = No or t-doc.internal Then Do:
      Put Stream Out_Stream  String( "Примечание : " + ( If ( Not Can-do( 'факт':U, T-doc.Status_ ) Or Logres ) And Not ( T-doc.Ps Begins "@" ) Then  T-doc.Ps Else " " ) ) At 10 Format "X(100)" Skip .
      run outretxl-write-cell-data in this-procedure (
          input "h_PS":U
        , input ( If ( Not Can-do( 'факт':U, T-doc.Status_ ) Or Logres ) And Not ( T-doc.Ps Begins "@" ) Then  T-doc.Ps Else " " )
      ).
    End.
...
procedure print-itog :
  do on error undo, return error return-value :
    ...
    put stream out_stream Line format "X(137)" skip .
    if PrintRubl then do:
      display stream out_stream
        sym1 sym2 sym3 sym4 "ИТОГО: " @ gds_name sym5 sym6 sym7 all-qnty @ qnty sym8 all-qnty-up @ qnty-up sym9 all-stoim @ stoim  sym10
      with frame rubl.
      down stream out_stream with frame rubl .
    end.
    else do:
      display stream out_stream
        sym1 sym2 sym3 sym4 "ИТОГО: " @ gds_name sym5 sym6 sym7 all-qnty @ qnty sym8 all-qnty-up @ qnty-up sym9 all-stoim @ stoim  sym10 sym11
      with frame val.
      down stream out_stream with frame val .
    end.
    put stream out_stream Line format "X(137)" skip .
    ...
    put stream Out_stream skip(1) space(5) "Всего " format "x(8)" all-qnty format ">,>>>,>>9.<<<"
      space(2) "единицы" format "x(8)" space(2) Lines_Counter format ">,>>>,>>9" space(2) "наименований" format "x(13)"
                                              "на сумму"  all-stoim  format ">,>>>,>>>,>>>,>>9.99"  " "  ( if not PrintRubl then buf_currency.curr-abbr else "РУБ" )
      skip(1).
    run outretxl-write-cell-data in this-procedure (
          input "f_lineAmount":U
        , input string( "Всего " + string(all-qnty,">,>>>,>>9.<<<") + " единицы " + string(Lines_Counter,">,>>>,>>9") + " наименований  на сумму " + string(all-stoim,">,>>>,>>>,>>>,>>9.99") + " " +  ( if not PrintRubl then buf_currency.curr-abbr else "РУБ" ) )
    ).
    if v-torgconf-outdisc = no and not CostPrice then do:
      put stream Out_stream space(5) "Скидка " format "x(8)"
        ...
        space(2) ( if not PrintRubl then buf_currency.curr-abbr else "РУБ" )                    format "x(4)" skip(1)
      .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run usrnick in g#library
  ...
  ,output v-oper-name
    ...
    if can-do( 'факт':U , t-doc.status_ ) or
       ( can-do( 'рас':U , t-doc.doc-type ) and can-do( 'накл,запрос':U , t-doc.status_ ) and
      ...
      if can-do( 'возврат':U , t-doc.doc-type ) then put stream Out_stream space(5) "Итого " format "x(6)" .
      ...
      if is-chek then do:
        ...
        if can-do( 'рас':U , t-doc.doc-type ) and ( not t-doc.internal )
            and ( ( can-do( 'запрос':U , t-doc.status_ )
            or ( can-do( 'накл':U , t-doc.status_ ) and t-doc.flag_ ) ) )
        ...
        then do:
          if t-doc.office then do:
            ...
            find first buf_doc-attr where buf_doc-attr.doc-code = v-doc-code and buf_doc-attr.attr-code = "t_accept-fname":U   no-error .
            if available buf_doc-attr then do :
                assign
                      v-accept-fname = buf_doc-attr.attr-value
                      .
            end.
            find first buf_doc-attr where buf_doc-attr.doc-code = v-doc-code and buf_doc-attr.attr-code = "t_accept-position":U   no-error .
            if available buf_doc-attr then do :
                assign
                    v-accept-fname = buf_doc-attr.attr-value + " " + v-accept-fname.
            end.
            find first buf_doc-attr where buf_doc-attr.doc-code = v-doc-code and buf_doc-attr.attr-code = "t_pass-fname":U   no-error .
            if available buf_doc-attr then do :
                assign
                      v-pass-fname = buf_doc-attr.attr-value
                      .
            end.
            else do:
                assign
                      v-pass-fname = Wrkr_name
                      .
            end.
            if v-pass-fname ne Wrkr_name then do:
              find first buf_doc-attr where buf_doc-attr.doc-code = v-doc-code and buf_doc-attr.attr-code = "t_pass-position":U   no-error .
                if available buf_doc-attr then do:
                    assign
                      v-pass-fname = buf_doc-attr.attr-value + " " + v-pass-fname
                      .
                end.
            end.
            put stream Out_stream space(10)  "Сдал : " v-pass-fname format "x(40)"  "Принял : " v-accept-fname format "x(30)" skip(1) .
          end.
        end.
        else do:
    ...
    run outretxl-write-cell-data in this-procedure (
       ...
       run outretxl-write-cell-data in this-procedure (
          input "f_AcceptFname":U
        , input v-accept-fname
    ).
