...
if session :set-wait-state( "compiler" ) then.
  ...
  put stream PrnLibStream
    ...
    space(5) string( "Идентификационный номер продавца (ИНН)   " + t-inn ) format "X(100)" skip
  ...
  put stream PrnLibStream
    ...
    space(5) string( "Адрес   " + t-addres ) format "X(90)" skip
    space(5) string( "Идентификационный номер покупателя (ИНН)   " + t-inn ) format "X(100)" skip
  ...
  .
  put stream PrnLibStream
    space(10) string( "Цены и суммы указаны в " + trim( ( if x-SET_val_TYPE = 1 then "рублях" else base-type ) ) + "." ) format "X(120)"
    skip(1) .
  ...
  if abs( v-tot-SLT ) >= 0.005 or ( v-tot-discnt >= 0.005 ) then do:
    put stream PrnLibStream
        ...
        + trim( ( if x-SET_val_TYPE = 1 then "РУБ" else base-type ) ) + ")"  format "X(120)"     at 40
    .
    if v-tot-SLT <> 0 then do:
      put stream PrnLibStream  skip space(10) "Налог с продаж: " trim( string( v-tot-SLT, "->>>,>>9.99" ) )
        + " (" + trim( ( if x-SET_val_TYPE = 1 then "РУБ" else base-type ) ) + ")" format "X(150)"     at 40
      .
    end.
  ...
  if v-tax-sum <> 0 then do:
    put stream PrnLibStream
      ...
      + " ("  + trim( if x-SET_val_TYPE = 1 then "РУБ" else base-type  )   + ")"
      format "X(150)"     at 40
    .
  end.
  put stream PrnLibStream
  ...
  do on error undo, return error :
    ...
    do while trim(v-start-string) <> "" :
      ...
      display stream PrnLibStream  sym1 fill(" ",17) + v-add-string @ gds-prop.gds-name
            sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9  sym11 sym12 sym13
      with frame factur .
...
procedure print-line :
  do on error undo, return error  :
    assign
      ...
      gds-str1 = ''
      gds-str2 = ''
      Gds-str1 = breakstr(gds-prop.gds-name, 59, input-output gds-str1, input-output gds-str2)
    .
    ...
    assign gds-str1 = breakstr(gds-prop.gds-name, 59, input-output gds-str1, input-output gds-str2).
    if gds-prop.gds-type = 'у':U then do:
