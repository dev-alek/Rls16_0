...
procedure scn-list-gds-name :
  ...
  :
if NOT can-find(first scn-list) then
    RETURN.
for each scn-list
by scn-list.gds-name
:
  find ub.goods no-lock
    where ub.goods.prod-type = scn-list.prod-type
      and ub.goods.prod-code = scn-list.prod-code
      and ub.goods.artic = scn-list.artic
  .
  ...
  assign
    ...
    list-qnty = scn-list.qnty
    v-part-code   = "":U
    v-promo-code  = scn-list.promo-code
    v-ActionId    = scn-list.ActionId
    v-db-num      = scn-list.db-num
  .
...
for each scn-list
by scn-list.artic
by scn-list.prod-type
by scn-list.prod-code
:
  find ub.goods no-lock
    where ub.goods.prod-type = scn-list.prod-type
      and ub.goods.prod-code = scn-list.prod-code
