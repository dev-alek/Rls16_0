...
:
  ...
  define frame rubl
        ...
        sym9 column-label ":!:" format "X(1)" space(0)
        SLT-sum column-label "Сумма налога!с продаж (РУБ)" format "->>>>>>>>>9.99" space(0)
        sym10 column-label ":!:" format "X(1)" space(0)
    header
        string( "Дата печати : " + string(TODAY,"99.99.9999") +  " , " + string(TIME, "HH:MM") ) at 5 format "X(35)"
        v-doc-code at 70 format "X(14)" " от " v-doc-date-string format "X(10)"
...
define variable vss-include-info34 as character format "x(65)" no-undo initial "@(#)$Workfile:currsysk.i $ $Revision: $".
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run basecode in g#library
  ...
  find first our_host    where our_host.obj-type     = 'орг':U         and our_host.obj-code    = t-doc.host-code no-lock.
  ...
  if v-torgconf-self-schet-exists then do:
    PUT STREAM Out_Stream
      space(9)  string ( "Поставщик: ИНН " + v-torgconf-self-host-inn + " , КПП " + v-torgconf-self-host-kpp + " " + v-torgconf-self-host-name ) format "X(130)" SKIP
  ...
  if buf_sysconf.cash-pay = t-doc.pay-code then assign prn_NP = yes .
  if prn_NP then do:
    ...
    form header
      ...
      with frame BottomframeNp width 232 page-bottom no-labels no-box .
  ...
  if can-do( 'накл':U , t-doc.status_ ) and ( not t-doc.flag_ ) then
  ...
  then do:
    ...
    if not t-doc.internal and not CostPrice then do:
      ...
      if v-torgconf-outdisc = no then do:
        put stream Out_stream space(1) "Скидка " format "x(8)"
          ...
          space(2) ( if not PrintRubl then base-type else "РУБ" )  format "x(4)" skip .
      end.
      else put stream Out_stream " " skip .
      if PrintRubl then do:
        if all-VAT-sum <> ? and all-VAT-sum > 0 then
          put stream Out_stream string( "НДС: " + string( all-VAT-sum , "->>>,>>>,>>9.99" ) + "РУБ" ) at 20 format "x(50)" skip .
        if prn_NP and all-slt-sum <> ? and all-slt-sum > 0  then
          put stream Out_stream string( "налог с продаж: " + string( all-SLT-sum, "->>>,>>>,>>9.99" ) + "РУБ") at 20 format "x(50)" skip .
      end.
      else do:
        if all-vat-sum <> ? and all-vat-sum > 0 then
  ...
  if can-do( 'рас':U , t-doc.doc-type ) and ( not t-doc.internal ) and ( ( can-do( 'запрос':U , t-doc.status_ )
          or ( can-do( 'накл':U , t-doc.status_ ) and t-doc.flag_ ) ) ) or t-doc.status_ = 'факт':U
  then do:
