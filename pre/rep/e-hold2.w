...
def var vss-description as character no-undo init "Отчет по межфирменным операциям - динамика закупки  (закладка № 2)".
...
DEFINE FRAME F-Main
     ...
     RADIO-SET-1 AT ROW 6.67 COL 4.63 NO-LABEL
     "Поставщики:" VIEW-AS TEXT
          SIZE 22.38 BY .67 AT ROW 5.71 COL 4
     RECT-6 AT ROW 5.54 COL 1.63
...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-5 dat RADIO-SET-1 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-5 dat RADIO-SET-1 WITH FRAME F-Main.
...
DO:
  ...
  if RADIO-SET-1 = 2 then do:
    ...
    if cli-list = "" or cli-list = ? then do:
      ...
      DISPLAY RADIO-SET-1 WITH FRAME F-Main.
    end.
  end.
END.
...
PROCEDURE local-initialize :
  ...
  assign
    dat :screen-value in frame F-Main = string(year(x-Date-Start))
  .
END PROCEDURE.
PROCEDURE my-report :
  ...
  run holdattr-value in this-procedure (input v-cat-code, input  'begin-date':U, output v-hold-date-value, output v-hold-date-type) .
 ...
 run rep/r-hold2.p  ( input dat, input RADIO-SET-1, input cli-list ) .
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main dat RADIO-SET-1 .
define variable jj as integer   no-undo .
define buffer buf_clients for clients.
ReportNAme = "Динамика закупки за "  + String(dat) + " год." .
if x-SET_val_TYPE = 1 then ReportHeader = "Цены указаны в: рублях" + chr(10).
else                       ReportHeader = "Цены указаны в: валюте" + chr(10).
if RADIO-SET-1 = 1 then assign  ReportHeader = ReportHeader + "Выбор поставщика: все" .
else do:
  assign  ReportHeader = ReportHeader + "Выбор поставщика: " .
  DO jj = 1 TO num-entries( cli-list ) :
    find first buf_clients where recid(buf_clients) = integer(entry(jj, cli-list)) no-error.
    if not avail buf_clients then next .
    assign  ReportHeader = ReportHeader + buf_clients.obj-name + " (" + buf_clients.obj-type + '#' + string(buf_clients.obj-code) + ")" + ", "  .
  end.
end.
END PROCEDURE.
