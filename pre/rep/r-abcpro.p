...
define variable vss-workfile    as character no-undo init "$Workfile: r-abcpro.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-abcpro.p $":U .
...
define buffer buf_abc-analysis-prod       for ub.abc-analysis-prod  .
 ...
 run get-report-num  in parParentProc(output g#report-num).
  ...
  for each temp-prod :
     ...
     number-group = string(i-numb,"99999") .
      run print-prod-name ( output v-row ) .
      for each temp-str where temp-str.prod-code =  temp-prod.prod-code and temp-str.prod-type =  temp-prod.prod-type no-lock
          break by temp-str.prc
          :
          run print-line in this-procedure .
      end.
      run print-prod-itogo (input v-row ) .
   end.
...
procedure PrintTitul :
  ...
  define variable tt as integer no-undo .
    ...
    num#col# = 2.
    cc = num#str# .
    run macr_excel_char ( "Сравнение ABC анализов по производителям"  , num#str# , num#col#   ) .
    num#str# = num#str# + 1.
    num#col# = 2.
...
End.
  ...
  v-repfrm-title = "Сбор по производителям товаров" .
  counter = 0 .
  for each buf_abc-analysis-prod no-lock where
           buf_abc-analysis-prod.abc-id = temp-abc.id  and
           buf_abc-analysis-prod.db-num = temp-abc.db-num :
    if not can-find ( first temp-prod where temp-prod.prod-code  = buf_abc-analysis-prod.prod-code and temp-prod.prod-type  = buf_abc-analysis-prod.prod-type) then do:
        find first buf_clients no-lock where buf_clients.obj-code =  buf_abc-analysis-prod.prod-code and buf_clients.obj-type =  buf_abc-analysis-prod.prod-type no-error .
        assign
          counter = counter + 1
        .
IF ( counter modulo v-account = 0 )  then DO:
...
End.
   if v-button-stop then  DO:
        ...
        create temp-prod .
        assign
            temp-prod.prc        = buf_abc-analysis-prod.abcg-abc + string ( buf_abc-analysis-prod.rating , "999999999999" )
            temp-prod.prod-type  = buf_clients.obj-type
            temp-prod.prod-code  = buf_clients.obj-code
            temp-prod.prod-name  = buf_clients.obj-name
        .
    end.
  end.
end.
...
num#str# = num#str# + 1.
num#col# = 1.
run macr_excel_char (  temp-prod.prod-name   , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
 assign    num#col# = num#col# + 1 .
 assign    num#col# = num#col# + 1 .
run macr_excel_char ( "Производитель"     , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
    run macr_cell_format
        ( 12     ,
          true   ,
          false  ,
...
run macr_excel_char ( temp-prod.prod-name  , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
 assign    num#col# = num#col# + 1 .
 assign    num#col# = num#col# + 1 .
run macr_excel_char ( "Итого по ПРОИЗВОДИТЕЛЮ"   , num#str# , num#col#   ) . assign num#col# = num#col# + 1 .
run macr_excel_char ( ""                  , num#str# , num#col#   ) . assign num#col# = num#col# + 1 .
for each temp-abc:
  find first ub.abc-analysis-prod no-lock where
             ub.abc-analysis-prod.abc-id   = temp-abc.id and
             ub.abc-analysis-prod.db-num   = temp-abc.db-num and
             ub.abc-analysis-prod.prod-type = temp-prod.prod-type and
             ub.abc-analysis-prod.prod-code = temp-prod.prod-code
             no-error .
  if available ub.abc-analysis-prod then do:
     run macr_excel_char ( ub.abc-analysis-prod.abcg-abc + string(ub.abc-analysis-prod.rating) , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
     run macr_excel_DEC ( ub.abc-analysis-prod.proc-from-all , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
     run macr_excel_DEC ( ub.abc-analysis-prod.abcg-sum-for-estimate   , num#str# , num#col#   ) .
define variable v-color as integer   no-undo .
    if ub.abc-analysis-prod.abcg-abc = "A" then
       assign
         v-color = 3
       .
    if ub.abc-analysis-prod.abcg-abc = "C" then
       assign
         v-color = 0
       .
    if ub.abc-analysis-prod.abcg-abc = "B" then
       assign
          v-color = 41
       .
    if ub.abc-analysis-prod.abcg-abc = "D" then
       assign
         v-color = 14
       .
    if ub.abc-analysis-prod.abcg-abc = "E" then
       assign
         v-color = 13
       .
    if ub.abc-analysis-prod.abcg-abc = "F" then
       assign
          v-color = 48
       .
   put  stream macr_excel unformatted
