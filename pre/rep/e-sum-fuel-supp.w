...
DEFINE new shared TEMP-TABLE tt-chk-type NO-UNDO
...
PROCEDURE adm-disable :
    DISABLE RECT-1 RECT-3 RECT-5 RECT-9 RECT-7 mSupps mGds mOilBases mSuppList mGds-list mOilBaseList mTranTimeMax t-delta-tank-ac t-delta-tank-fact t-itog rs-ac-type t-no-azk-itog rs-trk-err WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 RECT-3 RECT-5 RECT-9 RECT-7 mSupps mGds mOilBases mSuppList mGds-list mOilBaseList mTranTimeMax t-delta-tank-ac t-delta-tank-fact t-itog rs-ac-type t-no-azk-itog rs-trk-err WITH FRAME F-Main.
...
DO:
   apply "VALUE-CHANGED" to mSupps in frame F-Main.
END.
...
DO:
   apply "VALUE-CHANGED" to mGds in frame F-Main.
END.
...
DO:
   apply "VALUE-CHANGED" to mOilBases in frame F-Main.
END.
...
DO:
   assign frame F-Main mTranTimeMax.
END.
...
display mTranTimeMax with frame F-Main.
   ...
   do vI = 1 to num-entries(m-rid-list):
      ...
      if available clients then do:
         assign
            ...
            vSuppNamelist = vSuppNamelist + "," + chr(10) + clients.obj-name
            vOilBaseSupps = vOilBaseSupps + "," + clients.obj-type + string(clients.obj-code)
         .
      end.
   end.
   vSuppNamelist = trim(vSuppNamelist, "," + chr(10)).
   mSuppsList = trim(mSuppsList, ",").
   vOilBaseSupps = trim(vOilBaseSupps, ",").
   mSuppList:screen-value in frame F-Main = vSuppNamelist.
END PROCEDURE.
PROCEDURE select-gds :
   ...
   for each gds-list no-lock:
      ...
      if not can-do(units.type, 'топ':U) then do:
         delete gds-list .
         next.
      end.
      assign
         vGdsName-list = vGdsName-list + "," + chr(10) + string(goods.gds-code) + "(" + goods.gds-name + ")"
         mGdsCodeList  = mGdsCodeList  + "," + string(goods.gds-code)
         .
   end.
   assign
      vGdsName-list = trim(vGdsName-list, "," + chr(10))
      mGdsCodeList  = trim(mGdsCodeList, ",")
      .
   mGds-list:screen-value in frame F-Main = vGdsName-list.
END PROCEDURE.
   ...
   do vI = 1 to num-entries(m-rid-list):
      ...
      if available clients then do:
         assign
            ...
            vOilBaselist = vOilBaselist + "," + chr(10) + clients.obj-name.
      end.
   end.
   vOilBaselist = trim(vOilBaselist, "," + chr(10)).
   mOilBasesList = trim(mOilBasesList, ",").
   mOilBaseList:screen-value in frame F-Main = vOilBaselist.
END PROCEDURE.
