...
define variable gl-w as integer   no-undo .
...
PROCEDURE adm-disable :
    DISABLE r-font IMAGE-5 RECT-10 IMAGE-4 RECT-9 l-height IMAGE-1 BUTTON-2 BUTTON-1 l-width IMAGE-2 l-fontnum l-fontname l-fontsize l-fonttype l-name WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN r-font IMAGE-5 RECT-10 IMAGE-4 RECT-9 l-height IMAGE-1 BUTTON-2 BUTTON-1 l-width IMAGE-2 l-fontnum l-fontname l-fontsize l-fonttype l-name WITH FRAME F-Main.
...
run define-a4-size (
    ...
    ,output A4lans-H
    ,output A4lans-W ).
  l-Height:screen-value   in frame F-Main = string(A4port-H) .
  l-Width:screen-value    in frame F-Main = "278".
  run how-name
  ( input integer(l-height:screen-value  in frame F-Main),
    input integer(l-width:screen-value   in frame F-Main),
    output (l-name )
    ).
 display l-name with frame F-Main.
END.
...
run define-a4-size (
  ...
  if gl-w > A4port-W or gl-w  = 0 or gl-w = ? then do:
    l-Height:screen-value in frame F-Main = string(A4lans-H).
    l-Width:screen-value  in frame F-Main = string(A4lans-W).
  end.
  else do:
    l-Height:screen-value in frame F-Main = string(A4port-H).
    l-Width:screen-value  in frame F-Main = string(A4port-W).
  end.
  run How-name (INPUT integer(l-Height:screen-value  in frame F-Main),
                INPUT integer(l-Width:screen-value   in frame F-Main),
                OUTPUT (l-name ))
                .
 display l-name with frame F-Main.
END.
ON CHOOSE OF IMAGE-1 IN FRAME F-Main
DO:
  l-Width:screen-value   in frame F-Main =
  string (integer(l-Width:screen-value   in frame F-Main) - 1).
  run How-name (INPUT integer(l-Height:screen-value  in frame F-Main),
              INPUT integer(l-Width:screen-value   in frame F-Main),
              OUTPUT (l-name )).
              display l-name with frame F-Main.
END.
ON CHOOSE OF IMAGE-2 IN FRAME F-Main
DO:
  l-Width:screen-value in frame F-Main =
  string (integer(l-Width:screen-value   in frame F-Main) + 1) .
  run how-name
  ...
  display l-name with frame F-Main .
END.
ON CHOOSE OF IMAGE-4 IN FRAME F-Main
DO:
  l-Height:screen-value   in frame F-Main =
  string (integer(l-Height:screen-value   in frame F-Main) - 1).
  run How-name (INPUT integer(l-Height:screen-value  in frame F-Main),
              INPUT integer(l-Width:screen-value   in frame F-Main),
              OUTPUT (l-name )).
 display l-name with frame F-Main.
END.
ON CHOOSE OF IMAGE-5 IN FRAME F-Main
DO:
  l-Height:screen-value   in frame F-Main =
  string (integer(l-Height:screen-value   in frame F-Main) + 1).
  run How-name (INPUT integer(l-Height:screen-value  in frame F-Main),
...
DO:
  ...
  assign
    ...
    ReportPageWidth  = v-w
  .
  l-Height:screen-value   in frame F-Main =  string(ReportPageWidth).
  l-Width:screen-value    in frame F-Main =  string(ReportPageHeight).
...
End.
 ReportPageHeight = Integer(l-Height:screen-value in frame F-Main) .
 ReportPageWidth  = Integer(l-Width:screen-value  in frame F-Main) .
...
if ReportPageHeight = 0 and ReportPageWidth  = 0 then DO:
          Assign l-Height:screen-value  in frame F-Main = string(A4lans-H)
                 l-Width:screen-value   in frame F-Main = string(A4lans-W)
               .
               run How-name (INPUT integer(l-Height:screen-value  in frame F-Main),
                             INPUT integer(l-Width:screen-value   in frame F-Main),
     ...
     if available buf_usr-flt then  DO:
          repeat l-ind = 1 to num-entries(buf_usr-flt.list_) :
                    if entry(1,entry(l-ind,buf_usr-flt.list_),"=")  = "ReportPageHeight":U then
                       l-Height:screen-value in frame F-Main = entry(2,entry(l-ind,buf_usr-flt.list_),"=") no-error .
                       if error-status :error  then message "высота" .
                    if entry(1,entry(l-ind,buf_usr-flt.list_),"=")  = "ReportPageWidth":U then
                       l-Width:screen-value in frame F-Main = entry(2,entry(l-ind,buf_usr-flt.list_),"=") no-error .
                    ...
                    if ReportFontNum = 0 or ReportFontNum = ? or  error-status :error then ReportFontNum = 7.
                       l-FontNum:screen-value in frame F-Main = string(ReportFontNum) .
                        run define-a4-size (
                            input ReportFontNum
                            ,output A4port-H
                            ,output A4port-W
...
define input parameter l-w like ReportPageWidth no-undo .
          Assign l-Height:screen-value  in frame F-Main = string(ReportPageHeight)
                 l-Width:screen-value   in frame F-Main = string(l-w)
                 .
