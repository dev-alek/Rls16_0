...
.
define  temp-table inkas-num no-undo
...
field pay-desk   like chk-doc.pay-desk
INDEX pi IS PRIMARY     obj-type obj-code date_ pay-desk ASCENDING .
define  temp-table all-days_sum no-undo
field obj-type   like    clients.obj-type
field obj-code   like    clients.obj-code
field tot-base   like   chk-pay.tot-base
field tot-rubl   like   chk-pay.tot-rubl
...
define  temp-table ben-chk-count no-undo
...
.
define  temp-table tt-gds-sum no-undo
  field obj-type  like clients.obj-type
  field obj-code  like clients.obj-code
  field pay-code  like cash-pay.cdpay-code
  field curr-code like currency.curr-code
...
.
define  temp-table tt-grp-sum no-undo
  field obj-type  like clients.obj-type
  field obj-code  like clients.obj-code
...
define variable v-grp-code0  as integer no-undo .
define variable v-grp-code   as integer no-undo .
define variable v-upr-code   as integer no-undo .
define variable v-prt-root   as integer no-undo .
define variable v-upr-stack  as character no-undo .
define variable v-def-list   as character no-undo .
define variable v-gds-name   as character no-undo .
define variable v-grp-name   as character no-undo .
define variable v-obj-name   as character no-undo .
define variable v-level      as integer no-undo .
define variable v-level5     as integer no-undo .
define variable v-has-up-lvl as logical no-undo .
...
define output parameter p-tot-r-b    as decimal no-undo .
define variable v-tot-r-b as decimal no-undo .
    ...
    when 1 then do:
      FOR EACH buf-chk-doc NO-LOCK WHERE
        ...
        if not v-skip-line then do :
          v-is-sub-count = (  lookup(string(buf-chk-doc.chk-type), '14,15,16,36,17,8,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) > 0  ).
...
END.
  ...
  assign
    ...
    all-days_sum.tot-r-b = (if v-curr-r-b = 'rubl':U then acc-day-rubl
                                                        else acc-day-base)
...
break by tt-grp-sum.obj-code
  ...
  assign
    ...
    sale-price-type = if v-curr-r-b = 'base':U then
       base-type
                                                  else "рублей"
    ObjAmount = 0
  .
...
do:
  if length(str4) > 115 then do:
    v-str4 = entry(1, str4, chr(10)).
    do v-count4 = 2 to num-entries(str4, chr(10)) while length(v-str4) < 115:
      v-str4 = v-str4 + chr(10) + entry(v-count4, str4, chr(10)).
    end.
    v-str4 = v-str4 + chr(10) + "...".
  end.
  else v-str4 = str4.
  ...
  put stream OutStr-html unformatted
    ...
    '    <th>Сумма в ' (if v-curr-r-b = 'base':U then 'Б.Вал.' else 'рублях') '</th>' skip
    '    <th>~% от суммы</th>' skip
