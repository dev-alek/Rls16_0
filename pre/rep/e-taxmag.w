...
FIELD SLT-pc like doc-line.SLT-pc
FIELD SLT-r-b like inkas.netto
FIELD SLT-r-b-brutto like inkas.netto
FIELD VAT-pc like doc-line.VAT-pc
...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-8 RECT-7 Classify RS-Method RS-By T-neg WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-8 RECT-7 Classify RS-Method RS-By T-neg WITH FRAME F-Main.
...
END.
ON VALUE-CHANGED OF Tog-level IN FRAME F-Main
DO:
  assign tog-level.
  if tog-level = true then do:
    display var-level with frame F-Main .
    enable  var-level with frame F-Main .
  end.
  else do:
    display var-level with frame F-Main .
    disable var-level with frame F-Main .
  end.
END.
...
find first goods where goods.prod-type = doc-line.prod-type and
  ...
  if not available tt-allsum-line then do:
   ...
   assign
   varsum-dsc-r-b-acc   = (if v-curr-r-b = 'rubl':U then tt-allsum-line.sum-dsc-rubl-acc else tt-allsum-line.sum-dsc-base-acc)
   varvat-r-b-acc       = (if v-curr-r-b = 'rubl':U then tt-allsum-line.vat-rubl-acc else tt-allsum-line.vat-base-acc)
   var-qnty                = doc-line.fact-qnty
   varvat-r-b-doc       = (if v-curr-r-b = 'rubl':U then tt-allsum-line.vat-rubl-doc else tt-allsum-line.vat-base-doc)
   varslt-r-b-doc       = (if v-curr-r-b = 'rubl':U then tt-allsum-line.slt-rubl-doc else tt-allsum-line.slt-rubl-doc)
  ...
  FOR EACH gds-dtl WHERE
    ...
    assign
    cur-discnt = (if v-curr-r-b = 'rubl':U then gds-dtl.discnt-rubl else gds-dtl.discnt-base)
    .
    CASE method :
...
price-rubl-without-tax-sale = price-rubl-with-tax-sale - vat-rubl-sale - slt-rubl-sale - road-tax-rubl-sale.
    ...
    slt-calc = (if v-curr-r-b = 'rubl':U then slt-rubl-sale else slt-base-sale) * cur-quant
    vat-cost = (if v-curr-r-b = 'rubl':U then vat-rubl-sale else vat-base-sale) * cur-quant
    ...
    sj-goods.brutto-sum = sj-goods.brutto-sum + ( cur-quant * (if v-curr-r-b = 'rubl':U then gds-dtl.price-rubl else gds-dtl.price-base))
...
PROCEDURE My-var :
Assign
frame F-Main classify
frame F-Main RS-Method
frame F-Main T-neg
frame F-Main T-partsneg
frame F-Main RS-by
method = RS-method
negparts = t-partsneg
.
if classify = "n-level":u then do:
  assign
  frame F-Main tog-level
  frame F-Main var-level
  .
end.
Assign
STR-obj-type = ''
...
ReportHeader =  "Источник формирования: " +
                radio-label(string(RS-BY), RS-BY:radio-buttons) + chr(10) +
                "Детализация: " +
                           radio-label(string(RS-method), RS-method:radio-buttons) + chr(10) +
                (if T-neg then t-neg:label else "") + chr(10) +
...
if v-curr-r-b = 'base':U then do:
  ...
  assign
  v-header-base-curr = string( "( Вал. продаж - РУБЛИ )" )
  .
end.
assign
...
sheetf.sizes = "6,9,16," + (if method = "artic":u
                               then "25,15,"
                               else "40,") +
               "3,12,"  +
               "13,13,13," +
...
clients.obj-name column-label "(Производитель)"                             format "x(15)"
...
FOR EACH sj-goods use-index p2 BREAK BY sj-goods.is-out DESCENDING :
  ...
  IF method = "b-code":U THEN do:
    ...
    ACCUM SUB-COUNT BY sj-goods.is-out sj-goods.b-code CHR(9)
    sj-goods.b-code CHR(9)
    sj-goods.artic  CHR(9)
    sj-goods.name CHR(9)
    sj-goods.unit CHR(9)
    sj-goods.qnty CHR(9)
    sj-goods.uchet-with-vat-sum CHR(9)
    sj-goods.uchet-sum CHR(9)
    sj-goods.netto-sum CHR(9)
    sj-goods.n-u-sum CHR(9)
    sj-goods.u-pcnt CHR(9)
  ...
  if last-of( sj-goods.is-out ) then do:
    ...
    (IF method = "TOTALS":U
    THEN ""
    ELSE string(ACCUM SUB-COUNT BY sj-goods.is-out sj-goods.b-code, ">>>>9")) CHR(9)
    (IF method = "TOTALS":U
    THEN ""
    ELSE " наименований" ) CHR(9)
    string( "Итого " + ( if sj-goods.is-out then "продажи" else "возвраты" ) ) CHR(9)
    (if method = "artic" then CHR(9) else '')
    CHR(9)
    ACCUM SUB-TOTAL BY sj-goods.is-out sj-goods.qnty CHR(9)
    ACCUM SUB-TOTAL BY sj-goods.is-out sj-goods.uchet-with-vat-sum  CHR(9)
    ACCUM SUB-TOTAL BY sj-goods.is-out sj-goods.uchet-sum  CHR(9)
    ACCUM SUB-TOTAL BY sj-goods.is-out sj-goods.netto-sum  CHR(9)
    ACCUM SUB-TOTAL BY sj-goods.is-out sj-goods.n-u-sum CHR(9)
    CHR(9)
    CHR(9)
...
if classify <> "no-grp-totals" then do:
  ...
  do while true:
    ...
    assign
    s1 = substring(sj-grp.serv-name, 1, 16)
    s2 = (if length(sj-grp.serv-name) > 16
          then substring(sj-grp.serv-name, 16 + 1, 25)
          else '')
    s3 = (if length(sj-grp.serv-name) > (16 + 25)
          then substring(sj-grp.serv-name, 16 + 25 + 1, 15)
          else '')
    .
    DISPLAY  STREAM PrnLibStream
    sym1
...
(ACCUM TOTAL sj-goods.uchet-with-vat-sum)  CHR(9)
(ACCUM TOTAL sj-goods.uchet-sum)  CHR(9)
(ACCUM TOTAL sj-goods.netto-sum ) CHR(9)
(ACCUM TOTAL sj-goods.n-u-sum  ) CHR(9)
CHR(9)
CHR(9)
 ...
 :
  ...
  .
   IF LAST-OF(d-slt-vat.vat-pc) then do:
      ...
      if Make-Excel then  put   stream ForExcel unformatted
      "Итого по НДС "
       d-slt-vat.vat-pc  CHR(9)
         ( ACCUM TOTAL by d-slt-vat.vat-pc d-slt-vat.slt-r-b-brutto )   CHR(9)
       ( ACCUM TOTAL  by d-slt-vat.vat-pc d-slt-vat.uchet-with-vat-sum )  CHR(9)
       ( ACCUM TOTAL  by d-slt-vat.vat-pc d-slt-vat.uchet-sum )   CHR(9)
       ( ACCUM TOTAL  by d-slt-vat.vat-pc d-slt-vat.VAT-r-b )    CHR(9)
       (ACCUM TOTAL  by d-slt-vat.vat-pc d-slt-vat.n-u-sum) / (ACCUM TOTAL  by d-slt-vat.vat-pc d-slt-vat.uchet-sum) * 100    CHR(9)
...
SKIP.
if Make-Excel then  put   stream ForExcel unformatted
"Итого по налогам          "  CHR(9)
string(  ( ACCUM TOTAL d-slt-vat.slt-r-b-brutto ) , "->>>,>>>,>>9.99" )  CHR(9)
string(  ( ACCUM TOTAL d-slt-vat.uchet-with-vat-sum ) , "->>>,>>>,>>9.99" )   CHR(9)
string(  ( ACCUM TOTAL d-slt-vat.uchet-sum ) , "->>>,>>>,>>9.99" )  CHR(9)
string(  ( ACCUM TOTAL d-slt-vat.VAT-r-b ) , "->>>,>>>,>>9.99" )   CHR(9)
string( (ACCUM TOTAL d-slt-vat.n-u-sum) / (ACCUM TOTAL d-slt-vat.uchet-sum) * 100 , "->>>,>>9.99" )   CHR(9)
...
if classify <> "no-grp-totals" then do:
  ...
  do while true:
    ...
    if Make-Excel then  put   stream ForExcel unformatted
    sj-grp.serv-name CHR(9)
    sj-grp.qnty CHR(9)
    sj-grp.uchet-with-vat-sum CHR(9)
    sj-grp.uchet-sum CHR(9)
    sj-grp.netto-sum CHR(9)
    sj-grp.n-u-sum CHR(9)
    sj-grp.u-pcnt CHR(9)
    sj-grp.vat-r-b CHR(9)
    skip.
  end.
  if Make-Excel then  put   stream ForExcel unformatted
  "ИТОГО" CHR(9)
