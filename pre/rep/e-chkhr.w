...
define variable vss-description as character no-undo init "Почасовая статистика розничных продаж по КОЛИЧЕСТВУ ПОКУПОК".
...
DEFINE NEW SHARED temp-table chk-h no-undo
...
DEFINE NEW SHARED temp-table num-h no-undo
    field obj-code like ub.clients.obj-code
    field hour  as integer extent 24
    field qnty  as integer
...
DEFINE FRAME F-Main
     T-dis-card AT ROW 1.21 COL 49.75
     RS-dis-card AT ROW 2.88 COL 49.88 NO-LABEL
     BR-dis-card-type AT ROW 5.63 COL 49.13
     RS-OPTION AT ROW 12.71 COL 2 NO-LABEL
...
PROCEDURE adm-disable :
    DISABLE T-dis-card BR-dis-card-type RS-OPTION WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN T-dis-card BR-dis-card-type RS-OPTION WITH FRAME F-Main.
...
END PROCEDURE.
PROCEDURE enable_UI :
  DISPLAY T-dis-card RS-dis-card RS-OPTION
      WITH FRAME F-Main.
  ENABLE T-dis-card BR-dis-card-type RS-OPTION
      WITH FRAME F-Main.
  OPEN QUERY BR-dis-card-type FOR EACH temp-dis-card-type NO-LOCK.
END PROCEDURE.
...
DEFINE INPUT PARAMETER myp-state as character no-undo.
    CASE myp-state:
        WHEN 'все':U then do:
                ENABLE RS-OPTION with frame F-Main.
        END.
        WHEN "Текущий" then do:
                assign
                 RS-OPTION = 1.
                 Display RS-OPTION with frame F-Main.
                 DISABLE RS-OPTION with frame F-Main.
        END.
        WHEN "Выборочно" then do:
            if can-find(FIRST obj-list) then ENABLE RS-OPTION WITH FRAME F-Main.
        END.
    END CASE.
END PROCEDURE.
...
define variable for-ch-str as character no-undo.
define variable glog as logical no-undo .
...
glog = ?.
if num-cols > 10 AND num-cols < 14  and NOT vXL
then do:
...
then do:
  ...
  do ii = 1 to 24:
    for-h-str = for-h-str + string(for-h[ii]) + chr(44).
  end.
  for-h-str = right-trim(for-h-str, chr(44)).
  run rep/r-chkhr.p (
                input for-h-str
                ,input RS-option
                ,input T-dis-card
...
if can-find( first chk-h ) then do:
  IF vXL THEN DO:
    run rep/dxchk-h2.p ( input my-handle
                  , input p-xl-delim
                  , input X-date-start
                  , input X-date-end
                  , input str4
  ...
  END.
  ELSE DO:
    run rep/d-chk-h2.p ( input my-handle
                  , input X-date-start
                  , input X-date-end
                  , input str4
                  , input rs-option
...
End.
ReportNAme = "Почасовая статистика розничных продаж по КОЛИЧЕСТВУ ПОКУПОК".
