...
define variable vss-workfile    as character no-undo init "$Workfile: g-fincl3.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/g-fincl3.p $":U .
...
run rep/d-report.w
    ...
    , input "Форма №3 взаиморасчет с контрагентами":U
    , input 2
    , input ""
    , input v-object-select
    , input ""
    , input "1,2"
    , input "all,1" + ',' + substitute( "parent-handle=&1" , this-procedure )
    , input no
    ).
