...
DEFINE VARIABLE COMBO-crit AS CHARACTER
     ...
     RADIO-BUTTONS
 "Макс. количество", "Макс. количество",
 "Макс. сумма продаж в учетных ценах", "Макс. сумма продаж в учетных ценах",
 "Макс. сумма продаж в ценах док-та", "Макс. сумма продаж в ценах док-та",
 "Макс. эффективность", "Макс. эффективность",
 "Макс. количество по кассе", "Макс. количество по кассе",
 "Макс. сумма по кассе в ценах док-та", "Макс. сумма по кассе в ценах док-та"
...
PROCEDURE adm-disable :
    DISABLE RECT-11 RECT-12 COMBO-crit COMBO-Sort RECT-10 Tog-Scale Sc_Name BSAmount FILL-IN-1 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-11 RECT-12 COMBO-crit COMBO-Sort RECT-10 Tog-Scale Sc_Name BSAmount FILL-IN-1 WITH FRAME F-Main.
...
DO:
    ...
    do:
             ...
             if scale_recid = ? then
             do:
                  Scale:screen-value = 'все':U .
                  Sc_Name:screen-value IN FRAME F-Main = 'все':U .
                  Sc-node = 0.
                  Sc-Upper = 0.
             end.
             else
             do:
                  ...
                  Sc_Name:screen-value IN FRAME F-Main = gds-prt.node-name.
 ...
 aSSIGN Sc_Name.
END.
ON VALUE-CHANGED OF Tog-Scale IN FRAME F-Main
DO:
Assign Tog-scale.
IF Tog-scale = TRUE then DO:
   Enable Scale with frame F-Main .
  End.
  Else DO:
   Disable Scale   with frame F-Main .
   Display Scale  with frame F-Main .
  End.
END.
...
def var Ret# as logical no-undo .
  ...
  ASSIGN
   BSAmount:screen-value IN FRAME F-Main = "10"
   COMBO-crit:screen-value IN FRAME F-Main = "Макс. количество по кассе"
   COMBO-Sort:screen-value IN FRAME F-Main = "Макс. количество по кассе"
   FILL-IN-1:screen-value IN FRAME F-Main  = "Шкалы:"
   Scale:screen-value IN FRAME F-Main      = 'все':U
   Sc_Name:screen-value IN FRAME F-Main    = 'все':U
  .
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
   ...
   if not g#log then do:
      Ret# = COMBO-crit:Disable ("Макс. сумма продаж в учетных ценах") .
      Ret# = COMBO-crit:Disable ("Макс. эффективность") .
      Ret# = COMBO-sort:Disable ("Макс. сумма продаж в учетных ценах") .
      Ret# = COMBO-sort:Disable ("Макс. эффективность") .
   end.
   ENABLE BSAmount COMBO-crit COMBO-Sort FILL-IN-1 Scale  with frame F-Main .
   DISPLAY BSAmount COMBO-crit COMBO-Sort FILL-IN-1 Scale  with frame F-Main .
END PROCEDURE.
PROCEDURE my-report :
if BSAmount = ? or
   BSAmount = 0 then do:
  message "Задайте поле 'Сколько показать товаров' на второй закладке." view-as alert-box error.
  apply "entry" to BSAmount in frame F-Main.
  return.
end.
...
ReportNAme = "Б Е С Т С Е Л Л Е Р Ы ".
            Case COMBO-crit:
                when "Макс. количество"                   then   crit = 1.
                when "Макс. сумма продаж в учетных ценах"                   then   crit = 2.
                when "Макс. сумма продаж в ценах док-та"                   then   crit = 3.
                when "Макс. эффективность"                   then   crit = 4.
                when "Макс. количество по кассе"                   then   crit = 5.
                when "Макс. сумма по кассе в ценах док-та"                   then   crit = 6.
            End case.
            Case COMBO-Sort:
                when "Макс. количество"                   then   Sort = 1.
                when "Макс. сумма продаж в учетных ценах"                   then   Sort = 2.
                when "Макс. сумма продаж в ценах док-та"                   then   Sort = 3.
                when "Макс. эффективность"                   then   Sort = 4.
                when "Макс. количество по кассе"                   then   Sort = 5.
                when "Макс. сумма по кассе в ценах док-та"                   then   Sort = 6.
            End case.
