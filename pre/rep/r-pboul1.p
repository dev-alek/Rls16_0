...
if session :set-wait-state( "compiler" ) then.
  ...
  find last buf_stk-tot no-lock
    where buf_stk-tot.obj-type   = obj-list.obj-type
      ...
      and buf_stk-tot.sum-type   = 'cost':U
  ...
  for each buf_trn-doc no-lock
    ...
    if buf_trn-doc.internal = yes or
       ...
       buf_trn-doc.ext-doc-type = 'ie':U or
       buf_trn-doc.ext-doc-type = 'we':U then next .
...
End.
   if v-button-stop then  DO:
        ...
        .
        if buf_doc-line.ext-doc-type = 'rs':U or buf_doc-line.ext-doc-type = 're':U or buf_doc-line.ext-doc-type = 'vt':U or buf_doc-line.ext-doc-type = 'vp':U then do:
          assign
  ...
  for each temp-goods
    ...
    run macr_excel_sum (temp-goods.p-val,              v-row, 10,   3) .
    run macr_excel_sum ( temp-goods.p-sum-zak,          v-row, 11,  2) .
    run macr_excel_sum ( temp-goods.p-prod,             v-row, 12,  2) .
    run macr_excel_sum ( temp-goods.p-nds,              v-row, 13,  2) .
    run macr_excel_sum ( temp-goods.p-np,               v-row, 14,  2) .
    run macr_excel_sum ( temp-goods.p-prod-nal,         v-row, 15,  2) .
    run macr_excel_sum ( temp-goods.p-val,              v-row, 16,  3) .
    run macr_excel_sum ( temp-goods.p-sum-prod,         v-row, 17,  2) .
    run macr_excel_sum ( temp-goods.p-sum-nds,          v-row, 18,  2) .
    run macr_excel_sum ( temp-goods.p-sum-prod-nds,     v-row, 19,  2) .
    run macr_excel_sum ( temp-goods.p-sum-np,           v-row, 20,  2) .
    run macr_excel_sum ( temp-goods.p-sum-prod-nal,     v-row, 21,  2) .
    run macr_excel_sum ( temp-goods.p-nazen,            v-row, 22,  2) .
    assign v-row = v-row + 1 .
  end.
  run macr_excel_char( "Итого за интервал:",  v-row, 5) .
  run macr_excel_sum ( sum-val        ,  v-row, 10,   3) .
  run macr_excel_sum ( sum-zak        ,  v-row, 11,  2) .
  run macr_excel_sum ( sum-val        ,  v-row, 16,  3) .
  run macr_excel_sum ( sum-prod       ,  v-row, 17,  2) .
  run macr_excel_sum ( sum-nds        ,  v-row, 18,  2) .
  run macr_excel_sum ( sum-prod-nds   ,  v-row, 19,  2) .
  run macr_excel_sum ( sum-np         ,  v-row, 20,  2) .
  run macr_excel_sum ( sum-prod-nal   ,  v-row, 21,  2) .
  run macr_excel_sum ( sum-nazen      ,  v-row, 22,  2) .
  assign v-row = v-row + 1 .
  find last buf_stk-tot no-lock
    where buf_stk-tot.obj-type   = obj-list.obj-type
      and buf_stk-tot.obj-code   = obj-list.obj-code
...
procedure PutColumnTitulExcel :
  ...
  run macr_excel_char("Стоимостные показатели реализованных товаров, выполненных работ, оказанных услуг по единице (руб.)", 3, 12) .
  ...
  run macr_cell_size (4,?, v-row, v-col,?,?).       assign v-col = v-col + 1 .
  run macr_excel_char("Стоимость единицы товара (работы, услуги) (руб.)", v-row, v-col) .
  run macr_cell_size (12,?, v-row, v-col,?,?).      assign v-col = v-col + 1 .
  run macr_excel_char("Количество", v-row, v-col) .
  run macr_cell_size (12,?, v-row, v-col,?,?).      assign v-col = v-col + 1 .
  run macr_excel_char("Общая стоимость готовой продукции без НДС и НП из таб.№1-5А (руб.) ", v-row, v-col) .
  run macr_cell_size (12,?, v-row, v-col,?,?).      assign v-col = v-col + 1 .
  run macr_excel_char("Количество", v-row, v-col) .
  run macr_cell_size (12,?, v-row, v-col,?,?).      assign v-col = v-col + 1 .
