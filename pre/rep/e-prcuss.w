...
PROCEDURE adm-disable :
    DISABLE RECT-6 SortType bgrcliset r-gop-2 bglclilist WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 SortType bgrcliset r-gop-2 bglclilist WITH FRAME F-Main.
...
DO:
  ...
  case bgrcliset :
    when 1 then do :
      enable r-gop-2 with frame F-Main.
      disable r-gop-1 with frame F-Main.
    end.
    when 2 then do :
      enable r-gop-1 with frame F-Main.
      disable r-gop-2 with frame F-Main.
    end.
  end case.
END.
...
DO:
  ...
  repeat ix = 1 to num-entries (s-ref-rec) :
    for each clients where recid(clients) = int (entry (ix, s-ref-rec) ) no-lock :
      ...
      .
      assign
        bglclilist =  bglclilist + chr(10) + "   " + clients.obj-name
      .
    end.
  end.
  display
...
DO:
  ...
  repeat ix = 1 to num-entries (s-ref-rec) :
    for buyer-group where recid (buyer-group) = int (entry (ix, s-ref-rec) ) :
      ...
      .
      assign
        bglclilist = bglclilist + chr(10) + "   " + buyer-group.name
      .
    end.
  end.
  display
