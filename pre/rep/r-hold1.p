...
assign v-account = ( if integer( 25 ) = 0 then 100 else integer( 25 ) ).
  ...
  for each buf_hold-time no-lock where buf_hold-time.cat-code = 1 :
...
output stream OutStream to value( string( session:temp-directory +
  ...
  FORM HEADER
      ...
      with FRAME BottomFrame width 290  PAGE-BOTTOM NO-LABELS NO-BOX .
