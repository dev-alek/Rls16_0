...
run rep/tickets.w (input parparentproc, input p-obj-type, input p-obj-code, input Action, input DocType ).
if TicketName = "" then
    RETURN.
...
define variable bg as widget-handle no-undo .
  do
  on error undo, return error return-value
  :
      ...
      when "main" then do:
        ...
        :
          ...
          find first ub.units no-lock
            ...
            .
          if lookup('2ед':U, units.type) > 0 then do:
            assign nakl-qnty = ub.doc-line.cli-qnty .
          end.
          else do:
            assign nakl-qnty = ( if can-do( 'факт':U, ub.trn-doc.status_ ) then ub.gds-dtl.fact-qnty else ub.gds-dtl.doc-qnty ) .
          end.
          assign
          ...
          if not available ub.bar-code then
            ...
            do:
              find first ub.units no-lock
                where ub.units.unit-name = ub.bar-code.unit-cli
                .
              if ub.units.type = '2ед':U then do:
                assign
                ...
                assign
                  nakl-qnty = ( if can-do( 'факт':U, ub.trn-doc.status_ ) then ub.parts.fact-qnty else parts.qnty )
                .
              end.
              assign
        ...
        if TickOnS then do:
          ...
          :
            ...
            if qntytype = "документ" then do:
              assign
              nakl-qnty = ( if can-do( 'факт':U, ub.trn-doc.status_ ) then ub.gds-dtl.fact-qnty else ub.gds-dtl.doc-qnty ) .
            end.
run rep/ticket.p (
               buffer ub.goods
              ,buffer ub.bar-code
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  (input  b-price-list.obj-type
  ,input  b-price-list.obj-code
  ,input  b-price-list.b-code
  ,input  0
  ,input  b-price-list.fact-order
  ,output cur-dn
  ,output pr-doc-rb-old
  ,output cur-rt
  ,output cur-ex
  ) no-error .
      assign
...
run rep/ticket.p (
      ...
      when "part" then do:
        ...
        :
            ...
            then do:
              assign
                nakl-qnty = ( if can-do( 'акт':U, ub.price-doc.status_ ) then ub.parts.fact-qnty else ub.parts.qnty )
                v-part-code = ub.parts.part-code
              .
run rep/ticket.p (
               buffer ub.goods
