...
define variable vss-workfile    as character no-undo init "$Workfile: akt-sug-corr.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/akt-sug-corr.p $":U .
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_trn-doc.doc-code ,
  ...
  run doc-attr-write(INPUT buf_trn-doc.doc-code,INPUT 'ptbobj':U,OUTPUT v-attr-value) no-error .
...
run doc-attr-write(INPUT bf_trn-doc.doc-code,INPUT 'car-num':U,OUTPUT v-car-num) no-error .
  ...
  do:
    ...
    do iNum = 1 to v-InfoSectionsTotal:SectionNum:
      assign
        ...
        v-fact-qnty-after  = 0
      .
      for first buf_rvs-doc no-lock where buf_rvs-doc.out-code = v-doc-code and buf_rvs-doc.rvs-type = 'перед_док':U,
      ...
      for first buf_rvs-doc no-lock where buf_rvs-doc.out-code = v-doc-code and buf_rvs-doc.rvs-type = 'после_док':U,
        each buf_rvs-line no-lock where buf_rvs-line.rvs-code = buf_rvs-doc.rvs-code:
