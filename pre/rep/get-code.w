...
DO:
  if input frame Dialog-Frame varcst-code = "" or
     input frame Dialog-Frame varcst-code = ? then do:
        message "Введите номер ГТД." view-as alert-box error.
        apply "entry" to varcst-code in frame Dialog-Frame.
        return no-apply.
  end.
  if input frame Dialog-Frame varDate = ? THEN do:
     message "Введите дату на которую будем снимать отчет." view-as alert-box error.
     apply "entry" to vardate in frame Dialog-Frame.
     return no-apply.
  end.
  assign pardate     = input frame Dialog-Frame vardate
         parcst-code = input frame Dialog-Frame varcst-code
         parcst-unit = input frame Dialog-Frame varcst-unit
         is-ok = yes NO-ERROR.
END.
ON return OF varcst-code IN FRAME Dialog-Frame
DO:
  if input frame Dialog-Frame varcst-code <> "" and
     input frame Dialog-Frame varcst-code <> ? then apply "entry" to varDate in frame Dialog-Frame.
  else message "Введите номер ГТД." view-as alert-box error.
  return no-apply.
END.
ON return OF varcst-unit IN FRAME Dialog-Frame
DO:
  apply "entry" to b-ok in frame Dialog-Frame.
  return no-apply.
END.
ON return OF varDate IN FRAME Dialog-Frame
DO:
  if input frame Dialog-Frame varDate <> ? THEN apply "entry" to varcst-unit in frame Dialog-Frame.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  RUN enable_UI.
  WAIT-FOR GO OF FRAME Dialog-Frame FOCUS varcst-code.
END.
RUN disable_UI.
