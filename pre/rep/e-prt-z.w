...
PROCEDURE adm-disable :
    DISABLE RECT-12 Classify Itog Zero-ost Zero tog-obj rs-prizn txt-prizn WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-12 Classify Itog Zero-ost Zero tog-obj rs-prizn txt-prizn WITH FRAME F-Main.
...
If x-date-alone <> v-today Then do:
      if x-SelectGood = 1 Then DO:
          If x-SET_val_TYPE = 1
            then DO :
                run rep/r-prt-z5.p
                   (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input zero,  input zero-ost,  input Itog ,   input tog-obj ,   input table tt-season ,   input v-prizn  ) .
                End.
            Else do:
                run rep/r-prt-z6.p
                    (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input zero,  input zero-ost,  input Itog ,   input tog-obj ,   input table tt-season ,   input v-prizn  ) .
