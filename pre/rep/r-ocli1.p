...
def   shared  var discnt            as   decimal EXTENT 10 Format "->>>>>>>>>>>9.<<<" no-undo.
...
def   shared   var F-discnt          as   char  no-undo.
...
PROCEDURE display-line :
  i = i + 1.
If Integer(200) = 0 Then Temp1  = 100  .
                      Else Temp1 = Integer(200) .
...
define input parameter N as integer no-undo .
define input parameter Name as char no-undo .
    PUT stream  OutStream  UNFORMATTED    "Итого по : " + trim(Name) AT ((N - 1) * 10 ) format "x(100)"  chr(10).
    if Make-Excel then  put   stream ForExcel unformatted  "Итого по : " +  trim(Name) chr(10).
 END PROCEDURE.
