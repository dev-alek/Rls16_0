...
PROCEDURE adm-disable :
    DISABLE RECT-6 RECT-7 RS-Method T-neg WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-6 RECT-7 RS-Method T-neg WITH FRAME F-Main.
...
DO WHILE available t-doc :
    if NOT (t-doc.fact-date >= X-date-start and t-doc.fact-date <= X-date-end AND
           ...
           (t-doc.doc-type = 'рас':U OR t-doc.doc-type = 'возврат':U))
    then do:
        GET NEXT br-docs.
        next.
    end.
    ...
    IF objects  = 0 then
    FIND FIRST ub.sysconf where ub.sysconf.host-code = t-doc.host-code No-LOCK.
    if t-doc.cli-code = ub.sysconf.sale-code and t-doc.cli-type = 'орг':U then do:
        if t-doc.doc-date > X-date-end OR t-doc.doc-date < X-date-start then do:
        end.
    end.
    assign
    doc-num = t-doc.doc-code
    is-out = if t-doc.doc-type = 'рас':U then 1 else -1
    offc = t-doc.office
    .
