...
PROCEDURE adm-disable :
    DISABLE RECT-13 f-cashbook B-cashbook p-titul l-cashbook WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-13 f-cashbook B-cashbook p-titul l-cashbook WITH FRAME F-Main.
...
DO:
  ...
  then do :
    do ii = 1 to num-entries (v-cb-brw:out-list-id, chr(6)):
    find first ub.cashbook no-lock where ub.cashbook.id = int64(entry(ii,v-cb-brw:out-list-id,chr(6))) no-error.
    if ii = 1 then
    assign
    ...
    assign
    f-cashbook = f-cashbook + chr(6) + ub.CashBook.CashBookName
    f-cashbook_id = f-cashbook_id + chr(6) + string(ub.CashBook.id)
    .
  end.
    display
    f-cashbook
...
END.
ON VALUE-CHANGED OF p-titul IN FRAME F-Main
DO:
  assign p-titul .
END.
...
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main f-cashbook
.
  assign
