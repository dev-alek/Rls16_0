...
define variable v-PropisSumall as character no-undo.
...
procedure write-cells:
    ...
    find first bf_clients
        ...
        no-lock.
    run acmxl-write-cell-data("h_obj_name":U, bf_clients.obj-name).
    run acmxl-write-cell-data("h_obj_name_2":U, bf_clients.obj-name).
    v-host-code = bf_clients.host-code.
    find first bf_clients
        where bf_clients.obj-code = v-host-code
        and bf_clients.obj-type = 'орг':U
        no-lock.
    run acmxl-write-cell-data("h_firm_name":U, bf_clients.obj-name).
    run acmxl-write-cell-data("h_doc_code":U,  bf_trn-doc.doc-code).
    run acmxl-write-cell-data("h_doc_date":U, string(bf_trn-doc.doc-date, "99/99/99")).
    if bf_trn-doc.wrkr <> ? then do:
        find first bf_clients
            where bf_clients.obj-code = bf_trn-doc.wrkr
            and bf_clients.obj-type = 'чел':U
            no-lock.
        run acmxl-write-cell-data("h_mgr_name":U, bf_clients.obj-name).
    end.
    if bf_trn-doc.agnt <> ? then do:
        find first bf_clients
            ...
            and bf_clients.obj-type = 'чел':U
            no-lock.
        run acmxl-write-cell-data("h_performer_name":U, bf_clients.obj-name).
    end.
    if bf_trn-doc.boss <> ? then do:
        find first bf_clients
            ...
            and bf_clients.obj-type = 'чел':U
            no-lock.
        run acmxl-write-cell-data("h_stock_name":U, bf_clients.obj-name).
    end.
    v-doc-sum = 0 - v-doc-sum.
    run acmxl-write-cell-data("h_sum_all":U, v-doc-sum).
    run rep/wp-rub.p(input v-doc-sum, output v-PropisSumall, output v-abbr).
    run acmxl-write-cell-data("h_sum_str":U, v-PropisSumall).
    v-doc-qnty = 0 - v-doc-qnty.
    run acmxl-write-cell-data("h_qnty_all":U, v-doc-qnty).
end.
procedure write-lines:
    ...
    for each bf_doc-line
        ...
        find first bf_doc-line-sum
            ...
            and bf_doc-line-sum.sum-type = 'gen':U
            no-lock.
        ...
        run acmxl-sheet1-write-line-data(
                ...
                if bf_doc-line.fact-qnty = 0 then 0
                                      ...
                                      else bf_doc-line-sum.crsa-sum-rubl / bf_doc-line.fact-qnty ,
                if no-vat then 0 - (bf_doc-line-sum.cost-sum-rubl - bf_doc-line-sum.cost-VAT-rubl)
                             ...
                             else 0 - (bf_doc-line-sum.crsa-sum-rubl),
                if avail bf_trn-reason then bf_trn-reason.reason-name else ""
            ).
    end.
end.
