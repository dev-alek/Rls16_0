...
procedure Proc-b-code:
  ...
  do:
    for each bad-chk no-lock:
      if length(bad-chk-str) + length(bad-chk.doc-code + chr(32) +
        "погрешн." +  chr(32) + string(bad-chk.delta) + chr(10)) > 31900 then
      do:
        bad-chk-str = bad-chk-str + " .....".
        leave.
      end.
      assign
        bad-chk-str = bad-chk-str + bad-chk.doc-code + chr(32) +
                  "погрешность" +  chr(32) + string(bad-chk.delta) + chr(10)
        .
    end.
    run gbl/d-prompt.w (
...
procedure Proc-b-code-excel:
  ...
  CASE my-set_val_type :
    when 2 then
      do:
        Report:table-columns("60,125,30,30,120,70,70").
        Report:table-types = "String,String,String,String,String,Qnty,Number".
        Report:table-header("Код|Вид топлива|Вид опл.|Вал|Метод платежа|Литры|Сумма","40","9").
      end.
    when 3 then
      do:
        Report:table-columns("60,125,30,30,120,70,70,70,70").
        Report:table-types = "String,String,String,String,String,Qnty,String,Number,Number".
        Report:table-header("Код|Вид топлива|Вид опл.|Вал|Метод платежа|Литры|Сумма в валюте|Сумма в Баз. валюте|Сумма в рублях","40","9").
      end.
  END CASE .
  ...
  FOR EACH benefits
    ...
    END.
  END.
  if v-mode = "nototal":U and my-set_val_type = 2  then
  do:
    Report:table-total(  ""
      + "|" +    "Всего продано топлива"
      + "|" +    ""
  ...
  if v-mode = "nototal":U and my-set_val_type = 3  then
  do:
    Report:table-total(  ""
      + "|" +    "Всего продано топлива"
      + "|" +    ""
...
PROCEDURE Proc-pay-code:
  ...
  form header
    Line format "X(114)" at 1 skip
    "Продолжение - на следующей странице" at 30 skip
    with frame BottomFrame1 width 232 PAGE-BOTTOM NO-LABELS NO-BOX .
...
procedure Proc-pay-code-excel:
  xml_tmp = string(session:temp-directory + "report-tmp3.xml").
  Report = new ReportXml(xml_tmp).
  Report:worksheet("Лист 1").
  Report:worksheet-header("start").
  ...
  case my-set_val_type:
    when 2 then
      do:
        Report:table-columns("40,40,145,80,160,90,104").
        Report:table-types = "String,String,String,String,String,Qnty,Number".
        Report:table-header("Вид опл.|Вал|Метод платежа|Код|Вид топлива|Литры|Сумма","40","9").
      end.
    when 3 then
      do:
        Report:table-columns("40,40,145,80,160,90,104,104").
        Report:table-types = "String,String,String,String,String,Qnty,Number,Number".
        Report:table-header("Вид опл.|Вал|Метод платежа|Код|Вид топлива|Литры|Сумма в валюте|Сумма в рублях","50","9").
      end.
  end case.
  for each benefits break by benefits.pay-code by benefits.b-code:
    accumulate
        ...
        when 2 then
          do:
            Report:table-row(
              ...
              + "|" + (if v-curr-r-b = 'base':U then string(benefits.tot-base) else string(benefits.tot-rubl))
              ).
          end.
        when 3 then
          do:
        ...
        when 2 then
          do:
            Report:table-subtotal(
              ...
              + "|" + string((if v-curr-r-b = 'base':U then
          ...
          when 2 then
            do:
              Report:table-row(
                ...
                + "|" + string(b-codes.qnty)
                + "|" + (if v-curr-r-b = 'base':U then string(b-codes.tot-base)
        ...
        when 2 then
          do:
            Report:table-total(
              ...
              + "|" + if v-curr-r-b = 'base':U then string(accum total benefits.tot-base)
              else string(accum total benefits.tot-rubl)
              ).
            v-mode = "yestotal":U.
          end.
