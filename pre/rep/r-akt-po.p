...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  define frame f-doc1
        ...
        gds-prop.zen1     column-label "Старая уч.!цена (РУБ)"     format "->>>>>>>9.99"
        sym6              column-label ":!:"                       format "X(1)"
        gds-prop.sum1     column-label "Сумма в учет.!ценах(РУБ)"     format "->>>>>>>>>>>9.99"
        sym7              column-label ":!:"                       format "X(1)"
        gds-prop.zen2     column-label "Новая уч.!цена (РУБ)"      format "->>>>>>>9.99"
        sym8              column-label ":!:"                       format "X(1)"
        gds-prop.sum2     column-label "Новая сумма в!уч. ценах(РУБ)" format "->>>>>>>>>>>9.99"
        sym9              column-label ":!:"                       format "X(1)"
        gds-prop.del      column-label "Сумма  !разницы (РУБ)"     format "->>>>>>>>9.99"
