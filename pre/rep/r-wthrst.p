...
DEFINE TEMP-TABLE tt-line NO-UNDO
      ...
      FIELD summ-6         as integer    format "->>>>>9"
...
:
   ...
   FOR each buf_tt-line
      ...
      put stream excel-line unformatted
                            "Template":U
            CHR(9)   "DTA":U
            CHR(9)   buf_tt-line.wth-name
            CHR(9)   buf_tt-line.wth-num
            CHR(9)   SUBSTITUTE ("&1 &2", buf_tt-line.obj-code, buf_tt-line.obj-type)
            CHR(9)   buf_tt-line.obj-name
            CHR(9)   buf_tt-line.summ-4   format "->>>>>>>>>>9"
            CHR(9)   buf_tt-line.summ-5   format "->>>>>>>>>>9.999"
            CHR(9)   buf_tt-line.summ-6   format "->>>>>>>>>>9"
...
on error undo, return error
:
   DO v-count = 1 TO NUM-ENTRIES(p-wth-recid-list, chr(44) )
   on error undo, next
   :
      find first buf_wth-par
           where buf_wth-par.wth-code = integer( ENTRY(1, entry( v-count, p-wth-recid-list, chr(44)), chr(4) ))
             and buf_wth-par.par-code = integer( ENTRY(2, entry( v-count, p-wth-recid-list, chr(44)), chr(4) ))
            no-lock
            no-error
            .
...
:
    ...
    then do:
        assign
            v-rec-amount =  num-entries( "summ4,summ5":U )
        .
        do v-counter = 1 to v-rec-amount
        :
            run hide-list-add-item in this-procedure ( input entry( v-counter, "summ4,summ5":U ) ).
        end.
    end.
    if p-put-zone = no
    then do:
        assign
            v-rec-amount =  num-entries( "summ6,summ7":U )
        .
        do v-counter = 1 to v-rec-amount
        :
            run hide-list-add-item in this-procedure ( input entry( v-counter, "summ6,summ7":U ) ).
        end.
    end.
    assign
        p-hide-list = "":U
