...
DO:
    return BuhRetFlag .
END.
ON CHOOSE OF PrintTitle IN FRAME DIALOG-1
DO:
    run gbl/prnfilen.w
      (input  "Печать оглавления"
      ,input  DisabledOptions
      ,input  string( session:temp-directory + "plt" + string( p-rep-name ) )
      ,input  7
...
DO:
    run gbl/prnfilen.w
      ...
      ,input  DisabledOptions
      ,input  string( session:temp-directory + "rpt" + string( p-rep-name ) )
      ,input  7
      ,output v-user-action
      ,output v-printed
      ) .
