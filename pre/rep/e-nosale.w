...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-nosale.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-11 RECT-10 RECT-12 COMBO-crit COMBO-Sort Classify Tog-Scale Sc_Name BSAmount Tog-Sale FILL-IN-1 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-11 RECT-10 RECT-12 COMBO-crit COMBO-Sort Classify Tog-Scale Sc_Name BSAmount Tog-Sale FILL-IN-1 WITH FRAME F-Main.
...
DO:
    ...
    do:
             ...
             do:
                  ...
                  Sc_Name:screen-value IN FRAME F-Main = ub.gds-prt.node-name.
...
classify:list-items    in frame F-Main = "Без классификации" + ',' + "Производители" + ',' + "Группы товаров" .
combo-crit:list-items  in frame F-Main = "Макс. количество" + ',' + "Макс. сумма в учетных ценах" + ',' + "Макс. сумма в прод. ценах" .
combo-sort:list-items  in frame F-Main = "по коду" + ',' + "по артикулу" + ',' + "по названию" .
scale:radio-buttons    in frame F-Main = "все" + ',' + 'все':U + ',' + "Выбор шкалы" + ',' +  "choice-scala":u  .
  assign
   combo-crit:screen-value in frame F-Main = "Макс. количество"
   combo-sort:screen-value in frame F-Main = "по коду"
   classify:screen-value in frame F-Main   = "Без классификации"
   fill-in-1:screen-value in frame F-Main  = "Шкалы:"
   scale:screen-value in frame F-Main      = 'все':U
   sc_name :screen-value in frame F-Main   = 'все':U
   tog-sale = true.
   enable  combo-crit combo-sort fill-in-1 scale classify tog-sale  with frame F-Main .
   display combo-crit combo-sort fill-in-1 scale  classify tog-sale  with frame F-Main .
   hide bsamount in frame F-Main .
END PROCEDURE.
PROCEDURE my-report :
  Case classify :
      when "Без классификации"  then
            run rep/r-nosale.p
        ...
        when "Производители"        then
            run rep/r-nosal3.p
            ( input v-cntxt-obj-code ,input v-cntxt-obj-type ,input base-type  ,input base-code  ,input v-cntxt-host-code-obj,input crit ,
              input Sort ,input xClassify,input BSAmount ,input Sc-node, input Sc-upper, input Tog-Scale,input Tog-Sale).
      when "Группы товаров"  then
              run rep/r-nosal2.p
              ( input v-cntxt-obj-code ,input v-cntxt-obj-type ,input base-type  ,input base-code  ,input v-cntxt-host-code-obj,input crit ,
                input Sort ,input xClassify,input BSAmount ,input Sc-node, input Sc-upper, input Tog-Scale,input Tog-Sale).
  end case.
...
ReportNAme = "<<Зависшие>> товары".
            Case COMBO-crit:
                ...
                when "Макс. сумма в учетных ценах"                   then   crit = 2.
                when "Макс. сумма в прод. ценах"                   then   crit = 3.
            End case.
            Case COMBO-Sort:
                when "по коду"               then   Sort = 1.
                when "по артикулу"               then   Sort = 2.
                when "по названию"               then   Sort = 3.
            End case.
            Case classify :
                when "Без классификации"  then xclassify = "no-classify":U .
                when "Производители"  then xclassify = "prod":U         .
                when "Группы товаров"  then xclassify = "grp-goods":U     .
                when "Производители/Группы товаров"  then xclassify = "prod/grp-goods":U .
                when "Группы товаров/Производители"  then xclassify = "grp-goods/prod":U  .
            end case.
