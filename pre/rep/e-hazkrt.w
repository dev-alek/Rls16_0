...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-hazkrt.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE fi-date-start fi-date-end fi-time-start fi-time-end WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN fi-date-start fi-date-end fi-time-start fi-time-end WITH FRAME F-Main.
...
ON LEAVE OF fi-date-end IN FRAME F-Main
DO:
  assign
      fi-date-end
  .
END.
ON LEAVE OF fi-date-start IN FRAME F-Main
DO:
  assign fi-date-start.
END.
ON LEAVE OF fi-time-end IN FRAME F-Main
DO:
...
END.
ON LEAVE OF fi-time-start IN FRAME F-Main
DO:
  DEFINE VARIABLE v-ok AS LOGICAL    NO-UNDO.
  run check-time-format in this-procedure ( input self :handle
                                          , output v-ok
...
on ' ' of fi-date-start in frame F-Main
do:
...
on delete-character of fi-date-start in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of fi-date-start in frame F-Main
do:
...
on ctrl-b of fi-date-start in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of fi-date-start in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of fi-date-start in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date10
    ...
    .
  if fi-date-start :POPUP-MENU in frame F-Main = ?
  then do:
    ASSIGN
      fi-date-start :POPUP-MENU in frame F-Main = MENU m-ed-date10 :HANDLE
      fi-date-start :MENU-MOUSE in frame F-Main = 3
    .
  end.
  define variable v-label-handle10 as handle no-undo .
  assign
    v-label-handle10 = fi-date-start :side-label-handle in frame F-Main
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date10-1 in menu m-ed-date10 DO:
    apply "ctrl-b":U to fi-date-start in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date10-2 in menu m-ed-date10 DO:
    apply "ctrl-d":U to fi-date-start in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date10-3 in menu m-ed-date10 DO:
    apply "ctrl-e":U to fi-date-start in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date10-4 in menu m-ed-date10 DO:
    apply "ctrl-f":U to fi-date-start in frame F-Main .
  END.
define variable vss-include-info11 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on ' ' of fi-date-end in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on delete-character of fi-date-end in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of fi-date-end in frame F-Main
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of fi-date-end in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of fi-date-end in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of fi-date-end in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date12
    ...
    .
  if fi-date-end :POPUP-MENU in frame F-Main = ?
  then do:
    ASSIGN
      fi-date-end :POPUP-MENU in frame F-Main = MENU m-ed-date12 :HANDLE
      fi-date-end :MENU-MOUSE in frame F-Main = 3
    .
  end.
  define variable v-label-handle12 as handle no-undo .
  assign
    v-label-handle12 = fi-date-end :side-label-handle in frame F-Main
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date12-1 in menu m-ed-date12 DO:
    apply "ctrl-b":U to fi-date-end in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date12-2 in menu m-ed-date12 DO:
    apply "ctrl-d":U to fi-date-end in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date12-3 in menu m-ed-date12 DO:
    apply "ctrl-e":U to fi-date-end in frame F-Main .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date12-4 in menu m-ed-date12 DO:
    apply "ctrl-f":U to fi-date-end in frame F-Main .
  END.
...
PROCEDURE my-var :
  assign
    ReportHeader =    "Диапазон:" + chr(10)
                   + "    " + "C: " +  string( fi-date-start , "99/99/9999" ) + " " + string( v-time-start-sec , "hh:mm" ) + chr(10)
                   + "    " + "По: " +  string( fi-date-end , "99/99/9999" ) + " " + string( v-time-end-sec , "hh:mm" ) + chr(10)
  .
END PROCEDURE.
