...
PROCEDURE adm-disable :
    DISABLE RECT-4 RECT-1 RECT-3 MainTypes SET_PAY_TYPE SortType No-Prt WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-4 RECT-1 RECT-3 MainTypes SET_PAY_TYPE SortType No-Prt WITH FRAME F-Main.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-e-glprcl-host-code
  )  .
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run basecode in g#library
  (input  v-e-glprcl-host-code
  ,output v-e-glprcl-base-code
  )  .
   if v-e-glprcl-base-code = 0 then
    stat = SET_PAY_TYPE:DISABLE("В валюте") in frame  F-Main .
END PROCEDURE.
PROCEDURE my-report :
    run rep/r-glprcl.p (
          input v-e-glprcl-store-type
