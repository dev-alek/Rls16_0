...
PROCEDURE adm-disable :
    DISABLE RADPost PostName sort-by-name show-rcv WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RADPost PostName sort-by-name show-rcv WITH FRAME F-Main.
...
assign frame F-Main PostName RADPost sort-by-name show-rcv .
END PROCEDURE.
PROCEDURE my-report :
run rep/r-ordrcv.p
    (
...
END PROCEDURE.
PROCEDURE my-var :
assign frame F-Main RADPost sort-by-name show-rcv .
if RADPost = 0 or RADPost = 1 then PostName = 'все':U.
  assign
    ReportHeader = "Отчет сформирован: " + string(today, "99/99/9999") + chr(10) +
    "Сортировать поставщиков: " + (if sort-by-name then "по имени" else "по коду") + chr(10) +
    (if show-rcv then "Показывать поставки" else "Не показывать поставки")         + chr(10) +
    "Поставщики: " + chr(10) + PostName
  .
END PROCEDURE.
