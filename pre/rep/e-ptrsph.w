...
define button b-save   label "&Сохранить"  size-chars 12.00 by 1.00 tooltip "Сохранить настройки" .
define button   Btn_UnMark  label "Сн&ять *"    size-chars 12.00 by 1.00 tooltip "Снять все отметки" .
define variable Tog-Excel as logical no-undo initial no label "Только в E&xcel" view-as toggle-box size-chars 40.00 by 1.00 fgcolor 4 .
define rectangle r-rect-1 edge-pixels 3 graphic-edge no-fill size-chars 91.00 by 1.50 .
define frame F-Main
  r-rect-1         at row  1.25 col  1.50
  b-save      at row  1.50 col  2.50
  b-mark      at row  1.50 col 26.50
    Btn_UnMark     at row  1.50 col 38.50
  use-column[  1 ] at row  3.00 col  2.50 label "&Дата продажи"        view-as toggle-box size-chars 40.00 by 1.00
  use-column[  2 ] at row  4.25 col  2.50 label "&Время продажи"       view-as toggle-box size-chars 40.00 by 1.00
  use-column[  3 ] at row  5.50 col  2.50 label "&Код товара"          view-as toggle-box size-chars 40.00 by 1.00
  use-column[  4 ] at row  6.75 col  2.50 label "&Артикул товара"      view-as toggle-box size-chars 40.00 by 1.00
  use-column[  5 ] at row  8.00 col  2.50 label "&Наименование товара" view-as toggle-box size-chars 40.00 by 1.00
  use-column[  6 ] at row  9.25 col  2.50 label "Номер &ТРК"           view-as toggle-box size-chars 40.00 by 1.00
  use-column[  7 ] at row 10.50 col  2.50 label "Номер &пистолета"     view-as toggle-box size-chars 40.00 by 1.00
  use-column[  8 ] at row  3.00 col 42.50 label "Количество &чеков"    view-as toggle-box size-chars 40.00 by 1.00
  use-column[  9 ] at row  4.25 col 42.50 label "Количество, &л"       view-as toggle-box size-chars 40.00 by 1.00
  use-column[ 10 ] at row  5.50 col 42.50 label "Сумма прода&ж"        view-as toggle-box size-chars 40.00 by 1.00
  use-column[ 11 ] at row  6.75 col 42.50 label "Вид &оплаты"          view-as toggle-box size-chars 40.00 by 1.00
  use-column[ 12 ] at row  8.00 col 42.50 label "Номер &чека"          view-as toggle-box size-chars 40.00 by 1.00
  use-column[ 13 ] at row  9.25 col 42.50 label "Сухой &чек"           view-as toggle-box size-chars 40.00 by 1.00
  use-column[ 14 ] at row 10.50 col 42.50 label "№ заказа"             view-as toggle-box size-chars 40.00 by 1.00
  use-column[ 15 ] at row 11.75 col  2.50 label "№ кассы"              view-as toggle-box size-chars 40.00 by 1.00
...
PROCEDURE adm-disable :
    DISABLE b-mark Btn_UnMark b-save use-column[ 1 ] use-column[ 2 ] use-column[ 3 ] use-column[ 4 ] use-column[ 5 ] use-column[ 6 ] use-column[ 7 ] use-column[ 8 ] use-column[ 9 ] use-column[ 10 ] use-column[ 11 ] use-column[ 12 ] use-column[ 13 ] use-column[ 14 ] use-column[ 15 ] WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN b-mark Btn_UnMark b-save use-column[ 1 ] use-column[ 2 ] use-column[ 3 ] use-column[ 4 ] use-column[ 5 ] use-column[ 6 ] use-column[ 7 ] use-column[ 8 ] use-column[ 9 ] use-column[ 10 ] use-column[ 11 ] use-column[ 12 ] use-column[ 13 ] use-column[ 14 ] use-column[ 15 ] WITH FRAME F-Main.
...
on choose of Btn_UnMark in frame F-Main
  do:
    assign
      use-column[  1 ] = no
      use-column[  2 ] = no
...
on F7, CTRL-S, CTRL-Ы of frame F-Main anywhere do:
  if b-save :sensitive then DO: apply "CHOOSE":U to b-save in frame F-Main. END.
  return no-apply.
end.
define variable vss-include-info9 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on INS of frame F-Main anywhere do:
  if b-mark :sensitive then DO: apply "CHOOSE":U to b-mark in frame F-Main. END.
  return no-apply.
end.
if valid-handle( active-window ) and
   frame F-Main :parent = ?
then do:
   frame F-Main :parent = active-window .
end.
...
procedure local-initialize :
  ...
  if jj = 0
  then do:
    apply "CHOOSE":U to b-mark in frame F-Main .
  end.
  display
    use-column[  1 ]
    use-column[  2 ]
...
procedure my-report :
  ...
  on stop  undo, leave
  :
    apply "CHOOSE":U to b-save in frame F-Main .
    if available buf_usr-flt
    then do:
      assign
        r-flt = recid( buf_usr-flt )
...
procedure my-var :
  ...
  :
    assign
      STR-obj-type = STR-obj-type +         obj-list.obj-type   + chr(44)
      STR-obj-code = STR-obj-code + string( obj-list.obj-code ) + chr(44)
      STR-obj-name = STR-obj-name +         obj-list.obj-name   + chr(44)
      STR-obj      = STR-obj      +         obj-list.obj-type   + '#':U
                                  + string( obj-list.obj-code ) + chr(44)
    .
  end.
  assign
