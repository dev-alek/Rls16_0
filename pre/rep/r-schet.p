...
FIND Our_Host WHERE Our_Host.obj-type = 'орг':U AND Our_Host.obj-code = t-doc.host-code NO-LOCK.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run basecode in g#library
  ...
  PUT STREAM Out_Stream
    ...
    ( if available firm then ( ", ИНН " + trim( firm.inn ) ) else "" ) ) else " " ) )  format "X(130)" SKIP .
  PUT STREAM Out_Stream
    space(6) ( string( firm.addres1, "x(60)" ) +
    ( if available firm then ( "ОКОНХ " + trim( string( firm.okonh, "x(61)" ) ) ) else "" ) ) format "X(131)" SKIP .
...
then do:
    ...
    PUT STREAM Out_Stream
        ...
        string( "примечание   :" + ( if ( NOT can-do( 'факт':U, t-doc.status_ ) OR LogRes ) AND NOT ( t-doc.PS BEGINS "@" ) then  t-doc.PS else " " ) ) AT 10 format "X(100)" SKIP .
end.
else do:
    PUT STREAM Out_Stream
            skip(3)
