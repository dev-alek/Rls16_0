...
DO:
  assign frame Dialog-Frame sel-obj-type allow-days.
  run prepare-rows.
  run update-br("obj-code", false).
END.
ON CHOOSE OF b-sel-objs IN FRAME Dialog-Frame
DO:
...
DO:
  assign frame Dialog-Frame sel-obj-type.
  if sel-obj-type = 1 then do:
      enable b-sel-objs with frame Dialog-Frame.
  end.
  else do:
      disable b-sel-objs with frame Dialog-Frame.
  end.
END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-obj FOR EACH tt-obj.
END PROCEDURE.
...
if (bf_clients.obj-type = 'маг':U or bf_clients.obj-type = 'скл':U)
    and bf_clients.stts = 0  then do:
...
if sel-obj-type = 2 then do:
    for each bf_clients
        where (bf_clients.obj-type = 'маг':U or bf_clients.obj-type = 'скл':U)
