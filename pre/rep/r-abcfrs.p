...
define variable vss-workfile    as character no-undo init "$Workfile: r-abcfrs.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-abcfrs.p $":U .
...
define temp-table temp-abc no-undo
  ...
  index pi2 id db-num
.
DEFINE temp-table temp-grp1 no-undo
  field prc          as decimal
  field abc          as char
  field raiting      as integer
  field grp-code     as integer
  field grp-name     as char
  field node-name    as char
index  pi  IS PRIMARY  prc DESCENDING
 ...
 run get-report-num in parParentProc(output g#report-num).
  ...
  for each temp-grp1 :
     ...
     number-group = string(i-numb,"99999") .
      ...
      for each temp-str where temp-str.node-code =  temp-grp1.grp-code  no-lock :
...
run macr_excel_char in this-procedure ( temp-str.grp-name  , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
run macr_excel_char in this-procedure ( temp-str.prod-name , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
for each temp-abc:
  ...
  if available ub.abc-analysis-goods then do:
     run macr_excel_char in this-procedure ( ub.abc-analysis-goods.abcg-abc + string(ub.abc-analysis-goods.rating) , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
     run macr_excel_DEC in this-procedure  ( ub.abc-analysis-goods.proc-from-all , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
     run macr_excel_DEC in this-procedure  ( ub.abc-analysis-goods.abcg-sum-for-estimate   , num#str# , num#col#   ) .
define variable v-color as integer   no-undo .
   ...
   put  stream macr_excel unformatted
        substitute('select("r&1c&2:r&3c&4 ")' , num#str# , num#col# - 2 , num#str# , num#col# ) + chr(10)
        substitute('font.properties(,,,,,,,,,&1,,,)', v-color ) + chr(10) .
...
procedure PrintTitul :
  ...
  define variable tt as integer no-undo .
    ...
    num#col# = 2.
    cc = num#str# .
    run macr_excel_char in this-procedure ( "Сравнение ABC анализов по группам товаров"  , num#str# , num#col#   ) .
    num#str# = num#str# + 1.
    num#col# = 2.
    run macr_excel_char in this-procedure ("Печать " + cur-time-date()  , num#str# , num#col#   ) .
    ...
    run pshap in this-procedure .
     num#str# = num#str# + 1.
    num#col# = 1.
    run macr_excel_char in this-procedure ( "Артикул товара"   , num#str# , num#col#   ) .
    run macr_cell_size in this-procedure  ( 10 , ? , num#str# , num#col# , ?, ? ) .
    num#col# = 2.
    run macr_excel_char in this-procedure ( "Код"   , num#str# , num#col#   ) .
    run macr_cell_size in this-procedure  ( 10 , ? , num#str# , num#col# , ?, ? ) .
    num#col# = 3.
    run macr_excel_char in this-procedure ( "ТМЦ"   , num#str# , num#col#   ) .
    run macr_cell_size in this-procedure  ( 25 , ? , num#str# , num#col# , ?, ? ) .
    num#col# = 4.
    run macr_excel_char in this-procedure ( "Группа"   , num#str# , num#col#   ) .
    run macr_cell_size in this-procedure  ( 32 , ? , num#str# , num#col# , ?, ? ) .
    num#col# = 5.
    run macr_excel_char in this-procedure ( "Производитель"   , num#str# , num#col#   ) .
    run macr_cell_size in this-procedure  ( 20 , ? , num#str# , num#col# , ?, ? ) .
    for each temp-abc :
        num#col# = num#col# + 1.
        run macr_excel_char in this-procedure ( "Анализ № " + string(temp-abc.nn) , num#str# , num#col#   ) .
        run macr_cell_size in this-procedure  ( 10 , ? , num#str# , num#col# , ?, ? ) .
        num#col# = num#col# + 1.
        run macr_excel_char in this-procedure (  "%"  , num#str# , num#col#   ) .
        run macr_cell_size in this-procedure  ( 5 , ? , num#str# , num#col# , ?, ? ) .
        num#col# = num#col# + 1.
        run macr_excel_char in this-procedure ( "Сумма по критерию"  , num#str# , num#col#   ) .
...
for each temp-abc :
  ...
  for each buf_abc-analysis-grp no-lock where
    ...
    if not can-find ( first temp-grp where temp-grp.grp-code  = buf_abc-analysis-grp.grp-code) then do:
        ...
        create temp-grp .
        assign
            temp-grp.prc        = buf_abc-analysis-grp.abcg-prcnt-for-estimate
    ...
    FOR EACH temp-str  BY temp-str.grp-name :
        find first temp-grp1 where temp-grp1.grp-name  = n-lavel(temp-str.grp-name , 1 ) no-error .
        if not available temp-grp1 then do:
            create temp-grp1.
            assign
              temp-grp1.grp-name  = n-lavel(temp-str.grp-name , 1 ) .
            .
        end.
    END.
for each temp-abc :
  ...
  for each temp-grp1 :
        for each buf_abc-analysis-goods no-lock where
                buf_abc-analysis-goods.abc-id = temp-abc.id  and
                buf_abc-analysis-goods.db-num = temp-abc.db-num ,
          first temp-str where
                temp-str.gds-code = buf_abc-analysis-goods.gds-code and
                temp-str.grp-name begins temp-grp1.grp-name :
              v-sum-prc = v-sum-prc + temp-str.prc .
        end.
        if v-sum-prc = 0 then delete temp-grp1.
                         else do:
                             temp-grp1.prc = v-sum-prc .
                         end.
  end.
end.
  end.
    ...
    for each temp-abc :
        run macr_excel_char in this-procedure ( "Анализ № " + string(temp-abc.nn )                , num#str# , num#col#   ) .
            ...
            ( 10    ,
              ...
              num#col# ) .
        num#str# = num#str# + 1.
        run macr_excel_char in this-procedure ( temp-abc.name               , num#str# , num#col#   ) . num#str# = num#str# + 1.
        if temp-abc.des <> "" then do:  run macr_excel_char( temp-abc.des                , num#str# , num#col#   ) . num#str# = num#str# + 1. end.
        run macr_excel_char in this-procedure ( temp-abc.name-cral          , num#str# , num#col#   ) . num#str# = num#str# + 1.
        run macr_excel_char in this-procedure ( "A=" + string( temp-abc.A )                 , num#str# , num#col#   ) . num#str# = num#str# + 1.
        run macr_excel_char in this-procedure ( "B=" + string( temp-abc.B )                 , num#str# , num#col#   ) . num#str# = num#str# + 1.
        run macr_excel_char in this-procedure ( "C=" + string( temp-abc.C )                 , num#str# , num#col#   ) . num#str# = num#str# + 1.
        if temp-abc.D > 0 then do:  run macr_excel_char in this-procedure ( "D=" + string(temp-abc.D )   , num#str# , num#col#   ) .    num#str# = num#str# + 1. end.
        if temp-abc.E > 0 then do:  run macr_excel_char in this-procedure ( "E=" + string(temp-abc.E )   , num#str# , num#col#   ) .    num#str# = num#str# + 1. end.
        if temp-abc.F > 0 then do:  run macr_excel_char in this-procedure ( "F=" + string(temp-abc.F )   , num#str# , num#col#   ) .    num#str# = num#str# + 1. end.
        if temp-abc.type <> " " then do: run macr_excel_char in this-procedure ( temp-abc.type          , num#str# , num#col#   ) .  num#str# = num#str# + 1.    end.
        run macr_excel_char in this-procedure ( "создан : " + string(temp-abc.date-cr , "99/99/9999")            , num#str# , num#col#   ) . num#str# = num#str# + 1.
        run macr_excel_char in this-procedure ( temp-abc.period             , num#str# , num#col#   ) .  num#str# = num#str# + 1.
        run macr_excel_char in this-procedure ( temp-abc.ext-doc-type       , num#str# , num#col#   ) .  num#str# = num#str# + 1.
        run macr_excel_char in this-procedure ( temp-abc.obj                , num#str# , num#col#   ) .  num#str# = num#str# + 1.
    end.
  end.
end procedure.
...
num#str# = num#str# + 1.
num#col# = 1.
run macr_excel_char in this-procedure (  temp-grp1.grp-name   , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
 assign    num#col# = num#col# + 1 .
 assign    num#col# = num#col# + 1 .
...
for each temp-abc:
  ...
  if available ub.abc-analysis-grp then do:
     run macr_excel_char in this-procedure ( ub.abc-analysis-grp.abcg-abc + string(ub.abc-analysis-grp.rating) , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
     run macr_excel_DEC in this-procedure  ( ub.abc-analysis-grp.proc-from-all , num#str# , num#col#   ) . assign    num#col# = num#col# + 1 .
     run macr_excel_DEC in this-procedure  ( ub.abc-analysis-grp.abcg-sum-for-estimate   , num#str# , num#col#   ) .
define variable v-color as integer   no-undo .
