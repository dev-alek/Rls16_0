...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-corr-check.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE cb-check-type v-doc-osnov BUTTON-1 f-date-corr f-num-corr r-shift WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN cb-check-type v-doc-osnov BUTTON-1 f-date-corr f-num-corr r-shift WITH FRAME F-Main.
...
DO:
  ...
  def var rid# as character no-undo .
    run ref/codelayout.p('ВЫБОР':U,"","OsnovCorr", "Основание коррекции",output table tmprecid).
    for first tmprecid where tmprecid.fTable = "code" no-lock:
      ...
      display v-doc-osnov with frame F-Main .
      apply "LEAVE":U to v-doc-osnov.
    end.
END.
ON VALUE-CHANGED OF cb-check-type IN FRAME F-Main
DO:
  assign cb-check-type .
END.
ON LEAVE OF f-date-corr IN FRAME F-Main
DO:
  assign f-date-corr .
END.
ON LEAVE OF f-num-corr IN FRAME F-Main
DO:
  assign f-num-corr .
END.
...
DO:
  assign r-shift .
END.
ON LEAVE OF v-doc-osnov IN FRAME F-Main
DO:
...
on ' ' of f-date-corr in frame F-Main
do:
...
on delete-character of f-date-corr in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of f-date-corr in frame F-Main
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of f-date-corr in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of f-date-corr in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of f-date-corr in frame F-Main
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date13
    ...
    .
  if f-date-corr :POPUP-MENU in frame F-Main = ?
  then do:
    ASSIGN
      f-date-corr :POPUP-MENU in frame F-Main = MENU m-ed-date13 :HANDLE
      f-date-corr :MENU-MOUSE in frame F-Main = 3
    .
  end.
  define variable v-label-handle13 as handle no-undo .
  assign
    v-label-handle13 = f-date-corr :side-label-handle in frame F-Main
  .
...
END PROCEDURE.
PROCEDURE my-var :
  assign frame F-Main v-doc-osnov .
END PROCEDURE.
