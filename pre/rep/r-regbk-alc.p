...
define temp-table tt-rep1
    field cnt-line as integer
    field alc-type-name like ub.alc-type.alc-type-name
    field alc-type-code like ub.alc-type.alc-type-code
    field cli-obj-name like ub.clients.obj-name
    field inn like ub.firm.inn
    field date-trn as date
    field doc-line-code like ub.doc-line.doc-code
    field doc-line-fact-order like ub.doc-line.fact-order
    field doc-code like ub.doc-line.doc-code
    field volume-piece-litres like ub.goods.ms-base
    field fact-qnty like doc-line.fact-qnty
    field inc-total-quontity like doc-line.fact-qnty
    field exp-categoryes-prod as character
    field exp-alc-type-name like ub.alc-type.alc-type-name
     field exp-alc-type-code like ub.alc-type.alc-type-code
     field exp-doc-line-code like ub.doc-line.doc-code
     field exp-td-fact-date as date
     field exp-fact-order like ub.doc-line.fact-order
    field exp-volume-piece-litres like ub.goods.ms-base
    field exp-fact-qnty like doc-line.fact-qnty
    field exp-total-quontity like doc-line.fact-qnty
...
procedure ConvertStr-ext-doc-type:
    ...
    define variable v-num-element as integer no-undo.
        v-num-element = lookup(p-ext-doc-type, 'ie,ee,ep,es,re,rs,we,vt,vp,iv,ev,rv,em,wm,im,ot,ap,mp,pc,io,eo':U).
        p-name-ext-doc-type = entry(v-num-element, 'приход внешний,расход внешний,возврат пост.,касса продажа,возврат внешний,касса возврат,списание,инвентаризация,пересортица,приход внутренний,расход внутренний,возврат внутренний,расход  произв.,списан. произв.,приход  произв.,переоценка,коррекция учетных цен,корректировка отрицательных партий,смена типа приобретения,приход внутриобъектный,расход внутриобъектный':U).
    ...
    output stream OutStr-html to value(v-file-name-rep-htm) append convert target 'UTF-8'.
        put stream OutStr-html unformatted
            substitute(
                ...
                buf_tt.doc-code,
                v-volume-piece-litres,
                if buf_tt.fact-qnty = 0 and buf_tt.volume-piece-litres = 0 then "" else string(fnc-fmt-dec-tc-qnty(buf_tt.fact-qnty))
            )
            +
            substitute(
                ...
                v-exp-volume-piece-litres,
                if buf_tt.exp-fact-qnty = 0 and buf_tt.exp-volume-piece-litres = 0 then "" else string(fnc-fmt-dec-tc-qnty(buf_tt.exp-fact-qnty)),
                ""
                ).
    output stream OutStr-html close.
            end.
...
do:
    do:
        ...
        run day-begin-fact-order in this-procedure ( input X-Date-Start
                                                    , output v-fact-order-start
                                                    ).
        run factord-end-day in this-procedure ( input X-Date-End
                                                , output v-fact-order-end
                                                ).
        run get-report-num in parParentProc (
            output g#report-num
        ).
        v-file-name-rep-htm = session:temp-directory + "rpt" + string(g#report-num) + ".html".
    ...
    do:
        ...
        do:
            find first ub.shop where
                       ...
                       ub.shop.sub-store-on = yes and
                       ub.shop.sub-store-type <> '':U and
                       ub.shop.sub-store-code <> 0
            no-error.
        ...
        do:
            ...
            :
                ...
                do:
                    v-alc-type-name = "".
                    v-alc-type-code = "".
                    for first ub.alc-type-gds where
                              ub.alc-type-gds.gds-code = ub.gds-obj.gds-code no-lock
                    :
                        do:
                            for first ub.alc-type where
                                      ub.alc-type.alc-type-inner-code = ub.alc-type-gds.alc-type-inner-code no-lock
                            :
                                do:
                                    assign
                                        v-alc-type-name = ub.alc-type.alc-type-name
                                        v-alc-type-code = ub.alc-type.alc-type-code
                                    .
                                    ...
                                    :
                                        if lookup(ub.doc-line.ext-doc-type, 'ie,re,rs,vt,vp,ap,mp,pc,iv,rv,im,io':U) > 0 then
                                        do:
                                            if v-sub-store-on = yes then
                                            do:
                                                if (can-find(first buf2_obj-list no-lock where
                                                    ...
                                                    and
                                                    (ub.doc-line.ext-doc-type = 'iv':U or
                                                     ub.doc-line.ext-doc-type = 'rv':U)
                                                then next.
                                            end.
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input v-doc-code ,
                       ...
                       output v-attr-type )  .
                                                ...
                                                assign
                                                    ...
                                                    tt-rep1.alc-type-code = v-alc-type-code
                                                    tt-rep1.cli-obj-name = v-fmtcli-name
                                                    tt-rep1.inn = v-fmtcli-inn
                                                    tt-rep1.date-trn = v-attr-doc-data
                                                    tt-rep1.doc-line-code = doc-line.doc-code
                                                    tt-rep1.doc-line-fact-order = doc-line.fact-order
                                                    tt-rep1.doc-code = v-attr-doc-code
                                                    tt-rep1.volume-piece-litres =
                                                        if (goods.ms-base * doc-line.cli-base-rate) = ? then 0
                                                        else (goods.ms-base * doc-line.cli-base-rate)
                                                .
                                            end.
                                            tt-rep1.fact-qnty = tt-rep1.fact-qnty +
                                                if (doc-line.fact-qnty / doc-line.cli-base-rate) = ? then 0
                                                else (doc-line.fact-qnty / doc-line.cli-base-rate).
                                        end.
                                        else
                                        if lookup(ub.doc-line.ext-doc-type, 'ee,ep,es,we,ev,em,wm,eo':U) > 0 then
                                        do:
                                            do:
                                            end.
                                            if v-sub-store-on = yes then
                                            ...
                                            assign
                                                tt-rep1.exp-doc-line-code = doc-line.doc-code
                                                tt-rep1.exp-alc-type-code = v-alc-type-code
                                                tt-rep1.exp-td-fact-date = buf_trn-doc.fact-date
                                                tt-rep1.doc-line-fact-order = doc-line.fact-order
                                                tt-rep1.exp-categoryes-prod = v-name-ext-doc-type
                                                tt-rep1.exp-alc-type-name = v-alc-type-name
...
define variable v-dec as character no-undo.
        ...
        :
            ...
            v-inc-litres-line =
                ...
                else ((decimal(tt-rep1.volume-piece-litres) * decimal(tt-rep1.fact-qnty)) / 10).
            v-inc-litres-tot = v-inc-litres-tot + v-inc-litres-line.
            v-inc-qnty-tot = v-inc-qnty-tot +
                (if decimal(tt-rep1.fact-qnty) = ? then 0 else decimal(tt-rep1.fact-qnty)).
            v-out-litres-line =
            ...
            v-out-litres-tot =
                ...
                (if v-out-litres-line = ? then 0 else v-out-litres-line).
            v-out-qnty-tot =
                (if v-out-qnty-tot = ? then 0 else v-out-qnty-tot) +
                (if tt-rep1.exp-fact-qnty = ? then 0 else tt-rep1.exp-fact-qnty).
        end.
        v-cnt-line = 0.
    end.
...
procedure my-watch-table:
    ...
    define buffer tt-rep1 for tt-rep1.
    v-table-handle = buffer tt-rep1:handle.
    ...
    v-full-file-name = "C:\work15_0\my-watch2-tt-rep1.txt".
    ...
    output stream MyWatch-strm to value(v-full-file-name)   convert target "utf-8".
        put stream MyWatch-strm unformatted
            today format "99.99.9999" " " string(time, "HH:MM") " " "Исследуемая таблица: " "tt-rep1" "." skip
            v-list-field-label skip
            v-list-field-name skip
            v-list-field-type skip
        .
        if not can-find(first tt-rep1) then
        do:
            v-message = "Исследуемая таблица tt-rep1 пуста!".
            put stream MyWatch-strm unformatted
                v-message
            .
            ...
            for each  tt-rep1 no-lock:
                export stream MyWatch-strm delimiter "$"  tt-rep1.
            end.
