    ...
    then do:
      ...
      assign
        ...
        v-cmdln  = v-exefile
                 ...
                 + chr(32) + "-p":U + chr(32) + "rep/extexcel.p":u
                 + chr(32) + "-param":U + chr(32) + chr(34)
                 + tempfile + chr(44)
                 + err-file + chr(44)
                 + string(make-excel) + chr(44)
                 + string(make-excel-com) + chr(44)
                 + tempfile-frm + chr(34)
      .
      run waitfram-show in this-procedure
        (input "Выполнение команды" + chr(32) + v-cmdln
        ).
      run gbl/syn3.p
        (input v-cmdln
        ,input err-file
      ...
      if res <> '0'
      then do:
        v-mess = substitute("Не  удалось  вывести  файл  в   EXCEL").
        if not v-silent then do:
          message
          v-mess
        view-as alert-box error.
