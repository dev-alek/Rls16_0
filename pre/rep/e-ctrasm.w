...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-ctrasm.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE RECT-8 RECT-9 RECT-10 tg-gds-by-am tg-group-by-order fi-days-wt-goods fi-critical-qnty-balance fi-critical-qnty-sale fi-critical-qnty-order tog-1 tog-2 tog-3 tog-4 tog-5 tog-6 WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-8 RECT-9 RECT-10 tg-gds-by-am tg-group-by-order fi-days-wt-goods fi-critical-qnty-balance fi-critical-qnty-sale fi-critical-qnty-order tog-1 tog-2 tog-3 tog-4 tog-5 tog-6 WITH FRAME F-Main.
...
ON VALUE-CHANGED OF tg-gds-by-am IN FRAME F-Main
DO:
  if not tg-gds-by-am:checked then do:
    disable tog-1 tog-2 tog-3 tog-4 tog-5 tog-6 with frame F-Main.
  end.
  else do:
    enable tog-1 tog-2 tog-3 tog-4 tog-5 tog-6 with frame F-Main.
...
DO:
  if tog-1:checked then do :
    disable tog-2 tog-3 tog-4 tog-5 tog-6 with frame F-Main.
  end.
  else do :
    enable tog-2 tog-3 tog-4 tog-5 tog-6 with frame F-Main.
  end.
END.
...
define variable v-naim           as character        no-undo.
  ...
  :
    assign
      tog-1:label = 'все':U
      tog-2:label = 'Новинка':U
      tog-3:label = 'Основная группа':U
      tog-4:label = 'Нештатный':U
      tog-5:label = 'На вывод из ассортимента':U
      tog-6:label = 'Пусто':U
    .
  end.
  CASE place-call:
    ...
    otherwise do:
      ...
      if num-entries(v-list , chr(4)) = 15
      then do:
        assign
          tg-gds-by-am              = logical(entry( 1, v-list, chr(4)))
          tg-group-by-post          = logical(entry( 2, v-list, chr(4)))
          tg-detailed               = logical(entry( 3, v-list, chr(4)))
          fi-critical-qnty-balance  = decimal(entry( 4, v-list, chr(4)))
          fi-critical-qnty-sale     = decimal(entry( 5, v-list, chr(4)))
          fi-critical-qnty-order    = decimal(entry( 6, v-list, chr(4)))
          fi-days-wt-goods          = decimal(entry( 7, v-list, chr(4)))
          fi-gds-qnty               = decimal(entry( 8, v-list, chr(4)))
          tog-1                     = logical(entry( 9, v-list, chr(4)))
          tog-2                     = logical(entry( 10, v-list, chr(4)))
          tog-3                     = logical(entry( 11, v-list, chr(4)))
          tog-4                     = logical(entry( 12, v-list, chr(4)))
          tog-5                     = logical(entry( 13, v-list, chr(4)))
          tog-6                     = logical(entry( 14, v-list, chr(4)))
          tg-group-by-order         = logical(entry( 15, v-list, chr(4)))
        no-error .
        display
          tg-gds-by-am
          tg-group-by-order
   ...
   WHEN "get" THEN DO:
    IF v-profile-id = 54  THEN DO:
       ...
       RUN rcps_get-value IN parent-handle (
                                         ...
                                         ,output v-value-character
                                         ,output v-value-date
                                         ,output v-critical-qnty-balance
                                         ,output v-value-integer
    ...
    IF v-profile-id = 54 THEN DO:
      ...
      for each obj-list :
        ...
        RUN rcps_set-value IN parent-handle (
                                        input "p-objects"
                                        ,INPUT v-index-id
                                        ,input substitute("&1&2", obj-list.obj-type, obj-list.obj-code)
                                        ,input ?
                                        ,input 0.0
                                        ,input 0
                                        ,input no
...
define variable v-type-val       as logical          no-undo.
  ...
  assign
    v-list =  string(tg-gds-by-am     , "yes/no") + chr(4) +
              string(tg-group-by-post , "yes/no") + chr(4) +
              string(tg-detailed      , "yes/no") + chr(4) +
              string(fi-critical-qnty-balance   ) + chr(4) +
              string(fi-critical-qnty-sale      ) + chr(4) +
              string(fi-critical-qnty-order     ) + chr(4) +
              string(fi-days-wt-goods           ) + chr(4) +
              string(fi-gds-qnty                ) + chr(4) +
              string(tog-1            , "yes/no") + chr(4) +
              string(tog-2            , "yes/no") + chr(4) +
              string(tog-3            , "yes/no") + chr(4) +
              string(tog-4            , "yes/no") + chr(4) +
              string(tog-5            , "yes/no") + chr(4) +
              string(tog-6            , "yes/no") + chr(4) +
              string(tg-group-by-order, "yes/no")
  .
  run uf-set( input 'ctrasm':U
            , input v-cntxt-userid
            , input v-list
            , input v-naim
            , input v-print-graft
...
PROCEDURE my-var :
  ...
  then do:
    assign
      ReportHeader = chr(10) + "По АМ":U
    .
  end.
  if tg-group-by-post = yes
  then do:
    assign
      ReportHeader = ReportHeader + chr(10) + "Группировка по поставщику":U
    .
  end.
  if tg-detailed = yes
  then do:
    assign
      ReportHeader = ReportHeader + chr(10) + "Детализированный отчет":U
    .
  end.
  assign
    ReportHeader = ReportHeader + chr(10) + substitute( "Критический остаток: &1" , fi-critical-qnty-balance ) +
                                  chr(10) + substitute( "Критическая продажа: &1" , fi-critical-qnty-sale    ) +
                                  chr(10) + substitute( "Критический заказ: &1"   , fi-critical-qnty-order   ) +
                                  chr(10) + substitute( "Фильтры:"                                           ) +
                                  chr(10) + substitute( "Дней без товара: &1"     , fi-days-wt-goods         )
  .
  assign
    ReportHeader = ReportHeader + chr(10) + "Показывать товары с ИЖТ:":U +
      ( if tog-1 = yes then 'все':U else
      ( if tog-2 = yes then 'Новинка':U   + "," else "" ) +
      ( if tog-3 = yes then 'Основная группа':U   + "," else "" ) +
      ( if tog-4 = yes then 'Нештатный':U  + "," else "" ) +
      ( if tog-5 = yes then 'На вывод из ассортимента':U   + "," else "" ) +
      ( if tog-6 = yes then 'Пусто':U       else "" ) )
  .
  assign
