...
define temp-table tt-info
   ...
   field src-sum as dec
   field discnt as dec
   field src-sum-no-disc as dec
   field src-sum-disc as dec
   ...
   field itog-sum-no-disc as dec
   field itog-sum-disc as dec
   field itog-proc as dec
   field sts as char
   field b-code as integer
   field change-BL as char
   field ret-nodiscnt as log
   field pay-desk as int
   field ext-code as char
   index sort1 chk-date chk-num.
define variable v-report-name       as character no-undo .
define variable v-file-name-rep-htm as character no-undo .
define variable v-filt-1            as character no-undo .
define variable v-filt-2            as character no-undo .
define variable v-date-start        as date      no-undo .
define variable v-page-name         as character no-undo .
define variable v-obj-code          as integer   no-undo .
define variable v-wdaylist          as character no-undo .
define variable v-timelist          as character extent 7 no-undo .
define variable v-sdaynum           as character no-undo .
define variable v-idaynum           as integer   no-undo .
...
run clients-write(INPUT v-cntxt-host-code-obj, INPUT 'орг':U, OUTPUT v-obj-name) no-error .
for each obj-list no-lock:
  ...
  do:
    for each buf_chk-doc no-lock
      ...
      and buf_chk-doc.chk-type = int('1':U)
    ...
    if tt-info.promo-id  = "" then do:
      ...
      for first buf_PromoAction no-lock where
          ...
          find first ub.promoAttr where
                     ...
                     buf_PromoAction.id = int64(entry(1,ub.PromoAttr.p-key,chr(3))) and
                     buf_PromoAction.db-num = integer(entry(2,ub.PromoAttr.p-key,chr(3)))
                no-error.
...
procedure ConvertStr-pay-type:
   ...
   find first ub.cash-pay no-lock where ub.cash-pay.cdpay-code = p-pay-code and ub.cash-pay.status_ = 'тек':U no-error .
