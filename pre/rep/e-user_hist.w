...
IF NOT THIS-PROCEDURE:PERSISTENT THEN DO:
  MESSAGE "c:\tester\Rls_16_0\rc_160_rus\cmpdir\src\rep\e-user_hist.w should only be RUN PERSISTENT.":U
...
PROCEDURE adm-disable :
    DISABLE user-name object-name WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN user-name object-name WITH FRAME F-Main.
...
if x-SelectObject = "firm" then user-name:list-item-pairs in frame F-Main = "ВСЕ,-1" .
if x-SelectObject <> "firm" then do:
user-name:list-item-pairs in frame F-Main = "ВСЕ,-1" .
...
End.
        ...
        and bf_c-user-log.corr-date <= x-Date-End by bf_c-user-log.head-table :
            ...
            if v-table begins "c-" and v-table <> 'c-usr-hist':U and v-table <> 'c-plc-hist':U then
            do:
...
object-name:list-item-pairs in frame F-Main = "ВСЕ,-1" .
