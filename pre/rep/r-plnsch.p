    ...
    for each temp-doc break by temp-doc.cli-type by temp-doc.cli-code :
      if first-of(temp-doc.cli-code) then do:
        if itog-comp = no then do:
          PUT STREAM PrnLibStream string("|Контрагент: " + temp-doc.cli-name) format "X(89)" "|" at 110 skip .
      ...
      PUT STREAM PrnLibStream string("|Договор: " + temp-doc.contr-name) format "X(89)" "|" at 110  skip .
      run is-page in this-procedure .
      if itog-comp = no then do:
        for each temp-contr where temp-contr.contr = temp-doc.contr :      run prn-line in this-procedure .    end.
      end.
      PUT STREAM PrnLibStream string("|Всего по " + temp-doc.contr-name) format "X(29)" "|" at 35 temp-doc.sum-fo format "->>>,>>>,>>>,>>>,>>9.99"  "|" at 60 temp-doc.sum-pl format "->>>,>>>,>>>,>>>,>>9.99" "|" at 85 (temp-doc.sum-fo - temp-doc.sum-pl)  format "->>>,>>>,>>>,>>>,>>9.99"   "|" at 110 skip  .
      run is-page in this-procedure .
      assign
      ...
      if last-of(temp-doc.cli-code)  then do:
        ...
        run is-page in this-procedure .
        PUT STREAM PrnLibStream string("|Всего по " + temp-doc.cli-name) format "X(29)" "|" at 35 v-sum format "->>>,>>>,>>>,>>>,>>9.99"  "|" at 60 v-sum1 format "->>>,>>>,>>>,>>>,>>9.99" "|" at 85 (v-sum - v-sum1)  format "->>>,>>>,>>>,>>>,>>9.99"   "|" at 110 skip  .
        run is-page in this-procedure .
        PUT STREAM PrnLibStream Line format "X(110)" skip .
        run is-page in this-procedure .
      end.
  ...
  do on error undo, return error return-value :
    ...
    if temp-contr.day > 0 then
      put stream PrnLibStream
        "|"  At 1   temp-contr.dat  Format "99/99/9999"
        "|"  At 12   temp-contr.num  Format "X(15)"
        "|"  At 29   temp-contr.day  Format ">>>9"
        "|"  At 35   temp-contr.sum  Format "->>>,>>>,>>>,>>>,>>9.99"
        "|"  At 60   "|"  At 85   "|"  At 110
      skip .
    else
      put stream PrnLibStream
...
procedure is-page :
  ...
  :
    if line-counter( PrnLibStream ) + 2 > page-size( PrnLibStream ) then do:
      put stream PrnLibStream  skip Line format "X(110)" skip "продолжение - на следующей странице" AT 30 SKIP .
...
procedure PrintTitul :
  ...
  :
    ...
    put stream PrnLibStream
      skip  Line format "X(110)"  skip
      "|"           "План. дата"     format "X(10)"
      "|" at 12  " № фин.об."      format "X(10)"
      "|" at 29  "Про-" format "X(4)"
      "|" at 35  " Непогашенная"  format "X(15)"
      "|" at 60  " Сумма "  format "X(15)"
      "|" at 85  " Итого "  format "X(15)"
      "|" at 110
      skip
      "|"           "платежа"     format "X(10)"
      "|" at 12
      "|" at 29  "сроч"        format "X(4)"
      "|" at 35  string(" сумма (" + s-val + ")")    format "X(17)"
      "|" at 60  "несвязанных"                       format "X(15)"
      "|" at 85  string(" к оплате (" + s-val + ")") format "X(17)"
      "|" at 110
      skip
      "|"
      "|" at 12
      "|" at 29  "дней " format "X(4)"
      "|" at 35
      "|" at 60  string("платежей (" + s-val + ")") format "X(17)"
      "|" at 85
      "|" at 110
      skip  Line format "X(110)"  skip
    .
