...
DEFINE VARIABLE Show-vat-Cost AS LOGICAL INITIAL no
     LABEL "НДС в учетных ценах":L
     VIEW-AS TOGGLE-BOX
     SIZE 23.5 BY .83 TOOLTIP "Показывать суммы НДС в учетных ценах" NO-UNDO.
DEFINE VARIABLE Show-vat-Sale AS LOGICAL INITIAL no
     LABEL "НДС в ценах документа":L
     VIEW-AS TOGGLE-BOX
     SIZE 23.63 BY .83 TOOLTIP "Показывать суммы НДС в ценах документа" NO-UNDO.
DEFINE VARIABLE Showcost AS LOGICAL INITIAL yes
     LABEL "Учетные цены"
     VIEW-AS TOGGLE-BOX
     SIZE 17.38 BY .83 TOOLTIP "Показать суммы в учетных ценах" NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Cli-art AT ROW 6 COL 48.38
     Classify AT ROW 6.79 COL 2.75 NO-LABEL
     type-stor AT ROW 7.38 COL 48.25 NO-LABEL
     ...
     Show-vat-Cost AT ROW 12.67 COL 23.75
     Showcost AT ROW 12.71 COL 2.75
     Show-vat-Sale AT ROW 13.5 COL 23.75
     Showsale AT ROW 13.54 COL 2.75
     ShowZero-2 AT ROW 14.58 COL 2.75
     ShowZero AT ROW 15.58 COL 2.75
     SumsOnly AT ROW 16.46 COL 2.75
     ...
     "Показать :" VIEW-AS TEXT
          SIZE 11.5 BY .75 AT ROW 11.75 COL 3.38
          FGCOLOR 4
...
PROCEDURE adm-disable :
    DISABLE RECT-7 RECT-6 RECT-5 RECT-8 RECT-9 RADPost PostName SortType Tog-obj Cli-art Classify type-stor Show-vat-Cost Showcost Showsale ShowZero-2 ShowZero SumsOnly WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-7 RECT-6 RECT-5 RECT-8 RECT-9 RADPost PostName SortType Tog-obj Cli-art Classify type-stor Show-vat-Cost Showcost Showsale ShowZero-2 ShowZero SumsOnly WITH FRAME F-Main.
...
ASSIGN
       ShowZero:HIDDEN IN FRAME F-Main           = TRUE.
...
assign Showsale.
  if showsale then DO:
                  ...
                  enable Show-vat-Sale  with frame F-Main.
                  End.
              else do:
                                     Show-vat-Sale = no.
                   enable  type-stor with frame F-Main.
                   disable Show-vat-Sale  with frame F-Main .
                    display Show-vat-Sale  with frame F-Main.
                   end.
END.
...
DO:
  ...
  if tog-obj = false then do:
    assign
      ...
      tog-Lavel   = false
      .
      display Classify SumsOnly type-stor ShowZero  var-Lavel-2 var-Lavel tog-Lavel-2 tog-Lavel with frame F-Main.
      disable Classify SumsOnly type-stor  ShowZero var-Lavel-2 var-Lavel tog-Lavel-2 tog-Lavel  with frame F-Main.
  End.
  Else do:
...
PROCEDURE my-var :
assign frame F-Main
Cli-art  PostName  RADPost
ShowZero Classify  SortType  SumsOnly Tog-obj
Tog-lavel  var-lavel Tog-lavel-2  Var-lavel-2
Show-vat-cost Show-vat-sale ShowZero-2 showcost
showsale type-stor
.
show-cost =  showcost .
show-sale =  showsale .
...
ReportHeader = ReportHeader +
               ...
               (if Show-vat-Cost     then "НДС в учетных ценах, "  else " "    ) +
               (if Show-Sale         then "Суммы в ценах документа, "  else " "  ) +
               (if Show-vat-Sale     then "НДС в ценах документа, "  else " "  ) +
...
Sheetf.Excel-Column-Lable = "Код,Артикул,Название товара,Ед.изм,т.у,
 Расход,,,
 Касса,,,
 Инвентаризация,,,
 Возврат,,,
...
" кол-во,учет.сумма,док-т.сумма ," +
" кол-во,учет.сумма,док-т.сумма ," +
" кол-во,учет.сумма,док-т.сумма ," +
" кол-во,учет.сумма,док-т.сумма ," +
" кол-во,учет.сумма,док-т.сумма ," +
" кол-во,учет.сумма,док-т.сумма, ".
 Sheetf.Sizes = "10,16,60,7,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,".
