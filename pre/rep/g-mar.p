...
define variable v-value-character12 as character no-undo .
...
run rep/d-report.w (
  ...
  ,input "*"
  ,input ""
  ,input (if v-curr-r-b = 'base':U
        then (if base-code = 0 then "2" else "2,3")
        else "":U)
  ,input "shop,4,22"
  ,input no).
