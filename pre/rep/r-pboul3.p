...
procedure end-proc :
 ...
 :
  ...
  define temp-table temp-goods-cli no-undo
...
assign
  price-rubl-with-tax-loc = temp-parts.price-rubl
  price-base-with-tax-loc = temp-parts.price-base
.
define variable vss-include-info30 as character format "x(65)" no-undo initial "@(#)$Workfile$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output varinvprb
  )  .
  if temp-parts.out-code = 'free-zone':U     or
     temp-parts.out-code = 'out-zone':U   or
     temp-parts.doc-type = 'акт':U then do:
    assign
      in-vatp-have-vat-slt = yes.
  end.
  else do:
    find first in-vatp_doc-attr no-lock
      where in-vatp_doc-attr.doc-code  = temp-parts.out-code
        and in-vatp_doc-attr.attr-code = 'envd':U
      no-error .
    if not available in-vatp_doc-attr then do:
      assign
  ...
  assign
   price-cli-with-tax-loc = temp-parts.price-cli
   cli-base-rate          = temp-parts.cli-base-rate.
  ASSIGN   road-tax-base-loc  = (if temp-parts.road-tax-base  = ? then 0 else temp-parts.road-tax-base)
           road-tax-rubl-loc  = (if temp-parts.road-tax-rubl  = ? then 0 else temp-parts.road-tax-rubl).
  ASSIGN  transport-base-loc = (if temp-parts.transport-base = ? then 0 else temp-parts.transport-base)
          transport-rubl-loc = (if temp-parts.transport-rubl = ? then 0 else temp-parts.transport-rubl)
          other-base-loc     = (if temp-parts.other-base     = ? then 0 else temp-parts.other-base)
          other-rubl-loc     = (if temp-parts.other-rubl     = ? then 0 else temp-parts.other-rubl)
          vat-pc-loc         = (if temp-parts.vat-pc         = ? then 0 else temp-parts.vat-pc)
          slt-pc-loc         = (if temp-parts.slt-pc         = ? then 0 else temp-parts.slt-pc).
          ASSIGN   slt-base-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-base-with-tax-loc - ((if road-tax-base-loc  = ? then 0 else road-tax-base-loc) + (if transport-base-loc = ? then 0 else transport-base-loc) + (if other-base-loc = ? then 0 else other-base-loc)))                           * slt-pc-loc / (100 + slt-pc-loc))                        vat-base-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-base-with-tax-loc - ((if road-tax-base-loc  = ? then 0 else road-tax-base-loc) + (if transport-base-loc = ? then 0 else transport-base-loc) + (if other-base-loc = ? then 0 else other-base-loc))) * (1 - slt-pc-loc / (100 + slt-pc-loc)) * vat-pc-loc / (100 + vat-pc-loc)).
    ASSIGN   slt-rubl-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-rubl-with-tax-loc - ((if road-tax-rubl-loc  = ? then 0 else road-tax-rubl-loc) + (if transport-rubl-loc = ? then 0 else transport-rubl-loc) + (if other-rubl-loc = ? then 0 else other-rubl-loc)))                           * slt-pc-loc / (100 + slt-pc-loc))                        vat-rubl-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-rubl-with-tax-loc - ((if road-tax-rubl-loc  = ? then 0 else road-tax-rubl-loc) + (if transport-rubl-loc = ? then 0 else transport-rubl-loc) + (if other-rubl-loc = ? then 0 else other-rubl-loc))) * (1 - slt-pc-loc / (100 + slt-pc-loc)) * vat-pc-loc / (100 + vat-pc-loc)).
  assign
    exch-rate-cli-loc = (temp-parts.price-rubl - transport-rubl-loc - other-rubl-loc - road-tax-rubl-loc - (if temp-parts.vat-type <> 'в т. ч.':U then vat-rubl-loc else 0) - (if temp-parts.slt-type <> 'в т. ч.':U then slt-rubl-loc else 0)) / temp-parts.price-cli .
  assign
    slt-cli-loc        = slt-rubl-loc       / exch-rate-cli-loc
    vat-cli-loc        = vat-rubl-loc       / exch-rate-cli-loc
    road-tax-cli-loc   = road-tax-rubl-loc  / exch-rate-cli-loc
...
.
  ...
  for each buf_trn-doc no-lock
    ...
    :
    if ( buf_trn-doc.internal = yes and buf_trn-doc.ext-doc-type <> 'vt':U and buf_trn-doc.ext-doc-type <> 'vp':U ) then next .
    for each buf_doc-line no-lock
...
procedure PutColumnTitulExcel :
  ...
  run macr_cell_size (12,?, v-row, v-col,?,?).      assign v-col = v-col + 1 .
  run macr_excel_char("НДС (руб.)", v-row, v-col) .
  run macr_cell_size (12,?, v-row, v-col,?,?).      assign v-col = v-col + 1 .
