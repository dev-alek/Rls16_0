...
:
    ...
    PUT STREAM excel-line UNFORMATTED
        ...
        chr(9)   p-gds_code
        chr(9)   p-auto
        chr(9)   p-income_qnty_t
        chr(9)   p-normal_wastage
        chr(9)   p-wastage_qnty
        chr(9)   p-total_wast_qnty
        chr(9)   p-price
        chr(9)   p-summ
        chr(10)
    .
...
if session :set-wait-state( "compiler" ) then.
  ...
  if buf_rvs-doc.rvs-type <> 'контроль':U then do:
    run waitfram-hide in this-procedure .
    message
      substitute( 'Форма НП-34. Сверка имеет тип "&1", а должен быть "&2".', buf_rvs-doc.rvs-type, 'контроль':U ) skip
      view-as alert-box error .
    undo, return error.
  end.
  ...
  :
    for each buf_doc-line no-lock
        where  ( buf_doc-line.obj-type         = buf_trn-doc.obj-type
              ...
              ( buf_doc-line.obj-type         = buf_trn-doc.obj-type
                ...
                and buf_doc-line.prod-code    = buf_tt-line.prod-code
                and buf_doc-line.artic        = buf_tt-line.artic
                and buf_doc-line.ext-doc-type = 'ep':U
                and buf_doc-line.status_      = 'факт':U
                and buf_doc-line.fact-order   > buf_tt-line.fact-order
                and buf_doc-line.fact-order   < v-fact-order-end
              )
    on error undo, return error substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( 1 ) )
    :
      if buf_doc-line.ext-doc-type = 'ie':U then do:
        assign
          v-sign = 1
        .
      end.
...
procedure print-titul :
  ...
  :
    ...
    .
    run np34-xl-write-cell-data in this-procedure ( input "h_firm":U,           input v-firm-name ).
    run np34-xl-write-cell-data in this-procedure ( input "h_obj":U,            input v-obj-name ).
    run np34-xl-write-cell-data in this-procedure ( input "h_date_begin":U,       input SUBSTITUTE( "За период с &2 по &1"
                                                                                                      , STRING(v-date-end, "99/99/9999")
                                                                                                      , IF v-date-begin = ? THEN "":U ELSE STRING(v-date-begin, "99/99/9999")
                                                                                                      ) ).
    run np34-xl-write-cell-data in this-procedure ( input "h_type":U,           input v-pp ).
    run np34-xl-write-cell-data in this-procedure ( input "h_director":U,       input v-director ).
    run np34-xl-write-cell-data in this-procedure ( input "h_gen_acct":U,       input v-gen-acct ).
  end.
  return .
end procedure.
procedure print-body :
  ...
  with width 196 down stream-io no-labels no-box.
  do
  on error undo, return error substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( 1 ) )
  :
