...
if NOT can-find(first scnblist) then
    RETURN.
for each scnblist
by scnblist.gds-name
:
  if scnblist.b-str = "":U then next.
  find ub.goods no-lock
    where ub.goods.prod-type = scnblist.prod-type
      and ub.goods.prod-code = scnblist.prod-code
      and ub.goods.artic = scnblist.artic
  .
  find ub.gds-prt no-lock
    where ub.gds-prt.upper-code = ub.goods.prt-root
  .
  if scnblist.qnty = ? then do:
    run gbl/d-prompt.w (
      'title=':u +  substitute("ЕАН &1" , scnblist.b-str )  + '\':u
    ...
    if return-value = 'false':u
        then scnblist.qnty .
        else scnblist.qnty = int(v-value12) .
  end.
  assign
    rootnode_code = ub.gds-prt.node-code
    list-qnty = scnblist.qnty
  .
  ...
  find first ub.bar-code no-lock where
            ub.bar-code.b-code = scnblist.b-code no-error .
  if available ub.bar-code then do:
    if scnblist.b-str <> "":U
    and not scnblist.loc-ean then do:
      define buffer buf_prod-bc for ub.prod-bc.
      find first buf_prod-bc where buf_prod-bc.b-code =
                  scnblist.b-code
              AND buf_prod-bc.b-str = scnblist.b-str no-error .
...
run rep/ticket.p (
    ...
    if scnblist.b-str <> "":U
    and scnblist.loc-ean then do:
      ListProdbc = scnblist.b-str.
      do on error undo, return error :
        if prn-prt = false     and bar-code.node-code <> rootnode_code   then do:     next.   end.   if prn-prt = true then do:     find first gds-prt no-lock       where gds-prt.node-code = bar-code.node-code     .     if gds-prt.is-term = false then do:       next.     end.   end.
run rep/ticket.p (
               buffer ub.goods
...
for each scnblist
by scnblist.artic
by scnblist.prod-type
by scnblist.prod-code
:
  if scnblist.b-str = "":U then next.
  find ub.goods no-lock
    where ub.goods.prod-type = scnblist.prod-type
  ...
  if scnblist.qnty = ? then do:
    ...
    if return-value = 'false':u
        then scnblist.qnty .
        else scnblist.qnty = int(v-value14) .
  end.
  assign
    rootnode_code = ub.gds-prt.node-code
    list-qnty = scnblist.qnty
...
:
  ...
  if scnblist.qnty = ? then do:
    ...
    if return-value = 'false':u
        then scnblist.qnty .
        else scnblist.qnty = int(v-value16) .
  end.
  assign
    rootnode_code = ub.gds-prt.node-code
    list-qnty = scnblist.qnty
...
define variable v-value18 as character no-undo .
if NOT can-find(first scnblist) then
    RETURN.
for each scnblist
by scnblist.order-num
:
  if scnblist.b-str = "":U then next.
  find ub.goods no-lock
    where ub.goods.prod-type = scnblist.prod-type
  ...
  if scnblist.qnty = ? then do:
    ...
    + 'max-chars=5\':u
    + 'readonly=no\':u
    , input-output v-value18
    ).
    if return-value = 'false':u
        then scnblist.qnty .
        else scnblist.qnty = int(v-value18) .
  end.
  assign
    rootnode_code = ub.gds-prt.node-code
    list-qnty = scnblist.qnty
