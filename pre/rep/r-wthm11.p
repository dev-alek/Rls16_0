...
define variable vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  run mainmenu_getcntxt in p-parent-proc
...
HEADER
  ...
  ":--------------:-----------------------------------:-----------------:----------------------------:руб. коп.   :  учета НДС,: номер по :" SKIP
  ": счет, : код  :       наименование        :номен- :код :наименование: затребовано :   отпущено   :            :руб. коп.   :складской :" SKIP
  ...
  PUT STREAM PrnLibStream UNFORMATTED
    ": " + STRING( buf_doc.doc-date, "99.99.9999":U ) +
            ...
            ":" + STRING( v-torgconf-self-obj-name, "x(21)":U)  +
                ":            :" +
                (if buf_doc.doc-type = 'рас':U then STRING(  v-torgconf-cli-name, "x(21)":U) else STRING( v-torgconf-self-obj-name, "x(21)":U) ) +
                                                                                     ":            :       :              :                :" AT 1 SKIP.
  PUT STREAM PrnLibStream UNFORMATTED
    "---------------------------------------------------------------------------------------------------------------------------------------" AT 1 SKIP.
  PUT STREAM PrnLibStream UNFORMATTED
...
PROCEDURE get-dtl-vars :
  ...
  DO ON ERROR UNDO, RETURN :
    ...
    ASSIGN p-wealth-name   = ( IF AVAILABLE buf-wealth  THEN buf-wealth.wth-name   ELSE "":U ).
    if not buf-wealth.is-money then do:
