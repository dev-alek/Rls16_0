...
PROCEDURE adm-disable :
    DISABLE RECT-12 Classify Itog Zero R-coll E-coll WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-12 Classify Itog Zero R-coll E-coll WITH FRAME F-Main.
...
DO:
  ...
  IF r-coll = 1  THEN DO:
    ...
    for each ub.season no-lock where
       ...
       e-coll = e-coll + ub.season.sea-name + chr(10)  .
    end.
  END.
  ELSE DO:
...
PROCEDURE local-apply-layout :
 ...
 R-coll:RADIO-BUTTONS   in frame F-Main  = substitute("&1,1,&2,2", "Все " + v-par-val ,  'Выборочно'  ) .
  e-coll:READ-ONLY IN FRAME F-Main = true .
  R-coll = 1.
...
If x-date-alone <> v-today Then do:
      if x-SelectGood = 1 Then DO:
          If x-SET_val_TYPE = 1
            then DO :
                run rep/r-prtsz5.p
                    (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input zero,  input no ,  input Itog ,   input true ,   input table tt-season ) .
                End.
            Else do:
                run rep/r-prtsz6.p
                    (input v-cntxt-obj-code ,  input v-cntxt-obj-type ,  input base-type ,  input base-code ,  input Classify,  input zero,  input no ,  input Itog ,   input true ,   input table tt-season ) .
