...
define variable v-host-city   as   character                      no-undo .
...
define variable v-line        as   character                      no-undo format "x(196)":U .
...
assign
  v-line        = fill("-", 196 )
  v-header-name = "                        Ж У Р Н А Л   П О С Т У П И В Ш И Х   Н Е Ф Т Е П Р О Д У К Т О В   З А   П Е Р И О Д"
.
...
if (valid-handle(g#lib-calc) <> true) then do:   run str/lib-calc.p persistent no-error .   if error-status :error or (valid-handle(g#lib-calc) <> true) then do:     message       "Error starting lib-calc.p" skip       g#lib-calc skip       g#lib-calc :type skip       g#lib-calc :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-calc_chkqtpl in g#lib-calc
  ...
  if ptrlprop-inpptrl = 'weight':U then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  :
    put stream PrnLibStream unformatted
      ...
      "                        Ж У Р Н А Л   П О С Т У П И В Ш И Х   Н Е Ф Т Е П Р О Д У К Т О В   З А   П Е Р И О Д" skip( 1 )
...
:
  ...
  find first ub.clients-attr no-lock
    where ub.clients-attr.obj-type   = ub.trn-doc.cli-type
      ...
      and ub.clients-attr.attr-code  = 'shftrep2':U
  ...
  for each doc-line-attr no-lock
     where doc-line-attr.doc-code  = ub.doc-line.doc-code
       and doc-line-attr.gds-code  = gds-list.gds-code :
    if entry(1, doc-line-attr.attr-code, chr(4)) = "ttn-temp":U then do :
      if num-entries (doc-line-attr.attr-code, chr(4)) > 1 then do :
        v-n-section = integer (  entry(2, doc-line-attr.attr-code, chr(4))  ) no-error .
  ...
  :
    ...
    assign
      ...
      v-doc-qnty    = ub.doc-pl.cli-doc-qnty
      v-fact-qnty   = ub.doc-pl.cli-fact-qnty
      varinaccuracy = ub.doc-pl.cli-doc-qnty * v-pogresh
      varmeasure    = ( if available bef-rvs-line and
                          available aft-rvs-line
                        then ( aft-rvs-line.measure-cli-qnty - bef-rvs-line.measure-cli-qnty )
                        else ? )
      vardifference = varmeasure - ub.doc-pl.cli-doc-qnty
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run usrnick in g#library
  ...
  ,output v-oper-name
    ...
    display stream PrnLibStream
      ...
      sym17
      sym18
    with frame doc-line-frm-cli .
    down stream PrnLibStream 1 with frame doc-line-frm-cli .
  end.
end.
if v-ind > 0 then do:
  ...
  :
    ...
    assign
      varshift-id   = ( if x-radio-task <= 2 then string( ub.trn-doc.fact-date  )
                                                  ...
                                                  string( ub.trn-doc.shift-date )
      v-doc-qnty    = ub.doc-pl.doc-qnty
      v-fact-qnty   = ub.doc-pl.fact-qnty
      varinaccuracy = ub.doc-pl.doc-qnty * v-pogresh
      varmeasure    = ( if available bef-rvs-line and
                          available aft-rvs-line
                        then ( aft-rvs-line.measure-qnty - bef-rvs-line.measure-qnty )
                        else ? )
      vardifference = varmeasure - ub.doc-pl.doc-qnty
      v-tot-doc-qnty   = v-tot-doc-qnty   + v-doc-qnty
      v-tot-fact-qnty  = v-tot-fact-qnty  + v-fact-qnty
      v-tot-measure    = v-tot-measure    + varmeasure
      v-tot-difference = v-tot-difference + vardifference
