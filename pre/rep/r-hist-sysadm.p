...
v-report-name-html-list = session:temp-directory + "rpt" + string(p-report-id) + "_hist" + ".html".
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run tblnmusr in g#library
  ...
  then do :
    assign
      ...
      tt-usr-hist.head-table    = entry(5, bf_c-user-log.head-table-key, chr(6))
      tt-usr-hist.uniq-key-rec  = entry(4, bf_c-user-log.des, ";")
    .
    if entry(5, bf_c-user-log.head-table-key, chr(6)) = 'autofree':U
    then
      tt-usr-hist.head-table = tt-usr-hist.head-table + " - " + entry(6, bf_c-user-log.head-table-key, chr(6))
    .
    if entry(15, bf_c-user-log.head-table-key, chr(6)) = "del"
    then
