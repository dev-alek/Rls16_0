...
:
    ...
    put stream excel-line unformatted
        ...
        CHR(9)   buf_temp_line-data.d_desk
        CHR(9)   buf_temp_line-data.d_kkm_prod
        CHR(9)   buf_temp_line-data.d_kkm_reg
        CHR(9)   buf_temp_line-data.d_z
        chr(10)
    .
end.
end procedure.
...
END.
   ...
   find first This_Object  WHERE This_Object.obj-type = obj-list.obj-type
                           ...
                           no-lock.
   find first clients      WHERE clients.obj-type     = 'орг':U
...
do on error undo, return error return-value  :
  ...
  if length(v-pril-kass) > 18 then do:
   ...
   run km7xl-write-cell-data in this-procedure (
       input "h_pril_kass1":U
       , input string( v-pril-kass1 )
   ).
   run km7xl-write-cell-data in this-procedure (
       input "h_pril_kass2":U
   ...
   PUT STREAM Out-Stream
       ...
       space(5) Line           format "X(140)"  "по ОКРО" format "X(7)" AT 167 "|"  AT 177 t-okpo format "X(16)" "|" AT 194 skip
       space(30) "организация, адрес, номер телефона" format "X(120)"                                            "+----------------+" AT 177 skip
       space(5) v-object format "X(120)"                                    "ИНН|" AT 174  t-INN format "X(15)"  "|" AT 194 skip
...
do on error undo, return error return-value  :
   run  rep/wp-qnty.p ( TRUNCATE(v-summ-return,0), output PropisSumAll  ).
   if PropisSumAll = ''
   ...
   END.
      ...
      o-head-position = mCashBook:getSinglRule(0 , This_Object.obj-type, This_Object.obj-code, "ManagerPosition") .
      o-director      = mCashBook:getSinglRule(0 , This_Object.obj-type, This_Object.obj-code, "ManagerFIO") .
