...
def var   name-Type     as   integer no-undo.
...
DEFINE VARIABLE Rs-name AS INTEGER
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS
          "Полное название товара", 1,
"Название на этикетке", 2
     SIZE 32 BY 2.5 NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     Rs-name AT ROW 4.75 COL 3.5 NO-LABEL WIDGET-ID 2
...
PROCEDURE adm-disable :
    DISABLE RECT-1 RECT-3 MainTypes Rs-name WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN RECT-1 RECT-3 MainTypes Rs-name WITH FRAME F-Main.
...
END PROCEDURE.
PROCEDURE my-report :
run rep/r-prcsigar.p
    ( ChosedType ,
      name-Type   ).
END PROCEDURE.
PROCEDURE my-var :
    ChosedType = integer(MainTypes:screen-value in frame F-Main).
    name-type = int(rs-name:screen-value) .
END PROCEDURE.
