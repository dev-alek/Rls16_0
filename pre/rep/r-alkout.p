...
header
  ...
  v-line format "X(114)" at 1
with width 136 down stream-io.
define frame BottomFrame with width 136 down stream-io.
form header
        v-line format "X(114)" at 1 SKIP
...
:
  ...
  assign
    ...
    temp-str.bottling-date = buf_parts.alc-bottling-date
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  find first buf_our_host   no-lock
       where buf_our_host.obj-type = 'орг':U
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input t-doc.doc-code ,
  ...
  put stream out-stream
    string("РАСПОРЯЖЕНИЕ НА СКЛАД № " + string( t-doc.doc-code )) at 49 format "X(40)"  v-str-date at right-field( 114 , length( v-str-date ) ) format "X(20)" skip
    buf_our_host.obj-name at right-field( 114 , length( buf_our_host.obj-name ) ) skip
    ...
    v-auto at right-field( 114 , length( v-auto ) ) format "X(40)" skip
  .
end.
end procedure.
...
:
  ...
  put stream out-stream v-line format "X(114)" at 1 skip.
  display stream out-stream
  ...
  with frame f-doc.
  put stream out-stream v-line format "X(114)" at 1 skip(2)
