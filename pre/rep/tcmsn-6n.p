...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pftxvalg in g#library
  ...
  for each temp-gds-list no-lock
    , First goods     where goods.gds-code  = temp-gds-list.gds-code no-lock
      BREAK BY temp-gds-list.vat-pc  BY temp-gds-list.gds-name :
    run item-goods ( "6" , "goods" ) .
      if return-value <> "" then NEXT.
        If Last-of(temp-gds-list.vat-pc) Then Do:
        If String(Entry(2,"temp-gds-list.vat-pc",".")) = "Grp-name"
          Then  Assign
                  S-bar-code   = " Итого по "
...
if x-selectobject = "currency":u  or  xtog-obj then do :
define variable first-l23 as logical   no-undo .
  first-l23 = true .
      for  each gds-obj where
...
if x-selectobject = "currency":u or xtog-obj then do:
define variable first-l25 as logical   no-undo .
  first-l25 = true .
  for  each gds-obj
...
IF x-SelectObject = "currency":U  OR  xTog-obj then do :
define variable first-l27 as logical   no-undo .
  first-l27 = true .
  for each gds-obj
    where gds-obj.obj-type = x-store-type
      and gds-obj.obj-code = x-store-code
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pftxvalg in g#library
  ...
  for each temp-gds-list no-lock
    , First gds-list  where gds-list.gds-code  = temp-gds-list.gds-code no-lock
      BREAK BY temp-gds-list.vat-pc  BY gds-list.gds-name :
    run item-goods ( "6" , "gds-list" ) .
      if return-value <> "" then NEXT.
        If Last-of(temp-gds-list.vat-pc) Then Do:
        If String(Entry(2,"temp-gds-list.vat-pc",".")) = "Grp-name"
