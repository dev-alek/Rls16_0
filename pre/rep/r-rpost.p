...
define variable vss-workfile    as character no-undo init "$Workfile: r-rpost.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-rpost.p $":U .
define variable vss-description as character no-undo init "Оборот  расхода ".
...
view stream outstream frame bottomframe .
  ...
  case retsorttype :
      ...
      if not can-find(first g#post no-lock)
          then   run rep/r-rpost1.p (x-store-code, x-store-type, x-base-type , x-base-code ).
          else   run rep/r-rpost2.p (x-store-code, x-store-type, x-base-type , x-base-code ).
      when "sort-code":u  then
      if not can-find(first g#post no-lock)
          then   run rep/r-rpost3.p (x-store-code, x-store-type, x-base-type , x-base-code ).
          else   run rep/r-rpost4.p (x-store-code, x-store-type, x-base-type , x-base-code ).
  end case.
