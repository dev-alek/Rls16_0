...
def var vss-workfile    as character no-undo init "$Workfile: rbblsts.p $":U .
def var vss-archive     as character no-undo init "$Archive: rep/rbblsts.p $":U .
...
if not can-find(first scnblist) then do:
  message
    "Список пустой"
...
FOR EACH scnblist USE-INDEX art NO-LOCK:
  assign
  v-ind = v-ind + 1
  .
  ...
  FIND buf_clients WHERE
     buf_clients.obj-type = scnblist.prod-type
 AND buf_clients.obj-code = scnblist.prod-code NO-LOCK .
  CREATE gds-zap .
  assign
  gds-zap.unit-base  = scnblist.unit-base
  gds-zap.prt-root     = scnblist.prt-root
  gds-zap.gds-name = scnblist.gds-name
  gds-zap.prod-type  = scnblist.prod-type
  gds-zap.prod-code = scnblist.prod-code
  gds-zap.artic          = scnblist.artic
  gds-zap.prod-name = buf_clients.obj-name
  gds-zap.grp-name = scnblist.grp-name
  gds-zap.order-num  = scnblist.order-num
  gds-zap.qnty     = scnblist.qnty
  .
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  (input  scnblist.gds-code
  ,input  ?
  ,output gds-zap.b-code
  )  .
  assign
    gds-zap.weight-bc = ""
  .
  FIND buf_units WHERE
      buf_units.unit-name = scnblist.unit-base NO-LOCK.
  ...
  gds-zap.b-str     = (if gds-zap.weight-bc <> ? and gds-zap.weight-bc <> '':U then '':U else scnblist.b-str).
END.
...
CASE p-Classify :
  when "Classify_1" then do:
    FOR EACH gds-zap BREAK BY  (if p-sorttype = "artic":U                     then gds-zap.artic                     else (if p-sorttype = "b-code":U                           then string( gds-zap.b-code, "9999999999" )                           else string(gds-zap.order-num, "9999999999"))                           ) :
  ...
  when "Classify_2" then do:
    ...
    BY (if p-sorttype = "artic":U                     then gds-zap.artic                     else (if p-sorttype = "b-code":U                           then string( gds-zap.b-code, "9999999999" )                           else string(gds-zap.order-num, "9999999999"))                           ) :
