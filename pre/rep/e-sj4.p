...
define variable vss-workfile    as character no-undo init "$Workfile: e-sj4.p $":u .
define variable vss-archive     as character no-undo init "$Archive: rep/e-sj4.p $":u .
...
if v-curr-r-b = 'base':U then do:
  ...
  with FRAME BottomFramebasesj-base-d-t width  232 PAGE-BOTTOM NO-LABELS NO-BOX .
  VIEW STREAM PrnLibStream FRAME BottomFramebasesj-base-d-t .
end.
else do:
  FORM HEADER
  line format "X(196)"
  AT 1 SKIP
  string( "Продолжение - на следующей странице" ) FORMAT "X(35)" AT 30 SKIP
  with FRAME BottomFramerublsj-base-d-t width 232 PAGE-BOTTOM NO-LABELS NO-BOX .
    VIEW STREAM PrnLibStream FRAME BottomFramerublsj-base-d-t .
end.
CASE my-Set_Val_Type :
  when 2 then
      FORM with FRAME sj-base-d-t .
  when 3 then
      FORM with FRAME sj-full-d-t .
END CASE .
assign
p-frame-width = frame sj-base-d-t:width.
CASE SHRS-by :
...
if first( sj-goods.obj-attr ) then do:
  if v-curr-r-b = 'base':U then do:
                        if my-set_val_type = 2  then
                            DOWN STREAM PrnLibStream 1 with FRAME sj-base-d-t .
                        else
                            DOWN STREAM PrnLibStream 1 with FRAME sj-full-d-t .
  end.
  else do:
                            DOWN STREAM PrnLibStream 1 with FRAME sj-base-d-t .
  end.
...
if v-curr-r-b = 'base':U then do:
                        ...
                        sj-goods.artic
                        sj-goods.name
                        with FRAME sj-base-d-t .
                        else
                        UNDERLINE STREAM PrnLibStream
                        sj-goods.artic
                        sj-goods.name
                        with FRAME sj-full-d-t .
end.
else do:
                        UNDERLINE STREAM PrnLibStream
                        sj-goods.artic
      ...
      if first-of( sj-goods.grp-name  ) then do:
        if v-curr-r-b = 'base':U then do:
          if my-Set_Val_Type = 2 then do:
            if frame sj-base-d-t:line = 0 then do:
              down 1 stream PrnLibStream
              with frame sj-base-d-t.
            end.
          end.
          else do:
            if frame sj-full-d-t:line = 0 then do:
              down 1 stream PrnLibStream
              with frame sj-full-d-t.
            end.
          end.
...
define variable vss-include-info29 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    ...
    .
IF FIRST-OF(sj-adv.discnt) then do:
      chk-gds-lines = 0.
end.
  ...
  if last-of (sj-adv.discnt) then  do:
    ...
    CASE my-Set_Val_Type :
      when 2 then  do:
        ...
        ACCUM SUB-TOTAL BY  ( sj-adv.discnt )  sj-adv.qnty-2 @ sj-adv.qnty-2
        ACCUM SUB-TOTAL BY  ( sj-adv.discnt )  sj-adv.qnty-3 @ sj-adv.qnty-3
        sj-adv.price
