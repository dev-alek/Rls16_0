...
:
    ...
    put stream excel-line unformatted
        ...
        chr(9)   buf_temp_line-data.num
        chr(9)   buf_temp_line-data.artic
        chr(9)   buf_temp_line-data.name
        chr(9)   buf_temp_line-data.numblank
        chr(9)   buf_temp_line-data.numsertif
        chr(9)   buf_temp_line-data.date1
        chr(9)   buf_temp_line-data.date2
        chr(9)   buf_temp_line-data.orgsertif
        chr(9)   buf_temp_line-data.srok
        chr(9)   buf_temp_line-data.datestart
        chr(9)   buf_temp_line-data.techusl
        chr(10)
    .
end.
end procedure.
...
def var v-bar-code            like bar-code.b-code        no-undo.
...
assign
    ...
    .
find first ub.clients no-lock where ub.clients.obj-code = buf_firm.tobj-code and ub.clients.obj-type = 'чел':U no-error .
...
run r-sertxl-write-cell-data in this-procedure (
  ...
  , input substitute( "к накладной N &1 от &2 (&3)", v-doc-code, v-doc-date, buf_trn-doc.status_)
    ...
    form header
        ...
        with frame BottomFrame width 235 page-bottom no-box .
...
.
run r-sertxl-write-cell-data in this-procedure ( input "f_OrgName":U, input string("Оригиналы сертификатов или их заверенная копия находятся у: " + v-org-name)).
run r-sertxl-write-cell-data in this-procedure ( input "f_contact":U, input string("Уполномоченный представитель " + v-contact-name + "                                           М.П.")).
