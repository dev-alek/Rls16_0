...
define variable  xserv as character init 'все':U no-undo.
...
define buffer buf_aht-stk-line  for ub.aht-stk-line .
 find last  ub.aht-stk-line where
                  ...
                  and   ub.aht-stk-line.sum-type   = "c"
                        use-index category no-lock no-error.
 find last  buf_aht-stk-line where
                        buf_aht-stk-line.gds-code   = gds-zap-b-code
                  and   buf_aht-stk-line.fact-order <= fact-order-1
                  and   buf_aht-stk-line.obj-code   = p-store-code
                  and   buf_aht-stk-line.obj-type   = p-store-type
                  and   buf_aht-stk-line.sum-type   = "b"
                        use-index category no-lock no-error.
        if available ub.aht-stk-line
           and available buf_aht-stk-line
        then do:
            if  tprintrubl  then
                  assign
                        ostatok-start[1]  = ostatok-start[1] + round(  ub.aht-stk-line.fact-qnty,3)
                        ostatok-start[2]  = ostatok-start[2] + round( ub.aht-stk-line.cost-sum-rubl ,2)
                        ostatok-start[3]  = ostatok-start[3] + round( ub.aht-stk-line.cost-vat-rubl ,2)
                        ostatok-start[4]  = ostatok-start[4] +  0
                        ostatok-start[5]  = ostatok-start[5] + round( buf_aht-stk-line.crsa-sum-rubl ,2) + round( ub.aht-stk-line.crsa-sum-rubl ,2)
                        ostatok-start[6]  = ostatok-start[6] + round( buf_aht-stk-line.crsa-vat-rubl ,2) + round( ub.aht-stk-line.crsa-vat-rubl ,2)
                        ostatok-start[7]  = ostatok-start[7] + round( buf_aht-stk-line.sale-discnt-rubl ,2)
                        ostatok-start[8]  = ostatok-start[8] + round( buf_aht-stk-line.crsa-sum-rubl ,2)
                        ostatok-start[9]  = ostatok-start[9] + round( buf_aht-stk-line.crsa-vat-rubl ,2)
                        ostatok-start[10] = ostatok-start[10] + round( buf_aht-stk-line.crsa-slt-rubl ,2)
                        .
              else
                  assign
                        ostatok-start[1]  = ostatok-start[1] + round( ub.aht-stk-line.fact-qnty,3)
                        ostatok-start[2]  = ostatok-start[2] + round( ub.aht-stk-line.cost-sum-base ,2)
                        ostatok-start[3]  = ostatok-start[3] + round( ub.aht-stk-line.cost-vat-base ,2)
                        ostatok-start[4]  = ostatok-start[4] + 0
                        ostatok-start[5]  = ostatok-start[5] + round( buf_aht-stk-line.crsa-sum-base ,2) + round( ub.aht-stk-line.crsa-sum-base ,2)
                        ostatok-start[6]  = ostatok-start[6] + round( buf_aht-stk-line.crsa-vat-base ,2) + round( ub.aht-stk-line.crsa-vat-base ,2)
                        ostatok-start[7]  = ostatok-start[7] + round( buf_aht-stk-line.sale-discnt-base ,2)
                        ostatok-start[8]  = ostatok-start[8] + round( buf_aht-stk-line.crsa-sum-base ,2)
                        ostatok-start[9]  = ostatok-start[9] + round( buf_aht-stk-line.crsa-vat-base ,2)
                        ostatok-start[10] = ostatok-start[10] + round( buf_aht-stk-line.crsa-slt-base ,2)
                  .
             end.
 find last  ub.aht-stk-line where
                        ub.aht-stk-line.gds-code   = gds-zap-b-code
 ...
 find last  buf_aht-stk-line where
                         buf_aht-stk-line.gds-code   = gds-zap-b-code
                  and    buf_aht-stk-line.fact-order <= fact-order-2
                  and    buf_aht-stk-line.obj-code   = p-store-code
                  and    buf_aht-stk-line.obj-type   = p-store-type
                  and    buf_aht-stk-line.sum-type   = "b"
                        use-index category no-lock no-error.
        if available ub.aht-stk-line and
           available buf_aht-stk-line then do:
            if  tprintrubl  then
                  assign
                        ostatok-end[1]  = ostatok-end[1] + round( ub.aht-stk-line.fact-qnty,3)
                        ostatok-end[2]  = ostatok-end[2] + round( ub.aht-stk-line.cost-sum-rubl ,2)
                        ostatok-end[3]  = ostatok-end[3] + round( ub.aht-stk-line.cost-vat-rubl ,2)
                        ostatok-end[4]  = ostatok-end[4] + 0
                        ostatok-end[5]  = ostatok-end[5] + round( buf_aht-stk-line.crsa-sum-rubl ,2) + round( ub.aht-stk-line.crsa-sum-rubl ,2)
                        ostatok-end[6]  = ostatok-end[6] + round( buf_aht-stk-line.crsa-vat-rubl ,2) + round( ub.aht-stk-line.crsa-vat-rubl ,2)
                        ostatok-end[7]  = ostatok-end[7] + round( buf_aht-stk-line.sale-discnt-rubl ,2)
                        ostatok-end[8]  = ostatok-end[8] + round( buf_aht-stk-line.crsa-sum-rubl ,2)
                        ostatok-end[9]  = ostatok-end[9] + round( buf_aht-stk-line.crsa-vat-rubl ,2)
                        ostatok-end[10] = ostatok-end[10] + round( buf_aht-stk-line.crsa-slt-rubl ,2)
                        .
              else
                  assign
                        ostatok-end[1]  = ostatok-end[1] + round( ub.aht-stk-line.fact-qnty,3)
                        ostatok-end[2]  = ostatok-end[2] +  round( ub.aht-stk-line.cost-sum-base ,2)
                        ostatok-end[3]  = ostatok-end[3] +  round( ub.aht-stk-line.cost-vat-base ,2)
                        ostatok-end[4]  = ostatok-end[4] +  0
                        ostatok-end[5]  = ostatok-end[5] +  round( buf_aht-stk-line.crsa-sum-base ,2) + round( ub.aht-stk-line.crsa-sum-rubl     , 2)
                        ostatok-end[6]  = ostatok-end[6] +  round( buf_aht-stk-line.crsa-vat-base ,2) + round( ub.aht-stk-line.crsa-vat-base , 2)
                        ostatok-end[7]  = ostatok-end[7] +  round( buf_aht-stk-line.sale-discnt-base ,2)
                        ostatok-end[8]  = ostatok-end[8] +  round( buf_aht-stk-line.crsa-sum-base ,2)
                        ostatok-end[9]  = ostatok-end[9] +  round( buf_aht-stk-line.crsa-vat-base ,2)
                        ostatok-end[10] = ostatok-end[10] + round( buf_aht-stk-line.crsa-slt-base ,2)
                  .
             end.
end.
end procedure.
...
procedure lavel1 :
              case select-good :
                  when 1  then do:
    define buffer buf_obj-list24 for obj-list.
  for each buf_obj-list24 no-lock :
      if xtog-obj and  not (buf_obj-list24.obj-type = x-store-type  and
                            buf_obj-list24.obj-code = x-store-code )
      then next.
        for  each gds-obj   where
                            gds-obj.obj-type = buf_obj-list24.obj-type    and
                            gds-obj.obj-code = buf_obj-list24.obj-code
                            and ( (v-show-all-goods = true ) or   ( gds-obj.last-doc = ? or gds-obj.last-doc >= x-date-start or gds-obj.fact-qnty <> 0 or gds-obj.avrg-qnty <> 0 or gds-obj.fact-sale <> 0 or gds-obj.fact-base <> 0 ) )
                            no-lock ,
        first goods where gds-obj.gds-code  = goods.gds-code
                            no-lock :
  ...
  run foreach in this-procedure .
   ...
   if last-of( temp-gds-list.grp-name ) then do:
      ...
      run clear-b2 in this-procedure .
                  ...
                  when 2  then do:
    define buffer buf_obj-list25 for obj-list.
  for each buf_obj-list25 no-lock :
      if xtog-obj and  not (buf_obj-list25.obj-type = x-store-type  and
                            buf_obj-list25.obj-code = x-store-code )
      then next.
        for  each gds-obj   where
                            gds-obj.obj-type = buf_obj-list25.obj-type    and
                            gds-obj.obj-code = buf_obj-list25.obj-code
                            and ( (v-show-all-goods = true ) or   ( gds-obj.last-doc = ? or gds-obj.last-doc >= x-date-start or gds-obj.fact-qnty <> 0 or gds-obj.avrg-qnty <> 0 or gds-obj.fact-sale <> 0 or gds-obj.fact-base <> 0 ) )
                              no-lock ,
        first  tmp#grp
...
procedure display-str1  :
         ...
         if xshowsale    then do: run di ( "комисс." , 8,"","","","","" ).  end.
         if vat-cost    then do: run di ( "уч.НДС", 3,"","","","","" ). end.
         if vat-crsa    then do: run di ( "пр.НДС", 6,"","","","","" ).  end.
         if vat-sale    then do: run di ( "км.НДС", 9,"","","","","" ).  end.
end procedure.
procedure display-bi  :
         ...
         if xshowsale    then do: run di ("комисс." , 8, "","", "", "",  "bi":u).  end.
         if vat-cost    then do: run di ( "уч.НДС", 3,"","","","","bi":u ). end.
         if vat-crsa    then do: run di ( "пр.НДС", 6,"","","","","bi":u ).  end.
         if vat-sale    then do: run di ( "км.НДС", 9,"","","","","bi":u ).  end.
end procedure.
procedure display-bo  :
         ...
         if xshowsale    then do: run di ("комисс." , 8, "","", "", "",  "bo":u).  end.
         if vat-cost    then do: run di ( "уч.НДС", 3,"","","","","bo":u ). end.
         if vat-crsa    then do: run di ( "пр.НДС", 6,"","","","","bo":u ).  end.
         if vat-sale    then do: run di ( "км.НДС", 9,"","","","","bo":u ).  end.
end procedure.
procedure display-b1  :
      if not( not show-negativ-2 and
        ...
        if xshowsale    then do: run di in this-procedure ("комисс." , 8, "","", "", "", "b1":u).  end.
        ...
        if vat-sale    then do: run di in this-procedure ( "км.НДС", 9,"","","","","b1":u ).  end.
...
procedure display-b2  :
     if not( not show-negativ-2 and
        ...
        if xshowsale    then do: run di ("комисс.", 8 ,"","", "", "", "b2":u).  end.
         if vat-cost    then do: run di ( "уч.НДС", 3,"","","","","b2":u ). end.
         if vat-crsa    then do: run di ( "пр.НДС", 6,"","","","","b2":u ).  end.
         if vat-sale    then do: run di ( "км.НДС", 9,"","","","","b2":u ).  end.
 end.
end.
end procedure.
procedure clear-b1  :
 ...
 if x-sum-type = arh-type-sale then tt# = 6 .
    ...
    case ub.aht-ot-line.ext-doc-type:
              ...
              assign
                     ...
                     prih[8 ]   = prih[8]   +  if tprintrubl then ub.aht-ot-line.crsa-sum-rubl else  ub.aht-ot-line.crsa-sum-base
              ...
              do:
                assign
                    ...
                    rash[8 ]   = rash[8]   +  if tprintrubl then ub.aht-ot-line.crsa-sum-rubl - ub.aht-ot-line.sale-slt-rubl  else  ub.aht-ot-line.crsa-sum-base - ub.aht-ot-line.sale-slt-base
              ...
              do:
                  assign
                    ...
                    kassa[8 ]   = kassa[8]   +  if tprintrubl then ub.aht-ot-line.crsa-sum-rubl - ub.aht-ot-line.crsa-slt-rubl else  ub.aht-ot-line.crsa-sum-base - ub.aht-ot-line.crsa-slt-base
          ...
          when       'vp':U       then
              ...
              assign
                ...
                inv[8 ]   = inv[8]   +  if tprintrubl then ub.aht-ot-line.crsa-sum-rubl else  ub.aht-ot-line.crsa-sum-base
          ...
          when       'ap':U then
              do:
                 assign
                    ...
                    overturn[8 ]   = overturn[8]   +  if tprintrubl then ub.aht-ot-line.crsa-sum-rubl else  ub.aht-ot-line.crsa-sum-base
