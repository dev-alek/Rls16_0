...
:
    ...
    put stream excel-line unformatted
        ...
        CHR(9)   "DTA":U
        CHR(9)   buf_temp_sheet1_line-data.artic
        CHR(9)   buf_temp_sheet1_line-data.barcode
        CHR(9)   buf_temp_sheet1_line-data.gdsname
        CHR(9)   buf_temp_sheet1_line-data.prodname
        CHR(9)   buf_temp_sheet1_line-data.unitname
        CHR(9)   buf_temp_sheet1_line-data.price
        CHR(9)   buf_temp_sheet1_line-data.unitqnty
        CHR(9)   buf_temp_sheet1_line-data.unitprice
        chr(10)
    .
end.
end procedure.
...
define frame optprc
  ...
  tt-optprc.artic                 no-label format "X(16)":U                   space(0)
  sym2                            no-label format "X(1)"                          space(0)
  tt-optprc.b-code-str            no-label format "X(10)":U                   space(0)
  sym3                            no-label format "X(1)"                          space(0)
  tt-optprc.gds-name              no-label format "X(30)":U                   space(0)
  sym4                            no-label format "X(1)"                          space(0)
  tt-optprc.prod-name             no-label format "X(20)":U                   space(0)
  sym5                            no-label format "X(1)"                          space(0)
  tt-optprc.unit-name             no-label format "X(4)":U                   space(0)
  sym6                            no-label format "X(1)"                          space(0)
  tt-optprc.price-str             no-label format "X(14)":U                   space(0)
  sym7                            no-label format "X(1)"                          space(0)
  tt-optprc.unit-qnty-str         no-label format "X(14)":U                   space(0)
  sym8                            no-label format "X(1)"                          space(0)
  tt-optprc.unit-price-str        no-label format "X(14)":U                   space(0)
  sym9                            no-label format "X(1)"                          space(0)
with width 131 down stream-io no-label no-box.
form header
      ":----------------:----------:------------------------------:--------------------:----:--------------:--------------:--------------:":U skip
      ":       1        :    2     :               3              :           4        : 5  :      6       :      7       :      8       :":U skip
      ":----------------:----------:------------------------------:--------------------:----:--------------:--------------:--------------:":U
with frame HeaderFrame width 198 PAGE-TOP NO-LABELS NO-BOX .
form header
  ...
  ,output v-excise
    ...
    then do:
      ...
      assign
        ...
        buf_tt-optprc.b-code-str     = string(base-bar-code.b-code, "999999999":U)
        ...
        buf_tt-optprc.price-str      = string(v-price-sale, ">>>,>>>,>>9.99":U)
        buf_tt-optprc.unit-qnty      = base-bar-code.cli-base-rate
        buf_tt-optprc.unit-price     = v-price-sale
        buf_tt-optprc.unit-price-str = string(v-price-sale, ">>>,>>>,>>9.99":U)
      .
      assign
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  ...
  ,output v-excise
        ...
        assign
          ...
          buf_tt-optprc.b-code-str     = string(buf_bar-code.b-code, "999999999":U)
          buf_tt-optprc.gds-name       = buf_tt-goods.gds-name
          buf_tt-optprc.grp-code       = buf_tt-goods.grp-code
          buf_tt-optprc.grp-name       = buf_tt-goods.grp-name
          buf_tt-optprc.prod-type      = buf_tt-goods.prod-type
          ...
          buf_tt-optprc.price-str      = string(buf_tt-optprc.price, ">>>,>>>,>>9.99":U)
          buf_tt-optprc.unit-qnty      = buf_bar-code.cli-base-rate
          buf_tt-optprc.unit-price     = v-price-sale
          buf_tt-optprc.unit-price-str = string(buf_tt-optprc.unit-price, ">>>,>>>,>>9.99":U)
        .
        assign
          buf_tt-optprc.unit-qnty-str  = get-unit-qnty-str(buf_tt-optprc.unit-qnty)
        .
...
:
  ...
  :
    ...
    assign
      v-gds-name-lines  = length(tt-optprc.gds-name)  / 30
      v-prod-name-lines = length(tt-optprc.prod-name) / 20
    .
    ...
    then do:
      ...
      :
        assign
          v-gds-name  = substring(tt-optprc.gds-name, 30 * (v-i - 1) + 1, 30)
          v-prod-name = substring(tt-optprc.prod-name, 20 * (v-i - 1) + 1, 20)
        .
...
:
  ...
  put stream out-stream unformatted
    ...
    ":----------------:----------:------------------------------:--------------------:----:--------------:--------------:--------------:":U skip
 .
  run optprcxl-write-cell-data in this-procedure ( input "h_org":U
                                                 , input v-org-str
                                                 ).
  run optprcxl-write-cell-data in this-procedure ( input "h_time":U
                                                 , input v-curr-time-str
                                                 ).
  run optprcxl-write-cell-data in this-procedure ( input "h_currency":U
...
function get-unit-qnty-str returns character (p-unit-qnty as decimal):
  ...
  then do:
    ...
    then do:
        assign
          v-str = string(v-qnty-int , ">,>>>,>>>,>>>9":U)
        .
    end.
    else do:
        assign
          v-str = string(p-unit-qnty , ">>>,>>>,>>9.99":U)
        .
    end.
  end.
  else do:
    assign
      v-str = string(p-unit-qnty , ">,>>>,>>>,>>>9":U)
    .
  end.
  return v-str.
end.
