    ...
    :
        ...
        then do:
            ...
            find first buf_tt-allsum-line
                    where buf_tt-allsum-line.sum-type = 'основная_сумма':U
            .
            assign
...
:
    ...
    put stream out-stream
        skip
        p-fmt-align-string( v-host-name, 124, 'right':U )
                                                    format "X(124)"
        skip
        p-fmt-align-string( v-obj-name, 124, 'right':U )
                                                    format "X(124)"
        skip(2) space( 5 )
            "Срок оплаты: ___________________"
        skip(1) space( 5 )
            "Менеджер:    "
            buf_temp_manager.manager-name           format "X(124)"
        skip(2)
            p-fmt-align-string( substitute( "С П Р А В К А    &1", p-trn-doc-code ) , 124, 'center':U )
                                                    format "X(124)"
        skip(1) space( 5 )
            p-fmt-align-string( v-supp-name, 124, 'center':U )
                                                    format "X(124)"
        skip    space( 5 )
            p-fmt-align-string( v-supp-inn, 124, 'center':U )
                                                    format "X(124)"
        skip(1) space( 5 )
            "Товар на сумму: "
            string( v-sum-all, ">>>>>>>>>9.99" )   format "X(13)"
            space( 1 )
            ( if PrintRubl then "РУБ" else "баз.вал" )
    .
    if PrintRubl
    then do:
        run rep/wp-rub.p (
