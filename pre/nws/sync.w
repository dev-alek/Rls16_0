...
procedure create-routes :
  ...
  run nws/cr-route.p ( input 'send-tbl':U, input 'db':U, input (buffer ub.db:handle), input string(p-dbnum) ) no-error.
  if error-status :error then do:
    return error return-value.
  end.
  for each ub.db-attr no-lock where ub.db-attr.db-num = ub.db.db-num :
    ...
    if p-news = true then do:
      run nws/cr-route.p ( input 'send-tbl':U, input 'db-attr':U, input (buffer ub.db-attr:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
  ...
  run nws/cr-route.p ( input 'send-tbl':U, input 'clients':U, input (buffer ub.clients:handle), input string(p-dbnum) ) no-error.
  if error-status :error then do:
    return error return-value.
  end.
  find first ub.shop no-lock where ub.shop.obj-code = ub.clients.obj-code .
  run nws/cr-route.p ( input 'send-tbl':U, input 'shop':U, input (buffer ub.shop:handle), input string(p-dbnum) ) no-error.
  if error-status :error then do:
    return error return-value.
  end.
  for each ub.clients-attr no-lock where  ub.clients-attr.obj-type = ub.clients.obj-type
    ...
    if  p-news then do:
      run nws/cr-route.p ( input 'send-tbl':U, input 'clients-attr':U, input (buffer ub.clients-attr:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
  ...
  for each ub.ext-system no-lock where ub.ext-system.esys-db-num-exp = p-dbnum or ub.ext-system.esys-db-num-imp = p-dbnum :
    run nws/cr-route.p ( input 'send-tbl':U, input 'ext-system':U, input (buffer ub.ext-system:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
    for each ub.ext-system-attr no-lock where ub.ext-system-attr.esys-id  = ub.ext-system.esys-id
      ...
      run ext-system-attr-news in this-procedure ( input ub.Ext-system-attr.esya-attr-code
                                                  ,output p-news).
      if  p-news then do:
        run nws/cr-route.p ( input 'send-tbl':U, input 'ext-system-attr':U, input (buffer ub.ext-system-attr:handle), input string(p-dbnum) ) no-error.
        if error-status :error then do:
          return error return-value.
        end.
      end.
  ...
  for each ub.user-login-action-role no-lock where ub.user-login-action-role.db-num = p-dbnum :
    run nws/cr-route.p ( input 'send-tbl':U, input 'user-login-action-role':U, input (buffer ub.user-login-action-role:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.action-role no-lock where ub.action-role.db-num = p-dbnum :
    run nws/cr-route.p ( input 'send-tbl':U, input 'action-role':U, input (buffer ub.action-role:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.action-role-item no-lock where ub.action-role-item.db-num = p-dbnum :
    run nws/cr-route.p ( input 'send-tbl':U, input 'action-role-item':U, input (buffer ub.action-role-item:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.schedule no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'schedule':U, input (buffer ub.schedule:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
    for each ub.schedule-attr no-lock where ub.schedule-attr.cre-db-num = ub.schedule.cre-db-num
      ...
      run nws/cr-route.p ( input 'send-tbl':U, input 'schedule-attr':U, input (buffer ub.schedule-attr:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
  ...
  for each ub.config no-lock where ub.config.db-num = p-dbnum :
    run nws/cr-route.p ( input 'send-tbl':U, input 'config':U, input (buffer ub.config:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.thbj-attr no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'thbj-attr':U, input (buffer ub.thbj-attr:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  ...
  for each ub.pay-type no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'pay-type':U, input (buffer ub.pay-type:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.cash-pay no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'cash-pay':U, input (buffer ub.cash-pay:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.cash-pay-attr no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'cash-pay-attr':U, input (buffer ub.cash-pay-attr:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.tax no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'tax':U, input (buffer ub.tax:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.tax-rate no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'tax-rate':U, input (buffer ub.tax-rate:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.tax-rate-value no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'tax-rate-value':U, input (buffer ub.tax-rate-value:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.tax-units no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'tax-units':U, input (buffer ub.tax-units:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.currency no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'currency':U, input (buffer ub.currency:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.curr-bank no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'curr-bank':U, input (buffer ub.curr-bank:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.curr-shop no-lock where ub.curr-shop.obj-type = v-curr-obj-type
    ...
    if Year(ub.curr-shop.exch-date ) <> 9999 then do:
      run nws/cr-route.p ( input 'send-tbl':U, input 'curr-shop':U, input (buffer ub.curr-shop:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
  ...
  for each ub.country no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'country':U, input (buffer ub.country:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.regions no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'regions':U, input (buffer ub.regions:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.cash-desk no-lock where ub.cash-desk.db-num = p-dbnum :
    run nws/cr-route.p ( input 'send-tbl':U, input 'cash-desk':U, input (buffer ub.cash-desk:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.cash-desk-attr no-lock where ub.cash-desk-attr.db-num = p-dbnum :
    run nws/cr-route.p ( input 'send-tbl':U, input 'cash-desk-attr':U, input (buffer ub.cash-desk-attr:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.cashbookrule no-lock where ub.CashBookRule.Obj-type = v-curr-obj-type
    ...
    run nws/cr-route.p ( input 'send-tbl':U, input 'CashBookRule':U, input (buffer ub.cashbookrule:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.OperServ no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'OperServ':U, input (buffer ub.OperServ:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
    for each ub.goods-attr exclusive-lock where ub.goods-attr.attr-code = 'oper-serv-idd':U
      ...
      run nws/cr-route.p ( input 'send-tbl':U, input 'goods-attr':U, input (buffer ub.goods-attr:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
  ...
  for each ub.OperServAttr no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'OperServAttr':U, input (buffer ub.OperServAttr:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  ...
  for each ub.dis-card-type no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-card-type':U, input (buffer ub.dis-card-type:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
    assign
    ...
    for each buf_rp-by-call where buf_rp-by-call.call_id = ub.dis-card-type.uniq-key-rec :
      run nws/cr-route.p ( input 'send-tbl':U, input 'rp-by-call':U, input (buffer buf_rp-by-call:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
    for each buf_rule-by-call where buf_rule-by-call.call_id = ub.dis-card-type.uniq-key-rec :
      run nws/cr-route.p ( input 'send-tbl':U, input 'rule-by-call':U, input (buffer buf_rule-by-call:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
    for each buf_rule-call-param where buf_rule-call-param.call_id = ub.dis-card-type.uniq-key-rec :
      run nws/cr-route.p ( input 'send-tbl':U, input 'rule-call-param':U, input (buffer buf_rule-call-param:handle), input string(p-dbnum) ) no-error.
      if error-status :error then do:
        return error return-value.
      end.
    end.
  ...
  for each ub.dis-card-mask no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-card-mask':U, input (buffer ub.dis-card-mask:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.dis-card no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-card':U, input (buffer ub.dis-card:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
    run str/saledc.p
  ...
  for each ub.dis-host no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-host':U, input (buffer ub.dis-host:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.dis-card-property no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-card-property':U, input (buffer ub.dis-card-property:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.dis-rule no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-rule':U, input (buffer ub.dis-rule:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.dis-gds-rule no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-gds-rule':U, input (buffer ub.dis-gds-rule:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
  for each ub.dis-time-rule no-lock :
    run nws/cr-route.p ( input 'send-tbl':U, input 'dis-time-rule':U, input (buffer ub.dis-time-rule:handle), input string(p-dbnum) ) no-error.
    if error-status :error then do:
      return error return-value.
    end.
  end.
