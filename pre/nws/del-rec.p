...
define input parameter p-find-msg    as logical   no-undo .
...
:
  ...
  if bh_tbl-name:available then do:
    ...
    then do:
      assign
        v-msg     = substitute( "&1&2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  )
        v-deleted = false
      .
    end.
  end.
