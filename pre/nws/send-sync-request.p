...
:
  ...
  assign
    file-info:file-name = v-source-dir + chr(92) + v-pack-name
  .
  ...
  assign v-req = "needsync" + chr(4) + string(buf-src_db.db-num) + chr(4) + string(v-real-last-sent-pck) + chr(4) + string(v-real-last-rcv-pck) + chr(4) + string(now) .
  copy-lob from v-req to vMem .
  v-req64 = base64-encode(vMem) .
  copy-lob from v-req64 to file (v-source-dir + chr(92) + v-pack-name) .
  run nws/s-g-pack.p
    ( input "put":U
     ,input "7zip":U
     ,input v-pack-name
