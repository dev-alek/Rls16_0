...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1':u,p-key-rec)
    .
end procedure.
...
:
  ...
  if error-status:error then do:
    undo, return  error substitute( "&1. Ошибка при создании temp-table &2 &3&4&5", vss-workfile, p-table-name, return-value, chr(10), error-status :get-message ( error-status :num-messages ) ).
  end.
  ...
  if error-status:error then do:
    undo, return  error substitute( "&1. Ошибка при создании записи в temp-table &2 &3&4&5", p-table-name, vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) ).
  end.
  ...
  if error-status:error then do:
    undo, return  error substitute( "&1. Ошибка при поиcке записи &2 в БД  &3&4&5", p-table-name, vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) ).
  end.
  ...
  if error-status:error then do:
    undo, return  error substitute( "&1. Ошибка при копировании записи &2 в БД  &3&4&5", p-table-name, vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) ).
  end.
  glog = v-bh:buffer-release no-error .
  if error-status:error then do:
    undo, return  error substitute( "&1. Ошибка при сохранении записи &2 в БД  &3&4&5", p-table-name, vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) ).
  end.
