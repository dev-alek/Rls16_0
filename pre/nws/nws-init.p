...
:
  ...
  :
    ...
    if nws-exch-dir = ? then do:
      assign
        v-str-err = v-str-err + substitute( "Отсутствует настройка каталога СПН (nws-exch-dir).&1", chr(10) )
      .
    end.
    else do:
      assign
      ...
      then do:
        assign
          v-str-err = v-str-err + substitute( "Каталог &1 отсутствует.&2", nws-exch-dir, chr(10) )
        .
      end.
      else do:
        assign
    ...
    if nws-heap-dir = ? then do:
      assign
        v-str-err = v-str-err + substitute( "Отсутствует настройка каталога СПН (nws-heap-dir).&1", chr(10) )
      .
    end.
    else do:
      assign
      ...
      then do:
        assign
          v-str-err = v-str-err + substitute( "Каталог &1 отсутствует.&2", nws-heap-dir, chr(10) )
        .
      end.
      else do:
        assign
          nws-heap-dir = file-info:full-pathname
          log-file-name = nws-heap-dir + chr(92) + "news.log"
        .
      end.
    end.
    assign
