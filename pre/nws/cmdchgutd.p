...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3':u,buf_utd.db-num,buf_utd.doc-id,buf_utd.sts)
    .
end procedure.
...
define buffer buf_clients for ub.clients.
  ...
  run begin-create-command in v-cmd-proc-handle
    (input  'cmd-chg-utd-sts':U + chr(6) + string(buf_utd.db-num) + chr(6) + string(buf_utd.doc-id) + chr(6) + string(buf_utd.sts) + chr(6) + string(buf_utd.sts-edi) + chr(6) + string(buf_utd.doc-code)
    ,input "":U
    ,output v-cmd-code1
    ) no-error.
  if error-status :error
