...
:
  ...
  on error undo, return error
  :
    file-info:file-name = v-path + chr(47) + buf_tt-ext-file.file-name.
    if FILE-INFO:FULL-PATHNAME = ? then do:
    ...
    if buF_tt-ext-file.file-name = buf_tt-ext-file.file-type   then do:
      ...
      buf_tt-ext-file.status_ = 'auto':U
      .
    end.
  end.
end.
