...
DEFINE NEW GLOBAL SHARED VAR adm-broker-hdl    AS HANDLE  NO-UNDO.
    ASSIGN adm-object-hdl = FRAME nws-prp:HANDLE.
RUN ensure-broker.
...
PROCEDURE adm-disable :
    DISABLE b-exit b-help WITH FRAME nws-prp.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN b-exit b-help WITH FRAME nws-prp.
...
PROCEDURE view-page :
  ...
  RUN broker-view-page IN adm-broker-hdl (INPUT THIS-PROCEDURE,
      INPUT p-page#).
  END PROCEDURE.
ON WINDOW-CLOSE OF FRAME nws-prp
DO:
  APPLY "END-ERROR":U TO SELF.
END.
define variable vss-include-info0 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame nws-prp
do:
  run gbl/app_help.p
...
on choose of b-help in frame nws-prp
do:
  apply "help":u to frame nws-prp .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame nws-prp:width - 0.3
                fh            = frame nws-prp:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME nws-prp:PARENT eq ?
THEN FRAME nws-prp:PARENT = ACTIVE-WINDOW.
MAIN-BLOCK:
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ON END-KEY UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK:
  RUN dispatch IN THIS-PROCEDURE ('initialize':U).
  WAIT-FOR GO OF FRAME nws-prp.
END.
