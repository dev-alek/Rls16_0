...
define input  parameter p-pack-name   as   character            no-undo .
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3':u,p-db-num,p-pack-num,p-source-dir,p-pack-name)
    .
end procedure.
...
def var vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  procedure nws-exp :
    ...
    do
    on error  undo, return error substitute( "&1 (nws-exp). &2&3&4", vss-include-info4, return-value, chr(10), error-status :get-message ( 1 ) )
    on stop   undo, return error substitute( "&1 (nws-exp). stop", vss-include-info4 )
    on endkey undo, return error substitute( "&1 (nws-exp). endkey", vss-include-info4 )
    :
      ...
      on error undo, return error substitute( "&1 (nws-exp). &2", vss-include-info4, error-status :get-message ( 1 ) )
      :
          assign
            v-fh = p-tbl-handle:buffer-field(v-ind).
          if v-fh:data-type = 'raw':U then do:
...
mFramBachModHandle = frame exp-pck:handle.
...
if mBatchMode = ? then do:
 ...
 if  log-manager:logfile-name ne ?
  ...
  assign
    ...
    v-pck-full-name = p-source-dir + chr(92) + p-pack-name
    v-tmp-file      = p-source-dir + chr(92) + substring( p-pack-name, 1, r-index( p-pack-name, '.':u) ) + 'tmp':U
  .
define variable vss-include-info9 as character format "x(65)" no-undo initial "@(#)$Workfile:currsysk.i $ $Revision: $".
...
def var vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  output stream ddl to value( v-tmp-file ).
  assign
    rec-cnt       = 1
    lin-cnt       = 2
    v-uniq-key-rt = "":U
...
def var vss-include-info11 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7&2&8&2&9", 'pck-conf', chr(1), '', '', '', '', 0, 0, rec-cnt) .
  ...
  on error  undo, return error substitute( "&1 (for each ub.pck-sent). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
...
def var vss-include-info12 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7&2&8&2&9", 'pck-sent', chr(1), '', '', '', '', 0, 0, rec-cnt) .
    run nws-exp in this-procedure
  ...
  for each ub.pck-rcvd no-lock
     where ub.pck-rcvd.db-num     = p-db-num
       and ub.pck-rcvd.rcvd       = no
       and ub.pck-rcvd.total-recs = ub.pck-rcvd.rcvd-recs
  on error  undo, return error substitute( "&1 (for each ub.pck-rcvd). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
...
def var vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7&2&8&2&9", 'pck-rcvd', chr(1), '', '', '', '', 0, 0, rec-cnt) .
    run nws-exp in this-procedure
...
def var vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7&2&8&2&9", 'pck-rcvd-attr', chr(1), '', '', '', '', 0, 0, rec-cnt) .
    run nws-exp in this-procedure
      ( input (buffer ub.pck-rcvd-attr:handle)
      ) .
    display
      rec-cnt
      lin-cnt
  ...
  for each buf_route no-lock
     where buf_route.db-num    = p-db-num
       and buf_route.last-pack = buf_pck-sent.pack-num
     use-index pi
  on error  undo, return error substitute( "&1 (for each buf_route). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
  ...
  :
      ...
      if not available ub.route then do:
        if locked ub.route then do:
          run write-to-log( substitute( "&1. Формирование пакета прервано на захваченной записи route (&2)", vss-workfile, buf_route.name-rec )
                            + chr(10) + substitute( "recid route: &1 уникальный ключ записи: &2 ", recid( buf_route ), buf_route.uniq-key-rec )
                          ).
      ...
      assign
        v-uniq-key-rt = substitute("&1&2&3", ub.route.tbl-ord, chr(5), ub.route.dump-ord )
      .
      case entry(1,ub.route.name-rec,chr(1)) :
        when "delete"
        or when "create"
        or when "command"
        or when "get-seq"
...
def var vss-include-info15 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7", ub.route.name-rec, chr(1), ub.route.uniq-gate-rec, v-uniq-key-rt, ub.route.num-dump, 0, rec-cnt ) .
        end.
        otherwise do :
          assign
            rec-cnt = rec-cnt + 1
      ...
      for each ub.route-dump exclusive-lock
          where ub.route-dump.dump-ord = ub.route.dump-ord
          use-index pi
      on error  undo, return error substitute( "&1 (for each ub.route-dump). &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
...
def var vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7&2&8&2&9", ub.route-dump.dump-name, chr(1), ub.route-dump.action, ub.route.uniq-key-rec, ub.route-dump.uniq-gate-rec, v-uniq-key-rt, ub.route.num-dump, ub.route-dump.rec-ord, rec-cnt) .
        end.
        else do:
           assign
          ...
          if ub.route-dump.uniq-gate-rec = '':U then do:
            ...
            then do:
              return error substitute( "&1. Ошибка при создании временной таблицы &2 (1).&3&4", vss-workfile, tt-name, chr(10), error-status:get-message(1) ).
            end.
            assign
            ...
            if v-ok <> true
              or error-status :error
            then do:
              return error substitute( "&1. Ошибка при создании временной таблицы &2 (2).&3&4", vss-workfile, tt-name, chr(10), error-status:get-message(1) ).
            end.
            assign
          ...
          then do:
            ...
            return error substitute( "&1. Ошибка при создании буфера временной &2 .&3&4", vss-workfile, tt-name, chr(10), error-status:get-message(1) ).
          end.
          assign
  ...
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7&2&8&2&9", ub.route-dump.dump-name, chr(1), ub.route-dump.action, ub.route.uniq-key-rec, ub.route-dump.uniq-gate-rec, v-uniq-key-rt, ub.route.num-dump, ub.route-dump.rec-ord, rec-cnt) .
    run nws-exp in this-procedure
      ( input bh_tt
          ...
          then do:
            run all-gates-clear in this-procedure ( buffer buf_temp-xml-tables).
            return error substitute( "&1. Ошибка при очистке временной временной &2 .&3&4", vss-workfile, tt-name, chr(10), error-status:get-message(1) ).
          end.
          if ub.route-dump.uniq-gate-rec = '':U then do:
            delete object tth .
          end.
...
def var vss-include-info18 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  export stream ddl substitute( "&1&2&3&2&4&2&5&2&6&2&7&2&8&2&9", 'pck-sent', chr(1), '', '', '', '', 0, 0, rec-cnt) .
    run nws-exp in this-procedure
      ( input (buffer buf_pck-sent:handle)
      ) .
    end.
    assign
      rec-cnt               = rec-cnt + 1
...
def var vss-include-info20 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if v-err <> 0 then do:
    ...
    if error-status :error then do:
      run write-to-log( vss-workfile + chr(32) + substitute( "&1", return-value ) ).
      return error return-value .
    end.
    return .
  end.
