...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3':u,p-full-cmd,p-counter,p-db-source)
    .
end procedure.
...
:
  assign
    v-command = entry( 1, p-full-cmd, chr(6) )
  .
  ...
  case v-command
  :
    when 'cmd-request-goods':U
    then do:
      assign
        v-obj-type = entry( 2, p-full-cmd, chr(6) )
        v-obj-code = integer(entry( 3, p-full-cmd, chr(6) ))
      .
      run nws/cmdsndgd.p
        ( input p-imp-handle
         ,input p-counter
      ...
      then do:
        ...
        return error substitute( "&1. ошибка при вызове процедуры cmdsndgd.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-transfer-goods':U
    then do:
      assign
        v-obj-type = entry( 2, p-full-cmd, chr(6) )
        v-obj-code = integer(entry( 3, p-full-cmd, chr(6) ))
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmdcmpgd.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-parts-split':U
    then do:
      run nws/cmdpart.p
        ( input p-imp-handle
         ,input p-counter
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmdpart.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-imp-rec-without-trg':U
    then do:
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmd-imp-rec-without-trg! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-trn-doc-fact':U
    then do:
      define variable v-type-trn as character no-undo .
      define variable v-doc-code-trn as character no-undo .
      assign
        v-type-trn     = entry( 2, p-full-cmd, chr(6) )
        v-doc-code-trn = entry( 3, p-full-cmd, chr(6) )
      .
      run nws/cmd-trni.p
        ( input p-imp-handle
         ,input p-counter
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmd-trni.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-rcv-doc-rcv':U
    then do:
      ...
      assign
        v-type-rcv     = entry( 1, p-full-cmd, chr(6) )
        v-doc-code-rcv = entry( 2, p-full-cmd, chr(6) )
        v-rcv-code-rcv = entry( 3, p-full-cmd, chr(6) )
      .
      run nws/cmd-rcvi.p
        ( input p-imp-handle
         ,input p-counter
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmd-rcvi.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-s-f-doc-fact':U
    then do:
      ...
      assign
        v-type-s-f     = entry( 1, p-full-cmd, chr(6) )
        v-doc-code-s-f = entry( 2, p-full-cmd, chr(6) )
        v-db-num-s-f   = entry( 3, p-full-cmd, chr(6) )
      .
      run nws/cmd-s-fi.p
        ( input p-imp-handle
         ,input p-counter
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmd-s-fi.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-pdf-fact':U
    then do:
      ...
      assign
        v-type-pdf     = entry( 2, p-full-cmd, chr(6) )
        v-1            = int(entry( 3, p-full-cmd, chr(6) ))
        v-2            = int(entry( 4, p-full-cmd, chr(6) ))
        v-3            = int(entry( 5, p-full-cmd, chr(6) ))
        v-4            = int(entry( 6, p-full-cmd, chr(6) ))
      .
      run nws/cmd-pdfi.p
        ( input p-imp-handle
         ,input p-counter
    ...
    when 'cmd-del-rec-without-trg':U
    then do:
      ...
      if error-status :error
      then do:
        return error substitute( "&1. ошибка при вызове процедуры cmd-del-rec-without-trg! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-send-binary':U then do:
      assign
        v-mode = entry( 2, p-full-cmd, chr(6) )
        v-file-num = integer(entry( 3, p-full-cmd, chr(6) ))
        v-file-name = entry( 4, p-full-cmd, chr(6) )
        v-path-type = integer(entry( 5, p-full-cmd, chr(6) ))
        v-path = entry( 6, p-full-cmd, chr(6) )
        v-md5-signature = entry( 7, p-full-cmd, chr(6) )
      .
      define variable v-ok as logical no-undo .
      run nws/bin-i.p
        ( input auto-window-h
    ...
    when 'cmd-send-lob':U then do:
      assign
        v-table-name = entry( 2, p-full-cmd, chr(6) )
        v-db-num =   integer(entry( 3, p-full-cmd, chr(6) ))
        v-int64-id = int64(entry( 4, p-full-cmd, chr(6) ))
      .
      run nws/lob-i.p
        ( input auto-window-h
         ,input this-procedure:handle
    ...
    then do:
      ...
      assign
      v-step          = integer(entry( 2, p-full-cmd, chr(6) ))
      v-source-db-num = integer(entry( 3, p-full-cmd, chr(6) ))
      v-obj-type = entry( 4, p-full-cmd, chr(6) )
      v-obj-code = integer(entry( 5, p-full-cmd, chr(6) ))
      v-doc-code = entry( 6, p-full-cmd, chr(6) )
      v-doc-type = entry( 7, p-full-cmd, chr(6) )
      v-ext-doc-type = entry( 8, p-full-cmd, chr(6) )
      v-doc-date = date(entry( 9, p-full-cmd, chr(6) ))
      v-fact-date = date(entry( 10, p-full-cmd, chr(6) ))
      v-sign =  integer(entry( 11, p-full-cmd, chr(6) ))
      cre-pay =  integer(entry( 12, p-full-cmd, chr(6) ))
      .
      run nws/cmdp-dc.p
        ( input auto-window-h
         ,input this-procedure:handle
    ...
    then do:
      ...
      assign
      v-emitent-host-code = integer(entry( 2, p-full-cmd, chr(6) ))
      v-type              = entry( 3, p-full-cmd, chr(6) )
      .
      run nws/cmd-dct.p
        ( input auto-window-h
         ,input this-procedure:handle
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmd-dct.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-rum-send':U
    then do:
      define variable v-uniq-key-rec as character no-undo .
      v-uniq-key-rec = str-decode(entry(2, p-full-cmd, chr(6))
                                 ,"").
      run nws/cmd-rum.p
        ( input auto-window-h
         ,input this-procedure:handle
      ...
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmd-rum.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-nws2esys-general':U
    then do:
      define variable v-esys-id as integer no-undo .
      define variable v-db-num-exp as integer no-undo .
      define variable v-esys-uniq-gate-rec as character no-undo .
      assign
      v-esys-id = integer(entry(2, p-full-cmd, chr(6)))
      v-db-num-exp = integer(entry(3, p-full-cmd, chr(6)))
      v-esys-uniq-gate-rec = str-decode(entry(4, p-full-cmd, chr(6)) , '')
      .
      run nws/cmdnw2es.p
        ( input auto-window-h
         ,input this-procedure:handle
         ...
         ,input v-esys-uniq-gate-rec
         ,input p-counter
        ) no-error .
      if error-status :error
      then do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при вызове процедуры cmdnw2es.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    when 'cmd-chg-utd-sts':U
    then do:
...
run gbl/getobjsrvhndl.p (input-output ObjSrv).
      assign
        v-db-num-utd = integer (entry( 2, p-full-cmd, chr(6) ))
        v-doc-id     = integer (entry( 3, p-full-cmd, chr(6) ))
        v-sts        = integer (entry( 4, p-full-cmd, chr(6) ))
        v-sts-edi    = integer (entry( 5, p-full-cmd, chr(6) ))
        v-doc-code   = (entry( 6, p-full-cmd, chr(6) ))
      no-error.
      if error-status:error
        then
      do:
        if valid-handle(v_dataseth) then do:                    run gate-clear in this-procedure ( input v_dataseth                                                     , input (buffer buf_temp-xml-tables:handle)).                   end.
        return error substitute( "&1. ошибка при обработке комманды на изменения статуса УТД! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
...
procedure cmd-imp-rec-without-trg :
  ...
  :
    ...
    :
      ...
      if error-status :error
      then do:
        return error substitute( "&1. ошибка при вызове процедуры cmdintrg.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    run waitfram-hide .
  end.
...
procedure cmd-del-rec-without-trg :
  ...
  :
    ...
    :
      ...
      if error-status :error
      then do:
        return error substitute( "&1. ошибка при вызове процедуры cmd-dtrg.p! &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message( error-status :num-messages )  ).
      end.
    end.
    run waitfram-hide .
  end.
