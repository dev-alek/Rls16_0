...
END.
ON CHOOSE OF b-sch IN FRAME v-route
DO:
...
on f5 of frame v-route anywhere
do:
  run reopen-query in this-procedure ( input true, input false, input '' ) .
    apply "VALUE-CHANGED" to br-route.
end.
...
define input parameter p-filter-name as character no-undo .
  do with frame v-route:
    if p-filter-name > "" then do:
      assign
        frame v-route:title
          = frame v-route:title + "   ФИЛЬТР: " + p-filter-name.
      .
      assign
...
PROCEDURE enable_UI :
  ENABLE b-quit b-sch b-help br-route
      WITH FRAME v-route.
  VIEW FRAME v-route.
  OPEN QUERY br-route for each X_route no-lock   WHERE X_route.db-num = p-db-num   by X_route.tbl-ord .
END PROCEDURE.
PROCEDURE MyEnable :
  assign
...
PROCEDURE reopen-query :
  ...
  assign
    ...
    frame v-route:title = substitute( "&1", v-title0 )
  .
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-16 as character no-undo .
      ...
      parameter-4-16 =
        (
          if ("x_route.db-num = p-db-num                         " + " " + where-phrase-16) <> ""
          then substitute( 'x_route.db-num = &2                                       '                                       ,chr(34)                                       ,p-db-num                                     )                          + " " + where-phrase-16
          else "true"
        )
      parameter-5-16 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-16 =
          ("x_route.db-num = p-db-num                         " + " " + where-phrase-16 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-route :handle
  ...
  if l-filter-open-16 = false then do:
    open query br-route for each x_route
  ...
  assign
    v-recid = recid( x_route )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-route :handle:get-buffer-handle(1) = (buffer x_route:handle) then do:
      assign
      parameter-2-16 = (if p-find-next then "true":u else "false":u )
      parameter-4-16 =
        "where ":u + substitute( 'x_route.db-num = &2                                       '                                       ,chr(34)                                       ,p-db-num                                     )                          + " ":u + where-phrase-16 + " ":u + p-find-condition + " " + ""
      parameter-5-16 = " "
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input no-lock
                          ,input (buffer x_route:handle)
                          ,input parameter-4-16
                          ,input parameter-5-16
                          ) no-error.
      .
      assign
        v-recid = integer(return-value)
        v-fltopend-rowid[1] = ?
      .
    end.
    else do:
      assign
      parameter-2-16 = (if p-find-next then "true":u else "false":u )
      parameter-3-16 =  "for each x_route"
      parameter-4-16 =
        (
          if ("x_route.db-num = p-db-num                         " + " " + where-phrase-16) <> ""
          then substitute( 'x_route.db-num = &2                                       '                                       ,chr(34)                                       ,p-db-num                                     )                          + " " + where-phrase-16
  ...
  if p-open-query = false then do:
    if v-recid <> ? then do:
      reposition br-route to recid v-recid no-error .
    end.
    else do:
      message
        substitute("!") skip
...
FUNCTION get-descr-route returns character
  ...
  case entry( 1, p-name-rec, chr(1) ):
    when "delete":u then do:
      assign
        v-route-descr = "(удаление)" + chr(32)
        v-delim-key   = chr(1)
        v-key-rec     = substring( p-name-rec, index( p-name-rec, chr(1) ) + 1 )
      .
    end.
    when "command":u then do:
      case entry( 2, p-name-rec, chr(1) ):
        when "delete":u then do:
          assign
            v-route-descr = "(удаление)" + chr(32)
            v-delim-key   = chr(3)
            v-key-rec     = entry( 3, p-name-rec, chr(1) )
          .
        end.
        otherwise do:
          assign
        ...
        if entry( 3, p-name-rec, v-delim-key ) = "ren-art":u then do:
          assign
            v-route-descr = v-route-descr
                            ...
                            + entry( 4, p-name-rec, v-delim-key ) + chr(32)
                            + "ст. арт." + chr(32) + entry( 5, p-name-rec, v-delim-key ) + chr(32)
                            + "ст. тип пр." + chr(32) + entry( 6, p-name-rec, v-delim-key ) + chr(32)
                            + "ст. код пр." + chr(32) + entry( 7, p-name-rec, v-delim-key ) + chr(32)
                            + "н. арт." + chr(32) + entry( 8, p-name-rec, v-delim-key ) + chr(32)
                            + "н. тип пр." + chr(32) + entry( 9, p-name-rec, v-delim-key ) + chr(32)
                            + "н. код пр." + chr(32) + entry( 10, p-name-rec, v-delim-key )
          .
        end.
      end.
      otherwise do:
