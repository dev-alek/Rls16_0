...
:
  ...
  assign
    v-curr-db      = buf_sys-ctrl.db-num
    v-action       = entry(3, p-command, chr(1))
    v-uniq-key-rec = entry(4, p-command, chr(1))
    v-db-init      = integer( entry(5, p-command, chr(1)) )
    v-parameters   = entry(6, p-command, chr(1))
  .
  ...
  if v-curr-db = 0 then do:
    ...
    :
      assign
        v-db-num-char = entry( v-ind, v-send-db-list, chr(44) )
      .
