...
for each old-rename-fld  no-lock on error undo, return error SUBSTITUTE("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2)):
   create new-rename-fld.
   buffer-copy old-rename-fld to new-rename-fld.
end.
for each old-rename-fld-attr  no-lock on error undo, return error SUBSTITUTE("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2)):
   create new-rename-fld-attr.
   buffer-copy old-rename-fld-attr to new-rename-fld-attr.
end.
