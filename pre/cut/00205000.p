...
for each old-upgrade  no-lock on error undo, return error SUBSTITUTE("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2)):
   create new-upgrade.
   buffer-copy old-upgrade to new-upgrade.
end.
for each old-upgrade-attr  no-lock on error undo, return error SUBSTITUTE("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2)):
   create new-upgrade-attr.
   buffer-copy old-upgrade-attr to new-upgrade-attr.
end.
