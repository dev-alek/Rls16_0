...
for each old-config  no-lock on error undo, return error SUBSTITUTE("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2)):
   create new-config.
   buffer-copy old-config to new-config.
end.
  if varstay-history then do:
for each old-c-config  no-lock on error undo, return error SUBSTITUTE("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2)):
   create new-c-config.
   buffer-copy old-c-config to new-c-config.
end.
  end.
    ...
    :
      find first new-db-attr exclusive-lock
        where new-db-attr.db-num    = old-db.db-num
          and new-db-attr.attr-code = 'cut-fin-date':U
        no-error .
      if not available new-db-attr then do:
        create new-db-attr .
        assign
          new-db-attr.db-num    = old-db.db-num
          new-db-attr.attr-code = 'cut-fin-date':U
        .
      end.
      assign
...
for each old-sys-ctrl-attr  no-lock on error undo, return error SUBSTITUTE("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2)):
   create new-sys-ctrl-attr.
   buffer-copy old-sys-ctrl-attr to new-sys-ctrl-attr.
end.
