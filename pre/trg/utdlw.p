...
trigger procedure for write of ub.utd-lines
  new buffer new-utd-lines
  old buffer old-utd-lines
.
...
define variable vss-description as character no-undo init "Тригер изменение utd-lines".
if new-utd-lines.gds-code eq 0
then
   new-utd-lines.gds-code = ?.
...
define buffer buf_c-utd-lines  for ub.c-utd-lines .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define buffer buf_c-utd-head for ub.c-utd-head .
      buffer-compare new-utd-lines to old-utd-lines case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("utd-lines","s-c-utd-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-utd-lines)
       then do:
 for first buf_c-utd-lines where buf_c-utd-lines.db-num eq new-utd-lines.db-num
and buf_c-utd-lines.doc-id eq new-utd-lines.doc-id
and buf_c-utd-lines.LineNum eq new-utd-lines.LineNum
  and buf_c-utd-lines.corr-user-db-num   = g#db-num and buf_c-utd-lines.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-utd-lines where buf_c-utd-lines.db-num eq old-utd-lines.db-num
and buf_c-utd-lines.doc-id eq old-utd-lines.doc-id
and buf_c-utd-lines.LineNum eq old-utd-lines.LineNum
  and buf_c-utd-lines.corr-user-db-num   = g#db-num and buf_c-utd-lines.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-utd-lines)
   then do:
      if not available buf_c-utd-lines
      then do:
         create buf_c-utd-lines.
      buffer-copy new-utd-lines to buf_c-utd-lines
      assign
         buf_c-utd-lines.chip-num           = v-Seq
         buf_c-utd-lines.corr-date          = v-date
         buf_c-utd-lines.corr-time          = v-time
         buf_c-utd-lines.corr-user-db-num   = g#db-num
         buf_c-utd-lines.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-utd-lines.action             = 1
         buf_c-utd-lines.is-del             = false
      .
      end.
      else
         if buf_c-utd-lines.action eq 99
         then
            buf_c-utd-lines.action = 2.
   end.
   else do:
      if not available buf_c-utd-lines
      then do:
         create buf_c-utd-lines.
         buffer-copy old-utd-lines to buf_c-utd-lines
         assign
            buf_c-utd-lines.chip-num           = v-Seq
            buf_c-utd-lines.corr-date          = v-date
            buf_c-utd-lines.corr-time          = v-time
            buf_c-utd-lines.corr-user-db-num   = g#db-num
            buf_c-utd-lines.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-utd-lines.action             = 2 when buf_c-utd-lines.action ne 1
            buf_c-utd-lines.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-utd-lines)
         then do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq new-utd-lines.db-num
and buf_c-utd-head.doc-id eq new-utd-lines.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq old-utd-lines.db-num
and buf_c-utd-head.doc-id eq old-utd-lines.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      ...
      then do:
         ...
         buffer-copy  buf_c-utd-lines to buf_c-utd-head
         assign
            buf_c-utd-head.subject = "utd-lines"
