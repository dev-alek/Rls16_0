...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  and old-doc.status_ = 'акт':U
  and ub.price-doc.status_ <> 'акт':U then do:
    message
      vss-workfile vss-revision vss-description skip
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdburt in g#library
  (output ub.price-doc.user-db-num
  ,output ub.price-doc.user-name
  ,output ub.price-doc.sys-date
  ,output ub.price-doc.sys-time
  ,output ub.price-doc.sys-time-int
  )  .
  end.
  if ub.price-doc.status_ = 'приказ':U then do:
  ...
  if ub.price-doc.status_ = 'акт':U then do:
    run change-status-act in this-procedure
      (input old-doc.status_ = 'разрешен':U
      ) no-error .
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
  ...
  if not g#news and ub.price-doc.status_ <> 'новый':U then do:
    run trg/pr-doch.p
      (buffer old-doc
      ,buffer ub.price-doc
      ) no-error .
...
procedure change-status-act :
  ...
  :
    ...
    if not g#news then do:
      ...
      assign
        ub.price-doc.fact-num = next-value (s-trn-fact, ub)
      .
      define variable v-fact-order           as decimal no-undo .
      define variable v-shift-end-fact-order as decimal no-undo .
      define variable v-day-end-fact-order   as decimal no-undo .
  ...
  ,output l-shift-on
    ...
    :
      ...
      if not available buf_temp-goods then do:
        ...
        .
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  (input  buf_temp-goods.gds-code
  ,input  ?
  ,output buf_temp-goods.root-b-code
  )  .
      end.
    ...
    :
      ...
      run prdoclib-process-goods in this-procedure
        ...
        ,input  true
        ,input  l-check-price-parts
        ,input  ub.price-doc.doc-num
        ,input  ub.price-doc.fact-date
        ,input  ub.price-doc.user-db-num
        ,input  ub.price-doc.user-name
        ,input  ub.price-doc.sys-date
        ,input  ub.price-doc.sys-time-int
        ,input  ub.price-doc.sys-time
        ,output v-gds-obj-fact-qnty
        ) no-error .
      if error-status :error then do:
        message
          vss-workfile vss-revision vss-description skip
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjat in g#library
  ...
  ,input  ub.price-list.prod-type
  ,input  ub.price-list.prod-code
  ,input  v-set-attribute
  ,output l-return-attribute
  ) no-error .
          if error-status :error then do:
            message
              vss-workfile vss-revision vss-description skip
  ...
  ,input ''
    ...
    if not g#news then do:
      ...
      find price-doc_clients no-lock
        ...
        .
      if (ub.price-doc.status_ = 'приказ':U
          and g#db-num = 0
          and ub.clients.db-num <> 0
         )
      or (ub.price-doc.status_ = 'акт':U
          and g#db-num <> 0
         )
      then do:
        run show-action in this-procedure
    ...
    :
      ...
      if not available ub.goods then do:
        message
          ...
          "Артикул" ub.price-list.artic ub.price-list.prod-type ub.price-list.prod-code skip
          "" (if g#db-num = 0
              then "Если товар был переименован," + chr(10) + "необходимо принять новости в УБД и переформировать пакеты"
              else ""
            ) skip
          view-as alert-box error .
        undo, return error .
