...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1':u,pBatchProcess#)
    .
end procedure.
...
then do:
  ...
  if BatchProcess.BP_Type <> 'trnhd':U
  then do:
    message
      vss-workfile vss-revision vss-description skip
  ...
  for each BatchProcess no-lock
    where BatchProcess.bp_type   = 'trnhd':U
...
procedure process-BatchProcess :
  ...
  define variable bpr_lprocessedok as logical  init false .
  big_loop:
  do transaction
  on error undo big_loop, return error
  :
    find first buf_batchprocess exclusive-lock
      where rowid( buf_batchprocess) = rowid(pbuf_batchprocess)
      no-error
      no-wait
      .
    ...
    if buf_batchprocess.bp_status <> 'N':U then do:
      find current buf_batchprocess no-lock .
      undo big_loop, return error .
    end.
    ...
    assign
      buf_batchprocess.bp_status         = 'X':U
      buf_batchprocess.bp_execcounttries = buf_batchprocess.bp_execcounttries + 1
      buf_batchprocess.bp_execuser_id    = g#userid
      buf_batchprocess.bp_execsysdate    = v-btpr_upd-today-1
      buf_batchprocess.bp_execsystime    = string(v-btpr_upd-time-1, 'hh:mm')
      buf_batchprocess.bp_execsystimeint = v-btpr_upd-time-1
    .
    find current buf_batchprocess no-lock .
  end.
  ...
  do transaction
  :
    find current buf_batchprocess exclusive-lock
      no-error .
    ...
    find current buf_batchprocess no-lock no-error .
  end.
end procedure.
