...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1', new-sr-izmerenia.node-code)
    .
end procedure.
...
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
   new-sr-izmerenia.node-code = if new-sr-izmerenia.node-code eq 0 then next-value (s-sr-izmerenia, ub) else new-sr-izmerenia.node-code .
   ...
   if error-status :error then do:
    ...
    substitute( "&1. Ошибка при отправке в новости команды на &5 записи. &2&3&2&4",
      vss-workfile, chr(10), return-value, error-status :get-message ( error-status :num-messages ),
      if new(new-sr-izmerenia) then 'Создание':U else 'Изменение':U
              )
      ) .
   end.
    ...
    assign
      ...
      buf_c-sr-izmerenia.chip-num           = next-value (s-ref-corr-chip, ub)
      ...
      buf_c-sr-izmerenia.action             = if new(new-sr-izmerenia) then '1':U else '2':U
  ...
  if not g#news then do:
        run trg/userlog.p (
            input if new(buf_c-sr-izmerenia) then 'create':U else 'update':U
            , input 'c-sr-izmerenia':U
