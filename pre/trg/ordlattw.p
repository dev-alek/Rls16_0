...
trigger procedure for write of ub.order-line-attr
  new buffer new-order-line-attr
  old buffer old-order-line-attr
.
...
define variable v-tbl-name as character no-undo.
define buffer buf_c-order-head for ub.c-order-head .
      buffer-compare new-order-line-attr to old-order-line-attr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("order-line-attr","s-c-order-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-c-order-chip-num, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-order-line-attr)
       then do:
 for first buf_c-order-line-attr where buf_c-order-line-attr.db-num eq new-order-line-attr.db-num
and buf_c-order-line-attr.doc-code eq new-order-line-attr.doc-code
and buf_c-order-line-attr.line-num eq new-order-line-attr.line-num
and buf_c-order-line-attr.attr-code eq new-order-line-attr.attr-code
  and buf_c-order-line-attr.corr-user-db-num   = g#db-num and buf_c-order-line-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-order-line-attr where buf_c-order-line-attr.db-num eq old-order-line-attr.db-num
and buf_c-order-line-attr.doc-code eq old-order-line-attr.doc-code
and buf_c-order-line-attr.line-num eq old-order-line-attr.line-num
and buf_c-order-line-attr.attr-code eq old-order-line-attr.attr-code
  and buf_c-order-line-attr.corr-user-db-num   = g#db-num and buf_c-order-line-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-order-line-attr)
   then do:
      if not available buf_c-order-line-attr
      then do:
         create buf_c-order-line-attr.
      buffer-copy new-order-line-attr to buf_c-order-line-attr
      assign
         buf_c-order-line-attr.chip-num           = v-Seq
         buf_c-order-line-attr.corr-date          = v-date
         buf_c-order-line-attr.corr-time          = v-time
         buf_c-order-line-attr.corr-user-db-num   = g#db-num
         buf_c-order-line-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-order-line-attr.action             = 1
         buf_c-order-line-attr.is-del             = false
      .
      end.
      else
         if buf_c-order-line-attr.action eq 99
         then
            buf_c-order-line-attr.action = 2.
   end.
   else do:
      if not available buf_c-order-line-attr
      then do:
         create buf_c-order-line-attr.
         buffer-copy old-order-line-attr to buf_c-order-line-attr
         assign
            buf_c-order-line-attr.chip-num           = v-Seq
            buf_c-order-line-attr.corr-date          = v-date
            buf_c-order-line-attr.corr-time          = v-time
            buf_c-order-line-attr.corr-user-db-num   = g#db-num
            buf_c-order-line-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-order-line-attr.action             = 2 when buf_c-order-line-attr.action ne 1
            buf_c-order-line-attr.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-order-line-attr)
         then do:
 for first buf_c-order-head where buf_c-order-head.db-num eq new-order-line-attr.db-num
and buf_c-order-head.doc-code eq new-order-line-attr.doc-code
  and buf_c-order-head.corr-user-db-num   = g#db-num and buf_c-order-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-order-head where buf_c-order-head.db-num eq old-order-line-attr.db-num
and buf_c-order-head.doc-code eq old-order-line-attr.doc-code
  and buf_c-order-head.corr-user-db-num   = g#db-num and buf_c-order-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      if not available  buf_c-order-head
      then do:
         create buf_c-order-head.
         buffer-copy  buf_c-order-line-attr to buf_c-order-head
         assign
            buf_c-order-head.subject = "order-line-attr"
            buf_c-order-head.is-news = g#news
            buf_c-order-head.source-type = (if g#news
                                          then 'db':U
                                          else (if g#esys
                                                then 'esys':U
                                                else "":U)
                                          )
            buf_c-order-head.source-ref = (if g#news
         ...
         if     buf_c-order-head.subject ne "*"
         then
            buf_c-order-head.subject = "*".
         if buf_c-order-head.action  ne 88
         then
            buf_c-order-head.action             = 88 .
      end.
