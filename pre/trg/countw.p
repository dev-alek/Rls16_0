...
trigger procedure for write of ub.counter
  new buffer new-counter
  old buffer old-counter
.
...
define variable vss-description as character no-undo init "Тригер изменение counter".
...
define buffer buf_c-counter  for ub.c-counter .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
if new-counter.CountValue ne old-counter.CountValue + 1
then do:
   if new-counter.file-name eq "cashbookrule"
   then do:
      buffer-compare new-counter to old-counter case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("counter","s-c-counter-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-c-counter-chip-num, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-counter)
       then do:
 for first buf_c-counter where buf_c-counter.db eq new-counter.db
and buf_c-counter.file-name eq new-counter.file-name
and buf_c-counter.key eq new-counter.key
and buf_c-counter.code eq new-counter.code
  and buf_c-counter.corr-user-db-num   = g#db-num and buf_c-counter.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-counter where buf_c-counter.db eq old-counter.db
and buf_c-counter.file-name eq old-counter.file-name
and buf_c-counter.key eq old-counter.key
and buf_c-counter.code eq old-counter.code
  and buf_c-counter.corr-user-db-num   = g#db-num and buf_c-counter.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-counter)
   then do:
      if not available buf_c-counter
      then do:
         create buf_c-counter.
      buffer-copy new-counter to buf_c-counter
      assign
         buf_c-counter.chip-num           = v-Seq
         buf_c-counter.corr-date          = v-date
         buf_c-counter.corr-time          = v-time
         buf_c-counter.corr-user-db-num   = g#db-num
         buf_c-counter.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-counter.action             = 1
         buf_c-counter.is-del             = false
      .
      end.
      else
         if buf_c-counter.action eq 99
         then
            buf_c-counter.action = 2.
   end.
   else do:
      if not available buf_c-counter
      then do:
         create buf_c-counter.
         buffer-copy old-counter to buf_c-counter
         assign
            buf_c-counter.chip-num           = v-Seq
            buf_c-counter.corr-date          = v-date
            buf_c-counter.corr-time          = v-time
            buf_c-counter.corr-user-db-num   = g#db-num
            buf_c-counter.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-counter.action             = 2 when buf_c-counter.action ne 1
            buf_c-counter.is-del             = false
         .
      end.
   end.
...
then do:
  if not g#news then do :
       ...
       if     available old-counter
          and not new(new-counter)
       then do:
          buffer-compare new-counter
                using db file-name key code
                to old-counter
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'counter':U
                  ,input (buffer old-counter:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление counter для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'counter':U
      ,input (buffer ub.new-counter:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать counter для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
