...
:
  if not g#news then do:
    ...
    buf_c-dis-cp-rule.chip-num           = next-value (s-corr-chip, ub)
    ...
    if buf_c-dis-cp-rule.cdpay-code > 0 then do:
      ...
      assign
      buf_c-cash-pay.subject            = 'dis-cp-rule':U
      buf_c-cash-pay.action             = integer(if new(ub.dis-cp-rule) then '1':U else '2':U)
      .
    end.
  end.
  run str/callnews.p
