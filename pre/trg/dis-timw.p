...
:
  ...
  if ub.dis-time-rule.time-rule-num <= 99999 then do:
      _ii:
    ...
    if not ( g#db-num > 0 ) then do:
      if (v-chr <> "sts" and v-changed
          AND old_dis-time-rule.sts <> integer('1':U)
  ...
  if ub.dis-time-rule.upper-time-rule-num <= 99999 then do:
    run str/callnews.p
  ...
  buF_c-dis-time-rule.action             = (if new(ub.dis-time-rule) then integer('1':U) else integer('2':U))
