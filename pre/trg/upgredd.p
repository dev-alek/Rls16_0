...
trigger procedure for delete of upgrade.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления upgrade".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'upgrade':U
       ,input (buffer ub.upgrade:handle)
       ,input "":U
      ) no-error .
...
for each upgrade-attr where upgrade-attr.db           eq upgrade.db
                            and upgrade-attr.version-num  eq upgrade.version-num
exclusive-lock:
   delete upgrade-attr.
end.
