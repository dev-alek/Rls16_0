...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdburt in g#library
  (output ub.ord-doc-rcv.user-db-num
  ,output ub.ord-doc-rcv.user-name
  ,output ub.ord-doc-rcv.sys-date
  ,output ub.ord-doc-rcv.sys-time
  ,output ub.ord-doc-rcv.sys-time-int
  )  .
  end.
  if ub.ord-doc-rcv.status_ = 'новый':U
  then do:
    ...
    assign
      ub.ord-doc-rcv.fact-num = next-value (s-ord-fact, ub)
    .
  end.
  if ub.ORD-doc-rcv.doc-type = "in":U then do:
     ...
     find first c-clients where
       ...
       if ub.ORD-doc-rcv.cli-type = 'маг':U then do:
          find first shop where shop.obj-code = ub.ORD-doc-rcv.cli-code no-lock .
          f-host = shop.host-code.
       end.
       if ub.ORD-doc-rcv.cli-type = 'скл':U then do:
  ...
  if ub.ORD-doc-rcv.status_ = 'факт':U and g#news = false  then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  ,output l-shift-on
      ...
      run factord in this-procedure
        (input  ub.ORD-doc-rcv.fact-date
        ,input  ub.ORD-doc-rcv.fact-time
        ,input  ub.ORD-doc-rcv.fact-num
        ,input  ub.ORD-doc-rcv.shift-date
        ,input  ub.ORD-doc-rcv.shift-num
        ,input  l-shift-on
        ,output v-fact-order
...
if g#news  = false and ub.ord-doc-rcv.status_  <> 'новый':U then do:
    create ub.c-ord-doc.
 ...
 if ub.ORD-doc-rcv.status_ = 'факт':U or
  ( ub.ORD-doc-rcv.status_ = 'поставка':U AND OLD_ORD-doc-rcv.status_ <> 'поставка':U ) or
    ub.ORD-doc-rcv.status_ = 'запрос':U
    then do:
     if not g#news  then do:
        run str/callnews.p
          (input 'ord-doc-rcv':U
  ...
  if  not g#news  and
     ub.ord-doc-rcv.status_  = 'поставка':U and
     old_ord-doc-rcv.status_ = 'поставка':U and
  (  old_ord-doc-rcv.ship-time <> ub.ord-doc-rcv.ship-time or
    ...
    if ub.ORD-doc-rcv.doc-type = 'запрос':U and g#db-num = 0 and  g#news then do:
        run str/callnews.p
          (input 'ord-doc-rcv':U
          ,input (buffer ub.ORD-doc-rcv:handle)
          ) no-error .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run rum-runa in g#library
  ...
  ,input ?
  ,input 'event_rcv':U
  ,input  buffer old_ord-doc-rcv:handle
  ,input  buffer ub.ord-doc-rcv:handle
  ,input ''
  ,input ''
  ) no-error .
