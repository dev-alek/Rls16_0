...
trigger procedure for write of ub.cashbook
  new buffer new-cashbook
  old buffer old-cashbook
.
...
define variable vss-description as character no-undo init "Тригер изменение cashbook".
...
define buffer buf_c-cashbook  for ub.c-cashbook .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define buffer buf_c-cashbook-head for ub.c-cashbook-head .
      buffer-compare new-cashbook to old-cashbook case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("cashbook","s-c-cashbook-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    then do:
                     if new(new-cashbook)
       then do:
 for first buf_c-cashbook where buf_c-cashbook.id eq new-cashbook.id
  and buf_c-cashbook.corr-user-db-num   = g#db-num and buf_c-cashbook.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-cashbook where buf_c-cashbook.id eq old-cashbook.id
  and buf_c-cashbook.corr-user-db-num   = g#db-num and buf_c-cashbook.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-cashbook)
   then do:
      if not available buf_c-cashbook
      then do:
         create buf_c-cashbook.
      buffer-copy new-cashbook to buf_c-cashbook
      assign
         buf_c-cashbook.chip-num           = v-Seq
         buf_c-cashbook.corr-date          = v-date
         buf_c-cashbook.corr-time          = v-time
         buf_c-cashbook.corr-user-db-num   = g#db-num
         buf_c-cashbook.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-cashbook.action             = 1
         buf_c-cashbook.is-del             = false
      .
      end.
      else
         if buf_c-cashbook.action eq 99
         then
            buf_c-cashbook.action = 2.
   end.
   else do:
      if not available buf_c-cashbook
      then do:
         create buf_c-cashbook.
         buffer-copy old-cashbook to buf_c-cashbook
         assign
            buf_c-cashbook.chip-num           = v-Seq
            buf_c-cashbook.corr-date          = v-date
            buf_c-cashbook.corr-time          = v-time
            buf_c-cashbook.corr-user-db-num   = g#db-num
            buf_c-cashbook.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-cashbook.action             = 2 when buf_c-cashbook.action ne 1
            buf_c-cashbook.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if vuniq-key-rec eq ""
            or vuniq-key-rec eq ?
         then do:
            run gen-key-rec in this-procedure ( input "c-cashbook"
                                               ,input (buffer buf_c-cashbook:handle)
      ...
      then do:
         ...
         buffer-copy  buf_c-cashbook to buf_c-cashbook-head
         assign
            buf_c-cashbook-head.subject = "cashbook"
  ...
  if not g#news then do :
       ...
       if     available old-cashbook
          and not new(new-cashbook)
       then do:
          buffer-compare new-cashbook
                using id
                to old-cashbook
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'CashBook':U
                  ,input (buffer old-cashbook:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление cashbook для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'CashBook':U
      ,input (buffer ub.new-cashbook:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать cashbook для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
