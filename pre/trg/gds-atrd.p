...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2', ub.goods-attr.gds-code, ub.goods-attr.attr-code)
    .
end procedure.
...
:
  if not ub.goods-attr.attr-code = 'lock':U then do:
  ...
  if lookup( ub.goods-attr.attr-code,  '8x50,15x80,6x50,6x50,8x50,8x50,8x50,8x50,8x50':U) > 0
  then do:
    find first buf_goods no-lock where buf_goods.gds-code = ub.goods-attr.gds-code no-error.
    if available buf_goods then do:
define variable vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-attr-code4 as character no-undo .
define buffer buf4_gds-prt for ub.gds-prt.
define buffer buf4_bar-code for ub.bar-code.
define buffer buf4_scales-gds for ub.scales-gds.
define buffer buf4_scales for ub.scales.
find buf4_gds-prt no-lock
  where buf4_gds-prt.upper-code = buf_goods.prt-root
  .
find first buf4_bar-code no-lock
  where buf4_bar-code.gds-code  = buf_goods.gds-code
    and buf4_bar-code.node-code = buf4_gds-prt.node-code
    and buf4_bar-code.part-code = ""
    and buf4_bar-code.in-code   = ""
    and buf4_bar-code.unit-cli  = buf_goods.unit-base
  no-error .
if available buf4_bar-code then do:
  for each buf4_scales-gds
    where buf4_scales-gds.db-num = g#db-num
      AND buf4_scales-gds.b-code = buf4_bar-code.b-code
  :
    find first buf4_scales
      where buf4_scales.scales-num = buf4_scales-gds.scales-num
       and buf4_scales.db-num = buf4_scales-gds.db-num
      .
        assign
    v-attr-code4 = entry (lookup (buf4_scales.scales-type, 'CAS_lp-16x,DIGI-SM,CAS_CL5000J,CAS_CL5000,TIGER-SPCT2,TIGER-SPCT1,CAS_LP-15,SHTRIH-M,CAS_LP-15v1.6':U), '8x50,15x80,6x50,6x50,8x50,8x50,8x50,8x50,8x50':U) no-error .
    if lookup(buf4_scales.scales-type, 'CAS_lp-16x,DIGI-SM,CAS_CL5000J,CAS_CL5000,TIGER-SPCT2,TIGER-SPCT1,CAS_LP-15,SHTRIH-M,CAS_LP-15v1.6':U) > 0
    and (ub.goods-attr.attr-code = "struct" or v-attr-code4 = ub.goods-attr.attr-code)
    then do:
    run ref/ves-pbc.p (
                    input ?
                  , input 'ИЗМЕНЕНИЕ':U
                    , input buf4_scales-gds.obj-type
                    , input buf4_scales-gds.obj-code
                    , input ?
                    , input ?
                    , input ?
                    , input ?
                    , buffer buf4_bar-code
                    , buffer buf4_scales) .
      end.
  end.
end.
...
if (valid-handle(g#lib-nws) <> true) then do:   run nws/lib-nws.p persistent no-error .   if error-status :error or (valid-handle(g#lib-nws) <> true) then do:     message       "Error starting nws/lib-nws.p" skip       g#lib-nws skip       g#lib-nws :type skip       g#lib-nws :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-nws_get-hn-option in g#lib-nws
  ...
  or v-send >= 0 then do:
    ...
    assign
    buf_c-goods-attr.chip-num           = next-value (s-gds-chip, ub)
    ...
    buf_c-gds-hist.subject = 'goods-attr':U
       ...
       then do:
          ...
          assign
             ...
             buf_c-goods-attr-any.chip-num           = if ub.goods-attr.attr-code eq "operservid"
                                                      then
                                                         next-value (s-c-operserv-chip-num, ub)
                                                      else
                                                         next-value (s-c-cashbook-chip-num, ub)
             ...
             buf_c-goods-attr-any.action             = 99
          .
  ...
  send-ref = (IF error-status:error or conf-par <> "yes" then no else yes).
    if send-ref  then do:
       ...
       do jj = 1 to num-entries(v-other, chr(47)):
         if entry(jj, v-other, chr(47)) = "" then NEXT _do.
         assign
         v-dop1 = entry(1, entry(jj, v-other, chr(47)), '=':U) no-error .
         v-dop2 = entry(2, entry(jj, v-other, chr(47)), '=':U) no-error .
