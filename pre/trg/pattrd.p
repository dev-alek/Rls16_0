...
trigger procedure for delete of ub.PromoAttr.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления PromoAttr".
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-PromoAttr where buf_c-PromoAttr.tablename eq PromoAttr.tablename
and buf_c-PromoAttr.p-key eq PromoAttr.p-key
  and buf_c-PromoAttr.corr-user-db-num   = g#db-num and buf_c-PromoAttr.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.PromoAttr to buf_c-PromoAttr
       assign
         ...
         buf_c-PromoAttr.action             = 99
         buf_c-PromoAttr.is-del             = true
       .
    end.
    else
       buf_c-PromoAttr.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'PromoAttr':U
       ,input (buffer ub.PromoAttr:handle)
       ,input "":U
      ) no-error .
