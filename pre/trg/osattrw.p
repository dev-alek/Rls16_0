...
trigger procedure for write of ub.OperServAttr
  new buffer new-OperServAttr
  old buffer old-OperServAttr
.
...
define variable vss-description as character no-undo init "Тригер изменение OperServAttr".
...
define buffer buf_c-OperServAttr  for ub.c-OperServAttr .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-OperServAttr to old-OperServAttr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("OperServAttr","s-c-OperServ-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-c-OperServ-chip-num, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-OperServAttr)
       then do:
 for first buf_c-OperServAttr where buf_c-OperServAttr.id eq new-OperServAttr.id
and buf_c-OperServAttr.attr-code eq new-OperServAttr.attr-code
  and buf_c-OperServAttr.corr-user-db-num   = g#db-num and buf_c-OperServAttr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-OperServAttr where buf_c-OperServAttr.id eq old-OperServAttr.id
and buf_c-OperServAttr.attr-code eq old-OperServAttr.attr-code
  and buf_c-OperServAttr.corr-user-db-num   = g#db-num and buf_c-OperServAttr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-OperServAttr)
   then do:
      if not available buf_c-OperServAttr
      then do:
         create buf_c-OperServAttr.
      buffer-copy new-OperServAttr to buf_c-OperServAttr
      assign
         buf_c-OperServAttr.chip-num           = v-Seq
         buf_c-OperServAttr.corr-date          = v-date
         buf_c-OperServAttr.corr-time          = v-time
         buf_c-OperServAttr.corr-user-db-num   = g#db-num
         buf_c-OperServAttr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-OperServAttr.action             = 1
         buf_c-OperServAttr.is-del             = false
      .
      end.
      else
         if buf_c-OperServAttr.action eq 99
         then
            buf_c-OperServAttr.action = 2.
   end.
   else do:
      if not available buf_c-OperServAttr
      then do:
         create buf_c-OperServAttr.
         buffer-copy old-OperServAttr to buf_c-OperServAttr
         assign
            buf_c-OperServAttr.chip-num           = v-Seq
            buf_c-OperServAttr.corr-date          = v-date
            buf_c-OperServAttr.corr-time          = v-time
            buf_c-OperServAttr.corr-user-db-num   = g#db-num
            buf_c-OperServAttr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-OperServAttr.action             = 2 when buf_c-OperServAttr.action ne 1
            buf_c-OperServAttr.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-OperServAttr
          and not new(new-OperServAttr)
       then do:
          buffer-compare new-OperServAttr
                using id attr-code
                to old-OperServAttr
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'OperServAttr':U
                  ,input (buffer old-OperServAttr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление OperServAttr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'OperServAttr':U
      ,input (buffer ub.new-OperServAttr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать OperServAttr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
