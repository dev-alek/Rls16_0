...
define buffer buf_c-wealth for ub.c-wealth.
  ...
  :
    ...
    create buf_c-wealth.
    buffer-copy oldwealth to buf_c-wealth
    assign
    buf_c-wealth.wth-code           = (if p-new-record then ub.wealth.wth-code else oldwealth.wth-code)
    buf_c-wealth.chip-num           = next-value (s-wth-chip, ub)
    buf_c-wealth.corr-time          = v-time
    buf_c-wealth.corr-user-db-num   = g#db-num
    buf_c-wealth.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) else g#userid)
    ...
    buf_c-wth-hist.subject = 'wealth':U
...
:
  ...
  run wealthh_write-wealth-trigger in this-procedure  (
                                      ...
                                      ,input integer(if new(wealth) then '1':U else '2':U)
                                    ) no-error .
