...
:
  ...
  or g#db-num <> 0 then do:
    ...
    buf_c-rule-profile.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    buf_c-rule-profile.action = integer(if new(ub.rule-profile) then '1':U else '2':U)
    buf_c-rule-profile.subject = 'rule-profile':U
    .
  end.
  if not g#news
  and g#db-num = 0
