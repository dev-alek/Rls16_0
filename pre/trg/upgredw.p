...
trigger procedure for write of ub.upgrade
  new buffer new-upgrade
  old buffer old-upgrade
.
...
define variable vss-description as character no-undo init "Тригер изменение upgrade".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
       ...
       if     available old-upgrade
          and not new(new-upgrade)
       then do:
          buffer-compare new-upgrade
                using db-num version-num
                to old-upgrade
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'upgrade':U
                  ,input (buffer old-upgrade:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление upgrade для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'upgrade':U
      ,input (buffer ub.new-upgrade:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать upgrade для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
