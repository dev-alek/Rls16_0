...
TRIGGER PROCEDURE FOR WRITE OF ub.marking
   new buffer new-marking
   old buffer old-marking
   .
...
if new-marking.gds-code eq 0
then
   new-marking.gds-code = ?.
if not new(new-marking) then do:
   ...
   and
      not  (new-marking.sts = objSrv:Env:Marking:Sts:Mark:Ungrouped:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:FreeZone:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:OutZone:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:SaleLock:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:SaleWaitLock:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:ReturnLock:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:ReturnWaitLock:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:Reserved:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:Checked_:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:NotAvailable:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:UsedInProduction:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:WrittenOff:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:Moved:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:DeliveryControl:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:Returned:KeyIntDB
         or new-marking.sts = objSrv:Env:Marking:Sts:Mark:OutOfInventory:KeyIntDB)
   then do:
      if old-marking.loc-key <> "" then
         new-marking.loc-key = old-marking.loc-key.
   end.
   if new-marking.sts <> old-marking.sts then do:
      new-marking.last-change = now.
   end .
   if new-marking.sts = objSrv:Env:Marking:Sts:Mark:FreeZone:KeyIntDB then do:
      new-marking.loc-key = "".
   end .
   if new-marking.mark <> old-marking.mark then do:
      for each marking-attr exclusive-lock
         where marking-attr.mark = old-marking.mark:
               marking-attr.mark = new-marking.mark.
      end.
      for each marking-lines exclusive-lock
         where marking-lines.mark = old-marking.mark:
               marking-lines.mark = new-marking.mark.
      end.
      for each marking-chk exclusive-lock
         where marking-chk.mark = old-marking.mark:
               marking-chk.mark = new-marking.mark.
      end.
      for each c-marking where c-marking.mark eq old-marking.mark exclusive-lock:
               c-marking.mark = new-marking.mark.
      end.
      for each c-marking-attr where c-marking-attr.mark eq old-marking.mark exclusive-lock:
               c-marking.mark = new-marking.mark.
      end.
   end .
end.
   ...
   new-marking.last-change = now.
if new(new-marking) and new-marking.mark-parent <> "" then
do:
  for first parentMarking where
            parentMarking.mark = new-marking.mark-parent
      no-lock:
    if can-do(objSrv:Env:Marking:Sts:Mark:Sale_Return_Wait,string(parentMarking.sts)) then
     new-marking.sts = parentMarking.sts.
  end.
end.
end.
      buffer-compare new-marking to old-marking case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("marking","s-c-mark-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-c-mark-chip-num, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-marking)
       then do:
 for first buf_c-marking where buf_c-marking.mark eq new-marking.mark
  and buf_c-marking.corr-user-db-num   = g#db-num and buf_c-marking.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-marking where buf_c-marking.mark eq old-marking.mark
  and buf_c-marking.corr-user-db-num   = g#db-num and buf_c-marking.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-marking)
   then do:
      if not available buf_c-marking
      then do:
         create buf_c-marking.
      buffer-copy new-marking to buf_c-marking
      assign
         buf_c-marking.chip-num           = v-Seq
         buf_c-marking.corr-date          = v-date
         buf_c-marking.corr-time          = v-time
         buf_c-marking.corr-user-db-num   = g#db-num
         buf_c-marking.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-marking.action             = 1
         buf_c-marking.is-del             = false
      .
      end.
      else
         if buf_c-marking.action eq 99
         then
            buf_c-marking.action = 2.
   end.
   else do:
      if not available buf_c-marking
      then do:
         create buf_c-marking.
         buffer-copy old-marking to buf_c-marking
         assign
            buf_c-marking.chip-num           = v-Seq
            buf_c-marking.corr-date          = v-date
            buf_c-marking.corr-time          = v-time
            buf_c-marking.corr-user-db-num   = g#db-num
            buf_c-marking.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-marking.action             = 2 when buf_c-marking.action ne 1
            buf_c-marking.is-del             = false
         .
      end.
   end.
...
do:
  if not g#news then do :
     run str/callnews.p
      (input 'marking':U
      ,input (buffer ub.new-marking:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать marking для отправки в новости" skip
