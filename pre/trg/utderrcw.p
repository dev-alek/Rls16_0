...
trigger procedure for write of ub.c-utd-err
  new buffer new-c-utd-err
  old buffer old-c-utd-err
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd-err".
...
if g#db-num = 0 then return.
  if not g#news then do :
       ...
       if     available old-c-utd-err
          and not new(new-c-utd-err)
       then do:
          buffer-compare new-c-utd-err
                using db-num doc-id CheckType CodeErr CheckObj reckey corr-user-db-num chip-num
                to old-c-utd-err
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'c-utd-err':U
                  ,input (buffer old-c-utd-err:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-utd-err для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-utd-err':U
      ,input (buffer ub.new-c-utd-err:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd-err для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
