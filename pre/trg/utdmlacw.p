...
trigger procedure for write of ub.c-utd-marking-lines-attr
  new buffer new-c-utd-marking-lines-attr
  old buffer old-c-utd-marking-lines-attr
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd-marking-lines-attr".
...
if g#db-num = 0 then return.
  if not g#news then do :
       ...
       if     available old-c-utd-marking-lines-attr
          and not new(new-c-utd-marking-lines-attr)
       then do:
          buffer-compare new-c-utd-marking-lines-attr
                using db-num doc-id LineNum mark attr-code corr-user-db-num chip-num
                to old-c-utd-marking-lines-attr
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'c-utd-marking-lines-attr':U
                  ,input (buffer old-c-utd-marking-lines-attr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-utd-marking-lines-attr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-utd-marking-lines-attr':U
      ,input (buffer ub.new-c-utd-marking-lines-attr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd-marking-lines-attr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
