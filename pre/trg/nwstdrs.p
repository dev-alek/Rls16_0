...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2':u,p-doc-code,p-rsrv-direction)
    .
end procedure.
...
:
  ...
  or buf_trn-doc.status_ = 'готов':U
  or buf_trn-doc.status_ = 'отказ':U
    then do:
    return .
  end.
  ...
  ,output v-place-rsrv
    ...
    :
      ...
      run partcopy-rsrv-parts in this-procedure
        (input rowid(buf_trn-doc)
        ,input rowid(buf_parts)
        ,input p-rsrv-direction
        ...
        :
          run trndocrs-pl-gds-accum in this-procedure
            ...
            ,input buf_doc-pl.cli-doc-qnty
                   ...
                   * (if lookup(buf_trn-doc.doc-type, 'рас,спи':U ) <> 0 then -1.0 else 1.0)
            ,input 0.0
            ,input 0.0
            ) no-error .
          if error-status :error then do:
