...
define variable vss-description as character no-undo init "—оздать запись о том, что необходимо переслать данные о клиенте - продавце - на кассу".
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6',p-seller,p-psn-code,p-host-code,p-obj-type,p-obj-code,p-action)
    .
end procedure.
...
:
  find first ub.batchprocess no-lock
    where ub.batchprocess.bp_type   = 'slr':U
      and ub.batchprocess.bp_status = 'N':U
      and  ub.batchprocess.key#_one  = p-psn-code
      and ub.batchprocess.key#_two = p-host-code
      and ub.batchprocess.key#_three = p-obj-code
      and ub.batchprocess.charkey_one = string(p-seller)
      and ub.batchprocess.charkey_two = p-obj-type
      and ub.batchprocess.charkey_three = p-s-password
  no-error .
  if not available ub.BatchProcess then do:
    ...
    .
    assign
      ub.BatchProcess.Key#_One      = p-psn-code
      ub.BatchProcess.Key#_Two      = p-host-code
      ub.BatchProcess.Key#_Three    = p-obj-code
      ub.BatchProcess.CharKey_One   = string(p-seller)
      ub.BatchProcess.CharKey_Two   = p-obj-type
      ub.BatchProcess.CharKey_Three = p-s-password
    .
  end.
