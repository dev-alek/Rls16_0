...
trigger procedure for delete of ub.OperServAttr.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления OperServAttr".
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-OperServAttr where buf_c-OperServAttr.id eq OperServAttr.id
and buf_c-OperServAttr.attr-code eq OperServAttr.attr-code
  and buf_c-OperServAttr.corr-user-db-num   = g#db-num and buf_c-OperServAttr.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.OperServAttr to buf_c-OperServAttr
       assign
         ...
         buf_c-OperServAttr.action             = 99
         buf_c-OperServAttr.is-del             = true
       .
    end.
    else
       buf_c-OperServAttr.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'OperServAttr':U
       ,input (buffer ub.OperServAttr:handle)
       ,input "":U
      ) no-error .
