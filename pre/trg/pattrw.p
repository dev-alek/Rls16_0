...
trigger procedure for write of ub.PromoAttr
  new buffer new-PromoAttr
  old buffer old-PromoAttr
.
...
define variable vss-description as character no-undo init "Тригер изменение PromoAttr".
...
define buffer buf_c-PromoAttr  for ub.c-PromoAttr .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-PromoAttr to old-PromoAttr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("PromoAttr","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-PromoAttr)
       then do:
 for first buf_c-PromoAttr where buf_c-PromoAttr.tablename eq new-PromoAttr.tablename
and buf_c-PromoAttr.p-key eq new-PromoAttr.p-key
  and buf_c-PromoAttr.corr-user-db-num   = g#db-num and buf_c-PromoAttr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-PromoAttr where buf_c-PromoAttr.tablename eq old-PromoAttr.tablename
and buf_c-PromoAttr.p-key eq old-PromoAttr.p-key
  and buf_c-PromoAttr.corr-user-db-num   = g#db-num and buf_c-PromoAttr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-PromoAttr)
   then do:
      if not available buf_c-PromoAttr
      then do:
         create buf_c-PromoAttr.
      buffer-copy new-PromoAttr to buf_c-PromoAttr
      assign
         buf_c-PromoAttr.chip-num           = v-Seq
         buf_c-PromoAttr.corr-date          = v-date
         buf_c-PromoAttr.corr-time          = v-time
         buf_c-PromoAttr.corr-user-db-num   = g#db-num
         buf_c-PromoAttr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-PromoAttr.action             = 1
         buf_c-PromoAttr.is-del             = false
      .
      end.
      else
         if buf_c-PromoAttr.action eq 99
         then
            buf_c-PromoAttr.action = 2.
   end.
   else do:
      if not available buf_c-PromoAttr
      then do:
         create buf_c-PromoAttr.
         buffer-copy old-PromoAttr to buf_c-PromoAttr
         assign
            buf_c-PromoAttr.chip-num           = v-Seq
            buf_c-PromoAttr.corr-date          = v-date
            buf_c-PromoAttr.corr-time          = v-time
            buf_c-PromoAttr.corr-user-db-num   = g#db-num
            buf_c-PromoAttr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-PromoAttr.action             = 2 when buf_c-PromoAttr.action ne 1
            buf_c-PromoAttr.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-PromoAttr
          and not new(new-PromoAttr)
       then do:
          buffer-compare new-PromoAttr
                using tablename p-key
                to old-PromoAttr
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'PromoAttr':U
                  ,input (buffer old-PromoAttr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление PromoAttr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'PromoAttr':U
      ,input (buffer ub.new-PromoAttr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать PromoAttr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
