...
trigger procedure for write of ub.PromoAction
  new buffer new-PromoAction
  old buffer old-PromoAction
.
...
define variable vss-description as character no-undo init "Тригер изменение PromoAction".
...
define buffer buf_c-PromoAction  for ub.c-PromoAction .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-PromoAction to old-PromoAction case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("PromoAction","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-PromoAction)
       then do:
 for first buf_c-PromoAction where buf_c-PromoAction.id eq new-PromoAction.id
and buf_c-PromoAction.db-num eq new-PromoAction.db-num
  and buf_c-PromoAction.corr-user-db-num   = g#db-num and buf_c-PromoAction.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-PromoAction where buf_c-PromoAction.id eq old-PromoAction.id
and buf_c-PromoAction.db-num eq old-PromoAction.db-num
  and buf_c-PromoAction.corr-user-db-num   = g#db-num and buf_c-PromoAction.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-PromoAction)
   then do:
      if not available buf_c-PromoAction
      then do:
         create buf_c-PromoAction.
      buffer-copy new-PromoAction to buf_c-PromoAction
      assign
         buf_c-PromoAction.chip-num           = v-Seq
         buf_c-PromoAction.corr-date          = v-date
         buf_c-PromoAction.corr-time          = v-time
         buf_c-PromoAction.corr-user-db-num   = g#db-num
         buf_c-PromoAction.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-PromoAction.action             = 1
         buf_c-PromoAction.is-del             = false
      .
      end.
      else
         if buf_c-PromoAction.action eq 99
         then
            buf_c-PromoAction.action = 2.
   end.
   else do:
      if not available buf_c-PromoAction
      then do:
         create buf_c-PromoAction.
         buffer-copy old-PromoAction to buf_c-PromoAction
         assign
            buf_c-PromoAction.chip-num           = v-Seq
            buf_c-PromoAction.corr-date          = v-date
            buf_c-PromoAction.corr-time          = v-time
            buf_c-PromoAction.corr-user-db-num   = g#db-num
            buf_c-PromoAction.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-PromoAction.action             = 2 when buf_c-PromoAction.action ne 1
            buf_c-PromoAction.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-PromoAction
          and not new(new-PromoAction)
       then do:
          buffer-compare new-PromoAction
                using id db-num
                to old-PromoAction
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'PromoAction':U
                  ,input (buffer old-PromoAction:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление PromoAction для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'PromoAction':U
      ,input (buffer ub.new-PromoAction:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать PromoAction для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
