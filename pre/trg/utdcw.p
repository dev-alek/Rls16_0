...
trigger procedure for write of ub.c-utd
  new buffer new-c-utd
  old buffer old-c-utd
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd".
...
if g#db-num = 0 then return.
  if not g#news then do :
       ...
       if     available old-c-utd
          and not new(new-c-utd)
       then do:
          buffer-compare new-c-utd
                using db-num doc-id corr-user-db-num chip-num
                to old-c-utd
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'c-utd':U
                  ,input (buffer old-c-utd:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-utd для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-utd':U
      ,input (buffer ub.new-c-utd:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
