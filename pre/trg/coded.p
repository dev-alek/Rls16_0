...
trigger procedure for delete of ub.code.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления code".
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-code where buf_c-code.parent eq code.parent
and buf_c-code.code eq code.code
  and buf_c-code.corr-user-db-num   = g#db-num and buf_c-code.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       create buf_c-code.
       buffer-copy ub.code to buf_c-code
       assign
         ...
         buf_c-code.action             = 99
         buf_c-code.is-del             = true
       .
    end.
    else
       buf_c-code.action             = 99.
...
vparent = (if code.parent = "" then "" else ( code.parent  +  chr(4)) )  + code.code.
