...
define variable vss-description as character no-undo init "Пометить складской архив по типам приобретения, как требующий перерасчета с определенной даты".
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3',p-obj-type,p-obj-code,p-fact-date)
    .
end procedure.
...
:
  ...
  then do:
    message
      ...
      "Невозможно произвести пометить складской архив по типам приобретения, как требующий перерасчета" skip
  ...
  run clntattr-write in this-procedure
    ...
    ,input p-obj-code
    ,input 'aht-recalc':U
    ,input string(p-fact-date, '99/99/9999':U)
    ) .
end.
