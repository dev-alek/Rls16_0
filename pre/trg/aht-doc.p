    ...
    :
      ...
      then do:
        ...
        export stream ahtlog
          vss-include-info0 buf_temp-aht-ot-tot.doc-code .
...
:
  ...
  then do:
    v-msgstr =
      ...
      + "Ошибка при обработке документа" + chr(10)
      + "Документ " + p-doc-code + chr(10)
      + "Ограничение на обновление складского архива " + string(p-cut-date) + chr(10)
      + return-value
    .
...
procedure process-trn-doc :
  ...
  then display
    ...
    then do:
      v-msgstr =
        "Нельзя рассчитать складской архив по типам приобретения для складского документа не закрытого до статуса " + 'факт':U + chr(10)
        + "Документ " + p-doc-code
      .
      if v-message-on then
      message
  ...
  ,output v-shift-on
    ...
    then do:
      v-msgstr =
        ...
        + "Объект " + buf_trn-doc.obj-type + " " + string(buf_trn-doc.obj-code) + chr(10)
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output v-curr-r-b
    ...
    then do:
      v-msgstr =
        "Ошибка при определении фактического номера складского документа" + chr(10)
        + "doc-code " +          buf_trn-doc.doc-code   + chr(10)
        + "fact-date " +  string(buf_trn-doc.fact-date) + chr(10)
        + "fact-time " +  string(buf_trn-doc.fact-time, "HH:MM") + chr(10)
        + "fact-num " +   string(buf_trn-doc.fact-num)  + chr(10)
        + "shift-date " + string(buf_trn-doc.shift-date) + chr(10)
        + "shift-num " +  string(buf_trn-doc.shift-num) + chr(10)
        + "v-fact-order " + string(v-fact-order)        + chr(10)
        + "v-shift-end-fact-order " + string(v-shift-end-fact-order) + chr(10)
        + "v-day-end-fact-order" + string(v-day-end-fact-order) + chr(10)
        + error-status:get-message(1) + chr(10)
        + return-value
      .
      if v-message-on then
    ...
    then do:
      v-msgstr =
        "Ошибка при добавлении документа в складской архив по типам приобретения" + chr(10)
        + "Документ " + p-doc-code + chr(10)
        + error-status:get-message(1) + chr(10)
        + return-value
      .
    ...
    then do:
      v-msgstr =
        "Ошибка при добавлении даты в складской архив по типам приобретения" + chr(10)
        + "Документ " + p-doc-code + chr(10)
        + error-status:get-message(1) + chr(10)
        + return-value
      .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gds-code in g#library
  ...
  ,output v-gds-code
      ...
      then do:
        v-msgstr =
          "Ошибка при обработке строки документа" + chr(10)
          + "Документ " + buf_doc-line.doc-code + chr(10)
          + "Артикул " + buf_doc-line.artic + " " + buf_doc-line.prod-type + " " + string(buf_doc-line.prod-code) + chr(10)
          + error-status:get-message(1) + chr(10)
          + return-value
        .
        if v-message-on then
    ...
    then do:
      v-msgstr =
        "Ошибка при расчете оборотов по документу" + chr(10)
        + error-status:get-message(1) + chr(10)
        + return-value
      .
      if v-message-on then
    ...
    then do:
      v-msgstr =
        "Ошибка при сохранении оборотов по документу" + chr(10)
        + error-status:get-message(1) + chr(10)
        + return-value
      .
      if v-message-on then
    ...
    then do:
      v-msgstr =
        "Ошибка при расчете остатков" + chr(10)
        + error-status:get-message(1) + chr(10)
        + return-value
      .
      if v-message-on then
...
procedure process-doc-line :
  ...
  :
    ...
    then do:
      v-msgstr =
        "Ошибка при вызове процедуры clcprtsl_calc-line" + chr(10)
        + "Документ " + p-doc-code + chr(10)
        + "Артикул " + p-artic + " " + p-prod-type + " " + string(p-prod-code) + chr(10)
        + error-status:get-message(1) + chr(10)
        + return-value
      .
      if v-message-on then
    ...
    :
      ...
      then do:
        ...
        message
          ...
          "Артикул" p-artic p-prod-type p-prod-code skip
          "Тип суммы" v-allsum-sum-type skip
                                                                      "crsa-sum-base"       v-crsa-sum-base        skip     "crsa-sum-rubl"       v-crsa-sum-rubl        skip     "crsa-vat-base"       v-crsa-vat-base        skip     "crsa-vat-rubl"       v-crsa-vat-rubl        skip     "crsa-slt-base"       v-crsa-slt-base        skip     "crsa-slt-rubl"       v-crsa-slt-rubl        skip     "crsa-road-tax-base"  v-crsa-road-tax-base   skip     "crsa-road-tax-rubl"  v-crsa-road-tax-rubl   skip     "crsa-excise-base"    v-crsa-excise-base     skip     "crsa-excise-rubl"    v-crsa-excise-rubl     skip     "crsa-transport-base" v-crsa-transport-base  skip     "crsa-transport-rubl" v-crsa-transport-rubl  skip     "crsa-other-base"     v-crsa-other-base      skip     "crsa-other-rubl"     v-crsa-other-rubl      skip     "crsa-discnt-base"    v-crsa-discnt-base     skip     "crsa-discnt-rubl"    v-crsa-discnt-rubl     skip
          view-as alert-box error .
        undo, return error v-msgstr .
      end.
      if
                              v-sale-sum-base       = ? or    v-sale-sum-rubl       = ? or    v-sale-vat-base       = ? or    v-sale-vat-rubl       = ? or    v-sale-slt-base       = ? or    v-sale-slt-rubl       = ? or    v-sale-road-tax-base  = ? or    v-sale-road-tax-rubl  = ? or    v-sale-excise-base    = ? or    v-sale-excise-rubl    = ? or    v-sale-transport-base = ? or    v-sale-transport-rubl = ? or    v-sale-other-base     = ? or    v-sale-other-rubl     = ? or    v-sale-discnt-base    = ? or    v-sale-discnt-rubl    = ?
      then do:
        v-msgstr = "Программа clcprtsl.i вернула неопределенные значения" .
        if v-message-on then
        message
          ...
          "Артикул" p-artic p-prod-type p-prod-code skip
          "Тип суммы" v-allsum-sum-type skip
                                                                      "sale-sum-base"       v-sale-sum-base        skip     "sale-sum-rubl"       v-sale-sum-rubl        skip     "sale-vat-base"       v-sale-vat-base        skip     "sale-vat-rubl"       v-sale-vat-rubl        skip     "sale-slt-base"       v-sale-slt-base        skip     "sale-slt-rubl"       v-sale-slt-rubl        skip     "sale-road-tax-base"  v-sale-road-tax-base   skip     "sale-road-tax-rubl"  v-sale-road-tax-rubl   skip     "sale-excise-base"    v-sale-excise-base     skip     "sale-excise-rubl"    v-sale-excise-rubl     skip     "sale-transport-base" v-sale-transport-base  skip     "sale-transport-rubl" v-sale-transport-rubl  skip     "sale-other-base"     v-sale-other-base      skip     "sale-other-rubl"     v-sale-other-rubl      skip     "sale-discnt-base"    v-sale-discnt-base     skip     "sale-discnt-rubl"    v-sale-discnt-rubl     skip
          view-as alert-box error .
        undo, return error v-msgstr .
      end.
      ...
      then do:
        v-msgstr =
          "Ошибка при сохранении сумм по документу" + chr(10)
          + "Тип приобретения " + v-aht-type + chr(10)
          + "Документ " + p-doc-code + chr(10)
          + "Артикул " + p-artic + " " + p-prod-type + " " + string(p-prod-code) + chr(10)
          + error-status:get-message(1) + chr(10)
          + return-value
