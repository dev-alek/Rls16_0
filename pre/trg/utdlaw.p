...
trigger procedure for write of ub.utd-lines-attr
  new buffer new-utd-lines-attr
  old buffer old-utd-lines-attr
.
...
define variable vss-description as character no-undo init "Тригер изменение utd-lines-attr".
...
define buffer buf_c-utd-lines-attr  for ub.c-utd-lines-attr .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define buffer buf_c-utd-head for ub.c-utd-head .
      buffer-compare new-utd-lines-attr to old-utd-lines-attr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("utd-lines-attr","s-c-utd-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-utd-lines-attr)
       then do:
 for first buf_c-utd-lines-attr where buf_c-utd-lines-attr.db-num eq new-utd-lines-attr.db-num
and buf_c-utd-lines-attr.doc-id eq new-utd-lines-attr.doc-id
and buf_c-utd-lines-attr.LineNum eq new-utd-lines-attr.LineNum
and buf_c-utd-lines-attr.attr-code eq new-utd-lines-attr.attr-code
  and buf_c-utd-lines-attr.corr-user-db-num   = g#db-num and buf_c-utd-lines-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-utd-lines-attr where buf_c-utd-lines-attr.db-num eq old-utd-lines-attr.db-num
and buf_c-utd-lines-attr.doc-id eq old-utd-lines-attr.doc-id
and buf_c-utd-lines-attr.LineNum eq old-utd-lines-attr.LineNum
and buf_c-utd-lines-attr.attr-code eq old-utd-lines-attr.attr-code
  and buf_c-utd-lines-attr.corr-user-db-num   = g#db-num and buf_c-utd-lines-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-utd-lines-attr)
   then do:
      if not available buf_c-utd-lines-attr
      then do:
         create buf_c-utd-lines-attr.
      buffer-copy new-utd-lines-attr to buf_c-utd-lines-attr
      assign
         buf_c-utd-lines-attr.chip-num           = v-Seq
         buf_c-utd-lines-attr.corr-date          = v-date
         buf_c-utd-lines-attr.corr-time          = v-time
         buf_c-utd-lines-attr.corr-user-db-num   = g#db-num
         buf_c-utd-lines-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-utd-lines-attr.action             = 1
         buf_c-utd-lines-attr.is-del             = false
      .
      end.
      else
         if buf_c-utd-lines-attr.action eq 99
         then
            buf_c-utd-lines-attr.action = 2.
   end.
   else do:
      if not available buf_c-utd-lines-attr
      then do:
         create buf_c-utd-lines-attr.
         buffer-copy old-utd-lines-attr to buf_c-utd-lines-attr
         assign
            buf_c-utd-lines-attr.chip-num           = v-Seq
            buf_c-utd-lines-attr.corr-date          = v-date
            buf_c-utd-lines-attr.corr-time          = v-time
            buf_c-utd-lines-attr.corr-user-db-num   = g#db-num
            buf_c-utd-lines-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-utd-lines-attr.action             = 2 when buf_c-utd-lines-attr.action ne 1
            buf_c-utd-lines-attr.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-utd-lines-attr)
         then do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq new-utd-lines-attr.db-num
and buf_c-utd-head.doc-id eq new-utd-lines-attr.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq old-utd-lines-attr.db-num
and buf_c-utd-head.doc-id eq old-utd-lines-attr.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      ...
      then do:
         ...
         buffer-copy  buf_c-utd-lines-attr to buf_c-utd-head
         assign
            buf_c-utd-head.subject = "utd-lines-attr"
