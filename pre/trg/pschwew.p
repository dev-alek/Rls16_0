...
trigger procedure for write of ub.promo-schedule-week
  new buffer new-promo-schedule-week
  old buffer old-promo-schedule-week
.
...
define variable vss-description as character no-undo init "Тригер изменение promo-schedule-week".
...
define buffer buf_c-promo-schedule-week  for ub.c-promo-schedule-week .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-promo-schedule-week to old-promo-schedule-week case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("promo-schedule-week","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-promo-chip, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-promo-schedule-week)
       then do:
 for first buf_c-promo-schedule-week where buf_c-promo-schedule-week.id eq new-promo-schedule-week.id
and buf_c-promo-schedule-week.db-num eq new-promo-schedule-week.db-num
  and buf_c-promo-schedule-week.corr-user-db-num   = g#db-num and buf_c-promo-schedule-week.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-promo-schedule-week where buf_c-promo-schedule-week.id eq old-promo-schedule-week.id
and buf_c-promo-schedule-week.db-num eq old-promo-schedule-week.db-num
  and buf_c-promo-schedule-week.corr-user-db-num   = g#db-num and buf_c-promo-schedule-week.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-promo-schedule-week)
   then do:
      if not available buf_c-promo-schedule-week
      then do:
         create buf_c-promo-schedule-week.
      buffer-copy new-promo-schedule-week to buf_c-promo-schedule-week
      assign
         buf_c-promo-schedule-week.chip-num           = v-Seq
         buf_c-promo-schedule-week.corr-date          = v-date
         buf_c-promo-schedule-week.corr-time          = v-time
         buf_c-promo-schedule-week.corr-user-db-num   = g#db-num
         buf_c-promo-schedule-week.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-promo-schedule-week.action             = 1
         buf_c-promo-schedule-week.is-del             = false
      .
      end.
      else
         if buf_c-promo-schedule-week.action eq 99
         then
            buf_c-promo-schedule-week.action = 2.
   end.
   else do:
      if not available buf_c-promo-schedule-week
      then do:
         create buf_c-promo-schedule-week.
         buffer-copy old-promo-schedule-week to buf_c-promo-schedule-week
         assign
            buf_c-promo-schedule-week.chip-num           = v-Seq
            buf_c-promo-schedule-week.corr-date          = v-date
            buf_c-promo-schedule-week.corr-time          = v-time
            buf_c-promo-schedule-week.corr-user-db-num   = g#db-num
            buf_c-promo-schedule-week.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-promo-schedule-week.action             = 2 when buf_c-promo-schedule-week.action ne 1
            buf_c-promo-schedule-week.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-promo-schedule-week
          and not new(new-promo-schedule-week)
       then do:
          buffer-compare new-promo-schedule-week
                using id db-num
                to old-promo-schedule-week
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'promo-schedule-week':U
                  ,input (buffer old-promo-schedule-week:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление promo-schedule-week для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'promo-schedule-week':U
      ,input (buffer ub.new-promo-schedule-week:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать promo-schedule-week для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
