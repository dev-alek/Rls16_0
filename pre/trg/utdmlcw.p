...
trigger procedure for write of ub.c-utd-marking-lines
  new buffer new-c-utd-marking-lines
  old buffer old-c-utd-marking-lines
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd-marking-lines".
...
if g#db-num = 0 then return.
  if not g#news then do :
       ...
       if     available old-c-utd-marking-lines
          and not new(new-c-utd-marking-lines)
       then do:
          buffer-compare new-c-utd-marking-lines
                using db-num doc-id LineNum mark corr-user-db-num chip-num
                to old-c-utd-marking-lines
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'c-utd-marking-lines':U
                  ,input (buffer old-c-utd-marking-lines:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-utd-marking-lines для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-utd-marking-lines':U
      ,input (buffer ub.new-c-utd-marking-lines:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd-marking-lines для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
