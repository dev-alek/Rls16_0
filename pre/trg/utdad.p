...
trigger procedure for delete of ub.utd-attr.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления utd-attr".
...
define buffer buf_c-utd-head for ub.c-utd-head .
    ...
    if vFlagseq
    then do:
 for first buf_c-utd-attr where buf_c-utd-attr.db-num eq utd-attr.db-num
and buf_c-utd-attr.doc-id eq utd-attr.doc-id
and buf_c-utd-attr.attr-code eq utd-attr.attr-code
  and buf_c-utd-attr.corr-user-db-num   = g#db-num and buf_c-utd-attr.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.utd-attr to buf_c-utd-attr
       assign
         ...
         buf_c-utd-attr.action             = 99
         buf_c-utd-attr.is-del             = true
       .
    end.
    else
       buf_c-utd-attr.action             = 99.
      if vFlagSeq
      then do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq utd-attr.db-num
and buf_c-utd-head.doc-id eq utd-attr.doc-id
  ...
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'utd-attr':U
       ,input (buffer ub.utd-attr:handle)
       ,input "":U
      ) no-error .
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать удаление utd-attr для отправки в новости" skip
