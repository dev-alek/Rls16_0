...
:
  ...
  if error-status :error
  then do:
    undo, return error substitute( "&1. &2&3&4", vss-include-info4, return-value, chr(10), error-status :get-message (1)).
  end.
end.
end procedure.
...
find first buf_sysconf no-lock where buf_sysconf.host-code = ub.fin-ob.host-code.
  if not g#news then do:
    if ub.fin-ob.doc-type = 'рас':U then do:
  ...
  if ub.fin-ob.status_ = 'факт':U and not g#news then do:
      if ub.fin-ob.doc-type = 'рас':U then do:
...
.
        ...
        if g#db-num = 0 and v-fo-buyer-nws = 1 then do:
                run str/callnews.p
                (input "fin-ob"
                ,input (buffer ub.fin-ob:handle)
                ) no-error .
