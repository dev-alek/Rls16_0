...
trigger procedure for write of ub.c-code
  new buffer new-c-code
  old buffer old-c-code
.
...
define variable vss-description as character no-undo init "Тригер изменение c-code".
...
define variable v-tbl-name as character no-undo.
if    (    g#db-num eq 0
   and new-c-code.nwsgbd )
   or (    g#db-num ne 0
   and new-c-code.nwsubd )
   then
do:
  if not g#news then do :
       define variable mCompare as logical no-undo.
       if     available old-c-code
          and not new(new-c-code)
       then do:
          buffer-compare new-c-code
                using parent code corr-user-db-num chip-num
                to old-c-code
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'c-Code':U
                  ,input (buffer old-c-code:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-code для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-Code':U
      ,input (buffer ub.new-c-code:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-code для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
