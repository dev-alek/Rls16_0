...
trigger procedure for write of ub.PromoGift
  new buffer new-PromoGift
  old buffer old-PromoGift
.
...
define variable vss-description as character no-undo init "Тригер изменение PromoGift".
...
define buffer buf_c-PromoGift  for ub.c-PromoGift .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-PromoGift to old-PromoGift case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("PromoGift","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-PromoGift)
       then do:
 for first buf_c-PromoGift where buf_c-PromoGift.id eq new-PromoGift.id
and buf_c-PromoGift.db-num eq new-PromoGift.db-num
  and buf_c-PromoGift.corr-user-db-num   = g#db-num and buf_c-PromoGift.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-PromoGift where buf_c-PromoGift.id eq old-PromoGift.id
and buf_c-PromoGift.db-num eq old-PromoGift.db-num
  and buf_c-PromoGift.corr-user-db-num   = g#db-num and buf_c-PromoGift.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-PromoGift)
   then do:
      if not available buf_c-PromoGift
      then do:
         create buf_c-PromoGift.
      buffer-copy new-PromoGift to buf_c-PromoGift
      assign
         buf_c-PromoGift.chip-num           = v-Seq
         buf_c-PromoGift.corr-date          = v-date
         buf_c-PromoGift.corr-time          = v-time
         buf_c-PromoGift.corr-user-db-num   = g#db-num
         buf_c-PromoGift.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-PromoGift.action             = 1
         buf_c-PromoGift.is-del             = false
      .
      end.
      else
         if buf_c-PromoGift.action eq 99
         then
            buf_c-PromoGift.action = 2.
   end.
   else do:
      if not available buf_c-PromoGift
      then do:
         create buf_c-PromoGift.
         buffer-copy old-PromoGift to buf_c-PromoGift
         assign
            buf_c-PromoGift.chip-num           = v-Seq
            buf_c-PromoGift.corr-date          = v-date
            buf_c-PromoGift.corr-time          = v-time
            buf_c-PromoGift.corr-user-db-num   = g#db-num
            buf_c-PromoGift.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-PromoGift.action             = 2 when buf_c-PromoGift.action ne 1
            buf_c-PromoGift.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-PromoGift
          and not new(new-PromoGift)
       then do:
          buffer-compare new-PromoGift
                using id db-num
                to old-PromoGift
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'PromoGift':U
                  ,input (buffer old-PromoGift:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление PromoGift для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'PromoGift':U
      ,input (buffer ub.new-PromoGift:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать PromoGift для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
