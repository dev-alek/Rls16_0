...
:
  ...
  or g#db-num > 0 then do:
    ...
    buf_c-prop-script.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    if ub.prop-script.dtm-code > 0 then do:
      ...
      to buf_c-prop-head
      assign
      buf_c-prop-head.action             = integer(if new(ub.prop-script) then '1':U else '2':U)
      buf_C-prop-head.subject            = 'prop-script':U
      .
    end.
  end.
  if not g#news
