...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1', ub.assortment-matrix.asmt-id )
    .
end procedure.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  (input  ub.assortment-matrix.obj-type
  ,input  ub.assortment-matrix.obj-code
  ,output ub.assortment-matrix.db-num-obj
  )  .
          end.
      else
          assign
    ...
    if old_assortment-matrix.asmt-db-num-create <> ? then do:
        ...
        assign
          buf_c-assortment-matrix.chip-num        = next-value (s-ref-obj-corr-chip, ub)
  ...
  if g#db-num = 0 and g#news then do:
     if ub.assortment-matrix.asmt-type = 'Объект':U and ub.assortment-matrix.asmt-status = 0 then do:
