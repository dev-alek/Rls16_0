...
trigger procedure for delete of ub.PromoGoods.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления PromoGoods".
define buffer buf_promogoods for PromoGoods.
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-PromoGoods where buf_c-PromoGoods.id eq PromoGoods.id
and buf_c-PromoGoods.db-num eq PromoGoods.db-num
  and buf_c-PromoGoods.corr-user-db-num   = g#db-num and buf_c-PromoGoods.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.PromoGoods to buf_c-PromoGoods
       assign
         ...
         buf_c-PromoGoods.action             = 99
         buf_c-PromoGoods.is-del             = true
       .
    end.
    else
       buf_c-PromoGoods.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'PromoGoods':U
       ,input (buffer ub.PromoGoods:handle)
       ,input "":U
      ) no-error .
  ...
  for each buf_promogoods where buf_promogoods.idaction eq PromoGoods.idAction
                          and buf_promogoods.IdSet      eq PromoGoods.id
  exclusive-lock:
     delete buf_promogoods.
  end.
  for each  ub.PromoAttr where ub.PromoAttr.tablename eq "PromoGoods"
                            and ub.PromoAttr.p-key     eq string(ub.PromoGoods.id) + chr(3) + string(ub.PromoGoods.db-num)
   exclusive-lock:
       delete ub.PromoAttr.
   end.
