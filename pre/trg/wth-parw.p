...
define buffer buf_c-wth-par for ub.c-wth-par.
  ...
  :
    ...
    buffer-copy oldwth-par to buf_c-wth-par
    assign
    buf_c-wth-par.wth-code           = (if p-new-record then ub.wth-par.wth-code else oldwth-par.wth-code)
    buf_c-wth-par.par-code           = (if p-new-record then ub.wth-par.par-code else oldwth-par.par-code)
    buf_c-wth-par.chip-num           = next-value (s-wth-chip, ub)
    buf_c-wth-par.corr-time          = v-time
    buf_c-wth-par.corr-user-db-num   = g#db-num
    buf_c-wth-par.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) else g#userid)
    ...
    buf_c-wth-hist.subject = 'wth-par':U
...
:
  ...
  run wth-parh_write-wth-par-trigger in this-procedure  (
                                      ...
                                      ,input (if g#news then 'db':U else "":U)
                                      ,input (if g#news then string(g#news-source-db) else "":U)
                                      ,input (if new(wth-par) then '1':U else '2':U)
                                    ) no-error .
