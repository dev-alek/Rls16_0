...
TRIGGER PROCEDURE FOR WRITE OF ub.order-doc
  new buffer new-order-doc
  old buffer old-order-doc
.
...
define variable vss-description as character no-undo init "Триггер на изменение таблицы order-doc".
...
define variable v-tbl-name as character no-undo.
if new(new-order-doc) and not g#news
then do:
  new-order-doc.db-num = g#db-num.
end.
define buffer buf_c-order-head for ub.c-order-head .
      buffer-compare new-order-doc to old-order-doc case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("order-doc","s-c-order-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-order-doc)
       then do:
 for first buf_c-order-doc where buf_c-order-doc.db-num eq new-order-doc.db-num
and buf_c-order-doc.doc-code eq new-order-doc.doc-code
  and buf_c-order-doc.corr-user-db-num   = g#db-num and buf_c-order-doc.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-order-doc where buf_c-order-doc.db-num eq old-order-doc.db-num
and buf_c-order-doc.doc-code eq old-order-doc.doc-code
  and buf_c-order-doc.corr-user-db-num   = g#db-num and buf_c-order-doc.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-order-doc)
   then do:
      if not available buf_c-order-doc
      then do:
         create buf_c-order-doc.
      buffer-copy new-order-doc to buf_c-order-doc
      assign
         buf_c-order-doc.chip-num           = v-Seq
         buf_c-order-doc.corr-date          = v-date
         buf_c-order-doc.corr-time          = v-time
         buf_c-order-doc.corr-user-db-num   = g#db-num
         buf_c-order-doc.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-order-doc.action             = 1
         buf_c-order-doc.is-del             = false
      .
      end.
      else
         if buf_c-order-doc.action eq 99
         then
            buf_c-order-doc.action = 2.
   end.
   else do:
      if not available buf_c-order-doc
      then do:
         create buf_c-order-doc.
         buffer-copy old-order-doc to buf_c-order-doc
         assign
            buf_c-order-doc.chip-num           = v-Seq
            buf_c-order-doc.corr-date          = v-date
            buf_c-order-doc.corr-time          = v-time
            buf_c-order-doc.corr-user-db-num   = g#db-num
            buf_c-order-doc.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-order-doc.action             = 2 when buf_c-order-doc.action ne 1
            buf_c-order-doc.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-order-doc)
         then do:
 for first buf_c-order-head where buf_c-order-head.db-num eq new-order-doc.db-num
and buf_c-order-head.doc-code eq new-order-doc.doc-code
  and buf_c-order-head.corr-user-db-num   = g#db-num and buf_c-order-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-order-head where buf_c-order-head.db-num eq old-order-doc.db-num
and buf_c-order-head.doc-code eq old-order-doc.doc-code
  and buf_c-order-head.corr-user-db-num   = g#db-num and buf_c-order-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      ...
      then do:
         ...
         buffer-copy  buf_c-order-doc to buf_c-order-head
         assign
            buf_c-order-head.subject = "order-doc"
