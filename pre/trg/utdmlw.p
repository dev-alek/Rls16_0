...
trigger procedure for write of ub.utd-marking-lines
  new buffer new-utd-marking-lines
  old buffer old-utd-marking-lines
.
...
define variable vss-description as character no-undo init "Тригер изменение utd-marking-lines".
...
define buffer buf_c-utd-marking-lines  for ub.c-utd-marking-lines .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
if new-utd-marking-lines.gds-code eq 0
then
   new-utd-marking-lines.gds-code = ?.
if     not g#news
   and new new-utd-marking-lines
   and new-utd-marking-lines.doc-level eq 1
then
   addMark(buffer new-utd-marking-lines ).
if     g#esys
   and new new-utd-marking-lines
then do:
  for first marking where
            marking.mark = new-utd-marking-lines.mark
      no-lock:
    if can-do(objSrv:Env:Marking:Sts:Mark:EqualChecked,string(marking.sts)) then
      new-utd-marking-lines.sts = marking.sts.
  end.
end.
define buffer buf_c-utd-head for ub.c-utd-head .
      buffer-compare new-utd-marking-lines to old-utd-marking-lines case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("utd-marking-lines","s-c-utd-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-utd-marking-lines)
       then do:
 for first buf_c-utd-marking-lines where buf_c-utd-marking-lines.db-num eq new-utd-marking-lines.db-num
and buf_c-utd-marking-lines.doc-id eq new-utd-marking-lines.doc-id
and buf_c-utd-marking-lines.LineNum eq new-utd-marking-lines.LineNum
and buf_c-utd-marking-lines.mark eq new-utd-marking-lines.mark
  and buf_c-utd-marking-lines.corr-user-db-num   = g#db-num and buf_c-utd-marking-lines.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-utd-marking-lines where buf_c-utd-marking-lines.db-num eq old-utd-marking-lines.db-num
and buf_c-utd-marking-lines.doc-id eq old-utd-marking-lines.doc-id
and buf_c-utd-marking-lines.LineNum eq old-utd-marking-lines.LineNum
and buf_c-utd-marking-lines.mark eq old-utd-marking-lines.mark
  and buf_c-utd-marking-lines.corr-user-db-num   = g#db-num and buf_c-utd-marking-lines.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-utd-marking-lines)
   then do:
      if not available buf_c-utd-marking-lines
      then do:
         create buf_c-utd-marking-lines.
      buffer-copy new-utd-marking-lines to buf_c-utd-marking-lines
      assign
         buf_c-utd-marking-lines.chip-num           = v-Seq
         buf_c-utd-marking-lines.corr-date          = v-date
         buf_c-utd-marking-lines.corr-time          = v-time
         buf_c-utd-marking-lines.corr-user-db-num   = g#db-num
         buf_c-utd-marking-lines.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-utd-marking-lines.action             = 1
         buf_c-utd-marking-lines.is-del             = false
      .
      end.
      else
         if buf_c-utd-marking-lines.action eq 99
         then
            buf_c-utd-marking-lines.action = 2.
   end.
   else do:
      if not available buf_c-utd-marking-lines
      then do:
         create buf_c-utd-marking-lines.
         buffer-copy old-utd-marking-lines to buf_c-utd-marking-lines
         assign
            buf_c-utd-marking-lines.chip-num           = v-Seq
            buf_c-utd-marking-lines.corr-date          = v-date
            buf_c-utd-marking-lines.corr-time          = v-time
            buf_c-utd-marking-lines.corr-user-db-num   = g#db-num
            buf_c-utd-marking-lines.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-utd-marking-lines.action             = 2 when buf_c-utd-marking-lines.action ne 1
            buf_c-utd-marking-lines.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-utd-marking-lines)
         then do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq new-utd-marking-lines.db-num
and buf_c-utd-head.doc-id eq new-utd-marking-lines.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq old-utd-marking-lines.db-num
and buf_c-utd-head.doc-id eq old-utd-marking-lines.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      ...
      then do:
         ...
         buffer-copy  buf_c-utd-marking-lines to buf_c-utd-head
         assign
            buf_c-utd-head.subject = "utd-marking-lines"
