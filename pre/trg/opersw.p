...
trigger procedure for write of ub.OperServ
  new buffer new-OperServ
  old buffer old-OperServ
.
...
define variable vss-description as character no-undo init "Тригер изменение OperServ".
if new-OperServ.gds-code eq 0
then
   new-OperServ.gds-code = ?.
...
define buffer buf_c-OperServ  for ub.c-OperServ .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-OperServ to old-OperServ case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("OperServ","s-c-OperServ-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    then do:
                     if new(new-OperServ)
       then do:
 for first buf_c-OperServ where buf_c-OperServ.id eq new-OperServ.id
  and buf_c-OperServ.corr-user-db-num   = g#db-num and buf_c-OperServ.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-OperServ where buf_c-OperServ.id eq old-OperServ.id
  and buf_c-OperServ.corr-user-db-num   = g#db-num and buf_c-OperServ.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-OperServ)
   then do:
      if not available buf_c-OperServ
      then do:
         create buf_c-OperServ.
      buffer-copy new-OperServ to buf_c-OperServ
      assign
         buf_c-OperServ.chip-num           = v-Seq
         buf_c-OperServ.corr-date          = v-date
         buf_c-OperServ.corr-time          = v-time
         buf_c-OperServ.corr-user-db-num   = g#db-num
         buf_c-OperServ.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-OperServ.action             = 1
         buf_c-OperServ.is-del             = false
      .
      end.
      else
         if buf_c-OperServ.action eq 99
         then
            buf_c-OperServ.action = 2.
   end.
   else do:
      if not available buf_c-OperServ
      then do:
         create buf_c-OperServ.
         buffer-copy old-OperServ to buf_c-OperServ
         assign
            buf_c-OperServ.chip-num           = v-Seq
            buf_c-OperServ.corr-date          = v-date
            buf_c-OperServ.corr-time          = v-time
            buf_c-OperServ.corr-user-db-num   = g#db-num
            buf_c-OperServ.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-OperServ.action             = 2 when buf_c-OperServ.action ne 1
            buf_c-OperServ.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-OperServ
          and not new(new-OperServ)
       then do:
          buffer-compare new-OperServ
                using id
                to old-OperServ
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'OperServ':U
                  ,input (buffer old-OperServ:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление OperServ для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'OperServ':U
      ,input (buffer ub.new-OperServ:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать OperServ для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
