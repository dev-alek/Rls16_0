...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2':u                              ,ub.bar-code-attr.b-code                              ,ub.bar-code-attr.attr-code)
    .
end procedure.
...
if (valid-handle(g#lib-nws) <> true) then do:   run nws/lib-nws.p persistent no-error .   if error-status :error or (valid-handle(g#lib-nws) <> true) then do:     message       "Error starting nws/lib-nws.p" skip       g#lib-nws skip       g#lib-nws :type skip       g#lib-nws :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-nws_get-hn-option in g#lib-nws
  ...
  ,output v-send
    ...
    buffer-copy ub.bar-code-attr to buf_c-bar-code-attr
    assign
    buf_c-bar-code-attr.gds-code           = (if p-new-record then ub.bar-code-attr.gds-code else ub.bar-code-attr.gds-code)
    buf_c-bar-code-attr.b-code             = (if p-new-record then ub.bar-code-attr.b-code else ub.bar-code-attr.b-code)
    buf_c-bar-code-attr.attr-code          = (if p-new-record then ub.bar-code-attr.attr-code else ub.bar-code-attr.attr-code)
    buf_c-bar-code-attr.chip-num           = next-value (s-gds-chip, ub)
    ...
    buf_c-gds-hist.action = (if p-new-record
                              then integer('1':U)
                              else (if ub.bar-code-attr.gds-code = ub.bar-code-attr.gds-code
                                    and ub.bar-code-attr.b-code  = ub.bar-code-attr.b-code
                                    then p-action
                                    else integer('9':U))
                            )
    buf_c-gds-hist.b-code             = (if buf_c-gds-hist.action = integer('9':U)
                                        and ub.bar-code-attr.b-code  <>  ub.bar-code-attr.b-code
                                        then ub.bar-code-attr.b-code
                                        else ub.bar-code-attr.b-code)
    buf_c-gds-hist.subject = 'bar-code-attr':U
    ...
    if p-action = integer('99':U) then do:
      ...
      OR (g#news
          and not ( g#db-num > 0 )
          and buf_c-bar-code-attr.corr-user-name <> (chr(4) +  'СПН':U)
          )
      or (g#news
          and ( g#db-num > 0 )
          and buf_c-bar-code-attr.corr-user-name = (chr(4) +  'СПН':U)
