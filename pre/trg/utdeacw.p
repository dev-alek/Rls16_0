...
trigger procedure for write of ub.c-utd-err-attr
  new buffer new-c-utd-err-attr
  old buffer old-c-utd-err-attr
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd-err-attr".
...
if g#db-num = 0 then return.
  if not g#news then do :
       ...
       if     available old-c-utd-err-attr
          and not new(new-c-utd-err-attr)
       then do:
          buffer-compare new-c-utd-err-attr
                using db-num doc-id CheckType CodeErr CheckObj reckey attr-code corr-user-db-num chip-num
                to old-c-utd-err-attr
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'c-utd-err-attr':U
                  ,input (buffer old-c-utd-err-attr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-utd-err-attr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-utd-err-attr':U
      ,input (buffer ub.new-c-utd-err-attr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd-err-attr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
