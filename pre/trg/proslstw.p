...
:
  ...
  or g#db-num <> 0 then do:
    ...
    buf_c-pscript-ruleset.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    if ub.pscript-ruleset.dtm-code > 0 then do:
      ...
      buf_c-prop-head.action             = integer(if new(ub.pscript-ruleset) then '1':U else '2':U)
      buf_C-prop-head.subject            = 'pscript-ruleset':U
      .
    end.
  end.
  if not g#news
