...
trigger procedure for delete of ub.PromoCriterion.
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-PromoCriterion where buf_c-PromoCriterion.id eq PromoCriterion.id
and buf_c-PromoCriterion.db-num eq PromoCriterion.db-num
  and buf_c-PromoCriterion.corr-user-db-num   = g#db-num and buf_c-PromoCriterion.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.PromoCriterion to buf_c-PromoCriterion
       assign
         ...
         buf_c-PromoCriterion.action             = 99
         buf_c-PromoCriterion.is-del             = true
       .
    end.
    else
       buf_c-PromoCriterion.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'PromoCriterion':U
       ,input (buffer ub.PromoCriterion:handle)
       ,input "":U
      ) no-error .
  ...
  for each ub.PromoGift where ub.PromoGift.idaction eq PromoCriterion.idAction
                          and ub.PromoGift.idCrit   eq PromoCriterion.id
  exclusive-lock:
     delete ub.PromoGift.
  end.
   for each  ub.PromoAttr where ub.PromoAttr.tablename eq "PromoCriterion"
                            and ub.PromoAttr.p-key     eq string(ub.PromoCriterion.id) + chr(3) + string(ub.PromoCriterion.db-num)
   exclusive-lock:
       delete ub.PromoAttr.
   end.
