...
:
  ...
  or g#db-num <> 0 then do:
    ...
    if not v-cmp
    and not (ub.rule-call-param.call_id begins ('layout-elem-rule':U + chr(3) ))
    then do:
      ...
      run  gen-key-fv in this-procedure (
                                        ...
                                        ,output v-field-list
                                        ,output v-value-list).
      if ub.rule-call-param.call_id begins 'dis-card-type':U then do:
        ...
        buf_c-dis-card-type.action             = integer(if new(ub.rule-call-param) then '1':U else '2':U)
        buf_c-dis-card-type.subject            = 'rule-call-param':U
        .
      end.
      if ub.rule-call-param.call_id begins 'thbj-attr':U then do:
        ...
        buf_c-thbj-attr.action             = integer(if new(ub.rule-call-param) then '1':U else '2':U)
        buf_c-thbj-attr.subject            = 'rule-call-param':U
        .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
      ...
      if ub.rule-call-param.call_id begins 'schedule':U then do:
        buf_c-rule-call-param.chip-num = next-value(s-ref-corr-chip, ub).
      end.
    end.
  end.
  if g#oxml = yes
