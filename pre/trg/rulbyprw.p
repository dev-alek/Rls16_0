...
:
  ...
  or g#db-num <> 0 then do:
    ...
    buf_c-rule-by-profile.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    assign
    buf_c-rule-profile.action = integer(if new(ub.rule-by-profile) then '1':U else '2':U)
    buf_c-rule-profile.subject = 'rule-by-profile':U
    .
  end.
  if not g#news
  and g#db-num = 0
