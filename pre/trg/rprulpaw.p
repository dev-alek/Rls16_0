...
:
  if not g#news then do:
    ...
    if not available buf_ruledict-param then do:
      ...
      find first rule_ruledict no-lock where
                rule_ruledict.entry-type = 'rule':U
  ...
  or g#db-num <> 0 then do:
    ...
    buf_c-rp-rule-param.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    assign
    buf_c-rule-profile.action = integer(if new(ub.rp-rule-param) then '1':U else '2':U)
    buf_c-rule-profile.subject = 'rp-rule-param':U
    .
  end.
  if not g#news
  and g#db-num = 0
