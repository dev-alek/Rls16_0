...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6|&7':u,ub.tax-rate-gds.fact-order,ub.tax-rate-gds.gds-code,ub.tax-rate-gds.host-code,ub.tax-rate-gds.obj-code,ub.tax-rate-gds.obj-type,ub.tax-rate-gds.rate-code,ub.tax-rate-gds.tax-code)
    .
end procedure.
...
:
  find ub.goods no-lock
    ...
    assign
    ub.tax-rate-gds.chip-num           = (if not g#news then next-value (s-gds-chip, ub) else ub.tax-rate-gds.chip-num)
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  (input  ub.tax-rate-gds.obj-type
  ,input  ub.tax-rate-gds.obj-code
  ,output v-host-code
  )  .
    end.
    assign
    v-upgrade = (if num-entries (ub.tax-rate-gds.corr-user-name, chr(4)) > 1
                and entry(2, ub.tax-rate-gds.corr-user-name, chr(4)) = 'upgrade':U
                then yes
                else no).
    ...
    buf_c-gds-hist.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) else g#userid)
    ...
    buf_c-gds-hist.action = (if new(ub.tax-rate-gds) then integer('1':U) else integer('2':U))
    buf_c-gds-hist.subject = 'tax-rate-gds':U
