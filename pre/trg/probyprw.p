...
:
  ...
  or g#db-num <> 0 then do:
    ...
    buf_c-profile-by-profile.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    assign
    buf_c-rule-profile.action = integer(if new(ub.profile-by-profile) then '1':U else '2':U)
    buf_c-rule-profile.subject = 'profile-by-profile':U
    .
  end.
  if not g#news
  and g#db-num = 0
