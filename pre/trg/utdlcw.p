...
trigger procedure for write of ub.c-utd-lines
  new buffer new-c-utd-lines
  old buffer old-c-utd-lines
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd-lines".
...
if g#db-num = 0 then return.
  if not g#news then do :
       ...
       if     available old-c-utd-lines
          and not new(new-c-utd-lines)
       then do:
          buffer-compare new-c-utd-lines
                using db-num doc-id LineNum corr-user-db-num chip-num
                to old-c-utd-lines
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'c-utd-lines':U
                  ,input (buffer old-c-utd-lines:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-utd-lines для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-utd-lines':U
      ,input (buffer ub.new-c-utd-lines:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd-lines для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
