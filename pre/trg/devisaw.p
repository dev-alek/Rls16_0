...
trigger procedure for write of ub.devisPC-attr
  new buffer new-devisPC-attr
  old buffer old-devisPC-attr
.
...
define variable vss-description as character no-undo init "Тригер изменение devisPC-attr".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
       ...
       if     available old-devisPC-attr
          and not new(new-devisPC-attr)
       then do:
          buffer-compare new-devisPC-attr
                using db-num id attr-code date time_
                to old-devisPC-attr
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'devisPC-attr':U
                  ,input (buffer old-devisPC-attr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление devisPC-attr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'devisPC-attr':U
      ,input (buffer ub.new-devisPC-attr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать devisPC-attr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
