...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5',p-key-rec,p-subject,p-subject-key,p-source-db-num,p-reaction-level)
    .
end procedure.
...
:
  find first ub.batchprocess no-lock
    where ub.batchprocess.bp_type   = 'coll':u
      and ub.batchprocess.bp_status = 'N':U
      and  ub.batchprocess.key#_one  = p-source-db-num
      and ub.batchprocess.key#_two = p-reaction-level
      and ub.batchprocess.charkey_one = p-key-rec
      and ub.batchprocess.charkey_two = p-subject
      and ub.batchprocess.charkey_three = p-subject-key
  no-error .
  if not available ub.BatchProcess then do:
    ...
    assign
      ub.BatchProcess.BP_Type       = 'coll':u
    ...
    .
    assign
      ub.BatchProcess.Key#_One      = p-source-db-num
      ub.BatchProcess.Key#_Two      = p-reaction-level
      ub.BatchProcess.CharKey_One   = p-key-rec
      ub.BatchProcess.CharKey_Two   = p-subject
      ub.BatchProcess.CharKey_Three = p-subject-key
    .
  end.
end.
