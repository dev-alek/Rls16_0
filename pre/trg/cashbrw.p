...
trigger procedure for write of ub.cashbookrule
  new buffer new-cashbookrule
  old buffer old-cashbookrule
.
...
define variable vss-description as character no-undo init "Тригер изменение cashbookrule".
...
define buffer buf_c-cashbookrule  for ub.c-cashbookrule .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
define buffer buf_c-cashbook-head for ub.c-cashbook-head .
      buffer-compare new-cashbookrule to old-cashbookrule case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("cashbookrule","s-c-cashbook-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-c-cashbook-chip-num, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-cashbookrule)
       then do:
 for first buf_c-cashbookrule where buf_c-cashbookrule.CashBookID eq new-cashbookrule.CashBookID
and buf_c-cashbookrule.Obj-type eq new-cashbookrule.Obj-type
and buf_c-cashbookrule.Obj-code eq new-cashbookrule.Obj-code
and buf_c-cashbookrule.Code eq new-cashbookrule.Code
  and buf_c-cashbookrule.corr-user-db-num   = g#db-num and buf_c-cashbookrule.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-cashbookrule where buf_c-cashbookrule.CashBookID eq old-cashbookrule.CashBookID
and buf_c-cashbookrule.Obj-type eq old-cashbookrule.Obj-type
and buf_c-cashbookrule.Obj-code eq old-cashbookrule.Obj-code
and buf_c-cashbookrule.Code eq old-cashbookrule.Code
  and buf_c-cashbookrule.corr-user-db-num   = g#db-num and buf_c-cashbookrule.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-cashbookrule)
   then do:
      if not available buf_c-cashbookrule
      then do:
         create buf_c-cashbookrule.
      buffer-copy new-cashbookrule to buf_c-cashbookrule
      assign
         buf_c-cashbookrule.chip-num           = v-Seq
         buf_c-cashbookrule.corr-date          = v-date
         buf_c-cashbookrule.corr-time          = v-time
         buf_c-cashbookrule.corr-user-db-num   = g#db-num
         buf_c-cashbookrule.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-cashbookrule.action             = 1
         buf_c-cashbookrule.is-del             = false
      .
      end.
      else
         if buf_c-cashbookrule.action eq 99
         then
            buf_c-cashbookrule.action = 2.
   end.
   else do:
      if not available buf_c-cashbookrule
      then do:
         create buf_c-cashbookrule.
         buffer-copy old-cashbookrule to buf_c-cashbookrule
         assign
            buf_c-cashbookrule.chip-num           = v-Seq
            buf_c-cashbookrule.corr-date          = v-date
            buf_c-cashbookrule.corr-time          = v-time
            buf_c-cashbookrule.corr-user-db-num   = g#db-num
            buf_c-cashbookrule.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-cashbookrule.action             = 2 when buf_c-cashbookrule.action ne 1
            buf_c-cashbookrule.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if vuniq-key-rec eq ""
            or vuniq-key-rec eq ?
         then do:
            run gen-key-rec in this-procedure ( input "c-cashbookrule"
                                               ,input (buffer buf_c-cashbookrule:handle)
                                               ,output vuniq-key-rec).
         end.
         define variable vhnc-cashbook-head as handle no-undo.
         run gen-hn-keyr-tab(input "c-cashbook-head"
                            ,input substring("c-cashbook-head",1,length("c-cashbook-head") - 5)
                            ,input  vuniq-key-rec
                            ,input ?
                            ,input  "ub"
                            ,input  ?
                            ,input  no-lock
                            ,output vhnc-cashbook-head
                        ) .
         if vhnc-cashbook-head:available
         then
            find first buf_c-cashbook-head  where rowid(buf_c-cashbook-head)           eq vhnc-cashbook-head:rowid
            exclusive-lock no-error.
         delete object vhnc-cashbook-head no-error.
      end.
      if not available  buf_c-cashbook-head
      then do:
         create buf_c-cashbook-head.
         buffer-copy  buf_c-cashbookrule to buf_c-cashbook-head
         assign
            buf_c-cashbook-head.subject = "cashbookrule"
            buf_c-cashbook-head.is-news = g#news
            buf_c-cashbook-head.source-type = (if g#news
                                          then 'db':U
                                          else (if g#esys
                                                then 'esys':U
                                                else "":U)
                                          )
            buf_c-cashbook-head.source-ref = (if g#news
         ...
         if     buf_c-cashbook-head.subject ne "*"
         then
            buf_c-cashbook-head.subject = "*".
         if buf_c-cashbook-head.action  ne 88
         then
            buf_c-cashbook-head.action             = 88 .
      end.
  if not g#news then do :
       define variable mCompare as logical no-undo.
       if     available old-cashbookrule
          and not new(new-cashbookrule)
       then do:
          buffer-compare new-cashbookrule
                using CashBookID Obj-type Obj-code Code
                to old-cashbookrule
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'CashBookRule':U
                  ,input (buffer old-cashbookrule:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление cashbookrule для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'CashBookRule':U
      ,input (buffer ub.new-cashbookrule:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать cashbookrule для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
