...
:
if lookup( ub.gds-obj-prop-attr.attr-code, 'CorrIztDel':u) = 0 then do:
  if g#news then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  (input  ub.gds-obj-prop-attr.obj-type
  ,input  ub.gds-obj-prop-attr.obj-code
  ,output v-host-code
  )  .
    create buf_c-gds-hist.
    buffer-copy buf_c-gds-obj-attr to buf_c-gds-hist
    assign
    buf_c-gds-hist.action = (if new ub.gds-obj-prop-attr then integer('1':U) else integer('2':U))
    buf_c-gds-hist.subject = 'gds-obj-prop-attr':U
    ...
    or g#db-num > 0 then do:
      run str/callnews.p ( input 'gds-obj-prop-attr':U
