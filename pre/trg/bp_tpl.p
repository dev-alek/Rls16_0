...
if ub.price-list-type.stts <> integer('0':U) then return .
...
v-recid1 = string(recid(ub.price-list-type)) .
  ...
  find first ub.batchprocess no-lock
    where ub.batchprocess.bp_type   = 'twotpl':U
      and ub.batchprocess.bp_status = 'N':U
      and ub.batchprocess.charkey_one = v-recid1
      and ub.batchprocess.charkey_two = v-recid
  no-error .
  if not available ub.BatchProcess then do:
    ...
    assign
      ub.BatchProcess.BP_Type       = 'twotpl':U
    ...
    .
    assign
      ub.BatchProcess.CharKey_One   = v-recid1
      ub.BatchProcess.CharKey_Two   = v-recid
    .
  end.
      end.
      else do:
