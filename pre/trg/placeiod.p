...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3', ub.place-io.obj-type, ub.place-io.obj-code, ub.place-io.place-io-code )
    .
end procedure.
...
main-block:
do
on error  undo main-block, return error substitute("&1. error &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message ( 1 ) )
...
:
  ...
  buffer-copy ub.place-io to buf_c-place-io
    assign
      buf_c-place-io.chip-num         = dynamic-next-value( "s-chip-place-io", "ub")
  ...
  if error-status :error then do:
    return error substitute( "&1. Ошибка при отправке в новости команды на удаление записи. &1&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message ( error-status :num-messages ) ).
  end.
