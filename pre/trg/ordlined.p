...
trigger procedure for delete of ub.order-line.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления order-line".
define buffer buf_order-line-attr for ub.order-line-attr.
for each buf_order-line-attr where
         buf_order-line-attr.db-num   = order-line.db-num
     and buf_order-line-attr.doc-code = order-line.doc-code
    exclusive-lock:
  delete buf_order-line-attr.
end.
...
define buffer buf_c-order-head for ub.c-order-head .
    ...
    if vFlagseq
    then do:
 for first buf_c-order-line where buf_c-order-line.db-num eq order-line.db-num
and buf_c-order-line.doc-code eq order-line.doc-code
and buf_c-order-line.line-num eq order-line.line-num
  and buf_c-order-line.corr-user-db-num   = g#db-num and buf_c-order-line.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.order-line to buf_c-order-line
       assign
         ...
         buf_c-order-line.action             = 99
         buf_c-order-line.is-del             = true
       .
    end.
    else
       buf_c-order-line.action             = 99.
      if vFlagSeq
      then do:
 for first buf_c-order-head where buf_c-order-head.db-num eq order-line.db-num
and buf_c-order-head.doc-code eq order-line.doc-code
