...
trigger procedure for delete of ub.cashbook.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления cashbook".
...
define buffer buf_c-cashbook-head for ub.c-cashbook-head .
    ...
    if vFlagseq
    then do:
 for first buf_c-cashbook where buf_c-cashbook.id eq cashbook.id
  and buf_c-cashbook.corr-user-db-num   = g#db-num and buf_c-cashbook.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.cashbook to buf_c-cashbook
       assign
         ...
         buf_c-cashbook.action             = 99
         buf_c-cashbook.is-del             = true
       .
    end.
    else
       buf_c-cashbook.action             = 99.
  ...
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'CashBook':U
       ,input (buffer ub.cashbook:handle)
       ,input "":U
      ) no-error .
...
for each cashbookrule where CashBookRule.CashBookID eq cashbook.id
exclusive-lock:
   delete cashbookrule.
end.
for each cashbookattr where CashBookAttr.id eq cashbook.id
exclusive-lock:
   delete cashbookattr.
end.
