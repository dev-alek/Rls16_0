...
trigger procedure for write of ub.code
   new buffer new-code
   old buffer old-code
   .
...
define variable vss-description as character no-undo init "Тригер изменение code".
...
define buffer buf_c-code  for ub.c-code .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-code to old-code case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("code","s-c-code", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-c-code, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-code)
       then do:
 for first buf_c-code where buf_c-code.parent eq new-code.parent
and buf_c-code.code eq new-code.code
  and buf_c-code.corr-user-db-num   = g#db-num and buf_c-code.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-code where buf_c-code.parent eq old-code.parent
and buf_c-code.code eq old-code.code
  and buf_c-code.corr-user-db-num   = g#db-num and buf_c-code.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-code)
   then do:
      if not available buf_c-code
      then do:
         create buf_c-code.
      buffer-copy new-code to buf_c-code
      assign
         buf_c-code.chip-num           = v-Seq
         buf_c-code.corr-date          = v-date
         buf_c-code.corr-time          = v-time
         buf_c-code.corr-user-db-num   = g#db-num
         buf_c-code.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-code.action             = 1
         buf_c-code.is-del             = false
      .
      end.
      else
         if buf_c-code.action eq 99
         then
            buf_c-code.action = 2.
   end.
   else do:
      ...
      then do:
         create buf_c-code.
         buffer-copy old-code to buf_c-code
         assign
            buf_c-code.chip-num           = v-Seq
            buf_c-code.corr-date          = v-date
            buf_c-code.corr-time          = v-time
            buf_c-code.corr-user-db-num   = g#db-num
            buf_c-code.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-code.action             = 2 when buf_c-code.action ne 1
            buf_c-code.is-del             = false
         .
      end.
   end.
...
if new-code.parent  ne ""
then do:
   if num-entries (new-code.parent,chr(4)) > 1
   then do:
      mcode = entry(num-entries (new-code.parent,chr(4)),
                    new-code.parent,
                    chr(4)).
      mparent = substring (new-code.parent,1 ,length (new-code.parent) - length (chr(4) + mcode)).
   end.
   else
      mcode = new-code.parent.
   ...
   then do:
      ...
      assign
         ...
         buf_code.nwsgbd   = new-code.nwsgbd
         buf_code.nwsubd   = new-code.nwsubd
         buf_Code.export_  = new-code.export_
      .
   end.
end.
if     new-code.nwsgbd ne old-code.nwsgbd
   and new-code.nwsubd ne old-code.nwsubd
   then
do:
   for each buf_code where buf_code.parent begins new-code.code
      exclusive-lock:
      buf_code.nwsgbd = new-code.nwsgbd.
      buf_code.nwsubd = new-code.nwsubd.
   end.
end.
if old-code.code ne new-code.code
then do:
   for each buf_code where buf_code.parent eq old-code.parent + chr(4) + old-code.code
      exclusive-lock:
      buf_code.parent = new-code.parent + chr(4) + new-code.code.
   end.
end.
if new-code.parent begins "MarkType" then
do:
    vErrorMsg = "".
    if new-code.code      = "" or
       new-code.codevalue = "" or
       new-code.codename  = "" or
       new-code.misc1     = "" or
       new-code.misc2     = "" or
       new-code.misc3     = "" or
       new-code.misc4     = "" then
    ...
    if vErrorMsg = "" and not new(new-code) and
       new-code.codevalue <> old-code.codevalue then
    do:
    ...
    do:
       find first buf_code where
                  buf_code.parent    =  new-code.parent
              and buf_code.code      <> new-code.code
              and buf_code.codevalue =  new-code.codevalue
            no-lock no-error.
    ...
    do:
      if not new(new-code) then
        assign
          new-code.codevalue = old-code.codevalue
          new-code.misc1 = old-code.misc1
        .
    end.
...
if g#db-num <> 0 and
   new-code.parent <> "" and
   not new-code.parent begins "Versions" then
do:
   def var vi    as int no-undo.
   mParent = substitute("Versions&1&2",chr(4),0).
   mcode = new-code.parent.
   find first buf_code where
              buf_code.parent = mParent
          and buf_code.code = new-code.parent
        no-lock no-error.
   if not avail buf_code then
   do:
     vi = num-entries(new-code.parent,chr(4)).
     do while vi > 1 and not avail buf_code :
        vi = vi - 1.
        if r-index(mcode,chr(4)) > 0
        then
           mcode = substring(mcode,1,r-index(mcode,chr(4)) - 1).
        find first buf_code where
              buf_code.parent = mParent
   ...
   do:
      ...
      mParent = substitute("Versions&1&2",chr(4),g#db-num).
      find first buf_code where
                 buf_code.parent = mParent
             and buf_code.code   = mcode
           no-error.
...
do:
  if not g#news then do :
       ...
       if     available old-code
          and not new(new-code)
       then do:
          buffer-compare new-code
                using parent code
                to old-code
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'Code':U
                  ,input (buffer old-code:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление code для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'Code':U
      ,input (buffer ub.new-code:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать code для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
