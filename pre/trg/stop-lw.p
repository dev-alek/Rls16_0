...
:
  if not (old-stop-list.user-db-num = ub.stop-list.user-db-num
    ...
    to buf_c-stop-list
    assign
    buf_c-stop-list.action = (if new(ub.stop-list) then integer('1':U) else integer('2':U))
    ...
    buf_c-stop-list.chip-num = next-value(s-ref-corr-chip, ub)
    .
  end.
  if not g#news
  then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdburt in g#library
  (output ub.stop-list.user-db-num
  ,output ub.stop-list.user-name
  ,output ub.stop-list.sys-date
  ,output ub.stop-list.sys-time
  ,output ub.stop-list.sys-time-int
  )  .
  end.
  ...
  and old-stop-list.status_ = 'факт':U
  and ub.stop-list.status_  <> old-stop-list.status_ then do:
    message
      ...
      "Стоплист закрыты до статуса" 'факт':U skip
      "Нельзя изменить статус стоплиста на" ub.stop-list.status_ skip
      view-as alert-box error .
    undo main-block, return error.
  end.
  if  not g#news
  and ub.stop-list.status_ = 'факт':U
  then do:
    run str/callnews.p
      (input 'stop-list':U
