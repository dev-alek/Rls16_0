...
:
  ...
  and new(ub.rp-by-call) then do:
    if ub.rp-by-call.call#_id = 0 then do:
      run rul/g-callid.p ( input entry(1, ub.rp-by-call.call_Id, chr(4))
  ...
  or g#db-num <> 0 then do:
    ...
    if not v-cmp then do:
      ...
      run  gen-key-fv in this-procedure (
                                        ...
                                        ,output v-field-list
                                        ,output v-value-list).
      if ub.rp-by-call.call_id begins 'dis-card-type':U then do:
        ...
        buf_c-dis-card-type.action             = integer(if new(ub.rp-by-call) then '1':U else '2':U)
        buf_c-dis-card-type.subject            = 'rp-by-call':U
        .
      end.
      if ub.rp-by-call.call_id begins 'thbj-attr':U then do:
        ...
        buf_c-thbj-attr.action             = integer(if new(ub.rp-by-call) then '1':U else '2':U)
        buf_c-thbj-attr.subject            = 'rp-by-call':U
        .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
      ...
      if ub.rp-by-call.call_id begins 'schedule':U then do:
        buf_c-rp-by-call.chip-num = next-value(s-ref-corr-chip, ub).
      end.
    end.
  end.
  if g#oxml = yes
