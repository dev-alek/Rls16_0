...
:
  ...
  assign
    ...
    buf_c-user-account.corr-user-name   = (if g#news then (chr(4) +  'СПН':U) else g#userid)
    buf_c-user-account.chip-num         = v-chip-num
  .
  create buf_c-usr-hist .
  buffer-copy buf_c-user-account to buf_c-usr-hist
  assign
    buf_c-usr-hist.subject = 'user-account':U
    buf_c-usr-hist.action  = (if new(ub.user-account) then integer('1':U) else integer('2':U))
  ...
  .
  run trg/userlog.p (
                      input if new(ub.user-account) then 'create':U else 'update':U
                    , input 'c-user-account':U
