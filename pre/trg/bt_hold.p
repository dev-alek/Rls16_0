  ...
  :
    ...
    for each buf_hold-time share-lock
      where buf_hold-time.cat-code  = p-cat-code
        ...
        and buf_hold-time.status_   = 'удаленные':U
    on error undo, return error
    :
      if v-get-ro_read-only = false
      then do:
        run trg/harhrclc.p
          (input buf_hold-time.cat-code
          ,input p-lock-code
          ,input p-btpr-type-code
          ,input buf_hold-time.start-date
          ) no-error .
        if error-status :error
        then do:
          return error return-value .
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_archive-hold_update':U
    ,input  'global':U
    ,input  0
    ,input  '':U
    ,input  0
  ...
  if update_batchprocess.bp_status <> 'N':U then do:
    ...
    define variable v-btpr_upd-today-6 as date      no-undo.
  define variable v-btpr_upd-time-6  as integer   no-undo.
  run cur-time in this-procedure ( output v-btpr_upd-today-6
                                 , output v-btpr_upd-time-6
                                 ).
  assign
    update_batchprocess.bp_status         = 'D':U
    update_batchprocess.bp_execcounttries = update_batchprocess.bp_execcounttries + 1
    update_batchprocess.bp_execuser_id    = g#userid
    update_batchprocess.bp_execsysdate    = v-btpr_upd-today-6
    update_batchprocess.bp_execsystime    = string(v-btpr_upd-time-6, 'hh:mm')
    update_batchprocess.bp_execsystimeint = v-btpr_upd-time-6
  .
                run trg/harhtclc.p
                  (input p-cat-code
                  ,input p-lock-code
