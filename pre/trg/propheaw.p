...
:
  if not g#news
  then do:
    run gen-key-rec in this-procedure ( input 'prop-head':U
  ...
  or g#db-num <> 0 then do:
    ...
    assign
    buf_c-prop-head.action             = integer(if new(ub.prop-head) then '1':U else '2':U)
    buf_C-prop-head.subject            = 'prop-head':U
    buf_c-prop-head.dtm-code           = ub.prop-head.dtm-code
    buf_c-prop-head.chip-num           = next-value (s-ref-corr-chip, ub)
