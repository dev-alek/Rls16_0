...
if g#news = false then do:
   ...
   if ub.gds-obj-prop.gdop-igt = "" or ub.gds-obj-prop.gdop-igt = ? then do:
      assign
        ub.gds-obj-prop.gdop-igt = 'Пусто':U
        .
   end.
...
if (valid-handle(g#lib-nws) <> true) then do:   run nws/lib-nws.p persistent no-error .   if error-status :error or (valid-handle(g#lib-nws) <> true) then do:     message       "Error starting nws/lib-nws.p" skip       g#lib-nws skip       g#lib-nws :type skip       g#lib-nws :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-nws_get-hn-option in g#lib-nws
  ...
  or v-send >= 0 then do:
    ...
    buf_c-gds-obj-prop.chip-num         = next-value (s-gds-chip , ub)
    ...
    .
    if ub.gds-obj-prop.obj-type <> 'орг':U then do:
define variable vss-include-info7 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  (input  ub.gds-obj-prop.obj-type
  ,input  ub.gds-obj-prop.obj-code
  ,output v-host-code
  )  .
    end.
    else do:
    ...
    assign
    buf_c-gds-hist.action = (if new ub.gds-obj-prop then integer('1':U) else integer('2':U))
    buf_c-gds-hist.subject = 'gds-obj-prop':U
...
find first buf_assortment-matrix no-lock where
           ...
           buf_assortment-matrix.obj-code    = ub.gds-obj-prop.obj-code and
           buf_assortment-matrix.asmt-status = 0 and
           buf_assortment-matrix.asmt-type   = 'Объект':U
           no-error .
find first buf_assortment-matrix-goods no-lock where
           buf_assortment-matrix-goods.asmg-status = 0 and
           buf_assortment-matrix-goods.gds-code    = ub.gds-obj-prop.gds-code and
...
find first del_assortment-matrix-goods no-lock where
           ...
           no-error .
if (new(ub.gds-obj-prop) AND ub.gds-obj-prop.gdop-igt = 'Новинка':U) OR
...
define buffer x-gds-obj-prop-attr for ub.gds-obj-prop-attr  .
  ...
  :
   find first x-gds-obj-prop-attr no-lock WHERE
              ...
              x-gds-obj-prop-attr.attr-code = 'CorrIztDel':U no-error .
   if not available x-gds-obj-prop-attr then do:
      ...
      assign
        ...
        x-gds-obj-prop-attr.attr-code = 'CorrIztDel':U
