...
TRIGGER PROCEDURE FOR WRITE OF ub.ext-classif old old-ext-classif.
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6|&7|&8|&9'
...
:
  if lookup(ub.ext-classif.classif-name, 'th-th150_clients,th-th14_clients,th-th150_goods,th-th14_goods,gds-grp,th-th_gds-grp,gds-grp,th-th_gds-grp,goods_fibegais-transId,FormF1-esys':U) > 0
  and lookup(ub.ext-classif.classif-name, 'th-th150_clients,th-th14_clients,th-th150_goods,th-th14_goods,gds-grp,th-th_gds-grp,rpm_gds-grp,goods_fib':U) > 0
  then return.
  if g#news then do:
...
if (valid-handle(g#lib-nws) <> true) then do:   run nws/lib-nws.p persistent no-error .   if error-status :error or (valid-handle(g#lib-nws) <> true) then do:     message       "Error starting nws/lib-nws.p" skip       g#lib-nws skip       g#lib-nws :type skip       g#lib-nws :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-nws_get-hn-option in g#lib-nws
  ...
  if (not g#news
  or v-send >= 0)
  and lookup(ub.ext-classif.classif-name, 'th-th150_clients,th-th14_clients,th-th150_goods,th-th14_goods,gds-grp,th-th_gds-grp,gds-grp,th-th_gds-grp,goods_fibegais-transId,FormF1-esys':U) = 0
  then do:
    run cur-time in this-procedure(output v-date, output v-time).
    create buf_c-ext-classif.
    buffer-copy old-ext-classif to buf_c-ext-classif
    assign
    buf_c-ext-classif.classif-subject    = ub.ext-classif.classif-subject
    buf_c-ext-classif.uniq-key-rec       = ub.ext-classif.uniq-key-rec
    buf_c-ext-classif.classif-name       = ub.ext-classif.classif-name
    buf_c-ext-classif.charkey_one        = ub.ext-classif.charkey_one
    buf_c-ext-classif.charkey_two        = ub.ext-classif.charkey_two
    buf_c-ext-classif.charkey_three      = ub.ext-classif.charkey_three
    buf_c-ext-classif.key#_one           = ub.ext-classif.key#_one
    buf_c-ext-classif.key#_two           = ub.ext-classif.key#_two
    buf_c-ext-classif.key#_three         = ub.ext-classif.key#_three
    buf_c-ext-classif.db-num             = ub.ext-classif.db-num
    ...
    .
    if lookup(ub.ext-classif.classif-name, 'oss-ref,rvd-reason,reasons-suspicious-check,egais-transId,FormF1-esys':U) = 0 then
        do:
      ...
      case ub.ext-classif.classif-subject :
        when 'clients':U then do:
          ...
          v-obj-code = integer(entry(lookup("obj-code":U
                                      ...
                                      , v-value-list, chr(3)))
          no-error .
          buf_c-ext-classif.chip-num           = next-value (s-cli-chip, ub).
          create buf_c-cli-hist.
          buffer-copy buf_c-ext-classif
          to buf_c-cli-hist
          assign
          buf_c-cli-hist.subject  = 'ext-classif':U
          buf_c-cli-hist.obj-type = v-obj-type
          buf_c-cli-hist.obj-code = v-obj-code
          buf_c-cli-hist.action   = integer( if new ub.ext-classif then '1':U else '2':U )
          .
        end.
        when 'goods':U then do:
          if ub.ext-classif.uniq-key-rec begins 'goods':U then
          do:
            assign
              v-gds-code = integer(entry(lookup("gds-code":U
                                                ...
                                                , chr(3))
                                          , v-value-list, chr(3)))
              buf_c-ext-classif.chip-num           = next-value (s-gds-chip, ub)
            .
            create buf_c-gds-hist.
            buffer-copy buf_c-ext-classif
              to buf_c-gds-hist
              assign
              buf_c-gds-hist.subject = 'ext-classif':U
              buf_c-gds-hist.gds-code = v-gds-code
            .
          end.
          else
            buf_c-ext-classif.chip-num = next-value (s-ref-corr-chip, ub).
        end.
        otherwise do:
          buf_c-ext-classif.chip-num           = next-value (s-ref-corr-chip, ub).
        end.
  ...
  if lookup(ub.ext-classif.classif-name, 'th-th150_clients,th-th14_clients,th-th150_goods,th-th14_goods,gds-grp,th-th_gds-grp,rpm_gds-grp,goods_fib':U) = 0 then do:
  run str/callnews.p
    ( input 'ext-classif':U
