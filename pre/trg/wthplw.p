...
define buffer buf_c-wth-place for ub.c-wth-place.
  ...
  :
    ...
    buffer-copy oldwth-place to buf_c-wth-place
    assign
    buf_c-wth-place.host-code           = (if p-new-record then ub.wth-place.host-code else oldwth-place.host-code)
    buf_c-wth-place.obj-type           = (if p-new-record then ub.wth-place.obj-type else oldwth-place.obj-type)
    buf_c-wth-place.obj-code           = (if p-new-record then ub.wth-place.obj-code else oldwth-place.obj-code)
    buf_c-wth-place.w-p-code           = (if p-new-record then ub.wth-place.w-p-code else oldwth-place.w-p-code)
    buf_c-wth-place.chip-num           = next-value (s-wth-chip, ub)
    buf_c-wth-place.corr-time          = v-time
    buf_c-wth-place.corr-user-db-num   = g#db-num
    buf_c-wth-place.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) else g#userid)
...
:
  ...
  if not g#news then do:
    run wth-placeh_write-wth-place-trigger in this-procedure  (
                                        ...
                                        ,input (if new(wth-place) then '1':U else '2':U)
                                      ) no-error .
    if error-status:error then do:
        message
