...
trigger procedure for write of ub.upgrade-attr
  new buffer new-upgrade-attr
  old buffer old-upgrade-attr
.
...
define variable vss-description as character no-undo init "Тригер изменение upgrade-attr".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
       ...
       if     available old-upgrade-attr
          and not new(new-upgrade-attr)
       then do:
          buffer-compare new-upgrade-attr
                using db-num version-num attr-code
                to old-upgrade-attr
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'upgrade-attr':U
                  ,input (buffer old-upgrade-attr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление upgrade-attr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'upgrade-attr':U
      ,input (buffer ub.new-upgrade-attr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать upgrade-attr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
