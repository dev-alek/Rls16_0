...
trigger procedure for delete of ub.promo-schedule.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления promo-schedule".
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-promo-schedule where buf_c-promo-schedule.id eq promo-schedule.id
and buf_c-promo-schedule.db-num eq promo-schedule.db-num
  and buf_c-promo-schedule.corr-user-db-num   = g#db-num and buf_c-promo-schedule.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.promo-schedule to buf_c-promo-schedule
       assign
         ...
         buf_c-promo-schedule.action             = 99
         buf_c-promo-schedule.is-del             = true
       .
    end.
    else
       buf_c-promo-schedule.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'promo-schedule':U
       ,input (buffer ub.promo-schedule:handle)
       ,input "":U
      ) no-error .
