...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6'                             , ub.point-point-rel.from-db-num                              , ub.point-point-rel.from-point-code                              , ub.point-point-rel.to-db-num                              , ub.point-point-rel.to-point-code                              , ub.point-point-rel.deliv-type-code                              , ub.point-point-rel.cond-keep-code                               )
    .
end procedure.
...
do on error undo main-block, return error return-value :
  ...
  buf_c-point-point-rel.chip-num  = next-value (s-chip-point-io, ub) .
define variable vss-include-info1 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdburt in g#library
  (output buf_c-point-point-rel.corr-user-db-num
  ,output buf_c-point-point-rel.corr-user-name
  ,output buf_c-point-point-rel.corr-date
  ,output v-sys-time
  ,output buf_c-point-point-rel.corr-time
  )  .
  run str/callnews.p ( input 'point-point-rel':U
                     , input (buffer ub.point-point-rel:handle) ) .
    if g#oxml = yes
    then do:
    run str/calloxml.p (
          input 'update':U
        , input 'point-point-rel':U
        , input ( buffer ub.point-point-rel:handle )
    ) no-error.
