...
TRIGGER PROCEDURE FOR WRITE OF ub.order-line
  new buffer new-order-line
  old buffer old-order-line
.
...
define variable vss-description as character no-undo init "Триггер на изменение таблицы order-line".
...
define buffer buf_c-order-head for ub.c-order-head .
      buffer-compare new-order-line to old-order-line case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("order-line","s-c-order-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-order-line)
       then do:
 for first buf_c-order-line where buf_c-order-line.db-num eq new-order-line.db-num
and buf_c-order-line.doc-code eq new-order-line.doc-code
and buf_c-order-line.line-num eq new-order-line.line-num
  and buf_c-order-line.corr-user-db-num   = g#db-num and buf_c-order-line.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-order-line where buf_c-order-line.db-num eq old-order-line.db-num
and buf_c-order-line.doc-code eq old-order-line.doc-code
and buf_c-order-line.line-num eq old-order-line.line-num
  and buf_c-order-line.corr-user-db-num   = g#db-num and buf_c-order-line.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-order-line)
   then do:
      if not available buf_c-order-line
      then do:
         create buf_c-order-line.
      buffer-copy new-order-line to buf_c-order-line
      assign
         buf_c-order-line.chip-num           = v-Seq
         buf_c-order-line.corr-date          = v-date
         buf_c-order-line.corr-time          = v-time
         buf_c-order-line.corr-user-db-num   = g#db-num
         buf_c-order-line.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-order-line.action             = 1
         buf_c-order-line.is-del             = false
      .
      end.
      else
         if buf_c-order-line.action eq 99
         then
            buf_c-order-line.action = 2.
   end.
   else do:
      if not available buf_c-order-line
      then do:
         create buf_c-order-line.
         buffer-copy old-order-line to buf_c-order-line
         assign
            buf_c-order-line.chip-num           = v-Seq
            buf_c-order-line.corr-date          = v-date
            buf_c-order-line.corr-time          = v-time
            buf_c-order-line.corr-user-db-num   = g#db-num
            buf_c-order-line.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-order-line.action             = 2 when buf_c-order-line.action ne 1
            buf_c-order-line.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-order-line)
         then do:
 for first buf_c-order-head where buf_c-order-head.db-num eq new-order-line.db-num
and buf_c-order-head.doc-code eq new-order-line.doc-code
  and buf_c-order-head.corr-user-db-num   = g#db-num and buf_c-order-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-order-head where buf_c-order-head.db-num eq old-order-line.db-num
and buf_c-order-head.doc-code eq old-order-line.doc-code
  and buf_c-order-head.corr-user-db-num   = g#db-num and buf_c-order-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      ...
      then do:
         ...
         buffer-copy  buf_c-order-line to buf_c-order-head
         assign
            buf_c-order-head.subject = "order-line"
