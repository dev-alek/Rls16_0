...
trigger procedure for delete of ub.PromoGift.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления PromoGift".
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-PromoGift where buf_c-PromoGift.id eq PromoGift.id
and buf_c-PromoGift.db-num eq PromoGift.db-num
  and buf_c-PromoGift.corr-user-db-num   = g#db-num and buf_c-PromoGift.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.PromoGift to buf_c-PromoGift
       assign
         ...
         buf_c-PromoGift.action             = 99
         buf_c-PromoGift.is-del             = true
       .
    end.
    else
       buf_c-PromoGift.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'PromoGift':U
       ,input (buffer ub.PromoGift:handle)
       ,input "":U
      ) no-error .
...
for each  ub.PromoAttr where ub.PromoAttr.tablename eq "PromoGift"
                            and ub.PromoAttr.p-key     eq string(ub.PromoGift.id) + chr(3) + string(ub.PromoGift.db-num)
   exclusive-lock:
       delete ub.PromoAttr.
   end.
