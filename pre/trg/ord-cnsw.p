...
TRIGGER PROCEDURE FOR WRITE OF ub.ord-cons .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdburt in g#library
  (output ub.ord-cons.user-db-num
  ,output ub.ord-cons.user-name
  ,output ub.ord-cons.sys-date
  ,output ub.ord-cons.sys-time
  ,output ub.ord-cons.sys-time-int
  )  .
  end.
  if ub.ORD-cons.status_ = 'новый':U
  then do:
    ...
    assign
      ub.ORD-cons.fact-num = next-value (s-ORD-fact, ub)
    .
  end.
    if ub.ORD-cons.status_ = 'распределение':U then do:
      assign
        ub.ORD-cons.creid = g#userid
      .
    end.
if g#db-num = 0 then do:
  if ub.ORD-cons.status_ = 'факт':U then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  ,output l-shift-on
      ...
      run factord in this-procedure
        (input  ub.ORD-cons.fact-date
        ,input  ub.ORD-cons.fact-time
        ,input  ub.ORD-cons.fact-num
        ,input  ub.ORD-cons.shift-date
        ,input  ub.ORD-cons.shift-num
        ,input  l-shift-on
        ,output v-fact-order
