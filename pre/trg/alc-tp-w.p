...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2':u, ub.alc-type.alc-type-inner-code, ub.alc-type.create-user-db-num)
    .
end procedure.
...
:
   ...
   then do:
      ...
      assign
         bf_c-alc-type.chip-num         = next-value (s-alc-type-chip, ub)
   ...
   then do:
      undo main-block, return error substitute("&1. Невозможно маршрутизировать alc-type для отправки в новости &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message ( 1 ) ).
   end.
   if g#oxml = yes
   then do:
   run str/calloxml.p (
