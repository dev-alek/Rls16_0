...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3',  ub.scales-attr.db-num
...
:
  ...
  if p-hist then do:
    if not g#news then do:
      ...
      buf_c-scales-attr.chip-num           = next-value (s-scales-chip, ub)
      ...
      assign
      buf_c-scales.subject            = 'scales-attr':U
      buf_c-scales.action             = integer(if new(ub.scales-attr) then '1':U else '2':U)
      .
    end.
  end.
  if p-news then do:
