...
define variable vss-description as character no-undo init "—оздать запись о том, что необходимо переслать запись группы меню меню на кассы".
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6',p-obj-type, p-obj-code, p-node-code, p-upper-code, p-out-code, p-action)
    .
end procedure.
...
:
  find first ub.batchprocess no-lock
    where ub.batchprocess.bp_type   = 'fgrp':U
      and ub.batchprocess.bp_status = 'N':U
      and  ub.batchprocess.key#_one  = p-obj-code
      and ub.batchprocess.key#_two = p-node-code
      and ub.batchprocess.key#_three = p-upper-code
      and ub.batchprocess.charkey_one = p-obj-type
      and ub.batchprocess.charkey_two = string(p-out-code)
      and ub.batchprocess.charkey_three = string(p-upper-out-code) + chr(44) + string(p-lvl-num)
  no-error .
  if not available ub.BatchProcess then do:
    ...
    assign
      ub.BatchProcess.BP_Type       = 'fgrp':U
    ...
    .
    assign
      ub.BatchProcess.Key#_One      = p-obj-code
      ub.BatchProcess.Key#_Two      = p-node-code
      ub.BatchProcess.Key#_Three    = p-upper-code
      ub.BatchProcess.CharKey_One   = p-obj-type
      ub.BatchProcess.CharKey_Two   = string(p-out-code)
      ub.BatchProcess.CharKey_Three = string(p-upper-out-code) + chr(44) + string(p-lvl-num)
    .
  end.
