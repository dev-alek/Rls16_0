...
:
  ...
  if not g#news then do:
    ...
    buf_c-fbr-prn-grp.chip-num           = next-value (s-scales-chip, ub)
    .
    ...
    buf_c-fbr-prn.subject            = 'fbr-prn-grp':U
    buf_c-fbr-prn.action             = integer(if new(ub.fbr-prn-grp) then '1':U else '2':U)
    .
  end.
  if g#oxml = yes
  then do:
