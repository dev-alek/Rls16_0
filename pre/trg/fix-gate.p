...
define new shared temp-table tt-clob-data no-undo like ub.clob-data . find first buf_temp-tables where buf_temp-tables.tbl-name = "clob-data" no-error. if not available buf_temp-tables then do:   create buf_temp-tables.   assign   buf_temp-tables.tbl-name = "clob-data"    buf_temp-tables.buf-handle = buffer tt-clob-data:handle    buf_temp-tables.tbl-handle = buf_temp-tables.buf-handle:table-handle   .   release buf_temp-tables. end.
define new shared temp-table tt-clob-bind no-undo like ub.clob-bind . find first buf_temp-tables where buf_temp-tables.tbl-name = "clob-bind" no-error. if not available buf_temp-tables then do:   create buf_temp-tables.   assign   buf_temp-tables.tbl-name = "clob-bind"    buf_temp-tables.buf-handle = buffer tt-clob-bind:handle    buf_temp-tables.tbl-handle = buf_temp-tables.buf-handle:table-handle   .   release buf_temp-tables. end.
...
then do:
   ...
   and p-read-only then do:
    ...
    run gbl/md5.p (
      input  "cmp/fix-gate.txt"
    ,output v-md5-signature
    ) .
  if v-md5-signature <> "C2DEC59EF921E3FE00FF75C4995E0C05" then do:
    message
  ...
  or buf_tt-clob-bind.descr <> "v15_1.60" then do:
    message
