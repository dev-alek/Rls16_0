...
:
  if not g#news then do:
    if ub.ruledict.uniq-key-rec = '':u then do:
      run gen-key-rec in this-procedure ( input 'ruledict':U
  ...
  or g#db-num <> 0 then do:
    ...
    buf_c-ruledict.action             = integer(if new(ub.ruledict) then '1':U else '2':U)
    buf_C-ruledict.subject            = 'ruledict':U
    buf_c-ruledict.entry-id            = ub.ruledict.entry-id
    buf_c-ruledict.chip-num           = next-value (s-ref-corr-chip, ub)
