...
:
  ...
  and ub.dis-rule.sts <> integer('1':U)
  and ub.dis-rule.rule-num < 93
  then do:
    message
  ...
  if not g#news then do:
    if ( g#db-num > 0 ) then do:
      ...
      or ub.dis-rule.obj-code = 0 then do:
        ...
        if buf_dis-rule.sts <> integer('1':U) then do:
          message
          vss-workfile vss-revision vss-description skip
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  ,output v-db-num
    ...
    if can-find(first buf_dis-rule no-lock where
                ...
                AND buf_dis-rule.rule-num < 93 ) then do:
      message
      vss-workfile vss-revision vss-description skip
  ...
  if ub.dis-rule.upper-rule-num <= 99999
  then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objdbnum in g#library
  ...
  buf_c-dis-rule.action             = integer('99':U)
  .
  if g#oxml = yes
  then do:
    run str/calloxml.p (
