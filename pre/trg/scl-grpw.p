...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3',  ub.scales-grp.db-num
...
:
  ...
  if not g#news then do:
    ...
    buf_c-scales-grp.chip-num           = next-value (s-scales-chip, ub)
    ...
    assign
    buf_c-scales.subject            = 'scales-grp':U
    buf_c-scales.action             = integer(if new(ub.scales-grp) then '1':U else '2':U)
    .
  end.
    run str/callnews.p
    ( input 'scales-grp':U
