...
if ub.staff-attr.attr-code =  "CashierQRCode" then do:
  ...
  do:
    find first ub.clients NO-LOCK WHERE ub.clients.obj-type = 'чел':U and ub.clients.obj-code = ub.staff.psn-code no-error .
...
ELSE (substring (p-name,1, 25) +
                FILL (chr(32), 25 - LENGTH (substring (p-name, 1, 25)) )) +
                '---  УДАЛЕН  ---':U).
  end.
END PROCEDURE.
end.
