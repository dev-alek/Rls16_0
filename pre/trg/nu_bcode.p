...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5',p-b-code,p-gds-code,p-part-code,p-in-code,p-unit-cli,p-action)
    .
end procedure.
...
:
  find first ub.batchprocess no-lock
    where ub.batchprocess.bp_type   = 'bcode':U
      and ub.batchprocess.bp_status = 'N':U
      and  ub.batchprocess.key#_one  = p-b-code
      and ub.batchprocess.key#_two = p-node-code
      and ub.batchprocess.key#_three = p-gds-code
      and ub.batchprocess.charkey_one = p-part-code
      and ub.batchprocess.charkey_two = p-in-code
      and ub.batchprocess.charkey_three = p-unit-cli
  no-error .
  if not available ub.BatchProcess then do:
    ...
    assign
      ub.BatchProcess.BP_Type       = 'bcode':U
    ...
    .
    assign
      ub.BatchProcess.Key#_One      = p-b-code
      ub.BatchProcess.Key#_Two      = p-node-code
      ub.BatchProcess.Key#_Three    = p-gds-code
      ub.BatchProcess.CharKey_One   = p-part-code
      ub.BatchProcess.CharKey_Two   = p-in-code
      ub.BatchProcess.CharKey_Three = p-unit-cli
    .
  end.
