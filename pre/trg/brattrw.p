...
trigger procedure for write of ub.cashbookruleattr
  new buffer new-cashbookruleattr
  old buffer old-cashbookruleattr
.
...
define variable vss-description as character no-undo init "Тригер изменение cashbookruleattr".
...
define buffer buf_c-cashbookruleattr  for ub.c-cashbookruleattr .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-cashbookruleattr to old-cashbookruleattr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("cashbookruleattr","s-c-cashbook-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-cashbookruleattr)
       then do:
 for first buf_c-cashbookruleattr where buf_c-cashbookruleattr.cashbookid eq new-cashbookruleattr.cashbookid
and buf_c-cashbookruleattr.obj-type eq new-cashbookruleattr.obj-type
and buf_c-cashbookruleattr.obj-code eq new-cashbookruleattr.obj-code
and buf_c-cashbookruleattr.code eq new-cashbookruleattr.code
and buf_c-cashbookruleattr.attr-code eq new-cashbookruleattr.attr-code
  and buf_c-cashbookruleattr.corr-user-db-num   = g#db-num and buf_c-cashbookruleattr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-cashbookruleattr where buf_c-cashbookruleattr.cashbookid eq old-cashbookruleattr.cashbookid
and buf_c-cashbookruleattr.obj-type eq old-cashbookruleattr.obj-type
and buf_c-cashbookruleattr.obj-code eq old-cashbookruleattr.obj-code
and buf_c-cashbookruleattr.code eq old-cashbookruleattr.code
and buf_c-cashbookruleattr.attr-code eq old-cashbookruleattr.attr-code
  and buf_c-cashbookruleattr.corr-user-db-num   = g#db-num and buf_c-cashbookruleattr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-cashbookruleattr)
   then do:
      if not available buf_c-cashbookruleattr
      then do:
         create buf_c-cashbookruleattr.
      buffer-copy new-cashbookruleattr to buf_c-cashbookruleattr
      assign
         buf_c-cashbookruleattr.chip-num           = v-Seq
         buf_c-cashbookruleattr.corr-date          = v-date
         buf_c-cashbookruleattr.corr-time          = v-time
         buf_c-cashbookruleattr.corr-user-db-num   = g#db-num
         buf_c-cashbookruleattr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-cashbookruleattr.action             = 1
         buf_c-cashbookruleattr.is-del             = false
      .
      end.
      else
         if buf_c-cashbookruleattr.action eq 99
         then
            buf_c-cashbookruleattr.action = 2.
   end.
   else do:
      if not available buf_c-cashbookruleattr
      then do:
         create buf_c-cashbookruleattr.
         buffer-copy old-cashbookruleattr to buf_c-cashbookruleattr
         assign
            buf_c-cashbookruleattr.chip-num           = v-Seq
            buf_c-cashbookruleattr.corr-date          = v-date
            buf_c-cashbookruleattr.corr-time          = v-time
            buf_c-cashbookruleattr.corr-user-db-num   = g#db-num
            buf_c-cashbookruleattr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-cashbookruleattr.action             = 2 when buf_c-cashbookruleattr.action ne 1
            buf_c-cashbookruleattr.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-cashbookruleattr
          and not new(new-cashbookruleattr)
       then do:
          buffer-compare new-cashbookruleattr
                using cashbookid obj-type obj-code code attr-code
                to old-cashbookruleattr
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'CashBookRuleAttr':U
                  ,input (buffer old-cashbookruleattr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление cashbookruleattr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'CashBookRuleAttr':U
      ,input (buffer ub.new-cashbookruleattr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать cashbookruleattr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
