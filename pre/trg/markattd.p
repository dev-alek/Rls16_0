...
trigger procedure for delete of ub.marking-attr.
...
:
  if marking-attr.attr-code = "notOnlineCheck" then
  do:
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-marking-attr where buf_c-marking-attr.mark eq marking-attr.mark
and buf_c-marking-attr.attr-code eq marking-attr.attr-code
  and buf_c-marking-attr.corr-user-db-num   = g#db-num and buf_c-marking-attr.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.marking-attr to buf_c-marking-attr
       assign
         ...
         buf_c-marking-attr.action             = 99
         buf_c-marking-attr.is-del             = true
       .
    end.
    else
       buf_c-marking-attr.action             = 99.
