  ...
  .
    ...
    and can-find(first buf_cash-desk where
      ...
      run str/sendclia.p (
                    ...
                    , input p-log-handle
                    , (string(g#db-num) + chr(4) + "optimize":U + chr(4) + "yes":U + chr(4) + "S":U)
                    )
      no-error .
...
procedure close-dc-list :
  ...
  find first buf_batchprocess exclusive-lock
    where rowid(buf_batchprocess) = buf_temp-dc-list.bp_rowid
    no-error .
  ...
  if buf_batchprocess.bp_status <> 'N':U then do:
    ...
    define variable v-btpr_upd-today-9 as date      no-undo.
  define variable v-btpr_upd-time-9  as integer   no-undo.
  run cur-time in this-procedure ( output v-btpr_upd-today-9
                                 , output v-btpr_upd-time-9
                                 ).
  assign
    ...
    buf_batchprocess.bp_execsysdate    = v-btpr_upd-today-9
    buf_batchprocess.bp_execsystime    = string(v-btpr_upd-time-9, 'hh:mm')
    buf_batchprocess.bp_execsystimeint = v-btpr_upd-time-9
  .
      end.
    end.
  end.
