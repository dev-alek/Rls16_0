...
define variable vss-description as character no-undo init "“риггер на запись в таблице ”—Ћќ¬»я ’–јЌ≈Ќ»я".
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1', ub.condition-keeping.cond-keep-code)
    .
end procedure.
...
:
  if not g#news then do:
    if ( g#db-num > 0 ) then do:
      ...
      "Ќельз€ измен€ть запись ”—Ћќ¬»я ’–јЌ≈Ќ»я в ”Ѕƒ" skip
  ...
  if not g#news then do:
    ...
    buf_c-condition-keeping.chip-num           = next-value (s-corr-chip, ub)
  ...
  run str/callnews.p
    ...
    then do:
        undo, return error substitute( "&2&1ќшибка при отправке записи в систему OpenXML&1&3&1&4"
                             , chr(10)
                             , vss-workfile
                             , return-value
