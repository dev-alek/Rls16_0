...
trigger procedure for write of ub.PromoGoods
  new buffer new-PromoGoods
  old buffer old-PromoGoods
.
...
define variable vss-description as character no-undo init "Тригер изменение PromoGoods".
...
define buffer buf_c-PromoGoods  for ub.c-PromoGoods .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-PromoGoods to old-PromoGoods case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("PromoGoods","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-PromoGoods)
       then do:
 for first buf_c-PromoGoods where buf_c-PromoGoods.id eq new-PromoGoods.id
and buf_c-PromoGoods.db-num eq new-PromoGoods.db-num
  and buf_c-PromoGoods.corr-user-db-num   = g#db-num and buf_c-PromoGoods.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-PromoGoods where buf_c-PromoGoods.id eq old-PromoGoods.id
and buf_c-PromoGoods.db-num eq old-PromoGoods.db-num
  and buf_c-PromoGoods.corr-user-db-num   = g#db-num and buf_c-PromoGoods.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-PromoGoods)
   then do:
      if not available buf_c-PromoGoods
      then do:
         create buf_c-PromoGoods.
      buffer-copy new-PromoGoods to buf_c-PromoGoods
      assign
         buf_c-PromoGoods.chip-num           = v-Seq
         buf_c-PromoGoods.corr-date          = v-date
         buf_c-PromoGoods.corr-time          = v-time
         buf_c-PromoGoods.corr-user-db-num   = g#db-num
         buf_c-PromoGoods.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-PromoGoods.action             = 1
         buf_c-PromoGoods.is-del             = false
      .
      end.
      else
         if buf_c-PromoGoods.action eq 99
         then
            buf_c-PromoGoods.action = 2.
   end.
   else do:
      if not available buf_c-PromoGoods
      then do:
         create buf_c-PromoGoods.
         buffer-copy old-PromoGoods to buf_c-PromoGoods
         assign
            buf_c-PromoGoods.chip-num           = v-Seq
            buf_c-PromoGoods.corr-date          = v-date
            buf_c-PromoGoods.corr-time          = v-time
            buf_c-PromoGoods.corr-user-db-num   = g#db-num
            buf_c-PromoGoods.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-PromoGoods.action             = 2 when buf_c-PromoGoods.action ne 1
            buf_c-PromoGoods.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-PromoGoods
          and not new(new-PromoGoods)
       then do:
          buffer-compare new-PromoGoods
                using id db-num
                to old-PromoGoods
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'PromoGoods':U
                  ,input (buffer old-PromoGoods:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление PromoGoods для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'PromoGoods':U
      ,input (buffer ub.new-PromoGoods:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать PromoGoods для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
