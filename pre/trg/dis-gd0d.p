...
:
  ...
  if avail buf_dis-gds-rule then do:
      if buf_dis-gds-rule.templ-rl-root = 91 and buf_dis-gds-rule.pos-type = 'NCR-AS@R':U then do:
          if buf_dis-gds-rule.obj-type = 'орг':U or buf_dis-gds-rule.obj-type = '' then do:
              for each buf_clients no-lock
              where buf_clients.db-num = g#db-num
                and ( buf_clients.obj-type = 'маг':U or buf_clients.obj-type = 'скл':U )
                and ( if buf_dis-gds-rule.obj-type = 'орг':U then buf_clients.host-code = buf_dis-gds-rule.obj-code else true )
              :
                  assign v-obj-lst = v-obj-lst + ( if v-obj-lst = '' then '' else chr(4) ) + string(buf_clients.obj-code) .
              end.
          end.
          else  assign v-obj-lst = string(buf_dis-gds-rule.obj-code) .
          for each buf_cash-desk no-lock
          where lookup( string(buf_cash-desk.obj-code), v-obj-lst, chr(4) ) > 0
            and buf_cash-desk.db-num   = g#db-num
            and buf_cash-desk.pos-type = 'NCR-AS@R':U
          ...
          :
              if first-of(buf_cash-desk.obj-code) then do:
                   ...
                   if not error-status:error then do:
                       run str/ncr-bnsd.p (
                            string( recid(buf_dis-gds-rule) ) + chr(4) +
                            out + chr(4) +
                            ub.dis-gds-rule-attr.attr-code + chr(4) +
  ...
  if error-status :error then do:
      return error substitute( "&1. Ошибка при передачи данных об удалении записи на кассу. &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message ( error-status :num-messages ) ).
  end.
end.
