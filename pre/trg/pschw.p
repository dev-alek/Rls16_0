...
trigger procedure for write of ub.promo-schedule
  new buffer new-promo-schedule
  old buffer old-promo-schedule
.
...
define variable vss-description as character no-undo init "Тригер изменение promo-schedule".
...
define buffer buf_c-promo-schedule  for ub.c-promo-schedule .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-promo-schedule to old-promo-schedule case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("promo-schedule","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-promo-schedule)
       then do:
 for first buf_c-promo-schedule where buf_c-promo-schedule.id eq new-promo-schedule.id
and buf_c-promo-schedule.db-num eq new-promo-schedule.db-num
  and buf_c-promo-schedule.corr-user-db-num   = g#db-num and buf_c-promo-schedule.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-promo-schedule where buf_c-promo-schedule.id eq old-promo-schedule.id
and buf_c-promo-schedule.db-num eq old-promo-schedule.db-num
  and buf_c-promo-schedule.corr-user-db-num   = g#db-num and buf_c-promo-schedule.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-promo-schedule)
   then do:
      if not available buf_c-promo-schedule
      then do:
         create buf_c-promo-schedule.
      buffer-copy new-promo-schedule to buf_c-promo-schedule
      assign
         buf_c-promo-schedule.chip-num           = v-Seq
         buf_c-promo-schedule.corr-date          = v-date
         buf_c-promo-schedule.corr-time          = v-time
         buf_c-promo-schedule.corr-user-db-num   = g#db-num
         buf_c-promo-schedule.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-promo-schedule.action             = 1
         buf_c-promo-schedule.is-del             = false
      .
      end.
      else
         if buf_c-promo-schedule.action eq 99
         then
            buf_c-promo-schedule.action = 2.
   end.
   else do:
      if not available buf_c-promo-schedule
      then do:
         create buf_c-promo-schedule.
         buffer-copy old-promo-schedule to buf_c-promo-schedule
         assign
            buf_c-promo-schedule.chip-num           = v-Seq
            buf_c-promo-schedule.corr-date          = v-date
            buf_c-promo-schedule.corr-time          = v-time
            buf_c-promo-schedule.corr-user-db-num   = g#db-num
            buf_c-promo-schedule.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-promo-schedule.action             = 2 when buf_c-promo-schedule.action ne 1
            buf_c-promo-schedule.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-promo-schedule
          and not new(new-promo-schedule)
       then do:
          buffer-compare new-promo-schedule
                using id db-num
                to old-promo-schedule
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'promo-schedule':U
                  ,input (buffer old-promo-schedule:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление promo-schedule для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'promo-schedule':U
      ,input (buffer ub.new-promo-schedule:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать promo-schedule для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
