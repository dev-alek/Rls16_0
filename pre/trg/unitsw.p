...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1', ub.units.unit-name)
    .
end procedure.
...
define variable vss-include-info1 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  assign
    old-type  = (if lookup ('шту':U, oldb.type)     = 0 then "0" else "1")
              + (if lookup ('дро':U, oldb.type) = 0 then "0" else "1")
              + (if lookup ('сер':U, oldb.type)     = 0 then "0" else "1")
              + (if lookup ('вес':U, oldb.type)     = 0 then "0" else "1")
              + (if lookup ('топ':U, oldb.type)  = 0 then "0" else "1")
              + (if lookup ('2ед':U, oldb.type)    = 0 then "0" else "1")
              + (if lookup ('доп':U, oldb.type)    = 0 then "0" else "1")
              + (if lookup ('сте':U, oldb.type)     = 0 then "0" else "1")
  ...
  if old-type <> new-type then do:
    if lookup ((old-type + "-" + new-type),
      ...
      message
        ...
        "Единица измерения" ub.units.unit-name skip
        "Изменение типа:" oldb.type "->" ub.units.type skip
    ...
    do:
        ...
        buffer-copy oldb to buf_c-units
            ...
            buf_c-units.chip-num           = next-value (s-ref-corr-chip, ub)
