...
:
  run str/callnews.p ( input 'pump-nozzle':U
  ...
  if g#news <> yes then do:
    ...
    buf_c-pump-nozzle.chip-num           = next-value (s-pmp-chip, ub)
    ...
    assign
    buf_c-pmp-hist.action = (if new ub.pump-nozzle then integer('1':U) else integer('2':U))
    buf_c-pmp-hist.subject = 'pump-nozzle':U
    ...
    buf_c-nzl-hist.action = (if new ub.pump-nozzle then integer('1':U) else integer('2':U))
    buf_c-nzl-hist.subject = 'pump-nozzle':U
    ...
    buf_c-table-bind.tbl-name-src   = 'c-pmp-hist':U
    buf_c-table-bind.is-news         = g#news
    buf_c-table-bind.corr-user-name  = g#userid
    buf_c-table-bind.subject         = 'pump-nozzle':U
    .
  end.
  if g#oxml = yes
  then do:
