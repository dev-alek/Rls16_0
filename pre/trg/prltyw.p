...
run cur-time in this-procedure ( output v-today, output start-time ) .
      ...
      assign
        ub.c-price-list-type.chip-num           = next-value (s-corr-chip, ub)
    ...
    .
   if ub.price-list-type.stts <> integer('0':U) then do:
       for each ub.batchprocess exclusive-lock
           where ub.batchprocess.bp_type   = 'twotpl':U and
                 ub.batchprocess.BP_Status     = 'N':U and
