...
TRIGGER PROCEDURE FOR WRITE OF ub.marking-attr
   new buffer new-marking-attr
   old buffer old-marking-attr
   .
...
define buffer buf_c-marking-attr  for ub.c-marking-attr .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
:
  if new-marking-attr.attr-code = "notOnlineCheck" or
     new-marking-attr.attr-code = "weight" then
  do:
      buffer-compare new-marking-attr to old-marking-attr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("marking-attr","s-c-mark-attr-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
       v-Seq  = next-value (s-c-mark-attr-chip-num, ub).
    else
       vFlagSeq = yes.
    if vFlagSeq
    then do:
                     if new(new-marking-attr)
       then do:
 for first buf_c-marking-attr where buf_c-marking-attr.mark eq new-marking-attr.mark
and buf_c-marking-attr.attr-code eq new-marking-attr.attr-code
  and buf_c-marking-attr.corr-user-db-num   = g#db-num and buf_c-marking-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-marking-attr where buf_c-marking-attr.mark eq old-marking-attr.mark
and buf_c-marking-attr.attr-code eq old-marking-attr.attr-code
  and buf_c-marking-attr.corr-user-db-num   = g#db-num and buf_c-marking-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-marking-attr)
   then do:
      if not available buf_c-marking-attr
      then do:
         create buf_c-marking-attr.
      buffer-copy new-marking-attr to buf_c-marking-attr
      assign
         buf_c-marking-attr.chip-num           = v-Seq
         buf_c-marking-attr.corr-date          = v-date
         buf_c-marking-attr.corr-time          = v-time
         buf_c-marking-attr.corr-user-db-num   = g#db-num
         buf_c-marking-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-marking-attr.action             = 1
         buf_c-marking-attr.is-del             = false
      .
      end.
      else
         if buf_c-marking-attr.action eq 99
         then
            buf_c-marking-attr.action = 2.
   end.
   else do:
      if not available buf_c-marking-attr
      then do:
         create buf_c-marking-attr.
         buffer-copy old-marking-attr to buf_c-marking-attr
         assign
            buf_c-marking-attr.chip-num           = v-Seq
            buf_c-marking-attr.corr-date          = v-date
            buf_c-marking-attr.corr-time          = v-time
            buf_c-marking-attr.corr-user-db-num   = g#db-num
            buf_c-marking-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-marking-attr.action             = 2 when buf_c-marking-attr.action ne 1
            buf_c-marking-attr.is-del             = false
         .
      end.
   end.
