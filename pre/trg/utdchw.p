...
trigger procedure for write of ub.c-utd-head
  new buffer new-c-utd-head
  old buffer old-c-utd-head
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd-head".
...
if g#db-num = 0 then return.
  if not g#news then do :
     run str/callnews.p
      (input 'c-utd-head':U
      ,input (buffer ub.new-c-utd-head:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd-head для отправки в новости" skip
