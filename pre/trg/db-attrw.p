...
:
  ...
  if p-news = true then do:
    ...
    if error-status :error then do:
      undo, return error substitute( "&1. Невозможно маршрутизировать db-attr для отправки в новости. &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message ( error-status :num-messages ) ).
    end.
  end.
  if g#oxml = yes
  then do:
