...
trigger procedure for write of ub.PromoCriterion
  new buffer new-PromoCriterion
  old buffer old-PromoCriterion
.
...
define variable vss-description as character no-undo init "Тригер изменение PromoCriterion".
...
define buffer buf_c-PromoCriterion  for ub.c-PromoCriterion .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-PromoCriterion to old-PromoCriterion case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("PromoCriterion","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-PromoCriterion)
       then do:
 for first buf_c-PromoCriterion where buf_c-PromoCriterion.id eq new-PromoCriterion.id
and buf_c-PromoCriterion.db-num eq new-PromoCriterion.db-num
  and buf_c-PromoCriterion.corr-user-db-num   = g#db-num and buf_c-PromoCriterion.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-PromoCriterion where buf_c-PromoCriterion.id eq old-PromoCriterion.id
and buf_c-PromoCriterion.db-num eq old-PromoCriterion.db-num
  and buf_c-PromoCriterion.corr-user-db-num   = g#db-num and buf_c-PromoCriterion.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-PromoCriterion)
   then do:
      if not available buf_c-PromoCriterion
      then do:
         create buf_c-PromoCriterion.
      buffer-copy new-PromoCriterion to buf_c-PromoCriterion
      assign
         buf_c-PromoCriterion.chip-num           = v-Seq
         buf_c-PromoCriterion.corr-date          = v-date
         buf_c-PromoCriterion.corr-time          = v-time
         buf_c-PromoCriterion.corr-user-db-num   = g#db-num
         buf_c-PromoCriterion.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-PromoCriterion.action             = 1
         buf_c-PromoCriterion.is-del             = false
      .
      end.
      else
         if buf_c-PromoCriterion.action eq 99
         then
            buf_c-PromoCriterion.action = 2.
   end.
   else do:
      if not available buf_c-PromoCriterion
      then do:
         create buf_c-PromoCriterion.
         buffer-copy old-PromoCriterion to buf_c-PromoCriterion
         assign
            buf_c-PromoCriterion.chip-num           = v-Seq
            buf_c-PromoCriterion.corr-date          = v-date
            buf_c-PromoCriterion.corr-time          = v-time
            buf_c-PromoCriterion.corr-user-db-num   = g#db-num
            buf_c-PromoCriterion.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-PromoCriterion.action             = 2 when buf_c-PromoCriterion.action ne 1
            buf_c-PromoCriterion.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-PromoCriterion
          and not new(new-PromoCriterion)
       then do:
          buffer-compare new-PromoCriterion
                using id db-num
                to old-PromoCriterion
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'PromoCriterion':U
                  ,input (buffer old-PromoCriterion:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление PromoCriterion для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'PromoCriterion':U
      ,input (buffer ub.new-PromoCriterion:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать PromoCriterion для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
