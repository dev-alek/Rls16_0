...
define stream slog .
define temp-table doc-list no-undo    field doc-code   like ub.trn-doc.doc-code     field fact-order like ub.trn-doc.fact-order   field fact-date  like ub.trn-doc.fact-date    field obj-type   like ub.trn-doc.obj-type     field obj-code   like ub.trn-doc.obj-code     field batchprocess_rowid as rowid             index xpk is primary unique doc-code          index xfact obj-type obj-code fact-order   index xdate fact-date  .
    ...
    then do:
      ...
      then do:
        for each BatchProcess exclusive-lock
          where BatchProcess.bp_type       = 'ahsp':U
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  p-check-act-user-id
    ,input  0
    ,input  'actn_archive-ahsp_update':U
    ,input  'object':U
...
do:
  ...
  if buf_batchprocess.bp_status <> 'N':U then do:
    ...
    define variable v-btpr_upd-today-10 as date      no-undo.
  define variable v-btpr_upd-time-10  as integer   no-undo.
  run cur-time in this-procedure ( output v-btpr_upd-today-10
                                 , output v-btpr_upd-time-10
                                 ).
  assign
    buf_batchprocess.bp_status         = 'D':U
    buf_batchprocess.bp_execcounttries = buf_batchprocess.bp_execcounttries + 1
    buf_batchprocess.bp_execuser_id    = g#userid
    buf_batchprocess.bp_execsysdate    = v-btpr_upd-today-10
    buf_batchprocess.bp_execsystime    = string(v-btpr_upd-time-10, 'hh:mm')
    buf_batchprocess.bp_execsystimeint = v-btpr_upd-time-10
  .
