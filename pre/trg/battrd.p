...
trigger procedure for delete of ub.cashbookattr.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления cashbookattr".
...
define buffer buf_c-cashbook-head for ub.c-cashbook-head .
    ...
    if vFlagseq
    then do:
 for first buf_c-cashbookattr where buf_c-cashbookattr.id eq cashbookattr.id
and buf_c-cashbookattr.attr-code eq cashbookattr.attr-code
  and buf_c-cashbookattr.corr-user-db-num   = g#db-num and buf_c-cashbookattr.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.cashbookattr to buf_c-cashbookattr
       assign
         ...
         buf_c-cashbookattr.action             = 99
         buf_c-cashbookattr.is-del             = true
       .
    end.
    else
       buf_c-cashbookattr.action             = 99.
  ...
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'CashBookAttr':U
       ,input (buffer ub.cashbookattr:handle)
       ,input "":U
      ) no-error .
