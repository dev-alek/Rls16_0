...
TRIGGER PROCEDURE FOR WRITE OF ub.cash-param-hist
  NEW BUFFER new-cash-param-hist
  OLD BUFFER old-cash-param-hist
.
...
define variable vss-description as character no-undo init "Тригер изменение cash-param-hist".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
       ...
       if     available old-cash-param-hist
          and not new(new-cash-param-hist)
       then do:
          buffer-compare new-cash-param-hist
                using obj-type obj-code cash-num param_section param_group param_name
                to old-cash-param-hist
                case-sensitive
                save result in mCompare .
          if not mCompare
          then do:
             run nws/cmd-del.p
                 ( input 'Cash-param-hist':U
                  ,input (buffer old-cash-param-hist:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление cash-param-hist для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'Cash-param-hist':U
      ,input (buffer ub.new-cash-param-hist:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать cash-param-hist для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
