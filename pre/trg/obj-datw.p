...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4', ub.obj-date.obj-type, ub.obj-date.obj-code, ub.obj-date.sys-date, ub.obj-date.status_)
    .
end procedure.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  (input  ub.obj-date.obj-type
  ,input  ub.obj-date.obj-code
  ,input  'check-exist':u
  ,output l-obj-exist
  ) no-error .
  ...
  if new ub.obj-date = false then do:
    ...
    if  old-obj-date.status_ = 'зкр':U
    and ub.obj-date.status_  <> 'зкр':U
    and not g#news
    then do:
      message
        vss-workfile vss-revision vss-description skip
  ...
  if lookup (ub.obj-date.status_, 'тек,зкр':U) = 0 then do:
    message
      vss-workfile vss-revision vss-description skip
  ...
  if not g#news then do:
    if ub.obj-date.status_ = 'тек':U then do:
      find first next_obj-date
        where next_obj-date.obj-type = ub.obj-date.obj-type
          and next_obj-date.obj-code = ub.obj-date.obj-code
      ...
      find first prev_obj-date
        where prev_obj-date.obj-type = ub.obj-date.obj-type
          ...
          and prev_obj-date.status_  = 'тек':U
    ...
    if ub.obj-date.status_ = 'зкр':U  then do:
      run cur-time in this-procedure ( output v-today
                                     , output v-time
                                     ).
      assign
