...
:
  ...
  then do:
    ...
    assign
    buf_c-rule.action             = integer(if new(ub.rule) then '1':U else '2':U)
  ...
  if not g#news
  and g#db-num = 0
  and ub.rule.sts <> integer('-10':U)
  and not ((v-result  = 'sts' and ub.rule.sts = integer('99':U))
           or
           (v-result  = 'sts' and ub.rule.sts = integer('98':U))
         )
  then do:
    run str/callnews.p
      (input 'rule':U
