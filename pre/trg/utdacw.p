...
trigger procedure for write of ub.c-utd-attr
  new buffer new-c-utd-attr
  old buffer old-c-utd-attr
.
...
define variable vss-description as character no-undo init "Тригер изменение c-utd-attr".
...
if g#db-num = 0 then return.
  if not g#news then do :
       ...
       if     available old-c-utd-attr
          and not new(new-c-utd-attr)
       then do:
          buffer-compare new-c-utd-attr
                using db-num doc-id attr-code corr-user-db-num chip-num
                to old-c-utd-attr
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'c-utd-attr':U
                  ,input (buffer old-c-utd-attr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-utd-attr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-utd-attr':U
      ,input (buffer ub.new-c-utd-attr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-utd-attr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
