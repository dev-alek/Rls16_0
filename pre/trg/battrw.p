...
trigger procedure for write of ub.cashbookattr
  new buffer new-cashbookattr
  old buffer old-cashbookattr
.
...
define variable vss-description as character no-undo init "Тригер изменение cashbookattr".
...
define buffer buf_c-cashbookattr  for ub.c-cashbookattr .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define buffer buf_c-cashbook-head for ub.c-cashbook-head .
      buffer-compare new-cashbookattr to old-cashbookattr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("cashbookattr","s-c-cashbook-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-cashbookattr)
       then do:
 for first buf_c-cashbookattr where buf_c-cashbookattr.id eq new-cashbookattr.id
and buf_c-cashbookattr.attr-code eq new-cashbookattr.attr-code
  and buf_c-cashbookattr.corr-user-db-num   = g#db-num and buf_c-cashbookattr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-cashbookattr where buf_c-cashbookattr.id eq old-cashbookattr.id
and buf_c-cashbookattr.attr-code eq old-cashbookattr.attr-code
  and buf_c-cashbookattr.corr-user-db-num   = g#db-num and buf_c-cashbookattr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-cashbookattr)
   then do:
      if not available buf_c-cashbookattr
      then do:
         create buf_c-cashbookattr.
      buffer-copy new-cashbookattr to buf_c-cashbookattr
      assign
         buf_c-cashbookattr.chip-num           = v-Seq
         buf_c-cashbookattr.corr-date          = v-date
         buf_c-cashbookattr.corr-time          = v-time
         buf_c-cashbookattr.corr-user-db-num   = g#db-num
         buf_c-cashbookattr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-cashbookattr.action             = 1
         buf_c-cashbookattr.is-del             = false
      .
      end.
      else
         if buf_c-cashbookattr.action eq 99
         then
            buf_c-cashbookattr.action = 2.
   end.
   else do:
      if not available buf_c-cashbookattr
      then do:
         create buf_c-cashbookattr.
         buffer-copy old-cashbookattr to buf_c-cashbookattr
         assign
            buf_c-cashbookattr.chip-num           = v-Seq
            buf_c-cashbookattr.corr-date          = v-date
            buf_c-cashbookattr.corr-time          = v-time
            buf_c-cashbookattr.corr-user-db-num   = g#db-num
            buf_c-cashbookattr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-cashbookattr.action             = 2 when buf_c-cashbookattr.action ne 1
            buf_c-cashbookattr.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if vuniq-key-rec eq ""
            or vuniq-key-rec eq ?
         then do:
            run gen-key-rec in this-procedure ( input "c-cashbookattr"
                                               ,input (buffer buf_c-cashbookattr:handle)
      ...
      then do:
         ...
         buffer-copy  buf_c-cashbookattr to buf_c-cashbook-head
         assign
            buf_c-cashbook-head.subject = "cashbookattr"
  ...
  if not g#news then do :
       ...
       if     available old-cashbookattr
          and not new(new-cashbookattr)
       then do:
          buffer-compare new-cashbookattr
                using id attr-code
                to old-cashbookattr
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'CashBookAttr':U
                  ,input (buffer old-cashbookattr:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление cashbookattr для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'CashBookAttr':U
      ,input (buffer ub.new-cashbookattr:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать cashbookattr для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
