...
:
  if ub.rule.upper_rule_id = 0 then do:
    if ub.rule.sts <> integer('-10':U)
    and ub.rule.sts <> integer('-1':U)
    and ub.rule.sts <> integer('1':U)
    and g#news = no
    then do:
       message
       "Нельзя удалить правило" skip
       "правило находится в статусе" entry (lookup (string(ub.rule.sts), '-1,-10,0,1,99,98':U), 'готов,нов,исп,удал,удаление,запр.удал':U)
  ...
  if ub.rule.upper_rule_id = 0 then do:
    ...
    or g#db-num <> 0 then do:
      ...
      assign
      buf_c-rule.action             = integer('99':U)
      buf_C-rule.subject            = 'rule':U
      buf_c-rule.rule_id            = ub.rule.rule_id
      buf_c-rule.chip-num           = next-value (s-ref-corr-chip, ub)
