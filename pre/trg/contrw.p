...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3', ub.contract.contract-code, ub.contract.host-code, ub.contract.status_)
    .
end procedure.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curdburt in g#library
  ...
  ,output v-Sys-Time-Int
     ...
     IF ERROR-STATUS:ERROR THEN DO:                               MESSAGE                                                      vss-workfile vss-revision vss-description SKIP            ERROR-STATUS:GET-MESSAGE(1) SKIP                          RETURN-VALUE SKIP                                         ""                                                        VIEW-AS ALERT-BOX ERROR                                 .                                                         UNDO main-block, RETURN ERROR                                 substitute( "&1. &2&3&4",                                              vss-workfile,                                             return-value,                                             chr(10),                                              ERROR-STATUS :GET-MESSAGE(1)                             ).                                    END.
