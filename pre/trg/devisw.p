...
trigger procedure for write of ub.devisPC
  new buffer new-devisPC
  old buffer old-devisPC
.
...
define variable vss-description as character no-undo init "Тригер изменение devisPC".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
       ...
       if     available old-devisPC
          and not new(new-devisPC)
       then do:
          buffer-compare new-devisPC
                using DB-num id
                to old-devisPC
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'devisPC':U
                  ,input (buffer old-devisPC:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление devisPC для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'devisPC':U
      ,input (buffer ub.new-devisPC:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать devisPC для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
