...
:
  if ub.schedule-attr.task-num <> -1 then do:
    run str/callnews.p
      (input 'schedule-attr':U
      ,input (buffer ub.schedule-attr:handle)
      ) no-error .
    if error-status :error then do:
      undo main-block, return error substitute( "&1. Невозможно маршрутизировать атрибут строки расписания для отправки в новости. &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message ( 1 ) ).
    end.
    if g#oxml = yes
    then do:
      run str/calloxml.p (
