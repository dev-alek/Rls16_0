...
TRIGGER PROCEDURE FOR DELETE OF ub.scales-gds.
define variable vss-revision    as character no-undo init "$Revision$":U .
define variable vss-author      as character no-undo init "$Author$":U .
define variable vss-date        as character no-undo init "$Date$":U .
define variable vss-workfile    as character no-undo init "$Workfile$":U .
define variable vss-archive     as character no-undo init "$Archive$":U .
define variable vss-description as character no-undo init "Триггер на удаление товаров на весах".
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3',  ub.scales-gds.db-num
...
:
  if not g#news and ub.scales-gds.db-num <> g#db-num then do:
    message
    "Нельзя удалять запись об товаре на весах," skip
  ...
  if not g#news then do:
   ...
   if not g#news then do:
    ...
    buffer-copy ub.scales-gds to buf_c-scales-gds
    assign
    buf_c-scales-gds.chip-num           = next-value (s-scales-chip, ub)
    ...
    buf_c-scales-gds.is-del             = yes
    .
    create buf_c-scales.
    buffer-copy buf_c-scales-gds
    to buf_c-scales
    assign
    buf_c-scales.action               = integer('99':U)
    buf_c-scales.subject               = 'scales-gds':U
    .
  end.
    if g#oxml = yes
    then do:
