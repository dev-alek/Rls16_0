...
trigger procedure for delete of ub.OperServ.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления OperServ".
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-OperServ where buf_c-OperServ.id eq OperServ.id
  and buf_c-OperServ.corr-user-db-num   = g#db-num and buf_c-OperServ.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.OperServ to buf_c-OperServ
       assign
         ...
         buf_c-OperServ.action             = 99
         buf_c-OperServ.is-del             = true
       .
    end.
    else
       buf_c-OperServ.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'OperServ':U
       ,input (buffer ub.OperServ:handle)
       ,input "":U
      ) no-error .
