...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-PromoAction where buf_c-PromoAction.id eq PromoAction.id
and buf_c-PromoAction.db-num eq PromoAction.db-num
  and buf_c-PromoAction.corr-user-db-num   = g#db-num and buf_c-PromoAction.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.PromoAction to buf_c-PromoAction
       assign
         ...
         buf_c-PromoAction.action             = 99
         buf_c-PromoAction.is-del             = true
       .
    end.
    else
       buf_c-PromoAction.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'PromoAction':U
       ,input (buffer ub.PromoAction:handle)
       ,input "":U
      ) no-error .
   ...
   exclusive-lock:
    ...
    for each  ub.PromoAttr where ub.PromoAttr.tablename eq  "PromoAction" and ub.PromoAction.id = int64(entry(1,ub.PromoAttr.p-key,chr(3)))
   exclusive-lock:
       delete ub.PromoAttr.
   end.
   ...
   for each  ub.PromoAttr where ub.PromoAttr.tablename eq "PromoAction"
                            and ub.PromoAttr.p-key     eq string(ub.PromoAction.id) + chr(3) + string(ub.PromoAction.db-num)
   exclusive-lock:
       delete ub.PromoAttr.
   end.
