...
:
  ...
  if ub.dis-gds-rule.obj-type = 'маг':U
  or ub.dis-gds-rule.obj-type = 'скл':U then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  send-ref = (IF error-status:error or conf-par <> "yes" then no else yes).
      if send-ref and not g#news then do:
        if ub.dis-gds-rule.obj-type = 'маг':U then do:
        ...
        or ub.dis-gds-rule.obj-type = 'орг':U)
           and lookup(ub.dis-gds-rule.pos-type, 'bo':U) = 0)
        then do:
          run trg/nu_gds.p (
...
if (valid-handle(g#lib-nws) <> true) then do:   run nws/lib-nws.p persistent no-error .   if error-status :error or (valid-handle(g#lib-nws) <> true) then do:     message       "Error starting nws/lib-nws.p" skip       g#lib-nws skip       g#lib-nws :type skip       g#lib-nws :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-nws_get-hn-option in g#lib-nws
  ...
  ,output v-send
    ...
    or v-send >= 0 then do:
      ...
      buf_c-dis-gds-rule.chip-num           = next-value (s-gds-chip, ub)
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  ...
  ,output v-host-code
      ...
      if ub.dis-gds-rule.obj-type = 'орг':U then do:
        v-host-code = ub.dis-gds-rule.obj-code.
      end.
      create buf_c-gds-hist.
      buffer-copy buf_c-dis-gds-rule to buf_c-gds-hist
      assign
      buf_c-gds-hist.obj-type = (if ub.dis-gds-rule.obj-type = 'маг':U
                                or ub.dis-gds-rule.obj-type = 'скл':U
                                then ub.dis-gds-rule.obj-type
                                else '':U)
      buf_c-gds-hist.obj-code = (if ub.dis-gds-rule.obj-type = 'маг':U
                                or ub.dis-gds-rule.obj-type = 'скл':U
                                then ub.dis-gds-rule.obj-code
                                else 0)
      buf_c-gds-hist.action = (if new ub.dis-gds-rule then integer('1':U) else integer('2':U))
      buf_c-gds-hist.subject = 'dis-gds-rule':U
