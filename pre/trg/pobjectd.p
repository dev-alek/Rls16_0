...
trigger procedure for delete of ub.PromoObject.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления PromoObject".
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-PromoObject where buf_c-PromoObject.id eq PromoObject.id
and buf_c-PromoObject.db-num eq PromoObject.db-num
  and buf_c-PromoObject.corr-user-db-num   = g#db-num and buf_c-PromoObject.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.PromoObject to buf_c-PromoObject
       assign
         ...
         buf_c-PromoObject.action             = 99
         buf_c-PromoObject.is-del             = true
       .
    end.
    else
       buf_c-PromoObject.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'PromoObject':U
       ,input (buffer ub.PromoObject:handle)
       ,input "":U
      ) no-error .
...
for each  ub.PromoAttr where ub.PromoAttr.tablename eq "PromoObject"
                            and ub.PromoAttr.p-key     eq string(ub.PromoObject.id) + chr(3) + string(ub.PromoObject.db-num)
   exclusive-lock:
       delete ub.PromoAttr.
   end.
