...
trigger procedure for write of ub.utd-err
  new buffer new-utd-err
  old buffer old-utd-err
.
...
define variable vss-description as character no-undo init "Тригер изменение utd-err".
...
define buffer buf_c-utd-err  for ub.c-utd-err .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define buffer buf_c-utd-head for ub.c-utd-head .
      buffer-compare new-utd-err to old-utd-err case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("utd-err","s-c-utd-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    then do:
                     if new(new-utd-err)
       then do:
 for first buf_c-utd-err where buf_c-utd-err.db-num eq new-utd-err.db-num
and buf_c-utd-err.doc-id eq new-utd-err.doc-id
and buf_c-utd-err.CheckType eq new-utd-err.CheckType
and buf_c-utd-err.CodeErr eq new-utd-err.CodeErr
and buf_c-utd-err.CheckObj eq new-utd-err.CheckObj
and buf_c-utd-err.reckey eq new-utd-err.reckey
  and buf_c-utd-err.corr-user-db-num   = g#db-num and buf_c-utd-err.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-utd-err where buf_c-utd-err.db-num eq old-utd-err.db-num
and buf_c-utd-err.doc-id eq old-utd-err.doc-id
and buf_c-utd-err.CheckType eq old-utd-err.CheckType
and buf_c-utd-err.CodeErr eq old-utd-err.CodeErr
and buf_c-utd-err.CheckObj eq old-utd-err.CheckObj
and buf_c-utd-err.reckey eq old-utd-err.reckey
  and buf_c-utd-err.corr-user-db-num   = g#db-num and buf_c-utd-err.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-utd-err)
   then do:
      if not available buf_c-utd-err
      then do:
         create buf_c-utd-err.
      buffer-copy new-utd-err to buf_c-utd-err
      assign
         buf_c-utd-err.chip-num           = v-Seq
         buf_c-utd-err.corr-date          = v-date
         buf_c-utd-err.corr-time          = v-time
         buf_c-utd-err.corr-user-db-num   = g#db-num
         buf_c-utd-err.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-utd-err.action             = 1
         buf_c-utd-err.is-del             = false
      .
      end.
      else
         if buf_c-utd-err.action eq 99
         then
            buf_c-utd-err.action = 2.
   end.
   else do:
      if not available buf_c-utd-err
      then do:
         create buf_c-utd-err.
         buffer-copy old-utd-err to buf_c-utd-err
         assign
            buf_c-utd-err.chip-num           = v-Seq
            buf_c-utd-err.corr-date          = v-date
            buf_c-utd-err.corr-time          = v-time
            buf_c-utd-err.corr-user-db-num   = g#db-num
            buf_c-utd-err.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-utd-err.action             = 2 when buf_c-utd-err.action ne 1
            buf_c-utd-err.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-utd-err)
         then do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq new-utd-err.db-num
and buf_c-utd-head.doc-id eq new-utd-err.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq old-utd-err.db-num
and buf_c-utd-head.doc-id eq old-utd-err.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      ...
      then do:
         ...
         buffer-copy  buf_c-utd-err to buf_c-utd-head
         assign
            buf_c-utd-head.subject = "utd-err"
