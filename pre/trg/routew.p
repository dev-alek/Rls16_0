...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3',buf-new_route.db-num,buf-new_route.last-pack,buf-new_route.tbl-ord)
    .
end procedure.
...
:
  ...
  if available buf_BatchProcess
    and not ( buf-new_route.name-rec begins string( "command":U + chr(1) + "upgrade":U ) )
  then do:
