...
define variable vss-include-info5 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  assign
    ...
    buf_c-user-login.corr-user-name   = (if g#news then (chr(4) +  'СПН':U) else g#userid)
  ...
  assign
    buf_c-usr-hist.subject = 'user-login':U
    buf_c-usr-hist.action  = (if new(ub.user-login) then integer('1':U) else integer('2':U))
    buf_c-usr-hist.is-news = g#news
    buf_c-usr-hist.source-type = (if g#news then 'db':U else "":U)
  ...
  .
    ...
    then
       run trg/userlog.p (
                      input if ub.user-login.user-administrator then 'create':U else 'delete':U
                    , input 'c-user-login':U + chr(3) + "adm"
                    , input ( buffer buf_c-user-login :handle )
                    , input ?
                    , input ""
                ) no-error.
    ...
    then do:
       ...
       then do:
          ...
          run trg/userlog.p (
                      input 'update':U
                    , input 'c-user-login':U + chr(3) + "user-password"
                    , input ( buffer buf_c-user-login :handle )
                    , input ?
                    , input ""
                ) no-error.
