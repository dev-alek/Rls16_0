...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1',  ub.dis-time-rule.time-rule-num)
    .
end procedure.
...
:
  if not g#news
  and ub.dis-time-rule.time-rule-num <= 99999
    and ub.dis-time-rule.sts <> integer('1':U)
    and ub.dis-time-rule.time-rule-num < (13 + 50000) then do:
    message
  ...
  if not g#news then do:
    if can-find(first buf_dis-time-rule no-lock where
                  ...
                  AND buf_dis-time-rule.time-rule-num < (13 + 50000)
                      ) then do:
      message
      vss-workfile vss-revision vss-description skip
  ...
  if ub.dis-time-rule.upper-time-rule-num <= 99999
  then do:
    run nws/cmd-del.p
  ...
  buf_c-dis-time-rule.chip-num           = next-value (s-dc-chip, ub)
  ...
  buf_c-dis-time-rule.action             = integer('99':U)
  .
    if g#oxml = yes
    then do:
    run str/calloxml.p (
