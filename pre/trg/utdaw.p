...
trigger procedure for write of ub.utd-attr
  new buffer new-utd-attr
  old buffer old-utd-attr
.
...
define variable vss-description as character no-undo init "Тригер изменение utd-attr".
...
define buffer buf_c-utd-attr  for ub.c-utd-attr .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define buffer buf_c-utd-head for ub.c-utd-head .
      buffer-compare new-utd-attr to old-utd-attr case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("utd-attr","s-c-utd-chip-num", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    then do:
                     if new(new-utd-attr)
       then do:
 for first buf_c-utd-attr where buf_c-utd-attr.db-num eq new-utd-attr.db-num
and buf_c-utd-attr.doc-id eq new-utd-attr.doc-id
and buf_c-utd-attr.attr-code eq new-utd-attr.attr-code
  and buf_c-utd-attr.corr-user-db-num   = g#db-num and buf_c-utd-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-utd-attr where buf_c-utd-attr.db-num eq old-utd-attr.db-num
and buf_c-utd-attr.doc-id eq old-utd-attr.doc-id
and buf_c-utd-attr.attr-code eq old-utd-attr.attr-code
  and buf_c-utd-attr.corr-user-db-num   = g#db-num and buf_c-utd-attr.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-utd-attr)
   then do:
      if not available buf_c-utd-attr
      then do:
         create buf_c-utd-attr.
      buffer-copy new-utd-attr to buf_c-utd-attr
      assign
         buf_c-utd-attr.chip-num           = v-Seq
         buf_c-utd-attr.corr-date          = v-date
         buf_c-utd-attr.corr-time          = v-time
         buf_c-utd-attr.corr-user-db-num   = g#db-num
         buf_c-utd-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-utd-attr.action             = 1
         buf_c-utd-attr.is-del             = false
      .
      end.
      else
         if buf_c-utd-attr.action eq 99
         then
            buf_c-utd-attr.action = 2.
   end.
   else do:
      if not available buf_c-utd-attr
      then do:
         create buf_c-utd-attr.
         buffer-copy old-utd-attr to buf_c-utd-attr
         assign
            buf_c-utd-attr.chip-num           = v-Seq
            buf_c-utd-attr.corr-date          = v-date
            buf_c-utd-attr.corr-time          = v-time
            buf_c-utd-attr.corr-user-db-num   = g#db-num
            buf_c-utd-attr.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-utd-attr.action             = 2 when buf_c-utd-attr.action ne 1
            buf_c-utd-attr.is-del             = false
         .
      end.
   end.
      ...
      then do:
         if new(new-utd-attr)
         then do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq new-utd-attr.db-num
and buf_c-utd-head.doc-id eq new-utd-attr.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
         else do:
 for first buf_c-utd-head where buf_c-utd-head.db-num eq old-utd-attr.db-num
and buf_c-utd-head.doc-id eq old-utd-attr.doc-id
  and buf_c-utd-head.corr-user-db-num   = g#db-num and buf_c-utd-head.chip-num = v-Seq exclusive-lock: leave. end.
         end.
      end.
      ...
      then do:
         ...
         buffer-copy  buf_c-utd-attr to buf_c-utd-head
         assign
            buf_c-utd-head.subject = "utd-attr"
