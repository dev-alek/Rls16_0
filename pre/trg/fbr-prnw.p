...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2',ub.fbr-prn.db-num,ub.fbr-prn.prn-num)
    .
end procedure.
...
:
  ...
  if not g#news then do:
    ...
    buf_c-fbr-prn.chip-num           = next-value (s-scales-chip, ub)
    ...
    buf_c-fbr-prn.subject            = 'fbr-prn':U
    buf_c-fbr-prn.action             = integer(if new(ub.fbr-prn) then '1':U else '2':U)
    .
  end.
    if g#oxml = yes
    then do:
