...
TRIGGER PROCEDURE FOR DELETE OF ub.rule-by-call.
define variable vss-revision    as character no-undo init "$Revision$":U .
define variable vss-author      as character no-undo init "$Author$":U .
define variable vss-date        as character no-undo init "$Date$":U .
define variable vss-workfile    as character no-undo init "$Workfile$":U .
define variable vss-archive     as character no-undo init "$Archive$":U .
define variable vss-description as character no-undo init "Триггер на удаление алгоритма расчета".
...
:
  ...
  or g#db-num <> 0 then do:
    if entry(1, ub.rule-by-call.call_id, chr(3)) <> 'layout-elem-rule':U then do:
      run cur-time in this-procedure(output v-date, output v-time).
      create buf_c-rule-by-call.
      buffer-copy ub.rule-by-call to buf_c-rule-by-call
      assign
      ...
      run  gen-key-fv in this-procedure (
                                        ...
                                        ,output v-field-list
                                        ,output v-value-list).
      case entry(1, ub.rule-by-call.call_id, chr(3)):
        when 'dis-card-type':U then do:
          ...
          buf_c-dis-card-type.type =  entry(lookup("type", v-field-list, chr(3)), v-value-list, chr(3))
          buf_c-dis-card-type.emitent-host-code = integer(entry(lookup("emitent-host-code", v-field-list, chr(3)), v-value-list, chr(3)))
          buf_c-dis-card-type.action =  integer('99':U)
          buf_c-dis-card-type.subject = 'rule-by-call':U
        ...
        when 'thbj-attr':U then do:
          ...
          assign
          buf_c-thbj-attr.chip-num          = next-value (s-cli-chip, ub)
          buf_c-rule-by-call.chip-num       = buf_c-thbj-attr.chip-num
          buf_c-thbj-attr.prop-code         = entry(lookup("prop-code", v-field-list, chr(3)), v-value-list, chr(3))
          buf_c-thbj-attr.upper-prop-code   = entry(lookup("upper-prop-code", v-field-list, chr(3)), v-value-list, chr(3))
          buf_c-thbj-attr.obj-type          = entry(lookup("obj-type", v-field-list, chr(3)), v-value-list, chr(3))
          buf_c-thbj-attr.obj-code          = integer(entry(lookup("obj-code", v-field-list, chr(3)), v-value-list, chr(3)))
          buf_c-thbj-attr.action            = integer('99':U)
          buf_c-thbj-attr.subject = 'rule-by-call':U
          .
          if buf_c-thbj-attr.obj-type = 'маг':U
          or buf_c-thbj-attr.obj-type = 'скл':U then do:
define variable vss-include-info2 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  (input  buf_c-thbj-attr.obj-type
  ,input  buf_c-thbj-attr.obj-code
  ,output v-host-code
  )  .
          end.
          if buf_c-thbj-attr.obj-type = 'орг':U then do:
            assign
          ...
          assign
          buf_c-cli-hist.action = integer('99':U)
          buf_c-cli-hist.subject = 'thbj-attr':U
          buf_c-cli-hist.host-code = v-host-code
          buf_c-cli-hist.is-news = g#news
          buf_c-cli-hist.source-type = (if g#news then 'db':U else "":U)
          buf_c-cli-hist.source-ref = (if g#news then string(g#news-source-db) else "":U)
          .
        end.
      when 'schedule':U then do:
        buf_c-rule-by-call.chip-num = next-value(s-ref-corr-chip, ub).
      end.
        otherwise do:
          message
          vss-workfile vss-revision vss-description skip
          "НЕПОНЯТНАЯ ТОЧКА ВЫЗОВА " entry(1, ub.rule-by-call.call_id, chr(3))
