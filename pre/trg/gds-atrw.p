...
if (valid-handle(g#lib-nws) <> true) then do:   run nws/lib-nws.p persistent no-error .   if error-status :error or (valid-handle(g#lib-nws) <> true) then do:     message       "Error starting nws/lib-nws.p" skip       g#lib-nws skip       g#lib-nws :type skip       g#lib-nws :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-nws_get-hn-option in g#lib-nws
  ...
  ,output v-send
    ...
    or v-send >= 0 then do:
      ...
      assign
      buf_c-gds-hist.action = (if new ub.goods-attr then integer('1':U) else integer('2':U))
    ...
    find first ub.clients no-lock where ub.clients.obj-type = 'маг':U and ub.clients.db-num = g#db-num no-error .
    ...
    if ub.goods-attr.attr-code = 'mark-type':U and ub.goods-attr.attr-value <> "not-type" and
  ...
  send-ref = (IF error-status:error or conf-par <> "yes" then no else yes).
        if send-ref  then do:
           ...
           do jj = 1 to num-entries(v-other, chr(47)):
             ...
             v-dop2 = if num-entries(entry(jj, v-other, chr(47)), '=':U) > 1
                      then entry(2, entry(jj, v-other, chr(47)), '=':U)
                      else ""
             .
             if v-dop1 = "check":U  then do:
               run value(v-dop2) in this-procedure (
                  ...
                  if avail old-goods-attr then 'ИЗМЕНЕНИЕ':U else 'ДОБАВЛЕНИЕ':U,
                  output vOk,
                  output vError
               ) no-error.
