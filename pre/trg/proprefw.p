...
:
  ...
  then do:
    ...
    if not v-confirmed then do:
      find first buf_ruledict where
                buf_ruledict.entry-type  = 'sum-id':U
  ...
  or g#db-num > 0 then do:
    ...
    buf_c-prop-ref.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    if ub.prop-ref.dtm-code > 0 then do:
      ...
      to buf_c-prop-head
      assign
      buf_c-prop-head.action             = integer(if new(ub.prop-ref) then '1':U else '2':U)
      buf_C-prop-head.subject            = 'prop-ref':U
      .
    end.
  end.
  if not g#news
