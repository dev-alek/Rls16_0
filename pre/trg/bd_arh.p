...
define variable vss-description as character no-undo init "Создать запись о том, что необходимо пересчитать складской архив по товарам".
...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6',p-doc-code,p-table-name,p-obj-type,p-obj-code,p-fact-date,p-action)
    .
end procedure.
...
on error undo, return error return-value
:
  if lookup( p-table-name, 'trn-doc':U + chr(44) + 'price-doc':U ) = 0 then do:
    message
  ...
  if p-action = ?
  or lookup(p-action, 'doc-delete':u + chr(44) + 'doc-change':u) = 0
  then do:
    message
      vss-workfile vss-revision vss-description skip
  ...
  then do:
    ...
    :
      ...
      assign
        ...
        stop-arh-news-lock_btpr.CharKey_Two   = p-doc-code
      ...
      :
        ...
        run waitfram-show in this-procedure
          (input waitfram-join-function("Архив по товарам рассчитывается на другой машине"
                                        ,"Отправлено сообщение о необходимости остановки расчёта складского архива по товарам"
  ...
  find first ub.batchprocess exclusive-lock
    where ub.batchprocess.bp_type   = 'arh':U
      ...
      and ub.batchprocess.charkey_two = p-table-name
  ...
  no-error .
    ...
    then do:
      ...
      if error-status :error then do:
        message
          vss-workfile vss-revision vss-description skip
          "Ошибка при вызове процедуры markarh.p" skip
