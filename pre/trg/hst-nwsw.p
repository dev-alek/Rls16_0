...
:
  ...
  or (g#db-num > 0 and ub.hist-nws-option.db-num = g#db-num) then do:
    ...
    and ub.hist-nws-option.subject-group <> 'c-dc-hist':U
    then do:
    end.
  end.
  ...
  if not v-cmp then do:
    if ub.hist-nws-option.subject-group <> 'c-dc-hist':U
    or ub.hist-nws-option.db-num = 0 then do:
      ...
      buf_c-hist-nws-option.chip-num           = next-value (s-ref-corr-chip, ub)
      ...
      .
      if buf_c-hist-nws-option.subject-group = 'c-dc-hist':U then do:
        ...
        buf_c-dis-card-type.subject = 'hist-nws-option':U
        ...
        buf_c-dis-card-type.action             = (if new ub.hist-nws-option then integer('1':U) else integer('2':U))
        .
        ...
        buf_c-table-bind.tbl-name-rec   = 'c-dis-card-type':U
        buf_c-table-bind.tbl-name-src   = 'c-hist-nws-option':U
