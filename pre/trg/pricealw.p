...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  if logical ( buf_price-doc-forming.have-start-period ) =  true then  do:
      ...
      if buf2_price-list-type.work-date = integer( '3':U ) then do:
         ub.price-all.fact-order-sys-from =  ub.price-all.fact-order .
      end.
      else do:
          ub.price-all.fact-order-sys-from = TRUNCATE (  ub.price-all.fact-order , 2 )  .
      end.
      if logical ( buf_price-doc-forming.have-end-period ) =  false then do:
        ub.price-all.fact-order-sys-to = max-fact-order .
      end.
...
define variable v-cmp as character no-undo .
    ...
    .
    if v-cmp <> "":U  then do:
    if  ub.price-all.status_ = 'акт':U and old_price-all.status_ <> 'акт':U    then do:
      run str/callnews.p
        (input "price-all"
      ...
      if available buf_price-doc and g#db-num = 0 then do:
          run str/callnews.p
            (input "price-all"
            ,input (buffer ub.price-all:handle)
            ) no-error .
