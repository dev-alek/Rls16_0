...
trigger procedure for write of ub.PromoObject
  new buffer new-PromoObject
  old buffer old-PromoObject
.
...
define variable vss-description as character no-undo init "Тригер изменение PromoObject".
...
define buffer buf_c-PromoObject  for ub.c-PromoObject .
define variable v-date      as date      no-undo .
define variable v-time      as integer   no-undo .
...
define variable v-tbl-name as character no-undo.
      buffer-compare new-PromoObject to old-PromoObject case-sensitive save result in v-field-chg.
      if v-field-chg > "":U then . else return .
      run cur-time in this-procedure (output v-date, output v-time).
    publish "getNextseq" ("PromoObject","s-promo-chip", "ub", output v-Seq ).
    if v-Seq = ?
    then
    ...
    if vFlagSeq
    then do:
                     if new(new-PromoObject)
       then do:
 for first buf_c-PromoObject where buf_c-PromoObject.id eq new-PromoObject.id
and buf_c-PromoObject.db-num eq new-PromoObject.db-num
  and buf_c-PromoObject.corr-user-db-num   = g#db-num and buf_c-PromoObject.chip-num = v-Seq exclusive-lock: leave. end.
       end.
       else do:
 for first buf_c-PromoObject where buf_c-PromoObject.id eq old-PromoObject.id
and buf_c-PromoObject.db-num eq old-PromoObject.db-num
  and buf_c-PromoObject.corr-user-db-num   = g#db-num and buf_c-PromoObject.chip-num = v-Seq exclusive-lock: leave. end.
       end.
    end.
   if new(new-PromoObject)
   then do:
      if not available buf_c-PromoObject
      then do:
         create buf_c-PromoObject.
      buffer-copy new-PromoObject to buf_c-PromoObject
      assign
         buf_c-PromoObject.chip-num           = v-Seq
         buf_c-PromoObject.corr-date          = v-date
         buf_c-PromoObject.corr-time          = v-time
         buf_c-PromoObject.corr-user-db-num   = g#db-num
         buf_c-PromoObject.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
         buf_c-PromoObject.action             = 1
         buf_c-PromoObject.is-del             = false
      .
      end.
      else
         if buf_c-PromoObject.action eq 99
         then
            buf_c-PromoObject.action = 2.
   end.
   else do:
      if not available buf_c-PromoObject
      then do:
         create buf_c-PromoObject.
         buffer-copy old-PromoObject to buf_c-PromoObject
         assign
            buf_c-PromoObject.chip-num           = v-Seq
            buf_c-PromoObject.corr-date          = v-date
            buf_c-PromoObject.corr-time          = v-time
            buf_c-PromoObject.corr-user-db-num   = g#db-num
            buf_c-PromoObject.corr-user-name     = (if g#news then (chr(4) +  'СПН':U) + " " else "") + g#userid
            buf_c-PromoObject.action             = 2 when buf_c-PromoObject.action ne 1
            buf_c-PromoObject.is-del             = false
         .
      end.
   end.
  if not g#news then do :
       ...
       if     available old-PromoObject
          and not new(new-PromoObject)
       then do:
          buffer-compare new-PromoObject
                using id db-num
                to old-PromoObject
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'PromoObject':U
                  ,input (buffer old-PromoObject:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление PromoObject для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'PromoObject':U
      ,input (buffer ub.new-PromoObject:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать PromoObject для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
