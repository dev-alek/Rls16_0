...
:
  ...
  if ub.config.stts <> -1
  then do:
    if lookup( ub.config.conf-type, 'к,о':U ) > 0
    then do:
      assign
  ...
  buffer-copy ub.config to buf_c-config
    assign
      ...
      buf_c-config.action           = integer('99':U)
  .
  assign
    v-send-db = ""
  .
  ...
  then do:
    ...
    then do:
      assign
        v-message = substitute( "&1. Ошибка при отправке в новости команды на удаление параметра конфигурации. &2&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message ( error-status :num-messages ) )
      .
      if v-msg = true
      then do:
        message
