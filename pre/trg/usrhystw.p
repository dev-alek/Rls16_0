...
trigger procedure for write of ub.c-user-log
  new buffer new-c-user-log
  old buffer old-c-user-log
.
...
define variable vss-description as character no-undo init "Тригер изменение c-user-log".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
       ...
       if     available old-c-user-log
          and not new(new-c-user-log)
       then do:
          buffer-compare new-c-user-log
                using corr-user-db-num cusr-id
                to old-c-user-log
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'c-user-log':U
                  ,input (buffer old-c-user-log:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление c-user-log для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'c-user-log':U
      ,input (buffer ub.new-c-user-log:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать c-user-log для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
...
if not g#news
and not g#esys
and not (buffer new-c-user-log:handle:buffer-compare (buffer old-c-user-log:handle))
then do:
  if new-c-user-log.head-table  = 'rvd-reasons':U
  or new-c-user-log.head-table  = 'schedule':U
  or new-c-user-log.head-table  = 'mi-change-1C':U
  then do :
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run rum-runa in g#library
  (input ?
  ,input this-procedure:handle
  ,input ?
  ,input 'event_user-action':U
  ,input  buffer new-c-user-log:handle
  ,input ?
  ,input ''
  ,input ''
  ) no-error .
