...
:
  ...
  or g#db-num <> 0 then do:
    ...
    buf_c-rule-process.chip-num           = next-value (s-ref-corr-chip, ub)
    ...
    buf_c-rule-process.action = integer(if new(ub.rule-process) then '1':U else '2':U)
    buf_c-rule-process.subject = 'rule-process':U
    .
  end.
  if not g#news
  and g#db-num = 0
