...
trigger procedure for delete of ub.counter.
define variable vss-revision    as character no-undo initial "$Revision$":U .
define variable vss-author      as character no-undo initial "$Author$":U .
define variable vss-date        as character no-undo initial "$Date$":U .
define variable vss-workfile    as character no-undo initial "$Workfile$":U .
define variable vss-archive     as character no-undo initial "$Archive$":U .
define variable vss-description as character no-undo init "Тригер удаления counter".
if counter.file-name eq "cashbookrule"
then do:
...
define variable v-tbl-name as character no-undo.
    ...
    if vFlagseq
    then do:
 for first buf_c-counter where buf_c-counter.db eq counter.db
and buf_c-counter.file-name eq counter.file-name
and buf_c-counter.key eq counter.key
and buf_c-counter.code eq counter.code
  and buf_c-counter.corr-user-db-num   = g#db-num and buf_c-counter.chip-num = v-Seq exclusive-lock: leave. end.
    ...
    then do:
       ...
       buffer-copy ub.counter to buf_c-counter
       assign
         ...
         buf_c-counter.action             = 99
         buf_c-counter.is-del             = true
       .
    end.
    else
       buf_c-counter.action             = 99.
  if not g#news then do :
    run nws/cmd-del.p
      ( input 'counter':U
       ,input (buffer ub.counter:handle)
       ,input "":U
      ) no-error .
