...
:
  ...
  if g#db-num = 0 or ( g#db-num <> 0 and g#news = false  )  then do:
      ...
      if error-status :error then
         return error substitute( "&1. Ошибка при отправке в новости команды на удаление записи grp-obj-price. &1&3&2&4", vss-workfile, chr(10), return-value, error-status :get-message ( error-status :num-messages ) ).
  end.
    if g#oxml = yes
    then do:
    run str/calloxml.p (
