...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2', ub.scales.db-num,ub.scales.scales-num)
    .
end procedure.
...
:
  ...
  if not g#news then do:
    ...
    buf_c-scales.chip-num           = next-value (s-scales-chip, ub)
    ...
    buf_c-scales.subject            = 'scales':U
    buf_c-scales.action             = integer(if new(ub.scales) then '1':U else '2':U)
