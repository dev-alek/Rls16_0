...
:
  ...
  then do:
    create temp-clients.
    assign
    temp-clients.obj-type = (if p-buf-handle:table = 'clients':U
                             then p-buf-handle::obj-type
                             else (if p-buf-handle:table = 'firm':U
                                   then 'орг':U
                                   else 'чел':U)
                            )
    temp-clients.obj-code =  (if p-buf-handle:table = 'clients':U
    ...
    .
    if p-buf-handle:table = 'firm':U then
    run gen-key-rec in this-procedure (
                                        input  ('firm':U)
                                        ,input  p-buf-handle
                                        ,output v-firm-our-key-rec ).
    if p-buf-handle:table = 'person':U then
    run gen-key-rec in this-procedure (
                                        input  ('person':U)
                                        ,input  p-buf-handle
    ...
    if v-new-inn <> '':u then do:
      ...
      if available buf_ext-classif then do:
        ...
        or (temp-clients.obj-type = 'орг':U and buf_ext-classif.uniq-key-rec = v-firm-our-key-rec)
        or (temp-clients.obj-type = 'чел':U and buf_ext-classif.uniq-key-rec = v-person-our-key-rec)
        then do:
          return.
        end.
        else do:
          ...
          if entry(1, buf_ext-classif.uniq-key-rec, chr(3)) = 'clients':U then do:
            assign
            v-obj-type = entry(2, buf_ext-classif.uniq-key-rec, chr(3))
            v-obj-code = integer(entry(3, buf_ext-classif.uniq-key-rec, chr(3)))
            .
          end.
          else do:
            assign
            v-obj-type = (if entry(1, buf_ext-classif.uniq-key-rec, chr(3)) = 'firm':U then 'орг':U else 'чел':U)
            v-obj-code = integer(entry(lookup((if v-obj-type = 'чел':U
                                                then "obj-code"
                                                else "firm-code")
                                              , v-field-list
                                              , chr(3))
          ...
          if g#news then do:
            if g#db-num = 0 then do:
              p-inn = v-new-inn + chr(63).
            end.
            if g#db-num <> 0 then do:
              if v-obj-type = 'орг':U then do:
            ...
            assign
            v-err-mess = substitute("Уже есть контрагент с ИНН &1: &2&3&4" +
                                    "Согласно настройкам в Вашей системе НЕ ДОЛЖНО БЫТЬ ДВУХ КОНТРАГЕНТОВ с одинаковым ИНН"
                                  , v-new-inn
                                  , v-obj-type
                                  , v-obj-code
                                  , chr(10)
      ...
      then do:
        find first buf_ext-classif where
                  buf_ext-classif.classif-name = 'inn':U
        ...
        and g#db-num <> 0 then do:
          find first buf2_ext-classif exclusive-lock where
                    buf2_ext-classif.classif-name = 'inn':U
          ...
          if locked buf2_ext-classif then do:
            undo, return error substitute("Ошибка при обновлении уникального ИНН:&1&2&1&3"
                                          ,chr(10)
          ...
          for each buf2_ext-classif where
              ...
              and buf2_ext-classif.classif-name = 'inn':U
              and buf2_ext-classif.uniq-key-rec = v-our-key-rec
              and buf2_ext-classif.db-num = -1
          on error undo, return error substitute("&1&2&3Ошибка при удалении записи ИНН для &4&5:&2&6"
                                               , vss-workfile
                                               , vss-revision
                                               , chr(10)
                                               , p-obj-type
          ...
          on stop undo, return error substitute("&1&2Занята запись ИНН для &3&4"
                                               , vss-workfile
                                               , chr(10)
                                               , p-obj-type
                                               , p-obj-code):
      ...
      and buf_ext-classif.db-num  = -1
      and buf_ext-classif.uniq-key-rec = (if temp-clients.obj-type = 'орг':U
    ...
    for each buf_batchprocess exclusive-lock where
        ...
        AND buf_batchprocess.bp_type = 'coll':u
    ...
    if available another_firm then do:
      if p-silent then do:
        ...
        message
        substitute("Уже есть фирма с ИНН &1: фирма &2&3" +
                  "Вы уверены, что хотите добавить/изменить контрагента с таким же ИНН?"
                  , v-new-inn
    ...
    if inn-uniq = 1 then do:
      run waitfram-show in this-procedure ( input "Ждите... Идет поиск повторных ИНН" ).
      for each another_person no-lock :
        if another_person.inn = v-new-inn then do:
          ...
          or (p-obj-type = 'чел':U
            ...
            if p-silent then do:
              assign
              v-err-mess = substitute("Уже есть физ.лицо с ИНН &1: чел &2&3" +
                                      "Изменение/добавление контрагента должно быть подтверждено оператором&3" +
                                      "Автоматическое изменение добавление/изменение контрагента с таким ИНН НЕВОЗМОЖНО"
                                    , v-new-inn
              ...
              message
              substitute("Уже есть физ лицо с ИНН &1: чел &2&3" +
                        "Вы уверены, что хотите добавить/изменить контрагента с таким же ИНН?"
                        , v-new-inn
                        , another_person.psn-code
                        , chr(10)
