...
trigger procedure for write of ub.tran-fuel
  new buffer new-tran-fuel
  old buffer old-tran-fuel
.
...
define variable vss-description as character no-undo init "Тригер изменение tran-fuel".
...
define variable v-tbl-name as character no-undo.
  if not g#news then do :
       ...
       if     available old-tran-fuel
          and not new(new-tran-fuel)
       then do:
          buffer-compare new-tran-fuel
                using db-num uuid uuid-cheq
                to old-tran-fuel
                case-sensitive
          ...
          then do:
             run nws/cmd-del.p
                 ( input 'tran-fuel':U
                  ,input (buffer old-tran-fuel:handle)
                  ,input "":U
                 ) no-error .
             if error-status :error then do:
                message
                   vss-workfile vss-revision vss-description skip
                   "Невозможно маршрутизировать удаление tran-fuel для отправки в новости" skip
     ...
     run str/callnews.p
      (input 'tran-fuel':U
      ,input (buffer ub.new-tran-fuel:handle)
      ) no-error.
    if error-status :error then do:
      message
        vss-workfile vss-revision vss-description skip
        "Невозможно маршрутизировать tran-fuel для отправки в новости" skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      undo , return error return-value .
