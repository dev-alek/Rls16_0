...
DO:
    ...
    OPEN QUERY BROWSE-1 FOR EACH rcs-shops SHARE-LOCK.
END.
ON CHOOSE OF b-chg IN FRAME Dialog-Frame
DO:
    run rcs/rcscnfmf.w (
            input 'изменить-узлы':U
          , input rcs-shops.id
    ) no-error .
    if error-status :error
    then do:
...
:
    ...
    assign
        v-focused-row      = browse-1 :focused-row in frame Dialog-Frame.
        v-repositioned-row = current-result-row( "browse-1" )
    .
    if rcs-shops.status_ = '*'
    then do:
        assign
            rcs-shops.status_ = ''
        .
    ...
    browse-1 :set-repositioned-row(v-focused-row, "ALWAYS") in frame Dialog-Frame.
...
define input parameter p-id  as character    no-undo.
    ...
    then do:
        undo, return error "delete-objects: Не удалось удалить запись." + chr(10) + return-value.
    end.
    else do:
