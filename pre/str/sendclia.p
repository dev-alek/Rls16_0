...
p-optimize = entry(2, p-parameter, chr(4))
...
p-action = entry(4, p-parameter, chr(4))
no-error
.
...
if p-optimize = "":U then do:
  ...
  FOR EACH buf_clients no-lock where
    ...
    run str/send-cli.p (
                  ...
                  ,input p-log-handle
                  ,input (string(buf_clients.obj-code) + chr(4) + p-action + chr(4) + "no":U +
                          chr(4) + "yes":U)
                    ) no-error .
...
if available dc-list then do:
  ...
  .
    ...
    if can-find (first dcp-list no-lock where dcp-list.obj-type = 'маг':U and dcp-list.obj-code = buf_clients.obj-code)
    ...
    then  do:
      run str/send-cli.p (
                    ...
                    ,input p-log-handle
                    ,input (string(buf_clients.obj-code) + chr(4) + p-action + chr(4) + "no":U +
                            chr(4) + (if p-action = "E":U then "no" else "yes")
                              )
                      ) no-error  .
      if error-status:error then do:
        assign
