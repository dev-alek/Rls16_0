...
define SHARED temp-table cash-obj no-undo
     ...
     :
      if first-of(cash-obj.km-objname) then do:
        ...
        run bgelib-tag-put in this-procedure ( input 3, input "ObjType":U
                                            , input string(cash-obj.km-objtype), input 1 ).
...
define variable v-first  as logical no-undo .
  FOR EACH for-cash-desk NO-LOCK WHERE
    ...
    :
    if ub.cash-desk.pos-type <> 'IBM-XML':U or
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка данных по объектам БД'" + " &1")
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'добавление данных по объектам БД')
                  else ('Ждите - ' + 'удаление данных по объектам БД') ) +
...
By ub.cash-desk.autonomy:
  if ub.cash-desk.pos-type <> 'IBM-XML':U then NEXT.
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute("Пересылка на кассы &1 БД &2 информации по объектам БД", 'IBM-XML':U, p-db-num)
                                          ).
