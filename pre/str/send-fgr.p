...
define variable vss-workfile    as character no-undo init "$Workfile: send-fgr.p $":U .
define variable vss-archive     as character no-undo init "$Archive: str/send-fgr.p $":U .
define variable vss-description as character no-undo init "Пересылка групп блюд на кассу".
...
p-batch = (if entry(3, p-parameter, chr(4)) = "yes":U
                 then yes
  ...
  when 'MAGIA-XML':U then do:
    ...
    if action = "U":U then do:
      ...
      by cash-fgrp.lvl-num  :
        ...
        run bgelib-tag-open in this-procedure ( input 2, input "PriceGroup"                                             , input substitute("ctrl='&1' tms='&2' code='&3'", 'ADD':U, OS2-time, cash-fgrp.out-code)).       run bgelib-tag-put in this-procedure ( input 3, input "PriceGroupName"                                           , input trim(cash-fgrp.node-name, chr(32)), input 1 ).       run bgelib-tag-put in this-procedure ( input 3, input "PriceParentGroupID":U                                           , input (string(cash-fgrp.upper-out-code)), input 1 ).       run bgelib-tag-put in this-procedure ( input 3, input "PriceListID":U                                           , input (string(buf_shop.obj-code)), input 1 ).       run bgelib-tag-put in this-procedure ( input 3, input "ImageIndex":U                                           , input (string(0)), input 1 ).       run bgelib-tag-put in this-procedure ( input 3, input "PriceGroupLock"                                           , input string(if action = "U":U then 0 else 1), input 1 ).       run bgelib-tag-close in this-procedure ( input 2, input "PriceGroup").
      end.
    end.
...
define buffer for-cash-desk for ub.cash-desk.
 FOR EACH for-cash-desk NO-LOCK WHERE
    ...
    :
    if for-cash-desk.pos-type <> 'MAGIA-XML':U or
...
then do:
  if for-cash-desk.pos-type = 'MAGIA-XML':U then do:
    run str/waitpxml.w ( replace( v-xml-file-name-path, "/", "\" ) + "xml",
                ...
                ( if action = 'U'
                  then ('Ждите - ' + 'добавление групп блюд (на объекте) - групп прейскуранта')
                  else ('Ждите - ' + 'удаление групп блюд (на объекте) - групп прейскуранта') ) +
...
define variable dflt-cd16 as character no-undo .
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type17 as character no-undo .
define variable v-value-date17 as date no-undo .
define variable v-value-decimal17 as decimal no-undo .
define variable v-value-integer17 as INTEGER no-undo .
define variable v-value-logical17 AS LOGICAL no-undo .
define variable v-tth17 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd16
    ,output v-value-date17
    ,output v-value-decimal17
    ,output v-value-integer17
    ,output v-value-logical17
    ,output v-param-type17
    ,INPUT-OUTPUT table-handle v-tth17
    ) no-error .
delete object v-tth17 no-error.
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute("Пересылка на кассы &1&2 информации о группах блюд по объекту", 'маг':U, i-obj-code)
                                          ).
...
if error-status:error then do:
  run write-log-and-file in p-log-handle (
      ...
      , input log-file-name
      , input 1
      , input substitute( "!!!Ошибки при отсылке информации о группах блюд по объекту на кассы  маг&1"
                         , i-obj-code
                        )
                                        ).
  assign
