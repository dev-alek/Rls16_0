...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-docs FOR EACH buf_c-price-doc no-lock.
END PROCEDURE.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-16 as character no-undo .
      ...
      parameter-4-16 =
        (
          if (" buf_c-price-doc.doc-num = par-doc-num  " + " " + where-phrase-16) <> ""
          then  substitute(' buf_c-price-doc.doc-num =  &1 '  , chr(34) , par-doc-num )  + " " + where-phrase-16
          else "true"
        )
      parameter-5-16 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-16 =
          (" buf_c-price-doc.doc-num = par-doc-num  " + " " + where-phrase-16 = "")
      .
  ...
  if l-filter-open-16 = false then do:
    OPEN QUERY br-docs FOR EACH buf_c-price-doc
      where  buf_c-price-doc.doc-num = par-doc-num
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  on error undo, leave
  :
    if query br-docs:handle:get-buffer-handle(1) = (buffer buf_c-price-doc:handle) then do:
      assign
      parameter-2-16 = (if p-find-next then "true":u else "false":u )
      parameter-4-16 =
        "where ":u +  substitute(' buf_c-price-doc.doc-num =  &1 '  , chr(34) , par-doc-num )  + " ":u + where-phrase-16 + " ":u + p-find-condition + " " + ""
      parameter-5-16 = "  "
    .
      ...
      assign
      parameter-2-16 = (if p-find-next then "true":u else "false":u )
      parameter-3-16 =  "FOR EACH BUF_c-price-doc"
      parameter-4-16 =
        (
          if (" buf_c-price-doc.doc-num = par-doc-num  " + " " + where-phrase-16) <> ""
          then  substitute(' buf_c-price-doc.doc-num =  &1 '  , chr(34) , par-doc-num )  + " " + where-phrase-16
...
when "doc-num":U then do:     create temp-changes.       assign         temp-changes.f_name = "doc-num":U         temp-changes.l_name = "Номер"         temp-changes.v_old = string(buf_c-price-doc.doc-num)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.doc-num)                                  else string(current_price-doc.doc-num))       .   end.
when "host-code":U then do:     create temp-changes.       assign         temp-changes.f_name = "host-code":U         temp-changes.l_name = "Код фирмы"         temp-changes.v_old = string(buf_c-price-doc.host-code)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.host-code)                                  else string(current_price-doc.host-code))       .   end.
when "status_":U then do:     create temp-changes.       assign         temp-changes.f_name = "status_":U         temp-changes.l_name = "Статус"         temp-changes.v_old = string(buf_c-price-doc.status_)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.status_)                                  else string(current_price-doc.status_))       .   end.
when "PS":U then do:     create temp-changes.       assign         temp-changes.f_name = "PS":U         temp-changes.l_name = "Примечание"         temp-changes.v_old = string(buf_c-price-doc.PS)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.PS)                                  else string(current_price-doc.PS))       .   end.
when "base-rate":U then do:     create temp-changes.       assign         temp-changes.f_name = "base-rate":U         temp-changes.l_name = "Курс"         temp-changes.v_old = string(buf_c-price-doc.base-rate)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.base-rate)                                  else string(current_price-doc.base-rate))       .   end.
when "base-scale":U then do:     create temp-changes.       assign         temp-changes.f_name = "base-scale":U         temp-changes.l_name = "Масш-б"         temp-changes.v_old = string(buf_c-price-doc.base-scale)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.base-scale)                                  else string(current_price-doc.base-scale))       .   end.
when "doc-date":U then do:     create temp-changes.       assign         temp-changes.f_name = "doc-date":U         temp-changes.l_name = "Дата документа"         temp-changes.v_old = string(buf_c-price-doc.doc-date)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.doc-date)                                  else string(current_price-doc.doc-date))       .   end.
when "fact-date":U then do:     create temp-changes.       assign         temp-changes.f_name = "fact-date":U         temp-changes.l_name = "Дата факт"         temp-changes.v_old = string(buf_c-price-doc.fact-date)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.fact-date)                                  else string(current_price-doc.fact-date))       .   end.
when "fact-num":U then do:     create temp-changes.       assign         temp-changes.f_name = "fact-num":U         temp-changes.l_name = "Номер факт"         temp-changes.v_old = string(buf_c-price-doc.fact-num)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.fact-num)                                  else string(current_price-doc.fact-num))       .   end.
when "fact-order":U then do:     create temp-changes.       assign         temp-changes.f_name = "fact-order":U         temp-changes.l_name = "Номер факт-ордер"         temp-changes.v_old = string(buf_c-price-doc.fact-order)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.fact-order)                                  else string(current_price-doc.fact-order))       .   end.
when "fact-time":U then do:     create temp-changes.       assign         temp-changes.f_name = "fact-time":U         temp-changes.l_name = "Время факт закрытия"         temp-changes.v_old = string(buf_c-price-doc.fact-time)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.fact-time)                                  else string(current_price-doc.fact-time))       .   end.
when "obj-code":U then do:     create temp-changes.       assign         temp-changes.f_name = "obj-code":U         temp-changes.l_name = "Код Объекта"         temp-changes.v_old = string(buf_c-price-doc.obj-code)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.obj-code)                                  else string(current_price-doc.obj-code))       .   end.
when "obj-type":U then do:     create temp-changes.       assign         temp-changes.f_name = "obj-type":U         temp-changes.l_name = "Тип Объекта"         temp-changes.v_old = string(buf_c-price-doc.obj-type)         temp-changes.v_new = (if available new_c-price-doc                                  then string(new_c-price-doc.obj-type)                                  else string(current_price-doc.obj-type))       .   end.
END CASE.
end.
end.
...
v-2 = num-entries(v-chg-fields) .
        do ii = 1 to v-2 :
            CASE entry(ii, v-chg-fields):
                                          when "doc-qnty":U then do:     create temp-changes.       assign         temp-changes.f_name ="line" + string(ub.c-price-list.line-num ) + "doc-qnty":U         temp-changes.l_name ="Изм.строки-" + string(ub.c-price-list.line-num ) + " " + "Количество"         temp-changes.v_new = string(ub.c-price-list.doc-qnty)         temp-changes.v_old =  (if available old_c-price-list                                  then string(old_c-price-list.doc-qnty)                                  else "")       .   end.
                                          when "price-sale":U then do:     create temp-changes.       assign         temp-changes.f_name ="line" + string(ub.c-price-list.line-num ) + "price-sale":U         temp-changes.l_name ="Изм.строки-" + string(ub.c-price-list.line-num ) + " " + "Цена продажи"         temp-changes.v_new = string(ub.c-price-list.price-sale)         temp-changes.v_old =  (if available old_c-price-list                                  then string(old_c-price-list.price-sale)                                  else "")       .   end.
            end case.
        end.
end.
Open QUery br-changes for each temp-changes.
