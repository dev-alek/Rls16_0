...
if bf-in_trn-doc.obj-type = 'маг':U then do:
...
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_is-petrl in g#lib-trn
  (
     input bf-in_doc-line.artic
  ,  input bf-in_doc-line.prod-type
  ,  input bf-in_doc-line.prod-code
  ...
  then do:
    ...
    for each bf-in_parts where bf-in_parts.out-code  = bf-in_doc-line.doc-code  and
      ...
      if bf-supp_clients.obj-type = 'маг':U  or
         bf-supp_clients.obj-type = 'скл':U then do:
        assign
          tt-in-doc-line.defect-qnty = tt-in-doc-line.defect-qnty + bf-in_parts.fact-qnty.
      end.
      else do:
        find first bf-minus_parts where bf-minus_parts.out-code  = 'free-zone':U    and
        ...
        then do :
          ...
          for each bf-minus_parts where bf-minus_parts.out-code  = 'free-zone':U    and
                                  bf-minus_parts.obj-type  = bf-in_parts.obj-type  and
                                  bf-minus_parts.obj-code  = bf-in_parts.obj-code  and
                                  bf-minus_parts.artic     = bf-in_parts.artic     and
                                  bf-minus_parts.prod-type = bf-in_parts.prod-type and
...
,input 1
,input 1
,input bf-in_trn-doc.host-code
,input 'орг':U
,input ?
,input bf_clients.db-num
,input bf-in_trn-doc.creid
,input ' '
,input vardoc-code
,input bf-in_trn-doc.doc-date
,input 'инв':U
,input no
,input bf-in_trn-doc.host-code
,input no
,input bf-in_trn-doc.obj-code
,input bf-in_trn-doc.obj-type
,input no
,input varinv-pay
,input substitute('@ Создан по внешней приходной накладной &1', bf-in_trn-doc.doc-code)
,input no
,input 'без':U
,input 'накл':U
,input 'в т. ч.':U
    ...
    for each tt-minus-parts where tt-minus-parts.artic     = tt-minus-doc-line.artic     and
      ...
      if bf-in_doc-line.fact-qnty - tt-in-doc-line.defect-qnty - tt-minus-doc-line.rsrv-qnty > 0 then do:
        ...
        run trg/rsrv-dtl.p ( parmainmenu-handle,
                      'reserv':U
                + "," + 'rsrv-single-part':U
                + "," + 'rsrv-in-code':U   + "=" + str-encode(tt-minus-parts.in-code,   "", ",=":u)
                + "," + 'rsrv-part-code':U + "=" + str-encode(tt-minus-parts.part-code, "", ",=":u)
                + (if available ub.pl-gds then ("," + 'plcode':U + "=" + str-encode(tt-minus-parts.part-code, "", ",=":u)) else "")
                ,
                buffer bf_gds-dtl,
        ...
        if varneed-rsrv-parts <> 0 then do:
          ...
          for each tt-in-parts where tt-in-parts.out-code    = bf-in_trn-doc.doc-code and
            ...
            run trg/rsrv-dtl.p ( parmainmenu-handle,
                           'reserv':U
                + "," + 'rsrv-single-part':U
                + "," + 'rsrv-in-code':U   + "=" + str-encode(tt-in-parts.in-code,   "":u, ",=":u)
                + "," + 'rsrv-part-code':U + "=" + str-encode(tt-in-parts.part-code, "":u, ",=":u)
                + (if available ub.pl-gds then ("," + 'plcode':U + "=" + str-encode(tt-in-parts.part-code, "", ",=":u)) else "")
                , buffer bf_gds-dtl,
