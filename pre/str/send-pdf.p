...
p-doc-num = integer(entry(4, p-parameter, chr(4)))
p-pdf-db = integer(entry(5, p-parameter, chr(4)))
i-obj-code = integer(entry(6, p-parameter, chr(4)))
no-error
.
...
define variable vss-include-info1 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont1 = yes then do:
    if integer( substring( tmp-str1, 1, length( bc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = bc-pfx + substring( tmp-str1, length( bc-pfx ) + 1, length( tmp-str1 ) - length( bc-pfx ) )
...
END PROCEDURE.
define variable vss-include-info2 as character format "x(65)" no-undo initial "@(#)$Workfile: defc-gds.i $ $Revision: 47e5c2a27e63, 2885, rls $".
...
define variable varscales-pref-type49 as character no-undo.
varscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,output varscales-pref-type49
  ) no-error .
if varscales-pref = ? then do:
  assign
  varscales-pref = '21,23,25':U.
end.
define variable varpgscales-pref-type49 as character no-undo.
varpgscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'scpgpref':u
  ,input  '':u
  ...
  ,output varpgscales-pref-type49
  ) no-error .
...
define variable v-param-type52 as character no-undo .
define variable v-value-character52 as date no-undo .
define variable v-value-date52 as date no-undo .
define variable v-value-decimal52 as decimal no-undo .
define variable v-value-integer52 as INTEGER no-undo .
define variable v-value-logical52 AS LOGICAL no-undo .
define variable v-tth52 as handle no-undo .
...
run adm/shattri.p (
    ...
    ,input  'cd-sending':U
    ,input  'cdpcknum':U
    ,output v-value-character52
    ,output v-value-date52
    ,output v-value-decimal52
    ,output cdpcknum
    ,output v-value-logical52
    ,output v-param-type52
    ,INPUT-OUTPUT table-handle v-tth52
    )  .
delete object v-tth52.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'txfixnum'
  ,input  0
  ,input  ''
...
if ub.shop.cd-pb-base or ub.shop.cd-pb-alt or ub.shop.cd-sc-base OR petrol-trk then do:
  FOR EACH ub.prod-bc NO-LOCK WHERE
      ...
      if  ub.prod-bc.b-str = string( ub.bar-code.b-code )
          AND
          (
          (ub.goods.unit-base = ub.bar-code.unit-cli AND ub.shop.cd-loc-base) OR
          (NOT ub.goods.unit-base = ub.bar-code.unit-cli AND ub.shop.cd-loc-alt)
          ) AND
          (NOT petrol-trk
          )
          then NEXT.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbcat in g#library
  ...
  ,output g#log
    ...
    if ((ub.shop.cd-pb-base AND ub.bar-code.unit-cli = ub.goods.unit-base
    AND LOOKUP('вес':U, units.type) = 0
    and ub.prod-bc.bc-on-type <> 'pglc':U
    ) OR
        (ub.shop.cd-pb-alt AND ub.bar-code.unit-cli <> ub.goods.unit-base)) then do:
      run asc-gds in this-procedure (
        buffer ub.goods,
        buffer ub.bar-code,
        buffer ub.gds-prt,
...
if ub.shop.cd-pb-base or ub.shop.cd-pb-alt or ub.shop.cd-sc-base OR petrol-trk then do:
  FOR EACH ub.prod-bc NO-LOCK WHERE
      ...
      if  ub.prod-bc.b-str = string( p-bar-code.b-code )
          AND
          (
          (ub.goods.unit-base = p-bar-code.unit-cli AND ub.shop.cd-loc-base) OR
          (NOT ub.goods.unit-base = p-bar-code.unit-cli AND ub.shop.cd-loc-alt)
          ) AND
          (NOT petrol-trk
          )
          then NEXT.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbcat in g#library
  ...
  ,output g#log
    ...
    if ((ub.shop.cd-pb-base AND p-bar-code.unit-cli = ub.goods.unit-base
    AND LOOKUP('вес':U, units.type) = 0
    and ub.prod-bc.bc-on-type <> 'pglc':U
    ) OR
        (ub.shop.cd-pb-alt AND p-bar-code.unit-cli <> ub.goods.unit-base)) then do:
      run asc-gds in this-procedure (
        buffer ub.goods,
        buffer p-bar-code,
        buffer ub.gds-prt,
...
:
    ...
    FIND FIRST ub.gds-obj WHERE
     ...
     run get-gds-obj-fields in this-procedure(                                                  buffer ub.gds-obj                                                 ,input no                                                 ,input ub.goods.gds-code                                                 ,input i-obj-code                                                 ,input 'маг':U                                                 ,output for-fact-qnty                                                 ,output cashparts                                                 ,output v-is-null-price                                                 ,output v-is-menu                                                 ,output v-is-semi-finished                                                 ,output v-is-modificator                                                 ,output v-fbr-grp-code                                                 ,output v-fbr-obj-code                                                 ) no-error .       if error-status:error then do:         run write-log-and-file in p-log-handle (                 input 1                                                                              , input log-file-name                                                                  , input 1                                                                              , input substitute("!!!Ошибка при получении характеристик товара &1 &2&3 на объекте"                                      , ub.gds-obj.artic                                                               , ub.gds-obj.prod-type                                                           , ub.gds-obj.prod-code                                                           )                                                                                         ).                                                            assign                                                                                  v-view-log = yes                                                                        .      end.
...
define variable vss-include-info81 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str81  as character no-undo.
  define variable tmp-num81  as character no-undo.
  define variable i81        as integer   no-undo.
  define variable sum81      as integer   no-undo.
  define variable len-code81 as integer   no-undo.
  define variable varcont81  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str81 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str81 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont81 = yes then do:
    if integer( substring( tmp-str81, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str81, length( ncrsc-pfx ) + 1, length( tmp-str81 ) - length( ncrsc-pfx ) )
        len-code81    = length( IBm-good-code )
      .
      define variable v-sum-char81 as character no-undo .
      assign
        sum81 = 0
      .
      do i81 = 1 to len-code81 by 2
      :
        assign
          v-sum-char81 = substr(IBm-good-code, len-code81 - i81 + 1, 1)
        .
        if v-sum-char81 < "0"
        or v-sum-char81 > "9"
        then do:
        ...
        assign
          sum81 = sum81 + integer(v-sum-char81)
        .
      end.
      if varcont81 = yes then do:
        assign
        ...
        if varcont81 = yes then do:
           if sum81 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum81 mod 10)
             .
           end.
        end.
      end.
...
define variable vss-include-info82 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str82  as character no-undo.
  define variable tmp-num82  as character no-undo.
  define variable i82        as integer   no-undo.
  define variable sum82      as integer   no-undo.
  define variable len-code82 as integer   no-undo.
  define variable varcont82  as logical   initial yes no-undo.
  CASE ncrpg-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str82 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str82 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont82 = yes then do:
    if integer( substring( tmp-str82, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str82, length( ncrpg-pfx ) + 1, length( tmp-str82 ) - length( ncrpg-pfx ) )
        len-code82    = length( IBm-good-code )
      .
      define variable v-sum-char82 as character no-undo .
      assign
        sum82 = 0
      .
      do i82 = 1 to len-code82 by 2
      :
        assign
          v-sum-char82 = substr(IBm-good-code, len-code82 - i82 + 1, 1)
        .
        if v-sum-char82 < "0"
        or v-sum-char82 > "9"
        then do:
        ...
        assign
          sum82 = sum82 + integer(v-sum-char82)
        .
      end.
      if varcont82 = yes then do:
        assign
        ...
        if varcont82 = yes then do:
           if sum82 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum82 mod 10)
             .
           end.
        end.
      end.
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action84   as character no-undo .
  define variable v-printed84       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action84
    ,output v-printed84
    ) .
end.
if v-view-log = true
...
define variable vss-include-info85 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
then do:
  ...
  then do:
  define variable v-plu-pet91 as character no-undo .
  define buffer  bufpet_cd-plu91 for ub.cd-plu.
  for each bufpet_cd-plu91  where
          bufpet_cd-plu91.obj-type = 'маг':U
      and bufpet_cd-plu91.obj-code = abs(i-obj-code)
      and bufpet_cd-plu91.pos-type = 'MARIA':U
      and bufpet_cd-plu91.plu-type = 'топ':U
      AND bufpet_cd-plu91.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu91.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet91 = TRIM(string( bufpet_cd-plu91.plu-code, "X(40)":U ))
    v-plu-pet91 = substring(v-plu-pet91, length(v-plu-pet91) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code91 as character no-undo .
    assign
      maria-good-code91 = left-trim(ibm-good-code, '0')
      maria-good-code91 = fill('0', 14 - length(maria-good-code91)) + maria-good-code91
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code91, 1, 5) + chr(4) +
                                                    substring(maria-good-code91, 6, 14)
                                                  )
...
define variable vss-include-info92 as character format "x(65)" no-undo initial "@(#)$Workfile$".
...
define variable v-found112 as logical no-undo .
define variable v-is-script112 as logical no-undo.
define variable v-fields-shift112 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet115 as character no-undo .
  define buffer  bufpet_cd-plu115 for ub.cd-plu.
  for each bufpet_cd-plu115  where
          bufpet_cd-plu115.obj-type = 'маг':U
      and bufpet_cd-plu115.obj-code = abs(i-obj-code)
      and bufpet_cd-plu115.pos-type = 'MARIA':U
      and bufpet_cd-plu115.plu-type = 'топ':U
      AND bufpet_cd-plu115.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu115.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet115 = TRIM(string( bufpet_cd-plu115.plu-code, "X(40)":U ))
    v-plu-pet115 = substring(v-plu-pet115, length(v-plu-pet115) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code115 as character no-undo .
    assign
      maria-good-code115 = left-trim(ibm-good-code, '0')
      maria-good-code115 = fill('0', 14 - length(maria-good-code115)) + maria-good-code115
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code115, 1, 5) + chr(4) +
                                                    substring(maria-good-code115, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
then do:
  ...
  then do:
  define variable v-plu-pet116 as character no-undo .
  define buffer  bufpet_cd-plu116 for ub.cd-plu.
  for each bufpet_cd-plu116  where
          bufpet_cd-plu116.obj-type = 'маг':U
      and bufpet_cd-plu116.obj-code = abs(i-obj-code)
      and bufpet_cd-plu116.pos-type = 'MARIA':U
      and bufpet_cd-plu116.plu-type = 'топ':U
      AND bufpet_cd-plu116.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu116.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet116 = TRIM(string( bufpet_cd-plu116.plu-code, "X(40)":U ))
    v-plu-pet116 = substring(v-plu-pet116, length(v-plu-pet116) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code116 as character no-undo .
    assign
      maria-good-code116 = left-trim(ibm-good-code, '0')
      maria-good-code116 = fill('0', 14 - length(maria-good-code116)) + maria-good-code116
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code116, 1, 5) + chr(4) +
                                                    substring(maria-good-code116, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable dflt-cd119 as character no-undo .
define variable vss-include-info120 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type120 as character no-undo .
define variable v-value-date120 as date no-undo .
define variable v-value-decimal120 as decimal no-undo .
define variable v-value-integer120 as INTEGER no-undo .
define variable v-value-logical120 AS LOGICAL no-undo .
define variable v-tth120 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd119
    ,output v-value-date120
    ,output v-value-decimal120
    ,output v-value-integer120
    ,output v-value-logical120
    ,output v-param-type120
    ,INPUT-OUTPUT table-handle v-tth120
    ) no-error .
delete object v-tth120 no-error.
...
define variable vss-include-info138 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str138  as character no-undo.
  define variable tmp-num138  as character no-undo.
  define variable i138        as integer   no-undo.
  define variable sum138      as integer   no-undo.
  define variable len-code138 as integer   no-undo.
  define variable varcont138  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str138 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str138 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont138 = yes then do:
    if integer( substring( tmp-str138, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str138, length( ncrsc-pfx ) + 1, length( tmp-str138 ) - length( ncrsc-pfx ) )
        len-code138    = length( v-tmpchar )
      .
      define variable v-sum-char138 as character no-undo .
      assign
        sum138 = 0
      .
      do i138 = 1 to len-code138 by 2
      :
        assign
          v-sum-char138 = substr(v-tmpchar, len-code138 - i138 + 1, 1)
        .
        if v-sum-char138 < "0"
        or v-sum-char138 > "9"
        then do:
        ...
        assign
          sum138 = sum138 + integer(v-sum-char138)
        .
      end.
      if varcont138 = yes then do:
        assign
        ...
        if varcont138 = yes then do:
           if sum138 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum138 mod 10)
             .
           end.
        end.
      end.
