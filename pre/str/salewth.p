...
if loc-inkas.obj-type = 'скл':U then do:
...
BY ub.inkas-pay-desk.curr-code
  ...
  if first-of(ub.inkas-pay-desk.pay-code) then do:
    find first tt-wth-doc where     tt-wth-doc.host-code = loc-inkas.host-code
           ...
           and tt-wth-doc.source-type = 'касса':U
           and tt-wth-doc.ext-doc-type = (if  available ub.wealth and ub.wealth.is-ser = 1 and  tt-wth-doc.doc-type = 'при':U
                                        then 'pc':U
                                        else (if  tt-wth-doc.doc-type = 'при':U  then  'ci':U else 'ce':U) )
    no-error.
    ...
    tt-wth-doc.fact-date = (if  available ub.wealth and ub.wealth.is-ser = 1 and loc-inkas.shift-num <> 0 then loc-inkas.shift-date else loc-inkas.fact-date )
    ...
    tt-wth-doc.source-type = 'касса':U
    tt-wth-doc.auto-fill = yes
    tt-wth-doc.ps = "@":U + "Накладная" + chr(32) +
                    (
                    if ub.inkas-pay-desk.doc-type = 'при':U
                    then pardoc-code1
                    else pardoc-code2
                    )
    tt-wth-doc.status_ = 'разрешен':U
    tt-wth-doc.doc-sum = 0
    tt-wth-doc.fact-sum = 0
    tt-wth-doc.ext-doc-type = (if  available ub.wealth and ub.wealth.is-ser = 1 and  tt-wth-doc.doc-type = 'при':U
                               then 'pc':U
                               else (if  tt-wth-doc.doc-type = 'при':U  then  'ci':U else 'ce':U) )
    .
    end.
  ...
  if ub.wealth.is-ser = 1 then do:
    if not (ub.inkas-pay-desk.doc-type = 'при':U  or ub.inkas-pay-desk.doc-type = 'рас':U) then next _inkas-pay-desk.
    for each buf_chk-pay no-lock where
      ...
      , each buf_chk-doc no-lock where
        ...
        and buf_chk-doc.cashier = ub.inkas-pay-desk.cashier
        and buf_chk-doc.chk-type = (if ub.inkas-pay-desk.doc-type = 'при':U then 1 else int('6':U))
            :
          ...
          run str/wthidnt.p ( input buf_chk-pay.pay-card
          ...
          if error-status:error then do:
              var-mes = var-mes +  chr(10) + error-status:get-message(1) + chr(32) + return-value.
              return error var-mes.
          end.
          if vidn-wth-code <> ub.wealth.wth-code then do:
            var-mes = var-mes + chr(10) + substitute("Код МЦ &1, идентифицируемый по штрих-коду, не соответствует коду МЦ &2 для указанного типа оплаты",vidn-wth-code,ub.wealth.wth-code).
            return error var-mes.
          end.
          if vidn-zone <> 'cli-zone':U then do:
            var-mes = var-mes + chr(10) + substitute("Зона &1 идентифицируемого штрих-кода не соотвествует зоне 'cli-zone':U", vidn-zone).
            return error var-mes.
          end.
...
DO ON ERROR UNDO _main-block, return error var-mes:
  ...
  for each tt-wth-doc:
    var-mes = substitute("Не удалось создать документ МЦ к продаже &1&2" +
                         ...
                         ,ub.inkas-pay-desk.curr-code
                         ,ub.inkas-pay-desk.pay-desk
                         ,(if ub.inkas-pay-desk.doc-type = 'при':U then "продажа" else "возврат")
...
assign
tt-dtl-line.q-ty-doc  = 0
tt-dtl-line.q-ty-fact = 0
tt-dtl-line.doc-sum   = 0
tt-dtl-line.fact-sum  = 0
tt-dtl-line.sum-gds-rubl = 0
tt-dtl-line.sum-gds-base = 0
.
for each buf_wth-parts no-lock where buf_wth-parts.w-p-code = tt-dtl-line.w-p-code
                       and buf_wth-parts.wth-code = tt-dtl-line.wth-code
                       and buf_wth-parts.par-code = tt-dtl-line.par-code
                       and buf_wth-parts.out-code = tt-dtl-line.doc-code
                       and buf_wth-parts.stts = 0 :
   assign
  tt-dtl-line.q-ty-doc     =  tt-dtl-line.q-ty-doc  + buf_wth-parts.qnty-doc
  tt-dtl-line.q-ty-fact    =  tt-dtl-line.q-ty-fact + buf_wth-parts.fact-qnty
  tt-dtl-line.sum-gds-rubl =  tt-dtl-line.sum-gds-rubl + buf_wth-parts.price-rubl * buf_wth-parts.fact-qnty
  tt-dtl-line.sum-gds-base =  tt-dtl-line.sum-gds-base + buf_wth-parts.price-base * buf_wth-parts.fact-qnty
  no-error
  .
end.
assign
  tt-dtl-line.doc-sum     =  tt-dtl-line.q-ty-doc  * tt-dtl-line.par-rate
  tt-dtl-line.fact-sum    =  tt-dtl-line.q-ty-fact * tt-dtl-line.par-rate
  no-error
  .
        is-dtl = yes.
      end.
...
for each tt-dtl-line no-lock where tt-dtl-line.w-p-code = tt-wth-line.w-p-code
                       and tt-dtl-line.wth-code = tt-wth-line.wth-code
                       and tt-dtl-line.doc-code = tt-wth-line.doc-code
                       :
  assign
  tt-wth-line.doc-sum      =  tt-wth-line.doc-sum  + tt-dtl-line.doc-sum
  tt-wth-line.fact-sum     =  tt-wth-line.fact-sum + tt-dtl-line.fact-sum
  tt-wth-line.sum-gds-rubl =  tt-wth-line.sum-gds-rubl + tt-dtl-line.sum-gds-rubl
  tt-wth-line.sum-gds-base =  tt-wth-line.sum-gds-base + tt-dtl-line.sum-gds-base
  .
end.
assign
  ...
  .
    ...
    assign
    locked_wth-doc.status_ = 'разрешен':U
    .
