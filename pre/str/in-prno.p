...
procedure check-use-bar-code :
  ...
  do
  on error  undo, return error substitute( "&1. &2&3&4", vss-include-info21, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
  on stop   undo, return error substitute( "&1. stop", vss-include-info21 )
  on endkey undo, return error substitute( "&1. endkey", vss-include-info21 )
  :
    ...
    if not available buf_bar-code then do:
      return error substitute( "&1 (check-use-bar-code). Не найден бар-код &2", vss-include-info21, p-b-code ) .
    end.
...
do on error undo, return error return-value :
find ub.trn-doc where ub.trn-doc.doc-code = p-doc-code no-lock.
...
v-old-price-sale = p-price-sale .
  if ub.trn-doc.ext-doc-type = 'ie':U then do:
     if not (par-pr-nakl-ie = yes and par-gen-mrgn-ie = 'before-margin':U ) then return .
  end.
  if ub.trn-doc.ext-doc-type = 'iv':U then do:
     if not (par-pr-nakl-iv = yes and par-gen-mrgn-iv = 'before-margin':U ) then return .
  end.
  if  ub.trn-doc.doc-type <> 'при':U or
...
define variable vss-include-info85 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
    define buffer buf_alt-calc_price-doc86 for ub.price-doc .
  find first buf_alt-calc_price-doc86 no-lock
    where buf_alt-calc_price-doc86.doc-num = old-num
    .
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  'object':U
    ,input  buf_alt-calc_price-doc86.host-code
    ,input  buf_alt-calc_price-doc86.obj-type
    ,input  buf_alt-calc_price-doc86.obj-code
    ,input  0
    ,input  0
    ,input  0
    ,input  false
