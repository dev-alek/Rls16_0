...
define variable vss-workfile    as character no-undo init "$Workfile: goodsrum.p $":u .
define variable vss-archive     as character no-undo init "$Archive: str/goodsrum.p $":u .
...
define variable vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  run mainmenu_getcntxt in parparentproc
    ...
    when 'goods-batchwork':U then do:
      run cur-time in this-procedure ( output v-today, output v-time).
      assign
      v-doc-date = v-today
      v-fact-date = v-today
  ...
  do v-jj = 1 to num-entries(v-codex-id-list):
    ...
    do v-ii = 1 to num-entries(v-ruleset-id-list[v-jj]):
      ...
      on endkey undo _main, return error substitute( "&1. endkey", vss-workfile ) :
        ...
        if p-process = 'xml-esys-import':U
