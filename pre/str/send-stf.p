...
DEFINE SHARED TEMP-TABLE cash-cash no-undo
          ...
          if cash-cash.cash-code > 0 then do:
            ...
            run bgelib-tag-put in this-procedure ( input 3, input "StaffLock"  ,
                                                            input  string(if action = "U":U
                                                                          and cash-cash.stts = integer('0':U)
...
define variable v-first as logical no-undo .
  FOR EACH for-cash-desk NO-LOCK WHERE
    ...
    (LOOKUP(ub.cash-desk.pos-type,
     ...
     and for-cash-desk.autonomy = integer('1':U))
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка данных по персоналу'" + " &1")
...
then do:
  if for-cash-desk.pos-type = 'MAGIA-XML':U then do:
    run str/waitpxml.w ( replace( v-xml-file-name-path, "/", "\" ) + "xml",
                ...
                ( if action = 'U'
                  then ('Ждите - ' + 'добавление данных по персоналу')
                  else ('Ждите - ' + 'удаление данных по персоналу') ) +
