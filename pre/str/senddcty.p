  ...
  ,output v-host-code
    ...
    and v-rid-list <> "":U then do:
      ...
      do ii = 1 to num-entries(v-rid-list):
        ...
        and buf_dis-card-mask.use-on = integer('2':U) then do:
          if choice = 2 then do:
            assign
            v-view-log = yes.
            run write-log-and-file in p-log-handle (
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка данных по типам-маскам карт'" + " &1")
...
END CASE.
    ...
    if v-version-dec < 4.4
    and for-cash-desk.pos-type = 'IBM':U then do:
      run write-log-and-file in p-log-handle (
          input 1
        , input log-file-name
        , input 1
...
then do:
  if for-cash-desk.pos-type = 'MAGIA-XML':U then do:
    run str/waitpxml.w ( replace( v-xml-file-name-path, "/", "\" ) + "xml",
                ...
                ( if action = 'U'
                  then ('Ждите - ' + 'добавление данных по типам-маскам карт')
                  else ('Ждите - ' + 'удаление данных по типам-маскам карт') ) +
...
define variable dflt-cd50 as character no-undo .
...
define variable v-value-logical51 AS LOGICAL no-undo .
...
run adm/shattri.p (
    ...
    ,output v-value-logical51
    ,output v-param-type51
    ,INPUT-OUTPUT table-handle v-tth51
    ) no-error .
delete object v-tth51 no-error.
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action56   as character no-undo .
  define variable v-printed56       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action56
    ,output v-printed56
    ) .
end.
if v-view-log = true
