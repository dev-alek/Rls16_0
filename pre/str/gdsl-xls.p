...
define variable vss-include-info21 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str21  as character no-undo.
  define variable tmp-num21  as character no-undo.
  define variable i21        as integer   no-undo.
  define variable sum21      as integer   no-undo.
  define variable len-code21 as integer   no-undo.
  define variable varcont21  as logical   initial yes no-undo.
  CASE bc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str21 = string( internal-b-code, "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str21 = string( internal-b-code, "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont21 = yes then do:
    if integer( substring( tmp-str21, 1, length( bc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = bc-pfx + substring( tmp-str21, length( bc-pfx ) + 1, length( tmp-str21 ) - length( bc-pfx ) )
        len-code21    = length( full-b-code )
      .
      define variable v-sum-char21 as character no-undo .
      assign
        sum21 = 0
      .
      do i21 = 1 to len-code21 by 2
      :
        assign
          v-sum-char21 = substr(full-b-code, len-code21 - i21 + 1, 1)
        .
        if v-sum-char21 < "0"
        or v-sum-char21 > "9"
        then do:
        ...
        assign
          sum21 = sum21 + integer(v-sum-char21)
        .
      end.
      if varcont21 = yes then do:
        assign
        ...
        if varcont21 = yes then do:
           if sum21 mod 10 = 0 then do:
             ...
             assign
               full-b-code = full-b-code + string(10 - sum21 mod 10)
             .
           end.
        end.
      end.
...
define variable v-last-inv-date-num as character no-undo.
  ...
  :
    CASE p-f-name:
      ...
      when "qnty":U then do:
        return string(gds-list.qnty,  p-format).
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output main-b-code
      ...
      when "last-inv" then do :
        find last buf_doc-line no-lock where
                  buf_doc-line.artic = gds-list.artic
...
FOR EACH gds-list no-lock,
    first ub.goods no-lock where
          ub.goods.gds-code = gds-list.gds-code:
...
for each gds-list-hist:
  if Make-Excel then  put   stream ForExcel unformatted
  (if gds-list-hist.line = 0
   then string(gds-list-hist.id, ">>>>>>>>9")
   else '':U)
  CHR(9)
  (if gds-list-hist.item_ <> '':U
   then gds-list-hist.hist-mode
   else '':U)  CHR(9)
   (if gds-list-hist.item_ <> '':U
   then string(gds-list-hist.num-add, "->>>>>>>>9")
   else '':U)  CHR(9)
  (if gds-list-hist.item_ <> '':U
  then string(gds-list-hist.num-recs, ">>>>>>>>9")
  else '':U)  CHR(9)
  gds-list-hist.des
  skip.
end.
...
DEFINE VARIABLE vother               as character           no-undo .
  ...
  :
    ...
    tbl = 'goods':U + chr(44) +
          ...
          'function'
    c-point = "Список товаров" + chr(4) +  "PRNT":u + chr(4) + substitute("Список товаров - &1&2 фирма &3", p-curr-obj-type, p-curr-obj-code, v-host-code)
    fld = ""
    lab = ""
    spr = ""
    dim = '0'
