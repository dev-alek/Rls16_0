...
define variable filter-point     as character no-undo initial 'c-trn-reason':U.
define variable filter-point0    as character no-undo initial 'c-trn-reason':U.
define variable filter-label     as character no-undo initial 'История оснований (причин) создания документа':U.
define variable filter-label0    as character no-undo initial 'История оснований (причин) создания документа':U.
...
define query br-reasons for buf_c-trn-reason scrolling.
define query br-changes for buf_changes scrolling.
define browse br-reasons query br-reasons display
  mark-string( buffer buf_c-trn-reason )  column-label '*'  format 'x(1)':U
  buf_c-trn-reason.reason-code  column-label 'Код основания'  format "->,>>>,>>>,>>9":U
  buf_c-trn-reason.reason-name  column-label 'Основание (причина) создания документа'  format "x(78)":U
  attr-sign( buf_c-trn-reason.reason-code )  column-label 'А'  format "x(1)":U
  buf_c-trn-reason.PS  column-label 'Примечание'  format "x(95)":U
  usrfulnf(buf_c-trn-reason.corr-user-name)  column-label 'Изменил'  format "x(8)":U
  ShowAction( buf_c-trn-reason.action )  column-label 'Действие'  format "x(10)":U
  buf_c-trn-reason.corr-date  column-label 'Дата корр.'  format "99/99/9999":U
  STRING( buf_c-trn-reason.corr-time, 'HH:MM:SS':U )  column-label 'Время'  format "x(8)":U
  buf_c-trn-reason.chip-num column-label 'Щепка' format "->,>>>,>>>,>>9":U
  buf_c-trn-reason.corr-user-db-num column-label 'БД' format ">>>>9":U
...
b-help       at row  1 col 91
  br-reasons at row  3.00 col  1.50
...
with view-as dialog-box keep-tab-order side-labels no-underline three-d scrollable
     title 'История оснований (причин) создания документа':U
     default-button b-quit cancel-button b-quit.
assign frame fr-D-reason-0 :scrollable = no.
assign br-reasons         :num-locked-columns in frame  fr-D-reason-0  = 1
       buf_c-trn-reason.PS :read-only          in browse br-reasons = yes
       buf_changes.l_name :read-only          in browse   br-changes   = yes.
assign b-mark    :tooltip in frame fr-D-reason-0 = "Поставить/снять отметку записи"
         b-quit     :tooltip in frame fr-D-reason-0 = "Вернуться в окно вызова"
       ...
       b-sel  :tooltip in frame fr-D-reason-0 = "Выбрать текущую(ие) запись(и)"
         b-Attr     :tooltip in frame fr-D-reason-0 = "История атрибутов основания (причины)"
       br-reasons :tooltip in frame fr-D-reason-0 = "Список действий над причинами"
         br-changes   :tooltip in frame fr-D-reason-0 = "Список изменений причины"
         ...
         mark-num     :tooltip in frame fr-D-reason-0 = "Отмеченные записи".
on delete-character of br-reasons in frame fr-D-reason-0 do:
  if b-mark :sensitive in frame fr-D-reason-0 then do: apply "CHOOSE":U to b-mark in frame fr-D-reason-0. end.
end.
on insert-mode of br-reasons in frame fr-D-reason-0 do:
  if           b-mark   :sensitive in frame fr-D-reason-0 then do:
    apply "CHOOSE":U to b-mark     in frame fr-D-reason-0.
  end. else if b-sel :sensitive in frame fr-D-reason-0 then do:
    apply "CHOOSE":U to b-sel   in frame fr-D-reason-0.
  end.
end.
on choose of b-mark in frame fr-D-reason-0 do:
...
assign
  v-str-recid9 = trim( string( recid( buf_c-trn-reason ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry9 > 0 then do:
  ...
  .
    ...
    br-reasons :refresh( ) in frame fr-D-reason-0.
    if last-event :function <> "MOUSE-SELECT-DBLCLICK" then do:
      br-reasons :select-next-row( ) in frame fr-D-reason-0.
    end.
    apply "VALUE-CHANGED":U to br-reasons in frame fr-D-reason-0.
    if num-entries( p-rid-list ) = 0 then do:
       hide
       mark-num   in frame fr-D-reason-0.
    end.
    else do:
      display
  ...
  END.
  apply "ENTRY":U to br-reasons in frame fr-D-reason-0.
end.
on choose of b-quit in frame fr-D-reason-0 do:
...
on choose of b-Attr in frame fr-D-reason-0 do:
...
on return                of br-reasons in frame fr-D-reason-0 or
   mouse-select-dblclick of br-reasons in frame fr-D-reason-0 do:
  if           b-mark   :sensitive in frame fr-D-reason-0 then do:
      apply "CHOOSE":U to b-mark   in frame fr-D-reason-0.
  end.
  else IF b-sel :sensitive in frame fr-D-reason-0 then do:
      apply "CHOOSE":U to b-sel in frame fr-D-reason-0.
  end.
end.
on value-changed of br-reasons in frame fr-D-reason-0 do:
...
on INS of frame fr-D-reason-0 anywhere do:
  if b-mark :sensitive then DO: apply "CHOOSE":U to b-mark in frame fr-D-reason-0. END.
  return no-apply.
end.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-reasons :handle
      .
...
DEF VAR cur-clmn-numbr-reasons as INT EXTENT 11 no-undo.
DEF VAR varmvibr-reasons       as INT no-undo.
DEF VAR varmvjbr-reasons       as INT no-undo.
DEF VAR varmvkbr-reasons       as INT no-undo.
DEF VAR varmvlbr-reasons       as INT no-undo.
DEF VAR move-elementbr-reasons as INT no-undo.
def var jjbr-reasons           as int no-undo.
do varmvibr-reasons = 1 to EXTENT(cur-clmn-numbr-reasons):
  ASSIGN cur-clmn-numbr-reasons[varmvibr-reasons] = varmvibr-reasons.
END.
RUN start-mv-clmnbr-reasons.
PROCEDURE start-mv-clmnbr-reasons:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-reasons do:
  RUN re-move-clmnbr-reasons ( 2, 11).
END.
ON ctrl-cursor-left OF BROWSE br-reasons do:
  RUN re-move-clmnbr-reasons (11, 2).
END.
PROCEDURE re-move-clmnbr-reasons:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-reasons = 1 TO EXTENT(cur-clmn-numbr-reasons):
    if cur-clmn-numbr-reasons[varmvibr-reasons] = source-column THEN cur-clmn-numbr-reasons[varmvibr-reasons] = -1.
  END.
  if br-reasons:MOVE-COLUMN(source-column, target-column) IN FRAME fr-D-reason-0 then.
  if source-column > target-column THEN
  DO varmvjbr-reasons = source-column - 1 to target-column BY -1:
    DO varmvibr-reasons = 1 TO EXTENT(cur-clmn-numbr-reasons):
        if cur-clmn-numbr-reasons[varmvibr-reasons] = varmvjbr-reasons THEN DO:
          cur-clmn-numbr-reasons[varmvibr-reasons] = cur-clmn-numbr-reasons[varmvibr-reasons] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-reasons = source-column + 1 to target-column:
    DO varmvibr-reasons = 1 TO EXTENT(cur-clmn-numbr-reasons):
      if cur-clmn-numbr-reasons[varmvibr-reasons] = varmvjbr-reasons THEN DO:
        cur-clmn-numbr-reasons[varmvibr-reasons] = cur-clmn-numbr-reasons[varmvibr-reasons] - 1.
      END.
  ...
  END.
  DO varmvibr-reasons = 1 TO EXTENT(cur-clmn-numbr-reasons):
    if cur-clmn-numbr-reasons[varmvibr-reasons] = -1 THEN cur-clmn-numbr-reasons[varmvibr-reasons] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-reasons:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 2 then do:
    return .
  end.
  DO varmvibr-reasons = 1 TO EXTENT(cur-clmn-numbr-reasons):
    if cur-clmn-numbr-reasons[varmvibr-reasons] = cur-clmn-loc THEN move-elementbr-reasons = varmvibr-reasons.
  END.
  RUN re-move-clmnbr-reasons (cur-clmn-loc, 2).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-reasons:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-reasons = 2 to EXTENT(cur-clmn-numbr-reasons):
    RUN re-move-clmnbr-reasons (cur-clmn-numbr-reasons[varmvlbr-reasons], varmvlbr-reasons).
  END.
  RUN start-mv-clmnbr-reasons.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
.
def var sort-labelbr-reasons   as character no-undo .
def var sort-clmnbr-reasons    as handle    no-undo .
def var cur-clmnbr-reasons     as handle    no-undo .
def var cur-clmn-locbr-reasons as integer   no-undo .
def var re-querybr-reasons     as logical   initial no no-undo .
on start-search, ctrl-o of br-reasons in frame fr-D-reason-0 do:
   run sort-brbr-reasons
     (input (if available buf_c-trn-reason
             then recid(buf_c-trn-reason)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-reasons :
  define input parameter p-recid as recid no-undo .
  if re-querybr-reasons = no then do:
    assign
       cur-clmnbr-reasons = br-reasons:current-column in frame fr-D-reason-0
    .
    if sort-clmnbr-reasons <> ? then sort-clmnbr-reasons:column-fgcolor = 0.
    if cur-clmnbr-reasons = sort-clmnbr-reasons then do:
      assign
         sort-labelbr-reasons = ""
         sort-clmnbr-reasons = ?
      .
     end.
     else do:
       assign
         sort-labelbr-reasons = cur-clmnbr-reasons:label
         sort-clmnbr-reasons  = cur-clmnbr-reasons
         sort-clmnbr-reasons:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locbr-reasons = 1
  .
  def var column-handle as handle no-undo .
  column-handle = br-reasons:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnbr-reasons then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locbr-reasons = cur-clmn-locbr-reasons + 1
    .
  end.
  case sort-labelbr-reasons:
        when 'Код основания'  then DO:    assign       sort-column-name = "buf_c-trn-reason.reason-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Основание (причина) создания документа'  then DO:    assign       sort-column-name = "buf_c-trn-reason.reason-name"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'А'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1attr-sign&1, buf_c-trn-reason.reason-code)', chr(34))     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Примечание'  then DO:    assign       sort-column-name = "buf_c-trn-reason.PS"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Изменил'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1usrfulnf&1, buf_c-trn-reason.corr-user-name)', chr(34))     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Действие'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1ShowAction&1, buf_c-trn-reason.action)', chr(34))     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Дата корр.'  then DO:    assign       sort-column-name = "buf_c-trn-reason.corr-date"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Время'  then DO:    assign       sort-column-name = "STRING( buf_c-trn-reason.corr-time, 'HH:MM:SS':U )"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Щепка'  then DO:    assign       sort-column-name = "buf_c-trn-reason.chip-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'БД'  then DO:    assign       sort-column-name = "buf_c-trn-reason.corr-user-db-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U ).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultbr-reasons') then do:
          run mv-brw-defaultbr-reasons.
        end.
      if sort-labelbr-reasons <> "" then do:
        assign
          cur-clmnbr-reasons:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locbr-reasons = ?
      .
    end.
  end case.
  if p-recid <> ? then do:
    reposition br-reasons to recid p-recid no-error.
    apply "value-changed" to br-reasons in frame fr-D-reason-0.
  end.
  apply "entry" to br-reasons in frame fr-D-reason-0.
END PROCEDURE.
procedure re-open-query-srt-clmnbr-reasons:
if cur-clmnbr-reasons = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U ).
end.
else do:
   assign re-querybr-reasons = yes.
   run sort-brbr-reasons
     (input (if available buf_c-trn-reason
             then recid(buf_c-trn-reason)
             else ?
...
PROCEDURE re-move-clmnbr-changes:
  ...
  END.
  if br-changes:MOVE-COLUMN(source-column, target-column) IN FRAME fr-D-reason-0 then.
  if source-column > target-column THEN
...
do on error   undo Main-Block, leave Main-Block
  ...
  in frame fr-D-reason-0.
  if p-rid-list <> "":U then do:
    reposition br-reasons to recid v-doc-rec no-error.
  end.
  br-reasons :set-repositioned-row( 5, "CONDITIONAL":U ).
...
case p-mode :
  when 'все':U  then do:
    assign
    frame fr-D-reason-0 :title = "История оснований (причин) создания документа".
...
if p-open-query then do:
  ...
  if l-filter-open-20 = false then do:
    open query br-reasons for each buf_c-trn-reason
      where  yes
      indexed-reposition
  .
      assign
  ...
  assign
    v-doc-rec = recid( buf_c-trn-reason )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-reasons:handle:get-buffer-handle(1) = (buffer buf_c-trn-reason:handle) then do:
      assign
    ...
    .
      ...
      assign
      parameter-2-20 = (if p-find-next then "true":u else "false":u )
      parameter-3-20 =  "for each buf_c-trn-reason"
      parameter-4-20 =
        (
          if (" yes " + " " + where-phrase-20) <> ""
          then " yes " + " " + where-phrase-20
  ...
  when 'one':U then do:
    ...
    assign frame fr-D-reason-0 :title =
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-22 as character no-undo .
      ...
      parameter-4-22 =
        (
          if (" buf_c-trn-reason.reason-code = p-reason-code  " + " " + where-phrase-22) <> ""
          then  substitute('buf_c-trn-reason.reason-code = &1', p-reason-code )  + " " + where-phrase-22
          else "true"
        )
      parameter-5-22 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-22 =
          (" buf_c-trn-reason.reason-code = p-reason-code  " + " " + where-phrase-22 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-reasons:handle
  ...
  if l-filter-open-22 = false then do:
    open query br-reasons for each buf_c-trn-reason
      where  buf_c-trn-reason.reason-code = p-reason-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-reasons:handle:get-buffer-handle(1) = (buffer buf_c-trn-reason:handle) then do:
      ...
      parameter-4-22 =
        "where ":u +  substitute('buf_c-trn-reason.reason-code = &1', p-reason-code )  + " ":u + where-phrase-22 + " ":u + p-find-condition + " " + ""
      parameter-5-22 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-22 = (if p-find-next then "true":u else "false":u )
      parameter-3-22 =  "for each buf_c-trn-reason"
      parameter-4-22 =
        (
          if (" buf_c-trn-reason.reason-code = p-reason-code  " + " " + where-phrase-22) <> ""
          then  substitute('buf_c-trn-reason.reason-code = &1', p-reason-code )  + " " + where-phrase-22
...
procedure proc-filter :
  ...
  do on error   undo Filter-Block, leave Filter-Block
    ...
    if return-value = 'undo':U then do:
      apply "ENTRY":U to browse br-reasons.
      return no-apply.
    end.
    assign mark-num = 0
           sch-num  = 0.
    hide   mark-num in frame fr-D-reason-0.
    hide   sch-num  in frame fr-D-reason-0.
    run OpenBr in this-procedure ( input yes, input no, input '':U ).
    assign b-sch :tooltip in frame fr-D-reason-0 = "Установить/снять фильтр".
  end.
end procedure.
...
v-label-param =
   "reason-code" + chr(4) + "Код причины" + chr(4) + "" + chr(8)
 + "reason-name" + chr(4) + "Основание" + chr(4) + "" + chr(8)
 + "PS" + chr(4) + "Примечание" + chr(4).
 run proc-full-temp-changes in this-procedure (
                                             input  (buf_c-trn-reason.action = integer('1':U))
                                            ,input  (buf_c-trn-reason.action = integer('99':U))
                                            ,input  buffer  buf_c-trn-reason:handle
                                            ,input  'c-trn-reason':U
...
procedure proc-find-code :
  ...
  if v-doc-rec <> ? then do:
    ...
    display sch-num with frame fr-D-reason-0.
  end.
  else do:
    assign  sch-num = 0.
    hide    sch-num in frame fr-D-reason-0.
