...
DO:
  ...
  OPEN QUERY b-doc-attr FOR EACH ub.wth-doc-attr       WHERE ub.wth-doc-attr.doc-code = pardoc-code,              first tt-upd-attr where tt-upd-attr.code = ub.wth-doc-attr.attr-code and                                        tt-upd-attr.output-display = yes  NO-LOCK .
  if varrec-id <> ? then reposition b-doc-attr to recid varrec-id.
end.
...
if available tt-upd-attr then do:
  if tt-upd-attr.user-can-edit then do:
    do jj = 1 to num-entries(tt-upd-attr.other, chr(47)):
    if entry(1, entry(jj, tt-upd-attr.other, chr(47)), "=":U) = "spr":U
    then do:
      assign
      v-spr = string(entry(2, entry(jj, tt-upd-attr.other, chr(47)), "=":U))
      .
      end.
    end.
    assign
...
if valid-handle( g#wthcalib ) <> yes then do:       run str/wthcalib.p persistent no-error.       if error-status :error or valid-handle( g#wthcalib ) <> yes then do:         message "Error starting wthcalib.p"    skip( 0 )                 g#wthcalib                     skip( 0 )                 g#wthcalib   :type             skip( 0 )                 g#wthcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run wthcalib_wthat-oth in g#wthcalib ( input ub.wth-doc-attr.doc-code ,
                       input ub.wth-doc-attr.attr-code ,
                       input vartemp-char ) no-error .
          if error-status :error then do:
...
if available tt-upd-attr then do:
  ...
  if varg-log then do:
if valid-handle( g#wthcalib ) <> yes then do:       run str/wthcalib.p persistent no-error.       if error-status :error or valid-handle( g#wthcalib ) <> yes then do:         message "Error starting wthcalib.p"    skip( 0 )                 g#wthcalib                     skip( 0 )                 g#wthcalib   :type             skip( 0 )                 g#wthcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run wthcalib_wthat-del in g#wthcalib (  input ub.wth-doc-attr.doc-code ,
                        input ub.wth-doc-attr.attr-code ,
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse b-doc-attr :handle
      .
