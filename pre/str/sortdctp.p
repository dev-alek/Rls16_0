...
define    temp-table doc-list-hist no-undo
...
DO ii = 1 to num-entries(prid-list):
  ...
  doc-list.znak = if can-do ('рас,спи':U, ub.trn-doc.doc-type) then -1 else 1
  .
END.
  end.
  when yes then do:
    DO ii = 1 to num-entries(prid-list):
      ...
      doc-list.znak = if can-do ('рас,спи':U, ub.c-trn-doc.doc-type) then -1 else 1
      .
    END.
  end.
end case.
...
FOR EACH doc-list use-index xpk:
  assign
  prid-list = prid-list + (if prid-list = "" then "" else chr(44)) +
              doc-list.doc-code.
end.
