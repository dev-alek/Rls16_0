...
CASE vardoc-status_:
  when 'накл':U then do:
    ASSIGN
    tt-par-dtl.q-ty-doc = INPUT BROWSE BR-dtl tt-par-dtl.q-ty-doc.
    ASSIGN
  ...
  when 'разрешен':U then do:
    ASSIGN
    tt-par-dtl.q-ty-fact = INPUT BROWSE BR-dtl tt-par-dtl.q-ty-fact.
    ASSIGN
...
define variable vss-include-info6 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  IF NOT AVAIL buf_c-wth-doc THEN DO:
    ...
    RETURN error.
  END.
  if LOOKUP(buf_c-wth-doc.doc-type, "при,рас,спи") = 0 then do:
    MESSAGE
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH buf_c-wth-line SHARE-LOCK.
...
if not can-find(first tt-par-dtl) then do:
    ...
    ub.c-wth-dtl.par-code :
      ...
      IF NOT AVAIL tt-par-dtl THEN DO:
        NEXT.
      END.
      IF buf_c-wth-doc.doc-type = 'инв':U THEN DO:
        ASSIGN
...
IF par-mode = 'ПРОСМОТР':U THEN DO:
  ASSIGN
  tt-par-dtl.q-ty-doc:READ-ONLY IN BROWSE BR-dtl = YES
  tt-par-dtl.q-ty-fact:READ-ONLY IN BROWSE BR-dtl = YES
  .
END.
  ASSIGN FRAME Dialog-Frame:TITLE = CAPS( buf_wealth.wth-name ) + " (номиналы)".
  DISPLAY
  d_doc-sum @ for-d_doc-sum
  d_fact-sum @ for-d_fact-sum
  WITH FRAME Dialog-Frame.
  ...
  VIEW FRAME Dialog-Frame.
  OPEN QUERY BR-dtl FOR EACH tt-par-dtl USE-INDEX tt-i1.
END PROCEDURE.
