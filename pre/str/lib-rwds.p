...
procedure lib-rwds_crtrnsum :
  ...
  do on error undo, return error return-value :
    ...
    find first bf_doc-attr where
               ...
               bf_doc-attr.attr-code = 'addsum':U  no-error.
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input bf_trn-doc.doc-code ,
                       input 'addsum':U ,
                       input bf_doc-attr.attr-value + ',' + p-sum-type ) no-error .
      end.
    end.
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
    ...
    empty temp-table thbjattr_thbj-attr.
    if lookup( 'bd':U,     p-sum-type-list ) <> 0 or
       lookup( 'bcd':U, p-sum-type-list ) <> 0 then do:
      ...
      if lookup( 'bd':U, p-sum-type-list ) <> 0 then do:
        find first bf_bef_doc-line-sum where
                   ...
                   bf_bef_doc-line-sum.sum-type = 'bd':U    no-error.
      ...
      if v_invclcsp = "yes" then do:
        if lookup( 'bcd':U, p-sum-type-list ) <> 0 then do:
          find first bf_bef-cli_doc-line-sum where
                     ...
                     bf_bef-cli_doc-line-sum.sum-type = 'bcd':U no-error.
          if not available bf_bef_doc-line-sum then do:
            return error substitute(
              'Не найдена запись о дополнительных суммах в документе "&1", товар &2 &3 &4, тип суммы "&4".',
              bf_doc-line.doc-code,
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run barcodcr in g#library
  ...
  ,buffer bf_bar-code
            ...
            if error-status :error then do:
               undo, return error "Не могу создать(найти) первичный бар-кода признака "        + chr(10) +
                                  "Код товара "                + string( bf_goods.gds-code   ) + chr(10) +
                                  "Код признака "              + string( bf_prt-obj.prt-code ) + chr(10) +
                                  "Базовая единица измерения " + string( bf_goods.unit-base  ) + chr(10) +
                                  return-value.
            end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  ...
  ,input  varfact-order
  ,output v_doc-num
  ,output d_curprt-price-sale
  ,output d_curprt-price-road-tax
  ,output d_curprt-price-excise
  )  .
          ...
          run clcprtsl_calc-ttable in this-procedure ( input no
                                                     , input yes
                                                     , input ?
                                                     , input ?
                                                     , input ?
                                                     ...
                                                     , input v_base-scale
                                                     , input v_curr-r-b
                                                     , input d_cur-price-sale
                                                     , input d_cur-price-road-tax
                                                     , input d_cur-price-excise
                                                     , input d_vat-pc
                                                     , input d_cons-vat-pc
                                                     , input d_slt-pc              ) no-error .
          if error-status :error then do:
            undo, return error "Ошибка при расчете учетных цен по партии".
          end.
          find first tt-allsum-line where
                     tt-allsum-line.sum-type = 'основная_сумма':U no-error.
          if error-status :error then do:
            return error substitute( 'Не найдена запись по типу "&1" для товара &2 &3 &4.',
                                     'основная_сумма':U, bf_goods.artic, bf_goods.prod-type, bf_goods.prod-code ).
          end.
          if lookup( 'bd':U, p-sum-type-list ) <> 0 then do:
    ...
    if available bf_parts then do:
      if lookup( 'gen':U,     p-sum-type-list ) <> 0 or
         lookup( 'genc':U, p-sum-type-list ) <> 0 then do:
        if lookup( 'gen':U, p-sum-type-list ) <> 0 then do:
          find first bf_gen_doc-line-sum exclusive-lock where
                     ...
                     bf_gen_doc-line-sum.sum-type = 'gen':U no-error.
        ...
        if v_invclcsp = "yes" then do:
          if lookup( 'genc':U, p-sum-type-list ) <> 0 then do:
            find first bf_gen-cli_doc-line-sum exclusive-lock where
                       ...
                       bf_gen-cli_doc-line-sum.sum-type = 'genc':U no-error.
        ...
        find first bf_tt-allsum-line where
                   bf_tt-allsum-line.sum-type = 'основная_сумма':U no-error.
        if not available bf_tt-allsum-line then do:
    ...
    if lookup( 'ad':U, p-sum-type-list ) <> 0 then do:
      ...
      find first bf_aft_doc-line-sum exclusive-lock where
                 ...
                 bf_aft_doc-line-sum.sum-type = 'ad':U  no-error.
    ...
    if lookup( 'acd':U, p-sum-type-list ) <> 0 then do:
      if v_invclcsp = "yes" then do:
        ...
        find first bf_aft-cli_doc-line-sum exclusive-lock where
                   ...
                   bf_aft-cli_doc-line-sum.sum-type = 'acd':U no-error.
  ...
  , output v-is-pieces
    ...
    if can-find( first tt-wast-line ) then do:
      for each wast-trn-doc no-lock where
               ...
               wast-trn-doc.status_      = 'факт':U                   and
               wast-trn-doc.doc-type     = 'инв':U              and
               wast-trn-doc.internal     = no                        and
               wast-trn-doc.ext-doc-type = 'vt':U              use-index stat-fact
...
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_is-petrl in g#lib-trn
  (
     input tt-wast-line.artic
  ,  input tt-wast-line.prod-type
  ,  input tt-wast-line.prod-code
  , output v-is-petrol
  , output v-is-pieces
  ) .
      if  v-is-petrol = yes
 ...
 if v-petrol then do:
   for each wast-exp-line where wast-exp-line.obj-type  = tt-wast-line.obj-type  and
              ...
              first wast-exp-doc where  wast-exp-doc.obj-type       = wast-exp-line.obj-type  and
                                        ...
                                        wast-exp-doc.doc-type       = 'при':U               and
                                        wast-exp-doc.is-del         = false                   and
                                        wast-exp-doc.ext-doc-type   = 'ie':U
                                        ...
                                        wast-exp-doc.doc-code       = wast-exp-line.doc-code  and
                                        wast-exp-doc.internal       = no                      and
                                        wast-exp-doc.doc-type       = 'рас':U              and
                                        wast-exp-doc.ext-doc-type   = 'ep':U   and
                                        wast-exp-doc.is-del         = false
                                        no-lock on error undo, return error return-value :
    if wast-exp-doc.ext-doc-type = 'ie':U then do:
      assign v-sign = 1.
    end.
    else do:
      assign v-sign = -1.
  ...
  for each wast-exp-line where wast-exp-line.obj-type  = tt-wast-line.obj-type  and
              ...
              first wast-exp-doc where  wast-exp-doc.obj-type       = wast-exp-line.obj-type  and
                                        ...
                                        wast-exp-doc.doc-type       = 'рас':U              and
                                        wast-exp-doc.is-del         = false                   and
                                        wast-exp-doc.ext-doc-type   = 'ee':U      or
                                        wast-exp-doc.obj-type       = wast-exp-line.obj-type  and
                                        wast-exp-doc.obj-code       = wast-exp-line.obj-code  and
                                        wast-exp-doc.doc-code       = wast-exp-line.doc-code  and
                                        wast-exp-doc.internal       = no                      and
                                        wast-exp-doc.doc-type       = 'рас':U              and
                                        wast-exp-doc.ext-doc-type   = 'es':U and
                                        wast-exp-doc.is-del         = false
                                        no-lock on error undo, return error return-value :
    assign
      varwast-sum-sale-base = 0
...
assign
 ...
 if v-petrol then do:
   for each wast-exp-line where wast-exp-line.obj-type  = tt-wast-line.obj-type  and
                                ...
                                wast-exp-line.status_   = 'факт':U
                                and
                                wast-exp-line.fact-order < tt-wast-line.fact-order
                                no-lock ,
              first wast-exp-doc where  wast-exp-doc.obj-type       = wast-exp-line.obj-type  and
                                        wast-exp-doc.obj-code       = wast-exp-line.obj-code  and
                                        wast-exp-doc.doc-code       = wast-exp-line.doc-code  and
...
assign
 ...
 if v-petrol then do:
   for each wast-exp-line where wast-exp-line.obj-type  = tt-wast-line.obj-type  and
                                ...
                                wast-exp-line.status_   = 'факт':U
                                and
                                wast-exp-line.fact-order > tt-wast-line.prev-inv-fact-order
                                no-lock ,
              first wast-exp-doc where  wast-exp-doc.obj-type       = wast-exp-line.obj-type  and
                                        wast-exp-doc.obj-code       = wast-exp-line.obj-code  and
                                        wast-exp-doc.doc-code       = wast-exp-line.doc-code  and
    ...
    assign
      ...
      .
        ...
        find first bf_doc-line-sum where
                   ...
                   bf_doc-line-sum.sum-type = 'wst':U.
        if v_invclcsp = "yes" then do:
          find first bf_cli_doc-line-sum where
                     ...
                     bf_cli_doc-line-sum.sum-type = 'wstc':U.
        end.
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
    ...
    find first bf_gen_trn-doc-sum exclusive-lock where
               ...
               bf_gen_trn-doc-sum.sum-type = 'gen':U   no-error.
    ...
    find first bf_ext_trn-doc-sum exclusive-lock where
               ...
               bf_ext_trn-doc-sum.sum-type = 'ext':U     no-error.
    ...
    find first bf_mis_trn-doc-sum exclusive-lock where
               ...
               bf_mis_trn-doc-sum.sum-type = 'mis':U      no-error.
    ...
    if v_invclcsp = "yes" then do:
      find first bf_gen-cli_trn-doc-sum exclusive-lock where
                 ...
                 bf_gen-cli_trn-doc-sum.sum-type = 'genc':U no-error.
      ...
      find first bf_ext-cli_trn-doc-sum exclusive-lock where
                 ...
                 bf_ext-cli_trn-doc-sum.sum-type = 'extc':U no-error.
      ...
      find first bf_mis-cli_trn-doc-sum exclusive-lock where
                 ...
                 bf_mis-cli_trn-doc-sum.sum-type = 'misc':U  no-error.
    ...
    find first bf_gen_tt-old-doc-line-sum where
               ...
               bf_gen_tt-old-doc-line-sum.sum-type = 'gen':U   no-error.
    ...
    if v_invclcsp = "yes" then do:
      find first bf_gen-cli_tt-old-doc-line-sum where
                 ...
                 bf_gen-cli_tt-old-doc-line-sum.sum-type = 'genc':U no-error.
    ...
    if p-mode = "update":U then do:
      find first bf_gen_tt-doc-line-sum where
                 ...
                 bf_gen_tt-doc-line-sum.sum-type = 'gen':U   no-error.
      ...
      if v_invclcsp = "yes" then do:
        find first bf_gen-cli_tt-doc-line-sum where
                   ...
                   bf_gen-cli_tt-doc-line-sum.sum-type = 'genc':U no-error.
    ...
    find first bf_aft_trn-doc-sum exclusive-lock where
               ...
               bf_aft_trn-doc-sum.sum-type = 'ad':U     no-error.
    ...
    find first bf_aft_tt-old-doc-line-sum where
               ...
               bf_aft_tt-old-doc-line-sum.sum-type = 'ad':U     no-error.
    ...
    if v_invclcsp = "yes" then do:
      find first bf_aft-cli_trn-doc-sum exclusive-lock where
                 ...
                 bf_aft-cli_trn-doc-sum.sum-type = 'acd':U no-error.
      ...
      find first bf_aft-cli_tt-old-doc-line-sum where
                 ...
                 bf_aft-cli_tt-old-doc-line-sum.sum-type = 'acd':U no-error.
    ...
    if p-mode = "update":U then do:
      find first bf_aft_tt-doc-line-sum where
                 ...
                 bf_aft_tt-doc-line-sum.sum-type = 'ad':U     no-error.
      ...
      if v_invclcsp = "yes" then do:
        find first bf_aft-cli_tt-doc-line-sum where
                   ...
                   bf_aft-cli_tt-doc-line-sum.sum-type = 'acd':U no-error.
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input bf_trn-doc.doc-code ,
                        input 'addsum':U ,
                       output v_attr-value ,
                       output v_type ) no-error .
    if error-status :error then do:
      return error substitute( 'Ошибка при вызове процедуры tdat-value&3&1&3&2.',
                               return-value, error-status :get-message( 1 ), chr(10) ).
    end.
    if v_wastage   = "yes" and
       p-calc-wast =  yes  then do:
      if lookup( 'wst':U, v_attr-value ) = 0 then do:
        find first bf_trn-doc-sum where
                   bf_trn-doc-sum.doc-code = bf_trn-doc.doc-code and
                   bf_trn-doc-sum.sum-type = 'wst':U  no-error.
        if not available bf_trn-doc-sum then do:
          ...
          run waitfram-show in p-rwdshandle ( input substitute( 'Запись атрибута для сумм типа "&1". Время &2.'
                                                              , 'wst':U
                                                              , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
          assign v_attr-value = v_attr-value + min( v_attr-value, ',' ) + 'wst':U.
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input bf_trn-doc.doc-code ,
                       input 'addsum':U ,
                       input v_attr-value ) no-error .
          if error-status :error then do:
            run waitfram-hide in p-rwdshandle no-error.
            undo, return error substitute(
      ...
      if lookup( 'wstc':U, v_attr-value ) = 0 then do:
        if v_invclcsp = "yes" then do:
          find first bf_trn-doc-sum where
                     bf_trn-doc-sum.doc-code = bf_trn-doc.doc-code    and
                     bf_trn-doc-sum.sum-type = 'wstc':U no-error.
          if not available bf_trn-doc-sum then do:
            run waitfram-show in p-rwdshandle ( input substitute( 'Создаем запись для сумм типа "&1". Время &2.'
                                                                , 'wstc':U
                                                                , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
            ...
            run waitfram-show in p-rwdshandle ( input substitute( 'Запись атрибута для сумм типа "&1". Время &2.'
                                                                , 'wstc':U
                                                                , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
            assign v_attr-value = v_attr-value + min( v_attr-value, ',' ) + 'wstc':U.
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input bf_trn-doc.doc-code ,
                       input 'addsum':U ,
                       input v_attr-value ) no-error .
            if error-status :error then do:
      ...
      if lookup( 'wst':U,     v_attr-value ) = 0 or
         lookup( 'wstc':U, v_attr-value ) = 0 then do:
...
.
    ...
    if p-calc-addsum = yes then do:
      if lookup( 'gen':U, v_attr-value ) =  0 then do:
        if lookup( 'ad':U, v_attr-value ) <> 0 then do:
          run waitfram-hide in p-rwdshandle no-error.
          undo, return error substitute(
        ...
        find first bf_trn-doc-sum where
                   bf_trn-doc-sum.doc-code = bf_trn-doc.doc-code and
                   bf_trn-doc-sum.sum-type = 'gen':U  no-error.
        if not available bf_trn-doc-sum then do:
          run waitfram-show in p-rwdshandle ( input substitute( 'Создаем запись для сумм типа "&1". Время &2.'
                                                              , 'gen':U
                                                              , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
          ...
          run waitfram-show in p-rwdshandle ( input substitute( 'Записываем атрибут для сумм типа "&1". Время &2.'
                                                              , 'gen':U
                                                              , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
          assign v_attr-value = v_attr-value + min( v_attr-value, ',' ) + 'gen':U +
                                                                  ','   + 'ext':U   +
                                                                  ','   + 'mis':U.
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input bf_trn-doc.doc-code ,
                       input 'addsum':U ,
        ...
        on error undo, return error return-value :
          ...
          find first bf_doc-line-sum where
                     ...
                     bf_doc-line-sum.gds-code = bf_goods.gds-code    and
                     bf_doc-line-sum.sum-type = 'gen':U   no-error.
...
.
      ...
      if v_invclcsp = "yes" then do:
        if lookup( 'genc':U, v_attr-value ) =  0 then do:
          if lookup( 'acd':U, v_attr-value ) <> 0 then do:
          ...
          find first bf_trn-doc-sum where
                     bf_trn-doc-sum.doc-code = bf_trn-doc.doc-code    and
                     bf_trn-doc-sum.sum-type = 'genc':U no-error.
          if not available bf_trn-doc-sum then do:
            run waitfram-show in p-rwdshandle ( input substitute( 'Создаем запись для сумм типа "&1". Время &2.'
                                                                , 'genc':U
                                                                , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
            ...
            run waitfram-show in p-rwdshandle ( input substitute( 'Записываем атрибут для сумм типа "&1". Время &2.'
                                                                , 'genc':U
                                                                , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
            assign v_attr-value = v_attr-value + min( v_attr-value, ',' ) + 'genc':U +
                                                                    ','   + 'extc':U   +
                                                                    ','   + 'misc':U.
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input bf_trn-doc.doc-code ,
                       input 'addsum':U ,
          ...
          on error undo, return error return-value :
            ...
            find first bf_doc-line-sum where
                       bf_doc-line-sum.doc-code = bf_doc-line.doc-code   and
                       bf_doc-line-sum.gds-code = bf_goods.gds-code      and
                       bf_doc-line-sum.sum-type = 'genc':U no-error.
            if not available bf_doc-line-sum then do:
              run lib-rwds_crlinsum in this-procedure ( input bf_doc-line.doc-code
                                                      , input 'genc':U
                                                      , input bf_doc-line.artic
            ...
            run waitfram-join in p-rwdshandle (  input "Расчитываем записи строк сумм типа 'genc':U."
...
.
      ...
      if lookup( 'ad':U, v_attr-value ) = 0 then do:
        find first bf_trn-doc-sum where
                   bf_trn-doc-sum.doc-code = bf_trn-doc.doc-code and
                   bf_trn-doc-sum.sum-type = 'ad':U    no-error.
        if not available bf_trn-doc-sum then do:
          run waitfram-show in p-rwdshandle ( input substitute( 'Создаем запись для сумм типа "&1". Время &2.'
                                                              , 'ad':U
                                                              , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
          ...
          run waitfram-show in p-rwdshandle ( input substitute( 'Запись атрибута для сумм типа "&1". Время &2.'
                                                              , 'ad':U
                                                              , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
          assign v_attr-value = v_attr-value  + min( v_attr-value, ',' ) + 'ad':U.
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input bf_trn-doc.doc-code ,
                       input 'addsum':U ,
                       input v_attr-value ) no-error .
          if error-status :error then do:
        ...
        on error undo, return error return-value :
          ...
          find first bf_doc-line-sum where
                     bf_doc-line-sum.doc-code = bf_doc-line.doc-code and
                     bf_doc-line-sum.gds-code = bf_goods.gds-code    and
                     bf_doc-line-sum.sum-type = 'ad':U     no-error.
          if not available bf_doc-line-sum then do:
            run lib-rwds_crlinsum in this-procedure ( input bf_doc-line.doc-code
                                                    , input 'ad':U
                                                    , input bf_doc-line.artic
...
.
      ...
      if v_invclcsp = "yes" then do:
        if lookup( 'acd':U, v_attr-value ) = 0 then do:
          find first bf_trn-doc-sum where
                     bf_trn-doc-sum.doc-code = bf_trn-doc.doc-code  and
                     bf_trn-doc-sum.sum-type = 'acd':U no-error.
          if not available bf_trn-doc-sum then do:
            run waitfram-show in p-rwdshandle ( input substitute( 'Создаем запись для сумм типа "&1". Время &2.'
                                                                , 'acd':U
                                                                , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
            ...
            run waitfram-show in p-rwdshandle ( input substitute( 'Запись атрибута для сумм типа "&1". Время &2.'
                                                                , 'ad':U
                                                                , string( time - j#time, "hh:mm:ss":U ) ) ) no-error.
            assign v_attr-value = v_attr-value + min( v_attr-value, ',' ) + 'acd':U.
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-wrt in g#trdcalib ( input bf_trn-doc.doc-code ,
                       input 'addsum':U ,
                       input v_attr-value ) no-error .
            if error-status :error then do:
          ...
          on error undo, return error return-value :
            ...
            find first bf_doc-line-sum where
                       bf_doc-line-sum.doc-code = bf_doc-line.doc-code and
                       bf_doc-line-sum.gds-code = bf_goods.gds-code    and
                       bf_doc-line-sum.sum-type = 'acd':U no-error.
            if not available bf_doc-line-sum then do:
              run lib-rwds_crlinsum in this-procedure ( input bf_doc-line.doc-code
                                                      , input 'acd':U
                                                      , input bf_doc-line.artic
