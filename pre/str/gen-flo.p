...
define input  parameter p-fo-type as character no-undo .
    ...
    for each temp-obj-firm on error undo, return error :
           for each buf_ord-doc no-lock where
                   ...
                   (buf_ord-doc.obj-type   = temp-obj-firm.obj-type and
                    buf_ord-doc.obj-code   = temp-obj-firm.obj-code and
                    buf_ord-doc.host-code  = par-host-code          and
                    buf_ord-doc.status_ = 'факт':U  and
            ...
            on error undo, return error :
                  ...
                  for each tt-fin-ob where
                      ...
                      first buf_contract no-lock where buf_contract.contract-code = tt-fin-ob.contract-code and
                                                       ...
                                                       lookup( buf_contract.usl-opl ,  'По заказу':U + "," + 'Отсрочка платежа по заказу':U + "," + 'Предоплата,Предоплата(%)' )   > 0
                            break
...
p-res = p-res  + chr(10)   +
       ...
       "Просмотрено заказов              :" + string(col-trn)           + chr(10) +
...
if v-may-be = false then do:
   p-res = p-res + chr(10) + substitute("По заказу &1 нельзя создавать ФО в этой БД" , buf_ord-doc.doc-code ) .
   return .
end.
assign
...
if ( col-trn  modulo temp1 = 0 ) and ( col-trn >= temp1 ) then run waitfram-show( "Обработано заказов : " + string( col-trn )) .
      for each buf_ord-line no-lock where buf_ord-line.doc-code  = buf_ord-doc.doc-code  ,
              ...
              first buf_contract no-lock where buf_contract.contract-code = buf_ord-doc.contract-code and
                                               ...
                                               lookup( buf_contract.usl-opl ,  'По заказу':U
                                                                       + "," + 'Отсрочка платежа по заказу':U
              ...
              break
                  ...
                  when 'Предоплата(%)':U then do:
                        if buf1_ord-doc.status_ = 'факт':U then do:
                  ...
                  if buf_contract.usl-opl = 'Предоплата(%)':U then do:
                    if buf1_ord-doc.status_ = 'факт':U then do:
                    ...
                    if buf1_ord-doc.status_ = 'поставка':U then do:
...
.
 if  buf_contract.usl-opl = 'По заказу':U or buf_contract.usl-opl = 'По поставке заказа':U
   then v-date-pay = today  .
   else dO:
        if p-cons = 2 and available   buf_ord-doc then do:
            var-date = buf_ord-doc.fact-date + (if buf_contract.srok-opl <> ? then buf_contract.srok-opl else 0 ) .
...
.
 if  buf_contract.usl-opl = 'Предоплата':U  then v-date-pay = today  .
   else do:
        if p-cons = 2 and available   buf_ord-doc then do:
            var-date = buf_ord-doc.doc-date .
            if var-date < today then var-date = today.
            v-date-pay = var-date.
        end.
   end.
 ...
 if p-cons = 2 then do:
      if available   buf_ord-doc
         then
         assign
           n-trn-doc-code       =  buf_ord-doc.doc-code
         .
         else  n-trn-doc-code       = ?   .
     end.
     else do:
       if pfo = true and available   buf_ord-doc
         then  assign
           n-trn-doc-code       =  buf_ord-doc.doc-code
         .
         else  n-trn-doc-code      = ?   .
...
assign
  ...
  n-sum-contract       =  (  n-exch-scale   / n-exch-rate ) * v-sum-rubl
  ...
  run create-fin-liab (
    ...
    .
        ...
        if available buf2_ord-doc then do:
            if buf_contract.usl-opl = 'Предоплата(%)':U and buf2_ord-doc.status_ = 'факт':U then
            assign
  ...
  :
    for each buf_ord-line-rcv no-lock where
            ...
            each buf_doc-line no-lock where
                  ...
                  buf_doc-line.ext-doc-type = 'ee':U   and
                  buf_doc-line.status_      = 'факт':U
            :
