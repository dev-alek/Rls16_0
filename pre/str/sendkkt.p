...
p-value    = entry(4, p-parameter, chr(4))
no-error
.
...
FIND FIRST ub.cash-desk NO-LOCK WHERE
           ...
           ub.cash-desk.pos-type = 'IBM-XML':U AND
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка схемы интеграции ККТ '" + " &1")
...
END CASE.
  RUN putc-kkt in this-procedure
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'изменение схемы интеграции ККТ ')
                  else ('Ждите - ' + 'удаление схемы интеграции ККТ ') ) +
