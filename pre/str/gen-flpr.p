...
if p-trn-doc = 1  then do:
    p-usl-opl =  'По факту поставки,Отсрочка платежа (по поставке)' .
   end.
   else do:
    p-usl-opl =  'По факту реализации,По реализации части приход. накладной,Отсрочка платежа (по реализации)'  .
   end.
...
if p-trn-doc = ? then assign
                       v-type-trn-doc = 'ie,ep,':U + 'ee,es,re,rs,we,wm,pc,':U + 'vt,ap,mp,vp,':U
                       v-sign-list    = '1,-1,':U + '1,1,-1,-1,1,1,1,':U + '1,1,1,1,':U
                       .
if p-trn-doc = 1 then assign
                       v-type-trn-doc = 'ie,ep,':U + 'vt,ap,mp,vp,':U
                       v-shot-type-trn-doc = 'при':U
                       v-sign-list    = '1,-1,':U + '1,1,1,1,':U
                       .
if p-trn-doc = 2 then assign
                      v-type-trn-doc = 'ee,es,re,rs,we,wm,pc,':U + 'vt,ap,mp,vp,':U
                      v-shot-type-trn-doc = 'рас':U
                      v-sign-list    = '1,1,-1,-1,1,1,1,':U + '1,1,1,1,':U
                      .
...
if p-trn-doc <> ?  then do:
    ...
    p-usl-opl = 'По факту поставки,Отсрочка платежа (по поставке)' + "," + 'По факту реализации,По реализации части приход. накладной,Отсрочка платежа (по реализации)' .
    for each tt-trn-doc  no-lock  where
        tt-trn-doc.is-del     = true and
        lookup(tt-trn-doc.ext-doc-type , v-type-trn-doc) > 0
        and    tt-trn-doc.status_    = 'факт':U
              ...
              col-contract =  0 .
                   for each tt-fin-ob where
                      ...
                      first buf_contract no-lock where buf_contract.contract-code = tt-fin-ob.contract-code and
                                                        ...
                                                        buf_contract.usl-opl       <> 'По реализации части приход. накладной':U
...
p-res = p-res  + chr(10)   +  chr(10)   +
       ...
       " Создано ПредФинанОбязательств   :" + string(col-bfo) + chr(10) +  chr(10) +
...
if buf_trn-doc.ext-doc-type = 'ap':U then return.
...
if ( col-trn  modulo temp1 = 0 ) and ( col-trn >= temp1 ) then run waitfram-show( "Обработано накладных : " + string( col-trn )) .
      for each buf_parts no-lock where buf_parts.out-code  = buf_trn-doc.doc-code   and
              ...
              first buf_contract no-lock where buf_contract.contract-code = buf_parts.contract-code and
                            ...
                            ((
                            lookup ( buf_contract.usl-opl , 'По факту поставки,Отсрочка платежа (по поставке)' ) > 0 and
                            lookup ( buf_trn-doc.ext-doc-type , 'ie,ep,':U  ) > 0
                            ) OR
                            ( lookup ( buf_contract.usl-opl , 'По факту реализации,По реализации части приход. накладной,Отсрочка платежа (по реализации)' ) > 0   and
                              lookup ( buf_trn-doc.ext-doc-type , 'ee,es,re,rs,we,wm,pc,':U + 'vt,ap,mp,vp,':U ) > 0
                              ) )
              break
         ...
         Run clc-price.
         if lookup ( buf_trn-doc.ext-doc-type , 'vt,ap,mp,vp,':U ) > 0 then do:
             find first c-doc-attr no-lock where
                        c-doc-attr.doc-code  = buf_trn-doc.doc-code and
                        c-doc-attr.attr-code = 'olsuppcntr':U no-error .
              ...
              if
               ...
               then do:
                  if buf_trn-doc.ext-doc-type <> 'vp':U then do:
                  if buf_contract.usl-opl = 'По реализации части приход. накладной':U
                    then  assign
                  ...
                  .
                    ...
                    if buf_contract.usl-opl = 'По реализации части приход. накладной':U then
                            assign
                              var-sum-rublb     = var-sum-rublb     +  abs(tt-allsum.sum-dsc-rubl-acc - ( tt-allsum.transport-rubl-acc +  tt-allsum.other-rubl-acc) )  * (-1)
                              var-sum-contractb = var-sum-contractb +  abs(tt-allsum.sum-dsc-cli-acc                                                                )  * (-1)
                              var-sum-baseb     = var-sum-baseb     +  abs(tt-allsum.sum-dsc-base-acc - ( tt-allsum.transport-base-acc +  tt-allsum.other-base-acc) )  * (-1)
         ...
         if lookup ( buf_trn-doc.ext-doc-type , 'vt,ap,mp,vp,':U ) = 0  and
              (
               ( lookup(buf_contract.usl-opl , 'По факту поставки,Отсрочка платежа (по поставке)' ) > 0 and
                    v-flag-po = false )
                  or
         ...
         then do:
            ...
            if last-of ( buf_parts-attr.income-in-code ) then do:
                  if buf_contract.usl-opl = 'По реализации части приход. накладной':U  and
                     lookup ( buf_trn-doc.ext-doc-type , 'ee,es,re,rs,we,wm,pc,':U + 'vt,ap,mp,vp,':U ) > 0
                     then do:
            ...
            if last-of(buf_parts.contract-code) or (p-nalog = 2 and last-of(buf_parts.vat-pc)) then do:
               ...
               var-sum-baseb = 0 .
               var-sum-contractb = 0 .
                  if buf_contract.usl-opl <> 'По реализации части приход. накладной':U then do:
                      if p-cons = 1 then do:
      ...
      for each buf_parts no-lock where buf_parts.out-code  = buf_trn-doc.doc-code   and
            ...
            if last-of(buf_parts.contract-code) or (p-nalog = 2 and last-of(buf_parts.vat-pc)) then do:
              run make-fo-corr ( input var-sum-rubl ,
                                     ...
                                     input var-sum-contract )
                                     no-error .
              if error-status :error then p-res = p-res + chr(10) + error-status :get-message(1) .
              var-sum-rubl = 0 .
              var-sum-rublb = 0 .
              var-sum-base      = 0 .
              var-sum-baseb     = 0 .
...
.
 if lookup ( buf_contract.usl-opl , "По факту поставки,По факту реализации,По реализации части приход. накладной") > 0
...
define buffer buf5_trn-doc for trn-doc.
define variable pfo as logical no-undo .
pfo = true .
...
for each    t-buf_fin-ob-trn no-lock where
            ...
            :
              ...
              v-doc-code-fo = string(v-n) .
              v-recid = recid(t-buf_fin-ob) .
end.
    ...
    for each bufv_fin-ob-before  exclusive-lock   where
        ...
        if first-of (bufv_fin-ob-before.doc-code) then do:
          ...
          if available bufv_contract then v-proc-contr = bufv_contract.srok-opl .
                                        ...
                                        v-proc-contr = 0 .
                                        v-message-text =  v-message-text + chr(10)  + error-status :get-message(1) + " Вн.№ договора " + string(bufv_fin-ob-before.contract-code) .
                                    end.
          run sum-in-parts (
...
(
  ...
  .
  if error-status :error then  p-res = p-res + chr(10) + error-status :get-message(1) + return-value .
...
:
 ...
 p-ok = true .
  if buf_trn-doc.ext-doc-type   = 'pc':U then do:
    p-ok = false   .
    if  buf_contract.contract-type = 'Ответственного хранения':U and
        buf_parts.purch-code       = 3 then p-ok = true .
  end.
end.
end procedure.
...
:
 ...
 p-ok = true .
  if buf_trn-doc.ext-doc-type   <> 'pc':U then do:
    if  buf_contract.contract-type = 'Ответственного хранения':U and
        buf_parts.purch-code      <> 3  then p-ok = false  .
  end.
end.
end procedure.
