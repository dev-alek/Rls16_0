...
define temp-table tt-parts-for-cnt-lk no-undo like ub.parts.
define temp-table tt-cnt-parts-lk no-undo
field host-code          like ub.contract.host-code
field contract-code      like ub.contract.contract-code
field sum-dsc-base-acc   like ub.doc-line.price-base
field sum-dsc-rubl-acc   like ub.doc-line.price-base
...
field supp-name          like ub.clients.obj-name
index pi is unique primary host-code contract-code supp-type supp-code.
procedure cntparts_calc-table-cnt-lk :
define input parameter pardoc-code like ub.trn-doc.doc-code no-undo.
define buffer bf_trn-doc  for ub.trn-doc.
define buffer bf_parts    for ub.parts.
do on error undo, return error substitute ("&1 &2 &3", return-value, error-status:get-message(1), error-status:get-message(2) ) :
...
for each tt-parts-for-cnt-lk on error undo, return error return-value :
  delete tt-parts-for-cnt-lk.
end.
for each bf_parts where bf_parts.out-code = bf_trn-doc.doc-code no-lock on error undo, return error return-value :
  create tt-parts-for-cnt-lk.
  buffer-copy bf_parts to tt-parts-for-cnt-lk.
end.
run cntparts_calc-tt-table-cnt-lk in this-procedure.
end.
end procedure.
procedure cntparts_calc-tt-table-cnt-lk:
define buffer bf_contract for ub.contract.
define buffer bf_clients  for ub.clients.
do on error undo, return error return-value :
for each tt-cnt-parts-lk on error undo, return error return-value :
...
for each tt-parts-for-cnt-lk on error undo, return error return-value :
  ...
  create tt-clcparts.
  buffer-copy tt-parts-for-cnt-lk to tt-clcparts.
  run clcprtsl_calc-parts (
      input recid(tt-clcparts),
      input no,
      input no,
  ...
  find first tt-allsum where tt-allsum.sum-type = 'основная_сумма':U.
  find first tt-cnt-parts-lk where tt-cnt-parts-lk.host-code     = tt-parts-for-cnt-lk.host-code     and
                                   tt-cnt-parts-lk.contract-code = tt-parts-for-cnt-lk.contract-code and
                                   tt-cnt-parts-lk.supp-type     = tt-parts-for-cnt-lk.supp-type     and
                                   tt-cnt-parts-lk.supp-code     = tt-parts-for-cnt-lk.supp-code     no-error.
  if not available tt-cnt-parts-lk then do:
    create tt-cnt-parts-lk.
    assign
      tt-cnt-parts-lk.host-code     = tt-parts-for-cnt-lk.host-code
      tt-cnt-parts-lk.contract-code = tt-parts-for-cnt-lk.contract-code.
    if tt-parts-for-cnt-lk.contract-code <> 0 then do:
      find first bf_contract where bf_contract.host-code     = tt-parts-for-cnt-lk.host-code     and
                                   bf_contract.contract-code = tt-parts-for-cnt-lk.contract-code no-lock no-error.
      if not available bf_contract then do:
        return error substitute ("Не найден договор по фирме &1 с внутренним кодом &2.", tt-parts-for-cnt-lk.host-code, tt-parts-for-cnt-lk.contract-code).
      end.
      assign
        tt-cnt-parts-lk.contract-prn-code = bf_contract.contract-prn-code.
    end.
    else do:
      assign
        tt-cnt-parts-lk.contract-prn-code = "без договора".
    end.
    find first bf_clients where bf_clients.obj-type = tt-parts-for-cnt-lk.supp-type and
                                bf_clients.obj-code = tt-parts-for-cnt-lk.supp-code no-lock no-error.
    if not available bf_clients then do:
      return error substitute ("Не найден поставщик &1 &2.", tt-parts-for-cnt-lk.supp-type, tt-parts-for-cnt-lk.supp-code).
    end.
    assign
      tt-cnt-parts-lk.supp-type = bf_clients.obj-type
      tt-cnt-parts-lk.supp-code = bf_clients.obj-code
      tt-cnt-parts-lk.supp-name = bf_clients.obj-name.
  end.
  assign
    tt-cnt-parts-lk.sum-dsc-base-acc   = tt-cnt-parts-lk.sum-dsc-base-acc   + tt-allsum.sum-dsc-base-acc
    tt-cnt-parts-lk.sum-dsc-rubl-acc   = tt-cnt-parts-lk.sum-dsc-rubl-acc   + tt-allsum.sum-dsc-rubl-acc
    tt-cnt-parts-lk.vat-base-acc       = tt-cnt-parts-lk.vat-base-acc       + tt-allsum.vat-base-acc
    tt-cnt-parts-lk.vat-rubl-acc       = tt-cnt-parts-lk.vat-rubl-acc       + tt-allsum.vat-rubl-acc
    tt-cnt-parts-lk.slt-base-acc       = tt-cnt-parts-lk.slt-base-acc       + tt-allsum.slt-base-acc
    tt-cnt-parts-lk.slt-rubl-acc       = tt-cnt-parts-lk.slt-rubl-acc       + tt-allsum.slt-rubl-acc
    tt-cnt-parts-lk.road-tax-base-acc  = tt-cnt-parts-lk.road-tax-base-acc  + tt-allsum.road-tax-base-acc
    tt-cnt-parts-lk.road-tax-rubl-acc  = tt-cnt-parts-lk.road-tax-rubl-acc  + tt-allsum.road-tax-rubl-acc
    tt-cnt-parts-lk.excise-base-acc    = tt-cnt-parts-lk.excise-base-acc    + tt-allsum.excise-base-acc
    tt-cnt-parts-lk.excise-rubl-acc    = tt-cnt-parts-lk.excise-rubl-acc    + tt-allsum.excise-rubl-acc
    tt-cnt-parts-lk.transport-base-acc = tt-cnt-parts-lk.transport-base-acc + tt-allsum.transport-base-acc
    tt-cnt-parts-lk.transport-rubl-acc = tt-cnt-parts-lk.transport-rubl-acc + tt-allsum.transport-rubl-acc
    tt-cnt-parts-lk.other-base-acc     = tt-cnt-parts-lk.other-base-acc     + tt-allsum.other-base-acc
    tt-cnt-parts-lk.other-rubl-acc     = tt-cnt-parts-lk.other-rubl-acc     + tt-allsum.other-rubl-acc
  .
end.
end.
end procedure.
procedure cntparts_calc-c-table-cnt-lk :
define input parameter pardoc-code like ub.c-trn-doc.doc-code no-undo.
define input parameter parchip-num like ub.c-trn-doc.chip-num no-undo.
define buffer bf_c-trn-doc for ub.c-trn-doc.
define buffer bf_c-parts   for ub.c-parts.
...
for each bf_c-parts where bf_c-parts.out-code = bf_c-trn-doc.doc-code and
  ...
  find first tt-allsum where tt-allsum.sum-type = 'основная_сумма':U.
  find first tt-cnt-parts-lk where tt-cnt-parts-lk.host-code     = bf_c-parts.host-code     and
                                   tt-cnt-parts-lk.contract-code = bf_c-parts.contract-code and
                                   tt-cnt-parts-lk.supp-type     = bf_c-parts.supp-type     and
                                   tt-cnt-parts-lk.supp-code     = bf_c-parts.supp-code     no-error.
  if not available tt-cnt-parts-lk then do:
    create tt-cnt-parts-lk.
    assign
      tt-cnt-parts-lk.host-code     = bf_c-parts.host-code
      tt-cnt-parts-lk.contract-code = bf_c-parts.contract-code.
    if bf_c-parts.contract-code <> 0 then do:
      find first bf_contract where bf_contract.host-code     = bf_c-parts.host-code     and
                                   bf_contract.contract-code = bf_c-parts.contract-code no-lock no-error.
      if not available bf_contract then do:
...
find first buf_doc-attr no-lock where
           ...
           buf_doc-attr.attr-code = 'olsuppcntr':U  no-error .
...
case varinc-exp:
  ...
  when 2 then do:
    ...
    for each buf_parts where buf_parts.out-code = buf_trn-doc.doc-code no-lock on error undo, return error return-value :
      if buf_parts.contract-code <> 0 then do:
        find first buf_contract where buf_contract.contract-code = buf_parts.contract-code no-lock no-error.
        if available buf_contract then do:
          if lookup (buf_contract.usl-opl, 'По факту реализации,По реализации части приход. накладной,Отсрочка платежа (по реализации)') > 0 and
             not (buf_parts.purch-code = integer('1':U) and buf_contract.contract-type = 'Ответственного хранения':U) then do:
            assign
              varcr-expfo = yes.
            leave parts-cycle.
          end.
 ...
 if buf_trn-doc.ext-doc-type = 'pc':U then do:
   define buffer bf_trn-doc for ub.trn-doc .
...
if ( buf_trn-doc.ext-doc-type = 'ee':U or
     buf_trn-doc.ext-doc-type = 're':U ) and
     buf_trn-doc.contract-code <> 0 then
        for first buf_contract where buf_contract.contract-code = buf_trn-doc.contract-code no-lock :
          if buf_contract.doc-type = 'рас':U and lookup (buf_contract.usl-opl, 'Предоплата,Предоплата(%)') > 0 then do :
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
              ...
              if buf_trn-doc.status_ = 'накл':U and buf_trn-doc.flag_ and (v-fo-gen = 2 or v-fo-gen = 3) then buf_trn-doc.need-buyer = 1.
              if buf_trn-doc.status_ = 'разрешен':U                  and (v-fo-gen = 4 or v-fo-gen = 5) then buf_trn-doc.need-buyer = 1.
              if buf_trn-doc.status_ = 'факт':U                       and (v-fo-gen = 6 or v-fo-gen = 7) then buf_trn-doc.need-buyer = 1.
              if buf_trn-doc.status_ = 'факт':U then do :
                for each buf_fin-ob-trn where buf_fin-ob-trn.trn-doc-code = buf_trn-doc.doc-code  and
                        ...
                        :
                           if buf_fin-ob.status_ <> 'факт':U then do :
                            undo, return error "Данная накладная не может быть закрыта на факт, т.к. по ней есть незакрытое ФО" .
                           end.
                end.
              end.
          end.
          if buf_trn-doc.status_ = 'факт':U and lookup (buf_contract.usl-opl, 'По факту поставки покупателю,Отсрочка платежа по поставке') > 0 then do:
              assign
                buf_trn-doc.need-buyer = 1.
          end.
          if buf_trn-doc.status_ = 'факт':U and buf_contract.usl-opl = 'Не определено':U then do:
            assign
