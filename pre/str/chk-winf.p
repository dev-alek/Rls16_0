...
if from-doc then do:
  ...
  not-doced-err-ch = substitute("ОШИБОЧНЫX чеков МЦ ЗА &1 НЕТ&2"
                               ,v-shift-str
                               ,chr(10))
  doc-ch = "Нет НЕЗАКРЫТЫХ автодокументов МЦ " + chr(10)
  .
  if chk-inf then do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
 ...
 define variable v-ii as integer no-undo .
  v-status-list = 'накл':U + chr(4) + 'разрешен':U.
  _do:
  do v-ii = 1 to num-entries(v-status-list, chr(4)):
    _for:
    for each ub.wth-doc No-LOCK WHERE
        ...
        AND ub.wth-doc.status_ = entry(v-ii, v-status-list, chr(4) )
  ...
  assign
  not-doced-ch = "НЕУЧТЕННЫХ ЧЕКОВ МЦ НЕТ" + chr(10)
  not-doced-err-ch =   "ОШИБОЧНЫX чеков НЕТ " + chr(10)
  doc-ch = "Нет НЕЗАКРЫТЫХ автодокументов МЦ" + chr(10)
  .
  ...
  :
    if lookup(string(ub.chk-doc.chk-type), '2,3,4,5,7':U) = 0 then next.
