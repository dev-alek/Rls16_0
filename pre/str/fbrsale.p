...
if session :set-wait-state( "compiler" ) then.
   ...
   assign
      v-doc-code     = entry( 1, p-parameters, chr(4) )
      v-kitchen-rest = ( entry( 2, p-parameters, chr(4) ) = "yes" )
      v-store-rest   = ( entry( 3, p-parameters, chr(4) ) = "yes" )
      .
   do transaction
      on error undo, return error
      :
  ...
  ,output v-host-code
      ...
      for each temp_fbr-objects
         ...
         for each buf_sale-doc where
            ...
            then do:
               if buf_recipe.recipe-type = 'производство':U and v-ban-recipes then
               do:
               ...
               if buf_recipe.recipe-type = 'альтернатива':U and v-ban-altr then
               do:
                  for first buf_goods no-lock where buf_goods.gds-code = buf_recipe.gds-code,
                     ...
                     do:
                        ...
                        if v-value <> "yes" and v-value <> "true" then
                        do:
                           v-mess =  "Рецепт альтернатива " + buf_recipe.recipe-code + " " + buf_recipe.recipe-name + chr(10) + "входит в группу, у которой не установлен атрибут: " + chr(10) + "Запрет передачи на кассу." .
                           run write-log in p-log-handle (
                              input 5
                              , input v-mess
                              ).
               ...
               if buf_recipe.recipe-type = 'комплектация':U and v-ban-recipes then
               do:
                  for each ub.recipe-gds no-lock where ub.recipe-gds.recipe-code = buf_recipe.recipe-code:
                     run gds-attr-value in this-procedure  ( input  ub.recipe-gds.gds-code
                        , input  'mark-type':U
         ...
         do:
            ...
            do:
               define variable v-user-action as character no-undo .
               define variable v-printed     as logical   no-undo .
               if search ('fbr-rsrv-errors-tt.txt') <> ? then
               do:
