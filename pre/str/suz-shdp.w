...
DO v-ii = 1 TO NUM-ENTRIES(v-param-list, chr(124)):
   IF v-ii = 1  THEN DO:
     fi-rep-dir = entry(v-ii, v-param-list, chr(124)).
   END.
   IF v-ii = 2  THEN DO:
     fi-rep-dir-xml = entry(v-ii, v-param-list, chr(124)).
   END.
END.
...
then do :
  ...
  v-uniq-key-rec = "schedule" + chr(3) + string(p-cre-db-num) + chr(3) + p-task-type + chr(3) + string(p-task-num) .
end.
...
then do :
  find first buf_schedule-attr no-lock
       where buf_schedule-attr.cre-db-num = p-cre-db-num
         ...
         and buf_schedule-attr.task-num   = p-task-num
         no-error .
  if not available buf_schedule-attr then v-mode = 'ДОБАВЛЕНИЕ':U .
end.
run rul/thbjrum1.p (
                 input v-mode
                ,input 'rep':U
...
run schedule-attr-write in this-procedure
  ...
  ,input p-task-num
  ,input 'schedule-param-list':U
  ,INPUT (fi-rep-dir + chr(124) + fi-rep-dir-xml)
  ).
end.
END PROCEDURE.
