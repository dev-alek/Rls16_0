...
DEFINE SHARED TEMP-TABLE cash-grp no-undo
...
grp-code
.
DEFINE SHARED TEMP-TABLE cash-units no-undo  like ub.units
.
DEFINE SHARED TEMP-TABLE cash-gds-prt no-undo like ub.gds-prt
.
DEFINE SHARED TEMP-TABLE cash-country no-undo like ub.country
.
...
CASE pos-type:
    ...
    then do:
        FOR EACH cash-grp NO-LOCK use-index pi:
            PUT stream IBMstream unformatted
            '8':U chr(32)
            chr(34) string((if cash-grp.news-action
                            then "D":U
                            else action
                            ), "x(1)" ) chr(34)  chr(32)
            cash-grp.grp-code format ">>9" chr(32)
            chr(34)
    ...
    if action = 'U':U then do:
      for each cash-dis-grp-rule where
              cash-Dis-grp-rule.pos-type = 'NCR-AS@R':U
          and cash-Dis-grp-rule.host-code = v-host-code
          AND cash-dis-grp-rule.obj-type = 'маг':U
          AND cash-dis-grp-rule.obj-code = i-obj-code
          and cash-dis-grp-rule.classif-type = 'sum-grp':U
          and cash-dis-grp-rule.discnt-role = 'gds-grp-pcnt':U,
      ...
      if action = "D":U then do:
        FOR EACH cash-grp NO-LOCK use-index pi:
          run create-ncr-kat-discnt in this-procedure (
                                                        ...
                                                        ,input ('DP' + fill( chr(32) , 10) + string(cash-grp.grp-code, '>>>9'))
                                                        ,input '':U
                                                        ,input 0
                                                        ,input 0
                                                        ,input 'time-rule-num':U
  ...
  when 'MARIA':U then do:
    ...
    FOR EACH cash-grp NO-LOCK use-index pi:
      ...
      assign
      v-record = v-record +
                  fill(chr(63) + chr(4), cash-grp.grp-code - v-prev-grp-code - 1) +
                  v-maria-discnt-value + chr(4) .
      assign
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + substitute('&1: добавление', is-bo-name))
                  else ('Ждите - ' + substitute('&1: удаление', is-bo-name)) ) +
...
define variable v-found25 as logical no-undo .
define variable v-is-script25 as logical no-undo.
define variable v-fields-shift25 as integer no-undo .
find first temp-tekka-tsk no-error.
if available temp-tekka-tsk then do:
   v-found25 = yes.
end.
if v-found25 = yes then do:
...
By ub.cash-desk.pos-type :
    IF FIRST-OF(ub.cash-desk.pos-type) then do:
      ...
      and ub.cash-desk.pos-type <> 'InfoKiosk':U then NEXT.
      if p-subject = '':u then do:
...
define variable v-param-type42 as character no-undo .
define variable v-value-date42 as date no-undo .
define variable v-value-decimal42 as decimal no-undo .
define variable v-value-integer42 as INTEGER no-undo .
define variable v-value-logical42 AS LOGICAL no-undo .
define variable v-tth42 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ,input  i-obj-code
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd
    ,output v-value-date42
    ,output v-value-decimal42
    ,output v-value-integer42
    ,output v-value-logical42
    ,output v-param-type42
    ,INPUT-OUTPUT table-handle v-tth42
    )  .
delete object v-tth42 no-error.
if dflt-cd = 'NCR-AS@R':U
and p-subject = '':U
then do:
 if action= 'U' then do:
    ...
    if p-dis-rule-num > 0 then do:
      ...
      for each buf_cash-dis-rule no-lock where
        ...
        cash-ncr-dis-kat.cd-subject-code  =  ( if p-templ-rl-root = 37
                                               then ('SI' + fill( chr(32) , 10) + string(p-node-code, '>>>9'))
                                               else ('DP' + fill( chr(32) , 10) + string(p-node-code, '9999')) )
        cash-ncr-dis-kat.cd-subject-name  = SUBSTRING(buf_cash-dis-rule.des, 1, 20) +
                                          ( if length(buf_cash-dis-rule.des) < 20 then fill( chr(32) , 20 - length(buf_cash-dis-rule.des) ) else '' )
        ...
        .
        assign
        cash-ncr-dis-kat.cd-other =   fill(chr(32), 10) +  "xx ":U +  "%":U +                                                                                             ~
...
procedure fill-cash-dis-grp-rule.
    ...
    do ii = 1 to 3:
      ...
      for each buf_dis-grp-rule no-lock where
          ...
          and buf_dis-grp-rule.classif-type = 'sum-grp':U
          and buf_dis-grp-rule.discnt-role = 'gds-grp-pcnt':U,
        ...
        if not available cash-dis-grp-rule then do:
          ...
          cash-dis-grp-rule.obj-type = 'маг':U
      ...
      for each buf2_dis-thbj-rule no-lock where
              ...
              and buf2_dis-thbj-rule.discnt-role = 'kat-gds-grp':U,
         each buf2_dis-rule no-lock where
            ...
            first buf_dis-grp-rule no-lock where
             ...
             and buf_dis-grp-rule.classif-type = 'sum-grp':U
             and buf_dis-grp-rule.discnt-role = 'gds-grp-pcnt-kat':U
               :
        find first cash-dis-grp-rule where
              cash-Dis-grp-rule.pos-type = dflt-cd
          and cash-Dis-grp-rule.host-code = v-host-code
          ...
          AND cash-dis-grp-rule.obj-code = i-obj-code
          and cash-dis-grp-rule.classif-type = 'sum-grp':U
          and cash-dis-grp-rule.discnt-role = 'gds-grp-pcnt-kat':U
          and cash-dis-grp-rule.node-code = buf_dis-grp-rule.node-code no-error.
        if not available cash-dis-grp-rule then do:
          create cash-dis-grp-rule.
          buffer-copy buf_dis-grp-rule
