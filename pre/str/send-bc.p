...
assign
inp-recid = integer(entry(1, p-parameter, chr(4)))
...
define variable vss-include-info3 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont3 = yes then do:
    if integer( substring( tmp-str3, 1, length( bc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = bc-pfx + substring( tmp-str3, length( bc-pfx ) + 1, length( tmp-str3 ) - length( bc-pfx ) )
...
END PROCEDURE.
define variable vss-include-info4 as character format "x(65)" no-undo initial "@(#)$Workfile: defc-gds.i $ $Revision: 47e5c2a27e63, 2885, rls $".
...
define variable v-param-type56 as character no-undo .
define variable v-value-date56 as date no-undo .
define variable v-value-decimal56 as decimal no-undo .
define variable v-value-integer56 as INTEGER no-undo .
define variable v-value-logical56 AS LOGICAL no-undo .
define variable v-tth56 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd
    ,output v-value-date56
    ,output v-value-decimal56
    ,output v-value-integer56
    ,output v-value-logical56
    ,output v-param-type56
    ,INPUT-OUTPUT table-handle v-tth56
    ) no-error .
delete object v-tth56 no-error.
IF error-status:error then do:
  assign
  dflt-cd = ''.
end.
...
DEFINE var dpcnt  as decimal no-undo.
...
run get-gds-obj-fields in this-procedure(                                                  buffer ub.gds-obj                                                 ,input yes                                                 ,input ub.goods.gds-code                                                 ,input i-obj-code                                                 ,input 'маг':U                                                 ,output for-fact-qnty                                                 ,output cashparts                                                 ,output v-is-null-price                                                 ,output v-is-menu                                                 ,output v-is-semi-finished                                                 ,output v-is-modificator                                                 ,output v-fbr-grp-code                                                 ,output v-fbr-obj-code                                                 ) no-error .       if error-status:error then do:         run write-log-and-file in p-log-handle (                 input 1                                                                              , input log-file-name                                                                  , input 1                                                                              , input substitute("!!!Ошибка при получении характеристик товара &1 &2&3 на объекте"                                      , ub.gds-obj.artic                                                               , ub.gds-obj.prod-type                                                           , ub.gds-obj.prod-code                                                           )                                                                                         ).                                                            assign                                                                                  v-view-log = yes                                                                        .      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output main-b-code
      ...
      find first buf_producer no-lock where
                 buf_producer.obj-type = ub.goods.prod-type
             AND buf_producer.obj-code = ub.goods.prod-code no-error .
      assign
      for-producer = (if available buf_producer
                      then buf_producer.obj-name
                      else (ub.goods.prod-type + string(ub.goods.prod-code)))
      for-producer-int = (if ub.goods.prod-type = 'орг':U then 1000000 else 0 ) + ub.goods.prod-code
      .
          ...
          FOR EACH tt-tax No-LOCK:
            ...
            if tt-tax.individual and tt-tax.rate-value <> 0 then do:
              FIND FIRST cash-txr where
                    ...
                    and cash-txr.rc = recid(ub.goods) no-error.
...
if v-is-restaurant and v-is-null-price then.
    ...
    def var l-in-ov61 as logical no-undo .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjat in g#library
  ...
  if (ub.shop.in-ov and  l-in-ov61 ) then
              return.
...
if (incode = "" and LOOKUP('сер':U, ub.units.type) = 0) OR
   (NOT ub.shop.cd-parts-ser and LOOKUP('сер':U, ub.units.type) > 0 ) then do:
...
if  l-empty-scale
    AND
    incode <> "" then do:
 if (ub.shop.cd-parts-all AND LOOKUP('сер':U, ub.units.type) = 0)
    OR
    (ub.shop.cd-parts-not-blank and partcode <> "" AND LOOKUP('сер':U, ub.units.type) = 0)
    OR
    (ub.shop.cd-parts-ser and LOOKUP('сер':U, ub.units.type) > 0 ) then do:
        if ((ub.shop.cd-pb-base AND unitcli = ub.goods.unit-base) OR
...
define variable vss-include-info66 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str66  as character no-undo.
  define variable tmp-num66  as character no-undo.
  define variable i66        as integer   no-undo.
  define variable sum66      as integer   no-undo.
  define variable len-code66 as integer   no-undo.
  define variable varcont66  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str66 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str66 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont66 = yes then do:
    if integer( substring( tmp-str66, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str66, length( ncrsc-pfx ) + 1, length( tmp-str66 ) - length( ncrsc-pfx ) )
        len-code66    = length( IBm-good-code )
      .
      define variable v-sum-char66 as character no-undo .
      assign
        sum66 = 0
      .
      do i66 = 1 to len-code66 by 2
      :
        assign
          v-sum-char66 = substr(IBm-good-code, len-code66 - i66 + 1, 1)
        .
        if v-sum-char66 < "0"
        or v-sum-char66 > "9"
        then do:
        ...
        assign
          sum66 = sum66 + integer(v-sum-char66)
        .
      end.
      if varcont66 = yes then do:
        assign
        ...
        if varcont66 = yes then do:
           if sum66 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum66 mod 10)
             .
           end.
        end.
      end.
...
define variable vss-include-info67 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str67  as character no-undo.
  define variable tmp-num67  as character no-undo.
  define variable i67        as integer   no-undo.
  define variable sum67      as integer   no-undo.
  define variable len-code67 as integer   no-undo.
  define variable varcont67  as logical   initial yes no-undo.
  CASE ncrpg-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str67 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str67 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont67 = yes then do:
    if integer( substring( tmp-str67, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str67, length( ncrpg-pfx ) + 1, length( tmp-str67 ) - length( ncrpg-pfx ) )
        len-code67    = length( IBm-good-code )
      .
      define variable v-sum-char67 as character no-undo .
      assign
        sum67 = 0
      .
      do i67 = 1 to len-code67 by 2
      :
        assign
          v-sum-char67 = substr(IBm-good-code, len-code67 - i67 + 1, 1)
        .
        if v-sum-char67 < "0"
        or v-sum-char67 > "9"
        then do:
        ...
        assign
          sum67 = sum67 + integer(v-sum-char67)
        .
      end.
      if varcont67 = yes then do:
        assign
        ...
        if varcont67 = yes then do:
           if sum67 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum67 mod 10)
             .
           end.
        end.
      end.
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action69   as character no-undo .
  define variable v-printed69       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action69
    ,output v-printed69
    ) .
end.
if v-view-log = true
...
define variable vss-include-info70 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
AND IBM-good-code <> "":U
then do:
   run bgelib-tag-open in this-procedure ( input 2, input "ItemBarCode", input substitute("ctrl='&1' tms='&2' code='&3'"                                           , (if vaction = "U"                                              then if avail buf_cash-gds                                                   then if buf_cash-gds.bc-on eq yes then "ADD":U  else "DEL":U                                                    else if     cash-gds.bc-on eq yes then "ADD":U  else "DEL":U                                               else "DEL":U)                                             , OS2-time                                                   , trim(IBM-good-code, chr(32) ))).                                                  run bgelib-tag-put in this-procedure ( input 3, input "IBCCode"                                                                                    , input cash-gds.main-prt-b-code                                                                                      , input 1 ).                                                           if vaction = 'U':U then do:                                                                                     run bgelib-tag-put in this-procedure ( input 3, input "IBCProducer"                                                                                , input string(cash-gds.producer-int), input 1 ).                         run bgelib-tag-put in this-procedure ( input 3, input "IBCIngredient"                                                                              , input trim(string( cash-gds.ingredient, "X(40)")), input 1 ).           if cash-gds.gtd <> "":U then                                                                                   run bgelib-tag-put in this-procedure ( input 3, input "IBCGTD"                                                                                     , input trim(string( cash-gds.gtd, "X(40)")), input 1 ).                  find first ub.country no-lock where ub.country.alpha1 = cash-gds.alpha1 no-error.                                    if available ub.country then                                                                                      run bgelib-tag-put in this-procedure ( input 3, input "IBCCountry"                                                                                 , input ub.country.short-name, input 1 ).                                    run bgelib-tag-put in this-procedure ( input 3, input "IBCPrice"                                                                                   , input string( cash-gds.price-sale ), input 1 ).                         run bgelib-tag-put in this-procedure ( input 3, input "IBCType"                                                                                   , input string( v-IBCType ), input 1 ).                                   run bgelib-tag-put in this-procedure ( input 3, input "IBC_EMRC"                                                                                  , input  v-attr-emrc, input 1 ).                                          run bgelib-tag-put in this-procedure ( input 3, input "IBCUnitFactor"                                                                             , input  string(cash-gds.cli-base-rate), input 1 ).                     end.                                                                                                          run bgelib-tag-close in this-procedure ( input 2, input "ItemBarCode").
end.
  end.
    end.
    if pos-type = 'IBM':U
...
then do:
  ...
  then do:
  define variable v-plu-pet76 as character no-undo .
  define buffer  bufpet_cd-plu76 for ub.cd-plu.
  for each bufpet_cd-plu76  where
          bufpet_cd-plu76.obj-type = 'маг':U
      and bufpet_cd-plu76.obj-code = abs(i-obj-code)
      and bufpet_cd-plu76.pos-type = 'MARIA':U
      and bufpet_cd-plu76.plu-type = 'топ':U
      AND bufpet_cd-plu76.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu76.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet76 = TRIM(string( bufpet_cd-plu76.plu-code, "X(40)":U ))
    v-plu-pet76 = substring(v-plu-pet76, length(v-plu-pet76) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code76 as character no-undo .
    assign
      maria-good-code76 = left-trim(ibm-good-code, '0')
      maria-good-code76 = fill('0', 14 - length(maria-good-code76)) + maria-good-code76
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code76, 1, 5) + chr(4) +
                                                    substring(maria-good-code76, 6, 14)
                                                  )
...
define variable vss-include-info77 as character format "x(65)" no-undo initial "@(#)$Workfile$".
...
define variable v-found97 as logical no-undo .
define variable v-is-script97 as logical no-undo.
define variable v-fields-shift97 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet100 as character no-undo .
  define buffer  bufpet_cd-plu100 for ub.cd-plu.
  for each bufpet_cd-plu100  where
          bufpet_cd-plu100.obj-type = 'маг':U
      and bufpet_cd-plu100.obj-code = abs(i-obj-code)
      and bufpet_cd-plu100.pos-type = 'MARIA':U
      and bufpet_cd-plu100.plu-type = 'топ':U
      AND bufpet_cd-plu100.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu100.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet100 = TRIM(string( bufpet_cd-plu100.plu-code, "X(40)":U ))
    v-plu-pet100 = substring(v-plu-pet100, length(v-plu-pet100) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code100 as character no-undo .
    assign
      maria-good-code100 = left-trim(ibm-good-code, '0')
      maria-good-code100 = fill('0', 14 - length(maria-good-code100)) + maria-good-code100
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code100, 1, 5) + chr(4) +
                                                    substring(maria-good-code100, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
then do:
  ...
  then do:
  define variable v-plu-pet101 as character no-undo .
  define buffer  bufpet_cd-plu101 for ub.cd-plu.
  for each bufpet_cd-plu101  where
          bufpet_cd-plu101.obj-type = 'маг':U
      and bufpet_cd-plu101.obj-code = abs(i-obj-code)
      and bufpet_cd-plu101.pos-type = 'MARIA':U
      and bufpet_cd-plu101.plu-type = 'топ':U
      AND bufpet_cd-plu101.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu101.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet101 = TRIM(string( bufpet_cd-plu101.plu-code, "X(40)":U ))
    v-plu-pet101 = substring(v-plu-pet101, length(v-plu-pet101) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code101 as character no-undo .
    assign
      maria-good-code101 = left-trim(ibm-good-code, '0')
      maria-good-code101 = fill('0', 14 - length(maria-good-code101)) + maria-good-code101
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code101, 1, 5) + chr(4) +
                                                    substring(maria-good-code101, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable v-found102 as logical no-undo .
define variable v-is-script102 as logical no-undo.
define variable v-fields-shift102 as integer no-undo .
   if can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 6 )
   or can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 13 )
   then do:
     v-fields-shift102 = - 1.
...
define variable dflt-cd104 as character no-undo .
...
define variable v-value-logical105 AS LOGICAL no-undo .
...
run adm/shattri.p (
    ...
    ,output v-value-logical105
    ,output v-param-type105
    ,INPUT-OUTPUT table-handle v-tth105
    ) no-error .
delete object v-tth105 no-error.
...
define variable vss-include-info123 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str123  as character no-undo.
  define variable tmp-num123  as character no-undo.
  define variable i123        as integer   no-undo.
  define variable sum123      as integer   no-undo.
  define variable len-code123 as integer   no-undo.
  define variable varcont123  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str123 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str123 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont123 = yes then do:
    if integer( substring( tmp-str123, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str123, length( ncrsc-pfx ) + 1, length( tmp-str123 ) - length( ncrsc-pfx ) )
        len-code123    = length( v-tmpchar )
      .
      define variable v-sum-char123 as character no-undo .
      assign
        sum123 = 0
      .
      do i123 = 1 to len-code123 by 2
      :
        assign
          v-sum-char123 = substr(v-tmpchar, len-code123 - i123 + 1, 1)
        .
        if v-sum-char123 < "0"
        or v-sum-char123 > "9"
        then do:
        ...
        assign
          sum123 = sum123 + integer(v-sum-char123)
        .
      end.
      if varcont123 = yes then do:
        assign
        ...
        if varcont123 = yes then do:
           if sum123 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum123 mod 10)
             .
           end.
        end.
      end.
