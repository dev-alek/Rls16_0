...
DO:
if vardoc-status_ = 'накл':U then do:
  if tt-par-dtl.q-ty-doc:read-only  in BROWSE BR-dtl = no then do:
    ASSIGN
  ...
  if tt-par-dtl.q-ty-fact:read-only  in BROWSE BR-dtl = no then do:
    ASSIGN
...
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH buf_wth-line SHARE-LOCK.
...
if not can-find(first tt-par-dtl) then do:
    ...
    ub.wth-dtl.par-code :
      ...
      tt-par-dtl.q-ty-fact = tt-par-dtl.fact-sum / tt-par-dtl.par-rate
      d_doc-sum    = d_doc-sum   + tt-par-dtl.doc-sum
      d_fact-sum   = d_fact-sum   + tt-par-dtl.fact-sum
      .
    END.
...
END PROCEDURE.
PROCEDURE MyEnable :
tt-par-dtl.q-ty-doc:label-BGCOLOR IN BROWSE BR-dtl = 15.
tt-par-dtl.q-ty-fact:label-BGCOLOR IN BROWSE BR-dtl = 15.
IF par-mode = 'ПРОСМОТР':U THEN DO:
  ASSIGN
  tt-par-dtl.q-ty-doc:READ-ONLY IN BROWSE BR-dtl = YES
  tt-par-dtl.q-ty-fact:READ-ONLY IN BROWSE BR-dtl = YES
  tt-par-dtl.q-ty-doc:label-BGCOLOR IN BROWSE BR-dtl = ?
  tt-par-dtl.q-ty-fact:label-BGCOLOR IN BROWSE BR-dtl = ?
    .
...
END.
  ...
  IF par-mode <> 'ПРОСМОТР':U THEN DO:
    ...
    WITH FRAME Dialog-Frame.
  END.
 if buf_wth-doc.doc-type = 'декл':U then do:
    HIDE
    wth-line.fact-sum
    for-d_fact-sum
  ...
  if buf_wealth.is-ser = 1 then do:
      ...
      tt-par-dtl.q-ty-doc:label-BGCOLOR IN BROWSE BR-dtl = ?.
      tt-par-dtl.q-ty-fact:label-BGCOLOR IN BROWSE BR-dtl = ?.
      enable b-parts with frame Dialog-Frame.
  end.
  else do:
      if buf_wth-doc.doc-type = 'при':U and buf_wth-doc.exter_ = no and par-mode <> 'ПРОСМОТР':U then do:
             tt-par-dtl.q-ty-doc:READ-ONLY IN BROWSE BR-dtl = YES.
             tt-par-dtl.q-ty-doc:label-BGCOLOR IN BROWSE BR-dtl = ?.
      end.
      else  if buf_wth-doc.status_ =  'накл':U then dO:
            tt-par-dtl.q-ty-fact:READ-ONLY IN BROWSE BR-dtl = YES.
            tt-par-dtl.q-ty-fact:label-BGCOLOR IN BROWSE BR-dtl = ?.
            HIDE
            wth-line.fact-sum
