...
def var vss-workfile    as character no-undo init "$Workfile: gdslbxls.p $":U .
def var vss-archive     as character no-undo init "$Archive: str/gdslbxls.p $":U .
...
define variable v-last-inv-date-num as character no-undo.
  ...
  :
    CASE p-f-name:
      ...
      when "qnty":U then do:
        return string(bb-list.qnty,  p-format).
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output main-b-code
      ...
      when "last-inv" then do :
        find last buf_doc-line no-lock where
                  buf_doc-line.artic = bb-list.artic
              and buf_doc-line.prod-code = bb-list.prod-code
              and buf_doc-line.prod-type = bb-list.prod-type
...
FOR EACH bb-list no-lock,
    first ub.goods no-lock where
          ub.goods.gds-code = bb-list.gds-code:
  find first ub.bar-code no-lock  where ub.bar-code.b-code = bb-list.b-code no-error.
  if not available ub.bar-code then do:
    assign
    v-bc-ne = v-bc-ne + 1.
    release ub.prod-bc.
  end.
  if not bb-list.loc-ean and available ub.bar-code then
  find first ub.prod-bc no-lock where ub.prod-bc.b-str = bb-list.b-str and ub.prod-bc.b-code = ub.bar-code.b-code no-error.
    ...
    CASE v-dop1 :
      ...
      when 'prod-bc':U then do:
          if available prod-bc then  do:
            ...
            if bb-list.loc-ean or bb-list.b-str <> "":U then do:
              CASE v-dop2:
                when "b-str" then do:
                  f-value-1 = chr(4) + bb-list.b-str.
                end.
                when "b-on" then do:
                  f-value-1 = "yes":U.
                end.
...
for each bb-list-hist:
  if Make-Excel then  put   stream ForExcel unformatted
  (if bb-list-hist.line = 0
   then string(bb-list-hist.id, ">>>>>>>>9")
   else '':U)
  CHR(9)
  (if bb-list-hist.item_ <> '':U
   then bb-list-hist.hist-mode
   else '':U)  CHR(9)
   (if bb-list-hist.item_ <> '':U
   then string(bb-list-hist.num-add, "->>>>>>>>9")
   else '':U)  CHR(9)
  (if bb-list-hist.item_ <> '':U
  then string(bb-list-hist.num-recs, ">>>>>>>>9")
  else '':U)  CHR(9)
  bb-list-hist.des
  skip.
end.
