...
do transaction on error undo tr, return error return-value :
  ...
  if not available bf_pl-pump then do:
     return error substitute("Не найден запись резервуар-ТРК с номером резервуара &1 и номером ТРК &2", parpl-code, parpump-code) + SUBSTITUTE(" на объекте &1 &2 .", parobj-type, parobj-code).
  end.
...
PROCEDURE adm-display-fields :
      IF AVAILABLE ub.pl-pump THEN
          DISPLAY ub.pl-pump.pl-code ub.pl-pump.pump-code WITH BROWSE br_table
            NO-ERROR.
...
message
  ...
  "Неверное имя файла d-plpump.w." skip
...
message
  vss-workfile vss-revision vss-description skip
  "Ошибка при вызове файла d-plpump.w." skip
  "-----------Cистемная ошибка------------" skip
  return-value skip
  "------Ошибка исполнения программы------" skip
  trim(error-status :get-message(1)) +
...
message
  vss-workfile vss-revision vss-description skip
  "Ошибка при удалении записи резервуар-ТРК." skip
  "-----------Cистемная ошибка------------" skip
  return-value skip
  "------Ошибка исполнения программы------" skip
  trim(error-status :get-message(1)) +
...
DO:
  define buffer bf_pl-pump for ub.pl-pump.
  if available ub.pl-pump then do:
    if ub.pl-pump.ps <> input frame F-Main varps then do:
      if varps-upd = no then do:
        ...
        .
        message
          "Вы хотите изменить примечание к " "резервуар-ТРК"
          view-as alert-box question buttons yes-no update varmes-log.
        if varmes-log = true then do:
          ...
          :
            ...
            find first bf_pl-pump exclusive-lock
              where recid(bf_pl-pump) = recid(ub.pl-pump)
            .
            assign
              frame F-Main varps
            .
...
PROCEDURE adm-open-query-cases :
  OPEN QUERY br_table FOR EACH ub.pl-pump WHERE ub.pl-pump.obj-type = varobj-type and                                                ub.pl-pump.obj-code = varobj-code NO-LOCK      INDEXED-REPOSITION.
END PROCEDURE.
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "ub.pl-pump":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE ub.pl-pump THEN STRING(ROWID(ub.pl-pump))
        ELSE "?":U.
        OTHERWISE
        DO:
