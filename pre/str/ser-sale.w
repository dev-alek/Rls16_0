...
END.
ON RETURN OF SerNum IN FRAME Dialog-Frame
DO:
...
run waitfram-show in this-procedure ( 'Подождите ...').
  ...
  FOR EACH buf_parts NO-LOCK WHERE
          ...
          buf_parts.out-code  <> 'out-zone':U and
          buf_parts.part-code = Sernum AND
          buf_parts.status_   = yes AND
          buf_parts.doc-type  <> 'акт':U AND
          buf_parts.doc-type  <> 'при':U
  :
...
gds-name =  (buf_goods.artic + chr(32) + buf_goods.prod-type + string(buf_goods.prod-code) +
              chr(32) + buf_goods.gds-name).
DISPLAY
gds-name
WITH FRAME Dialog-Frame.
END PROCEDURE.
