...
assign
  v-str-recid13 = trim( string( recid( X_c-wth-parts ) , "->>>>>>>>>>>9":U ) )
...
DO:
  ...
  input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
  run fltfield-add in this-procedure('obj-type*obj-code', 'Объект', 'obj',
  input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
  ...
  input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
  run fltfield-add in this-procedure('supp-type*supp-code', 'Поставщик', 'supp',
  input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
  run fltfield-add in this-procedure('sale-obj-type*sale-obj-code', 'Объект реализации', 'obj',
  input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
  run fltfield-add in this-procedure('out-obj-type*out-obj-code', 'Объект погашения', 'obj',
  input-output fld, input-output lab, input-output spr, input-output dim)  no-error.
...
DO:
  IF AVAILABLE X_c-wth-parts THEN DO WITH FRAME Dialog-Frame:
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-parts FOR EACH X_c-wth-parts NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
...
IF p-coll-point = 'document' THEN DO:
    FIND FIRST b-c-wth-doc WHERE b-c-wth-doc.doc-code = p-c-wth-doc NO-LOCK NO-ERROR.
END.
ENABLE b-exit when not p-edit-mode = 'ПРОСМОТР':U
    b-quit
    b-lkp
    br-parts
    b-sch
...
IF AVAILABLE b-c-wth-doc THEN do:
    ...
    cur-wth-ext-doc-type-text = ENTRY (lookup(b-c-wth-doc.ext-doc-type, 'ie,ee,ii,ei,ij,ej,fj,jj,pj,oj,we,ci,ce,iy,rj,ip,ep,rp,ff,ef,rf,pc,ps,pz,df,dp,dc,de,xc':u), 'приход внешний,расход внешний,приход внутренний,расход внутренний,приход внутри объекта,расход внутри объекта,приход внутриобъектн. в своб. зону,расход внутриобъектн. из своб. зоны,приход внутриобъектн. в зону погаш.,расход внутриобъектн. из зоны погаш.,списание,приход внешний через кассы,возврат покупателю через кассы,инвентаризация,возврат внутренний,приход внутр. в зону погашения,расход внутр. из зоны погашения,возврат внутр. в зону погашения,приход внутр. в своб. зону,расход внутр. из своб. зоны,возврат внутр. в своб зону,погашение через кассу,погашение за реализованное топливо,возврат от покупателя,уничтожение в свободной зоне,уничтожение в зоне погашения,уничтожение в зоне клиента,декларация,обмен':u).
...
CASE p-coll-point:
    when 'document' then do:
      ASSIGN frame Dialog-Frame:TITLE = substitute("Партии номинала &1 &2 МЦ &3 Документ: &4 № &5 &6 " ,cur-par-val, cur-par-unit, cur-wth-name, cur-wth-ext-doc-type-text ,cur-c-wth-doc-code, if p-edit-mode = 'ПРОСМОТР':U then 'ПРОСМОТР':U else "":U).
...
run gbl/flt-get.p
  ...
  then do:
    ...
    define variable  parameter-7-24 as character no-undo .
      assign
      parameter-3-24 =
                              "FOR EACH X_c-wth-parts"
      parameter-4-24 =
        (
          if (" X_c-wth-parts.wth-code = p-wth-code and X_c-wth-parts.w-p-code = p-w-p-code             and X_c-wth-parts.par-code = p-par-code and X_c-wth-parts.out-code = p-c-wth-doc " + " " + where-phrase-24) <> ""
          then  substitute('
              X_c-wth-parts.wth-code = &2     and               X_c-wth-parts.w-p-code = &3     and               X_c-wth-parts.par-code = &4     and               X_c-wth-parts.out-code = &1&5&1                   '               , chr(34)
              , p-wth-code                , p-w-p-code                , p-par-code                , p-c-wth-doc               )                + " " + where-phrase-24
          else "true"
        )
      parameter-5-24 = (" " + "" + " " + "")
      parameter-6-24 = if sort-phrase-24 = ''
        ...
        " " + "  " +
          " " + sort-column-phrase +
        " " + " by X_c-wth-parts.fact-rangefrom "
        )
                           else
        (
        " " + "  " +
    ...
    :
      assign
        l-disable-where-24 =
          (" X_c-wth-parts.wth-code = p-wth-code and X_c-wth-parts.w-p-code = p-w-p-code             and X_c-wth-parts.par-code = p-par-code and X_c-wth-parts.out-code = p-c-wth-doc " + " " + where-phrase-24 = "")
      .
  ...
  if l-filter-open-24 = false then do:
    OPEN QUERY br-parts FOR EACH X_c-wth-parts
      where  X_c-wth-parts.wth-code = p-wth-code and X_c-wth-parts.w-p-code = p-w-p-code             and X_c-wth-parts.par-code = p-par-code and X_c-wth-parts.out-code = p-c-wth-doc
       by X_c-wth-parts.fact-rangefrom
  .
      assign
        l-query-was-opened = true
