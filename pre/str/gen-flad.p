...
if p-trn-doc <> ?  then do:
    for each temp-obj-firm on error undo, return error :
           for each buf_add-doc no-lock where
                    ...
                    buf_add-doc.status_    = 'факт':U                and
            ...
            on error undo, return error :
                  ...
                  for each tt-fin-ob where
                      ...
                      first buf_contract no-lock where buf_contract.contract-code = tt-fin-ob.contract-code and
                                                       ...
                                                       lookup( buf_contract.usl-opl ,  'По факту поставки':U + "," + 'Отсрочка платежа (по поставке)':U )   > 0
                            break
...
p-res = p-res  + chr(10)   +
       ...
       "Просмотрено допРасходов          :" + string(col-trn)           + chr(10) +
...
if v-cntxt-db-num <> 0 then do:
   p-res = p-res + chr(10) + substitute("По ДопРасходу &1 нельзя создавать ФО в этой БД" , buf_add-doc.doc-code ) .
   return .
end.
assign
...
.
 if  buf_contract.usl-opl = 'По заказу':U or buf_contract.usl-opl = 'По поставке заказа':U
   then v-date-pay = today  .
        ...
        if p-cons = 2 and available   buf_add-doc then do:
            var-date = buf_add-doc.fact-date + (if buf_contract.srok-opl <> ? then buf_contract.srok-opl else 0 ) .
...
.
 ...
 if p-cons = 2 then do:
      if available   buf_add-doc
         then
         assign
           n-trn-doc-code       =  buf_add-doc.doc-code
         .
         else  n-trn-doc-code       = ?   .
     end.
     else do:
       if pfo = true and available   buf_add-doc
         then  assign
           n-trn-doc-code       =  buf_add-doc.doc-code
         .
         else  n-trn-doc-code      = ?   .
