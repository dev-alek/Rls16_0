...
DO:
  run show-detail in this-procedure .
END.
ON CHOOSE OF btn-select-doc IN FRAME Dialog-Frame
DO:
  run select-doc in this-procedure .
END.
ON CHOOSE OF btn-select-obj IN FRAME Dialog-Frame
DO:
  run select-obj in this-procedure .
END.
ON CHOOSE OF btn-select-pck-num IN FRAME Dialog-Frame
DO:
  run select-pck-num in this-procedure .
END.
ON VALUE-CHANGED OF toggle-date IN FRAME Dialog-Frame
DO:
  if toggle-date :checked
  then do:
    define variable v-date as date      no-undo .
    define variable v-time as integer   no-undo .
    run cur-time in this-procedure
    ...
    .
    if input frame Dialog-Frame fi-date-from = ?
    then do:
      display
        v-date @ fi-date-from
        with frame Dialog-Frame .
    end.
    if input frame Dialog-Frame fi-date-to = ?
    then do:
      display
        v-date @ fi-date-to
        with frame Dialog-Frame .
...
END.
ON VALUE-CHANGED OF toggle-doc-num IN FRAME Dialog-Frame
DO:
  if toggle-doc-num :checked
  then do:
    assign
...
END.
ON VALUE-CHANGED OF toggle-object-list IN FRAME Dialog-Frame
DO:
  if toggle-object-list :checked
  then do:
    assign
...
END.
ON VALUE-CHANGED OF toggle-pck-num IN FRAME Dialog-Frame
DO:
  if toggle-pck-num :checked
  then do:
    assign
...
do:
  ...
  then do:
    ASSIGN
      fi-date-from :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date5 :HANDLE
      fi-date-from :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle5 as handle no-undo .
  assign
    v-label-handle5 = fi-date-from :side-label-handle in frame Dialog-Frame
  .
...
do:
  ...
  then do:
    ASSIGN
      fi-date-to :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date7 :HANDLE
      fi-date-to :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle7 as handle no-undo .
  assign
    v-label-handle7 = fi-date-to :side-label-handle in frame Dialog-Frame
  .
...
do with frame Dialog-Frame:
  assign
    cb-obj-type :list-items = 'маг':U + ',' + 'скл':U
  .
  ...
  assign
    v-doc-type-orig-list  = 'trn-doc':U
                    + ',' + 'price-doc':U
                    + ',' + 'wth-doc':U
                    + ',' + 'inkas':U
                    + ',' + 'fbr-doc':U
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run tblnmusr in g#library
  (input  entry(v-ind,v-doc-type-orig-list)
  ,output v-table-name
  )  .
    assign
...
PROCEDURE open-query-nws-doc-hist :
  ...
  :
    ...
    open query BROWSE-1 for each ub.nws-doc-hist no-lock
...
PROCEDURE select-doc :
  ...
  assign
    v-select-doc-type = input frame Dialog-Frame cb-doc-type
    v-select-doc-code = input frame Dialog-Frame fi-doc-code
  .
  run str/nwhdocsl.w
...
PROCEDURE select-obj :
  ...
  assign
    v-select-obj-type = input frame Dialog-Frame cb-obj-type
    v-select-obj-code = input frame Dialog-Frame fi-obj-code
  .
  run str/nwhobjsl.w
...
PROCEDURE select-pck-num :
  ...
  assign
    v-select-db-num  = input frame Dialog-Frame fi-pck-db-num
    v-select-pck-num = input frame Dialog-Frame fi-pck-pack-num
  .
  run str/nwhpcksl.w
...
PROCEDURE show-detail :
  ...
  then do:
    run str/showtbl.p
      (input parparentproc
      ,input ub.nws-doc-hist.doc-type
      ,input ub.nws-doc-hist.doc-code
      ,input 0
      ) .
  end.
END PROCEDURE.
PROCEDURE validate-filter-options :
  do with frame Dialog-Frame:
    assign
      v-filter-date    = input frame Dialog-Frame toggle-date
      v-filter-obj     = input frame Dialog-Frame toggle-object-list
      v-filter-doc     = input frame Dialog-Frame toggle-doc-num
      v-filter-pck-num = input frame Dialog-Frame toggle-pck-num
    .
    if v-filter-date = true
    then do:
      assign
        v-date-from = input frame Dialog-Frame fi-date-from
        v-date-to   = input frame Dialog-Frame fi-date-to
      .
      if v-date-from = ?
      then do:
        message
    ...
    then do:
      assign
        v-obj-type = input frame Dialog-Frame cb-obj-type
        v-obj-code = input frame Dialog-Frame fi-obj-code
      .
      if v-obj-type = ?
      or v-obj-type = ""
      then do:
    ...
    then do:
      assign
        v-doc-code = input frame Dialog-Frame fi-doc-code
      .
define variable vss-include-info9 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run tblusrnm in g#library
  (input  input frame Dialog-Frame cb-doc-type
  ,output v-doc-type
  )  .
      if v-doc-type = ?
      or v-doc-type = ""
    ...
    then do:
      assign
        v-pck-db-num   = input frame Dialog-Frame fi-pck-db-num
        v-pck-pack-num = input frame Dialog-Frame fi-pck-pack-num
      .
      if v-pck-db-num = ?
      then do:
        message
