...
run write-log-and-file in p-log-handle (
        ...
        , input log-file-name
        , input 1
        , input substitute("Пересылка на кассы &1&2 стоплистов", 'маг':U, i-obj-code)
                                              ).
  ...
  when 'IBM-XML':U  then do:
    ...
    for each buf_stop-list-line no-lock where
        ...
        and buf_stop-list-line.stop-list-code =  p-full-stop-list-code
            :
        if buf_stop-list-line.key#_one = integer('4':U) then next.
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'добавление стоплиста')
                  else ('Ждите - ' + 'удаление стоплиста') ) +
