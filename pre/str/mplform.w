...
define input parameter r-m           as character no-undo.
define input parameter r-b           as decimal no-undo.
define input parameter c-m           as character no-undo.
...
.
define variable vss-include-info82 as character format "x(65)" no-undo initial "@(#)$Workfile$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output varinvprb
  )  .
   find first in-vatp_doc-attr no-lock
...
assign
  ...
  price-base-with-tax-loc = buf_doc-line.price-base
.
define variable vss-include-info90 as character format "x(65)" no-undo initial "@(#)$Workfile$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output varinvprb
  )  .
   find first in-vatp_doc-attr no-lock
...
procedure check-use-bar-code :
  ...
  do
  on error  undo, return error substitute( "&1. &2&3&4", vss-include-info91, return-value, chr(10), error-status :get-message ( error-status :num-messages ) )
  on stop   undo, return error substitute( "&1. stop", vss-include-info91 )
  on endkey undo, return error substitute( "&1. endkey", vss-include-info91 )
  :
    define buffer buf_bar-code for ub.bar-code .
    find first buf_bar-code no-lock
    ...
    if not available buf_bar-code then do:
      return error substitute( "&1 (check-use-bar-code). Не найден бар-код &2", vss-include-info91, p-b-code ) .
    end.
...
DO:
if line-mode = 'ПРОСМОТР':U then return .
...
DO:
 ...
 assign frame  Dialog-Frame
  ...
  assign
   ...
   .
END.
ON LEAVE OF buf_price-doc-forming-gds.excise-doc IN FRAME Dialog-Frame
DO:
    run upd-field in this-procedure no-error.
END.
ON LEAVE OF buf_price-doc-forming-gds.price-sale-doc IN FRAME Dialog-Frame
DO:
  run upd-field in this-procedure no-error.
  run calc-pr in this-procedure .
END.
ON LEAVE OF buf_price-doc-forming-gds.road-tax-doc IN FRAME Dialog-Frame
DO:
    run upd-field in this-procedure no-error.
END.
IF VALID-HANDLE(ACTIVE-WINDOW) AND FRAME Dialog-Frame:PARENT eq ?
THEN FRAME Dialog-Frame:PARENT = ACTIVE-WINDOW.
if line-mode = 'ПРОСМОТР':U then
...
v-tmp-price-doc = input frame Dialog-Frame buf_price-doc-forming-gds.price-sale-doc .
...
if available ub.gds-obj then do:
  ...
  display  pc-avrg pc-last new-avrg new-last old-avrg old-last op-avrg op-last with frame Dialog-Frame no-error .
end.
if v-rb-base = true  then do:
...
display  new-old pc-prev   with frame Dialog-Frame no-error .
END PROCEDURE.
...
c-m   = 'Отсутствует':U .
v-dis =  p-disc.
  frame Dialog-Frame:title = frame Dialog-Frame:title + " -  " + line-mode + " объект " + v-obj-type + string (v-obj-code).
  run tax-name in this-procedure ( input 'rdt':U, output  dor-nal) .
  assign buf_price-doc-forming-gds.road-tax-doc :label in frame Dialog-Frame = dor-nal .
      If available  buf_goods Then DO:
          v-rec = recid (buf_goods).
           t-ret =  session:SET-WAIT-STATE("GENERAL") .
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_is-petrl in g#lib-trn
  (
     input buf_price-doc-forming-gds.artic
  ,  input buf_price-doc-forming-gds.prod-type
  ,  input buf_price-doc-forming-gds.prod-code
  , output is-petrolium
  , output is-pieces
  ) .
           t-ret =  session:SET-WAIT-STATE("") .
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,output ff
                    ...
                    if ff then do:
                           enable buf_price-doc-forming-gds.road-tax-doc with frame Dialog-Frame .
                    end.
              end.
      end.
...
run adm/shattri.p (
  ...
  new_par = new_par + trim(par-pr-discm) + chr(4) .
end.
define variable v-i as integer   no-undo init 0.
  for each  x_obj-group :
      v-i = v-i + 1.
      if entry( v-i, new_par , chr(4) ) <> trim ( par-pr-discm ) then do:
...
define variable calc-method    as character no-undo.
...
define variable round-base     as decimal no-undo.
    assign
       ...
       round-base     = r-b
       .
  if decimal ( buf_price-doc-forming-gds.price-sale-doc :screen-value in frame Dialog-Frame) <> buf_price-doc-forming-gds.price-sale-doc then do:
    assign
      buf_price-doc-forming-gds.calc-method     = c-m
      buf_price-doc-forming-gds.price-calc-doc  = buf_price-doc-forming-gds.price-sale-doc
      buf_price-doc-forming-gds.price-sale-doc  = decimal ( buf_price-doc-forming-gds.price-sale-doc :screen-value in frame Dialog-Frame)
      buf_price-doc-forming-gds.price-sale-rubl = buf_price-doc-forming-gds.price-sale-doc  * p-exch-rate / p-exch-scale
      buf_price-doc-forming-gds.price-sale-base = buf_price-doc-forming-gds.price-sale-rubl / p-base-rate * p-base-scale
      .
    assign
...
If decimal ( buf_price-doc-forming-gds.excise-doc :screen-value in frame Dialog-Frame ) <> buf_price-doc-forming-gds.excise-doc then do:
    assign
      buf_price-doc-forming-gds.excise-doc     = decimal ( buf_price-doc-forming-gds.excise-doc     :screen-value in frame Dialog-Frame)
      buf_price-doc-forming-gds.excise-rubl = buf_price-doc-forming-gds.excise-doc  * p-exch-rate / p-exch-scale
      buf_price-doc-forming-gds.excise-base = buf_price-doc-forming-gds.excise-rubl / p-base-rate * p-base-scale
    .
End.
If decimal ( buf_price-doc-forming-gds.road-tax-doc :screen-value in frame Dialog-Frame) <> buf_price-doc-forming-gds.road-tax-doc then do:
       Find first buf_goods no-lock where
            buf_goods.artic     = buf_price-doc-forming-gds.artic and
            buf_goods.prod-type = buf_price-doc-forming-gds.prod-type and
            buf_goods.prod-code = buf_price-doc-forming-gds.prod-code
...
do:
    ...
    ,output ff
                    ...
                    if not ff then      do :
                       buf_price-doc-forming-gds.road-tax-doc   = buf_price-doc-forming-gds.road-tax-doc.
                        ...
                        buf_price-doc-forming-gds.road-tax-doc = decimal ( buf_price-doc-forming-gds.road-tax-doc:screen-value in frame Dialog-Frame ) .
                    end.
                    buf_price-doc-forming-gds.road-tax-rubl = buf_price-doc-forming-gds.road-tax-doc  * p-exch-rate / p-exch-scale.
                    buf_price-doc-forming-gds.road-tax-base = buf_price-doc-forming-gds.road-tax-rubl / p-base-rate * p-base-scale.
              end.
 ...
 Display buf_price-doc-forming-gds.excise-doc buf_price-doc-forming-gds.price-sale-doc buf_price-doc-forming-gds.road-tax-doc  with frame Dialog-Frame .
end procedure.
