...
define input param inv-code like trn-doc.doc-code no-undo.
...
define variable fill-gds-list as log no-undo.
define variable doc-cnt as integer no-undo.
...
do on stop undo lst-calc, return error on error undo lst-calc, return error :
  ...
  for each trn-doc where trn-doc.inv-num = inv-code
    ...
    if trn-doc.status_ = 'запрос':U then next.
    if trn-doc.doc-type = 'инв':U and trn-doc.status_ = 'накл':U then next.
    trn-doc.inv-num = "".
  end.
 ...
 each parts no-lock
        where
            (     parts.prod-type = doc-line.prod-type
              ...
              and parts.rsrv-free = true
              and parts.status_   = false
              and parts.out-code  <> 'free-zone':U
              and parts.out-code  <> inv-code
            )
            or
            (     parts.prod-type = doc-line.prod-type
              and parts.prod-code = doc-line.prod-code
              ...
              and parts.obj-type  = doc-line.obj-type
              and parts.obj-code  = doc-line.obj-code
              and parts.rsrv-free = false
              and parts.status_   = false
              and parts.out-code  <> 'out-zone':U
              and parts.out-code  <> inv-code
            )
       on stop undo lst-calc, return error on error undo lst-calc, return error :
...
if available gds-list then do:
  ...
  define variable v-last3 as integer no-undo .
  find last gds-list use-index oi no-error.
  if available gds-list then do:
    v-last3 = gds-list.order-num .
  end.
  else do:
    v-last3 = 0 .
  end.
