  ...
  :
    ...
    IF NOT AVAIL ub.chk-doc AND NOT LOCKED ub.chk-doc  AND NOT AMBIGUOUS ub.chk-doc then do:
      ...
      if v-versiond >= 33.0 then do:
        assign
        ub.chk-doc.cash-rate = ( if (get-chkc_context.r-b = 'base':U and get-chkc_context.base-code = 0) or get-chkc_context.r-b = 'rubl':U then 1 else v-base-rate )
        ub.chk-doc.cash-scale = ( if (get-chkc_context.r-b = 'base':U and get-chkc_context.base-code = 0) or get-chkc_context.r-b = 'rubl':U then 1 else v-base-scale )
        .
      end.
      else do:
        assign
  ...
  :
    ...
    if v-versiond >= 33.0 then do:
      ...
      var-sub-d  = ((DEC(substr(p-ss, 20, 12)) - DEC(substr(p-ss, 51,11))) / 100)/
                  ( if (get-chkc_context.r-b = 'base':U and get-chkc_context.base-code = 0) or get-chkc_context.r-b = 'rubl':U then 1 else (ub.chk-doc.cash-rate / ub.chk-doc.cash-scale))
      no-error .
    end.
    else do:
      assign
  ...
  :
    ...
    if not v-versiond >= 33.0 then do:
      if m-sign = '-' and lng = 0 then do:
        assign
        ub.chk-doc.chk-type = integer('6':U).
      end.
    end.
    if unq-artc then  do:
