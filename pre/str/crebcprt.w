...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1':u,p-gds-code)
    .
end procedure.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BROWSE-level :handle
      .
...
do:
  browse-level :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
define variable vss-include-info5 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  browse-prt :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
MAIN-BLOCK:
...
do:
  ...
  run open-query-prt in this-procedure .
  RUN enable_UI.
  apply 'entry':u to browse BROWSE-level .
  apply 'entry':u to temp-prt.prt-value in browse BROWSE-level .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-level FOR EACH temp-prt .    run open-query-prt in this-procedure .
END PROCEDURE.
...
PROCEDURE open-query-level :
  ...
  open query BROWSE-level for each temp-prt by temp-prt.prt-level .
  if v-prt-level <> ?
  then do:
    ...
    then do:
      reposition BROWSE-level to rowid rowid(buf_temp-prt) no-error .
    end.
  end.
END PROCEDURE.
