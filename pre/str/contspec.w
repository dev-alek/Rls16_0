...
head-col =
  ...
  'Группа товара'    + '#' +
  'Ед.!изм!пост'    + '#' +
  'Ед.изм!пост в!заказе'    + '#' +
  'Коэф.!в!заказе'    + '#' +
  'Ед.изм!пост в!поставке'    + '#' +
  'Коэф.!в!поставке'    + '#' +
  '%!Бонус'    + '#' +
  'Внешний!Артикул'
  .
...
DEFINE BROWSE spec-List
  QUERY spec-List DISPLAY
     ...
     get-b-code(buf_contract-specif.gds-code) @ p-bcode  COLUMN-LABEL 'Код'  Format "X(16)"
     buf_contract-specif.artic    COLUMN-LABEL 'Артикул'  Format "x(16)"
     string (buf_contract-specif.prod-type + ' ' + string(buf_contract-specif.prod-code)) @ p-prod  COLUMN-LABEL 'Произво-!дитель'  Format "x(18)"
     buf_contract-specif.gds-name    COLUMN-LABEL 'Наименование'  format "x(112)"
     ...
     buf_contract-specif.unit-base    COLUMN-LABEL 'Ед.!изм'
     buf_contract-specif.qnty    COLUMN-LABEL 'Количество'
     buf_contract-specif.cli-base-rate    COLUMN-LABEL '*'
     buf_contract-specif.sum-cli   COLUMN-LABEL 'Сумма'
     ...
     get-grp(buf_contract-specif.gds-code) @ p-grp COLUMN-LABEL 'Группа товара'  Format "x(100)"
     buf_contract-specif.unit-cli   COLUMN-LABEL 'Ед.!изм!пост'
     buf_contract-specif.unit-cli-ord   COLUMN-LABEL 'Ед.изм!пост в!заказе'
     buf_contract-specif.cli-base-rate-ord   COLUMN-LABEL 'Коэф.!в!заказе'
     buf_contract-specif.unit-cli-rcv   COLUMN-LABEL 'Ед.изм!пост в!поставке'
     buf_contract-specif.cli-base-rate-rcv   COLUMN-LABEL 'Коэф.!в!поставке'
     f-bonus(recid(buf_contract-specif))   COLUMN-LABEL '%!Бонус' Format "->>9.99"
...
END.
ON CHOOSE OF b-sch1 IN FRAME Dialog-Frame
DO:
  assign
    tbl = 'contract-specif'
...
END.
ON CHOOSE OF MENU-ITEM m-export-excel
DO:
        ...
        if lookup("retro-bonus", v-change-fields) > 1 then do :
          find first ub.contract-specif-attr exclusive-lock  where
             ...
             if not available ub.contract-specif-attr then do:
               ...
               is-ok = true.
               do i = 1 to num-entries(ub.contract-specif-attr.attr-value, ';') - 1 :
...
END.
ON CHOOSE OF MENU-ITEM m-export-text
DO:
...
do:
  ...
  run openbr in this-procedure ( input yes, input no, input '':u).
END.
ON CHOOSE OF MENU-ITEM m-import-excel
DO:
  run proc-import-excel in this-procedure .
END.
ON CHOOSE OF MENU-ITEM m-import-text
DO:
...
END.
ASSIGN b-imp:POPUP-MENU IN FRAME Dialog-Frame = MENU m-import:HANDLE.
ASSIGN b-imp:MENU-MOUSE = 1.
ASSIGN b-exp:POPUP-MENU IN FRAME Dialog-Frame = MENU m-export:HANDLE.
...
define variable vss-include-info40 as character format "x(65)" no-undo initial "@(#)$Workfile$ Библиотека изменения размеров окна".
...
PROCEDURE sort-brspec-List :
  ...
  case sort-labelspec-List:
        when 'Код'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1get-b-code&1, &2)', chr(34), buf_contract-specif.gds-code)     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Артикул'  then DO:    assign       sort-column-name = "buf_contract-specif.artic"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Произво-!дитель'  then DO:    assign       sort-column-name = "string (buf_contract-specif.prod-type + ' ' + string(buf_contract-specif.prod-code))"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Наименование'  then DO:    assign       sort-column-name = "buf_contract-specif.gds-name"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Цена!поставщика'  then DO:    assign       sort-column-name = "buf_contract-specif.price-cli"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when '% Отклон в!большую сторону'  then DO:    assign       sort-column-name = "buf_contract-specif.prc"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when '% Отклон в!меньшую сторону'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1f-prc-min&1, recid(buf_contract-specif))', chr(34) )     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Ед.!изм'  then DO:    assign       sort-column-name = "buf_contract-specif.unit-base"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Количество'  then DO:    assign       sort-column-name = "buf_contract-specif.qnty"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Коэф.'  then DO:    assign       sort-column-name = "buf_contract-specif.cli-base-rate"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Сумма'  then DO:    assign       sort-column-name = "buf_contract-specif.sum-cli"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'НДС'  then DO:    assign       sort-column-name = "buf_contract-specif.vat-pc"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'тип!НДС'  then DO:    assign       sort-column-name = "buf_contract-specif.vat-type"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Принято'  then DO:    assign       sort-column-name = "buf_contract-specif.income-qnty"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Группа товара'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1get-grp&1, &2)', chr(34), buf_contract-specif.gds-code)     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when '*'  then DO:    assign       sort-column-name = "mark-string(recid(buf_contract-specif))"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Ед.изм!пост в!заказе'  then DO:    assign       sort-column-name = "buf_contract-specif.unit-cli-ord"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Коэф.!в!заказе'  then DO:    assign       sort-column-name = "buf_contract-specif.cli-base-rate-ord"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Ед.изм!пост в!поставке'  then DO:    assign       sort-column-name = "buf_contract-specif.unit-cli-rcv"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Коэф.!в!поставке'  then DO:    assign       sort-column-name = "buf_contract-specif.cli-base-rate-rcv"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when '%!Бонус'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1f-bonus&1, &2)', chr(34), recid(buf_contract-specif))     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when '*'  then DO:    assign       sort-column-name = "mark-string(recid(buf_contract-specif))"     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
        when 'Внешний!Артикул'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1get-ext-artic&1, recid(buf_contract-specif))', chr(34) )     .     run OpenBr in this-procedure ( input yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  assign frame Dialog-Frame:title =  substitute(" Фирма: (&1) &2 Товарная спецификация к договору &3 от &4"
                                                  ,p-host-code
                                                  ,ub.clients.obj-name
  ...
  .
  if v-contr-type = 'о Дополнительных расходах':U then assign frame Dialog-Frame:title =
...
DEF VAR cur-clmn-numspec-List as INT EXTENT 23 no-undo.
...
ON ctrl-cursor-right OF BROWSE spec-List do:
  RUN re-move-clmnspec-List ( 1, 23).
END.
ON ctrl-cursor-left OF BROWSE spec-List do:
  RUN re-move-clmnspec-List (23, 1).
END.
...
if v-spis-size     = ? or v-spis-size    = ""  or error-status :error  then  v-spis-size     = '1,16,16,10,30,10,17,17,4,10,5,10,5,5,10,20,6,6,6,6,6,6,6':U .
...
PROCEDURE MyEnable :
hide b-sch1 in frame Dialog-Frame .
DISPLAY
sch-str
RADIO-find
mark-num
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-46 as character no-undo .
      ...
      parameter-4-46 =
        (
          if (" buf_contract-specif.host-code = p-host-code and buf_contract-specif.contract-num = p-doc-num " + " " + where-phrase-46) <> ""
          then  substitute(' buf_contract-specif.host-code = &1 and buf_contract-specif.contract-num = &2 ', p-host-code, p-doc-num )  + " " + where-phrase-46
          else "true"
        )
      parameter-5-46 = (" " + "" + " " + substitute(' , first temp-trn-doc where ( &1 = 1 or buf_contract-specif.gds-code = temp-trn-doc.gds-code )',  r-2 ))
    ...
    :
      assign
        l-disable-where-46 =
          (" buf_contract-specif.host-code = p-host-code and buf_contract-specif.contract-num = p-doc-num " + " " + where-phrase-46 = "")
      .
  ...
  if l-filter-open-46 = false then do:
    OPEN QUERY spec-List FOR EACH buf_contract-specif NO-LOCK
      where  buf_contract-specif.host-code = p-host-code and buf_contract-specif.contract-num = p-doc-num
  ...
  :
    if query spec-List:handle:get-buffer-handle(1) = (buffer buf_contract-specif:handle) then do:
      ...
      parameter-4-46 =
        "where ":u +  substitute(' buf_contract-specif.host-code = &1 and buf_contract-specif.contract-num = &2 ', p-host-code, p-doc-num )  + " ":u + where-phrase-46 + " ":u + p-find-condition + " " + ""
      parameter-5-46 = "  "
    .
      ...
      assign
      parameter-2-46 = (if p-find-next then "true":u else "false":u )
      parameter-3-46 =  "FOR EACH buf_contract-specif"
      parameter-4-46 =
        (
          if (" buf_contract-specif.host-code = p-host-code and buf_contract-specif.contract-num = p-doc-num " + " " + where-phrase-46) <> ""
          then  substitute(' buf_contract-specif.host-code = &1 and buf_contract-specif.contract-num = &2 ', p-host-code, p-doc-num )  + " " + where-phrase-46
      ...
      parameter-5-46 = (" " + "" + " " + substitute(' , first temp-trn-doc where ( &1 = 1 or buf_contract-specif.gds-code = temp-trn-doc.gds-code )',  r-2 ) + " " + p-find-condition)
      parameter-6-46 = if sort-phrase-46 = ''
                           then
        (
        " " + "  " +
  ...
  END.
  APPLY
     "entry" to spec-List in frame Dialog-Frame.
END PROCEDURE.
PROCEDURE proc-add :
...
if ibs.th.gbl.gbl-var:g#db-num <> 0 then do :
   if can-find ( first ub.assortment-matrix where  ub.assortment-matrix.asmt-status = integer ('0':U) and
                                                   ub.assortment-matrix.db-num = ibs.th.gbl.gbl-var:g#db-num )  then v-ass-m = true  .
end.
else do:
   if can-find ( first ub.assortment-matrix where  ub.assortment-matrix.asmt-status = integer ('0':U))  then v-ass-m = true  .
end.
define variable v-log as logical   no-undo .
...
run str/diallog.w (
      input parParentProc
    , input this-procedure
    , input ("utl/thbjrumr.w":U + chr(4)
                                + chr(4)
              + string(2)       + chr(4)
            )
    , input ('edoc':U + chr(4) +
          'excel-export_specif':U + chr(4) +
...
define variable b-ext as character no-undo .
  DEFINE frame f-doc
    ...
    sym9 b-ext        COLUMN-LABEL 'Ед.!изм!пост' Format "X(12)"                          space(0)
    sym2 b-name       COLUMN-LABEL 'Наименование'  format "x(30)"                          space(0)
    ...
    sym5 buf_contract-specif.artic    COLUMN-LABEL 'Артикул'  Format "x(16)"                          space(0)
    sym6 b-prod       COLUMN-LABEL 'Произво-!дитель'  Format "x(12)"                          space(0)
    sym7 b-bonus      COLUMN-LABEL 'Группа товара'                                         space(0)
    sym8
  HEADER
  ...
  FOR EACH buf_contract-specif NO-LOCK where buf_contract-specif.host-code = p-host-code and buf_contract-specif.contract-num = p-doc-num :
    ...
    display stream PrnLibStream  sym1    b-code
                              ...
                              sym5    buf_contract-specif.artic
                              sym6    b-prod
                              sym7    b-bonus
                              sym8
    with frame f-doc.
