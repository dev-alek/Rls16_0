...
DO:
  if available ub.fin-doc then do:
    run str/finconn.w ( input parParentProc, input p-host-code, input 'при':U, input "fin-doc", input string(ub.fin-doc.fin-doc-code)) .
    br-doc:refresh() .
  end.
END.
...
DO:
  if available temp_fin-ob then do:
    run str/finconn.w ( input parParentProc, input p-host-code, input 'при':U, input "fin-ob", input temp_fin-ob.doc-code) .
    br-doc:refresh() .
  end.
END.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY Br-doc FOR EACH temp_fin-ob WHERE TRUE  NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE StartProc :
