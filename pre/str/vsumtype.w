...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input pardoc-code ,
  ...
  do:
    ...
    for each doc-line-sum where doc-line-sum.doc-code = pardoc-code and
      ...
      if num-entries (doc-line-sum.sum-type, chr(4)) > 1
        then do:
          v-sum-type =  entry (1, doc-line-sum.sum-type, chr(4)).
          isPrlSum = true.
        end.
    ...
    find first tt-sum where isPrlSum and tt-sum.sum-type = 'wst':U no-error.
    if available (tt-sum) then delete tt-sum.
  end.
  run tax-name in this-procedure ('rdt':U, output rdtaxname).
  assign
    tt-sum.cost-road-tax-base:label in browse BROWSE-1 = rdtaxname + "  учет. (вал)"
    tt-sum.cost-road-tax-rubl:label in browse BROWSE-1 = rdtaxname + " учет. (руб)"
    tt-sum.crsa-road-tax-base:label in browse BROWSE-1 = rdtaxname + " тек. прод. (вал)"
    tt-sum.crsa-road-tax-rubl:label in browse BROWSE-1 = rdtaxname + " тек. прод. (руб)"
    tt-sum.sale-road-tax-base:label in browse BROWSE-1 = rdtaxname + " прод. (вал)"
    tt-sum.sale-road-tax-rubl:label in browse BROWSE-1 = rdtaxname + " прод. (руб)"   .
  if paris-doc = no then
  do:
    find first goods where goods.gds-code = pargds-code no-lock.
    assign
      frame Dialog-Frame:title = "Суммы по товару: " + goods.artic + " " + goods.prod-type + " " + string(goods.prod-code) + " " + string(goods.gds-name, "x(30)").
  end.
  RUN enable_UI.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1 FOR EACH tt-sum NO-LOCK.
END PROCEDURE.
FUNCTION fname-sum-type RETURNS CHARACTER
  ...
  do:
    if num-entries (local-tt-sum.sum-type, chr(4)) > 1
    then do:
      case entry (1, local-tt-sum.sum-type, chr(4)) + chr(4) + entry (2, local-tt-sum.sum-type, chr(4)):
        when 'wst':U + chr(4) + "base":U then do:
          return "Сумма_естест_убыли,_л,_мест.хр.-" + entry (3, local-tt-sum.sum-type, chr(4)).
        end.
        when 'wst':U + chr(4) + "cli":U then do:
          return "Сумма_естест_убыли,_кг,_мест.хр.-" + entry (3, local-tt-sum.sum-type, chr(4)).
        end.
        when "mterr" + chr(4) + "base":U then do:
          return "Погреш. измерения,_л,_мест.хр.-" + entry (3, local-tt-sum.sum-type, chr(4)).
        end.
        when "mterr" + chr(4) + "cli":U then do:
          return "Погреш. измерения,_кг,_мест.хр.-" + entry (3, local-tt-sum.sum-type, chr(4)).
        end.
      end.
    end.
    else
    do:
      RETURN entry (lookup (local-tt-sum.sum-type, 'bd,bcd,gen,genc,ad,acd,wst,wstc,ext,extc,mis,misc,exp,inp':U ), 'Сумма_до_документа,Сумма_до_док_ед_пост,Сумма_по_документу,Сумма_по_док_ед_пост,Сумма_после_документа,Сумма_после_док_ед_пост,Сумма_естеств_убыли,Сумма_естеств_убыли_ед_пост,Сумма_излиш_по_документу,Сумма_излиш_по_док_ед_пост,Сумма_недост_по_документу,Сумма_недост_по_док_ед_пост,Сумма_по_израсх_партиям,Сумма_по_созд_партиям':U).
    end.
  end.
END FUNCTION.
