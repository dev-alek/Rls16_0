...
define variable sys-key         as character        no-undo.
...
for each thbjattr_thbj-attr :
    ...
    OUTPUT CLOSE.
    End.
    v-rep-util = new ReportXsltUtil(string( session :temp-directory ) + "rpt" + string( g#report-num )).
...
DO:
  ...
  then do:
      assign
          Tmp#List.type-price :bgcolor in browse BROWSE-2 = GREY_COLOR
      .
  end.
END.
...
define variable vss-include-info11 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  assign tg-rubl:label = "Печать в рублях" .
...
for each bf_fbr-line where bf_fbr-line.doc-code = fbr-doc.doc-code no-lock,
  first bf_recipe
   where bf_recipe.recipe-code = bf_fbr-line.recipe-code
     and bf_recipe.recipe-type = 'разделка':U  no-lock on error undo, return error return-value :
  assign
...
initial "@(#)$Workfile$ $Revision$".
        run menu-doc-create-menu-item in this-procedure
          ( input xtype
           ...
           , input '*'
           , input '*'
           , input 'Акт производства готовой продукции'
...
initial "@(#)$Workfile$ $Revision$".
        run menu-doc-create-menu-item in this-procedure
          ( input xtype
           ...
           , input '*'
           , input '*'
           , input 'Акт о разделке мяса-сырья на полуфабрикаты'
           , input 'cost,sale,rubl,base'
           , input 'rep/op-23.p'
           , input ''
           , input '------'
...
initial "@(#)$Workfile$ $Revision$".
        run menu-doc-create-menu-item in this-procedure
          ( input xtype
           ...
           , input '*'
           , input '*'
           , input 'Калькуляционные карты в текущих продажных ценах'
           , input 'cost,sale,rubl,base'
           , input 'rep/op-1.p'
           , input '1,no,no,no'
           , input '+-----'
...
initial "@(#)$Workfile$ $Revision$".
        run menu-doc-create-menu-item in this-procedure
          ( input xtype
           ...
           , input '*'
           , input '*'
           , input 'Калькуляционные карты в ценах документа'
           , input 'cost,sale,rubl,base'
           , input 'rep/op-1.p'
           , input '1,no,no,yes'
           , input '+-----'
...
initial "@(#)$Workfile$ $Revision$".
        run menu-doc-create-menu-item in this-procedure
          ( input xtype
           ...
           , input '*'
           , input '*'
           , input 'Акт производства промежуточных ингредиентов'
           , input 'cost,sale,rubl,base'
           , input 'rep/fbr-actp.p'
           , input ''
           , input '------'
           , input ''
           , input 'A4lans'
