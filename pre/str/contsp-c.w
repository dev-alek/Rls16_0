  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-c-contract :handle
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  assign frame Dialog-Frame:title = "История спецификации" + chr(32)  + substitute("Фирма: (&1) &2 Договор : &3 от &4", p-host-code, ub.clients.obj-name,  X_contract.contract-prn-code, string(X_contract.contract-date,"99/99/9999")) .
  assign
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY br-c-contract FOR EACH ub.c-contract-specif NO-LOCK       WHERE       ub.c-contract-specif.contract-num = p-contract-code AND       ub.c-contract-specif.gds-code = p-gds-code and       ub.c-contract-specif.host-code = p-host-code      BY ub.c-contract-specif.corr-date DESCENDING      BY ub.c-contract-specif.corr-time DESCENDING.
END PROCEDURE.
  ...
  if available ub.c-contract-specif then do:
    ...
    if not available new_c-contract-specif then do:
      ...
      CASE entry(ii, v-chg-fields):
                        when "prc":U then do:       create temp-changes.       assign       temp-changes.f_name = "prc":U       temp-changes.l_name = "Процент отклонения"       temp-changes.v_old = string(ub.c-contract-specif.prc)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.prc)                                else string(current_contract-specif.prc))       .       end.
                        when "price-cli":U then do:       create temp-changes.       assign       temp-changes.f_name = "price-cli":U       temp-changes.l_name = "Цена поставки"       temp-changes.v_old = string(ub.c-contract-specif.price-cli)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.price-cli)                                else string(current_contract-specif.price-cli))       .       end.
                        when "qnty":U then do:       create temp-changes.       assign       temp-changes.f_name = "qnty":U       temp-changes.l_name = "Количество"       temp-changes.v_old = string(ub.c-contract-specif.qnty)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.qnty)                                else string(current_contract-specif.qnty))       .       end.
                        when "cli-base-rate":U then do:       create temp-changes.       assign       temp-changes.f_name = "cli-base-rate":U       temp-changes.l_name = "Коэф."       temp-changes.v_old = string(ub.c-contract-specif.cli-base-rate)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.cli-base-rate)                                else string(current_contract-specif.cli-base-rate))       .       end.
                        when "sum-cli":U then do:       create temp-changes.       assign       temp-changes.f_name = "sum-cli":U       temp-changes.l_name = "Сумма"       temp-changes.v_old = string(ub.c-contract-specif.sum-cli)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.sum-cli)                                else string(current_contract-specif.sum-cli))       .       end.
                        when "VAT-pc":U then do:       create temp-changes.       assign       temp-changes.f_name = "VAT-pc":U       temp-changes.l_name = "НДС"       temp-changes.v_old = string(ub.c-contract-specif.VAT-pc)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.VAT-pc)                                else string(current_contract-specif.VAT-pc))       .       end.
                        when "VAT-type":U then do:       create temp-changes.       assign       temp-changes.f_name = "VAT-type":U       temp-changes.l_name = "тип НДС"       temp-changes.v_old = string(ub.c-contract-specif.VAT-type)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.VAT-type)                                else string(current_contract-specif.VAT-type))       .       end.
                        when "income-qnty":U then do:       create temp-changes.       assign       temp-changes.f_name = "income-qnty":U       temp-changes.l_name = "Принятое кол-во"       temp-changes.v_old = string(ub.c-contract-specif.income-qnty)       temp-changes.v_new = (if available new_c-contract-specif                                then string(new_c-contract-specif.income-qnty)                                else string(current_contract-specif.income-qnty))       .       end.
      END CASE.
    end.
  end.
