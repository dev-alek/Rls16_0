...
procedure gen-key-rec :
  ...
  :
    ...
    if p-tbl-name = ?
      or p-tbl-name = "":U
    then do:
      return error substitute( "&1 (gen-key-rec). Ошибка задания входных параметров. Не задано имя таблицы.", vss-include-info25 ).
    end.
    if not p-bh_tbl-name:available then do:
      return error substitute( "&1 (gen-key-rec). Ошибка задания входных параметров. Переданый буфер таблицы &2 не доступен", vss-include-info25, p-tbl-name ).
    end.
    assign
    ...
    then do:
      return error substitute( "&1. Таблица &2 не имеет первичного ключа в БД", vss-include-info25, p-tbl-name ).
    end.
    else do:
      assign
      ...
      if v-idx-field-qnty < 2 then do:
        return error substitute( "&1. Определенный первичный индекс (&2) не содержит списка полей для таблицы &3", vss-include-info25, v-inform, p-tbl-name ).
      end.
    ...
    if p-key-rec = ? then do:
      assign
        p-key-rec = "":U
      .
      return error substitute( "&1. Поле(поля) первичного ключа таблицы &2 имеет(ют) неопределенное значение", vss-include-info25, p-tbl-name ).
    end.
  end.
  return.
end procedure.
procedure gen-where-keyr-tab :
  ...
  :
    ...
    if p-key-handle <> ? then do:
      ...
      then do:
        return error substitute( "&1 (gen-row-keyr). Ошибка задания входных параметров. Задан невалидный буфер для поиска.", vss-include-info25 ).
      end.
      if num-entries( p-key-rec, chr(3) ) > 1
        or p-key-rec = ?
        or p-key-rec = "":U
      then do:
        return error substitute( "&1 (gen-row-keyr). Ошибка задания входных параметров. При поиске по буферу вместо ключа (&2) должено быть 'имя таблицы'.", vss-include-info25, p-key-rec ).
      end.
    end.
    else do:
      if p-key-rec = ?
        or p-key-rec = "":U
      then do:
        return error substitute( "&1 (gen-row-keyr). Ошибка задания входных параметров. Не задан уникальный ключ.", vss-include-info25 ).
      end.
    end.
    assign
    ...
    then do:
      return error substitute( "&1 (gen-row-keyr). Ошибка задания входных параметров. &2&3Передан невалидный handle для поиска или handle не типа BUFFER", vss-include-info25, vTable, chr(10) ).
    end.
    ...
    then do:
      ...
      return error substitute( "&1. Таблица &2 не имеет первичного ключа", vss-include-info25, vTable ).
    end.
    assign
      v-idx-field-qnty = num-entries( v-inform ) - 4
    .
    if v-idx-field-qnty < 2 then do:
      delete object bh_tbl-name no-error.
      return error substitute( "&1. Определенный первичный индекс (&2) не содержит списка полей для таблицы &3", vss-include-info25, v-inform, vTable ).
    end.
    assign
      o-where     = "where":U
      v-word-link = "":U
    ...
    :
      ...
      if p-key-handle = ? then do:
        ...
        then do:
          ...
          return error substitute( "&1. Буфер &2 не содержит поля &3 необходимого для поиска.", vss-include-info25, p-key-handle:name, v-field-name ).
        end.
        assign
    ...
    if p-key-handle = ?
      and v-count-fld <> v-field-num
    then do:
      return error substitute( "&1. Не совпадает количество полей первичного ключа для таблицы &2", vss-include-info25, vTable ).
    end.
  end.
end procedure.
...
procedure gen-row-keyr :
  ...
  if p-tbl-row = ? then do:
    return substitute( "Не найдена запись таблицы &2 по ключу &3", vss-include-info25, p-tbl-name, p-key-rec ).
  end.
  else do:
    return.
  end.
...
procedure gen-key-fv :
  ...
  :
    ...
    if p-key-rec = ?
      or p-key-rec = "":U
    then do:
      return error substitute( "&1 (gen-key-fv). Ошибка задания входных параметров. Не задан уникальный ключ.", vss-include-info25 ).
    end.
    assign
    ...
    if error-status:error then return error substitute( "&1 (gen-key-fv). Ошибка задания входных параметров. Неверный уникальный ключ.", vss-include-info25 ).
    assign
      v-inform = bh_tbl-name:index-information(1)
      v-ind    = 2
    .
    ...
    then do:
      delete object bh_tbl-name no-error.
      return error substitute( "&1. Таблица &2 не имеет первичного ключа в БД", vss-include-info25, v-tbl-name ).
    end.
    assign
      v-idx-field-qnty = num-entries( v-inform ) - 4
    .
    if v-idx-field-qnty < 2 then do:
      delete object bh_tbl-name no-error.
      return error substitute( "&1. Определенный первичный индекс (&2) не содержит списка полей для таблицы &3", vss-include-info25, v-inform, v-tbl-name ).
    end.
    assign
      p-field-list = "":U
      p-value-list = "":U
    ...
    delete object bh_tbl-name.
    if v-count-fld <> v-field-num then do:
      return error substitute( "&1. Не совпадает количество полей первичного ключа для таблицы &2 в БД", vss-include-info25, v-tbl-name ).
    end.
  end.
end procedure.
procedure gen-key-field :
  ...
  :
    ...
    if p-table = ?
      or p-table = "":U
    then do:
      return error substitute( "&1 (gen-key-fv). Ошибка задания входных параметров. Не задан таблица.", vss-include-info25 ).
    end.
    assign
      v-tbl-name      = p-table
      v-full-tbl-name = substitute( "ub.&1":U, v-tbl-name )
    .
    create buffer bh_tbl-name for table v-full-tbl-name no-error .
    if error-status:error then return error substitute( "&1 (gen-key-fv). Ошибка задания входных параметров. Неверная таблица.", vss-include-info25 ).
    assign
      v-inform = bh_tbl-name:index-information(1)
      v-ind    = 2
    .
  ...
  price-rubl-without-tax-sale = price-rubl-with-tax-sale - vat-rubl-sale - slt-rubl-sale - road-tax-rubl-sale.
    ...
    on error undo _main,  return error :
      ...
      if available temp-supp then do:
        ...
        OR not (supp_trn-doc.doc-type = 'инв':U
          AND internal = no
          AND supp_trn-doc.ext-doc-type = 'pc':U
          AND supp_trn-doc.status_ = 'накл':U
              ) then do:
...
(input buf_trn-doc.acc-date
,input buf_trn-doc.bge-date
...
,input no
,input v-inv-pay
,input ('@документ коррекции типа приобретения для накладной' + chr(32) + buf_trn-doc.doc-code)
,input no
,input ?
,input 'накл':U
,input ?
...
if (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:   run str/lib-trn.p persistent no-error .   if error-status :error or (valid-handle(ibs.th.gbl.gbl-hndllib:g#lib-trn) <> true) then do:     message       "Error starting lib-trn.p" skip       ibs.th.gbl.gbl-hndllib:g#lib-trn skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :type skip       ibs.th.gbl.gbl-hndllib:g#lib-trn :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run lib-trn_crgdsdtl in g#lib-trn
  ( input supp_doc-line.obj-code
   ,input supp_doc-line.obj-type
   ,input supp_doc-line.doc-code
   ,input supp_doc-line.artic
   ,input supp_doc-line.prod-code
   ,input supp_doc-line.prod-type
   ,input var-term-node
   ,input no
  ) no-error .
          if error-status:error then do:
            undo _main, return error substitute("Ошибка при создании строки документа &1 смены типа приобретения для &2:&3" +
                                              "товар  &4&2&5 &6"
                                                ...
                                                ,par-doc-code
                                                ,chr(10)
                                                ,(supp_doc-line.artic + chr(32) + supp_doc-line.prod-type + string(supp_doc-line.prod-code))
                                                ,error-status:get-message(1)
                                                ,return-value ).
          end.
        FIND FIRST supp_gds-dtl WHERE
        ...
        supp_doc-line.road-tax   =  (if v-curr-r-b = 'base':U
                                    then road-tax-base-sale
                                    else road-tax-rubl-sale
                                  )
        supp_doc-line.excise   =  (if v-curr-r-b = 'base':U
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  (input  supp_gds-dtl.obj-type
  ,input  supp_gds-dtl.obj-code
  ,input  v-prt-b-code
  ,input  0
  ,input  0
  ,output v-doc-num
...
assign
  price-rubl-with-tax-loc = supp_doc-line.price-rubl
  price-base-with-tax-loc = supp_doc-line.price-base
.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output varinvprb
   ...
   find first in-vatp_doc-attr no-lock
    where in-vatp_doc-attr.doc-code  = supp_trn-doc.doc-code
      and in-vatp_doc-attr.attr-code = 'envd':U
    no-error .
   ...
   find first in-vatp-goods where in-vatp-goods.artic     = supp_doc-line.artic     and
                                     in-vatp-goods.prod-type = supp_doc-line.prod-type and
                                     in-vatp-goods.prod-code = supp_doc-line.prod-code no-lock.
   if (not supp_trn-doc.internal and
           supp_trn-doc.doc-type = 'при':U) or
      in-vatp-goods.gds-type = 'у':U then do:
      if varinvprb = "base":u then do:
        assign
          road-tax-base-loc = supp_doc-line.road-tax
          road-tax-rubl-loc = supp_doc-line.road-tax * supp_trn-doc.base-rate / supp_trn-doc.base-scale.
      end.
      else do:
        ASSIGN
          road-tax-rubl-loc = supp_doc-line.road-tax
          road-tax-base-loc = supp_doc-line.road-tax / supp_trn-doc.base-rate * supp_trn-doc.base-scale.
      end.
      ...
      assign
        road-tax-cli-loc = ?.
      ASSIGN
        transport-base-loc = (if supp_doc-line.transport-base = ? then 0 else supp_doc-line.transport-base)
        transport-rubl-loc = (if supp_doc-line.transport-rubl = ? then 0 else supp_doc-line.transport-rubl)
        transport-cli-loc  = 0
        other-base-loc     = (if supp_doc-line.other-base     = ? then 0 else supp_doc-line.other-base)
        other-rubl-loc     = (if supp_doc-line.other-rubl     = ? then 0 else supp_doc-line.other-rubl)
        other-cli-loc      = 0
        vat-pc-loc         = (if supp_doc-line.vat-pc         = ? then 0 else supp_doc-line.vat-pc)
        slt-pc-loc         = (if supp_doc-line.slt-pc         = ? then 0 else supp_doc-line.slt-pc).
                              ASSIGN   slt-base-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-base-with-tax-loc - ((if road-tax-base-loc  = ? then 0 else road-tax-base-loc) + (if transport-base-loc = ? then 0 else transport-base-loc) + (if other-base-loc = ? then 0 else other-base-loc)))                           * slt-pc-loc / (100 + slt-pc-loc))                        vat-base-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-base-with-tax-loc - ((if road-tax-base-loc  = ? then 0 else road-tax-base-loc) + (if transport-base-loc = ? then 0 else transport-base-loc) + (if other-base-loc = ? then 0 else other-base-loc))) * (1 - slt-pc-loc / (100 + slt-pc-loc)) * vat-pc-loc / (100 + vat-pc-loc)).
            ASSIGN   slt-rubl-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-rubl-with-tax-loc - ((if road-tax-rubl-loc  = ? then 0 else road-tax-rubl-loc) + (if transport-rubl-loc = ? then 0 else transport-rubl-loc) + (if other-rubl-loc = ? then 0 else other-rubl-loc)))                           * slt-pc-loc / (100 + slt-pc-loc))                        vat-rubl-loc    = (if in-vatp-have-vat-slt = no then 0 else (price-rubl-with-tax-loc - ((if road-tax-rubl-loc  = ? then 0 else road-tax-rubl-loc) + (if transport-rubl-loc = ? then 0 else transport-rubl-loc) + (if other-rubl-loc = ? then 0 else other-rubl-loc))) * (1 - slt-pc-loc / (100 + slt-pc-loc)) * vat-pc-loc / (100 + vat-pc-loc)).
      assign
        vat-cli-loc            = ?
        ...
        price-cli-with-tax-loc = ?.
                                                ...
                                                for each in-vatp-parts where in-vatp-parts.out-code  = supp_doc-line.doc-code  and
                                      in-vatp-parts.obj-type  = supp_doc-line.obj-type  and
                                      in-vatp-parts.obj-code  = supp_doc-line.obj-code  and
                                      in-vatp-parts.artic     = supp_doc-line.artic     and
                                      in-vatp-parts.prod-type = supp_doc-line.prod-type and
                                      in-vatp-parts.prod-code = supp_doc-line.prod-code
      ...
      ASSIGN
        road-tax-base-loc   = if supp_doc-line.fact-qnty <> 0 then (accum total in-vatp-parts.road-tax-base  * in-vatp-parts.fact-qnty) / supp_doc-line.fact-qnty  else 0
        road-tax-rubl-loc   = if supp_doc-line.fact-qnty <> 0 then (accum total in-vatp-parts.road-tax-rubl  * in-vatp-parts.fact-qnty) / supp_doc-line.fact-qnty  else 0
        transport-base-loc  = if supp_doc-line.fact-qnty <> 0 then (accum total in-vatp-parts.transport-base * in-vatp-parts.fact-qnty) / supp_doc-line.fact-qnty  else 0
        transport-rubl-loc  = if supp_doc-line.fact-qnty <> 0 then (accum total in-vatp-parts.transport-rubl * in-vatp-parts.fact-qnty) / supp_doc-line.fact-qnty  else 0
        other-base-loc      = if supp_doc-line.fact-qnty <> 0 then (accum total in-vatp-parts.other-base     * in-vatp-parts.fact-qnty) / supp_doc-line.fact-qnty  else 0
        other-rubl-loc      = if supp_doc-line.fact-qnty <> 0 then (accum total in-vatp-parts.other-rubl     * in-vatp-parts.fact-qnty) / supp_doc-line.fact-qnty  else 0
                                        vat-base-loc        = if supp_doc-line.fact-qnty <> 0 then (accum total (if in-vatp-have-vat-slt = no then 0 else (in-vatp-parts.price-base - ((if in-vatp-parts.road-tax-base  = ? then 0 else in-vatp-parts.road-tax-base) + (if in-vatp-parts.transport-base = ? then 0 else in-vatp-parts.transport-base) + (if in-vatp-parts.other-base = ? then 0 else in-vatp-parts.other-base)))   * in-vatp-parts.fact-qnty * (1 - in-vatp-parts.slt-pc / (100 + in-vatp-parts.slt-pc)) * in-vatp-parts.vat-pc / (100 + in-vatp-parts.vat-pc))) / supp_doc-line.fact-qnty   else 0
        slt-base-loc        = if supp_doc-line.fact-qnty <> 0 then (accum total (if in-vatp-have-vat-slt = no then 0 else (in-vatp-parts.price-base - ((if in-vatp-parts.road-tax-base  = ? then 0 else in-vatp-parts.road-tax-base) + (if in-vatp-parts.transport-base = ? then 0 else in-vatp-parts.transport-base) + (if in-vatp-parts.other-base = ? then 0 else in-vatp-parts.other-base)))   * in-vatp-parts.fact-qnty                     * in-vatp-parts.slt-pc / (100 + in-vatp-parts.slt-pc))) / supp_doc-line.fact-qnty   else 0
                vat-rubl-loc        = if supp_doc-line.fact-qnty <> 0 then (accum total (if in-vatp-have-vat-slt = no then 0 else (in-vatp-parts.price-rubl - ((if in-vatp-parts.road-tax-rubl  = ? then 0 else in-vatp-parts.road-tax-rubl) + (if in-vatp-parts.transport-rubl = ? then 0 else in-vatp-parts.transport-rubl) + (if in-vatp-parts.other-rubl = ? then 0 else in-vatp-parts.other-rubl)))   * in-vatp-parts.fact-qnty * (1 - in-vatp-parts.slt-pc / (100 + in-vatp-parts.slt-pc)) * in-vatp-parts.vat-pc / (100 + in-vatp-parts.vat-pc))) / supp_doc-line.fact-qnty   else 0
        slt-rubl-loc        = if supp_doc-line.fact-qnty <> 0 then (accum total (if in-vatp-have-vat-slt = no then 0 else (in-vatp-parts.price-rubl - ((if in-vatp-parts.road-tax-rubl  = ? then 0 else in-vatp-parts.road-tax-rubl) + (if in-vatp-parts.transport-rubl = ? then 0 else in-vatp-parts.transport-rubl) + (if in-vatp-parts.other-rubl = ? then 0 else in-vatp-parts.other-rubl)))   * in-vatp-parts.fact-qnty                     * in-vatp-parts.slt-pc / (100 + in-vatp-parts.slt-pc))) / supp_doc-line.fact-qnty   else 0
