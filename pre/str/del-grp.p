...
i-grp-code = integer(entry(2, p-parameter, chr(4)))
modetype = (if num-entries(p-parameter, chr(4) ) > 2
            then logical(entry(3, p-parameter, chr(4)))
            else no)
modetype = (if num-entries(p-parameter, chr(4) ) > 2
            then logical(entry(3, p-parameter, chr(4)))
            else no)
...
FIND FIRST cash-desk No-LOCK WHERE
          ...
          (cash-desk.pos-type = 'IBM':U OR
           cash-desk.pos-type = 'IBM-XML':U OR
           cash-desk.pos-type = 'IPC-Servis+':U OR
           cash-desk.pos-type = 'NCR-GM':U OR
           cash-desk.pos-type = 'NCR-AS@R':U OR
           cash-desk.pos-type = 'MAGIA-XML':U or
           cash-desk.pos-type = 'Autotank':U
           ) AND
