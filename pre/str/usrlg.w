...
ON VALUE-CHANGED OF br-head IN FRAME Dialog-Frame
    DO:
    END.
ON CHOOSE OF bt-doc-hist IN FRAME Dialog-Frame
    DO:
        ...
        run local-open-query-head in this-procedure. .
    END.
ON VALUE-CHANGED OF cb-table IN FRAME Dialog-Frame
    DO:
        assign
            v-table   = ""
            v-c-table = ""
        ...
        run local-open-query-head in this-procedure. .
    END.
ON RETURN OF fi-date-for IN FRAME Dialog-Frame
    DO:
        assign
            fi-date-for
            .
...
do:
  ...
  then do:
    ASSIGN
      fi-date-to :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date5 :HANDLE
      fi-date-to :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle5 as handle no-undo .
  assign
    v-label-handle5 = fi-date-to :side-label-handle in frame Dialog-Frame
  .
...
on ' ' of fi-date-for in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on delete-character of fi-date-for in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of fi-date-for in frame Dialog-Frame
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of fi-date-for in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of fi-date-for in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of fi-date-for in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date7
    ...
    .
  if fi-date-for :POPUP-MENU in frame Dialog-Frame = ?
  then do:
    ASSIGN
      fi-date-for :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date7 :HANDLE
      fi-date-for :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle7 as handle no-undo .
  assign
    v-label-handle7 = fi-date-for :side-label-handle in frame Dialog-Frame
  .
  ...
  ON CHOOSE OF MENU-ITEM m-ed-date7-1 in menu m-ed-date7 DO:
    apply "ctrl-b":U to fi-date-for in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date7-2 in menu m-ed-date7 DO:
    apply "ctrl-d":U to fi-date-for in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date7-3 in menu m-ed-date7 DO:
    apply "ctrl-e":U to fi-date-for in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date7-4 in menu m-ed-date7 DO:
    apply "ctrl-f":U to fi-date-for in frame Dialog-Frame .
  END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-head :handle
      .
...
:
    ...
    then
       assign
          p-unique-key-rec = substring (p-unique-key-rec, index(p-unique-key-rec,chr(3)) + 1 )
          p-unique-key-string = replace( p-unique-key-rec, chr(3), ",":U )
       .
    else
       assign
           p-unique-key-string = replace( v-field-value-list, chr(3), ",":U )
       .
end.
END PROCEDURE.
