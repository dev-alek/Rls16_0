...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pftaxval in g#library
  ...
  ,output v-value
          ...
          if error-status:error then do:
            message
            "Неверное значение ставки налога" buf_tax-rate.rate-code 'маг':U  shop.obj-code
            view-as alert-box error .
            return error .
          end.
         if v-value = ?
          and not cash-txn.news-action then do:
            ...
            ' "'
            (substring(cash-txn.tax-name, 1, 8) + chr(32) + string((if v-value = ? then 0 else v-value) , ">9.99%":U)) format "X(15)"
            '"'
            chr(10).
          end.
        end.
  ...
  when 'MAGIA-XML':U then do:
    ...
    run bgelib-tag-put in this-procedure ( input 3, input "TaxCatName"
                                          , input trim(cash-txn.tax-name, chr(32)), input 1 ).
  end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pftaxvlx in g#library
  (input  ?
  ,input  cash-txr.tax-code
  ,input  cash-txr.rate-code
  ,input  v-today
  ,input  shop-buffer.host-code
  ,input  'маг':U
  ,input  i-obj-code
  ,output x-host-code
  ,output x-obj-type
...
FOR EACH for-cash-desk NO-LOCK WHERE
  ...
  and for-cash-desk.autonomy = integer('2':U) then do:
    assign
...
then do:
  if for-cash-desk.pos-type = 'MAGIA-XML':U then do:
    run str/waitpxml.w ( replace( v-xml-file-name-path, "/", "\" ) + "xml",
                ...
                ( if action = 'U'
                  then ('Ждите - ' + 'добавление категорий и ставок налогов')
                  else ('Ждите - ' + 'удаление категорий и ставок налогов') ) +
...
define variable v-found26 as logical no-undo .
define variable v-is-script26 as logical no-undo.
define variable v-fields-shift26 as integer no-undo .
find first temp-tekka-tsk no-error.
if available temp-tekka-tsk then do:
   v-found26 = yes.
end.
if v-found26 = yes then do:
...
define variable dflt-cd28 as character no-undo .
define variable vss-include-info29 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type29 as character no-undo .
define variable v-value-date29 as date no-undo .
define variable v-value-decimal29 as decimal no-undo .
define variable v-value-integer29 as INTEGER no-undo .
define variable v-value-logical29 AS LOGICAL no-undo .
define variable v-tth29 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd28
    ,output v-value-date29
    ,output v-value-decimal29
    ,output v-value-integer29
    ,output v-value-logical29
    ,output v-param-type29
    ,INPUT-OUTPUT table-handle v-tth29
    ) no-error .
delete object v-tth29 no-error.
