...
:
  ...
  if error-status:error
  then do:
      run write-to-log in p-log-handle( vss-workfile + chr(32)
  ...
  if v-param-list = "":U then do:
      run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Не заданы параметры досылки данных файлов, недошедших до POS IBM-XML, в задаче &1&2"                                     , p-task-num                                                                                                           , chr(10))).
      return.
  end.
  assign
  v-dir-name = entry(1, v-param-list, chr(4) )
  no-error
  .
  if error-status :error then do:
    run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Ошибки при получении параметров досылки&1" +                            "&2&1&3&1"                                                                          , error-status:get-message(1)                                                       , return-value )).
  end.
  case v-dir-name:
    when 'ini':U then do:
  ...
  if index( v-dir-type, "D" ) = 0 then do:
    run write-log-and-file in p-log-handle (          input 1                                                            , input log-file-name                                                , input 1                                                            , input substitute("!!!Выбранный каталог &1 - недоступен", v-dir-name)).
    return .
  end.
