...
END.
ON LEAVE OF tt-wth-line.aft-sum IN FRAME Dialog-Frame
DO:
  if ub.wth-doc.status_ = 'разрешен':U   then do:
  DISPLAY
        (input frame Dialog-Frame tt-wth-line.aft-sum - tt-wth-line.bef-sum) @ tt-wth-line.fact-sum
...
DO:
  ...
  FIND FIRST buf_wth-place NO-LOCK WHERE
    ...
    buf_wth-place.w-p-code    = INPUT FRAME Dialog-Frame tt-wth-line.w-p-code NO-ERROR.
...
run display-bef-sum in this-procedure no-error.
END.
ON LEAVE OF tt-wth-line.w-p-code IN FRAME Dialog-Frame
DO:
   FIND FIRST buf_wth-place NO-LOCK WHERE
    buf_wth-place.host-code = ub.wth-DOC.host-code AND
    buf_wth-place.obj-type = tt-wth-line.obj-type AND
  ...
  IF NOT AVAIL buf_wth-place THEN DO:
    IF tt-wth-line.w-p-code = INPUT FRAME Dialog-Frame tt-wth-line.w-p-code THEN DO:
        RETURN.
    END.
    MESSAGE
      "Место хранения МЦ" INPUT FRAME Dialog-Frame tt-wth-line.w-p-code
...
DO:
  FIND FIRST buf_wealth NO-LOCK WHERE
                   buf_wealth.wth-code = INPUT FRAME Dialog-Frame tt-wth-line.wth-code NO-ERROR.
  IF AVAIL buf_wealth THEN DO:
    DISPLAY
    buf_wealth.wth-name @ ub.wealth.wth-name
    WITH FRAME Dialog-Frame.
...
IF AVAIL buf_wealth THEN DO:
  RUN wth-lib_cur-stock-place (
                                 INPUT tt-wth-line.obj-type,
                                 INPUT tt-wth-line.obj-code,
                                 INPUT ( INPUT FRAME Dialog-Frame tt-wth-line.w-p-code ),
                                 INPUT ( INPUT FRAME Dialog-Frame tt-wth-line.wth-code ),
                                OUTPUT for-bef-sum
                              ).
      DISPLAY
...
CASE par-mode:
  when 'ДОБАВЛЕНИЕ':U THEN DO:
    ...
    tt-wth-line.Aft-sum IN FRAME Dialog-Frame
  ...
  when 'ИЗМЕНЕНИЕ':U  THEN DO:
    ...
    when 'ПРОСМОТР':U  THEN DO:
      IF ub.wth-doc.status_ = 'накл':U THEN DO:
        ...
        DISPLAY
        TT-WTH-LINE.AFT-SUM
        (tt-wth-line.aft-sum - tt-wth-line.bef-sum) @ tt-wth-line.fact-sum WITH FRAME Dialog-Frame.
      END.
      ENABLE
      B-Next
      B-Prev
      ...
      WITH FRAME Dialog-Frame.
      HIDE
      B-save IN FRAME Dialog-Frame
      B-Wealth IN FRAME Dialog-Frame
      B-PLACE IN FRAME Dialog-Frame
      .
      locked-wth = yes.
    END.
  END CASE.
  ...
  FRAME Dialog-Frame:TITLE =
      ...
      ( IF wth-doc.doc-type = 'при':U     THEN "ПРИХОД"         ELSE
      ( IF wth-doc.doc-type = 'рас':U    THEN "РАСХОД"         ELSE
      ( IF wth-doc.doc-type = 'спи':U  THEN "СПИСАНИЕ"       ELSE
      ( IF wth-doc.doc-type = 'инв':U  THEN "ИНВЕНТАРИЗАЦИЯ" ELSE wth-doc.doc-type ) ) ) ) +
...
frame Dialog-Frame tt-wth-line.w-p-code
frame Dialog-Frame tt-wth-line.bef-sum
frame Dialog-Frame tt-wth-line.aft-sum
    ...
    if var-entry <> '':U then do:
      CASE var-entry:
        ...
        when "w-p-code":U then do:
            APPLY "ENTRY":U TO tt-wth-line.w-p-code IN FRAME Dialog-Frame.
        end.
        when "aft-sum":U then do:
            APPLY "ENTRY":U TO tt-wth-line.aft-sum IN FRAME Dialog-Frame.
        end.
