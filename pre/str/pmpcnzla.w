...
define variable vss-description as character no-undo initial "Просмотр карточки истории изменения соответствия ТРК-пистолет":U.
...
define frame fr-D-pump-8
  ub.c-pump-nozzle.nozzle-code at row  1.25 col 11.00 colon-aligned view-as fill-in    size-chars 10.50 by 1.00    label "Пистолет" fgcolor  4
  ub.c-pump-nozzle.pump-code   at row  2.50 col 11.00 colon-aligned view-as fill-in    size-chars  3.50 by 1.00    label "ТРК"      fgcolor  4
  ub.c-pump-nozzle.obj-type    at row  3.75 col 11.00 colon-aligned view-as fill-in    size-chars  4.50 by 1.00    label "Объект"
  ub.c-pump-nozzle.obj-code    at row  3.75 col 18.00          view-as fill-in    size-chars 10.50 by 1.00 no-label
    bf_cli.obj-name    at row  3.75 col 29.00          view-as fill-in    size-chars 70.00 by 1.00 no-label            fgcolor  4
  ub.c-pump-nozzle.is-meas     at row  5.00 col  2.50          view-as toggle-box size-chars  23.50 by 1.00
  ub.c-pump-nozzle.status_     at row  6.25 col 11.00 colon-aligned view-as fill-in    size-chars  9.50 by 1.00
  ub.c-pump-nozzle.PS          at row  7.50 col  1.50          view-as editor no-word-wrap scrollbar-horizontal scrollbar-vertical size-chars 98.25 by 3.50 format "x(512)":U                no-label            bgcolor 15
  r-rect-0             at row 11.25 col  1.50
    Btn_Exit           at row 11.50 col  2.50
    Btn_OK             at row 11.50 col 78.00
  b-help          at row 11.50 col 88.75
...
do on error   undo Main-Block, leave Main-Block
  ...
  find ub.c-pump-nozzle no-lock where recid( ub.c-pump-nozzle ) = p-rec-id no-error.
  if not available ub.c-pump-nozzle then do:
    message "Карточка истории соответствия пистолет-ТРК не найдена!" view-as alert-box error.
    undo Main-Block, leave Main-Block.
  end.
  find bf_cli   no-lock where
       bf_cli.obj-type   = ub.c-pump-nozzle.obj-type and
       bf_cli.obj-code   = ub.c-pump-nozzle.obj-code no-error.
  assign frame fr-D-pump-8 :title = substitute(
    'Карточка изменения соответствия ТРК &1 - пистолет &2 (объект &3 &4 "&5") -- &6',
    ub.c-pump-nozzle.pump-code, ub.c-pump-nozzle.nozzle-code, ub.c-pump-nozzle.obj-type, ub.c-pump-nozzle.obj-code, bf_cli.obj-name, p-mode ).
  display ub.c-pump-nozzle.nozzle-code
          ub.c-pump-nozzle.pump-code
          ub.c-pump-nozzle.obj-type
          ub.c-pump-nozzle.obj-code
          ub.c-pump-nozzle.is-meas
          ub.c-pump-nozzle.status_
          ub.c-pump-nozzle.PS
  with frame fr-D-pump-8.
  if available bf_cli then do: display bf_cli.obj-name with frame fr-D-pump-8. end.
  enable ub.c-pump-nozzle.PS Btn_Exit b-help with frame fr-D-pump-8.
  assign ub.c-pump-nozzle.PS :read-only in frame fr-D-pump-8 = yes.
