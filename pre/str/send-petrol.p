...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка данных по соответствию товаров/кошельков '" + " &1")
...
END CASE.
  RUN putc-petrol in this-procedure
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'добавление/изменение данных по соответствию товаров/кошельков ')
                  else ('Ждите - ' + 'удаление данных по соответствию товаров/кошельков ') ) +
...
define variable dflt-cd19 as character no-undo .
