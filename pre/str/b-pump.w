...
if not available bf_pump then
   return error SUBSTITUTE("Не найден пистолет с номером &1", parpump-code) + SUBSTITUTE(" на объекте &1 &2 .", parobj-type, parobj-code).
...
PROCEDURE adm-display-fields :
      IF AVAILABLE ub.pump THEN
          DISPLAY ub.pump.pump-code WITH BROWSE br_table
            NO-ERROR.
...
message
  ...
  "Неверное имя файла d-pump.w." skip
...
message
  vss-workfile vss-revision vss-description skip
  "Ошибка при вызове файла d-pump.w." skip
  "-----------Cистемная ошибка------------" skip
  return-value skip
  "------Ошибка исполнения программы------" skip
  trim(error-status :get-message(1)) +
...
DO:
  define buffer bf_pump for ub.pump.
  if available ub.pump then do:
    if ub.pump.ps <> input frame F-Main varps then do:
      if varps-upd = no then do:
        ...
        .
        message
          "Вы хотите изменить примечание к " "ТРК"
          view-as alert-box question buttons yes-no update varmes-log.
        if varmes-log = true then do:
          ...
          :
            ...
            find first bf_pump exclusive-lock
              where recid(bf_pump) = recid(ub.pump)
            .
            assign
              frame F-Main varps
            .
...
PROCEDURE adm-open-query-cases :
  OPEN QUERY br_table FOR EACH ub.pump WHERE ub.pump.obj-type = varobj-type and                                             ub.pump.obj-code = varobj-code NO-LOCK       INDEXED-REPOSITION.
END PROCEDURE.
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "ub.pump":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE ub.pump THEN STRING(ROWID(ub.pump))
        ELSE "?":U.
        OTHERWISE
        DO:
