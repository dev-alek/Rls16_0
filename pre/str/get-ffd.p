...
define variable vss-revision9    as character no-undo init "$Revision:$":U .
define variable vss-author9      as character no-undo init "$Author:$":U .
define variable vss-date9        as character no-undo init "$Date:$":U .
define variable vss-workfile9    as character no-undo init "$Workfile:$":U .
define variable vss-archive9     as character no-undo init "$Archive:$":U .
define variable vss-description9 as character no-undo init "Работа С сокетом".
...
FOR EACH buf_cash-desk WHERE
   ...
   assign
      v-obj-list = 'маг':U + string(buf_cash-desk.obj-code)
      .
...
GISMT_FAST_ANSWER".
   ...
   run ConectSocet (entry(1,entry(2, buf_cash-desk.addr-path, chr(4)),":"),
                    ...
                    30,
                    no,
                    substitute ("Чтение параметры ФФД с кассы &1. ",entry(2, buf_cash-desk.addr-path, chr(4)))
                    ).
   if mWebResp eq ""
   then do:
      run write-log-and-file in p-log-handle (
