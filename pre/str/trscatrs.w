...
define variable filter-point     as character no-undo initial 'c-trn-rsn-attr':U.
define variable filter-point0    as character no-undo initial 'c-trn-rsn-attr':U.
define variable filter-label     as character no-undo initial 'История атрибутов оснований (причин) создания документа':U.
define variable filter-label0    as character no-undo initial 'История атрибутов оснований (причин) создания документа':U.
...
define query br-rattrs for buf_c-trn-rsn-attr scrolling.
define query br-changes for buf_changes scrolling.
define browse br-rattrs query br-rattrs display
  mark-string( buffer buf_c-trn-rsn-attr ) column-label '*' format 'x(1)':U
  buf_c-trn-rsn-attr.attr-code column-label 'Код' format "x(24)":U
  buf_c-trn-rsn-attr.attr-value column-label 'Значение' format "x(69)":U
  usrfulnf(buf_c-trn-rsn-attr.corr-user-name) column-label 'Изменил' format "x(8)":U
  ShowAction( buf_c-trn-rsn-attr.action ) column-label 'Действие' format "x(10)":U
  buf_c-trn-rsn-attr.corr-date column-label 'Дата корр.' format "99/99/9999":U
  STRING( buf_c-trn-rsn-attr.corr-time, 'HH:MM:SS':U ) column-label 'Время' format "x(8)":U
  buf_c-trn-rsn-attr.chip-num column-label 'Щепка' format "->,>>>,>>>,>>9":U
  buf_c-trn-rsn-attr.corr-user-db-num column-label 'БД' format ">>>>9":U
  enable
  buf_c-trn-rsn-attr.attr-value
...
with view-as dialog-box keep-tab-order side-labels no-underline three-d scrollable
     title 'История атрибутов оснований (причин) создания документа':U
     default-button b-quit cancel-button b-quit.
assign frame fr-D-rattr-0 :scrollable = no.
assign br-rattrs         :num-locked-columns in frame  fr-D-rattr-0  = 1
       buf_c-trn-rsn-attr.attr-value :read-only          in browse br-rattrs = yes
       buf_changes.l_name :read-only          in browse   br-changes   = yes.
assign b-mark    :tooltip in frame fr-D-rattr-0 = "Поставить/снять отметку записи"
         b-quit     :tooltip in frame fr-D-rattr-0 = "Вернуться в окно вызова"
       b-sch  :tooltip in frame fr-D-rattr-0 = "Установить/снять фильтр"
       b-help    :tooltip in frame fr-D-rattr-0 = "Интерактивная помощь в формате *.html"
       b-sel  :tooltip in frame fr-D-rattr-0 = "Выбрать текущую(ие) запись(и)"
       br-rattrs :tooltip in frame fr-D-rattr-0 = "Список действий над атрибутами"
         br-changes   :tooltip in frame fr-D-rattr-0 = "Список изменений причины"
         sch-code     :tooltip in frame fr-D-rattr-0 = "Код атрибута" + " для поиска. Поиск первой записи - <ВВОД>; поиск следующей - <CTRL-J>"
         sch-num      :tooltip in frame fr-D-rattr-0 = "Количество найденных записей"
         mark-num     :tooltip in frame fr-D-rattr-0 = "Отмеченные записи".
on delete-character of br-rattrs in frame fr-D-rattr-0 do:
  if b-mark :sensitive in frame fr-D-rattr-0 then do: apply "CHOOSE":U to b-mark in frame fr-D-rattr-0. end.
end.
on insert-mode of br-rattrs in frame fr-D-rattr-0 do:
  if           b-mark   :sensitive in frame fr-D-rattr-0 then do:
    apply "CHOOSE":U to b-mark     in frame fr-D-rattr-0.
  end.
  else if b-sel :sensitive in frame fr-D-rattr-0 then do:
    apply "CHOOSE":U to b-sel   in frame fr-D-rattr-0.
  end.
end.
on choose of b-mark in frame fr-D-rattr-0 do:
...
assign
  v-str-recid9 = trim( string( recid( buf_c-trn-rsn-attr ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry9 > 0 then do:
  ...
  .
    ...
    br-rattrs :refresh( ) in frame fr-D-rattr-0.
    if last-event :function <> "MOUSE-SELECT-DBLCLICK" then do:
      br-rattrs :select-next-row( ) in frame fr-D-rattr-0.
    end.
    apply "VALUE-CHANGED":U to br-rattrs in frame fr-D-rattr-0.
    if num-entries( p-rid-list ) = 0 then do:
      hide
      mark-num   in frame fr-D-rattr-0.
    end.
    else do:
      display
      num-entries( p-rid-list ) @ mark-num
      with frame fr-D-rattr-0.
    end.
  END.
  apply "ENTRY":U to br-rattrs in frame fr-D-rattr-0.
end.
on choose of b-quit in frame fr-D-rattr-0 do:
  run gbl/markqwa.p ( input b-mark :sensitive, input p-rid-list ) no-error.
  if error-status :error then do:
    return no-apply.
  end.
end.
on choose of b-sch in frame fr-D-rattr-0 do:
  run proc-filter in this-procedure no-error.
  if error-status :error then do:
    return no-apply.
  end.
end.
on choose of b-sel in frame fr-D-rattr-0 do:
...
on return                of br-rattrs in frame fr-D-rattr-0 or
   mouse-select-dblclick of br-rattrs in frame fr-D-rattr-0 do:
  if           b-mark   :sensitive in frame fr-D-rattr-0 then do:
      apply "CHOOSE":U to b-mark   in frame fr-D-rattr-0.
  end.
  else IF b-sel :sensitive in frame fr-D-rattr-0 then do:
...
on value-changed of br-rattrs in frame fr-D-rattr-0 do:
  run proc-view-changes in this-procedure no-error.
end.
on entry of sch-code in frame fr-D-rattr-0 do:
...
on leave of sch-code in frame fr-D-rattr-0 do:
  ...
  hide
  sch-num
  in frame fr-D-rattr-0.
end.
on CTRL-J of sch-code in frame fr-D-rattr-0 do:
  if input frame fr-D-rattr-0 sch-code <> sch-code then do:
    assign sch-code.
    assign FoundRec = ?
           sch-num  = 0.
    hide
...
on return of sch-code in frame fr-D-rattr-0 do:
  assign sch-code.
  assign FoundRec = ?
         sch-num  = 0.
  hide   sch-num  in frame fr-D-rattr-0.
  run proc-find-code in this-procedure ( input no,  input sch-code ) no-error.
  if error-status :error then do:
    return no-apply.
  end.
end.
on mouse-select-dblclick of sch-code in frame fr-D-rattr-0 do:
  if input frame fr-D-rattr-0 sch-code <> sch-code then do:
    assign sch-code.
    assign FoundRec = ?
           sch-num  = 0.
...
on F1 of frame fr-D-rattr-0 anywhere do:
  if b-help :sensitive then DO: apply "CHOOSE":U to b-help in frame fr-D-rattr-0. END.
  return no-apply.
end.
define variable vss-include-info11 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on INS of frame fr-D-rattr-0 anywhere do:
  if b-mark :sensitive then DO: apply "CHOOSE":U to b-mark in frame fr-D-rattr-0. END.
  return no-apply.
end.
define variable vss-include-info12 as character format "X(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on F2 of frame fr-D-rattr-0 anywhere do:
  if b-sel :sensitive then DO: apply "CHOOSE":U to b-sel in frame fr-D-rattr-0. END.
  return no-apply.
end.
if valid-handle( active-window ) and frame fr-D-rattr-0 :parent = ? then frame fr-D-rattr-0 :parent = active-window.
if current-window :window-state = window-minimized then do: current-window :window-state = window-normal. end.
on window-close of frame fr-D-rattr-0 do: apply "END-ERROR":U to self. end.
define variable vss-include-info13 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
on help of frame fr-D-rattr-0
do:
  run gbl/app_help.p
...
on choose of b-help in frame fr-D-rattr-0
do:
  apply "help":u to frame fr-D-rattr-0 .
end.
...
procedure minbtn-set :
    do
        ...
        do
            ...
            :
            assign
                v-frame-width = frame fr-D-rattr-0:width - 0.3
                fh            = frame fr-D-rattr-0:first-child
                hh            = fh:first-child
                ii            = 1
                .
...
procedure diasize_change-height :
  ...
  if p-change-value > 0
  then do:
    if frame fr-D-rattr-0 :height + p-change-value
        > decimal(session :work-area-height-pixels) / session :pixels-per-row
    then do:
      assign
        p-change-value = decimal(session :work-area-height-pixels) / session :pixels-per-row
                        - (frame fr-D-rattr-0 :height-chars)
      .
      if p-change-value <= 0
      then do:
  ...
  if p-change-value < 0
  then do:
    if frame fr-D-rattr-0 :height + p-change-value < v-diasize-orig-frame-height
    then do:
      assign
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame fr-D-rattr-0 :height = v-frame-height
          .
          if frame fr-D-rattr-0 :scrollable = true
          then do:
            assign
              frame fr-D-rattr-0 :virtual-height = v-frame-virtual-height
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-height = frame fr-D-rattr-0 :height
      v-frame-virtual-height = frame fr-D-rattr-0 :virtual-height
    ...
    assign
      v-field-group-handle = frame fr-D-rattr-0 :first-child
    .
    ...
    then do:
      ...
      then do:
        assign
          frame fr-D-rattr-0 :virtual-height = frame fr-D-rattr-0 :virtual-height + p-change-value
          no-error .
        if error-status :error
      ...
      assign
        frame fr-D-rattr-0 :height = frame fr-D-rattr-0 :height + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  'height':u
          ,input  string(frame fr-D-rattr-0 :height - v-diasize-orig-frame-height)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-height :
  ...
  :
    run diasize_change-height in this-procedure
      (input  (p-new-height - frame fr-D-rattr-0 :height)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_change-width :
  ...
  if p-change-value > 0
  then do:
    if frame fr-D-rattr-0 :width + p-change-value >
        session :width-chars
    then do:
      assign
        p-change-value = session :width-chars - frame fr-D-rattr-0 :width
      .
      if p-change-value <= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  if p-change-value < 0
  then do:
    if frame fr-D-rattr-0 :width + p-change-value < v-diasize-orig-frame-width
    then do:
      assign
        p-change-value = v-diasize-orig-frame-width
                       - frame fr-D-rattr-0 :width
      .
      if p-change-value >= 0
      then do:
        run diasize_position-resize-button in this-procedure .
  ...
  :
    ...
    then do:
      ...
      :
        ...
        then do:
          ...
          assign
            frame fr-D-rattr-0 :width = v-frame-width
          .
          if frame fr-D-rattr-0 :scrollable = true
          then do:
            assign
              frame fr-D-rattr-0 :virtual-width = v-frame-virtual-width
            .
          end.
          run diasize_position-resize-button in this-procedure .
        end.
    ...
    assign
      v-frame-width = frame fr-D-rattr-0 :width
      v-frame-virtual-width = frame fr-D-rattr-0 :virtual-width
    ...
    then do:
      ...
      then do:
        assign
          frame fr-D-rattr-0 :virtual-width = frame fr-D-rattr-0 :virtual-width + p-change-value
          no-error .
        if error-status :error
        or error-status :get-message(1) <> ""
        then do:
      ...
      assign
        frame fr-D-rattr-0 :width = v-frame-width + p-change-value
        no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
      ...
      assign
        frame fr-D-rattr-0 :width = frame fr-D-rattr-0 :width + p-change-value
      no-error .
      if error-status :error
      or error-status :get-message(1) <> ""
      then do:
    ...
    then do:
      ...
      then do:
        ...
        run gbl/wndsizew.p
          ...
          ,input  v-diasize-proc-name
          ,input  'width':u
          ,input  string(frame fr-D-rattr-0 :width - v-diasize-orig-frame-width)
          ) .
      end.
    end.
  end.
...
procedure diasize_set-width :
  ...
  :
    run diasize_change-width in this-procedure
      (input  (p-new-width - frame fr-D-rattr-0 :width)
      ,input  p-move-resize
      ) .
  end.
end procedure.
procedure diasize_position-resize-button :
  ...
  :
    hide v-diasize-resize-button .
    assign
      v-diasize-resize-button :row = frame fr-D-rattr-0 :height - v-diasize-resize-button :height
                  - 1
                  - (frame fr-D-rattr-0 :border-bottom-pixels / session :pixels-per-row)
      v-diasize-resize-button :col = frame fr-D-rattr-0 :width - v-diasize-resize-button :width
                  - 1
                  - (frame fr-D-rattr-0 :border-right-pixels / session :pixels-per-column)
    .
...
on alt-enter of frame fr-D-rattr-0
do:
...
procedure diasize_end-move :
  ...
  :
    ...
    .
    assign
      v-row-delta = v-new-row - frame fr-D-rattr-0 :height
      v-col-delta = v-new-col - frame fr-D-rattr-0 :width
    .
    run diasize_change-height in this-procedure
      (input v-row-delta
      ,input true
    ...
    then do:
      ...
      run diasize_change-width in this-procedure
        (input session :width-chars
            - frame fr-D-rattr-0 :width-chars
        ,input true
        ) .
      assign
...
procedure diasize_restore-orig-size :
  ...
  :
    assign
      v-diasize-current-frame-width  = frame fr-D-rattr-0 :width
      v-diasize-current-frame-height = frame fr-D-rattr-0 :height
    .
...
procedure diasize_init :
  ...
  :
    ...
    :
      assign
        v-diasize-orig-frame-height = frame fr-D-rattr-0 :height
        v-diasize-orig-frame-width  = frame fr-D-rattr-0 :width
        v-diasize-browse-handle     = browse br-rattrs :handle
      .
      create button v-diasize-resize-button
      assign
        parent        = frame fr-D-rattr-0 :first-child
        label         = "s"
        height-pixels = 16
        width-pixels  = 16
        visible       = true
...
DEF VAR cur-clmn-numbr-rattrs as INT EXTENT 9 no-undo.
...
ON ctrl-cursor-right OF BROWSE br-rattrs do:
  RUN re-move-clmnbr-rattrs ( 2, 9).
END.
ON ctrl-cursor-left OF BROWSE br-rattrs do:
  RUN re-move-clmnbr-rattrs (9, 2).
END.
PROCEDURE re-move-clmnbr-rattrs:
  ...
  END.
  if br-rattrs:MOVE-COLUMN(source-column, target-column) IN FRAME fr-D-rattr-0 then.
...
PROCEDURE ch-clmnbr-rattrs:
  ...
  END.
  RUN re-move-clmnbr-rattrs (cur-clmn-loc, 2).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-rattrs:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-rattrs = 2 to EXTENT(cur-clmn-numbr-rattrs):
...
on start-search, ctrl-o of br-rattrs in frame fr-D-rattr-0 do:
   run sort-brbr-rattrs
     (input (if available buf_c-trn-rsn-attr
             then recid(buf_c-trn-rsn-attr)
             else ?
            )
     ).
end.
PROCEDURE sort-brbr-rattrs :
  ...
  if re-querybr-rattrs = no then do:
    assign
       cur-clmnbr-rattrs = br-rattrs:current-column in frame fr-D-rattr-0
    .
  ...
  case sort-labelbr-rattrs:
        when 'Код'  then DO:    assign       sort-column-name = "buf_c-trn-rsn-attr.attr-code"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Значение'  then DO:    assign       sort-column-name = "buf_c-trn-rsn-attr.attr-value"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Изменил'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1usrfulnf&1, buf_c-trn-rsn-attr.corr-user-name)', chr(34))     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Действие'  then DO:   assign       sort-column-name = substitute('dynamic-function(&1ShowAction&1, buf_c-trn-rsn-attr.action)', chr(34))     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Дата корр.'  then DO:    assign       sort-column-name = "buf_c-trn-rsn-attr.corr-date"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Время'  then DO:    assign       sort-column-name = "STRING( buf_c-trn-rsn-attr.corr-time, 'HH:MM:SS':U )"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'Щепка'  then DO:    assign       sort-column-name = "buf_c-trn-rsn-attr.chip-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
        when 'БД'  then DO:    assign       sort-column-name = "buf_c-trn-rsn-attr.corr-user-db-num"     .     run OpenBr in this-procedure ( input yes, input no, input '':U ).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
  ...
  if p-recid <> ? then do:
    ...
    apply "value-changed" to br-rattrs in frame fr-D-rattr-0.
  end.
  apply "entry" to br-rattrs in frame fr-D-rattr-0.
END PROCEDURE.
...
PROCEDURE re-move-clmnbr-changes:
  ...
  END.
  if br-changes:MOVE-COLUMN(source-column, target-column) IN FRAME fr-D-rattr-0 then.
  if source-column > target-column THEN
...
do on error   undo Main-Block, leave Main-Block
  ...
  if lookup( p-mode, 'все,code,one':U ) = 0 then do:
  ...
  enable  b-mark when lookup( "b-mark":U,   p-bttns ) > 0 or p-bttns = "*"
          ...
          b-sch b-help b-quit  br-rattrs br-changes sch-code
  with frame fr-D-rattr-0.
  run OpenBr in this-procedure ( input yes, input no, input '':U ).
  hide mark-num in frame fr-D-rattr-0.
  hide  sch-num in frame fr-D-rattr-0.
  if p-rid-list <> "":U then do: reposition br-rattrs to recid v-doc-rec no-error. end.
  br-rattrs :set-repositioned-row( 5, "CONDITIONAL":U ).
  wait-for go of frame fr-D-rattr-0.
end.
hide frame fr-D-rattr-0 no-pause.
procedure OpenBr :
...
case p-mode :
  when 'все':U   then do:
    assign
    frame fr-D-rattr-0 :title = "История атрибутов оснований (причин) создания документа".
...
if p-open-query then do:
  ...
  if l-filter-open-19 = false then do:
    open query br-rattrs for each buf_c-trn-rsn-attr
      where  yes
      indexed-reposition
  .
      assign
  ...
  assign
    v-doc-rec = recid( buf_c-trn-rsn-attr )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-rattrs:handle:get-buffer-handle(1) = (buffer buf_c-trn-rsn-attr:handle) then do:
      assign
    ...
    .
      run fltopend_fltfindd in this-procedure  (
                          ...
                          ,input query br-rattrs:handle
                          ,input rowid(buf_c-trn-rsn-attr)
      ...
      assign
      parameter-2-19 = (if p-find-next then "true":u else "false":u )
      parameter-3-19 =  "for each buf_c-trn-rsn-attr"
      parameter-4-19 =
        (
          if (" yes " + " " + where-phrase-19) <> ""
          then " yes " + " " + where-phrase-19
  ...
  when 'code':U then do:
    ...
    assign frame fr-D-rattr-0 :title =
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-21 as character no-undo .
      ...
      parameter-4-21 =
        (
          if (" buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.chip-num    <= p-chip-num  " + " " + where-phrase-21) <> ""
          then  substitute('buf_c-trn-rsn-attr.reason-code = &1 and                         buf_c-trn-rsn-attr.chip-num    <= &2 ', p-reason-code, p-chip-num ) + " " + where-phrase-21
          else "true"
        )
      parameter-5-21 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-21 =
          (" buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.chip-num    <= p-chip-num  " + " " + where-phrase-21 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-rattrs:handle
  ...
  if l-filter-open-21 = false then do:
    open query br-rattrs for each buf_c-trn-rsn-attr
      where  buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.chip-num    <= p-chip-num
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-rattrs:handle:get-buffer-handle(1) = (buffer buf_c-trn-rsn-attr:handle) then do:
      ...
      parameter-4-21 =
        "where ":u +  substitute('buf_c-trn-rsn-attr.reason-code = &1 and                         buf_c-trn-rsn-attr.chip-num    <= &2 ', p-reason-code, p-chip-num ) + " ":u + where-phrase-21 + " ":u + p-find-condition + " " + ""
      parameter-5-21 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-21 = (if p-find-next then "true":u else "false":u )
      parameter-3-21 =  "for each buf_c-trn-rsn-attr"
      parameter-4-21 =
        (
          if (" buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.chip-num    <= p-chip-num  " + " " + where-phrase-21) <> ""
          then  substitute('buf_c-trn-rsn-attr.reason-code = &1 and                         buf_c-trn-rsn-attr.chip-num    <= &2 ', p-reason-code, p-chip-num ) + " " + where-phrase-21
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-23 as character no-undo .
      ...
      parameter-4-23 =
        (
          if (" buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.attr-code   = p-attr-code   and                         buf_c-trn-rsn-attr.chip-num    = p-chip-num " + " " + where-phrase-23) <> ""
          then  substitute('buf_c-trn-rsn-attr.reason-code = &1 and                         buf_c-trn-rsn-attr.attr-code   = &2&3&2  and                         buf_c-trn-rsn-attr.chip-num    = &4 ', p-reason-code, chr(34), p-attr-code, p-chip-num) + " " + where-phrase-23
          else "true"
        )
      parameter-5-23 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-23 =
          (" buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.attr-code   = p-attr-code   and                         buf_c-trn-rsn-attr.chip-num    = p-chip-num " + " " + where-phrase-23 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-rattrs:handle
  ...
  if l-filter-open-23 = false then do:
    open query br-rattrs for each buf_c-trn-rsn-attr
      where  buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.attr-code   = p-attr-code   and                         buf_c-trn-rsn-attr.chip-num    = p-chip-num
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if query br-rattrs:handle:get-buffer-handle(1) = (buffer buf_c-trn-rsn-attr:handle) then do:
      ...
      parameter-4-23 =
        "where ":u +  substitute('buf_c-trn-rsn-attr.reason-code = &1 and                         buf_c-trn-rsn-attr.attr-code   = &2&3&2  and                         buf_c-trn-rsn-attr.chip-num    = &4 ', p-reason-code, chr(34), p-attr-code, p-chip-num) + " ":u + where-phrase-23 + " ":u + p-find-condition + " " + ""
      parameter-5-23 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-23 = (if p-find-next then "true":u else "false":u )
      parameter-3-23 =  "for each buf_c-trn-rsn-attr"
      parameter-4-23 =
        (
          if (" buf_c-trn-rsn-attr.reason-code = p-reason-code and                         buf_c-trn-rsn-attr.attr-code   = p-attr-code   and                         buf_c-trn-rsn-attr.chip-num    = p-chip-num " + " " + where-phrase-23) <> ""
          then  substitute('buf_c-trn-rsn-attr.reason-code = &1 and                         buf_c-trn-rsn-attr.attr-code   = &2&3&2  and                         buf_c-trn-rsn-attr.chip-num    = &4 ', p-reason-code, chr(34), p-attr-code, p-chip-num) + " " + where-phrase-23
...
v-label-param =
 ...
 run proc-full-temp-changes in this-procedure (
                                             input  (buf_c-trn-rsn-attr.action = integer('1':U))
                                            ,input  (buf_c-trn-rsn-attr.action = integer('99':U))
                                            ,input  buffer  buf_c-trn-rsn-attr:handle
                                            ,input  'c-trn-rsn-attr':U
...
define input parameter p-filter-name as character no-undo .
  do with frame fr-D-rattr-0:
    if p-filter-name > "" then do:
      assign
        frame fr-D-rattr-0:title
          = frame fr-D-rattr-0:title + "   ФИЛЬТР: " + p-filter-name.
      .
      assign
...
procedure proc-find-code :
  ...
  if v-doc-rec <> ? then do:
    ...
    display sch-num with frame fr-D-rattr-0.
  end.
  else do:
    assign  sch-num = 0.
    hide    sch-num in frame fr-D-rattr-0.
  end.
  apply "ENTRY":U to sch-code in frame fr-D-rattr-0.
end procedure.
