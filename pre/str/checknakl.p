   ...
   do:
      ...
      do:
         find first buf_trn-doc no-lock where buf_trn-doc.doc-code = buf_utd.doc-code
            and buf_trn-doc.status_ = 'факт':U no-error .
         if not available (buf_trn-doc) then
         do:
...
do:
   ...
   do:
      ...
      v-mes = "Через ЭДО получен(ы) закрытый(е) УПД " + chr(10) + chr(10) +
         v-DocumenNumber + ", необходимо создать по УПД накладную(ые) и закрыть до статуса Факт." + chr(10) + chr(10) +
...
procedure schet-factur:
   ...
   find first buf_doc-attr no-lock where buf_doc-attr.attr-code = 'nsf':U and
      buf_doc-attr.attr-value = p-DocumentNumber no-error .
   if available (buf_doc-attr) then
   do:
      find first bf_doc-attr no-lock where bf_doc-attr.attr-code = 'dsf':U and
