...
assign
action = entry(1, p-parameter, chr(4))
p-doc-num = entry(2, p-parameter, chr(4))
i-obj-code = integer(entry(3, p-parameter, chr(4)))
no-error
.
...
do while true:
  ...
  if LOC-bar-code.unit-cli <> LOC-GOODS.UNIT-BASE then do:
    ...
    IF AVAIL buf_bar-code then do:
      find first buf_price-list no-lock where                             buf_price-list.doc-num    = ub.price-list.doc-num AND                             buf_price-list.b-code     = buf_bar-code.B-CODE AND                             buf_price-list.price-type = ""  no-error.                             if avail buf_price-list then do:                               assign                               FOR-PRICE = buf_price-list.PRICE-SALE                               plist-unit-cli = buf_bar-code.unit-cli.                               LEAVE.                             end.
    end.
  END.
...
define variable vss-include-info82 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont82 = yes then do:
    if integer( substring( tmp-str82, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str82, length( ncrsc-pfx ) + 1, length( tmp-str82 ) - length( ncrsc-pfx ) )
...
define variable vss-include-info83 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str83  as character no-undo.
  define variable tmp-num83  as character no-undo.
  define variable i83        as integer   no-undo.
  define variable sum83      as integer   no-undo.
  define variable len-code83 as integer   no-undo.
  define variable varcont83  as logical   initial yes no-undo.
  CASE ncrpg-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str83 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str83 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont83 = yes then do:
    if integer( substring( tmp-str83, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str83, length( ncrpg-pfx ) + 1, length( tmp-str83 ) - length( ncrpg-pfx ) )
        len-code83    = length( IBm-good-code )
      .
      define variable v-sum-char83 as character no-undo .
      assign
        sum83 = 0
      .
      do i83 = 1 to len-code83 by 2
      :
        assign
          v-sum-char83 = substr(IBm-good-code, len-code83 - i83 + 1, 1)
        .
        if v-sum-char83 < "0"
        or v-sum-char83 > "9"
        then do:
        ...
        assign
          sum83 = sum83 + integer(v-sum-char83)
        .
      end.
      if varcont83 = yes then do:
        assign
        ...
        if varcont83 = yes then do:
           if sum83 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum83 mod 10)
             .
           end.
        end.
      end.
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action85   as character no-undo .
  define variable v-printed85       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action85
    ,output v-printed85
    ) .
end.
if v-view-log = true
...
define variable vss-include-info86 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
then do:
  ...
  then do:
  define variable v-plu-pet92 as character no-undo .
  define buffer  bufpet_cd-plu92 for ub.cd-plu.
  for each bufpet_cd-plu92  where
          bufpet_cd-plu92.obj-type = 'маг':U
      and bufpet_cd-plu92.obj-code = abs(i-obj-code)
      and bufpet_cd-plu92.pos-type = 'MARIA':U
      and bufpet_cd-plu92.plu-type = 'топ':U
      AND bufpet_cd-plu92.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu92.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet92 = TRIM(string( bufpet_cd-plu92.plu-code, "X(40)":U ))
    v-plu-pet92 = substring(v-plu-pet92, length(v-plu-pet92) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code92 as character no-undo .
    assign
      maria-good-code92 = left-trim(ibm-good-code, '0')
      maria-good-code92 = fill('0', 14 - length(maria-good-code92)) + maria-good-code92
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code92, 1, 5) + chr(4) +
                                                    substring(maria-good-code92, 6, 14)
                                                  )
...
define variable v-found113 as logical no-undo .
define variable v-is-script113 as logical no-undo.
define variable v-fields-shift113 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet117 as character no-undo .
  define buffer  bufpet_cd-plu117 for ub.cd-plu.
  for each bufpet_cd-plu117  where
          bufpet_cd-plu117.obj-type = 'маг':U
      and bufpet_cd-plu117.obj-code = abs(i-obj-code)
      and bufpet_cd-plu117.pos-type = 'MARIA':U
      and bufpet_cd-plu117.plu-type = 'топ':U
      AND bufpet_cd-plu117.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu117.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet117 = TRIM(string( bufpet_cd-plu117.plu-code, "X(40)":U ))
    v-plu-pet117 = substring(v-plu-pet117, length(v-plu-pet117) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code117 as character no-undo .
    assign
      maria-good-code117 = left-trim(ibm-good-code, '0')
      maria-good-code117 = fill('0', 14 - length(maria-good-code117)) + maria-good-code117
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code117, 1, 5) + chr(4) +
                                                    substring(maria-good-code117, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable dflt-cd120 as character no-undo .
define variable vss-include-info121 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type121 as character no-undo .
define variable v-value-date121 as date no-undo .
define variable v-value-decimal121 as decimal no-undo .
define variable v-value-integer121 as INTEGER no-undo .
define variable v-value-logical121 AS LOGICAL no-undo .
define variable v-tth121 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd120
    ,output v-value-date121
    ,output v-value-decimal121
    ,output v-value-integer121
    ,output v-value-logical121
    ,output v-param-type121
    ,INPUT-OUTPUT table-handle v-tth121
    ) no-error .
delete object v-tth121 no-error.
...
define variable vss-include-info139 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str139  as character no-undo.
  define variable tmp-num139  as character no-undo.
  define variable i139        as integer   no-undo.
  define variable sum139      as integer   no-undo.
  define variable len-code139 as integer   no-undo.
  define variable varcont139  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str139 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str139 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont139 = yes then do:
    if integer( substring( tmp-str139, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str139, length( ncrsc-pfx ) + 1, length( tmp-str139 ) - length( ncrsc-pfx ) )
        len-code139    = length( v-tmpchar )
      .
      define variable v-sum-char139 as character no-undo .
      assign
        sum139 = 0
      .
      do i139 = 1 to len-code139 by 2
      :
        assign
          v-sum-char139 = substr(v-tmpchar, len-code139 - i139 + 1, 1)
        .
        if v-sum-char139 < "0"
        or v-sum-char139 > "9"
        then do:
        ...
        assign
          sum139 = sum139 + integer(v-sum-char139)
        .
      end.
      if varcont139 = yes then do:
        assign
        ...
        if varcont139 = yes then do:
           if sum139 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum139 mod 10)
             .
           end.
        end.
      end.
