...
procedure MainProc:
  ...
  no-lock:
    ...
    run ConectSocet (entry(1,entry(2, cash-desk.addr-path, chr(4)),":"),
                     ...
                     300,
                     no,
                     substitute ("Чтение данных по слипам с кассы &1. ",entry(2, cash-desk.addr-path, chr(4)))
                     ).
    if    mWebResp eq ""
       or OerrMsg  ne ""
    then do:
    ...
    do transaction:
      for each tt-chk-slip-head where
               tt-chk-slip-head.cash-num = m-cash-num:
    ...
    then do :
      do ii = 1 to num-entries(m-textCheq, chr(10)) :
        ...
        assign
          ...
          tt-chk-slip-string.str-value = entry(ii, m-textCheq, chr(10))
        .
      end .
    end .
  end .
