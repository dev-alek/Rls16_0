...
define query br-list for ub.doc-line except ,tt-goods except
.
define browse br-list query br-list no-lock display
    fncgele( buffer ub.doc-line )   @ inv-mark              column-label 'К' format "x(1)":U  if ub.doc-line.prt-OK then '*' else ''   @ prt-mark              column-label 'Ш' format "x(1)":U  ub.doc-line.artic                           column-label 'Артикул'  format "x(38)":U  tt-goods.gds-name                           column-label 'Имя ' format "x(150)":U  ub.doc-line.doc-qnty                           column-label 'Стало'
...
PROCEDURE sort-brbr-list :
  ...
  case sort-labelbr-list:
        when 'К'  then DO:   open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code , first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic.   . END.
        when 'Ш'  then DO:   open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code , first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic.   . END.
        when 'Артикул'  then DO:   open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code , first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic.   . END.
        when 'Имя '  then DO:   open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code , first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic.   . END.
        when 'Стало'  then DO:   open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code , first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic.   . END.
    otherwise do:
      open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code , first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic BY ub.doc-line.line-num.
...
if available cli-buf then do:
  ...
  find cli-buf where cli-buf.obj-code = input frame d-inv-doc t-doc.wrkr
                 and cli-buf.obj-type = 'чел':U no-lock no-error.
  assign v-ref-rec50 = ( if available cli-buf then recid( cli-buf ) else ? ).
  ref-rec = ( if available cli-buf then recid( cli-buf ) else ? ).
  release cli-buf.
  if not available cli-buf or ( NOT can-do( 'чел':U, cli-buf.obj-type ) ) then do:
    run ref/cli-all.w (  input parparentproc
                  ...
                  ,  input "lock-cli-type"
                  , output ref-list ) .
    assign v-ref-rec50 = integer( ref-list ).
    ref-rec = integer( ref-list ) .
    find cli-buf where recid (cli-buf) =
       ref-rec
       no-lock no-error.
...
do:
  ...
  then do:
    ASSIGN
      t-doc.fact-date :POPUP-MENU in frame d-inv-doc = MENU m-ed-date56 :HANDLE
      t-doc.fact-date :MENU-MOUSE in frame d-inv-doc = 3
    .
  end.
  define variable v-label-handle56 as handle no-undo .
  assign
    v-label-handle56 = t-doc.fact-date :side-label-handle in frame d-inv-doc
  .
...
do:
  ...
  then do:
    ASSIGN
      t-doc.shift-date :POPUP-MENU in frame d-inv-doc = MENU m-ed-date58 :HANDLE
      t-doc.shift-date :MENU-MOUSE in frame d-inv-doc = 3
    .
  end.
  define variable v-label-handle58 as handle no-undo .
  assign
    v-label-handle58 = t-doc.shift-date :side-label-handle in frame d-inv-doc
  .
...
define variable vss-include-info61 as character format "x(65)" no-undo initial "@(#)$Workfile$ Библиотека изменения размеров окна".
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input t-doc.doc-code ,
    ...
    assign
      dif-only = "all":U
    .
    tt-goods.gds-name:width     in browse br-list   = 40.
...
define variable p-type  as character no-undo.
  ...
  enable
    b-exit b-history b-arch b-sum-doc b-sum-goods b-help b-marks br-list b-lkp a-n-c b-notes b-unscn b-cnt
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input t-doc.doc-code ,
  ...
  display t-doc.tot-doc t-doc.tot-rubl t-doc.fact-base
          ...
          t-doc.fact-date
  with frame d-inv-doc.
  define variable v-ref-rec62   as recid no-undo .
  find cli-buf where cli-buf.obj-code = input frame d-inv-doc t-doc.wrkr
                 and cli-buf.obj-type = 'чел':U no-lock no-error.
  ...
  if parmode <> "no-query":U THEN DO:
    case dif-only:
      ...
      when "shortage" then do:
                open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code and ub.doc-line.fact-qnty < 0, first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic by ub.doc-line.line-num.
      end.
      when "surplus" then do:
                open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code and ub.doc-line.fact-qnty > 0, first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic.
      end.
        ...
        for each ub.doc-line no-lock where ub.doc-line.doc-code = t-doc.doc-code:
          ...
          if ub.doc-line.doc-qnty - ub.doc-line.fact-qnty ne v-qnty
            then v-rec-list = string (recid(ub.doc-line)) + "," + v-rec-list.
                ...
                open query br-list for each  ub.doc-line no-lock where       ub.doc-line.doc-code = t-doc.doc-code and lookup (string (recid (ub.doc-line)), v-rec-list) > 0, first tt-goods no-lock where       tt-goods.artic     = ub.doc-line.artic by ub.doc-line.line-num.
      end.
    end case.
    if line-rec <> ? then do:
...
procedure m-parts-1 :
  ...
  do transaction on error undo, return error return-value:
    ...
    if available ub.doc-line then do:
      run local-reclcinv in this-procedure ( input "update":U ).
      display fncgele( buffer ub.doc-line )   @ inv-mark              column-label 'К' format "x(1)":U  if ub.doc-line.prt-OK then '*' else ''   @ prt-mark              column-label 'Ш' format "x(1)":U  ub.doc-line.artic                           column-label 'Артикул'  format "x(38)":U  tt-goods.gds-name                           column-label 'Имя ' format "x(150)":U  ub.doc-line.doc-qnty                           column-label 'Стало' with browse br-list.
    end.
...
if available gds-list then do:
  ...
  define variable v-last69 as integer no-undo .
  find last gds-list use-index oi no-error.
  if available gds-list then do:
    v-last69 = gds-list.order-num .
  end.
  else do:
    v-last69 = 0 .
  end.
