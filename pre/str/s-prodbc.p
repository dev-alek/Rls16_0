...
if (incode = "" AND LOOKUP('сер':U, units.type) = 0) OR
   (NOT ub.shop.cd-parts-ser and LOOKUP('сер':U, ub.units.type) > 0 ) then do:
        ...
        then do:
  if ub.shop.cd-sc-base AND LOOKUP('вес':U, ub.units.type) > 0 and
...
define variable vss-include-info67 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont67 = yes then do:
    if integer( substring( tmp-str67, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str67, length( ncrsc-pfx ) + 1, length( tmp-str67 ) - length( ncrsc-pfx ) )
...
define variable vss-include-info68 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  CASE ncrpg-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str68 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str68 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
  ...
  if varcont68 = yes then do:
    if integer( substring( tmp-str68, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str68, length( ncrpg-pfx ) + 1, length( tmp-str68 ) - length( ncrpg-pfx ) )
        len-code68    = length( IBm-good-code )
      .
      ...
      :
        assign
          v-sum-char68 = substr(IBm-good-code, len-code68 - i68 + 1, 1)
        .
        if v-sum-char68 < "0"
        or v-sum-char68 > "9"
        then do:
      ...
      if varcont68 = yes then do:
        ...
        if varcont68 = yes then do:
           if sum68 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum68 mod 10)
             .
           end.
        end.
      end.
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action70   as character no-undo .
  define variable v-printed70       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action70
    ,output v-printed70
    ) .
end.
if v-view-log = true
...
define variable vss-include-info71 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
then do:
  ...
  then do:
  define variable v-plu-pet77 as character no-undo .
  define buffer  bufpet_cd-plu77 for ub.cd-plu.
  for each bufpet_cd-plu77  where
          bufpet_cd-plu77.obj-type = 'маг':U
      and bufpet_cd-plu77.obj-code = abs(i-obj-code)
      and bufpet_cd-plu77.pos-type = 'MARIA':U
      and bufpet_cd-plu77.plu-type = 'топ':U
      AND bufpet_cd-plu77.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu77.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet77 = TRIM(string( bufpet_cd-plu77.plu-code, "X(40)":U ))
    v-plu-pet77 = substring(v-plu-pet77, length(v-plu-pet77) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code77 as character no-undo .
    assign
      maria-good-code77 = left-trim(ibm-good-code, '0')
      maria-good-code77 = fill('0', 14 - length(maria-good-code77)) + maria-good-code77
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code77, 1, 5) + chr(4) +
                                                    substring(maria-good-code77, 6, 14)
                                                  )
...
define variable v-found98 as logical no-undo .
define variable v-is-script98 as logical no-undo.
define variable v-fields-shift98 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet102 as character no-undo .
  define buffer  bufpet_cd-plu102 for ub.cd-plu.
  for each bufpet_cd-plu102  where
          bufpet_cd-plu102.obj-type = 'маг':U
      and bufpet_cd-plu102.obj-code = abs(i-obj-code)
      and bufpet_cd-plu102.pos-type = 'MARIA':U
      and bufpet_cd-plu102.plu-type = 'топ':U
      AND bufpet_cd-plu102.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu102.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet102 = TRIM(string( bufpet_cd-plu102.plu-code, "X(40)":U ))
    v-plu-pet102 = substring(v-plu-pet102, length(v-plu-pet102) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code102 as character no-undo .
    assign
      maria-good-code102 = left-trim(ibm-good-code, '0')
      maria-good-code102 = fill('0', 14 - length(maria-good-code102)) + maria-good-code102
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code102, 1, 5) + chr(4) +
                                                    substring(maria-good-code102, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable v-found103 as logical no-undo .
define variable v-is-script103 as logical no-undo.
define variable v-fields-shift103 as integer no-undo .
   if can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 6 )
   or can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 13 )
   then do:
     v-fields-shift103 = - 1.
...
define variable dflt-cd105 as character no-undo .
define variable vss-include-info106 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type106 as character no-undo .
define variable v-value-date106 as date no-undo .
define variable v-value-decimal106 as decimal no-undo .
define variable v-value-integer106 as INTEGER no-undo .
define variable v-value-logical106 AS LOGICAL no-undo .
define variable v-tth106 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd105
    ,output v-value-date106
    ,output v-value-decimal106
    ,output v-value-integer106
    ,output v-value-logical106
    ,output v-param-type106
    ,INPUT-OUTPUT table-handle v-tth106
    ) no-error .
delete object v-tth106 no-error.
...
define variable vss-include-info124 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str124  as character no-undo.
  define variable tmp-num124  as character no-undo.
  define variable i124        as integer   no-undo.
  define variable sum124      as integer   no-undo.
  define variable len-code124 as integer   no-undo.
  define variable varcont124  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str124 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str124 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont124 = yes then do:
    if integer( substring( tmp-str124, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str124, length( ncrsc-pfx ) + 1, length( tmp-str124 ) - length( ncrsc-pfx ) )
        len-code124    = length( v-tmpchar )
      .
      define variable v-sum-char124 as character no-undo .
      assign
        sum124 = 0
      .
      do i124 = 1 to len-code124 by 2
      :
        assign
          v-sum-char124 = substr(v-tmpchar, len-code124 - i124 + 1, 1)
        .
        if v-sum-char124 < "0"
        or v-sum-char124 > "9"
        then do:
        ...
        assign
          sum124 = sum124 + integer(v-sum-char124)
        .
      end.
      if varcont124 = yes then do:
        assign
        ...
        if varcont124 = yes then do:
           if sum124 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum124 mod 10)
             .
           end.
        end.
      end.
