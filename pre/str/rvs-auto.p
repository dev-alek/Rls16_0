...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  cre_obj-list:
  do jj = 1 to num-entries( v-obj-list, chr(44) )
  :
    assign
      v-curr-obj = entry( jj, v-obj-list,  chr(44)   )
      v-obj-type = entry(  1, v-curr-obj, '*':U )
    .
    assign
      v-obj-code = integer( entry(  2, v-curr-obj, '*':U ) ) no-error
    .
    if error-status :error
      or v-obj-code = ?
      or v-obj-code <= 0
    ...
    if lookup( v-obj-type, 'маг,скл':U ) = 0 then do:
      run add-msg in this-procedure
        ( input true
