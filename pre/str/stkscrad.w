...
DO:
  ...
  if varadd = yes then do:
    oPEN QUERY BROWSE-1 FOR EACH tt-usrstko use-index level indexed-reposition.
    ...
    reposition BROWSE-1 to recid recid(tt-usrstko).
  end.
  assign par-chg = yes.
END.
...
DO:
  ...
  if available tt-usrstko then do:
    ...
    reposition BROWSE-1 to recid recid(bf_tt-usrstko).
    assign par-chg = yes.
  end.
END.
...
DO:
  ...
  if available tt-usrstko then do:
    ...
    if available bf_tt-usrstko then do:
     ...
     delete tt-usrstko.
     if BROWSE-1:delete-current-row() then.
     assign par-chg = yes.
  end.
END.
...
if available tt-usrstko then do:
  ...
  oPEN QUERY BROWSE-1 FOR EACH tt-usrstko use-index level indexed-reposition.
  reposition BROWSE-1 to recid varrec-id.
end.
END.
