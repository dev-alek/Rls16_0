...
if v-is-restaurant and v-is-null-price then.
    ...
    def var l-in-ov65 as logical no-undo .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjat in g#library
  ...
  if (ub.shop.in-ov and  l-in-ov65 ) then
              NEXT _b-g-p.
...
define variable vss-include-info85 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str85  as character no-undo.
  define variable tmp-num85  as character no-undo.
  define variable i85        as integer   no-undo.
  define variable sum85      as integer   no-undo.
  define variable len-code85 as integer   no-undo.
  define variable varcont85  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str85 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str85 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont85 = yes then do:
    if integer( substring( tmp-str85, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str85, length( ncrsc-pfx ) + 1, length( tmp-str85 ) - length( ncrsc-pfx ) )
        len-code85    = length( IBm-good-code )
      .
      define variable v-sum-char85 as character no-undo .
      assign
        sum85 = 0
      .
      do i85 = 1 to len-code85 by 2
      :
        assign
          v-sum-char85 = substr(IBm-good-code, len-code85 - i85 + 1, 1)
        .
        if v-sum-char85 < "0"
        or v-sum-char85 > "9"
        then do:
        ...
        assign
          sum85 = sum85 + integer(v-sum-char85)
        .
      end.
      if varcont85 = yes then do:
        assign
        ...
        if varcont85 = yes then do:
           if sum85 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum85 mod 10)
             .
           end.
        end.
      end.
...
define variable vss-include-info86 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont86 = yes then do:
    if integer( substring( tmp-str86, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str86, length( ncrpg-pfx ) + 1, length( tmp-str86 ) - length( ncrpg-pfx ) )
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action88   as character no-undo .
  define variable v-printed88       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action88
    ,output v-printed88
    ) .
end.
if v-view-log = true
...
define variable vss-include-info89 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
then do:
  ...
  then do:
  define variable v-plu-pet95 as character no-undo .
  define buffer  bufpet_cd-plu95 for ub.cd-plu.
  for each bufpet_cd-plu95  where
          bufpet_cd-plu95.obj-type = 'маг':U
      and bufpet_cd-plu95.obj-code = abs(i-obj-code)
      and bufpet_cd-plu95.pos-type = 'MARIA':U
      and bufpet_cd-plu95.plu-type = 'топ':U
      AND bufpet_cd-plu95.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu95.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet95 = TRIM(string( bufpet_cd-plu95.plu-code, "X(40)":U ))
    v-plu-pet95 = substring(v-plu-pet95, length(v-plu-pet95) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code95 as character no-undo .
    assign
      maria-good-code95 = left-trim(ibm-good-code, '0')
      maria-good-code95 = fill('0', 14 - length(maria-good-code95)) + maria-good-code95
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code95, 1, 5) + chr(4) +
                                                    substring(maria-good-code95, 6, 14)
                                                  )
...
define variable v-found116 as logical no-undo .
define variable v-is-script116 as logical no-undo.
define variable v-fields-shift116 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet119 as character no-undo .
  define buffer  bufpet_cd-plu119 for ub.cd-plu.
  for each bufpet_cd-plu119  where
          bufpet_cd-plu119.obj-type = 'маг':U
      and bufpet_cd-plu119.obj-code = abs(i-obj-code)
      and bufpet_cd-plu119.pos-type = 'MARIA':U
      and bufpet_cd-plu119.plu-type = 'топ':U
      AND bufpet_cd-plu119.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu119.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet119 = TRIM(string( bufpet_cd-plu119.plu-code, "X(40)":U ))
    v-plu-pet119 = substring(v-plu-pet119, length(v-plu-pet119) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code119 as character no-undo .
    assign
      maria-good-code119 = left-trim(ibm-good-code, '0')
      maria-good-code119 = fill('0', 14 - length(maria-good-code119)) + maria-good-code119
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code119, 1, 5) + chr(4) +
                                                    substring(maria-good-code119, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable v-found121 as logical no-undo .
define variable v-is-script121 as logical no-undo.
define variable v-fields-shift121 as integer no-undo .
   if can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 6 )
   or can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 13 )
   then do:
     v-fields-shift121 = - 1.
...
define variable dflt-cd123 as character no-undo .
define variable vss-include-info124 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type124 as character no-undo .
define variable v-value-date124 as date no-undo .
define variable v-value-decimal124 as decimal no-undo .
define variable v-value-integer124 as INTEGER no-undo .
define variable v-value-logical124 AS LOGICAL no-undo .
define variable v-tth124 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd123
    ,output v-value-date124
    ,output v-value-decimal124
    ,output v-value-integer124
    ,output v-value-logical124
    ,output v-param-type124
    ,INPUT-OUTPUT table-handle v-tth124
    ) no-error .
delete object v-tth124 no-error.
...
define variable vss-include-info142 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str142  as character no-undo.
  define variable tmp-num142  as character no-undo.
  define variable i142        as integer   no-undo.
  define variable sum142      as integer   no-undo.
  define variable len-code142 as integer   no-undo.
  define variable varcont142  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str142 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str142 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont142 = yes then do:
    if integer( substring( tmp-str142, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str142, length( ncrsc-pfx ) + 1, length( tmp-str142 ) - length( ncrsc-pfx ) )
        len-code142    = length( v-tmpchar )
      .
      define variable v-sum-char142 as character no-undo .
      assign
        sum142 = 0
      .
      do i142 = 1 to len-code142 by 2
      :
        assign
          v-sum-char142 = substr(v-tmpchar, len-code142 - i142 + 1, 1)
        .
        if v-sum-char142 < "0"
        or v-sum-char142 > "9"
        then do:
        ...
        assign
          sum142 = sum142 + integer(v-sum-char142)
        .
      end.
      if varcont142 = yes then do:
        assign
        ...
        if varcont142 = yes then do:
           if sum142 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum142 mod 10)
             .
           end.
        end.
      end.
