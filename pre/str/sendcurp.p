...
CASE p-pos-type:
  when 'MAGIA-XML':U then do:
    ...
    for each buf_cash-pay no-lock,
      ...
      if buf_cash-pay.status_ <> 'тек':U and action = 'U' and not p-selective then NEXT.
      if v-cp-is-use then do:
        ...
        if v-value <> '*':U
        and lookup(string(p-cash-num) + chr(44) + p-pos-type, chr(4)) = 0 then next _for.
      end.
...
define variable v-first as logical no-undo .
  FOR EACH for-cash-desk NO-LOCK WHERE
    ...
    :
    if for-cash-desk.pos-type <> 'MAGIA-XML':U
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка данных по валютам-оплатам'" + " &1")
...
then do:
  if for-cash-desk.pos-type = 'MAGIA-XML':U then do:
    run str/waitpxml.w ( replace( v-xml-file-name-path, "/", "\" ) + "xml",
                ...
                ( if action = 'U'
                  then ('Ждите - ' + 'добавление данных по валютам-оплатам')
                  else ('Ждите - ' + 'удаление данных по валютам-оплатам') ) +
...
define variable dflt-cd22 as character no-undo .
