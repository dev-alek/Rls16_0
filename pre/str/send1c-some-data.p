...
if not this-procedure:persistent then do:
  ...
  if v-es then do:
      run write-to-log in p-log-handle (substitute( "&1. &2&3&4", vss-workfile, v-rv, chr(10), v-esm)) .
      run garbcoll_clear in this-procedure .
...
:
  ...
  v-esys-id-list = "" .
  for each buf_ext-system no-lock where buf_ext-system.delivery-method = integer('11':U)
                                    and buf_ext-system.esys-have-export = yes
                                    and buf_ext-system.esys-db-num-exp = buf_sys-ctrl.db-num :
    v-esys-id-list = v-esys-id-list + string(buf_ext-system.esys-id) + chr(1) .
  end.
  v-esys-id-list = trim(v-esys-id-list, chr(1)) .
  if v-esys-id-list = ""
  then do :
  ...
  if ExpData1:esys-add-dump-data ( input p-data,
                                   ...
                                   ('+update' + chr(4) + entry(1,p-init-sec-tag,chr(4))  +
                                    (if num-entries(p-init-sec-tag,chr(4)) > 1
                                     then substitute("&1&2&3&4",
                                                     chr(4),
                                                     entry(2,p-init-sec-tag,chr(4))
                                                     )
                                     else "")) ) = false
  then do:
    undo _main, return error v-last-error-message .
