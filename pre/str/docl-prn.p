...
Assign l-col-type="CHARACTER" l-col-len=10 l-col-format= "X(10)"     l-col-lable="Тип документа".
  ...
  define variable c-for-doc-type as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[2] = true then DO:
        ...
        CREATE EDITOR L-2 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-for-doc-type IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME Doc:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
FOR EACH Doc-list No-LOCK
    ...
    for-cli-type = get-client(Doc-list.doc-code, Doc-list.doc-type).
  assign
  accum-count = accum-count + 1
  .
  if use-column[1]
  then  C-for-doc-code:screen-value = string(Doc-list.doc-code, entry(1, c-for-doc-code:private-data, chr(4))).
  if use-column[2]
  then  C-for-doc-type:screen-value = string(Doc-list.doc-type, entry(1, c-for-doc-type:private-data, chr(4))).
  if use-column[3]
  then  C-for-obj-type:screen-value = string(Doc-list.obj-type, entry(1, c-for-obj-type:private-data, chr(4))).
  if use-column[4]
  then  C-for-obj-code:screen-value = string(Doc-list.obj-code, entry(1, c-for-obj-code:private-data, chr(4))).
  if use-column[5]
  then  C-for-cli-type:screen-value = string(for-cli-type, entry(1, c-for-cli-type:private-data, chr(4))).
  if use-column[6]
  then  C-for-cli-code:screen-value = string(for-cli-code, entry(1, c-for-cli-code:private-data, chr(4))).
  if use-column[7]
  ...
  if use-column[1]
  then (reg-output(
                    string(Doc-list.doc-code, entry(1, c-for-doc-code:private-data, chr(4)))
  ...
  if use-column[2]
  then (reg-output(
                    string(Doc-list.doc-type, entry(1, c-for-doc-type:private-data, chr(4)))
                   ,c-for-doc-type:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[3]
  then (reg-output(
                    string(Doc-list.obj-type, entry(1, c-for-obj-type:private-data, chr(4)))
  ...
  if use-column[4]
  then (reg-output(
                    string(Doc-list.obj-code, entry(1, c-for-obj-code:private-data, chr(4)))
  ...
  if use-column[5]
  then (reg-output(
                    string(for-cli-type, entry(1, c-for-cli-type:private-data, chr(4)))
  ...
  if use-column[6]
  then (reg-output(
                    string(for-cli-code, entry(1, c-for-cli-code:private-data, chr(4)))
  ...
  skip.
  IF LAST(Doc-list.doc-code) then do:
  if use-column[1]
  then  C-FOR-doc-code:screen-value = string(fill14).
  if use-column[2]
  then  C-for-doc-type:screen-value = string(fill10).
  if use-column[3]
  ...
  if use-column[7]
  then  C-for-cli-name:screen-value = string(fill40).
        DISPLAY stream  PrnLibStream with frame Doc.       DOWN 1 stream PrnLibStream with frame Doc.
  if use-column[6]
  then  C-for-cli-code:screen-value = string(accum-count, entry(1, c-for-cli-code:private-data, chr(4))).
  if use-column[7]
  then  C-for-cli-name:screen-value = string( 'документов в спискe' , entry(1, c-for-cli-name:private-data, chr(4))).
    DISPLAY stream  PRnLibStream with frame Doc.                                      DOWN 1 stream PrnLibStream with frame Doc.
  end.
END.
...
FUNCTION get-client RETURNS CHARACTER
  ...
  if loc-doc-type = 'переоценка':U then do:
    assign
    for-cli-type = ""
    for-cli-code = ?
