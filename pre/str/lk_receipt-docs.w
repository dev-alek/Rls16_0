...
DO:
END.
ON CHOOSE OF b-errors IN FRAME d-utd
DO:
  if available X_utd
  then do :
...
END.
ON CHOOSE OF b-comment IN FRAME d-utd
DO:
  if available X_utd
  then do :
    row_utd = rowid (X_utd) .
    ...
    OPEN QUERY br-utd FOR EACH X_utd NO-LOCK INDEXED-REPOSITION.
    br-utd:refresh ().
...
DO:
    ...
    if f-sertif <> "" then
    do:
      enable B-write-sertif with frame d-utd .
    end.
    else
    do:
      disable B-write-sertif with frame d-utd .
    end.
    display
        F-sertif
        with frame d-utd .
...
assign
  v-str-recid24 = trim( string( recid( X_utd ) , "->>>>>>>>>>>9":U ) )
...
END.
ON choose OF b-trn-doc IN FRAME d-utd
DO:
...
DO:
  ...
  then do :
    ...
    then do :
      enable b-comment with frame d-utd .
    end .
    else do :
      disable b-comment with frame d-utd .
    end .
  end .
END.
...
assign
  v-str-recid31 = trim( string( recid( X_utd ) , "->>>>>>>>>>>9":U ) )
  v-num-entry31 = lookup( v-str-recid31 , v-rid-list )
.
if v-num-entry31 > 0 then do:
  assign
    entry( v-num-entry31, v-rid-list ) = "":U
    v-rid-list = trim( replace( v-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rid-list = v-rid-list + ( if v-rid-list = "":U then "":U else chr(44) ) + v-str-recid31
  .
end.
          loc#log = br-utd:refresh() .
      end.
...
DO:
  ...
  return no-apply .
END.
ON leave OF F-date-to IN FRAME d-utd
DO:
...
do:
  ...
  then do:
    ASSIGN
      f-date-from :POPUP-MENU in frame d-utd = MENU m-ed-date38 :HANDLE
      f-date-from :MENU-MOUSE in frame d-utd = 3
    .
  end.
  define variable v-label-handle38 as handle no-undo .
  assign
    v-label-handle38 = f-date-from :side-label-handle in frame d-utd
  .
...
do:
  ...
  then do:
    ASSIGN
      f-date-to :POPUP-MENU in frame d-utd = MENU m-ed-date40 :HANDLE
      f-date-to :MENU-MOUSE in frame d-utd = 3
    .
  end.
  define variable v-label-handle40 as handle no-undo .
  assign
    v-label-handle40 = f-date-to :side-label-handle in frame d-utd
  .
...
procedure get-context :
  ...
  apply "value-changed" to br-utd IN FRAME d-utd.
  WAIT-FOR GO OF FRAME d-utd focus br-utd .
END.
...
FUNCTION doc-type RETURNS CHARACTER
  ...
  for first bf_trn-doc no-lock where bf_trn-doc.doc-code = p-doc-code :
    case bf_trn-doc.ext-doc-type :
      when 'wm':U then return "Производство" .
      when 'vt':U then return "Инвентаризация" .
      when 'we':U then return "Списание" .
      otherwise do :
        return "?" .
      end .
    end case .
