...
DO:
  IF input frame Dialog-Frame varsec-num <= 0 THEN DO:
  ...
  END.
      if (par-neck = 0 and partype-AC = 1) OR
         ...
         (par-neck = 3 and input frame Dialog-Frame varsec-diam-depth <> '' and
         input frame Dialog-Frame varsec-diam-out <> '' and
         (parmode = 'ДОБАВЛЕНИЕ':U or parmode = 'ИЗМЕНЕНИЕ':U) AND
         input frame Dialog-Frame varsec-num <> 0 AND
         input frame Dialog-Frame varsec-qnty <> 0) OR
         (par-neck = 3 and input frame Dialog-Frame varsec-diam-depth <> '' and
         input frame Dialog-Frame varsec-diam-lenght <> '' AND
         (parmode = 'ДОБАВЛЕНИЕ':U or parmode = 'ИЗМЕНЕНИЕ':U) AND
         input frame Dialog-Frame varsec-num <> 0 AND
         input frame Dialog-Frame varsec-qnty <> 0) OR
         (par-neck = 3 and input frame Dialog-Frame varsec-diam-in <> '' and
         varsec-diam-depth = "" and varsec-diam-lenght = "" and varsec-diam-out = "" and
         (parmode = 'ДОБАВЛЕНИЕ':U or parmode = 'ИЗМЕНЕНИЕ':U) AND
         input frame Dialog-Frame varsec-num <> 0 AND
         input frame Dialog-Frame varsec-qnty <> 0)
         then
      do:
    end.
      else
  ...
  if parmode = 'ДОБАВЛЕНИЕ':U or
     ...
     case par-neck:
         ...
         if partype-AC = 1 then do:
            ...
            BROWSE-2
            with frame Dialog-Frame .
            OPEN QUERY BROWSE-2 FOR EACH tt-section exclusive-LOCK INDEXED-REPOSITION.
         end.
         else do:
            hide
            BROWSE-2
     ...
     enable varsec-num varsec-qnty b-save with frame Dialog-Frame.
     ...
     disable varsec-num varsec-qnty b-save with frame Dialog-Frame.
