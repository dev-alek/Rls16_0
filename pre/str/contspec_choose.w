...
DEFINE BROWSE spec-List
  ...
  get-b-code(tt_contract-specif.gds-code) @ p-bcode  COLUMN-LABEL 'Код'  Format "X(16)"
     tt_contract-specif.artic    COLUMN-LABEL 'Артикул'  Format "x(16)"
     string (tt_contract-specif.prod-type + ' ' + string(tt_contract-specif.prod-code)) @ p-prod  COLUMN-LABEL 'Произво-!дитель'  Format "x(18)"
     tt_contract-specif.gds-name    COLUMN-LABEL 'Наименование'  format "x(50)"
     tt_contract-specif.price-cli    COLUMN-LABEL 'Цена!поставщика'  format ">,>>>,>>>,>>9.99"
     tt_contract-specif.prc    COLUMN-LABEL '% Отклон в!большую сторону'  Format "->>>>9.99"
     f-prc-min(recid(tt_contract-specif))    COLUMN-LABEL '% Отклон в!меньшую сторону'  Format "->>>>9.99"
     tt_contract-specif.unit-base    COLUMN-LABEL 'Ед.!изм'
     tt_contract-specif.qnty    COLUMN-LABEL 'Количество'
     tt_contract-specif.cli-base-rate    COLUMN-LABEL '*'
     tt_contract-specif.sum-cli   COLUMN-LABEL 'Сумма'
     tt_contract-specif.vat-pc   COLUMN-LABEL 'НДС' Format ">>9.9"
     tt_contract-specif.vat-type   COLUMN-LABEL 'тип!НДС'
     tt_contract-specif.income-qnty   COLUMN-LABEL 'Принято'
     get-grp(tt_contract-specif.gds-code) @ p-grp COLUMN-LABEL 'Группа товара'  Format "x(100)"
     tt_contract-specif.unit-cli   COLUMN-LABEL 'Ед.!изм!пост'
     tt_contract-specif.unit-cli-ord   COLUMN-LABEL 'Ед.изм!пост в!заказе'
     tt_contract-specif.cli-base-rate-ord   COLUMN-LABEL 'Коэф.!в!заказе'
     tt_contract-specif.unit-cli-rcv   COLUMN-LABEL 'Ед.изм!пост в!поставке'
     tt_contract-specif.cli-base-rate-rcv   COLUMN-LABEL 'Коэф.!в!поставке'
     f-bonus(recid(tt_contract-specif))   COLUMN-LABEL '%!Бонус' Format "->>9.99"
     get-ext-artic(recid(tt_contract-specif))   COLUMN-LABEL 'Внешний!Артикул' Format "X(16)"
     enable tt_contract-specif.artic
...
assign
  v-str-recid17 = trim( string( recid( tt_contract-specif ) , "->>>>>>>>>>>9":U ) )
...
assign
  v-str-recid19 = trim( string( recid( tt_contract-specif ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry19 > 0 then do:
  ...
  .
      ...
      do:
        loc#log = spec-List:select-next-row ().
    ...
    do:
      ...
      do:
        for first buf_contract-specif no-lock where buf_contract-specif.contract-num = tt_contract-specif.contract-num and buf_contract-specif.gds-code = tt_contract-specif.gds-code:
          find first ub.contract no-lock where ub.contract.contract-code = buf_contract-specif.contract-num and ub.contract.status_ = 'тек':U no-error .
          create gds-list .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  assign
    ...
    tt_contract-specif.artic:read-only in browse spec-List = yes
    .
  find first ub.clients no-lock where
    ub.clients.obj-type = 'орг':U
    and ub.clients.obj-code = p-host-code .
  assign
    frame Dialog-Frame:title = substitute(" Фирма: (&1) &2 Товарная спецификация к договорам"
                                                  ,p-host-code
                                                  ,ub.clients.obj-name).
  run init-browse-p  in this-procedure  no-error .
  run myenable in this-procedure no-error .
  if error-status:error then  return .
  OPEN QUERY spec-List FOR EACH tt_contract-specif NO-LOCK indexed-reposition.
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-27 as character no-undo .
      assign
      parameter-3-27 =
                              "FOR EACH tt_contract-specif"
      parameter-4-27 =
        (
          if (" tt_contract-specif.host-code = p-host-code " + " " + where-phrase-27) <> ""
          then  substitute(' tt_contract-specif.host-code = &1 ', p-host-code )  + " " + where-phrase-27
          else "true"
        )
      parameter-5-27 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-27 =
          (" tt_contract-specif.host-code = p-host-code " + " " + where-phrase-27 = "")
      .
  ...
  if l-filter-open-27 = false then do:
    OPEN QUERY spec-List FOR EACH tt_contract-specif NO-LOCK
      where  tt_contract-specif.host-code = p-host-code
  .
      assign
        l-query-was-opened = true
      .
  ...
  on error undo, leave
  :
    if query spec-List:handle:get-buffer-handle(1) = (buffer tt_contract-specif:handle) then do:
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-4-27 =
        "where ":u +  substitute(' tt_contract-specif.host-code = &1 ', p-host-code )  + " ":u + where-phrase-27 + " ":u + p-find-condition + " " + ""
      parameter-5-27 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query spec-List:handle
                          ,input rowid(tt_contract-specif)
                          ,input logical(parameter-2-27)
                          ,input no-lock
                          ,input (buffer tt_contract-specif:handle)
                          ,input parameter-4-27
                          ,input parameter-5-27
                          ) no-error.
      .
      ...
      assign
      parameter-2-27 = (if p-find-next then "true":u else "false":u )
      parameter-3-27 =  "FOR EACH tt_contract-specif"
      parameter-4-27 =
        (
          if (" tt_contract-specif.host-code = p-host-code " + " " + where-phrase-27) <> ""
          then  substitute(' tt_contract-specif.host-code = &1 ', p-host-code )  + " " + where-phrase-27
    ...
    when 1 then
      do:
        ...
        do:
          ...
          loc#log = spec-List:select-next-row () in frame Dialog-Frame.
          reposition spec-List to rowid row_spec.
        end.
        apply "entry" to spec-List in frame Dialog-Frame.
      end.
