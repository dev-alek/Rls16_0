...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6':u,f-title,p-cond,parcli-type,parcli-code,parhost-code,parartic)
    .
end procedure.
...
ASSIGN FRAME d-chs-gds:SCROLLABLE = FALSE.
ON CHOOSE OF b-quit IN FRAME d-chs-gds  DO:
ref-list = "".
END.
...
ON return, MOUSE-SELECT-DBLCLICK OF goods.artic, goods.prod-type, goods.prod-code, ext-artic.ext-artic  IN FRAME d-chs-gds DO:
apply "choose" to b-exit in frame d-chs-gds.
return no-apply.
END.
ON CHOOSE OF b-exit IN FRAME d-chs-gds DO:
run check-goods no-error.
if error-status:error then RUN run-ref.
if ref-list = '' then do:   apply "entry" to goods.artic in frame d-chs-gds.   return no-apply. end. if  ref-list <> 'cb_create_gds-list_from-chs-gds' then do:   find goods where recid (goods) = integer (entry (1, ref-list)) no-lock.   assign  parartic     = goods.artic.   disp goods.prod-type goods.prod-code goods.artic with frame d-chs-gds. end. apply "go" to frame d-chs-gds.
END.
ON CHOOSE OF r-goods IN FRAME d-chs-gds DO:
RUN run-ref.
if ref-list = '' then do:   apply "entry" to goods.artic in frame d-chs-gds.   return no-apply. end. if  ref-list <> 'cb_create_gds-list_from-chs-gds' then do:   find goods where recid (goods) = integer (entry (1, ref-list)) no-lock.   assign  parartic     = goods.artic.   disp goods.prod-type goods.prod-code goods.artic with frame d-chs-gds. end. apply "go" to frame d-chs-gds.
END.
ON CHOOSE OF r-list IN FRAME d-chs-gds DO:
RUN run-list.
if ref-list = '' then do:   apply "entry" to goods.artic in frame d-chs-gds.   return no-apply. end. if  ref-list <> 'cb_create_gds-list_from-chs-gds' then do:   find goods where recid (goods) = integer (entry (1, ref-list)) no-lock.   assign  parartic     = goods.artic.   disp goods.prod-type goods.prod-code goods.artic with frame d-chs-gds. end. apply "go" to frame d-chs-gds.
END.
ON ctrl-cursor-down OF goods.artic, goods.prod-type, goods.prod-code IN FRAME d-chs-gds DO:
   RUN run-ref.
   if ref-list = '' then do:   apply "entry" to goods.artic in frame d-chs-gds.   return no-apply. end. if  ref-list <> 'cb_create_gds-list_from-chs-gds' then do:   find goods where recid (goods) = integer (entry (1, ref-list)) no-lock.   assign  parartic     = goods.artic.   disp goods.prod-type goods.prod-code goods.artic with frame d-chs-gds. end. apply "go" to frame d-chs-gds.
END.
ON alt-cursor-down OF goods.artic, goods.prod-type, goods.prod-code IN FRAME d-chs-gds DO:
   RUN run-list.
   if ref-list = '' then do:   apply "entry" to goods.artic in frame d-chs-gds.   return no-apply. end. if  ref-list <> 'cb_create_gds-list_from-chs-gds' then do:   find goods where recid (goods) = integer (entry (1, ref-list)) no-lock.   assign  parartic     = goods.artic.   disp goods.prod-type goods.prod-code goods.artic with frame d-chs-gds. end. apply "go" to frame d-chs-gds.
END.
...
if available clients then enable ext-artic.ext-artic with frame d-chs-gds.
                     else ext-artic.ext-artic:visible in frame d-chs-gds = no.
disp parartic     @ goods.artic
with frame d-chs-gds.
if input frame d-chs-gds goods.prod-type = "" or
   input frame d-chs-gds goods.prod-type = ?  or
   input frame d-chs-gds goods.prod-type = "?"
   then
   disp 'орг':U @ goods.prod-type with frame d-chs-gds.
WAIT-FOR GO OF FRAME d-chs-gds focus goods.artic.
END.
RUN disable_UI.
...
if can-find (clients where clients.obj-type = input frame d-chs-gds goods.prod-type
                                 and clients.obj-code = input frame d-chs-gds goods.prod-code no-lock) then do:
  find g-producer where
       g-producer.obj-type = input frame d-chs-gds goods.prod-type
   and g-producer.obj-code = input frame d-chs-gds goods.prod-code no-lock.
...
ref-list = ''.
if ext-artic.ext-artic:sensitive in frame d-chs-gds = yes and
   input frame d-chs-gds ext-artic.ext-artic <> ""        and
   input frame d-chs-gds ext-artic.ext-artic <> "?"       and
   input frame d-chs-gds ext-artic.ext-artic <> ?         then do:
   find first ext-artic where ext-artic.cli-type    = parcli-type
                          and ext-artic.cli-code    = parcli-code
                          and ext-artic.ext-artic   = input frame d-chs-gds ext-artic.ext-artic
                          and ext-artic.status_    <> 'удаленные':U
                          no-lock no-error.
   if available ext-artic then do:
      ...
      if available ( buf_ext-artic ) then do:
        message "С внешним артикулом :" input frame d-chs-gds ext-artic.ext-artic
                        "связано несколько товаров." skip (2)
                        "Укажите Производителя или выберите товар из справочника.".
        apply "entry" to goods.prod-code in frame d-chs-gds.
        return.
      end.
      ...
      display "" @ ext-artic.ext-artic with frame d-chs-gds.
      apply "entry" to goods.prod-code in frame d-chs-gds.
      return .
   end.
end.
if input frame d-chs-gds goods.artic = '' then return error.
find first goods where goods.artic  = input frame d-chs-gds goods.artic no-lock no-error.
if not available goods then do:
  message "Неправильный Артикул - такого товара нет.".
  apply "entry" to goods.artic in frame d-chs-gds.
  return.
end.
find first gds-b where gds-b.artic  = input frame d-chs-gds goods.artic
                   and recid (gds-b) <> recid (goods)
                   and gds-b.stts = 0 no-lock no-error.
if input frame d-chs-gds goods.prod-code <> 0 then do:
  find first goods where goods.prod-code   = input frame d-chs-gds goods.prod-code
                     and  goods.artic      = input frame d-chs-gds goods.artic no-lock no-error.
  if not available goods then do:
  ...
  find first gds-b where gds-b.artic     = input frame d-chs-gds goods.artic
                     and gds-b.prod-code = input frame d-chs-gds goods.prod-code
                     and recid (gds-b) <> recid (goods)
                     and gds-b.stts = 0 no-lock no-error.
end.
else do:
  if available gds-b then do:
    ...
    if not available g-producer or (available g-producer and not available goods) then do:
      message "С артикулом :" input frame d-chs-gds goods.artic
...
if input frame d-chs-gds goods.prod-code <> 0  and
   input frame d-chs-gds goods.prod-type <> "" then do:
   find goods where goods.prod-type = input frame d-chs-gds goods.prod-type and
                   goods.prod-code = input frame d-chs-gds goods.prod-code and
                   goods.artic     = input frame d-chs-gds goods.artic no-lock no-error.
   if not available goods then do:
    message "Неправильный Тип производителя - такого товара нет.".
    apply "entry" to goods.prod-type in frame d-chs-gds.
    return.
   end.
end.
else do:
...
v-nabor = false .
  if not ( p-list-mode begins 'is-flor':U ) then do:
    run ver-gds-grp-nabor( input p-gds-code, output v-nabor) .
  end.
  else do:
    if  p-stat <> 'запрос':U then run ver-gds-grp-nabor( input p-gds-code, output v-nabor) .
  end.
...
if p-stat = 'запрос':U and
   parext-doc-type = 'ee':U and
   ( p-list-mode begins 'is-flor':U )
   then return .
    if p-curr-obj-type = 'маг':U or p-curr-obj-type = 'скл':U then do:
     v-event-code = substitute("&1-" , parext-doc-type ) .
     if parcli-type = 'маг':U or parcli-type = 'скл':U then do:
     define variable v-c-host-code as integer   no-undo .
define variable vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  (input  parcli-type
  ,input  parcli-code
  ,output v-c-host-code
  )  .
...
if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run goassizt in g#library2
  ...
  if parext-doc-type <> 'ie':U and
     parext-doc-type <> 'iv':U and
    ( parcli-type = 'маг':U or parcli-type = 'скл':U) then do:
     define variable v-ot-host-code as integer   no-undo .
define variable vss-include-info12 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hostcode in g#library
  (input  parcli-type
  ,input  parcli-code
  ,output v-ot-host-code
  )  .
