...
define  temp-table stf-list no-undo like ub.staff
...
FIND FIRST ub.cash-desk NO-LOCK WHERE
           ...
           or
           ub.cash-desk.pos-type = 'IBM-XML':U) No-error.
...
do:
  ...
  run gbl/d-askw.w (input "Выбор кассиров для пересылки",
              input ( (if mode = "U"
                        ...
                        if g#db-num = 0
                        then
                        chr(10) + "информацию о кассирах"
                        else
                        chr(10) + "информацию о кассирах, заведенную в ГБД и текущей БД"
                        ),
              input "|",
