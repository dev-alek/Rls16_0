...
assign
    v-type-trn-doc = "rcv"
    v-shot-type-trn-doc = 'поставка':U
    v-sign-list    = "1"
.
if p-trn-doc <> ?  then do:
    for each temp-obj-firm on error undo, return error :
           for each buf_ord-doc-rcv no-lock where
                    ...
                    buf_ord-doc-rcv.status_    = 'факт':U                and
            ...
            on error undo, return error :
                  ...
                  for each tt-fin-ob where
                      ...
                      first buf_contract no-lock where buf_contract.contract-code = tt-fin-ob.contract-code and
                                                       ...
                                                       lookup( buf_contract.usl-opl ,  'По поставке заказа':U + "," + 'Отсрочка платежа по поставке заказа':U )   > 0
                            break
...
p-res = p-res  + chr(10)   +
       ...
       "Просмотрено поставок             :" + string(col-trn)           + chr(10) +
...
if v-may-be = false then do:
   p-res = p-res + chr(10) + substitute("По поставке &1 нельзя создавать ФО в этой БД" , buf_ord-doc-rcv.rcv-code ) .
   return .
end.
assign
...
.
 if  buf_contract.usl-opl = 'По заказу':U or buf_contract.usl-opl = 'По поставке заказа':U
   then v-date-pay = today  .
        ...
        if p-cons = 2 and available   buf_ord-doc-rcv then do:
            var-date = buf_ord-doc-rcv.fact-date + (if buf_contract.srok-opl <> ? then buf_contract.srok-opl else 0 ) .
...
.
 if  buf_contract.usl-opl = 'Предоплата':U  then v-date-pay = today  .
        ...
        if p-cons = 2 and available   buf_ord-doc-rcv then do:
            var-date = buf_ord-doc-rcv.doc-date .
            if var-date < today then var-date = today.
            v-date-pay = var-date.
        end.
   end.
 ...
 if p-cons = 2 then do:
      if available   buf_ord-doc-rcv
         then
         assign
           n-trn-doc-code       =  buf_ord-doc-rcv.doc-code
         .
         else  n-trn-doc-code       = ?   .
     end.
     else do:
       if pfo = true and available   buf_ord-doc-rcv
         then  assign
           n-trn-doc-code       =  buf_ord-doc-rcv.doc-code
         .
         else  n-trn-doc-code      = ?   .
