...
modetype = (if num-entries(p-parameter, chr(4) ) > 1
...
define variable vss-include-info5 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont5 = yes then do:
    if integer( substring( tmp-str5, 1, length( bc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = bc-pfx + substring( tmp-str5, length( bc-pfx ) + 1, length( tmp-str5 ) - length( bc-pfx ) )
...
END PROCEDURE.
define variable vss-include-info6 as character format "x(65)" no-undo initial "@(#)$Workfile: defc-gds.i $ $Revision: 47e5c2a27e63, 2885, rls $".
...
def var vss-include-info51 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
...
define variable varscales-pref-type53 as character no-undo.
varscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,output varscales-pref-type53
  ) no-error .
if varscales-pref = ? then do:
  assign
  varscales-pref = '21,23,25':U.
end.
define variable varpgscales-pref-type53 as character no-undo.
varpgscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'scpgpref':u
  ,input  '':u
  ...
  ,output varpgscales-pref-type53
  ) no-error .
...
define variable v-value-character56 as date no-undo .
...
define variable v-param-type60 as character no-undo .
define variable v-value-date60 as date no-undo .
define variable v-value-decimal60 as decimal no-undo .
define variable v-value-integer60 as INTEGER no-undo .
define variable v-value-logical60 AS LOGICAL no-undo .
define variable v-tth60 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ,input  abs(i-obj-code)
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd
    ,output v-value-date60
    ,output v-value-decimal60
    ,output v-value-integer60
    ,output v-value-logical60
    ,output v-param-type60
    ,INPUT-OUTPUT table-handle v-tth60
    ) no-error .
delete object v-tth60 no-error.
IF error-status:error then do:
  assign
  dflt-cd = ''.
end.
...
FIND FIRST ub.cash-desk No-LOCK WHERE
          ...
          (ub.cash-desk.pos-type = 'IBM':U OR
           ub.cash-desk.pos-type = 'IBM-XML':U OR
           ub.cash-desk.pos-type = 'IPC-Servis+':U OR
           ub.cash-desk.pos-type = 'NCR-GM':U OR
           ub.cash-desk.pos-type = 'NCR-AS@R':U OR
           ub.cash-desk.pos-type = 'MAGIA-XML':U or
           ub.cash-desk.pos-type = 'Autotank':U
           ) AND
...
if modetype = ? then do:
  ...
  if ModeType then do:
      ...
      run str/gds-list.w (input parparentproc, input ub.shop.host-code, input 'маг':U, input abs(i-obj-code)).
...
if v-is-restaurant and v-is-null-price then.
    ...
    def var l-in-ov67 as logical no-undo .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjat in g#library
  ...
  if (ub.shop.in-ov and  l-in-ov67 ) then
              NEXT _b-g-p.
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute("Удалены товары c касс объекта &1&2", 'маг':U, i-obj-code)
                                          ).
run finish-send in this-procedure no-error .
...
define variable vss-include-info87 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont87 = yes then do:
    if integer( substring( tmp-str87, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str87, length( ncrsc-pfx ) + 1, length( tmp-str87 ) - length( ncrsc-pfx ) )
...
define variable vss-include-info88 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str88  as character no-undo.
  define variable tmp-num88  as character no-undo.
  define variable i88        as integer   no-undo.
  define variable sum88      as integer   no-undo.
  define variable len-code88 as integer   no-undo.
  define variable varcont88  as logical   initial yes no-undo.
  CASE ncrpg-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str88 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str88 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont88 = yes then do:
    if integer( substring( tmp-str88, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str88, length( ncrpg-pfx ) + 1, length( tmp-str88 ) - length( ncrpg-pfx ) )
        len-code88    = length( IBm-good-code )
      .
      define variable v-sum-char88 as character no-undo .
      assign
        sum88 = 0
      .
      do i88 = 1 to len-code88 by 2
      :
        assign
          v-sum-char88 = substr(IBm-good-code, len-code88 - i88 + 1, 1)
        .
        if v-sum-char88 < "0"
        or v-sum-char88 > "9"
        then do:
        ...
        assign
          sum88 = sum88 + integer(v-sum-char88)
        .
      end.
      if varcont88 = yes then do:
        assign
        ...
        if varcont88 = yes then do:
           if sum88 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum88 mod 10)
             .
           end.
        end.
      end.
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action90   as character no-undo .
  define variable v-printed90       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action90
    ,output v-printed90
    ) .
end.
if v-view-log = true
...
define variable vss-include-info91 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
then do:
  ...
  then do:
  define variable v-plu-pet97 as character no-undo .
  define buffer  bufpet_cd-plu97 for ub.cd-plu.
  for each bufpet_cd-plu97  where
          bufpet_cd-plu97.obj-type = 'маг':U
      and bufpet_cd-plu97.obj-code = abs(i-obj-code)
      and bufpet_cd-plu97.pos-type = 'MARIA':U
      and bufpet_cd-plu97.plu-type = 'топ':U
      AND bufpet_cd-plu97.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu97.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet97 = TRIM(string( bufpet_cd-plu97.plu-code, "X(40)":U ))
    v-plu-pet97 = substring(v-plu-pet97, length(v-plu-pet97) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code97 as character no-undo .
    assign
      maria-good-code97 = left-trim(ibm-good-code, '0')
      maria-good-code97 = fill('0', 14 - length(maria-good-code97)) + maria-good-code97
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code97, 1, 5) + chr(4) +
                                                    substring(maria-good-code97, 6, 14)
                                                  )
...
define variable v-found118 as logical no-undo .
define variable v-is-script118 as logical no-undo.
define variable v-fields-shift118 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet122 as character no-undo .
  define buffer  bufpet_cd-plu122 for ub.cd-plu.
  for each bufpet_cd-plu122  where
          bufpet_cd-plu122.obj-type = 'маг':U
      and bufpet_cd-plu122.obj-code = abs(i-obj-code)
      and bufpet_cd-plu122.pos-type = 'MARIA':U
      and bufpet_cd-plu122.plu-type = 'топ':U
      AND bufpet_cd-plu122.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu122.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet122 = TRIM(string( bufpet_cd-plu122.plu-code, "X(40)":U ))
    v-plu-pet122 = substring(v-plu-pet122, length(v-plu-pet122) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code122 as character no-undo .
    assign
      maria-good-code122 = left-trim(ibm-good-code, '0')
      maria-good-code122 = fill('0', 14 - length(maria-good-code122)) + maria-good-code122
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code122, 1, 5) + chr(4) +
                                                    substring(maria-good-code122, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable v-found123 as logical no-undo .
define variable v-is-script123 as logical no-undo.
define variable v-fields-shift123 as integer no-undo .
   if can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 6 )
   or can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 13 )
   then do:
     v-fields-shift123 = - 1.
...
define variable dflt-cd125 as character no-undo .
define variable vss-include-info126 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type126 as character no-undo .
define variable v-value-date126 as date no-undo .
define variable v-value-decimal126 as decimal no-undo .
define variable v-value-integer126 as INTEGER no-undo .
define variable v-value-logical126 AS LOGICAL no-undo .
define variable v-tth126 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd125
    ,output v-value-date126
    ,output v-value-decimal126
    ,output v-value-integer126
    ,output v-value-logical126
    ,output v-param-type126
    ,INPUT-OUTPUT table-handle v-tth126
    ) no-error .
delete object v-tth126 no-error.
...
define variable vss-include-info144 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str144  as character no-undo.
  define variable tmp-num144  as character no-undo.
  define variable i144        as integer   no-undo.
  define variable sum144      as integer   no-undo.
  define variable len-code144 as integer   no-undo.
  define variable varcont144  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str144 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str144 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont144 = yes then do:
    if integer( substring( tmp-str144, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str144, length( ncrsc-pfx ) + 1, length( tmp-str144 ) - length( ncrsc-pfx ) )
        len-code144    = length( v-tmpchar )
      .
      define variable v-sum-char144 as character no-undo .
      assign
        sum144 = 0
      .
      do i144 = 1 to len-code144 by 2
      :
        assign
          v-sum-char144 = substr(v-tmpchar, len-code144 - i144 + 1, 1)
        .
        if v-sum-char144 < "0"
        or v-sum-char144 > "9"
        then do:
        ...
        assign
          sum144 = sum144 + integer(v-sum-char144)
        .
      end.
      if varcont144 = yes then do:
        assign
        ...
        if varcont144 = yes then do:
           if sum144 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum144 mod 10)
             .
           end.
        end.
      end.
