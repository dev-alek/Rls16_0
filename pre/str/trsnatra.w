...
define button   Btn_History label "Истори&я" size-chars 10.00 by 1.00 default.
...
define frame fr-D-pump-8
  ub.trn-rsn-attr.attr-code  at row  1.25 col 20.50 colon-aligned view-as fill-in size-chars 25.50 by 1.00 label "&Код"      format "x(24)":U
  ub.trn-rsn-attr.attr-value at row  2.50 col 20.50 colon-aligned view-as fill-in size-chars 67.25 by 1.00 label "&Значение" format "x(80)":U
  r-rect-0            at row  3.75 col  1.50
    Btn_Exit          at row  4.00 col  2.50
    Btn_History       at row  4.00 col 67.75
  b-help         at row  4.00 col 78.00
...
then do:
  ...
  if p-mode = 'ПРОСМОТР':U then do: return. end.
  if ub.trn-rsn-attr.attr-code  :sensitive in frame fr-D-pump-8 then do: assign ub.trn-rsn-attr.attr-code.  end.
  if ub.trn-rsn-attr.attr-value :sensitive in frame fr-D-pump-8 then do: assign ub.trn-rsn-attr.attr-value. end.
...
on choose of Btn_History in frame fr-D-pump-8 do:
...
do on error   undo Main-Block, leave Main-Block
  ...
  enable Btn_Exit Btn_History b-help with frame fr-D-pump-8.
  if           p-mode = 'ДОБАВЛЕНИЕ':U then do:
    assign frame fr-D-pump-8 :title = "Карточка атрибута основания (причины) создания документа  -- " + p-mode.
    create ub.trn-rsn-attr.
    assign ub.trn-rsn-attr.reason-code = buf_refer.reason-code.
    assign ub.trn-rsn-attr.attr-code :fgcolor = 0.
    enable ub.trn-rsn-attr.attr-code Btn_OK with frame fr-D-pump-8.
    assign p-rec-id = recid( ub.trn-rsn-attr ).
  end. else if p-mode = 'ИЗМЕНЕНИЕ':U  then do:
    find ub.trn-rsn-attr exclusive-lock where recid( ub.trn-rsn-attr ) = p-rec-id.
    assign frame fr-D-pump-8 :title =
    ...
    enable Btn_OK with frame fr-D-pump-8.
  end. else if p-mode = 'ПРОСМОТР':U  then do:
    assign frame fr-D-pump-8 :title =
      substitute( 'Карточка атрибута "&1" основания (причины) создания документа  -- &2', ub.trn-rsn-attr.attr-code, p-mode ).
    assign ub.trn-rsn-attr.attr-code  :fgcolor = 4
  ...
  if p-mode <> 'ПРОСМОТР':U then do: enable ub.trn-rsn-attr.attr-value with frame fr-D-pump-8. end.
