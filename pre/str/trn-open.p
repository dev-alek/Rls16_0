...
assign varlns-cnt = 0 .
run waitfram-show in this-procedure (substitute("Переход документа в статус &1&2.", varstatus, (if varstatus = 'факт':U then '' else (if varflag = yes then "+" else "-")))).
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run hold-doc in g#library
  ...
  when 'при':U then do:
     if not bf_trn-doc.internal                          and
        lookup (bf_trn-doc.status_, 'накл,запрос':U) > 0 then do:
  ...
  when 'возврат':U    then do:
    if bf_trn-doc.status_ =  'разрешен':U and
    ...
    if bf_trn-doc.status_ <> 'разрешен':U then do:
      run waitfram-show in this-procedure (substitute ("Очистка документа. Время: &1", string (time - vartime, "hh:mm:ss"))) no-error.
      run clear-fact in this-procedure no-error.
      if error-status:error then do:
        undo, return error return-value.
    ...
    if bf_trn-doc.status_ = 'накл':U      OR
       bf_trn-doc.status_ = 'запрос':U   OR
...
run adm/shattri.p (
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
    ...
    if bf_trn-doc.status_ = 'накл':U or
       (bf_trn-doc.status_ = 'разрешен':U and (v-fo-gen = 4 or v-fo-gen = 5))
       then do :
        ...
        for each del_fin-ob-trn where del_fin-ob-trn.trn-doc-code = bf_trn-doc.doc-code  and
                ...
                if del_fin-ob.status_ <> 'факт':U  and v-del-fo then do :
                  assign
                ...
                else if del_fin-ob.status_ = 'факт':U then do :
...
( input  bf_doc-line.doc-code
 ...
 ,input  0
 ,input  0
 ,input  bf_doc-line.doc-qnty * bf_doc-line.doc-density
