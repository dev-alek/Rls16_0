...
define variable varscales-pref7 as character no-undo .
define variable varpgscales-pref7 as character no-undo .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,input  '':u
  ,input  no
  ,output varscales-pref7
  ,output varscales-pref-type7
  ) no-error .
  if varscales-pref7 = ? then do:
    assign
      varscales-pref7 = '21,23,25':U.
  end.
varpgscales-pref7  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'scpgpref':u
  ...
  ,input  '':u
  ,input  no
  ,output varpgscales-pref7
  ,output varpgscales-pref-type7
  ) no-error .
  if varpgscales-pref7 = ? then do:
    assign
      varpgscales-pref7 = '24IIIIIQQ000C,28IIIIIQQQ00C':U.
  end.
...
,input  NO
,input  YES
,input  varscales-pref7
,input  varpgscales-pref7
,output varresult
,output vartype-bc
,output varweight
,buffer buf_bar-code
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_cashdesk-sellers_update':U
    ,input  'object':U
    ,input  v-host-code
    ,input  p-curr-obj-type
    ,input  p-curr-obj-code
...
run ref/staffs.w (
                  input parparentproc
                , input "b-sel,b-mark"
                , input (if choice = 2 then 'S':U else 'C':U)
                , input v-db-num
                , input 0
                , output varrid-list ) .
...
if error-status:error then do:                           find first pos_cd-clu no-lock where                                   recid(pos_cd-clu) = loc-doc-rec no-error .                             message                             "Невозможно позиционироваться на записи ПЕРСОНАЛА" skip                            string(if avail pos_cd-clu                                     then  substitute("Идентификатор: &1, название &2"                                                     , pos_cd-clu.clu-code                                                      , pos_cd-clu.charkey_one)                                     else "":U) skip                             "Запись была добавлена (или изменена или удалена) -" skip                             "и теперь не попадает в текущую выборку"                             view-as alert-box WARNING.                           end.
apply "entry" to br-rkep-cli in frame Dialog-Frame.
apply "value-changed" to br-rkep-cli in frame Dialog-Frame.
END.
...
assign
  v-str-recid19 = trim( string( recid( X_cd-clu ) , "->>>>>>>>>>>9":U ) )
...
DO:
  run proc-find-id in this-procedure (input yes, input frame Dialog-Frame sch-id) no-error.
  if error-status:error then return no-apply.
END.
ON RETURN OF sch-id IN FRAME Dialog-Frame
DO:
  run proc-find-id in this-procedure (input no, input frame Dialog-Frame sch-id) no-error.
  if error-status:error then return no-apply.
END.
...
DO:
  ...
  IF ERROR-STATUS:ERROR  THEN RETURN NO-APPLY.
END.
ON VALUE-CHANGED OF T-role IN FRAME Dialog-Frame
DO:
  ASSIGN
  t-role
  .
  case t-role :
      WHEN YES THEN DISABLE b-chg WITH FRAME Dialog-Frame.
      WHEN no THEN enable b-chg WITH FRAME Dialog-Frame.
  END CASE.
  RUN openbr IN THIS-PROCEDURE ( input  YES, input NO, input '':U) NO-ERROR.
  IF ERROR-STATUS:ERROR  THEN RETURN NO-APPLY.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-rkep-cli :handle
      .
...
def var sort-labelBR-rkep-cli   as character no-undo .
def var sort-clmnBR-rkep-cli    as handle    no-undo .
def var cur-clmnBR-rkep-cli     as handle    no-undo .
def var cur-clmn-locBR-rkep-cli as integer   no-undo .
def var re-queryBR-rkep-cli     as logical   initial no no-undo .
on start-search, ctrl-o of BR-rkep-cli in frame Dialog-Frame do:
   run sort-brBR-rkep-cli
...
PROCEDURE sort-brBR-rkep-cli :
  define input parameter p-recid as recid no-undo .
  if re-queryBR-rkep-cli = no then do:
    assign
       cur-clmnBR-rkep-cli = BR-rkep-cli:current-column in frame Dialog-Frame
    .
    if sort-clmnBR-rkep-cli <> ? then sort-clmnBR-rkep-cli:column-fgcolor = 0.
    if cur-clmnBR-rkep-cli = sort-clmnBR-rkep-cli then do:
      assign
         sort-labelBR-rkep-cli = ""
         sort-clmnBR-rkep-cli = ?
      .
     end.
     else do:
       assign
         sort-labelBR-rkep-cli = cur-clmnBR-rkep-cli:label
         sort-clmnBR-rkep-cli  = cur-clmnBR-rkep-cli
         sort-clmnBR-rkep-cli:column-fgcolor = 4
       .
     end.
   end.
  assign
    cur-clmn-locBR-rkep-cli = 1
  .
  def var column-handle as handle no-undo .
  column-handle = BR-rkep-cli:first-column.
  do while valid-handle(column-handle) :
    if column-handle = cur-clmnBR-rkep-cli then do:
      leave .
    end.
    column-handle = column-handle:NEXT-COLUMN.
    assign
      cur-clmn-locBR-rkep-cli = cur-clmn-locBR-rkep-cli + 1
    .
  end.
  case sort-labelBR-rkep-cli:
        when v-id  then DO:    assign       sort-column-name = "X_cd-clu.clu-code"     .     run OpenBr in this-procedure (input  yes, input no, input '':U).   . END.
        when X_cd-clu.charkey_one:label in browse BR-rkep-cli then DO:    assign       sort-column-name = "X_cd-clu.charkey_one"     .     run OpenBr in this-procedure (input  yes, input no, input '':U).   . END.
    otherwise do:
      assign
        sort-column-name = ""
      .
      run OpenBr in this-procedure ( input yes, input no, input '':U).
        if can-do( this-procedure:internal-entries, 'mv-brw-defaultBR-rkep-cli') then do:
          run mv-brw-defaultBR-rkep-cli.
        end.
      if sort-labelBR-rkep-cli <> "" then do:
        assign
          cur-clmnBR-rkep-cli:column-fgcolor = 0
        .
      end.
      assign
        cur-clmn-locBR-rkep-cli = ?
      .
    end.
  end case.
    if cur-clmn-locBR-rkep-cli <> ? then do:
      if can-do( this-procedure:internal-entries, 'ch-clmnBR-rkep-cli') then do:
        run ch-clmnBR-rkep-cli in this-procedure (cur-clmn-locBR-rkep-cli).
      end.
    end.
  if p-recid <> ? then do:
    reposition BR-rkep-cli to recid p-recid no-error.
    apply "value-changed" to BR-rkep-cli in frame Dialog-Frame.
  end.
  apply "entry" to BR-rkep-cli in frame Dialog-Frame.
END PROCEDURE.
procedure re-open-query-srt-clmnBR-rkep-cli:
if cur-clmnBR-rkep-cli = ? then do:
   run OpenBr in this-procedure ( input yes, input no, input '':U).
end.
else do:
   assign re-queryBR-rkep-cli = yes.
   run sort-brBR-rkep-cli
     (input (if available X_cd-clu
             then recid(X_cd-clu)
             else ?
   ...
   assign re-queryBR-rkep-cli = no.
end.
end.
define variable vss-include-info26 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
do:
  BR-rkep-cli :SET-REPOSITIONED-ROW(5, "CONDITIONAL") .
end.
...
DEF VAR cur-clmn-numbr-rkep-cli as INT EXTENT 9 no-undo.
DEF VAR varmvibr-rkep-cli       as INT no-undo.
DEF VAR varmvjbr-rkep-cli       as INT no-undo.
DEF VAR varmvkbr-rkep-cli       as INT no-undo.
DEF VAR varmvlbr-rkep-cli       as INT no-undo.
DEF VAR move-elementbr-rkep-cli as INT no-undo.
def var jjbr-rkep-cli           as int no-undo.
do varmvibr-rkep-cli = 1 to EXTENT(cur-clmn-numbr-rkep-cli):
  ASSIGN cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = varmvibr-rkep-cli.
END.
RUN start-mv-clmnbr-rkep-cli.
PROCEDURE start-mv-clmnbr-rkep-cli:
def var old-session as logical no-undo.
   old-session = SESSION:IMMEDIATE-DISPLAY.
   IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
       IF  true  THEN DO:
   DO jjbr-rkep-cli = NUM-ENTRIES('1,2,3,4,5,6,7,8,9') TO 1 BY -1:
     RUN re-move-clmnbr-rkep-cli ( cur-clmn-numbr-rkep-cli[INTEGER(ENTRY (jjbr-rkep-cli, '1,2,3,4,5,6,7,8,9'))] , 2).
   END.
       END.
   SESSION:IMMEDIATE-DISPLAY = old-session.
END.
ON ctrl-cursor-right OF BROWSE br-rkep-cli do:
  RUN re-move-clmnbr-rkep-cli ( 2, 9).
END.
ON ctrl-cursor-left OF BROWSE br-rkep-cli do:
  RUN re-move-clmnbr-rkep-cli (9, 2).
END.
PROCEDURE re-move-clmnbr-rkep-cli:
  DEFINE INPUT PARAMETER source-column as INTEGER NO-UNDO.
  DEFINE INPUT PARAMETER target-column as INTEGER NO-UNDO.
  DO varmvibr-rkep-cli = 1 TO EXTENT(cur-clmn-numbr-rkep-cli):
    if cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = source-column THEN cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = -1.
  END.
  if br-rkep-cli:MOVE-COLUMN(source-column, target-column) IN FRAME Dialog-Frame then.
  if source-column > target-column THEN
  DO varmvjbr-rkep-cli = source-column - 1 to target-column BY -1:
    DO varmvibr-rkep-cli = 1 TO EXTENT(cur-clmn-numbr-rkep-cli):
        if cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = varmvjbr-rkep-cli THEN DO:
          cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] + 1.
        END.
    END.
  END.
  ELSE
  DO varmvjbr-rkep-cli = source-column + 1 to target-column:
    DO varmvibr-rkep-cli = 1 TO EXTENT(cur-clmn-numbr-rkep-cli):
      if cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = varmvjbr-rkep-cli THEN DO:
        cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] - 1.
      END.
  ...
  END.
  DO varmvibr-rkep-cli = 1 TO EXTENT(cur-clmn-numbr-rkep-cli):
    if cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = -1 THEN cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = target-column.
  END.
END PROCEDURE.
PROCEDURE ch-clmnbr-rkep-cli:
  DEFINE INPUT PARAMETER cur-clmn-loc as INTEGER NO-UNDO.
  if cur-clmn-loc <= 2 then do:
    return .
  end.
  DO varmvibr-rkep-cli = 1 TO EXTENT(cur-clmn-numbr-rkep-cli):
    if cur-clmn-numbr-rkep-cli[varmvibr-rkep-cli] = cur-clmn-loc THEN move-elementbr-rkep-cli = varmvibr-rkep-cli.
  END.
  RUN re-move-clmnbr-rkep-cli (cur-clmn-loc, 2).
END PROCEDURE.
PROCEDURE mv-brw-defaultbr-rkep-cli:
def var old-session as logical no-undo.
  old-session = SESSION:IMMEDIATE-DISPLAY.
  IF old-session = YES THEN SESSION:IMMEDIATE-DISPLAY = NO.
  do varmvlbr-rkep-cli = 2 to EXTENT(cur-clmn-numbr-rkep-cli):
    RUN re-move-clmnbr-rkep-cli (cur-clmn-numbr-rkep-cli[varmvlbr-rkep-cli], varmvlbr-rkep-cli).
  END.
  RUN start-mv-clmnbr-rkep-cli.
  SESSION:IMMEDIATE-DISPLAY = old-session.
END PROCEDURE.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-rkep-cli FOR EACH X_cd-clu NO-LOCK,             FIRST X_staff OUTER-JOIN NO-LOCK where X_staff.psn-code = X_cd-clu.cli-code         AND  X_staff.role = entry(lookup(X_cd-clu.clu-type,        v-rkep-cli-role-list),        v-th-role-list)         and X_staff.db-num = v-db-num         and X_staff.date-end = 12/31/9999,            FIRST X_clients OUTER-JOIN NO-LOCK WHERE X_clients.obj-type = 'чел':U         AND X_clients.obj-code = X_staff.psn-code      INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE MyEnable :
assign
...
br-rkep-cli:num-locked-columns in frame Dialog-Frame = 1
...
t-role = logical(entry(2, p-status, chr(4)))
.
rs-sch = "id":U.
DISPLAY
rs-mode
...
title0 = "Справочник персонала на кассе R-KEEPER" + chr(32).
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-36 as character no-undo .
      ...
      parameter-4-36 =
        (
          if (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code and X_cd-clu.pos-type = 'r-keeper':U " + " " + where-phrase-36) <> ""
          then  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3 and X_cd-clu.pos-type = &1&4&1 '                              , chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " " + where-phrase-36
          else "true"
        )
      parameter-5-36 = (" " + "" + " " +  substitute(', FIRST X_staff outer-join NO-LOCK WHERE X_staff.psn-code = X_cd-clu.cli-code                                         /*and X_staff.role = entry(lookup(X_cd-clu.clu-type, &1&2&1), &1&3&1)*/                                         and X_staff.db-num = &4                                         and X_staff.date-end = &5,                                   first X_clients outer-join no-lock where X_clients.obj-type = &1&6&1 AND                                        X_clients.obj-code  = X_staff.psn-code ', chr(34), v-rkep-cli-role-list, v-th-role-list, v-db-num, 12/31/9999, 'чел':U))
    ...
    :
      assign
        l-disable-where-36 =
          (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code and X_cd-clu.pos-type = 'r-keeper':U " + " " + where-phrase-36 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-rkep-cli:handle
  ...
  if l-filter-open-36 = false then do:
    OPEN QUERY br-rkep-cli FOR EACH X_cd-clu
      where  X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code and X_cd-clu.pos-type = 'r-keeper':U
    , FIRST X_staff outer-join NO-LOCK WHERE X_staff.psn-code = X_cd-clu.cli-code                                                                                  and X_staff.db-num = v-db-num                                         and X_staff.date-end = 12/31/9999,                                   first X_clients outer-join no-lock where X_clients.obj-type = 'чел':U AND                                        X_clients.obj-code  = X_staff.psn-code
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  on error undo, leave
  :
    if QUERY br-rkep-cli:handle:get-buffer-handle(1) = (buffer X_cd-clu:handle) then do:
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-4-36 =
        "where ":u +  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3 and X_cd-clu.pos-type = &1&4&1 '                              , chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " ":u + where-phrase-36 + " ":u + p-find-condition + " " + ""
      parameter-5-36 = "  "
    .
      ...
      assign
      parameter-2-36 = (if p-find-next then "true":u else "false":u )
      parameter-3-36 =  "FOR EACH X_cd-clu"
      parameter-4-36 =
        (
          if (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code and X_cd-clu.pos-type = 'r-keeper':U " + " " + where-phrase-36) <> ""
          then  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3 and X_cd-clu.pos-type = &1&4&1 '                              , chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " " + where-phrase-36
      ...
      parameter-5-36 = (" " + "" + " " +  substitute(', FIRST X_staff outer-join NO-LOCK WHERE X_staff.psn-code = X_cd-clu.cli-code                                         /*and X_staff.role = entry(lookup(X_cd-clu.clu-type, &1&2&1), &1&3&1)*/                                         and X_staff.db-num = &4                                         and X_staff.date-end = &5,                                   first X_clients outer-join no-lock where X_clients.obj-type = &1&6&1 AND                                        X_clients.obj-code  = X_staff.psn-code ', chr(34), v-rkep-cli-role-list, v-th-role-list, v-db-num, 12/31/9999, 'чел':U) + " " + p-find-condition)
      parameter-6-36 = if sort-phrase-36 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-38 as character no-undo .
      ...
      parameter-4-38 =
        (
          if (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code                       and X_cd-clu.pos-type = 'r-keeper':U
                      and (X_cd-clu.cli-code = 0 OR X_cd-clu.cli-code = ?)" + " " + where-phrase-38) <> ""
          then  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3                       and X_cd-clu.pos-type = &1&4&1
                      and (X_cd-clu.cli-code = 0 OR X_cd-clu.cli-code = ?)', chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " " + where-phrase-38
          else "true"
        )
      parameter-5-38 = (" " + "" + " " + ", FIRST X_staff                                      , first X_clients OUTER-JOIN ")
    ...
    :
      assign
        l-disable-where-38 =
          (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code                       and X_cd-clu.pos-type = 'r-keeper':U
                      and (X_cd-clu.cli-code = 0 OR X_cd-clu.cli-code = ?)" + " " + where-phrase-38 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-rkep-cli:handle
  ...
  if l-filter-open-38 = false then do:
    ...
    , FIRST X_staff                                      , first X_clients OUTER-JOIN
      indexed-reposition
  .
      assign
        l-query-was-opened = true
  ...
  :
    if QUERY br-rkep-cli:handle:get-buffer-handle(1) = (buffer X_cd-clu:handle) then do:
      ...
      parameter-4-38 =
        "where ":u +  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3                       and X_cd-clu.pos-type = &1&4&1
                      and (X_cd-clu.cli-code = 0 OR X_cd-clu.cli-code = ?)', chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " ":u + where-phrase-38 + " ":u + p-find-condition + " " + ""
      parameter-5-38 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-38 = (if p-find-next then "true":u else "false":u )
      parameter-3-38 =  "FOR EACH X_cd-clu"
      parameter-4-38 =
        (
          if (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code                       and X_cd-clu.pos-type = 'r-keeper':U
                      and (X_cd-clu.cli-code = 0 OR X_cd-clu.cli-code = ?)" + " " + where-phrase-38) <> ""
      ...
      parameter-5-38 = (" " + "" + " " + ", FIRST X_staff                                      , first X_clients OUTER-JOIN " + " " + p-find-condition)
      parameter-6-38 = if sort-phrase-38 = ''
                           then
        (
        " " + "  " +
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-40 as character no-undo .
      ...
      parameter-4-40 =
        (
          if (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code and X_cd-clu.pos-type = 'r-keeper':U " + " " + where-phrase-40) <> ""
          then  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3 and X_cd-clu.pos-type = &1&4&1 '                              , chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " " + where-phrase-40
          else "true"
        )
      parameter-5-40 = (" " + "" + " " +  substitute(' , FIRST X_staff outer-join NO-LOCK WHERE X_staff.psn-code = X_cd-clu.cli-code                                               /*and X_staff.role = entry(lookup(X_cd-clu.charkey_one, &1&2&1), &1&3&1)*/                                               and X_staff.db-num = &4                                               and X_staff.date-end = &5,                                         first X_clients outer-join no-lock where X_clients.obj-type = &1&6&1 AND                                               X_clients.obj-code  = X_staff.psn-code ', chr(34), v-rkep-cli-role-list, v-th-role-list, v-db-num, 12/31/9999, 'чел':U))
    ...
    :
      assign
        l-disable-where-40 =
          (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code and X_cd-clu.pos-type = 'r-keeper':U " + " " + where-phrase-40 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input QUERY br-rkep-cli:handle
  ...
  :
    if QUERY br-rkep-cli:handle:get-buffer-handle(1) = (buffer X_cd-clu:handle) then do:
      ...
      parameter-4-40 =
        "where ":u +  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3 and X_cd-clu.pos-type = &1&4&1 '                              , chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " ":u + where-phrase-40 + " ":u + p-find-condition + " " + ""
      parameter-5-40 = "  "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
      ...
      assign
      parameter-2-40 = (if p-find-next then "true":u else "false":u )
      parameter-3-40 =  "FOR EACH X_cd-clu"
      parameter-4-40 =
        (
          if (" X_cd-clu.obj-type = p-curr-obj-type and X_cd-clu.obj-code = p-curr-obj-code and X_cd-clu.pos-type = 'r-keeper':U " + " " + where-phrase-40) <> ""
          then  substitute('X_cd-clu.obj-type = &1&2&1 and X_cd-clu.obj-code = &3 and X_cd-clu.pos-type = &1&4&1 '                              , chr(34), p-curr-obj-type, p-curr-obj-code, 'r-keeper':U) + " " + where-phrase-40
...
FUNCTION get-cname-diff RETURNS LOGICAL
 ...
 RETURN (loc-clients.obj-name <> X_cd-clu.charkey_one).
 RETURN NO.
END FUNCTION.
FUNCTION get-crole-diff RETURNS LOGICAL
