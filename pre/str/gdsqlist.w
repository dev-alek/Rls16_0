...
define buffer l-gds-list for gds-list.
define stream sout.
...
.
DEFINE QUERY br-list FOR gds-list SCROLLING.
DEFINE BROWSE br-list QUERY br-list NO-LOCK DISPLAY
      gds-list.to-sel  format "+/" column-label "*"
      gds-list.gds-code FORMAT "99999999999":U
      gds-list.artic
      gds-list.gds-name
      gds-list.unit-base column-label "Изм" format "x(3)"
      (if gds-list.stts = 0 then no else yes) column-label "-" format "-/ "
      gds-list.grp-name
      (gds-list.prod-type + " " + string (gds-list.prod-code, ">>>>>>>>>9") ) column-label "Производитель" format "x(9)"
...
run create-gds-list-hist in this-procedure (input 'title'
                                     , input-output v-seq
                                     , input 0
                                     , input 'N':U
                                     , input v-value
...
PROCEDURE proc-any-printable-br-list :
  if input frame Dialog-Frame a-n-c = "art" then do:
    if last-event:label = " " and
       loc-art = "" then
    return error.
    find first l-gds-list where
                l-gds-list.artic begins (loc-art + last-event:label)
               no-lock no-error.
    if available l-gds-list then do:
      loc-art = loc-art + last-event:label.
      disp loc-art with frame Dialog-Frame.
      line-rec = recid (l-gds-list).
...
PROCEDURE proc-backspace-br-list:
  if input frame Dialog-Frame a-n-c = "art" then do:
    ...
    find first l-gds-list where
                l-gds-list.artic begins loc-art
               no-lock.
    disp loc-art with frame Dialog-Frame.
    line-rec = recid (l-gds-list).
    reposition br-list to recid line-rec no-error.
...
,buffer buf_place
  ...
  buf_bar-code.gds-code No-LOCK.
        find first l-gds-list where
                  l-gds-list.artic = l-goods.artic AND
                  l-gds-list.prod-type = l-goods.prod-type AND
                  l-gds-list.prod-code = l-goods.prod-code no-lock no-error.
    if available l-gds-list then do:
      line-rec = recid (l-gds-list).
      reposition br-list to recid line-rec no-error.
      apply "value-changed" to br-list in frame Dialog-Frame.
...
PROCEDURE  proc-mouse-dbl-click-loc-name:
  ...
  loc-name.
    if last-event:label = "Ctrl-J" then
      find next l-gds-list where
                can-find (ub.goods where ub.goods.artic = l-gds-list.artic and
                ub.goods.prod-type = l-gds-list.prod-type and
                ub.goods.prod-code = l-gds-list.prod-code and
...
if not available gds-list or recid (gds-list) <> line-rec then do:
    hide loc-art in frame Dialog-Frame.
    loc-art = "".
end.
if available gds-list then do:     find ub.clients where ub.clients.obj-type = gds-list.prod-type                    and ub.clients.obj-code = gds-list.prod-code no-lock.     find ub.gds-prt where ub.gds-prt.upper-code = gds-list.prt-root no-lock.     find ub.bar-code where ub.bar-code.gds-code  = gds-list.gds-code                     and ub.bar-code.node-code = ub.gds-prt.node-code                     and ub.bar-code.unit-cli  = gds-list.unit-base                     and ub.bar-code.in-code   = ""                     and ub.bar-code.part-code = "" no-lock.     disp ub.clients.obj-name ub.gds-prt.node-name ub.bar-code.b-code tot-lns @ f-tot-lns with frame Dialog-Frame.   end.   else display f-tot-lns with frame Dialog-Frame.
end.
...
find ub.goods where gds-list.artic = ub.goods.artic
            and gds-list.prod-type = ub.goods.prod-type
            and gds-list.prod-code = ub.goods.prod-code no-lock.
...
:
  ...
  string(buf_gds-list-hist.des, "X(155)") skip.
end.
...
ON CHOOSE OF b-ext IN FRAME Dialog-Frame DO:
    ...
    then do:
        ...
        :
            find first buf_goods no-lock
                where buf_goods.gds-code = buf_gds-list.gds-code
            .
...
define variable glog as logical no-undo .
define buffer buf-gds-list-hist for gds-list-hist.
glog = no.
...
tot-lns = 0.
v-seq = 1.
for each buf-gds-list-hist:
  delete buf-gds-list-hist.
end.
...
define buffer buf_keep-macro-list-hist for keep-macro-list-hist.
 if p-keep-query then do:
   ...
   run create-gds-list-hist in this-procedure (
                                          input 'ДОБАВЛЕНИЕ':U
                                        , input-output v-seq
                                        , input 0
                                        , input '0':U
...
DO:
  if not available gds-list then return no-apply.
  assign rs-list-method = "single".
...
for each buf_gds-list-hist where
        buf_gds-list-hist.id >=  an-listp-start-macro-id
  and  buf_gds-list-hist.id <=  an-listp-end-macro-id:
  create macro-list-hist.
  buffer-copy buf_gds-list-hist
  ...
  for each macro-list-hist where (p-id = 0 or macro-list-hist.id = p-id):
    ...
    CASE entry(1, macro-list-hist.option_, chr(4)):
      ...
      when 'clear':U then do:
        ...
        for each gds-list-hist where gds-list-hist.id > an-listp-macros-label-id:
        ...
        to buf_gds-list-hist
        assign buf_gds-list-hist.id = v-seq
        v-temp-seq = v-seq
        v-seq = v-seq + 1
      ...
      when 'single' then do:
        ...
        if not error-status:error then do:
          ...
          find first  buf_gds-list-hist where
                  buf_gds-list-hist.id = v-seq - 1 no-error .
          if available buf_gds-list-hist then
          assign
          macro-list-hist.num-recs = buf_gds-list-hist.num-recs
          macro-list-hist.num-add = buf_gds-list-hist.num-add
          macro-list-hist.num-ignored = buf_gds-list-hist.num-ignored
          .
        end.
        else do:
          return ("error" + chr(4) + return-value).
        ...
        then do:
          if macro-list-hist.line = 0 then do:
            ...
            for each buf_macro-list-hist where
              ...
              to buf_gds-list-hist
              assign
              buf_gds-list-hist.id = (if buf_macro-list-hist.line =0 then v-seq else v-temp-seq)
            ...
            for each buf_gds-list-hist where
                    buf_gds-list-hist.id = v-temp-seq,
               first buf_macro-list-hist where
                    buf_macro-list-hist.id = v-id
                AND buf_macro-list-hist.line = buf_gds-list-hist.line:
               assign
               buf_macro-list-hist.num-recs = buf_gds-list-hist.num-recs
               buf_macro-list-hist.num-add = buf_gds-list-hist.num-add
               buf_macro-list-hist.num-ignored = buf_gds-list-hist.num-ignored
               .
            end.
...
if available gds-list then   do:
  find first buf_gds-list where
           recid(buf_gds-list) = recid(gds-list).
  assign
  buf_gds-list.to-sel = not (buf_gds-list.to-sel).
...
do:
  ...
  gds-list.gds-name:resizable in browse br-list = true
  gds-list.grp-name:resizable in browse br-list = true
...
define buffer buf_gds-list-hist for gds-list-hist.
  find first buf_gds-list-hist where buf_gds-list-hist.id = 0 no-error.
  if available buf_gds-list-hist then
  assign
  frame Dialog-Frame:title = substitute("СПИСОК  ТОВАРОВ &1",  string(buf_gds-list-hist.des, "X(60)"))
  .
    assign
  ...
  if tot-lns = ? then do:
    v-start = yes.
    for each l-gds-list :
      accumulate l-gds-list.artic (count).
    end.
    tot-lns = (accum count l-gds-list.artic).
    if tot-lns > 0 then do:
      find last  buf_gds-list-hist no-error .
      v-seq = (if available buf_gds-list-hist then buf_gds-list-hist.id else 0)  + 1.
      run create-gds-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                           , input-output v-seq
                                           , input 0
                                           , input 'S':U
      ...
      for each buf_gds-list-hist:
        delete buf_gds-list-hist.
      end.
      v-seq = 1.
      run create-gds-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                           , input-output v-seq
  ...
  if v-seq > 1 then
  find last buf_gds-list-hist no-lock where
            buf_gds-list-hist.id = (v-seq - 1)
       and  buf_gds-list-hist.line = 0 no-error .
  DISPLAY br-option
  (if available buf_gds-list-hist
  then buf_gds-list-hist.des
  else '') @ dsp-rs
  ...
  if v-start then do:
    ...
    then do:
      gds-list.to-sel:visible in browse br-list = yes.
    end.
    else do:
      gds-list.to-sel:visible in browse br-list = no.
    end.
    if bttns <> '':U then do:
  ...
  open query br-list for each gds-list no-lock indexed-reposition.
  if line-rec <> ? then
    reposition br-list to recid line-rec no-error.
  apply "entry" to br-list in frame Dialog-Frame.
  if available gds-list then do:     find ub.clients where ub.clients.obj-type = gds-list.prod-type                    and ub.clients.obj-code = gds-list.prod-code no-lock.     find ub.gds-prt where ub.gds-prt.upper-code = gds-list.prt-root no-lock.     find ub.bar-code where ub.bar-code.gds-code  = gds-list.gds-code                     and ub.bar-code.node-code = ub.gds-prt.node-code                     and ub.bar-code.unit-cli  = gds-list.unit-base                     and ub.bar-code.in-code   = ""                     and ub.bar-code.part-code = "" no-lock.     disp ub.clients.obj-name ub.gds-prt.node-name ub.bar-code.b-code tot-lns @ f-tot-lns with frame Dialog-Frame.   end.   else display f-tot-lns with frame Dialog-Frame.
        ...
        do:
            ...
            do:
                for first ub.Code no-lock where ub.Code.parent = "TiketPrint" and
                    ...
                    find first gds-list where gds-list.artic = buf_goods.artic
  ...
  .
                ...
                do:
                    for first ub.Code no-lock where ub.Code.parent = "TiketPrint" and
                        ...
                        if available gds-list then
                        do:
                            gds-list.qnty = decimal(ub.Code.CodeValue) .
                        end.
                    end.
                end.
            end.
...
case rs-list-method:
  when "all" then do:
    ...
    assign                                                           buf_gds-list-hist.num-add  = lns-cnt - v-num-add                       buf_gds-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_gds-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_gds-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
  end.
  when "gds-grp" then do:
    for each buf_gds-list-hist where
             buf_gds-list-hist.id = p-id
        and  buf_gds-list-hist.item_ <> '':U:
      run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_gds-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then  next.
      grp-path = "".
  ...
  when "object" then do:
    find first buf_gds-list-hist where
             buf_gds-list-hist.id = p-id
        and  buf_gds-list-hist.item_  <> '':U.
    run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_gds-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then do: end. else do:
  ...
  when "waybill" then do:
    ...
    for each buf_gds-list-hist where
             buf_gds-list-hist.id = p-id
        and  buf_gds-list-hist.item_  <> '':U:
      run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_gds-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then next _ii.
      ...
      for each ub.doc-line where ub.doc-line.doc-code = ub.trn-doc.doc-code no-lock,
        ...
        run ex-gds in this-procedure ( buffer ub.goods, input rs-list-method, input rs-status, input line-mode).
        if glog and
            available gds-list and
            gds-list.artic = ub.goods.artic and
            gds-list.prod-type = ub.goods.prod-type and
            gds-list.prod-code = ub.goods.prod-code then do:
          gds-list.qnty = if can-do ('рас,спи':U, ub.trn-doc.doc-type) then
                      gds-list.qnty - ub.doc-line.fact-qnty
                    else
                      gds-list.qnty + ub.doc-line.fact-qnty.
          if gds-list.qnty < 0 then do:
            ...
            message "Артикул:" ub.doc-line.artic skip
                    ...
                    "Итоговое количество в списке:" gds-list.qnty skip (2)
                    "Заменено на 0."
                    view-as alert-box .
            gds-list.qnty = 0.
          end.
...
FOR EACH
  ...
  when "overvalue" then do:
    ...
    for each buf_gds-list-hist where
             buf_gds-list-hist.id = p-id
        and  buf_gds-list-hist.item_  <> '':U:
      run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_gds-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then next _jj.
  ...
  when "pdf" then do:
    ...
    when "collection" then do:
        run str/any-li01.p (
                        ...
                        ,input line-mode
                        ,input p-id
                        ,INPUT-OUTPUT table buf_gds-list-hist
                        ) no-error .
  end.
  when "abcxyz"
  or
  ...
  when "izt" then do:
    for each buf_gds-list-hist where
        ...
        and  buf_gds-list-hist.item_  <> '':U:
      assign
      v-attr-code  = entry(1, buf_gds-list-hist.item_, chr(3))
      v-obj-type = entry(2, buf_gds-list-hist.item_, chr(3))
      v-obj-code = integer(entry(3, buf_gds-list-hist.item_, chr(3)))
      .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run emptyscl in g#library
  ...
  when "is-ptrl" then do:
    find first buf_gds-list-hist where
             buf_gds-list-hist.id = p-id  no-error .
  ...
  then do:
    find first buf_gds-list-hist where
         ...
         AND buf_gds-list-hist.item_ <> '':U .
    assign
    v-obj-type = entry(1, buf_gds-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_gds-list-hist.item_, chr(3)))
    v-attr-code  = (if rs-list-method = "fbr-gds-obj-val"
                    then entry(3, buf_gds-list-hist.item_, chr(3))
                    else '':U)
    vvalue = (if rs-list-method = "fbr-gds-obj-val"
              then entry(4, buf_gds-list-hist.item_, chr(3))
              else '':U)
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "dis-gds-rule" or when "dis-gds-rule-num"  then do:
    find first buf_gds-list-hist where
             buf_gds-list-hist.id = p-id
         AND buf_gds-list-hist.item_ <> '':U no-error .
    assign
    v-obj-type = entry(1, buf_gds-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_gds-list-hist.item_, chr(3)))
    v-discnt-role = entry(3, buf_gds-list-hist.item_, chr(3))
    vvalue-int = (if rs-list-method = "dis-gds-rule" then 0 else integer(entry(4, buf_gds-list-hist.item_, chr(3))))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "gds-obj-attr" or when "gds-obj-attr-val"  then do:
    ...
    v-obj-type = entry(1, buf_gds-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_gds-list-hist.item_, chr(3)))
    v-attr-code = entry(3, buf_gds-list-hist.item_, chr(3))
    vvalue = (if rs-list-method = "gds-obj-attr" then '':U else entry(4, buf_gds-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "gds-host-attr-val" then do:
    find first buf_gds-list-hist where
         ...
         AND buf_gds-list-hist.item_ <> '':U .
    assign
    v-host-code = integer(entry(1, buf_gds-list-hist.item_, chr(3)))
    v-attr-code = entry(2, buf_gds-list-hist.item_, chr(3))
    vvalue = (if rs-list-method = "gds-host-attr" then '':U else entry(3, buf_gds-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  then do:
    ...
    assign
    v-attr-code = entry(1, buf_gds-list-hist.item_, chr(3))
    vvalue = (if rs-list-method = "goods-attr" then '':U else entry(2, buf_gds-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "tax-rate-value" then do:
    ...
    v-obj-type = entry(1, buf_gds-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_gds-list-hist.item_, chr(3)))
    v-date = date(entry(3, buf_gds-list-hist.item_, chr(3)))
    v-attr-code = entry(4, buf_gds-list-hist.item_, chr(3))
    vvalue = entry(5, buf_gds-list-hist.item_, chr(3))
    no-error
    .
    if error-status:error then do: end. else do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pftxvalg in g#library
  ...
  when "scales-gds" or when "scales-gds-num"  then do:
    ...
    v-obj-type = entry(1, buf_gds-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_gds-list-hist.item_, chr(3)))
    vvalue = (if rs-list-method = "scales-gds" then '':U else entry(3, buf_gds-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "parts-last-date" then do:
    ...
    v-obj-type = entry(1, buf_gds-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_gds-list-hist.item_, chr(3)))
    vvalue-int = integer(entry(3, buf_gds-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "filter" then do:
    ...
    find first buf_gds-list-hist where
             buf_gds-list-hist.id = p-id
         AND buf_gds-list-hist.item_ <> '':U .
    run proc-write-filter-expression-var in this-procedure ( input buf_gds-list-hist.item_, output v-filter-var ).
      run gbl/gds-fill.p (
  ...
  when "move-date":U then do:
      ...
      for each gds-obj  no-lock where gds-obj.obj-type = v-cntxt-obj-type
                                  and gds-obj.obj-code = v-cntxt-obj-code
                                  and gds-obj.last-doc >= date(buf_gds-list-hist.item_):
...
if rs-list-method = "single" then do:
  if v-no-hist < 0 then do:
    run create-gds-list-hist in this-procedure(input 'ДОБАВЛЕНИЕ':U
                                        ...
                                        , input substitute("ТОВАР код &1 &2 &3&4 &5", gds-list.gds-code, gds-list.artic, gds-list.prod-type, gds-list.prod-code, gds-list.gds-name)
                                        , input tot-lns
                                        , input rs-list-method
                                        , input rs-status
                                        , input ('goods':U + chr(3) + string(gds-list.gds-code))
                                        , input '':U
                                        , input ?
                                        ).
  end.
    ...
    do v-ii = 0 to v-no-hist:
      run create-gds-list-hist in this-procedure(input ('ИЗМЕНЕНИЕ':U + chr(4) + 'mode':U)
...
for each gds-list:
find ub.goods where ub.goods.artic     = gds-list.artic     and
                ub.goods.prod-type = gds-list.prod-type and
                ub.goods.prod-code = gds-list.prod-code no-lock.
    create tt-goods.
  ...
  :
    ...
    if rs-list-method = "single" then do:
      ...
      if p-from-macro then do:
        ...
        find first gds-list where gds-list.gds-code = ub.goods.gds-code no-error.
      end.
      if available gds-list then do:
        line-rec = recid (gds-list).
        get next br-list.
        if available gds-list then v-rep-rec = recid (gds-list).
        else do:
          reposition br-list to recid line-rec no-error.
          get prev br-list.
          if available gds-list then v-rep-rec = recid (gds-list).
  ...
  :
    ...
    if rs-list-method = "single" then do:
      ...
      if available gds-list then do:
        ...
        run write-hist  in this-procedure ( p-from-macro, rs-list-method, rs-status, line-mode).
        for each gds-list:
          if line-rec <> recid (gds-list) then delete gds-list.
        end.
        assign
        tot-lns = 1
        .
...
run gbl/filename.p
    (input  (if rs-list-method = "clob-data" then v-file-name else buf_gds-list-hist.item_  )
  ,output v-full-path
  ,output v-path
  ,output v-file-name
  ,output v-file-name-no-ext
  ...
  CASE rs-list-method:
        ...
        if imp-doc-type = 'переоценка':U then do:
          ...
          if avail(ub.price-doc) then do:
            FOR EACH ub.price-list No-LOCK WHERE
              ...
              run ex-gds  in this-procedure ( buffer ub.goods, input rs-list-method, input rs-status, input line-mode).
              if available gds-list and
                gds-list.artic = ub.goods.artic and
                gds-list.prod-type = ub.goods.prod-type and
                gds-list.prod-code = ub.goods.prod-code then
                gds-list.qnty = scan-qnty.
            END.
          end.
        end.
        else do:
    ...
    then do:
      repeat:
        ...
        if available ub.goods and  available gds-list and
          gds-list.artic = ub.goods.artic and
          gds-list.prod-type = ub.goods.prod-type and
          gds-list.prod-code = ub.goods.prod-code then
          gds-list.qnty = scan-qnty.
...
,buffer ub.place
        ...
        if available gds-list and
          ...
          gds-list.prod-type = ub.goods.prod-type and
          gds-list.prod-code = ub.goods.prod-code then
          gds-list.qnty = gds-list.qnty + scan-qnty * bc-qnty.
      end.
    end.
  END CASE.
  input stream sout close.
