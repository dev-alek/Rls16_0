...
define variable vss-description as character no-undo init "Настройка параметров произвольных заданий, работающих через машину правил".
...
define variable v-rum-option as character no-undo .
...
DEFINE BUTTON b-quit AUTO-GO
     ...
     SIZE 10 BY 1
     BGCOLOR 8 .
DEFINE BUTTON b-profile DEFAULT
     LABEL "Выбор профайла":L
     SIZE 20 BY 1.
...
get-rule-name(tt0-rule-by-call.rule_id) COLUMN-LABEL "Название правила" FORMAT "X(255)":U WIDTH 50
...
DEFINE FRAME Dialog-Frame
     ...
     b-profile AT ROW 1 COL 40 WIDGET-ID 6
     ...
     SPACE(0.69) SKIP(5.22)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE
         TITLE "Параметры работы в автоматическом режиме"
...
case v-rum-option:
  when 'ord':U then do:
    v-list = ('ord':U + chr(4) + 'batchwork':U ).
  end.
  when 'goods':U then do:
    v-list = ('goods':U + chr(4) + 'goods-batchwork':U ).
  end.
end case.
    run utl/thbjrumr.w (
                        input parparentproc
...
DO:
  v-param-action = 'ПРОСМОТР':U.
  RUN proc-b-params IN THIS-PROCEDURE ( INPUT 'ПРОСМОТР':U) NO-ERROR.
  IF ERROR-STATUS:ERROR THEN do:
      v-param-action = ''.
      RETURN NO-APPLY.
  END.
...
DO:
  v-param-action = 'ИЗМЕНЕНИЕ':U.
  RUN proc-b-params IN THIS-PROCEDURE ( INPUT 'ИЗМЕНЕНИЕ':U) NO-ERROR.
  IF ERROR-STATUS:ERROR THEN do:
      v-param-action = ''.
      RETURN NO-APPLY.
  END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse br-rule-by-call :handle
      .
...
OPEN QUERY br-rule-by-call FOR EACH tt0-rule-by-call WHERE     tt0-rule-by-call.call_id = v-uniq-key-rec,            FIRST X_rule-profile NO-LOCK WHERE         X_rule-profile.profile_id = tt0-rule-by-call.profile_id   BY tt0-rule-by-call.codex_id  BY tt0-rule-by-call.ruleset_id  BY tt0-rule-by-call.order_id  INDEXED-REPOSITION.
end procedure.
MAIN-BLOCK:
...
define variable vss-include-info16 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  v-rum-option = entry(1, v-rum-option, "_").
...
END PROCEDURE.
PROCEDURE enable_UI :
  DISPLAY ED-profile E-rule-name
...
define variable v-uniq-key-rec as character no-undo .
run gen-key-rec in this-procedure ( input 'schedule':U
                                    ,input (buffer buf_schedule:handle)
                                    , output v-uniq-key-rec).
run rul/g-callid.p ( input 'ord':U
                  ,input v-uniq-key-rec
                  ,output p-call#-id).
...
DEFINE VARIABLE ii AS integer NO-UNDO.
define variable v-thbj-uniq-key-rec as character no-undo .
define buffer buf_thbj-attr for ub.thbj-attr.
DO ii = 1 TO br-rule-by-call:NUM-COLUMNS IN FRAME Dialog-Frame:
    clh = BROWSE br-rule-by-call:get-browse-column(ii).
    IF clh:LABEL BEGINS "Название правила" THEN DO:
      ASSIGN
      clh:RESIZABLE = YES
     .
    END.
...
IF NOT AVAILABLE buf_rule THEN RETURN chr(63).
