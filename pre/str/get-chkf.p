...
output stream Dirstream
to value(v-file-name) append .
Put  stream Dirstream unformatted
p-plu
chr(3)
p-value
skip.
...
define variable vss-revision12    as character no-undo init "$Revision:$":U .
define variable vss-author12      as character no-undo init "$Author:$":U .
define variable vss-date12        as character no-undo init "$Date:$":U .
define variable vss-workfile12    as character no-undo init "$Workfile:$":U .
define variable vss-archive12     as character no-undo init "$Archive:$":U .
define variable vss-description12 as character no-undo init "Работа С сокетом".
...
then do:
  ...
  p-remote = integer(entry(3, p-parameter, chr(4)))
  p-auto   = if num-entries(p-parameter, chr(4)) >= 4
             then integer(entry(4, p-parameter, chr(4)))
             else 0
  p-shft-close  = if num-entries(p-parameter, chr(4)) >= 5
             then integer(entry(5, p-parameter, chr(4)))
             else 0
  no-error .
  if error-status:error then do:
    assign
...
then do:
  message
  substitute('!!!При приеме информации с касс &1&2 произошли ошибки!!!'                                   ,p-obj-type                                                                                 ,p-obj-code)  skip
  ...
  run gbl/prnfilen.w
    (input  (substitute('!!!При приеме информации с касс &1&2 произошли ошибки!!!'                                   ,p-obj-type                                                                                 ,p-obj-code))
    ,input  0
    ,input  (string("./":U) + 'get-chkf.log')
    ,input  7
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'get-chkf.log').
end.
                        return "error":U.                  end.
end.
...
define variable v-value-character21 as character no-undo .
...
define variable hnum21 as logical no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'орг':U
    ,input  v-host-code
...
IF not error-status:error then  do:
  assign
  hnum21 = v-value-logical21
  .
end.
delete object v-tth21.
v-lock-global = hnum21.
...
then do:
  ...
  "!!!Внимательно прочитайте Log-file!!"
  view-as alert-box error .
     define variable v-user-action24   as character no-undo .
  define variable v-printed24       as logical   no-undo .
  run gbl/prnfilen.w
    (input  (substitute('!!!При приеме информации с касс &1&2 произошли ошибки!!!'                                   ,p-obj-type                                                                                 ,p-obj-code))
    ,input  0
    ,input  (string("./":U) + 'get-chkf.log')
    ,input  7
    ,output v-user-action24
    ,output v-printed24
    ) .
end.
if v-view-log = true
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'get-chkf.log').
        ...
        if cash-desk.pos-type = 'MAGIA-XML':U then do:
          assign
          kassa-rub-code = 0
          ibmgroup = no
          ibmspool = "3"
          ...
          v-effective-pos-type =  (if (cash-desk.pos-type = 'IBM':U
                                      or
                                      cash-desk.pos-type = 'Emulator-NKT-IBM':U)
                                  then 'cd-type-ibm':U
                                  else 'cd-type-IBM-XML':U
                                  ).
          run adm/shattri.p (
              input "get":U
              ,input  p-obj-type
          ...
          on error undo, return error :
            case thbjattr_thbj-attr.prop-code :
              when 'ibmgroup':U then do:
                assign
                ibmgroup = thbjattr_thbj-attr.property-value-logical.
              end.
              when 'ibmrubc':U
              or when 'ibmrubc':U then do:
                kassa-rub-code = thbjattr_thbj-attr.property-value-integer.
              end.
              when 'specgrp':U
              or when 'specgrp':U then do:
                specgrp = thbjattr_thbj-attr.property-value-character.
              end.
              when 'ibmspool':U then do:
                if cash-desk.pos-type = 'IBM':U
                or cash-desk.pos-type = 'Emulator-NKT-IBM':U
                then do:
                  assign
          ...
          and entry(1, v-spec-command) = "version" then do:
            ...
            run gen-key-fv in this-procedure ( input replace(v-spec-command, "version" + chr(44), '')
                                              ,output v-field-list
                                              ,output v-value-list
                                              ).
            assign
        ...
        FOR EACH for-cash-desk NO-LOCK WHERE
          ...
          if for-cash-desk.autonomy = integer('2':U)
          and for-cash-desk.pos-type <> 'Autotank':U
          then next _ibm-cash-desk.
          if for-cash-desk.autonomy = integer('1':U)
          and for-cash-desk.pos-type = 'Autotank':U
          then next _ibm-cash-desk.
          if v-cash-num <> ? and
          ...
          if for-cash-desk.remote = 1 then do:
            ...
            assign
            var-spl-suffix = v-dir-remote + chr(47) +
                            "spl":U + v-spl-obj-cash-name
            var-spl-suffix-tmp = v-dir-remote-tmp + chr(47) +
                            "spl":U + v-spl-obj-cash-name
            .
          end.
          else do:
          ...
          if (p-remote = 0 and Not for-cash-desk.remote = 1) or
            ...
            then do:
              ...
              run xml-cd-filename in this-procedure (
                ...
                assign
                v-obj-list = 'маг':U + string(for-cash-desk.obj-code)
                .
              ...
              output stream stmxmlout to value( v-xml-file-name-path + "xm1" ) convert target "1251" append.
                ...
                run bgelib-tag-put in this-procedure ( input 3, input "SpoolDateFrom":U,
                                                      input (Xml-CD-DatetoString (v-date) + chr(32)  + string(v-time, "HH:MM:SS":U)), input 1 ).
                run bgelib-tag-put in this-procedure ( input 3, input "SpoolDateTo":U,
                                                      input (Xml-CD-DatetoString (v-today-date + 2) + chr(32) + string(0, "HH:MM:SS":U)), input 1 ).
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'get-chkf.log').
                 ...
                 if entry(1, v-spec-command) = "version" then do:
                    ...
                    hSAXWriter:WRITE-data-ELEMENT("CDateFrom" , Xml-CD-DatetoString (v-date) + chr(32)  + string(v-time, "HH:MM:SS":U) ) no-error.
                    if error-status:error then do:
                       run write-log-and-file in p-log-handle (
                          input 1
                        , input log-file-name
              ...
              if for-cash-desk.remote = 1 then do:
                ...
                var-found-not-remote = yes
                .
                if for-cash-desk.autonomy = integer('0':U)
                or (for-cash-desk.autonomy = integer('2':U)
                    and
                    for-cash-desk.pos-type = 'Autotank':U)
                then do:
                    ...
                    then do:
                       run str/post-xml.p
                         ...
                         ,input substitute('Чтение данных с кассы &1://&2'
                                           ,entry(1, for-cash-desk.addr-path, chr(4))
                                           ,entry(2, for-cash-desk.addr-path, chr(4))
                                         )
                         ) no-error .
                       if error-status:error
                       ...
                       run ConectSocet (entry(1,entry(2, for-cash-desk.addr-path, chr(4)),":"),
                                        entry(2,entry(2, for-cash-desk.addr-path, chr(4)),":"),
                                         ...
                                         30,
                                         no,
                                         substitute ("Чтение данных с кассы &1. ",entry(2, for-cash-desk.addr-path, chr(4)))
                                         ).
                        if mWebResp eq ""
                        then do:
                           run write-log-and-file in p-log-handle (
        ...
        then do:
          ...
          if (cash-desk.pos-type = 'IBM-XML':U
          and cash-desk.autonomy = integer('0':U))
          or (cash-desk.pos-type = 'Autotank':U
             and cash-desk.autonomy = integer('2':U))
          then do:
          end.
          else do:
            run waitp in this-procedure (
                        input p-auto
                        ,input (chr(4) + (in_ + spl) + chr(4) + "fl*")
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'get-chkf.log').
          ...
          run str/getxibmf.p (
                        ...
                        ,input log-file-name
                        ,input (if mReadBuffer then "readbuffer_" else "") +  "spool":U + (if p-other <> '':U then chr(4) + v-spec-command else '':U)
                        ,input
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'get-chkf.log').
              ...
              run str/getxibmf.p (
                            ...
                            ,input log-file-name
                            ,input "spool":U + (if p-other <> '':U then chr(4) + v-spec-command else '':U)
  ...
  OS-DELETE value(string("./":U) + 'get-chkf.log').
        ...
        when 'OMRON-NEW':U then  do:
          ...
          for each thbjattr_thbj-attr where
                  thbjattr_thbj-attr.obj-type = p-obj-type
              and thbjattr_thbj-attr.obj-code = p-obj-code
              and thbjattr_thbj-attr.upper-prop-code =  'cd-type-omron-new':U
          on error undo, return error :
            case thbjattr_thbj-attr.prop-code :
              when 'omrnbase':U then do:
                assign
                base-cass = thbjattr_thbj-attr.property-value-integer.
              end.
              when 'omrncurl':U then do:
                curr-list = thbjattr_thbj-attr.property-value-character.
              end.
              when 'omrnpayl':U then do:
                pay-list = thbjattr_thbj-attr.property-value-character.
              end.
              when 'omrnnal':U then do:
                nal = thbjattr_thbj-attr.property-value-integer.
              end.
              when 'omrnntnl':U then do:
                not-nal = thbjattr_thbj-attr.property-value-integer.
              end.
            end case.
          end.
        ...
        when 'IPC-Servis+':U then do:
          ...
          for each thbjattr_thbj-attr where
                  thbjattr_thbj-attr.obj-type = p-obj-type
              and thbjattr_thbj-attr.obj-code = p-obj-code
              and thbjattr_thbj-attr.upper-prop-code =  'cd-type-ipc-servispl':U
          on error undo, return error :
            case thbjattr_thbj-attr.prop-code :
              when 'ipcspayn':U then do:
                assign
                pay-nal = thbjattr_thbj-attr.property-value-integer.
              end.
              when 'ipcsbasc':U then do:
                 base-cass = thbjattr_thbj-attr.property-value-integer.
              end.
              when 'ipcsccrd':U then do:
                cass-card = thbjattr_thbj-attr.property-value-character.
              end.
              when 'ipcstcrd':U then do:
                trade-card = thbjattr_thbj-attr.property-value-character.
              end.
              when 'ipcscurc':U then do:
      ...
      then  do:
        ...
        run waitp in this-procedure (
                     input p-auto
                    ,input (chr(4) + IN_ + SPL + chr(4) + (if spl = "":U then "":U else "\":U) + "HOCIDC.001":U)
                    ,input "Прием данных с кассы"
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,output par-type
        ...
        FOR EACH for-cash-desk NO-LOCK WHERE
                ...
                for-cash-desk.obj-code = p-obj-code AND
                for-cash-desk.cash-on = yes :
          if for-cash-desk.autonomy = integer('2':U) then next _maria-cash-desk.
          file = substring( string( next-value( s-spool, ub ), '99999999999999999999'), 13, 8 ).
          if v-spec-command <> '':U then do:
            do ii = 1 to num-entries(v-spec-command, ';'):
             ...
             run cd-attr-value in this-procedure (
                                                ...
                                                ,output v-logical
                                                ,output v-attr-type     ) no-error.
            do shift-maria = (if entry(1, for-cash-desk.addr-path, chr(4)) = 'ftp' then 2 else 1) to 2:
              if shift-maria = 1 then do:
                 assign
                 v-p-spl = '30,31,32,33':U
                 v-spl-doc = '17':U
                 v-spl = '43':U
                 .
              end.
        ...
        output stream DIrstream to VALUE(in_ + spl + chr(47) + file + '.tsk').
        v-is-script = no.
