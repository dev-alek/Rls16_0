...
do:
  ...
  no-error .
    ...
    if num-entries(p-parameter, chr(4)) > 7 then
    do:
        p-other = p-parameter.
...
define variable v-value-character19 as character no-undo .
...
define variable hnum19 as logical no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'орг':U
    ,input  v-host-code
...
IF not error-status:error then  do:
  assign
  hnum19 = v-value-logical19
  .
end.
delete object v-tth19.
v-lock-global = hnum19.
...
if not v-view-log and search("cdviewlg_do-not-delete-log-file.txt") = ? then do:
  OS-DELETE value(string("./":U) + 'get-chkf.log').
                    ...
                    if cash-desk.pos-type = 'MAGIA-XML':U then
                    do:
                        assign
                            kassa-rub-code = 0
                            ibmgroup       = no
