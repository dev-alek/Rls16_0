...
DO:
 ...
 (  input   p-pdf-id     ,
    ...
    input   integer(entry(2,tt-table.f1," " ))
     ...
     reposition BROWSE-4 to recid v-recid no-error .
END.
...
dO:
 ...
 if p-exist =  true then do:
    assign
        tt-table.f1:fgcolor  in browse BROWSE-4 = 8
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
   ...
   frame Dialog-Frame:TITLE  = entry(1 , p-title , chr(4) ) + " " + string(p-pdf-id) + " БД:" + string( p-pdf-db-num) .
   tt-table.f1 :label  = entry( 2 , p-title , chr(4) ).
   tt-table.f2 :label  = entry( 3 , p-title , chr(4) ).
   tt-table.f3 :label  = entry( 4 , p-title , chr(4) ).
   if tt-table.f4 <> "" then tt-table.f4 :label  = entry( 5 , p-title , chr(4) ).
  run enable_UI in this-procedure .
  if p-mode = 'ПРОСМОТР':U  then  disable b-del b-ins b-del-2 b-ins-2 with frame Dialog-Frame .
