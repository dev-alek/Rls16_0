...
define shared temp-table scn-list no-undo like ub.goods
...
define  shared  temp-table scn-list-hist no-undo
...
procedure create-scn-list-hist :
...
define variable v-num-add as integer no-undo .
define variable v-num-ignored as integer no-undo .
define buffer buf_scn-list-hist for scn-list-hist.
  do
  on error undo, return error
  :
    CASE p-mode:
      when 'title' then do:
        find first buf_scn-list-hist where
                   buf_scn-list-hist.id = 0   no-error.
        if not available buf_scn-list-hist then do:
          create buf_scn-list-hist.
          assign
          buf_scn-list-hist.id = 0
          buf_scn-list-hist.line = 0
          buf_scn-list-hist.list-table = '':U
          .
        end.
        assign
        buf_scn-list-hist.des =  p-des
        buf_scn-list-hist.num-recs = p-num-recs
        buf_scn-list-hist.option_  = p-option
        buf_scn-list-hist.item_ = p-item
        .
      end.
      when 'ДОБАВЛЕНИЕ':U then do:
        ...
        create buf_scn-list-hist.
        assign
        buf_scn-list-hist.id = p-seq
        buf_scn-list-hist.list-table = p-list-table
         p-seq = (if p-line = 0 then (p-seq + 1) else p-seq)
        buf_scn-list-hist.des =  p-des
        buf_scn-list-hist.line = p-line
        buf_scn-list-hist.num-recs = p-num-recs
        buf_scn-list-hist.option_  = p-option
        buf_scn-list-hist.item_ = p-item
        buf_scn-list-hist.hist-mode =  p-hist-mode
        buf_scn-list-hist.status_ =  p-status_
        buf_scn-list-hist.num-add = v-num-add
        buf_scn-list-hist.num-ignored = v-num-ignored
        .
      end.
      when 'ИЗМЕНЕНИЕ':U + chr(4) + 'des' then do:
        find first buf_scn-list-hist where
                  buf_scn-list-hist.id = p-seq
              and buf_scn-list-hist.line = p-line no-error .
        if  available buf_scn-list-hist then do:
          assign
          buf_scn-list-hist.des =  p-des
          buf_scn-list-hist.num-recs = p-num-recs
          buf_scn-list-hist.option_  = p-option
          buf_scn-list-hist.item_ = p-item
          buf_scn-list-hist.list-table = (if p-list-table <> '':U and p-list-table <> ?
                                     then p-list-table
                                     else buf_scn-list-hist.list-table)
          .
        end.
      end.
      when 'ИЗМЕНЕНИЕ':U + chr(4) + 'mode' then do:
        ...
        if available buf_scn-list-hist then do:
          assign
          buf_scn-list-hist.hist-mode =  p-hist-mode
          buf_scn-list-hist.num-recs  = (if buf_scn-list-hist.line = 0 then p-num-recs else buf_scn-list-hist.num-recs)
          buf_scn-list-hist.list-table = (if p-list-table <> '':U and p-list-table <> ?
                                     then p-list-table
                                     else buf_scn-list-hist.list-table)
          .
...
define buffer l-scn-list for scn-list.
define stream sout.
...
.
DEFINE QUERY br-list FOR scn-list SCROLLING.
DEFINE BROWSE br-list QUERY br-list NO-LOCK DISPLAY
      scn-list.to-sel  format "+/" column-label "*"
      scn-list.gds-code FORMAT "99999999999":U
      scn-list.artic
      scn-list.gds-name format "x(25)"
      scn-list.qnty column-label "Количество" format "->>>,>>9.999"
      scn-list.unit-base column-label "Изм" format "x(3)"
      (if scn-list.stts = 0 then no else yes) column-label "-" format "-/ "
      scn-list.grp-name
      (scn-list.prod-type + " " + string (scn-list.prod-code, ">>>>>>>>>9") ) column-label "Производитель" format "x(9)"
      enable scn-list.qnty
...
on
  return of
  scn-list.qnty in browse br-list,
...
run create-scn-list-hist in this-procedure (input 'title'
                                     , input-output v-seq
                                     , input 0
                                     , input 'N':U
                                     , input v-value
...
define variable varscales-pref30 as character no-undo .
define variable varpgscales-pref30 as character no-undo.
define variable vss-include-info31 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable varscales-pref-type31 as character no-undo.
varscales-pref30  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  ...
  ,input  '':u
  ,input  no
  ,output varscales-pref30
  ,output varscales-pref-type31
  ) no-error .
if varscales-pref30 = ? then do:
  assign
  varscales-pref30 = '21,23,25':U.
end.
define variable varpgscales-pref-type31 as character no-undo.
varpgscales-pref30  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'scpgpref':u
  ,input  '':u
  ...
  ,input  '':u
  ,input  no
  ,output varpgscales-pref30
  ,output varpgscales-pref-type31
  ) no-error .
if varpgscales-pref30 = ? then do:
  assign
  varpgscales-pref30 = '24IIIIIQQ000C,28IIIIIQQQ00C':U.
end.
...
PROCEDURE proc-any-printable-br-list :
  if input frame Dialog-Frame a-n-c = "art" then do:
    if last-event:label = " " and
       loc-art = "" then
    return error.
    find first l-scn-list where
                l-scn-list.artic begins (loc-art + last-event:label)
               no-lock no-error.
    if available l-scn-list then do:
      loc-art = loc-art + last-event:label.
      disp loc-art with frame Dialog-Frame.
      line-rec = recid (l-scn-list).
...
PROCEDURE proc-backspace-br-list:
  if input frame Dialog-Frame a-n-c = "art" then do:
    ...
    find first l-scn-list where
                l-scn-list.artic begins loc-art
               no-lock.
    disp loc-art with frame Dialog-Frame.
    line-rec = recid (l-scn-list).
    reposition br-list to recid line-rec no-error.
...
,input  yes
,input  no
,input  varscales-pref30
,input  varpgscales-pref30
,output varresult
,output vartype-bc
,output varweight
,buffer buf_bar-code
...
,buffer buf_place
  ...
  buf_bar-code.gds-code No-LOCK.
        find first l-scn-list where
                  l-scn-list.artic = l-goods.artic AND
                  l-scn-list.prod-type = l-goods.prod-type AND
                  l-scn-list.prod-code = l-goods.prod-code no-lock no-error.
    if available l-scn-list then do:
      line-rec = recid (l-scn-list).
      reposition br-list to recid line-rec no-error.
      apply "value-changed" to br-list in frame Dialog-Frame.
...
PROCEDURE  proc-mouse-dbl-click-loc-name:
  ...
  loc-name.
    if last-event:label = "Ctrl-J" then
      find next l-scn-list where
                can-find (ub.goods where ub.goods.artic = l-scn-list.artic and
                ub.goods.prod-type = l-scn-list.prod-type and
                ub.goods.prod-code = l-scn-list.prod-code and
...
if not available scn-list or recid (scn-list) <> line-rec then do:
    hide loc-art in frame Dialog-Frame.
    loc-art = "".
end.
if available scn-list then do:     find ub.clients where ub.clients.obj-type = scn-list.prod-type                    and ub.clients.obj-code = scn-list.prod-code no-lock.     find ub.gds-prt where ub.gds-prt.upper-code = scn-list.prt-root no-lock.     find ub.bar-code where ub.bar-code.gds-code  = scn-list.gds-code                     and ub.bar-code.node-code = ub.gds-prt.node-code                     and ub.bar-code.unit-cli  = scn-list.unit-base                     and ub.bar-code.in-code   = ""                     and ub.bar-code.part-code = "" no-lock.     disp ub.clients.obj-name ub.gds-prt.node-name ub.bar-code.b-code tot-lns @ f-tot-lns with frame Dialog-Frame.   end.   else display f-tot-lns with frame Dialog-Frame.
end.
...
if not available scn-list then do:
  message "Неправильно выбран товар."
          view-as alert-box error.
  return no-apply.
end.
find ub.goods where scn-list.artic = ub.goods.artic
            and scn-list.prod-type = ub.goods.prod-type
            and scn-list.prod-code = ub.goods.prod-code no-lock.
...
find first buf_scn-list-hist no-lock where buf_scn-list-hist.id = 0 no-error .
PUT  STREAM PrnLibStream unformatted
SPACE(25) "История создания списка товаров "
(if available buf_scn-list-hist
then buf_scn-list-hist.des
...
skip(0)
.
for each buf_scn-list-hist where buf_scn-list-hist.id > 0
by buf_scn-list-hist.id
:
  put stream PrnLibStream unformatted
  (if buf_scn-list-hist.line = 0
   then string(buf_scn-list-hist.id, ">>>>>>>>9")
   else fill(chr(32) , 9)
  )  chr(32)
  (if buf_scn-list-hist.item_ <> '':U
   then string(buf_scn-list-hist.hist-mode, "X(8)")
   else fill( chr(32), 8)) chr(32)
  string(buf_scn-list-hist.num-add, ">>>>>>>>9") chr(32) chr(32) chr(32) chr(32)
  string(buf_scn-list-hist.num-recs, ">>>>>>>>9")  chr(32)
  string(buf_scn-list-hist.des, "X(155)") skip.
end.
...
define variable glog as logical no-undo .
define buffer buf-scn-list-hist for scn-list-hist.
glog = no.
...
tot-lns = 0.
v-seq = 1.
for each buf-scn-list-hist:
  delete buf-scn-list-hist.
end.
run create-scn-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                     , input-output v-seq
                                     , input 0
                                     , input '0':U
...
ON VALUE-CHANGED OF RS-Status IN FRAME Dialog-Frame DO:
  ...
  RS-status.
END.
ON RETURN of scn-list.qnty in browse br-list do:
  APPLY "LEAVE" to self.
END.
...
define buffer buf_keep-macro-list-hist for keep-macro-list-hist.
 if p-keep-query then do:
   ...
   run create-scn-list-hist in this-procedure (
                                          input 'ДОБАВЛЕНИЕ':U
                                        , input-output v-seq
                                        , input 0
                                        , input '0':U
...
DO:
  if not available scn-list then return no-apply.
  assign rs-list-method = "single".
...
for each buf_scn-list-hist where
        buf_scn-list-hist.id >=  an-listp-start-macro-id
  and  buf_scn-list-hist.id <=  an-listp-end-macro-id:
  create macro-list-hist.
  buffer-copy buf_scn-list-hist
  ...
  for each macro-list-hist where (p-id = 0 or macro-list-hist.id = p-id):
    ...
    CASE entry(1, macro-list-hist.option_, chr(4)):
      ...
      when 'clear':U then do:
        ...
        for each scn-list-hist where scn-list-hist.id > an-listp-macros-label-id:
          delete scn-list-hist.
        end.
        tot-lns = 0.
        ...
        to buf_scn-list-hist
        assign buf_scn-list-hist.id = v-seq
        v-temp-seq = v-seq
        v-seq = v-seq + 1
      ...
      when 'single' then do:
        ...
        if not error-status:error then do:
          ...
          find first  buf_scn-list-hist where
                  buf_scn-list-hist.id = v-seq - 1 no-error .
          if available buf_scn-list-hist then
          assign
          macro-list-hist.num-recs = buf_scn-list-hist.num-recs
          macro-list-hist.num-add = buf_scn-list-hist.num-add
          macro-list-hist.num-ignored = buf_scn-list-hist.num-ignored
          .
        end.
        else do:
          return ("error" + chr(4) + return-value).
        ...
        then do:
          if macro-list-hist.line = 0 then do:
            ...
            for each buf_macro-list-hist where
              ...
              to buf_scn-list-hist
              assign
              buf_scn-list-hist.id = (if buf_macro-list-hist.line =0 then v-seq else v-temp-seq)
            ...
            for each buf_scn-list-hist where
                    buf_scn-list-hist.id = v-temp-seq,
               first buf_macro-list-hist where
                    buf_macro-list-hist.id = v-id
                AND buf_macro-list-hist.line = buf_scn-list-hist.line:
               assign
               buf_macro-list-hist.num-recs = buf_scn-list-hist.num-recs
               buf_macro-list-hist.num-add = buf_scn-list-hist.num-add
               buf_macro-list-hist.num-ignored = buf_scn-list-hist.num-ignored
               .
            end.
...
if available scn-list then   do:
  find first buf_scn-list where
           recid(buf_scn-list) = recid(scn-list).
  assign
  buf_scn-list.to-sel = not (buf_scn-list.to-sel).
...
define variable varscales-pref-type42 as character no-undo.
varscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'sclspref':u
  ,input  '':u
  ...
  ,output varscales-pref-type42
  ) no-error .
if varscales-pref = ? then do:
  assign
  varscales-pref = '21,23,25':U.
end.
define variable varpgscales-pref-type42 as character no-undo.
varpgscales-pref  = ?.
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run conf-rd in g#library
  (input  'scpgpref':u
  ,input  '':u
  ...
  ,output varpgscales-pref-type42
  ) no-error .
...
do:
  ...
  scn-list.gds-name:resizable in browse br-list = true
  scn-list.grp-name:resizable in browse br-list = true
...
define buffer buf_scn-list-hist for scn-list-hist.
  find first buf_scn-list-hist where buf_scn-list-hist.id = 0 no-error.
  if available buf_scn-list-hist then
  assign
  frame Dialog-Frame:title = substitute("СПИСОК  ТОВАРОВ &1",  string(buf_scn-list-hist.des, "X(60)"))
  .
  if tot-lns = ? then do:
    v-start = yes.
    for each l-scn-list :
      accumulate l-scn-list.artic (count).
    end.
    tot-lns = (accum count l-scn-list.artic).
    if tot-lns > 0 then do:
      find last  buf_scn-list-hist no-error .
      v-seq = (if available buf_scn-list-hist then buf_scn-list-hist.id else 0)  + 1.
      run create-scn-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                           , input-output v-seq
                                           , input 0
                                           , input 'S':U
      ...
      for each buf_scn-list-hist:
        delete buf_scn-list-hist.
      end.
      v-seq = 1.
      run create-scn-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                           , input-output v-seq
  ...
  if v-seq > 1 then
  find last buf_scn-list-hist no-lock where
            buf_scn-list-hist.id = (v-seq - 1)
       and  buf_scn-list-hist.line = 0 no-error .
  DISPLAY br-option
  (if available buf_scn-list-hist
  then buf_scn-list-hist.des
  else '') @ dsp-rs
  ...
  if v-start then do:
    ...
    then do:
      scn-list.to-sel:visible in browse br-list = yes.
    end.
    else do:
      scn-list.to-sel:visible in browse br-list = no.
    end.
    if bttns <> '':U then do:
  ...
  open query br-list for each scn-list no-lock indexed-reposition.
  if line-rec <> ? then
    reposition br-list to recid line-rec no-error.
  if available scn-list then do:     find ub.clients where ub.clients.obj-type = scn-list.prod-type                    and ub.clients.obj-code = scn-list.prod-code no-lock.     find ub.gds-prt where ub.gds-prt.upper-code = scn-list.prt-root no-lock.     find ub.bar-code where ub.bar-code.gds-code  = scn-list.gds-code                     and ub.bar-code.node-code = ub.gds-prt.node-code                     and ub.bar-code.unit-cli  = scn-list.unit-base                     and ub.bar-code.in-code   = ""                     and ub.bar-code.part-code = "" no-lock.     disp ub.clients.obj-name ub.gds-prt.node-name ub.bar-code.b-code tot-lns @ f-tot-lns with frame Dialog-Frame.   end.   else display f-tot-lns with frame Dialog-Frame.
END PROCEDURE.
        ...
        do:
            ...
            do:
                for first ub.Code no-lock where ub.Code.parent = "TiketPrint" and
                    ...
                    find first scn-list where scn-list.artic = buf_goods.artic
                        and scn-list.prod-type = buf_goods.prod-type
                        and scn-list.prod-code = buf_goods.prod-code no-error.
                    if available scn-list then
                    do:
...
find scn-list
  where scn-list.prod-type = buf_goods.prod-type
    and scn-list.prod-code = buf_goods.prod-code
    and scn-list.artic     = buf_goods.artic
  no-error .
if available scn-list then do:
  assign
    scn-list.to-del = no
  ...
  define variable v-last48 as integer no-undo .
  find last scn-list use-index oi no-error.
  if available scn-list then do:
    v-last48 = scn-list.order-num .
  end.
  else do:
    v-last48 = 0 .
  end.
  create scn-list .
  buffer-copy buf_goods to scn-list
  assign
    scn-list.to-del = no
    scn-list.order-num = v-last48 + 1
  .
  assign
    lns-cnt = lns-cnt + 1
  ...
  .
                ...
                do:
                    for first ub.Code no-lock where ub.Code.parent = "TiketPrint" and
                        ub.code.status_ = 0 and ub.Code.code = string(buf_goods.gds-code):
                        find first scn-list where scn-list.gds-code = buf_goods.gds-code no-error.
                        if available scn-list then
                        do:
                            scn-list.qnty = decimal(ub.Code.CodeValue) .
                        end.
                    end.
                end.
            end.
...
case rs-list-method:
  when "all" then do:
    ...
    assign                                                           buf_scn-list-hist.num-add  = lns-cnt - v-num-add                       buf_scn-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_scn-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_scn-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
  end.
  when "gds-grp" then do:
    for each buf_scn-list-hist where
             buf_scn-list-hist.id = p-id
        and  buf_scn-list-hist.item_ <> '':U:
      run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_scn-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then  next.
      grp-path = "".
  ...
  when "object" then do:
    find first buf_scn-list-hist where
             buf_scn-list-hist.id = p-id
        and  buf_scn-list-hist.item_  <> '':U.
    run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_scn-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then do: end. else do:
  ...
  when "waybill" then do:
    ...
    for each buf_scn-list-hist where
             buf_scn-list-hist.id = p-id
        and  buf_scn-list-hist.item_  <> '':U:
      run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_scn-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then next _ii.
      ...
      if p-from-macro and not p-step then do:
        ...
        if can-do ('рас,при,возврат,спи':U, trn-doc.doc-type) then
      ...
      for each ub.doc-line where ub.doc-line.doc-code = ub.trn-doc.doc-code no-lock,
        ...
        run ex-gds in this-procedure ( buffer ub.goods, input rs-list-method, input rs-status, input line-mode).
        if glog and
            available scn-list and
            scn-list.artic = ub.goods.artic and
            scn-list.prod-type = ub.goods.prod-type and
            scn-list.prod-code = ub.goods.prod-code then do:
          scn-list.qnty = if can-do ('рас,спи':U, ub.trn-doc.doc-type) then
                      scn-list.qnty - ub.doc-line.fact-qnty
                    else
                      scn-list.qnty + ub.doc-line.fact-qnty.
          if scn-list.qnty < 0 then do:
            ...
            message "Артикул:" ub.doc-line.artic skip
                    ...
                    "Итоговое количество в списке:" scn-list.qnty skip (2)
                    "Заменено на 0."
                    view-as alert-box .
            scn-list.qnty = 0.
          end.
...
FOR EACH
  ...
  when "overvalue" then do:
    ...
    for each buf_scn-list-hist where
             buf_scn-list-hist.id = p-id
        and  buf_scn-list-hist.item_  <> '':U:
      run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_scn-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then next _jj.
  ...
  when "pdf" then do:
    ...
    when "collection" then do:
        run str/any-li01.p (
                        ...
                        ,input line-mode
                        ,input p-id
                        ,INPUT-OUTPUT table buf_scn-list-hist
                        ) no-error .
  end.
  when "abcxyz"
  or
  ...
  when "izt" then do:
    for each buf_scn-list-hist where
        ...
        and  buf_scn-list-hist.item_  <> '':U:
      assign
      v-attr-code  = entry(1, buf_scn-list-hist.item_, chr(3))
      v-obj-type = entry(2, buf_scn-list-hist.item_, chr(3))
      v-obj-code = integer(entry(3, buf_scn-list-hist.item_, chr(3)))
      .
  ...
  when "available" then do:
    ...
    run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_scn-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then do: end. else do:
      ...
      for each ub.gds-obj where
        ...
        run ex-gds in this-procedure ( buffer goods, input rs-list-method, input rs-status, input line-mode).
          if available scn-list then do:
            scn-list.qnty = gds-obj.free-qnty.
          end.
      end.
      assign                                                           buf_scn-list-hist.num-add  = lns-cnt - v-num-add                       buf_scn-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_scn-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_scn-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
    end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run emptyscl in g#library
  ...
  when "is-ptrl" then do:
    find first buf_scn-list-hist where
             buf_scn-list-hist.id = p-id  no-error .
  ...
  then do:
    find first buf_scn-list-hist where
         ...
         AND buf_scn-list-hist.item_ <> '':U .
    assign
    v-obj-type = entry(1, buf_scn-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_scn-list-hist.item_, chr(3)))
    v-attr-code  = (if rs-list-method = "fbr-gds-obj-val"
                    then entry(3, buf_scn-list-hist.item_, chr(3))
                    else '':U)
    vvalue = (if rs-list-method = "fbr-gds-obj-val"
              then entry(4, buf_scn-list-hist.item_, chr(3))
              else '':U)
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "dis-gds-rule" or when "dis-gds-rule-num"  then do:
    find first buf_scn-list-hist where
             buf_scn-list-hist.id = p-id
         AND buf_scn-list-hist.item_ <> '':U no-error .
    assign
    v-obj-type = entry(1, buf_scn-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_scn-list-hist.item_, chr(3)))
    v-discnt-role = entry(3, buf_scn-list-hist.item_, chr(3))
    vvalue-int = (if rs-list-method = "dis-gds-rule" then 0 else integer(entry(4, buf_scn-list-hist.item_, chr(3))))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "gds-obj-attr" or when "gds-obj-attr-val"  then do:
    ...
    v-obj-type = entry(1, buf_scn-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_scn-list-hist.item_, chr(3)))
    v-attr-code = entry(3, buf_scn-list-hist.item_, chr(3))
    vvalue = (if rs-list-method = "gds-obj-attr" then '':U else entry(4, buf_scn-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "gds-host-attr-val" then do:
    find first buf_scn-list-hist where
         ...
         AND buf_scn-list-hist.item_ <> '':U .
    assign
    v-host-code = integer(entry(1, buf_scn-list-hist.item_, chr(3)))
    v-attr-code = entry(2, buf_scn-list-hist.item_, chr(3))
    vvalue = (if rs-list-method = "gds-host-attr" then '':U else entry(3, buf_scn-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  then do:
    ...
    assign
    v-attr-code = entry(1, buf_scn-list-hist.item_, chr(3))
    vvalue = (if rs-list-method = "goods-attr" then '':U else entry(2, buf_scn-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "tax-rate-value" then do:
    ...
    v-obj-type = entry(1, buf_scn-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_scn-list-hist.item_, chr(3)))
    v-date = date(entry(3, buf_scn-list-hist.item_, chr(3)))
    v-attr-code = entry(4, buf_scn-list-hist.item_, chr(3))
    vvalue = entry(5, buf_scn-list-hist.item_, chr(3))
    no-error
    .
    if error-status:error then do: end. else do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run pftxvalg in g#library
  ...
  when "scales-gds" or when "scales-gds-num"  then do:
    ...
    v-obj-type = entry(1, buf_scn-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_scn-list-hist.item_, chr(3)))
    vvalue = (if rs-list-method = "scales-gds" then '':U else entry(3, buf_scn-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "parts-last-date" then do:
    ...
    v-obj-type = entry(1, buf_scn-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_scn-list-hist.item_, chr(3)))
    vvalue-int = integer(entry(3, buf_scn-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do: end. else do:
  ...
  when "filter" then do:
    ...
    find first buf_scn-list-hist where
             buf_scn-list-hist.id = p-id
         AND buf_scn-list-hist.item_ <> '':U .
    run proc-write-filter-expression-var in this-procedure ( input buf_scn-list-hist.item_, output v-filter-var ).
    run gbl/scn-fill.p (
  ...
  when "move-date":U then do:
      ...
      for each gds-obj  no-lock where gds-obj.obj-type = v-cntxt-obj-type
                                  and gds-obj.obj-code = v-cntxt-obj-code
                                  and gds-obj.last-doc >= date(buf_scn-list-hist.item_):
...
if rs-list-method = "single" then do:
  if v-no-hist < 0 then do:
    run create-scn-list-hist in this-procedure(input 'ДОБАВЛЕНИЕ':U
                                        ...
                                        , input substitute("ТОВАР код &1 &2 &3&4 &5", scn-list.gds-code, scn-list.artic, scn-list.prod-type, scn-list.prod-code, scn-list.gds-name)
                                        , input tot-lns
                                        , input rs-list-method
                                        , input rs-status
                                        , input ('goods':U + chr(3) + string(scn-list.gds-code))
                                        , input '':U
                                        , input ?
                                        ).
  end.
    ...
    do v-ii = 0 to v-no-hist:
      run create-scn-list-hist in this-procedure(input ('ИЗМЕНЕНИЕ':U + chr(4) + 'mode':U)
...
for each scn-list:
find ub.goods where ub.goods.artic     = scn-list.artic     and
                ub.goods.prod-type = scn-list.prod-type and
                ub.goods.prod-code = scn-list.prod-code no-lock.
    create tt-goods.
...
define variable v-param-type72 as character no-undo .
define variable v-value-date72 as date no-undo .
define variable v-value-decimal72 as decimal no-undo .
define variable v-value-integer72 as INTEGER no-undo .
define variable v-value-logical72 AS LOGICAL no-undo .
define variable v-tth72 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  v-cntxt-obj-type
    ,input  v-cntxt-obj-code
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd
    ,output v-value-date72
    ,output v-value-decimal72
    ,output v-value-integer72
    ,output v-value-logical72
    ,output v-param-type72
    ,INPUT-OUTPUT table-handle v-tth72
    )  .
delete object v-tth72 no-error.
  assign
...
define buffer buf_scn-list-hist for scn-list-hist.
  ...
  for each scn-list,
      each ub.gds-prt where ub.gds-prt.upper-code = scn-list.prt-root no-lock,
      each ub.bar-code where ub.bar-code.gds-code = scn-list.gds-code
                      and ub.bar-code.unit-cli = scn-list.unit-base
                      and ub.bar-code.node-code = ub.gds-prt.node-code
                      and ub.bar-code.in-code = ""
                      and ub.bar-code.part-code = "" no-lock:
    if scn-list.qnty <> 0 then
      put stream PrnLibStream unformatted string (ub.bar-code.b-code) + "," + string (scn-list.qnty) skip.
  end.
  output stream PrnLibStream close.
  run waitfram-hide in this-procedure .
  end.
  ...
  :
    ...
    if rs-list-method = "single" then do:
      ...
      if p-from-macro then do:
        ...
        find first scn-list where scn-list.gds-code = ub.goods.gds-code no-error.
      end.
      if available scn-list then do:
        line-rec = recid (scn-list).
        get next br-list.
        if available scn-list then v-rep-rec = recid (scn-list).
        else do:
          reposition br-list to recid line-rec no-error.
          get prev br-list.
          if available scn-list then v-rep-rec = recid (scn-list).
  ...
  :
    ...
    if rs-list-method = "single" then do:
      ...
      if available scn-list then do:
        ...
        run write-hist  in this-procedure ( p-from-macro, rs-list-method, rs-status, line-mode).
        for each scn-list:
          if line-rec <> recid (scn-list) then delete scn-list.
        end.
        assign
        tot-lns = 1
        .
...
run gbl/filename.p
    (input  (if rs-list-method = "clob-data" then v-file-name else buf_scn-list-hist.item_  )
  ,output v-full-path
  ,output v-path
  ,output v-file-name
  ,output v-file-name-no-ext
  ...
  CASE rs-list-method:
        ...
        if imp-doc-type = 'переоценка':U then do:
          ...
          if avail(ub.price-doc) then do:
            FOR EACH ub.price-list No-LOCK WHERE
              ...
              run ex-gds  in this-procedure ( buffer ub.goods, input rs-list-method, input rs-status, input line-mode).
              if available scn-list and
                scn-list.artic = ub.goods.artic and
                scn-list.prod-type = ub.goods.prod-type and
                scn-list.prod-code = ub.goods.prod-code then
                scn-list.qnty = scan-qnty.
            END.
          end.
        end.
        else do:
    ...
    then do:
      ...
      repeat:
        ...
        if available ub.goods and  available scn-list and
          scn-list.artic = ub.goods.artic and
          scn-list.prod-type = ub.goods.prod-type and
          scn-list.prod-code = ub.goods.prod-code then
          scn-list.qnty = scan-qnty.
...
,buffer ub.place
        ...
        if available scn-list and
          ...
          scn-list.prod-type = ub.goods.prod-type and
          scn-list.prod-code = ub.goods.prod-code then
          scn-list.qnty = scn-list.qnty + scan-qnty * bc-qnty.
      end.
    end.
  END CASE.
  input stream sout close.
...
find first buf_goods no-lock where
        buf_goods.gds-code = scn-list.gds-code no-error.
if available scn-list then do:
  assign
...
define input parameter p-gds-code as integer no-undo .
define input parameter p-bh as handle no-undo .
define buffer buf_gds-list for scn-list.
