...
CASE p-pos-type:
  ...
  then do:
    ...
    FOR EACH ub.currency NO-LOCK,
      ...
      if p-pos-type = 'Emulator-NKT-IBM':U
...
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  fname = string( year( today ) modulo 10 ) +
          ...
          string( next-value( s-file-num, ub ), "999" ) .
  .
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'добавление курсов валют')
                  else ('Ждите - ' + 'удаление курсов валют') ) +
...
put stream IBMStream ' ' skip(1).
  if Cash-OS2
  then
  put stream IbmStream unformatted  '  ' for-cash-desk.addr-path  ' plu':U skip .
  else
  put stream IbmStream unformatted  '  ' for-cash-desk.addr-path  ' currency':U skip .
output stream IBMStream close.
OS-RENAME
...
if action  = "U" then do:
  ...
  put ' ' skip(1).
  put unformatted '  ' for-cash-desk.addr-path ' currency' skip.
  output close.
  run str/wait.w ( out + fname + '.dat', "'добавление курсов валют'" ) NO-ERROR.
  if error-status:error then return .
  run write-log-and-file in p-log-handle (
        input 1
      , input log-file-name
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  0
    ,input  'actn_cashdesk-rates_update':U
    ,input  'object':U
    ,input  v-host-code
    ,input  p-obj-type
    ,input  i-obj-code
...
define variable v-param-type38 as character no-undo .
define variable v-value-date38 as date no-undo .
define variable v-value-decimal38 as decimal no-undo .
define variable v-value-integer38 as INTEGER no-undo .
define variable v-value-logical38 AS LOGICAL no-undo .
define variable v-tth38 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  p-obj-type
    ,input  i-obj-code
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd
    ,output v-value-date38
    ,output v-value-decimal38
    ,output v-value-integer38
    ,output v-value-logical38
    ,output v-param-type38
    ,INPUT-OUTPUT table-handle v-tth38
    )  .
delete object v-tth38 no-error.
  ...
  if v-curr-r-b = 'base':U then do:
    ...
    FOR EACH currency NO-LOCK :
        FIND LAST ub.curr-shop WHERE ub.curr-shop.obj-code = i-obj-code
                  ...
                  curr-list = curr-list + string( ub.curr-shop.curr-code ) + chr(44) .
    END.
    curr-list = right-trim( curr-list, chr(44) ) .
