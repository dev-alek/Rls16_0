...
END.
ON CHOOSE OF b-join IN FRAME d-utd
DO:
...
assign
  v-str-recid23 = trim( string( recid( X_utd ) , "->>>>>>>>>>>9":U ) )
...
assign
  v-str-recid25 = trim( string( recid( X_utd ) , "->>>>>>>>>>>9":U ) )
  v-num-entry25 = lookup( v-str-recid25 , v-rid-list )
.
if v-num-entry25 > 0 then do:
  assign
    entry( v-num-entry25, v-rid-list ) = "":U
    v-rid-list = trim( replace( v-rid-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    v-rid-list = v-rid-list + ( if v-rid-list = "":U then "":U else chr(44) ) + v-str-recid25
  .
end.
                loc#log = br-utd:refresh() .
            end.
...
do:
  ...
  then do:
    ASSIGN
      f-date-from :POPUP-MENU in frame d-utd = MENU m-ed-date28 :HANDLE
      f-date-from :MENU-MOUSE in frame d-utd = 3
    .
  end.
  define variable v-label-handle28 as handle no-undo .
  assign
    v-label-handle28 = f-date-from :side-label-handle in frame d-utd
  .
...
do:
  ...
  then do:
    ASSIGN
      f-date-to :POPUP-MENU in frame d-utd = MENU m-ed-date30 :HANDLE
      f-date-to :MENU-MOUSE in frame d-utd = 3
    .
  end.
  define variable v-label-handle30 as handle no-undo .
  assign
    v-label-handle30 = f-date-to :side-label-handle in frame d-utd
  .
...
PROCEDURE init-temp :
    RADIO-SET-1 = 2 .
    radio-set-1:screen-value in frame d-utd = "2" .
    run init-sort .
    OPEN QUERY br-utd FOR EACH X_utd NO-LOCK by X_utd.DocumentDate desc by X_utd.Timestamp desc INDEXED-REPOSITION.
END PROCEDURE.
FUNCTION CliName RETURNS CHARACTER
