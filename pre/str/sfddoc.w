...
DEFINE BROWSE b-fin-doc
 ...
 tt-fd.sum-rubl FORMAT "->>>,>>>,>>>,>>9.99" COLUMN-LABEL "Сумма платежа (abbr_rub)"
...
DEFINE BROWSE b-parts
 ...
 bf_parts.part-code FORMAT "x(13)" COLUMN-LABEL "Код партии"
...
END.
ON VALUE-CHANGED OF b-fin-doc IN FRAME Dialog-Frame
DO:
  RUN open-query-fo IN THIS-PROCEDURE.
  RUN open-parts-query IN THIS-PROCEDURE.
END.
ON VALUE-CHANGED OF b-fin-ob IN FRAME Dialog-Frame
DO:
  RUN open-parts-query IN THIS-PROCEDURE.
END.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse b-fin-doc :handle
      .
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  ASSIGN FRAME
    Dialog-Frame:TITLE = "Платежи по складскому документу " + pardoc-code.
  FIND FIRST bf_trn-doc WHERE bf_trn-doc.doc-code = pardoc-code NO-LOCK.
  RUN cr-tt-fd IN THIS-PROCEDURE.
  assign
  tt-fd.sum-rubl:label in browse b-fin-doc         = "Сумма платежа (руб)"
  tt-fd.sum-rubl-fo:label in browse b-fin-doc      = "Сумма связей С ФО (руб)"
  tt-fo.sum-rubl:label in browse b-fin-ob          = "Общая сумма (руб)"
  bf_fin-connect.sum-rubl:label in browse b-fin-ob = "Сумма связи (руб)"
  tt-fo.sum-rubl-trn:label in browse b-fin-ob      = "Сумма по док. (руб)"
  bf_parts.price-rubl:label in browse b-parts      = "Цена (руб)"
  .
  RUN enable_UI.
...
IF AVAILABLE tt-fo THEN DO:
    ...
    DISPLAY b-parts WITH FRAME Dialog-Frame.
END.
END PROCEDURE.
...
IF AVAILABLE tt-fd THEN DO:
  ...
  DISPLAY b-fin-ob WITH FRAME Dialog-Frame.
END.
END PROCEDURE.
