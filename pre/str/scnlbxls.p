...
define new shared temp-table buf-gds-list no-undo like ub.goods
  field qnty   as decimal
  field to-del as logical
  field order-num as integer
...
define  new shared  temp-table buf-gds-list-hist no-undo
field list-table as character
field id as integer
field line as integer
field hist-mode as character
...
define variable vss-include-info17 as character format "x(65)" no-undo initial "@(#)$Workfile: prtmpldf.i $ $Revision: 06dcfe20b136, 752, rls $".
...
define variable vss-include-info22 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str22  as character no-undo.
  define variable tmp-num22  as character no-undo.
  define variable i22        as integer   no-undo.
  define variable sum22      as integer   no-undo.
  define variable len-code22 as integer   no-undo.
  define variable varcont22  as logical   initial yes no-undo.
  CASE bc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str22 = string( internal-b-code, "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str22 = string( internal-b-code, "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont22 = yes then do:
    if integer( substring( tmp-str22, 1, length( bc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = bc-pfx + substring( tmp-str22, length( bc-pfx ) + 1, length( tmp-str22 ) - length( bc-pfx ) )
        len-code22    = length( full-b-code )
      .
      define variable v-sum-char22 as character no-undo .
      assign
        sum22 = 0
      .
      do i22 = 1 to len-code22 by 2
      :
        assign
          v-sum-char22 = substr(full-b-code, len-code22 - i22 + 1, 1)
        .
        if v-sum-char22 < "0"
        or v-sum-char22 > "9"
        then do:
        ...
        assign
          sum22 = sum22 + integer(v-sum-char22)
        .
      end.
      if varcont22 = yes then do:
        assign
        ...
        if varcont22 = yes then do:
           if sum22 mod 10 = 0 then do:
             ...
             assign
               full-b-code = full-b-code + string(10 - sum22 mod 10)
             .
           end.
        end.
      end.
...
define buffer buf_usr-flt_custom-labels for usr-flt_custom-labels.
 ...
 if avail fi-goods then do:
      FIND FIRST fi-gds-obj NO-LOCK where
                 ...
                 fi-gds-obj.obj-type = p-curr-obj-type AND
                 fi-gds-obj.obj-code = p-curr-obj-code NO-ERROR.
...
define buffer buf_usr-flt_custom-labels for usr-flt_custom-labels.
  IF v-barcodfi = "" then
  v-barcodfi = 'bar-code.b-code,bar-code.unit-cli,bar-code.cli-base-rate,bar-code.gds-code,bar-code.#node-code-name,bar-code.in-code,bar-code.part-code,bar-code.cr-db-num':U.
...
vNum-entries = NUm-ENTRIES(v-barcodfi).
  DO ii = 1 to vNum-entries:
    ...
    find first buf_usr-flt_custom-labels where
              buf_usr-flt_custom-labels.tbl-name = entry(1, entry-ii, ".")
         and  buf_usr-flt_custom-labels.fld-name = entry(2, entry-ii, ".")
         and  buf_usr-flt_custom-labels.call-point = 'barcodfi':U
         and  buf_usr-flt_custom-labels.call-type = 'add-fields':U no-error.
    if  available buf_usr-flt_custom-labels then do:
      case buf_usr-flt_custom-labels.tbl-name:
...
define variable v-last-inv-date-num as character no-undo.
  ...
  :
    CASE p-f-name:
      ...
      when "qnty":U then do:
        return string(scnblist.qnty,  p-format).
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsbcode in g#library
  ...
  ,output main-b-code
        ...
        run gbl/bctotext.p (
                        input (if length(bar_code) = 13 then 'ean13':U else 'ean8':U)
                       ,input bar_code
                       ,output bar_bar_code) no-error .
        return (chr(4) + bar_bar_code).
      end.
      when "last-inv" then do :
        find last buf_doc-line no-lock where
                  buf_doc-line.artic = scnblist.artic
              and buf_doc-line.prod-code = scnblist.prod-code
              and buf_doc-line.prod-type = scnblist.prod-type
...
for each  buf_custom-labels no-lock where
          buf_custom-labels.call-type = 'add-fields':U
...
for each  buf_custom-labels no-lock where
          buf_custom-labels.call-type = 'add-fields':U
      and buf_custom-labels.call-point = 'barcodfi':U:
  create buf_usr-flt_custom-labels.
  buffer-copy buf_custom-labels to buf_usr-flt_custom-labels.
end.
...
do jj = 1 to num-entries(ubflt.filter.Fields-sort):
  ...
  do jj-1 = 1 to num-entries(entry(jj, ubflt.filter.Fields-sort), "*":U):
    if (entry(2, entry(jj-1, entry(jj, ubflt.filter.Fields-sort), "*":U), ".") = "artic"
        or
        entry(2, entry(jj-1, entry(jj, ubflt.filter.Fields-sort), "*":U), ".") = "b-str")
    ...
    case entry(1, entry(jj-1, entry(jj, ubflt.filter.Fields-sort), "*":U), "."):
      when 'goods':U or when 'gds-obj':U then do:
        assign
        f-n = entry(2, entry(jj-1, entry(jj, ubflt.filter.Fields-sort), "*":U), ".")
        t-name = entry(1, entry(jj-1, entry(jj, ubflt.filter.Fields-sort), "*":U), ".")
        .
      end.
      when 'gds-obj-attr':U
      or
      ...
      when "function":U then do:
        ...
        f-n = entry(jj-1, entry(jj, ubflt.filter.Fields-sort), "*":U)
        .
        if entry(2, entry(jj-1, entry(jj, ubflt.filter.Fields-sort), "*":U), ".") = 'gdsbcode-graphic':U
        then do:
          assign
          v-osn-frm = v-osn-frm + (if v-osn-frm = "":U then "":U else ";") + string(jj) + "=0":U
          v-eng-frm = v-eng-frm + (if v-eng-frm = "":U then "":U else ";") + string(jj) + "=@":U
    ...
    END CASE.
    assign
    f-name-1 = f-name-1 + (if f-name-1 = "":U then "":U else "*":U) + (if t-name <> '':U then (t-name + ".") else '') + f-n
    .
    if v-frm-set = no then do:
      ...
      v-osn-frm = v-osn-frm + (if v-osn-frm = "":U then "":U else ";") + string(jj) + "=" +
                  prnfield_get-fformat(entry(jj, ubflt.filter.lst-cend)
                             ,entry(jj, ubflt.filter.where-ysl-rus, chr(4)))
      .
    end.
  end.
  assign
...
.
Sheetf.Bas-Params = string(5) + chr(4) + Sheetf.Bas-Params.
Make-excel = yes.
...
run rep/extitle.p (1).
iColumn = 0.
FOR EACH scnblist no-lock,
    first ub.goods no-lock where
          ub.goods.gds-code = scnblist.gds-code:
  find first ub.bar-code no-lock  where ub.bar-code.b-code = scnblist.b-code no-error.
  if not available ub.bar-code then do:
    assign
    v-bc-ne = v-bc-ne + 1.
    release ub.prod-bc.
  end.
  if not scnblist.loc-ean and available ub.bar-code then
  find first ub.prod-bc no-lock where ub.prod-bc.b-str = scnblist.b-str and ub.prod-bc.b-code = ub.bar-code.b-code no-error.
  ...
  do jj = 1 to j-n:
    ...
    CASE v-dop1 :
      when "function":U then do:
        ...
        get-function(substr(v-dop0 ,10 ), entry(jj, ubflt.filter.Where-ysl-rus, chr(4)))
        (if jj < j-n
        then
        CHR(9)
        else chr(10))
      ...
      when 'bar-code':U then do:
        if index(v-dop0, "*":U) > 0 then do:
          do jj-1 = 1 to num-entries(v-dop0, "*":U):
            assign
            f-value-1 = "":U.
            run barcodfi in this-procedure (
                             buffer bar-code
                            ,INPUT entry(jj-1, v-dop0, "*":U)
      ...
      when 'prod-bc':U then do:
          if available prod-bc then  do:
            ...
            string((if v-dop2 = "b-str":U
                  ...
                  , (chr(34) + entry(jj, ubflt.filter.Where-ysl-rus, chr(4)) + chr(34)))
            (if jj < j-n
            then
            CHR(9)
            else chr(10))
            ...
            if scnblist.loc-ean or scnblist.b-str <> "":U then do:
              CASE v-dop2:
                when "b-str" then do:
                  f-value-1 = chr(4) + scnblist.b-str.
                end.
                when "b-on" then do:
                  f-value-1 = "yes":U.
                end.
              END CASE.
              if Make-Excel then  put   stream ForExcel unformatted
              string(f-value-1, (chr(34) + entry(jj, ubflt.filter.Where-ysl-rus, chr(4)) + chr(34)))
              (if jj < j-n
              then
              CHR(9)
              else chr(10))
      ...
      otherwise do:
        ...
        if Make-Excel then  put   stream ForExcel unformatted
        if entry(jj, f-name) = "artic" then chr(4) else "":U
        string(replace(f-value, chr(10), ""), substitute("X(&1)", entry(jj, ubflt.filter.Where-ysl)))
        (if jj < j-n
        then
        CHR(9)
        else chr(10))
...
for each scnblist-hist:
  if Make-Excel then  put   stream ForExcel unformatted
  (if scnblist-hist.line = 0
   then string(scnblist-hist.id, ">>>>>>>>9")
   else '':U)
  CHR(9)
  (if scnblist-hist.item_ <> '':U
   then scnblist-hist.hist-mode
   else '':U)  CHR(9)
   (if scnblist-hist.item_ <> '':U
   then string(scnblist-hist.num-add, "->>>>>>>>9")
   else '':U)  CHR(9)
  (if scnblist-hist.item_ <> '':U
  then string(scnblist-hist.num-recs, ">>>>>>>>9")
  else '':U)  CHR(9)
  scnblist-hist.des
  skip.
end.
...
DEFINE VARIABLE vother               as character           no-undo .
  ...
  :
    ...
    tbl = 'goods':U + chr(44) +
          ...
          'prod-bc':U + chr(44) +
          'goods-attr':U + chr(44) +
          'gds-obj-attr':U + chr(44) +
          'gds-host-attr':U + chr(44) + 'function'
    c-point = "Список кодов" + chr(4) +  "PRNT":u  + chr(4) + substitute("Список кодов - &1&2 фирма &3", p-curr-obj-type, p-curr-obj-code, v-host-code)
    fld = ""
    lab = ""
    spr = ""
    dim = '0'
    ...
    for each buf_usr-flt_custom-labels where
           buf_usr-flt_custom-labels.call-type = 'add-fields':U
       and buf_usr-flt_custom-labels.call-point = 'gdsreffi':U
       and buf_usr-flt_custom-labels.tbl-name = 'goods':U
    ...
    for each buf_usr-flt_custom-labels where
           buf_usr-flt_custom-labels.call-type = 'add-fields':U
       and buf_usr-flt_custom-labels.call-point = 'barcodfi':U
       and buf_usr-flt_custom-labels.tbl-name = 'bar-code':U
    by buf_usr-flt_custom-labels.custom-label
       :
      run prnfield-add in this-procedure(input buf_usr-flt_custom-labels.fld-name
                                        ,input buf_usr-flt_custom-labels.custom-label
    ...
    do na = 1 to num-entries('alcohol-prod,egais-name,is-gas,ptrl-without-rvs,office-type,mark-type,emrc-type,IS18Plus,loyalty-gift,item-matter-mark,type-method-calc,group-np,fuel-type,is-loyalty-payment,ban-bonus,null-price,fasovka,time-coock,mark,sum-grp-gl,min-zapas,mercur_FGIS,perishable,production-only,15x80,8x50,6x50,calories,protein,fat,carbohydrate,calc-cal-rec,cash-parts,ptrl-as-good,dflt-insalepr,gds-ptrl-densities,gds-CommodityCode,gds-code-AIS,length-of,width-of,height-of,qnty-in-box,weight-box,qnty-on-pallet,weight-of-pallet,image-list,MercUnits,weighed-gds':U):
      ...
      if NOT error-status:error and voutput-display = "yes":U then do:
        run prnfield-add in this-procedure(entry(na, 'alcohol-prod,egais-name,is-gas,ptrl-without-rvs,office-type,mark-type,emrc-type,IS18Plus,loyalty-gift,item-matter-mark,type-method-calc,group-np,fuel-type,is-loyalty-payment,ban-bonus,null-price,fasovka,time-coock,mark,sum-grp-gl,min-zapas,mercur_FGIS,perishable,production-only,15x80,8x50,6x50,calories,protein,fat,carbohydrate,calc-cal-rec,cash-parts,ptrl-as-good,dflt-insalepr,gds-ptrl-densities,gds-CommodityCode,gds-code-AIS,length-of,width-of,height-of,qnty-in-box,weight-box,qnty-on-pallet,weight-of-pallet,image-list,MercUnits,weighed-gds':U), vlabel, 'ATTR.' + VTYPE, prnfield_get-fsize(vtype, vformat, vlabel), vformat,
        input-output fld, input-output lab, input-output spr, input-output v-size, input-output v-format, input-output dim)  no-error.
      end.
    end.
    assign
    ...
    dim = dim + chr(44)
    .
    do na = 1 to num-entries('scales-code,fbr-cost-rubl,gds-margins,free-price,sum-grp,increase-pc,round-method,petrol-purse,min-zapas,need-auth,proprietor,no-income-goods,taracode,calories-o,protein-o,fat-o,carbohydrate-o,doc-tickets,normal-wastage-o,dop-alt-name-o,dt-seasons,change-dt-seasons,mark-collect-type':u):
      run gdsoattr-name in this-procedure (
                         input entry(na, 'scales-code,fbr-cost-rubl,gds-margins,free-price,sum-grp,increase-pc,round-method,petrol-purse,min-zapas,need-auth,proprietor,no-income-goods,taracode,calories-o,protein-o,fat-o,carbohydrate-o,doc-tickets,normal-wastage-o,dop-alt-name-o,dt-seasons,change-dt-seasons,mark-collect-type':u)
                        ,output vtype
                        ,output vformat
                        ,output vlabel
                        ,output vuser-can-edit
                        ...
                        ,output vother) no-error.
      if NOT error-status:error and voutput-display = "yes":U then do:
        run prnfield-add in this-procedure(entry(na, 'scales-code,fbr-cost-rubl,gds-margins,free-price,sum-grp,increase-pc,round-method,petrol-purse,min-zapas,need-auth,proprietor,no-income-goods,taracode,calories-o,protein-o,fat-o,carbohydrate-o,doc-tickets,normal-wastage-o,dop-alt-name-o,dt-seasons,change-dt-seasons,mark-collect-type':u), vlabel, 'ATTR.' + VTYPE, prnfield_get-fsize(vtype, vformat, vlabel), vformat,
        input-output fld, input-output lab, input-output spr, input-output v-size, input-output v-format, input-output dim)  no-error.
      end.
    end.
    assign
    ...
    dim = dim + chr(44)
    .
    do na = 1 to num-entries('no-envd,':u):
      run gdshattr-name(
                         input entry(na, 'no-envd,':u)
                        ,output vtype
                        ,output vformat
                        ,output vlabel
                        ,output vuser-can-edit
                        ...
                        ,output vother) no-error.
      if NOT error-status:error and voutput-display = "yes":U then do:
        run prnfield-add in this-procedure(entry(na, 'no-envd,':u), vlabel, 'ATTR.' + VTYPE, prnfield_get-fsize(vtype, vformat, vlabel), vformat,
        input-output fld, input-output lab, input-output spr, input-output v-size, input-output v-format, input-output dim)  no-error.
      end.
    end.
    assign
