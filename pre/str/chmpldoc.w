...
define variable v-t-doc as decimal   no-undo .
...
DO:
  ...
  do:
    ...
    is-color = true .
    OPEN QUERY BROWSE-3 FOR EACH tt_price-all NO-LOCK,              EACH buf_currency OF tt_price-all  NO-LOCK INDEXED-REPOSITION.
  end.
  else do:
...
if buf_global-state.pl-use-shift-date-num = false then do:
   ...
   if v-fact-num-fo = 24 then time-f =  "" .
end.
...
display date-f time-f  with frame Dialog-Frame .
