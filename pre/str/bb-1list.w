...
define variable v-num-add as integer no-undo .
define variable v-num-ignored as integer no-undo .
define buffer buf_bb-list-hist for bb-list-hist.
  do
  on error undo, return error
  :
    CASE p-mode:
      when 'title' then do:
        find first buf_bb-list-hist where
                   buf_bb-list-hist.id = 0   no-error.
        if not available buf_bb-list-hist then do:
          create buf_bb-list-hist.
          assign
          buf_bb-list-hist.id = 0
          buf_bb-list-hist.line = 0
          buf_bb-list-hist.list-table = '':U
          .
        end.
        assign
        buf_bb-list-hist.des =  p-des
        buf_bb-list-hist.num-recs = p-num-recs
        buf_bb-list-hist.option_  = p-option
        buf_bb-list-hist.item_ = p-item
        .
      end.
      when 'ДОБАВЛЕНИЕ':U then do:
        ...
        create buf_bb-list-hist.
        assign
        buf_bb-list-hist.id = p-seq
        buf_bb-list-hist.list-table = p-list-table
         p-seq = (if p-line = 0 then (p-seq + 1) else p-seq)
        buf_bb-list-hist.des =  p-des
        buf_bb-list-hist.line = p-line
        buf_bb-list-hist.num-recs = p-num-recs
        buf_bb-list-hist.option_  = p-option
        buf_bb-list-hist.item_ = p-item
        buf_bb-list-hist.hist-mode =  p-hist-mode
        buf_bb-list-hist.status_ =  p-status_
        buf_bb-list-hist.num-add = v-num-add
        buf_bb-list-hist.num-ignored = v-num-ignored
        .
      end.
      when 'ИЗМЕНЕНИЕ':U + chr(4) + 'des' then do:
        find first buf_bb-list-hist where
                  buf_bb-list-hist.id = p-seq
              and buf_bb-list-hist.line = p-line no-error .
        if  available buf_bb-list-hist then do:
          assign
          buf_bb-list-hist.des =  p-des
          buf_bb-list-hist.num-recs = p-num-recs
          buf_bb-list-hist.option_  = p-option
          buf_bb-list-hist.item_ = p-item
          buf_bb-list-hist.list-table = (if p-list-table <> '':U and p-list-table <> ?
                                     then p-list-table
                                     else buf_bb-list-hist.list-table)
          .
        end.
      end.
      when 'ИЗМЕНЕНИЕ':U + chr(4) + 'mode' then do:
        ...
        if available buf_bb-list-hist then do:
          assign
          buf_bb-list-hist.hist-mode =  p-hist-mode
          buf_bb-list-hist.num-recs  = (if buf_bb-list-hist.line = 0 then p-num-recs else buf_bb-list-hist.num-recs)
          buf_bb-list-hist.list-table = (if p-list-table <> '':U and p-list-table <> ?
                                     then p-list-table
                                     else buf_bb-list-hist.list-table)
          .
...
define buffer l-bb-list for bb-list.
...
.
DEFINE QUERY br-list FOR bb-list SCROLLING.
DEFINE BROWSE br-list QUERY br-list NO-LOCK DISPLAY
      bb-list.gds-code
      bb-list.b-code   format ">>>>>>>>9" column-label "Бар-код"
      bb-list.b-str    format "X(17)" column-label "ДопБК/лок.ЕАН"
      bb-list.bc-on    format "+/" column-label "В!к!л"
      bb-list.artic
      bb-list.gds-name
      bb-list.f-name
      bb-list.unit-base column-label "Осн!Ед!Изм" format "x(3)"
      bb-list.bc-unit-cli column-label "Ед!Изм!кода" format "x(3)"
      bb-list.bc-cli-base-rate column-label "кратность" format ">>,>>9.999"
      (if bb-list.stts = 0 then no else yes) column-label "-" format "-/ "
      bb-list.in-code column-label "№ ПН" format  "X(18)" width 16
      bb-list.part-code column-label "№ Партии" format  "X(255)" width 10
      bb-list.grp-name
      (bb-list.prod-type + string (bb-list.prod-code, ">>>>>>>>>9") ) column-label "Производитель" format "x(13)"
...
run create-bb-list-hist in this-procedure (input 'title'
                                     , input-output v-seq
                                     , input 0
                                     , input 'N':U
                                     , input v-value
...
then do:
  ...
  :
    if first-of(bb-list.gds-code) then
    export bb-list.prod-type
          bb-list.prod-code
          bb-list.artic
          .
  end.
...
PROCEDURE proc-any-printable-br-list :
  if input frame Dialog-Frame a-n-c = "art" then do:
    if last-event:label = " " and
       loc-art = "" then
    return error.
    find first l-bb-list where
                l-bb-list.artic begins (loc-art + last-event:label)
               no-lock no-error.
    if available l-bb-list then do:
      loc-art = loc-art + last-event:label.
      disp loc-art with frame Dialog-Frame.
      line-rec = recid (l-bb-list).
...
PROCEDURE proc-backspace-br-list:
  if input frame Dialog-Frame a-n-c = "art" then do:
    ...
    find first l-bb-list where
                l-bb-list.artic begins loc-art
               no-lock.
    disp loc-art with frame Dialog-Frame.
    line-rec = recid (l-bb-list).
    reposition br-list to recid line-rec no-error.
...
define buffer buf_place for ub.place.
  ...
  if available l-bar-code then do:
      if last-event:label = "Ctrl-J" then
        find next l-bb-list where
                 l-bb-list.gds-code = l-bar-code.gds-code
                          and l-bb-list.b-code   = l-bar-code.b-code
                no-lock no-error.
      else
        find first l-bb-list where
                 l-bb-list.gds-code = l-bar-code.gds-code
...
,buffer buf_place
  ...
  if available buf_prod-bc then do:
    ...
    find first l-bb-list where
                l-bb-list.gds-code = buf_bar-code.gds-code AND
                l-bb-list.b-code = buf_bar-code.b-code AND
                l-bb-list.b-str = buf_prod-bc.b-str no-lock no-error.
    if available l-bb-list then do:
      line-rec = recid (l-bb-list).
      reposition br-list to recid line-rec no-error.
      apply "value-changed" to br-list in frame Dialog-Frame.
    ...
    if available buf_bar-code then do:
      ...
      find first l-bb-list where
                  l-bb-list.gds-code = buf_bar-code.gds-code AND
                  l-bb-list.b-code = buf_bar-code.b-code AND
                  l-bb-list.b-str = loc-b-str no-lock no-error.
      if available l-bb-list then do:
        line-rec = recid (l-bb-list).
        reposition br-list to recid line-rec no-error.
        apply "value-changed" to br-list in frame Dialog-Frame.
...
PROCEDURE  proc-mouse-dbl-click-loc-name:
  ...
  loc-name.
    if last-event:label = "Ctrl-J" then
      find next l-bb-list where
                can-find (ub.goods where ub.goods.artic = l-bb-list.artic and
                ub.goods.prod-type = l-bb-list.prod-type and
                ub.goods.prod-code = l-bb-list.prod-code and
...
if not available bb-list or recid (bb-list) <> line-rec then do:
    hide loc-art in frame Dialog-Frame.
    loc-art = "".
end.
if available bb-list then do:     find ub.clients where ub.clients.obj-type = bb-list.prod-type                    and ub.clients.obj-code = bb-list.prod-code no-lock.     find ub.gds-prt where ub.gds-prt.upper-code = bb-list.prt-root no-lock.     find ub.bar-code where ub.bar-code.gds-code  = bb-list.gds-code                     and ub.bar-code.node-code = ub.gds-prt.node-code                     and ub.bar-code.unit-cli  = bb-list.unit-base                     and ub.bar-code.in-code   = ""                     and ub.bar-code.part-code = "" no-lock.     disp ub.clients.obj-name ub.gds-prt.node-name ub.bar-code.b-code tot-lns @ f-tot-lns with frame Dialog-Frame.   end.   disp tot-lns @ f-tot-lns with frame Dialog-Frame.
end.
...
if not available bb-list then do:
  message "Неправильно выбран товар."
          view-as alert-box error.
  return no-apply.
end.
find ub.goods where bb-list.artic = ub.goods.artic
            and bb-list.prod-type = ub.goods.prod-type
            and bb-list.prod-code = ub.goods.prod-code no-lock.
...
find first buf_bb-list-hist no-lock where buf_bb-list-hist.id = 0 no-error .
PUT  STREAM PrnLibStream unformatted
SPACE(25) "История создания списка кодов "
(if available buf_bb-list-hist
then buf_bb-list-hist.des
...
skip(0)
.
for each buf_bb-list-hist where buf_bb-list-hist.id > 0
by buf_bb-list-hist.id
:
  put stream PrnLibStream unformatted
  (if buf_bb-list-hist.line = 0
   then string(buf_bb-list-hist.id, ">>>>>>>>9")
   else fill(chr(32) , 9)
  )  chr(32)
  (if buf_bb-list-hist.item_ <> '':U
   then string(buf_bb-list-hist.hist-mode, "X(8)")
   else fill( chr(32), 8)) chr(32)
  string(buf_bb-list-hist.num-add, "->>>>>>>>9") chr(32) chr(32) chr(32) chr(32)
  string(buf_bb-list-hist.num-recs, ">>>>>>>>9")  chr(32)
  string(buf_bb-list-hist.des, "X(155)") skip.
end.
...
define variable glog as logical no-undo .
define buffer buf-bb-list-hist for bb-list-hist.
glog = no.
...
tot-lns = 0.
v-seq = 1.
for each buf-bb-list-hist:
  delete buf-bb-list-hist.
end.
run create-bb-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                     , input-output v-seq
                                     , input 0
                                     , input '0':U
...
define buffer buf_keep-macro-list-hist for keep-macro-list-hist.
 if p-keep-query then do:
   ...
   run create-bb-list-hist in this-procedure (
                                          input 'ДОБАВЛЕНИЕ':U
                                        , input-output v-seq
                                        , input 0
                                        , input '0':U
...
DO:
  if not available bb-list then return no-apply.
  assign rs-list-method = "single".
  run proc-b-del in this-procedure(no, ?, (rs-list-method + chr(4) +
                                           (if bb-list.b-str <> '':U and not bb-list.loc-ean
                                           then 'prod-bc':U
                                           else (if bb-list.loc-ean
                                                 then 'loc-ean':U
                                                 else 'bar-code':U)
                                           )
                                           )
...
for each buf_bb-list-hist where
        buf_bb-list-hist.id >=  an-listp-start-macro-id
  and  buf_bb-list-hist.id <=  an-listp-end-macro-id:
  create macro-list-hist.
  buffer-copy buf_bb-list-hist
...
PROCEDURE proc-fill-temp-list :
  ...
  :
    assign
      v-num-entries-options = num-entries("Текущая строка,single,                           Товары-коды по типам,goods,                       Товар-лок.код,goods-b-code,                       Файл списка товаров,file-gds,                     Все,all,                                          Список товаров,gds-list,                          Мобильн. сканер,scaner")
    .
    ...
    :
      ...
      assign
        temp-list.fname  = trim(entry(v-index
                                     ,"Текущая строка,single,                           Товары-коды по типам,goods,                       Товар-лок.код,goods-b-code,                       Файл списка товаров,file-gds,                     Все,all,                                          Список товаров,gds-list,                          Мобильн. сканер,scaner"
                                     )
                               ,chr(32)
                               )
  ...
  for each macro-list-hist where (p-id = 0 or macro-list-hist.id = p-id):
    ...
    CASE entry(1, macro-list-hist.option_, chr(4)):
      ...
      when 'clear':U then do:
        ...
        for each bb-list-hist where bb-list-hist.id > an-listp-macros-label-id:
          delete bb-list-hist.
        end.
        tot-lns = 0.
        ...
        to buf_bb-list-hist
        assign buf_bb-list-hist.id = v-seq
        v-temp-seq = v-seq
        v-seq = v-seq + 1
      ...
      when 'single' then do:
        ...
        if not error-status:error then do:
          ...
          find first  buf_bb-list-hist where
                  buf_bb-list-hist.id = v-seq - 1 no-error .
          if available buf_bb-list-hist then
          assign
          macro-list-hist.num-recs = buf_bb-list-hist.num-recs
          macro-list-hist.num-add = buf_bb-list-hist.num-add
          macro-list-hist.num-ignored = buf_bb-list-hist.num-ignored
          .
        end.
        else do:
          return ("error" + chr(4) + return-value).
        ...
        then do:
          if macro-list-hist.line = 0 then do:
            ...
            for each buf_macro-list-hist where
              ...
              to buf_bb-list-hist
              assign
              buf_bb-list-hist.id = (if buf_macro-list-hist.line =0 then v-seq else v-temp-seq)
            ...
            for each buf_bb-list-hist where
                    buf_bb-list-hist.id = v-temp-seq,
               first buf_macro-list-hist where
                    buf_macro-list-hist.id = v-id
                AND buf_macro-list-hist.line = buf_bb-list-hist.line:
               assign
               buf_macro-list-hist.num-recs = buf_bb-list-hist.num-recs
               buf_macro-list-hist.num-add = buf_bb-list-hist.num-add
               buf_macro-list-hist.num-ignored = buf_bb-list-hist.num-ignored
               .
            end.
...
define buffer buf_bb-list-hist for bb-list-hist.
bb-list.part-code:resizable in browse br-list = yes.
bb-list.in-code:resizable in browse br-list = yes.
if v-cntxt-level <> 'object':U
or v-cntxt-obj-type <> 'маг':U then do:
  assign
  MENU-ITEM m-cd:sensitive in menu m-save = no.
end.
  find first buf_bb-list-hist where buf_bb-list-hist.id = 0 no-error.
  if available buf_bb-list-hist then
  assign
  frame Dialog-Frame:title = substitute("СПИСОК  КОДОВ &1",  string(buf_bb-list-hist.des, "X(60)"))
  .
  if tot-lns = ? then do:
    v-start = yes.
    for each l-bb-list :
      accumulate l-bb-list.artic (count).
    end.
    tot-lns = (accum count l-bb-list.artic).
    if tot-lns > 0 then do:
      find last  buf_bb-list-hist no-error .
      if available buf_bb-list-hist and buf_bb-list-hist.id = 1 and v-start then delete buf_bb-list-hist.
      v-seq = (if available buf_bb-list-hist then buf_bb-list-hist.id else 0)  + 1.
      run create-bb-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                           , input-output v-seq
                                           , input 0
                                           , input 'S':U
      ...
      for each buf_bb-list-hist:
        delete buf_bb-list-hist.
      end.
      v-seq = 1.
      run create-bb-list-hist in this-procedure (input 'ДОБАВЛЕНИЕ':U
                                           , input-output v-seq
  ...
  open query br-list for each bb-list no-lock indexed-reposition.
 if v-seq > 1 then
  find last buf_bb-list-hist no-lock where
            buf_bb-list-hist.id = (v-seq - 1)
       and  buf_bb-list-hist.line = 0 no-error .
  DISPLAY br-option
  (if available buf_bb-list-hist
  then buf_bb-list-hist.des
  else '') @ dsp-rs
...
if rs-list-method begins "single":U or
  ...
  if line-mode = 'удаление':U or line-mode = 'ОСТАВИТЬ':U then do:
    find first bb-list where bb-list.gds-code  = ub.goods.gds-code
                     and bb-list.b-code    = buf_bar-code.b-code
                     and bb-list.b-str     = p-b-str  no-error.
...
find first bb-list
  where bb-list.gds-code = buf_bar-code.gds-code
    and bb-list.b-code   = buf_bar-code.b-code
    and bb-list.b-str    = p-b-str
  no-error .
if available bb-list then do:
  assign
    bb-list.to-del = no
  ...
  if available bb-list then do:
    v-last62 = bb-list.order-num .
  end.
  else do:
    v-last62 = 0 .
  end.
  create bb-list .
  buffer-copy ub.goods to bb-list
  assign
    bb-list.to-del = no
    bb-list.order-num = v-last62 + 1
    bb-list.b-code = buf_bar-code.b-code
    bb-list.bc-cli-base-rate = buf_bar-code.cli-base-rate
    bb-list.bc-cr-db-num     = buf_bar-code.cr-db-num
    bb-list.in-code       = buf_bar-code.in-code
    bb-list.node-code     = buf_bar-code.node-code
    bb-list.part-code     = buf_bar-code.part-code
    bb-list.stts_         = buf_bar-code.stts_
    bb-list.bc-unit-cli   = buf_bar-code.unit-cli
    bb-list.b-str         = p-b-str
    bb-list.f-name        = v-f-name
    bb-list.loc-ean       = p-is-loc-ean
    .
    if available buf_prod-bc
    then
    assign
    bb-list.bc-on-type    = buf_prod-bc.bc-on-type
    bb-list.bc-on         = buf_prod-bc.bc-on
    bb-list.pbc-cr-db-num = buf_prod-bc.cr-db-num
    .
  assign
    lns-cnt = lns-cnt + 1
...
case rs-list-method:
  when "optimize"  then do:
    find first buf_bb-list-hist where
               buf_bb-list-hist.id = p-id .
    run proc-b-optimize(input p-from-macro
                      , input rs-list-method
                      , input rs-status
                      , input buf_bb-list-hist.item_
                      ) no-error .
    assign                                                           buf_bb-list-hist.num-add  = lns-cnt - v-num-add                       buf_bb-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_bb-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_bb-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
  end.
  when "all" then do:
    find first buf_bb-list-hist where
               buf_bb-list-hist.id = p-id .
    if p-from-macro then run restore-codes in this-procedure (input buf_bb-list-hist.item_, p-curr-obj-type, p-curr-obj-code).
  ...
  when "goods" then do:
    for each buf_bb-list-hist where
             buf_bb-list-hist.id = p-id:
      if buf_bb-list-hist.line = 0
      and p-from-macro then run restore-codes in this-procedure (input buf_bb-list-hist.item_, p-curr-obj-type, p-curr-obj-code).
      if buf_bb-list-hist.line <> 0 then do:
        run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_bb-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then next.
        assign                 new-good = yes                 petrol-trk = no                 cashparts = no                 main-b-code = 0                 .
  ...
  when "last-check":U then do:
    find first buf_bb-list-hist where
         ...
         AND buf_bb-list-hist.item_ <> '':U .
    assign
    v-curr-obj-type = entry(1, buf_bb-list-hist.item_, chr(3))
    v-curr-obj-code = integer(entry(2, buf_bb-list-hist.item_, chr(3)))
    v-chk-date-chr = entry(3, buf_bb-list-hist.item_, chr(3))
...
,buffer ub.place
  ...
  when "cd-codes":U then do:
    for each buf_bb-list-hist where
      ...
      v-curr-obj-type = entry(1, buf_bb-list-hist.item_, chr(3))
      v-curr-obj-code = integer(entry(2, buf_bb-list-hist.item_, chr(3)))
      v-pos-type = entry(3, buf_bb-list-hist.item_, chr(3))
      no-error
      .
      if error-status:error then do: end. else do:
        find first buf_user-obj no-lock where
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run partbcod in g#library
  ...
  then do:
    for each buf_bb-list-hist where
      ...
      v-curr-obj-type = entry(1, buf_bb-list-hist.item_, chr(3))
      v-curr-obj-code = integer(entry(2, buf_bb-list-hist.item_, chr(3)))
      v-value-integer = integer(entry(3, buf_bb-list-hist.item_, chr(3)))
      no-error
      .
      if error-status:error then do: end. else do:
        find first buf_user-obj no-lock where
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run partbcod in g#library
  ...
  then do:
     for each buf_bb-list-hist where
      ...
      v-curr-obj-type = entry(1, buf_bb-list-hist.item_, chr(3))
      v-curr-obj-code = integer(entry(2, buf_bb-list-hist.item_, chr(3)))
      v-from-date =     date(entry(3, buf_bb-list-hist.item_, chr(3)))
      v-to-date = date(entry(4, buf_bb-list-hist.item_, chr(3)))
  ...
  then do:
    for each buf_bb-list-hist where
        ...
        and  buf_bb-list-hist.item_ <> '':U:
      assign
      v-db-num = integer(entry(1, buf_bb-list-hist.item_, chr(3)))
      v-using = (entry(2, buf_bb-list-hist.item_, chr(3)))
      no-error
      .
      if error-status:error then do:
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run prodbcat in g#library
  ...
  when "filter" then do:
    ...
    find first buf_bb-list-hist where
            buf_bb-list-hist.id = p-id
        AND buf_bb-list-hist.item_ <> '':U .
    run proc-write-filter-expression-var in this-procedure ( input buf_bb-list-hist.item_, output v-filter-var ).
    run gbl/bb-fill.p (
...
find first buf_bb-list-hist where
      ...
      AND buf_bb-list-hist.item_ <> '':U .
run gbl/filename.p
  (input  entry(1, buf_bb-list-hist.item_, chr(4) )
  ,output v-full-path
  ,output v-path
  ,output v-file-name
  ,output v-file-name-no-ext
  ...
  CASE rs-list-method:
    when "file-gds" then do:
      ...
      run restore-codes in this-procedure (input entry(2, buf_bb-list-hist.item, chr(4)), input p-curr-obj-type, input p-curr-obj-code).
      repeat:
...
,buffer ub.place
        ...
        if available ub.goods then do:
          ...
          run ex-bbc in this-procedure (input rs-list-method, input rs-status, input line-mode , input no, input "":U, input (not available prod-bc), buffer bar-code, buffer prod-bc).
          if available bb-list and
            bb-list.artic = ub.goods.artic and
            bb-list.prod-type = ub.goods.prod-type and
            bb-list.prod-code = ub.goods.prod-code then
            bb-list.qnty = bb-list.qnty + scan-qnty * bc-qnty.
        end.
      end.
    end.
  END CASE.
  ...
  if v-no-hist < 0 then do:
    run create-bb-list-hist in this-procedure(input 'ДОБАВЛЕНИЕ':U
                                        ...
                                        , input (if line-mode = 'ДОБАВЛЕНИЕ':U
                                                then (substitute("Коды по ТОВАРУ c гл.кодом &1 &2 &3&4 &5 -"
                                                       ...
                                                       (if t-loc-base then 'основн.'  else '':U) + chr(32) +                           (if t-bc-base  then 'основн.EAN'  else '':U) + chr(32) +                        (if t-bc-alt then 'неосновн.EAN'  else '':U) + chr(32) +                           (if t-loc-alt  then 'неосновн.'  else '':U) + chr(32) +                      (if t-pb-base then 'ДопБК основн.'  else '':U) + chr(32) +                      (if t-pb-alt then 'ДопБК неосновн.'  else '':U) + chr(32) +                     (if t-sc-base then 'весов. и топл.'  else '':U) + chr(32) +                     (if t-all-prt then 'все признаки'  else '':U) + chr(32) +                       (if t-parts-ser then 'на сер. товар'  else '':U) + chr(32) +                    (if t-parts-not-blank then 'на непуст. № парт.'  else '':U) + chr(32) +         (if t-parts-all then 'все партии'  else '':U))
                                                          ...
                                                          , (if bb-list.b-str <> '':U then bb-list.b-str else string(bb-list.b-code))
                                                          , bb-list.gds-code
                                                          , bb-list.artic
                                                          , bb-list.prod-type
                                                          , bb-list.prod-code
                                        ...
                                        , input (if line-mode = 'ДОБАВЛЕНИЕ':U
                                                 ...
                                                 else (if bb-list.b-str <> '':u and bb-list.loc-ean = no
                                                      then ('prod-bc':U + chr(3) + bb-list.b-str)
                                                      else ('bar-code':U + chr(3) + string(bb-list.b-code))
                                                      )
                                                 )
                                        , input '':U
                                        , input ?
    ...
    do v-ii = 0 to v-no-hist:
      run create-bb-list-hist in this-procedure(input ('ИЗМЕНЕНИЕ':U + chr(4) + 'mode':U)
                                          ...
                                          , input '':U
                                          , input '':U
                                          , input ('goods':U + chr(3) + string(bb-list.gds-code) +
                                                  (if t-loc-base then 'loc-base':U  else '':U) + chr(32) +                        (if t-bc-base  then 'bc-base':U  else '':U) + chr(32) +                         (if t-bc-alt then 'bc-alt':U  else '':U) + chr(32) +                            (if t-loc-alt  then 'loc-alt':U  else '':U) + chr(32) +                         (if t-pb-base then 'pb-base':U  else '':U) + chr(32) +                          (if t-pb-alt then 'pb-alt':U  else '':U) + chr(32) +                            (if t-sc-base then 'sc-base'  else '':U) + chr(32) +                            (if t-all-prt then 'all-prt'  else '':U) + chr(32) +                            (if t-parts-ser then 'parts-ser':U  else '':U) + chr(32) +                      (if t-parts-not-blank then 'parts-not-blank'  else '':U) + chr(32) +            (if t-parts-all then 'parts-all'  else '':U))
                                          , input '':U
                                          , input ?
                                          ).
...
procedure proc-scn-tsd :
  ...
  run str/diallog.w (parparentproc
              , this-procedure
              , 'str/send-tsd.p':U
              , (p-curr-obj-type + chr(4) + string(p-curr-obj-code) + chr(4) + "bb-list":U)
              , no
              , '':U
      ...
      if p-from-macro then do:
        CASE entry(2, rs-list-method, chr(4) ):
          when 'prod-bc':U then do:
            ...
            find first bb-list where
                       bb-list.b-code = buf_prod-bc.b-code
                   AND bb-list.b-str = buf_prod-bc.b-str no-error.
          end.
          when 'bar-code':U
          or
          when 'loc-ean':U then do:
            ...
            find first bb-list where
                       bb-list.b-code = buf_bar-code.b-code
                   AND bb-list.loc-ean = (if entry(2, rs-list-method, chr(4)) = 'bar-code':U then no else yes) no-error.
          end.
        END CASE.
      end.
      if available bb-list then do:
        line-rec = recid (bb-list).
        get next br-list.
        if available bb-list then v-rep-rec = recid (bb-list).
        else do:
          reposition br-list to recid line-rec no-error.
          get prev br-list.
          if available bb-list then v-rep-rec = recid (bb-list).
  ...
  :
    ...
    if rs-list-method begins "single" then do:
      ...
      if p-from-macro then do:
        ...
        run write-hist(p-from-macro, rs-list-method, rs-status, line-mode, buffer ub.goods).
        for each bb-list:
          if line-rec <> recid (bb-list) then delete bb-list.
        end.
        tot-lns = 1.
        run UI-on.
      end.
...
define variable l-prod-bc-weight as logical no-undo .
define buffer b-str_bb-list for bb-list.
define buffer b-code_bb-list for bb-list.
define buffer buf_prod-bc for ub.prod-bc.
define buffer buf_bar-code for ub.bar-code.
