...
if not available bf_pump-nozzle then
   return error SUBSTITUTE("Не найден запись ТРК-пистлет с номером ТРК &1 и номером пистлета &2", parpump-code, parnozzle-code) + SUBSTITUTE(" на объекте &1 &2 .", parobj-type, parobj-code).
...
PROCEDURE adm-disable :
    DISABLE br_table varis-meas varef-nid varis-activ varps b-add b-del b-hist b-help lef-nid WITH FRAME F-Main.
...
PROCEDURE adm-enable :
    ENABLE UNLESS-HIDDEN br_table varis-meas varef-nid varis-activ varps b-add b-del b-hist b-help lef-nid WITH FRAME F-Main.
...
PROCEDURE adm-display-fields :
      IF AVAILABLE pump-nozzle THEN
          DISPLAY pump-nozzle.pump-code pump-nozzle.nozzle-code WITH BROWSE br_table
            NO-ERROR.
      DISPLAY UNLESS-HIDDEN varis-meas varef-nid varis-activ varps lef-nid
...
message
  ...
  "Неверное имя файла d-pumpnz.w." skip
...
message
  vss-workfile vss-revision vss-description skip
  "Ошибка при вызове файла d-pumpnz.w." skip
  "-----------Cистемная ошибка------------" skip
  return-value skip
  "------Ошибка исполнения программы------" skip
  trim(error-status :get-message(1)) +
...
message
  vss-workfile vss-revision vss-description skip
  "Ошибка при удалении записи ТРК-резервуар." skip
  "-----------Cистемная ошибка------------" skip
  return-value skip
  "------Ошибка исполнения программы------" skip
  trim(error-status :get-message(1)) +
...
DO:
  ...
  IF is-ef  THEN DO:
  if available ub.pump-nozzle then do:
     if ub.pump-nozzle.ef-nid <> input frame F-Main varef-nid then do:
        ...
        if varmes-log then do:
           ...
           assign frame F-Main varef-nid.
           assign bf_pumpnz.ef-nid = varef-nid.
        end.
        else do:
            assign varef-nid = ub.pump-nozzle.ef-nid.
            DISPLAY varef-nid with frame F-Main.
        end.
     end.
  end.
  END.
END.
ON VALUE-CHANGED OF varis-meas IN FRAME F-Main
DO:
  define buffer bf_pumpnz for ub.pump-nozzle.
...
do:
  ...
  if NOT glog then do:
    assign
      varis-meas = ub.pump-nozzle.is-meas.
    display varis-meas with frame F-Main.
    return no-apply.
  end.
  if available ub.pump-nozzle then do:
     if ub.pump-nozzle.is-meas <> input frame F-Main varis-meas then do:
        assign varmes-log = yes.
        message "Вы хотите изменить атрибут?"
        view-as alert-box question buttons yes-no update varmes-log.
        if varmes-log then do:
           find first bf_pumpnz where recid(bf_pumpnz) = recid(ub.pump-nozzle) exclusive.
           assign frame F-Main varis-meas.
...
END.
ON VALUE-CHANGED OF varis-activ IN FRAME F-Main
   DO:
      define buffer bf_pumpnz      for ub.pump-nozzle.
  ...
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,output glog
      ...
      do:
         ...
         display varis-activ with frame F-Main.
         return no-apply.
      end.
      ...
      do:
         ...
         do:
            ...
            do:
               ...
               assign frame F-Main varis-activ.
               assign
                  bf_pumpnz-attr.attr-value = string(varis-activ).
...
DO:
  define buffer bf_pump-nozzle for ub.pump-nozzle.
  if available ub.pump-nozzle then do:
    if ub.pump-nozzle.ps <> input frame F-Main varps then do:
      if varps-upd = no then do:
          message
            "Вы не можете изменять примечание!"
            view-as alert-box.
          display
            ub.pump-nozzle.ps @ varps
            with frame F-Main
          .
      end.
      else do:
        ...
        .
        message
          "Вы хотите изменить примечание к " "ТРК-пистолет"
          view-as alert-box question buttons yes-no update varmes-log.
        if varmes-log = true then do:
          ...
          :
            ...
            find first bf_pump-nozzle exclusive-lock
              where recid(bf_pump-nozzle) = recid(ub.pump-nozzle)
            .
            assign
              frame F-Main varps
            .
            assign
              bf_pump-nozzle.ps = varps
            .
          end.
        end.
        else do:
...
PROCEDURE adm-open-query-cases :
  OPEN QUERY br_table FOR EACH pump-nozzle WHERE pump-nozzle.obj-type = varobj-type and pump-nozzle.obj-code = varobj-code NO-LOCK      INDEXED-REPOSITION.
END PROCEDURE.
...
define variable vss-include-info8 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if lookup('b-add', varbuttons) = 0 then do:
    assign
      varis-meas:sensitive in frame F-Main = no
      varef-nid:sensitive in frame F-Main = no
    .
  end.
  if not is-ef then do:
    hide
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "ub.pump-nozzle":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE ub.pump-nozzle THEN STRING(ROWID(ub.pump-nozzle))
        ELSE "?":U.
        OTHERWISE
        DO:
