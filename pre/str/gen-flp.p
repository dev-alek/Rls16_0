...
:
  ...
  if error-status :error
  then do:
    undo, return error substitute( "&1. &2&3&4", vss-include-info25, return-value, chr(10), error-status :get-message (1)).
  end.
end.
end procedure.
...
if not ( p-trn-doc <> ?  and p-trn-doc = 1 )  then do:
   run str/gen-bfl.p
    ( input parparentproc ,
      input par-host-code ,
      input no,
...
if valid-handle( g#trdcalib ) <> yes then do:       run str/trdcalib.p persistent no-error.       if error-status :error or valid-handle( g#trdcalib ) <> yes then do:         message "Error starting trdcalib.p"    skip( 0 )                 g#trdcalib                     skip( 0 )                 g#trdcalib   :type             skip( 0 )                 g#trdcalib   :file-name        skip( 0 )                 error-status :get-message( 1 ) skip( 0 )                 return-value                   skip( 0 )         view-as alert-box error.         stop.       end.      end.     run trdcalib_tdat-val in g#trdcalib (  input buf_trn-doc.doc-code ,
                        input 'olsuppcntr':U ,
                       output is-trdcattr-oldsuppcntr ,
                       output v-type )  .
          ...
          find first buf4_trn-doc  exclusive-lock  where buf4_trn-doc.doc-code = buf_trn-doc.doc-code no-error .
              if available buf4_trn-doc then do:
                  p-res = p-res + ">> Создание ПФО - На накладную " + string(var-income-in-code) +
                                 " уже создано ФО " + string(v-n)  + chr(10)
                                 .
                  v-fo-prih = true  .
              end.
          end.
     ...
     :
        find first buf_parts-attr no-lock where buf_parts-attr.in-code   = buf_parts.in-code   and
               ...
               if not available buf_parts-attr then
                  p-res = p-res + chr(10)   + " У документа " +  buf_trn-doc.doc-code + "Нет атрибутов партий !!! Используйте утилиту восстановления" .
  end.
  end.
end procedure.
