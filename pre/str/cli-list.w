...
procedure create-cli-list-hist :
...
define buffer buf_cli-list-hist for cli-list-hist.
  ...
  :
    CASE p-mode:
      when 'title' then do:
        find first buf_cli-list-hist where
                   buf_cli-list-hist.id = 0   no-error.
        if not available buf_cli-list-hist then do:
          create buf_cli-list-hist.
          assign
          buf_cli-list-hist.id = 0
          buf_cli-list-hist.line = 0
          buf_cli-list-hist.list-table = '':U
          .
        end.
        assign
        buf_cli-list-hist.des =  p-des
        buf_cli-list-hist.num-recs = p-num-recs
        buf_cli-list-hist.option_  = p-option
        buf_cli-list-hist.item_ = p-item
        .
      end.
      when 'ДОБАВЛЕНИЕ':U then do:
        ...
        create buf_cli-list-hist.
        assign
        buf_cli-list-hist.id = p-seq
        buf_cli-list-hist.list-table = p-list-table
         p-seq = (if p-line = 0 then (p-seq + 1) else p-seq)
        buf_cli-list-hist.des =  p-des
        buf_cli-list-hist.line = p-line
        buf_cli-list-hist.num-recs = p-num-recs
        buf_cli-list-hist.option_  = p-option
        buf_cli-list-hist.item_ = p-item
        buf_cli-list-hist.hist-mode =  p-hist-mode
        buf_cli-list-hist.status_ =  p-status_
        buf_cli-list-hist.num-add = v-num-add
        buf_cli-list-hist.num-ignored = v-num-ignored
        .
      end.
      when 'ИЗМЕНЕНИЕ':U + chr(4) + 'des' then do:
        find first buf_cli-list-hist where
                  buf_cli-list-hist.id = p-seq
              and buf_cli-list-hist.line = p-line no-error .
        if  available buf_cli-list-hist then do:
          assign
          buf_cli-list-hist.des =  p-des
          buf_cli-list-hist.num-recs = p-num-recs
          buf_cli-list-hist.option_  = p-option
          buf_cli-list-hist.item_ = p-item
          buf_cli-list-hist.list-table = (if p-list-table <> '':U and p-list-table <> ?
                                     then p-list-table
                                     else buf_cli-list-hist.list-table)
          .
        end.
      end.
      when 'ИЗМЕНЕНИЕ':U + chr(4) + 'mode' then do:
        ...
        if available buf_cli-list-hist then do:
          assign
          buf_cli-list-hist.hist-mode =  p-hist-mode
          buf_cli-list-hist.num-recs  = (if buf_cli-list-hist.line = 0 then p-num-recs else buf_cli-list-hist.num-recs)
          buf_cli-list-hist.list-table = (if p-list-table <> '':U and p-list-table <> ?
                                     then p-list-table
                                     else buf_cli-list-hist.list-table)
          .
...
define buffer l-cli-list for cli-list.
...
DEFINE BROWSE BR-list
  QUERY BR-list NO-LOCK DISPLAY
      cli-list.obj-type
      cli-list.obj-code
      IF (cli-list.stts = 0) THEN (cli-list.obj-name)
ELSE (substring (cli-list.obj-name, 1, 25) +
      FILL(chr(32), 25 -
      LENGTH(substring (cli-list.obj-name, 1, 25)) ) +       '---  УДАЛЕН  ---':U) COLUMN-LABEL "Название" FORMAT "x(40)"
      cli-list.grp-name FORMAT "X(60)"
      cli-list.is-prod COLUMN-LABEL "Пр-ль" FORMAT "+/"
      cli-list.sup-gds COLUMN-LABEL "Пост-к/т" FORMAT "+/"
      cli-list.sup-cons COLUMN-LABEL "Пост-к/к" FORMAT "+/"
      cli-list.buy-gds COLUMN-LABEL "Пок-ль/т" FORMAT "+/"
      cli-list.buy-cons COLUMN-LABEL "Пок-ль/к" FORMAT "+/"
      cli-list.buy-serv COLUMN-LABEL "Пок-ль/у" FORMAT "+/"
      is-dis-cardc(cli-list.obj-type, cli-list.obj-code) COLUMN-LABEL "Дисконтная карта" FORMAT "X(16)"
      is-cashierc(cli-list.obj-type, cli-list.obj-code) COLUMN-LABEL "Кассир" FORMAT "X(6)"
      cli-list.db-num
      is-sellerc(cli-list.obj-type, cli-list.obj-code) COLUMN-LABEL "Прод-ц" FORMAT "X(6)"
...
DO:
define variable glog as logical no-undo .
define buffer buf-cli-list-hist for cli-list-hist.
glog = no.
message
...
tot-lns = 0.
v-seq = 1.
for each buf-cli-list-hist:
  delete buf-cli-list-hist.
end.
run create-cli-list-hist in this-procedure (
                                       input 'ДОБАВЛЕНИЕ':U
                                     , input-output v-seq
                                     , input 0
                                     , input '0':U
...
:
  ...
  string(buf_cli-list-hist.des, "X(155)") skip.
end.
...
DO:
    if avail cli-list then do:     assign ed-notes = cli-list.ps.     display ed-notes      tot-lns @ f-tot-lns     with frame Dialog-Frame.   end.   else do:     assign ed-notes = ''.     display ed-notes      tot-lns @ f-tot-lns     with frame Dialog-Frame.   end.
END.
...
if error-status:error then return no-apply.
END.
ON CHOOSE OF MENU-ITEM m-macros-save-db
DO:
  assign
...
DO:
  if not available cli-list then return no-apply.
  assign rs-list-method = "single".
...
run create-cli-list-hist in this-procedure(input 'ДОБАВЛЕНИЕ':U
                                    , input-output v-seq
                                    , input 0
                                    , input 'o':U
...
for each buf_cli-list-hist where
        buf_cli-list-hist.id >=  an-listp-start-macro-id
  and  buf_cli-list-hist.id <=  an-listp-end-macro-id:
  create macro-list-hist.
  buffer-copy buf_cli-list-hist
...
PROCEDURE proc-fill-temp-list :
  ...
  :
    assign
      v-num-entries-options = num-entries("Текущая строка,single,                              Клиент,cli,                                          Группа клиентов,cli-grp,                             Группа покупателей,grp-buy,                          Оборот покупателя с - по,turnov-buy,                 Все производители,prod,                              Все поставщики товаров,sup-gds,                      Все консигнанты,sup-cons,                            Все покупатели товаров,buy-gds,                      Все покупатели конс. товаров,buy-cons,               Все покупатели услуг,buy-serv,                       Все влад-цы диск.карт,dis-card,                      Все кассиры,cashiers,                                Все продавцы,sellers,                                Все  организации,firm,                               Все физ.лица,person,                                 Все магазины,shop,                                   Все склады,store,                                    Контрагенты по док-там,waybill,                      С установл. атрибутом,attr,                          С атрибутом = ,attr-val,                             Произ-ли товаров списка,gds-list,                    Контрагенты док-тов списка,doc-list,                 Удаленные,deleted,                                   Файл,file,                                           Хранимый в БД список,clob-data,                        Фильтр,filter,                                       Все клиенты,all")
    .
    ...
    :
      ...
      assign
        temp-list.fname  = trim(entry(v-index
                                     ,"Текущая строка,single,                              Клиент,cli,                                          Группа клиентов,cli-grp,                             Группа покупателей,grp-buy,                          Оборот покупателя с - по,turnov-buy,                 Все производители,prod,                              Все поставщики товаров,sup-gds,                      Все консигнанты,sup-cons,                            Все покупатели товаров,buy-gds,                      Все покупатели конс. товаров,buy-cons,               Все покупатели услуг,buy-serv,                       Все влад-цы диск.карт,dis-card,                      Все кассиры,cashiers,                                Все продавцы,sellers,                                Все  организации,firm,                               Все физ.лица,person,                                 Все магазины,shop,                                   Все склады,store,                                    Контрагенты по док-там,waybill,                      С установл. атрибутом,attr,                          С атрибутом = ,attr-val,                             Произ-ли товаров списка,gds-list,                    Контрагенты док-тов списка,doc-list,                 Удаленные,deleted,                                   Файл,file,                                           Хранимый в БД список,clob-data,                        Фильтр,filter,                                       Все клиенты,all"
                                     )
                               ,chr(32)
                               )
...
:
  if v-macro-is-running then do:
    ...
    if an-listp-start-macro-id >= 1 then do:
      ...
      system-dialog get-file f-name
        filters "Макрос формирования списка *.clm" "*.clm"
  ...
  run str/listmcro.w (input parparentproc
                ,input this-procedure:handle
               ,input "Макрос формирования списка клиентов" + chr(4) + "clm"
                ,input "b-play,b-step"
  ...
  for each macro-list-hist where (p-id = 0 or macro-list-hist.id = p-id):
    ...
    CASE entry(1, macro-list-hist.option_, chr(4)):
      ...
      when 'clear':U then do:
        ...
        for each cli-list-hist where cli-list-hist.id > an-listp-macros-label-id:
          delete cli-list-hist.
        end.
        tot-lns = 0.
        ...
        to buf_cli-list-hist
        assign buf_cli-list-hist.id = v-seq
        v-temp-seq = v-seq
        v-seq = v-seq + 1
      ...
      when 'single' then do:
        ...
        if not error-status:error then do:
          ...
          find first  buf_cli-list-hist where
                  buf_cli-list-hist.id = v-seq - 1 no-error .
          if available buf_cli-list-hist then
          assign
          macro-list-hist.num-recs = buf_cli-list-hist.num-recs
          macro-list-hist.num-add = buf_cli-list-hist.num-add
          macro-list-hist.num-ignored = buf_cli-list-hist.num-ignored
          .
        end.
        else do:
          return ("error" + chr(4) + return-value).
        ...
        then do:
          if macro-list-hist.line = 0 then do:
            ...
            for each buf_macro-list-hist where
              ...
              to buf_cli-list-hist
              assign
              buf_cli-list-hist.id = (if buf_macro-list-hist.line =0 then v-seq else v-temp-seq)
            ...
            for each buf_cli-list-hist where
                    buf_cli-list-hist.id = v-temp-seq,
               first buf_macro-list-hist where
                    buf_macro-list-hist.id = v-id
                AND buf_macro-list-hist.line = buf_cli-list-hist.line:
               assign
               buf_macro-list-hist.num-recs = buf_cli-list-hist.num-recs
               buf_macro-list-hist.num-add = buf_cli-list-hist.num-add
               buf_macro-list-hist.num-ignored = buf_cli-list-hist.num-ignored
               .
            end.
...
if rs-list-method = "single":U or
  (ub.clients.stts = 0  and rs-status <> 'удаленные':U) or
  (ub.clients.stts <> 0 and rs-status <> 'текущие':U) then do:
  if line-mode = 'удаление':U or line-mode = 'ОСТАВИТЬ':U then do:
    find first cli-list where
               cli-list.obj-type = ub.clients.obj-type AND
               cli-list.obj-code = ub.clients.obj-code no-error.
...
do:
  ...
  run get-report-num in parparentproc ( output g#report-num ).
    ...
    "Название" + chr(44) + 'название':U + chr(44) +
                    "Код" + chr(44) + 'код':U
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-list FOR EACH cli-list NO-LOCK INDEXED-REPOSITION.     open query br-option for each temp-list no-lock .
END PROCEDURE.
...
define buffer buf_cli-list-hist for cli-list-hist.
find first buf_cli-list-hist where buf_cli-list-hist.id = 0 no-error.
if available buf_cli-list-hist then
assign
frame Dialog-Frame:title = substitute("СПИСОК  КАРТ &1",  string(buf_cli-list-hist.des, "X(60)"))
.
br-option:column-scrolling in frame Dialog-Frame  = no.
if tot-lns = ? then do:
  v-start = yes.
  for each l-cli-list :
    accumulate l-cli-list.obj-code (count).
  end.
  tot-lns = (accum count l-cli-list.obj-code).
  if tot-lns > 0 then do:
    find last  buf_cli-list-hist no-error .
    v-seq = (if available buf_cli-list-hist then buf_cli-list-hist.id else 0)  + 1.
    run create-cli-list-hist in this-procedure (
                                            input 'ДОБАВЛЕНИЕ':U
                                          , input-output v-seq
                                          , input 0
...
if v-seq > 1 then
find last buf_cli-list-hist no-lock where
          buf_cli-list-hist.id = (v-seq - 1)
      and  buf_cli-list-hist.line = 0 no-error .
DISPLAY br-option
(if available buf_cli-list-hist
then buf_cli-list-hist.des
else '') @ dsp-rs
...
if tot-lns > 0 then
  ...
  DISABLE b-print b-rest b-save b-del b-lkp b-clr n-c nameorcode WITH FRAME Dialog-Frame.
end.
...
PROCEDURE Openbr :
Open query br-list
for each cli-list No-LOCK indexed-reposition.
APPLY "ENTRY" to br-list in frame Dialog-Frame.
END PROCEDURE.
...
if rs-list-method = "single" then do:
  ...
  if p-from-macro then do:
    ...
    find first cli-list where
             cli-list.obj-type = ub.clients.obj-type
         AND cli-list.obj-code = ub.clients.obj-code no-error.
  end.
  if available cli-list then do:
    line-rec = recid (cli-list).
    get next br-list.
    if available cli-list then rep-rec = recid (cli-list).
    else do:
      reposition br-list to recid line-rec no-error.
      get prev br-list.
      if available cli-list then rep-rec = recid (cli-list).
...
if rs-list-method = "single" then do:
  ...
  if available cli-list then do:
    ...
    run write-hist in this-procedure ( input p-from-macro, input rs-list-method, input rs-status, input line-mode).
    for each cli-list:
      if line-rec <> recid (cli-list) then delete cli-list.
    end.
    tot-lns = 1.
    run Myenable in this-procedure .
  end.
...
:
  find first buf_cli-list-hist where
          buf_cli-list-hist.id = p-id
      AND buf_cli-list-hist.item_ <> '':U .
if rs-list-method = "clob-data" then do:
  run gen-row-keyr in this-procedure (
   input  buf_cli-list-hist.item_
  ,input  ?
  ,input  "ub"
  ,input  ?
  ,input NO-LOCK
  ...
  run gbl/filename.p (
   input  (if rs-list-method = "clob-data" then v-file-name else buf_cli-list-hist.item_  )
  ,output v-full-path
  ,output v-path
  ,output v-file-name
  ,output v-file-name-no-ext
  ...
  input stream sout close.
  assign                                                           buf_cli-list-hist.num-add  = lns-cnt - v-num-add                       buf_cli-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_cli-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_cli-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
end.
end procedure.
...
case n-c:
  when 'название':U then do:
    if last-event:label = "Ctrl-J" then
        find next l-cli-list no-lock where
                   l-cli-list.obj-name begins nameorcode no-error.
    else
        find first l-cli-list no-lock where
                   l-cli-list.obj-name begins nameorcode no-error.
  end.
  when 'код':U then do:
    if last-event:label = "Ctrl-J" then
        find next l-cli-list no-lock where
                   l-cli-list.obj-code = integer(nameorcode) no-error.
    else
        find first l-cli-list no-lock where
                   l-cli-list.obj-code = integer(nameorcode) no-error.
  end.
end case.
if available l-cli-list then do:
  line-rec = recid (l-cli-list).
  reposition br-list to recid line-rec no-error.
...
case rs-list-method:
  when "cli-grp" then do:
    for each buf_cli-list-hist where
             buf_cli-list-hist.id = p-id
        and  buf_cli-list-hist.item_ <> '':U:
      run gen-row-keyr  in this-procedure (  input prepare-rowid( buf_cli-list-hist.item_, v-no-context)                                                        ,input ?                                                                          ,input 'ub'                                                                       ,input ?                                                                          ,input no-lock                                                                    ,output v-rowid                                                                  ,output v-tbl-name) no-error .                  if error-status:error then  next.
      grp-path = "".
  ...
  when "firm" then do:
    find first buf_cli-list-hist where
               buf_cli-list-hist.id = p-id .
    for each ub.clients where ub.clients.obj-type = 'орг':U no-lock:
      run ex-cli in this-procedure ( input rs-list-method, input rs-status, input line-mode).
    end.
    assign                                                           buf_cli-list-hist.num-add  = lns-cnt - v-num-add                       buf_cli-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_cli-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_cli-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
  end.
  ...
  when "shop" then do:
    find first buf_cli-list-hist where
               buf_cli-list-hist.id = p-id .
    for each ub.clients where ub.clients.obj-type = 'маг':U no-lock:
      run ex-cli in this-procedure ( input rs-list-method, input rs-status, input line-mode).
    end.
    assign                                                           buf_cli-list-hist.num-add  = lns-cnt - v-num-add                       buf_cli-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_cli-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_cli-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
  end.
  when "store" then do:
    find first buf_cli-list-hist where
               buf_cli-list-hist.id = p-id .
    for each ub.clients where ub.clients.obj-type = 'скл':U no-lock:
      run ex-cli in this-procedure ( input rs-list-method, input rs-status, input line-mode).
    end.
    assign                                                           buf_cli-list-hist.num-add  = lns-cnt - v-num-add                       buf_cli-list-hist.num-recs = (if line-mode = 'ОСТАВИТЬ':U                                            then (tot-lns + buf_cli-list-hist.num-add)                                            else (tot-lns + (if line-mode = 'ДОБАВЛЕНИЕ':U then 1 else - 1) * lns-cnt))                   tot-lns               = (if line-mode = 'ОСТАВИТЬ':U then lns-cnt else tot-lns)                     buf_cli-list-hist.num-ignored = lns-ignore  - v-num-ignored                    v-num-add             = lns-cnt                                   v-num-ignored         = lns-ignore.
  end.
 ...
 when "attr" or when "attr-val" then do:
    find first buf_cli-list-hist where
             buf_cli-list-hist.id = p-id
         AND buf_cli-list-hist.item_ <> '':U no-error .
    assign
    v-obj-type = entry(1, buf_cli-list-hist.item_, chr(3))
    v-obj-code = integer(entry(2, buf_cli-list-hist.item_, chr(3)))
    v-attr-code = entry(3, buf_cli-list-hist.item_, chr(3))
    vvalue = (if rs-list-method = "attr" then '':U else entry(4, buf_cli-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do:
    end.
  ...
  when "turnov-buy"  then do:
    find first buf_cli-list-hist where
        ...
        and    buf_cli-list-hist.item_ <> '':U.
    assign
    v-sum-1 = decimal(entry(1, buf_cli-list-hist.item_, chr(3)))
    v-sum-2 = decimal(entry(2, buf_cli-list-hist.item_, chr(3)))
    no-error
    .
    if error-status:error then do:
    end.
  ...
  when "filter" then do:
    ...
    find first buf_cli-list-hist where
             buf_cli-list-hist.id = p-id
         AND buf_cli-list-hist.item_ <> '':U .
    run proc-write-filter-expression-var in this-procedure ( input buf_cli-list-hist.item_, output v-filter-var ).
    run gbl/cli-fill.p (
...
vvalue = ""
.
DO ii = 1 to num-entries('doc-start,arh-detail,arh-start,ahsp-detail,ahsp-start,aht-detail,aht-start,arh-del,ahsp-del,aht-del,arh-calc,ahsp-calc,aht-calc,arh-recalc,ahsp-recalc,aht-recalc,is-inkassator,shftrep2,db,is-superviser,purch-code,als-gds,alien,envd,kpp,pharm,upd-date-time,holdfirm-code,vat-register,bge-incr-last-shift-date,bge-incr-last-shift-num,bge-sap-sng-last-shift,egrip-date,egrip-num,cli-local,cli-alc-producer,region-code,foreign-producer,main-accholder,not-corr-op,veto-man-doc,requisite-alc-decl,division-code,supp-np,own-supp,supp-lgas,tank-farm-for,NPZ,auto-tank-for,code-KSK,code-AIS,owner-code,cli-for-close-fo,cli-clim-grp,cli-decommissioned,exp-isPM-last-date':U):
    run clntattr-code in this-procedure (
                                         input entry(ii, 'doc-start,arh-detail,arh-start,ahsp-detail,ahsp-start,aht-detail,aht-start,arh-del,ahsp-del,aht-del,arh-calc,ahsp-calc,aht-calc,arh-recalc,ahsp-recalc,aht-recalc,is-inkassator,shftrep2,db,is-superviser,purch-code,als-gds,alien,envd,kpp,pharm,upd-date-time,holdfirm-code,vat-register,bge-incr-last-shift-date,bge-incr-last-shift-num,bge-sap-sng-last-shift,egrip-date,egrip-num,cli-local,cli-alc-producer,region-code,foreign-producer,main-accholder,not-corr-op,veto-man-doc,requisite-alc-decl,division-code,supp-np,own-supp,supp-lgas,tank-farm-for,NPZ,auto-tank-for,code-KSK,code-AIS,owner-code,cli-for-close-fo,cli-clim-grp,cli-decommissioned,exp-isPM-last-date':U)
                                        ,output vtype
                                        ,output vformat
                                        ,output vlabel
    ...
    if NOT error-status:error anD VOUTPUT-DISPLAY = yes then do:
        assign
        vattr-codes = vattr-codes + chr(4) + entry(ii, 'doc-start,arh-detail,arh-start,ahsp-detail,ahsp-start,aht-detail,aht-start,arh-del,ahsp-del,aht-del,arh-calc,ahsp-calc,aht-calc,arh-recalc,ahsp-recalc,aht-recalc,is-inkassator,shftrep2,db,is-superviser,purch-code,als-gds,alien,envd,kpp,pharm,upd-date-time,holdfirm-code,vat-register,bge-incr-last-shift-date,bge-incr-last-shift-num,bge-sap-sng-last-shift,egrip-date,egrip-num,cli-local,cli-alc-producer,region-code,foreign-producer,main-accholder,not-corr-op,veto-man-doc,requisite-alc-decl,division-code,supp-np,own-supp,supp-lgas,tank-farm-for,NPZ,auto-tank-for,code-KSK,code-AIS,owner-code,cli-for-close-fo,cli-clim-grp,cli-decommissioned,exp-isPM-last-date':U)
...
assign
c-point = "cli-list" + chr(4) + "Список клиентов" + chr(4) + "no"
.
 assign
    tbl = 'clients'
    join-tbl = ''
...
run create-cli-list-hist in this-procedure (
              ...
              , input rs-list-method
              , input rs-status
              , input (string(v-sum-1) + chr(3) + string(v-sum-2))
              , input ""
              , input ?
              ).
END PROCEDURE.
...
if rs-list-method = "single" then do:
  if v-no-hist < 0 then do:
    run create-cli-list-hist in this-procedure (
                                        ...
                                        , input substitute("КЛИЕНТ &1&2 ", cli-list.obj-type, cli-list.obj-code)
                                        , input tot-lns
                                        , input rs-list-method
                                        , input rs-status
                                        , input ('clients':U + chr(3) + cli-list.obj-type + chr(3) + string(cli-list.obj-code))
                                        , input '':U
                                        , input ?
                                        ).
  end.
    ...
    do v-ii = 0 to v-no-hist:
      run create-cli-list-hist in this-procedure ( input ('ИЗМЕНЕНИЕ':U + chr(4) + 'mode':U)
                                          , input-output v-temp-seq
                                          , input v-ii
                                          , input get-hist-mode(line-mode)
                                          , input substitute("КЛИЕНТ &12", cli-list.obj-type, cli-list.obj-code)
                                          , input tot-lns
                                          , input '':U
                                          , input '':U
                                          , input ('clients':U + chr(3) + cli-list.obj-type + chr(3) + string(cli-list.obj-code))
  ...
  do v-ii = 0 to v-no-hist:
    run create-cli-list-hist in this-procedure (
                                          input ('ИЗМЕНЕНИЕ':U + chr(4) + 'mode':U)
                                        , input-output  v-temp-seq
                                        , input v-ii
                                        , input get-hist-mode(line-mode)
                                        , input '':U
...
( input loc-type as character, input loc-code as integer ) :
  if loc-type <> 'чел':U
  then return "".
  return string( gbclcode-is-psn-role ( input 'C':U, input loc-code, input ?)).
END FUNCTION.
FUNCTION is-dis-cardc RETURNS CHARACTER
( input loc-type as character, input loc-code as integer ) :
...
( input loc-type as character, input loc-code as integer ) :
  if loc-type <> 'чел':U
  then return "".
  return string( gbclcode-is-psn-role ( input 'S':U, input loc-code, input ?)).
END FUNCTION.
FUNCTION stat-line RETURNS CHARACTER
