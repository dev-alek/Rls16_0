...
if not available bf_nozzle then do:
   return error SUBSTITUTE("Не найден пистолет с номером &1", parnozzle-code)
                + SUBSTITUTE(" на объекте &1 &2 .", parobj-type, parobj-code).
end.
...
PROCEDURE adm-display-fields :
      IF AVAILABLE ub.nozzle THEN
          DISPLAY ub.nozzle.nozzle-code WITH BROWSE br_table
            NO-ERROR.
...
message
  ...
  "Неверное имя файла d-nozzle.w." skip
...
message
  vss-workfile vss-revision vss-description skip
  "Ошибка при вызове файла d-nozzle.w." skip
  "-----------Cистемная ошибка------------" skip
  return-value skip
  "------Ошибка исполнения программы------" skip
  trim(error-status :get-message(1)) +
...
message
  vss-workfile vss-revision vss-description skip
  "Ошибка при удалении пистолета ТРК." skip
  "-----------Cистемная ошибка------------" skip
  return-value skip
  "------Ошибка исполнения программы------" skip
  trim(error-status :get-message(1)) +
...
DO:
  define buffer bf_nozzle for ub.nozzle.
  if available ub.nozzle then do:
    if ub.nozzle.ps <> input frame F-Main varps then do:
      if varps-upd = no then do:
        ...
        .
        message
          "Вы хотите изменить примечание к " "пистолету ТРК"
          view-as alert-box question buttons yes-no update varmes-log.
        if varmes-log = true then do:
          ...
          :
            ...
            find first bf_nozzle exclusive-lock
              where recid(bf_nozzle) = recid(ub.nozzle)
            .
            assign
              frame F-Main varps
            .
...
PROCEDURE adm-open-query-cases :
  OPEN QUERY br_table FOR EACH ub.nozzle WHERE ub.nozzle.obj-type = varobj-type and                                               ub.nozzle.obj-code = varobj-code NO-LOCK      INDEXED-REPOSITION.
END PROCEDURE.
...
PROCEDURE send-records :
  ...
  DO i = 1 TO NUM-ENTRIES(p-tbl-list):
    ...
    WHEN "ub.nozzle":U THEN p-rowid-list = p-rowid-list +
        IF AVAILABLE ub.nozzle THEN STRING(ROWID(ub.nozzle))
        ELSE "?":U.
        OTHERWISE
        DO:
