  ...
  when 'MAGIA-XML':U then do:
    ...
    for each buf_fbr-gds-grp no-lock where
      ...
      run bgelib-tag-put in this-procedure ( input 3, input "GroupName"
                                          , input trim(buf_fbr-gds-grp.node-name, chr(32)), input 1 ).
    ...
    for each buf_clients no-lock where
      ...
      run bgelib-tag-put in this-procedure ( input 3, input "DepartName"
                                           , input trim(buf_clients.obj-name, chr(32)), input 1 ).
      run bgelib-tag-put in this-procedure ( input 3, input "DepartShortName":U
                                           , input ('маг':U + string(buf_shop.obj-code)), input 1 ).
  ...
  ,output v-r-b-curr-magia
      ...
      for each buf_cash-desk no-lock where
        ...
        run bgelib-tag-put in this-procedure ( input 3, input "StationName"
                                            , input trim(buf_cash-desk.addr-path, chr(32)), input 1 ).
...
define variable v-first as logical no-undo .
  FOR EACH for-cash-desk NO-LOCK WHERE
    ...
    :
    if ub.cash-desk.pos-type <> 'MAGIA-XML':U or
    (LOOKUP(ub.cash-desk.pos-type,
              ('NCR-GM':U + chr(44) +
               'IBM-XML':U + chr(44) +
               'MAGIA-XML':U + chr(44) +
               'NCR-AS@R':U + 'Autotank':U
               )) > 0
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка данных по подразделениям'" + " &1")
...
then do:
  if for-cash-desk.pos-type = 'MAGIA-XML':U then do:
    run str/waitpxml.w ( replace( v-xml-file-name-path, "/", "\" ) + "xml",
                ...
                ( if action = 'U'
                  then ('Ждите - ' + 'добавление данных по подразделениям')
                  else ('Ждите - ' + 'удаление данных по подразделениям') ) +
...
define variable dflt-cd20 as character no-undo .
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute("Пересылка на кассы &1 БД &2 информации о подразделениях и группах товаров", 'MAGIA-XML':U, p-db-num)
                                          ).
