...
define variable vss-include-info69 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str69  as character no-undo.
  define variable tmp-num69  as character no-undo.
  define variable i69        as integer   no-undo.
  define variable sum69      as integer   no-undo.
  define variable len-code69 as integer   no-undo.
  define variable varcont69  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str69 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str69 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont69 = yes then do:
    if integer( substring( tmp-str69, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str69, length( ncrsc-pfx ) + 1, length( tmp-str69 ) - length( ncrsc-pfx ) )
        len-code69    = length( IBm-good-code )
      .
      define variable v-sum-char69 as character no-undo .
      assign
        sum69 = 0
      .
      do i69 = 1 to len-code69 by 2
      :
        assign
          v-sum-char69 = substr(IBm-good-code, len-code69 - i69 + 1, 1)
        .
        if v-sum-char69 < "0"
        or v-sum-char69 > "9"
        then do:
        ...
        assign
          sum69 = sum69 + integer(v-sum-char69)
        .
      end.
      if varcont69 = yes then do:
        assign
        ...
        if varcont69 = yes then do:
           if sum69 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum69 mod 10)
             .
           end.
        end.
      end.
...
define variable vss-include-info70 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if varcont70 = yes then do:
    if integer( substring( tmp-str70, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str70, length( ncrpg-pfx ) + 1, length( tmp-str70 ) - length( ncrpg-pfx ) )
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action72   as character no-undo .
  define variable v-printed72       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action72
    ,output v-printed72
    ) .
end.
if v-view-log = true
...
define variable vss-include-info73 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
then do:
  ...
  then do:
  define variable v-plu-pet79 as character no-undo .
  define buffer  bufpet_cd-plu79 for ub.cd-plu.
  for each bufpet_cd-plu79  where
          bufpet_cd-plu79.obj-type = 'маг':U
      and bufpet_cd-plu79.obj-code = abs(i-obj-code)
      and bufpet_cd-plu79.pos-type = 'MARIA':U
      and bufpet_cd-plu79.plu-type = 'топ':U
      AND bufpet_cd-plu79.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu79.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet79 = TRIM(string( bufpet_cd-plu79.plu-code, "X(40)":U ))
    v-plu-pet79 = substring(v-plu-pet79, length(v-plu-pet79) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code79 as character no-undo .
    assign
      maria-good-code79 = left-trim(ibm-good-code, '0')
      maria-good-code79 = fill('0', 14 - length(maria-good-code79)) + maria-good-code79
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code79, 1, 5) + chr(4) +
                                                    substring(maria-good-code79, 6, 14)
                                                  )
...
define variable v-found100 as logical no-undo .
define variable v-is-script100 as logical no-undo.
define variable v-fields-shift100 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet103 as character no-undo .
  define buffer  bufpet_cd-plu103 for ub.cd-plu.
  for each bufpet_cd-plu103  where
          bufpet_cd-plu103.obj-type = 'маг':U
      and bufpet_cd-plu103.obj-code = abs(i-obj-code)
      and bufpet_cd-plu103.pos-type = 'MARIA':U
      and bufpet_cd-plu103.plu-type = 'топ':U
      AND bufpet_cd-plu103.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu103.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet103 = TRIM(string( bufpet_cd-plu103.plu-code, "X(40)":U ))
    v-plu-pet103 = substring(v-plu-pet103, length(v-plu-pet103) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code103 as character no-undo .
    assign
      maria-good-code103 = left-trim(ibm-good-code, '0')
      maria-good-code103 = fill('0', 14 - length(maria-good-code103)) + maria-good-code103
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code103, 1, 5) + chr(4) +
                                                    substring(maria-good-code103, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable v-found105 as logical no-undo .
define variable v-is-script105 as logical no-undo.
define variable v-fields-shift105 as integer no-undo .
   if can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 6 )
   or can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 13 )
   then do:
     v-fields-shift105 = - 1.
...
define variable dflt-cd107 as character no-undo .
define variable vss-include-info108 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type108 as character no-undo .
define variable v-value-date108 as date no-undo .
define variable v-value-decimal108 as decimal no-undo .
define variable v-value-integer108 as INTEGER no-undo .
define variable v-value-logical108 AS LOGICAL no-undo .
define variable v-tth108 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd107
    ,output v-value-date108
    ,output v-value-decimal108
    ,output v-value-integer108
    ,output v-value-logical108
    ,output v-param-type108
    ,INPUT-OUTPUT table-handle v-tth108
    ) no-error .
delete object v-tth108 no-error.
...
define variable vss-include-info126 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str126  as character no-undo.
  define variable tmp-num126  as character no-undo.
  define variable i126        as integer   no-undo.
  define variable sum126      as integer   no-undo.
  define variable len-code126 as integer   no-undo.
  define variable varcont126  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str126 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str126 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont126 = yes then do:
    if integer( substring( tmp-str126, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str126, length( ncrsc-pfx ) + 1, length( tmp-str126 ) - length( ncrsc-pfx ) )
        len-code126    = length( v-tmpchar )
      .
      define variable v-sum-char126 as character no-undo .
      assign
        sum126 = 0
      .
      do i126 = 1 to len-code126 by 2
      :
        assign
          v-sum-char126 = substr(v-tmpchar, len-code126 - i126 + 1, 1)
        .
        if v-sum-char126 < "0"
        or v-sum-char126 > "9"
        then do:
        ...
        assign
          sum126 = sum126 + integer(v-sum-char126)
        .
      end.
      if varcont126 = yes then do:
        assign
        ...
        if varcont126 = yes then do:
           if sum126 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum126 mod 10)
             .
           end.
        end.
      end.
