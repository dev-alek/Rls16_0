...
procedure vss-get-parameters :
  define output parameter p-vss-parameters as character no-undo .
    assign
      p-vss-parameters = substitute('&1|&2|&3|&4|&5|&6':u,h-call-prog,p-mode,p-doc-code,p-gds-code,p-pl-code,p-parts-recid)
    .
end procedure.
...
def var vss-include-info56 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
        ...
        if p-mode <> 'ПРОСМОТР':U then do:
            ...
            if v-save-flag then do:
                ...
                then do:
                    ...
                    chk-message = "ВНИМАНИЕ!" + (if buf_parts.in-code <> buf_parts.out-code then " Изменения только по расходной партии." else " Изменения по всем документам порожденным из исходного." ) + chr(10) + "Вы собираетесь изменить следующие поля:" + chr(10).
                    if buf_parts.mark-db-num     <> v-alc-mark-db-num or buf_parts.mark-code <> v-alc-mark-code then chk-message = chk-message + "- Код марки" + chr(10).
                    if v-alc-bottling-date       <> buf_parts.alc-bottling-date           then chk-message = chk-message + "- Дата розлива" + chr(10).
                    if v-alc-ref-ab-path         <> buf_parts.alc-ref-ab-path             then chk-message = chk-message + "- Справки А, Б" + chr(10).
                    if v-alc-quality-certif-path <> buf_parts.alc-quality-certif-path     then chk-message = chk-message + "- Удостоверение качества" + chr(10).
                    if v-alc-certif-path         <> buf_parts.alc-certif-path             then chk-message = chk-message + "- Сертификат соответствия" + chr(10).
                    if v-alc-imp-code            <> buf_parts.alc-imp-code or v-alc-imp-type            <> buf_parts.alc-imp-type                then chk-message = chk-message + "- Импортер" + chr(10).
...
END.
ON CHOOSE OF b-cst IN FRAME Dialog-Frame
DO:
    ...
    then do:
      ...
      run gbl/d-prompt.w
        ...
        + 'text1=':U + "Документ " + str-encode(buf_parts.in-code, "", '\=':U )
          + chr(32) + "Номер " + str-encode(buf_parts.part-code, "", '\=':U ) + '\':U
        + 'text2=' + "Артикул " + str-encode(buf_parts.artic, "", '\=':U )
          + chr(32) + string(buf_parts.prod-type)
          + chr(32) + string(buf_parts.prod-code) + '\':U
        + 'format=' + 'X(60)' + '\':U
        + 'type=character':U
...
END.
ON CHOOSE OF b-dop IN FRAME Dialog-Frame
DO:
...
then do:
  ...
  run enable-fields  in this-procedure .
END.
ON LEAVE OF ub.parts.cli-qnty IN FRAME Dialog-Frame
DO:
  run display-dependent-info in this-procedure .
END.
ON RETURN OF ub.parts.cli-qnty IN FRAME Dialog-Frame
DO:
  if v-create-part
  then do:
    run apply-focus-next-entry in this-procedure
      (input ub.parts.cli-qnty:handle
      ).
    return no-apply .
  end.
  else do:
...
END.
ON RETURN OF ub.parts.cst-code IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.cst-code:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.exch-code IN FRAME Dialog-Frame
DO:
  if chkleave
...
END.
ON RETURN OF ub.parts.exch-code IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.exch-code:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.fact-qnty IN FRAME Dialog-Frame
DO:
  run display-dependent-info in this-procedure .
END.
ON RETURN OF ub.parts.fact-qnty IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.fact-qnty:handle
    ).
  return no-apply .
END.
...
DO:
  ...
  return no-apply .
END.
ON LEAVE OF ub.parts.last-date IN FRAME Dialog-Frame
DO:
  run update-last-date-offset in this-procedure .
END.
ON RETURN OF ub.parts.last-date IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.last-date:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.part-code IN FRAME Dialog-Frame
DO:
  if chkleave
    (input last-event :widget-enter
  ...
  then do:
    ...
    assign
      v-part-code = input frame Dialog-Frame parts.part-code
    .
...
END.
ON RETURN OF ub.parts.part-code IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.part-code:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.price-base IN FRAME Dialog-Frame
DO:
  if v-same-currency
  then do:
    assign
  ...
  run display-dependent-info in this-procedure .
END.
ON RETURN OF ub.parts.price-base IN FRAME Dialog-Frame
DO:
  ...
  run apply-focus-next-entry in this-procedure
    (input ub.parts.price-base:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.price-cli IN FRAME Dialog-Frame
DO:
  if chkleave
    (input last-event :widget-enter
    ,input "b-quit,b-rest,b-help":u
  ...
  run display-dependent-info in this-procedure .
END.
ON RETURN OF ub.parts.price-cli IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.price-cli:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.price-rubl IN FRAME Dialog-Frame
DO:
  if v-can-change-supp = true
  then do:
    run update-dependent-price in this-procedure .
  ...
  run display-dependent-info in this-procedure .
END.
ON RETURN OF ub.parts.price-rubl IN FRAME Dialog-Frame
DO:
  define variable v-new-price-cli  like ub.parts.price-cli  no-undo .
  define variable v-new-price-base like ub.parts.price-rubl no-undo .
  define variable v-new-price-rubl like ub.parts.price-rubl no-undo .
  ...
  run apply-focus-next-entry in this-procedure
    (input ub.parts.price-rubl:handle
    ).
  return no-apply .
END.
ON RETURN OF ub.parts.PS IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.PS:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.qnty IN FRAME Dialog-Frame
DO:
  run display-dependent-info in this-procedure .
END.
ON RETURN OF ub.parts.qnty IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.qnty:handle
    ).
  return no-apply .
END.
...
then do:
  ...
  run validate-supp in this-procedure
    (input (input frame Dialog-Frame parts.supp-type)
    ,input (input frame Dialog-Frame parts.supp-code)
    ) no-error .
  if error-status :error
  then do:
    message
      "Неправильно задан поставщик" skip
      "" (input frame Dialog-Frame parts.supp-type)
         (input frame Dialog-Frame parts.supp-code) skip
      view-as alert-box error .
    undo, return no-apply .
  end.
...
then do:
  ...
  run choose-exch-code in this-procedure
    (input (input frame Dialog-Frame parts.exch-code)
    ).
END.
...
then do:
  ...
  assign
    v-select-supp-type = input frame Dialog-Frame parts.supp-type
    v-select-supp-code = input frame Dialog-Frame parts.supp-code
  .
  run str/clisel.p
    (input parparentproc
...
DO:
  ...
  then do:
    if  input frame Dialog-Frame parts.supp-code <> 0
    and input frame Dialog-Frame parts.supp-code <> ?
    then do:
      run validate-supp in this-procedure
        (input (input frame Dialog-Frame parts.supp-type)
        ,input (input frame Dialog-Frame parts.supp-code)
...
END.
ON LEAVE OF ub.parts.supp-code IN FRAME Dialog-Frame
DO:
  if chkleave
    (input last-event :widget-enter
    ,input "supp-type,supp-code,r-supp,b-quit,b-rest,b-help":u
...
END.
ON RETURN OF ub.parts.supp-code IN FRAME Dialog-Frame
DO:
  ...
  if input frame Dialog-Frame parts.supp-code = ?
  or input frame Dialog-Frame parts.supp-code = 0
  then do:
    run ref/selcli.p
      (input  parparentproc
      ,input  ?
  ...
  run apply-focus-next-entry in this-procedure
    (input ub.parts.supp-code:handle
    ).
  return no-apply .
END.
ON LEAVE OF ub.parts.supp-type IN FRAME Dialog-Frame
DO:
  if chkleave
    (input last-event :widget-enter
    ,input "supp-type,supp-code,r-supp,b-quit,b-rest,b-help":u
...
END.
ON RETURN OF ub.parts.supp-type IN FRAME Dialog-Frame
DO:
  run apply-focus-next-entry in this-procedure
    (input ub.parts.supp-type:handle
    ).
  return no-apply .
END.
...
on ' ' of parts.last-date in frame Dialog-Frame
do:
...
on delete-character of parts.last-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-d of parts.last-date in frame Dialog-Frame
do:
  define variable v-curr-sv-date as date no-undo .
  if (can-query (self, "sensitive")
     and
...
on ctrl-b of parts.last-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-e of parts.last-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
...
on ctrl-f of parts.last-date in frame Dialog-Frame
do:
  if (can-query (self, "sensitive")
     and
     self :sensitive = true
  ...
  define MENU m-ed-date76
    MENU-ITEM m-ed-date76-1 LABEL "Начало месяца" ACCELERATOR "ALT-1"
    MENU-ITEM m-ed-date76-2 LABEL "Сегодня"       ACCELERATOR "ALT-2"
    MENU-ITEM m-ed-date76-3 LABEL "Конец месяца"  ACCELERATOR "ALT-3"
    MENU-ITEM m-ed-date76-4 LABEL "Календарь"     ACCELERATOR "ALT-4"
    .
  if parts.last-date :POPUP-MENU in frame Dialog-Frame = ?
  then do:
    ASSIGN
      parts.last-date :POPUP-MENU in frame Dialog-Frame = MENU m-ed-date76 :HANDLE
      parts.last-date :MENU-MOUSE in frame Dialog-Frame = 3
    .
  end.
  define variable v-label-handle76 as handle no-undo .
  assign
    v-label-handle76 = parts.last-date :side-label-handle in frame Dialog-Frame
  .
  if valid-handle (v-label-handle76)
  then do:
    if v-label-handle76 :tooltip = ""
    or v-label-handle76 :tooltip = ?
    then do:
      assign
        v-label-handle76 :tooltip = "Ctrl-D - текущая, Ctrl-B - начало, Ctrl-E - конец, Ctrl-F - календарь, Прав.Клавиша Мыши - Popup Menu"
      .
    end.
  end.
  ON CHOOSE OF MENU-ITEM m-ed-date76-1 in menu m-ed-date76 DO:
    apply "ctrl-b":U to parts.last-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date76-2 in menu m-ed-date76 DO:
    apply "ctrl-d":U to parts.last-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date76-3 in menu m-ed-date76 DO:
    apply "ctrl-e":U to parts.last-date in frame Dialog-Frame .
  END.
  ON CHOOSE OF MENU-ITEM m-ed-date76-4 in menu m-ed-date76 DO:
    apply "ctrl-f":U to parts.last-date in frame Dialog-Frame .
  END.
...
PROCEDURE apply-focus-next-entry :
  ...
  do with frame Dialog-Frame
  :
        if v-apply-entry   then do:     if parts.cli-qnty :sensitive     then do:       apply 'entry':u to parts.cli-qnty .       return .     end.   end.   if parts.cli-qnty :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.qnty :sensitive     then do:       apply 'entry':u to parts.qnty .       return .     end.   end.   if parts.qnty :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.fact-qnty :sensitive     then do:       apply 'entry':u to parts.fact-qnty .       return .     end.   end.   if parts.fact-qnty :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.part-code :sensitive     then do:       apply 'entry':u to parts.part-code .       return .     end.   end.   if parts.part-code :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.cst-code :sensitive     then do:       apply 'entry':u to parts.cst-code .       return .     end.   end.   if parts.cst-code :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.supp-type :sensitive     then do:       apply 'entry':u to parts.supp-type .       return .     end.   end.   if parts.supp-type :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.supp-code :sensitive     then do:       apply 'entry':u to parts.supp-code .       return .     end.   end.   if parts.supp-code :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.last-date :sensitive     then do:       apply 'entry':u to parts.last-date .       return .     end.   end.   if parts.last-date :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        ...
        if v-apply-entry   then do:     if parts.price-cli :sensitive     then do:       apply 'entry':u to parts.price-cli .       return .     end.   end.   if parts.price-cli :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.exch-code :sensitive     then do:       apply 'entry':u to parts.exch-code .       return .     end.   end.   if parts.exch-code :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.price-rubl :sensitive     then do:       apply 'entry':u to parts.price-rubl .       return .     end.   end.   if parts.price-rubl :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
        if v-apply-entry   then do:     if parts.price-base :sensitive     then do:       apply 'entry':u to parts.price-base .       return .     end.   end.   if parts.price-base :handle = p-widget-handle   then do:     assign       v-apply-entry = true     .   end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run partppric in g#library
(  buffer buf_parts
 , output fi-price-prod
 , output fi-price-prodvat
 , output v-vat-pc
        )  .
      display fi-price-prod fi-price-prodvat  with frame Dialog-Frame .
      define variable v-display-part-code as character no-undo .
      run partsfnc_get-parts-show-code in this-procedure
        (input  buf_parts.part-code
        ,input  buf_parts.mark-db-num
      ...
      then do:
        ...
        find first free-parts no-lock
          where free-parts.obj-type  = buf_parts.obj-type
            ...
            and free-parts.out-code  = 'free-zone':U
        ...
        then do:
          ...
          then do:
            ...
            or (buf_parts.out-code = 'out-zone':U
                and rsrv-parts.fact-qnty < 0
              )
            then do:
              assign
  ...
  ,output v-base-code
      ...
      then do:
        ...
        find first buf_clients no-lock
          ...
          no-error .
        assign
          v-display-cli-info = (buf_trn-doc.doc-type = 'при':U
                                and buf_trn-doc.internal = false
                              )
        .
        assign
        ...
        assign
          ...
          parts.cst-code      :screen-value = string(buf_trn-doc.cst-code
                                                        ,parts.cst-code:format)
          parts.last-date     :screen-value = string(?
                                                        ,parts.cst-code:format)
          parts.supp-type     :screen-value = string(
        ( if buf_trn-doc.doc-type = 'при':U then buf_trn-doc.cli-type else buf_trn-doc.obj-type )
                                                        , parts.supp-type :format )
          parts.supp-code     :screen-value = string(
        ( if buf_trn-doc.doc-type = 'при':U then buf_trn-doc.cli-code else buf_trn-doc.obj-code )
                                                        , parts.supp-code :format )
...
PROCEDURE enable-fields :
  ...
  do with frame Dialog-Frame:
    ...
    then do:
      ...
      assign
        l-external-income = (buf_trn-doc.doc-type = 'при':U
                            and buf_trn-doc.internal = false
                            )
      .
      ...
      assign
        v-can-change-part-code =  ( buf_trn-doc.doc-type    <> 'инв':U
                                    and ((buf_trn-doc.status_   = 'накл':U
                                          and buf_trn-doc.flag_ = no
                                          )
                                          or
                                          (buf_trn-doc.status_ = 'касс':U)
                                        )
      ...
      assign
        v-can-change-supp = ( (buf_trn-doc.doc-type = 'возврат':U
                              and buf_trn-doc.internal = false
                              )
                              or
                              buf_trn-doc.ext-doc-type = 'vt':U
                              or
  ...
  ,INPUT-OUTPUT TABLE thbjattr_thbj-attr
      ...
      then do:
        ...
        then do:
          ...
          then do:
            assign
              parts.supp-type :screen-value = 'орг':U
            .
          end.
  ...
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,output l-ok
      ...
      if v-pharm = true then do:
         ...
         hide b-dop in frame Dialog-Frame .
      end.
    ...
    if v-pharm then display fi-price-prod fi-price-prodvat with frame Dialog-Frame .
                 else  hide fi-price-prod fi-price-prodvat  in frame Dialog-Frame .
  end.
END PROCEDURE.
PROCEDURE enable_UI :
  OPEN QUERY Dialog-Frame FOR EACH parts SHARE-LOCK.
...
define variable vss-include-info88 as character format "x(65)" no-undo initial "@(#)$Workfile$".
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curr-r-b in g#library
  (output v-curr-r-b
  )  .
...
PROCEDURE update-last-date-offset :
  ...
  :
    ...
    run godendo-date-to-offset in this-procedure
      (input  v-today
      ,input  (input frame Dialog-Frame parts.last-date)
...
PROCEDURE update-record :
  ...
  :
    ...
    then do:
      apply 'leave':u to ub.parts.part-code in frame Dialog-Frame .
      ...
      if error-status :error
      then do:
        apply 'entry':u to parts.part-code in frame Dialog-Frame .
        undo, return error .
      end.
    end.
    if v-can-change-supp
    ...
    then do:
      run partscr in this-procedure
        ...
        ,input  (input frame Dialog-Frame parts.supp-code)
        ,input  v-new-parts-part-code
        ,input  (input frame Dialog-Frame parts.cst-code )
        ,input  (input frame Dialog-Frame parts.PS)
        ...
        ,input  decimal(parts.price-base :screen-value)
        ,input  decimal(parts.price-rubl :screen-value)
        ,input  fi-vat-type :screen-value
        ,input  decimal(fi-vat-pc :screen-value)
        ,input  fi-slt-type :screen-value
        ,input  decimal(fi-slt-pc :screen-value)
        ,input  0
        ,input  'prompt=disable-create':u
        ,input  0
        ,input  (input frame Dialog-Frame parts.last-date )
        ,input  parts.hold-date
        ,input  p-pl-code
        ,buffer buf_doc-line
        ,buffer buf_parts
        ) no-error .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run rootnode in g#library
  ...
  ,output v-root-node
      ...
      then do:
        ...
        then do:
          assign
            buf_parts.price-cli = input frame Dialog-Frame parts.price-cli
          .
        end.
      end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run cntpurch in g#library
  ...
  ,output v-contract-purch-code
    ...
    case v-enable-qnty :
      ...
      then do:
        assign
          v-chg-cli-qnty = input frame Dialog-Frame parts.cli-qnty - buf_parts.cli-qnty
        .
        if v-goods-twounit = true
        then do:
          assign
            v-chg-qnty = input frame Dialog-Frame parts.qnty - buf_parts.qnty
          .
        end.
      end.
      when "qnty":u
      then do:
        case buf_parts.out-code :
          ...
          then do:
            assign
              v-chg-qnty = - input frame Dialog-Frame parts.qnty
            .
          end.
          when 'out-zone':U
          then do:
          ...
          then do:
            ...
            then do:
              assign
                v-chg-qnty = (buf_parts.qnty - input frame Dialog-Frame parts.qnty)
              .
            end.
            else do:
              assign
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run part-prc in g#library
  ...
  ,output l-process-part
        ...
        and input frame Dialog-Frame parts.qnty <> 0
        then do:
          message
            v-reason
            view-as alert-box information .
      ...
      then do:
        case buf_parts.out-code :
          ...
          then do:
            assign
              v-chg-fact-qnty = input frame Dialog-Frame parts.fact-qnty - buf_parts.fact-qnty
            .
          end.
          otherwise do:
            message
  ...
  ,output  v-price-no-vat-slt-base
        ...
        find first buf2_trn-doc no-lock
          where buf2_trn-doc.doc-code = buf_parts.out-code .
            if not ( buf_parts.out-code = buf_parts.in-code and buf2_trn-doc.ext-doc-type = 'ie':U) then do:
                if buf_parts.cli-base-rate <> 0
                then do:
                  assign
                    buf_parts.cli-qnty = buf_parts.fact-qnty / buf_parts.cli-base-rate
...
PROCEDURE validate-exch-code :
  ...
  on error undo, return error return-value
  :
    if input frame Dialog-Frame parts.exch-code <> v-exch-code
    then do:
      find first buf_currency no-lock
        where buf_currency.curr-code = input frame Dialog-Frame parts.exch-code
        no-error .
      if not available buf_currency
      then do:
        message
          "Неизвестный код валюты" skip
          "Код валюты" input frame Dialog-Frame parts.exch-code skip
          view-as alert-box error .
        apply 'entry':u to parts.exch-code in frame Dialog-Frame .
        undo, return error return-value .
      end.
      else do:
        assign
...
PROCEDURE validate-supp :
  ...
  :
    ...
    then do:
      ...
      apply 'entry':u to parts.supp-type in frame Dialog-Frame.
      return error.
    end.
