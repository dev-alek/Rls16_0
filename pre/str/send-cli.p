...
define variable v-found64 as logical no-undo .
define variable v-is-script64 as logical no-undo.
define variable v-fields-shift64 as integer no-undo .
...
define variable v-found67 as logical no-undo .
define variable v-is-script67 as logical no-undo.
define variable v-fields-shift67 as integer no-undo .
   if can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 1 ) then do:
    v-fields-shift67 = 13.
if v-record <> '':U then
    run maria-put in this-procedure (
...
define variable dflt-cd69 as character no-undo .
define variable vss-include-info70 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type70 as character no-undo .
define variable v-value-date70 as date no-undo .
define variable v-value-decimal70 as decimal no-undo .
define variable v-value-integer70 as INTEGER no-undo .
define variable v-value-logical70 AS LOGICAL no-undo .
define variable v-tth70 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd69
    ,output v-value-date70
    ,output v-value-decimal70
    ,output v-value-integer70
    ,output v-value-logical70
    ,output v-param-type70
    ,INPUT-OUTPUT table-handle v-tth70
    ) no-error .
delete object v-tth70 no-error.
