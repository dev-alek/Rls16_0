...
assign
  v-str-recid15 = trim( string( recid( X_c-usr-hist ) , "->>>>>>>>>>>9":U ) )
...
if v-num-entry15 > 0 then do:
  ...
  .
      ...
      do:
        ...
        apply "VALUE-CHANGED" to br-usr-hist in frame Dialog-Frame.
      end.
    ...
    apply "entry" to br-usr-hist in frame Dialog-Frame.
  END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  OPEN QUERY br-usr-hist FOR EACH X_c-usr-hist NO-LOCK by X_c-usr-hist.corr-date desc INDEXED-REPOSITION .
...
PROCEDURE MyEnable :
  assign
    br-usr-hist:num-locked-columns in frame Dialog-Frame  = 1
...
if p-open-query then do:
  ...
  then do:
    ...
    define variable  parameter-7-17 as character no-undo .
      ...
      parameter-4-17 =
        (
          if (" TRUE " + " " + where-phrase-17) <> ""
          then " TRUE " + " " + where-phrase-17
          else "true"
        )
      parameter-5-17 = (" " + "" + " " + "")
    ...
    :
      assign
        l-disable-where-17 =
          (" TRUE " + " " + where-phrase-17 = "")
      .
      run fltopend_fltopend in this-procedure  ( input this-procedure:handle
                          ,input query br-usr-hist:handle
  ...
  if l-filter-open-17 = false then do:
    OPEN QUERY br-usr-hist FOR EACH X_c-usr-hist
      where  TRUE
       use-index  ie02
      indexed-reposition
  .
  ...
  assign
    v-doc-rec = recid( X_c-usr-hist )
  .
  do
  on stop undo, leave
  on error undo, leave
  :
    if query br-usr-hist:handle:get-buffer-handle(1) = (buffer X_c-usr-hist:handle) then do:
      assign
      parameter-2-17 = (if p-find-next then "true":u else "false":u )
      parameter-4-17 =
        "where ":u + " TRUE " + " ":u + where-phrase-17 + " ":u + p-find-condition + " " + ""
      parameter-5-17 = " use-index  ie02 "
    .
      run fltopend_fltfindd in this-procedure  (
                          input this-procedure:handle
                          ,input query br-usr-hist:handle
                          ,input rowid(X_c-usr-hist)
                          ,input logical(parameter-2-17)
                          ,input no-lock
                          ,input (buffer X_c-usr-hist:handle)
                          ,input parameter-4-17
                          ,input parameter-5-17
                          ) no-error.
      .
      ...
      assign
      parameter-2-17 = (if p-find-next then "true":u else "false":u )
      parameter-3-17 =  "FOR EACH X_c-usr-hist"
      parameter-4-17 =
        (
          if (" TRUE " + " " + where-phrase-17) <> ""
          then " TRUE " + " " + where-phrase-17
