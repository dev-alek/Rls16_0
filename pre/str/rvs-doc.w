...
define browse br-line query br-line no-lock display
  ...
  ub.goods.artic  column-label 'Артикул'
      ...
      ub.rvs-line.pl-code  column-label 'Скл.место'  FORMAT "99999999999":U
...
on return, mouse-select-dblclick of br-pump in frame d-rvs
  do:
    ...
    do:
      apply "choose" to b-chg-pump in frame d-rvs.
    end.
    else
    do:
      apply "choose" to b-lkp-pump in frame d-rvs.
    end.
  end.
...
ON ROW-DISPLAY OF br-line IN FRAME d-rvs
  ...
  define variable vChkColor as log no-undo.
    if avail r-doc and
       (r-doc.rvs-type  = 'перед_док':U or
        r-doc.rvs-type  = 'после_док':U) then
    do:
       do ii = 1 to 37:
          bcol[ii]:FGcolor  = 7.
       end.
...
run get-ptrl-prop in this-procedure
  ...
  if r-doc.status_ = 'разрешен':U and
    pardoc-mode = 'ИЗМЕНЕНИЕ':U then
  do:
...
assign
  v-str-recid25 = trim( string( recid( ub.rvs-line ) , "->>>>>>>>>>>9":U ) )
  v-num-entry25 = lookup( v-str-recid25 , del-list )
.
if v-num-entry25 > 0 then do:
  assign
    entry( v-num-entry25, del-list ) = "":U
    del-list = trim( replace( del-list , chr(44) + chr(44) , chr(44) ) , chr(44) )
  .
end.
else do:
  assign
    del-list = del-list + ( if del-list = "":U then "":U else chr(44) ) + v-str-recid25
  .
end.
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curobjdt in g#library
  ...
  ,output v-obj-date
          ...
          if (r-doc.rvs-type = 'смена':U or parall-place) then
          do:
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_rvs-shift_upd-immeas-trk':U
    ,input  'object':U
    ,input  r-doc.host-code
    ,input  r-doc.obj-type
    ,input  r-doc.obj-code
...
do:
  if (valid-handle(g#library2) <> true) then do:   run gbl/library2.p persistent no-error .   if error-status :error or (valid-handle(g#library2) <> true) then do:     message       "Error starting library2.p" skip       g#library2 skip       g#library2 :type skip       g#library2 :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run chk-actg in g#library2
    ...
    ,input  v-cntxt-userid
    ,input  0
    ,input  'actn_rvs-control_upd-immeas-trk':U
    ,input  'object':U
    ,input  r-doc.host-code
    ,input  r-doc.obj-type
    ,input  r-doc.obj-code
