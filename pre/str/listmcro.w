...
define  shared  temp-table macro-list-hist no-undo
...
DO WHILE glog OR v-start <= p-max-id:
  ...
  by loc_macro-list-hist.id:
    ...
    or entry(1, return-value, chr(4))  = "error":U
    THEN DO:
      assign
      v-dop = return-value
      v-dop = substring(v-dop, index(v-dop, chr(4)) + 1)
      .
...
DO:
  ...
  if not available buf_macro-list-hist then do:
    ...
    assign
    f-name = "default." + entry(2, p-title, chr(4) )
    filter-str1 = substitute("&1 *.&2", entry(1, p-title, chr(4)), entry(2, p-title, chr(4) ))
    filter-str2 = substitute("*.&1", entry(2, p-title, chr(4)))
    glog = yes
    .
  system-dialog get-file f-name
    ...
    use-filename
    update glog
    default-extension entry(2, p-title, chr(4) ).
  if not glog then do:
    apply "entry" to br-macro in frame Dialog-Frame.
    return no-apply.
  end.
  ...
  :
    ...
    :
      assign
        ...
        v-diasize-browse-handle     = browse BR-macro :handle
      .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BR-macro FOR EACH buf_macro-list-hist.
END PROCEDURE.
PROCEDURE get-pause :
