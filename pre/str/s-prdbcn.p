...
def var vss-workfile    as character no-undo init "$Workfile: s-prdbcn.p $":U .
def var vss-archive     as character no-undo init "$Archive: str/s-prdbcn.p $":U .
...
if v-is-restaurant and v-is-null-price then.
    ...
    def var l-in-ov64 as logical no-undo .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run gdsobjat in g#library
  ...
  if (ub.shop.in-ov and  l-in-ov64 ) then
              return.
...
define variable vss-include-info70 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str70  as character no-undo.
  define variable tmp-num70  as character no-undo.
  define variable i70        as integer   no-undo.
  define variable sum70      as integer   no-undo.
  define variable len-code70 as integer   no-undo.
  define variable varcont70  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str70 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str70 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont70 = yes then do:
    if integer( substring( tmp-str70, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        IBm-good-code = ncrsc-pfx + substring( tmp-str70, length( ncrsc-pfx ) + 1, length( tmp-str70 ) - length( ncrsc-pfx ) )
        len-code70    = length( IBm-good-code )
      .
      define variable v-sum-char70 as character no-undo .
      assign
        sum70 = 0
      .
      do i70 = 1 to len-code70 by 2
      :
        assign
          v-sum-char70 = substr(IBm-good-code, len-code70 - i70 + 1, 1)
        .
        if v-sum-char70 < "0"
        or v-sum-char70 > "9"
        then do:
        ...
        assign
          sum70 = sum70 + integer(v-sum-char70)
        .
      end.
      if varcont70 = yes then do:
        assign
        ...
        if varcont70 = yes then do:
           if sum70 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum70 mod 10)
             .
           end.
        end.
      end.
...
define variable vss-include-info71 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str71  as character no-undo.
  define variable tmp-num71  as character no-undo.
  define variable i71        as integer   no-undo.
  define variable sum71      as integer   no-undo.
  define variable len-code71 as integer   no-undo.
  define variable varcont71  as logical   initial yes no-undo.
  CASE ncrpg-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str71 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str71 = string( decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont71 = yes then do:
    if integer( substring( tmp-str71, 1, length( ncrpg-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
        "для товара с кодом: " decimal(string(integer( cash-gds.b-str ), '99999':U) + '00000':U)
      ...
      assign
        IBm-good-code = ncrpg-pfx + substring( tmp-str71, length( ncrpg-pfx ) + 1, length( tmp-str71 ) - length( ncrpg-pfx ) )
        len-code71    = length( IBm-good-code )
      .
      define variable v-sum-char71 as character no-undo .
      assign
        sum71 = 0
      .
      do i71 = 1 to len-code71 by 2
      :
        assign
          v-sum-char71 = substr(IBm-good-code, len-code71 - i71 + 1, 1)
        .
        if v-sum-char71 < "0"
        or v-sum-char71 > "9"
        then do:
        ...
        assign
          sum71 = sum71 + integer(v-sum-char71)
        .
      end.
      if varcont71 = yes then do:
        assign
        ...
        if varcont71 = yes then do:
           if sum71 mod 10 = 0 then do:
             ...
             assign
               IBm-good-code = IBm-good-code + string(10 - sum71 mod 10)
             .
           end.
        end.
      end.
...
then do:
  ...
  view-as alert-box error .
     define variable v-user-action73   as character no-undo .
  define variable v-printed73       as logical   no-undo .
  run gbl/prnfilen.w
    (input  ('!!!При отсылке информации на кассы произошли ошибки!!!')
    ,input  0
    ,input  (string("./":U) + log-file-name)
    ,input  7
    ,output v-user-action73
    ,output v-printed73
    ) .
end.
if v-view-log = true
...
define variable vss-include-info74 as character format "x(65)" no-undo initial "@(#)$Workfile: putc-gds.i $ $Revision: b8cfd3560a3b, 3573, rls $".
PROCEDURE putc-gds.
define parameter buffer buf_cash-desk for ub.cash-desk.
...
then do:
  ...
  then do:
  define variable v-plu-pet80 as character no-undo .
  define buffer  bufpet_cd-plu80 for ub.cd-plu.
  for each bufpet_cd-plu80  where
          bufpet_cd-plu80.obj-type = 'маг':U
      and bufpet_cd-plu80.obj-code = abs(i-obj-code)
      and bufpet_cd-plu80.pos-type = 'MARIA':U
      and bufpet_cd-plu80.plu-type = 'топ':U
      AND bufpet_cd-plu80.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu80.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet80 = TRIM(string( bufpet_cd-plu80.plu-code, "X(40)":U ))
    v-plu-pet80 = substring(v-plu-pet80, length(v-plu-pet80) - 4 + 1)
    .
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code80 as character no-undo .
    assign
      maria-good-code80 = left-trim(ibm-good-code, '0')
      maria-good-code80 = fill('0', 14 - length(maria-good-code80)) + maria-good-code80
    .
      run maria-put in this-procedure (
                                      buffer buf_cash-desk
                                    , input out
                                    ...
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code80, 1, 5) + chr(4) +
                                                    substring(maria-good-code80, 6, 14)
                                                  )
...
define variable v-found101 as logical no-undo .
define variable v-is-script101 as logical no-undo.
define variable v-fields-shift101 as integer no-undo .
...
then do:
  ...
  then do:
  define variable v-plu-pet104 as character no-undo .
  define buffer  bufpet_cd-plu104 for ub.cd-plu.
  for each bufpet_cd-plu104  where
          bufpet_cd-plu104.obj-type = 'маг':U
      and bufpet_cd-plu104.obj-code = abs(i-obj-code)
      and bufpet_cd-plu104.pos-type = 'MARIA':U
      and bufpet_cd-plu104.plu-type = 'топ':U
      AND bufpet_cd-plu104.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu104.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet104 = TRIM(string( bufpet_cd-plu104.plu-code, "X(40)":U ))
    v-plu-pet104 = substring(v-plu-pet104, length(v-plu-pet104) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code104 as character no-undo .
    assign
      maria-good-code104 = left-trim(ibm-good-code, '0')
      maria-good-code104 = fill('0', 14 - length(maria-good-code104)) + maria-good-code104
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code104, 1, 5) + chr(4) +
                                                    substring(maria-good-code104, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
then do:
  ...
  then do:
  define variable v-plu-pet105 as character no-undo .
  define buffer  bufpet_cd-plu105 for ub.cd-plu.
  for each bufpet_cd-plu105  where
          bufpet_cd-plu105.obj-type = 'маг':U
      and bufpet_cd-plu105.obj-code = abs(i-obj-code)
      and bufpet_cd-plu105.pos-type = 'MARIA':U
      and bufpet_cd-plu105.plu-type = 'топ':U
      AND bufpet_cd-plu105.b-code = (if v-marketer-action = 'd'then temp-cd-plu.b-code else buf_cd-plu.b-code)
      AND bufpet_cd-plu105.b-str  = (if v-marketer-action = 'd'then temp-cd-plu.b-str else buf_cd-plu.b-str):
    assign
    v-plu-pet105 = TRIM(string( bufpet_cd-plu105.plu-code, "X(40)":U ))
    v-plu-pet105 = substring(v-plu-pet105, length(v-plu-pet105) - 4 + 1)
    .
    run maria-put in this-procedure (
                                          buffer for-cash-desk
                                        , input out
    ...
    run maria-put in this-procedure (
      ...
      run maria-put in this-procedure (
                                    ...
                                    , input v-plu
                                    , input '001').
      define variable maria-good-code105 as character no-undo .
    assign
      maria-good-code105 = left-trim(ibm-good-code, '0')
      maria-good-code105 = fill('0', 14 - length(maria-good-code105)) + maria-good-code105
    .
      run maria-put in this-procedure (
                                      buffer for-cash-desk
                                    , input out
                                    ...
                                    , input v-plu
                                    , input (if v-marketer-action = "U"
                                              then (substring(maria-good-code105, 1, 5) + chr(4) +
                                                    substring(maria-good-code105, 6, 14)
                                                  )
                                              else (fill('0', 5) + chr(4) + fill('0', 9)))
                                        ).
      run maria-put in this-procedure (
...
define variable v-found106 as logical no-undo .
define variable v-is-script106 as logical no-undo.
define variable v-fields-shift106 as integer no-undo .
   if can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 6 )
   or can-find(temp-tekka-tsk where temp-tekka-tsk.task-num = fname and temp-tekka-tsk.obj-num = 13 )
   then do:
     v-fields-shift106 = - 1.
...
define variable dflt-cd108 as character no-undo .
define variable vss-include-info109 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
define variable v-param-type109 as character no-undo .
define variable v-value-date109 as date no-undo .
define variable v-value-decimal109 as decimal no-undo .
define variable v-value-integer109 as INTEGER no-undo .
define variable v-value-logical109 AS LOGICAL no-undo .
define variable v-tth109 as handle no-undo .
run adm/shattri.p (
    input "get":U
    ,input  'маг':U
    ...
    ,input  'dflt-cd':U
    ,output dflt-cd108
    ,output v-value-date109
    ,output v-value-decimal109
    ,output v-value-integer109
    ,output v-value-logical109
    ,output v-param-type109
    ,INPUT-OUTPUT table-handle v-tth109
    ) no-error .
delete object v-tth109 no-error.
...
define variable vss-include-info127 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str127  as character no-undo.
  define variable tmp-num127  as character no-undo.
  define variable i127        as integer   no-undo.
  define variable sum127      as integer   no-undo.
  define variable len-code127 as integer   no-undo.
  define variable varcont127  as logical   initial yes no-undo.
  CASE ncrsc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str127 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str127 = string( decimal(string(integer( v-ean13 ), '99999':U) + '00000':U), "9999999" )
      .
    end.
    OTHERWISE DO:
        message "Неизвестный тип генерации бар-кода процедурой bc-gnrti.i: " ncrsc-frmt " ."
    ...
    END.
  END CASE.
  if varcont127 = yes then do:
    if integer( substring( tmp-str127, 1, length( ncrsc-pfx ) ) ) <> 0
    then do:
      message
        "Невозможно сформировать бар-код" SKIP
      ...
      assign
        v-tmpchar = ncrsc-pfx + substring( tmp-str127, length( ncrsc-pfx ) + 1, length( tmp-str127 ) - length( ncrsc-pfx ) )
        len-code127    = length( v-tmpchar )
      .
      define variable v-sum-char127 as character no-undo .
      assign
        sum127 = 0
      .
      do i127 = 1 to len-code127 by 2
      :
        assign
          v-sum-char127 = substr(v-tmpchar, len-code127 - i127 + 1, 1)
        .
        if v-sum-char127 < "0"
        or v-sum-char127 > "9"
        then do:
        ...
        assign
          sum127 = sum127 + integer(v-sum-char127)
        .
      end.
      if varcont127 = yes then do:
        assign
        ...
        if varcont127 = yes then do:
           if sum127 mod 10 = 0 then do:
             ...
             assign
               v-tmpchar = v-tmpchar + string(10 - sum127 mod 10)
             .
           end.
        end.
      end.
