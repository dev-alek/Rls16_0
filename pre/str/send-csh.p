...
CASE pos-type:
  when 'IBM':U then do:
    FOR EACH cash-cash NO-LOCK use-index icash:
      if cash-cash.cash-code > 999 then do:                          run write-log-and-file in p-log-handle (                                                   input 1                                                                            , input log-file-name                                                                , input 1                                                                            , input substitute("!!!Ошибка: код кассира больше &1: кассир &2 код &3"                                 , 999                                                                 , cash-cash.cash-code                                                                , cash-cash.cash-name)                                                                                ).                                           assign                                                                               v-view-log = yes                                                                     .                                                                                    NEXT.                                                                              end.
      PUT stream IBMstream unformatted
      '6 "'
...
END CASE .
END PROCEDURE .
define variable vss-include-info10 as character format "x(65)" no-undo initial "@(#)$Workfile: cd-cycl6.i $ $Revision: 0c1a62f6cd43, 1415, test $".
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'добавление кассиров')
                  else ('Ждите - ' + 'удаление кассиров') ) +
...
define variable dflt-cd18 as character no-undo .
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute("Пересылка на кассы &1&2 информации о кассирах", 'маг':U, i-obj-code)
                                          ).
