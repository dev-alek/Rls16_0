...
define input parameter pPromoAttr as character no-undo .
...
procedure putc-17 :
  ...
  define variable ii              as integer   no-undo.
  define variable v-goods         as integer   no-undo .
  define variable v-bc-code       as character no-undo .
  define variable v-PromoAttr     as character no-undo .
...
assign
  v-version-dec = decimal(p-pos-version)
    no-error .
do ii = 1 to num-entries (pPromoAttr,","):
  v-PromoAttr = entry(ii,pPromoAttr,",") .
  v-goods = integer(entry(3,(entry(2,v-PromoAttr,";")),chr(3))) .
...
PROCEDURE   for-cash-cycle:
   ...
   FOR EACH for-cash-desk NO-LOCK WHERE
      ...
      IF (LOOKUP(ub.cash-desk.pos-type,'IBM-XML':U) > 0
...
END CASE.
      RUN putc-17 in this-procedure (buffer for-cash-desk
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'добавление ШК промоакций ')
                  else ('Ждите - ' + 'удаление ШК промоакций ') ) +
...
define variable dflt-cd21 as character no-undo .
