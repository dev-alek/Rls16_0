...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run curshift in g#library
  ...
  if error-status :error then do: return no-apply. end.
  assign frame Dialog-Frame tt-rvs-line.state-measure-tc-qnty tt-rvs-line.state-add-qnty tt-rvs-line.state-brutto-tc-qnty tt-rvs-line.state-temperature.
...
if ptrlprop-calc-free-vol then do:
  ...
  infoSectionsTotal = new ibs.th.str.InfoSectionsTotal(buf_trn-doc.doc-code, tt-rvs-line.gds-code, 'ПРОСМОТР':U).
  sect_ :
...
DO:
  ...
  display tt-rvs-line.fact-sum-vol with frame Dialog-Frame .
END.
...
DO:
  if input frame Dialog-Frame tt-rvs-line.state-density <> tt-rvs-line.state-density
  then do:
...
DO:
  if input frame Dialog-Frame varstate-water-qnty <> varstate-water-qnty
  then do:
...
DO:
  if input frame Dialog-Frame tt-rvs-line.state-measure-qnty <> tt-rvs-line.state-measure-qnty
  then do:
    assign frame Dialog-Frame tt-rvs-line.state-measure-qnty .
    ...
    display tt-rvs-line.fact-sum-mass tt-rvs-line.state-measure-qnty abs-delta-mass-add-qnty with frame Dialog-Frame.
  end.
END.
...
DO:
  if input frame Dialog-Frame tt-rvs-line.izmer-density <> tt-rvs-line.izmer-density
  then do:
...
DO:
  if input frame Dialog-Frame tt-rvs-line.state-level-water <> tt-rvs-line.state-level-water
  then do:
...
DO:
  ...
  then do :
    ...
    then do :
      if input frame Dialog-Frame tt-rvs-line.state-level-total < 1
      then do :
        enable t-tank-cleaning with frame Dialog-Frame .
      end .
      else do :
        assign t-tank-cleaning = no .
        display t-tank-cleaning with frame Dialog-Frame .
        disable t-tank-cleaning with frame Dialog-Frame .
      end .
    end .
  end .
  else do :
...
DO:
  ...
  then do :
    ...
    .
    display tt-rvs-line.state-measure-cli-qnty tt-rvs-line.state-measure-qnty tt-rvs-line.fact-sum-mass varstate-sum-vol with frame Dialog-Frame.
  end .
END.
...
do:
  ...
  if buf_rvs-doc.rvs-type = 'контроль':U
  ...
  assign
    tt-rvs-line.calc-add-mass = tt-rvs-line.add-qnty * input frame Dialog-Frame tt-rvs-line.density
  ...
  if parmode <> 'ИЗМЕНЕНИЕ':U then do:
    ...
    disable b-calc b-temperature b-density with frame Dialog-Frame.
    if rdc-value =  "pomi-rn"
    then do :
      hide
        hide-text-dop-si
    ...
    and tt-rvs-line.state-level-total > 0
    then do :
      enable tt-rvs-line.state-density with frame Dialog-Frame.
    end.
    else do :
      disable tt-rvs-line.state-density with frame Dialog-Frame.
    end.
    if pl-rvd-lvl
    then do :
      if rdc-value =  "pomi-rn"
    ...
    then do :
      ...
      then do :
        ...
        then do :
          tt-rvs-line.temp-izm-vol = ? .
          tt-rvs-line.state-temperature = ? .
          display tt-rvs-line.temp-izm-vol tt-rvs-line.state-temperature with frame Dialog-Frame.
        end .
        if not v-revision-mode
        and v-mi-tmp > 0
        and tt-rvs-line.state-level-total > 0
...
.
abs-delta-mass-add-qnty = tt-rvs-line.fact-calc-add-mass * pl-error-mass / 100 no-error .
display tt-rvs-line.state-measure-cli-qnty tt-rvs-line.fact-calc-add-mass tt-rvs-line.state-measure-qnty tt-rvs-line.fact-sum-mass varstate-sum-vol with frame Dialog-Frame.
END PROCEDURE.
  ...
  if not available bf_pl-level then do:
    ...
    if varlevel-sm = varlevel-sm-q then do:
      ...
      assign
        ...
        tt-rvs-line.state-brutto-qnty = tt-rvs-line.state-measure-qnty + varstate-water-qnty
      .
        display tt-rvs-line.state-measure-qnty with frame Dialog-Frame.
...
then do :
  ...
  if input frame Dialog-Frame tt-rvs-line.state-brutto-qnty -
