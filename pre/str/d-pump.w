...
if not available bf_clients then
  ...
  if available bf_pump then do:
    return error SUBSTITUTE("Уже есть ТРК с номером &1", parpump-code) + SUBSTITUTE(" на объекте &1 &2 .", parobj-type, parobj-code).
  end.
  create bf_pump.
  assign
...
then do:
  ...
  assign
    frame Dialog-Frame varpump-code
  .
