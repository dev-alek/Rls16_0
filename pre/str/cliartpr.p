...
define frame f-doc
        ...
        buf_goods.gds-name COLUMN-LABEL "Наименование товара! " format "X(50)" space(0)
        sym2 column-label ":!:!" format "X(1)" space(0)
        buf_goods.artic COLUMN-LABEL "Артикул! " format "X(20)" space(0)
        sym3 column-label ":!:!" format "X(1)" space(0)
        buf_cli-gds.cli-art COLUMN-LABEL "Артикул поставщика! " format "X(20)" space(0)
        sym4 column-label ":!:!" format "X(1)" space(0)
    header
        v-notes at 2 format "X(35)" string( "Страница " + string( PAGE-NUMBER( PrnLibStream ), ">>9" ) ) at 83 format "X(13)" SKIP
        v-single-line format "X(94)" at 1
    with width 235 down stream-io.
form header
        v-single-line format "X(94)" at 1 SKIP
...
assign
ReportName = "Список товаров по поставщикам с артикулами поставщика"
...
.
  ...
  define variable c-v-cli-art as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[3] = true then DO:
        ...
        CREATE EDITOR L-3 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-v-cli-art IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME x1:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
.
  ...
  define variable c-v-cli-obj-name as widget-handle no-undo.
   if l-col-pos > 320 then l-col-pos = 320 .
  if use-column[4] = true then DO:
        ...
        CREATE EDITOR L-4 IN WIDGET-POOL "My-pool"
          ASSIGN
            ...
            HEIGHT-CHARS = 3
        .
        CREATE FILL-IN C-v-cli-obj-name IN WIDGET-POOL "My-pool"
          ASSIGN
            FRAME = FRAME x1:HANDLE
            DATA-TYPE = "CHARACTER"
            FORMAT = "X(":U + string(l-col-len) + ")":U
...
run rep/extitle.p ( 1 ) .
put stream PrnLibStream "Список товаров по поставщикам с артикулами поставщика" at 21 skip.
    ...
    :
      if first-of( buf_cli-list.obj-name ) then do :
        ...
        if not first( buf_cli-list.obj-name ) and line-counter( PrnLibStream ) <> 1 then do :
          put stream PrnLibStream v-single-line format "X(94)".
        end.
...
on error undo, return error :
  ...
  if use-column[1]
  then  C-v-gds-name:screen-value = string(buf_goods.gds-name, entry(1, c-v-gds-name:private-data, chr(4))).
  if use-column[2]
  then  C-v-artic:screen-value = string(buf_goods.artic, entry(1, c-v-artic:private-data, chr(4))).
  if use-column[3]
  then  C-v-cli-art:screen-value = string(buf_cli-gds.cli-art, entry(1, c-v-cli-art:private-data, chr(4))).
  if use-column[4]
  then  C-v-cli-obj-name:screen-value = string(buf_cli-list.obj-name, entry(1, c-v-cli-obj-name:private-data, chr(4))).
      if Make-Excel then  put   stream ForExcel unformatted
  if use-column[1]
  then (reg-output(
                    string(buf_goods.gds-name, entry(1, c-v-gds-name:private-data, chr(4)))
  ...
  if use-column[2]
  then (reg-output(
                    string(buf_goods.artic, entry(1, c-v-artic:private-data, chr(4)))
  ...
  if use-column[3]
  then (reg-output(
                    string(buf_cli-gds.cli-art, entry(1, c-v-cli-art:private-data, chr(4)))
                   ,c-v-cli-art:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
  ...
  if use-column[4]
  then (reg-output(
                    string(buf_cli-list.obj-name, entry(1, c-v-cli-obj-name:private-data, chr(4)))
                   ,c-v-cli-obj-name:private-data
                   ,v-reg-replace
                   ,no
                   ,v-dec-sep
                   ,v-th-sep)  +
...
on error undo, return error :
  display stream PrnLibStream
    ...
    sym1 sym4
  with frame f-doc.
  put stream PrnLibStream v-single-line format "X(94)" skip.
end.
end.
