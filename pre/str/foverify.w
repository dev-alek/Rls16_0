...
DO:
  OPEN QUERY BROWSE-3 FOR EACH tt-temp NO-LOCK WHERE tt-temp.trn-doc-code =     tt-trn.trn-doc-code INDEXED-REPOSITION.
END.
...
if v-ok = false then return .
  ASSIGN frame Dialog-Frame:TITLE = "Накладные по сгенеренным ФО для разбора "  + string( v-date , "99/99/9999" ) + " - " + string( v-date2 , "99/99/9999" ) .
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-3 FOR EACH tt-temp NO-LOCK WHERE tt-temp.trn-doc-code =     tt-trn.trn-doc-code INDEXED-REPOSITION.    OPEN QUERY BROWSE-4 FOR EACH tt-trn NO-LOCK INDEXED-REPOSITION.
END PROCEDURE.
PROCEDURE proc-calc :
  do
  on error undo, return error return-value
          ...
          for each buf_parts no-lock where
              ...
              find first ub.contract where
                         ...
                         ub.contract.contract-code = buf_parts.contract-code no-error .
                           if not ((
                            lookup ( ub.contract.usl-opl , 'По факту поставки,Отсрочка платежа (по поставке)' ) > 0 and
                            lookup ( buf_trn-doc.ext-doc-type , 'ie,ep,':U  ) > 0
                            ) OR
                            ( lookup ( ub.contract.usl-opl , 'По факту реализации,По реализации части приход. накладной,Отсрочка платежа (по реализации)' ) > 0   and
              ...
              v-q = 0.
                    for each  fin_parts no-lock where
                              ...
                              find first buf_fin-ob no-lock where
                                        ...
                                        buf_fin-ob.doc-code  = fin_parts.fin-ob-code
                                        no-error .
