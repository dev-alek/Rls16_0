...
define buffer buf_trn-doc for ub.trn-doc.
  ...
  on error undo, return error substitute( "&1. &2&3&4", vss-workfile, return-value, chr(10), error-status :get-message (1)):
    ...
    for each buf_inkas where
      ...
      .
run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("Обработка продажи &1............"                                                      , buf_inkas.inkas-code)).
      run str/salersrv.p (
                      input parparentproc
                     ,input this-procedure
                     ,input p-log-handle
...
if num-entries(p-parameter, chr(4) ) <> 3 then do:
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 &2 &3&4Неверное кол-во entry в составном параметре p-parameter = &5, должно быть 3"                               ,vss-workfile                               ,vss-revision                               ,vss-description                               ,chr(10)                               , num-entries(p-parameter, chr(4) )                               )).
  .
  return.
end.
assign
...
p-doc-code = entry(3, p-parameter, chr(4))
no-error
.
if error-status:error then do:
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 &2 &3&4Ошибка при получении значение из составного параметра p-parameter:&4&5"                               ,vss-workfile                               ,vss-revision                               ,vss-description                               ,chr(10)                               , error-status:get-message(1)                               )).
  .
  return.
end.
...
if v-obj-db-num <> g#db-num then do:
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 &2 &3&4Закачку и резервирование чеков можно сделать только в текущей БД"                               ,vss-workfile                               ,vss-revision                               ,vss-description                               ,chr(10)                               )).
  .
  return.
end.
...
if not available buf_inkas then do:
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "Создание нового документа продажи в &1&2..........."                         , p-obj-type                           , p-obj-code  )).
  run str/cre-sale.p (
                      INPUT parparentproc
                    , INPUT p-obj-type
                    , INPUT p-obj-code
  ...
  if error-status:error then do:
        run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "!!!Ошибка при создании документа продажи в &1&2&3" +                             "&4 &5"                           , p-obj-type                            , p-obj-code                             , chr(10)                           , error-status:get-message(1)                           , return-value )).
    return ''.
  end.
end.
...
if error-status:error then do:
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 &2 &3&4ошибка при закачке чеков в продажу с &5 на &6&7:&4&8&4&9 "                               ,vss-workfile                               ,vss-revision                               ,vss-description                               ,chr(10)                               ,p-doc-code                               ,p-obj-type                               ,p-obj-code                                , error-status:get-message(1)                               , return-value                               )).
  return.
end.
if error-status:error then do:
  run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute("&1 &2 &3&4ошибка при закачке чеков в продажу с &5 на &6&7:&4&8&4&9 "                             ,vss-workfile                             ,vss-revision                             ,vss-description                             ,chr(10)                             ,p-doc-code                             ,p-obj-type                             ,p-obj-code                              , error-status:get-message(1)                             , return-value                             )).
...
run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "Резервирование документа продажи в &1&2..........."                       , p-obj-type                       , p-obj-code                       )).
run proc-step-200 in this-procedure(
                                    ...
                                    ,input no  )  no-error .
if error-status:error then do:
    run write-log-and-file in p-log-handle (                 input 1                                          , input log-file-name                              , input 1                                          , input substitute( "!!!Ошибка при резервировании документа продажи &1&2&3&4 &5"                         , p-obj-type                         , p-obj-code                         , chr(10)                         , error-status:get-message(1)                         , return-value                         )).
  return.
end.
