...
define variable vss-include-info57 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  run mainmenu_getcntxt in parparentproc
    ...
    :
        ...
        :
            ...
            then do:
                ...
                :
                    ...
                    assign
                        buf_fbr-pln-line.status_ = 'факт':U
                    .
                end.
                run write-counter in p-log-handle (
                    input ""
        ...
        assign
            buf_fbr-pln.fact-num    = next-value( s-trn-fact, ub )
        .
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run objat in g#library
  ...
  ,output v-shift-on
        ...
        run factord in this-procedure (
              input  buf_fbr-pln.fact-date
            , input  buf_fbr-pln.fact-time
            , input  buf_fbr-pln.fact-num
            , input  buf_fbr-pln.shift-date
            , input  buf_fbr-pln.shift-num
            , input  v-shift-on
            , output v-fact-order
            , output v-shift-end-fact-order
            , output v-day-end-fact-order
