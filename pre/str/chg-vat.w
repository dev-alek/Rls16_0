...
then do:
  ...
  if varold-vat-pc = ? then do:
    ...
    apply "entry" to varold-vat-pc in frame Dialog-Frame.
    return no-apply.
  end.
  if varvat-pc = ? then do:
    message "Вы не установили новый процент НДС." view-as alert-box error.
    apply "entry" to varvat-pc in frame Dialog-Frame.
    return no-apply.
  end.
...
then do:
  ...
  if available tt-purch then do:
    assign
     ...
     display tt-purch.mark with browse BROWSE-1.
     get next BROWSE-1.
     if available tt-purch then do:
       reposition BROWSE-1 to recid recid(tt-purch).
     end.
  end.
END.
ON VALUE-CHANGED OF varchange-price IN FRAME Dialog-Frame
DO:
  assign
       frame Dialog-Frame varchange-price.
END.
ON LEAVE OF varold-vat-pc IN FRAME Dialog-Frame
DO:
  assign
    frame Dialog-Frame
    varold-vat-pc.
...
procedure get-context :
  ...
  BROWSE-1 :SET-REPOSITIONED-ROW(2, "CONDITIONAL") .
  do varcount = 1 to num-entries('1,2,3,4':U):
    create tt-purch.
    assign
      tt-purch.purch-code = varcount
      tt-purch.mark = no
      tt-purch.purch-name = entry (varcount, 'выкуп,консигнация,ответственное хранение,старая консигнация':U).
  end.
  RUN enable_UI.
...
PROCEDURE enable_UI :
  ...
  OPEN QUERY BROWSE-1 FOR EACH tt-purch.
END PROCEDURE.
