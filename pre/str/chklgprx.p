...
define variable vss-include-info14 as character format "x(65)" no-undo initial "@(#)$Workfile: prtmpldf.i $ $Revision: 06dcfe20b136, 752, rls $".
...
define variable vss-include-info19 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  define variable tmp-str19  as character no-undo.
  define variable tmp-num19  as character no-undo.
  define variable i19        as integer   no-undo.
  define variable sum19      as integer   no-undo.
  define variable len-code19 as integer   no-undo.
  define variable varcont19  as logical   initial yes no-undo.
  CASE bc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str19 = string( internal-b-code, "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str19 = string( internal-b-code, "9999999" )
      .
    end.
    OTHERWISE DO:
    ...
    END.
  END CASE.
  if varcont19 = yes then do:
    if integer( substring( tmp-str19, 1, length( bc-pfx ) ) ) <> 0
    then do:
      message
      ...
      assign
        full-b-code = bc-pfx + substring( tmp-str19, length( bc-pfx ) + 1, length( tmp-str19 ) - length( bc-pfx ) )
        len-code19    = length( full-b-code )
      .
      define variable v-sum-char19 as character no-undo .
      assign
        sum19 = 0
      .
      do i19 = 1 to len-code19 by 2
      :
        assign
          v-sum-char19 = substr(full-b-code, len-code19 - i19 + 1, 1)
        .
        if v-sum-char19 < "0"
        or v-sum-char19 > "9"
        then do:
        ...
        assign
          sum19 = sum19 + integer(v-sum-char19)
        .
      end.
      if varcont19 = yes then do:
        assign
        ...
        if varcont19 = yes then do:
           if sum19 mod 10 = 0 then do:
             ...
             assign
               full-b-code = full-b-code + string(10 - sum19 mod 10)
             .
           end.
        end.
      end.
...
return entry (lookup (STRING(if buf_chk-gds.write-off-code = ? then 0 else buf_chk-gds.write-off-code),  '?,0,1,-6,-9,2,-2,3,-3,-4,17':U), ',,Без_оплаты,Отмена_позиции,Полн_Отмена,Модификатор,Модификатор,Модификатор(+спис),Модификатор(-спис),Модификатор(-спис),Техпролив':U).
end function.
FUNCTION chkgdsfi_pass-gds-name returns character ( buffer buf_chk-gds for ub.chk-gds):
return entry (lookup (STRING(if buf_chk-gds.pass-gds = ? then 0 else buf_chk-gds.pass-gds), '0,1,2':U), 'Сканер,Вручную,Копирование':U).
end function.
...
define buffer buf_usr-flt_custom-labels for usr-flt_custom-labels.
  IF v-chkgdsfi = "" then
  v-chkgdsfi = 'chk-gds.doc-code,chk-gds.line-num,chk-gds.b-code,chk-gds.src-code,chk-gds.price-base,chk-gds.doc-qnty,chk-gds.discnt':U.
...
vNum-entries = NUm-ENTRIES(v-chkgdsfi).
  DO ii = 1 to vNum-entries:
    ...
    find first buf_usr-flt_custom-labels where
         ...
         and  buf_usr-flt_custom-labels.call-point = 'chkgdsfi':U
...
return entry (lookup (string(buf_chk-doc.chk-type), '1,6,8,69,96,14,15,16,36,17,11,12,13,40,101,106,108,169,196,114,115,116,117,111,112,136,113,201,206,208,301,306,2,3,4,5,7,43,44':U) + 1, ',' + 'Продажа,Возврат,Аннуляция,Списание,ВзврСпис,СбросТрнзкц,Перелив,ПеревТрнзкц,РазблТрнзкц,ТехПролив,Инвентаризация,Z-отчет,Закрытие_смены,Открытие_смены,_Продажа,_Возврат,_Аннуляция,_Списание,_ВзврСпис,_СбросТрнзкц,_Перелив,_ПеревТрнзкц,_ТехПролив,_Инвентаризация,_Z-отчет,_РазблТрнзкц,_Закрытие_смены,>Продажа,>Возврат,>Аннуляция,>>Продажа,>>Возврат,Инкассация,Касс_фонд,Перевод_опл,Расход_кассы,Декл_ден_ящ,Приход_Корр,Расход_Корр':U).
end function.
...
if lookup(string(buf_chk-doc.chk-type), '2,3,4,5,7':U) > 0 then return ''.
...
define buffer buf_usr-flt_custom-labels for usr-flt_custom-labels.
  IF v-chkdocfi = "" then
  v-chkdocfi = 'chk-doc.doc-code,chk-doc.obj-code,chk-doc.#chk-type-name,chk-doc.tot-doc,chk-doc.netto,chk-doc.chk-date,chk-doc.#chk-time-chr':U.
...
vNum-entries = NUm-ENTRIES(v-chkdocfi).
  DO ii = 1 to vNum-entries:
    ...
    find first buf_usr-flt_custom-labels where
              buf_usr-flt_custom-labels.tbl-name = entry(1, entry-ii, ".")
         and  buf_usr-flt_custom-labels.fld-name = entry(2, entry-ii, ".")
         and  buf_usr-flt_custom-labels.call-point = 'chkdocfi':U
         and  buf_usr-flt_custom-labels.call-type = 'add-fields':U no-error.
    if  available buf_usr-flt_custom-labels then do:
      case buf_usr-flt_custom-labels.tbl-name:
...
define buffer buf_usr-flt_custom-labels for usr-flt_custom-labels.
 ...
 if avail fi-goods then do:
      FIND FIRST fi-gds-obj NO-LOCK where
                 ...
                 fi-gds-obj.obj-type = buf_chk-list.obj-type AND
                 fi-gds-obj.obj-code = buf_chk-list.obj-code NO-ERROR.
  ...
  :
    CASE p-f-name:
      when "iColumn":U then do:
        if p-excel then do:
           return replace(string(iColumn, p-format), chr(44), '').
        end.
        else do:
...
if (valid-handle(g#library) <> true) then do:   run gbl/library.p persistent no-error .   if error-status :error or (valid-handle(g#library) <> true) then do:     message       "Error starting library.p" skip       g#library skip       g#library :type skip       g#library :file-name skip       error-status :get-message(1) skip       return-value skip       view-as alert-box error .     stop .   end. end. run bcodeprc in g#library
  ...
  ,output for-excise
        ...
        if p-excel then do:
          return replace(string(price-sale, p-format), chr(44), "").
        end.
        else do:
...
for each  buf_custom-labels no-lock where
      ...
      and buf_custom-labels.call-point = 'chkgdsfi':U
     :
...
for each  buf_custom-labels no-lock where
          buf_custom-labels.call-type = 'add-fields':U
      and buf_custom-labels.call-point = 'chkdocfi':U
     :
  create buf_usr-flt_custom-labels.
  buffer-copy buf_custom-labels to buf_usr-flt_custom-labels.
end.
    ...
    CASE  v-dop1:
      when "function":U then do:
        ...
        get-function( input substr(v-dop0 ,10 )
                     ,input entry(jj, ubflt.filter.Where-ysl-rus, chr(4))
                     ,input yes
                     )
        (if jj < j-n
        then
      ...
      when 'goods':U then do:
        if index(v-dop0, "*":U) > 0 then do:
          do jj-1 = 1 to num-entries(v-dop0, "*":U):
            ...
            run gds-ref-fi in this-procedure (
                            ...
                            ,input buf_chk-list.obj-type
                            ,input buf_chk-list.obj-code
                            ,INPUT entry(jj-1, entry(jj, f-name), "*":U)
                            ,input yes
                            ,INPUT-OUTPUT f-value-1
                            )  NO-ERROR .
            assign
        ...
        ii-excel = ii-excel + 1.
        if Make-Excel then  put   stream ForExcel unformatted
        if v-dop0 = "artic" then chr(4) else "":U
        string(f-value, substitute("X(&1)", entry(jj, ubflt.filter.Where-ysl)))
        (if jj < j-n
        then
        CHR(9)
...
DEFINE VARIABLE vother               as character           no-undo .
  ...
  :
    ...
    tbl = 'chk-gds':U + chr(44) +
          'chk-doc':U + chr(44) +
          'goods':U + chr(44) +
          'bar-code':U + chr(44) +
          "function"
    c-point =  "chklgprx" + chr(4) +  "PRNT":u  + chr(4) + "Товарные строки списка чеков"
    fld = ""
    lab = ""
    spr = ""
    dim = '0'
   ...
   for each buf_usr-flt_custom-labels where
       ...
       and buf_usr-flt_custom-labels.call-point = 'chkgdsfi':U
       and buf_usr-flt_custom-labels.tbl-name = 'chk-gds':U
   by buf_usr-flt_custom-labels.custom-label
    ...
    for each buf_usr-flt_custom-labels where
           buf_usr-flt_custom-labels.call-type = 'add-fields':U
       and buf_usr-flt_custom-labels.call-point = 'chkdocfi':U
       and buf_usr-flt_custom-labels.tbl-name = 'chk-doc':U
    by buf_usr-flt_custom-labels.custom-label
       :
      run prnfield-add in this-procedure(input buf_usr-flt_custom-labels.fld-name
                                        ,input buf_usr-flt_custom-labels.custom-label
