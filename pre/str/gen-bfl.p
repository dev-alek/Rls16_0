...
for each buf_fin-ob-before no-lock where
    ...
    if first-of (buf_fin-ob-before.trn-doc-code-orig) then do:
       ...
       if available buf_contract then v-proc-contr = buf_contract.srok-opl .
                                     ...
                                     v-proc-contr = 0 .
                                     v-message-text =  v-message-text + chr(10)  + error-status :get-message(1) + " Вн.№ договора " + string(buf_fin-ob-before.contract-code) .
                                 end.
      ...
      if available buf_incom-trn-doc
          then do:
           ...
           v-sum-pn = 0 .
            v-go = false .
            v-message-text =  v-message-text + chr(10)  + " ПН " + buf_fin-ob-before.trn-doc-code-orig + " УДАЛЕНА".
          end.
      v-sum-ras = 0 .
    end.
       ...
       if ( 100 * v-sum-ras / v-sum-pn ) >=  v-proc-contr  then do:
          if  v-go = true then do:
              run make-fo in this-procedure (input  buf_fin-ob-before.trn-doc-code-orig , output v-fo-code )  no-error .
                  if error-status :error then v-message-text =  v-message-text + chr(10)  + error-status :get-message(1) + " создание ФО " .
              run close-bfo in this-procedure (input buf_fin-ob-before.trn-doc-code-orig, input  v-fo-code  ) no-error .
                  if error-status :error then v-message-text =  v-message-text + chr(10)  + error-status :get-message(1) + " закрытие ПФО " .
          end.
       end.
       else do:
            if p-view-text = true then
                v-tt = chr(10)  +
                "№ ПН          "  + buf_fin-ob-before.trn-doc-code-orig + chr(10)  +
                "Cумма по ПН   "  + string( v-sum-pn  ) + chr(10)  +
                "Cумма по ПФО  "  + string( v-sum-ras ) + chr(10)  +
                "% расчетный   "  + string( 100 * v-sum-ras / v-sum-pn ) + chr(10) +
                "% по договору "  + string( v-proc-contr ) +  chr(10) +
                "-------------------------------"
                .
            if v-tt = ? then v-tt = "" .
            v-message-text =  v-message-text + chr(10)  + v-tt .
       end.
    end.
end.
v-message-text =  v-message-text + chr(10)  +
    chr(10)  +
...
define input parameter p-fo-code as character no-undo .
...
for each b_fin-ob-before  exclusive-lock  where
  ...
  assign
    ...
    b_fin-ob-before.status_     = 'факт':U
