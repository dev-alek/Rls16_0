...
define variable varstr-gen     as character no-undo.
...
if not paronly-b-code then do:
  ...
  if available bf_prod-bc then do:
    ...
    parresult = "prod-bc".
    if length (parstr-code) <= 5 then do:                                            assign                                                                        partype-bc = "Весовой код".                                                   if length (parstr-code) < 5 then do:                                            assign                                                                        partype-bc = partype-bc + " без ведущих нулей".                             end.                                                                       end.                                                                          else do:                                                                        partype-bc = "Дополнительный код".                                          end.
  end.
  else do:
...
and bf_bar-code.stts_ = integer('99':U) then do:
  assign
  parresult  = "error"
...
define variable vss-include-info2 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  CASE varbc-frmt :
    WHEN "EAN13" THEN do:
      assign
        tmp-str2 = string( varsrc-gen-int, "999999999999" )
      .
    end.
    WHEN "EAN8" THEN do:
      assign
        tmp-str2 = string( varsrc-gen-int, "9999999" )
      .
    end.
    OTHERWISE DO:
          assign varstr-gen     = ""
  ...
  if varcont2 = yes then do:
    if integer( substring( tmp-str2, 1, length( varbc-pfx ) ) ) <> 0
    then do:
         assign varstr-gen     = ""
                varcont2 = no.
    end.
      ...
      assign
        varstr-gen = varbc-pfx + substring( tmp-str2, length( varbc-pfx ) + 1, length( tmp-str2 ) - length( varbc-pfx ) )
        len-code2    = length( varstr-gen )
      .
      ...
      :
        assign
          v-sum-char2 = substr(varstr-gen, len-code2 - i2 + 1, 1)
        .
        if v-sum-char2 < "0"
        or v-sum-char2 > "9"
        then do:
      ...
      if varcont2 = yes then do:
        ...
        if varcont2 = yes then do:
           if sum2 mod 10 = 0 then do:
             assign
               varstr-gen = varstr-gen + '0'
             .
           end.
           else do:
             assign
               varstr-gen = varstr-gen + string(10 - sum2 mod 10)
             .
           end.
        end.
      end.
