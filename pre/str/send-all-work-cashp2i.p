...
procedure putc :
   ...
   mDevice = getDevice().
   mParent = substitute ("&2&1&3",chr(4),  "Cash-param",  mDevice ).
  for each code-group where code-group.parent     eq mParent no-lock:
      ...
      do:
         ...
         do:
            iSAXWriter:insert-attribute("scan", "*"      )   no-error.
         end.
      ...
      do:
         ...
         iSAXWriter:start-element("PosKey").
         do:
            iSAXWriter:insert-attribute("scan", "*"      )  .
         end.
         iSAXWriter:end-element("PosKey" ).
      end.
      iSAXWriter:end-element("KeyMap" ).
   ...
   mParent = substitute ("&2&1&3",chr(4),  mParent,  2 ).
end procedure.
...
procedure StartElement:
  ...
  if mElement = "PosKey" then do:
    ...
    assign
      ...
      tt-cash-param-hist.param_section      = "2"
   ...
   if qname = "ErrorMessage" then do:
      ...
      then do:
         ...
         then do:
            create code.
            assign
               code.parent    = mParent  + chr(4) + tt-cash-param-hist.param_group
