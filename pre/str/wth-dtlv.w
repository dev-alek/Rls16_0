...
DEFINE BROWSE BR-dtl
  QUERY BR-dtl DISPLAY
      ...
      tt-par-dtl.q-ty-aft
      tt-par-dtl.sum-aft
      (if vardoc-status_ = 'разрешен':U then tt-par-dtl.q-ty-bef - tt-par-dtl.q-ty-aft else 0)
      tt-par-dtl.sum-fact
ENABLE
      tt-par-dtl.q-ty-bef
tt-par-dtl.q-ty-aft
...
DO:
  CASE vardoc-status_:
    when 'накл':U then do:
      ASSIGN
      tt-par-dtl.q-ty-bef = INPUT BROWSE BR-dtl tt-par-dtl.q-ty-bef.
      ASSIGN
    ...
    when 'разрешен':U then do:
      ASSIGN
      tt-par-dtl.q-ty-aft = INPUT BROWSE BR-dtl tt-par-dtl.q-ty-aft.
      ASSIGN
...
if not can-find(first tt-par-dtl) then do:
    ...
    tt-par-dtl.sum-fact = if (vardoc-status_ = 'разрешен':U)
...
IF par-mode = 'ПРОСМОТР':U THEN DO:
  ASSIGN
  tt-par-dtl.q-ty-bef:READ-ONLY IN BROWSE BR-dtl = YES
  tt-par-dtl.q-ty-aft:READ-ONLY IN BROWSE BR-dtl = YES
  .
END.
