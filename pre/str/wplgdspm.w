...
END.
ON CHOOSE OF b-cur IN FRAME Dialog-Frame
DO:
  if available X_pl-gds-pump then do:
    run local-stts in this-procedure
      ( input 'тек':U
...
DO:
   ...
   IF AVAILABLE X_pl-gds-pump  THEN DO:
        run ref/cplchist.w (
                     ...
                     , input X_pl-gds-pump.gds-code
                     , input X_pl-gds-pump.pump-code
                     , input 0
                     , input 'pl-gds-pump':U
...
do:
   if available X_pl-gds-pump then v-doc-rec = recid(X_pl-gds-pump). OPEN QUERY b-plgdspm FOR EACH X_pl-gds-pump       WHERE X_pl-gds-pump.obj-type = parobj-type  AND X_pl-gds-pump.obj-code = parobj-code NO-LOCK,              EACH X_goods WHERE X_goods.gds-code = X_pl-gds-pump.gds-code NO-LOCK,              EACH X_place WHERE X_place.pl-code = X_pl-gds-pump.pl-code NO-LOCK,              EACH X_pl-pump-nozzle WHERE X_pl-pump-nozzle.obj-type = X_pl-gds-pump.obj-type   AND X_pl-pump-nozzle.obj-code = X_pl-gds-pump.obj-code   AND X_pl-pump-nozzle.pl-code = X_pl-gds-pump.pl-code   AND X_pl-pump-nozzle.pump-code = X_pl-gds-pump.pump-code OUTER-JOIN NO-LOCK     BY X_pl-gds-pump.obj-type      BY X_pl-gds-pump.obj-code       BY X_pl-pump-nozzle.pump-code        BY X_pl-pump-nozzle.nozzle-code.   reposition b-plgdspm to recid v-doc-rec no-error. apply 'ENTRY' to b-plgdspm.
...
define variable vss-include-info9 as character format "x(65)" no-undo initial "@(#)$Workfile$ $Revision$".
  ...
  if v-cntxt-db-num <> v-cntxt-db-num-obj then do:
    assign
      b-block :sensitive in frame Dialog-Frame = no
      b-cur :sensitive in frame Dialog-Frame = no
    .
  end.
