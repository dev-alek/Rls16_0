...
define variable v-sum        as decimal   no-undo .
  ...
  if pSubs = "" then do:
      ...
      do ii = 1 to num-entries (pSubs,chr(6)):
            for each buf_OperServ where buf_OperServ.Status_ = 0 and buf_OperServ.id = integer(entry(ii,pSubs,chr(6))):
        if action <> 'D' then do:
          v-sum = buf_OperServ.CommPerc + buf_OperServ.CommSumm .
          run bgelib-tag-open in this-procedure ( input 2, input "OperServ", input substitute("ctrl='&2' tms='&3' code='&1'", string (buf_OperServ.id), "ADD":u, OS2-time)).
          run bgelib-tag-put in this-procedure ( input 3, input "OSCode":U, input string (buf_OperServ.id), input 1 ).
...
FOR EACH for-cash-desk NO-LOCK WHERE
  ...
  if error-status:error
  or v-versiond < 1.11
  or for-cash-desk.pos-type <> 'IBM-XML':U
  then do:
    run write-log-and-file in p-log-handle (
          input 1
        , input log-file-name
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Удаление справочника ОСС '" + " &1")
...
then do:
  ...
  then do:
    run str/post-xml.p
      ...
      ,input 30
      ,input   ( if action = 'U'
                  then ('Ждите - ' + 'добавление/изменение справочника ОСС ')
                  else ('Ждите - ' + 'удаление справочника ОСС ') ) +
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute( ("'Пересылка справочника ОСС '" + " &1")
                            , replace( v-xml-file-name-path, "/", "\" ) + "xm1"
                      )
                                      ).
run write-log-and-file in p-log-handle (
