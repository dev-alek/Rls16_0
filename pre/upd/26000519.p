...
PROCEDURE ObjCodeList:
   ...
   for each buf_thbj-attr no-lock where
      ...
      if buf_thbj-attr.obj-type = 'БД':U or
...
if g#db-num <> 0 then do:
   run str/diallog.w (
      ...
      , input ( 'БД':U + chr(4) + string(g#db-num) + chr(4) + 'U':U + chr(4) + 'gismt':U + chr(4) + 'Передача настроек для проверки КМ':U)
      , input ?
      , input "":U
...
run ObjCodeList.
oOk = ThbjattrCr('MACC_Timeout':U,"0","decimal").
if oOk then
oOk = ThbjattrCr ('Resp_TH_required':U,"yes","logical").
if oOk then
oOk = ThbjattrCr('TH_IP':U,"","character").
if oOk then
oOk = ThbjattrCr('TH_Port':U,"1500","character").
if oOk then
oOk = ThbjattrCr('LmCHzPort':U,"5995","character").
if oOk then
oOk = ThbjattrCr('AddTimeoutPIoT':U,"1","character").
