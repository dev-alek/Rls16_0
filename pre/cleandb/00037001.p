...
:
    ...
    :
      ...
      for first next-icnt-doc where next-icnt-doc.obj-type   = icnt-doc.obj-type   and
                                    ...
                                    next-icnt-doc.status_    = 'факт':U                 and
      ...
      if varfind-next-line = no then do:
        ...
        find first buf_icnt-doc exclusive-lock where
           recid(buf_icnt-doc) = recid(icnt-doc) no-error no-wait.
if not avail buf_icnt-doc then
do:
  undo, return error "Ошибка удаления icnt-doc. Запись занята другим пользователем.".
end.
delete buf_icnt-doc.
vDeleted = vDeleted + 1.
        leave icnt-line.
...
vResult = substitute("Произведена чистка таблиц: &1~nУдалено записей - &2.", "Документы  инв. счетчиков ТРК", vDeleted).
return vResult.
procedure cleanTable:
  define buffer icnt-line for ub.icnt-line.
on delete of ub.icnt-line override do: end.
for each icnt-line exclusive-lock
     where icnt-line.doc-code = icnt-doc.doc-code
on error undo, return error
:
      delete icnt-line no-error.
