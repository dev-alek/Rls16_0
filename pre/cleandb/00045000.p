  ...
  if buf_clients.obj-type  = 'маг':U OR buf_clients.obj-type  = 'скл':U then DO:
     ...
     :
       ...
       find first buf_price-doc exclusive-lock where
           recid(buf_price-doc) = recid(price-doc) no-error no-wait.
if not avail buf_price-doc then
do:
  undo, return error "Ошибка удаления price-doc. Запись занята другим пользователем.".
end.
...
vResult = substitute("Произведена чистка таблиц: &1~nУдалено записей - &2.", "Переоценка с историей", vDeleted).
return vResult.
...
on delete of ub.price-list-attr override do: end.
for each price-list-attr exclusive-lock
     where price-list-attr.doc-num = price-doc.doc-num
on error undo, return error
:
      delete price-list-attr no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
...
:
  ...
  define buffer c-price-doc for ub.c-price-doc.
on delete of ub.c-price-doc override do: end.
for each c-price-doc exclusive-lock
     where c-price-doc.doc-num = price-doc.doc-num
on error undo, return error
:
      delete c-price-doc no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer c-price-list for ub.c-price-list.
on delete of ub.c-price-list override do: end.
for each c-price-list exclusive-lock
     where c-price-list.doc-num = price-doc.doc-num
on error undo, return error
:
      delete c-price-list no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer c-price-list-attr for ub.c-price-list-attr.
on delete of ub.c-price-list-attr override do: end.
for each c-price-list-attr exclusive-lock
     where c-price-list-attr.doc-num = price-doc.doc-num
on error undo, return error
:
      delete c-price-list-attr no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
...
procedure saveLastPriceList:
  ...
  then do:
    ...
    buffer-copy in-price-list to buf_price-list assign
      ...
      buf_price-list.calc-method = 'Не-считать':U
...
procedure copy-parts :
  ...
  :
    ...
    :
      ...
      if parts.out-code = 'free-zone':U then do:
        assign
