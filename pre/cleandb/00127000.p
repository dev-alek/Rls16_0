...
:
  ...
  :
    ...
    find first buf_order-doc exclusive-lock where
           recid(buf_order-doc) = recid(order-doc) no-error no-wait.
if not avail buf_order-doc then
do:
  undo, return error "Ошибка удаления order-doc. Запись занята другим пользователем.".
end.
delete buf_order-doc.
vDeleted = vDeleted + 1.
  end.
end.
vResult = substitute("Произведена чистка таблиц: &1~nУдалено записей - &2.", "Заказы с историей", vDeleted).
return vResult.
procedure cleanTable :
  define buffer order-doc-attr for ub.order-doc-attr.
on delete of ub.order-doc-attr override do: end.
for each order-doc-attr exclusive-lock
    where order-doc-attr.db-num   = order-doc.db-num
        and order-doc-attr.doc-code = order-doc.doc-code
on error undo, return error
:
      delete order-doc-attr no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer order-line for ub.order-line.
on delete of ub.order-line override do: end.
for each order-line exclusive-lock
    where order-line.db-num = order-doc.db-num
        and order-line.doc-code = order-doc.doc-code
on error undo, return error
:
      delete order-line no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer order-line-attr for ub.order-line-attr.
on delete of ub.order-line-attr override do: end.
for each order-line-attr exclusive-lock
    where order-line-attr.db-num   = order-doc.db-num
        and order-line-attr.doc-code = order-doc.doc-code
on error undo, return error
:
      delete order-line-attr no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer c-order-head for ub.c-order-head.
on delete of ub.c-order-head override do: end.
for each c-order-head exclusive-lock
    where c-order-head.db-num   = order-doc.db-num
        and c-order-head.doc-code = order-doc.doc-code
on error undo, return error
:
      delete c-order-head no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer c-order-doc for ub.c-order-doc.
on delete of ub.c-order-doc override do: end.
for each c-order-doc exclusive-lock
    where c-order-doc.db-num   = order-doc.db-num
        and c-order-doc.doc-code = order-doc.doc-code
on error undo, return error
:
      delete c-order-doc no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer c-order-doc-attr for ub.c-order-doc-attr.
on delete of ub.c-order-doc-attr override do: end.
for each c-order-doc-attr exclusive-lock
    where c-order-doc-attr.db-num   = order-doc.db-num
        and c-order-doc-attr.doc-code = order-doc.doc-code
on error undo, return error
:
      delete c-order-doc-attr no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer c-order-line for ub.c-order-line.
on delete of ub.c-order-line override do: end.
for each c-order-line exclusive-lock
    where c-order-line.db-num   = order-doc.db-num
        and c-order-line.doc-code = order-doc.doc-code
on error undo, return error
:
      delete c-order-line no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
  define buffer c-order-line-attr for ub.c-order-line-attr.
on delete of ub.c-order-line-attr override do: end.
for each c-order-line-attr exclusive-lock
    where c-order-line-attr.db-num   = order-doc.db-num
        and c-order-line-attr.doc-code = order-doc.doc-code
on error undo, return error
:
      delete c-order-line-attr no-error.
      if error-status:error then
        undo, return error error-status:get-message(1).
      vDeleted = vDeleted + 1.
end.
