...
$Workfile: mtreq006.p $
$Archive: gbl/mtreq006.p $

Мобильный терминал. Удаление строки чека
...
define variable vss-workfile    as character no-undo init "$Workfile: mtreq006.p $":U .
define variable vss-archive     as character no-undo init "$Archive: gbl/mtreq006.p $":U .
...
/*  find first buf_sys-ctrl no-lock .
  ...
  :
    ...
    then do:
      assign
        p-error-message = substitute( "Ошибка изменения строки чека &6.&1&2&1&3&1&4&1&5"
                                    , {&new-line}
                                    , return-value
