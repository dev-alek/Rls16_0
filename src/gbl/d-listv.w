...
DEFINE TEMP-TABLE temp-sel NO-UNDO LIKE ub.clients-attr
       ...
       field attr-format as character format "X(30)"
       field attr-type as character format "X(30)"
...
Форма выбора из неизвестного количества опций с заданием/изменением значений
...
/*список*/
DEFINE INPUT PARAMETER pattr-value as character no-undo.
/*список уже заданных значений*/
DEFINE INPUT PARAMETER pattr-format as character no-undo.
/*список заданных форматов*/
DEFINE INPUT PARAMETER pattr-type as character no-undo.
/*список заданных типов данных*/
...
/*перечень уже выбранных*/
DEFINE OUTPUT PARAMETER psel-values as character no-undo.
...
define variable vss-description as character no-undo init "Форма выбора из неизвестного количества опций  с заданием/изменением значений" .
{ cmp/vssrevis.i }
...
temp-sel.attr-label temp-sel.attr-value
...
&Scoped-Define ENABLED-OBJECTS b-quit B-mark B-sel B-value B-Help BR-list ~
mark-num
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS BR-list Dialog-Frame _STRUCTURED
  QUERY BR-list DISPLAY
      (IF ( CAN-DO (rid-list, string( recid( temp-sel ) ) ) ) THEN ("*") ELSE (" ")) COLUMN-LABEL "*" FORMAT "X(1)"
      temp-sel.attr-label COLUMN-LABEL "Выбор" FORMAT "X(60)"
      temp-sel.attr-value COLUMN-LABEL "Значение"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 87.75 BY 9.38.
...
DEFINE FRAME Dialog-Frame
     ...
     B-value AT ROW 1 COL 31
     B-Help AT ROW 1 COL 41
     BR-list AT ROW 2.83 COL 2
     mark-num AT ROW 1 COL 13.75 COLON-ALIGNED NO-LABEL
     SPACE(70.74) SKIP(10.28)
...
"(IF ( CAN-DO (rid-list, string( recid( temp-sel ) ) ) ) THEN (""*"") ELSE ("" ""))" "*" "X(1)" ? ? ? ? ? ? ? no ?
     _FldNameList[2]   > "_<CALC>"
"temp-sel.attr-label" "Выбор" "X(60)" ? ? ? ? ? ? ? no ?
     _FldNameList[3]   > Temp-Tables.temp-sel.attr-value
"temp-sel.attr-value" "Значение" ? "character" ? ? ? ? ? ? no ?
     _Query            is OPENED
*/  /* BROWSE BR-list */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME B-value
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-value Dialog-Frame
ON CHOOSE OF B-value IN FRAME Dialog-Frame /* Значение */
DO:
define variable vvalue as character no-undo.
define buffer buf_temp-sel for temp-sel.
        run gbl/d-prompt.w (
        'title=':u + temp-sel.attr-label + '\':u
      + 'format=' + (if temp-sel.attr-format = {&type-log} then "yes/no" else temp-sel.attr-format) + '\':u
      + 'type=' + temp-sel.attr-type + '\':u
      + 'fillin_row=2\':u
      + 'fillin_col=4\':u
      + 'fillin_width=20\':u
      + 'fillin_height=1\':u
...
find first buf_temp-sel where buf_temp-sel.line-num = temp-sel.line-num.
assign
buf_temp-sel.attr-value = vvalue
.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  run fill-table(pattr-codes, pattr-labels, pattr-value, pattr-format, pattr-type, pdelim) no-error.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE b-quit B-mark B-sel B-value B-Help BR-list mark-num
      WITH FRAME Dialog-Frame.
...
DEFINE OUTPUT PARAMETER loc-attr-values as chAracter no-undo.
...
DO II = 1 to num-entries(rid-list):
    ...
    loc-attr-values = loc-attr-values + (if loc-attr-values = "" then "" else loc-delim) +
                     loc-temp-sel.attr-value
                     .
end.
...
DEFINE INPUT PARAMETER loc-attr-value as character no-undo.
DEFINE INPUT PARAMETER loc-attr-format as character no-undo.
DEFINE INPUT PARAMETER loc-attr-type as character no-undo.
DEFINE INPUT PARAMETER loc-delim as character no-undo.
...
temp-sel.attr-value = entry(ii, loc-attr-value, loc-delim)
temp-sel.attr-format = entry(ii, loc-attr-format, loc-delim)
temp-sel.attr-type = entry(ii, loc-attr-type, loc-delim)
