...
$Workfile: mtreq004.p $
$Archive: gbl/mtreq004.p $

Мобильный терминал. Закрытие чека
...
define variable vss-workfile    as character no-undo init "$Workfile: mtreq004.p $":U .
define variable vss-archive     as character no-undo init "$Archive: gbl/mtreq004.p $":U .
define variable vss-description as character no-undo init "Мобильный терминал. Закрытие чека".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/library.i  }
{ gbl/integerm.i }
...
:
  ...
  :
    ...
    then do:
      assign
        p-error-message = substitute( "Ошибка закрытия чека &6.&1&2&1&3&1&4&1&5"
                                    , {&new-line}
                                    , return-value
