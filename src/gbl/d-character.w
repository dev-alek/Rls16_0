...
Универсальный диалог для ввода данных character
...
DEFINE FRAME Dialog-Frame
     ...
     FILL-IN-Character AT ROW 4 COL 2 NO-LABEL
     Btn-Choose AT ROW 4 COL 43.5
     FILL-IN-Text-1 AT ROW 2.13 COL 2.5 NO-LABEL
     FILL-IN-Text-2 AT ROW 3.03 COL 2.5 NO-LABEL
     SPACE(2.59) SKIP(3.62)
...
DO:
  ...
  THEN DO:
    ...
    THEN DO:
      ...
      then do:
        if h-callback :get-signature("cb-d-character-validate") <> ""
        then do:
          define variable v-ok as logical no-undo .
          define variable v-message as character no-undo .
          run cb-d-character-validate in h-callback
            (input  fill-in-character
            ,output v-ok
            ,output v-message
            ) no-error .
          ...
          then do:
            message
              ...
              "Ошибка при вызове процедуры проверки допустимости character" skip
              "Вызывающая программа" h-callback :file-name skip
              "Внутренняя процедура" "cb-d-character-validate" skip
          ...
          message
            ...
            "В вызывающей программе отсутствует внутренняя процедура" "cb-d-character-validate" skip
            "Вызывающая программа" h-callback :file-name skip
            view-as alert-box error .
          return no-apply .
        end.
