...
&Scoped-Define ENABLED-OBJECTS B-OK B-Cancel B-clear B-Help netto brutto
&Scoped-Define DISPLAYED-OBJECTS netto brutto
...
DEFINE VARIABLE brutto AS DECIMAL FORMAT "->,>>>,>>>,>>9.999":U INITIAL 0
     LABEL "Масса брутто груза, кг"
     VIEW-AS FILL-IN
     SIZE 20 BY 1 NO-UNDO.

DEFINE VARIABLE netto AS DECIMAL FORMAT "->,>>>,>>>,>>9.999":U INITIAL 0
     LABEL "Масса нетто груза, кг"
     VIEW-AS FILL-IN
     SIZE 20 BY 1 NO-UNDO.
...
DEFINE FRAME Dialog-Frame
     ...
     netto AT ROW 3.75 COL 31 COLON-ALIGNED WIDGET-ID 2
     brutto AT ROW 5 COL 31 COLON-ALIGNED WIDGET-ID 4
     SPACE(49.99) SKIP(2.99)
...
DO:
  ...
  assign
    ...
    p-full-name  = ""
  .
    display netto brutto with frame {&frame-name} .
END.
...
DO:
  ...
  .
    ...
    if netto = 0 and brutto = 0 then do:
        p-value      = "0,0" .
        p-full-name  = "" .
    end.
    else do:
      p-value      = string(netto) + "," + string(brutto) .
      p-full-name  = "Масса нетто:" + string(netto) + ",Масса брутто:" + string(brutto) .
    end.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  frame {&frame-name}:title = "Масса груза, кг" .

  RUN enable_UI.
  if v-mode <> {&update} then do:
     disable netto brutto b-clear  with frame {&frame-name}.
     B-ok:label = "Вы&ход"  .
...
------------------------------------------------------------------------------*/
  if p-value <> "" then do :
    assign
      netto  = decimal(entry (1, p-value ))
      brutto = decimal(entry (2, p-value ))
    .
  end.
  DISPLAY netto brutto
      WITH FRAME Dialog-Frame.
  ENABLE B-OK B-Cancel B-clear B-Help netto brutto
      WITH FRAME Dialog-Frame.
