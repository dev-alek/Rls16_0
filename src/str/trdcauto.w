...
&Scoped-Define ENABLED-OBJECTS B-OK B-Cancel B-clear B-Help model gos-num
&Scoped-Define DISPLAYED-OBJECTS model gos-num
...
DEFINE VARIABLE gos-num AS CHARACTER FORMAT "X(10)":U
     LABEL "Гос.номер"
     VIEW-AS FILL-IN
     SIZE 11 BY 1 NO-UNDO.

DEFINE VARIABLE model AS CHARACTER FORMAT "X(256)":U
     LABEL "Модель"
     VIEW-AS FILL-IN
...
DEFINE FRAME Dialog-Frame
     ...
     model AT ROW 3.75 COL 17 COLON-ALIGNED WIDGET-ID 2
     gos-num AT ROW 5 COL 17 COLON-ALIGNED WIDGET-ID 4
...
DO:
  ...
  assign
    ...
    p-full-name  = ""
  .
    display model gos-num with frame {&frame-name} .
END.
...
DO:
  ...
  .
    ...
    if model = "" and gos-num = "" then do:
        p-value      = "" .
        p-full-name  = "" .
    end.
    else do:
      p-value      = trim(model) + "," + trim(gos-num) .
      p-full-name  = "" .
    end.
END.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  frame {&frame-name}:title = "Автомобиль, Гос.номер" .

  RUN enable_UI.
  if v-mode <> {&update} then do:
     disable model gos-num  b-clear  with frame {&frame-name}.
     B-ok:label = "Вы&ход"  .
...
------------------------------------------------------------------------------*/
  if p-value <> "" then do :
    assign
      model = entry (1, p-value )
      gos-num = entry (2, p-value )
    .
  end.
  DISPLAY model gos-num
      WITH FRAME Dialog-Frame.
  ENABLE B-OK B-Cancel B-clear B-Help model gos-num
      WITH FRAME Dialog-Frame.
