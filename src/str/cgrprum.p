...
$Workfile: cgrprum.p $
$Archive: str/cgrprum.p $

Вызов процедур RUM для обработки групп клиентов
...
define variable vss-workfile    as character no-undo init "$Workfile: cgrprum.p $":u .
define variable vss-archive     as character no-undo init "$Archive: str/cgrprum.p $":u .
define variable vss-description as character no-undo init "Вызов процедур RUM для обработки групп клиентов" .
...
{ rul/calldscr.i }
{ ref/cgrplist.i cgrp-list def "NEW SHARED" }
{ ref/cgrplib.i }
...
-1 - p-save = no- проверка  пакетный режим  сохраняются изменения в temp-changes
...
1  - p-save = yes - расчет с сохранением пакетный режим  сохраняются изменения в temp-changes
*/
...
:
  ...
  CASE p-process:
    when {&cli-grp-proc_batchwork-export}
    or
    when {&cli-grp-proc_batchwork-routing}
    then do:
      ...
      if p-parent-handle:get-signature ( "cli-grps_rum-fill_cb") <> ? then do:
        run cli-grps_rum-fill_cb in p-parent-handle ( input (buffer cgrp-list:handle), input (buffer temp_cfound-result-nodelist:handle)).
      end.
    ...
    when {&cli-grp-proc_xml-file-import} then do:
      run cur-time in this-procedure ( output v-today, output v-time).
      assign
      v-obj-type = v-cntxt-obj-type
      v-obj-code = v-cntxt-obj-code
    ...
    when {&cli-grp-proc_xml-esys-import} then do:
      run cur-time in this-procedure ( output v-today, output v-time).
      assign
      v-obj-type = v-cntxt-obj-type
      v-obj-code = v-cntxt-obj-code
    ...
    otherwise do:
      undo _main, return error substitute("&1 &2 &3&4Ошибка входных параметров процедуры cgrprum.p&4Невернoе значение p-process = &5"
                                          ,vss-workfile
                                          ,vss-revision
                                          ,vss-description
                                          ,{&new-line}
  ...
  do v-jj = 1 to num-entries(v-codex-id-list):
    ...
    do v-ii = 1 to num-entries(v-ruleset-id-list[v-jj]):
      ...
      on stop   undo _main, return error substitute( "&1. stop", vss-workfile )
      on endkey undo _main, return error substitute( "&1. endkey", vss-workfile ) :
        if p-process = {&cli-grp-proc_batchwork-export}
        or p-process = {&cli-grp-proc_batchwork-routing}
        or p-process = {&cli-grp-proc_xml-file-import}
        then do:
        ...
        then do:
          undo _main, return error substitute("&1&2Ошибка при обработке групп клиентов&2" +
                                              "&3&2&4"
                                              ,vss-workfile
                                              ,{&new-line}
        ...
        if v-stop-leave-status > '' then do:
          undo _main, return error substitute("&1&2Процесс обработки групп клиентов прерван&2" +
                                              "&3&2&4"
                                              ,vss-workfile
                                              ,{&new-line}
                                              , error-status:get-message(1)
