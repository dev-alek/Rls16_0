...
Процедура отсылки данных по соответствию товаров/кошельков на кассы - цикл по всем кассам одного типа
...
FOR EACH for-cash-desk NO-LOCK WHERE
  ...
  &subject=gas-station
  &out-title="'Пересылка данных по соответствию товаров/кошельков '"
  &data-by=object
  &cdt-ibm=yes
  &cdt-ibm-xml=yes
  }
  ...
  RUN putc-petrol in this-procedure
               ( input for-cash-desk.pos-type
                ...
                ,input rid-list
                ,input action /*удалить весь справочник*/
                ).
