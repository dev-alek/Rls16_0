...
&Scoped-define FIELDS-IN-QUERY-br-trans mark-string(recid(X_cd-trans), v-rid-list) X_cd-trans.chk-date string(X_cd-trans.chk-time, "HH:MM") X_cd-trans.doc-code X_cd-trans.obj-type X_cd-trans.obj-code X_cd-trans.pay-desk X_cd-trans.out-code X_cd-trans.charkey_one X_cd-trans.charkey_two X_cd-trans.charkey_three X_cd-trans.key#_one X_cd-trans.key#_two X_cd-trans.key#_three X_cd-trans.key#_four X_cd-trans.key#_five X_cd-trans.deckey_one X_cd-trans.deckey_two X_cd-trans.deckey_three X_cd-trans.trans-type X_cd-trans.trans-id-chr
...
&Scoped-Define ENABLED-OBJECTS b-quit b-mark b-sel b-chk-doc b-lnk b-sch ~
B-Help br-trans sch-code sch-date mark-num
&Scoped-Define DISPLAYED-OBJECTS sch-code sch-date mark-num
...
&Scoped-define SELF-NAME b-lnk
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL b-lnk Dialog-Frame
ON CHOOSE OF b-lnk IN FRAME Dialog-Frame /* Связи */
DO:
...
CASE lnk-OPTION:
  WHEN {&cdt-achdata} THEN DO:
      run str/cd-trans.w (
                          input parparentproc
                         ,input "" /*bttns  */
                         ,input "trans-id-chr" /*p-list-mode*/
                         ,input integer({&cdt-achdata}) /*p-trans-type */
                         ,input p-obj-type
                         ,input p-obj-code
                         ,input 01/01/2008 /*p-start-date */
                         ,input {&end-of-age} /*p-end-date */
                         ,input "" /*p-charkey-one */
                         ,input X_cd-trans.trans-id-chr /*p-trans-id-chr*/
                         ,output varrid-list    ) no-error.

  end.
  when {&cdt-achexp} then do:
    run str/cd-trans.w (
                        input parparentproc
                       ,input "" /*bttns  */
                       ,input "trans-id-chr" /*p-list-mode*/
                       ,input integer({&cdt-achexp}) /*p-trans-type */
                       ,input p-obj-type
                       ,input p-obj-code
                       ,input 01/01/2008 /*p-start-date */
                       ,input {&end-of-age} /*p-end-date */
...
&Scoped-define SELF-NAME m_41
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_41 Dialog-Frame
ON CHOOSE OF MENU-ITEM m_41 /* Данные МБ о расходах */
DO:
  ASSIGN lnk-OPTION = {&cdt-achdata}.
  apply "CHOOSE" to b-lnk in frame {&frame-name}.
...
&Scoped-define SELF-NAME m_42
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL m_42 Dialog-Frame
ON CHOOSE OF MENU-ITEM m_42 /* Данные МБ по истории заправок */
DO:
  ASSIGN lnk-OPTION = {&cdt-achexp}.
  apply "CHOOSE" to b-lnk in frame {&frame-name}.
END.
...
ON CTRL-J OF sch-code IN FRAME Dialog-Frame /* коду транз. */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL sch-code Dialog-Frame
ON RETURN OF sch-code IN FRAME Dialog-Frame /* коду транз. */
DO:
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
 ...
 { gbl/getcntxt.i get }
  if lookup(p-list-mode, {&g___object} + {&comma-char} + "charkey_one" + {&comma-char} + "trans-id-chr") = 0 THEN DO:
...
menu-item m_41:sensitive in menu menu-b-lnk  = (p-trans-type = integer({&cdt-ach}))
menu-item m_42:sensitive in menu menu-b-lnk  = (p-trans-type = integer({&cdt-ach}))
.
DISPLAY
sch-code
sch-date
...
.
  CASE p-list-mode :
    ...
    WHEN "charkey_one":U THEN DO:
      ...
      .
      if p-open-query then do:
        FRAME {&frame-name}:TITLE = substitute("&1 &2 &3 &4", title0, v-tt-name, v-charkey-one-label, p-charkey-one) .
      end.
      { gbl/fltopend.i
        &where-cond = " ~
              X_cd-trans.trans-type  = p-trans-type     ~
          AND X_cd-trans.charkey_one  = p-charkey-one    ~
                      "
        &dyn_where-cond = " substitute(' X_cd-trans.trans-type  = &1 ~
                                     and X_cd-trans.charkey_one = &2&3&2',  p-trans-type, ~{&double-quote~}, p-charkey-one) "
    ...
    WHEN "trans-id-chr":U THEN DO:
      ...
      .
      if p-open-query then do:
        FRAME {&frame-name}:TITLE = substitute((title0 + " Связи для транзакции &1 &2")
                                                      , p-trans-id-chr
                                                      , v-tt-name
                                                      )
                                                      .
      ...
      { gbl/fltopend.i
        &where-cond = " ~
              X_cd-trans.trans-type  = p-trans-type     ~
          AND X_cd-trans.trans-id-chr  = p-trans-id-chr    ~
                      "
        &dyn_where-cond = " substitute(' X_cd-trans.trans-type  = &1 ~
                             and X_cd-trans.trans-id-chr = &2&3&2', p-trans-type, ~{&double-quote~}, p-trans-id-chr) "
...
with frame {&frame-name}.
assign
p-trans-id-chr = {&double-quote} + p-trans-id-chr + {&double-quote}.
