...
Настройка параметров произвольных заданий, работающих через машину правил
...
&Scoped-Define DISPLAYED-OBJECTS ED-profile E-rule-name
...
/* SETTINGS FOR BUTTON b-profile IN FRAME Dialog-Frame
   NO-ENABLE                                                            */
ASSIGN
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Параметры работы с заказами по расписанию */
DO:
...
ON CHOOSE OF b-profile IN FRAME Dialog-Frame /* Выбор профайла */
DO:
define variable v-list as character no-undo .
case v-rum-option:
  when {&ord} then do:
    v-list = ({&ord} + {&delim-par} + {&ord-proc_ord-batchwork} ).
  end.
  when {&table_goods} then do:
    v-list = ({&table_goods} + {&delim-par} + {&goods-proc_goods-batchwork} ).
  end.
end case.
    run utl/thbjrumr.w (
                        input parparentproc
                       ...
                       ,input ? /*p-log-handle*/
                       ,input v-list /*parameter - второй элемент списка - это radio-buttons rs-ruleset d thbjrumr*/
    ) no-error.
  IF ERROR-STATUS:ERROR THEN RETURN NO-APPLY.
END.
