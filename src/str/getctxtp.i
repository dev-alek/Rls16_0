...
Получение дополнительного контекста складов, магазинов и их фирмы
...
&if "{1}" = "def" &then
  ...
  define variable v-cntxp-conf-par        as   character                no-undo. /* для чтения параметра конфигурации */
  define variable v-cntxp-par-type        as   character                no-undo. /* тип параметра конфигурации */
...
&endif
&if "{1}" = "get" or "{1}" = "get-no-cntxt" &then
&if "{1}" = "get" &then
...
&endif
  if (v-cntxp-obj-type = {&shop} or v-cntxp-obj-type = {&stock}) and
     v-cntxp-obj-code <> 0 then do:

    { gbl/conf-rd.i "'is-prt'" 0 "''" 0 "''" "''" "''" yes v-cntxp-conf-par v-cntxp-par-type no-error }
