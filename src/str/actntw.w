...
Двухоконный интерфейс для системы прав
...
Input:
    ...
    p-runfilename        as character   - процедура для вызова по кнопке свойств (если "":U, то кнопка не видна)
                                          при выборе этой процедуре будут переданы:
                                            1. handle mainmenu.
                                            2. Код записи, определённый первым параметром процедуры actntw_add-item.
    p-runfilelabel       as character   - label для кнопки свойств
    table for temp_actntw_items         - заполненная таблица параметров для выбора (поле Selected - для правого окна)

Output:
    table for temp_actntw_itemsSelected        - таблица с кодами выбранных параметров
...
Пример:
    ...
    run actntw_add-item in this-procedure ( input "first-key",  input "Первая строка", input "Коммент1", input no ).
    run actntw_add-item in this-procedure ( input "second-key", input "Вторая строка", input "Коммент2", input no ).
    run actntw_add-item in this-procedure ( input "third-key",  input "Третья строка", input "Коммент3", input yes ).
    ...
    :
        assign
            v-str = substitute( "&1&2&3 (&4)"
                                ...
                                , temp_actntw_itemsSelected.itm-key
                                , temp_actntw_itemsSelected.itmExtKey
                                )
        .
    end.
    message
...
define input parameter p-head-code          as integer          no-undo. /* action-role куда это все и привязано */
define input parameter p-role-code          as integer          no-undo. /* action-role куда это все и привязано */
...
&Scoped-define INTERNAL-TABLES buf_left_temp_actntw_items ~
buf_right_temp_actntw_items

/* Definitions for BROWSE br-table-left                                 */
&Scoped-define FIELDS-IN-QUERY-br-table-left buf_left_temp_actntw_items.selLeft buf_left_temp_actntw_items.itmType buf_left_temp_actntw_items.itmName
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-table-left
&Scoped-define SELF-NAME br-table-left
&Scoped-define OPEN-QUERY-br-table-left /* OPEN QUERY {&SELF-NAME} FOR EACH buf_left_temp_actntw_items no-lock . */ run local-open-query-left in this-procedure .
&Scoped-define TABLES-IN-QUERY-br-table-left buf_left_temp_actntw_items
&Scoped-define FIRST-TABLE-IN-QUERY-br-table-left buf_left_temp_actntw_items


/* Definitions for BROWSE br-table-right                                */
&Scoped-define FIELDS-IN-QUERY-br-table-right buf_right_temp_actntw_items.selRight buf_right_temp_actntw_items.itmType buf_right_temp_actntw_items.itmName
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-table-right
&Scoped-define SELF-NAME br-table-right
&Scoped-define OPEN-QUERY-br-table-right /* OPEN QUERY {&SELF-NAME} FOR EACH buf_right_temp_actntw_items no-lock. */ run local-open-query-right in this-procedure .
&Scoped-define TABLES-IN-QUERY-br-table-right buf_right_temp_actntw_items
&Scoped-define FIRST-TABLE-IN-QUERY-br-table-right buf_right_temp_actntw_items
...
&Scoped-Define ENABLED-OBJECTS b-exit b-cancel cb-type bt-properties b-help ~
bt-filter tb-filter b-goods b-gds-grp br-table-right br-table-left ~
bt-select-type bt-deselect-type ed-desc-not-sel ed-desc-sel
&Scoped-Define DISPLAYED-OBJECTS cb-type fi-filter tb-filter ~
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br-table-right b-gds-grp Dialog-Frame */
...
/* Query rebuild information for BROWSE br-table-left
     _START_FREEFORM
/* OPEN QUERY {&SELF-NAME} FOR EACH buf_left_temp_actntw_items no-lock . */
...
/* Query rebuild information for BROWSE br-table-right
     _START_FREEFORM
/* OPEN QUERY {&SELF-NAME} FOR EACH buf_right_temp_actntw_items no-lock. */
...
&Scoped-define SELF-NAME b-gds-grp
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL b-gds-grp Dialog-Frame
ON CHOOSE OF b-gds-grp IN FRAME Dialog-Frame /* Группы */
DO:
...
ON VALUE-CHANGED OF cb-type IN FRAME Dialog-Frame /* Тема */
DO:
    assign
        cb-type
    .
...
:
    ...
    end.        /* for each buf_temp_actntw_itemsSelected */
    if p-changed = no
    then do:
        check-items-table:
    ...
    then do:
        ...
        :
            ...
            .
        end.        /* for each buf_temp_actntw_items */
    end.
end.
