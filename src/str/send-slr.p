...
$Workfile: send-slr.p $
$Archive: str/send-slr.p $

Пересылка продавцов на кассу
...
define variable vss-workfile    as character no-undo init "$Workfile: send-slr.p $":U .
define variable vss-archive     as character no-undo init "$Archive: str/send-slr.p $":U .
define variable vss-description as character no-undo init "Пересылка продавцов на кассу".
{ cmp/vssrevis.i }
...
run write-log-and-file in p-log-handle (
    ...
    , input log-file-name
    , input 1
    , input substitute("Пересылка на кассы &1&2 информации о продавцах", {&shop}, i-obj-code)
                                          ).
...
if error-status:error then do:
  run write-log-and-file in p-log-handle (
      ...
      , input log-file-name
      , input 1
      , input substitute( "!!!Ошибки при отсылке информации по продавцам на кассы  маг&1"
                         , i-obj-code
                        )
                                        ).
