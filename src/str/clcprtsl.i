...
{ str/lib-trn.i }
/*Временная таблица обсчетов по строке*/
...
field sum-type           as   character                /*тип сумм {&sum-general[-sign]}   - основна
                                                                  ...
                                                                  {&sum-old-consignation[-sign]} - старая консигнаци
                                                                  ...
                                                                  {&sum-office[-sign]}    - услуги */
field fact-qnty          like ub.doc-line.fact-qnty   /*фактическое количество*/
field cli-qnty           like ub.doc-line.cli-qnty    /*количество поставщика*/
...
&if "{2}" = "doc" &then
field doc-qnty               like ub.doc-line.doc-qnty   /*количество по документу */
field sum-dsc-base-doc-acc   like ub.doc-line.price-base /* учетные суммы по документарному количеству */
...
field sum-dsc-base-doc-cur   like ub.doc-line.price-base   /*текущие цены*/
field sum-dsc-rubl-doc-cur   like ub.doc-line.price-base
field sum-dsc-base-doc-doc   like ub.doc-line.price-base  /*цены документа*/
field sum-dsc-rubl-doc-doc   like ub.doc-line.price-base
&endif
field sum-dsc-base-cur   like ub.doc-line.price-base   /*текущие цены*/
...
.
/*Временная таблица обсчетов по партии*/
...
field sum-type           as   character                /*тип сумм {&sum-general[-sign]}   - основна
                                                                  ...
                                                                  {&sum-resp_stor[-sign]} - ответственное хранение
                                                                  {&sum-office[-sign]}    - услуги */
field fact-qnty             as decimal /*фактическое количество*/
field cli-qnty              as decimal /*кол-во поставщика*/
field sum-dsc-base-doc      as decimal /*цены документа*/
...
field sum-dsc-base-acc      as decimal /*учетные цена*/
...
field sum-dsc-base-doc-doc      as decimal /*цены документа*/
field sum-dsc-rubl-doc-doc      as decimal
&endif
field sum-dsc-base-cur      as decimal /*текущие цены*/
...
/*Расчет продажных сумм и налогов по партии из temporary table (сделано для универсальности)
  Если у Вас реальная партия скопируйте ее просто в temporary table*/
...
{ str/in-vatp.i  def "''" "''" "''" "cl"  }
{ str/out-vatp.i def "''" "''" "''" "cl"  }
...
{ str/in-vatp.i  calc-parts cl_tt-clcparts. par "''" "cl" }
/*ЕСЛИ НУЖНЫ ЦЕНЫ ДОКУМЕНТА ОНИ СЕЙЧАС ВСЕГДА ВЫЧИСЛЯЮТСЯ НА ОСНОВЕ gds-dtl ПО ДОКУМЕНТУ ПАРТИИ!!!*/
if paris-doc then do:
  ...
  { str/out-vatp.i doc-line par par "''" "cl" }
end.
if paris-cur then do:
  ...
  { str/out-vatp.i calc-parts parcur parcur parcur "cur" " " " " " " cl_tt-clcparts.}
end.
create bf_tt-allsum.
assign
  bf_tt-allsum.sum-type = {&sum-general}.
...
assign
  ...
  .
if cl_tt-clcparts.purch-code = integer({&consignation-code}) then do:
  assign
...
assign
  ...
  bf_tt-allsum.sum-dsc-cli-acc      = (if (price-cli-with-tax-loccl +
                                           road-tax-cli-loccl       +
                                           (if cl_tt-clcparts.vat-type <> {&inc-vat} then vat-cli-loccl else 0) +
                                           (if cl_tt-clcparts.slt-type <> {&inc-slt} then slt-cli-loccl else 0)
...
assign
  bfs_tt-allsum.sum-type = {&sum-general-sign}.
if pardoc-type = {&inventory} or
   pardoc-type = {&income}    or
   pardoc-type = {&return}    then do:
...
when {&bef-repayment-code}           then do:
  assign
    bfpc_tt-allsum.sum-type  = {&sum-repayment}
    bfspc_tt-allsum.sum-type = {&sum-repayment-sign}.
...
when {&bef-old-consignation-code}    then do:
  assign
    bfpc_tt-allsum.sum-type  = {&sum-old-cons}
    bfspc_tt-allsum.sum-type = {&sum-old-cons-sign}.
  buffer-copy bf_tt-allsum  except bf_tt-allsum.sum-type  to bfpc_tt-allsum.
  buffer-copy bfs_tt-allsum except bfs_tt-allsum.sum-type to bfspc_tt-allsum.
end.
when {&bef-responsible-storage-code} then do:
  assign
    bfpc_tt-allsum.sum-type  = {&sum-resp_stor}
    bfspc_tt-allsum.sum-type = {&sum-resp_stor-sign}.
  buffer-copy bf_tt-allsum  except bf_tt-allsum.sum-type  to bfpc_tt-allsum.
  buffer-copy bfs_tt-allsum except bfs_tt-allsum.sum-type to bfspc_tt-allsum.
end.
when {&bef-consignation-code} then do:
  assign
    bfpc_tt-allsum.sum-type  = {&sum-cons_benf}
    bfspc_tt-allsum.sum-type = {&sum-cons_benf-sign}.
  assign
...
&endif
  ...
  assign
    bfacc_tt-allsum.sum-type = {&sum-cons_acc}.
  create bfsacc_tt-allsum.
  assign
    bfsacc_tt-allsum.sum-type = {&sum-cons_acc-sign}.
  assign
...
do on error undo, return error return-value :
  ...
  if bf_goods.gds-type = {&gds-office} or
    ...
    assign
     bf_tt-allsum-line.sum-type = {&sum-general}.
...
&endif
    ...
    assign
    bfs_tt-allsum-line.sum-type = {&sum-general-sign}.
    if bf_trn-doc.doc-type = {&inventory} or
       bf_trn-doc.doc-type = {&income}    or
       bf_trn-doc.doc-type = {&return}    then do:
...
{ gbl/pftxvalg.i
    ...
    v-calcbypart = no.
    if bf_doc-line.whole-send-news = integer({&price-parts})   then
    v-calcbypart = yes.
    else do:
  ...
  /*   if bf_doc-line.whole-send-news = integer({&price-parts})  then do:  /* среднепродажная хрень */
              assign v-varsum = 0.
              for each buf_parts no-lock
              where buf_parts.out-code  = bf_doc-line.doc-code
                and buf_parts.artic     = bf_doc-line.artic
                and buf_parts.prod-type = bf_doc-line.prod-type
                and buf_parts.prod-code = bf_doc-line.prod-code
              :
                { gbl/partbcod.i
                  buf_parts
                  v-b-pcode
                ...
                assign v-varsum = v-varsum + varprice-salef * buf_parts.fact-qnty.
              end.
              if bf_trn-doc.doc-type = {&inventory} then  varprice-salef = if bf_gds-dtl.doc-qnty <> 0 then v-varsum / bf_gds-dtl.doc-qnty else v-varsum.
              else assign varprice-salef = if bf_gds-dtl.fact-qnty <> 0 then v-varsum / bf_gds-dtl.fact-qnty else 0.

              /*assign varprice-salef = v-varsum / bf_gds-dtl.fact-qnty .*/

/*          { gbl/avprpart.i bf_trn-doc.obj-type bf_trn-doc.obj-code varb-code 0 bf_trn-doc.fact-order vardoc-num varprice-salef varroad-tax varexcise }*/
          if varprice-salef = ? then do:
            { gbl/bcodeprc.i bf_trn-doc.obj-type bf_trn-doc.obj-code varb-code 0 bf_trn-doc.fact-order vardoc-num varprice-salef varroad-tax varexcise }
          end.
          varprice-sale       = varprice-salef .
          if varprice-sale = ?
          then do:
            assign
              varprice-sale = 0
          ...
          assign
            varprice-sale       = varprice-salef
            varlastcur-base     = varprice-salef
            varlastcur-road-tax = varroad-tax
            varlastcur-excise   = varexcise
            varcur-base         = varcur-base      + varprice-salef * (if bf_trn-doc.doc-type = {&inventory} and  bf_gds-dtl.doc-qnty = 0 then 1 else bf_gds-dtl.fact-qnty)
    ...
    /*Формируем временную таблицу и вызываем соответствующую процедуру*/
...
/*Расчет по временной таблице из партий*/
procedure clcprtsl_calc-ttable :
/*Нужны ли докуменарные(продажные) компоненты партии*/
define input parameter paris-doc           as   logical                 no-undo.
/*Нужны ли текущие продажные компоненты партии*/
define input parameter paris-cur           as   logical                 no-undo.
/*Налоги по документу из строки для подсчета сумм по документу.
  Если суммы по документу не нужны можете заполнить нулями*/
define input parameter parroad-tax         like ub.doc-line.road-tax    no-undo.
define input parameter parexcise           like ub.doc-line.excise      no-undo.
define input parameter parvat-pc           like ub.doc-line.vat-pc      no-undo.
define input parameter parcons-vat-pc      like ub.doc-line.cons-vat-pc no-undo.
