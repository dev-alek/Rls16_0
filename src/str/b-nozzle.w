...
def var vss-description as character no-undo init "Smart browser общения с пистолетами ТРК".
{ cmp/vssrevis.i }
{ cmp/trg-def.i  }
{ cmp/showinf.i }
{ gbl/getcntxt.i def }
...
&Scoped-define INTERNAL-TABLES ub.nozzle

/* Define KEY-PHRASE in case it is used by any query. */
&Scoped-define KEY-PHRASE TRUE

/* Definitions for BROWSE br_table                                      */
&Scoped-define FIELDS-IN-QUERY-br_table ub.nozzle.nozzle-code
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table
&Scoped-define SELF-NAME br_table
&Scoped-define QUERY-STRING-br_table FOR EACH ub.nozzle WHERE ub.nozzle.obj-type = varobj-type and                                               ub.nozzle.obj-code = varobj-code NO-LOCK     ~{&SORTBY-PHRASE} INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br_table OPEN QUERY {&SELF-NAME} FOR EACH ub.nozzle WHERE ub.nozzle.obj-type = varobj-type and                                               ub.nozzle.obj-code = varobj-code NO-LOCK     ~{&SORTBY-PHRASE} INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br_table ub.nozzle
&Scoped-define FIRST-TABLE-IN-QUERY-br_table ub.nozzle
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 17.88 BY 7.29.
...
/* Query rebuild information for BROWSE br_table
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH ub.nozzle WHERE ub.nozzle.obj-type = varobj-type and
                                              ub.nozzle.obj-code = varobj-code NO-LOCK
...
if available ub.nozzle then do:
   ...
   message "Вы хотите удалить запись <<пистолет ТРК>> с номером " ub.nozzle.nozzle-code " ?" skip
           "ПРИ ЭТОМ БУДУТ УДАЛЕНЫ ВСЕ СВЯЗИ ПИСТОЛЕТА С ДАННЫМ НОМЕРОМ НА ВСЕХ РЕЗЕРВУАРАХ И ТРК!!!" skip
           "Вы уверены?"
           view-as alert-box question buttons yes-no update varmes-log.
  if varmes-log = yes then do:
     run nozzledv (input ub.nozzle.obj-type,
                   input ub.nozzle.obj-code,
                   input ub.nozzle.nozzle-code) no-error.
     if error-status:error then do:
        { str/errmes.i "Ошибка при удалении пистолета ТРК."}
        return no-apply.
     end.
...
DO:
  ...
  if available ub.nozzle then do:
      run ref/cnzlhist.w (
                       INPUT parParentProc
                     , input ub.nozzle.obj-type
                     , input ub.nozzle.obj-code
                     , input "":U /*bttns  */
                     , "one":U /*p-mode*/
                     , input ub.nozzle.obj-type
                     , input ub.nozzle.obj-code
                     , input ub.nozzle.nozzle-code
                     , input 0 /*p-pump-code*/
                     , input 0 /*p-pl-code*/
                     , input '':U /*p-subject*/
...
DO:
  { str/ptrlv.i "lps" "nozzle" "пистолету ТРК" "{&frame-name}"}
END.
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "ub.nozzle"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE state-recid-nozzle B-table-Win
PROCEDURE state-recid-nozzle :
/*------------------------------------------------------------------------------
  Purpose:
  Parameters:  <none>
  Notes:
...
if available ub.nozzle then do:
     assign varPS = ub.nozzle.pS.
     display varPS with frame {&frame-name}.
     run set-attribute-list ("recid-nozzle=" + string(recid(ub.nozzle))).
     run notify ('recid-nozzle,recid-nozzle-target':u).
  end.
END PROCEDURE.
