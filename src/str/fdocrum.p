...
$Workfile: fdocrum.p $
$Archive: str/fdocrum.p $

Вызов процедур RUM для обработки операций с финансовыми документами

Автор: Бахтадзе Наталья Викторовна
Дата создания: 03/23/10
...
define variable vss-workfile    as character no-undo init "$Workfile: fdocrum.p $":u .
define variable vss-archive     as character no-undo init "$Archive: str/fdocrum.p $":u .
define variable vss-description as character no-undo init "Вызов процедур RUM для обработки операций с финансовыми документооборота" .
...
define variable log-file-name as character no-undo init "process-fdoc-list.txt".
...
&scop display-message    if log-file-name <> '':U then run write-log-and-file in p-log-handle (  ~
  ...
  , input ~{&my-message~})
if transaction
and (p-process =  "какой-то экспорт в ВС"
    )
then do:
  message
...
:
  ...
  CASE p-process:
    ...
    then do:
      ...
      v-doc-date = v-today
      v-fact-date = v-today
      v-doc-code = p-doc-code /*uniq-key-rec - смены */
  ...
  do v-jj = 1 to num-entries(v-codex-id-list):
    ...
    do v-ii = 1 to num-entries(v-ruleset-id-list[v-jj]):
      ...
      on stop   undo _main, return error substitute( "&1. stop", vss-workfile )
      on endkey undo _main, return error substitute( "&1. endkey", vss-workfile ) :
        if p-process = "какой-то экспорт в вс"
        or p-process = "какой-то импорт из вс"
        then do:
          if buf_rule-by-call.profile_id <> v-profile-id then next _rule-by-call.
        end.
        if  (p-process =  "какой-то импорт из ВС"
          )
        then do:
        ...
        then do:
          undo _main, return error substitute("&1&2Ошибка при операциях с фин документами&2" +
                                              "&3&2&4"
                                              ,vss-workfile
                                              ,{&new-line}
        ...
        if v-stop-leave-status > '' then do:
          undo _main, return error substitute("&1&2Процесс работы с фин документми прерван&2" +
                                              "&3&2&4"
                                              ,vss-workfile
                                              ,{&new-line}
                                              , error-status:get-message(1)
