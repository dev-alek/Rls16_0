...
$Workfile: ggrprum.p $
$Archive: str/ggrprum.p $

Вызов процедур RUM для обработки групп товаров
...
define variable vss-workfile    as character no-undo init "$Workfile: ggrprum.p $":u .
define variable vss-archive     as character no-undo init "$Archive: str/ggrprum.p $":u .
define variable vss-description as character no-undo init "Вызов процедур RUM для обработки групп товаров" .
...
{ rul/calldscr.i }
{ ref/ggrplist.i ggrp-list def "NEW SHARED" }
{ ref/grplib.i }
...
-1 - p-save = no- проверка  пакетный режим расчета  сохраняются изменения в temp-changes
-2 - одиночный режим расечта  - сохраняются изменения в temp-table.new-tbl-handle
и потом могут быть показаны в интерфейсе view-chg.w
1  - p-save = yes - расчет с сохранением пакетный режим расчета - сохраняются изменения в temp-changes
*/
...
:
  ...
  CASE p-process:
    when {&gds-grp-proc_batchwork-export}
    or
    when {&gds-grp-proc_batchwork-routing}
    then do:
      ...
      if p-parent-handle:get-signature ( "gds-grp_rum-fill_cb") <> ? then do:
        run gds-grp_rum-fill_cb in p-parent-handle ( input (buffer ggrp-list:handle), input (buffer temp_found-result-nodelist:handle)).
      end.
      /*
      output to kk.txt.
      for each ggrp-list:
    ...
    when {&gds-grp-proc_xml-file-import} then do:
     run cur-time in this-procedure ( output v-today, output v-time).
      assign
      v-obj-type = v-cntxt-obj-type
      v-obj-code = v-cntxt-obj-code
    ...
    when {&gds-grp-proc_xml-esys-import} then do:
      run cur-time in this-procedure ( output v-today, output v-time).
      assign
      v-obj-type = v-cntxt-obj-type
      v-obj-code = v-cntxt-obj-code
    ...
    otherwise do:
      undo _main, return error substitute("&1 &2 &3&4Ошибка входных параметров процедуры gds-grprum.p&4Невернoе значение p-process = &5"
                                          ,vss-workfile
                                          ,vss-revision
                                          ,vss-description
                                          ,{&new-line}
  ...
  do v-jj = 1 to num-entries(v-codex-id-list):
    ...
    do v-ii = 1 to num-entries(v-ruleset-id-list[v-jj]):
      ...
      on stop   undo _main, return error substitute( "&1. stop", vss-workfile )
      on endkey undo _main, return error substitute( "&1. endkey", vss-workfile ) :
        if p-process = {&gds-grp-proc_batchwork-export}
        or p-process = {&gds-grp-proc_batchwork-routing}
        or p-process = {&gds-grp-proc_xml-file-import}
        then do:
          if buf_rule-by-call.profile_id <> v-profile-id then next _rule-by-call.
        end.
        if p-process = {&gds-grp-proc_xml-esys-import}
        ...
        then do:
          undo _main, return error substitute("&1&2Ошибка при обработке групп товаров&2" +
                                              "&3&2&4"
                                              ,vss-workfile
                                              ,{&new-line}
        ...
        if v-stop-leave-status > '' then do:
          undo _main, return error substitute("&1&2Процесс обработки групп товаров прерван&2" +
                                              "&3&2&4"
                                              ,vss-workfile
                                              ,{&new-line}
                                              , error-status:get-message(1)
