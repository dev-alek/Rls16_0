...
Триггер на запись для таблицы ИСТОРИЯ ПРАВИЛ СКИДОК
...
TRIGGER PROCEDURE FOR WRITE OF ub.c-dis-rule OLD old_c-dis-rule.
...
define variable vss-description as character no-undo init "Триггер на запись для таблицы история правил скидок".
...
:
  ...
  OR (g#news
      and not ( g#db-num > 0 )
      and ub.c-dis-rule.corr-user-name <> {&nts-user}
      and ub.c-dis-rule.obj-code = 0
  ...
  run str/callnews.p
    (input "c-dis-rule"
    ,input (buffer ub.c-dis-rule:handle)
    ).
    if g#oxml = yes
    then do:
    run str/calloxml.p (
