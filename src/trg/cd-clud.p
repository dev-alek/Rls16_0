...
Триггер на удаление клиента на кассе
...
TRIGGER PROCEDURE FOR DELETE OF ub.cd-clu  .
...
define variable vss-description as character no-undo init "Триггер на удаление клиента на кассе".
...
:
  ...
  /*if not g#news then do:
    ...
    if v-db-num <> g#db-num then do:
      message
      "Нельзя удалять запись об клиенте на кассе," skip
  ...
  if not g#news then do:
    ...
    create buf_c-cd-clu.
    buffer-copy ub.cd-clu to buf_c-cd-clu
    assign
