...
TRIGGER PROCEDURE FOR WRITE OF ub.c-wealth.
...
define variable vss-description as character no-undo init "Триггер на запись истории МЦ".
...
:
  ...
  OR (g#news
      and  g#db-num = 0
      and ub.c-wealth.corr-user-name <> {&nts-user}
  ...
  run str/callnews.p
    (input "c-wealth"
    ,input (buffer ub.c-wealth:handle)
    ).
