...
Триггер на запись для таблицы ИСТОРИЯ АТРИБУТОВ ТИПОВ дисконтных карт
...
TRIGGER PROCEDURE FOR WRITE OF ub.c-dis-card-type-attr OLD old-c-dis-card-type-attr .
define variable vss-revision    as character no-undo init "$Revision$":U .
define variable vss-author      as character no-undo init "$Author$":U .
define variable vss-date        as character no-undo init "$Date$":U .
define variable vss-workfile    as character no-undo init "$Workfile$":U .
define variable vss-archive     as character no-undo init "$Archive$":U .
define variable vss-description as character no-undo init "Триггер на запись для таблицы история атрибутов типов дисконтных карт".
{ cmp/vssrevis.i "substitute('&1|&2|&3|&4|&5|&6|&7'
                        ...
                        , ub.c-dis-card-type-attr.obj-type + string( ub.c-dis-card-type-attr.obj-code)
                        , ub.c-dis-card-type-attr.attr-code
...
:
    ...
    OR (g#news
        and not ( g#db-num > 0 )
        and ub.c-dis-card-type-attr.corr-user-name <> {&nts-user}
    ...
    then do:
        ...
        run str/callnews.p
            (input "c-dis-card-type-attr"
            ,input (buffer ub.c-dis-card-type-attr:handle)
            ).
    end.
    if g#oxml = yes
    then do:
