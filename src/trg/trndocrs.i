...
end procedure. /* trndocrs-pl-gds-clear */
...
end procedure. /* temp-pl-gds-clear */
...
end procedure. /* temp-pl-gds-accum */
...
end procedure. /* trndocrs-gds-dtl-accum */
...
end procedure. /* trndocrs-pl-gds-request */
...
end procedure. /* trndocrs-pl-gds-calc-rsrv */
...
end procedure. /* trndocrs-need-rsrv */
...
procedure trndocrs-need-create-doc-pl :
  ...
  :
    if  not p-news
    and p-extended-doc-type <> {&TDEDT_Pri_Vnesh}
    and p-extended-doc-type <> {&TDEDT_Ras_Vnesh_Kass}
    and p-extended-doc-type <> {&TDEDT_Vozvrat_Vnesh_Kass}
    and not p-sale-auto
    then do:
      assign
...
end procedure. /* trndocrs-need-create-doc-pl */
...
end procedure. /* trndocrs-validate */
...
procedure trndocrs :
  ...
  /* todo - проверить точность p-chg-qnty */
  ...
  :
    ...
    if buf_gds-obj.place-rsrv = true then do:
      ...
      :
        ...
        if not available buf_pl-gds then do:
          /* todo что нам надо делать здесь */
          message
            vss-workfile vss-revision vss-description skip
            vss-include-info{&vssseq} skip
    ...
    :
      ...
      { gbl/termnode.i
        buf_temp-trndocrs-gds-dtl-rsrv.prt-code
        v-node-code
        no-error
      }
      if error-status :error then do:
