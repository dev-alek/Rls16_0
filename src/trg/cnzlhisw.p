...
Триггер на запись шапки истории пистолета
...
TRIGGER PROCEDURE FOR WRITE OF ub.c-nzl-hist.
...
define variable vss-description as character no-undo init "Триггер на запись шапки истории пистолета".
...
:
  ...
  run str/callnews.p
    (input "c-nzl-hist"
    ,input (buffer ub.c-nzl-hist:handle)
    ).
    if g#oxml = yes
    then do:
    run str/calloxml.p (
