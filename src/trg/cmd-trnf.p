...
Отправка в новости шапки trn-doc
...
define input  parameter p-comand-type as character no-undo . /* fo - отметки от ФО покупателей   */
                                                             /* factur - отметки от счетов-фактур  */
...
:
  ...
  then do:
    message
      vss-workfile vss-revision vss-description skip
      substitute( "Ошибка при создании команды &1", {&cmd-trn-doc-fact} ) skip
      error-status :get-message(1) skip
      return-value skip
      view-as alert-box error .
    delete procedure v-cmd-proc-handle .
  ...
  for each buf_trn-doc exclusive-lock where buf_trn-doc.doc-code  = p-doc-code
  on error undo, return error return-value
  :
    run add-dump in v-cmd-proc-handle
      (input v-cmd-code                   /* p-command-code */
      ,input {&table_trn-doc}             /* p-dump-name    */
      ,input '+update':U
      ,input (buffer buf_trn-doc :handle) /* p-tbl-handle   */
      ,input '':U
      ,output v-rec-ord
      ) no-error .
    if error-status :error
    then do:
      message
        vss-workfile vss-revision vss-description skip
        substitute( "Ошибка при добавлении записи &1 в команду с кодом &2", {&table_trn-doc}, v-cmd-code ) skip
        error-status :get-message(1) skip
        return-value skip
        view-as alert-box error .
      delete procedure v-cmd-proc-handle .
