...
Триггер на удаление связки ПРОФАЙЛ-ПРОФАЙЛ
...
TRIGGER PROCEDURE FOR DELETE OF ub.profile-by-profile.
...
define variable vss-description as character no-undo init "Триггер на удаление связки ПРОФАЙЛ-ПРОФАЙЛ".
...
:
  if not g#news then do:
    ...
    repeat while program-name( level ) <> ? :
      ...
      if index(v-p, "profile-by-profile3.") > 0
    ...
    if not v-confirmed then do:
      message
        ...
        "Физическое удаление связки ПРОФАЙЛ-ПРОФАЙЛ в системе запрещено" skip
