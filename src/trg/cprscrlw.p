...
Триггер на запись истории связки СКРИПТА ДЛЯ ОБЪЕКТА-НАБОР ПРАВИЛ
...
TRIGGER PROCEDURE FOR WRITE OF ub.c-pscript-ruleset.
define variable vss-revision    as character no-undo init "$Revision$":U .
define variable vss-author      as character no-undo init "$Author$":U .
define variable vss-date        as character no-undo init "$Date$":U .
define variable vss-workfile    as character no-undo init "$Workfile$":U .
define variable vss-archive     as character no-undo init "$Archive$":U .
define variable vss-description as character no-undo init "Триггер на запись истории связки СКРИПТА ДЛЯ ОБЪЕКТА-НАБОР ПРАВИЛ".
...
:
  ...
  then do:
    run str/calloxml.p (
        ...
        , input {&table_c-pscript-ruleset}
        , input ( buffer ub.c-pscript-ruleset:handle )
    ) no-error.
