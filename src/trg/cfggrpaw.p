...
Триггер на запись истории атрибутов групп блюд
...
TRIGGER PROCEDURE FOR WRITE OF ub.c-fbr-gds-grp-attr.
...
define variable vss-description as character no-undo init "Триггер на запись истории атрибутов групп блюд".
...
:
  ...
  then do:
    run str/callnews.p
      (input "c-fbr-gds-grp-attr"
      ,input (buffer ub.c-fbr-gds-grp-attr:handle)
      ).
  end.
    if g#oxml = yes
    then do:
