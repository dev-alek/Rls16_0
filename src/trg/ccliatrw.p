...
Триггер на запись истории АТРИБУТА КЛИЕНТА
...
TRIGGER PROCEDURE FOR WRITE OF ub.c-clients-attr.
...
define variable vss-description as character no-undo init "Триггер на запись истории АТРИБУТА КЛИЕНТА".
...
:
  ...
  OR (g#news
      and g#db-num = 0
      and ub.c-clients-attr.corr-user-name <> {&nts-user}
  ...
  then do:
      run str/callnews.p
                        ...
                        ,input (buffer ub.c-clients-attr:handle)
                        ).
  end.
end.