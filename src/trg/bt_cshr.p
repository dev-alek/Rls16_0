...
Обработка изменений клиента-кассира
...
define variable vss-description as character no-undo init "Обработка изменений клиента-кассира".
{ cmp/vssrevis.i }
{ cmp/trg-def.i  }
{ gbl/cur-time.i }
...
define temp-table temp-csh-list no-undo
...
if error-status:error or p-message-on = ? then return error.
  ...
  run gbl/lock-prc.p
    ...
    ,input ""
    ,input ""
    ,input ",,,,,,Обработка изменений клиента-кассира"
    ,input p-message-on
  ...
  then do:
    run write-log-and-file in p-log-handle (
        ...
        , input log-file-name
        , input 1
        , input substitute("В данный момент происходит Обработка изменений клиента-кассира" )
                                              ).
  ...
  run create-csh-list in this-procedure .

  run process-csh-list in this-procedure .

  run close-csh-list in this-procedure .

end.


procedure create-csh-list :

  define buffer buf_batchprocess for ub.batchprocess .
  define buffer buf_temp-csh-list for temp-csh-list .

  do
  on error undo, return error return-value
  :
    /* выбираем все задания по данному объекту для отсылки на кассу сущности кассиры */
    ...
    on error undo, return error return-value
    :
      create buf_temp-csh-list .
      assign
        buf_temp-csh-list.action-num = buf_BatchProcess.BatchProcess#
        buf_temp-csh-list.cashier    = integer(buf_BatchProcess.charkey_one)
        buf_temp-csh-list.psn-code   = buf_BatchProcess.key#_one
        buf_temp-csh-list.host-code  = buf_BatchProcess.key#_two
        buf_temp-csh-list.obj-type   = buf_BatchProcess.charkey_two
        buf_temp-csh-list.obj-code   = buf_BatchProcess.key#_three
        buf_temp-csh-list.action     = buf_BatchProcess.bp_execsystime
        buf_temp-csh-list.bp_rowid   = rowid(buf_BatchProcess)
      .
    end.
  end.
...
procedure process-csh-list :
  ...
  :
   ...
   { gbl/curdbnum.i v-db-num }
    for each buf_temp-csh-list
    on error undo, return error return-value
    :
      find first buf_person no-lock where
                 buf_person.psn-code = buf_temp-csh-list.psn-code no-error .
      if avail buf_person then do:
        ...
        find first cash-cash where
                   cash-cash.cash-code = buf_temp-csh-list.cashier no-error .
        if not avail cash-cash then do:
          create cash-cash.
          assign
          cash-cash.cash-code = buf_temp-csh-list.cashier
          cash-cash.psn-code = buf_temp-csh-list.psn-code
          .
        end.
        ...
        cash-cash.stts      = (if buf_temp-csh-list.action = "D":U
                               or v-cashier-code = 0
                               then 1
                               else 0)
        cash-cash.psswd     = v-c-password
        cash-cash.s-psswd   = v-s-password
        cash-cash.slr-code  = v-seller-code
    ...
    and can-find(first buf_cash-desk where
      ...
      FOR EACH cli_shops no-lock where
        ...
        run str/send-csh.p (  input parparentproc
    ...
    :
      ...
      :
        ...
        { trg/btpr_upd.i
          ...
          &btpr-rowid="buf_temp-csh-list.bp_rowid"
        }
      end.
    end.
  end.
