...
define variable vss-description as character no-undo init "Расчет складского архива по типам приобретени ".
...
:
  ...
  run gbl/lock-prc.p
    ...
    ,input  p-message-on               /* p-message-on      */
  ...
  then do:
    ...
    undo, return error "Складской архив по типам приобретения рассчитывается" . /* --->>>--- */
  end.
  ...
  /* признак того, что на объекте имеется рассчитанный архив */
  ...
  then do:
    ...
    then do:
      ...
      message
        ...
        "Расчет складского архива по типам приобретения невозможен" skip
        view-as alert-box error .
    end.
    undo, return error "Складской архив по типам приобретения: отсутствуют остатки" .
  end.
  ...
  then do:
    ...
    then do:
      ...
      undo, return error "Не задана дата перерасчета складского архива по типам приобретения" .
    end.
    else do:
      assign
    ...
    run day-begin-fact-order in this-procedure
      ...
      ,output v-aht-detail-fact-order /* p-day-begin-fact-order */
      ).

    run doclslib-check-aht-exist in this-procedure
      (input  p-obj-type              /* p-obj-type       */
      ,input  p-obj-code              /* p-obj-code       */
      ,input  v-aht-detail-fact-order /* p-cut-fact-order */
    ...
    then do:
      /* на выбранном объекте уже есть архив */

      /* определяем документы, для которых есть архив */
      run doclslib-check-doc-aht-exist in this-procedure
        .
      ...
      /* если требуется перерасчет архива */
      ...
      then do:
        ...
        or (v-last-fact-date <> ?
            and
            v-last-fact-date > v-aht-recalc-date
           )
        then do:
          assign
            v-last-fact-date        = v-aht-recalc-date
            v-last-fact-date-reason = "Складской архив по типам приобретения требует перерасчета из-за удаления или закрытия документа задним числом"
          .
        end.
      end.
      ...
      and v-last-fact-date  < v-aht-detail-date
      then do:
        message
          vss-workfile vss-revision vss-description skip
          "Складской архив по типам приобретения" skip
          ...
          "не может быть меньше чем дата начала подробного складского архива по типам приобретения" skip
      ...
      then do:
        ...
        if  p-last-fact-date = ?  then do:
        run gbl/d-askw.w
          (input "Вопрос"
          ,input "На объекте уже есть рассчитанный складской архив по типам приобретения" + {&new-line}
          ...
          ,input "Пересчитать архив по типам приобретения начиная с даты " +
                  (if v-last-fact-date <> ? then string(v-last-fact-date, '99/99/9999':u)
                                                  + {&new-line} + v-last-fact-date-reason
                    else "?"
                  ) + "|"
              + "Выбрать дату и пересчитать складской архив по типам приобретения начиная с выбранной даты" + "|"
              + "Удалить весь складской архив по типам приобретения и рассчитать их на основе документов" + {&new-line}
              + (if v-aht-detail-date <> ?
                then "Будут рассмотрены документы с даты " + string(v-aht-detail-date, '99/99/9999')
                else ""
                )
              + "|"
              + "Отказаться от расчета складского архива по типам приобретения"
          ,input 1
          ,input 4
          ,output v-num
          ).
        ...
        case v-num :
          when 1
          then do:
            /* пересчитать скдадской архив по типам приобретения начиная с v-last-fact-date */
            run cb-doclslib-log in this-procedure
          ...
          then do:
            ...
            :
              ...
              if  v-aht-recalc-date <> ?
              and v-last-fact-date   > v-aht-recalc-date
              then do:
                message
                  "Складской архив по типам приобретения" skip
                  "Объект" p-obj-type p-obj-code skip
                  "На объекте требуется перерасчет складского архива по типам приобретения с указанной даты" skip
                  "Введенная дата не может быть больше даты" v-aht-recalc-date skip
              ...
              then do:
                message
                  "Складской архив по типам приобретения" skip
                  "Объект" p-obj-type p-obj-code skip
                  "Введенная дата не может быть меньше даты" v-aht-detail-date skip
                  "Вы ввели дату" v-last-fact-date skip
                  view-as alert-box information .
                next update-block .
              end.
          ...
          when 3
          then do:
            /* удалить и пересчитать весь архив */
            assign
              v-last-fact-date = v-aht-detail-date
            .
    ...
    then do:
      ...
      then do:
        ...
        message
          "Складской архив по типам приобретения" skip
          "Объект" p-obj-type p-obj-code skip
          "Частичный расчет складского архива по типам приобретения" skip
          "Складской архив по типам приобретения за дату" string(v-last-fact-date, '99/99/9999':u) "и за более поздние даты будет удален" skip
  ...
  run show-action in this-procedure
    (input "Очистка складского архива по типам приобретения"
    ) .

  /* очистка складского архива по типам приобретения, начиная с выбранной даты */
  run cb-doclslib-log in this-procedure
  ...
  run doclslib-export-doc-list in this-procedure
    (input p-obj-type  /* p-obj-type        */
    ,input p-obj-code  /* p-obj-code        */
    ,input substitute('calcaht_&1_&2_doc_list.txt', p-obj-type, p-obj-code)
    ,input "Расчет складского архива по типам приобретения"
    ) .
  ...
  /* расчет архива по типам приобретения */
  ...
  then do:
    message
      vss-workfile vss-revision vss-description skip
      "Архив по типам приобретения" skip
      "Объект" p-obj-type p-obj-code skip
  ...
  run show-action in this-procedure
    (input "Расчет складского архива по типам приобретения закончен"
    ) .
...
procedure cb-doclslib-log :
  ...
  assign
    v-log-file-name = substitute("calcaht_&1_&2.txt", p-obj-type, p-obj-code)
  .
...
procedure log-information :
  ...
  on error undo, return error return-value
  :
    output stream slog to objaht.log append .
    export stream slog
      p-obj-type
      p-obj-code
      cur-time-string()
