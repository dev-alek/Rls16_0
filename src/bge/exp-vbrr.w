...
define variable vss-description as character no-undo init "Выгрузка товарного классификатора. ВББР".
/* Includes */
...
code_TNP code_system v-place v-directory v-ftp-address v-login v-password 
&Scoped-Define DISPLAYED-OBJECTS code_pool code_TNP code_system v-place ~
...
DEFINE VARIABLE code_system AS integer
...
DEFINE VARIABLE code_TNP AS CHARACTER FORMAT "X(256)":U 
     LABEL "Код соответствия для ТНП" 
     VIEW-AS FILL-IN 
     SIZE 30.5 BY 1 NO-UNDO.
...
DEFINE VARIABLE v-password AS CHARACTER FORMAT "X(256)":U 
     ...
     DEFINE VARIABLE v-long-code AS integer 
...
DEFINE FRAME Dialog-Frame
     ...
     code_pool AT ROW 4.75 COL 29 COLON-ALIGNED WIDGET-ID 2
     code_system AT ROW 6 COL 29 COLON-ALIGNED WIDGET-ID 6
     code_TNP AT ROW 7.25 COL 29 COLON-ALIGNED WIDGET-ID 4
     v-long-code AT ROW 8.5 COL 29 COLON-ALIGNED WIDGET-ID 18
     v-place AT ROW 10 COL 19 NO-LABEL WIDGET-ID 8
     v-directory AT ROW 13.5 COL 29 COLON-ALIGNED WIDGET-ID 12
     v-ftp-address AT ROW 14.75 COL 29 COLON-ALIGNED WIDGET-ID 14
     v-login AT ROW 16 COL 29 COLON-ALIGNED WIDGET-ID 16
     v-password AT ROW 17.25 COL 29 COLON-ALIGNED BLANK
     b-system AT ROW 6 COL 60 COLON-ALIGNED WIDGET-ID 6
          "Путь выгрузки:" VIEW-AS TEXT
          SIZE 14 BY 1.25 AT ROW 10.1 COL 4 WIDGET-ID 20
     SPACE(10.24) SKIP(1.87)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Выгрузка товарного классификатора. ВБРР"
...
/*    FILL-IN-esys:screen-value = entry(2,v-rid-list,{&delim-key}) no-error.*/
    
  code_system:screen-value =  entry(2,v-rid-list,{&delim-key}) no-error.
    code_system = integer(entry(2,v-rid-list,{&delim-key})) no-error.
  end.  
    /* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DO:
  ...
  run proc-start.
    ...
    if v-place = 1  then 
        ...
        run bge\exp-VBRR-p.p ( this-procedure:handle,
            v-directory,
            v-place ,
            "",
            "", 
    ...
    if v-place = 2  then    run bge\exp-VBRR-p.p ( this-procedure:handle,
            v-ftp-address,
            v-place,
            v-login,
            v-password,
...
if v-place = 1 then   v-param-list =  v-directory + {&delim-par} + string(v-place) + {&delim-par}  +  v-login  + {&delim-par} + v-password  + {&delim-par} + string(code_TNP) + {&delim-par} + string(code_pool) + {&delim-par} + string(code_system) + {&delim-par} + string(v-long-code).
if v-place = 2 then do:
     v-ftp-address = trim(trim(replace(v-ftp-address,'ftp:',""),{&slash-char}),{&back-slash-char}).
      v-param-list =  v-ftp-address + {&delim-par} + string(v-place) + {&delim-par} + 
                     v-login       + {&delim-par} +
                     v-password    + {&delim-par} +
                     string(code_TNP) + {&delim-par} +  string(code_pool) +  {&delim-par} + string(code_system) +  {&delim-par} + string(v-long-code).
                     end.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
 ...
 if error-status:error or ptrl = "" then  return error .
...
------------------------------------------------------------------------------*/
  DISPLAY code_pool code_TNP code_system v-place v-directory v-ftp-address b-system
          v-login v-password v-long-code
      WITH FRAME Dialog-Frame.
  ENABLE btn_start btn_save Btn_close code_pool code_TNP code_system v-place b-system v-long-code
         v-directory v-ftp-address v-login v-password 
      WITH FRAME Dialog-Frame.
                ...
                do:
                    ...
                    do:
                        ...
                        assign
                            ...
                            code_TNP    = entry(5,v-param-list,{&delim-par})
                            code_pool   = entry(6,v-param-list,{&delim-par})
                            code_system = integer(entry(7,v-param-list,{&delim-par}))
                            v-long-code = integer(entry(8,v-param-list,{&delim-par}))
   NO-ERROR.

                        display v-directory v-place  code_tnp code_pool code_system v-long-code with frame {&FRAME-NAME}.
...
DEFINE INPUT PARAMETER p-param-list AS CHARACTER NO-UNDO.
  ...
  FIND FIRST buf_schedule-attr NO-LOCK WHERE
    ...
    and buf_schedule-attr.attr-code = ({&attr-schd-free-id} + {&delim-par} + 'exp-VBBR') NO-ERROR.
