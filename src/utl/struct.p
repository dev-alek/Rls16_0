...
$Workfile: struct.p $
$Archive: utl/struct.p $

Закачка состава сырья товаров
...
артикул;состав сырья (без точек с запятой)
после последней строки Enter

пример :
арт-1;Пух гагачий, клей собачий


Автор : Андрей Исаков 25.11.99 */
...
define variable vss-workfile    as character no-undo init "$Workfile: struct.p $":U .
define variable vss-archive     as character no-undo init "$Archive: utl/struct.p $":U .
...
v-parameter = v-parameter +
 ...
 .
  ...
  run str/diallog.w ( parparentproc
              ...
              , no /*p-auto-go*/
              , "&Стоп"
              , 'Пакетное изменение состава сырья товара') .