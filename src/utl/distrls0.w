...
Список шаблонов правил расписаний
...
define variable vss-description as character no-undo init "Список шаблонов правил расписаний".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/tblfname.i }
...
&Scoped-define INTERNAL-TABLES X_dis-time-rule

/* Definitions for BROWSE br-dis-time-rule                              */
&Scoped-define FIELDS-IN-QUERY-br-dis-time-rule X_dis-time-rule.templ-rl-root X_dis-time-rule.des
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-dis-time-rule
&Scoped-define SELF-NAME br-dis-time-rule
&Scoped-define QUERY-STRING-br-dis-time-rule FOR EACH X_dis-time-rule NO-LOCK WHERE X_dis-time-rule.time-rule-num < {&max-num-dr-template}
&Scoped-define OPEN-QUERY-br-dis-time-rule OPEN QUERY {&SELF-NAME} FOR EACH X_dis-time-rule NO-LOCK WHERE X_dis-time-rule.time-rule-num < {&max-num-dr-template}.
...
b-lkp B-Help br-dis-time-rule mark-num
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-dis-time-rule Dialog-Frame _FREEFORM
  QUERY br-dis-time-rule NO-LOCK DISPLAY
      X_dis-time-rule.templ-rl-root FORMAT ">>>>>>>>9":U COLUMN-LABEL "Код!шаблона"
X_dis-time-rule.des FORMAT "X(255)":U WIDTH 85
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME Dialog-Frame
     ...
     br-dis-time-rule AT ROW 3 COL 1
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br-dis-time-rule B-Help Dialog-Frame */
ASSIGN
...
/* Query rebuild information for BROWSE br-dis-time-rule
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH X_dis-time-rule NO-LOCK WHERE X_dis-time-rule.time-rule-num < {&max-num-dr-template}.
     _END_FREEFORM
...
define variable v-rec as recid no-undo .
  run utl/distrui0.w ( input parparentproc
                      ,INPUT {&add-def}
  ...
  reposition br-dis-time-rule to recid(v-rec) no-error .
...
DO:
 ...
 { gbl/markstrn.i X_dis-time-rule v-rid-list }
  glog = br-dis-time-rule:refresh() .

  if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
      glog = br-dis-time-rule:select-next-row ().
...
DO:
  if available X_dis-time-rule then do:
    ...
    then  v-rid-list = string( recid( X_dis-time-rule ) ) .
  end.
END.
...
ch = br-dis-time-rule:FIRST-COLUMN IN FRAME {&FRAME-NAME}.
DO ii = 1 TO br-dis-time-rule:NUM-COLUMNS IN FRAME {&FRAME-NAME}:
    ASSIGN
    ch:RESIZABLE = YES.
    ch = ch:NEXT-COLUMN.
END.
...
FOR EACH X_dis-time-rule NO-LOCK WHERE X_dis-time-rule.time-rule-num <= {&max-num-dr-template} INDEXED-REPOSITION.
END PROCEDURE.
