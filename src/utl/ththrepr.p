...
Дата создания: 04/18/09
Author: Bakhtadze Natalya
Creation date: 04/18/09
...
for each tt_obj-list  :
  ...
  find first new_ext-classif no-lock where
          ...
          and new_ext-classif.uniq-key-rec    = v-key-rec use-index irec no-error .
...
run gbl/d-askw.w (input "Показывать данные:"
                        ,input  ""
                        ,input "|"
                        ,input "Все|Проблемы|Итоги|Отменить"
                        ,input "Все|Несхождения и слияния|Итоги|Отменить"
                        ,input 2
                        ,input 4
                        ,output choice).
if choice = 4 then return.
v-parameter = string(choice).
...
run str/diallog.w ( input parparentproc
          ...
          , input 'cmp/ththrep.p':U
          , input v-parameter + {&delim-par} + p-from-version
          , input no /*p-auto-go*/
          , input ''
          , input substitute('Отчет по утилите переноса из &1 БД в 16.0', p-from-version)) no-error .
