...
Список сущностей правил скидок
...
&Scoped-Define ENABLED-OBJECTS b-quit B-sel b-export B-Help rs-language ~
br-vrmtables
...
/* Query rebuild information for BROWSE br-vrmtables
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH buf_file NO-LOCK WHERE lookup(buf_file._file-name, v-dr-tables) > 0.
     _END_FREEFORM
     _Query            is OPENED
*/  /* BROWSE br-vrmtables */
&ANALYZE-RESUME
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  assign
  v-dr-tables = v-dr-tables + {&comma-char} + {&table_dis-rule} +
                                {&comma-char} + {&table_dis-cfg-rule} +
                                {&comma-char} + {&table_dis-time-rule}
                                .
  assign
  v-dr-table-names = v-dr-table-names + {&comma-char} + {&table_dis-rule-full} +
...
CASE buf_file._file-name:
  ...
  WHEN {&TABLE_dis-time-rule} THEN DO:
    run utl/distrls0.w ( INPUT parparentproc
                         ...
                         ,INPUT-OUTPUT v-rid-list) NO-ERROR.
  END.
  WHEN {&TABLE_dis-cfg-rule} THEN DO:
