...
$Workfile: ef-read.p $
$Archive: utl/ef-read.p $

Считывание всех данных с мобильного блока EasyFuel
...
define variable vss-workfile    as character no-undo init "$Workfile: ef-read.p $":U .
define variable vss-archive     as character no-undo init "$Archive: utl/ef-read.p $":U .
define variable vss-description as character no-undo init "Считывание всех данных с мобильного блока EasyFuel".
{ cmp/vssrevis.i }
{ cmp/trg-def.i }
{ gbl/getcntxt.i def }
{ gbl/getcntxt.i get }
...
if not v-rid-list  = "" then do:
  ...
  if v-setted then do:
    ...
    /*сначала все считаем*/
    run set-cmd-file in this-procedure (
                                        input "ReadID,ReadBrand,ReadLimit"
                                       ,input v-nid
                                       ,input buf_Dis-card.d-card
                                       ,input v-out-file).
    ...
    run write-log in p-log-handle ( input 0
                                   , substitute("Обращаюсь к программе связи с МБ...&1" +
                                                ...
                                                "Считывание марки транспортного средства...&1" +
...
&scop fnamelist1 "car-reg-number,car-brand,ef-format,issue-code,issued-by,issue-date,valid-from,valid-date,petrol-code-1,petrol-code-2,petrol-code-3,petrol-code-4"
&scop fnamelist2 "ef-petrol-code,petrol-num,common-limit,month-limit,day-limit,standard-dose,petrol-code"
...
define buffer buf2_temp-ef for vidtemp-ef1.
  ...
  :
    ...
    v-fnamelist = {&fnamelist1}.
    do v-jj = 1 to num-entries(v-fnamelist):
      v-fname = entry(v-jj, v-fnamelist).
      v-th:add-like-field( v-fname, buffer buf_temp-ef:handle:buffer-field(v-fname)).
    end.
