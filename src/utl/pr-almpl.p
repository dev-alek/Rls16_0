...
Обрабатывает все закрытые переоценки
...
define variable vss-workfile    as character no-undo init "$Workfile: pr-almpl.p $":U .
define variable vss-archive     as character no-undo init "$Archive: utl/pr-almpl.p $":U .
...
for each ub.price-doc no-lock  where
        ub.price-doc.obj-type = p-type and
        ub.price-doc.obj-code = p-code and
        ub.price-doc.status_ = {&act-overvalue} :

        v-pdf = next-value ( s-pdf , {&db-name_schema} ) .
    run waitfram-show  in this-procedure ( SUBSTITUTE ("Переоценка &1 ДНЦ &2 (ОБЪЕКТ &3 &4) " , ub.price-doc.doc-num ,  v-pdf ,p-type ,p-code  )) .
   ...
   assign
      ...
      ub.price-doc-forming.stts              = integer({&pdf-fact})
      ...
      ub.price-doc-forming.name              = "15v "  + p-type + string (p-code) + " № " + ub.price-doc.doc-num
      ub.price-doc-forming.des               = "ДНЦ при переходе на 15v "  + p-type + string (p-code) + ub.price-doc.doc-num
      ub.price-doc-forming.have-start-period = int( false )
      ub.price-doc-forming.start-sys-date    = ?
      ub.price-doc-forming.start-shift-num   = ?
      ub.price-doc-forming.start-date        = ?
      ...
      ub.price-doc-forming.out-code          = ub.price-doc.out-code
      ub.price-doc-forming.start-date        = ub.price-doc.fact-date
      v-status_    = ub.price-doc.status_
      v-fact-order = ub.price-doc.fact-order
      .
      find first buf_price-doc  share-lock /* exclusive-lock*/  where recid (buf_price-doc) = recid(ub.price-doc) no-error .
      if available  buf_price-doc then
        assign
          buf_price-doc.pdf-id      = v-pdf
          buf_price-doc.pdf-db      = v-cntxt-db-num
          buf_price-doc.plt-id      = v-plt-id
          buf_price-doc.plt-db-num  = v-cntxt-db-num
          .
...
/* Создание строк для ДНЦ */
  for each  ub.price-list no-lock where
         ...
         find first ub.bar-code no-lock where ub.bar-code.b-code = ub.price-list.b-code no-error .
         if error-status :error then next.
         find first buf_gds-prt  no-lock where buf_gds-prt.node-code = ub.bar-code.node-code no-error .
         if error-status :error then next.
         find first buf_goods no-lock where buf_goods.gds-code = ub.bar-code.gds-code no-error .
         if error-status :error then next.
 ...
 /* Создание строки ДНЦ */
      ...
      assign
          ub.price-doc-forming-gds.line-num        = ub.price-list.line-num
      ...
      .
         ...
         assign
            ...
            ub.price-all.status_     =  v-status_
