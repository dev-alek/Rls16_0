...
$Workfile: efregnum.p $
$Archive: utl/efregnum.p $

Изменение данных транспортного средства мобильного блока EasyFuel
...
define variable vss-workfile    as character no-undo init "$Workfile: efregnum.p $":U .
define variable vss-archive     as character no-undo init "$Archive: utl/efregnum.p $":U .
define variable vss-description as character no-undo init "Изменение данных транспортного средства мобильного блока EasyFuel".
{ cmp/vssrevis.i }
{ cmp/trg-def.i }
{ gbl/getcntxt.i def }
{ gbl/getcntxt.i get }
  ...
  if v-setted then do:
    ...
    run write-log in p-log-handle ( input 0
                                   , substitute("Обращаюсь к программе связи с МБ...&1" +
                                                ...
                                                "Считывание марки транспортного средства..." , {&new-line})).
    ...
    run ref/view-chg.w (
                        ...
                        ,input "Разница в данных о транспортном средстве"
    ...
    run write-log in p-log-handle ( input 0,  "Обращаюсь к программе связи с МБ...Запись данных по транспортному средству..." ).
    run gbl/syn4.p (
                     input v-cmd-line
                    ,input v-in-file
                    ,input ""
