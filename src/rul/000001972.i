...
Содержательная часть правила 1972
...
for each buf_temp-rule-call-param where
  ...
  find first buf_temp-discnt-role where
        ...
        and buf_temp-discnt-role.subject-type = integer({&discnt-sub-total})
        no-error.
        ...
        when {&table_dis-gds-rule} then do:
          case buf_temp-discnt-role.discnt-role:
            when {&dgr-without-disc} then do:
              v-line-type = integer({&discnt-gds-without-discnt}).
            end.
            otherwise do:
              v-line-type = integer({&discnt-sub-total}).
            end.
          end.
          create query v-qh.
          ...
          case buf_temp-discnt-role.link-prop:
            when integer({&dr-no-rule}) then do:
              ...
              v-found = (v-bh[{&chk-context}]:buffer-field("src-d-card"):buffer-value > ""
                        and
                        v-bh[{&chk-context}]:buffer-field("cash-d-pcnt"):buffer-value <> 0
                          )
