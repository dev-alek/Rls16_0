...
Задание и просмотр параметров вызова правил для профайла 62
...
define variable vss-description as character no-undo init "Задание и просмотр параметров вызова правил для профайла 62".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i }
...
FUNCTION display-clients returns character ( input p-obj-type as character
                                            ,input p-obj-code as integer
                                            ,input p-obj-name as character):
define variable v-string as character no-undo .
v-string = substitute("&1&2 &3",  p-obj-type, string(p-obj-code, ">>>>9"), p-obj-name).
return v-string.
end function.
...
&Scoped-Define ENABLED-OBJECTS B-exit b-quit B-Help button-obj BR-rcp
&Scoped-Define DISPLAYED-OBJECTS e-clients
...
DEFINE VARIABLE e-clients AS CHARACTER
...
DEFINE FRAME Dialog-Frame
     ...
     button-obj AT ROW 3.13 COL 3 WIDGET-ID 52
     e-clients AT ROW 3.13 COL 7.5 NO-LABEL WIDGET-ID 54
     BR-rcp AT ROW 16.97 COL 1 WIDGET-ID 100
     "Объекты, в которых проставляются флаги ЗАКОНЧИВШИХСЯ партий" VIEW-AS TEXT
          SIZE 68 BY 1 AT ROW 2 COL 2.5 WIDGET-ID 72
          FGCOLOR 4
     SPACE(27.50) SKIP(20.11)
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB BR-rcp e-clients Dialog-Frame */
ASSIGN
...
/* SETTINGS FOR EDITOR e-clients IN FRAME Dialog-Frame
   NO-ENABLE                                                            */
ASSIGN
       e-clients:READ-ONLY IN FRAME Dialog-Frame        = TRUE.
...
DO:
  run select-objects-proc in this-procedure no-error.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE B-exit b-quit B-Help button-obj BR-rcp
      WITH FRAME Dialog-Frame.
...
define variable v-object-type-code as character no-undo .
...
do while v-index-id >= 0 :
  RUN rcps_get-value IN THIS-PROCEDURE (
                                  ...
                                  ,output v-object-type-code /*p-value-character*/
  ...
  if error-status:error then v-index-id = -1.
    ...
    assign
    v-obj-type = substring(v-object-type-code, 1, 3)
    v-obj-code = integer(substring(v-object-type-code, 4))
    .
    find first obj-list where
              obj-list.obj-type = v-obj-type
          and obj-list.obj-code = v-obj-code  no-error.
      if available obj-list then  next.
    { cmp/cr-objls.i  v-obj-type v-obj-code }
    find first obj-list where
          obj-list.obj-code = v-obj-code
        and obj-list.obj-type = v-obj-type no-error.
    if available obj-list then do:
      glog = e-clients:INSERT-STRING ( display-clients(v-obj-type
                                                  , v-obj-code
                                                  ,obj-list.obj-name )) in frame {&frame-name} .
      glog = e-clients:INSERT-STRING ( {&new-line} ) in frame {&frame-name} .
    end.
  end.
...
VIEW FRAME {&frame-name}.
assign
e-clients:read-only in frame {&frame-name} = yes.
...
END.
ASSIGN
FRAME {&FRAME-NAME}:TITLE = substitute("Параметры простановки флагов ЗАКОНЧИВШИХСЯ партий по товару, продающемуся по партиям (профайл &1)", p-profile-id).
APPLY "ENTRY" to e-clients.
APPLY "CTRL-HOME" to e-clients.
...
if not can-find(first obj-list) then do:
  message
  "Вы не выбрали ни одного объекта!" skip
  "Вы уверены, что хотите сохранить такие параметры?"
...
RUN rcps_set-value IN THIS-PROCEDURE (
                                ...
                                ,INPUT obj-list.obj-type + string(obj-list.obj-code) /*p-value-character*/
                                ,INPUT ? /*p-value-date*/
...
ASSIGN
e-clients:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ''.
for each obj-list :
    ...
    if available buf_clients then do:
      v-rid-list = v-rid-list + (if v-rid-list = '' then '' else {&comma-char}) + string(recID(BUF_clients)).
    end.
  delete obj-list.
end.
run ref/thobjs.w
...
if v-rid-list <> '' then do:
  do v-ii = 1 to num-entries(v-rid-list):
    find first buf_clients no-lock where
              recid(buf_clients) = integer(entry(v-ii, v-rid-list)) no-error.
    if available buf_clients then do:
      { cmp/cr-objls.i  buf_clients.obj-type buf_clients.obj-code  }
       find first obj-list where
            obj-list.obj-code = buf_clients.obj-code
         and obj-list.obj-type = buf_clients.obj-type.
      glog = e-clients:INSERT-STRING ( display-clients(
