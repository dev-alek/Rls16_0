...
/*------------------------------------------------------------------------
    ...
    Description : Экспорт icnt-doc 1C ERP RN
...
procedure load-ruleset-context :
  ...
  do
    ...
    case p-ruleset-id:
      when {&edoc-proc_18_event_icnt-doc_145} then 
        do:
          if v-has-newbh
            and v-newbh:table <> {&table_icnt-doc} then 
          do:
            undo, return error substitute("Передан неверный буфер вместо буфера для &1",  {&table_icnt-doc}).
          end.
          if v-has-oldbh
            and v-oldbh:table <> {&table_icnt-doc} then 
          do:
            undo, return error substitute("Передан неверный буфер вместо буфера для &1",  {&table_icnt-doc}).
          end.
        end.
    ...
    if num-entries(v-direction, {&delim-par}) > 1 then 
    do:
