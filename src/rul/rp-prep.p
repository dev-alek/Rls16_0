...
define variable vss-description as character no-undo init "Сборка одного профайла".
...
:
   ...
   if not available buf_Rule-profile then do:
     undo main-block, return error substitute("Не найден профайл правил &1", p-profile-id).
   end.
   /*
  ...
  run gbl/filename.p (
                  input substitute( "rul/rp-&1.i", entry(1, buf_rule-profile.profile-type, "_"))
                ,output v-full-path
                ,output v-path
                ,output v-file-name
  ...
  for each buf_rule-by-profile where
          buf_rule-by-profile.profile_id = p-profile-id,
     first buf_rule no-lock where
          buf_rule.rule_id = buf_rule-by-profile.rule_id
  break
  by buf_rule-by-profile.rule_id:
    if first-of( buf_rule-by-profile.rule_id) then do:
      if buf_rule.upper_rule_id <> 0 then do:
        undo main-block, return error substitute("Правило &1 не является корневым", buf_rule-by-profile.rule_id).
      end.
      v-level = 0.
  ...
  repeat :
    ...
    */
    if not v-include-flag then do:
      run temp-string_write in this-procedure ( input  ss).
    end.
    if index(ss, "&profile_id&") > 0 then do:
      run temp-string_write in this-procedure ( input  substitute("v-profile-id = &1.", p-profile-id)).
    end.
    if index(ss, "&rp-short-name&") > 0 then do:
      run temp-string_write in this-procedure ( input  substitute('v-rp-short-name = "&1".', buf_rule-profile.short-name)).
    end.

    if index(ss, "&start-using-class&") > 0 then do:
      run using-class in this-procedure (  buffer buf_rule-profile ).
    end.
    if index(ss, "&start-hn-option&") > 0 then do:
      run cycle-script-type in this-procedure ( input p-profile-id, "hist-nws") no-error .
      if error-status:error then do:
    ...
    if index(ss, "&start-i-script&") > 0 then do:
      run hist-news-class in this-procedure (  buffer buf_rule-profile ).
      ...
      do v-ii = 1 to num-entries(v-script-type-list):
        run cycle-script-type in this-procedure ( input p-profile-id, input entry(v-ii, v-script-type-list)) no-error .
        if error-status:error then do:
          undo main-block, return error substitute( "&1. &2&3&4", vss-workfile, return-value, {&new-line}, error-status :get-message (1)).
        end.
      end.
    ...
    */
    if index(ss, "&start-rule-block&") > 0 then do:
    ...
    if index(ss, "&start-codex_id") > 0 then do:
      ...
      for each buf_ruleset no-lock:
        ...
        if v-key-ruleset-2 = v-key-ruleset then do:
          assign
          v-codex-id = buf_ruleset.codex_id
          v-ruleset-id = buf_ruleset.ruleset_id
          .
          leave _ruleset.
        end.
      end. /*for each buf_ruleset no-lock:*/
      /*выгрузим во временный файл*/
      run gbl/_tmpfile.p (
                          input ""
                        , input "rp"
                        , output v-rp-file-name) .
      output stream outstream2 to value(v-rp-file-name).
      v-include-flag = yes.
       /* следующие start должны быть для каждого правила профайла поэтому их сначала перенесем в отдельный кусок */
    end.
    if index(ss, "&end-codex_id") > 0 then do:
      if v-include-flag =  yes then do:
        output stream outstream2 close.
        /*теперь читаем из временного файла*/
        for each buf_rule-by-profile no-lock where
                buf_rule-by-profile.profile_id = p-profile-id
           and buf_rule-by-profile.codex_id = v-codex-id
           and buf_rule-by-profile.ruleset_id = v-ruleset-id,
           first buf_rule no-lock where buf_rule.rule_id = buf_rule-by-profile.rule_id
        break
        by buf_rule-by-profile.codex_id
        by buf_rule-by-profile.ruleset_id
        by buf_rule-by-profile.rule_id:
          if first-of(buf_rule-by-profile.rule_id) then do:
            input stream instream2 from value(v-rp-file-name).
            _v-rp-file-name:
            repeat :
              import stream instream2 unformatted ss.

              if ss begins substitute("procedure r_&1_&2_"
                                     , v-codex-id
                                     , v-ruleset-id)
              then do:
                assign
                ss = substitute("procedure r_&1_&2_&3:"
                                , v-codex-id
                                , v-ruleset-id
              ...
              if index(ss, "&end-def-vars&") > 0 then do:
                run clear-def-vars in this-procedure (  input buf_rule.rule_id ) no-error.
                if error-status:error then do:
                  undo, return error .
                end.
              end.
              ...
              if index(ss, "&start-rule&") > 0 then do:
                if buf_rule.hidden-content > 0 then do:
                  run temp-string_write in this-procedure ( input  substitute("  &1 rul/&2.i &3 "
                                                          , "~{"
                                                          , string(buf_rule.rule_id, "999999999")
                                                          , "~}"
                                                          )).
                  /*run copy-file-to-temp-string ( input search( substitute("rul/&1.i", string(buf_rule.rule_id, "999999999")))).*/
                end.
                else do:
                  run display-rule in this-procedure ( input buf_rule-by-profile.rule_id
            ...
            input stream instream2 close.
          end. /*if first-of(buf_rule-by-profile.rule_id) then do:*/
        end. /*        for each buf_rule-by-profile no-lock where */
        v-include-flag = no.
        os-delete value(v-rp-file-name).
      end. /*if v-include-flag =  yes then do:*/
    end. /*if index(ss, "&end-codex_id") > 0 then do:*/
    if index(ss, "&end-rule-block&") > 0 then do:

    end.
    if v-include-flag = yes then do:
      put stream outstream2 unformatted
      ss skip.
    end.
    if index(ss, "&order-list_") > 0 then do:
      assign
      v-key-ruleset = entry(2, ss, "&")
      v-codex-id = 0
      v-ruleset-id = 0
      ...
      for each buf_ruleset no-lock:
        assign
        v-key-ruleset-2 = substitute("order-list_codex_id=&1;ruleset_id=&2"
                                      , buf_ruleset.codex_id
                                      , buf_ruleset.ruleset_id).
        if v-key-ruleset-2 = v-key-ruleset then do:
          assign
      ...
      by buf_rule-by-profile.rp_order_id:
        ...
        v-order-list = v-order-list +
                      (if v-order-list = '':U
                       ...
                       substitute("r_&1_&2_&3"
                                , v-codex-id
                                , v-ruleset-id
                                , buf_rule-by-profile.rule_id
                                ).
      end.
      run temp-string_write in this-procedure ( input substitute('v_&1_&2_order = "&3".'
                                                                  , v-codex-id
                                                                  , v-ruleset-id
                                                                  , v-order-list
                                                                  )).
  ...
  output stream outstream to value( substitute( "rul/rp-&1.p" , buf_rule-profile.profile_id)).
  ...
  COMPILE value( substitute( "rul/rp-&1.p", buf_rule-profile.profile_id)).
  assign
...
define buffer buf_dis-porp-script for ub.prop-script.
  ...
  :
    ...
    on error  undo main-block, return error substitute( "&1. &2&3&4", vss-workfile, return-value, {&new-line}, error-status :get-message (1))
    :
      if first-of(buf_prop-script.script-name) then do:
        if not (buf_prop-script.language = "ABL") then next _rule-i-script.
...
define buffer buf0_prop-script for ub.prop-script.
  ...
  :
    if first-of(buf_rule-i-script.script-name) then do:
      ...
      find first buf_temp-rule-i-script no-lock where
                buf_temp-rule-i-script.root_rule_id = buf_rule.rule_id
  ...
  end.   /*for each buf_rule-by-profile where*/
end. /*dor*/
...
define buffer buf_rule-by-profile for ub.rule-profile.
define buffer buf_temp-rule-i-script for temp-rule-i-script.
define buffer buf_rule-i-script for ub.rule-i-script.
define buffer buf_prop-script for ub.prop-script.
define buffer buf0_prop-script for ub.prop-script.
  ...
  for each buf_rule-by-profile where
      ...
      first buf_rule no-lock where
            buf_rule.rule_id = buf_rule-by-profile.profile_id,
      each buf_rule-i-script no-lock where
            buf_rule-i-script.root_rule_id = buf_rule.rule_id
  break
  by buf_rule-i-script.script-name
  ...
  on error undo _main, return error substitute( "&1. &2&3&4", vss-workfile, return-value, {&new-line}, error-status :get-message (1)):
    CASE buf_ruledict-param.param-mode:
      when {&script-parmode-input} then do:
        ...
        then do:
          /*
          ничего не надо делать - вовременную таблицу переписывается в момент персистеной загрузки процедуры
          */
        end.
        else do:
          assign
          v-string = substitute(" find first buf_temp-rule-call-param no-lock where&1" +
                                            "buf_temp-rule-call-param.codex_id = v-codex-id&1" +
                                        "and buf_temp-rule-call-param.ruleset_id = v-ruleset-id&1" +
                                        "and buf_temp-rule-call-param.call_id = p-call-id&1" +
                                        "and buf_temp-rule-call-param.order_id = p-order-id&1" +
                                        "and buf_temp-rule-call-param.rule_id = p-rule-id&1" +
                                        'and buf_temp-rule-call-param.param-name = "&2"&1 no-error.&1' +
                                        "if available buf_temp-rule-call-param then do:&1" +
                                        "assign &2 = buf_temp-rule-call-param.param-value-&3.&1" +
                                        "end.&1"
                                  , {&new-line}
                                  , buf_ruledict-param.param-name
                                  , buf_ruledict-param.param-data-type).
...
:
  ...
  :
    if first-of(buf_rule-i-script.i-script-name) then do:
      find first buf_temp-rule-i-script  where
                buf_temp-rule-i-script.root_rule_id = p-rule-id
            and buf_temp-rule-i-script.i-script-name = buf_rule-i-script.script-name
            and buf_temp-rule-i-script.i-script-type = {&prop-script-type-variable}
            no-error.
      if available buf_temp-rule-i-script then delete buf_temp-rule-i-script.
    end. /*if first-of(buf_rule-i-script.i-script-name) then do:*/
  end.   /*for each buf_rule-i-script no-lock where*/
