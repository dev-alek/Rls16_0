...
/* ------------------------- &end-hn-option -----------------------------------*/
  ...
  On error undo _d-card, retry _d-card:
    ...
    /* ------------------------- &start-rule& -----------------------------------*/
/* % скидки на товар на следующий период (для выбранного периода) -> текущ. % скидки на товар
Алгоритмы зависимости скидок в следующем периоде от суммы оплаченных покупок за период времени */

_1553:
do:
                                                                      /* Salience 0 rule_id 1553*/
/* define variable Card1 as class Dis-card_ no-undo .*/
                                                                      /* Salience 1 rule_id 1553*/
/* define variable Next-discount1 as class Next-discount_ no-undo .*/
                                                                      /* salience 2 rule_id 1553*/
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:   
/* salience 3 in upper-rule-id 1553*/
  _1555:
  do:
                                                                      /* salience 0 rule_id 1555*/
  IF  Next-discount1:find_next-discount_sum-id( INPUT v-current-d-card, INPUT p-sum-id, INPUT p-caller-id) = true  THEN do:   
/* salience 1 in upper-rule-id 1555*/
    _1554:
    do:
                                                                      /* Salience 0 rule_id 1554*/
     Card1:d-pcnt = Next-discount1:d-pcnt .
                                                                      /* salience 1 rule_id 1554*/
    IF  Card1:dis-card_save( ) = false  THEN do:   
/* salience 2 in upper-rule-id 1554*/
      _1556:
      do:
                                                                      /* Salience 0 rule_id 1556*/
