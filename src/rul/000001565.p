...
Card1 = new Dis-card_{&constructor_1} .
 define variable first-main-card as  character no-undo .
...
/* ------------------------- &end-hn-option -----------------------------------*/
  ...
  On error undo _d-card, retry _d-card:
    ...
    /* ------------------------- &start-rule& -----------------------------------*/
/* Сумма накоплений по всем картам пула -> % скидки на товар на всех картах пула
Прогрессивный алгоритм зависимости Сумма накоплений с учетом дополн. карт -> % скидки на товар на все карты пула */

_1565:
do:
                                                                      /* Salience 0 rule_id 1565*/
/* define variable Card1 as class Dis-card_ no-undo .*/
                                                                      /* Salience 1 rule_id 1565*/
/* define variable v-netto-sum1 as  decimal no-undo .*/
                                                                      /* Salience 2 rule_id 1565*/
/* define variable first-main-card as  character no-undo .*/
                                                                      /* salience 3 rule_id 1565*/
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:   
                                                                      /* Salience 4 rule_id 1565*/
 v-netto-sum1 = 0 .
/* salience 5 in upper-rule-id 1565*/
  _1567:
  do:
                                                                      /* salience 0 rule_id 1567*/
  IF  Card1:emitent-host-code = 0  THEN do:   
/* salience 1 in upper-rule-id 1567*/
    _1569:
    do:
                                                                      /* Salience 0 rule_id 1569*/
/*     define variable Tot-sum1 as class Dis-tot_ no-undo .*/
                                                                      /* salience 1 rule_id 1569*/
    IF  Tot-sum1:find_dis-tot_( INPUT v-current-d-card) = true  THEN do:   
/* salience 2 in upper-rule-id 1569*/
      _1568:
      do:
                                                                      /* salience 0 rule_id 1568*/
      IF  p-r-b = "rubl"  THEN do:   
/* salience 1 in upper-rule-id 1568*/
        _1570:
        do:
                                                                      /* Salience 0 rule_id 1570*/
         v-netto-sum1 = Tot-sum1:pay-tot-rubl_Byfm# .

        end. /*of rule 1570*/
      end. /*of rule 1570*/
                                                                      /* salience 4 rule_id 1568*/
      IF  p-r-b = "base"  THEN do:   
/* salience 5 in upper-rule-id 1568*/
        _1571:
        do:
                                                                      /* Salience 0 rule_id 1571*/
         v-netto-sum1 = Tot-sum1:pay-tot-rubl_Byfm# .
                                                                      ...
                                                                      /* salience 2 rule_id 1567*/
  IF  Card1:emitent-host-code > 0  THEN do:   
/* salience 3 in upper-rule-id 1567*/
    _1566:
    do:
                                                                      /* Salience 0 rule_id 1566*/
/*     define variable Tot-sum-host1 as class Dis-tot_host no-undo .*/
                                                                      /* salience 1 rule_id 1566*/
    IF  Tot-sum-host1:find_dis-tot_host( INPUT v-current-d-card, INPUT ( Card1:emitent-host-code )) = true  THEN do:   
/* salience 2 in upper-rule-id 1566*/
      _1572:
      do:
                                                                      /* salience 0 rule_id 1572*/
      IF  p-r-b = "rubl"  THEN do:   
/* salience 1 in upper-rule-id 1572*/
        _1573:
        do:
                                                                      /* Salience 0 rule_id 1573*/
         v-netto-sum1 = Tot-sum-host1:pay-tot-rubl_Byfm# .

        end. /*of rule 1573*/
      end. /*of rule 1573*/
                                                                      /* salience 4 rule_id 1572*/
      IF  p-r-b = "base"  THEN do:   
/* salience 5 in upper-rule-id 1572*/
        _1574:
        do:
                                                                      /* Salience 0 rule_id 1574*/
         v-netto-sum1 = Tot-sum-host1:pay-tot-base_Byfm# .
                                                                      ...
                                                                      /* Salience 4 rule_id 1567*/
   first-main-card = Card1:first-main-card .
                                                                      /* Salience 5 rule_id 1567*/
   Card1:select_dis-cards( INPUT substitute('first-main-card = "&1"', ( first-main-card ))) .
                                                                      /* salience 6 rule_id 1567*/
  DO WHILE  Card1:get_dis-card_( INPUT "next") = true :   
/* salience 7 in upper-rule-id 1567*/
    _1575:
    do:
                                                                      /* salience 0 rule_id 1575*/
    IF  Card1:status_ = {&current-status}  THEN do:   
/* salience 1 in upper-rule-id 1575*/
      _1576:
      do:
                                                                      /* Salience 0 rule_id 1576*/
       Card1:d-pcnt = dis-rule_00057( INPUT p-rule-num, INPUT v-current-doc-date, INPUT v-current-doc-time, INPUT v-netto-sum1) .
                                                                      /* salience 1 rule_id 1576*/
      IF  Card1:dis-card_save( ) = false  THEN do:   
/* salience 2 in upper-rule-id 1576*/
        _1577:
        do:
                                                                      /* Salience 0 rule_id 1577*/
