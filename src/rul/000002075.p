...
&undefine cr-edist_i
{ cus/cr-edist.i  tt }
{ cus/edocsord.i edi position-t  proc-edi-reply-process }
{ ref/extclass.i }
{ cus/str-edi.i }
...
:
  if retry then do:
    ...
    v-ord-int1 = (if ordrsp-t.action = 29
                          then integer({&edi-ordrsp-sts})
                          else (if ordrsp-t.action = 27
                                then integer({&edi-err})
                                else integer({&edi-ordrsp})
                                )
                          ) /*такой будет новый статус*/
    v-cli-out-doc = ordrsp-t.number + {&delim-par} + iso-date(ordrsp-t.date) + {&delim-par} + ordrsp-t.time_
    ...
    if error-status:error then do:
      ...
      v-edist-mess = cr-edist_add-edist-mess( v-edist-mess, {&edist_ediinterchangeid}, head-t.ediinterchangeid ).
      v-date-status = ?.
