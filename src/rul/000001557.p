...
/* ------------------------- &end-hn-option -----------------------------------*/
  ...
  On error undo _d-card, retry _d-card:
    ...
    /* ------------------------- &start-rule& -----------------------------------*/
/* % скидки на итог на следующий период (для выбранного периода) -> текущ. % скидки на итог
Алгоритмы зависимости скидок в следующем периоде от суммы оплаченных покупок за период времени */

_1557:
do:
                                                                      /* Salience 0 rule_id 1557*/
/* define variable Card1 as class Dis-card_ no-undo .*/
                                                                      /* Salience 1 rule_id 1557*/
/* define variable Next-discount1 as class Next-discount_ no-undo .*/
                                                                      /* salience 2 rule_id 1557*/
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:   
/* salience 3 in upper-rule-id 1557*/
  _1559:
  do:
                                                                      /* salience 0 rule_id 1559*/
  IF  Next-discount1:find_next-discount_sum-id( INPUT v-current-d-card, INPUT p-sum-id, INPUT p-caller-id)  THEN do:   
/* salience 1 in upper-rule-id 1559*/
    _1558:
    do:
                                                                      /* Salience 0 rule_id 1558*/
     Card1:cash-d-pcnt = Next-discount1:cash-d-pcnt .
                                                                      /* salience 1 rule_id 1558*/
    IF  Card1:dis-card_save( ) = false  THEN do:   
/* salience 2 in upper-rule-id 1558*/
      _1560:
      do:
                                                                      /* Salience 0 rule_id 1560*/
