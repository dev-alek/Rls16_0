...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
&scop my-message substitute(".............Импорт данных по видам оплат из ВС")
  {&display-message}.
&scop my-message substitute("Импорт данных по видам оплат из файла &1", file-name)
  {&display-message}.
  ...
  REPEAT:
    ...
    _rule:
      ...
      /* ------------------------- &start-rule& -----------------------------------*/
          IF  ImpData1:current-tbl-name( ) = {&table_pay-type}  THEN do:
            trans_save:
            do transaction
            ...
            :
              ...
              assign
                v-line-num      = ImpData1:route-data_get-field-integer  ( input {&table_pay-type}, input "line-num":U )
                v-pay-type-code = ImpData1:route-data_get-field-integer  ( input {&table_pay-type}, input "obj-code":U)
                v-pay-type-name = ImpData1:route-data_get-field-character( input {&table_pay-type}, input "obj-name":U)
              .
              ...
              if error-status:error then do:
                if return-value <> '' then do:
                  &scop my-message substitute("Ошибка идентификации принимаемых данных по ставке налога из ВС:&1&2", {&new-line}, return-value )
                  {&display-message}.
                end.
                run set-err-type in p-cont-handle
                  ( input v-err-type
              ...
              if error-status:error then do:
                if return-value <> '' then do:
                  &scop my-message substitute("Ошибка при сохранении данных по ставке налога при приеме данных из ВС:&1&2", {&new-line}, return-value )
                  {&display-message}.
                end.
                run set-err-type in p-cont-handle
                  ( input v-err-type
          ...
          end. /*IF  ImpData1:current-tbl-name( ) = "pay-type"  THEN do:*/
...
procedure proc-save-pay-type :
  ...
  do
  on error  undo, return error substitute( "&1 (proc-save-pay-type). &2&3&4", vss-workfile, return-value, {&new-line}, error-status :get-message (1))
  ...
  :
    ...
    if error-status:error then do:
      undo, return error substitute( "&1 (proc-save-pay-type). &2&3&4", vss-workfile, return-value, {&new-line}, error-status :get-message (1)) .
    end.

  end.

end procedure. /* proc-save-pay-type */