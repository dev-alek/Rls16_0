...
Задание и просмотр параметров вызова правил для профайла 63
...
define variable vss-description as character no-undo init "Задание и просмотр параметров вызова правил для профайла 63".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i }
...
&Scoped-Define ENABLED-OBJECTS B-exit b-quit B-Help BR-rcp button-obj ~
f-keep-days f-luft-days
&Scoped-Define DISPLAYED-OBJECTS e-shops f-keep-days f-luft-days
...
DEFINE VARIABLE f-keep-days AS INTEGER FORMAT ">>,>>9":U INITIAL 0
     LABEL "Код ЗАКОНЧИВШЕЙСЯ партии будет удалятся с кассы через (дн.)"
     VIEW-AS FILL-IN
     SIZE 10.5 BY 1 NO-UNDO.

DEFINE VARIABLE f-luft-days AS INTEGER FORMAT ">>,>>9":U INITIAL 0
     LABEL "Код считается удаленным через (дн)"
     VIEW-AS FILL-IN
     SIZE 10.5 BY 1 NO-UNDO.
...
DEFINE FRAME Dialog-Frame
     ...
     e-shops AT ROW 3.13 COL 7.5 NO-LABEL WIDGET-ID 54
     f-keep-days AT ROW 17 COL 60 COLON-ALIGNED WIDGET-ID 74
     f-luft-days AT ROW 18.33 COL 60 COLON-ALIGNED WIDGET-ID 76
     "от начала удаления" VIEW-AS TEXT
          SIZE 20 BY .8 AT ROW 18.33 COL 73 WIDGET-ID 80
     "Магазины, в которых удаляются партионные коды" VIEW-AS TEXT
          SIZE 48 BY 1.07 AT ROW 2.07 COL 2.5 WIDGET-ID 72
          FGCOLOR 4
     SPACE(48.50) SKIP(20.11)
...
------------------------------------------------------------------------------*/
  DISPLAY e-shops f-keep-days f-luft-days
      WITH FRAME Dialog-Frame.
  ENABLE B-exit b-quit B-Help BR-rcp button-obj f-keep-days f-luft-days
      WITH FRAME Dialog-Frame.
...
RUN rcps_get-value IN THIS-PROCEDURE (
                                ...
                                ,output f-keep-days /*p-value-integer*/
...
RUN rcps_get-value IN THIS-PROCEDURE (
                                ...
                                ,output v-value-date /*p-value-date*/
                                ,output v-value-decimal /*p-value-decimal*/
                                ,output f-luft-days /*p-value-integer*/
                                ,output v-value-logical /*p-value-logical*/
                                ).
...
do while v-index-id >= 0 :
  ...
  if error-status:error then v-index-id = -1.
      ...
      find first obj-list where
          ...
          and obj-list.obj-code =  v-shop-code no-error.
...
ENABLE
f-keep-days WHEN p-mode <> {&LOOKUP}
f-luft-days WHEN p-mode <> {&LOOKUP}
...
VIEW FRAME {&frame-name}.
assign
e-shops:read-only in frame {&frame-name} = yes.
...
END.
ASSIGN
FRAME {&FRAME-NAME}:TITLE = substitute("Параметры удаления с касс партионных кодов по флагам ЗАКОНЧИВШИХСЯ партий по товару, продающемуся по партиям (профайл &1)", p-profile-id).
...
RUN rcps_set-value IN THIS-PROCEDURE (
                                ...
                                ,INPUT f-keep-days /*p-value-integer*/
...
RUN rcps_set-value IN THIS-PROCEDURE (
                                ...
                                ,INPUT ? /*p-value-date*/
                                ,INPUT 0.0 /*p-value-decimal*/
                                ,INPUT f-luft-days /*p-value-integer*/
                                ,INPUT NO /*p-value-logical*/
                                ).
...
run ref/thobjs.w
    ...
    , input "b-mark,b-sel"
    , input (if v-running-mode then "db" else {&all})
    , input {&shop} /*p-obj-type*/
    , input (if v-running-mode then v-cntxt-db-num else ?) /*p-db-num*/
    , input ? /*p-host-code*/
