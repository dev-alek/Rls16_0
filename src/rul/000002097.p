...
/*    end.                                                                                                                          */
/*                                                                                                                                  */
/*    if buf_ord-doc.contract-code <> ? then                                                                                        */
/*    find first buf_contract where buf_contract.host-code = buf_ord-doc.host-code                                                  */
/*                              and buf_contract.contract-code = buf_ord-doc.contract-code no-lock no-error .                       */
    ...
    v-custom-pack-name = "RETANN_&pack-num.xml". 
    ...
    sw:insert-attribute ("id", (v-current-doc-code + "_" + string(TODAY) + string(TIME))) .
        sw:start-element ("interchangeHeader") .
            ...
            sw:write-data-element ("documentType", "RETANN") .
/*            sw:write-data-element ("isTest", "1") .*/
        ...
        sw:start-element ("announcementForReturns") .
        sw:insert-attribute ("number", v-current-doc-code) .
        sw:insert-attribute ("date", v-date-chr) .
/*            sw:write-empty-element ("contractIdentificator") .                  */
/*            sw:insert-attribute ("number", string(buf_contract.contract-code)) .*/
/*            sw:insert-attribute ("date", iso-date(buf_contract.contract-date)) .*/
...
/*                sw:write-data-element ("deliveryDate", (iso-date(buf_ord-doc.ship-date) + "T" + string(buf_ord-doc.ship-time, "HH:MM:SS") + ".000Z")) .*/
    ...
    :
      ...
      run find-bcod(buf_goods.gds-code, output v-b-code).
                ...
                sw:start-element ("lineItem") .
                    sw:write-data-element ("gtin", v-b-code) .
                    sw:write-data-element ("internalBuyerCode", string(buf_goods.gds-code)) .
                    sw:write-data-element ("internalSupplierCode", if avail buf_ext-artic then buf_ext-artic.ext-artic else "") .
                    sw:write-data-element ("lineNumber", string(buf_doc-line.line-num)) .
                    ...
                    sw:write-data-element ("returnQuantity", trim(string(buf_doc-line.fact-qnty, "->>>>>>>>9.999")), "", swa) .                       
/*                    sw:write-data-element ("netPrice", trim(string(buf_doc-line.price-cli, "->>>>>>>>9.9999"))) .*/
