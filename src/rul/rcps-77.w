...
Задание и просмотр параметров вызова правил для профайла 77

Автор: Бахтадзе Наталья Викторовна
Дата создания: 08/18/10
Author: Bakhtadze Natalya
Creation date: 08/18/10
...
define variable vss-description as character no-undo init "Задание и просмотр параметров вызова правил для профайла 77".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i }
...
&Scoped-Define ENABLED-OBJECTS B-exit b-quit B-Help BR-rcp t-rule-desadv-sts
&Scoped-Define DISPLAYED-OBJECTS t-rule-desadv-sts
...
DEFINE VARIABLE t-rule-desadv-sts AS LOGICAL INITIAL no
     LABEL "Отправлять подтверждение на принятый документ DESADV"
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME Dialog-Frame
     ...
     t-rule-desadv-sts AT ROW 9 COL 16 WIDGET-ID 88
     SPACE(25.00) SKIP(13.25)
...
------------------------------------------------------------------------------*/
  DISPLAY t-rule-desadv-sts
      WITH FRAME Dialog-Frame.
  ENABLE B-exit b-quit B-Help BR-rcp t-rule-desadv-sts
      WITH FRAME Dialog-Frame.
...
run rcps_get-rule-on-off in this-procedure ( input {&edoc-proc_18}
                                            ,input {&edoc-proc_18_event_rcv_105}
                                            ,input {&rule-desadv-sts}
                                            ,input p-profile-id
                                            ,input p-once-more
                                            ,output t-rule-desadv-sts ).
...
ENABLE
t-rule-desadv-sts WHEN p-mode <> {&LOOKUP}
...
END.
ASSIGN
FRAME {&FRAME-NAME}:TITLE = substitute("Параметры обмена данными по заказам поставщику через систему  Exite-EDI (профайл &1)", p-profile-id).
...
t-rule-desadv-sts
.
run rcps_set-rule-on-off in this-procedure ( input {&edoc-proc_18}
                                           ,input {&edoc-proc_18_event_rcv_105}
                                           ,input {&rule-desadv-sts}
                                           ,input p-profile-id
                                           ,input p-once-more
                                           ,input t-rule-desadv-sts).
