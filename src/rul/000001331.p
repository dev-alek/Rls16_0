...
/* ------------------------- &end-hn-option -----------------------------------*/
  ...
  On error undo _d-card, retry _d-card:
    ...
    /* ------------------------- &start-rule& -----------------------------------*/
/* Сумма накоплений -> % скидки на итог
Простой прогрессивный алгоритм зависимости от суммы оплаченных покупок в <ТИП ВАЛЮТЫ> за все время жизни ДК <С/БЕЗ УЧЕТА ПЕРЕВЫПУСКА ДК> */

_1331:
do:
                                                                      /* Salience 0 rule_id 1331*/
/* define variable Card1 as class Dis-card_ no-undo .*/
                                                                      /* Salience 1 rule_id 1331*/
/* define variable v-netto-sum1 as  decimal no-undo .*/
                                                                      /* salience 2 rule_id 1331*/
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:   
                                                                      /* Salience 3 rule_id 1331*/
 v-netto-sum1 = 0 .
/* salience 4 in upper-rule-id 1331*/
  _1339:
  do:
                                                                      /* salience 0 rule_id 1339*/
  IF  Card1:emitent-host-code = 0  THEN do:   
/* salience 1 in upper-rule-id 1339*/
    _1337:
    do:
                                                                      /* Salience 0 rule_id 1337*/
/*     define variable Tot-sum1 as class Dis-tot_ no-undo .*/
                                                                      /* salience 1 rule_id 1337*/
    IF  Tot-sum1:find_dis-tot_( INPUT v-current-d-card) = true  THEN do:   
/* salience 2 in upper-rule-id 1337*/
      _1335:
      do:
                                                                      /* salience 0 rule_id 1335*/
      IF  p-r-b = "rubl" AND p-is-over = false  THEN do:   
/* salience 1 in upper-rule-id 1335*/
        _1342:
        do:
                                                                      /* Salience 0 rule_id 1342*/
         v-netto-sum1 = Tot-sum1:pay-tot-rubl .

        end. /*of rule 1342*/
      end. /*of rule 1342*/
                                                                      /* salience 2 rule_id 1335*/
      IF  p-r-b = "rubl" AND p-is-over = true  THEN do:   
/* salience 3 in upper-rule-id 1335*/
        _1334:
        do:
                                                                      /* Salience 0 rule_id 1334*/
         v-netto-sum1 = Tot-sum1:pay-tot-rubl_byf# .

        end. /*of rule 1334*/
      end. /*of rule 1334*/
                                                                      /* salience 4 rule_id 1335*/
      IF  p-r-b = "base" AND p-is-over = false  THEN do:   
/* salience 5 in upper-rule-id 1335*/
        _1336:
        do:
                                                                      /* Salience 0 rule_id 1336*/
         v-netto-sum1 = Tot-sum1:pay-tot-base .

        end. /*of rule 1336*/
      end. /*of rule 1336*/
                                                                      /* salience 6 rule_id 1335*/
      IF  p-r-b = "base" AND p-is-over = true  THEN do:   
/* salience 7 in upper-rule-id 1335*/
        _1344:
        do:
                                                                      /* Salience 0 rule_id 1344*/
         v-netto-sum1 = Tot-sum1:pay-tot-base_byf# .
                                                                      ...
                                                                      /* salience 2 rule_id 1339*/
  IF  Card1:emitent-host-code > 0  THEN do:   
/* salience 3 in upper-rule-id 1339*/
    _1338:
    do:
                                                                      /* Salience 0 rule_id 1338*/
/*     define variable Tot-sum-host1 as class Dis-tot_host no-undo .*/
                                                                      /* salience 1 rule_id 1338*/
    IF  Tot-sum-host1:find_dis-tot_host( INPUT v-current-d-card, INPUT ( Card1:emitent-host-code )) = true  THEN do:   
/* salience 2 in upper-rule-id 1338*/
      _1333:
      do:
                                                                      /* salience 0 rule_id 1333*/
      IF  p-r-b = "rubl" AND p-is-over = false  THEN do:   
/* salience 1 in upper-rule-id 1333*/
        _1340:
        do:
                                                                      /* Salience 0 rule_id 1340*/
         v-netto-sum1 = Tot-sum-host1:pay-tot-rubl .

        end. /*of rule 1340*/
      end. /*of rule 1340*/
                                                                      /* salience 2 rule_id 1333*/
      IF  p-r-b = "rubl" AND p-is-over = true  THEN do:   
/* salience 3 in upper-rule-id 1333*/
        _1343:
        do:
                                                                      /* Salience 0 rule_id 1343*/
         v-netto-sum1 = Tot-sum-host1:pay-tot-rubl_byf# .

        end. /*of rule 1343*/
      end. /*of rule 1343*/
                                                                      /* salience 4 rule_id 1333*/
      IF  p-r-b = "base" AND p-is-over = false  THEN do:   
/* salience 5 in upper-rule-id 1333*/
        _1341:
        do:
                                                                      /* Salience 0 rule_id 1341*/
         v-netto-sum1 = Tot-sum-host1:pay-tot-base .

        end. /*of rule 1341*/
      end. /*of rule 1341*/
                                                                      /* salience 6 rule_id 1333*/
      IF  p-r-b = "base" AND p-is-over = true  THEN do:   
/* salience 7 in upper-rule-id 1333*/
        _1332:
        do:
                                                                      /* Salience 0 rule_id 1332*/
         v-netto-sum1 = Tot-sum-host1:pay-tot-base_byf# .
                                                                      ...
                                                                      /* Salience 5 rule_id 1331*/
 Card1:cash-d-pcnt = dis-rule_00058( INPUT p-rule-num, INPUT v-current-doc-date, INPUT v-current-doc-time, INPUT v-netto-sum1) .
                                                                      /* salience 6 rule_id 1331*/
IF  Card1:dis-card_save( ) = false  THEN do:   
/* salience 7 in upper-rule-id 1331*/
  _1345:
  do:
                                                                      /* Salience 0 rule_id 1345*/
