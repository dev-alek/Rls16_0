...
:
  ...
  if available previous-shift-obj then do:
    ...
    run rep/chk-ahz.p (
                  ...
                  ,input       g#db-num /* p-check-act-db-num  */
                  ,input       g#userid /* p-check-act-user-id */
    ...
    or buf_shift-obj.shift-date > v-current-shift-date  then do:
      &scop my-message substitute("Объект &1&2 Печать отчета за выбранную смену невозможна&3"  + ~
  ...
  X-tog-shift = yes
  x-SET_val_TYPE = (if p-r-b = {&r-b-base}
                    then {&v-base}
                    else {&v-rubl})
  x-SelectGood = {&g-all}
  .
