...
Вспомогательный файл для кодекса правил 18 набор правил 12
...
Импорт данных по документам назначения цены
...
run write-log  in p-log-handle (
                                 input 0
                               , "&DLine").
&scop my-message substitute(".............Импорт данных по Документам назначения цены из ВС")
  {&display-message}.
  &scop my-message substitute("Импорт данных по документам назначения цены из файла &1", file-name)
  {&display-message}.
...
ImpData1:Route-data_{&release_2} .
    ...
    _rule:
      ...
      /* ------------------------- &start-rule& -----------------------------------*/
          ...
          end. /*IF  ImpData1:current-tbl-name( ) = "price-doc"  THEN do:*/
          ...
          end. /*IF  ImpData1:current-tbl-name( ) = "price-list"  THEN do:*/
      /* ------------------------- &end-rule -------------------------------------*/
      end.
    end.
    v-retry-action = 0 .
  ...
  if error-status:error then do:
    if return-value <> '' then do:
      &scop my-message substitute("Ошибка при сохранении данных по ДНЦ из ВС:&1&2&1&3", {&new-line}, return-value , error-status :get-message(1)  )
      {&display-message}.
    end.
