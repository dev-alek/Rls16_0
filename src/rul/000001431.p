...
/* ------------------------- &end-hn-option -----------------------------------*/
  ...
  On error undo _d-card, retry _d-card:
    ...
    /* ------------------------- &start-rule& -----------------------------------*/
/* Сумма накоплений за текущий период -> % скидки на товар в следующем периоде
...
_1431:
do:
                                                                      /* Salience 0 rule_id 1431*/
/* define variable Card1 as class Dis-card_ no-undo .*/
                                                                      /* Salience 1 rule_id 1431*/
/* define variable v-netto-sum1 as  decimal no-undo .*/
                                                                      /* salience 2 rule_id 1431*/
IF  get_current_sum-id_by-date( INPUT v-current-doc-date, INPUT p-tot-caller-id) = "?"  THEN do:   
/* salience 3 in upper-rule-id 1431*/
  _1446:
  do:
                                                                      /* Salience 0 rule_id 1446*/
   leave _1431 .

  end. /*of rule 1446*/
end. /*of rule 1446*/
                                                                      /* salience 4 rule_id 1431*/
IF  get_current_nd_sum-id_by-date( INPUT v-current-doc-date, INPUT p-nd-caller-id) = "?"  THEN do:   
/* salience 5 in upper-rule-id 1431*/
  _1447:
  do:
                                                                      /* Salience 0 rule_id 1447*/
   leave _1431 .

  end. /*of rule 1447*/
end. /*of rule 1447*/
                                                                      /* salience 7 rule_id 1431*/
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:   
                                                                      /* Salience 8 rule_id 1431*/
 v-netto-sum1 = 0 .
/* salience 9 in upper-rule-id 1431*/
  _1433:
  do:
                                                                      /* salience 0 rule_id 1433*/
  IF  Card1:emitent-host-code = 0  THEN do:   
/* salience 1 in upper-rule-id 1433*/
    _1432:
    do:
                                                                      /* Salience 0 rule_id 1432*/
/*     define variable Tot-period-sum1 as class Dis-tot_ no-undo .*/
                                                                      /* salience 1 rule_id 1432*/
    IF  Tot-period-sum1:find_dis-tot_date( INPUT v-current-d-card, INPUT v-current-doc-date, INPUT p-tot-caller-id) = true  THEN do:   
/* salience 2 in upper-rule-id 1432*/
      _1438:
      do:
                                                                      /* salience 0 rule_id 1438*/
      IF  p-r-b = "rubl" AND p-is-over = false  THEN do:   
/* salience 1 in upper-rule-id 1438*/
        _1437:
        do:
                                                                      /* Salience 0 rule_id 1437*/
         v-netto-sum1 = Tot-period-sum1:pay-tot-rubl .

        end. /*of rule 1437*/
      end. /*of rule 1437*/
                                                                      /* salience 2 rule_id 1438*/
      IF  p-r-b = "rubl" AND p-is-over = true  THEN do:   
/* salience 3 in upper-rule-id 1438*/
        _1448:
        do:
                                                                      /* Salience 0 rule_id 1448*/
         v-netto-sum1 = Tot-period-sum1:pay-tot-rubl_byf# .

        end. /*of rule 1448*/
      end. /*of rule 1448*/
                                                                      /* salience 4 rule_id 1438*/
      IF  p-r-b = "base" AND p-is-over = false  THEN do:   
/* salience 5 in upper-rule-id 1438*/
        _1444:
        do:
                                                                      /* Salience 0 rule_id 1444*/
         v-netto-sum1 = Tot-period-sum1:pay-tot-base .

        end. /*of rule 1444*/
      end. /*of rule 1444*/
                                                                      /* salience 6 rule_id 1438*/
      IF  p-r-b = "base" AND p-is-over = true  THEN do:   
/* salience 7 in upper-rule-id 1438*/
        _1443:
        do:
                                                                      /* Salience 0 rule_id 1443*/
         v-netto-sum1 = Tot-period-sum1:pay-tot-base_byf# .
                                                                      ...
                                                                      /* salience 2 rule_id 1433*/
  IF  Card1:emitent-host-code > 0  THEN do:   
/* salience 3 in upper-rule-id 1433*/
    _1440:
    do:
                                                                      /* salience 1 rule_id 1440*/
...
/* salience 2 in upper-rule-id 1440*/
      _1435:
      do:
                                                                      /* Salience 0 rule_id 1435*/
/*       define variable Tot-period-sum-host1 as class Dis-tot_host no-undo .*/
                                                                      /* salience 3 rule_id 1435*/
      IF  p-r-b = "rubl" AND p-is-over = false  THEN do:   
/* salience 4 in upper-rule-id 1435*/
        _1441:
        do:
                                                                      /* Salience 0 rule_id 1441*/
         v-netto-sum1 = Tot-period-sum-host1:pay-tot-rubl .

        end. /*of rule 1441*/
      end. /*of rule 1441*/
                                                                      /* salience 5 rule_id 1435*/
      IF  p-r-b = "rubl" AND p-is-over = true  THEN do:   
/* salience 6 in upper-rule-id 1435*/
        _1442:
        do:
                                                                      /* Salience 0 rule_id 1442*/
         v-netto-sum1 = Tot-period-sum-host1:pay-tot-rubl_byf# .

        end. /*of rule 1442*/
      end. /*of rule 1442*/
                                                                      /* salience 7 rule_id 1435*/
      IF  p-r-b = "base" AND p-is-over = false  THEN do:   
/* salience 8 in upper-rule-id 1435*/
        _1439:
        do:
                                                                      /* Salience 0 rule_id 1439*/
         v-netto-sum1 = Tot-period-sum-host1:pay-tot-base .

        end. /*of rule 1439*/
      end. /*of rule 1439*/
                                                                      /* salience 9 rule_id 1435*/
      IF  p-r-b = "base" AND p-is-over = true  THEN do:   
/* salience 10 in upper-rule-id 1435*/
        _1434:
        do:
                                                                      /* Salience 0 rule_id 1434*/
         v-netto-sum1 = Tot-period-sum-host1:pay-tot-base_byf# .
                                                                      ...
                                                                      /* Salience 10 rule_id 1431*/
/* define variable Next-discount1 as class Next-discount_ no-undo .*/
                                                                      /* salience 11 rule_id 1431*/
...
/* salience 12 in upper-rule-id 1431*/
  _1445:
  do:
                                                                      /* salience 1 rule_id 1445*/
...
/* salience 2 in upper-rule-id 1445*/
    _1449:
    do:
                                                                      /* Salience 0 rule_id 1449*/
     undo _main, return error v-last-error-message .
                                                                      ...
                                                                      /* Salience 13 rule_id 1431*/
 Next-discount1:d-pcnt = dis-rule_00060( INPUT p-rule-num, INPUT v-current-doc-date, INPUT v-current-doc-time, INPUT v-netto-sum1) .
                                                                      /* salience 14 rule_id 1431*/
IF  Next-discount1:next-discount_save( ) = false  THEN do:   
/* salience 15 in upper-rule-id 1431*/
  _1436:
  do:
                                                                      /* Salience 0 rule_id 1436*/
   undo _main, return error v-last-error-message .
