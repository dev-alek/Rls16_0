...
/* ------------------------- &end-hn-option -----------------------------------*/
  ...
  On error undo _d-card, retry _d-card:
    ...
    /* ------------------------- &start-rule& -----------------------------------*/
/* Суммы накоплений за текущий и предыдущий период -> категория скидки в следующем периоде
...
_1525:
do:
                                                                      /* Salience 0 rule_id 1525*/
/* define variable Card1 as class Dis-card_ no-undo .*/
                                                                      /* Salience 1 rule_id 1525*/
/* define variable v-netto-current as  decimal no-undo .*/
                                                                      /* Salience 2 rule_id 1525*/
/* define variable v-netto-prev as  decimal no-undo .*/
                                                                      /* Salience 3 rule_id 1525*/
/* define variable prev-sum-id as  character no-undo .*/
                                                                      /* salience 4 rule_id 1525*/
IF  get_current_sum-id_by-date( INPUT v-current-doc-date, INPUT p-tot-caller-id) = "?"  THEN do:   
/* salience 5 in upper-rule-id 1525*/
  _1533:
  do:
                                                                      /* Salience 0 rule_id 1533*/
   leave _1525 .

  end. /*of rule 1533*/
end. /*of rule 1533*/
                                                                      /* salience 6 rule_id 1525*/
IF  get_current_nd_sum-id_by-date( INPUT v-current-doc-date, INPUT p-nd-caller-id) = "?"  THEN do:   
/* salience 7 in upper-rule-id 1525*/
  _1538:
  do:
                                                                      /* Salience 0 rule_id 1538*/
   leave _1525 .

  end. /*of rule 1538*/
end. /*of rule 1538*/
                                                                      /* Salience 8 rule_id 1525*/
 prev-sum-id = get_prev_sum-id_by-date( INPUT v-current-doc-date, INPUT p-tot-caller-id) .
                                                                      /* salience 10 rule_id 1525*/
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:   
                                                                      /* Salience 11 rule_id 1525*/
 v-netto-current = 0 .
                                                                      /* Salience 12 rule_id 1525*/
 v-netto-prev = 0 .
/* salience 13 in upper-rule-id 1525*/
  _1539:
  do:
                                                                      /* salience 0 rule_id 1539*/
  IF  Card1:emitent-host-code = 0  THEN do:   
/* salience 1 in upper-rule-id 1539*/
    _1531:
    do:
                                                                      /* Salience 0 rule_id 1531*/
/*     define variable Tot-sum-current as class Dis-tot_ no-undo .*/
                                                                      /* Salience 1 rule_id 1531*/
/*     define variable Tot-sum-prev as class Dis-tot_ no-undo .*/
                                                                      /* salience 2 rule_id 1531*/
    IF  prev-sum-id <> "?"  THEN do:   
/* salience 3 in upper-rule-id 1531*/
      _1821:
      do:
                                                                      /* salience 1 rule_id 1821*/
      IF  Tot-sum-prev:find_dis-tot_sum-id( INPUT v-current-d-card, INPUT prev-sum-id, INPUT p-tot-caller-id) = true  THEN do:   
/* salience 2 in upper-rule-id 1821*/
        _1823:
        do:
...
/* salience 7 in upper-rule-id 1542*/
        ...
        do:
         ...
         v-netto-current = Tot-sum-current:pay-tot-base_byf# .
                                                                      ...
                                                                      /* salience 2 rule_id 1539*/
  IF  Card1:emitent-host-code > 0  THEN do:   
/* salience 3 in upper-rule-id 1539*/
    _1534:
    do:
                                                                      /* Salience 0 rule_id 1534*/
/*     define variable Tot-sum-host-current as class Dis-tot_host no-undo .*/
                                                                      /* Salience 1 rule_id 1534*/
/*     define variable Tot-sum-host-prev as class Dis-tot_host no-undo .*/
                                                                      /* salience 2 rule_id 1534*/
    IF  prev-sum-id <> "?"  THEN do:   
/* salience 3 in upper-rule-id 1534*/
      _1822:
      do:
                                                                      /* salience 1 rule_id 1822*/
...
/* salience 2 in upper-rule-id 1822*/
        _1824:
        do:
...
/* salience 8 in upper-rule-id 1534*/
      _1527:
      do:
                                                                      /* salience 0 rule_id 1527*/
      IF  p-r-b = "rubl" AND p-is-over = false  THEN do:   
/* salience 1 in upper-rule-id 1527*/
        _1540:
        do:
                                                                      /* Salience 0 rule_id 1540*/
         v-netto-current = Tot-sum-host-current:pay-tot-rubl .

        end. /*of rule 1540*/
      end. /*of rule 1540*/
                                                                      /* salience 2 rule_id 1527*/
      IF  p-r-b = "rubl" AND p-is-over = true  THEN do:   
/* salience 3 in upper-rule-id 1527*/
        _1547:
        do:
                                                                      /* Salience 0 rule_id 1547*/
         v-netto-current = Tot-sum-host-current:pay-tot-rubl_byf# .

        end. /*of rule 1547*/
      end. /*of rule 1547*/
                                                                      /* salience 4 rule_id 1527*/
      IF  p-r-b = "base" AND p-is-over = false  THEN do:   
/* salience 5 in upper-rule-id 1527*/
        _1526:
        do:
                                                                      /* Salience 0 rule_id 1526*/
         v-netto-current = Tot-sum-host-current:pay-tot-base .

        end. /*of rule 1526*/
      end. /*of rule 1526*/
                                                                      /* salience 6 rule_id 1527*/
      IF  p-r-b = "base" AND p-is-over = true  THEN do:   
/* salience 7 in upper-rule-id 1527*/
        _1528:
        do:
                                                                      /* Salience 0 rule_id 1528*/
         v-netto-current = Tot-sum-host-current:pay-tot-base_byf# .
                                                                      ...
                                                                      /* salience 14 rule_id 1525*/
IF  prev-sum-id <> "?" AND v-netto-prev = 0  THEN do:   
/* salience 15 in upper-rule-id 1525*/
  _1825:
  do:
                                                                      /* salience 0 rule_id 1825*/
...
/* salience 1 in upper-rule-id 1825*/
    _1826:
    do:
                                                                      /* Salience 0 rule_id 1826*/
     v-netto-prev = v-netto-current .
                                                                      ...
                                                                      /* Salience 16 rule_id 1525*/
/* define variable next-discount1 as class Next-discount_ no-undo .*/
                                                                      /* salience 17 rule_id 1525*/
...
/* salience 18 in upper-rule-id 1525*/
  _1544:
  do:
                                                                      /* salience 1 rule_id 1544*/
...
/* salience 2 in upper-rule-id 1544*/
    _1551:
    do:
                                                                      /* Salience 0 rule_id 1551*/
     undo _main, return error v-last-error-message .
                                                                      ...
                                                                      /* Salience 19 rule_id 1525*/
 next-discount1:category = dis-rule_00065( INPUT p-rule-num, INPUT v-current-doc-date, INPUT v-current-doc-time, INPUT v-netto-current, INPUT p-prev-rule-num, INPUT v-netto-prev) .
                                                                      /* salience 20 rule_id 1525*/
IF  next-discount1:next-discount_save( ) = false  THEN do:   
/* salience 21 in upper-rule-id 1525*/
  _1552:
  do:
                                                                      /* Salience 0 rule_id 1552*/
   undo _main, return error v-last-error-message .
