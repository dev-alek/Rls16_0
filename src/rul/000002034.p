...
/*                      ,input ? /*buffer для ошибок XML  - не нужен*/*/
/*                      ,input ? /*buffer для destination не нужен*/*/
                      ...
                      ,input lookup({&output-type-xml}, p-output-type) > 0
                      ,input entry(1, v-dirs, {&vertical-line}) /*p-dir-excel*/
                      ,input entry(2, v-dirs, {&vertical-line}) /*p-dir-xml*/
                      ,output v-dataseth
                      ,output v-xmlh
                      ) no-error.
  if error-status:error then do:
    ...
    v-rv = return-value .
    if p-ruleset-id = {&rep-proc_22_batchwork_1}
    then do:
