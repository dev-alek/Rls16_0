...
/* ------------------------- &end-hn-option -----------------------------------*/
  ...
  On error undo _d-card, retry _d-card:
    ...
    /* ------------------------- &start-rule& -----------------------------------*/
/* Категория скидки по выборке товаров -> текущ. категория скидки
Алгоритмы зависимости скидок от суммы оплаченных товаров по выборке товаров */

_1663:
do:
                                                                      /* Salience 0 rule_id 1663*/
/* define variable Card1 as class Dis-card_ no-undo .*/
                                                                      /* Salience 2 rule_id 1663*/
/* define variable Sel-goods-discnt1 as class sel-goods-discount_ no-undo .*/
                                                                      /* salience 4 rule_id 1663*/
IF  Card1:find_dis-card_( INPUT v-current-d-card) = true  THEN do:   
/* salience 5 in upper-rule-id 1663*/
  _1664:
  do:
                                                                      /* salience 0 rule_id 1664*/
  IF  Sel-goods-discnt1:find_sel-goods-discount_sum-id( INPUT v-current-d-card, INPUT p-sum-id, INPUT '':U) = true  THEN do:   
/* salience 1 in upper-rule-id 1664*/
    _1665:
    do:
                                                                      /* Salience 0 rule_id 1665*/
     Card1:category = Sel-goods-discnt1:category .
                                                                      /* salience 1 rule_id 1665*/
    IF  Card1:dis-card_save( ) = false  THEN do:   
/* salience 2 in upper-rule-id 1665*/
      _1666:
      do:
                                                                      /* Salience 0 rule_id 1666*/
