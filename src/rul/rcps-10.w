...
Задание и просмотр параметров вызова правил для профайла 10
...
define variable vss-description as character no-undo init "Задание и просмотр параметров вызова правил для профайла 10".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i }
...
&scop dis-rule-templ-rl-root 65
&scop rule-sale-in  1561
&scop rule-sale-out 1561
&scop rule-trn-in   1561
&scop rule-trn-out  1561
&scop rule-recalc   1561
...
f-rule-num b-dis-rule b-lkp-dis-rule f-prev-rule-num b-prev-dis-rule ~
b-lkp-prev-dis-rule f-caller-id f-sum-id b-sum-id E-rules T-rule-sale-out ~
T-rule-trn-out T-rule-recalc T-rule-sale-in T-rule-trn-in BR-rcp
&Scoped-Define DISPLAYED-OBJECTS rs-r-b t-is-over f-rule-num ~
f-prev-rule-num f-caller-id f-sum-id f-nd-caller-id E-rules T-rule-sale-out ~
T-rule-trn-out T-rule-recalc T-rule-sale-in T-rule-trn-in l-r-b ~
f-dis-rule-name f-prev-dis-rule-name
...
DEFINE VARIABLE f-nd-caller-id AS CHARACTER FORMAT "X(256)":U
     LABEL "Доп.метка среза катег в след. периоде (с типом Период Дат)"
...
DEFINE VARIABLE f-prev-rule-num AS INTEGER FORMAT ">>>>>>9":U INITIAL 0
     LABEL "Правило зависимости СУММА НАКОПЛЕНИЙ  -> КАТЕГОРИЯ ДК в предыд. периоде"
     VIEW-AS FILL-IN
     SIZE 16 BY 1 NO-UNDO.

DEFINE VARIABLE f-rule-num AS INTEGER FORMAT ">>>>>>9":U INITIAL 0
     LABEL "Правило зависимости СУММА НАКОПЛЕНИЙ  -> КАТЕГОРИЯ ДК в текущем периоде"
     VIEW-AS FILL-IN
     SIZE 16 BY 1 NO-UNDO.

DEFINE VARIABLE f-sum-id AS CHARACTER FORMAT "X(256)":U
     LABEL "Идентификатор среза категории в следующем периоде"
     VIEW-AS FILL-IN
...
DEFINE FRAME Dialog-Frame
     ...
     f-nd-caller-id AT ROW 9.53 COL 2 WIDGET-ID 146
...
/* SETTINGS FOR FILL-IN f-nd-caller-id IN FRAME Dialog-Frame
   NO-ENABLE ALIGN-L                                                    */
/* SETTINGS FOR FILL-IN f-prev-dis-rule-name IN FRAME Dialog-Frame
   NO-ENABLE ALIGN-L                                                    */
...
&Scoped-define SELF-NAME b-lkp-prev-dis-rule
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL b-lkp-prev-dis-rule Dialog-Frame
...
&Scoped-define SELF-NAME b-prev-dis-rule
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL b-prev-dis-rule Dialog-Frame
ON CHOOSE OF b-prev-dis-rule IN FRAME Dialog-Frame /* Btn 1 */
DO:
DEFINE VARIABLE v-rid-list AS CHARACTER NO-UNDO.
DEFINE VARIABLE v-sts AS INTEGER NO-UNDO.
DEFINE BUFFER buf_dis-rule FOR ub.dis-rule.
...
RUN rcps_get-value IN THIS-PROCEDURE (
                                ...
                                ,output v-value-date /*p-value-date*/
                                ,output v-value-decimal /*p-value-decimal*/
                                ,output f-prev-rule-num /*p-value-integer*/
                                ,output v-value-logical /*p-value-logical*/
                                ).
...
b-prev-dis-rule WHEN p-mode <> {&LOOKUP}
...
RUN rcps_set-value IN THIS-PROCEDURE (
                                ...
                                ,INPUT ? /*p-value-date*/
                                ,INPUT 0.0 /*p-value-decimal*/
                                ,INPUT f-prev-rule-num /*p-value-integer*/
                                ,INPUT no /*p-value-logical*/
                                ).
