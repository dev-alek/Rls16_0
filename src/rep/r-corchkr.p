...
$Workfile: r-corchkr.p $
$Archive: rep/r-corchkr.p $

Промежуточный вызов отчета по общему отчету по чекам транзакции
...
define variable vss-workfile    as character no-undo init "$Workfile: r-corchkr.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-corchkr.p $":U .
define variable vss-description as character no-undo init "Промежуточный вызов отчета по общему отчету по чекам коррекции".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/library.i  }
{ cmp/showinf.i  }
