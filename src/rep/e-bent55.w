...
Состояние запаса по объектам в Excel для всех
...
def var vss-description as character no-undo init "Состояние запаса по объектам в Excel для всех (закладка № 2)".
{ cmp/vssrevis.i }
...
{ gbl/hostname.i v-cntxt-obj-type v-cntxt-obj-code  g#host-code g#host-name }

  DEFINE VARIABLE base-type    AS CHARACTER NO-UNDO.
  define variable base-code    as integer   no-undo .
  { gbl/basecode.i g#host-code base-code }
  find first currency no-lock where  currency.curr-code = base-code .
  assign base-type = currency.curr-abbr .
