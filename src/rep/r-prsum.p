...
define variable vss-workfile    as character no-undo init "$Workfile: r-prsum.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-prsum.p $":U .
...
run factord-end-day in this-procedure
 ...
 define buffer buf_sum-group for ub.sum-group  .
    /* Количественные группы {&bef-mpl-sum} */
        find first buf_sum-group no-lock where recid(buf_sum-group) = p-recid no-error .
        if available buf_sum-group then assign
            v-name =  buf_sum-group.name
          .
 ...
 /* создаем временный файл */
    ...
    for each obj-list:
      ...
      find ub.clients      where ub.clients.obj-type = {&cmp}                and
                              ub.clients.obj-code = v-cntxt-host-code-obj no-lock .
...
define output parameter ii as integer   no-undo  .
  ...
  :
    ...
    for each buf_sum-in-sum-group no-lock where
              ...
              break by buf_sum-in-sum-group.ssg-sum
              :
              find first  last_sum-in-sum-group no-lock  where
                          ...
                          last_sum-in-sum-group.ssg-sum   > buf_sum-in-sum-group.ssg-sum
        ...
        create  temp-header.
          assign
          temp-header.id     = ii
          temp-header.first_ = buf_sum-in-sum-group.ssg-sum
          temp-header.to_  = if available last_sum-in-sum-group then  last_sum-in-sum-group.ssg-sum   else ?
          temp-header.last_  = if available last_sum-in-sum-group then ( last_sum-in-sum-group.ssg-sum - 1 )  else ?
          temp-header.name   = "не менее " + string( buf_sum-in-sum-group.ssg-sum )
...
define variable v-i as integer   no-undo .
  ...
  for each buf_price-all no-lock where
          ...
          :
          if buf_price-all.main-indication = {&bef-mpl-sum} then do:
             find first temp-header where
                        temp-header.first_ = buf_price-all.sum-from and
                        temp-header.to_    = buf_price-all.sum-to   no-error .
