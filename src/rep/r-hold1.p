...
define variable vss-workfile    as character no-undo init "$Workfile: r-hold1.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-hold1.p $":U .
define variable vss-description as character no-undo init "динамика продаж".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/r-page1.i  }
{ rep/r-sym.i    }
...
:
  ...
  DEFINE temp-table temp-hold1 no-undo
    field   artic            as char
  ...
  define variable  grp-value   as decimal  EXTENT 15       no-undo.
  define variable  all-value   as decimal  EXTENT 15       no-undo.
  ...
  for each temp-hold1 : delete temp-hold1 . end.
  ...
  for each buf_hold-time no-lock where buf_hold-time.cat-code = {&hold-main-cat-code} :
    ...
    for each buf_hold-sale no-lock
      ...
      find first temp-hold1 where temp-hold1.gds-code = buf_hold-sale.gds-code  no-error .

      if not available temp-hold1 then do:
        create temp-hold1 .

        run grplib-get-full-name in this-procedure ( input buf_goods.grp-code,output temp-hold1.grp-name) .
        if g#gds-engl then assign temp-hold1.gds-name = buf_goods.engl-name.
        else               assign temp-hold1.gds-name = buf_goods.gds-name.

        assign
          temp-hold1.artic     = buf_goods.artic
          temp-hold1.prod-type = buf_goods.prod-type
          temp-hold1.prod-code = buf_goods.prod-code
          temp-hold1.gds-unit = buf_goods.unit-base
          temp-hold1.gds-code = buf_goods.gds-code
          temp-hold1.sum-naz  = 0
          temp-hold1.sum-prod = 0
        .
        do ii = 1 to 12 : assign temp-hold1.qnty [ii] = 0 . end.
      end.
      do ii = 1 to 12 :
        if buf_hold-time.start-date >= date-start [ii] and buf_hold-time.end-date <= date-end [ii] then do:
          assign temp-hold1.qnty [ii] = temp-hold1.qnty [ii] - buf_hold-sale.fact-qnty .
          leave.
        end.
      end.
      if x-SET_val_TYPE = 1 then do:
        assign
          temp-hold1.sum-naz   = temp-hold1.sum-naz  - (buf_hold-sale.sale-sum-rubl - buf_hold-sale.purch-sum-rubl)
/*          temp-hold1.sum-naz   = temp-hold1.sum-naz  + buf_hold-sale.profit-rubl*/
          temp-hold1.sum-prod  = temp-hold1.sum-prod - buf_hold-sale.sale-sum-rubl
        .
      end.
      else do:
        assign
          temp-hold1.sum-naz   = temp-hold1.sum-naz  - (buf_hold-sale.sale-sum-base - buf_hold-sale.purch-sum-base)
/*          temp-hold1.sum-naz   = temp-hold1.sum-naz  + buf_hold-sale.profit-base*/
          temp-hold1.sum-prod  = temp-hold1.sum-prod - buf_hold-sale.sale-sum-base
        .
      end.
    end.
  end.
  ...
  DEFINE frame f-doc
        sym19  temp-hold1.artic  column-label " Артикул! " format "X(12)"         space(0)
        ...
        sym17 v-rent         column-label "%    !рентаб."          format "->>>9.99"      space(0)
        sym18
  HEADER
        ...
        Line format "X(287)" AT 1
  with width 290 /*{&DOS_CW}*/ down stream-io.

  { cmp/open-out.i stream OutStream " " {&LS_PS_A4} }

  FORM HEADER
      Line format "X(287)" AT 1 SKIP
      "Продолжение - на следующей странице" AT 30 SKIP
      with FRAME BottomFrame width 290 /*{&DOS_CW}*/ PAGE-BOTTOM NO-LABELS NO-BOX .
  ...
  for each temp-hold1 no-lock
    break by temp-hold1.grp-name
    :
    ...
    if first-of(temp-hold1.grp-name) then do:
      run StartGroup in this-procedure .
    end.

    run PrintLine in this-procedure .

    if last-of(temp-hold1.grp-name) then do:
      run EndGroup in this-procedure .
    end.
  end. /* for each temp-hold1  */
...
procedure PutColumnTitulExcel : /* заголовки для колонок экселя */
  ...
  run macr_excel_char("% рентаб.", 3, 19) .

  run macr_cell_bordur ( 3, 1, 3, 19) .
  run macr_cell_format ( 10, yes, no, 35, 3, 1, 3, 19) .
  run macr_cell_size (12,?, 3, 3, 3, 19) .
...
procedure StartGroup :
  ...
  :
      ...
      assign v-goods-name = temp-hold1.grp-name .
      display stream outstream  sym1 v-goods-name sym2 sym3 sym4 sym5 sym6 sym7 sym8 sym9 sym10 sym11 sym12 sym13 sym14 sym15 sym16 sym17 sym18  sym19  sym20 with frame f-doc.
      down stream outstream with frame f-doc .

      run macr_excel_char(temp-hold1.grp-name, v-row, 3) .
...
procedure EndGroup :
  ...
  :
      ...
      assign
        v-goods-name = "Итого по группе: " + temp-hold1.grp-name
        v-sum-value  = grp-value[13]
        v-sum        = grp-value[14]
        v-rent       = grp-value[15] * 100 / (grp-value[14] - grp-value[15])
      .
      ...
      run macr_excel_sum  ( v-rent , v-row, 19, 2) .
      assign v-row = v-row + 1 .
  end.
...
procedure PrintLine :
  ...
  :
    ...
    do ii = 1 to 12:
      assign
        grp-value[ii] = grp-value[ii] + temp-hold1.qnty[ii]
        v-value[ii]   = temp-hold1.qnty[ii]
        v-sum-value   = v-sum-value + temp-hold1.qnty[ii]
      .
    end.
    assign
      v-goods-name = temp-hold1.gds-name
      v-goods-unit = temp-hold1.gds-unit
      v-prod = temp-hold1.prod-type + " " + string(temp-hold1.prod-code)
      v-sum        = temp-hold1.sum-prod
      v-rent       = temp-hold1.sum-naz * 100 / (temp-hold1.sum-prod - temp-hold1.sum-naz)
      grp-value[13] = grp-value[13] + v-sum-value
      grp-value[14] = grp-value[14] + v-sum
      grp-value[15] = grp-value[15] + temp-hold1.sum-naz
    .
    if v-rent = ? then assign v-rent = 0 .

    display stream outstream  sym1    v-goods-name  temp-hold1.artic   v-prod
                              sym2    v-goods-unit
                              sym3    v-value[1]
                              sym4    v-value[2]
                              sym5    v-value[3]
    ...
    down stream outstream with frame f-doc .
      ...
      run macr_excel_char(temp-hold1.artic, v-row, 1) .
...
procedure PrintItog :
  ...
  assign
    ...
    v-sum        = all-value[14]
    v-rent       = all-value[15] * 100 / (all-value[14] - all-value[15])
  .
  if v-rent = ? then assign v-rent = 0 .
  do ii = 1 to 12:
  ...
  PUT STREAM OutStream Line format "X(287)".
