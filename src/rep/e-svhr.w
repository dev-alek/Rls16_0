...
Почасовая статистика розничных продаж по ВЕЛИЧИНЕ СУММ ПРОДАЖ
...
&Scoped-Define ENABLED-OBJECTS T-dis-card BR-dis-card-type RS-OPTION B-vals ~
Tree t-byobject Rets
&Scoped-Define DISPLAYED-OBJECTS T-dis-card RS-dis-card RS-OPTION Tree ~
t-byobject Rets
...
&Scoped-define SELF-NAME B-vals
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL B-vals F-Frame-Win
ON CHOOSE OF B-vals IN FRAME F-Main /* Суммы */
DO:
  run str/sum-vals.w .
END.
...
DO:
    assign RS-option.
    if RS-option <> "LINE":U then disable Tree with FRAME {&frame-name}.
    else enable Tree with FRAME {&frame-name}.
...
then do:
   ...
   run waitfram-show in this-procedure ( "Подождите ..." ).
    ...
    end. /*if Rs-Option = "Строки чеков" или ОПЛАТЫ*/
      else do:   /*чеки*/
        run rep/r-svhr.p (
                         input RS-option
                        ,input (if x-SelectObject <> {&obj-currency} and t-byobject then yes else no)
...
ReportHeader =  radio-label(string(Rs-Option), RS-Option:radio-buttons) + ", "
                            ...
                            (IF t-byobject THEN t-byobject:LABEL IN FRAME {&FRAME-NAME} ELSE '':U) + ", " +
