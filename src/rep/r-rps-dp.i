...
def input parameter p7 as char no-undo.
 CASE CAPS(p7) :
   WHEN "P":U  Then
             { rep/di-obrt.i 'oborot-ras' ->>>>>>>>>>>>9.<< p-  }
   WHEN "O":U  Then
             { rep/di-obrt.i 'oborot-ras' ->>>>>>>>>>>>9.<< o-  }
   WHEN "B1":U  Then
             { rep/di-obrt.i 'oborot-ras' ->>>>>>>>>>>>9.<< b1-  }
   WHEN "B2":U  Then
             { rep/di-obrt.i 'oborot-ras' ->>>>>>>>>>>>9.<< b2-  }
   WHEN "BI":U Then
             { rep/di-obrt.i 'oborot-ras' ->>>>>>>>>>>>9.<< bi-  }
   WHEN ""  Then DO:
             { rep/di-obrt.i 'oborot-ras'  ->>>>>>>>>>>>9.<<     }
              End.
   End case.
               {&FRAME-d}.
 END PROCEDURE.
...
def input parameter p7 as char no-undo.
 CASE CAPS(p7) :
   WHEN "O":U  Then DO :
              { rep/di-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< o-}
              { rep/ex-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< o-}
                End.
   WHEN "P":U  Then DO :
             { rep/di-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< p-}
             { rep/ex-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< p-}
             End.
   WHEN "B1":U  Then DO :
              { rep/di-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< b1-}
              { rep/ex-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< b1-}
                End.
   WHEN "B2":U  Then DO :
             { rep/di-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< b2-}
             { rep/ex-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< b2-}
             End.
   WHEN "BI":U Then  DO :
             { rep/di-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< bi-}
             { rep/ex-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< bi-}
             End.
   WHEN ""  Then     DO :
              { rep/di-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< }
              { rep/ex-obrt.i 'oborot-ras'  ->>>>>>>>>>>9.<<< }
              End.
   End case.
               {&FRAME-d}.
 END PROCEDURE.
...
def var tt-sale like  ub.gds-dtl.price-rubl no-undo .
 ...
 if (x-sum-type = {&arh-crsa}  or x-sum-type = {&arh-crsa-service}) then tt# = 6.
...
/*
              ...
              find first ub.gds-dtl no-lock where
                    ub.gds-dtl.artic        = ub.parts.artic
              AND   ub.gds-dtl.prt-code     = gds-zap-prt-root
              AND   ub.gds-dtl.prod-code    = ub.parts.prod-code
              AND   ub.gds-dtl.prod-type    = ub.parts.prod-type
              AND   ub.gds-dtl.obj-code     = ub.parts.obj-code
              AND   ub.gds-dtl.obj-type     = ub.parts.obj-type
              AND   ub.gds-dtl.doc-code     = ub.parts.out-code .

tt-vat  = if tprintrubl then  ub.gds-dtl.VAT-rubl else ub.gds-dtl.VAT-base.
tt-cost = if tprintrubl then  ub.gds-dtl.price-rubl * ub.gds-dtl.fact-qnty ELSE  ub.gds-dtl.price-base * ub.gds-dtl.fact-qnty.
tt-sale = ub.gds-dtl.cur-base * ub.gds-dtl.fact-qnty.
