...
$Workfile: r-hold7.p $
$Archive: rep/r-hold7.p $

Отчет по межфирменным операциям - динамика списани
...
define variable vss-workfile    as character no-undo init "$Workfile: r-hold7.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-hold7.p $":U .
define variable vss-description as character no-undo init "Отчет по межфирменным операциям - динамика списани ".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/r-page1.i  }
{ rep/r-sym.i    }
...
:
  ...
  for each buf_hold-time where buf_hold-time.cat-code = {&hold-spi-cat-code}  no-lock  :
