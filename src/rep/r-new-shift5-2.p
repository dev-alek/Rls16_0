...
define output parameter p-report-json as character no-undo .
...
&scop All-sym6 sym1 sym2 sym3 sym4 sym5 sym6
    ...
    /* собирание данных */
find first obj-list no-lock where obj-list.obj-code = p-obj-code and
obj-list.obj-type = p-obj-type no-error .
if not available (obj-list) then do:
    create obj-list .
    assign
    obj-list.obj-code = p-obj-code
    obj-list.obj-type = p-obj-type
    .
end.
    /*Определение фирмы*/
    ...
    p-report-json = session:temp-directory + {&DF_Name} + string(p-report-id) + string(time) + "json.html". /*формирование имя файла для часть1*/ 
    ...
    output to value(p-report-json) convert target "UTF-8".
    put unformatted
        CHR(123) '"shift-report-TH":' CHR(123) skip 
        '"part-5": [' skip
        . 
    ...
    for each temp-fin-doc:
        put unformatted
            CHR(123) skip
            '"kass-book":"' string(temp-fin-doc.ext-code) '",' skip 
            '"balance-start-shift":"' string(temp-fin-doc.ost-begin) '",' skip 
            '"income":"' string(temp-fin-doc.income-realiZ) '",' skip 
            '"income-other":"' string(temp-fin-doc.income-other) '",' skip 
            '"expenditure":"' string(temp-fin-doc.expense-bank) '",' skip 
            '"expenditure-other":"' string(temp-fin-doc.expense-other) '",' skip 
            '"balance-end-shift":"' string(temp-fin-doc.ost-end) '",' skip 
            '}' skip .
        . 
    ...
    do:
        for each buf_cashbook no-lock where buf_cashbook.Status_ = 0:
                        ...
                        do :
                            ...
                            do:
                                ...
                                do:  
                                    ...
                                    if available (ub.CashBookRule) and buf_fin-doc.payer-type = {&cmp} then 
                                    do:
                                        assign
                                            v-income-realiZ = v-income-realiZ + buf_fin-doc.sum-doc
                                            .
            ...
            assign
                ...
                temp-fin-doc.ext-code      = buf_cashbook.ext-code
                temp-fin-doc.ost-begin     = v-ost-begin
                temp-fin-doc.income-realiZ = v-income-realiZ
                temp-fin-doc.income-other  = v-income-other
                temp-fin-doc.expense-bank  = v-expense-bank
