...
define variable vss-workfile    as character no-undo init "$Workfile: r-slg-k.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-slg-k.p $":U .
...
DEFINE temp-table temp-DiscSales no-undo
  ...
  define variable v-sum-del     as decimal   no-undo .
  ...
  if x-SelectGood = {&g-all} then do: /* все товары */
    for each buf_goods no-lock :
      for each obj-list :
        ...
        { rep/r-slg-k1.i } /* смотрим, были ли продажи и кладем в темп-тейбл  */
      end.
    end.
  end.
  else do:
...
/*  { rep/repfrm.i off } /* убрать окно информации о текущем процессе */*/
  ...
  DEFINE frame f-doc
      sym1  v-b-code     column-label " Код!        "          format ">>>>>>>>>>>>>>>9"  space(0)
      sym2  v-NameString column-label " Наименование товара! " format "X(50)"             space(0)
      sym3  v-unit-base  column-label "Ед.!изм!"               format "X(3)"              space(0)
      sym4  v-qnty       column-label "Кол-во  "               format ">>,>>9.999"        space(0)
      sym5  v-sum-sale   column-label "Сумма в  !розн. ценах"  format ">,>>>,>>9.99"      space(0)
      sym6  v-sum-cost   column-label "Сумма в  !учет. ценах"  format ">,>>>,>>9.99"      space(0)
      sym7  v-sum-del    column-label "Сумма -  !разница  "    format ">,>>>,>>9.99"      space(0)
      sym8
  HEADER
      string( "Дата печати : " + string(TODAY , "99.99.9999") + " , " + string(TIME, "HH:MM") ) AT 5 format "X(70)"
      string( "Страница " + string( PAGE-NUMBER( OutStream )  , ">>9") ) AT 110 format "X(15)" SKIP
  ...
  PUT stream OutStream SPACE(30) "Анализ по себестоимости с: " x-date-start format "99/99/9999" "г. по: " x-date-end format "99/99/9999" "г." SKIP .
...
/*        and   begins temp-DiscSales.full-grp-name*/
      ...
      if titul = yes then do:
        ...
        .
        display stream outstream sym1 sym2 v-NameString sym3 sym4 sym5 sym6 sym7 sym8 with frame f-doc.
      ...
      assign
        v-b-code     = temp-DiscSales.b-code
        v-NameString = temp-DiscSales.gds-name
        v-unit-base  = temp-DiscSales.unit-base
        v-qnty       = temp-DiscSales.qnty
        v-sum-sale   = temp-DiscSales.sum-sale
        v-sum-cost   = temp-DiscSales.sum-cost
        v-sum-del    = temp-DiscSales.sum-del
      .
      ...
      if last-of(temp-DiscSales.full-grp-name) then do:
        ...
        assign
          v-NameString = "Итого по группе " + temp-DiscSales.grp-name + ":"
          v-qnty       = cur-qnty
          v-sum-sale   = cur-sum-sale
          v-sum-cost   = cur-sum-cost
          v-sum-del    = cur-sum-cost - cur-sum-sale
          all-qnty     = all-qnty     + cur-qnty
          all-sum-sale = all-sum-sale + cur-sum-sale
          all-sum-cost = all-sum-cost + cur-sum-cost
        .
        display stream outstream sym1 sym2 v-NameString sym3 sym4 v-qnty sym5 v-sum-sale sym6 v-sum-cost sym7 v-sum-del sym8 with frame f-doc.
        down stream outstream with frame f-doc .
      End.
    End.
    ...
    do ii = v-old-level to ind by -1 : /* удаляем старые заголовки из списка */
      ...
      if available temp-grp-sum then do:
        assign
          ...
          v-qnty       = temp-grp-sum.qnty
          v-sum-sale   = temp-grp-sum.sum-sale
          v-sum-cost   = temp-grp-sum.sum-cost
          v-sum-del    = temp-grp-sum.sum-cost - temp-grp-sum.sum-sale
        .
        display stream outstream sym1 sym2 v-NameString sym3 sym4 v-qnty sym5 v-sum-sale sym6 v-sum-cost sym7 v-sum-del sym8 with frame f-doc.
        down stream outstream with frame f-doc .
      end.
    ...
    if titul = no then do:
      assign
        ...
        v-qnty       = all-qnty
        v-sum-sale   = all-sum-sale
        v-sum-cost   = all-sum-cost
        v-sum-del    = all-sum-cost - all-sum-sale
      .
      display stream outstream sym1 sym2 v-NameString sym3 sym4 v-qnty sym5 v-sum-sale sym6 v-sum-cost sym7 v-sum-del sym8 with frame f-doc.
      down stream outstream with frame f-doc .
      display stream outstream sym1 sym2 sym3 sym4 sym5 sym6 sym7 sym8 with frame f-doc.
      down stream outstream with frame f-doc .
    end.
  End.

  PUT STREAM OutStream Line format "X(130)".
...
procedure GrpSumTree :
  v-level      = num-entries( right-trim(temp-DiscSales.full-grp-name, {&delim-grp}), {&delim-grp} ) - 1 .

  assign CurrGrpName = "" .
  do ind = 1 to v-level :
    assign CurrGrpName = CurrGrpName + entry ( ind, temp-DiscSales.full-grp-name, {&delim-grp} )  + {&delim-grp}.
    find first temp-grp-sum
  ...
  do ii = v-old-level to ind by -1 : /* удаляем старые заголовки из списка */
/*  do ii = ind to v-old-level : /* удаляем старые заголовки из списка */*/
    find first temp-grp-sum
...
/*    down stream outstream with frame f-doc .*/
    create temp-grp-sum .
    if ii > ind then do:
      assign CurrGrpName = CurrGrpName + entry ( ii, temp-DiscSales.full-grp-name, {&delim-grp} )  + {&delim-grp}.
    end.
    assign
      temp-grp-sum.num = ii
...
procedure CalculSum :
  ...
  if p-num = 0 then do:
    ...
    assign
      ...
      buf_temp-grp-sum.sum-del  = buf_temp-grp-sum.sum-del  + cur-sum-cost - cur-sum-sale
    .
  end.
