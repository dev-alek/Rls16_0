...
Прайс-лист по количественной группе (закладка № 2)
...
def var vss-description as character no-undo init "Прайс-лист по количественной группе (закладка № 2)".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/r-page1.i  }
{ rep/rep-bt.i   }
...
&Scoped-Define ENABLED-OBJECTS RECT-5 RECT-6 Classify SortType
...
/* SETTINGS FOR TOGGLE-BOX TOG-prt IN FRAME F-Main
...
------------------------------------------------------------------------------*/
  ...
  case v-type :
    when "2" then do:
        run rep/r-prqnty.p  /* По количественным группам */
            ( v-tmp,
              p-curr-code,
              classify,
              sorttype
    ...
    when "3" then do:
        run rep/r-prsum.p  /* По суммовым группам */
            ( v-tmp,
              p-curr-code,
              classify,
              sorttype
    ...
    when "4" then do:
        run rep/r-prtnv.p  /* По группам оборотов */
            ( v-tmp,
              p-curr-code,
              classify,
              sorttype
    ...
    when "5" then do:  /* По группам покупателей */
        run rep/r-prcus.p
            ( v-tmp,
              p-curr-code,
              classify,
