...
Определение дополнительных параметров для ub.clients
...
&else
    case {1}clients.obj-type:
       when {&cmp}
       then do:
            FIND ub.firm WHERE ub.firm.firm-code = {1}clients.obj-code NO-LOCK .
       ...
       when {&shop}
       then do:
            FIND ub.shop WHERE ub.shop.obj-code = {1}clients.obj-code NO-LOCK .
       ...
       when {&stock}
       then do:
            FIND ub.store WHERE ub.store.obj-code = {1}clients.obj-code NO-LOCK .
       ...
       when {&prs}
       then do:
            find ub.person where ub.person.psn-code = {1}clients.obj-code no-lock .
