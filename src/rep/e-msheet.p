...
$Workfile: e-msheet.p $
$Archive: rep/e-msheet.p $

Запускалка отчета r-msheet.p
...
def var vss-workfile    as character no-undo init "$Workfile: e-msheet.p $":U .
def var vss-archive     as character no-undo init "$Archive: rep/e-msheet.p $":U .
def var vss-description as character no-undo init "Запускалка отчета r-msheet.p".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/r-page1.i }
...
run rep/r-msheet.p (input my-handle,
                input x-radio-task,
                input x-date-start,
                input x-shift-start,
                input x-date-end,
