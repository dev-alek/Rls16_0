...
/*define variable d_temp           as decimal   no-undo .*/
/*define variable d_temp1          as decimal   no-undo .*/
...
:
  ...
  */
  ...
  { gbl/ptrlprop.i run bf_trn-doc.obj-type bf_trn-doc.obj-code }
  ...
  :
    ...
    then do: /* если задана норма, то производится расчет естественной убыли */
      ...
      then do:
        find last bf_doc-line no-lock where
                  ...
                  bf_doc-line.ext-doc-type = {&TDEDT_Inv}          and
        ...
        find last bf_doc-line no-lock where
                  ...
                  bf_doc-line.artic        = bf_goods.artic        and
                  bf_doc-line.ext-doc-type = {&TDEDT_Inv}          and
                  bf_doc-line.status_      = {&fact}               use-index dt-fo no-error .
      end.
      ...
      for each bf_doc-line no-lock
        where ( bf_doc-line.obj-type         = bf_rvs-line.obj-type
              ...
              ( bf_doc-line.obj-type         = bf_rvs-line.obj-type
                ...
                and bf_doc-line.prod-code    = bf_goods.prod-code
                and bf_doc-line.artic        = bf_goods.artic
                and bf_doc-line.ext-doc-type = {&TDEDT_Ras_Vnesh_VP}
                and bf_doc-line.status_      = {&fact}
                and bf_doc-line.fact-order   > invent-fo
              )
      ...
      :
        ...
        if ptrlprop-expptrl = {&calc-petrol-volume}
        then do:
          assign
        ...
        if bf_doc-line.ext-doc-type = {&TDEDT_Pri_Vnesh} then do:
          assign
...
/*      .*/
    ...
    put stream s-out unformatted
      ...
      '            :        :                     :            :        :                     :            :        :            :        :' +
      string( OutQty( dLoss-qnty,  no  ),                   "x(12)":U ) + ":" + /* 21 */
      string( OutSum( dLoss-sum,   no  ),                   "x(7)":U  ) + ":" + /* 22 */
      string( OutQty( dNorm-qnty,  no  ),                   "x(12)":U ) + ":" + /* 23 */
      string( OutSum( dNorm-sum,   no  ),                   "x(8)":U  ) + ":" + /* 24 */
    ...
    run r-orsvxl-write-line-data in this-procedure
      ...
      , input OutDec( dLoss-qnty,  no  )                                        /* 21 */
      , input OutDec( dLoss-sum,   no  )                                        /* 22 */
      , input OutDec( dNorm-qnty,  no  )                                        /* 23 */
      , input OutDec( dNorm-sum,   no  )                                        /* 24 */
