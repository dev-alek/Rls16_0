...
define variable vss-workfile    as character no-undo init "$Workfile: r-prtnv.p $":U .
define variable vss-archive     as character no-undo init "$Archive: rep/r-prtnv.p $":U .
...
run factord-end-day in this-procedure
 ...
 define buffer buf_turnover-group for ub.turnover-group  .
    /* Количественные группы {&bef-mpl-tnv} */
        find first buf_turnover-group no-lock where recid(buf_turnover-group) = p-recid no-error .
        if available buf_turnover-group then assign
            v-name =  buf_turnover-group.name
          .
...
define output parameter ii as integer   no-undo  .
  ...
  :
    ...
    for each buf_tnv-in-turnover-group no-lock where
              buf_tnv-in-turnover-group.tog-db-num  = buf_turnover-group.tog-db-num  and
...
define variable v-i as integer   no-undo .
  ...
  for each buf_price-all no-lock where
          ...
          :
          if buf_price-all.main-indication = {&bef-mpl-tnv} then do:
             find first temp-header where
                        temp-header.first_ = buf_price-all.turnover-from and
                        temp-header.to_    = buf_price-all.turnover-to   no-error .
