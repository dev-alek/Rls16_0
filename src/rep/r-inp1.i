  ...
  &if "{1}" = "rubl" or "{1}" = "val" &then
    ...
    find first buf_gds-obj no-lock
      where buf_gds-obj.gds-code = buf_goods.gds-code
        and buf_gds-obj.obj-type = buf_trn-doc.obj-type
        and buf_gds-obj.obj-code = buf_trn-doc.obj-code
      no-error .
    if varr-b = "rubl":u then assign Up-fact = (buf_gds-obj.price-sale - buf_doc-line.price-rubl) / buf_doc-line.price-rubl * 100 .
                         else assign Up-fact = (buf_gds-obj.price-sale - buf_doc-line.price-base) / buf_doc-line.price-base * 100 .
    assign
    ...
    if empty-scale = no and PrintScale = yes then do: /*  шкала и ее печатаем */
      display stream out_stream
        sym1  Lines_Counter sym2 tb-code sym3 buf_goods.artic sym4 gds_name sym5 buf_goods.unit-base sym6 sym7 sym8 sym9 Up-fact sym10
      with frame {1}.
      down stream out_stream with frame {1} .
      run r-inpxl-write-line-data in this-procedure (
                  input Lines_Counter
                , input tb-code
      ...
      display stream out_stream
        sym1 Lines_Counter sym2 tb-code sym3 buf_goods.artic sym4 gds_name sym5 buf_goods.unit-base sym6  Price sym7 qnty sym8 stoim sym9 Up-fact sym10
      with frame {1}.
      down stream out_stream with frame {1} .
      run r-inpxl-write-line-data in this-procedure (
                  input Lines_Counter
    ...
    if empty-scale = no and PrintScale = yes then do: /*  шкала и ее печатаем */
      ...
      for each buf_gds-dtl no-lock
        ...
        display stream out_stream
          ...
          sym3 sym4    ('  /'+ buf_gds-prt.f-name)  @ gds_name
