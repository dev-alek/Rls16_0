...
$Workfile: r-outretu.p $
$Archive: rep/r-outretu.p $

Документ расхода, возврата и списания короткий (с упаковками)
...
procedure print-line :
  do on error undo, return error return-value :
    if PrintRubl then do :
      { rep/r-outrtu1.i rubl "r-outret" }
    end .
    else do :
      { rep/r-outrtu1.i val "r-outret" }
    end .
  end .
  ...
  do on error undo, return error return-value :
    ...
    if PrintRubl then do:
      display stream out_stream
        sym1 sym2 sym3 sym4 "ИТОГО: " @ gds_name sym5 sym6 sym7 all-qnty @ qnty sym8 all-qnty-up @ qnty-up sym9 all-stoim @ stoim /*sym9 all-SLT-sum @ SLT-sum*/ sym10
      with frame rubl.
      down stream out_stream with frame rubl .
    end.
    else do:
      display stream out_stream
        sym1 sym2 sym3 sym4 "ИТОГО: " @ gds_name sym5 sym6 sym7 all-qnty @ qnty sym8 all-qnty-up @ qnty-up sym9 all-stoim @ stoim /*sym9 all-SLT-sum @ SLT-sum*/ sym10 sym11
      with frame val.
    ...
    run outretxl-write-cell-data in this-procedure (
          input {&outretxl-f_PassFname}
        , input v-pass-fname
    ).
       run outretxl-write-cell-data in this-procedure (
          input {&outretxl-f_AcceptFname}
