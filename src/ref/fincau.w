...
История справочника аналитического учета (Фин.блок)
...
&SCOPED-DEFINE status-code STRING(buf_c-fin-code-an-uchet.status_ )
...
&Scoped-Define main-file ub.c-fin-code-an-uchet
...
&Scoped-define INTERNAL-TABLES temp-changes buf_c-fin-code-an-uchet
...
&Scoped-define FIELDS-IN-QUERY-BR-docs buf_c-fin-code-an-uchet.code-value buf_c-fin-code-an-uchet.descr buf_c-fin-code-an-uchet.fin-code buf_c-fin-code-an-uchet.host-code {&status-int-name} @ t-s buf_c-fin-code-an-uchet.level-1 "Уровень1" buf_c-fin-code-an-uchet.level-2 "Уровень2" buf_c-fin-code-an-uchet.level-3 "Уровень3"
&Scoped-define ENABLED-FIELDS-IN-QUERY-BR-docs
&Scoped-define SELF-NAME BR-docs
&Scoped-define QUERY-STRING-BR-docs FOR EACH buf_c-fin-code-an-uchet no-lock
&Scoped-define OPEN-QUERY-BR-docs OPEN QUERY {&SELF-NAME} FOR EACH buf_c-fin-code-an-uchet no-lock.
&Scoped-define TABLES-IN-QUERY-BR-docs buf_c-fin-code-an-uchet
&Scoped-define FIRST-TABLE-IN-QUERY-BR-docs buf_c-fin-code-an-uchet
...
/* Query rebuild information for BROWSE BR-docs
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH buf_c-fin-code-an-uchet no-lock.
     _END_FREEFORM
     _Query            is OPENED
*/  /* BROWSE BR-docs */
&ANALYZE-RESUME
...
&scop flt-open-open-query OPEN QUERY br-docs FOR EACH buf_c-fin-code-an-uchet

&scop flt-open-dyn_open-query  FOR EACH buf_c-fin-code-an-uchet

&scop flt-open-query-handle query br-docs:handle

&scop flt-open-find-buffer-name buf_c-fin-code-an-uchet
...
&scop flt-open-table-name buf_c-fin-code-an-uchet
...
&scop flt-open-find-buffer-def define buffer buf_c-fin-code-an-uchet for ub.c-fin-code-an-uchet.
...
define variable l-open-query as logical   no-undo .
      ...
      { gbl/fltopend.i
        &where-cond = " buf_c-fin-code-an-uchet.fin-code = par-fin-code and  buf_c-fin-code-an-uchet.host-code =  par-host-code  "
        &dyn_where-cond = "substitute ( ' buf_c-fin-code-an-uchet.fin-code = &1 and  buf_c-fin-code-an-uchet.host-code =  &2 ' , par-fin-code , par-host-code ) "
        &use-ind    = "  "
        &by         = "  " }
...
&scop  disp-field ~
  when "~{&field-name~}":U then do: ~
    ...
    temp-changes.v_old = string(buf_c-fin-code-an-uchet.~{&field-name~}) ~
    temp-changes.v_new = (if available new_c-fin-code-an-uchet  ~
                             then string(new_c-fin-code-an-uchet.~{&field-name~})  ~
                             else string(current_fin-code-an-uchet.~{&field-name~})) ~
    . ~
  end. ~
