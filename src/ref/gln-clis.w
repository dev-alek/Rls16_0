...
Дата создания: 06/25/10
Author: Bakhtadze Natalya
Creation date: 06/25/10
...
define variable vss-description as character no-undo init "GLN Коды клиентов".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i }
...
define variable filter-point     as character NO-UNDO INIT "gbl-clis".
define variable filter-label     as character NO-UNDO INIT "GLN Коды клиентов".
define variable filter-point0     as character NO-UNDO INIT "gbl-clis".
define variable filter-label0     as character NO-UNDO INIT "GLN Коды клиентов".
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME br-gln-cli

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES X_ext-classif X_clients

/* Definitions for BROWSE br-gln-cli                                    */
&Scoped-define FIELDS-IN-QUERY-br-gln-cli mark-string(recid(X_ext-classif), v-rid-list) X_ext-classif.charKEY_one X_clients.obj-type X_clients.obj-code X_clients.obj-name
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-gln-cli
&Scoped-define SELF-NAME br-gln-cli
&Scoped-define QUERY-STRING-br-gln-cli FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-gln-cli OPEN QUERY {&SELF-NAME} FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-gln-cli X_ext-classif X_clients
&Scoped-define FIRST-TABLE-IN-QUERY-br-gln-cli X_ext-classif
&Scoped-define SECOND-TABLE-IN-QUERY-br-gln-cli X_clients


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-br-gln-cli}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS b-quit B-mark B-sel b-add b-del b-cli ~
b-sch b-print B-Help br-gln-cli mark-num
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY br-gln-cli FOR X_ext-classif, X_clients SCROLLING.

&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE br-gln-cli
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-gln-cli Dialog-Frame _FREEFORM
  QUERY br-gln-cli NO-LOCK DISPLAY
      mark-string(recid(X_ext-classif), v-rid-list) COLUMN-LABEL "" FORMAT "X(1)"
X_ext-classif.charKEY_one  COLUMN-LABEL "GLN Код" FORMAT "X(13)"
...
DEFINE FRAME Dialog-Frame
     ...
     br-gln-cli AT ROW 2.87 COL 1.5 WIDGET-ID 100
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br-gln-cli B-Help Dialog-Frame */
ASSIGN
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE br-gln-cli
/* Query rebuild information for BROWSE br-gln-cli
     _START_FREEFORM
...
DO:
  ...
  if available X_ext-classif then do:
    ...
    loc#log = br-gln-cli:refresh() .

    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-gln-cli:select-next-row ().
        apply "VALUE-CHANGED" to br-gln-cli in frame {&frame-name}.
    end.
  ...
  apply "entry" to br-gln-cli in frame {&frame-name}.
END.
...
&scop flt-open-open-query         OPEN QUERY br-gln-cli FOR EACH X_ext-classif

&scop flt-open-dyn_open-query     FOR EACH X_ext-classif

&scop flt-open-query-handle      QUERY br-gln-cli:handle
...
{ gbl/fltopend.i
        &where-cond = " X_ext-classif.classif-subject = ~{&table_clients~} ~
                        and X_ext-classif.classif-name = ~{&extclass_clients_gln~} ~
        ...
        &dyn_where-cond = " substitute('X_ext-classif.classif-subject = &1&2&1 ~
                        ...
                        AND X_ext-classif.db-num = - 1', {&double-quote}, ~{&table_clients~}, ~{&extclass_clients_gln~})"
...
define buffer buf_clients for clients.
message
"Выберите клиента, для которого Вы хотите добавить код GLN"
view-as alert-box .
...
find first buf_clients where
       recid (buf_clients) = integer (v-rid-list) no-lock no-error.
...
run gbl/d-prompt.w (
    'title=':u + "Введите номер GLN" + '\':u
  + 'format=' + "9999999999999" + '\':u
  + 'type=' + {&type-dec} + '\':u
  + 'fillin_row=2\':u
  + 'fillin_col=4\':u
  + 'fillin_width=20\':u
...
run ref/extclas1.p ( INPUT {&add-def}
                  ...
                  ,INPUT {&extclass_clients_gln} /*p-classif-name*/
                  ,input (-1) /*p-db-num*/
                  ,input 0 /*p-key#_one*/
                  ,input 0 /*p-Key#_Two*/
                  ,input 0 /*p-key#_Three*/
                  ,input v-value /*p-CharKey_One */
...
if v-rid <> ? then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-gln-cli to recid v-rid no-error .
end.
apply "entry" to br-gln-cli in frame {&frame-name} .
END PROCEDURE.
...
if not error-status:error then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-gln-cli to row 1 no-error .
  apply "entry" to br-gln-cli in frame {&frame-name} .
end.
END PROCEDURE.
...
run fltfield-add in this-procedure('charkey_one', 'GLN Код клиента', '',
...
DO on stop undo, leave:
    ...
    run OpenBr IN THIS-PROCEDURE ( INPUT YES, INPUT NO, INPUT '':U).
    if v-ri <> ? then do:
      reposition br-gln-cli to recid v-ri no-error.
    end.
    APPLY "ENTRY" to br-gln-cli in frame {&frame-name} .
    APPLY "VALUE-CHANGED" to br-gln-cli.
END .
