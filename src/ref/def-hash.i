...
procedure def-hash :
  ...
  :
  define input  parameter p-full-string as character no-undo .              /* строка для хеширования */
...
end procedure. /* def-hash */


procedure find-from-hash : /*============================================================================================*/
/* Поиск по любым таблицам с hash */
  do
  on error undo, return error return-value
  :
define input  parameter  p-full-string            as character no-undo . /* полная строка сравнения ( РТД или Объекты - через , )*/
define input  parameter  p-table-name             as character no-undo . /* имя таблицы, где будем искать */
define input  parameter  p-field-possb-keep-name  as character no-undo . /* имя поля в этой таблице <<POSSB-KEEP >>  */
define input  parameter  p-field-string-name      as character no-undo . /* имя поля в этой таблице <<STRING     >>  */
define input  parameter  p-field-hash-string-name as character no-undo . /* имя поля в этой таблице <<HASH-STRING>>  */
define input  parameter  p-sub-table-name         as character no-undo . /* имя связной таблицы , где расшифровка hash */
define output parameter  p-recid                  as recid     no-undo . /* recid найденной записи в род.таблице  p-table-name*/

define variable v-possb-keep-string as logical   no-undo .      /* yes/no ~ possb-keep  */
define variable v-string            as character no-undo .      /* 150    ~ string      */
define variable v-hash-string       as character no-undo .      /* 16     ~ hash-string */
...
create query qh.
 ...
 /* message v-query-prepare. */

/* 1 - все уместилось в строке */
...
end procedure. /* find-hash */



procedure ver-sub-table :
/* проверка хэш по дочерней таблице */
  do
  on error undo, return error return-value
  :
...
/* Найдем уникальный ключ этой записи */
/*
...
PRIMARY           (0/1 )
word              (0/1 )
[название поле в ключе ( char) , desc (0/1) ....]
...
DO while ( v-inf-ind <> ? )
    ...
    if entry( 1 , v-inf-ind ) = v-name-pi then do:
       v-num-fl-inkey = ( num-entries(v-inf-ind) - 4 ) / 2 .  /* кол-во полей в ключе без описания*/
...
/* сравнить строку и дочернюю таблицу */
...
define variable k as integer   no-undo init 0 .
    ...
    qh-sub:query-open.
      ...
      /* сбор строки по дочерней таблице */
...
if not available buf_rang-abc-def then do:
    v-exist = false  .
    assign
      p-id = next-value(s-asmt, {&db-name_schema})
      p-db = g#db-num
    .
...
/* Поиск анализа по умолчанию на 1 объекте */
PROCEDURE find-def-analysis-obj :
define input  parameter p-type     as character no-undo .        /* тип анализа  */
...
define output parameter v-log      as logical   no-undo .
  ...
  if p-type = "abc"  then do:
      ...
      for each buf_abc-obj no-lock where                /* список объектов в анализе */
               buf_abc-obj.abc-id = p-abc-id and
               buf_abc-obj.db-num = p-db-num :

            /* проверим может на этом объекте  уже есть анализ по умолчанию */
  ...
  if p-type = "xyz"  then do:
      ...
      for each buf_xyz-obj no-lock where                /* список объектов в анализе */
...
end procedure. /* save-def-analysis-obj */