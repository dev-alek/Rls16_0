...
&Scoped-define INTERNAL-TABLES temp-changes X_c-cli-hist
...
/* Definitions for BROWSE br-cli-hist                                   */
&Scoped-define FIELDS-IN-QUERY-br-cli-hist mark-string(recid(X_c-cli-hist), v-rid-list) X_c-cli-hist.corr-date string(X_c-cli-hist.corr-time, "HH:MM:SS":U) usrfulnf(X_c-cli-hist.corr-user-name) get-action(X_c-cli-hist.action) X_c-cli-hist.corr-user-db-num X_c-cli-hist.is-news get-source-type(X_c-cli-hist.source-type) X_c-cli-hist.source-ref X_c-cli-hist.obj-type + string(X_c-cli-hist.obj-code) if v-find then get-cli(X_c-cli-hist.obj-type, X_c-cli-hist.obj-code) else "":U get-subject(X_c-cli-hist.subject) X_c-cli-hist.host-code   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-cli-hist X_c-cli-hist.corr-date   
&Scoped-define ENABLED-TABLES-IN-QUERY-br-cli-hist X_c-cli-hist
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br-cli-hist X_c-cli-hist
&Scoped-define SELF-NAME br-cli-hist
&Scoped-define QUERY-STRING-br-cli-hist FOR EACH X_c-cli-hist NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-cli-hist OPEN QUERY {&SELF-NAME} FOR EACH X_c-cli-hist NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-cli-hist X_c-cli-hist
&Scoped-define FIRST-TABLE-IN-QUERY-br-cli-hist X_c-cli-hist
...
B-lookup b-reason B-print B-sch B-Help br-cli-hist sch-db-num sch-corr-date ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-cli-hist Dialog-Frame _FREEFORM
  QUERY br-cli-hist NO-LOCK DISPLAY
      ...
      usrfulnf(X_c-cli-hist.corr-user-name) COLUMn-LABEL {&label-user} FORMAT "X(255)":U width 18
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      TABLE: find_c-cli-hist B "?" NO-UNDO ub c-cli-hist
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br-cli-hist B-Help Dialog-Frame */
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE br-cli-hist
/* Query rebuild information for BROWSE br-cli-hist
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH X_c-cli-hist NO-LOCK INDEXED-REPOSITION.
     _END_FREEFORM
...
ON CHOOSE OF B-lookup IN FRAME Dialog-Frame /* Контрагент */
DO:
run ref/showcli.p (
               input parparentproc
...
define variable loc#log as logical no-undo .
  if available X_c-cli-hist then do:
    { gbl/markstrn.i X_c-cli-hist  v-rid-list }
    loc#log = br-cli-hist:refresh() .

    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-cli-hist:select-next-row ().
        apply "VALUE-CHANGED" to br-cli-hist in frame {&frame-name}.
    end.
  ...
  apply "entry" to br-cli-hist in frame {&frame-name}.
END.
...
DO:
  ...
  end case. /* X_c-cli-hist.obj-type */
...
&Scoped-define BROWSE-NAME br-cli-hist
&Scoped-define SELF-NAME br-cli-hist
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br-cli-hist Dialog-Frame
...
{ gbl/app_help.i &disable_diasize_init=true &browse-name="br-cli-hist" }
{ gbl/brwrefre.i "v-doc-rec = recid(X_c-cli-hist). run OpenBr(yes, no, no, v-corr-user-db-num). reposition br-cli-hist to recid v-doc-rec no-error. ~
              apply 'value-changed' to br-cli-hist. " }

{ gbl/setfltnm.i }
{ gbl/ed_date.i sch-corr-date}

{ gbl/srt-clmd.i
  &browse-name    = "br-cli-hist"
  &frame-name     = "{&frame-name}"
  &table-name     = "X_c-cli-hist"
  &sort-clmn_1    = "X_c-cli-hist.corr-date"
  &sort-clmn_2    = "X_c-cli-hist.corr-user-db-num"
...
*/
 ...
 if p-mode = "db-num":U then do:
  ...
  { gbl/mv-clmn.i
    ...
    &prev-order-column_2 = "'1,2,3,5,6,7,8,9,11,12,13,4,10'"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE get-show-reason Dialog-Frame 
...
IF v-show-reason = no THEN DO:
  ...
  IN FRAME {&FRAME-NAME}.
END.
IF p-mode = {&ALL} THEN v-show-reason = ?.
END PROCEDURE.
...
assign
br-cli-hist:num-locked-columns in frame {&frame-name} = 1
...
v-h = br-cli-hist:FIRST-COLUMN IN FRAME {&FRAME-NAME}
.
...
&scop flt-open-open-query OPEN QUERY br-cli-hist FOR EACH X_c-cli-hist

&scop flt-open-dyn_open-query  FOR EACH X_c-cli-hist

&scop flt-open-query-handle query br-cli-hist:handle
...
&scop flt-open-table-name X_c-cli-hist
...
&scop flt-open-find-buffer-name X_c-cli-hist
    ...
    CASE p-mode :
      ...
      WHEN {&company} THEN DO:
        ...
        if p-open-query then do:
          ASSIGN
          frame {&frame-name}:TITLE = title0 + substitute(" Контрагент &1&2: &3 Фирма: (&4) &5",
        ...
        { gbl/fltopend.i
          ...
          &dyn_where-cond = " substitute( 'X_c-cli-hist.obj-type  = &1&2&1  and  ~
            X_c-cli-hist.obj-code  = &3  and  ~
          (X_c-cli-hist.host-code  = &4  or X_c-cli-hist.host-code = 0)', ~{&double-quote~}, p-obj-type, p-obj-code, p-host-code) ~
                        "
      ...
      WHEN "one":u THEN DO:
        ...
        .
        if p-open-query then do:
          ASSIGN frame {&frame-name}:TITLE = title0 + substitute(" Контрагент &1&2: &3",
        ...
        { gbl/fltopend.i
          ...
          X_c-cli-hist.obj-code  = p-obj-code   ~
                        "
          &dyn_where-cond = " substitute(' X_c-cli-hist.obj-type  = &1&2&1 AND  ~
          X_c-cli-hist.obj-code  = &3', ~{&double-quote~}, p-obj-type, p-obj-code)  "
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc-br-cli-hist Dialog-Frame 
...
------------------------------------------------------------------------------*/
&scop hn-cli-hist-code p-subject
  RETURN {&hn-cli-hist-name}.   /* Function return value. */
