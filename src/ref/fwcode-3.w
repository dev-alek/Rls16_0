...
Справочник Кодов аналитического учета
...
&Scoped-Define main-file ub.fin-code-an-uchet
...
&Scoped-define FIELDS-IN-QUERY-BR-docs mark-string(recid( ub.buf_fin-code), rid-list) buf_fin-code.code-value buf_fin-code.descr buf_fin-code.fin-code buf_fin-code.host-code {&status-int-name} @ t-s buf_fin-code.level-1 "Уровень1" buf_fin-code.level-2 "Уровень2" buf_fin-code.level-3 "Уровень3"
...
define variable rr as recid no-undo .
    ...
    if available buf_fin-code then do:
        rr = recid( buf_fin-code ).
                  run ref/fci-3.w ( {&update} , input-output rr , input par-host-code ).
...
define variable rr as recid no-undo .
    if available buf_fin-code then do:
        rr = recid( buf_fin-code ).
                  run ref/fci-3.w ( {&lookup} , input-output rr , input par-host-code ).
     end.
END.
...
define variable rr as recid no-undo .
            run ref/fci-3.w ( {&add-def} , input-output rr , input par-host-code ).
