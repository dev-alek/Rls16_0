...
$Workfile: p-licsa.p $
$Archive: ref/p-licsa.p $

Печать Лицензии на продажу алкогольной продукции
...
/* ***************************  Main Block  *************************** */
    ...
    run get-full-path-RepViewer(output v-full-path-RepView).    /* Перед работой с Просмотровщиком RepViewer (main.exe) - убедимся, что он существует и получим полный путь к нему. */

    run get-report-num in parParentProc(output g#report-num).   /* Получим СТАНДАРТНЫМ МЕТОДОМ ТН номер (как для файла отчёта). */

    run define-full-path-Report(input g#report-num, output v-file-name-rep-htm).   /* Сформируем стандартизованное в ТН имя файла (как отчёта). */

    run create-file(v-file-name-rep-htm).   /* Создадим на диске пустой файл со сформированным по стандарту именем файла. */

    run proc-create-HTML(input v-file-name-rep-htm, input p-cli-type, input p-cli-code). /* Формирование HTML-документа для отображения в RepViewer */

    run Report-Viewer(input v-full-path-RepView, input v-file-name-rep-htm). /* Запуск просмотровщика HTML-документа (или отчёта) RepViewer */
...
/* Вывод отчёта в файл html и через ReportView в Excel */
    ...
    do:  /* Параметры "глобальной" таблицы отчёта */
            put stream OutStr-html unformatted
                ...
                '         <td style="width: 43px; border: none;"></td>' skip    /* 1. "№ п/п" */
                ...
                '         <td style="width: 100px; border: none;"></td>' skip    /* 7. "Кем выдана" */
    ...
    do:  /* b5 */
        ...
        do:  /* Лицензии на продажу алкоголя по всем клиентам */
        ...
        end. /* Лицензии на продажу алкоголя по всем клиентам */
        else
        do:  /* Лицензии на продажу алкоголя по заданным пользователем клиентам */
        ...
        end. /* Лицензии на продажу алкоголя по заданным пользователем клиентам */
    end. /* b5 */
