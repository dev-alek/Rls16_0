...
$Workfile: calctur3.p $
$Archive: ref/calctur3.p $

Расчет оборота по покупателю от складского документа
...
define variable vss-workfile    as character no-undo init "$Workfile: calctur3.p $":U .
define variable vss-archive     as character no-undo init "$Archive: ref/calctur3.p $":U .
define variable vss-description as character no-undo init "Расчет оборота по покупателю от складского документа ".
...
if not ( buf_trn-doc.status_   = {&fact} and
   ( buf_trn-doc.ext-doc-type = {&TDEDT_Ras_Vnesh} or
     buf_trn-doc.ext-doc-type = {&TDEDT_Vozvrat_Vnesh} )) then return .


find buf_clients no-lock where
     buf_clients.obj-code = buf_trn-doc.cli-code and
     buf_clients.obj-type = buf_trn-doc.cli-type no-error .
/* Нужен оборот */
...
*/
run waitfram-show ( "Ждите... Расчет оборота по покупателю ..." ) .
run case-type.
...
find first buf_turnover-buyer no-lock where
    buf_turnover-buyer.cli-type  =  buf_trn-doc.cli-type and
    buf_turnover-buyer.cli-code  =  buf_trn-doc.cli-code and
    buf_turnover-buyer.obj-type  =  buf_trn-doc.obj-type and
    buf_turnover-buyer.obj-code  =  buf_trn-doc.obj-code and
    ...
    for each ub.turnover-buyer exclusive-lock where
            ub.turnover-buyer.cli-type  = buf_trn-doc.cli-type and
            ub.turnover-buyer.cli-code  = buf_trn-doc.cli-code and
            ub.turnover-buyer.obj-type  = buf_trn-doc.obj-type and
            ub.turnover-buyer.obj-code  = buf_trn-doc.obj-code and
    ...
    find first ub.turnover-buyer-main exclusive-lock where
      ub.turnover-buyer-main.cli-code               = buf_trn-doc.cli-code and
      ub.turnover-buyer-main.cli-type               = buf_trn-doc.cli-type and
      ub.turnover-buyer-main.obj-code               = buf_trn-doc.obj-code and
      ub.turnover-buyer-main.obj-type               = buf_trn-doc.obj-type no-error .
      if not available ub.turnover-buyer-main then  create ub.turnover-buyer-main .

      assign
      ub.turnover-buyer-main.cli-code               = buf_trn-doc.cli-code
      ub.turnover-buyer-main.cli-type               = buf_trn-doc.cli-type
      ub.turnover-buyer-main.des                    = ""
      ub.turnover-buyer-main.obj-code               = buf_trn-doc.obj-code
      ub.turnover-buyer-main.obj-type               = buf_trn-doc.obj-type
