...
$Workfile: calctur4.p $
$Archive: ref/calctur4.p $

Расчет оборота по покупателю от продажи
...
define input  parameter p-chk-doc as character no-undo .

define variable vss-revision    as character no-undo init "$Revision: aea5316774be, 0, rls $":U .
define variable vss-author      as character no-undo init "$Author: expertek $":U .
define variable vss-date        as character no-undo init "$Date: Mon Jan 27 18:27:46 2014 +0400 $":U .
define variable vss-workfile    as character no-undo init "$Workfile: calctur4.p $":U .
define variable vss-archive     as character no-undo init "$Archive: ref/calctur4.p $":U .
define variable vss-description as character no-undo init "Расчет оборота по покупателю от продажи".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/library.i  }
{ str/clcprtsl.i }
...
find first buf_chk-doc no-lock where buf_chk-doc.doc-code = p-chk-doc no-error .
...
/* По документу касса-продажа или возврат */

if string(buf_chk-doc.chk-type) = {&rcpt-sale}
then do:
  find first buf_trn-doc where buf_trn-doc.doc-code = buf_chk-doc.out-code no-error .
  if error-status :error then return .
end.
else do:
  find first buf0_trn-doc where buf0_trn-doc.doc-code = buf_chk-doc.out-code no-error .
  find first buf_trn-doc  where buf_trn-doc.doc-code  = buf0_trn-doc.out-code no-error .
  if error-status :error then return .
end.


if not ( buf_trn-doc.status_   = {&fact} and
   ( buf_trn-doc.ext-doc-type = {&TDEDT_Ras_Vnesh_Kass} or
     buf_trn-doc.ext-doc-type = {&TDEDT_Vozvrat_Vnesh_Kass} )) then return .

find buf_clients no-lock where
     buf_clients.obj-code = buf_chk-doc.cli-code and
     buf_clients.obj-type = buf_chk-doc.cli-type no-error .
/* Нужен оборот */
...
find first buf_turnover-buyer no-lock where
    buf_turnover-buyer.cli-type  =  buf_chk-doc.cli-type and
    buf_turnover-buyer.cli-code  =  buf_chk-doc.cli-code and
    buf_turnover-buyer.obj-type  =  buf_chk-doc.obj-type and
    buf_turnover-buyer.obj-code  =  buf_chk-doc.obj-code and
    buf_turnover-buyer.fact-order < buf_trn-doc.fact-order use-index clients no-error .
    ...
    for each ub.turnover-buyer exclusive-lock where
            ub.turnover-buyer.cli-type  = buf_chk-doc.cli-type and
            ub.turnover-buyer.cli-code  = buf_chk-doc.cli-code and
            ub.turnover-buyer.obj-type  = buf_chk-doc.obj-type and
            ub.turnover-buyer.obj-code  = buf_chk-doc.obj-code and
    ...
    find first ub.turnover-buyer-main exclusive-lock where
      ub.turnover-buyer-main.cli-code = buf_chk-doc.cli-code and
      ub.turnover-buyer-main.cli-type = buf_chk-doc.cli-type and
      ub.turnover-buyer-main.obj-code = buf_chk-doc.obj-code and
      ub.turnover-buyer-main.obj-type = buf_chk-doc.obj-type no-error .
      if not available ub.turnover-buyer-main then
         create ub.turnover-buyer-main .
      assign
      ub.turnover-buyer-main.cli-code               = buf_chk-doc.cli-code
      ub.turnover-buyer-main.cli-type               = buf_chk-doc.cli-type
      ub.turnover-buyer-main.des                    = ""
      ub.turnover-buyer-main.obj-code               = buf_chk-doc.obj-code
      ub.turnover-buyer-main.obj-type               = buf_chk-doc.obj-type
 ...
 for each buf_doc-line no-lock
    ...
    if not available tt-allsum-line then next.
        assign
          ...
          v-doc-base      = tt-allsum-line.sum-dsc-base-cur
          v-doc-rubl      = tt-allsum-line.sum-dsc-rubl-cur
          v-slt-acc-base  = tt-allsum-line.slt-base-acc
          v-slt-acc-rubl  = tt-allsum-line.slt-rubl-acc
          v-slt-doc-base  = tt-allsum-line.slt-base-cur
          v-slt-doc-rubl  = tt-allsum-line.slt-rubl-cur
          v-vat-acc-base  = tt-allsum-line.vat-base-acc
          v-vat-acc-rubl  = tt-allsum-line.vat-rubl-acc
          v-vat-doc-base  = tt-allsum-line.vat-base-cur
          v-vat-doc-rubl  = tt-allsum-line.vat-rubl-cur
       .
    assign
  ...
  then do:
      if buf_trn-doc.ext-doc-type =  {&TDEDT_Vozvrat_Vnesh_Kass} then v-sign = (-1) .
        else v-sign = 1 .
      find first ub.turnover-buyer-gds exclusive-lock where
                 ub.turnover-buyer-gds.gds-code     = buf_goods.gds-code      and
                 ub.turnover-buyer-gds.cli-code     = buf_chk-doc.cli-code    and
                 ub.turnover-buyer-gds.cli-type     = buf_chk-doc.cli-type    and
                 ub.turnover-buyer-gds.fact-order   = buf_trn-doc.fact-order  and
                 ub.turnover-buyer-gds.obj-code     = buf_chk-doc.obj-code    and
                 ub.turnover-buyer-gds.obj-type     = buf_chk-doc.obj-type    and
                 ub.turnover-buyer-gds.ext-doc-type = buf_trn-doc.ext-doc-type and
                 ub.turnover-buyer-gds.sum-type     = "" no-error .

      if not available ub.turnover-buyer-gds then
        create ub.turnover-buyer-gds .
        assign
        ub.turnover-buyer-gds.gds-code     = buf_goods.gds-code
        ub.turnover-buyer-gds.cli-code     = buf_chk-doc.cli-code
        ub.turnover-buyer-gds.cli-type     = buf_chk-doc.cli-type
        ub.turnover-buyer-gds.d-card       = buf_chk-doc.d-card
        ...
        ub.turnover-buyer-gds.inkas-code   = buf_chk-doc.doc-code
        ub.turnover-buyer-gds.obj-code     = buf_chk-doc.obj-code
        ub.turnover-buyer-gds.obj-type     = buf_chk-doc.obj-type
        ub.turnover-buyer-gds.shift-date       = buf_chk-doc.shift-date
        ub.turnover-buyer-gds.shift-num        = buf_chk-doc.shift-num
        ub.turnover-buyer-gds.shift-name       = buf_chk-doc.shift-name
...
define input  parameter v-sum-vat-doc-rubl   as decimal   no-undo .
  find first ub.turnover-buyer exclusive-lock where
              ub.turnover-buyer.cli-code     = buf_chk-doc.cli-code    and
              ub.turnover-buyer.cli-type     = buf_chk-doc.cli-type    and
              ub.turnover-buyer.fact-order   = buf_trn-doc.fact-order  and
              ub.turnover-buyer.obj-code     = buf_chk-doc.obj-code    and
              ub.turnover-buyer.obj-type     = buf_chk-doc.obj-type    and
              ub.turnover-buyer.ext-doc-type = buf_trn-doc.ext-doc-type and
              ub.turnover-buyer.sum-type     = "" no-error .

    if not available ub.turnover-buyer then create ub.turnover-buyer .
    assign
      ub.turnover-buyer.cli-code         = buf_chk-doc.cli-code
      ub.turnover-buyer.cli-type         = buf_chk-doc.cli-type
      ub.turnover-buyer.d-card           = buf_chk-doc.d-card
      ...
      ub.turnover-buyer.fact-order       = buf_trn-doc.fact-order
      ub.turnover-buyer.inkas-code       = buf_chk-doc.doc-code
      ub.turnover-buyer.obj-code         = buf_chk-doc.obj-code
      ub.turnover-buyer.obj-type         = buf_chk-doc.obj-type
      ub.turnover-buyer.qnty-check       = 1
      ub.turnover-buyer.qnty-doc-itog    = 0
      ub.turnover-buyer.shift-date       = buf_chk-doc.shift-date
      ub.turnover-buyer.shift-num        = buf_chk-doc.shift-num
      ub.turnover-buyer.shift-name       = buf_chk-doc.shift-name
...
/*
нужны следующие пол
turnover-buyer.cli-code
turnover-buyer.cli-type
turnover-buyer.obj-code
turnover-buyer.obj-type
turnover-buyer.d-card
turnover-buyer.inkas-code
turnover-buyer.fact-date


turnover-buyer.sum-acc-base
turnover-buyer.sum-acc-rubl
turnover-buyer.sum-doc-base
turnover-buyer.sum-doc-rubl
turnover-buyer.sum-slt-acc-base
turnover-buyer.sum-slt-acc-rubl
turnover-buyer.sum-slt-doc-base
turnover-buyer.sum-slt-doc-rubl
turnover-buyer.sum-vat-acc-base
turnover-buyer.sum-vat-acc-rubl
turnover-buyer.sum-vat-doc-base
turnover-buyer.sum-vat-doc-rubl
turnover-buyer.ext-doc-type
turnover-buyer.fact-order
*/