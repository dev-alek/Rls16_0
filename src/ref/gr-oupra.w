...
Добавление новой группы оборотов
...
define variable vss-description as character no-undo init "Добавление новой группы оборотов".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/library.i  }
{ cmp/showinf.i      }
...
DEFINE VARIABLE v-name LIKE ub.turnover-group.name
     LABEL "Название группы оборотов"
     VIEW-AS FILL-IN
...
DEFINE FRAME Dialog-Frame
     ...
     v-name AT ROW 4.25 COL 27 COLON-ALIGNED HELP
          ""
          LABEL "Название группы оборотов" FORMAT "X(80)"
     SPACE(3.49) SKIP(1.74)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE
         TITLE "Добавление группы оборотов покупателей для ценообразования"
...
if p-mode = {&add-def} then do:
    create ub.turnover-group.
    assign
      ub.turnover-group.tog-db-num   = v-cntxt-db-num
      ub.turnover-group.tog-id       = next-value ( s-qgr , {&db-name_schema} )
      ub.turnover-group.db-num-chg   = v-cntxt-db-num
      ub.turnover-group.stts         = 0
      ub.turnover-group.sys-date     = today
      ub.turnover-group.sys-time     = time
      ub.turnover-group.sys-time-chr = string(ub.turnover-group.sys-time,"hh:mm")
      ub.turnover-group.who          = v-cntxt-userid
      ub.turnover-group.name         = v-name
      .
end.
else do:
    assign
...
p-recid = recid(ub.turnover-group) .
...
-------------------------------------------------------------*/
  if p-mode = {&update} then do:
    find first  ub.turnover-group exclusive-lock where recid(ub.turnover-group) = p-recid no-error .
