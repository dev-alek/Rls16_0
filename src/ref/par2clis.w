...
DEFINE BUFFER X_ext-classif FOR ext-classif.
...
Коды клиентов в системе ПАРУС-2
...
define variable vss-description as character no-undo init "Коды клиентов в системе ПАРУС-2".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i }
...
define variable filter-point     as character NO-UNDO INIT "par2clis".
define variable filter-label     as character NO-UNDO INIT "Коды клиентов в системе ПАРУС-2".
define variable filter-point0     as character NO-UNDO INIT "par2clis".
define variable filter-label0     as character NO-UNDO INIT "Коды клиентов в системе ПАРУС-2".
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME br-parus-2-cli

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES X_ext-classif X_clients

/* Definitions for BROWSE br-parus-2-cli                                  */
&Scoped-define FIELDS-IN-QUERY-br-parus-2-cli mark-string(recid(X_ext-classif), v-rid-list) X_ext-classif.cahrKEY_one X_clients.obj-type X_clients.obj-code X_clients.obj-name
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-parus-2-cli
&Scoped-define SELF-NAME br-parus-2-cli
&Scoped-define QUERY-STRING-br-parus-2-cli FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-parus-2-cli OPEN QUERY {&SELF-NAME} FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-parus-2-cli X_ext-classif X_clients
&Scoped-define FIRST-TABLE-IN-QUERY-br-parus-2-cli X_ext-classif
&Scoped-define SECOND-TABLE-IN-QUERY-br-parus-2-cli X_clients


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-br-parus-2-cli}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS b-quit B-mark B-sel b-add b-del b-cli b-sch ~
b-print B-Help br-parus-2-cli mark-num
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY br-parus-2-cli FOR X_ext-classif, X_clients SCROLLING.

&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE br-parus-2-cli
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-parus-2-cli Dialog-Frame _FREEFORM
  QUERY br-parus-2-cli NO-LOCK DISPLAY
      mark-string(recid(X_ext-classif), v-rid-list) COLUMN-LABEL "" FORMAT "X(1)"
X_ext-classif.charKEY_one  COLUMN-LABEL "Код в!ПАРУС-2" FORMAT "X(17)"
...
DEFINE FRAME Dialog-Frame
     ...
     br-parus-2-cli AT ROW 2.88 COL 1.5 WIDGET-ID 100
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br-parus-2-cli B-Help Dialog-Frame */
ASSIGN
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE br-parus-2-cli
/* Query rebuild information for BROWSE br-parus-2-cli
     _START_FREEFORM
...
DO:
  ...
  if available X_ext-classif then do:
    ...
    loc#log = br-parus-2-cli:refresh() .

    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-parus-2-cli:select-next-row ().
        apply "VALUE-CHANGED" to br-parus-2-cli in frame {&frame-name}.
    end.
  ...
  apply "entry" to br-parus-2-cli in frame {&frame-name}.
END.
...
&scop flt-open-open-query         OPEN QUERY br-parus-2-cli FOR EACH X_ext-classif

&scop flt-open-dyn_open-query     FOR EACH X_ext-classif

&scop flt-open-query-handle      QUERY br-parus-2-cli:handle
...
title0 = "Клиенты в системе ПАРУС-2".
...
{ gbl/fltopend.i
        &where-cond = " X_ext-classif.classif-subject = ~{&table_clients~} ~
                        and X_ext-classif.classif-name = ~{&extclass_clients_parus-2~} ~
        ...
        &dyn_where-cond = " substitute('X_ext-classif.classif-subject = &1&2&1 ~
                        ...
                        AND X_ext-classif.db-num = - 1', {&double-quote}, ~{&table_clients~}, ~{&extclass_clients_parus-2~})"
...
define buffer buf_clients for clients.
message
"Выберите клиента, для которого Вы хотите добавить запись в системе ПАРУС-2"
view-as alert-box .
...
run gbl/d-character.w (
       input ?
      ,input (
      'title=':u + substitute("Добавление кода клиента в системе ПАРУС-2 для &1&2"
                              ,buf_clients.obj-type
                              ,buf_clients.obj-code) + '\':u
    + 'text1=':u + "Код клиента в систем ПАРУС-2" + '\':u
    + 'format=' + "X(17)" + '\':u
    + 'fillin_row=2\':u
    + 'fillin_col=30\':u
    + 'fillin_width=10\':u
...
if v-rid <> ? then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-parus-2-cli to recid v-rid no-error .
  apply "entry" to br-parus-2-cli in frame {&frame-name} .
end.
END PROCEDURE.
...
if not error-status:error then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-parus-2-cli to row 1 no-error .
  apply "entry" to br-parus-2-cli in frame {&frame-name} .
end.
END PROCEDURE.
...
DO WHILE available X_ext-classif :
   GET prev br-parus-2-cli.
END.
GET next br-parus-2-cli.
DO WHILE available X_ext-classif :
...
with FRAME List1.
DISPLAY STREAM PrnLibStream
accum-count @ X_ext-classif.charkey_one
with frame List1.
...
run fltfield-add in this-procedure('charkey_one', 'Код клиента ПАРУС-2', '',
...
DO on stop undo, leave:
    ...
    run OpenBr IN THIS-PROCEDURE ( INPUT YES, INPUT NO, INPUT '':U).
    if v-ri <> ? then do:
      reposition br-parus-2-cli to recid v-ri no-error.
    end.
    APPLY "ENTRY" to br-parus-2-cli in frame {&frame-name} .
    APPLY "VALUE-CHANGED" to br-parus-2-cli.
END .
