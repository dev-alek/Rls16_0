...
define variable vss-description as character no-undo init "Скидка/категория на объекте/фирме".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/library.i }
...
&Scoped-Define ENABLED-OBJECTS B-exit l-d-pcnt l-cash-d-pcnt l-category ~
b-quit B-Help 
&Scoped-Define DISPLAYED-OBJECTS f-d-pcnt f-cash-d-pcnt f-category 
...
DEFINE VARIABLE f-cash-d-pcnt AS DECIMAL FORMAT "->9.99":U INITIAL 0 
     LABEL "Скидка на итог" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME Dialog-Frame
     ...
     f-d-pcnt AT ROW 3 COL 19 COLON-ALIGNED
     f-cash-d-pcnt AT ROW 5.25 COL 19 COLON-ALIGNED
     f-category AT ROW 7.42 COL 19 COLON-ALIGNED
     l-d-pcnt AT ROW 3 COL 32.5 WIDGET-ID 2
     l-cash-d-pcnt AT ROW 5.25 COL 32.5 WIDGET-ID 4
     l-category AT ROW 7.5 COL 28.5 WIDGET-ID 6
     SPACE(38.41) SKIP(1.14)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "Скидка и категория по объекту/фирме"
...
/* SETTINGS FOR FILL-IN f-cash-d-pcnt IN FRAME Dialog-Frame
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN f-category IN FRAME Dialog-Frame
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN f-d-pcnt IN FRAME Dialog-Frame
   NO-ENABLE                                                            */
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Скидка и категория по объекту/фирме */
DO:
...
&Scoped-define SELF-NAME f-cash-d-pcnt
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL f-cash-d-pcnt Dialog-Frame
ON RIGHT-MOUSE-CLICK OF f-cash-d-pcnt IN FRAME Dialog-Frame /* Скидка на итог */
DO:
  assign
    f-cash-d-pcnt:fgcolor = 15
    f-cash-d-pcnt = 0
    l-cash-d-pcnt:visible = true.
    display f-cash-d-pcnt with frame {&frame-name}.
    disable f-cash-d-pcnt with frame {&frame-name}.
END.
...
&Scoped-define SELF-NAME f-category
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL f-category Dialog-Frame
ON RIGHT-MOUSE-CLICK OF f-category IN FRAME Dialog-Frame /* Категория */
DO:
  assign
    f-category:fgcolor = 15
    f-category = 0
    l-category:visible = true.
    display f-category with frame {&frame-name}.
    disable f-category with frame {&frame-name}.
END.
...
&Scoped-define SELF-NAME f-d-pcnt
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL f-d-pcnt Dialog-Frame
ON RIGHT-MOUSE-CLICK OF f-d-pcnt IN FRAME Dialog-Frame /* Скидка на товар */
DO:
  assign
    f-d-pcnt:fgcolor = 15
    f-d-pcnt = 0
    l-d-pcnt:visible = true.
    display f-d-pcnt with frame {&frame-name}.
    disable f-d-pcnt with frame {&frame-name}.
END.
...
DO:
   IF l-cash-d-pcnt:visible then do:
    assign
    f-cash-d-pcnt:fgcolor = ?
    l-cash-d-pcnt:visible = false.
    enable f-cash-d-pcnt with frame {&frame-name}.
    APPLY "ENTRY" TO f-cash-d-pcnt.
...
DO:
  IF l-category:visible then do:
    ...
    enable f-category with frame {&frame-name}.
    APPLY "ENTRY" TO f-category.
...
DO:
   IF l-d-pcnt:visible then do:
    ...
    enable f-d-pcnt with frame {&frame-name}.
    APPLY "ENTRY" TO f-d-pcnt.
...
------------------------------------------------------------------------------*/
  DISPLAY f-d-pcnt f-cash-d-pcnt f-category 
      WITH FRAME Dialog-Frame.
  ENABLE B-exit l-d-pcnt l-cash-d-pcnt l-category b-quit B-Help 
      WITH FRAME Dialog-Frame.
...
FOR EACH temp-dis-card-property where
   ...
   CASE temp-dis-card-property.node-code:
     WHEN {&dc_prop_discount_d-pcnt} THEN DO:
       f-d-pcnt = temp-dis-card-property.property-value-decimal.
     END.
     WHEN {&dc_prop_discount_cash-d-pcnt} THEN DO:
       f-cash-d-pcnt = temp-dis-card-property.property-value-decimal.
     END.
     WHEN {&dc_prop_discount_category} THEN DO:
       f-category = temp-dis-card-property.property-value-integer.
     END.
  END CASE.
END.
...
if p-obj-type = '' then do:
  assign
  frame {&frame-name}:title = substitute("&1 Фирма &2", frame {&frame-name}:title, p-host-code).
end.
else do:
  assign
  frame {&frame-name}:title = substitute("&1 Фирма &2 &3&4", frame {&frame-name}:title, p-host-code, p-obj-type, p-obj-code ).
end.
DISPLAY
f-d-pcnt
f-cash-d-pcnt
...
b-quit
B-Help
l-d-pcnt       WHEN p-mode <> {&LOOKUP}
l-cash-d-pcnt  WHEN p-mode <> {&LOOKUP}
l-category     WHEN p-mode <> {&LOOKUP}
WITH FRAME {&frame-name}.
...
FOR EACH temp-dis-card-property where
   ...
   CASE temp-dis-card-property.node-code:
     WHEN {&dc_prop_discount_d-pcnt} THEN DO:
       ...
       l-d-pcnt:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
     END.
     WHEN {&dc_prop_discount_cash-d-pcnt} THEN DO:
       APPLY "mouse-select-click" TO l-cash-d-pcnt.
       l-cash-d-pcnt:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
     END.
     WHEN {&dc_prop_discount_category} THEN DO:
       APPLY "mouse-select-click" TO l-category.
       l-category:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
...
and temp-dis-card-property.obj-code = p-obj-code
   ...
   CASE temp-dis-card-property.node-code:
     WHEN {&dc_prop_discount_d-pcnt} THEN DO:
       temp-dis-card-property.property-value-decimal = f-d-pcnt.
     END.
     WHEN {&dc_prop_discount_cash-d-pcnt} THEN DO:
       temp-dis-card-property.property-value-decimal = f-cash-d-pcnt.
     END.
     WHEN {&dc_prop_discount_category} THEN DO:
       temp-dis-card-property.property-value-integer = f-category.
     END.
