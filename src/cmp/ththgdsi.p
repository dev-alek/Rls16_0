...
assign
p-import-type = integer(entry(1, p-parameter, {&delim-par} ))
...
v-ii = 0.
&scop my-message "Импорт новых данных..."
{&display-message}.
...
for each src_goods no-lock :
  ...
  if v-ii modulo 10 = 0 then do:
    &scop my-count-message substitute("Обработано &1 из них найдено соответствие для &2 пропущено &3 ранее сведено &4" ~
                                      , v-ii ~
                                      , v-ii-ok ~
                                      , v-ii-next ~
                                      , v-ii-next-done  )
  ...
  if p-import-type = 2 then do:
    ...
    if not available buf_ext-classif then do:
      &scop my-message substitute('Не найдена запись соответствия по товару &5 &1&2:&3&2&4' ~
                                    , src_goods.gds-code ~
                                    ,~{&new-line~} ~
  ...
  then do:
    ...
    if available buf2_ext-classif and recid(buf2_ext-classif) <> v-rec then do:
      &scop my-message substitute('Не могу привязать запись соответствия по товару &7 &1 (&2 &3&4 &5)&6'  ~
                                    , src_goods.gds-code ~
                                    , src_goods.artic ~
                                    , src_goods.prod-type ~
                                    , src_goods.prod-code ~
                       ...
                       substitute("ТОВАР v16.0 УЖЕ СВЯЗАН С ТОВАРОМ &7 &1 (&2 &3&4 &5)" ~
                                    ...
                                    , entry(1, buf2_ext-classif.charkey_three, ~{&delim-par~} ) ~
                                    ,~{&new-line~} ~
                                    , p-from-version )
       {&display-message}.
       v-ii-next = v-ii-next + 1.
  ...
  run ref/extclas1.p (
                       input (if p-import-type = 1 then {&add-def} else {&update})
                      ,input yes /*p-silent*/
                      ,input-output v-rec
                      ...
                      ,input src_goods.gds-name + {&delim-par} + src_goods.unit-base /*p-CharKey_Three*/
                      ,input 0 /*p-nonunique*/
                      ,input v-goods-uniq-key-rec /*p-uniq-key-rec*/
                      ) no-error.
  if error-status:error then do:
    &scop my-message substitute('Ошибка при сохранении записи соответствия по товару &5 &1&2:&3&2&4' ~
                                  , src_goods.gds-code ~
                                  ,~{&new-line~} ~
                                  , error-status:get-message(1) ~
                                  , return-value ~
...
&scop my-message substitute("Обработано &1, из них найдено соотв для &2, из отсутсв. &3 ДопБК удалось добавить &4&5пропущено &6 ранее сведено &7",  ~
                            v-ii, v-ii-ok, v-pbc, v-pbc-ok, {&new-line}, v-ii-next, v-ii-next-done)
{&display-message}.
{&hide-count-message}.
...
def var vss-include-info{&vssseq} as character format "x(65)" no-undo initial "@(#)$Workfile: ththgdsi.p $ $Revision: 1f78fe327cdf, 1091, rls $".
...
&scoped-define vssseq {&sequence}
define variable vss-include-info{&vssseq} as character format "x(65)" no-undo initial "@(#)$Workfile: ththgdsi.p $ $Revision: 1f78fe327cdf, 1091, rls $".
