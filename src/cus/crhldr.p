...
$Workfile: crhldr.p $
$Archive: cus/crhldr.p $

Создание МФ расхода по заказу
  ...
  for each  old_doc-line no-lock  where
    ...
    if buf_goods.stts <> 0 then do:
          if LENGTH(new_trn-doc.Ps) < 31900 then do:
              new_trn-doc.Ps = trim(new_trn-doc.Ps) + {&new-line} + 'товар удален '  + old_doc-line.artic.
          end.
    ...
    /* Ассортиментная политика */
        ...
        { gbl/goassizt.i
          v-event-code
          buf_goods.gds-code
          new_trn-doc.hold-obj-type
          new_trn-doc.hold-obj-code
          false
          v-Ok
          v-mess
          no-error }
    ...
    if ub.gds-obj.price-sale  = 0  or ub.gds-obj.price-sale  = ?  then do:
          if LENGTH(new_trn-doc.Ps) < 31900 then do:
              new_trn-doc.Ps = trim(new_trn-doc.Ps) + {&new-line} + 'нет цены у '  + old_doc-line.artic.
          end.
          put stream str unformatted
              substitute("&1,&2" , buf_goods.gds-code,  old_doc-line.qnty )
              skip.
  ...
  /* в ОРЦ берем цену с объекта РЦ */
    if old_doc-line.price-rubl <> ub.gds-obj.price-sale and old_doc-line.price-rubl <> 0 then do:
      if length(old_trn-doc.ps) < 31900 then do:
          ...
          old_trn-doc.Ps = trim(old_trn-doc.Ps) + {&new-line} + old_doc-line.artic  + " Цена взята с РЦ , была: " + string (old_doc-line.price-rubl) .
      end.
    end.
  ...
  .
      ...
      BUFFER-COPY old_doc-line to tt-doc-line
        assign
          ...
          tt-doc-line.ext-doc-type   = {&TDEDT_Ras_Vnesh}
          .
      { gbl/gdsobjcr.i
...
/*
message 'После удаления смотрим что осталось во временных таблицах' .
for each  tt-doc-line :
    message  'tt-doc-line' tt-doc-line.artic .
end.

for each  tt-gds-dtl :
    message  'tt-gds-dtl' tt-gds-dtl.artic .
end.
for each  tt-parts :
    message  'tt-parts' tt-parts.artic .
end.
*/
...
/* new_trn-doc.out-code = loc-ord-num . */
...
if not error-status :error then do:
 ...
 run create-chain in this-procedure (
   ...
   run cus/ordorcls.p (input parparentproc, input recid(old_trn-doc) , input false ) no-error .
    if  old_trn-doc.STATUS_ = {&ord-req} or error-status :error then do:
...
/*   message 'p-is-negostmess'  p-is-negostmess .*/
  end.
