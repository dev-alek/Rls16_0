...
$Workfile: e-zum4.p $
$Archive: cus/e-zum4.p $

Расширенный экспорт строк чеков в Excel для ЦУМа - с составом сырь
...
define variable vss-description as character no-undo init "Расширенный экспорт строк чеков в Excel для ЦУМа - с составом сырья".
{ cmp/vssrevis.i }
...
_chk-gds:
  FOR EACH buf_chk-doc No-LOCK WHERE
    ...
    if lookup(string(buf_chk-doc.chk-type), {&no-sale-receipt-codes}) > 0 then next _chk-gds.
      ...
      buf_goods.struct                          p-XL-delim  /*состав сырья*/
