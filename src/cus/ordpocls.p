...
$Workfile: ordpocls.p $
$Archive: cus/ordpocls.p $

Переход по графу статусов заказы покупателей
 ...
 case buf_ord-doc.status_ :
      ...
      when {&g___new} then do :
        ...
        if buf_ord-doc.flag_ = false  then do:
              ...
              if available t-doc-line then do:
                  if p-ask then
                  message   " Заказ"  buf_ord-doc.doc-code  "   содержит товары с неопределенной ценой ({&abbr_rub})! "
                            view-as alert-box information
                            title "Закрыть заказ "   .
                  return.
              end.
     ...
     /* НОВЫЙ+ --> в ПОСТАВКА- */
            if buf_ord-doc.flag_ = true  then do:
              /* Создание внешней расходной накладной НАКЛ- по заказу покупателЯ и если нехватает то заявки на объект */
              run cus/ord-espo.p
                  ( input parParentProc ,
              ...
              if buf_ord-doc.contract-code <> 0 then do:
                find first buf_contract no-lock where
                  ...
                  case buf_contract.usl-opl:
                    when {&contr-buyer-ord} then do: /* Предоплата */
                         assign
                          buf_ord-doc.need-fo  = 1
                          buf_ord-doc.need-fo2 = 0
                         .
                    end.
                    when {&contr-buyer-ord-prc} then do: /* Предоплата с % */
                          assign
                            buf_ord-doc.need-fo  = 1
                            buf_ord-doc.need-fo2 = 1
                          .
                    end.
                    when {&contr-pay-nodef} then do: /* Неопределено */
                          assign
                ...
                for each ub.ord-chain no-lock
                 ...
                 find first t-trn-doc no-lock
                    where t-trn-doc.doc-code  = ub.ord-chain.rel-doc-code
                      ...
                      and t-trn-doc.doc-type  = {&expense}
            ...
            for each t-ord-doc-rcv where  t-ord-doc-rcv.doc-code     = buf_ord-doc.doc-code no-lock :
              ...
              if buf_ord-doc.contract-code <> 0 then do:
                find first buf_contract no-lock where
                  ...
                  case buf_contract.usl-opl:
                    when {&contr-buyer-ord} or
                    when {&contr-buyer-ord-prc}
                    then do: /* Предоплата */
                         ...
                         if buf_ord-doc.need-fo2 <> 1 and buf_contract.usl-opl  = {&contr-buyer-ord-prc} then buf_ord-doc.need-fo2 = 1 .
                    end.
                  end case.
              end.
