...
DEFINE BUFFER X1_clients FOR ub.clients.
...
Коды фирм в системах клиентов

Автор: Самков Сергей Васильевич
Дата создания: 05/05/12
Author: Samkov Sergey
Creation date: 05/05/12
...
define variable vss-description as character no-undo init "Коды фирм в системах клиентов".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i  }
...
define variable filter-point     as character NO-UNDO INIT "enum-cli".
define variable filter-label     as character NO-UNDO INIT "Коды фирм в системах клиентов".
define variable filter-point0    as character NO-UNDO INIT "enum-cli".
define variable filter-label0    as character NO-UNDO INIT "Коды фирм в системах клиентов".
...
&Scoped-define FRAME-NAME Dialog-Frame
&Scoped-define BROWSE-NAME br-enum-cli

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES X_ext-classif X_clients

/* Definitions for BROWSE br-enum-cli                                   */
&Scoped-define FIELDS-IN-QUERY-br-enum-cli mark-string(recid(X_ext-classif), v-rid-list) X_ext-classif.KEY#_one get-firm-name(X_ext-classif.KEY#_one) X_clients.obj-type X_clients.obj-code X_clients.obj-name X_ext-classif.CharKey_Three
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-enum-cli
&Scoped-define SELF-NAME br-enum-cli
&Scoped-define QUERY-STRING-br-enum-cli FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-enum-cli OPEN QUERY {&SELF-NAME} FOR EACH X_ext-classif NO-LOCK, ~
       FIRST X_clients NO-LOCK INDEXED-REPOSITION.
&Scoped-define TABLES-IN-QUERY-br-enum-cli X_ext-classif X_clients
&Scoped-define FIRST-TABLE-IN-QUERY-br-enum-cli X_ext-classif
&Scoped-define SECOND-TABLE-IN-QUERY-br-enum-cli X_clients


/* Definitions for DIALOG-BOX Dialog-Frame                              */
&Scoped-define OPEN-BROWSERS-IN-QUERY-Dialog-Frame ~
    ~{&OPEN-QUERY-br-enum-cli}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS b-quit B-mark B-sel b-add b-del b-cli b-sconf ~
b-sch b-print B-Help br-enum-cli mark-num
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY br-enum-cli FOR X_ext-classif, X_clients SCROLLING.

&ANALYZE-RESUME

/* Browse definitions                                                   */
DEFINE BROWSE br-enum-cli
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-enum-cli Dialog-Frame _FREEFORM
  QUERY br-enum-cli NO-LOCK DISPLAY
      mark-string(recid(X_ext-classif), v-rid-list) COLUMN-LABEL "" FORMAT "X(1)"
X_ext-classif.KEY#_one  COLUMN-LABEL "Код фирмы" FORMAT ">>>>9"
get-firm-name(X_ext-classif.KEY#_one) COLUMN-LABEL "Фирма" FORMAT "X(30)"
...
X_ext-classif.CharKey_Three COLUMN-LABEL "Код фирмы!в системе клиента" FORMAT "X(20)"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME Dialog-Frame
     ...
     b-sconf AT ROW 1 COL 61 WIDGET-ID 18
     b-sch AT ROW 1 COL 86 WIDGET-ID 12
     b-print AT ROW 1 COL 89 WIDGET-ID 10
     B-Help AT ROW 1 COL 95
     br-enum-cli AT ROW 2.87 COL 1.5 WIDGET-ID 100
...
/* SETTINGS FOR DIALOG-BOX Dialog-Frame
   FRAME-NAME                                                           */
/* BROWSE-TAB br-enum-cli B-Help Dialog-Frame */
ASSIGN
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE br-enum-cli
/* Query rebuild information for BROWSE br-enum-cli
     _START_FREEFORM
...
&Scoped-define SELF-NAME b-sconf
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL b-sconf Dialog-Frame
ON CHOOSE OF b-sconf IN FRAME Dialog-Frame /* ВС */
DO:
...
define variable v-host-code   as integer        no-undo.
  IF NOT AVAILABLE X_ext-classif THEN RETURN NO-APPLY.
  ASSIGN
  v-host-code = X_ext-classif.key#_one.
  run adm/config.w (
...
DO:
  ...
  if available X_ext-classif then do:
    ...
    loc#log = br-enum-cli:refresh() .

    if last-event:function <> "MOUSE-SELECT-DBLCLICK" then do:
        loc#log = br-enum-cli:select-next-row ().
        apply "VALUE-CHANGED" to br-enum-cli in frame {&frame-name}.
    end.
  ...
  apply "entry" to br-enum-cli in frame {&frame-name}.
END.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE b-quit B-mark B-sel b-add b-del b-cli b-sconf b-sch b-print B-Help
         br-enum-cli mark-num
      WITH FRAME Dialog-Frame.
...
&scop flt-open-open-query         OPEN QUERY br-enum-cli FOR EACH X_ext-classif

&scop flt-open-dyn_open-query     FOR EACH X_ext-classif

&scop flt-open-query-handle      QUERY br-enum-cli:handle
...
case p-list-mode:
 when {&all} then do:
  title0 = "Коды фирм в системах клиентов".
  ASSIGN
  ...
  { gbl/fltopend.i
          &where-cond = " X_ext-classif.classif-subject = ~{&table_clients~} ~
                          and X_ext-classif.classif-name = ~{&extclass_code_firm_in_ext_client~} ~
          ...
          &dyn_where-cond = " substitute('X_ext-classif.classif-subject = &1&2&1 ~
                          ...
                          AND X_ext-classif.db-num = - 1', {&double-quote}, ~{&table_clients~}, ~{&extclass_code_firm_in_ext_client~})"
...
message
"Выберите фирму, для которой Вы хотите ввести код в системе клиента"
view-as alert-box .
run adm/sconfs.w (
      input parParentProc
    , input "b-sel":U
...
v-value-integer = buf_sysconf.host-code.

message
"Выберите клиента, для системы которого Вы хотите ввести код Фирмы"
view-as alert-box .
...
run gbl/d-prompt.w (
        'title=':u + "Ввод кода фирмы в системе клиента" + '\':u
      + 'text1=':u + "Код" + '\':u
      + 'format=' + "x(20)" + '\':u
      + 'type=' + {&type-char} + '\':u
      + 'fillin_row=2\':u
      ...
      + 'fillin_width=21\':u
      + 'fillin_height=1\':u
      + 'max-chars=20\':u     /*- максимальное количество символов для редактора*/
      + 'readonly=no'
      , input-output varcode
      ) no-error.
if return-value <> "TRUE" or error-status:error then do:
  undo, return error .
end.
...
run ref/extclas1.p ( INPUT {&add-def}
                    ...
                    ,INPUT {&extclass_code_firm_in_ext_client} /*p-classif-name*/
                    ,input (-1) /*p-db-num*/
                    ,input v-value-integer /*p-key#_one*/
                    ,input buf_clients.obj-code /*p-Key#_Two*/
                    ,input 0 /*p-key#_Three*/
                    ,input '':U /*p-CharKey_One */
                    ,input buf_clients.obj-type /*p-CharKey_two */
                    ,input varcode /*p-CharKey_three */
...
if v-rid <> ? then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-enum-cli to recid v-rid no-error .
  apply "entry" to br-enum-cli in frame {&frame-name} .
end.
END PROCEDURE.
...
if not error-status:error then do:
  run openbr in this-procedure ( input yes, input no, input '':U).
  reposition br-enum-cli to row 1 no-error .
  apply "entry" to br-enum-cli in frame {&frame-name} .
end.
END PROCEDURE.
...
DEFINE FRAME list1
X_ext-classif.KEY#_one  COLUMN-LABEL "Код!фирмы" FORMAT ">>>>9"
v-firm-name COLUMN-LABEL "Фирма" FORMAT "X(30)"
X_clients.obj-type COLUMN-LABEL "Тип!клиента" FORMAT "X(3)"
X_clients.obj-code COLUMN-LABEL "Код!клиента" FORMAT ">>>>>>>>9"
X_clients.obj-name COLUMN-LABEL "Название клиента" FORMAT "X(56)"
X_ext-classif.CharKey_Three COLUMN-LABEL "Код фирмы!в системе клиента" FORMAT "X(20)"
...
DO WHILE available X_ext-classif :
   GET prev br-enum-cli.
END.
GET next br-enum-cli.
DO WHILE available X_ext-classif :
  v-firm-name = get-firm-name(X_ext-classif.key#_one).
...
DO on stop undo, leave:
    ...
    run OpenBr IN THIS-PROCEDURE ( INPUT YES, INPUT NO, INPUT '':U).
    if v-ri <> ? then do:
      reposition br-enum-cli to recid v-ri no-error.
    end.
    APPLY "ENTRY" to br-enum-cli in frame {&frame-name} .
    APPLY "VALUE-CHANGED" to br-enum-cli.
END .
