...
Параметры импорта данных  по ДК из текстового файл
...
define variable vss-description as character no-undo init "Параметры импорта данных по ДК из текстового файла".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/trg-def.i  }
{ cmp/library.i  }
...
b-rule-profile b-dc-type b-param
...
&Scoped-Define DISPLAYED-OBJECTS rs-dir v-dir-name file-name-1 ~
f-rs-dir-label f-rs-file-rule-label f-file-name-label f-dc-type-label
...
DEFINE VARIABLE f-dc-type-label AS CHARACTER FORMAT "X(256)":U INITIAL "Тип ДК при импорте новой карты"
      VIEW-AS TEXT
     SIZE 33.5 BY .67
     FGCOLOR 4  NO-UNDO.

DEFINE VARIABLE f-file-name-label AS CHARACTER FORMAT "X(256)":U INITIAL "(Допускается маска имени файла)"
      VIEW-AS TEXT
     SIZE 32 BY .67
     FGCOLOR 4  NO-UNDO.

DEFINE VARIABLE f-rs-dir-label AS CHARACTER FORMAT "X(256)":U INITIAL "Настройка директории импорта данных"
      VIEW-AS TEXT
     SIZE 40 BY .67
     FGCOLOR 4  NO-UNDO.

DEFINE VARIABLE f-rs-file-rule-label AS CHARACTER FORMAT "X(256)":U INITIAL "Настройка имени файла импорта"
      VIEW-AS TEXT
     SIZE 45.5 BY .67
     FGCOLOR 4  NO-UNDO.
...
DEFINE VARIABLE rs-dir AS CHARACTER
     ...
     RADIO-BUTTONS
          "берется из Ini-файла ([schedule-free] dctxt-i=in)", "ini",
"привязана к строке расписания", "shd-attr"
...
DEFINE FRAME Dialog-Frame
     ...
     file-name-1 AT ROW 9 COL 3.5
     b-rule-profile AT ROW 10.5 COL 90 WIDGET-ID 26
     X_rule-profile.name AT ROW 10.57 COL 8 COLON-ALIGNED WIDGET-ID 10
          LABEL "Профайл"
          VIEW-AS FILL-IN
          SIZE 79 BY 1
     ub.X_rule.name AT ROW 11.67 COL 10 NO-LABEL WIDGET-ID 18
          VIEW-AS EDITOR SCROLLBAR-VERTICAL
          SIZE 79.5 BY 2
     b-dc-type AT ROW 15.93 COL 13.5
     b-param AT ROW 15.93 COL 39 WIDGET-ID 14
     X_dis-card-type.type AT ROW 16.03 COL 2.5 NO-LABEL
          VIEW-AS FILL-IN
          SIZE 10 BY 1
          BGCOLOR 8
     f-rs-dir-label AT ROW 2.77 COL 2.5 COLON-ALIGNED NO-LABEL
     f-rs-file-rule-label AT ROW 7.77 COL 1 COLON-ALIGNED NO-LABEL
     f-file-name-label AT ROW 9.27 COL 29.5 COLON-ALIGNED NO-LABEL
     f-dc-type-label AT ROW 14.87 COL 2 NO-LABEL
     X_dis-card-type.emitent-host-code AT ROW 16.2 COL 25.5 COLON-ALIGNED WIDGET-ID 22
          LABEL "Эмитент"
           VIEW-AS TEXT
          SIZE 6 BY .67
     "Правило:" VIEW-AS TEXT
          SIZE 8.5 BY 1 AT ROW 11.67 COL 1 WIDGET-ID 20
     SPACE(88.29) SKIP(10.02)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE
         TITLE "Параметры импорта данных по продажам по ДК из текстового файл"
  ...
  when 'shd':U then do:
    /*данная конкретная задача может быть ТОЛЬКО ОДНА в одной БД - отследим*/
      ...
      run gbl/lock-prc.p
          ...
          ,input ""
          ,input ""
          ,input "Импорт данных по ДК из текстового файла"
          ,input yes
      ...
      FIND FIRST buf_schedule-attr NO-LOCK WHERE
             ...
             and buf_schedule-attr.attr-code = ({&attr-schd-free-id} + {&delim-par} + 'dctxt-i') NO-ERROR.
      /*
      IF AVAILABLE  buf_schedule-attr
          ...
          THEN DO:
        MESSAGE
        substitute("Уже есть расписание для импорта данных по ДК для БД &1&2" +
                   "номер расписания &3"
                   ,buf_schedule-attr.db-num-char
                   ,{&NEW-LINE}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY rs-dir v-dir-name file-name-1 f-rs-dir-label f-rs-file-rule-label
          f-file-name-label f-dc-type-label
  ...
  IF AVAILABLE X_rule-profile THEN
    DISPLAY X_rule-profile.name
      WITH FRAME Dialog-Frame.
  ENABLE B-exit b-quit B-Help rs-dir b-dir-sel b-rule-profile
         X_rule-profile.name X_rule.name b-dc-type b-param
         X_dis-card-type.emitent-host-code
      WITH FRAME Dialog-Frame.
...
DEFINE VARIABLE v-in AS CHARACTER NO-UNDO.
...
IF rs-dir = "ini":u THEN DO:
    run verify-ini-entry in this-procedure (
                                        ...
                                        ,INPUT substitute("отсутствует параметр &1 секция &2 в ini-файле"
                                                          ...
                                                          , 'schedule-free')
                                        ,INPUT no
                                        ,output v-in) no-error .
    if error-status:error or v-in = ? then return error return-value .
    ...
    when 'shd':U then do:
      if p-task-num > 0 then do:
        ...
        for each buf_schedule no-lock where
            ...
            AND buf_schedule.task-type = p-task-type
            AND buf_schedule.task-num > 0,
            first buf_schedule-attr no-lock where
                  buf_schedule-attr.cre-db-num = p-cre-db-num
              AND buf_schedule-attr.task-type = p-task-type
              AND buf_schedule-attr.task-num = buf_schedule.task-num
              AND buf_schedule-attr.attr-code = ({&attr-schd-free-id} + {&delim-par} + 'dctxt-i') :
      ...
      if v-param-list = '':U THEN DO:
         v-param-list = string(12) + {&delim-par} + 'ini' + {&delim-par} + "f?.txt" + {&delim-par} + {&delim-par} + {&delim-par} +
                {&delim-par} + {&delim-par} + string(TODAY) + {&delim-par} + {&question-mark}.
...
CASE p-mode:
    WHEN 'run':U THEN DO:
        ...
        rs-dir:RADIO-BUTTONS IN FRAME {&FRAME-NAME}
                             = "берется из Ini-файла ([schedule-free] dctxt-i_in=)" + {&comma-char} +
                               "ini":U + {&comma-char} +
                               "задать директорию импорта" + {&comma-char} +
                               "other".
    END.
    WHEN 'shd':U THEN DO:
        ASSIGN
        rs-dir:RADIO-BUTTONS = "берется из Ini-файла ([schedule-free] dctxt-i_in=)" + {&comma-char} +
                               "ini":U + {&comma-char} +
...
/*проверим save*/
RUN verify-file in this-procedure
                                  ( input (trim(v-dir-name, {&back-slash-char}) + '\save\')
                                  , input substitute("Не найден подкаталог save в каталоге &1&2он необходим для сохранения копии обработанного файла"
                                                , v-dir-name
                                                , {&new-line}
                                                )
                                  ,input no
...
if error-status:error or not glog then return error return-value .
assign
file-info :file-name = (trim(v-dir-name, {&back-slash-char}) + '\save\')
.
if index( file-info:file-type, "W" ) = 0  then do:
  message
  'Вы не имеете права писать в директорию для сохранения копий обработанных файлов: '
  trim(v-dir-name, {&back-slash-char}) + '\save\' view-as alert-box error.
  return error.
end.

v-dop-file-name = replace(replace(file-name-1, {&question-mark}, '':U), "*", '':U).
...
if trim(v-dop-file-name) = '':U then do:
 message
 "Не задано имя файла для импорта"
 view-as alert-box error .
 return error.
end.
if NOT AVAILABLE X_dis-card-type then do:
 message
 "Не задан тип ДК для импорта новых карт"
 view-as alert-box error .
 return error.
end.
FOR EACH buf_tt0-rule-call-param
...
v-param-list = (string(v-rule-profile-id) + {&delim-par} +
                ...
                file-name-1 + {&delim-par} +
                string(X_dis-card-type.emitent-host-code) + {&delim-par} +
                X_dis-card-type.type + {&delim-par} +
                v-other-param
.
...
define buffer buf_rule-call-param for ub.rule-call-param.
    ...
    FIND FIRST X_rp-by-call NO-LOCK WHERE
              X_rp-by-call.profile_id = 12
        AND   X_rp-by-call.call_id = X_dis-card-type.uniq-key-rec NO-ERROR.
    IF NOT AVAILABLE X_rp-by-call THEN DO:
        MESSAGE
        substitute("Тип ДК &1 эмитент &2 не привязан к профайлу импорта-экспорта в текстового файл"
                   , p-dc-type
                   , p-emitent-host-code)
        VIEW-AS ALERT-BOX ERROR.
        UNDO, RETURN ERROR.
