...
DEFINE VARIABLE vss-workfile    AS CHARACTER NO-UNDO INITIAL "$Workfile: r-otvxr1.p $":U.
DEFINE VARIABLE vss-archive     AS CHARACTER NO-UNDO INITIAL "$Archive: cus/r-otvxr1.p $":U.
DEFINE VARIABLE vss-description AS CHARACTER NO-UNDO INITIAL "Печать объединенного счета-фактуры по ответственному хранению":U.
...
DEFINE TEMP-TABLE tt-line NO-UNDO
  ...
  INDEX pi           IS UNIQUE    PRIMARY prod-type prod-code artic VAT-pc price-no-VAT GTD.
...
DEFINE VARIABLE i-sign               AS INTEGER   NO-UNDO.
...
DO ON ERROR   UNDO Main-Block, LEAVE Main-Block
  ...
  SESSION :SET-WAIT-STATE( "COMPILER":U ).
  RUN print-rs IN THIS-PROCEDURE.
  SESSION :SET-WAIT-STATE( "":U ).
END. /* Main-Block */
  SESSION :SET-WAIT-STATE( "":U ).
...
PROCEDURE calc-sum :
  ...
  FOR EACH ub.parts NO-LOCK WHERE
    ...
    END.
    FIND obj-list WHERE
         obj-list.obj-type = ub.parts.obj-type AND
         obj-list.obj-code = ub.parts.obj-code NO-ERROR.
    ...
    ASSIGN i-sign      = ( IF is_positive THEN 1 ELSE -1 ).
    ...
    IF ub.goods.gds-type <> {&gds-office} THEN DO:
      ...
      IF LOOKUP( ub.trn-doc.ext-doc-type, {&neg-list} ) <> 0 or ub.trn-doc.ext-doc-type = {&TDEDT_Chg_Purch_Code} THEN DO: ASSIGN v-prt-qnty = - v-prt-qnty. END.
    ...
    IF NOT AVAILABLE tt-line THEN DO:
      ...
      ASSIGN tt-line.artic        = ub.goods.artic
             ...
             tt-line.GTD          = v-GTD.
    END.
    ASSIGN tt-line.gds-qty    = tt-line.gds-qty    + ABS( v-prt-qnty                        ) * i-sign
           tt-line.sum-no-VAT = tt-line.sum-no-VAT + ABS( v-parts-price-no-VAT * v-prt-qnty ) * i-sign
           tt-line.VAT        = tt-line.VAT        + ABS( v-parts-VAT          * v-prt-qnty ) * i-sign
           tt-line.sum        = tt-line.sum        + ABS( v-parts-sum                       ) * i-sign.
  END. /* FOR EACH ub.parts ... */

  FOR EACH tt-line :
    IF ( ( tt-line.gds-qty      = 0 ) OR ( tt-line.gds-qty      = ? ) ) AND
    /* ( ( tt-line.price-no-VAT = 0 ) OR ( tt-line.price-no-VAT = ? ) ) AND */
       ( ( tt-line.sum-no-VAT   = 0 ) OR ( tt-line.sum-no-VAT   = ? ) ) AND
       ( ( tt-line.VAT          = 0 ) OR ( tt-line.VAT          = ? ) ) AND
       ( ( tt-line.sum          = 0 ) OR ( tt-line.sum          = ? ) ) THEN DO:
      DELETE tt-line.
      NEXT.
    END.
    IF tt-line.price-no-VAT = ? THEN DO: ASSIGN tt-line.price-no-VAT = 0. END.
...
PROCEDURE print-rs :
  ...
  FOR EACH tt-line :
    ...
    ASSIGN v-tot-sum-no-VAT = v-tot-sum-no-VAT + ( IF tt-line.sum-no-VAT = ? THEN 0 ELSE tt-line.sum-no-VAT )
           v-tot-VAT        = v-tot-VAT        + ( IF tt-line.VAT        = ? THEN 0 ELSE tt-line.VAT        )
           v-tot-sum        = v-tot-sum        + ( IF tt-line.sum        = ? THEN 0 ELSE tt-line.sum        ).
  END. /* FOR EACH tt-line */
...
PROCEDURE print-line :
  ...
  DO ON ERROR UNDO, RETURN ERROR :
    ...
    DISPLAY STREAM PrnLibStream sym1  gds-str1                 @ ub.goods.gds-name
                                ...
                                sym3  tt-line.gds-qty          @ v-qnty
                                sym4  tt-line.price-no-VAT     @ v-price-no-VAT
                                sym5  tt-line.sum-no-VAT       @ v-sum-no-VAT
                                sym6  "   ---" FORMAT "x(6)":U @ v-sum-excise
                                sym7  tt-line.VAT-pc           @ ub.doc-line.VAT-pc
                                sym8  tt-line.VAT              @ v-VAT
                                sym9  tt-line.sum              @ v-sum
