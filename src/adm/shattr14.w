...
Редактирование атрибута магазина (thbj-attr) "cd-type-ncr-as-r"
...
define variable vss-description as character no-undo init "Редактирование атрибута магазина (thbj-attr) 'cd-type-ncr-as-r'".
{ cmp/vssrevis.i }
{ gbl/waitfram.i }
{ cmp/str-glbl.i }
{ cmp/library.i }
...
&Scoped-Define ENABLED-OBJECTS B-exit b-quit B-Help RS-ncrscpfx RS-ncrpgpfx ~
RS-save-param BR-xtd B-up B-down E-save-param l-ncrscpfx l-ncrpgpfx ~
l-save-param l-save-param-2 l-ncrdrank l-ncrdrank-2
&Scoped-Define DISPLAYED-OBJECTS RS-ncrscpfx RS-ncrpgpfx RS-save-param ~
E-save-param l-ncrscpfx l-ncrpgpfx l-save-param l-save-param-2 l-ncrdrank ~
l-ncrdrank-2
...
DEFINE FRAME Dialog-Frame
     ...
     RS-ncrscpfx AT ROW 2 COL 27 NO-LABEL
     RS-ncrpgpfx AT ROW 2 COL 80 NO-LABEL WIDGET-ID 4
     RS-save-param AT ROW 4.77 COL 49.5 NO-LABEL
     BR-xtd AT ROW 5.77 COL 2
     B-up AT ROW 5.77 COL 37
     B-down AT ROW 6.77 COL 37
     E-save-param AT ROW 8.5 COL 49.5 NO-LABEL
     l-ncrscpfx AT ROW 2 COL 1 NO-LABEL
     l-ncrpgpfx AT ROW 2 COL 42 NO-LABEL WIDGET-ID 2
     ...
     SPACE(65.24) SKIP(11.13)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE
         TITLE "Параметры POS NCR-AS@R"
...
ON WINDOW-CLOSE OF FRAME Dialog-Frame /* Параметры POS NCR-AS@R */
DO:
  ...
  CASE rs-save-param:
      WHEN 'no':U THEN DO:
          ...
          e-save-param:SCREEN-VALUE = "При передаче на кассы различных типов скидок " +
                                     "(на итог чека, скидок для различных категорий покупателей и т.д.) или настроек," +
                                     " хранящихся в файлах кассовых настроек S_PLURBT.DAT " +
                                     "и/или group_xx.dat и/или P_REGPAR.DAT, НЕ ИСПОЛЬЗУЕТСЯ дополнительная проверка, " +
                                     "можно ли данный тип скидки или настройки задавать пересылкой через IBS TH " +
      ...
      WHEN 'IBS':U THEN DO:
          ...
          e-save-param:SCREEN-VALUE = "При передаче на кассы различных типов скидок " +
                                     "(на итог чека, скидок для различных категорий покупателей и т.д.) или настроек," +
                                     " хранящихся в файлах кассовых настроек S_PLURBT.DAT " +
                                     "и/или group_xx.dat и/или P_REGPAR.DAT, ИСПОЛЬЗУЕТСЯ дополнительная проверка, " +
                                     "можно ли данный тип скидки или настройки задавать пересылкой через IBS TH " +
                                     "или же данная скидка может быть изменена ТОЛЬКО администратором вручную - " +
      ...
      WHEN 'NCR':U THEN DO:
          ...
          e-save-param:SCREEN-VALUE = "При передаче на кассы различных типов скидок " +
                                     ...
                                     "рядом с основными файлами - в директориях, " +
                                     "задаваемых настройками out2, out3, out4 секции [kassa-ncr-as-r] ini-файла системы IBS TH".
      END.
...
{ gbl/rethndmv.i v-tab-order underline-tb "APPLY 'CHOOSE' TO b-exit in frame {&frame-name}." }
  ...
  IF p-mode = {&UPDATE} THEN DO:
    FIND FIRST LOCKED_thbj-attr EXCLUSIVE-LOCK WHERE
        ...
        AND   LOCKED_thbj-attr.upper-prop-code = {&attr-cd-type-ncr-as-r}
...
------------------------------------------------------------------------------*/
  DISPLAY RS-ncrscpfx RS-ncrpgpfx RS-save-param E-save-param l-ncrscpfx
          l-ncrpgpfx l-save-param l-save-param-2 l-ncrdrank l-ncrdrank-2
      WITH FRAME Dialog-Frame.
  ENABLE B-exit b-quit B-Help RS-ncrscpfx RS-ncrpgpfx RS-save-param BR-xtd B-up
         B-down E-save-param l-ncrscpfx l-ncrpgpfx l-save-param l-save-param-2
...
FOR EACH thbjattr_thbj-attr:
  ...
  IF v-entry = {&attr-cd-type-ncr-as-r_ncrscpfx} THEN DO:
    ASSIGN
    RS-ncrscpfx = thbjattr_thbj-attr.property-value-integer.
  END.
  IF v-entry = {&attr-cd-type-ncr-as-r_ncrpgpfx} THEN DO:
    ASSIGN
    RS-ncrpgpfx = thbjattr_thbj-attr.property-value-integer.
  END.

  IF v-entry = {&attr-cd-type-ncr-as-r_ncrdrank} THEN DO:
    ASSIGN
    v-ncrdrank = thbjattr_thbj-attr.property-value-character.
  END.
  IF v-entry = {&attr-cd-type-ncr-as-r_save-param} THEN DO:
    ASSIGN
...
v-tab-order = "rs-ncrscpfx,rs-ncrpgpfx,br-xtd,b-up,b-down,rs-save-param,e-save-param".
DISPLAY
RS-ncrscpfx
l-ncrscpfx
RS-ncrpgpfx
...
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = {&attr-cd-type-ncr-as-r_ncrscpfx}.
assign
thbjattr_thbj-attr.property-value-integer = RS-ncrscpfx.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = {&attr-cd-type-ncr-as-r_ncrpgpfx}.
assign
thbjattr_thbj-attr.property-value-integer = RS-ncrpgpfx.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = {&attr-cd-type-ncr-as-r_ncrdrank}.
assign
thbjattr_thbj-attr.property-value-character = v-ncrdrank.
find first thbjattr_thbj-attr where thbjattr_thbj-attr.prop-code = {&attr-cd-type-ncr-as-r_save-param}.
assign
