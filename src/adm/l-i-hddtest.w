...
$Workfile: l-i-hddtest.w $
$Archive: adm/l-i-hddtest.w $

Окно входа в Систему мониторинга HDD
...
def var vss-workfile    as character no-undo init "$Workfile: l-i-hddtest.w $":U .
def var vss-archive     as character no-undo init "$Archive: adm/l-i-hddtest.w $":U .
def var vss-description as character no-undo init "Окно входа в Систему мониторинга HDD".
{ cmp/vssrevis.i }
{ cmp/str-glbl.i }
{ cmp/showinf.i  }
{ adm/auto-def.i new }
...
if v-connection
  ...
  assign
    log-file-name = "hddtest.log"
  .
  run adm/automain.w
    ( input {&btpr-type-hddtest}
     ,input v-mode
    ) no-error.
end.
run gbl/dbdiscon.p no-error.
