...
Окно входа в экран продавца (Остатки товаров)
...
then do:
  ...
  do ind = 1 to v-num-entries :
    ...
    if num-entries( v-param, ":":U ) = 2 then do:
      ...
      IF v-param BEGINS 'Sock:' THEN DO:
        ASSIGN
          v-soket-param = substring(v-param,6,LENGTH(v-param))
            .
      END.
    end.
...
:
 ...
 /*проерка кофигурационнго параметра */
   run gbl/conf-rd.p ("skt-serv":U, "":U, "":U, 0, "":U, "":U, "":U, yes, output v-par-val, output v-par-type) no-error.
   if error-status:error
      or v-par-type <> "L":U
   then do:
     message
       vss-workfile vss-revision vss-description skip
       "Ошибка чтения конфигурационного параметра skt-serv!"
       view-as alert-box error.
     leave do1.
   end.
   if v-par-val <> "yes" then do:
     message
       vss-workfile vss-revision vss-description skip
       "В вашей системе запрещено работать с сервером авторизации!" skip
       "(параметр skt-serv)"
       view-as alert-box error.
     leave do1.
   end.
